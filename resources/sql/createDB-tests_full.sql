-- This scrip creates memory tables for some frequently used tables to improve performance.
-- However, this may fail if data is too large.
...
DROP TABLE IF EXISTS `fs_clusters`;
DROP TABLE IF EXISTS `fs_cluster_assignments`;
DROP TABLE IF EXISTS `fs_geds`;
...
-- Create fs_geds table with 512 MB space
SET max_heap_table_size = 1024*1024*512;
create table fs_geds 
	(fs_fid1 varchar(40), 
	fs_fid2 varchar(40), 
	fs_ged double, 
	constraint pk_fs_geds primary key (fs_fid1, fs_fid2)) 
	engine=MEMORY;
	
-- Create fs_clusters table with 128 MB space
SET max_heap_table_size = 1024*1024*128;
create table `fs_clusters` (
	`fs_cluster_id`		varchar(40), 
	`fs_size`		int, 
	`fs_avg_fragment_size`	float, 
	`fs_medoid_id`		varchar(40), 
	`fs_benifit_cost_ratio`	double, 
	`fs_std_effort`		double, 
	`fs_refactoring_gain`	int, 
	constraint `pk_fs_clusters` primary key (`fs_cluster_id`)
) engine=MEMORY;

-- Create fs_cluster_assignments table with 512 MB space
SET max_heap_table_size = 1024*1024*512;
create table fs_cluster_assignments (
	fs_fragment_version_id varchar(40), 
	fs_cluster_id varchar(40), 
	fs_clone_id varchar(40), 
	fs_maximal boolean, 
	fs_core_object_nb int, 
	constraint pk_fs_cluster_assignments primary key (fs_fragment_version_id, fs_cluster_id)) 
	engine=MEMORY;
