...
public class GrappaAdapter implements GrappaConstants, GrappaListener, ActionListener {
	 ...
	 * @param panel specific panel where the action occurred
	 */
	public void grappaPressed(Subgraph subg, Element elem, GrappaPoint pt, int modifiers,
			GrappaPanel panel) {
		if ( //rightclick
				((modifiers & (InputEvent.BUTTON2_MASK | InputEvent.BUTTON3_MASK)) != 0 &&
			...
			if (panel instanceof ModelGraphPanel) {
				...
				Object clickedObject = ((ModelGraphPanel) panel).getOriginalObject();

				ProvidedObject[] pos = MainUI.getInstance().getProvidedObjects();
	 ...
	 * @param panel specific panel where the action occurred
	 */
	public void grappaDragged(Subgraph subg, GrappaPoint currentPt, int currentModifiers,
			Element pressedElem, GrappaPoint pressedPt, int pressedModifiers, GrappaBox outline,
			GrappaPanel panel) {
		if ((currentModifiers & InputEvent.BUTTON1_MASK) == InputEvent.BUTTON1_MASK) {
			if (currentModifiers == InputEvent.BUTTON1_MASK ||
				...
				GrappaBox box = GrappaSupport.boxFromCorners(pressedPt.x, pressedPt.y,
								currentPt.x,
								currentPt.y);
				g2d.draw(box);
				g2d.setPaintMode();
	...
	public void actionPerformed(ActionEvent aev) {
		...
		if (src instanceof javax.swing.JMenuItem) {
			...
			if (parent instanceof javax.swing.JPopupMenu) {
				Object invoker = ((javax.swing.JPopupMenu) (((javax.swing.JMenuItem) src).getParent())).
								 getInvoker();
				if (invoker instanceof GrappaPanel) {
					...
					if (text.startsWith("Cancel")) {
						...
						if (subg.currentSelection instanceof Element) {
							GrappaSupport.setHighlight((Element) (subg.currentSelection),
									DELETION_MASK, HIGHLIGHT_OFF);
						} else {
							Vector vec = (Vector) (subg.currentSelection);
							for (int i = 0; i < vec.size(); i++) {
								GrappaSupport.setHighlight((Element) (vec.elementAt(i)),
										DELETION_MASK, HIGHLIGHT_OFF);
							}
						}
						subg.getGraph().repaint();
						...
						if (subg.currentSelection instanceof Element) {
							GrappaSupport.setHighlight((Element) (subg.currentSelection), 0,
									HIGHLIGHT_OFF);
						} else {
							Vector vec = (Vector) (subg.currentSelection);
							for (int i = 0; i < vec.size(); i++) {
								GrappaSupport.setHighlight((Element) (vec.elementAt(i)), 0,
										HIGHLIGHT_OFF);
							}
						}
						subg.currentSelection = null;
