 ...
 */
public class GrappaNexus implements
		GrappaConstants,
	...
	public void updateShape() {
		...
		switch (element.getType()) {
			case NODE:
				...
				switch (type) {
					case CUSTOM_SHAPE:
						...
						if (custom == null) {
							throw new IllegalArgumentException("custom attibuted null for node (" +
									node.getName() + ") with custom shape");
						}
						Class custom_class;
						try {
							custom_class = Class.forName(custom, true,
										   Thread.currentThread().getContextClassLoader());
						} catch (Exception e) {
							throw new IllegalArgumentException(
									"custom class unavailable for custom shape '" + custom + "'");
						}
						...
						try {
							...
							throw new IllegalArgumentException(
									"constructor for custom class shape '" + custom + "' not found");
						}
						try {
							if (Grappa.centerPointNodes) {
								shape = (Shape) (custom_shape =
										ccustom.newInstance(new Object[] {node,
										new Double(pos.x - (width / 2.0)),
										new Double(pos.y - (height / 2.0)), new Double(width),
										new Double(height)}));
							} else {
								shape = (Shape) (custom_shape =
										ccustom.newInstance(new Object[] {node, new Double(pos.x),
										new Double(pos.y), new Double(width), new Double(height)}));
							}
						} catch (Exception e) {
							if (e instanceof InvocationTargetException) {
								...
								throw new IllegalArgumentException(
										...
										"' for node '" + node.getName() + "' because2: " +
										((UndeclaredThrowableException) e).getUndeclaredThrowable().
										getMessage());
							} else {
								throw new IllegalArgumentException(
										"cannot instantiate custom shape '" + custom +
										"' for node '" + node.getName() + "' because3: " +
										e.getMessage());
							}
						}
					...
					case BOX_SHAPE:
						if (
							...
							if (Grappa.centerPointNodes) {
								shape = new GrappaShape(shapeType, pos.x, pos.y, width, height,
										sides, peripheries, distortion, skew, orientation,
										style.rounded, style.diagonals, null);
							} else {
								shape = new GrappaShape(shapeType, pos.x + (width / 2.0),
										pos.y + (height / 2.0), width, height, sides, peripheries,
										distortion, skew, orientation, style.rounded,
										style.diagonals, null);
							}
						}
						break;
					case ROUNDEDBOX_SHAPE:
						if (
							...
							if (Grappa.centerPointNodes) {
								shape = new RoundRectangle2D.Double(pos.x - (width / 2.0),
										pos.y - (height / 2.0), width, height,
										arcWidthFactor * width, arcHeightFactor * height);
							} else {
								shape = new RoundRectangle2D.Double(pos.x, pos.y, width, height,
										arcWidthFactor * width, arcHeightFactor * height);
							}
							if (Peripheries != null && peripheries > 1) {
								path = new GeneralPath(shape);
					...
					case MSQUARE_SHAPE:
						...
						if (Grappa.centerPointNodes) {
							shape = new GrappaShape(shapeType, pos.x, pos.y, width, height, sides,
									peripheries, distortion, skew, orientation, style.rounded,
									style.diagonals, null);
						} else {
							shape = new GrappaShape(shapeType, pos.x + (width / 2.0),
									pos.y + (height / 2.0), width, height, sides, peripheries,
									distortion, skew, orientation, style.rounded, style.diagonals, null);
						}
						break;
					case RECORD_SHAPE:
					case MRECORD_SHAPE:
						...
						if (Grappa.centerPointNodes) {
							shape = new GrappaShape(shapeType, pos.x, pos.y, width, height, sides,
									peripheries, distortion, skew, orientation, style.rounded,
									style.diagonals, rects);
						} else {
							shape = new GrappaShape(shapeType, pos.x + (width / 2.0),
									pos.y + (height / 2.0), width, height, sides, peripheries,
									distortion, skew, orientation, style.rounded, style.diagonals,
									rects);
						}
						break;
					default:
				...
				if (Rotation != null && rotation != 0 && shape != null) {
					...
					if (Grappa.centerPointNodes) {
						...
						shape = AffineTransform.getRotateInstance(theta, pos.x + (width / 2.0),
								pos.y + (height / 2.0)).createTransformedShape(shape);
					}
				}
				break;
			case EDGE:
				...
				if ((shape = (Shape) edge.getAttributeValue(POS_ATTR)) == null) {
					...
					edge.setAttribute(
							"pos",
							new GrappaLine(new GrappaPoint[] {(GrappaPoint) (edge.getTail().
							getAttributeValue(POS_ATTR)),
							(GrappaPoint) (edge.getHead().getAttributeValue(POS_ATTR))}
							, edge.direction)
							);
			...
			case SUBGRAPH:
				...
				if (minSize != null) {
					if (sgbox == null) {
						sgbox = new java.awt.geom.Rectangle2D.Double(0, 0, minSize.getWidth(),
								minSize.getHeight());
					} else {
						sgbox.add(new java.awt.geom.Rectangle2D.Double(sgbox.getCenterX() -
								(minSize.getWidth() / 2.0),
								sgbox.getCenterY() - (minSize.getHeight() / 2.0), minSize.getWidth(),
								minSize.getHeight()));
					}
				}
				...
				if (minBox != null) {
					if (sgbox == null) {
						sgbox = new java.awt.geom.Rectangle2D.Double(minBox.x, minBox.y,
								minBox.width, minBox.height);
					} else {
						sgbox.add(new java.awt.geom.Rectangle2D.Double(minBox.x, minBox.y,
								minBox.width, minBox.height));
					}
				}
				if (sgbox == null) {
				...
				//PVDB added the following two lines to make the bounding box of a subgraph wider
				sgbox.add(sgbox.getX() + sgbox.getWidth() + 11,
						sgbox.getY() + sgbox.getHeight() + 11);
				sgbox.add(sgbox.getX() - 10, sgbox.getY() - 10);

				shape = sgbox;
				break;
			default:
				throw new IllegalArgumentException("unrecognized element type (" + element.getType() +
						") for " + element.getName());
		}
	...
	public void updateText() {
		...
		if (labelAttr != null && labelAttr.length() > 0) {
			...
			if (labelAttr.length() > 0) {
				...
				Integer fontadj = (Integer) (element.getGraph()).getGrappaAttributeValue(
								  GRAPPA_FONTSIZE_ADJUSTMENT_ATTR);

				// set font
				tfont = new Font(fontname, fontstyle.intValue(),
						fontsize.intValue() + fontadj.intValue());
				...
				if (
					...
					if (Grappa.autoPositionNodeLabel && element.isNode()) {
						lps[0] = ((Node) element).getCenterPoint();
									   ...
									   ((sg = element.getSubgraph()) != null &&
											  attr == sg.getLocalAttribute(LP_ATTR))) {
						Rectangle2D lbox;
						...
						if (element.isSubgraph() && lbox != null) {
							lps[0] = new GrappaPoint(lbox.getX() + lbox.getWidth() / 2.0,
									 (Grappa.labelGraphBottom ? lbox.getMaxY() : lbox.getMinY()));
					...
					if (labels[l] != null && lps[l] != null && labels[l].length() > 0) {
						...
						for (i = 0; i < array.length; i++) {
							if (
								...
								bnds[lines] = tfont.getStringBounds(tstr[offset + lines],
											  element.getGraph().REFCNTXT);
								mtrc[lines] = tfont.getLineMetrics(tstr[offset + lines],
											  element.getGraph().REFCNTXT);
	...
	private final static File[] fileList = new File(About.WATERMARKIMAGELOCATION()).listFiles(new
										   FileFilter() {
	...
	private final static HashMap<String, Image> imageList = new HashMap(fileList.length);

	static {
		for (int i = 0; i < fileList.length; i++) {
			try {
				imageList.put(fileList[i].getAbsolutePath(), ImageIO.read(fileList[i].toURL()));
			} catch (Exception e) {
	...
	private String nextRandomImage() {
		return fileList[(int) Math.floor(Math.random() * (double) fileList.length)].getAbsolutePath();
	}
	...
	public void updateImage() {
		...
		// Add a random image, if the image is not given,  the node has a label or
		// it has no label and is not filled.
		if (path == null && fileList.length > 0 && element instanceof Node
				&& (!(element.getAttributeValue(LABEL_ATTR).equals(""))
				|| (element.getAttributeValue(STYLE_ATTR) != null) &&
				!((GrappaStyle) element.getAttributeValue(STYLE_ATTR)).getFilled()
				)
				) {
	...
	private void bboxCheckSet() {
		...
		if (newbox == null) {
			if (element.isSubgraph() &&
					((Subgraph) element).countOfElements(SUBGRAPH | NODE | EDGE) == 0) {
	...
	public void update(java.util.Observable obs, Object arg) {
		...
		if (!(obs instanceof Attribute)) {
			throw new IllegalArgumentException(
					"expected to be observing attributes only (obs) for \"" + element.getName() +
					"\"");
		}
		...
		if (arg instanceof Object[]) {
			...
			if (args.length == 2 && args[0] instanceof Attribute && args[1] instanceof Long) {
				...
				throw new IllegalArgumentException("badly formated update information for \"" +
						element.getName() + "\"");
			}
		}
		// end boilerplate
		...
		if (arg instanceof Long) {
			...
			switch (element.getType()) {
				case NODE:
					if (
						...
						throw new InternalError("update called for \"" + element.getName() +
				...
				case SUBGRAPH:
					if (
						...
						if (lastStyleUpdate < thisUpdate) {
							...
							(COLOR_HASH == attrHash && COLOR_ATTR.equals(attrName))
							||
							(FILLCOLOR_HASH == attrHash && FILLCOLOR_ATTR.equals(attrName))
							||
							(FONTCOLOR_HASH == attrHash && FONTCOLOR_ATTR.equals(attrName))
							) {
						if (lastDecorationUpdate < thisUpdate) {
			...
			throw new InternalError("update called for shape of element \"" + element.getName() +
