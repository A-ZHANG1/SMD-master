...
import org.prom5.framework.models.bpmn.BpmnGraphVertex;
...
public class EventShape extends GrappaShape implements CustomRenderer{
  ...
  public EventShape(Element element, double x_, double y_, double width_, double height_)
  {
    super(CUSTOM_SHAPE, x_, y_, width_, height_, 0, 0, 0, 0, 0, false, false, null);
    ...
    if (this.bpmnEvent != null) {
      ...
      if (eType == BpmnEventType.Start) {
        path.append(new Ellipse2D.Double(x, y, width, height), false);
         ...
         path.append(new Ellipse2D.Double(x, y, width, height), false);
         path.append(new Ellipse2D.Double(x+2, y+2, width-4, height-4), false);
      } else if (eType == BpmnEventType.End) {
        //solid ellipse for end
         path.append(new Ellipse2D.Double(x, y, width, height), false);
         path.append(new Ellipse2D.Double(x+0.5, y+0.5, width-1, height-1), false);
         path.append(new Ellipse2D.Double(x+1, y+1, width-2, height-2), false);
      }
    }
  }
   ...
   * @param height
   */
  protected void defineTriggerShape(float x, float y, float width, float height) {
    if (this.bpmnEvent != null) {
      ...
      else if(trType == BpmnEventTriggerType.Error ||trType == BpmnEventTriggerType.Exception){
        //  shape of error and exception trigger
         defineErrorShape(x, y, width, height);
      }
      else if(trType == BpmnEventTriggerType.Compensation ||trType == BpmnEventTriggerType.Compensate){
   ...
   * @param height
   */
  protected void defineMessageShape(float x, float y, float width, float height) {
    // shape of message trigger
    float x0 = x + width/4;
    float y0 = y + height/4;
    float w0 = width/2;
   ...
   * @param height
   */
  protected void defineCompensateShape(float x, float y, float width, float height) {
   ...
   * @param height
   */
  protected void defineTerminateShape(float x, float y, float width, float height) {
    // shape of terminate trigger
    float x0 = x + 3;
    float y0 = y + 3;
    float w0 = width-6;
   ...
   * @param height
   */
  protected void defineMultipleShape(float x, float y, float width, float height) {
    // shape of multiple trigger
    float x0 = x + 2;
    float y0 = y + 2;
    float w0 = width-4;
