...
public class GatewayShape extends GrappaShape implements CustomRenderer{
  ...
  public GatewayShape(Element element, double x_, double y_, double width_, double height_)
  {
  ...
  protected void defineGatewayShape(float x, float y, float width, float height ) {
   ...
   * @param height
   */
  protected void defineGatewayTypeShape(float x, float y, float width, float height) {
    if (this.bpmnGateway != null) {
  ...
  protected void defineORShape(float x, float y, float w, float h) {
    // shape of or
      path.append(new Ellipse2D.Double(x+w/4, y+h/4,w/2,h/2),false);
