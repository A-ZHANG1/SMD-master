...
public class EPCOrgObjShape extends GrappaShape implements CustomRenderer {
	...
	public EPCOrgObjShape(Element element, double x_, double y_, double width_,
			double height_) {
		/*
		 public GrappaShape(int type, double x, double y, double width, double height, int sidesArg,
		   int peripheriesArg, double distortionArg, double skewArg, double orientationArg,
		   boolean roundedArg, boolean diagonalsArg, Object extra) {
		 */
		super(OVAL_SHAPE, x_ + 0.5 * width_, y_ + 0.5 * height_, width_, height_, 0, 1, 0, 0, 0, false, false, null);
...
//		path = new GeneralPath(Grappa.windingRule);
		...
		GrappaPoint Pt0 = new GrappaPoint(
						  w * CIRCLE_XDIAG / 2.0,
						  h * CIRCLE_YDIAG / 2.0
						  );
