...
public class InputShape extends GrappaShape implements CustomRenderer {
	private Element element;

	private static final float TD = 13;

	public InputShape(Element element, double x_, double y_, double width_, double height_) {
		...
		float a = (float) (1 / (2 * Math.sqrt(3)));

		path.moveTo(x + w / 2 - a * TD, y + h / 2 - TD / 2);
		path.lineTo(x + w / 2 - a * TD, y + h / 2 + TD / 2);
		path.lineTo(x + w / 2 + (1 - a) * TD, y + h / 2);
		path.lineTo(x + w / 2 - a * TD, y + h / 2 - TD / 2);
