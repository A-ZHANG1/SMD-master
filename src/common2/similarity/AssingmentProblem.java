...
public class AssingmentProblem {
	...
	private NodeSimilarity nodeSimilarity = new NodeSimilarity();
	private LabelEditDistance labelEditDistance = new LabelEditDistance();
	...
	public LinkedList<VertexPair> getMappingsGraph(Graph g1, Graph g2) {
		return getMappingsGraph(g1, g2, Settings.DEFAULT_THRESHOLD, null);
	}
	...
	public LinkedList<VertexPair> getMappingsVetrex(
			LinkedList<Vertex> vertices, LinkedList<Vertex> vertices2) {
		return getMappingsVetrex(vertices, vertices2,
				Settings.DEFAULT_THRESHOLD, null, 0);
	}
	...
	public boolean canMap(Vertex v1, Vertex v2) {
...
//							LabelEditDistance.edTokensWithStemmingWordnet(g1Vertices_fe
			...
			if (nrZeros != g1Vertices_fe.size() * g2Vertices_fe.size()) {
				...
				for (int i = 0; i < result.length; i++) {
					...
					if (result[i][0] < g1Vertices_fe.size()
							...
							&& pairCost >= threshold 
							&& canMap(g1Vertices_fe.get(result[i][0]), g2Vertices_fe.get(result[i][1]))
							) {
	...
	public LinkedList<VertexPair> getMappingsVetrexUsingNodeMapping(
		...
		if (g1Vertices.size() > 0 && g2Vertices.size() > 0) {
			...
			for (int i = 0; i < g1Vertices.size(); i++) {
				for (int j = 0; j < g2Vertices.size(); j++) {
					double edScore = nodeSimilarity.findNodeSimilarity(g1Vertices.get(i), g2Vertices.get(j));
//					System.out.println(">>> "+ g1Vertices.get(i)+" <> " + g2Vertices.get(j) + " "+edScore);
...
//					}
	...
	public boolean listContains(LinkedList<Vertex> list, Vertex v) {
		for (Vertex lv : list) {
			if (v.getID() == lv.getID()) {
				return true;
			}
	...
	public boolean mappingContainsVertexRight(LinkedList<VertexPair> mapping, Vertex v){
	...
	public boolean mappingContainsVertexLeft(LinkedList<VertexPair> mapping, Vertex v){
	 ...
	 * @return
	 */
	public LinkedList<VertexPair> getMappingsGateways(
									LinkedList<Vertex> g1Vertices, 
									LinkedList<Vertex> g2Vertices, 
									double threshold, 
									SnowballStemmer stemmer, 
	 ...
	 * @return
	 */
	public LinkedList<VertexPair> getMappingsWithAddingGateways(
			Graph g1, 
			Graph g2,
			LinkedList<Vertex> g1Vertices, 
			LinkedList<Vertex> g2Vertices,
		...
		for (Vertex v : g1Vertices) {
			if (v.getType().equals(Vertex.Type.gateway) 
					&& !this.mappingContainsVertexLeft(nodeMappingsPrev, v)
					&& !this.mappingContainsVertexLeft(process, v)
					&& !this.mappingContainsVertexLeft(processed, v)) {
				g1Gateways.add(v);
			}
		}
		...
		for (Vertex gw : g1Gateways) {
			if (lookParents == 1) {
				...
				LinkedList<VertexPair> nodeMappings = this.getMappingsVetrex(noparentGWs, g2Vertices, threshold, stemmer, lookParents);
				...
				for (Vertex parentGW : parentGWs) {
					...
					for (Vertex pr : toProcess) {
						...
						toGW.add(pr);
						LinkedList<VertexPair> nodeMappingsGW = this.getMappingsVetrex(toGW, g2Vertices, threshold, stemmer, lookParents);
						nodeMappingsGW.addAll(this.getMappingsVetrex(pr.getParents(), g2Vertices, threshold, stemmer, lookParents));
				...
				LinkedList<VertexPair> nodeMappings = this.getMappingsVetrex(nochildGWs, g2Vertices, threshold, stemmer, lookParents);
				...
				for (Vertex childGW : childGWs) {
					...
					for (Vertex pr : toProcess) {
						...
						toGW.add(pr);
						LinkedList<VertexPair> nodeMappingsGW = this.getMappingsVetrex(toGW, g2Vertices, threshold, stemmer, lookParents);
						nodeMappingsGW.addAll(this.getMappingsVetrex(pr.getChildren(), g2Vertices, threshold, stemmer, lookParents));
						for (VertexPair vpp : nodeMappingsGW) {
							if (vpp.getWeight() > 0) {
								if (bestMapping  == null) {
									bestMapping = vpp;
		...
		for (Vertex v : g2Vertices) {
			if (v.getType().equals(Vertex.Type.gateway)
					&& !this.mappingContainsVertexRight(nodeMappingsPrev, v)
					&& !this.mappingContainsVertexRight(process, v)
					&& !this.mappingContainsVertexRight(processed, v)) {
				g2Gateways.add(v);
			}
		}
		...
		for (Vertex gw : g2Gateways) {
			if (lookParents == 1) {
				...
				LinkedList<VertexPair> nodeMappings = this.getMappingsVetrex(g1Vertices, noparentGWs, threshold, stemmer, lookParents);
				...
				for (Vertex parentGW : parentGWs) {
					...
					for (Vertex pr : toProcess) {
						LinkedList<Vertex> toGW = new  LinkedList<Vertex>();
						toGW.add(pr);
						LinkedList<VertexPair> nodeMappingsGW = this.getMappingsVetrex(g1Vertices, toGW, threshold, stemmer, lookParents);
						nodeMappingsGW.addAll(this.getMappingsVetrex(g1Vertices, pr.getParents(), threshold, stemmer, lookParents));
						for (VertexPair vpp : nodeMappingsGW) {
							if (vpp.getWeight() > 0) {
								if (bestMapping  == null) {
									bestMapping = vpp;
				...
				LinkedList<VertexPair> nodeMappings = this.getMappingsVetrex(g1Vertices, nochildGWs, threshold, stemmer, lookParents);
				...
				for (Vertex childGW : childGWs) {
					...
					for (Vertex pr : toProcess) {
						...
						toGW.add(pr);
						LinkedList<VertexPair> nodeMappingsGW = this.getMappingsVetrex(g1Vertices, toGW, threshold, stemmer, lookParents);
						nodeMappingsGW.addAll(this.getMappingsVetrex(g1Vertices, pr.getChildren(), threshold, stemmer, lookParents));
						for (VertexPair vpp : nodeMappingsGW) {
							if (vpp.getWeight() > 0) {
								if (bestMapping  == null) {
									bestMapping = vpp;
	...
	LinkedList<StringPair> getMappingsLabels(
