...
import common2.algos.GraphEditDistanceGreedy;
import common2.algos.TwoVertices;
import common2.similarity.AssingmentProblem;

public class MergingPaper2 implements MergeI {
	
	private AssingmentProblem assingmentProblem = new AssingmentProblem();
	
	public double getModelSimilarity(Graph g1, Graph g2) {
		
		LinkedList<VertexPair> mapping = getMapping(g1, g2);
		
		if (Settings.REMOVE_ENTANGLEMENT) {
		...
		for (VertexPair vp : mapping) {
			similarity += (1-vp.getWeight());
		}
		
		similarity = similarity == 0 ? 0 : similarity / Math.max(g1.getVertices().size(), g2.getVertices().size());
		return similarity;
	}
	...
	public Graph mergeModels (Graph g1, Graph g2, LinkedList<VertexPair> mapping) {
		...
		if (mapping == null) {
			mapping = getMapping(g1, g2);
		}
		
//		for (VertexPair vp : mapping) {
//			System.out.println(vp.getLeft() + " <> "+ vp.getRight() + " -> "+vp.getWeight());
//		}
		System.out.println("NrMappings: " + mapping.size());
//		System.out.print("**********************************\n");
		...
		for (LinkedList<VertexPair> region : mappingRegions.getRegions()) {
			for (VertexPair vp : region) {
				...
				for (Vertex c : vp.getRight().getChildren()) {
					...
					if (MergeModels.containsVertex(region, c)) {
				...
				for (Vertex c : nodesToProcess) {
					...
					Vertex cLeft = MergeModels.getMappingPair(mapping, c);
		...
		for (LinkedList<VertexPair> region : mappingRegions.getRegions()) {
			for (VertexPair vp : region) {
				...
				for (Vertex p : vp.getLeft().getParents()) {
					if (MergeModels.containsVertex(region, p)) {
						addgw = false;
						break;
					}
				}
		...
		for (LinkedList<VertexPair> region : mappingRegions.getRegions()) {
			for (VertexPair vp : region) {
				...
				for (Vertex ch : vp.getLeft().getChildren()) {
					if (MergeModels.containsVertex(region, ch)) {
						addgw = false;
						break;
					}
				}
...
//		System.out.println(gwInf[4]+"\t"+gwInf[0]);
	...
	public Set<TwoVertices> getMappingIDs(Graph g1, Graph g2) {
		...
		return gedepc.compute(g1, g2);
	}
	
	public LinkedList<VertexPair> getMapping(Graph g1, Graph g2, Set<TwoVertices> map) {
		
		LinkedList<VertexPair> mapping = new LinkedList<VertexPair>();
		
		for (TwoVertices pair : map) {
	...
	public LinkedList<VertexPair> getMapping(Graph g1, Graph g2) {
		
		LinkedList<VertexPair> mapping = new LinkedList<VertexPair>();
		
		if (Settings.COMPARISON_METHOD.equals(Settings.ComparisonMethod.Greedy)) {
			GraphEditDistanceGreedy gedepc = new GraphEditDistanceGreedy();
			Object weights[] = {"vweight", Settings.vweight, 
								"sweight", Settings.sweight, 
								"eweight", Settings.eweight};
...
//				System.out.println(v1+ " <> "+ v2);
		...
		} else if (Settings.COMPARISON_METHOD.equals(Settings.ComparisonMethod.Hungarian)) {
			mapping = assingmentProblem.getMappingsVetrexUsingNodeMapping(g1, g2, Settings.MERGE_THRESHOLD, Settings.MERGE_CONTEXT_THRESHOLD);
		}
		return mapping;
	}
...
//					System.out.println(vp1.getLeft() +" <> "+ vp1.getRight());
	...
	private void removeNonDominanceMappings2(LinkedList<VertexPair> mapping) {
		int nrRemoved = 0;
		LinkedList<VertexPair> removeList = new LinkedList<VertexPair>();
		int i = 0;
...
//					System.out.println(">>*REMOVING MAPPINGS" + vp1.getLeft() +" <> "+ vp1.getRight());
		...
		System.out.println(">> nr removed mappings: " + nrRemoved);
	}
