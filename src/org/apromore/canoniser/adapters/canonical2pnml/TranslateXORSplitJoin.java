...
public class TranslateXORSplitJoin {
	DataHandler data;
	long ids;
	...
	public void translate(NodeType node) {
		...
		int joincount = data.get_specialoperatorscount_value("joincount-"
				+ node.getName());
		int splitcount = data.get_specialoperatorscount_value("splitcount-"
				+ node.getName());
		...
		for (int i = 1; i <= splitcount; i++) {
			...
			trandoub.getToolspecific().add(trantool);
			ArcType splitarc = new ArcType();
			splitarc.setId("a95" + String.valueOf(i + joincount - 1));
