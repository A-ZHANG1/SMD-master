    ...
    private static void buildEdges(List<EdgeType> edgeTypes, CPF graph, Map<String, FlowNode> nodes) {
        for (EdgeType edge : edgeTypes) {
            ...
            if (source != null && target != null) {
	            ...
	            graph.setVertexProperty(source.getId(), FSConstants.TYPE, FragmentUtil.getType(source));
	            graph.setVertexProperty(target.getId(), FSConstants.TYPE, FragmentUtil.getType(target));
            }
        }
    }
