...
public class ChangePropagator {
	...
	private void fillUnchangedDescendantIds(String parentId, String updatedChildId,
			List<String> composingFragmentIds) throws ExceptionDao {
		List<FragmentVersionDagId> allChild = fvdDao.getChildMappings(parentId);
        for (FragmentVersionDagId child : allChild) {
            if (!child.getChildFragmentVersionId().equals(updatedChildId)) {
                composingFragmentIds.add(child.getChildFragmentVersionId());
                fillDescendantIds(child.getChildFragmentVersionId(), composingFragmentIds);
            }
        }
	}

	private void fillDescendantIds(String fragmentId, List<String> composingFragmentIds) throws ExceptionDao {
        List<FragmentVersionDagId> allChild = fvdDao.getChildMappings(fragmentId);
        for (FragmentVersionDagId child : allChild) {
            composingFragmentIds.add(child.getChildFragmentVersionId());
            fillDescendantIds(child.getChildFragmentVersionId(), composingFragmentIds);
        }
    ...
    private Map<String, String> createChildMap(List<FragmentVersionDagId> fvds) {
        Map<String, String> childMappings = new HashMap<String, String>();
        for (FragmentVersionDagId fvd : fvds) {
            childMappings.put(fvd.getPocketId(), fvd.getChildFragmentVersionId());
        }
