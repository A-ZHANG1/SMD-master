...
import org.prom5.framework.log.filter.LogFilterCollection;
import org.prom5.framework.log.rfb.BufferedLogReader;
import org.prom5.framework.ui.About;
...
/**
 * Main class of the process mining tool.
 * All plugins are loaded and the <code>MainUI</code> is shown.
 *
...
//					processArgument = args[i].substring(3).trim();
		...
		if (!analysisPluginArgument.equals("") && !miningPluginArgument.equals("")) {
			// both --M and --A given.
			printArgs();
			return;
		}
		...
		if (RuntimeUtils.isRunningMacOsX()) {
			...
			try {
				...
				System.out.println("Mac OS X: Working directory set to " +
						System.getProperty("user.dir") + " (from " + here.getAbsolutePath() + ")");
			} catch (IOException e) {
				e.printStackTrace();
			}
			System.out.println("Mac OS X: Working directory set to " +
		...
		splash.changeText("Working directory: " + System.getProperty("user.dir"), Message.DEBUG);
		//splash = new ClassicSplash();

		// exchange class loader
		/*
		   ClassLoader currentCl = Thread.currentThread().getContextClassLoader();
		 ClassLoader extendedCl = URLClassLoader.newInstance(getClasspathExtensionUrls(), currentCl);
		   Thread.currentThread().setContextClassLoader(extendedCl);
		 */
		for (URL url : getClasspathExtensionUrls()) {
			try {
				ClassPathHacker.addURL(url);
				splash.changeText("Added extension: " + url.toString(), Message.DEBUG);
				//System.out.println("Extending classpath with URL " + url.toString());
			} catch (IOException e) {
				System.err.println("ERROR: Could not extend classpath with URL " + url.toString());
				e.printStackTrace();
			}
		}
		...
		/*		final String logArgument = logArgument.trim();
		  final String algArgument = miningPluginArgument.trim();
		  final String proArgument = processArgument.trim();
		 */
		...
		SwingWorker sw = new SwingWorker() {
			public Object construct() {
				...
				MiningPluginCollection.getInstance().loadFromIni(System.getProperty("user.dir",
						"") + System.getProperty("file.separator") + ALGORITHMS_FILE, splash);
				splash.setProgress(20);
				MiningPluginCollection.getInstance().loadFromIni(System.getProperty("user.home",
						"") + System.getProperty("file.separator") + ALGORITHMS_FILE, splash);
				splash.setProgress(30);

				splash.changeText("Analysis Plugins", Message.NORMAL);
				AnalysisPluginCollection.getInstance().loadFromIni(System.getProperty(
						"user.dir",
						"") + System.getProperty("file.separator") + ANALYSES_FILE, splash);
				...
				ImportPluginCollection.getInstance().loadFromIni(System.getProperty("user.dir",
						"") + System.getProperty("file.separator") + IMPORTS_FILE, splash);
				splash.setProgress(55);
				ImportPluginCollection.getInstance().loadFromIni(System.getProperty("user.home",
						"") + System.getProperty("file.separator") + IMPORTS_FILE, splash);
				splash.setProgress(60);

				splash.changeText("Export Plugins", Message.NORMAL);
				ExportPluginCollection.getInstance().loadFromIni(System.getProperty("user.dir",
						"") + System.getProperty("file.separator") + EXPORTS_FILE, splash);
				splash.setProgress(65);
				ExportPluginCollection.getInstance().loadFromIni(System.getProperty("user.home",
						"") + System.getProperty("file.separator") + EXPORTS_FILE, splash);
				splash.setProgress(70);

				splash.changeText("Conversion Plugins", Message.NORMAL);
				ConvertingPluginCollection.getInstance().loadFromIni(System.getProperty(
						"user.dir",
						"") + System.getProperty("file.separator") + CONVERT_FILE, splash);
				...
				LogFilterCollection.getInstance().loadFromIni(System.getProperty("user.dir",
						"") + System.getProperty("file.separator") + LOGFILTER_FILE, splash);
				LogFilterCollection.getInstance().loadFromIni(System.getProperty("user.home",
						"") + System.getProperty("file.separator") + LOGFILTER_FILE, splash);
				splash.setProgress(90);

				splash.changeText("Process Instance Scales", Message.NORMAL);
				ScaleCollection.getInstance().loadFromIni(System.getProperty("user.dir",
						"") + System.getProperty("file.separator") + SCALE_FILE, splash);
				ScaleCollection.getInstance().loadFromIni(System.getProperty("user.home",
						"") + System.getProperty("file.separator") + SCALE_FILE, splash);
				splash.setProgress(95);

				splash.changeText("Recommendation Contributors", Message.NORMAL);
				RecommendationCollection.getInstance().loadFromIni(System.getProperty("user.dir",
						"") + System.getProperty("file.separator") + CONTRIB_FILE, splash);
				RecommendationCollection.getInstance().loadFromIni(System.getProperty("user.home",
						"") + System.getProperty("file.separator") + CONTRIB_FILE, splash);
			...
			public void finished() {
				...
				while (it.hasNext()) {
					Message.add((String) it.next(), ((Integer) it.next()).intValue());
				}
				...
				if (argsGiven) {
					MainUI.getInstance().setParameters(miningPluginArgument, analysisPluginArgument, logArgument);
				}
				Message.add("ProM Framework Initialized");
				splash.close();
				// add beta warning
				if (About.VERSION.matches("(.*)[Bb][Ee][Tt][Aa](.*)") == true) {
					File logoFile = new File("./images/icons/default/applogo_small.png");
	...
	/**
	 * Scans all standard extension directories (provided by <code>getExtensionLibraryDirectories()</code>
	 * for contained jar files. The locations of all found jar files are added to an URL array which is
	 * returned.
	...
	private static final URL[] getClasspathExtensionUrls() {
		...
		for (Iterator it = getExtensionLibraryDirectories().iterator(); it.hasNext(); ) {
			...
			for (int i = 0; i < fileList.length; i++) {
				...
				if ((fileList[i].getAbsolutePath().substring(
						fileList[i].getAbsolutePath().length() - 3).equalsIgnoreCase("jar")) &&
						(!fileList[i].getAbsolutePath().substring(
						fileList[i].getAbsolutePath().length() - 9).equalsIgnoreCase(File.separator +
						"prom.jar"))) {
					try {
	...
	private static final ArrayList getExtensionLibraryDirectories() {
		...
		// the plugin and framework folder can be determined and added dynamically
	...
	/**
	 * Retrieves the location where dynamically loaded plugins are store
	 * and loaded from. "./lib/plugins"
	...
	/**
	 * Retrieves the location where dynamically loaded external resources are store
	 * and loaded from. "./lib/external"
	 * @return path to plugins folder
	 */
	...
	/**
	 * Retrieves the location where dynamically loaded models are store
	 * and loaded from. "./lib/models"
	 * @return path to plugins folder
	 */
