...
public class AnalysisPluginCollection extends PluginCollection {
	...
	/**
	 * Returns <code>true</code> if the given plugin is an <code>AnalysisPlugin</code>.
	 *
	 * @param plugin the plugin to check
	 * @return <code>true</code> if the given plugin is an <code>AnalysisPlugin</code>,
	...
	protected AnnotatedPlugin createFromAnnotation(Class<?> pluginClass, Method pluginMethod, Annotation annotation, ProMSplash splash) {
		return new AnnotatedAnalysisPlugin(pluginClass, pluginMethod, annotation);
	}
}
...
class AnnotatedAnalysisPlugin extends AnnotatedPlugin implements AnalysisPlugin {
	...
	public AnnotatedAnalysisPlugin(Class<?> pluginClass, Method pluginMethod, Annotation annotation) {
		super(pluginClass, pluginMethod, ((Analyzer) annotation).name(), ((Analyzer) annotation).help(), ((Analyzer) annotation).sortName());
	...
	public void validate() throws Exception {
		if (getPluginMethod().getParameterTypes().length == 0) {
			throw new Exception("Analysis plugin methods need to have at least one parameter (an object to analyze)");
		}
		if (!JComponent.class.isAssignableFrom(getPluginMethod().getReturnType())) {
			throw new Exception("Analysis plugin methods need to return an object which is derived from JComponent");
		}
	}

	public JComponent analyse(AnalysisInputItem[] inputs) {
		return connected ? analyseConnected(inputs) : analyseDisconnected(inputs);
	}

	public AnalysisInputItem[] getInputItems() {
		return connected ? getConnectedInputItems() : getDisconnectedInputItems();
	}
	...
	private JComponent analyseConnected(AnalysisInputItem[] inputs) {
		...
		for (int i = 0; i < parameters.length; i++) {
			...
			if (!found) {
				return new JLabel("Internal Error: provided objects do not match the requested objects. Please notify the developers of this problem.");
			}
		}
		try {
			return (JComponent) getPluginMethod().invoke(getNewPluginInstance(), arguments);
		} catch (Throwable t) {
			t.printStackTrace();
	...
	private AnalysisInputItem[] getConnectedInputItems() {
		...
		return new AnalysisInputItem[] {
				new AnalysisInputItem(name) {
					...
					if (object != null) {
						...
						if (o != null) {
							for (int i = 0; i < o.length; i++) {
								if (o[i] != null && parameterType.isAssignableFrom(o[i].getClass())) {
									return true;
								}
							}
						}
