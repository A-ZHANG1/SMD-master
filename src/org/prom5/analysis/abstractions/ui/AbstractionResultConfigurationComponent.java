...
//	protected JScrollPane abstractionElementsScrollPane;
	...
	public AbstractionResultConfigurationComponent(String absName, Set<String> absSet){
...
//		this.setPreferredSize(new Dimension(390, 120));
		...
		for(String activity : absSet){
			combinedActivityString += activity.replaceAll("complete", "").replaceAll("-", "")+delimiters[index%9];
			index++;
		}
		...
		TreeSet<String> filteredMRSet = new TreeSet<String>(new DescendingStrLengthComparator());
		for(String mr : mrSet)
			if(mr.trim().length() > 5)
...
//		abstractionName.setPreferredSize(new Dimension(this.getSize().width/3,this.getSize().height));
//		abstractionName.setMaximumSize(new Dimension(this.getSize().width/3,this.getSize().height));
		abstractionName.setColumns(20);
		abstractionName.setSize(this.getSize().width/3,this.getSize().height);
...
//		abstractionElementsScrollPane = new JScrollPane(abstractionElements);
		...
		activitySelectionPanel = new ActivitySelectionPanel(absSet, selectedActivities);
...
//				MainUI.getInstance().createFrame("Abstracted Activities", activitySelectionPanel);
		...
		removeButton.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				if(abstractionElements.getSelectedValues().length > 0){
					...
					if(removedActivities.size()+abstractionElements.getSelectedValues().length == originalAbstractionActivities.size()){
