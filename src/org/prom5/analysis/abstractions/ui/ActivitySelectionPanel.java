...
public class ActivitySelectionPanel extends JPanel implements ActionListener{
	...
	public ActivitySelectionPanel(Set<String> absSet, Set<String> selectedActivites){
	...
	protected void setupGui(){
		...
		tableScrollPane.setBorder(BorderFactory.createLineBorder(new Color(40, 40, 40)));
		...
		if (e.getActionCommand().equalsIgnoreCase("Deselect")){
			...
			if (selectedRowsLocal.length > 0){
				...
				for (int i = 0; i < selectedRowsLocal.length; i++){
					selectedActivities.add((String)table.getModel().getValueAt(selectedRowsLocal[i], 0));
				}
	...
	protected void closeFrame(){
		...
		enclosing frame (usually the object itself). Must be final for anonymous
		inner class to work.
		 */
		final JComponent myself = this;

		// walk through all frames to find enclosing instances
		for(JInternalFrame frame : MainUI.getInstance().getDesktop().getAllFrames()) {
