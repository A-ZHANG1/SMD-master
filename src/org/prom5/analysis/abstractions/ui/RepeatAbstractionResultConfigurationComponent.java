...
public class RepeatAbstractionResultConfigurationComponent extends SmoothPanel {
	...
	public RepeatAbstractionResultConfigurationComponent(String absName, Set<String> alphabet){
		...
		for(String symbol : alphabet)
			decodedActivitySet.add(RepeatAbstractionResUI.charActivityMap.get(symbol));
		
		String combinedActivityString = "";
		int index = 0;
		
		for(String decodedActivity : decodedActivitySet){
			combinedActivityString += decodedActivity.replaceAll("complete", "").replaceAll("-", "")+delimiters[index%9];
			index++;
		}
		...
		TreeSet<String> filteredSMRSet = new TreeSet<String>(new DescendingStrLengthComparator());
		...
		activitySelectionPanel = new ActivitySelectionPanel(alphabet, selectedActivities);
		...
		removeButton.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				if(abstractionActivitiesList.getSelectedValues().length > 0){
					...
					if(removedActivities.size()+abstractionActivitiesList.getSelectedValues().length == originalAbstractionActivities.size()){
						MainUI.getInstance().showGlassDialog("Invalid Selection", "Can't delete all elements in the abstraction with this Option");
						abstractionActivitiesList.clearSelection();
					}else{
						for(Object o  : abstractionActivitiesList.getSelectedValues()){
							removedActivities.add(RepeatAbstractionResUI.activityCharMap.get((String)o));
						}
...
//						System.out.println("No. Removed Elements: "+removeElementsList.size());
						...
						for(String symbol : originalAbstractionActivities)
							if(!removeElementsList.contains(symbol)){
								...
								decodedSelectedActivitySet.add(RepeatAbstractionResUI.charActivityMap.get(symbol));
							}
...
//		return abstractionName.getText();
		return "("+abstractionName.getText().replaceAll("\\[", "").replaceAll("\\]","")+")";
	}
