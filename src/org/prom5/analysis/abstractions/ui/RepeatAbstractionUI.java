...
import org.prom5.analysis.abstractions.util.ActivitySet;
import org.prom5.analysis.abstractions.util.UkkonenSuffixTree;
...
//			io.writeToFile("D:\\JC", "ActivityCharMapRepeatAbstraction.txt", activityCharMap, "^");
...
//			System.out.println("IO Exception while Reading Audit TrailEntryList in LoopAbstractionUI scanThread");
		...
		try{
			for(int i=0; i<log.numberOfInstances(); i++) {
				progress.setNote("Converting to CharStreams Process Instance "+(i+1));
				currentCharStream = "";
				AuditTrailEntryList ateList = log.getInstance(i).getAuditTrailEntryList();
				for(int a=0; a<ateList.size(); a++) {
...
//			combinedCharStream += charStream + activityCharMap.get("ProcessInstance"+charStreamIndex);
//			charStreamIndex++;
//		}
		
		StringBuilder combinedCharStreamBuilder = new StringBuilder();
		for(String charStream : charStreamsSet){
			combinedCharStreamBuilder.append(charStream + activityCharMap.get("ProcessInstance"+charStreamIndex));
			charStreamIndex++;
		}
		
//		UkkonenSuffixTree st = new UkkonenSuffixTree(encodingLength,combinedCharStream);
		UkkonenSuffixTree st = new UkkonenSuffixTree(encodingLength,combinedCharStreamBuilder.toString());
		st.findLeftDiverseNodes();
		HashSet<String> nearSuperMaximalRepeatSet = st.getNearSuperMaximalRepeats();
...
*/
		 ...
		 */
//		HashMap<String, TreeSet<String>> repeatRepeatAlphabetMap = new HashMap<String, TreeSet<String>>();
		...
		for(String nearSuperMaximalRepeat : nearSuperMaximalRepeatSet){
			nearSuperMaximalRepeatLength = nearSuperMaximalRepeat.length()/encodingLength;
			repeatAlphabet = new TreeSet<String>();
			for(int i = 0; i < nearSuperMaximalRepeatLength; i++)
				repeatAlphabet.add(nearSuperMaximalRepeat.substring(i*encodingLength, (i+1)*encodingLength));
			
//			if(!repeatRepeatAlphabetMap.containsKey(nearSuperMaximalRepeat))
//				repeatRepeatAlphabetMap.put(nearSuperMaximalRepeat, repeatAlphabet);
			
			if(nearSuperMaximalRepeatAlphabetRepeatMap.containsKey(repeatAlphabet)){
				nearSuperMaximalRepeatAlphabetRepeatSet = nearSuperMaximalRepeatAlphabetRepeatMap.get(repeatAlphabet);
			}else{
				nearSuperMaximalRepeatAlphabetRepeatSet = new TreeSet<String>(new DescendingStrLengthComparator());
			}
			
			nearSuperMaximalRepeatAlphabetRepeatSet.add(nearSuperMaximalRepeat);
			nearSuperMaximalRepeatAlphabetRepeatMap.put(repeatAlphabet, nearSuperMaximalRepeatAlphabetRepeatSet);
		}
...
//		combinedCharStream = null;
		...
		for(TreeSet<String> nsmra : nearSuperMaximalRepeatAlphabetRepeatMap.keySet()){
			nearSuperMaximalRepeatAlphabetRepeatSet = nearSuperMaximalRepeatAlphabetRepeatMap.get(nsmra);
			count = 0;
...
//		io.writeToFile("D:\\JC", "nsmRAMap.txt", nearSuperMaximalRepeatAlphabetRepeatMap,"^");
//		io.writeToFile("D:\\JC", "nsmRACountMap.txt", nearSuperMaximalRepeatAlphabetCountMap, "^");
		
		progress.setNote("Done");
		progress.close();
		view = new RepeatAbstractionResUI(encodingLength,nearSuperMaximalRepeatAlphabetRepeatMap, nearSuperMaximalRepeatAlphabetCountMap, charActivityMap);
