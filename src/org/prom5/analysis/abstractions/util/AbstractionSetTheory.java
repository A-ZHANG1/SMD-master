...
public class AbstractionSetTheory {
	...
	public AbstractionSetTheory(){
		sg = new SimpleDirectedGraph<TreeSet<String>, DefaultEdge>(DefaultEdge.class);
		maximalElementSubsumedElementMap = new HashMap<TreeSet<String>, TreeSet<TreeSet<String>>>();
	}
	
	public List<TreeSet<String>> getMaximalElementsSubsumption(Set<TreeSet<String>> alphabetSet){
			...
			for(TreeSet<String> alphabetJ : alphabetSet){
				if(!alphabetI.equals(alphabetJ)){
					if(alphabetJ.containsAll(alphabetI)){
						DijkstraShortestPath<TreeSet<String>, DefaultEdge> d = new DijkstraShortestPath<TreeSet<String>, DefaultEdge>(sg,alphabetI,alphabetJ);
	...
	public List<TreeSet<String>> getMaximalElementsApproximateSubsumption(Set<TreeSet<String>> alphabetSet, double commonElementsThreshold, double differenceElementsThreshold, DiffStrategy d){
...
//				if(!doneSet.contains(alphabetJ))
					if(!alphabetI.equals(alphabetJ) && !alphabetJ.containsAll(alphabetI)){
					if(alphabetJ.size() >= alphabetI.size()){
		
						minSize = alphabetI.size();
						
						commonThreshold = (int)Math.ceil(minSize * commonElementsThreshold);
						diffThreshold = (int)Math.ceil(minSize * differenceElementsThreshold);
						...
						if(noCommonElements >= commonThreshold && noDifferenceElements <= diffThreshold){
							
							if(alphabetI.size() < alphabetJ.size()){
								sg.addEdge(alphabetI, alphabetJ);
							}else{
								if(sg.inDegreeOf(alphabetJ) >= sg.inDegreeOf(alphabetI)){
									sg.addEdge(alphabetI, alphabetJ);
								}else{
									sg.addEdge(alphabetJ, alphabetI);
								}
...
//			doneSet.add(alphabetI);
	...
	public HashMap<TreeSet<String>, TreeSet<TreeSet<String>>> findMaximalElementSubsumedElementMap(Set<TreeSet<String>> alphabetSet, List<TreeSet<String>> maxElementList){
		...
		for(TreeSet<String> maxElement : maxElementList){
			maximalElementSubsumedElementSet = new TreeSet<TreeSet<String>>(new TreeSetComparator());
			for(TreeSet<String> alphabet : alphabetSet){
				if(!maxElement.equals(alphabet)){
					if(sg.containsEdge(alphabet, maxElement)){
						...
						if(alphabetMaximalElementMap.containsKey(alphabet)){
							alphabetMaximalElementSet = alphabetMaximalElementMap.get(alphabet);
						}else{
							alphabetMaximalElementSet = new TreeSet<TreeSet<String>>(new TreeSetComparator());
						}
						alphabetMaximalElementSet.add(maxElement);
						alphabetMaximalElementMap.put(alphabet, alphabetMaximalElementSet);
					}
				}else{
					alphabetMaximalElementSet = new TreeSet<TreeSet<String>>(new TreeSetComparator());
					alphabetMaximalElementSet.add(maxElement);
			...
			maxElementSubsumedElementMap.put(maxElement, maximalElementSubsumedElementSet);
			maximalElementSubsumedElementMap.put(maxElement, maximalElementSubsumedElementSet);
		}
...
//		System.out.println("TempSet: "+tempSet.size());
		if(tempSet.size() > 0){
			...
			List<TreeSet<String>> bMax = b.getMaximalElementsApproximateSubsumption(tempSet, commonElementsThreshold, differenceElementsThreshold, d);
			
//			System.out.println("no. Max : "+bMax.size());
//			System.out.println(bMax);
			HashMap<TreeSet<String>, TreeSet<TreeSet<String>>> bMaxSubMap = b.findMaximalElementSubsumedElementMap(tempSet, bMax);
			...
			for(TreeSet<String> bm : bMaxSubMap.keySet()){
				if(maximalElementSubsumedElementMap.containsKey(bm)){
					maximalElementSubsumedElementSet = maximalElementSubsumedElementMap.get(bm);
				}else{
					maximalElementSubsumedElementSet = new TreeSet<TreeSet<String>>(new TreeSetComparator());
				}
				maximalElementSubsumedElementSet.addAll(bMaxSubMap.get(bm));
				maximalElementSubsumedElementMap.put(bm, maximalElementSubsumedElementSet);
			}
			
			HashMap<TreeSet<String>, TreeSet<TreeSet<String>>> bAMEMap = b.getAlphabetMaximalElementMap();
			for(TreeSet<String> alp: bAMEMap.keySet()){
				if(alphabetMaximalElementMap.containsKey(alp)){
					alphabetMaximalElementSet = alphabetMaximalElementMap.get(alp);
				}else{
					alphabetMaximalElementSet = new TreeSet<TreeSet<String>>(new TreeSetComparator());
				}
	...
	public HashMap<Integer, TreeSet<TreeSet<String>>> getSizeAlphabetSetMap(Collection<TreeSet<String>> alphabetSet){
		...
		for(TreeSet<String> maximalElement : alphabetSet){
			...
			if(sizeAlphabetSetMap.containsKey(size))
				...
				sizeAlphabetSet = new TreeSet<TreeSet<String>>(new TreeSetComparator());
