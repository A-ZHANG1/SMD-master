...
package org.prom5.analysis.abstractions.util;
...
//		this.sequence = sequence;
	...
	public ArrayList<SuffixNode> getLeaves(SuffixNode node) throws StackOverflowError{
...
//		System.out.println("No. Leaf Nodes in the Tree: " + leaves.size());
	...
	/**
	 * Find NearSuperMaximal Repeats; A left diverse internal node v represents a
	 * near super maximal repeat alpha if and only if one of v's children is a leaf
	 * and its left character is the left character of no other leaf below v
	 * 
	 * Assumes that the parent method calling this would have already invoked
	 * findLeftDiverseNodes();
	 * 
	...
	/*private String getPrimitiveRepeat(String tandemRepeat){
		...
		for(int i = 0; i < tandemRepeatLength; i++){
			tandemRepeatAlphabet.add(tandemRepeat.substring(i*encodingLength, (i+1)*encodingLength));
		}
...
//			System.out.println(tr);
		...
		for (String tandemRepeatPair : tandemRepeatSet) {
			...
			tandemRepeatLength /= 2; // The division by 2 is to consider only the  repeat alpha
			
			tandemRepeat = tandemRepeatPair.substring(0,tandemRepeatLength*encodingLength);
			tandemRepeatArrayList.add(tandemRepeat);
			
			tandemRepeatAlphabetSet = new TreeSet<String>();
			for(int i = 0; i < tandemRepeatLength; i++){
				tandemRepeatAlphabetSet.add(tandemRepeat.substring(i*encodingLength, (i+1)*encodingLength));
			}
		...
		for(String tr : tandemRepeatArrayList){
			...
			if(!isTandemRepeatProcessed[index]){
				...
				/**
				 * Simple Cases
				 * 1. When the tandem repeat is made up of only one symbol
				 * i.e., the alphabet size is 1
				 * 2. When the tandem repeat, alpha is itself a primitive repeat
				 * i.e., the size of alpha is the same as the size of the alphabet
				 */
				...
				if(tandemRepeatAlphabetSet.size() == 1){
					...
					primitiveRepeat = tr;
					primitiveRepeatSet.add(tr);
				}/*else if(tandemRepeatLength/tandemRepeatAlphabetSet.size() > 25){
					
					*//**
					 * Get the contribution (frequency of each symbol)
					 * If the contribution of an alphabet is just 1 or a few (which is negligable to that of the total length), then
					 * split the tr using that negligable symbol
					 *//*
...
//					System.out.println(tr);
					...
					st.LZDecomposition();
					HashMap<TreeSet<String>,TreeSet<String>> tprAM =st.getPrimitiveTandemRepeats(); 
					for(TreeSet<String> prA : tprAM.keySet()){
						primitiveRepeatSet.addAll(tprAM.get(prA));
						if(alphabetPrimitiveRepeatMap.containsKey(prA))
							alphabetPrimitiveRepeatSet = alphabetPrimitiveRepeatMap.get(prA);
						else
							alphabetPrimitiveRepeatSet = new TreeSet<String>();
						alphabetPrimitiveRepeatSet.addAll(tprAM.get(prA));
						alphabetPrimitiveRepeatMap.put(prA, alphabetPrimitiveRepeatSet);
					}
...
//						System.out.println("Adding Complex: "+tr.substring(0*encodingLength,tandemRepeatLength*encodingLength/2));
						complexTandemRepeats.add(tr.substring(0*encodingLength,tandemRepeatLength*encodingLength/2));	
					}else{
//						System.out.println("Adding Complex: "+tr);
				...
				if(!isComplex && !isNegligable){
					...
					/**
					 * Applicable for both the simple cases 1. and 2.
					 * Since the tandem repeat list is sorted; the smallest tandem repeat pair would be alpha^2;
					 * However, there can be similar tandem repeats of the type alpha^(2*n);
					 * Set the primitive repeat type to all such tandem repeats
					 * 	a. Get all tandem repeats that share the same alphabet
					 * 	b. Verify if the tandem repeat is of type alpha^(2*n). 
					 * 	   There can be instances where two tandem repeats share the same alphabet 
					 *     but alpha's are different 
					 */
...
//						 System.out.println(primitiveRepeat+"  STR: "+similarTandemRepeat);
						 ...
						 if(prMatcher.replaceAll("").equals("")){
							 index = tandemRepeatArrayList.indexOf(similarTandemRepeat);
							 isTandemRepeatProcessed[index] = true;
							 noProcessed++;
...
//							 System.out.println(similarTandemRepeat+" @ "+primitiveRepeat);
		...
		/**
		 * By now all simple cases would have been handled; Let us process complex repeats now
		 * It is important to process all simple cases before we proceed to tackle complex cases
		 */
		String prRegEx;
		Iterator<String> it;
...
//			System.out.println(tandemRepeatAlphabetMap.size());
			if(tandemRepeatAlphabetMap.containsKey(tr))
				...
				for(int jj = 0; jj < trLength; jj++)
					tandemRepeatAlphabetSet.add(tr.substring(jj*encodingLength, (jj+1)*encodingLength));
				tandemRepeatAlphabetMap.put(tr, tandemRepeatAlphabetSet);
			}
//			System.out.println(tr+" @ "+tandemRepeatAlphabetSet);
			/**
			 * A Complex Case of a Tandem Repeat
			 * Many Scenarios exist
			 * 1. Check whether this tandem repeat is a combination of other primitive repeats
			 * 	a.Check if there exists primitive repeats with same alphabet as that of this tandem repeat 
			 *    Then it might be the case that this tandem repeat is a combination of the other primitive repeats 
			 *    (resulting out of variation in ordering - parallelism within loops)
			 *    
			 *    e.g., d5i4o4p1o4p1d5i4 is the current tandem repeat that is complex
			 *    there exist primitive repeats d5i4o4p1 and o4p1d5i4 sharing the same alphabet
			 *    
			 *  2. The exact alphabet doesn't exist; Check whether the complex tandem repeat is a combination of 
			 * 	   primitive repeats involving the subset alphabets
			 * 
			 *    e.g., o4p1o4p1d5i4d5i4
			 */
			
			toConsiderPrimitiveRepeatSet = new TreeSet<String>(new DescendingStrCompartor());	
			for(TreeSet<String> alphabetSet : alphabetPrimitiveRepeatMap.keySet()){
				
				if(tandemRepeatAlphabetSet.containsAll(alphabetSet)){
					for(String ptr : alphabetPrimitiveRepeatMap.get(alphabetSet))
						if(tr.contains(ptr))
...
//				System.out.println("Solved: "+tr);
				...
				/**
				 * Alphabet Matching: But a new Pattern exists
				 * Check if the remaining substring in the tandem repeat is a permutation of the alphabet
				 * if not a permutation,then it can be a case that the remaining substring consists of a 
				 * subset of the alphabet
				 * 
				 * e.g., ctr: d5i4o4p1p1o4d5i4
				 * now p1o4d5i4 doesn't exist as a pr; but the alphabet is the same; 
				 * may be in this log it is not manifested
				 * 
				 *  Write these as complex tandem repeats (unprocessed); 
				 *  If there exist another permutation in any other log, we can process them separately
				 */
...
//				 System.out.println("In Else: "+tr);
				 UkkonenSuffixTree st = new UkkonenSuffixTree(encodingLength,tr);
				 st.LZDecomposition();
				 HashMap<TreeSet<String>,TreeSet<String>> tempMap = st.getPrimitiveTandemRepeats();
		...
		for(String complexTR : complexTandemRepeats){
			if(tandemRepeatAlphabetMap.containsKey(complexTR))
				tandemRepeatAlphabetSet = tandemRepeatAlphabetMap.get(complexTR);
			else{
				tandemRepeatAlphabetSet = new TreeSet<String>();
				complexTRLength = complexTR.length()/encodingLength;
				for(int jj = 0; jj < complexTRLength; jj++)
					tandemRepeatAlphabetSet.add(complexTR.substring(jj*encodingLength, (jj+1)*encodingLength));
				tandemRepeatAlphabetMap.put(complexTR, tandemRepeatAlphabetSet);
			}
			if(complexAlphabetTandemRepeatMap.containsKey(tandemRepeatAlphabetSet)){
				tandemRepeatSet = complexAlphabetTandemRepeatMap.get(tandemRepeatAlphabetSet);
			}else{
				tandemRepeatSet = new TreeSet<String>();
			}
			tandemRepeatSet.add(complexTR);
			complexAlphabetTandemRepeatMap.put(tandemRepeatAlphabetSet, tandemRepeatSet);
		}
