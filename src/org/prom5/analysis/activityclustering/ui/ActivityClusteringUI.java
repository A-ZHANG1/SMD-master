 ...
 * 
 */
package org.prom5.analysis.activityclustering.ui;
...
import org.prom5.analysis.activityclustering.ClusterScanner;
import org.prom5.analysis.activityclustering.ClusterTypeSet;
import org.prom5.analysis.activityclustering.metrics.StrictCorrelationMetric;
import org.prom5.analysis.activityclustering.model.Cluster;
...
public class ActivityClusteringUI extends JPanel {
	...
	protected void startClustering() {
		...
		Thread scanThread = new Thread() {
			public void run() {
				ClusterScanner scanner = new ClusterScanner(log, metric, lookaheadSlider.getValue(), thresholdSlider.getValue());
				ClusterTypeSet typeSet = new ClusterTypeSet(minOverlapSlider.getValue());
				progress.setMinMax(0, log.numberOfInstances());
				for(int i=0; i<log.numberOfInstances(); i++) {
					progress.setProgress(i);
					progress.setNote("Scanning trace " + (i + 1) + " (of " + log.numberOfInstances() + ")");
					try {
				...
				Message.add("Scanned a total of " + typeSet.size() + " cluster types");
				remove(view);
	...
	protected void buildConfigurationPanel() {
		...
		topLabel.setFont(topLabel.getFont().deriveFont(16f).deriveFont(Font.BOLD));
		proximityLabel = new JLabel(StopWatch.formatDuration(600000));
		proximityLabel.setFont(proximityLabel.getFont().deriveFont(11f));
		useOriginatorBox = new JCheckBox("enforce same originator in clusters", true);
		...
		useDataTypesBox = new JCheckBox("cluster events with same attributes", true);
		...
		useDataValuesBox = new JCheckBox("cluster events with similar data", true);
		...
		proximitySlider.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				proximityLabel.setText(StopWatch.formatDuration(proximitySlider.getValue()));
		...
		thresholdSlider = new NiceDoubleSlider("Min. correlation", 0.0, 1.0, 0.2);
		minOverlapSlider = new NiceDoubleSlider("Footprint overlap", 0.5, 1.0, 0.7);
