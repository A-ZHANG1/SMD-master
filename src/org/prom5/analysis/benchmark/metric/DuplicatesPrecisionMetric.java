...
import org.prom5.converting.PetriNetToHeuristicNetConverter;
...
public class DuplicatesPrecisionMetric implements BenchmarkMetric {
	 ...
	 * @param referenceLog This parameter is not used for this metric. Therefore, it can be <code>null</code>
	 * @param referenceModel The Petri net used to measure
	 *   duplicates precision of the mined model.
	 * @param progress Progress
	 * @return The duplicates precision value (<code>[0, 1]</code>) of the mined model. If the
	 * duplicates precision value cannot be calculated, the value <code>BenchmarkMetric.INVALID_MEASURE_VALUE</code> is returned.
	 */
	public double measure(PetriNet model, LogReader referenceLog, PetriNet referenceModel,
			Progress progress) {
		
		// check precondition: same number of task labels in model and reference model
		if (model.getNumberOfNonDuplicateTasks() != referenceModel.getNumberOfNonDuplicateTasks()) {
			return BenchmarkMetric.INVALID_MEASURE_VALUE;
		}

		try {

			HeuristicsNet HNmodel = new PetriNetToHeuristicNetConverter().toHeuristicsNet(
									PetriNetToHeuristicNetConverter.
									removeUnnecessaryInvisibleTasksFromPetriNet( (PetriNet)model.clone()));
			HeuristicsNet HNreferenceModel = new PetriNetToHeuristicNetConverter().toHeuristicsNet(
											 PetriNetToHeuristicNetConverter.
											 removeUnnecessaryInvisibleTasksFromPetriNet(
											 (PetriNet)referenceModel.clone()));
			DuplicatesEquivalent duplicatesMetrics = new DuplicatesEquivalent(HNreferenceModel,
					HNmodel);
			return duplicatesMetrics.getPrecision();
		} catch (Exception e) {
			System.err.println(
					"DuplicatesPrecisionMetric >>> Could not calculate the duplicates precision value!");
			e.printStackTrace();
