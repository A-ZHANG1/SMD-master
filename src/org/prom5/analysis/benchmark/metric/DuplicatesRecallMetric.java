...
public class DuplicatesRecallMetric implements BenchmarkMetric {
	 ...
	 *   duplicates recall of the mined model.
	 * @param progress Progress
	 * @return The duplicates recall value (<code>[0, 1]</code>) of the mined model. If the
	 * duplicates recall value cannot be calculated, the value <code>BenchmarkMetric.INVALID_MEASURE_VALUE</code> is returned.
	 */
	public double measure(PetriNet model, LogReader referenceLog, PetriNet referenceModel,
		...
		try {
			...
			HeuristicsNet HNreferenceModel = new PetriNetToHeuristicNetConverter().toHeuristicsNet(
											 PetriNetToHeuristicNetConverter.
											 removeUnnecessaryInvisibleTasksFromPetriNet((PetriNet)
											 referenceModel.clone()));
			DuplicatesEquivalent duplicatesMetrics = new DuplicatesEquivalent(HNreferenceModel,
					HNmodel);
			return duplicatesMetrics.getRecall();
		} catch (Exception e) {
			System.err.println(
					"DuplicatesRecallMetric >>> Could not calculate the duplicates recall value!");
			e.printStackTrace();
