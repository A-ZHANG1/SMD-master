...
public class StructuralPrecisionMetric implements BenchmarkMetric {
	 ...
	 *   structural precision of the mined model.
	 * @param progress Progress
	 * @return The structural precision value (<code>[0, 1]</code>) of the mined model. If the
	 * structural precision value cannot be calculated, the value <code>BenchmarkMetric.INVALID_MEASURE_VALUE</code> is returned.
	 */
	public double measure(PetriNet model, LogReader referenceLog, PetriNet referenceModel,
		...
		try {
			...
			HeuristicsNet HNreferenceModel = new PetriNetToHeuristicNetConverter().toHeuristicsNet(
											 PetriNetToHeuristicNetConverter.removeUnnecessaryInvisibleTasksFromPetriNet((PetriNet)referenceModel.clone()));
			ConnectionEquivalent structuralMetrics = new ConnectionEquivalent(HNreferenceModel, HNmodel);
			return structuralMetrics.getPrecision();
		} catch (Exception e) {
			System.err.println(
					"StructuralPrecisionMetric >>> Could not calculate the structural precision value!");
					 e.printStackTrace();
