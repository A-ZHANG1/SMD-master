 ...
 */
public class BenchmarkItemList extends RoundedPanel {
	...
	public BenchmarkItemList(BenchmarkUI benchmarkUI) {
		super(10, 5, 5);
		...
		itemList.setCellRenderer(new BenchmarkItemListRenderer());
		modelBoxModel = new DefaultComboBoxModel(ui.getPetriNets().keySet().toArray());
		modelBox = new JComboBox(modelBoxModel);
		modelBox.setMaximumSize(new Dimension(1000, 24));
		modelBox.setPreferredSize(new Dimension(200, 24));
		...
		itemNameField.setPreferredSize(new Dimension(200, 24));
		...
		add(wrapNamedComponent("Plugin name:", itemNameField));
		add(Box.createVerticalStrut(5));
		add(wrapNamedComponent("Mining result:", modelBox));
		add(Box.createVerticalStrut(5));
		JButton addButton = new SlickerButton("add new benchmark item");
		if(RuntimeUtils.isRunningMacOsX()== true) {
	...
	public List<BenchmarkItem> getBenchmarkItems() {
		ArrayList<BenchmarkItem> items = new ArrayList<BenchmarkItem>();
		for(int i=0; i<itemListModel.getSize(); i++) {
			items.add((BenchmarkItem)itemListModel.get(i));
		}
		return items;
	}
	...
	protected void addItem() {
		...
		if(itemName != null && modelName != null && itemName.length() > 0 && modelName.length() > 0) {
			// valid element to be added
			PetriNet model = ui.getPetriNets().get(modelName);
			BenchmarkItem item = new BenchmarkItem(itemName, model, modelName);
	...
	protected void removeItem() {
		BenchmarkItem item = (BenchmarkItem)itemList.getSelectedValue();
		if(item != null) {
	...
	protected class BenchmarkItemListRenderer implements ListCellRenderer {
		...
		public Component getListCellRendererComponent(JList list, Object value,
			...
			String name = ((BenchmarkItem)value).getName();
