...
/**
 * Implements the 'Progress' interface of ProM
 * in form of a panel.
 *
...
public class BenchmarkRunPanel extends Progress {
	...
	public void runBenchmark(
			final LogReader referenceLog,
			final PetriNet referenceModel,
		...
		Thread benchmarkThread = new Thread() {
			public void run() {
				performBenchmarkRoutine(referenceLog, referenceModel, items, metrics);
				// notify finish listener
				finishListener.actionPerformed(
						new ActionEvent(this, ActionEvent.ACTION_PERFORMED, "calculation finished"));
			}
		};
		benchmarkThread.start();
	}

	protected void performBenchmarkRoutine(
			LogReader referenceLog,
			PetriNet referenceModel,
		...
		for(BenchmarkMetric metric : metrics) {
			for(BenchmarkItem item : items) {
				isCancelled = false;
				bigLabel.setText("Calculating " + metric.name() + " for " + item.getName());
				bigProgress.setValue(bigProgress.getValue() + 1);
				smallLabel.setText("calculation in progress...");
				smallProgress.setIndeterminate(true);
				double measurement = metric.measure(item.getModel(), item.getLog(), referenceModel, this);
