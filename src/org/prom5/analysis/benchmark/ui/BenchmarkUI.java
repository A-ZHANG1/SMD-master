 ...
 *
 */
package org.prom5.analysis.benchmark.ui;
...
import org.prom5.analysis.benchmark.BenchmarkItem;
import org.prom5.analysis.benchmark.metric.BehavioralAppropriatenessMetric;
import org.prom5.analysis.benchmark.metric.BehavioralPrecisionMetric;
import org.prom5.analysis.benchmark.metric.BehavioralRecallMetric;
import org.prom5.analysis.benchmark.metric.BenchmarkMetric;
import org.prom5.analysis.benchmark.metric.CausalFootprintMetric;
import org.prom5.analysis.benchmark.metric.DuplicatesPrecisionMetric;
import org.prom5.analysis.benchmark.metric.DuplicatesRecallMetric;
import org.prom5.analysis.benchmark.metric.ImprovedContinuousSemanticsFitnessMetric;
import org.prom5.analysis.benchmark.metric.ProperCompletionFitnessMetric;
import org.prom5.analysis.benchmark.metric.StructuralAppropriatenessMetric;
import org.prom5.analysis.benchmark.metric.StructuralPrecisionMetric;
import org.prom5.analysis.benchmark.metric.StructuralRecallMetric;
...
public class BenchmarkUI extends JPanel {
				...
				for(ProvidedObject providedObject : providedObjects) {
					for(Object object : providedObject.getObjects()) {
						if(object instanceof PetriNet) {
							petriNets.put(frame.getTitle() + " - " + providedObject.getName(), (PetriNet)object);
	...
	protected void showConfigurationPanel() {
		if(configurationPanel == null) {
			...
			refPanel.add(packHorizontallyLeftAligned(
					new Component[] {refLogLabel, Box.createHorizontalStrut(20), referenceLogBox}, 0));
			refPanel.add(packHorizontallyLeftAligned(
					new Component[] {refModelLabel, Box.createHorizontalStrut(20), referenceModelBox}, 0));
			// setup right panel
			...
			metricsList.addBenchmarkMetric(new ImprovedContinuousSemanticsFitnessMetric());
			// precision metrics
			metricsList.addBenchmarkMetric(new BehavioralAppropriatenessMetric());
			...
			// structure metrics
			metricsList.addBenchmarkMetric(new StructuralAppropriatenessMetric());
			...
			metricsList.setReferenceLogAvailable(this.getReferenceLogReader() != null);
			metricsList.setReferenceModelAvailable(this.getReferencePetriNet() != null);
			...
			helpHeaderPanel.setLayout(new BoxLayout(helpHeaderPanel, BoxLayout.Y_AXIS));
	...
	public void checkStartEnabled() {
		if(itemList != null && metricsList != null) {
			boolean enabled = (itemList.getBenchmarkItems().size() > 0 
					&& metricsList.getBenchmarkMetrics().size() > 0);
	...
	public void startBenchmark() {
		...
		final List<BenchmarkMetric> benchmarkMetrics = metricsList.getBenchmarkMetrics();
		if(referenceLog != null) {
			// map reference model and all item's models to reference log
			if(referenceModel != null) {
				// TODO: check whether log projection must also be stored for the reference model (not clear how this could be 
				// considered for the metrics, which only take (on mined model projected) log, mined model, and reference model)
				PetriNetResult result = connectModelWithLog(referenceModel, referenceLog);
				referenceModel = result.getPetriNet();
			}
			for(BenchmarkItem item : benchmarkItems) {
				try {
					// also store log reader as the mapping may change the log for this particular combination
					PetriNetResult result = connectModelWithLog(item.getModel(), referenceLog);
					item.setModel(result.getPetriNet());
					item.setLog(result.getLogReader());
				}
				catch (Exception e) {
					Message.add("Copying the log reader for connection with benchmark item failed: " + item.getName());
				}
			}
		}
	...
	public void showResults(List<BenchmarkItem> items, List<BenchmarkMetric> metrics) {
		BenchmarkResultTableModel tableModel = new BenchmarkResultTableModel(items, metrics);
		JTable resultTable = new JTable(tableModel);
		for(int x=1; x<resultTable.getColumnCount(); x++) {
			resultTable.getColumnModel().getColumn(x).setCellRenderer(new BenchmarkResultTableCellRenderer());
		}
		JScrollPane resultScrollPane = new JScrollPane(resultTable);
		//JTabbedPane tabbedPane = new JTabbedPane();
		SlickTabbedPane tabbedPane = new SlickTabbedPane();
		tabbedPane.addTab("Bar profile view", new BenchmarkResultBarView(metrics, items));
		tabbedPane.addTab("Table view", resultScrollPane);
		tabbedPane.addTab("Export CSV", new BenchmarkExportPanel(items, metrics));
		setView(tabbedPane);
	}
	...
	protected PetriNetResult connectModelWithLog(PetriNet model, LogReader log) {
		...
		MainUI.getInstance().connectResultWithLog(result, log, conn, false, true);
		return result;
	}
