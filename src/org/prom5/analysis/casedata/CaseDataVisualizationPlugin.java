package org.prom5.analysis.casedata;
...
import org.prom5.framework.util.NaturalSort;
...
public class CaseDataVisualizationPlugin {
	...
	public JComponent analyse(LogReader log) {
		...
		colorControls.setBorder(BorderFactory.createTitledBorder("Color settings"));
	...
	void update() {
		data.update(
				xAxis.getAttributeName(), xAxis.isDiscreteAttribute(), xAxis.getMin(), xAxis.getMax(), xAxis.getStep(), xAxis.getBuckets(), xAxis.isRelative(), xAxis.getTop(), xAxis.isSortedAlphabetically(),
				yAxis.getAttributeName(), yAxis.isDiscreteAttribute(), yAxis.getMin(), yAxis.getMax(), yAxis.getStep(), yAxis.getBuckets(), yAxis.isRelative(), yAxis.getTop(), yAxis.isSortedAlphabetically());
	...
	private JFreeChart createChart(XYZDataset xyzdataset) {
		...
		SymbolAxis numberaxis = new SymbolAxis(xAxis.getAttributeName(), data.getColumnNames());
        ...
        NumberAxis numberaxis1 = new SymbolAxis(yAxis.getAttributeName(), data.getRowNames());
        ...
        MyPaintScale graypaintscale = new MyPaintScale(min, max, (String) coloring.getSelectedItem(), data);
        MyPaintScale graypaintscale2 = new MyPaintScale(min, max, (String) coloring.getSelectedItem(), data);
        ...
        PaintScaleLegend paintscalelegend = new PaintScaleLegend(graypaintscale2, numberaxis2);
	...
	private XYZDataset createDataset(final VisTableModel data) {
		return new XYZDataset() {
			...
			public Number getX(int i, int j) { return new Double(getXValue(i, j)); }
			public Number getY(int i, int j) { return new Double(getYValue(i, j)); }
			public Number getZ(int i, int j) { return new Double(getZValue(i, j)); }
...
class MyPaintScale  implements PaintScale {
	...
	public MyPaintScale(double lower, double upper, String type, VisTableModel data) {
		this.lower = lower;
		this.upper = upper;
		this.type = type;
		...
		if (type.startsWith("Binned")) {
			...
			bins = new double[256];
			for (int i = 0; i < 256; i++) {
				int index = Math.max(0, Math.min(sortedValues.size(), (int) (((double) i / 256.0) * sortedValues.size())));
	...
	public Paint getPaint(double value) {
		...
		if (type.equals(LINEAR_BLACK_TO_RED)) {
			g = Math.max(0, Math.min(255, (int) ((value - lower) / (upper - lower) * 255.0)));
...
class VisTableModel extends AbstractTableModel {
	...
	public void update(
			String xAttr, boolean xDiscrete, Double xMin, Double xMax, Double xStep, Double xBuckets, boolean xRelative, Double xTop, boolean xSortAlpha, 
			String yAttr, boolean yDiscrete, Double yMin, Double yMax, Double yStep, Double yBuckets, boolean yRelative, Double yTop, boolean ySortAlpha) {
		...
		if (!xDiscrete || !yDiscrete) {
			...
			for (ProcessInstance pi : log.getInstances()) {
				try {
					Double x = Double.parseDouble(pi.getAttributes().get(xAttr));
					if (needXmin) { xMin = xMin == null ? x : Math.min(xMin, x); }
					if (needXmax) { xMax = xMax == null ? x : Math.max(xMax, x); }
				} catch (Throwable t) {}
				try {
					Double y = Double.parseDouble(pi.getAttributes().get(yAttr));
					if (needYmin) { yMin = yMin == null ? y : Math.min(yMin, y); }
					if (needYmax) { yMax = yMax == null ? y : Math.max(yMax, y); }
				} catch (Throwable t) {}
			}
		}
		...
		for (ProcessInstance pi : log.getInstances()) {
			String xKey = getKey(xDiscrete, xMin, xMax, xStep, xBuckets, pi.getAttributes().get(xAttr));
			String yKey = getKey(yDiscrete, yMin, yMax, yStep, yBuckets, pi.getAttributes().get(yAttr));
			
			if (xKey != null) { cols.add(xKey); }
			if (yKey != null) { rows.add(yKey); }
		...
		if (xDiscrete && xTop != null) {
			colNames = colNames.subList(0, Math.min(colNames.size(), xTop.intValue()));
		}
		if (yDiscrete && yTop != null) {
			rowNames = rowNames.subList(0, Math.min(rowNames.size(), yTop.intValue()));
		}
...
//			makeRelative(colNames, rowNames);
		...
		for (String firstKey : first) {
			...
			if (m != null) {
				...
				for (String secondKey : second) {
					...
					if (v != null) {
						double newValue = (total <= 0 ? ZERO : v / total * 100.0);
	...
	private String getKey(boolean discrete, Double min, Double max, Double step, Double buckets, String value) {
		if (discrete) {
			return value;
		} else {
			Double d = null;
			...
			return "" + (min + bucket * range) + " - " + (min + (bucket + 1) * range);
		}
	}
...
class MyCellRenderer extends DefaultTableCellRenderer {
	...
	public Component getTableCellRendererComponent(JTable table, Object value,  boolean isSelected, boolean hasFocus, int row,  int column) {
		...
		int color = Math.max(0, Math.min(0xFF, (int) Math.round(model.getRatio(d) * 0xFF)));
...
class AxisSettings extends JPanel {
	...
	private JRadioButton sortNumerical = new JRadioButton("Sum of attribute values");
	...
	public AxisSettings(final CaseDataVisualizationPlugin parent, final boolean isX, Vector<String> attributes) {
		...
		typePanel.setBorder(BorderFactory.createTitledBorder("Type of attribute"));
		...
		sortPanel.setBorder(BorderFactory.createTitledBorder("Sort " + (isX ? "columns" : "rows") + "  on"));
		
		JPanel attributeListPanel = new JPanel();
		attributeListPanel.add(attributeList);
		attributeListPanel.setBorder(BorderFactory.createTitledBorder("Attribute"));
	...
	private Double getValue(JTextField field) {
		return field.getText() == null || field.getText().equals("") ? null : Double.parseDouble(field.getText());
	}
