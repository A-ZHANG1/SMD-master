...
public class CausalFootprintAnalysisResult extends JPanel implements Provider {
	...
	public CausalFootprintAnalysisResult(final CausalFootprint cs,
		...
		final ContextSelectionPanel csVisualization = new ContextSelectionPanel(cs);
		...
		if (graphVisualization != null) {
			...
			graph.addGrappaListener(new GrappaAdapter() {
				 ...
				 */
				protected JMenuItem getCustomMenu(Subgraph subg, final Element elem, GrappaPoint pt,
						int modifiers,
						GrappaPanel panel) {
					if (elem != null &&
							elem.object != null &&
							cs.getCausalVertex((ModelGraphVertex) elem.object) != null) {
						JMenuItem menu = new JMenuItem("Show Causal Context");
						menu.addActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent e) {
								csVisualization.setSelectedObject(cs.getCausalVertex((
										ModelGraphVertex) elem.object));
							}
						});
						return menu;
					}
			...
			split.add(new JLabel(
					"<html>No model available or <br> visualization was aborted.</html>"),
					JSplitPane.LEFT);
		}
		/*
		   if (csVisualization != null) {
		 split.add(csVisualization, JSplitPane.RIGHT);
		   } else {
		 split.add(new JLabel(
		...
		results.setPreferredSize(new Dimension(300, combo.getPreferredSize().height));
		...
		ok.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (combo.getSelectedIndex() == 0) {
					...
					Message.add("<SingularTrapPatternsFound " + pat.size() + ">", Message.TEST);
				}
				if (combo.getSelectedIndex() == 1) {
					// find generalized Trap Pattern
					pat = findGeneralizedTrapPattern();
					Message.add("<GeneralizedTrapPatternsFound " + pat.size() + ">", Message.TEST);
				}
				if (combo.getSelectedIndex() == 2) {
					// find deadlock Trap Pattern
					pat = findDeadlockPatterns();
					Message.add("<DeadlockTrapPatternsFound " + pat.size() + ">", Message.TEST);
				}
				if (combo.getSelectedIndex() == 3) {
					// find multi term Trap Pattern
					pat = findMultiTermPatterns();
					Message.add("<MultiTermTrapPatternsFound " + pat.size() + ">", Message.TEST);
				}
	...
	public ProvidedObject[] getProvidedObjects() {
		if (original != null) {
			return new ProvidedObject[] {original, new ProvidedObject("Causal Footprint",
					new Object[] {cs})};
		} else {
			return new ProvidedObject[] {new ProvidedObject("Causal Footprint", new Object[] {cs})};
		}
	}
	...
	private void findGeneralizedTrapPattern(ArrayList result, HashSet setSoFar, int index,
			boolean forward) {
		...
		while (it.hasNext()) {
			...
			while (it2.hasNext()) {
				...
				if ((e.destinations.size() == 1) &&
						!edge.destinations.contains(e.destinations.iterator().next())) {
					// {d} = e.destinations
					...
					while (allFromB && it3.hasNext()) {
						...
						allFromB &= cs.containsEdge(B,
								(ModelGraphVertex) e.destinations.iterator().next());
					}
					if (allFromB) {
						// Deadlock found
	...
	private ArrayList findMultiTermPatterns() {
		...
		while (it.hasNext()) {
			...
			while (it2.hasNext()) {
				...
				if ((e.sources.size() == 1) && !edge.sources.contains(e.sources.iterator().next())) {
					// {a} = e.sources
					boolean allToB = true;
					...
					while (allToB && it3.hasNext()) {
						HashSet B = new HashSet(1);
						B.add(it3.next());
						allToB &= cs.containsEdge(
								(ModelGraphVertex) e.sources.iterator().next(), B);
					}
					if (allToB) {
						// Deadlock found
						HashSet result = new HashSet();
...
class ContextSelectionPanel extends JPanel {
	...
	public ContextSelectionPanel(boolean isDoubleBuffered, CausalFootprint footprint) {
	...
	public ContextSelectionPanel(LayoutManager layout, boolean isDoubleBuffered,
	...
	private JComboBox selectionBox = new ToolTipComboBox(new DefaultComboBoxModel());
