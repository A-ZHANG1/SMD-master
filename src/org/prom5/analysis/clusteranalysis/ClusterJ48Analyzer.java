 ...
 * modified from Anne's decision point analysis class
 *
...
public class ClusterJ48Analyzer extends DecisionAnalyzer {
	...
	public ClusterJ48Analyzer() {
		...
		unprunedTree = new GUIPropertyBoolean("Use unpruned tree",
					   ((J48) myClassifier).unprunedTipText(), ((J48) myClassifier).getUnpruned());
		...
		minNoInstances = new GUIPropertyInteger("Minimun number of instances in any leaf",
						 ((J48) myClassifier).minNumObjTipText(), ((J48) myClassifier).getMinNumObj(),
						 0, 1000);
		...
		numberFolds = new GUIPropertyInteger("Number of folds for reduced-error pruning",
					  ((J48) myClassifier).numFoldsTipText(), ((J48) myClassifier).getNumFolds(), 1,
					  100);
	...
	protected JPanel createResultVisualization() {
		...
		try {
			...
			((J48ResultPanel) resultViewPanel).setTreeVisualizer(new TreeVisualizer(null,
					((J48) myClassifier).graph(), new PlaceNode2()));
	...
	private class J48ResultPanel extends JPanel {
		...
		private TreeVisualizer tv; /** The decision tree visualization */
