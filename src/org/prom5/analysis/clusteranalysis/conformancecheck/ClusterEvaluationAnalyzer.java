...
import org.prom5.analysis.traceclustering.distance.sequence.GZipCompressionDistance;
import org.prom5.analysis.traceclustering.distance.sequence.LcsDistance;
import org.prom5.analysis.traceclustering.distance.sequence.LevenshteinEditDistance;
import org.prom5.analysis.traceclustering.distance.sequence.StringDistanceMetric;
...
import org.prom5.analysis.traceclustering.model.InstancePoint;
...
import org.prom5.mining.regionmining.RegionMiner;
...
//	private static final String ALPHA = "Alpha++ Miner";
	...
	protected ProgressPanel progressPanel = new ProgressPanel("Evaluate clustering results");
...
//		startAnalysistemp();
	...
	protected void initGUI()
	{
		...
		distanceMetricsBox = new GUIPropertyListEnumeration("Chart", "", distanceMetrics, null, 200);
...
//		minerList.add(ALPHA);
		...
		miningBox = new GUIPropertyListEnumeration("Mining ", "", minerList, null, 200);
		...
		for(Cluster cluster : clusters.getClusters()) {
			try {
				...
				if(miningBox.getValue().equals(HEUMINER)) {
					scrollPane = new JScrollPane(((HeuristicsNetResult)result.get(i)).getVisualization());
				} else {
					scrollPane = new JScrollPane(((PetriNetResult)result.get(i)).getVisualization());
				}
	...
	{   
		...
		Thread buildProfileThread = new Thread() {
			public void run() {
				...
				watch.stop();
				Message.add("Calcuate conformance values" + watch.formatDuration());
				updateGUI();
			}
		};
		...
		for(Cluster cluster : clusters.getClusters()) {
			try {
				if(miningBox.getValue().equals(PETRI)) {
					...
					result.add((PetriNetResult) miningPlugin.mine(cluster.getFilteredLog()));
...
//					result.add((PetriNetResult) miningPlugin.mine(cluster.getFilteredLog()));
					...
					result.add((HeuristicsNetResult)miningPlugin.mine(cluster.getFilteredLog()));
				}
			} catch(Exception e) {
				e.printStackTrace();
				continue;
	...
	private String getHtmlRepresentationFitness() {
		...
		for(Cluster cluster : clusters.getClusters()) {
			...
			try {
				double fitness=0.0;
				if(miningBox.getValue().equals(HEUMINER)) {
					HeuristicsNet net  = ((HeuristicsNetResult)result.get(i)).getHeuriticsNet();
					Fitness hnfitness = FitnessFactory.getFitness(0, cluster.getFilteredLog(),null);
					fitness = hnfitness.calculate(new HeuristicsNet[] {net})[0].getFitness();
				} else {
					PetriNet petriNet = ((PetriNetResult)result.get(i)).getPetriNet();
					TokenFitnessMetric tm = new TokenFitnessMetric();
					fitness = tm.measure(petriNet, cluster.getFilteredLog(), null, new ProgressDummy());
				}
		...
		sb.append("</table>");
		sb.append("average = "+String.valueOf(sum/clusters.getLog().numberOfInstances()));
		sb.append("</body></html>");
		return sb.toString();
	}
	...
	private String getHtmlRepresentationDistance() {		
		...
		sb.append("</table>");
		sb.append("average = "+String.valueOf(sum/(clusters.size()*(clusters.size()-1))));
		sb.append("<br>");
//		sb.append(getHtmlRepresentationDistanceInstance());
	...
	private String getHtmlRepresentationDistanceInstance() {		
		...
		if(metric instanceof StringDistanceMetric) {
			dm = ((StringDistanceMetric)metric).getDistanceMatrix(clusters.getLog());
		...
		for(int k=0; k < clusters.getLog().numberOfInstances(); k++) {
			tableHeader.append(tag(clusters.getLog().getInstance(k).getName(), TH));
		}
		...
		for(int k=0; k < clusters.getLog().numberOfInstances(); k++) {
			StringBuffer tempBuffer = new StringBuffer();
			tempBuffer.append(tag(tag(clusters.getLog().getInstance(k).getName(),BR),TD));
	...
	protected DoubleMatrix2D calculateInterClusterDistanceCentroid_()
	{
		...
		for(int i=0;i<clusters.size();i++){
			...
			if (tempCluster.size()==0) continue;
			instancePoints[i] = new InstancePoint();
			...
			while(itr.hasNext())
			{
				...
				while(itr2.hasNext())
				{
					...
					if(instancePoints[i].getItemKeys().contains(key))
						instancePoints[i].set(key,instancePoints[i].get(key)+iPoint.get(key));
					else
			...
			while(itr.hasNext())
			{
				...
				instancePoints[i].set(key,instancePoints[i].get(key)/tempCluster.size());
			}
			progressPanel.setProgress(progressPanel.getValue()+1);
		}
		
		DoubleMatrix2D D = DoubleFactory2D.dense.make(instancePoints.length, instancePoints.length, 0);
		for (int i = 0; i < instancePoints.length - 1; i++) {
			for (int j = i + 1; j < instancePoints.length; j++) {
				double temp = metric.getDistance(instancePoints[i],instancePoints[j]);
				D.set(i, j, temp);
				D.set(j, i, temp);
			}
		}
	...
	{
		DistanceMatrix dm = getDistanceMatrix();
		DoubleMatrix2D D = DoubleFactory2D.dense.make(clusters.size(), clusters.size(), 0);
		...
		for(int i=0;i<clusters.size()-1;i++){
			...
			for(int j=i+1;j<clusters.size();j++){
				...
				double temp = 0.0;
				double divider = 0.0;
				Iterator<Integer> itr = tempCluster.getTraceIndices().iterator();
				while(itr.hasNext())
				{
					int inst1 = itr.next();
					Iterator<Integer> itr2 = tempCluster2.getTraceIndices().iterator();
					while(itr2.hasNext())
					{
						int inst2 = itr2.next();
	...
	protected double[] calculateIntraClusterDistance()
	{
		...
		for(int i=0;i<clusters.size();i++){
			...
			for(int j=0;j< tempCluster.getTraceIndices().size()-1;j++)
			{
				...
				{
					temp += dm.get(tempCluster.getTraceIndices().get(j), tempCluster.getTraceIndices().get(k));
					divider++;
				}
			}
			divider = Math.max(divider,1.0);
	...
	public DistanceMatrix getDistanceMatrix() {
		...
		if(metric instanceof StringDistanceMetric) {
			DistanceMatrix dm = ((StringDistanceMetric)metric).getDistanceMatrix(clusters.getLog());
			dm.normalizeToMaximum(1.0);
			return dm;
		}
