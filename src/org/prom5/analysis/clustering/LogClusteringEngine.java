...
import org.prom5.analysis.clustering.algorithm.AHC;
...
public class LogClusteringEngine {
	...
	public void calculateSim(double alpha, int tranType, int measureType, int freqOption) {
...
//			nodeList[i]=nodeList[i]+" (n="+logS.getFrequencyOfProcSequence(simC.proc[i])+")";
		...
		return getGraphPanel(comp, sim, nodeList, makeClusteringMatrix2D(leng, zeroValueCount),
				leng);
	}
	...
	public DoubleMatrix2D makeClusteringMatrix2D(int leng, int zeroValueCount) {
		...
		DoubleMatrix2D matrix = DoubleFactory2D.sparse.make(matrixSize, matrixSize, 0);
	...
	public JPanel getGraphPanel(int comp, double sim, String[] nodeList, DoubleMatrix2D matrix,
			int procCount) {
		BufferedWriter bw;
		Process dot;
		Parser parser;
		...
		try {
			...
			bw.write("node [height=\".1\",width=\".2\",fontname=\"Arial\",fontsize=\"14\"];\n");
			Vector v = new Vector();
			...
			for (int i = 0; i < v.size(); i++) {
				//bw.write("t" + i + " [layer=\"1\",shape=\"box\",label=\"" + users[i] + "\"];\n");
				bw.write("" + v.get(i) + " [layer=\"1\",shape=\"box\",label=\"" + v.get(i) +
						"\"];\n");
			}
...
//                      create the visual component and return it
	...
	public JScrollPane getProcTablePane() {
		...
		JTable table = new JTable(logS.getProcSeqeunceTable(), // Table Value
					   new String[] {"id", "log sequences", "frequency", "process instances"});
