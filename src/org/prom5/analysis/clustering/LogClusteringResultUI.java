 ...
 * @version 1.0
 */
public class LogClusteringResultUI extends JPanel implements ActionListener, Provider {
	...
	public LogClusteringResultUI(MiningResult result, LogClusteringEngine engine, int comparison,
			double defaultSim) {
	...
	private void jbInit() throws Exception {
		...
		splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, leftScrPane, rightScrPane);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == jbPreview) {
			...
			/*			rightScrPane = null;
			   piListPanel = new JPanel(new BorderLayout());
	...
	public void visualizePIListPanel() {
		JInternalFrame[] iframe = MainUI.getInstance().getDesktop().getAllFrames();
		...
		SwingWorker worker = new SwingWorker() {
			...
			public void finished() {
				if (result != null) {
					...
					piListPanel.add(new JScrollPane(jTable1), BorderLayout.CENTER);
