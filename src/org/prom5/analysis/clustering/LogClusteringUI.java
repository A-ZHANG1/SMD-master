 ...
 * @version 1.0
 */
public class LogClusteringUI extends JPanel implements ActionListener, MiningResult, Provider {
...
//To
	...
	private JSplitPane splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, leftScrPane,
								   rightScrPane);
	...
	private void jbInit() throws Exception {
		...
		selectAlgorithmPanelGrid.setBorder(new TitledBorder("Clustering algorithm"));
		selectFrequencyPanelGrid.setBorder(new TitledBorder("Frequency weights"));
		...
		//selectSimTypePanel.add(new JLabel("Similarity type"), BorderLayout.NORTH);
		...
		//diagramPanel.add(new JLabel("Please select a clustering algorithm and press \"Start clustering\""), BorderLayout.CENTER);
	...
	public void initialize() {
		...
		//font setting
		/*		actTypeButton.setFont(f12);
		  tranTypeButton.setFont(f12);
		  totalTypeButton.setFont(f12);

		  jaccardButton.setFont(f12);
		  cosineButton.setFont(f12);

		  KMeansButton.setFont(f12);
		  AHCButton.setFont(f12);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == actTypeButton) {
			...
			double alpha = getSimTypeRatio(); //if 0, tranSim; if 1, actSim; otherwise, total
			...
			/*			JFrame f = new JFrame("Clustering Result : "+getSettingString(alpha, tranType, measureType, freqOption));
			   f.setSize(600, 400);
			   f.setVisible(true);
			   f.getContentPane().add(diagramPanel, BorderLayout.CENTER);

			   f.validate();
			   f.repaint();


			 String tmp = "Clustering Result : "+getSettingString(ratio, tranType, measureType, freqOption));
			   LogClusteringResult cResult = new LogClusteringResult(tmp);
			 */

			LogClusteringResultUI f2 = new LogClusteringResultUI(null, engine,
									   compCombo.getSelectedIndex(),
									   Double.valueOf(compSimText.getText()));
			f2.setSize(600, 400);
			f2.setVisible(true);
			...
			MainUI.getInstance().createFrame(" Result - Log Clustering :: " +
					getSettingString(alpha, tranType, measureType, freqOption),
					f2);
	...
	public String getSettingString(double ratio, int tranType, int measureType, int freqOption) {
		String str = "";
		if (ratio == 0) {
	...
	/*
	 public String getSummaryString(){
	  String str = "";
	  str = str + "Activity Similarity = "+dformat3.format(engine.getMeanActSimilarityOfProcLog());
	 str = str + "<br>Transition Similarity = "+dformat3.format(engine.getMeanTranSimilarityOfProcLog());
	  str = str + "<br>Complexity = "+dformat3.format(0.0)+"<br>";
	  return str;
	 }
	 */
	public JScrollPane getSimTablePane() {
		...
		//String[][] str = {{"activity", "0.7"}, {"explicit transition","0.2"}, {"explicit transition","0.2"}};
...
//Loading time saving
		/*		table.setValueAt(""+dformat3.format(simC.getMeanActSimilarity()), 0, 1);
		  table.setValueAt(""+dformat3.format(simC.getMeanExpTranSimilarity()), 1, 1);
		  table.setValueAt(""+dformat3.format(simC.getMeanImpTranSimilarity()), 2, 1);
