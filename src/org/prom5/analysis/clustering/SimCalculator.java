package org.prom5.analysis.clustering;
...
public class SimCalculator {
	...
	//Clutering Options
	double alpha = 0.0; //alpha is the weight of activity. i.e. if 0, totalSim=transSim. If 1, totalSim=actSim.
	...
	public SimCalculator(LogSequence logS) { //default alpha=0(tran), tranType=1(imp), simMeasure=1(cosine), int freqOption

		this(logS, 0, 1, 1, 0);
	}

	public SimCalculator(LogSequence logS, double alpha, int tranType, int simMeasure,
			int freqOption) {
		...
		if (freqOption == 1) {
			calculateActInversePFWeight(); //count the number of processes with each activity
			calculateTranInversePFWeight(); //count the number of processes with each activity
		}
	...
	public void calSimProc() {
		...
		for (int i = 0; i < (numOfProcess - 1); i++) {
			for (int j = i + 1; j < numOfProcess; j++) {
				if (alpha == 1) {
					...
					System.out.println("actSimilarity[" + i + "][" + j + "]=" + actSimilarity[i][j]);
					inter_connectivity_act += proc_freq[i] * proc_freq[j] * actSimilarity[i][j];
				} else if (alpha == 0) {
					...
					System.out.println("tranSimilarity[" + i + "][" + j + "]=" +
							tranSimilarity[i][j]);
					inter_connectivity_tran += proc_freq[i] * proc_freq[j] * tranSimilarity[i][j];
				} else {
					...
					totalSimilarity[i][j] = actSimilarity[i][j] * alpha + tranSimilarity[i][j] * (1 -
											alpha);
		...
		if (numOfProcess > 1) {
			meanActSimilarity = (self_connectivity +
								inter_connectivity_act) / (totalNumOfProcess * (totalNumOfProcess -
								1) / 2);
			meanImpTranSimilarity = (self_connectivity +
									inter_connectivity_tran) / (totalNumOfProcess *
									(totalNumOfProcess - 1) / 2);
		}
	...
	private double calculateActivitySimilarity(int indexProc1, int indexProc2) {
		...
		for (int i = 0; i < pro1.length(); i++) {
			for (int j = 0; j < pro2.length(); j++) {
				...
				if (pro1.charAt(i) == pro2.charAt(j)) {
					...
					if (freqOption == 1) { //in fact, pro1.charAt(i) equals pro2.char(j)
						weight1 = ((Double) actIPFWeight.get(new Character(pro1.charAt(i)))).
								  doubleValue();
						weight2 = ((Double) actIPFWeight.get(new Character(pro2.charAt(j)))).
								  doubleValue();
					}
		...
		if (simMeasure == 0) { //"Jaccard Coefficient"
			...
			} else { //inverse process frequency  (sum of weights = actWeightSumOfProcess[indexProc1])
				lower = actWeightSumOfProcess[indexProc1] + actWeightSumOfProcess[indexProc2] -
						upper;
			}
		} else if (simMeasure == 1) { //"Cosine Function"
			if (freqOption == 0) {
				lower = Math.pow(pro1.length(), 0.5) * Math.pow(pro2.length(), 0.5);
			} else { //inverse process frequency (squared sum of weights = actWeightSumOfProcess[indexProc1])
				lower = Math.pow(actWeightSumOfProcess[indexProc1],
						0.5) * Math.pow(actWeightSumOfProcess[indexProc2], 0.5);
			}
		}
	...
	private double calculateTransitionSimilarity(int indexProc1, int indexProc2) {
		...
		double[][] pro1TranVal_Exp = new double[pro1.length()][pro1.length()]; //Explicit Transition Value
		...
		double[][] pro1TranVal_Imp = new double[pro1.length()][pro1.length()]; //Implicit Transition Value
		...
		if (tranType == 0) { //exp transition
			...
			for (int i = 0; i < pro1.length() - 1; i++) {
				...
				if (simMeasure == 1) { //cosine measure
					sumOfPro1TranValSquare_Exp += Math.pow(pro1TranVal_Exp[i][i + 1], 2);
			...
			for (int i = 0; i < pro2.length() - 1; i++) {
				...
				if (simMeasure == 1) { //cosine measure
					sumOfPro2TranValSquare_Exp += Math.pow(pro2TranVal_Exp[i][i + 1], 2);
				} else if (simMeasure == 0) { //jaccard measure
			...
			for (int i = 0; i < pro1.length() - 1; i++) {
				...
				for (int m = 0; m < pro2.length() - 1; m++) {
					...
					if (pro1Tran[i][i + 1].equals(pro2Tran[m][m + 1])) {
						...
						if (freqOption == 1) { //in fact, two value below are the same..
							weight1 *= ((Double) tranIPFWeight.get(pro1Tran[i][i + 1])).doubleValue();
							weight2 *= ((Double) tranIPFWeight.get(pro2Tran[m][m + 1])).doubleValue();
						}

						if (simMeasure == 1) { //"Cosine Coefficient"
							upper_Exp += pro1TranVal_Exp[i][i + 1] * pro2TranVal_Exp[m][m +
									1] * weight1 * weight2;
						} else if (simMeasure == 0) { //"Jaccard Coefficient"
			...
			for (int i = 0; i < pro1.length(); i++) {
				for (int j = i + 1; j < pro1.length(); j++) {
					...
					if (simMeasure == 1) { //cosine measure
						sumOfPro1TranValSquare_Imp += Math.pow(pro1TranVal_Imp[i][j], 2);
					} else if (simMeasure == 0) { //jaccard measure
			...
			for (int i = 0; i < pro2.length(); i++) {
				for (int j = i + 1; j < pro2.length(); j++) {
					...
					if (simMeasure == 1) { //cosine measure
						sumOfPro2TranValSquare_Imp += Math.pow(pro2TranVal_Imp[i][j], 2);
					} else if (simMeasure == 0) { //jaccard measure
					...
					for (int m = 0; m < pro2.length(); m++) {
						for (int n = m + 1; n < pro2.length(); n++) {
							...
							if (pro1Tran[i][j].equals(pro2Tran[m][n])) {
								...
								double weight2 = 1; //weight2 = af(pro2)
								if (freqOption == 1) { //in fact, two value below are the same..
									weight1 *= ((Double) tranIPFWeight.get(pro1Tran[i][j])).
											doubleValue();
									weight2 *= ((Double) tranIPFWeight.get(pro2Tran[m][n])).
											doubleValue();
								}

								if (simMeasure == 1) { //"Cosine Coefficient"
									upper_Imp += pro1TranVal_Imp[i][j] * pro2TranVal_Imp[m][n] *
											weight1 * weight2;
								} else if (simMeasure == 0) { //"Jaccard Coefficient"
									upper_Imp += (pro1TranVal_Imp[i][j] * weight1 +
											pro2TranVal_Imp[m][n] * weight2) / 2;
								}
		...
		if (simMeasure == 1) { //"Cosine Coefficient"
			if (freqOption == 0) {
				if (tranType == 0) {
					lower_Exp = Math.pow(sumOfPro1TranValSquare_Exp,
								0.5) * Math.pow(sumOfPro2TranValSquare_Exp, 0.5);
				} else {
					lower_Imp = Math.pow(sumOfPro1TranValSquare_Imp,
								0.5) * Math.pow(sumOfPro2TranValSquare_Imp, 0.5);
				}
			} else { //inverse process frequency  (sum of weights = actWeightSumOfProcess[indexProc1])
				if (tranType == 0) {
					lower_Exp = Math.pow(tranWeightSumOfProcess[indexProc1],
								0.5) * Math.pow(tranWeightSumOfProcess[indexProc2], 0.5);
				} else {
			...
			if (freqOption == 0) {
				if (tranType == 0) {
					lower_Exp = sumOfPro1TranValSquare_Exp + sumOfPro2TranValSquare_Exp - upper_Exp;
				} else {
					lower_Imp = sumOfPro1TranValSquare_Imp + sumOfPro2TranValSquare_Imp - upper_Imp;
				}
			} else { //inverse process frequency  (sum of weights = actWeightSumOfProcess[indexProc1])
				if (tranType == 0) {
	...
	private void calculateActInversePFWeight() { //calculate the activity Weight Sum of each Process
		...
		for (int i = 0; i < numOfProcess; i++) {
			...
			for (int j = 0; j < proc[i].length(); j++) {
				...
				/*				System.out.println("~~~~~~~~~~~~~~ "+i+" ~~~ "+j+" ~~~"+ch.toString());
					//if proc has more than one activity
					c = (Integer) tmpMemory.get(ch);
					if(c==null) tmpMemory.put(c, new Integer(1));
					else break;
				 */
		...
		if (simMeasure == 0) { //Jaccard coefficient
			for (int i = 0; i < numOfProcess; i++) {
				...
				for (int j = 0; j < proc[i].length(); j++) {
					...
					Double d = (Double) actIPFWeight.get(new Character(proc[i].charAt(j)));
					sumOfProcess += d.doubleValue(); //jaccard -> sum of weights
				}
	...
	private void calculateTranInversePFWeight() { //calculate the transition Weight Sum of each Process
			...
			for (int j = 0; j < proc[i].length(); j++) {
				for (int k = j + 1; k < proc[i].length(); k++) {
					...
					if (c == null) {
						...
						tranProcessFrequency.put(str, new Integer(c.intValue() + 1));
					}
				}
			}
		}
		...
		//in case of Jaccard, calculate Sum of weights for each process
		//in case of Cosine, calculate Root of Squared Sum of weights for each process
			...
			for (int i = 0; i < numOfProcess; i++) {
				...
				for (int j = 0; j < proc[i].length(); j++) {
					for (int k = j + 1; k < proc[i].length(); k++) {
						...
						Double d = (Double) tranIPFWeight.get("" + proc[i].charAt(j) +
								   proc[i].charAt(k));
						sumOfProcess += d.doubleValue(); //jaccard -> sum of weights
					}
				}
	 ...
	 public double calculateIntraVar(){
	  ...
	  for(int i=0; i<numOfProcess; i++) {
	   ...
	   for(int j=0; j<numOfProcess; j++) {
		if(i==j) continue;
		tmpVar += Math.pow( 1 - totalSimilarity[i][j] , 2);
	   }
	   tmpVar /= numOfProcess-1 ;

	   intraVar += tmpVar;
	  }
	  intraVar /= numOfProcess;
