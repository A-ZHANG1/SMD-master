package org.prom5.analysis.clustering.algorithm;

import java.util.TreeMap;
import java.util.Vector;

import org.prom5.analysis.clustering.SimCalculator;
...
public class AHC {
	...
	private double findNearestCluster() {
			...
			for (int i = 0; i < numOfProcess - 1; i++) {
				for (int j = i + 1; j < numOfProcess; j++) {
					...
					if (meanSim > maxMeanSim) {
						maxMeanSim = meanSim; // + 0.00000000001;   //Orderig Tuning Tip
						cluster1 = i;
						cluster2 = j;
					}
				}
				//System.out.println("maxMeanSim=" + maxMeanSim + " : meanSim=" + meanSim);
			}
		} else {
			for (int i = 0; i < numOfProcess - 1; i++) {
		...
		//cluster 2 => cluster 1
		System.out.println("Cluster " + cluster2 + "->" + cluster1 + " : " + maxMeanSim);
		...
		Message.add("The number of clusters = " + (numOfProcess - clusteringCount));
		...
		for (int j = 0; j < numOfProcess; j++) {
			if (j < cluster1) {
				simOfClusters[j][cluster1] = (n1 * simOfClusters[j][cluster1] +
											 n2 * simOfClusters[j][cluster2]) / (n1 + n2);
			} else if (j < cluster2) {
				simOfClusters[cluster1][j] = (n1 * simOfClusters[cluster1][j] +
											 n2 * simOfClusters[j][cluster2]) / (n1 + n2);
			} else {
				simOfClusters[cluster1][j] = (n1 * simOfClusters[cluster1][j] +
											 n2 * simOfClusters[cluster2][j]) / (n1 + n2);
			}
		}
	...
	/*
	 private double calculateSimilarityOfTwoCluster(int cluster1, int cluster2) {
	  ...
	  double meanSim;
	  if (count==0) meanSim = 0;
	  else meanSim = sumSim / count;
	...
	public void AHCAlgorithm() {
		...
		for (int i = 1; i < numOfProcess - K; i++) {
			...
			//System.out.println("-------------->("+i+") maxMeanSim = " + maxMeanSim);
