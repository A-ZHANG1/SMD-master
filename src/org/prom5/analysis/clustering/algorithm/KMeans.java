 ...
 */
/*K-means �˰?��*/
/*select K centroid from N process
 for(no change) {
  for (N process){
   calculate mean similarities b/w the process and K clusters
   cluster the process to the most similar cluster
  }
 }
 */
	...
	public void kmeansAlgorithm() {
		...
		do {
			...
			for (int i = 0; i < numOfProcess; i++) {
				...
				belongedCluster[i] = mergeToCluster(i); //cluster the process to most similar cluster
