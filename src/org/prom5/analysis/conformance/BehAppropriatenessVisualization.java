...
public class BehAppropriatenessVisualization extends DiagnosticPetriNet {
	...
	/**
	 * Creates a diagnostic Petri net with successorship relation visualization. The copy
	...
	/**
	 * Adds diagnostic edges to the model view (indicating the global successorship and predecessorship
	 * relations).
	 */
	protected void finishDotWriting(Writer bw) throws IOException {
		...
		if (alwaysFollowsOption == false && neverFollowsOption == false &&
				alwaysPrecedesOption == false && neverPrecedesOption == false) {
			// use default
			...
			try {
				...
				while (it.hasNext()) {
					...
					DiagnosticTransition currentTrans = (DiagnosticTransition) it.next();
					...
					if (alwaysFollowsOption == true &&
						...
						Iterator<Transition>
								alwaysFollower = currentTrans.getAlwaysFollows().iterator();
						while (alwaysFollower.hasNext()) {
							Transition current = alwaysFollower.next();
							bw.write("t" + currentTrans.getNumber() + " -> t" + current.getNumber() +
					...
					if (neverFollowsOption == true &&
						...
						Iterator<Transition>
								neverFollower = currentTrans.getNeverFollows().iterator();
					...
					if (alwaysPrecedesOption == true &&
						...
						Iterator<Transition>
								alwaysPrecedent = currentTrans.getAlwaysPrecedes().iterator();
					...
					if (neverPrecedesOption == true &&
						...
						Iterator<Transition>
								neverPrecedent = currentTrans.getNeverPrecedes().iterator();
