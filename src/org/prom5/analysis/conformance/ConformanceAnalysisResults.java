...
public class ConformanceAnalysisResults extends JPanel implements Provider,
	...
	private JButton selectFittingInstancesButton = new JButton(" Select Fitting ");
	...
	private JLabel percentageSelectedInstancesLabel = new JLabel("   Selected Instances in % ");
	private SpinnerNumberModel model = new SpinnerNumberModel(100 /*initial value*/, 1 /*min*/,
									   100
									   /*max*/, 1 /*step*/);
	 ...
	 * @param analysisOptions the configuration options
	 * @param depthValue the user-specified value for depth restriction during log replay (-1 means "no restriction")
	 */
	public ConformanceAnalysisResults(AnalysisConfiguration analysisOptions, int depthValue) {
		//	ConformanceLogReplayResult logReplayResult) {
		userOptions = analysisOptions;
		Iterator<AnalysisResult> allResults = userOptions.getChildrenResultObjects().iterator();
		...
		if (replayResult != null) {
			...
			//diagnosticPetriNet = (DiagnosticPetriNet) logReplayResult.replayedPetriNet;
			inputLog = replayResult.inputLogReader;
			inputPetriNet = replayResult.inputPetriNet;
			// keep the fitting traces from the first analysis (since subsequently
			// only a subset of the log might be analysed - containing only a subset of
	 ...
	 */
	private void jbInit() {
		percentageSelectedInstancesLabel.setForeground(new Color(100, 100, 100));

		updateGraphButton.setToolTipText(
				"Show all the results only for the process instances currently selected.");
		selectFittingInstancesButton.setToolTipText(
				"Select all instances that are 100% compliant with the model.");
		invertSelectionButton.setToolTipText("Invert the current selection status.");
		percentageSelectedInstancesLabel.setToolTipText(
				"<HTML>Percentage of process instances covered by the latest evaluation.<BR>" +
				"Adjusting the spinner will select the most frequent traces in the log.</HTML>");

		// user may have aborted the log replay
		if (replayResult != null) {
			processInstanceIDsTable = new DoubleClickTable(new DiagnosticLogTable(),
									  updateGraphButton);
		...
		ArrayList<AnalysisConfiguration> categories = userOptions.getChildConfigurations();
		...
		while (it.hasNext()) {
			...
			if (currentCategory.isSelected()) {
				...
				myTabbedPane.addTab(currentCategory.getName(), null, analysisGUI,
						currentCategory.getToolTip());

				/// TODO: find better solution for storing the analysis GUI objects (needed in order to retrieve
	 ...
	 * @return int[] an array of indeces indicating those instances that are currently selected
	 */
	...
	private ArrayList getSelectedInstanceIDs() {
		...
		for (int i = 0; i < indexArray.length; i++) {
			selectedInstanceIDs.add(diagnosticLog.getLogTraceIDs().get(indexArray[i]));
		}
	...
	private ArrayList getSelectedInstances() {
		...
		int[] indexArray = getSelectionStatus();
		for (int i = 0; i < indexArray.length; i++) {
			selectedInstances.add(diagnosticLog.getDiagnosticLogTraces().get(indexArray[i]));
		}
	 ...
	 * It is, e.g., called when the spinner state changes, adapting the selection
	 * status of the table to the most frequent traces determined by the
	 * given number.
	 * @param newSelection a list of Strings containing the IDs of those traces that
	 * should be selected.
	 */
	...
	/**
	 * Updates both the visualization and the conformance measures by taking only
	 * the selected instances into account. If all instances have been selected, the
	 * result corresponds to the initial state.
	 */
	private void updateResults() {
		try {
			...
			if (selectionModel.isSelectionEmpty()) {
				selectionModel.addSelectionInterval(0, diagnosticLog.getSizeOfLog() - 1);
			}
			...
			// update the percentage of selected process instances displayed in the spinner
			int selectedPercentage = diagnosticLog.getPercentage(selectedInstanceIDs);
			model.setValue(new Integer(selectedPercentage));
			/**@todo anne: Try to display the percentage without triggering automatic selection
			 * of the most frequent instances (and then this updateSelection could be omitted)*/
			// reset selection status as it was automatically adapted (workaround)
			spinnerListener.updateSelection(selectedInstanceIDs);

			// create new log reader from selection
			LogReader updatedLog = LogReaderFactory.createInstance(inputLog, getSelectionStatus());
			LogReplayAnalysisMethod newLogReplay = new LogReplayAnalysisMethod(inputPetriNet, updatedLog, new ConformanceMeasurer(), new Progress(""));
			// make sure repeated log replay is executed according to initial user settings
			newLogReplay.setMaxDepth(logReplayDepth);
			AnalysisMethodExecutionThread worker = new AnalysisMethodExecutionThread(newLogReplay, this);
			try {
				worker.start();
			} catch (OutOfMemoryError err) {
				/**
				 * @todo anne: check whether this works +
				 * deselect category (for results frame) since it could not be successfully computed
				 */
	...
	/*
	 *  (non-Javadoc)
	 * @see org.processmining.analysis.conformance.ThreadNotificationTarget#getAnalysisConfiguration()
	 */
	...
	/*
	 *  (non-Javadoc)
	 * @see org.processmining.analysis.conformance.ThreadNotificationTarget#threadDone(org.processmining.analysis.conformance.AnalysisMethodExecutionThread)
	 */
	public void threadDone(AnalysisMethodExecutionThread thread) {
		...
		Iterator<AnalysisConfiguration>
				tabCategories = getAnalysisConfiguration().getChildConfigurations().iterator();
		while (tabCategories.hasNext()) {
			// TODO - think about how this can be done without this string comparison
			...
			if (current.getName() == "Fitness" && current.isSelected()) {
				...
				boolean failedTransitionsOption = fitGUI.getFailedTransitionsOption();
				boolean remainingTransitionsOption = fitGUI.getRemainingTransitionsOption();
				...
				boolean visualizationTypeOption = fitGUI.getVisualizationTypeOption();

				Set<AnalysisResult> fitnessResultObjects = current.getResultObjects();
				current.setResultPanel(new FitnessAnalysisGUI(fitnessResultObjects,
						tokenCounterOption, failedTransitionsOption, remainingTransitionsOption,
						pathCoverageOption, passedEdgesOption, failedEventsOption,
			...
			if (current.getName() == "Precision" && current.isSelected()) {
				...
				Set<AnalysisResult> behApprResultObjects = current.getResultObjects();
				current.setResultPanel(new BehAppropriatenessAnalysisGUI(behApprResultObjects,
						alwaysPrecedes, neverPrecedes, alwaysFollows, neverFollows));
			}
			// don't rebuild structural appropriateness tab as
			// only update those GUIs that were affected by log replay need to be reconstructed
		}
	 ...
	 * e.g., export an item.
	 * @return the provided objects offered by the {@link AnalysisGUI AnalysisGUI}
	...
	public ProvidedObject[] getProvidedObjects() {
		try {
			...
			objects.add(new ProvidedObject("Initial Petri net", new Object[] {inputPetriNet}));
			// offer input log reader
			objects.add(new ProvidedObject("Whole Log", new Object[] {inputLog}));
			...
			if (processInstanceIDsTable != null &&
				...
				objects.add(new ProvidedObject("Log Selection", new Object[] {
						LogReaderFactory.createInstance(inputLog, getSelectionStatus())}));
			}
			// offer diagnostic visualizations offered by the various analysis tabs
			AnalysisGUI currentTab = (AnalysisGUI) myTabbedPane.getSelectedComponent();
			// get the provided objects from the currently selected tab component
	...
	/**
	 * Change listener for the spinner representing the current
	 * percentage of selected process instances.
	 */
	public class SpinnerListener implements ChangeListener {
		...
		public void stateChanged(ChangeEvent e) {
			ArrayList newSelection = diagnosticLog.getMostFrequentTraces(model.getNumber().intValue());
			updateSelection(newSelection);
		}
	...
	private class DiagnosticLogTable extends AbstractTableModel {
		...
		public Object getValueAt(int row, int column) {
			...
			if (column == 0) {
				...
				DiagnosticLogTrace trace = (DiagnosticLogTrace) diagnosticLog.getLogTrace((String)
										   diagnosticLog.getLogTraceIDs().get(row));
