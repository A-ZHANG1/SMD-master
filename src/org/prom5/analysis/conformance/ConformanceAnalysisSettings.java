...
import org.prom5.framework.models.hlprocess.gui.HLProcessGui;
...
import org.prom5.framework.models.petrinet.algorithms.logReplay.AnalysisMethod;
 ...
 * The user are offered certain analysis options, which are specified in a logical way
 * using a hierarchical {@link AnalysisConfiguration AnalysisConfiguration} object.
 * The analysis categories themselves are associated to {@link AnalysisMethod AnalysisMethod}
 * objects, which are an orthorgonal concept reflecting the kind of technique that is
 * used to collect the data needed for a specific metric.
 * The actual analysis will only be carried out for those methods that were selected by
 * the user.
 *
 ...
 * @author arozinat
 */
public class ConformanceAnalysisSettings extends JPanel implements ThreadNotificationTarget, GuiNotificationTarget {
	...
	/**
	 * This configuration object specifies the available analysis options
	 * in a hierarchical way and displays them automatically to the user
	 * whithin the settings frame.
	 */
	...
	private JPanel upperPanel = new JPanel(); // upper panel containing the invitation to select analysis options
	private JPanel restrictionPanel = new JPanel(); // panel containing the depth restriction check box and the spinner 
	private JPanel buttonsPanel = new JPanel(); // lower panel containing the buttons
	private GuiPropertyStringTextarea optionsStatement = new GuiPropertyStringTextarea("Furthermore, you can choose which kind of analysis you would like to perform. " + 
	...
	private GuiPropertyStringTextarea restrictionStatement = new GuiPropertyStringTextarea("The Conformance Checker has automatically determined the maximum search depth " + 
	...
	private JButton docsButton = new SlickerButton("Help..."); // shows the plugin documentation
	private GUIPropertyBoolean isRestricted;
	public GUIPropertyInteger restrictedDepth;
	protected GUIPropertyBoolean bestInvisibles = new GUIPropertyBoolean("Choose best shortest sequence of invisible tasks", 
			"<html>Tell Log replay to look into future for choosing best shortest path of invisible tasks<br>(try this if you want to replay EPC models containing OR splits after conversion).<html>", false, this);
	...
	public ConformanceAnalysisSettings(ConformanceAnalysisPlugin algorithm,
		...
		myNet = net;
		myLog = log;
		// Builds the conformance-specific configuration object, which links the semantic view at the
		// possible analysis options with the analysis methods needed to compute them
		analysisOptions = new ConformanceAnalysisConfiguration();
		// automatically with estimated search depth needed for replaying this model
		int searchDepth = MaximumSearchDepthDiagnosis.determineMaximumSearchDepth(net);
		if (searchDepth == -1) {
			isRestricted = new GUIPropertyBoolean("Restrict search depth for invisible tasks", "Restricts search for sequences of invisible tasks that might enable another task during log replay", false, this);
			restrictedDepth = new GUIPropertyInteger("Maximum depth: ", "Corresponds to the maximum length of a sequence of invisible tasks that can be found", 
					0, 0, 100);
		} else {
			isRestricted = new GUIPropertyBoolean("Restrict search depth for invisible tasks", "Restricts search for sequences of invisible tasks that might enable another task during log replay", true, this);
			restrictedDepth = new GUIPropertyInteger("Maximum depth: ", "Corresponds to the maximum length of a sequence of invisible tasks that can be found", 
					searchDepth, 0, 100);
		}
		// build GUI
	...
	/**
	 * Build the Conformance Analysis Settings GUI, that is to let
	 * people specify which kind of conformance analysis they want.
	 */
	private void jbInit() {
		...
		restrictionPanel.setLayout(new BoxLayout(restrictionPanel, BoxLayout.X_AXIS));
	...
	private void registerGuiActionListener() {
		...
		startButton.addActionListener(new ActionListener() {
			 ...
			 * their return before building the
			 * {@link ConformanceAnalysisResults ConformanceAnalysisResults} frame.
			 * Note that the {@link AnalysisConfiguration AnalysisConfiguration}
			 * object is cloned before passed to the analyis methods in order to
			...
			public void actionPerformed(ActionEvent e) {
				...
				LogReplayAnalysisMethod logReplayAnalysis = new LogReplayAnalysisMethod(
						myNet, myLog, new ConformanceMeasurer(),
				...
				execute(new StateSpaceExplorationMethod(myNet,
						new Progress("State space analysis..")));
	...
	/**
	 * Will be called as soon the user restricts or unrestricts the search 
	 * depth for invisible tasks during log replay. 
	 * <br>
	 * Per default the restriction is selected and a depth value can be provided.
	 * However, as soon as the user deselects the depth limitation the corresponding
	 * spinner will be disabled (and it will be enabled as soon as the restriction 
	 * is selected again). 
	 */
	...
	public void execute(AnalysisMethod method) {
		...
		// and be
		// able to abort it
		AnalysisMethodExecutionThread worker = new AnalysisMethodExecutionThread(method, this);
	 ...
	 * Should be called each time the "Start Analysis" has been pressed.
	 */
	public void initAnalysis() {
		// prevent user to press button again while analysis is performed
		startButton.setEnabled(false);
		// clone configuration and init thread list
		// (actual analysis may be invoked multiple times from the same results frame)
	...
	/**
	 * Retrieves the copy of the current analysis configuration in order to carry out the
	 * analysis based on the chosen options.
	 * Should only be invoked after {@link #initAnalysis initAnalysis} has been called.
	 * <br>
	 * Note that all the method is <code>synchronized<code> as it will be accessed
	 ...
	 * This is used in order to ensure every method execution being finished
	 * before their analysis results can be used for building the results frame.
	 * @see #buildResultsFrame
	 * @param thread the method execution thread to be added
	 */
	public synchronized void addMethodExecutionThread(AnalysisMethodExecutionThread thread) {
		myMethodExecutionThreads.add(thread);
	}

	/**
	 * Called by each thread after having finished.
	 * Checks whether there are still other analysis methods running. Builds
	 * the result frame if not, does nothing otherwise.
	 * <br>
	 * Note that all the method is <code>synchronized<code> as it will be accessed
	 * by concurrent threads executing the different analysis methods.
	 ...
	 * frame.
	 * This procedure is invoked after each thread has indicated being ready via
	 * the {@link #threadDone threadDone} method.
	 */
	private void buildResultsFrame() {
		...
		while (tabCategories.hasNext()) {
			...
			if (current.getName() == "Structure") {
				Set<AnalysisResult> structApprResultObjects = current.getResultObjects();
				current.setResultPanel(new StructAppropriatenessAnalysisGUI(structApprResultObjects));
			}
		...
		// spawn result window
		ConformanceAnalysisResults resultWindow = new ConformanceAnalysisResults(getAnalysisConfiguration(), depthValue);
		resultWindow.setVisible(true);
		MainUI.getInstance().createAnalysisResultFrame(myAlgorithm, myInput, resultWindow);
		// sets the "Start Analysis" button active, so that it can be invoked again
