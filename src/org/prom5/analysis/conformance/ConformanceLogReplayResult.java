 ...
 * Can be used to retrieve values for the implemented metrics and to get
...
public class ConformanceLogReplayResult extends LogReplayAnalysisResult {
	...
	///////////// analysis options (if no configuration is given, all options are treated enabled)
	...
	/**
	 * Contains the diagnostic data structures for the activities
	 * (and their relations) in the current log selection.
	 * Non-existing entries in pre- and post-relations are counted as 0.
	 */
	...
	/**
	 * Contains the sum of mean numbers of enabled transitions for all
	 * process instances (taking the number of similar instances into account).
	 */
	 ...
	 * basic fitness measure of being successfully executed based on missing tokens.
	 */
	private int numberOfConsumedTokens = 0;

	/**
	 * The number of produced tokens during log replay is used to calculate the
	 * basic fitness measure of being properly terminated based on remaining tokens.
	 */
	...
	/**
	 * {@inheritDoc}
	 * Evaluate the given configuration options, that is,
	 * retrieves the choices made by the user and converts them into analysis option variables
	 * indicating which parts of the analysis needs to be done.
	 */
	public ConformanceLogReplayResult(AnalysisConfiguration analysisOptions, PetriNet net,
			LogReader log, LogReplayAnalysisMethod method) {
		super(analysisOptions, net, log, method);

		if (analysisOptions != null) {
			// retrieve the options
			ArrayList configurationOptions = analysisOptions.getEnabledOptionsForAnalysisMethod(new
											 ArrayList(), method.getIdentifier());
			Iterator allOptions = configurationOptions.iterator();
			while (allOptions.hasNext()) {
				AnalysisConfiguration currentOption = (AnalysisConfiguration) allOptions.next();
		...
		// perform log replay as soon as only one of the options has been chosen by the user
		/**@todo anne: automate (so that not needs to be extended if a new option is added) */
		performLogReplay = (calculateFitness() == true || calculateSuccessfulExecution() == true ||
	...
	/*
	 *  (non-Javadoc)
	 * @see org.processmining.framework.models.petrinet.algorithms.logReplay.LogReplayAnalysisResult#initDiagnosticDataStructures()
	 */
	protected void initDiagnosticDataStructures() {
		replayedLog = new DiagnosticLogReader(inputLogReader);
		replayedPetriNet = new FitnessVisualization(inputPetriNet, replayedLog.getLogTraceIDs(),
	 ...
	 * @return <code>true</code> if this option has been chosen, <code>false</code> otherwise
	 */
	...
	/**
	 * Indicates whether the "Improved Behavioral Appropriateness" metric has been
	...
	/**
	 * Determine the fraction [0,1] of properly terminated process instances from those
	 * log traces specified in the list.
	 * Note that the number of process instances represented by each dignostic log trace
	 * is taken into account, and every log trace is treated as a whole. That means that
	 * there is no possibiltiy to retrieve this measure for only a fraction of similar
	...
	public float getFractionOfProperlyTerminated() {
		...
		if (noOfInstances > 0) {
			...
			Message.add("The fraction of properly completed log traces pPC can only be calculated if there were any log traces replayed.");
			return 0;
		}
	}

	/**
	 * Determine the fraction [0,1] of successfully executed process instances from those
	 * log traces specified in the list.
	 * Note that the number of process instances represented by each dignostic log trace
	 * is taken into account, and every log trace is treated as a whole. That means that
	 * there is no possibiltiy to retrieve this measure for only a fraction of similar
	 * instances for a certain logical trace.
	 * @param piList The list of specified log traces.
	...
	public float getFractionOfSuccessfullyExecuted() {
		...
		if (noOfInstances > 0) {
			...
			Message.add("The fraction of successfully executed log traces pSE can only be calculated if there were any log traces replayed.");
			return 0;
		}
	}

	/**
	 * Retrieves the successful execution measure based on missing tokens
	 * (i.e., amount of missing / amount of consumed) during log replay.
	 * @return The measure for the selected traces.
	 */
	public float getSuccessfulExecutionTokenMeasure() {
		float missing = (float) ((DiagnosticPetriNet) replayedPetriNet).getMissingTokens();
		float consumed = (float) numberOfConsumedTokens;
		if (consumed > 0) {
			// calculate
			return (1 - (missing / consumed));
		} else {
			// if the log is completely empty then one could think of a single empty trace with a 
			// start token being produced and a token being consumed from the final place, which leads to 1-1 = 0
			return 0;
		}
	}

	/**
	 * Retrieves the proper completion measure based on remaining tokens
	 * (i.e., amount of remaining / amount of produced) during log replay.
	 * @return The measure for the selected traces.
	 */
	public float getProperCompletionTokenMeasure() {
		float remaining = (float) ((DiagnosticPetriNet) replayedPetriNet).getRemainingTokens();
	...
	public float getFitnessMeasure() {
		...
		return ((float) 0.5) * successfulExecution + ((float) 0.5) * properCompletion;
	}

	/**
	 * Retrieves the behavioral appropriateness measure based on the mean number of
	 * enabled transitions during log replay.
	 * @param piList The list of diagnostic log traces.
	 * @return The measure for the selected traces (0 <= value <= 1).
	 * @throws Exception In the case that the entry for one
	 * of the traces is missing.
	 */
	public float getBehavioralAppropriatenessMeasure() throws Exception {
		// calculate mean number of enabled transitions
		float meanEnabled = overallMeanNumber / (float) overallNumberOfInstances;
		...
		if (numberOfTransitons > 1 && overallNumberOfInstances > 0) {
			...
			Message.add("The Behavioral appropriateness metric aB can only be calculated for " +
	...
	/**
	 * Retrieves the global log event relations gathered during log
	 * replay.
	...
	/**
	 * Adds the given process instance to the set of perfectly fitting
	 * log traces.
	 * @param pi the fitting log trace to be added
	 */
	...
	/**
	 * TODO: check whether can be obtained from the log directly
	 * Adds up the total number of process instances.
	 * @param corresponds to the number of similar instances for the updated trace
	 */
	...
	/**
	 * Adds up the global "properly-completed" measurement by the
	 * given value.
	 * @param corresponds to the number of similar instances for the updated trace
	 */
	...
	/**
	 * Adds up the global "successfully-executed" measurement by the
	 * given value.
	 * @param corresponds to the number of similar instances for the updated trace
	 */
	 ...
	 * @param consumed the tokens that were consumed for the trace
	 */
	 ...
	 * @param produced the tokens that were consumed for the trace
	 */
	...
	/**
	 * Updates the mean number of enabled transitions measurement for the
	 * given log trace according to the number of similar instances.
	 * @param the mean number of enabled transitions during replay for the log trace
	 * @param the number of similar process instances (control-flow perspective)
	 */
	...
	/**
	 * Set the new visualization state, which leads to a replacement of the diagnostic
	 * Petri net that actually delivers the customized visualization.
	 * @param newVisualization The new visualization state.
	 */
	private void setVisualizationState(DisplayState newVisualization) {
		// exchange the diagnostic Petri net for delivering the correct visualization
		if (newVisualization.toString().equals(DisplayState.MODEL.toString())) {
			// only change if state is new
			if (getCurrentVisualizationState().equals(newVisualization.toString()) == false) {
				replayedPetriNet = new FitnessVisualization((DiagnosticPetriNet) replayedPetriNet);
			}
			...
			currentVisualization.remainingTransitionsOption = newVisualization.
		...
		} else if (newVisualization.toString().equals(DisplayState.LOG.toString())) {
			// only change if state is new
			if (getCurrentVisualizationState().equals(newVisualization.toString()) == false) {
				replayedPetriNet = new FitnessLogTraceVisualization((DiagnosticPetriNet)
								   replayedPetriNet);
			}
			// but set always the current options state
			FitnessLogTraceVisualization currentVisualization = (FitnessLogTraceVisualization)
					replayedPetriNet;
			currentVisualization.failedEventsOption = newVisualization.failedEventsOption;
		} else {
			Message.add("Correct visualization state could not be determined.\n", 2);
		}
		currentVisualization = newVisualization;
	}

	/**
	 * Creates a visualization of the conformance check results. Note that a change of the
	 * display state by the user will have no effect before calling this methods. This is
	 * intended to prevent unnecessary cloning of the diagnostic petri net, which actually
	 * delivers the custom visualization of the conformance analysis results.
	 * @param selectedInstances The process instances that have been selected for updating
	 * the visualization.
	 * @param currentVisualization The current display state.
	...
	public GrappaPanel getVisualization(DisplayState currentVisualization) {
		...
		myResultVisualization = ((DiagnosticPetriNet) replayedPetriNet).getGrappaVisualization();
