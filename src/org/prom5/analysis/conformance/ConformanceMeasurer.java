...
package org.prom5.analysis.conformance;
...
import org.prom5.framework.models.petrinet.State;
import org.prom5.framework.models.petrinet.StateSpace;
import org.prom5.framework.models.petrinet.Transition;
import org.prom5.framework.models.petrinet.algorithms.CoverabilityGraphBuilder;
import org.prom5.framework.models.petrinet.algorithms.logReplay.AnalysisConfiguration;
...
import org.prom5.framework.models.petrinet.algorithms.logReplay.Measurer;
import org.prom5.framework.models.petrinet.algorithms.logReplay.ReplayedLogTrace;
import org.prom5.framework.models.petrinet.algorithms.logReplay.ReplayedPlace;
import org.prom5.framework.models.petrinet.algorithms.logReplay.ReplayedTransition;
import org.prom5.framework.ui.Message;

/**
 * Actually takes those measurements that are needed to calculate the
 * selected conformance metrics.
 *
 * @author arozinat
 */
public class ConformanceMeasurer extends Measurer {
	...
	/**
	 * Maps the name of each log event within the current trace on its diagnostic data structure
	 * (which includes their relations to other log events).
	 */
	private DiagnosticLogEventRelation logEvents = new DiagnosticLogEventRelation();

	/// fitness
	/**
	 * Records every replayed trace for which tokens were missing during log replay.
	 * The total amount is then recorded at that place at the end of the replay.
	 */
	private HashMap<DiagnosticPlace, Integer> missingTokens = new HashMap<DiagnosticPlace, Integer>();
	/**
	 * Remembers those places that had either tokens missing or remaining in this trace
	...
	/**
	 * The number of consumed tokens during log replay is used to
	 * (a) calculate the basic fitness measure of being successfully executed based on missing tokens
	 * (b) record the fitness measurement for this trace directly in the event log itself
	 */
	private int numberOfConsumedTokens = 0;
	/**
	 * The number of produced tokens during log replay of this trace is used to
	 * (a) calculate the basic fitness measure of being properly terminated based on remaining tokens
	 * (b) record the fitness measurement for this trace directly in the event log itself
	 */
	private int numberOfProducedTokens = 0;
	/**
	 * The number of missing tokens during log replay of this trace is used to
	 * record the fitness measurement for this trace directly in the event log itself
	 */
	...
	/**
	 * The sume of enabled transitions during replay of current trace
	 * (to be set in relation with the number of measurements).
	 */
	 ...
	 * tokens are left after replaying the trace) of each process instance.
	 * For every entry the key is the ID of the corresponding process instance
	 * and the value is the bolean value for that case.
	 */
	...
	private ProcessInstance currentInstance; // for writing the actual fitness value

	/**
	 * {@inheritDoc}
	 * Creates an analysis result of type {@link ConformanceLogReplayResult ConformanceLogReplayResult}.
	 */
	protected LogReplayAnalysisResult initLogReplayAnalysisResult(AnalysisConfiguration
			analysisOptions,
			PetriNet petriNet, LogReader log, LogReplayAnalysisMethod analysisMethod) {
		// notify the user that
		if (log instanceof BufferedLogReader == false) {
			Message.add("This Log-reader is READ-ONLY, so the fitness results will not be stored." +
					" Please select another Log-Reader under 'Help'.");
		}
		// initialize the result object and the analysis options
		return new ConformanceLogReplayResult(analysisOptions, petriNet, log, analysisMethod);
	}

	/**
	 * {@inheritDoc}
	 * Resets the log-based data structures for the replay of this trace.
	 * @param pi (not used)
	 *
	 */
	protected void initTraceReplay(ReplayedLogTrace pi, LogReplayAnalysisResult result) {
		...
		numberOfRemainingTokens = 0; // for fitness recording at log trace itself
		...
		if (((ConformanceLogReplayResult) result).calculateImprovedBehavioralAppropriateness() == true) {
	...
	/**
	 * {@inheritDoc}
	 * For behavioral appropriateness metric the number of enabled transitions
	 * needs to be checked before each replay step.
	 */
	protected void takePreStepExecutionMeasurement(LogReplayAnalysisResult result,
			ReplayedLogTrace pi, int maxDepth) {
		// only if the corresponding metric was chosen to be computed (performance reasons)
		if (((ConformanceLogReplayResult) result).calculateBehavioralAppropriateness() == true) {
			// take measurement before each replay step (needed for appropriateness metric)
			determineNumberOfEnabledTransitions((DiagnosticPetriNet) result.replayedPetriNet,
	...
	/**
	 * {@inheritDoc}
	 * For the improved behavioral appropriateness metric one needs to keep track of the
	 * relations between log events.
	 */
	protected void takeLogEventRecordingMeasurement(LogReplayAnalysisResult result,
			ReplayedLogTrace pi, AuditTrailEntry ate) {
	...
	/**
	 * {@inheritDoc}
	 * The replay failure will be recorded for the corresponding trace, transition,
	 * and audit trail entry.
	 */
	protected void takeFailedTaskMeasurement(ReplayedLogTrace pi, ReplayedTransition t,
			AuditTrailEntry ate) {
		try {
			...
			Message.add("Punishing failed task produced an error for transition " +
	...
	/**
	 * {@inheritDoc}
	 * Records the missing token at the corresponding place in the Petri net.
	 */
	protected void takeMissingTokenMeasurement(ReplayedPlace p, ReplayedLogTrace pi) {
		...
		numberOfMissingTokens++; // for the fitness measurement of this trace in the event log itself
	}

	/*
	 *  (non-Javadoc)
	 * @see org.processmining.framework.models.petrinet.algorithms.logReplay.Measurer#takePostTraceReplayMeasurement(org.processmining.framework.models.petrinet.algorithms.logReplay.LogReplayAnalysisResult, org.processmining.framework.models.petrinet.algorithms.logReplay.ReplayedLogTrace)
	 */
	protected void takePostTraceReplayMeasurement(LogReplayAnalysisResult result,
			ReplayedLogTrace pi) {
		// updating the total number of traces is needed both for fitness metrics and for simple behavioral
		// appropriateness metric!
		ConformanceLogReplayResult conformanceResult = (ConformanceLogReplayResult) result;
		conformanceResult.updateNumberOfProcessInstances(pi.getNumberOfProcessInstances());
	...
	/*
	 *  (non-Javadoc)
	 * @see org.processmining.framework.models.petrinet.algorithms.logReplay.Measurer#takePostReplayMeasurement(org.processmining.framework.models.petrinet.algorithms.logReplay.LogReplayAnalysisResult)
	 */
	 ...
	 * @param result the result object filled by the log replay method
	 * @param pi the trace which is currently replayed
	 */
	private void takeRemainingTokensMeasurement(LogReplayAnalysisResult result, ReplayedLogTrace pi) {
		try {
			// finally count the amount of remaining tokens for that instance and store it
			Iterator remainingTokenChecker = result.replayedPetriNet.getPlaces().iterator();
			while (remainingTokenChecker.hasNext()) {
				DiagnosticPlace currentPlace = (DiagnosticPlace) remainingTokenChecker.next();
				...
				if (currentPlace.outDegree() == 0) {
					...
					if (currentPlace.getNumberOfTokens() == 0) {
						...
						currentPlace.addMissingTokens(1, pi.getNumberOfProcessInstances());
						...
						currentPlace.addRemainingTokens(carryover, pi.getNumberOfProcessInstances());
						// record that something were "wrong"
						wrongTokenPlaces.add(currentPlace);
						// set proper completion measure
						this.setProperlyTerminated(false);
						// for the fitness measurement of this trace in the event log itself
						numberOfRemainingTokens = numberOfRemainingTokens + carryover;
					}
				}
				// treat non-final places
					...
					if (currentPlace.getNumberOfTokens() > 0) {
						...
						Iterator potentiallyEnabled = currentPlace.getSuccessors().iterator();
						while (potentiallyEnabled.hasNext()) {
							DiagnosticTransition currentTransition = (DiagnosticTransition)	potentiallyEnabled.next();
			...
			Message.add("Taking post replay measurements failed for trace " + pi.getName() + ".\n" +
					ex.toString(), 2);
			ex.printStackTrace();
		}
	}
	...
	/**
	 * Updates all the fitness-related measurements on the global level,
	 * so that the currently replayed trace is taken into account.
	 */
	private void updateGlobalFitnessMeasurements(LogReplayAnalysisResult result,
			ReplayedLogTrace pi) {
		ConformanceLogReplayResult conformanceResult = (ConformanceLogReplayResult) result;
		int numberOfSimilar = pi.getNumberOfProcessInstances();
		// update consumed tokens counter
		conformanceResult.updateConsumedTokens(getNumberOfConsumedTokens() * numberOfSimilar);
		// update produced tokens counter
		conformanceResult.updateProducedTokens(getNumberOfProducedTokens() * numberOfSimilar);
		...
		while (it.hasNext()) {
			...
			place.addMissingTokens(missingValue.intValue(), pi.getNumberOfProcessInstances());
			// record that something were "wrong"
			wrongTokenPlaces.add(place);
		}

		// check for each place whether there were tokens missing or remaining during the
		// replay of this trace, and if not: record that zero tokens were "wrong"
		Iterator<Place> placesInNet = result.replayedPetriNet.getPlaces().iterator();
		...
		// check for each transition whether (a) failed execution or (b) remained enabled
		// (used for tool tip in visualization)
		Iterator<Transition> transitionsInNet = result.replayedPetriNet.getTransitions().iterator();
		while (transitionsInNet.hasNext()) {
			DiagnosticTransition currentTransition = (DiagnosticTransition) transitionsInNet.next();
			...
			if (failed == true) {
				currentTransition.addFailedInstances(pi.getNumberOfProcessInstances());
			}
			if (remained == true) {
				currentTransition.addRemainedInstances(pi.getNumberOfProcessInstances());
			}
			if (failed == false && remained == false) {
				currentTransition.addNumberInstancesNeitherFailedNorRemaining(pi.
						getNumberOfProcessInstances());
			}
		}
		...
		if (hasProperlyTerminated() == true) {
			conformanceResult.updateProperlyCompletedInstances(pi.getNumberOfProcessInstances());
		}
		if (hasSuccessfullyExecuted() == true) {
			conformanceResult.updateSuccessfullyExecutedInstances(pi.getNumberOfProcessInstances());
		}
		if (hasProperlyTerminated() == true && hasSuccessfullyExecuted() == true) {
		...
		if (currentInstance.getAttributes().containsKey("Fitness") == false) {
			...
			currentInstance.setAttribute("Fitness", getFitnessAttributeForCurrentTrace());
		}
	}

	/**
	 * Updates the mean number of enabled transitions measurement on the aggregate
	 * level for the whole log (selection).
	 * @param result the result object filled by the log replay method
	 * @param pi the trace which is currently replayed
	 */
	private void updateGlobalMeanNumberOfEnabledTransitions(LogReplayAnalysisResult result,
			ReplayedLogTrace pi) {
		ConformanceLogReplayResult conformanceResult = (ConformanceLogReplayResult) result;
		conformanceResult.updateMeanOfEnabledTransitions(getMeanNumberOfEnabledTransitions(),
				pi.getNumberOfProcessInstances());
	}
	 ...
	 * @param pi the trace which is currently replayed
	 */
	private void updateGlobalLogEventRelations(LogReplayAnalysisResult result, ReplayedLogTrace pi) {
		...
		Iterator<DiagnosticLogEvent> it = logEvents.getDiagnosticLogEvents().iterator();
		DiagnosticLogEventRelation globalRelation = ((ConformanceLogReplayResult) result).
				getLogEventRelations();
		while (it.hasNext()) {
			DiagnosticLogEvent logEvent = it.next();
			globalRelation.updateLogEventRelation(logEvent, pi.getNumberOfProcessInstances());
		}
	}
	 ...
	 * mean number of enabled transitions value for the given trace, which is used
	 * to calculate the behavioral appropriateness metric aB.
	 * @param diagnosticNet The enhanced Petri net in which the log is replayed.
	 * @param pi The log trace currently replayed.
	 */
	private void determineNumberOfEnabledTransitions(DiagnosticPetriNet diagnosticNet,
			DiagnosticLogTrace pi, int maxDepth) {
		// build coverability graph from the current marking,
		// note that the net must be cloned before to preserve its replay state
		DiagnosticPetriNet clonedNet = (DiagnosticPetriNet) diagnosticNet.clone();
		// maxDepth + 1 as looking at enabled tasks from current marking are on next level
		// (different from transparent firing)
		StateSpace coverabilityGraph = CoverabilityGraphBuilder.build(clonedNet, maxDepth+1);
		...
		// been reached in the state space (i.e., were already counted being enabled)
		...
		alreadyCounted = traceEnabledTasks(alreadyCounted, coverabilityGraph, replayState,
	 ...
	 * @param alreadyCounted Contains the visible (transparently) enabled transitions so far encountered.
	 * @param coverabilityGraph The coverability graph to be traversed.
	 * @param currentState The current traversal state.
	 * @return The IDs of the transitions so far encountered.
	 */
	private HashSet traceEnabledTasks(HashSet alreadyCounted,
			StateSpace coverabilityGraph, State currentState, int maxEnabled, HashSet visitedStates) {
		...
		// TODO - find out whether, if no outgoing edges exist for this state, not rather an empty list should be
		...
		if (currentState.getOutEdges() != null) {
			...
			while (outgoingEdges.hasNext()) {
				...
				DiagnosticTransition associatedTransition = (DiagnosticTransition) currentEdge.
						object;
				...
				if (associatedTransition.isInvisibleTask()) {
					...
					mergedAlreadyCounted.addAll(traceEnabledTasks(alreadyCounted, coverabilityGraph,
							nextState, maxEnabled, visitedStates));
				}
	 ...
	 * enabled transitions.
	 * @param newNumberEnabled The new measurement for the number of enabled transitions.
	 */
	 ...
	 * This is used to write this diagnostic information back to the log.
	...
	public String getFitnessAttributeForCurrentTrace() {
		...
		float numResult = ((float) 0.5) * successfulExecution + ((float) 0.5) * properCompletion;
		return "" + numResult;
	}
}
