...
public class DiagnosticLogEventRelation {
	 ...
	 * @param namePlusType the string consisting of name and type specifies the log event to be retrieved
	 ...
	 * @param first the first element in the relation
	 * @param second the second element in the relation
	 * @param threshold the threshold for the sometimes relation
	 * @return whether they are in sometimes follows relation (according to threshold)
	 */
	public boolean areInSFRelation(String first, String second, int threshold) {
		...
		if (firstLE != null) {
			Set<DiagnosticLogEvent> SF = firstLE.getSometimesRelationForwards(threshold);
			...
			// firstLE has never been observed in the log (per definition Never relation)
			// TODO check: should actually never happen because of external normalization of log relation
			return false;
		}
	}
	 ...
	 * @param first the first element in the relation
	 * @param second the second element in the relation
	 * @return whether they are in sometimes follows relation (according to threshold 0)
	 */
	 ...
	 * @param second the second element in the relation
	 * @param threshold the threshold for the sometimes relation
	 * @return whether they are in sometimes Precedes relation (according to threshold)
	 */
	public boolean areInSBRelation(String first, String second, int threshold) {
		...
		// check whether they are in SF relation
		if (firstLE != null) {
			Set<DiagnosticLogEvent> SB = firstLE.getSometimesRelationsBackwards(threshold);
	 ...
	 * @param first the first element in the relation
	 * @param second the second element in the relation
	 * @return whether they are in sometimes Precedes relation (according to threshold 0)
	 */
	 ...
	 * @param first the first element in the relation
	 * @param second the second element in the relation
	 * @param threshold the threshold for the Always relation
	 * @return whether they are in Always follows relation (according to threshold)
	 */
	public boolean areInAFRelation(String first, String second, int threshold) {
		...
		// check whether they are in SF relation
		if (firstLE != null) {
			Set<DiagnosticLogEvent> AF = firstLE.getAlwaysRelationsForwards(threshold);
	 ...
	 * @param first the first element in the relation
	 * @param second the second element in the relation
	 * @return whether they are in Always follows relation (according to threshold 0)
	 */
	 ...
	 * @param second the second element in the relation
	 * @param threshold the threshold for the Always relation
	 * @return whether they are in Always Precedes relation (according to threshold)
	 */
	public boolean areInABRelation(String first, String second, int threshold) {
		...
		// check whether they are in SF relation
		if (firstLE != null) {
			Set<DiagnosticLogEvent> AB = firstLE.getAlwaysRelationsBackwards(threshold);
	 ...
	 * @param first the first element in the relation
	 * @param second the second element in the relation
	 * @return whether they are in Always Precedes relation (according to threshold 0)
	 */
	 ...
	 * @param first the first element in the relation
	 * @param second the second element in the relation
	 * @param threshold the threshold for the Never relation
	 * @return whether they are in Never follows relation (according to threshold)
	 */
	public boolean areInNFRelation(String first, String second, int threshold) {
		...
		// check whether they are in SF relation
		if (firstLE != null) {
			Set<DiagnosticLogEvent> NF = firstLE.getNeverRelationsForwards(threshold);
			...
			// although this is per definition Never relation we do not want to show
	 ...
	 * @param first the first element in the relation
	 * @param second the second element in the relation
	 * @return whether they are in Never follows relation (according to threshold 0)
	 */
	 ...
	 * @param second the second element in the relation
	 * @param threshold the threshold for the Never relation
	 * @return whether they are in Never Precedes relation (according to threshold)
	 */
	public boolean areInNBRelation(String first, String second, int threshold) {
		...
		// check whether they are in SF relation
		if (firstLE != null) {
			Set<DiagnosticLogEvent> NB = firstLE.getNeverRelationsBackwards(threshold);
	 ...
	 * @param first the first element in the relation
	 * @param second the second element in the relation
	 * @return whether they are in Never Precedes relation (according to threshold 0)
	 */
	 ...
	 * <br>
	 * Call this method in order to add a newly observed log event for
	 * a log trace or possible execution trace in the model.
	 * @param name the name of the log event
	 * @param type the event type (such as start or complete)
	 */
	 ...
	 * @param similarInst
	 */
	public void updateLogEventRelation(DiagnosticLogEvent newEvent, int similarInst) {
		String namePlusType = newEvent.getModelElementName() + newEvent.getEventType();
		...
		if (logEvents.containsKey(namePlusType)) {
			...
			// note that occurrence count is initialized with 0 (in contrast to super class)
			eventInGlobalRelation = new DiagnosticLogEvent(newEvent.getModelElementName(),
									newEvent.getEventType());
			// add to global relation!
			logEvents.put(namePlusType, eventInGlobalRelation);
		}

		// increment occurrence counter for this log event (occurrence means occurrence per trace)
		// note that Always, Never and Sometimes relations must be calculated with respect to
		// occurrence of the log event
		eventInGlobalRelation.incOccurrenceCount(similarInst);

		// increment pre-activity counter
		Iterator<DiagnosticLogEvent> preActivities = newEvent.getPreActivities().iterator();
		while (preActivities.hasNext()) {
			DiagnosticLogEvent currentPre = preActivities.next();
			// check whether pre-activity is already contained in relation matrix
			String currentPreString = currentPre.getModelElementName() + currentPre.getEventType();
			...
			if (logEvents.containsKey(currentPreString)) {
				...
				preInGlobalRelation = new DiagnosticLogEvent(currentPre.getModelElementName(),
									  currentPre.getEventType());
				// add to global relation!
				logEvents.put(currentPreString, preInGlobalRelation);
			}
			// now increment pre-actvity counter
			eventInGlobalRelation.addPreActivity(preInGlobalRelation, similarInst);
		}

		// increment post-activity counter
		Iterator<DiagnosticLogEvent> postActivities = newEvent.getPostActivities().iterator();
		while (postActivities.hasNext()) {
			DiagnosticLogEvent currentPost = postActivities.next();
			// check whether pre-activity is already contained in relation matrix
			String currentPreString = currentPost.getModelElementName() + currentPost.getEventType();
			DiagnosticLogEvent postInGlobalRelation = null;
			if (logEvents.containsKey(currentPreString)) {
				postInGlobalRelation = logEvents.get(currentPreString);
			} else {
				postInGlobalRelation = new DiagnosticLogEvent(currentPost.getModelElementName(),
									   currentPost.getEventType());
				// add to global relation!
				logEvents.put(currentPreString, postInGlobalRelation);
			}
			// now increment pre-actvity counter
			eventInGlobalRelation.addPostActivity(postInGlobalRelation, similarInst);
		}
	}

	/**
	 * Walks through all the local log event relations and adds a zero entry
	 * for those log events that are not present in the local relation.
	 * This is needed in order to calculate the correct Never (and Sometimes)
	 * relations.
	 */
	public void completeRelationByZeroEntries() {
		...
		while (temp.hasNext()) {
			...
			// update all log events that do not contain current with zero relation
	 ...
	 * This is needed in order to calculate the correct Never (and Sometimes)
	 * relations.
	 * @param the set of log events that should be covered by the relation
	 */
	public void completeRelationByExternalZeroEntries(Collection<DiagnosticLogEvent>
			referenceEvents) {
		...
		while (temp.hasNext()) {
			...
			DiagnosticLogEvent currentInLog = getDiagnosticLogEvent(currentInModel.
											  getModelElementName() + currentInModel.getEventType());
			if (currentInLog == null) {
				DiagnosticLogEvent neverOccurred = new DiagnosticLogEvent(currentInModel.
						getModelElementName(),
						currentInModel.getEventType());

				// put dummy log event entry (in order to find it in isInNF() and isInNB() relation methods)
				// but don't keep any relations to other log events
				logEvents.put(neverOccurred.getModelElementName() + neverOccurred.getEventType(),
						neverOccurred);

				// there is no such log event contained in this relation yet --> normalize!
				// update all log events that do not contain current with zero relation
				Iterator<DiagnosticLogEvent> all = logEvents.values().iterator();
				while (all.hasNext()) {
					DiagnosticLogEvent toBeUpdated = all.next();
	 ...
	 * @param threshold the barrier telling when something counts as
	 * happened "always" or "never".
	 */
	public void printRelationsToMessageConsole(int threshold) {
		...
		while (it.hasNext()) {
			...
			Message.add("\n" + logEvent.toString() + ": " + logEvent.getOccurrenceCount());

			///// FORWARD RELATIONS
			// print always follows relation
			Message.add("Actvities in Always-follows relation: ");
			Iterator<DiagnosticLogEvent>
					print = logEvent.getAlwaysRelationsForwards(threshold).iterator();
			while (print.hasNext()) {
				DiagnosticLogEvent current = print.next();
				Message.add(current.getModelElementName() + current.getEventType());
			}
			...
			print = logEvent.getSometimesRelationsBackwards(threshold).iterator();
			while (print.hasNext()) {
				DiagnosticLogEvent current = print.next();
				Message.add(current.getModelElementName() + current.getEventType());
			}
	...
	/**
	 * Prints how often any log event has occurred some time before or after each
	 * log event.
	 */
	public void printOccurrenceCountsToMessageConsole() {
		...
		while (it.hasNext()) {
			...
			Iterator<DiagnosticLogEvent> preActivities = logEvent.getPreActivities().iterator();
			while (preActivities.hasNext()) {
				DiagnosticLogEvent currentPre = preActivities.next();
				int occurrenceCount = logEvent.getPreActivityStatus(currentPre);
				Message.add(currentPre.getModelElementName() + currentPre.getEventType()
						+ " occurred in " + occurrenceCount +
						" traces some time before current activity.");
			}

			// print post-activities including occurrence count
			Message.add("Post-activities: ");
			Iterator<DiagnosticLogEvent> postActivities = logEvent.getPostActivities().iterator();
			while (postActivities.hasNext()) {
				DiagnosticLogEvent currentPost = postActivities.next();
				int occurrenceCount = logEvent.getPostActivityStatus(currentPost);
				Message.add(currentPost.getModelElementName() + currentPost.getEventType()
						+ " occurred in " + occurrenceCount +
