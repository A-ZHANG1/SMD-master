...
/**
 * The diagnostic place can be enhanced by diagnostic information
 * to be stored during log replay and will be used to form a part of the diagnostic
 * Petri net.
 *
 * @author arozinat
 */
public class DiagnosticPlace extends ReplayedPlace {
	...
	/**
	 * Data structure for storage of the amount of missing tokens
	 * (takes the number of similar process instances into account).
	 */
	private int missingTokens;

	/**
	 * Data structure for storage of the amount of missing tokens
	 * (does not take the number of similar process instances into account).
	 */
	private int missingTokensSemanticLevel;

	/**
	 * Data structure for storage of the amount of remaining tokens
	 * (takes the number of similar process instances into account).
	 */
	...
	/**
	 * The number of process instances for which this there were
	 * neither tokens missing nor remaining in this place.
	 */
	...
	/**
	 * Records for each amount of missing tokens for how many process
	 * instances that many tokens were missing (for tooltip).
	 */
	 ...
	 * the counter of missing and remaining tokens for every process instance specified.
	 ...
	 * @param the number of similar instances for the current trace
	 */
	public void addMissingTokens(int value, int similarInstances) {
		...
		if (aggregatedMissing.containsKey(new Integer(value))) {
			...
			Integer oldValue = (Integer) aggregatedMissing.get(new Integer(value));
			int newValue = oldValue.intValue() + similarInstances;
			aggregatedMissing.put(new Integer(value), new Integer(newValue));
		} else {
			// add new entry
			aggregatedMissing.put(new Integer(value), new Integer(similarInstances));
		}
	}
	...
	public void addRemainingTokens(int value, int similarInstances) {
		...
		if (aggregatedRemaining.containsKey(new Integer(value))) {
			// merge with existing entry
			Integer oldValue = (Integer) aggregatedRemaining.get(new Integer(value));
			int newValue = oldValue.intValue() + similarInstances;
			aggregatedRemaining.put(new Integer(value), new Integer(newValue));
		} else {
			// add new entry
			aggregatedRemaining.put(new Integer(value), new Integer(similarInstances));
		}
	}
	 ...
	 * @param similarInstances number of process instances for this trace
	 */
	 ...
	 * Note that the number of similar instances subsumed by a logical log trace
	 * is not taken into account here, which among others is used for the
	 * visual feedback where solely the trace as a logical unit is of interest.
	 *
	 * @return The amount of missing tokens in this place
	 */
	 ...
	 * Note that the number of similar instances subsumed by the logical log trace
	 * is not taken into account here, which among others is used for the
	 * visual feedback where solely the trace as a logical unit is of interest.
	 *
	 * @return The amount of remaining tokens in this place
	...
	public String getDiagnosticToolTip(DiagnosticPetriNet net) {
		...
		if ((aggregatedMissing.size() != 0) || (aggregatedRemaining.size() != 0)) {
			...
			// start from the middle to the beginning (because of ascending sorting)
			String toolTip = "";
			while (traverseRemaining.hasNext()) {
				Integer currentAmount = (Integer) traverseRemaining.next();
				Integer currentInstances = (Integer) sortedRemaining.get(currentAmount);
				// append in front
				toolTip = "<TR><TD BGCOLOR=#FF8080>+" + currentAmount.intValue() +
						  "</TD><TD BGCOLOR=#FF8080>" + currentInstances.intValue() +
						  "</TD></TR>" + toolTip;
			}
			// build head of table
			toolTip =
					"<HTML><TABLE><TR><TD BGCOLOR=#FFFFFF># Tokens</TD><TD  BGCOLOR=#FFFFFF># Instances</TD></TR>" +
					toolTip;
			// proceed from middle to the end
			if (aggregatedZero != 0) {
				toolTip = toolTip + "<TR><TD BGCOLOR=#80AA80>&#160;0 </TD><TD BGCOLOR=#80AA80>" +
			...
			while (traverseMissing.hasNext()) {
				...
				Integer currentInstances = (Integer) sortedMissing.get(currentAmount);
				toolTip = toolTip + "<TR><TD BGCOLOR=#FF8080>-" + currentAmount.intValue() +
	 ...
	 * @param piNames a list of process IDs
	 * @return the HTML tool tip (ready to be displayed)
	 */
	/*public String getPerformanceToolTip(ArrayList piNames) {
	 double mt = getMeanSyncTime(piNames);
	 double mw = getMeanWaitingTime(piNames);
	 return "<html><table><tr><td>Synchronization time: </td><td>" + mt + "</td></tr>" +
	   "<tr><td>Waiting time: </td><td>" + mw + "</td></tr>" +
	...
	public Object clone() {
		...
		/**@todo anne: check whether each of the timeMeasurement objects
		 * needs to be cloned explicitly */
 ...
 * @param waitTime the waiting time is the time a token spends time in a place although
 * the transition is already enabled (i.e., t - tm)
 * @param syncTime the synchronization time is the time the token spends in a place until
 * the transition becomes enabled (i.e., tm - tj)
 */
