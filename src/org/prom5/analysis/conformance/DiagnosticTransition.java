...
/**
 * The diagnostic transition can be enhanced by diagnostic information
...
public class DiagnosticTransition extends ReplayedTransition {
	...
	/**
	 * Counts the number of process instances for which this transition has failed
	 * successful execution.
	 */
	private int failedInstances = 0;

	/**
	 * Counts the number of process instances for which this transition has remained
	 * enabled.
	 */
	...
	/**
	 * Contains all those log events that always followed the key log event in the log
	 * (but only sometimes in the model).
	 */
	private Set<Transition> alwaysFollows = new HashSet<Transition>();

	/**
	 * Contains all those log events that never followed the key log event in the log
	...
	/**
	 * <p>The constructor creates a normal transition but additionally initializes
	 * the diagnostic information for every process instance specified.</p>
	 * <p>Note that the the copy constructor must be simulated on this level to assign the
	 * right Petri net to the new Transition, which eventually is required by Grappa.</p>
	 ...
	 * @param caseIDs The list of process instances that want to be able to
	 * store results of the conformance analysis in this diagnostic transition.
	 */
	...
	/**
	 * {@inheritDoc}
	 * Counts the consumed and produced tokens and remembers the passed edges.
	 */
	protected void takePreFiringMeasurement(ReplayedLogTrace diagnosticTrace, Measurer measurer) {
		// store diagnostic information
		countConsumedAndProducedTokens((DiagnosticLogTrace) diagnosticTrace, measurer);
		rememberPassedEdges(diagnosticTrace);
	}

	/**
	 * {@inheritDoc}
	 * Increments the number of firings for this transition.
	 */
	...
	/**
	 * Adds the specified number of process instances to the
	 * counter of traces that failed execution for this transition.
	 * @param value corresponds to the number of similar process instances
	 */
	...
	/**
	 * Adds the specified number of process instances to the
	 * counter of traces that remained enabled for this transition.
	 * @param value corresponds to the number of similar process instances
	 */
	...
	/**
	 * Increments the corresponding numberOfFirings entry by the given
	 * value.
	 * @param value corresponds to the number of similar process instances
	 */
	...
	/**
	 * Count the number of tokens to be consumed and produced by this transition and
	 * update the respective attributes of the diagnostic log trace.
	 * @param diagnosticTrace the diagnostic log trace which is currently replayed and
	 * for which all the diagnostic infomation is remembered
	 * @param measurer the measurer object in order to record diagnostic information
	 */
	private void countConsumedAndProducedTokens(DiagnosticLogTrace diagnosticTrace,
			Measurer measurer) {
	 ...
	 * @param piName the ID of the log trace which is currently replayed and
	 * for which all the diagnostic infomation is remembered
	 */
	private void rememberPassedEdges(ReplayedLogTrace pi) {
		...
		while (it.hasNext()) {
			...
			// update entry for that instance
			currentEdge.incrementNumberOfPasses(pi.getNumberOfProcessInstances());
		}
	 ...
	 * (but only sometimes in the model.
	 * @param trans task that always followed this task
	 */
	 ...
	 * (but sometimes in the model.
	 * @param trans task that never followed this task
	 */
	 ...
	 * (but only sometimes in the model.
	 * @param trans task that always preceded this task
	 */
	 ...
	 * (but sometimes in the model.
	 * @param trans task that never preceded this task
	 */
	...
	/**
	 * Resets the log relation element counter to 0, so that it can be
	...
	public String getDiagnosticToolTip(DiagnosticPetriNet net) {
		if ((remainingInstances != 0) || (failedInstances != 0)) {
			...
			// build head of table
			String toolTip =
					"<HTML><TABLE><TR><TD BGCOLOR=#FFFFFF>Error Type</TD><TD  BGCOLOR=#FFFFFF># Instances</TD></TR>";
			if (aggregatedZero != 0) {
				toolTip = toolTip + "<TR><TD BGCOLOR=#80AA80>Neither </TD><TD BGCOLOR=#80AA80>" +
						  aggregatedZero + "</TD></TR>";
			}
			if (remainingInstances != 0) {
				toolTip = toolTip + "<TR><TD BGCOLOR=#FF8080>Remained </TD><TD BGCOLOR=#FF8080>" +
						  remainingInstances + "</TD></TR>";
			}
			if (failedInstances != 0) {
				toolTip = toolTip + "<TR><TD BGCOLOR=#FF8080>Failed </TD><TD BGCOLOR=#FF8080>" +
