...
public class FitnessLogTraceVisualization extends DiagnosticPetriNet {
	...
	/**
	 * Represents the visualization option of indicating the log events
	 * that have failed execution in the log trace.
	 * The public attribute is necessary because the writeToDot() method
	 * has a fixed interface.
	 */
	public boolean failedEventsOption = false;

	/**
	 * Create a diagnostic Petri net with log trace visualization.
	 * @param net the Petri net to be passed to super
	 * @param caseIDs the trace IDs to be passed to super
	 */
	 ...
	 * diagnostic results.
	 * @param copyTemplate The Petri net containing all the diagnostic information
	 * that should be preserved (to be passed to super).
	 */
	...
	/**
	 * Overrides the default visualization to show the log traces instead of the model
	 * (the default visualization of process instances is used internally).
	 ...
	 * ProcessInstance. Note that this is only used for a single trace being selected.
	 * @param bw The writer used by the framework to create the temporary dot file.
	 * @throws IOException If writing to the writer fails.
	 */
	private void writeDefaultVisualization(Writer bw) throws IOException {
		...
		DiagnosticLogTrace visualizedInstance = (DiagnosticLogTrace)this.myReplayedLog.next();
		//DiagnosticLogTrace visualizedInstance = (DiagnosticLogTrace) currentlySelectedInstances.get(0);

		// get audit trail list as arraylist to cast the entries to
		// diagnostic audit trail entries afterwards
		Iterator currentATEListIterator = visualizedInstance.getProcessInstance().
										  getAuditTrailEntryList().iterator();
		// write header
		bw.write("pidata [shape=\"box\",rank=\"source\",label=\"Process Instance Data:");
		Iterator it = visualizedInstance.getProcessInstance().getAttributes().keySet().iterator();
		while (it.hasNext()) {
			String key = (String) it.next();
			bw.write("\\n" + key + " = " +
					(String) visualizedInstance.getProcessInstance().getAttributes().get(key));
		}
		bw.write("\"];\n");
		// write log events
		int i = 0;
		while (currentATEListIterator.hasNext()) {
			AuditTrailEntry ate = (AuditTrailEntry) currentATEListIterator.next();
			DiagnosticAuditTrailEntry ateDiag = visualizedInstance.getAteDiagnostic(ate);
			// only mark failed events if respective option was chosen by user
			if (failedEventsOption == true && (ateDiag.getFailedExecutionValue() == true)) {
				bw.write("ate" + i +
						" [shape=\"box\",style=\"filled\",fillcolor=\"orange1\",label=\"" +
						ate.getElement() +
						"\\n" + ate.getType() + "\\n" +
						(ate.getTimestamp() == null ? "" :
						ate.getTimestamp().toString()) + "\"];\n");
			} else {
				// write normally
	...
	/**
	 * Write a log visualization incorporating all the selected traces.
	 * Note that this is used for more than one traces being selected and the
	...
	public void writeLogVisualization(Writer bw) throws IOException {
		...
		// In the case of failedEventsOption being selected, an alternative method
		// called writeDiagnosticLogVisualization is called as a temporary solution)
		bw.write("digraph G {\nfontsize=\"8\"; \nremincross=true;\n");
		bw.write("fontname=\"Arial\"; \nrankdir=\"LR\"; \n");
		bw.write("edge [arrowsize=\"0.7\",fontname=\"Arial\",fontsize=\"8\"];\n");
		bw.write(
				"node [shape=\"box\",height=\".2\",width=\".2\",fontname=\"Arial\",fontsize=\"8\"];\n");
		// traverse arraylist manually instead of using an iterator
		// to ensure the right order of the traces on the screen (from top to bottom)
		...
		while (myReplayedLog.hasNext()) {
			//DiagnosticLogTrace currentTrace = (DiagnosticLogTrace) currentlySelectedInstances.get(traceCount);
			DiagnosticLogTrace currentTrace = (DiagnosticLogTrace) myReplayedLog.next();

			// write the name of the trace
			bw.write("traceID_" + traceCount + " [color=\"white\",label=\"Log Trace " +
			...
			int eventCount = 0;
			// start trace record
			bw.write("trace_" + traceCount + " [shape=\"record\",label=\"{<s> ");
			while (currentATEListIterator.hasNext()) {
				AuditTrailEntry currentATE = (AuditTrailEntry) currentATEListIterator.next();
				//DiagnosticAuditTrailEntry ateDiag = currentTrace.getAteDiagnostic(currentATE);
				// write box for current audit trail entry (ATE)
				bw.write(currentATE.getElement() + "\\n" + currentATE.getType());
				// prepare next entry only if is not the last element of trace
				if (eventCount !=
						currentTrace.getProcessInstance().getAuditTrailEntryList().size() - 1) {
					bw.write(" | ");
				}
				eventCount++;
			}
	...
	public void writeDiagnosticLogVisualization(Writer bw) throws IOException {
		...
		// write a whole graph for every log trace
		//while (traceCount >= 0) {
		//DiagnosticLogTrace currentTrace = (DiagnosticLogTrace) currentlySelectedInstances.get(
		//								  traceCount);
		while (myReplayedLog.hasNext()) {
			//DiagnosticLogTrace currentTrace = (DiagnosticLogTrace) currentlySelectedInstances.get(traceCount);
			DiagnosticLogTrace currentTrace = (DiagnosticLogTrace) myReplayedLog.next();
			...
			while (currentATEListIterator.hasNext()) {
				...
				// write normally
					...
					bw.write("logEvent_" + traceCount + "_" + eventCount +
							" [shape=\"box\",label=\"" + currentATE.getElement() +
							"\\n" + currentATE.getType() + "\"];\n");
				}
				// connect with header
				if (eventCount == 0) {
					bw.write("traceID" + "_" + traceCount + " -> logEvent_" + traceCount + "_" +
							eventCount + "[color=\"white\"];");
				}
				// connect with previous log event
				else {
					bw.write("logEvent_" + traceCount + "_" + (eventCount - 1) + " -> logEvent_" +
