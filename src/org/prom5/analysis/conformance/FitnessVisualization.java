...
/**
 * Overrides the default visualization methods to include all the
...
public class FitnessVisualization extends DiagnosticPetriNet {
	...
	/**
	 * Represents the visualization option of indicating the token counter results
	 * in the model.
	 ...
	 * that an edge has been passed during log replay.
	 * The public attribute is necessary because the writeToDot() method
	 * has a fixed interface.
	 */
	 ...
	 */
	public FitnessVisualization(PetriNet net, ArrayList caseIDs, DiagnosticLogReader replayedLog) {
	...
	/**
	 * This is the transition writing part of the writeToDot procedure (refer to it
	 * for further information), which gets affected if the
	...
	protected void writeTransitionsToDot(Writer bw) throws IOException {
		...
		if (pathCoverageOption == false && failedTransitionsOption == false) {
			...
			while (it.hasNext()) {
				DiagnosticTransition myDiagnosticTransition = (DiagnosticTransition) (it.next());
				...
				try {
					...
					if ((pathCoverageOption == true) && (failedTransitionsOption == true) &&
							(myDiagnosticTransition.hasFired() == true) &&
							(myDiagnosticTransition.hasFailedExecution() == true)) {
						bw.write("t" + myDiagnosticTransition.getNumber() +
					...
					nodeMapping.put(new String("t" + myDiagnosticTransition.getNumber()),
	 ...
	 * for further information), which gets affected if the
	 * tokenCounterOption evaluates to true.
	 * @param bw The writer used by the framework to create the temporary dot file.
	 * @throws IOException If writing to the writer fails.
	 */
		...
		if (tokenCouterOption == false) {
			...
			try {
				...
				while (it.hasNext()) {
					...
					// in the case that something went wrong, paint the place red
					...
					if ((p.getMissingTokensOnSemanticLevel() > 0) ||
						...
						if ((p.getMissingTokensOnSemanticLevel() > 0) &&
								(p.getRemainingTokensOnSemanticLevel() >
								0)) {
							bw.write("p" + p.getNumber() +
									...
									p.getMissingTokens() +
									"\"];\n");
						} else if ((p.getMissingTokensOnSemanticLevel() >
								0)) {
							// only tokens missing
							bw.write("p" + p.getNumber() +
									" [shape=\"circle\",color=\"red\",style=\"bold\",fontcolor=\"red\",label=\"-" +
	...
	/**
	 * This is the edge writing part of the {@link #writeToDot writeToDot} procedure.
	 * This part of the visualization gets affected as soon as the passedEdgesOption
	 * evaluates to true.
	 * @param bw the writer used by the framework to create the temporary dot file
	 * @throws IOException if writing to the writer fails
	 */
			...
			try {
				...
				while (it.hasNext()) {
					...
					if (e.isPT()) {
						...
						bw.write("t" + t.getNumber() + " -> p" + p.getNumber() + " [label=" +
								e.getNumberOfPasses() + "];\n");
					}
				}
			} catch (Exception ex) {
	...
	/**
	 * This is the finishing part of the writeToDot procedure (refer to it
	 * for further information), which gets affected if the
			...
			try {
				...
				while (it.hasNext()) {
					...
					if (p.getRemainingTokensOnSemanticLevel() > 0) {
						...
						Iterator potentiallyEnabled = p.getSuccessors().iterator();
						String transitions = "";
						while (potentiallyEnabled.hasNext()) {
							DiagnosticTransition currentTransition = (DiagnosticTransition)
									potentiallyEnabled.next();
							if (currentTransition.hasRemainedEnabled()) {
								transitions = transitions + "t" + currentTransition.getNumber() +
											  ";\n";
							}
						}
						// create only visualization of remaining enabled transitions, if there are any
						if (transitions != "") {
							bw.write("subgraph cluster_Place_" + p.getNumber() +
									" {\nstyle=\"filled\";\ncolor=\"lightgrey\";\n");
