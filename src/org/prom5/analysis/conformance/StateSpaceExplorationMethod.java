...
import org.prom5.framework.models.petrinet.algorithms.logReplay.AnalysisMethodEnum;
...
public class StateSpaceExplorationMethod implements AnalysisMethod {
	...
	private StateSpaceExplorationResult myResult; // corresponding analysis result
	private HashMap<LogEvent, HashSet<Transition>> myAlternativeDuplicateTasks; // remaining alternative duplicates
	 ...
	 * Note that this analysis only depends on the model (and the
	 * mapping established between the log and the model) but not on the log.
	...
	/*
	 *  (non-Javadoc)
	 * @see org.processmining.framework.models.petrinet.algorithms.logReplay.AnalysisMethod#analyse(org.processmining.framework.models.petrinet.algorithms.logReplay.AnalysisConfiguration)
	 */
	public AnalysisResult analyse(AnalysisConfiguration analysisOptions) {
		// initialize the result object and the analysis options
		myResult = new StateSpaceExplorationResult(analysisOptions, myPetriNet, this);

		// only do the state space analysis if any of the relevant metrics has been
		// requested
		if (myResult.calculateImprovedBehavioralAppropriateness() == true ||
			...
			// build coverability graph from the initial marking in the cloned Petri net
			PetriNet clonedNet = (PetriNet) myPetriNet.clone();
			// remove all tokens and initialize the petri net with one token in start place
			/// TODO - check whether initial marker also necessary for original PN
			InitialPlaceMarker.mark(clonedNet, 1);
			StateSpace coverabilityGraph = CoverabilityGraphBuilder.build(clonedNet);
			...
			for (ModelGraphVertex endState : coverabilityGraph.getEndNodes()) {
				...
				for (Edge inEdge : endState.getInEdges()) {
					...
					if (associatedTransition.isInvisibleTask() == false) {
						wrongState = false; // if at least one real transition lead to this state, it is legitimate
						break;
					}
				}
				if (wrongState == true) {
			...
			if (myResult.calculateImprovedStructuralAppropriateness() == true) { // configurated
				...
				Iterator<Transition> allTrans = clonedNet.getTransitions().iterator();
				HashMap<LogEvent,
						HashSet<Transition>> mapping = new HashMap<LogEvent, HashSet<Transition>>();
				...
				Iterator<Entry<LogEvent,
						HashSet<Transition>>> mappingIt = mapping.entrySet().iterator();
				while (mappingIt.hasNext()) {
					Entry<LogEvent, HashSet<Transition>> entry = mappingIt.next();
			...
			myProgress.setProgress(1); // otherwise first time it is not visible!

			try {
				// start recursive state space exploration for the first time
				tracePathsInStateSpace(initialSequence, coverabilityGraph, replayState,
						visitedEdges, blockedEdges);
			} catch (Exception e) {
				// TODO - check how this error can be handled in a better way (e.g. dialog)
				Message.add("Analysis aborted by user while traversing the state space. " +
						e.toString());
			} catch (OutOfMemoryError err) {
				// TODO - check how this error can be handled in a better way (e.g. dialog)
				Message.add("Out of Memory while traversing the state space. " + err.toString());
			}

			// fill the missing relation parts with zero entries
			DiagnosticLogEventRelation globalRelation = myResult.getActivityRelations();
			...
			if (myResult.calculateImprovedStructuralAppropriateness() == true) { // configurated
				...
				Iterator<HashSet<Transition>> remainingDupl = myAlternativeDuplicateTasks.values().iterator();
				while (remainingDupl.hasNext()) {
					Iterator<Transition> currentSetDupl = remainingDupl.next().iterator();
					while (currentSetDupl.hasNext()) {
						Transition currentDuplicate = currentSetDupl.next();
						// remember alternative duplicate status at diagnostic transition
	...
	/**
	 * Recursive helper method for tracing different execution sequences of tasks
	 * in the process model (state space).
	 * <br>
	 * The purpose is an exhaustive exploration of the state space (without being
	 * lost in infinite cycles) and Measurement steps can be injected.
	 *
	 * @param currentPath the firing sequence traced so far
	 * @param coverabilityGraph the state space to traverse
	 * @param currentState the current state of the traversal
	 * @param passedEdges contains the set of edges that were already followed
	 * (in order to prevent infinite cycles)
	 * @param blockedEdges contains the set of edges that should not be followed
	...
	protected void tracePathsInStateSpace(ArrayList<Transition> currentPath,
			...
			State currentState, HashSet<ModelGraphEdge> passedEdges,
			HashSet<ModelGraphEdge> blockedEdges) throws Exception {
		
//		Message.add("Path " + temp + " - State " + currentState.getIdentifier() + " " + currentState.getLabel(), Message.DEBUG);
//		System.out.println("Path " + temp + " - Current path length: " + currentPath.size() + " - Blocked Edges: " + blockedEdges.size() + 
//				"Passed Edges: " + passedEdges.size());

		// abort if user cancelled the log replay
		if (myProgress.isCanceled() == true) {
			throw new Exception("State Space traversal aborted while Processing Path No." + temp);
			...
			while (outgoingEdges.hasNext()) {
				...
				// only follow an edge that has not been followed before in order to prevent infinite loops
				if (passedEdges.contains(currentEdge) == false) {
					noPathTracable = false;
					// remember to prevent infinite loops
					HashSet<ModelGraphEdge>
							extendedPassedEdges = new HashSet<ModelGraphEdge>(passedEdges);
					extendedPassedEdges.add(currentEdge);
					// remember firing sequence so far
					ArrayList<Transition>
							extendedFiringSequence = new ArrayList<Transition>(currentPath);
					extendedFiringSequence.add(associatedTransition);
					State nextState = (State) currentEdge.getDest();
					// recursive call
					tracePathsInStateSpace(extendedFiringSequence, coverabilityGraph, nextState,
							extendedPassedEdges, blockedEdges);
				}
				// second check: every edge is traced at most 2 times! (afterwards it becomes blocked)
				else if (blockedEdges.contains(currentEdge) == false) {
					noPathTracable = false;
					// remember to prevent infinite loops
					HashSet<ModelGraphEdge>
							extendedBlockedEdges = new HashSet<ModelGraphEdge>(blockedEdges);
					...
					// recursive call
					tracePathsInStateSpace(extendedFiringSequence, coverabilityGraph, nextState,
							passedEdges, extendedBlockedEdges);
				}
			}
			if (noPathTracable == true) { // is the same as if there were no further outedges in this state
	...
	/**
	 * Builds up the log event relations for the given execution
	 * sequence. Also updates the global relations at the analysis
	 * result object.
	 * @param path the firing sequence derived from the process model
	 */
	protected void evaluatePath(ArrayList<Transition> path) {
		...
		while (transitions.hasNext()) {
			...
			if (le != null) {
				leRelation.addDiagnosticLogEvent(le.getModelElementName(), le.getEventType());
		...
		DiagnosticLogEventRelation globalRelation = myResult.getActivityRelations();
		Iterator<DiagnosticLogEvent> it = leRelation.getDiagnosticLogEvents().iterator();
		while (it.hasNext()) {
	...
	/**
	 * Helper method in order to detect non-alternative duplicate tasks and remove them from
	 * the set of duplicate tasks (such that in the end only alternative duplicate tasks
	 * remain in that list.
	 * @param path the whole model execution path
	 * @param trans the transition currently being checked
	 */
	protected void checkForAlternativeDuplicateTasks(ArrayList<Transition> path, Transition trans) {
		if (myResult.calculateImprovedStructuralAppropriateness() == true) { // configurated
			LogEvent le = trans.getLogEvent();
			...
			if (dupl != null) {
				...
				while (duplIt.hasNext()) {
					...
					if (currentDupl != trans && path.contains(currentDupl) == true) {
