...
public class StateSpaceExplorationResult extends AnalysisResult {
	...
	public PetriNet inputPetriNet; /** The original Petri net passed to the analysis method. */
	public DiagnosticPetriNet exploredPetriNet; /** The Petri net enhanced with diagnostic information. */
	 ...
	 * (and their relations) in the the process model.
	...
	/**
	 * Contains the number of redundant invisible tasks that have been encountered
	 * during the state space exploration for the improved structural appropriateness method.
	 */
	private int numberOfRedundantInvisibles = 0;

	/**
	 * Contains the number of alternative duplicate tasks that have been encountered
	 * during the state space exploration for the improved structural appropriateness method.
	 */
	...
	/**
	 * Contains the number of sometimes relations (forwards) in the log and in the model,
	 * i.e., represents already the intersection of SFm and SFl
	 * (needed for calculating advanced behavioral appropriateness).
	 */
	...
	/**
	 * Contains the number of sometimes relations (backwards) in the log and in the model,
	 * i.e., represents already the intersection of SFm and SFl
	 * (needed for calculating advanced behavioral appropriateness).
	 */
	...
	/**
	 *
	 * @param analysisOptions AnalysisConfiguration
	 * @param net PetriNet
	 * @param method StructuralAnalysisMethod
	 */
	public StateSpaceExplorationResult(AnalysisConfiguration analysisOptions, PetriNet net,
			StateSpaceExplorationMethod method) {
		super(analysisOptions);

		inputPetriNet = net;
		// init diagnostic data structures
		exploredPetriNet = new BehAppropriatenessVisualization(inputPetriNet, new ArrayList(), null);

		if (analysisOptions != null) {
			// retrieve the options
			ArrayList<AnalysisConfiguration>
					configurationOptions = analysisOptions.getEnabledOptionsForAnalysisMethod(new
										   ArrayList<AnalysisConfiguration>(), method.getIdentifier());
	...
	public float getImprovedStructuralAppropriatenessMeasure() {
		float totalNumberOfTasks = (float) inputPetriNet.getTransitions().size();
		...
		// TODO: make these figures available in the GUI rather than writing them to the console
		Message.add("Total Number of tasks in model: "+totalNumberOfTasks, 3);
		Message.add("Number of redundant invisible tasks in the model: "+redundantInvisibles, 3);
		Message.add("Number of alternative duplicate tasks in the model: "+numberOfAlternativeDuplicates, 3);
		
		if (totalNumberOfTasks > 0) {
			// calculate metric
			return (totalNumberOfTasks - (redundantInvisibles +
					alternativeDuplicates)) / totalNumberOfTasks;
		} else {
			Message.add(
					"The Improved Structural appropriateness metric aaS can only be calculated for " +
	...
	public float getImprovedBehavioralAppropriatenessMeasure() {
		...
		// TODO - make additional views for the model and the log relations that were derived
		Message.add("No. Elements in 'Sometimes Follows' relation of the model: "+numberOfSometimesRelationForwardElementsInModel, 3);
		Message.add("No. Elements in (intersection with) 'Sometimes Follows' relation of the log: "+numberOfSometimesRelationForwardElementsInLog, 3);
		Message.add("No. Elements in 'Sometimes Precedes' relation of the model: "+numberOfSometimesRelationBackwardsElementsInModel, 3);
		Message.add("No. Elements in (intersection with) 'Sometimes Precedes' relation of the log: "+numberOfSometimesRelationBackwardsElementsInLog, 3);
	...
	public float getDegreeOfModelFlexibility() {
		// get |L| from log event relation (is normalized to elements in model relation)
		int L = logEvents.getDiagnosticLogEvents().size();
		// because of artificial start and end certain fields can never be in Sometimes relation
		...
		if (max > 0) {
			...
			Message.add("The degree of model flexibility can only be calculated for " +
	...
	/**
	 * Returns the set of activity relations that have been derived from the state space of the process
	 * model. They still need to be matched with the relations derived for the log events from the
	 * log, and their difference is to be recorded in the diagnostic petri net.
	 * @return the table containing the name of the log events and the diagnostic log event itself
	 */
	...
	public void incSFModel() {
		numberOfSometimesRelationForwardElementsInModel =
				numberOfSometimesRelationForwardElementsInModel + 1;
	}

	/**
	 * Increments the number of sometimes relations (forwards) in the log and in the model,
	 * i.e., represents already the intersection of SFm and SFl
	 * (needed for calculating advanced behavioral appropriateness).
	 */
	public void incSFLog() {
		numberOfSometimesRelationForwardElementsInLog =
				numberOfSometimesRelationForwardElementsInLog + 1;
	}
	...
	public void incSBModel() {
		numberOfSometimesRelationBackwardsElementsInModel =
				numberOfSometimesRelationBackwardsElementsInModel + 1;
	}

	/**
	 * Increments the number of sometimes relations (backwards) in the log and in the model,
	 * i.e., represents already the intersection of SFm and SFl
	 * (needed for calculating advanced behavioral appropriateness).
	 */
	public void incSBLog() {
		numberOfSometimesRelationBackwardsElementsInLog =
				numberOfSometimesRelationBackwardsElementsInLog + 1;
	}
	...
	private void setVisualizationState(DisplayState newVisualization) {
		...
		if (newVisualization.toString().equals(DisplayState.BEHAVIORAL.toString())) {
			// only change if state is new
			if (getCurrentVisualizationState().equals(newVisualization.toString()) == false) {
				exploredPetriNet = new BehAppropriatenessVisualization((DiagnosticPetriNet)
								   exploredPetriNet);
			}
			// but set always the current options state
			BehAppropriatenessVisualization currentVisualization = (BehAppropriatenessVisualization)
					exploredPetriNet;
	...
	public GrappaPanel getVisualization(DisplayState currentVisualization) {
		...
		myResultVisualization = ((DiagnosticPetriNet) exploredPetriNet).getGrappaVisualization();
