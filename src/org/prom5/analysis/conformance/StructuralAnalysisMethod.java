...
public class StructuralAnalysisMethod implements AnalysisMethod {
	 ...
	 * Note that the structural analysis only depends on the model (and the
	 ...
	 * @param analysisOptions the given analysis configuration
	 * @return a result object of type {@link StructuralAnalysisResult StructuralAnalysisResult}
	 */
	public AnalysisResult analyse(AnalysisConfiguration analysisOptions) {
		// initialize the result object and the analysis options
		myResult = new StructuralAnalysisResult(analysisOptions, myPetriNet, this);

		// determine redundant invisible task using the Petri Net reduction in ProM
		if (myResult.calculateImprovedStructuralAppropriateness() == true) { // configurated

			/// start monitoring progress of log replay
			Progress progress = new Progress("Checking for redundant tasks...", 0, 2);
			progress.setProgress(0);

			// TODO - check whether identifier can be assumed to be equal instead (numbers don't work
			// as it is not preserved during reduction technique!
			// give unique number for each transition in order to find them back
			// (after reduction equals method might yield different result for formerly equal tasks)
			...
			// remember all invisible tasks to check for their presence in the reduced net later on
			ArrayList<Transition> allInvisibles = myPetriNet.getInvisibleTasks();

			// only if there are invisible tasks at all
			if (allInvisibles.size() > 0) {

				progress.setProgress(1); // otherwise first time it is not visible!
				...
				// clone the input Petri net in order to keep the structure of it
				...
				Iterator<Transition>
						invisiblesInReducedNet = reducedNet.getInvisibleTasks().iterator();
				while (invisiblesInReducedNet.hasNext()) {
					Transition currentNonRedundant = invisiblesInReducedNet.next();
					Iterator<Transition> potentiallyRedundantInvisibles = allInvisibles.iterator();
					while (potentiallyRedundantInvisibles.hasNext()) {
						Transition currentCandidate = potentiallyRedundantInvisibles.next();
						if (currentCandidate.getIdentifier().equals(currentNonRedundant.
								getIdentifier()) == true) {
				...
				// add all the remaining invisibles to the result object as they are redundant
				// (have been removed in reduction!)
				Iterator<Transition> remainingRedundants = allInvisibles.iterator();
				while (remainingRedundants.hasNext()) {
					myResult.addRedundantInvisibleTask(remainingRedundants.next());
				}
