 ...
 * Can be used to retrieve values for the implemented metrics.
...
public class StructuralAnalysisResult extends AnalysisResult {
	...
	public PetriNet reducedPetriNet; /** The Petri net reduced by redundant invisible tasks */
	public DiagnosticPetriNet analyzedPetriNet; /** The Petri net enhanced with diagnostic information. */
	 ...
	 * @param analysisOptions the option chosen by the user
	 * @param net the Petri net to be analyzed
	 * @param method the corresponding analysis method
	 */
	public StructuralAnalysisResult(AnalysisConfiguration analysisOptions, PetriNet net,
			StructuralAnalysisMethod method) {
		super(analysisOptions);
		inputPetriNet = net;
		
		// init diagnostic data structures
		analyzedPetriNet = new StrAppropriatenessVisualization(inputPetriNet, new ArrayList(), null);
	...
	public float getStructuralAppropriatenessMeasure() {
		float numberOfNonDuplicates = inputPetriNet.getNumberOfNonDuplicateTasks();
		...
		if (numberOfNodes > 0) {
			...
			Message.add("The structural appropriateness metric aS cannot be calculated for " +
	...
	/**
	 * Retrieves the list of redundant invisible tasks that have detected
	 * by the reduction rule analysis in ProM.
	...
	/**
	 * Retrieves the reduced Petri Net in case there were
	 * redundant invisible tasks found in the model.
	 * @return the reduced Petri Net if there were redundant invisible
	 * tasks found in the model, <code>null</code> otherwise
	 */
	...
	/**
	 * Adds the given invisible transition to the list of redundant invisible tasks
	 * for this petri net.
	 * @param trans the transition to be added as a redundant invisible one
	 */
	 ...
	 * @param reducedNet the reduced Petri net
	 */
	...
	private void setVisualizationState(DisplayState newVisualization) {
		...
		if (newVisualization.toString().equals(DisplayState.STRUCTURAL.toString())) {
			// only change if state is new
			if (getCurrentVisualizationState().equals(newVisualization.toString()) == false) {
				analyzedPetriNet = new StrAppropriatenessVisualization((DiagnosticPetriNet)
						analyzedPetriNet);
			}
			// but set always the current options state
			StrAppropriatenessVisualization currentVisualization = (StrAppropriatenessVisualization)
					analyzedPetriNet;
			currentVisualization.redundantInvisiblesOption = newVisualization.
					redundantInvisiblesOption;
			currentVisualization.alternativeDuplicatesOption = newVisualization.
	...
	public GrappaPanel getVisualization(DisplayState currentVisualization) {
		...
		myResultVisualization = ((DiagnosticPetriNet) analyzedPetriNet).getGrappaVisualization();
