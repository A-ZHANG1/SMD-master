...
/**
 * Class holding the machine learning settings and performing
 * the actual data mining tasks (with the help of the weka library).
 *
...
public abstract class DecisionAnalyser {
	 ...
	 */
	public void analyse(List<DecisionPoint> decisionPoints, DecisionMiningLogReader log,
		...
		while (allDecisionPoints.hasNext()) {
			...
			while (categoryIterator.hasNext()) {
				...
				while (traceIterator.hasNext()) {
					...
					Instance instance = trace.makeInstance(data, attributeInfo.size(), branch, log, currentDP.getContext());
					// classify instance
			...
			if (data.numInstances() == 0) {
				currentDP.getContext().setResultViewPanel(createMessagePanel(
						"No learning instances available"));
			}
			// actually solve the classification problem
				...
				try {
					...
					currentDP.getContext().setEvaluationViewPanel(createEvaluationVisualization(
							data));
					...
					ex.printStackTrace();
					currentDP.getContext().setResultViewPanel(createMessagePanel(
							"Error while solving the classification problem"));
				}
			}
		}
	}
	...
	/**
	 * Specify the name of this algorithm to be displayed in the combo box of the
	 * algorithm view.
	 ...
	 * available for this type of decision analyser (i.e., the used algorithm).
	 * @return the parameters panel to be displayed in the algorithm settings of the decsion miner
	 */
	 ...
	 * Note that the visualization is not re-created but only adjusted,
	 * e.g., with respect to positioning graphical components which cannot
	 * be done before the result panel is actually packed to the frame.
