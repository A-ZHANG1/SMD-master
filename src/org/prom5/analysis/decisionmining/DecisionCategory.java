...
public class DecisionCategory {
	...
	/** The list of log events characterizing this decision category
	...
	/** The expression object representing the overall decision rule for this branch */
	 ...
	 * @param parent the belonging decisin point
	 * @param ID the local ID with respect to the belonging decision point
	 * @param logEvents the log events associated to this category
	 * @param target the target node of the data dependency associated to this decision category
	 * (necessary for exporting a simulation model to CPN)
	 */
	public DecisionCategory(DecisionPoint parent, int ID, ArrayList<LogEvent> logEvents,
	...
	/**
	 * Retrieves the target node of the data dependency associated to this decision category
	 * (i.e., the first transition on this alternative branch).
	...
	/**
	 * Creates a compound ID of the form "ParentDecisionPointID.DecisionCategoryID",
	 * which can be, e.g., used for serialization.
	 ...
	 * @return a descriptive string containing the ID and the associated log events
	 * for this decision category
	 */
	...
	public String getBranchSpecification() {
		...
		for (LogEvent current : myLogEvents) {
			result = result + current.getModelElementName() + "/" + current.getEventType() + " ";
		}
		result = result + "}";
		return result;
	}
	 ...
	 * @return <code>false</code> if there is at least one log event assigned
	 * to this category, <code>true</code> otherwise
	 */
	 ...
	 * category.
	 * @param toCompare the log event to be compared with the assigned log events
	 * @return <code>true</code> if there could be a log event found in the list that
	 * is equal to the given one, <code>false</code> otherwise
	 */
	 ...
	 * If the first task in an alternative branch is an invisible or duplicate task
	 * it must be traced and there may be more than one log event associated.
	 * In this case each of the log events indicates the occurrence of this decision class.
	...
	/**
	 * Adds the given sub rule to the expression tree (eventually, 
	 * all sub rules will be connected by an OR operator). 
	 * @param rule the sub rule to be added to the expression
	 */
	 ...
	 * This method should be called as soon as new rules have been calculated and
	 * should be updated.
	 */
	...
	/**
	 * Returns the decision rule for this branch as a high level 
	 * data expression. 
