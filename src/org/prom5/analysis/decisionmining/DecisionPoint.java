...
import org.prom5.framework.models.hlprocess.expr.operator.HLNotEqualOperator;
...
public abstract class DecisionPoint {
	...
	/** Represents the concept to be learned (i.e., all the alternative paths that can be taken).
	 * Each category is specified by a list of log events determining that the corresponding
	...
	/** ID counter for the associated decision categories
	 * (i.e., children of this decision point) */
	private int myCategoryCounter = 1;

	/** the enclosing analysis result frame */
	private DecisionPointAnalysisResult myParent;

	/** the node reflecting this choice construct
	 * (i.e., the source node for all data dependencies discovered for this decision point) */
	protected ModelGraphVertex myNode; // TODO check how this can be aligned with the place stored for a DecisionPointPetriNet
	
	/**
	 * Creates an empty decision point.
	 * @param name the name of the decision point
	 * @param parent the enclosing parent analysis result (providing access to the model
	 * and the log being analysed)
	 */
	 ...
	 * That is, the list of alternative branches that can be taken from here (each containing
	 * another list of log events which all indicate that a certain path has been taken).
	 ...
	 * That is, an enumeration type (i.e., a nominal one) listing the name of the log event
	 * associated to each of the paths.
	 * Note that empty categories are not treated properly yet (future work).
	 * @return the nominal weka attribute representing the target concept for this decision point
	 */
	...
	/**
	 * Retrieves the enclosing parent analysis result (providing access to the model
	 * and the log being analysed).
	 ...
	 * @param index the global ID of the decision class to be found
	 ...
	 * @param logEvents the list of log events representing the new category
	 * to be added to this target concept
	 */
	public void addTargetCategory(ArrayList logEvents, ModelGraphVertex targetNode) {
		DecisionCategory newCategory = new DecisionCategory(this, myCategoryCounter, logEvents,
									   targetNode);
	 ...
	 * @param tree the decision tree in prefix notation
	...
	public void setDataDependencies(String tree, HLPetriNet highLevelPN) {
		// TODO - in general: better retrieve decision rules (i.e., data dependencies) directly
		...
		try {
			// split the prefix notation tree into branches (corresponds to no. of rules)
			...
			if (endIndex > 0) {
				...
				String rootAttribute = first.substring(startIndex + 1, endIndex);
				...
				expressions.add(createAttributeExpression(rootAttribute, first.substring(endIndex + 1), hlProcess));
	
				// recursively parse prefix notation of decision tree and assign decision rules
				// to respective decision category
				followRules(prefixFragments, 1, rootAttribute, expressions, null, hlProcess);
	
				// create DataDependencies
				for (DecisionCategory current : myTargetConcept) {
					// get the expression related to this dependency
					// (can be converted to CPN guard condition notation from there)
					HLDataExpression expression = current.getDataExpression();
					// find target node in high level model
					ModelGraphVertex targetNodeSimPN = pn.findTransition((Transition) current.getTargetNode());
					HLActivity targetAct = highLevelPN.findActivity(targetNodeSimPN);
					HLCondition condition = choice.getCondition(targetAct.getID());
	...
	/**
	 * Recursive helper method to parse the decision rules from the weka decision tree.
	 * @param fragments contains all the fragments from the prefix notation
	 * @param pos indicates the current fragment being processed
	 * @param currentAttribute the decision attribute that is currently traced
	 * @param expressions the list of rule fragments (currently traced)
	 * @param exprPrefix the started rule fragment to be be expanded
	 * @param hlProcess the high level process to retrieve attribute objects
	 * @return the index to be processed next
	 */
	private int followRules(String[] fragments, int pos, String currentAttribute,
			ArrayList<HLExpressionElement> expressions, HLExpressionElement exprPrefix, HLProcess hlProcess) {
		// process fragment at current position
		int i = pos;
		while (i < fragments.length) {
			HLExpressionElement expression;
			// check whether represents last branch on that level
			// --> "[" indicates that now the leafs (or subtrees) are listed for all branches before
			if (fragments[i].indexOf("[") >= 0) { // -1 is returned if "[" is not contained
				// make rule from attribute value at the beginning of this fragment
				int splitIndex = fragments[i].indexOf("[");
				String attributeValue = fragments[i].substring(0, splitIndex);
				if (exprPrefix == null) {
					expression = createAttributeExpression(currentAttribute, attributeValue, hlProcess);
				} else {
					// connect to previous rules
					HLExpressionElement newExpr = createAttributeExpression(currentAttribute, attributeValue, hlProcess);
					expression = concatExpressions(exprPrefix, newExpr);
				}
				expressions.add(expression);

				// Note: according to the prefix format from weka the number of started rules matches the number
				// of leafs or nodes now listed after the last branch attribute value
				Iterator<HLExpressionElement> exIt = expressions.iterator();
				// remove the attribute value at the beginning of this fragment and the first "["
				fragments[i] = fragments[i].substring(splitIndex + 1);
				while (exIt.hasNext()) {
					HLExpressionElement currentExpr = exIt.next();
					// if next child node is new attribute test: evaluate subtree
					// (substring cannot contain "]" in that case)
					if (fragments[i].indexOf("]") < 0) { // -1 is returned if "]" is not contained
						// determine new attribute name from start to before ":"
						int colonIndex = fragments[i].indexOf(":");
						String newAttribute = fragments[i].substring(0, colonIndex);
						// determine first attribute value from after ":" until end
						String attValue = fragments[i].substring(colonIndex + 1);
						...
						HLDataExpression dataExpr = (HLDataExpression) new HLDataExpression(currentExpr).clone();
						HLExpressionElement clonedCurrentExpr = dataExpr.getRootExpressionElement();
						// now combine
						expandedExpression.addSubExpression(clonedCurrentExpr.getExpressionNode());
						HLExpressionElement newExpression = createAttributeExpression(newAttribute, attValue, hlProcess);
						expandedExpression.addSubExpression(newExpression.getExpressionNode());
						expandedExpressions.add(expandedExpression);

						// start recursive procedure
						i = followRules(fragments, pos + 1, newAttribute, expandedExpressions, currentExpr, hlProcess);
					}
					// if not: add current rule to belonging decision category
						...
						// start reading decision class index after "Branch" und stop before "{"
						String classID = fragments[i].substring(6, curlyIndex - 1);
	...
	/**
	 * Concatenates the given expressions with a new AND connector only if the prefix is not already an 
	 * AND connector (in which case the new expression is directly added as a new child. 
	 * @param prefix the existing expression to which it should be connected as AND
	 * @param newExpr the new expression to be concatenated
	 * @return the combined expression
	 */
	private HLExpressionElement concatExpressions(HLExpressionElement prefix, HLExpressionElement newExpr) {
		if (prefix instanceof HLAndOperator) {
			...
			HLDataExpression dataExpr = (HLDataExpression) new HLDataExpression(prefix).clone();
			HLExpressionElement clonedCurrentExpr = dataExpr.getRootExpressionElement();
			andConnector.addSubExpression(clonedCurrentExpr.getExpressionNode());
	...
	/**
	 * Creates a sub expression referring to the given attribute and parsing the operator and 
	 * value from the given String. 
	 * @param attName the name of the attribute in the high level process (will connect first matching one)
	 * @param operatorAndValue assumes that operator and value are separated by a space, such as
	 * "> 500".
	 * @return the operator expression element (corresponding tree node will have sub nodes 
	 * referring to the attribute and the value)
	 */
	private HLExpressionElement createAttributeExpression(String attName, String operatorAndValue, HLProcess hlProcess) {
		...
		HLAttributeOperand attOperand = new HLAttributeOperand(att.getID(), hlProcess);
	...
	/**
	 * Removes existing data dependencies from all decision categories.
	 * This happens as soon as new rules have been calculated and
	 * should be updated.
	 */
