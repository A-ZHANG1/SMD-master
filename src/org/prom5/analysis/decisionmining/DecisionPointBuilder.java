...
public interface DecisionPointBuilder {
	 ...
	 * If no decision points have been determined yet, the list will be built before.
	 * @return a list containing all decision points belonging to this model
	 */
	public List<DecisionPoint> getDecisionPoints();

	/**
	 * Retrieves the belonging log events of all directly preceding tasks
	 * with respect to the given decision point.
	 * Note that invisible tasks are transparently traced, without being counted
	 * as directly adjacent.
	 * @param refPoint the decision point for which the preceding tasks are to be found
	 * @return a set of nodes belonging to the directly preceding tasks in the model
	 */
	public Set getDirectPredecessors(DecisionPoint refPoint);

	/**
	 * Retrieves the belonging log events of all preceding tasks with
	 * respect to the given decision point.
	 * Note that invisible tasks do not have a log event associated and are not
