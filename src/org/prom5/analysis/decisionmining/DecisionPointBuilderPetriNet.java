...
public class DecisionPointBuilderPetriNet implements DecisionPointBuilder {
	...
	/** An internal mapping from each decision point to a place in the model
	 * (key = DecisionPoint, value = Place). */
	 ...
	 * @param model the given Petri net model
	 * @param parent the enclosing parent analysis result
	 */
		...
		if (myDecisionPointMapping == null || myDecisionPointList == null) {
			...
			while (allPlaces.hasNext()) {
				...
				if (place.outDegree() > 1) {
					DecisionPoint decisionPoint = new DecisionPointPetriNet(place.getIdentifier(),
												  myParent, myModel, place);
					...
					while (outgoingArcs.hasNext()) {
						...
						if (destTransition.isInvisibleTask() == true ||
							...
							// trace until visible and non-duplicate tasks have been found
							targetCategory = traceTargetCategory(destTransition);
						} else {
							LogEvent logEvent = destTransition.getLogEvent();
							// although only one element, representation is list of disjunctions
							targetCategory = new ArrayList();
							targetCategory.add(logEvent);
						}

						// only if could be successfully specified (otherwise discard that branch from the analysis)
						if (targetCategory.size() > 0) {
							decisionPoint.addTargetCategory(targetCategory, destTransition);
						}
					}
		...
		if (p != null) {
			...
			while (directPredecessors.hasNext()) {
				...
				if (predecessor.isInvisibleTask() == true) {
					...
					Set recursiveList = traceForNextVisiblePredecessors(predecessor, new HashSet());
	...
	public Set getAllPredecessors(DecisionPoint refPoint) {
		...
		if (p != null) {
			...
			while (directPredecessors.hasNext()) {
				...
				Set recursiveList = traceForAllVisiblePredecessors(predecessor, new HashSet());
				resultList.addAll(recursiveList);
			}
		}
		return resultList;
	...
	/**
	 * Traces the succesors of an invisible task to determine the succeeding visible tasks.
	 * Note that the search stops if an OR-join has been reached, and that the nature of
	 * the succeeding splits does not matter (i.e., succeeding parallel and alternative tasks
	 * are treated in the same way).
	 * @param invisibleTask the task determining the staring point for the recursive search
	 * @return a list of log events that can be understood as a disjunctive definition of
	 * the corresponding target category (i.e., the occurrence of any of them means that
	 * one specific alternative branch has been taken)
	 */
	private ArrayList traceTargetCategory(Transition invisibleTask) {
		...
		while (nextPlaces.hasNext()) {
			...
			// stop if OR-join has been encountered (temporary simplification: no loops)
			if (successorPlace.inDegree() == 1) {
				Iterator nextTransitions = successorPlace.getSuccessors().iterator();
				while (nextTransitions.hasNext()) {
					Transition successorTransition = (Transition) nextTransitions.next();
	 ...
	 * Note that invisible tasks are transparently traced until the next adjacent visible tasks
	 * have been found (they are not represented in the list as they do not have a log event associated).
	 * @param refTrans the reference transition for the predecessor search
	 * @param alreadyVisited a set of those transitions that have been traced already
	 * (in order to prevent infinite loops in the case the process model is cyclic)
	 * @return the set of all (visible) tasks directly prededing the given reference transition
	 */
	private Set traceForNextVisiblePredecessors(Transition refTrans, HashSet alreadyVisited) {
		HashSet resultList = new HashSet();
		...
		if (alreadyVisited.contains(refTrans) == false) {
			...
			while (allPrecedingPlaces.hasNext()) {
				...
				Iterator allPrecedingTransitions = precedingPlace.getPredecessors().iterator();
				while (allPrecedingTransitions.hasNext()) {
					Transition precedingTransition = (Transition) allPrecedingTransitions.next();
					if (precedingTransition.isInvisibleTask() == true) {
						Set recursiveList = traceForNextVisiblePredecessors(precedingTransition,
											alreadyVisited);
						resultList.addAll(recursiveList);
					} else {
	...
	/**
	 * Recursively retrieves all predecessors or the given transition.
	 * Note that invisible tasks are transparently traced (and not represented in the list as they
	 * do not have a log event associated).
	 * @param refTrans the reference transition for the predecessor search
	 * @param alreadyVisited a set of those transitions that have been traced already
	 * (in order to prevent infinite loops in the case the process model is cyclic)
	 * @return the set of all (visible) tasks prededing the given reference transition
	 */
	private Set traceForAllVisiblePredecessors(Transition refTrans, HashSet alreadyVisited) {
		HashSet resultList = new HashSet();
		// prevent infinite loops
		if (alreadyVisited.contains(refTrans) == false) {
			alreadyVisited.add(refTrans);
			...
			while (allPrecedingPlaces.hasNext()) {
				...
				while (allPrecedingTransitions.hasNext()) {
					...
					// further trace backwards
					Set recursiveList = traceForAllVisiblePredecessors(precedingTransition,
										alreadyVisited);
					resultList.addAll(recursiveList);
				}
			}
