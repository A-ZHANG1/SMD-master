...
/**
 * A decision point context for Petri nets offers the attributes according different selection
 * scopes within a Petri net mode.
 *
...
public class DecisionPointContextPetriNet implements DecisionPointContext {
	...
	/**
	 * The set of relevant DecisionAttribute objects (according to the chosen selection scope).
	 */
	private HashSet myRelevantAttribues;

	/**
	 * Default constructor.
	 * @param decisionPoint the belonging decision point
	 */
	...
	public JPanel getAttributesViewPanel() {
		// TODO: currently not the best solution - it must be rebuilt every time since the panel created by
		// the decision attribute cannot be used by multiple panels at the same time
	...
	/*
	 * (non-Javadoc)
	 * @see org.processmining.analysis.decisionmining.DecisionPointContext#getModelViewPanel()
	 */
	...
	/*
	 * (non-Javadoc)
	 * @see org.processmining.analysis.decisionmining.DecisionPointContext#getResultViewPanel()
	 */
	...
	/*
	 * (non-Javadoc)
	 * @see org.processmining.analysis.decisionmining.DecisionPointContext#setResultViewPanel(javax.swing.JPanel)
	 */
	...
	/*
	 * (non-Javadoc)
	 * @see org.processmining.analysis.decisionmining.DecisionPointContext#getEvaluationViewPanel()
	 */
	...
	/*
	 * (non-Javadoc)
	 * @see org.processmining.analysis.decisionmining.DecisionPointContext#setEvaluationViewPanel(javax.swing.JPanel)
	 */
	...
	/*
	 * (non-Javadoc)
	 * @see org.processmining.analysis.decisionmining.DecisionPointContext#getAttributeInfo()
	 */
	public FastVector getAttributeInfo() {
		...
		while (attributesIterator.hasNext()) {
			DecisionAttribute currentAtt = (DecisionAttribute) attributesIterator.next();
		...
		// add target concept
		Attribute targetConcept = myDecisionPoint.getTargetConceptAsWekaAttribute();
	...
	/*
	 * (non-Javadoc)
	 * @see org.processmining.analysis.decisionmining.DecisionPointContext#getAttributeSelectionScope()
	 */
	...
	private void buildAttributesPanel() {
		...
		myAttributesViewPanel.setLayout(new BoxLayout(myAttributesViewPanel, BoxLayout.X_AXIS));

		attributesContentPanel = new JPanel();
		attributesContentPanel.setLayout(new BoxLayout(attributesContentPanel, BoxLayout.Y_AXIS));

		JPanel selScopeManualPanel = new JPanel();
		selScopeManualPanel.setLayout(new BoxLayout(selScopeManualPanel, BoxLayout.X_AXIS));
		String scopeDescription = new String("Please choose the desired attribute selection scope. The selection scope determines which attributes are used for analysis: \n" +
		...
		Iterator<AttributeSelectionScope> possibleScopes = AttributeSelectionScope.getValues(myAttributeSelectionScope).iterator();
		while (possibleScopes.hasNext()) {
			AttributeSelectionScope scope = possibleScopes.next();
			attributeScopeComboBox.addItem(scope);
		}
		attributeScopeComboBox.setMaximumSize(attributeScopeComboBox.getMinimumSize());
		...
		attributeScopeComboBox.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				AttributeSelectionScope newSelectionType = (AttributeSelectionScope) cb.getSelectedItem();
				// only redraw if selection scope has actually changed
				if (myAttributeSelectionScope != newSelectionType) {
					// use global attribute as the same update procedure is also called in case a new
		...
		attributesManualPanel.setLayout(new BoxLayout(attributesManualPanel, BoxLayout.X_AXIS));
		...
		GuiPropertyStringTextarea attributesManual = new GuiPropertyStringTextarea(attDescription);
		...
		mySelectionPanel.setLayout(new BoxLayout(mySelectionPanel, BoxLayout.Y_AXIS));
		...
		myAttributesViewPanel.add(Box.createHorizontalGlue());
		myAttributesViewPanel.setBorder(BorderFactory.createEmptyBorder(15, 15, 15, 15));
	}

	/**
	 * Retrieves all those tasks that are contained within the currently selected attribute selection scope.
	...
	private ArrayList getTasksWithinAttributeSelectionScope() {
		...
		if (myAttributeSelectionScope == AttributeSelectionScope.WHOLE_CASE) {
			// current decision point does not matter if selection scope is "whole log"
			resultList = myPetriNetModel.getTransitions();
		} else if (myAttributeSelectionScope == AttributeSelectionScope.JUST_BEFORE) {
			Iterator direclyPrecedingTransitions = myDecisionPoint.getParent().getModelAnalyzer().
					getDirectPredecessors(myDecisionPoint).iterator();
			while (direclyPrecedingTransitions.hasNext()) {
				Transition trans = (Transition) direclyPrecedingTransitions.next();
				resultList.add(trans);
			}
		} else if (myAttributeSelectionScope == AttributeSelectionScope.ALL_BEFORE) {
			Iterator allPrecedingTransitions = myDecisionPoint.getParent().getModelAnalyzer().
											   getAllPredecessors(myDecisionPoint).iterator();
	...
	/**
	 * Updates the attributes offered for selection after the selection scope has been changed.
	 */
...
//		 todo: solve without this boolean flag (seperate methods differently)
		...
		Iterator allRelevantTasks = getTasksWithinAttributeSelectionScope().iterator();
		while (allRelevantTasks.hasNext()) {
			...
			Iterator<DecisionAttribute> attributesCurrentTrans = myDecisionPoint.getParent().getLogAnalyzer().
											  getAttributesForLogEvent(trans.getLogEvent()).
											  iterator();
		...
		if (myAttributeSelectionScope == AttributeSelectionScope.WHOLE_CASE) {
			Iterator<DecisionAttribute> globalAtts = myDecisionPoint.getParent().getLogAnalyzer().getGlobalAttributes().iterator();
		...
		if (myVizModel.attributeSelectionScope != null) {
			// collect the tasks belonging to the current attribute selection scope
	...
	private void buildModelPanel() {
		...
		Iterator decisionClassesIterator = myDecisionPoint.getTargetConcept().iterator();
		...
		while (decisionClassesIterator.hasNext()) {
			DecisionCategory currentClass = (DecisionCategory) decisionClassesIterator.next();
		...
		visualizationType.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (selectedObject.getClass() != DecisionCategory.class) {
					// "none" was chosen --> remove previous class visualizations
					myVizModel.currentDecisionClass = null;
				} else {
					DecisionCategory classToVisualize = (DecisionCategory) selectedObject;
					// collect the tasks belonging to the log events charecterizing the decision class
					ArrayList associatedTasks = new ArrayList();
					Iterator associatedLogEvents = classToVisualize.getAssociatedLogEvents().
							iterator();
					while (associatedLogEvents.hasNext()) {
						LogEvent currentLE = (LogEvent) associatedLogEvents.next();
						// since duplicate tasks are not used to characterize decision classes
						// there is only one transition associated
						Transition trans = myVizModel.findRandomTransition(currentLE);
		...
		graphTypePanel.setLayout(new BoxLayout(graphTypePanel, BoxLayout.LINE_AXIS));
		graphTypePanel.add(visualizationHeading);
		graphTypePanel.add(visualizationType);

		JCheckBox attSelectionScope = new JCheckBox("Show attribute selection scope ");
		...
		graphTypePanel.setBorder(BorderFactory.createEmptyBorder(15, 15, 15, 15));
		...
		myModelViewPanel.add(myModelVisualizationOptionsPanel, BorderLayout.SOUTH);
	}
	...
	private void buildResultPanel() {
		...
		JLabel message = new JLabel("     Please press the button 'Update results'.");
