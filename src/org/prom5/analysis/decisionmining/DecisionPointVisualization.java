...
public class DecisionPointVisualization extends ReplayedPetriNet {
	...
	/**
	 * Holds the transitions belonging to the decision class currently to be displayed.
	 * The public attribute is necessary because the writeToDot() method
	 * has a fixed interface.
	 */
	 ...
	 * @param net the Petri net to be visualized
	 */
			...
			while (it.hasNext()) {
				...
				try {
					...
					if ((currentDecisionClass != null) &&
						...
						bw.write("t" + currentTrans.getNumber() +
	...
	protected void finishDotWriting(Writer bw) throws IOException {
		...
		if (toVisualize == null) {
			...
			Iterator succeedingTransitions = toVisualize.getSuccessors().iterator();
			while (succeedingTransitions.hasNext()) {
				Transition successor = (Transition) succeedingTransitions.next();
