...
public class DifferencesPlugin implements AnalysisPlugin{
	...
	private boolean debugging = false; //Special flags that triggers debug behaviour; should be set to false
	private static final boolean testingMultiple = false; //Special flags that triggers debug behaviour; should be set to false 
	...
	public AnalysisInputItem[] getInputItems() {
		AnalysisInputItem[] items = {
				new AnalysisInputItem(FIRST_NET_LABEL,1,1) {
					public boolean accepts(ProvidedObject object) {
						...
						for (int i = 0; (i < object.getObjects().length) && !acceptedModel; i++) {
							if (object.getObjects()[i] != null){
								if (ProcessAutomaton.acceptedModelType(object.getObjects()[i])){
									acceptedModel = true;													
								}
							}
						}
		...
		if (testingMultiple){
			AnalysisInputItem[] multiItems = {
					new AnalysisInputItem(FIRST_NET_LABEL,1,100) {
						public boolean accepts(ProvidedObject object) {
							boolean acceptedModel = false; 
							for (int i = 0; (i < object.getObjects().length) && !acceptedModel; i++) {
								if (object.getObjects()[i] != null){
	...
	public JComponent analyse(AnalysisInputItem[] inputs) {
		...
		for (int i = 0; (i < inputs[0].getProvidedObjects()[0].getObjects().length) && (req == null); i++) {
			if (ProcessAutomaton.acceptedModelType(inputs[0].getProvidedObjects()[0].getObjects()[i])){
				req = (ModelGraph) inputs[0].getProvidedObjects()[0].getObjects()[i];
			}			
		}
		for (int i = 0; (i < inputs[1].getProvidedObjects()[0].getObjects().length) && (prov == null); i++) {
			if (ProcessAutomaton.acceptedModelType(inputs[1].getProvidedObjects()[0].getObjects()[i])){
				prov = (ModelGraph) inputs[1].getProvidedObjects()[0].getObjects()[i];
			}			
		}
		//invoke the GUI
