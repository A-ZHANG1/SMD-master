...
import org.prom5.analysis.differences.processdifferences.ProcessAutomaton;
...
public class DifferencesUI extends JPanel {
	...
	final JLabel jlbProvTransitions = new JLabel("Messages in provided behaviour");
	final JLabel jlbReqTransitions = new JLabel("Equivalent messages in required behaviour   ");
	final JLabel jlbMapping = new JLabel("<html>Mapping of messages:<br><br></html>");
	...
	private ArrayList<ModelGraphVertex> provTransitions = new ArrayList<ModelGraphVertex>();//contains only visible transitions
	private ArrayList<ModelGraphVertex> reqTransitions = new ArrayList<ModelGraphVertex>();//contains only visible transitions		
	...
	public DifferencesUI(ModelGraph pBehaviour, ModelGraph rBehaviour) {
		...
		try {
			if (pBehaviour == null) {
				...
				this.provBehaviour = (PetriNet) ((PetriNetResult) p.importFile(new FileInputStream("C:/Documents and Settings/RDIJKMAN/workspaceprom/Prom2/Marian/process2.pnml"))).getProvidedObjects()[0].getObjects()[0];
				debugging = true;
			} else {
			...
			if (rBehaviour == null) {
				PnmlImport p = new PnmlImport();
				this.reqBehaviour = (PetriNet) ((PetriNetResult) p.importFile(new FileInputStream("C:/Documents and Settings/RDIJKMAN/workspaceprom/Prom2/Marian/process1.pnml"))).getProvidedObjects()[0].getObjects()[0];
			} else {
	...
	public JPanel prepareLeftPanel() {
		...
		for (int i = 0; i < provTransitions.size(); i++) {
			strProvTransitions.add(provTransitions.get(i).getIdentifier().replace("\\n", " "));
		}
		...
		for (int i = 0; i < reqTransitions.size(); i++) {
			strReqTransitions.add(reqTransitions.get(i).getIdentifier().replace("\\n", " "));
		}

		// set captions
		jlbProvTransitions.setPreferredSize(new Dimension((int) jlbProvTransitions.getPreferredSize().getWidth(),
				(int) jlbProvTransitions.getPreferredSize().getHeight()));
		jlbReqTransitions.setPreferredSize(new Dimension((int) jlbReqTransitions.getPreferredSize().getWidth(),
		...
		panel.add(pCaptions, new GridBagConstraints(0, 0, 1, 1, 1, 0.0, GridBagConstraints.WEST,
				GridBagConstraints.NONE, new Insets(1, 1, 1, 1), 10, 0));

		int i;
		for (i = 0; i < strProvTransitions.size(); i++) {
			// make a new label with the event name
			JTextField jtfProvTransitions = new JTextField(strProvTransitions.get(i));
			jtfProvTransitions.setFont(jtfProvTransitions.getFont().deriveFont(Font.PLAIN));
			jtfProvTransitions.setEditable(false);
			jtfProvTransitions.setPreferredSize(new Dimension((int) jlbProvTransitions.getPreferredSize().getWidth(),
			...
			// log
			JComboBox jcmbReqTransitions = new JComboBox(strReqTransitions.toArray());
			jcmbReqTransitions.setBackground(Color.WHITE);
			jcmbReqTransitions.setPreferredSize(new Dimension((int)jlbReqTransitions.getPreferredSize().getWidth(), (int)jlbReqTransitions.getPreferredSize().getHeight()));
			jcmbReqTransitions.setFont(jtfProvTransitions.getFont().deriveFont(Font.PLAIN));
			...
			panel.add(p, new GridBagConstraints(0, i + 1, 1, 1, 1, 0.0, GridBagConstraints.WEST,
					GridBagConstraints.NONE, new Insets(1, 1, 1, 1), 10, 0));
		}
		...
		panel.add(p, new GridBagConstraints(0, i + 1, 1, 1, 1, 0.0, GridBagConstraints.WEST, GridBagConstraints.NONE,
				new Insets(1, 1, 1, 1), 10, 0));

		JPanel p1 = new JPanel(new BorderLayout());
		p1.add(jlbResult, BorderLayout.WEST);
		panel.add(p1, new GridBagConstraints(0, i + 2, 1, 1, 1, 0.0, GridBagConstraints.WEST, GridBagConstraints.NONE,
				new Insets(1, 1, 1, 1), 10, 0));
		...
		jtaResult.setBorder(javax.swing.BorderFactory.createLineBorder(Color.gray, 1));
		JScrollPane jspResultScroll = new JScrollPane(jtaResult, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
	...
	public void findMatch() {
		...
		while (it1.hasNext()) {
			...
			for (int i = 0; i < cmb.getItemCount(); i++) {
				...
				if (debugging) {
					if (str1.equals(". Produce offer") && str2.equals(". Enter client and offer details")) {
						cmbList.get(index).setSelectedIndex(i);
					}
					if (str1.equals(". Fiat") && str2.equals(". Fiat")) {
						cmbList.get(index).setSelectedIndex(i);
					}
					if (str1.equals(". Check credit") && str2.equals(". Check credit")) {
						cmbList.get(index).setSelectedIndex(i);
					}
					if (str1.equals(". Make changes") && str2.equals(". Make changes")) {
						cmbList.get(index).setSelectedIndex(i);
					}
					if (str1.equals(". Check changes") && str2.equals(". Check changes")) {
						cmbList.get(index).setSelectedIndex(i);
					}
					if (str1.equals(". Print") && str2.equals(". Print")) {
						cmbList.get(index).setSelectedIndex(i);
					}
					if (str1.equals(". Check complete") && str2.equals(". Check complete")) {
						cmbList.get(index).setSelectedIndex(i);
					}
					if (str1.equals(". Plan appointment") && str2.equals(". Notify sales")) {
						cmbList.get(index).setSelectedIndex(i);
					}
	...
	public void evaluate() {
		...
		try {
			ProcessDifferences diffsProcessor = new ProcessDifferences((ModelGraph) provBehaviour,
					(ModelGraph) reqBehaviour, fillRelation());
			jtaResult.setText("");
			Iterator<ProcessDifference> j = diffsProcessor.proceduralDifferences().iterator();
			if (!j.hasNext()) {
			...
			while (j.hasNext()) {
				...
				if (pd.getOfTransitionsfromProvBehaviour() != null) {
					redTransitions.addAll(pd.getOfTransitionsfromProvBehaviour());
				}
				if (pd.getOfTransitionsfromReqBehaviour() != null) {
					redTransitions.addAll(pd.getOfTransitionsfromReqBehaviour());
				}

				Set<ModelGraphVertex> orangeTransitions = new HashSet<ModelGraphVertex>();
				if (pd.getinvolvedTransitionsfromProvBehaviour() != null) {
					orangeTransitions.addAll(pd.getinvolvedTransitionsfromProvBehaviour());
				}
				if (pd.getinvolvedTransitionsfromReqBehaviour() != null) {
					orangeTransitions.addAll(pd.getinvolvedTransitionsfromReqBehaviour());
				}

				jtaResult.append(pd.toString() + "\n\n");
				Message.add(pd.toString() + "#" + redTransitions.toString() + orangeTransitions.toString(),
						Message.TEST);
			}
			Message.add("</Differences>", Message.TEST);
		} catch (Exception e) {
	...
	public Relation<ModelGraphVertex, ModelGraphVertex> fillRelation() {
		...
		while (it.hasNext()) {
			...
			if (cmb.getSelectedIndex() > 0) {
				...
				Tuple<ModelGraphVertex,ModelGraphVertex> tuple = new Tuple<ModelGraphVertex,ModelGraphVertex>(tProv, tReq);
				r.addR(tuple);					
			}			
			position++;
		}
		...
		while (it.hasNext()) {
			...
			if (cmb.getSelectedIndex() > 0) {
				...
				ModelGraphVertex tProv = provTransitions.get(position);
				ModelGraphVertex tReq = reqTransitions.get(index);
				Tuple<ModelGraphVertex,ModelGraphVertex> tuple = new Tuple<ModelGraphVertex,ModelGraphVertex>(tReq,tProv);
				r.addR(tuple);					
			}			
			position++;
		}
