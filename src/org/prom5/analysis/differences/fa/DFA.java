...
/** 
* The DFA defines a deterministic finite automaton, by extending the finite automaton class FA.
*
...
public class DFA extends FA {
   ...
   /**
    * Constructs an empty DFA initialized to a capacity of 16 states. This is later doubled on need.
    *
    ...
    * @return				false iff the transition could not be added (i.e. it already exists, one of its states does not exist, or it violates the determinism).
    * 
    */
	public boolean addTransition(Marking sourceState, Marking targetState, int symbol){
	...
	/**
	 *	Changes the DFA such that it accepts the language that is the complement of the 
	...
	public NFA getNFA(){		
	    ...
	    * @throws				FAException if something goes wrong during minization.
	    * 
	    */
		public DFA minimize(){
			...
			/* We create a new DFA, and chooses a representing state from each subgroup, which will be a state in the new DFA */
			...
			for(int i=0;i<nrStateGroups;i++) {
				int sState = stateGroups[i].firstElement(); // The first element of each group is the rep. state
			...
			for(int i=0;i<nrStateGroups;i++) {
				...
				for(int k=0;k<alphabetSize;k++) {
					...
					if(t != -1) {
						...
						minDFA.addTransition(getStateName(sState), getStateName(r), alphabet[k]);
					}
									
				}
			}

			minDFA.setInitialState(getStateName(representativeState(pNew, getInitialState())));
	   ...
	   /**
	    * Removes "dead" states from the automaton. A state is dead if it is no accepting and has transitions
	    * to itself on all input symbols.
	    *
	    */
		...
		private IntegerPartition refineStatePartition(IntegerPartition p) {
			...
			for(int i=0;i<len;i++) {
				...
				for(int j=0;j<Glen;j++) {
					...
					/* We put together the list of the transitions for this state on all alphabet symbols */
					for(int k=0;k<alphabetSize;k++) {
						int a = alphabet[k];
					...
					/* Have we seen this exact list of transitions of the alphabetsymbols before? */
					int listNum = getIndexOfArray(seenTransitionLists, nrSeen, groupb, alphabetSize);
					
					if(listNum == -1) {
						/* If not, we add this list of transition to the list of seen ones */
...
//				newP.removeGroup(i);
		...
		private int getIndexOfArray(int[][] lists, int nrLists, int[] array, int arraySize) {
		...
		private String braceIfUnbracedU(String toBrace){
			...
			do{
				...
				int nextIndexOfOpenBrace = toBrace.indexOf('(',nextIndexOfBraceOrU);
				nextIndexOfOpenBrace = (nextIndexOfOpenBrace==-1)?toBrace.length():nextIndexOfOpenBrace; 
				int nextIndexOfCloseBrace = toBrace.indexOf(')',nextIndexOfBraceOrU);
				nextIndexOfCloseBrace = (nextIndexOfCloseBrace==-1)?toBrace.length():nextIndexOfCloseBrace; 
				int nextIndexOfU = toBrace.indexOf('U',nextIndexOfBraceOrU);
				nextIndexOfU = (nextIndexOfU==-1)?toBrace.length():nextIndexOfU; 
				
				nextIndexOfBraceOrU = Math.min(nextIndexOfU,Math.min(nextIndexOfOpenBrace, nextIndexOfCloseBrace));
		...
		private String braceIfUnbracedPart(String toBrace){
			...
			do{
				...
				nextIndexOfCloseBrace = (nextIndexOfCloseBrace==-1)?toBrace.length():nextIndexOfCloseBrace; 
				int nextIndexStraightCloseBrace = toBrace.indexOf(']',nextIndexOfBrace);
				nextIndexStraightCloseBrace = (nextIndexStraightCloseBrace==-1)?toBrace.length():nextIndexStraightCloseBrace; 
				
				nextIndexOfBrace = Math.min(nextIndexStraightCloseBrace,Math.min(nextIndexOfOpenBrace, nextIndexOfCloseBrace));
				...
				if (nextIndexOfBrace < toBrace.length()){
					if (toBrace.charAt(nextIndexOfBrace) == '('){
						...
						openBraces--;
						if ((openBraces == 0) && (nextIndexOfBrace != toBrace.length()-1)){
		...
		public String deriveRegExp(){
			/* The matrix used here represents a 'regular expression automaton', such that:
			 * regaut[i][j] = r represents that the automaton can transition from state i
			 * into state j by processing a string that is accepted by regexp r.
			 */
			...
			for (int fi = 0; fi < finalStates.length; fi++){
				...
				LinkedList<Integer> nonIntFinStates = new LinkedList<Integer>(); //The set of states that is nether the initial nor the final
				...
				for (int fromState = 0; fromState < statesLength; fromState++){
					...
					for (int i = 0; i < symbols.length; i++){
						...
						int toState = states[fromState].getSymbolTargetState(symbol);
						if (regaut[fromState][toState] == null){
							regaut[fromState][toState] = "[" + Integer.toString(symbol) + "]";
						}else{
							String existing = regaut[fromState][toState].startsWith("(")?regaut[fromState][toState].substring(1,regaut[fromState][toState].length()):regaut[fromState][toState]; //to remove brackets
							regaut[fromState][toState] = "(" + existing + " U [" + Integer.toString(symbol) + "])";
						}
					}
				}
				...
				while (!nonIntFinStates.isEmpty()){
					...
					for (int j = 0; j < statesLength; j++){
						for (int k = 0; k < statesLength; k++){
							if ((j != state) && (k != state) && (regaut[j][state] != null) && (regaut[state][k] != null)){
								regaut[j][k] = (regaut[j][k] != null)?(regaut[j][k] + " U "):"";
								if (regaut[state][state] == null){
									regaut[j][k] += braceIfUnbracedU(regaut[j][state]) + braceIfUnbracedU(regaut[state][k]);  
								}else{
									regaut[j][k] += braceIfUnbracedU(regaut[j][state]) + braceIfUnbracedPart(regaut[state][state]) + "*"+ braceIfUnbracedU(regaut[state][k]);  									
								}
							}
						}
					}
				...
				String aexp = "";
				if (initialState == f){
					aexp = (regaut[initialState][f] == null)?"EMPTY":braceIfUnbracedPart(regaut[initialState][f]) + "*";
				}else{
					String w1star = (regaut[initialState][initialState] == null)?"":braceIfUnbracedPart(regaut[initialState][initialState]) + "*";
					String w2 = (regaut[initialState][f] == null)?"":regaut[initialState][f];
					String w3 = (regaut[f][f] == null)?"":regaut[f][f];
					String w4 = (regaut[f][initialState] == null)?"":regaut[f][initialState];
					if (w2.length()==0){
						aexp = w1star;
					}else{
						if ((w3.length() != 0)&&(w4.length()!=0)){
							aexp = w1star + braceIfUnbracedU(w2) + "(" + braceIfUnbracedU(w3) + " U " + braceIfUnbracedU(w4) + w1star + braceIfUnbracedU(w2) + ")*";
						}else if (w3.length() != 0){
							aexp = w1star + braceIfUnbracedU(w2) + braceIfUnbracedPart(w3) + "*";
						}else if (w4.length() != 0){
							aexp = w1star + braceIfUnbracedU(w2) + "(" + braceIfUnbracedU(w4) + w1star + braceIfUnbracedU(w2) + ")*";							
						}else{
			...
			for (int statei = 0; statei < statesLength; statei++){
				...
				for (int alphabeti = 0; alphabeti < alphabetsize;alphabeti++){
					if (state.getSymbolTargetState(alphabet[alphabeti]) == -1){
						...
						addTransition(getStateName(statei),sink,alphabet[alphabeti]);
					}
				}
			}
		}
		...
		/**
		 * Returns true if the automaton accepts a trace consisting of only the specified symbol.
		 * 
		...
		public boolean acceptsFromStart(int symbol){
			int targetState = getFATransitions()[getInitialState()].getSymbolTargetState(symbol);
			if (targetState == -1){
				return false;
			}else{
