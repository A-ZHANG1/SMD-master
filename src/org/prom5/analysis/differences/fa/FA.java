package org.prom5.analysis.differences.fa;
...
/** 
 * The FA defines an abstract class for finite automatons. This class should be overridden by primarily
 * the deterministic- and non-deterministic implementations. The class wraps the data of the automaton,
 * and provides various methods for manipulating a finite automaton.
 *
...
public abstract class FA{
	...
	/**
	 * Constructs an empty FA initialized to a capacity of 16 states. This is later doubled on need.
	 *
	 * @param alphabet			The alphabet that the NFA should accept.
	 ...
	 * @param alphabet			The alphabet that the NFA should accept.
	 * @param alphabetSize		The size of the alphabet.
	 * @param maxStates			The maximum number of states in the automaton. This is later doubled on need.
	 * 
	 */
	...
	/**
	 * Sets the initial state of the automaton. Does nothing if that state does not exist
	 *
	 ...
	 *  
	 * @param stateIndex	The index of the initial state
	 */
	...
	/**
	 * Method for adding a state to the automaton without giving the state a name.
	 * The state will be given a unique name in the context of the automaton.
	 *
	 * @param accepting		Specifices whether the state is accepting or not.
	 ...
	 *
	 * @param sourceState	The state from which the transition should start.
	 * @param targetState	The state to which the transition should go.
	 * @param symbol			The input symbol for which the transition should occur.
	 * @param nonDeterministic	Specifies whether the transition may be non-deterministic.
	 * @return				false iff the transition cannot be added (i.e. it already exists, one of its states does not exist, or it violates determinism).
	 * 
	 */
	public boolean addTransition(Marking sourceState, Marking targetState, int symbol, boolean nonDeterministic){		
		adjacencyRelation = null;
	 ...
	 * Method for adding a transition to the automaton.
	 *
	 * @param sourceStateIndex	The index of the state from which the transition should start.
	 * @param targetStateIndex	The index of the state to which the transition should go.
	 * @param symbol			The input symbol for which the transition should occur.
	 * @param nonDeterministic	Specifies whether the transition may be non-deterministic.
	 * @return				false iff the transition cannot be added (i.e. it already exists, one of its states does not exist, or it violates determinism).
	 * 
	 */
	public boolean addTransition(int sourceStateIndex, int targetStateIndex, int symbol, boolean nonDeterministic){
		adjacencyRelation = null;		
		if(sourceStateIndex == -1 || targetStateIndex == -1) {
			return false;
		}else{
			addSymbolToState(sourceStateIndex,symbol);			
			return s[sourceStateIndex].addTransition(targetStateIndex, symbol, nonDeterministic);
		}
	}
	...
	public void replaceSymbol(int symbol, int replacement, boolean allowNonDeterminism){
		adjacencyRelation = null;
		int c = 0;
		int symbolFoundAt = -1;
		int replacementFoundAt = -1;
		while ((c < alphabetSize) && ((symbolFoundAt == -1) || (replacementFoundAt == -1))){
	...
	/**
	 * Method for deleting a state from the automaton. If the given state does not exist in the automaton, it remains unchanged.
	 *
	 * @param name		The name of the state to delete. I
	 * 
	 */
		...
		for(int j=0;j<topState;j++) {
			...
			if(s[j] != null) {
				...
				for (int k = 0; k < symbolsfromj.length; k++){
					...
					onlyStateI.add(i);
					if (s[j].getSymbolTargetStates(symbolsfromj[k]).equals(onlyStateI)){
	...
	/**
	 * Method for deleting a transition from the automaton. If the given transition does not exist in the automaton, it remains unchanged.
	 *
	 ...
	 * 
	 */
	public void delTransition(Marking sourceState, Marking targetState, int symbol) {
		adjacencyRelation = null;
		int i = getStateIndex(sourceState);
		int j = getStateIndex(targetState);
	...
	/**
	 * Returns an array with the characters that are part of the alphabet (EPSILON excluded). 
	 *
	 * @return				The alphabet.
	 * 
	 */
	...
	/**
	 * Returns the array of state transitions of the NFA. 
	 * Each index in the array holds the transitions object of the state with internal id index.
	 *
	...
	/**
	 * Returns index of the top state. This is used in conjunction with getStates in order to know the utilized length
	 * of the returned state transitions array.
	 *
	...
	private void computeReachabilityMatrix(){
			...
			for (int c = 0; c < topState; c++){
				...
				for (int targeti = 0; targeti < targetStates.length; targeti++){
					adjacencyRelation.addR(new Tuple<Integer,Integer>(c,targetStates[targeti]));
				}
			}
	}
	...
	public Set<Marking> reachableFrom(Marking fromState){
		...
		if (from != -1){
			...
			for (Iterator<Integer> i = intermediateResult.iterator();i.hasNext();){
	...
	public boolean isEmptyLanguage(){
		IntegerSet reachableFromStart = new IntegerSet(reachableFrom(getInitialState()));
	...
	/* This returns minimal cycles, but we are not looking for minimal cycles, we are looking for all cycles 
	private Set<List<Integer>> cyclesHelper(int startingAt, int from, Set<Integer> visited, List<Integer> listSoFar){
		Set<List<Integer>> result = new HashSet<List<Integer>>();

		int symbols[] = s[from].getSymbolArray();
		for (int i = 0; i < symbols.length; i++){
			int symbol = symbols[i];
			int toStates[] = s[from].getSymbolTargetStates(symbol).getArray();
			for (int j = 0; j < toStates.length; j++){
				int toState = toStates[j];
				if (toState == startingAt){
					List<Integer> listSoFarPrime = new LinkedList<Integer>();
					listSoFarPrime.addAll(listSoFar);
					listSoFarPrime.add(symbol);					
					result.add(listSoFarPrime);
				}else if (visited.contains(toState)){					
				}else{
					Set<Integer> visitedPrime = new HashSet<Integer>();
					visitedPrime.addAll(visited);
					visitedPrime.add(toState);
					List<Integer> listSoFarPrime = new LinkedList<Integer>();
					listSoFarPrime.addAll(listSoFar);
					listSoFarPrime.add(symbol);					
					result.addAll(cyclesHelper(startingAt,toState,visitedPrime,listSoFarPrime));
				}
			}
		}
		return result;
	...
	protected Set<List<Integer>> cyclesContaining(int symbol){
		...
		for (Iterator<Integer> i = fromStates.iterator(); i.hasNext();){
			...
			int toStates[] = s[fromState].getSymbolTargetStates(symbol).getArray();
			for (int j = 0; j < toStates.length; j++){
				int toState = toStates[j];
	...
	*/
	...
	private Set<List<Integer>> cyclesHelper(Tuple<Integer,Integer> startEdge, Tuple<Integer,Integer> fromEdge, Set<Tuple<Integer,Integer>> visited, List<Integer> listSoFar){
		Set<List<Integer>> result = new HashSet<List<Integer>>();

		int fromStates[] = s[fromEdge.e1].getSymbolTargetStates(fromEdge.e2).getArray();
		for (int i = 0; i<fromStates.length; i++){
			int symbols[] = s[fromStates[i]].getSymbolArray();
			for (int j = 0; j < symbols.length; j++){
				Tuple<Integer,Integer> edgeExploring = new Tuple<Integer,Integer>(fromStates[i],symbols[j]);
				if (edgeExploring.equals(startEdge)){
					...
					result.addAll(cyclesHelper(startEdge,edgeExploring,visitedPrime,listSoFarPrime));
				}
			}			
		}		
		return result;
	...
	protected Set<List<Integer>> cyclesContaining(int symbol){
		...
		for (Iterator<Integer> i = fromStates.iterator(); i.hasNext();){
			int fromState = i.next();
			Tuple<Integer,Integer> edgeExploring = new Tuple<Integer,Integer>(fromState,symbol);
			Set<Tuple<Integer,Integer>> visited = new HashSet<Tuple<Integer,Integer>>();
			visited.add(edgeExploring);
			List<Integer> listSoFar = new LinkedList<Integer>();
			listSoFar.add(symbol);
			result.addAll(cyclesHelper(edgeExploring, edgeExploring, visited, listSoFar));
		}
		return result;
	}
	...
	protected Marking nonExistingMarking(){
		Marking m;
		PetriNet p = new PetriNet(); // Added by Marian maybe causes problems!!!!
		Integer uniqueNumber = 0;
		do{
			uniqueNumber++;
			m = new Marking();
			m.addPlace(new Place("a",p), uniqueNumber); // p is added by Marian maybe causes problem
	...
	public IntegerSet statesToWhichSPoints(int sym){
		...
		for (Iterator<Integer> i = statesInWhichSOriginates.iterator(); i.hasNext();){
			int origin = i.next();
