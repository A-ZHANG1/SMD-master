...
public class FATransitions {
   ...
   /**
    * Checks whether a given transition (targetstate, symbol-pair) is contained in the set.
    *
    * @param 	state		The target state of the transition to look for.
    * @param 	symbol		The symbol of the transition to look for.
	* @return				true if the transition is contained within the list, otherwise false.
    * 
    */
	public boolean contains(int state, int symbol) {
		...
		while(next != null) {
			if(next.getTransitionState() == state && next.getTransitionSymbol() == symbol) {
				return true;
			}
    ...
    * @param	allowNonDeterministic	Indicates whether non deterministic transitions are allowed.
	* @return	false 			iff the given transition already exists in the list, or a violation of
	*							the integrity of the automaton occurs.
    * 
    */
	public boolean addTransition(int targetState, int sym, boolean allowNonDeterministic){
	...
	/**
	 * Replaces transitions with symbol sym by transitions with symbol replacement 
	 * Does nothing if a transition already exists or if an addition would violate deterministic constraints
	 * 
	 * @param sym					symbol to replace
	 * @param replacement			symbol to replace by
	 * @param allowNonDeterministic	true iff determinism must be maintained
	 */
	public void replaceSymbol(int sym, int replacement, boolean allowNonDeterministic){
		if (allowNonDeterministic){
			int[] targetStates = getSymbolTargetStates(sym).getArray();
			for (int i = 0; i < targetStates.length; i++){
				removeTransition(targetStates[i],sym);
				this.addTransition(targetStates[i],replacement,allowNonDeterministic);
			}
		}else{
    ...
    *
    * @param 	state		The target state to which transitions should be removed.
    * 
    */
    ...
    *
    * @param 	state		The target state of the transition to remove.
    ...
    */
	public void removeTransition(int state, int sym) {
		...
		while(next != null) {
			if(next.getTransitionSymbol() == sym && next.getTransitionState() == state) {
				if(prev == null) {
					if(top == root) {
						top = next.nextTransition();
					}
   ...
   /**
    * Returns the target state of the a symbol. This should only be used if the transition list i deterministic.
    *
    * @param 	sym			The symbol which associated target state should be returned.
   ...
   /**
    * Returns the states reachable through a given symbol. Note that this should only be used when the transitions
    * are non-deterministic. Otherwise the set of states will contain at most one element, and the
    * getSymbolTargetState(int sym)-method should be used instead.
    *
    * @param 	sym			The symbol.
	* @return				The set of states reachable through transitions on the given symbol.
    * 
    */
    ...
    *
	* @return				Array of integers containing the target states of the transition list.
    * 
    */
    ...
    */
	public String toString() {
		...
		while(next != null) {
			...
			str += (next.getTransitionSymbol() + "|" + next.getTransitionState());
			next = next.nextTransition();
		}
