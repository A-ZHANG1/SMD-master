...
/** 
 * The NFA defines a non-deterministic finite automaton by extending the finite automaton class FA.
 *
...
public class NFA extends FA {
	...
	/**
	 * Constructs an empty NFA initialized to a capacity of 16 states. This is later doubled on need.
	 *
	...
	public NFA(FA fa){
		...
		for(int i=0;i<size;i++) {
			...
			for(int j=0;j<tSize;j++) {
				addTransition(getStateName(stateMap[i]), getStateName(stateMap[target[j]]), symbol[j]);
			}
		}
	 ...
	 * @return				false iff the transition could not be added (i.e. it already exists, or one of its states does not exist).
	 * 
	 */
	 ...
	 * 
	 */
	public boolean addTransition(int sourceStateIndex, int targetStateIndex, int symbol){
	...
	public DFA getDFA(){
		...
		initialMarking.addPlace(new Place(Integer.toString(tStateNum),p), 0); // p added by marian, addmark changed to add place
		dfa.addState(initialMarking, !(temp.intersection(acceptingStates)).isEmpty());
		...
		while(tStateNum != -1) {
			...
			for(int i=0;i<alphabetSize;i++) {
				...
				if(uStateNum == -1) {
					...
					boolean accepting = !(reachable.intersection(acceptingStates)).isEmpty();

					Marking newMarking = new Marking();
					newMarking.addPlace(new Place(Integer.toString(uStateNum),p), 0);// p added by marian, addmark changed to add place
					dfa.addState(newMarking, accepting);
				}

				Marking fromMarking = new Marking();
				fromMarking.addPlace(new Place(Integer.toString(tStateNum),p), 0);// p added by marian, addmark changed to add place
				Marking toMarking = new Marking();
				toMarking.addPlace(new Place(Integer.toString(uStateNum),p), 0);// p added by marian, addmark changed to add place
	 ...
	 *
	 * @param merger			The NFA to merge with.
	 * @param startStateName	The name of the new initial state in the NFA.
	 */
	...
	public void merge(FA merger, Marking startStateName){
		...
		for(int i=0;i<size;i++) {
			if(i == initial) {
				...
				stateMap[i] = addState(nonExistingMarking(), accepting.contains(i));
			}
		}
	 ...
	 *
	 * @param alphabet		The alphabet of the created NFA.
	 * @param alphabetSize	The size of the alphabet.
	 * @param a				NFA a.
	 * @param b				NFA b.
	 * @return				NFA N(a|b).
	...
	public static NFA or(int[] alphabet, int alphabetSize, NFA a, NFA b){
		...
		int or1StartState = orNFA.addState(orNFA.nonExistingMarking(), a.acceptingStates().contains(a.getInitialState()));
		int or2StartState = orNFA.addState(orNFA.nonExistingMarking(), b.acceptingStates().contains(b.getInitialState()));
