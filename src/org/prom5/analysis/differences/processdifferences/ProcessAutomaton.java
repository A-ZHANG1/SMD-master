...
import org.prom5.analysis.differences.fa.DFA;
import org.prom5.analysis.differences.fa.IntegerSet;
import org.prom5.analysis.differences.fa.NFA;
import org.prom5.analysis.differences.relations.Relation;
import org.prom5.converting.PetriNetReduction;
...
import org.prom5.framework.models.petrinet.Marking;
...
import org.prom5.framework.models.petrinet.algorithms.ReachabilityGraphBuilder;

import att.grappa.Edge;

/**
 * This class defines the abstract (and concrete) business process notation 
 * for which we can compute differences (definitions 1, 2 and 3 from 2007 BETA WP.) 
 * To make the abstract notation suitable for another concrete notation, adapt the methods:
 * - inFor
 * - visibleTasks
 * - constructAutomaton
 * - acceptedModelType
...
public class ProcessAutomaton extends NFA{
	...
	public ProcessAutomaton(ProcessAutomaton ptnetsem){
		this((NFA)ptnetsem,ptnetsem.getSymbol2Object(),ptnetsem.getObject2Symbol(),ptnetsem.getProcess());
	}

	public ProcessAutomaton(NFA nfa, Map<Integer,Object> symbol2Object, Map<Object,Integer> object2Symbol, ModelGraph n){
		super(nfa);
	...
	/**
	 * Replaces each occurrence of an object by another object
	 * Also removes the object from the alphabet (and adds the other object if necessary)
	 * 
	 * @param	o			object to replace
	 * @param	replacement	object to replace by
	 */
	public void replace(Object o, Object replacement){
		/* If object2Symbol(replacement) already exists in the NFA
		 *   then object2Symbol(o) must be replaced by object2Symbol(replacement) in the NFA
		 *        (such that o is treated as equivalent to replacement by the NFA)
		 *        and o must be removed from object2symbol and symbol2object
		 * If object2Symbol(replacement) does not exist in the NFA
		 *   then o must be replaced by replacement in object2symbol and symbol2object
		 */
	...
	/**
	 * Replaces each occurrence of an object by an occurrence of its equivalence class in the given relation.
	 * 
	 * @param r	relation to obtain equivalence classes from.
	 */
	public void replaceObjectsByEquivalenceClass(Relation<Object,Object> r){
		Set<Object> objectsToReplace = new HashSet<Object>(object2Symbol.keySet());
	 ...
	 * 
	 * @param s	the set of objects to retain; all other objects are abstracted from.
	 */
	public void restrictTo(Set<Object> s){
		Set<Object> objectsToAbstractFrom = new HashSet<Object>(object2Symbol.keySet());
		objectsToAbstractFrom.removeAll(s);
		abstractFromAll(objectsToAbstractFrom);
	}

	/**
	 * Abstracts from each occurrence of an object by labeling it silent
	 * Also removes the object from the alphabet
	 * 
	 * @param	o	object to abstract from
	 */
	...
	/**
	 * Abstracts from each occurrence of an object from the argument set by labeling it silent.
	 * Also removes the objects from the alphabet.
	 * 
	 * @param os	the objects to abstract from
	 */
	 ...
	 *	and the set of traces accepted by pt2. Warning - postcondition: getProcess()==null.
	 *
	 *	@param ptnet1	automaton 1 to compute the disjunction of
	 *	@param ptnet2	automaton 2 to compute the disjunction of
	 *	@return			the disjunction of ptnet1 and ptnet2
	 */
	public static ProcessAutomaton or(ProcessAutomaton ptnet1, ProcessAutomaton ptnet2){
		ProcessAutomaton pt1 = new ProcessAutomaton(ptnet1); //TODO: This copy should not be necessary, but somewhere the procedure below has undesirable side-effects.
		ProcessAutomaton pt2 = new ProcessAutomaton(ptnet2); //TODO: This copy should not be necessary, but somewhere the procedure below has undesirable side-effects.
		...
		for (Iterator<Entry<Integer,Object>> i = pt1.getSymbol2Object().entrySet().iterator(); i.hasNext();){
		...
		//Replace all symbols in pt2 by symbols that do not exist in pt1 nor in pt2		
		...
		for (Iterator<Entry<Integer,Object>> i = pt2.getSymbol2Object().entrySet().iterator(); i.hasNext();){
			Entry<Integer,Object> ent = i.next();
			int sym = ent.getKey();
			Object obj = ent.getValue();
		...
		NFA jointNFA = NFA.or(alphabet, jointAlphabet.size(), (NFA)pt1, (NFA)pt2);

		return new ProcessAutomaton(jointNFA, jointSymbol2Object, jointObject2Symbol, null);
	}

	/**
	 *	Returns the automaton that accepts the complement of the set of 
	 *  traces accepted by the argument automaton.
	 *  
	 *  @param ptnetsem		the automaton to return the complement of.
	...
	public static ProcessAutomaton complement(ProcessAutomaton ptnetsem){
		...
		return new ProcessAutomaton(complementSem.getNFA(),ptnetsem.getSymbol2Object(),ptnetsem.getObject2Symbol(),ptnetsem.getProcess());
	}
	...
	/**
	 *	Returns the regular expression that represents the traces accepted by this automaton.
	 *
	 * 	@return the resular expression that represents the traces accepted by this sutomaton.
	 */
	public String deriveRegExp(){
		DFA myDFA = this.getDFA();
		myDFA.minimize();
		String regexp = myDFA.deriveRegExp();
		for (Iterator<Entry<Integer,Object>> i = getSymbol2Object().entrySet().iterator();i.hasNext();){
			Entry<Integer,Object> e = i.next();
			regexp = regexp.replaceAll("\\[" + e.getKey() + "\\]", "'" + e.getValue().toString() + "'");
		}
		return regexp;
	}
	 ...
	 * 
	 * @return the set of cycles that contain o. Cycles are specified as lists starting with o.
	 */
	public Set<List<Object>> cyclesContaining(Object o){
		Set<List<Integer>> superResult = super.cyclesContaining(object2Symbol.get(o));
		...
		for (Iterator<List<Integer>> i = superResult.iterator(); i.hasNext();){
			...
			for (Iterator<Integer> j = superPartialResult.iterator(); j.hasNext();){
	...
	/**
	 * Returns the symbols that precede the given symbol in some execution trace. 
	 * 
	 * @param o	the given symbol.
	...
	public Set<Object> symbolsPreceding(Object o){
		...
		IntegerSet statesEqTFrom = new IntegerSet(); //TODO: We should actually get rid of the stupid IntegerSet
		Iterator<Integer> it = dfa.statesInWhichSOriginates(getObject2Symbol().get(o)).iterator();
		while(it.hasNext()){
		...
		for (int i = 0; i < dfa.getAlphabetSize(); i++){
			...
			if (!dfa.statesToWhichSPoints(sym).intersection(statesEqTFrom).isEmpty()){
	 ...
	 * Definition 2 from 2007 BETA WP.
	 * Ignores the 'skipped activities' while doing that.
	 * 
	 * @param t			the activity to determine the input for.
	 * @param toSkip	the skipped activities.
	 * @return			the input of the given activity, which is the set of all activities from which
	 * 					there is a path to t with no activity on that path other than a silent or skipped activity.
	 */
	public Set<ModelGraphVertex> inFor(ModelGraphVertex t, Set<ModelGraphVertex> toSkip){
	...
	private Set<ModelGraphVertex> inForHelper(ModelGraphVertex t, Set<ModelGraphVertex> visited, Set<ModelGraphVertex> toSkip){
		Set<ModelGraphVertex> result = new HashSet<ModelGraphVertex>();
		
		for (Iterator<ModelGraphVertex> i = t.getPredecessors().iterator();i.hasNext();){			
			ModelGraphVertex tPre = i.next();				
				if (visited.contains(tPre)){ //if it is visited do nothing
				}else if (toSkip.contains(tPre)){ //if it is skipped add its pre's
	...
	private void setAlphabet(){
		...
		for (Iterator<ModelGraphVertex> i = visibleTasks().iterator();i.hasNext();){
	...
	public static Set<ModelGraphVertex> visibleTasks(ModelGraph model){
		...
		if ((model instanceof EPC) ||
		    (model instanceof ConfigurableEPC)){
	...
	/**
	 * @param forEPC the automaton is constructed for a Petri net that was derived from an EPC 
	 */
	private void constructPetriNetAutomaton(PetriNet ptnet, boolean forEPC){
		...
		for (ModelGraphVertex v: rgraph.getVerticeList()){
			if (v instanceof State){
				State s = (State) v;
				boolean isAccepting = (s.getOutEdges()==null)?true:(s.getOutEdges().size()==0);
		...
		for (Object e: rgraph.getEdges()){
			if (e instanceof FSMTransition){
				...
				Transition trigger = (Transition) t.object; //Should be instanceof Transition
				...
				if (trigger.isInvisibleTask()){
					this.addTransition(sourceMarking, targetMarking, NFA.EPSILON);					
				}else{
					Object o = null;
					if (forEPC){
						//If the transition is derived from an EPC function the EPC function should be on 
						//in the transitionsystem rather than the PetriNet transition.
						//Assumption: (toFire.isVisibleTask()) iff (toFire.object instanceof EPCFunction)
						o = trigger.object;
					}else{
						o = trigger;						
					}
					this.addTransition(sourceMarking, targetMarking, object2Symbol.get(o));					
				}				
			}
		}
	}
	...
	private PetriNet reduceNet(PetriNet petrinet){
		...
		//Since the reduction removes all transition.object, we need to keep track of those.
			...
			if (place.getOutEdges() != null){
				for (Edge mge: place.getOutEdges()){
					if (mge.getHead() instanceof Transition){
						if ((mge.getHead().outDegree() == 0) && ((Transition)mge.getHead()).isInvisibleTask()){
