...
public class ProcessDifference {
	...
	public ProcessDifference(Set<ModelGraphVertex> ofTransitionsfromProvBehaviour, Set<ModelGraphVertex> ofTransitionsfromReqBehaviour, 
			Set<ModelGraphVertex> involvedTransitionsfromProvBehaviour, Set<ModelGraphVertex> involvedTransitionsfromReqBehaviour, 
			String characteristicProvBehaviour, String characteristicReqBehaviour, int differenceType, int differenceDirection) {
	...
	public void setCharacteristicProvBehaviour(String characteristicProvBehaviour) {
		this.characteristicProvBehaviour = characteristicProvBehaviour;
	}
	...
	public void setinvolvedTransitionsfromProvBehaviour(Set<ModelGraphVertex> involvedTransitionsfromProvBehaviour) {
		this.involvedTransitionsfromProvBehaviour = involvedTransitionsfromProvBehaviour;
	}
	...
	public void setinvolvedTransitionsfromReqBehaviour(Set<ModelGraphVertex> involvedTransitionsfromReqBehaviour) {
		this.involvedTransitionsfromReqBehaviour = involvedTransitionsfromReqBehaviour;
	}
	...
	public void setOfTransitionsfromProvBehaviour(Set<ModelGraphVertex> ofTransitionsfromProvBehaviour) {
		this.ofTransitionsfromProvBehaviour = ofTransitionsfromProvBehaviour;
	}
	...
	public void setOfTransitionsfromReqBehaviour(Set<ModelGraphVertex> ofTransitionsfromReqBehaviour) {
		this.ofTransitionsfromReqBehaviour = ofTransitionsfromReqBehaviour;
	}
	...
	public String transitionNames(Set<ModelGraphVertex> trSet){
		...
		for (Iterator<ModelGraphVertex> i = trSet.iterator(); i.hasNext();){
			result += "'" + i.next().getIdentifier().replace("\\n", " ") + "'"; // getname changed intos get identifier by Marian
			if (i.hasNext()){
				result += ", ";
			}
		}
	...
	public String toString(){
		...
		case TYPE_SKIPPED_ACTIVITY:
			...
			if (differenceDirection == DIRECTION_PROVMORE){
				result += "transition " + transitionNames(ofTransitionsfromProvBehaviour) + " from provided behaviour has no equivalent in required behaviour"; 
			}else{
				result += "transition " + transitionNames(ofTransitionsfromReqBehaviour) + " from required behaviour has no equivalent in provided behaviour"; 				
			}
			break;
		case TYPE_DIFF_START:
			result += "Different start of process; ";
			if (differenceDirection == DIRECTION_PROVMORE){
				result += (ofTransitionsfromProvBehaviour.size() == 1)?"transition " + transitionNames(ofTransitionsfromProvBehaviour) + " from provided behaviour has ":"equivalent transitions " + transitionNames(ofTransitionsfromProvBehaviour) + " from provided behaviour have ";				
			}else{
				result += (ofTransitionsfromReqBehaviour.size() == 1)?"transition " + transitionNames(ofTransitionsfromReqBehaviour) + " from required has ":"equivalent transitions " + transitionNames(ofTransitionsfromReqBehaviour) + " from required behaviour have ";				
			}
			result += "an additional possibility to start immediately as opposed to ";
			if (differenceDirection == DIRECTION_PROVMORE){
				result += (ofTransitionsfromReqBehaviour.size() == 1)?"transition " + transitionNames(ofTransitionsfromReqBehaviour) + " from required behaviour":"equivalent transitions " + transitionNames(ofTransitionsfromReqBehaviour) + " from required behaviour";
			}else{
				result += (ofTransitionsfromProvBehaviour.size() == 1)?"transition " + transitionNames(ofTransitionsfromProvBehaviour) + " from provided behaviour":"equivalent transitions " + transitionNames(ofTransitionsfromProvBehaviour) + " from provided behaviour";				
			}			
			break;
		case TYPE_ITERATIVE:
			result += "Iterative versus once-off; ";
			result += (differenceDirection == DIRECTION_PROVMORE)?"provided behaviour ":"required behaviour ";
			result += "has the additional (normalized) cycle ";
			result += (differenceDirection == DIRECTION_PROVMORE)?characteristicProvBehaviour:characteristicReqBehaviour;
			break;
		case TYPE_ADD_DEPENDENCIES:
			...
			if (differenceDirection == DIRECTION_PROVMORE){
				...
				result += (ofTransitionsfromReqBehaviour.size() == 1)?"transition " + transitionNames(ofTransitionsfromReqBehaviour) + " from required behaviour has ":"equivalent transitions " + transitionNames(ofTransitionsfromReqBehaviour) + " from required behaviour have ";				
			}
			result += "additional dependencies with respect to ";
			if (differenceDirection == DIRECTION_PROVMORE){
				result += (ofTransitionsfromReqBehaviour.size() == 1)?"transition " + transitionNames(ofTransitionsfromReqBehaviour) + " from required behaviour on ":"equivalent transitions " + transitionNames(ofTransitionsfromReqBehaviour) + " from required behaviour on ";
				result += transitionNames(involvedTransitionsfromProvBehaviour);
			}else{
				result += (ofTransitionsfromProvBehaviour.size() == 1)?"transition " + transitionNames(ofTransitionsfromProvBehaviour) + " from provided behaviour on ":"equivalent transitions " + transitionNames(ofTransitionsfromProvBehaviour) + " from provided behaviour on ";				
		...
		case TYPE_DIFF_MOMENTS:
			...
			result += (ofTransitionsfromProvBehaviour.size() == 1)?"transition " + transitionNames(ofTransitionsfromProvBehaviour) + " from provided behaviour occurs ":"equivalent transitions " + transitionNames(ofTransitionsfromProvBehaviour) + " from provided behaviour occur ";				
