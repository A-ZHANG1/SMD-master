package org.prom5.analysis.differences.relations;
...
public class Relation<T1,T2> {
	...
	/**
	 * Computes the transitive closure of the adjacency matrix using Warshall's algorithm
	 * Pre: getDom() == getRan()
	 */
	public void tclos(){
		tclos(true);
	...
	/**
	 * Computes the reflexive closure of the adjacency matrix
	 * Pre: getDom() == getRan()
	 */
	public void rclos(){
		rclos(true);
	...
	/**
	 * Computes the symmetric closure of the adjacency matrix
	 * Pre: getDom() == getRan()
	 */
	public void sclos(){
		sclos(true);
	 ...
	 * More efficient than computing the three successively
	 * Pre: getDom() == getRan()
	 */	
	public void clos(){
		if (!adjmatComputed){
	...
	/**
	 * Puts the adjacency matrix back into the relation
	 * Pre: getDom() == getRan()
	 */
	private void putback(){
		...
		for(int i = 0;i < max; i++){
			for(int j = 0;j < max; j++){
				if(adjmat[i][j] == 1){
					s.add(new Tuple<T1,T2>(intToElement.get(i),(T2) intToElement.get(j)));
				}
			}
		}				
	}
	...
	private void computeAdjMat(){
		 ...
		 * adjmat[i,j] == 1 iff (elementToInt-1(i), elementToInt-1(j)) in s
		 */
	 ...
	 * Pre: getDom() == getRan() and the relation is an equivalence relation
	 * 
	 * @param e the element to compute the equivalence class for
