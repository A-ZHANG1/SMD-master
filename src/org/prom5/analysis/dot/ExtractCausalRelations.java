package org.prom5.analysis.dot;
...
public class ExtractCausalRelations {
	...
	private void createReport(HashMap<String, Set<String>> map, String output) throws IOException {
		StringBuffer sb;
	...
	private void reverseMappings(){
		...
		for(String cause: causeToEffectsKeys){
			...
			for(String effect: effects){
				...
				causes.add(cause + " " + connectionToSignificance.get(cause + "->" + effect));
		...
		while((line = raf.readLine()) != null){
			...
			if(line.indexOf("node_")>=0 && line.indexOf("label=")>=0){
				if(line.indexOf("->") >= 0){
					...
					String sourceNode = line.substring(0,line.indexOf("->")-1).trim();
					String destinationNode = line.substring(line.indexOf("->")+2,line.indexOf("[")).trim();
					String significance = line.substring(line.indexOf("\"")+1, line.indexOf("\"", line.indexOf("\"")+1)-1).trim();
					if(nodeToLabel.get(destinationNode).indexOf("SYSTEM ERROR")>=0){
						Set destinationNodes = null;
						if(!causeToEffects.containsKey(nodeToLabel.get(sourceNode))){
							destinationNodes = new TreeSet<String>();
						}else{
							destinationNodes = causeToEffects.get(nodeToLabel.get(sourceNode));
						}
						destinationNodes.add(nodeToLabel.get(destinationNode));
						causeToEffects.put(nodeToLabel.get(sourceNode), destinationNodes);
						connectionToSignificance.put(nodeToLabel.get(sourceNode) + "->" + nodeToLabel.get(destinationNode), significance);
					}
				}
			}
		}
	...
	public static void main(String[] args){
		...
		try {
			ExtractCausalRelations ecr = new ExtractCausalRelations(inputDirectory, inputFile, outputFile);
		} catch (IOException e) {
