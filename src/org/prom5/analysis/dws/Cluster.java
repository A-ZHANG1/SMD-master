 ...
 * Al momento della creazione non viene associata la HeuristicNet
 */
public class Cluster {

	private HeuristicsNet en; //contiene il modello associato al cluster
	private String name; //nome del cluster
	private LogReader log; //log su file; il suo filtro ci dice indice delle tracce associate al cluster
	...
	private Cluster parent; //il genitore nel clustering: attualmente non � usato
	...
	private double gamma = 1; //parametro da leggere: seconda soglia sulle features
	private int l = 3; //parametro da leggere: lunghezza delle features
	private int maxFeatures = 5; //parametro da leggere: numero massimo di features
	 ...
	 * @param k Number of clusters to be mined.
	 * @param sigma Threshold for the mining of the features.
	 * @param gamma Threshold for the mining of the features.
	 * @param l Maximum legth of each feature.
	 * @param maxFeatures Maximum number of features.
	 */
	public Cluster(LogReader log, String name, int k, double sigma, double gamma, int l,
			int maxFeatures) {
		this.log = log;
		this.name = name;
		this.en = null;
...
//        this.ls=new LogSketch(original);
	 ...
	 *
	 * @param log Log file.
	 * @param parent Parent of the cluster in the hierarchy.
	 * @param name Name of the cluster.
	 * @param ls
	 */
	...
	/*
	 * Filtra un lista di tracce restituendo solo quelle la cui frequenza � maggiore di sigma
	 */
	...
	//costruzione vettore delle features
		...
		//questo ordinamento serve a far si che anche un rimescolamento dei log produca lo stesso risultato
		Arrays.sort(lf);
		...
		while (logInstancesIterator.hasNext()) {
			...
			AuditTrailEntryList ates = pi.getAuditTrailEntryList();
			int repetitions = MethodsForWorkflowLogDataStructures.getNumberSimilarProcessInstances(
							  pi);
			Trace tLog = new Trace(ates, en);
			...
			if (minOccurrence <= nLogs) {
				...
				while (itF.hasNext() && featureMassimale) {
					...
					if (f.ridond(lf[minIndex]))
						...
						//fine 
					/*if(this.gamma>= this.sigma){
						if (t.contains(daInserire)&& f.getHead().equals(lf[minIndex].getHead())) featureMassimale=false;
					else if (daInserire.contains(t)&& f.getHead().equals(lf[minIndex].getHead())){
						itF.remove();
						selectedFeatures--;
					}
					}
					...
					if (daInserire.contains(t)&& f.getHead().equals(lf[minIndex].getHead())) featureMassimale=false;
						else if (t.contains(daInserire)&& f.getHead().equals(lf[minIndex].getHead())){
						itF.remove();
						selectedFeatures--;
						}
				}
	...
	public void mineClusters() {
		...
		/*anto  aggiunto un controllo
		 * se features.size()==0 non deve splittare
		 */
		if (features.size() == 0) {
			return;
		}
		...
		for (int i = 0; i < logChildren.length; i++) {
			if (logChildren[i].size() > 0 && logChildren[i].size() != vs.points.size()) {
				int[] tracesToKeepForChildren = new int[logChildren[i].size()];
				ArrayList l = new ArrayList();
				for (int j = 0; j < tracesToKeepForChildren.length; j++) {
					tracesToKeepForChildren[j] = ((Integer) logChildren[i].get(j)).intValue();
					l.add(new Integer(tracesToKeepForChildren[j]));
				}
				try {
					LogReader r = LogReaderFactory.createInstance(log, tracesToKeepForChildren);
					Cluster c = new Cluster(r, this, name + "." + i, new LogSketch(r));
					children.add(c);
				} catch (Exception e) {
	 ...
	 * The cluster has to be equipped with a HeursticsNet.
	 * If less than maxFeatures are discovered, the value maxFeatures is updated.
	 * Otherwise, the best maxFeatures features are mined.
	 */
	...
	/**
	 * @param en The HeuristicsNet to associate with the cluster.
	 */
