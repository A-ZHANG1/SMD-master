 ...
 * al framework il risultato dell'algoritmo, mediante il metodo getProvidedObjects
 */
...
public class DWSOutputGUI extends JPanel implements Provider {
	...
	private JComponent jComponentModel = null; //pannello su cui si disegna il modello
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		if (selectedCluster != null) {
			if (selectedCluster.getHeuristicsNet() != null) {
				a.add(new ProvidedObject("Heuristics net with " + selectedCluster.getName(),
	...
	/*
	 * Costruisce il jTree che viene visualizzato percorrend in modo ricorsivo la gerarchia.
	 */
	private void createNodes(DefaultMutableTreeNode top) {
		...
		for (int i = 0; i < nodoInfo.getChildren().size(); i++) {
			...
			DefaultMutableTreeNode next = new DefaultMutableTreeNode((Cluster) nodoInfo.getChildren().
										  get(i));
			top.add(next);
			createNodes(next);
		}
