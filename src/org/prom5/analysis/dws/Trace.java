...
public class Trace implements Comparable {
	...
	/*
	 * Restituisce l'indice associato ad un certo AuditTrailEntry
	 * FORSE DA MODIFICARE NEL CASO DI DuplicateTaskHeuristicsNet
	 */
	...
	public String toString() {
		...
		for (int i = 0; i < sequence.length - 1; i++) {
			ris += en.getLogEvents().getEvent(sequence[i]).getModelElementName() + ",";
		}
		ris += en.getLogEvents().getEvent(sequence[sequence.length - 1]).getModelElementName();
		return ris;
	}
	...
	public int compareTo(Object o) {
		...
		for (int i = 0; i < this.size() && ris == 0; i++) {
			if (en.getLogEvents().getEvent(sequence[i]).getModelElementName().compareTo(
					en.getLogEvents().getEvent(t.sequence[i]).getModelElementName()) < 0) {
				ris = -1;
			}
			if (en.getLogEvents().getEvent(sequence[i]).getModelElementName().compareTo(
					en.getLogEvents().getEvent(t.sequence[i]).getModelElementName()) > 0) {
	...
	public boolean extend(Trace t) {
		...
		if(t.size() >= this.size())
			return false;
		int inizio;
		for(inizio=0; inizio<this.size() && (this.sequence[inizio]!= t.sequence[0]); inizio++)
			;	
		boolean verify= true;
