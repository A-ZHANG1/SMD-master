...
package org.prom5.analysis.epc;
...
public class EPCCorrectnessCheckerUI extends JPanel implements Provider {
	...
	public final static String TRIVCORRECT =
			"The EPC is trivially correct, regardless of initial states.";
	public final static String CORRECT =
			"The EPC is correct, with respect to the chosen initial states.";
	public final static String INCORRECT = "The EPC contains structural errors.";
	public final static String RELCORRECT =
			"The EPC can be correct, but allows for undesired behaviour.";
	...
	private void jbInit() throws Exception {
		...
		rightTabPane.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				if (rightTabPane.getSelectedIndex() == 1 && (reducedEPC != null) &&
						(rightTabPane.getSelectedComponent() == null)) {
					if (reducedEPCVis == null) {
						reducedEPCVis = reducedEPC.getGrappaVisualization();
					}
					rightTabPane.setComponentAt(1, new JScrollPane(reducedEPCVis)); ;
				} else
				if (rightTabPane.getSelectedIndex() == 2 && (pnet != null) &&
						(rightTabPane.getSelectedComponent() == null)) {
					if (pnetVis == null) {
				...
				if (rightTabPane.getSelectedIndex() == 3 && (coverability != null) &&
						(rightTabPane.getSelectedComponent() == null)) {
					if (coverabilityVis == null) {
						coverabilityVis = coverability.getGrappaVisualization();
					}
					rightTabPane.setComponentAt(3, new JScrollPane(coverabilityVis)); ;
				}
			}
		});
		...
		tab0NextButton.addActionListener(new NavActionListener(tabbedPane, this));
...
//		Message.add("Initial places: " + initialEvents.values().toString());
	...
	protected void makeCoverabilityGraph() {
		...
		while (it.hasNext()) {
			...
			while (it2.hasNext()) {
				...
				if (p != null) {
					pnet.addEdge(t, p);
					pnet.addEdge(pnet.findPlace(COUNTER + p.getIdentifier()), t);
				} // if p==null then we found an event that is not in the reduced EPC
			}
		}
		...
		tab1nextButton.addActionListener(new NavActionListener(tabbedPane, this));
		tab1ButPanel.add(tab1nextButton);
		tab1.add(tab1ButPanel, BorderLayout.SOUTH);
		tab1Panel = new FinalEventSetSelectorPanel(finalStatesToKeep, finalStatesToRemove, epcVis);
	...
	protected void reduceEPC() {
		...
		if ((reducedEPC.getEvents().size() == 2) && (reducedEPC.getEdges().size() == 1) &&
				(((EPCEvent) reducedEPC.getEvents().get(0)).getSuccessors().contains(reducedEPC.
				getEvents().get(1)) ||
				((EPCEvent) reducedEPC.getEvents().get(1)).getSuccessors().contains(reducedEPC.
				getEvents().get(0)))) {
	...
	protected void colorCoverabilityGraph() {
		...
		// Then, we color the graph. We add all nodes to a hashmap that have an outgoing arc
...
//		this.add(jPanel2, BorderLayout.SOUTH);
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		if (epc != null) {
			objects.add(new ProvidedObject("Original EPC", new Object[] {epc}));
		}
		if (reducedEPC != null) {
			objects.add(new ProvidedObject("Reduced EPC", new Object[] {reducedEPC}));
		}
		if (pnet != null) {
			objects.add(new ProvidedObject("Petri net", new Object[] {pnet}));
		}
		if (coverability != null) {
			objects.add(new ProvidedObject("Coverability graph", new Object[] {coverability}));
		}
	...
	private void reportResult(String result) {
		...
		JOptionPane.showMessageDialog(MainUI.getInstance(), result, "Verification result",
...
class NavActionListener implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (tabbedPane.getSelectedIndex() == 0) {
			...
			// This step has edited the PetriNet and built the coverabilitygraph in "coverability"
		...
		if (tabbedPane.getSelectedIndex() == 1) {
			...
			// from an initial marking to a final marking in "coloredTransitions".
			// all other transitions are present in "notColoredTransitions"

			ui.checkORCovering();
			// The final step should check whether all OR-connectors are covered.
