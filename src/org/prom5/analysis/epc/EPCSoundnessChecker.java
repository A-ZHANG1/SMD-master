...
import org.prom5.converting.epc2transitionsystem.EPCMarking;
import org.prom5.converting.epc2transitionsystem.EpcToTransitionSystem;
...
import org.prom5.framework.models.transitionsystem.TransitionSystemVertexSet;
...
public class EPCSoundnessChecker {
    ...
    @Analyzer(name = "EPC Soundness Analysis (with existing TS)", names = {"EPC", "Transition System"}, connected=false)
    public JComponent analyze(ConfigurableEPC epc, TransitionSystem ts) {
        ...
        for (int i = ts.getVerticeList().size()-1; i>=0; i--) {
            ...
            if (node.getAttribute(EPCMarking.NOTE)==null ||
                !node.getAttributeValue(EPCMarking.NOTE).equals(EPCMarking.END)) {
        ...
        TransitionSystemVertexSet init = new TransitionSystemVertexSet("init", ts);
        ...
        for (ModelGraphVertex v: ts.getVerticeList()) {
            if (v != init && v.inDegree()==0) {
                ...
                for (String doc: vertex.getDocs()) {
                    if (doc.startsWith("+")) {
                        String toAdd = inEdge2inEvent.get(doc.substring(1)).getIdentifier();
                        if (toAdd.endsWith("\\n")) {
                ...
                label+="}";
                ts.addEdge(new TransitionSystemEdge(label,init,(TransitionSystemVertexSet)v));
            }
        }
    ...
    public String checkSoundness(TransitionSystem transitionSystem,
        ...
        while (it.hasNext()) {
            ...
            if (next.getInEdges() == null && next.getOutEdges() == null) {
                continue;
            } else if (transitionSystem.getStartStates().contains(next) /*next.getInEdges() == null*/) {
        ...
        if (noBadEnds == 0) {
            ...
            TransitionSystemVertexSet next = (TransitionSystemVertexSet)
                                             todolist.pop();
            if (next.getInEdges() != null) {
                ...
                while (it.hasNext()) {
                    ...
                    /**
                     * HV 2008-05-27: The start state has an incoming edge from a (dummy) node that is not
                     * a TransitionSytsemVertexSet! 
                     */
	                if (edge.getSource() instanceof TransitionSystemVertexSet) {
	                    TransitionSystemVertexSet vex = (TransitionSystemVertexSet)
	                            edge.getSource();
	                    if (vex.getAttribute(EPCMarking.NOTE) == null) {
	                        vex.setAttribute(EPCMarking.NOTE, EPCMarking.DEADLOCK);
                ...
                if (next.getAttribute(EPCMarking.NOTE) != null &&
                    next.getAttributeValue(EPCMarking.NOTE).equals(EPCMarking.
                        DEADLOCK)) {
                    next.setAttribute(EPCMarking.HIGHLIGHT, EPCMarking.DEADLOCK);
        ...
        while (toendlist.size() > 0) {
            ...
            if (next.getOutEdges() != null) {
                ...
                while (it.hasNext()) {
                    TransitionSystemVertexSet target = (TransitionSystemVertexSet) ((ModelGraphEdge) it.next()).
                                              getDest();
        ...
        while (it.hasNext()) {
            ...
            if (endV.getAttribute(EPCMarking.NOTE) != null &&
                endV.getAttributeValue(EPCMarking.NOTE).equals(EPCMarking.END)) {
        ...
        HashMap<String, ModelGraphEdge>
                IdEdge = new HashMap<String, ModelGraphEdge>();
        it = edges.iterator();
        ...
        if (isSound == true) {
            ...
            it = goodstarts.iterator();
            while (it.hasNext()) {
                result = result + ((ModelGraphVertex) it.next()).getIdentifier() +
                         "<br>";
            }
            if (goodends.size() > 1) {
                result = result + "There are " + goodends.size() +
                         " final markings which can be reached from the initial markings:<br>";
            } else if (goodends.size() == 1) {
                result = result + "There is " + goodends.size() +
            ...
            if (badEndArcs.size() > 0) {
                ...
                while (it.hasNext()) {
                    id = (String) it.next();
                    String endLabel = ((ModelGraphEdge) IdEdge.get(id)).getDest().
                                      getIdentifier();
