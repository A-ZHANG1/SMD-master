...
public class SettingsPanel extends JPanel {
	private RedRulePanel[] rr = new RedRulePanel[] {new SingleInSingleOutRedRule(),
								new SplitSameJoinRedRule(),
								new SplitOrJoinRedRule(),
								new SimilarSplitSeqRedRule(),
								new SimilarJoinSeqRedRule(),
								new XorLoopRedRule(),
								new OrLoopRedRule()
	...
	public ConfigurableEPC reduce(ConfigurableEPC epc) {
		return ConnectorStructureExtractor.extract(epc,
				rr[0].include(),
				rr[1].include(),
				rr[2].include(),
				rr[3].include(),
...
abstract class RedRulePanel extends JPanel {
	...
	public RedRulePanel() {
		...
		p2.add(new JLabel("<html><center><B>" + getDescription() + "</B></center></html>"));
...
class SplitOrJoinRedRule extends RedRulePanel {
	...
	protected String getHelp() {
		return
				"Removes multiple paths from split nodes that are directly followed by a join node of type \"OR\"." +
				"Note that the paths are not removed if they contain any node, such as an event or function.";
	}
}
