...
public class ControlFlow implements ICalculator {
		...
		for(int count = 0; count <i; count++){
			...
			if(epc.getConnectors().get(count).toString().startsWith("XOR")){
				...
				Message.add("\t\t<XORConnector " +
						"name=\""+((EPCConnector)epc.getConnectors().get(count)).toString()+"\" " +
						"value=\""+fanout(epcObj)+"\"/>", Message.TEST);
			} else
				if (epc.getConnectors().get(count).toString().startsWith("OR")){
					EPCObject epcObj = (EPCObject) epc.getConnectors().get(count);
					...
					if (epc.getConnectors().get(count).toString().startsWith("AND")){
						++result;
						Message.add("\t\t<ANDConnector " +
						"name=\""+((EPCConnector)epc.getConnectors().get(count)).toString()+"\" " +
						"value=\""+1+"\"/>", Message.TEST);
					}
		}
	...
	/*The basic requirements for this  metric: At least one connector,one function and one*/
