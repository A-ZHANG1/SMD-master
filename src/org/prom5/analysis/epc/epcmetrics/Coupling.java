...
public class Coupling implements ICalculator {
	...
	public String Calculate() {
		...
		for(int i=0; i<numEdges; i++){
			...
			if(tailName.startsWith("OR")){
				...
				double second = ((power(2,outEdges)-1)-1)/(double)((power(2,outEdges)-1)*outEdges);
			...
			if(headName.startsWith("OR")){
				...
				double second = ((power(2,inEdges)-1)-1)/(double)((power(2,inEdges)-1)*inEdges);
			...
			Message.add("\t\t<ArcValue name=\""+edge.toString()
					+"\" value=\""+(headValue*tailValue)+"\"/>", Message.TEST);
		}
		result = 0;
		...
		for(int count = 0; count<paths.size(); count++){
			Double newdouble = (Double)((Vector)paths.elementAt(count)).lastElement();
			result = result + (double)newdouble;
		}
		double numOfPossibleConnections = this.getNumberOfActivities() * (this.getNumberOfActivities() - 1);
	...
	private void checkPaths(Vector aVec){
		for(int i =0; i<paths.size(); i++){
			if(((Vector)paths.elementAt(i)).firstElement().equals(aVec.firstElement())){
				Node node1 = (Node)((Vector)paths.elementAt(i)).elementAt(((Vector)paths.elementAt(i)).size()-2);
	...
	private float getNumberOfActivities(){
		float result = this.epc.getEvents().size() + this.epc.getFunctions().size();
		return result;
	}
