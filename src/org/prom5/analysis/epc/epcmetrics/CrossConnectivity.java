...
/**
 * <p>Title: EPC complexity analysis: Cross-Connectivity</p>
 *
 * <p>Description: Calculates the Cross-Connectivity value for a process model</p>
 *
...
/*	   Iterator itpa = paths.iterator();
	   while (itpa.hasNext()){
		   Vector v = (Vector) itpa.next();
...
//			   System.out.print(n2.toString()+ ": ");
			   ...
			   // calculate the maximum value over all paths connecting node 'n1' and 'n2'
...
//			   Message.add("\t</PathValue \"" + n1 + "->" + n2 + ": " + conValue + "\">", Message.TEST);
		   }
	   }
	   int numNodes = nodes.size();
	   result = conn / (numNodes * (numNodes-1));
	   Message.add("\t<CrossConnectivityValue value=\"" + result + "\"/>", Message.TEST);
	   Message.add("\t</CrossConnectivity>", Message.TEST);
...
//	System.out.print(v2.firstElement().toString() + " -> " + v2.lastElement().toString() + ": ");
	while (v2.size() > 1) {
...
public void calculatePaths(Vector path){
	...
	else numOutEdges = outEdges.size();
	if (numOutEdges > 0){
	    for (int i = 0; i < numOutEdges; i++) {
			Vector path2 = (Vector) path.clone();
			EPCEdge edge = (EPCEdge) outEdges.get(i);
			Node node3 = edge.getHead(); // the head of the arc points to this node
			...
			// If not, then add the edge and node to the path and add the path to the set of paths
			if (!nodeExists) {
				path2.add(edge);
				path2.add(node3);
				paths.add(path2);
...
//	Message.add("\t</ArcWeight \"" +edge.toString() + ": " +result + "\">", Message.TEST);
	return result;
}
...
public double getNodeWeight(Node node){
	...
	// And calculate the weight of the node based on the definition in the CAiSE'08 paper
	if (name.startsWith("OR")) {
		double first = 1.0 / (double) (power(2, degree) - 1.0);
		double second = ((power(2, degree) - 1.0) - 1.0) / (double) ((power(2,degree) - 1.0) * degree);
		result = first + second;
	}
