...
public class EPCComplexityCalculatorUI extends JPanel implements Provider {
	...
	public EPCComplexityCalculatorUI(ConfigurableEPC orgEPC) {
		...
		Message.add("<EPCComplexityCalculator numoffunctions=\""+epc.getFunctions().size()
				+"\" numofevents=\""+epc.getEvents().size()
				+"\" numofedges=\""+epc.getEdges().size()+"\">", Message.TEST);
		try {
			jbInit();
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void jbInit() throws Exception {
		...
		this.splitpaneLeft.setOrientation(javax.swing.JSplitPane.VERTICAL_SPLIT);
        ...
        this.spTable.addMouseMotionListener(new java.awt.event.MouseMotionAdapter() {
	...
	private String[][] getValues(){
		...
		out1[0] = calc.getName();
		out1[1] = calc.getType();
		if(!calc.VerifyBasicRequirements().startsWith(".")){ //returns the error message in case the EPC has something wrong
			String text = this.ErrorTextArea.getText();
			this.ErrorTextArea.setText(text + "\n[Control Flow]"+calc.VerifyBasicRequirements());
		...
		if(!calc.VerifyBasicRequirements().startsWith(".")){ //returns the error message in case the EPC has something wrong
			String text = this.ErrorTextArea.getText();
			this.ErrorTextArea.setText(text + "\n[Density]"+calc.VerifyBasicRequirements());
		...
		if(!calc.VerifyBasicRequirements().startsWith(".")){ //returns the error message in case the EPC has something wrong
			String text = this.ErrorTextArea.getText();
			this.ErrorTextArea.setText(text + "\n[Functions]"+calc.VerifyBasicRequirements());
		...
		if(!calc.VerifyBasicRequirements().startsWith(".")){ //returns the error message in case the EPC has something wrong
			String text = this.ErrorTextArea.getText();
			this.ErrorTextArea.setText(text + "\n[Events]"+calc.VerifyBasicRequirements());
		...
		if(!calc.VerifyBasicRequirements().startsWith(".")){ //returns the error message in case the EPC has something wrong
			String text = this.ErrorTextArea.getText();
			this.ErrorTextArea.setText(text + "\n[ORs]"+calc.VerifyBasicRequirements());
		...
		if(!calc.VerifyBasicRequirements().startsWith(".")){ //returns the error message in case the EPC has something wrong
			String text = this.ErrorTextArea.getText();
			this.ErrorTextArea.setText(text + "\n[XORs]"+calc.VerifyBasicRequirements());
		...
		if(!calc.VerifyBasicRequirements().startsWith(".")){ //returns the error message in case the EPC has something wrong
			String text = this.ErrorTextArea.getText();
			this.ErrorTextArea.setText(text + "\n[ANDs]"+calc.VerifyBasicRequirements());
		...
		if(!calc.VerifyBasicRequirements().startsWith(".")){ //returns the error message in case the EPC has something wrong
			String text = this.ErrorTextArea.getText();
			this.ErrorTextArea.setText(text + "\n[Coupling]"+calc.VerifyBasicRequirements());
		...
		if(!calc.VerifyBasicRequirements().startsWith(".")){ //returns the error message in case the EPC has something wrong
			String text = this.ErrorTextArea.getText();
			this.ErrorTextArea.setText(text + "\n[Cross-Connectivity]"+calc.VerifyBasicRequirements());
		}out7[2] = calc.Calculate();



		String[][] final_output = {out1,out2,
									out7,out4,out5,
									out6, out8, out9
									};
		return final_output;
