package org.prom5.analysis.epc.similarity;
...
import org.prom5.analysis.causality.FootprintSimilarityResult;
import org.prom5.analysis.causality.FootprintSimilarityResultUI;
import org.prom5.analysis.causality.Similarity;
...
public class ActivityFragmentBuilder implements AnalysisPlugin {
	...
	public AnalysisInputItem[] getInputItems() {
		AnalysisInputItem base = new AnalysisInputItem("Comparison Base", 1, 1000) {
		...
		AnalysisInputItem comp = new AnalysisInputItem(
				"Compare to (optional; if left empty then the base is compared with itself)", 0, 1000) {
			public boolean accepts(ProvidedObject object) {
				Object[] o = object.getObjects();
	...
	public void buildList(AnalysisInputItem item, ArrayList<ConfigurableEPC> list) {
		for (ProvidedObject obj : item.getProvidedObjects()) {
			for (Object o : obj.getObjects()) {
				if (o instanceof ConfigurableEPC) {
					((ConfigurableEPC) o).object = StringNormalizer.normalize(getPath((ConfigurableEPC) o, null));
					...
					for (Object o2 : hierarchy.getAllObjects()) {
						if (o2 instanceof ConfigurableEPC) {
							((ConfigurableEPC) o2).object = StringNormalizer.normalize(getPath((ConfigurableEPC) o2,
									hierarchy));
	...
	public JComponent analyse(AnalysisInputItem[] analysisInputItemArray) {
		...
		if (baseEpcs.size() == 0 || compareToEpcs.size() == 0) {
			Message.add("At least one syntactically valid EPC is necessary for comparison.", Message.ERROR);
			return null;
		}

		Message.add("Starting comparison of " + baseEpcs.size() + " base epcs with " + compareToEpcs.size() + " epcs.");

		Progress progress = new Progress("Comparing EPCs", 0, baseEpcs.size() * compareToEpcs.size() + 2);
		...
		Comparator comp = new Comparator() {
			public int compare(Object o1, Object o2) {
				...
				return -(epc1.getFunctions().size() * epc1.getEdges().size() - epc2.getFunctions().size()
						* epc2.getEdges().size());
			}
		...
		progress.setNote("Initializing Checker (might take some time, but happens only once)");
		...
		if (dialog.showSettings()) {
			...
			FootprintSimilarityResult result = new FootprintSimilarityResult(basePaths, baseEpcs, compareToPaths,
					compareToEpcs, options.getFootprintFolder());

			String title = options.toString();
			similarities = new Similarities(title, baseEpcs.size(), compareToEpcs.size());
		...
		progress.close();
		Message.add("Performed " + comps + " comparisons with these options: " + options.toString());
	...
	public void addToInvalidEntryReplaceList(int baseIndex, int compareToIndex, int copyFromBaseIndex,
			int copyFromCompareToIndex) {
		synchronized (replacementList) {
			replacementList.add(new int[] { baseIndex, compareToIndex, copyFromBaseIndex, copyFromCompareToIndex });
		}
	}
	...
	public void fillSimilarities(FootprintSimilarityResult result, Progress progress, SimilarityOptions options) {
		...
		FootprintBuilderThread[] threads = new FootprintBuilderThread[options.getParallelThreads()];
		for (int i = 0; i < options.getParallelThreads(); i++) {
			SimilarityCalculator simCalc;
			if (!progress.isCanceled()) {
				simCalc = new SimilarityCalculator(options.getFunctionThreshold(), options.getFunctionSyntaxWeight(),
						options.getFunctionSemanticWeight(), options.getFunctionStructureWeight());
				simCalc.setContextSimilarityCalculator(new SimilarityCalculator(options.getEventThreshold(), options
						.getEventSyntaxWeight(), options.getEventSemanticWeight(), 0));

				threads[i] = new FootprintBuilderThread(result, progress, this, options.getRemoveInitialFinalNodes(), options.getUseSimilarityValues(), simCalc);
	...
	protected final Similarity compare(int baseEpc, int compEpc, CausalFootprint cfp1, CausalFootprint cfp2,
			List<ActivityContextFragment> frags1, List<ActivityContextFragment> frags2, Progress progress,
			boolean removeInitialAndFinalNodes, boolean useSimilarityValues, SimilarityCalculator simCalc) {
		...
		if ((comps % messagePoint) == 0) {
			...
			long expectedRemaining = Math.round(((double) timeSoFar / (double) comps)
					* (((baseEpcs.size() * (compareToEpcs.size()))) - comps));
			// Message.add("Expected time remaining: " +
			// StopWatch.formatDuration(expectedRemaining));
			progress.setNote("Remaining: " + StopWatch.formatDurationNoMilliSeconds(expectedRemaining));
		}
		int[] funcMapping;
		double[] similarities;
		...
		try {
			...
			// similarity 0.0
			funcMapping = simCalc.getBestPossibleMapping(similarities, frags1, frags2);
		} catch (Exception ex) {
		...
		for (int i = 0; i < baseEpcs.get(baseEpc).getFunctions().size(); i++) {
			EPCFunction f1 = (EPCFunction) baseEpcs.get(baseEpc).getFunctions().get(i);
			Entry entry;
			if (funcMapping[i] == -1) {
				mapping.put(f1, CausalFootprintSimilarityResult.NOMAP);
				noMaps++;
			} else {
				mapping.put(f1, (EPCFunction) compareToEpcs.get(compEpc).getFunctions().get(funcMapping[i]));
				if (useSimilarityValues) {
					similarityMap.put(new SimpleEntry(f1, mapping.get(f1)), similarities[i * frags2.size()
							+ funcMapping[i]]);
				}
			}
		}
		...
		if (noMaps == mapping.keySet().size()) {
			...
			CausalFootprintSimilarityResult footPrintSimilarity = new CausalFootprintSimilarityResult(cfp1, cfp2,
					mapping);
			sim = footPrintSimilarity.calculateSimilarity(progress, removeInitialAndFinalNodes, similarityMap);
	...
	private void addConditionalToList(ConfigurableEPC epc, ArrayList<ConfigurableEPC> epcs) {
		...
		if (epc.getFunctions().size() > 0 && startEvent && endEvent && epc.isValidEPC().length() == 0) {
			epcs.add(epc);
			// paths.add(label);
		} else {
			return;
	...
	private List<ActivityContextFragment> getFragments(ConfigurableEPC epc) {
		...
		for (EPCFunction function : (ArrayList<EPCFunction>) epc.getFunctions()) {
			...
			ArrayList<EPCEvent> pre = (ArrayList<EPCEvent>) epc.getPreceedingEvents(function);
			for (EPCEvent event : pre) {
				fragment.addToInputContext(event.getIdentifier());
			}

			ArrayList<EPCEvent> post = (ArrayList<EPCEvent>) epc.getSucceedingEvents(function);
...
class FootprintBuilderThread extends org.prom5.framework.ui.SwingWorker {
	...
	public FootprintBuilderThread(FootprintSimilarityResult result, Progress progress, ActivityFragmentBuilder owner,
			boolean removeInitialAndFinalNodes, boolean useSimilarityValues, SimilarityCalculator simCalc) {
	...
	private void buildNextFootprintAndCompare(int baseIndex) {
		CausalFootprint footprint = result.getBaseFootPrint(baseIndex, progress);
		...
		while (totalDone != done.length && !progress.isCanceled()) {
			...
			try {
				...
				double similarity = result.getSimilarity(owner.getCompareToPath(j), owner.getBasePath(baseIndex));
				if (similarity < 0) {
					// similarity has not been determined yet.
					owner.getSimilarities().set(baseIndex, j, Similarities.BEING_PROCESSED);
					sim = owner.compare(baseIndex, j, footprint, compFootprint, owner.getBaseFragments(baseIndex),
							owner.getCompareToFragments(j), progress, removeInitialAndFinalNodes, useSimilarityValues, simCalc);
					compFootprint = null;
					...
					// being compared
					int copyFromCompareToIndex = result.getCompareToIndex(owner.getBasePath(baseIndex));
					int copyFromBaseIndex = result.getBaseIndex(owner.getCompareToPath(j));

					if (similarity != Similarities.BEING_PROCESSED) {
						owner.getSimilarities().set(baseIndex, j, similarity);
					} else {
						owner.addToInvalidEntryReplaceList(baseIndex, j, copyFromBaseIndex, copyFromCompareToIndex);
					}
				}
			} catch (OutOfMemoryError ex) {
				owner.getSimilarities().set(baseIndex, j, Similarities.INVALID);
				Message.add("Out of memory while comparing: " + owner.getBaseEPCIdentifier(baseIndex) + " with "
