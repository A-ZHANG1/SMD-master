...
public class SimilarityCalculator {
	...
	public SimilarityCalculator(double threshold, double syntaxWeight, double semanticWeight,
		...
		if (checker == null) {
			Message.add(
					"Initializing semantic checker... (might take some time, but happens only once in a ProM session)");
			this.checker = new Checker(UISettings.getProMDirectoryPath() + "lib" +
	...
	private double getStructuralEquivalence(ActivityContextFragment f1, ActivityContextFragment f2) throws Exception{
		...
		return (getContextSimilarity(f1.getInputContext(), f2.getInputContext()) +
				getContextSimilarity(f1.getOutputContext(), f2.getOutputContext())) / 2;
	}

	private double getContextSimilarity(Vector<String> input1, Vector<String> input2) throws Exception{
		if (contextCalculator == null) {
			//ERROR, context similarity requested, but context calculator not set.
			throw new Exception() {

			};
		}

		int[] mapping = contextCalculator.getBestPossibleMapping(input1, input2);
		...
		return ((double) (number)) / (Math.sqrt(input1.size()) * Math.sqrt(input2.size()));
	}

	private double getSimilarity(Object f1, Object f2) throws Exception{
		if ((f1 instanceof ActivityContextFragment) && (f2 instanceof ActivityContextFragment)) {
			return getSimilarity((ActivityContextFragment) f1, (ActivityContextFragment) f2);
		} else {
			return (syntaxWeight * getSyntacticEquivalence(f1.toString(), f2.toString()) +
					semanticWeight * getSemanticEquivalence(f1.toString(), f2.toString())) /
	...
	public double getSimilarity(ActivityContextFragment f1, ActivityContextFragment f2) throws Exception{
		...
		double syntax = getSyntacticEquivalence(f1.getActivityName(), f2.getActivityName());
		double semantic = getSemanticEquivalence(f1.getActivityName(), f2.getActivityName());
		double structural = getStructuralEquivalence(f1, f2);
		double overall = (syntaxWeight * syntax +
				semanticWeight * semantic +
				structureWeight * structural )/
				(syntaxWeight + semanticWeight + structureWeight);

//		System.out.println(f1);
//		System.out.println(f2);
//		System.out.println("syntax: "+syntax+", semantic: "+semantic+", structural: "+structural+", overall: "+ overall);
//		System.out.println("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
	...
	public int[] getBestPossibleMapping(
			Vector<String> frags1,
			Vector<String> frags2
			) throws Exception {
	...
	public int[] getBestPossibleMapping(double[] similarities, List from, List to) {
		...
		//   for all 0 < i < frags1.size(): sum 0 < j < frags2.size() y[i*frags2.size()+j] <= 1
		//   for all 0 < j < frags2.size(): sum 0 < i < frags1.size() y[i*frags2.size()+j] <= 1
		//   maximize sum 0 < i < frags1.size(), 0 < j < frags2.size()  y[i*frags2.size()+j]* x[i*frags2.size()+j]
		...
		try {
			...
			for (int i = 0; i < similarities.length; i++) {
				...
				solver.setUpbo(i + 1, (similarities[i] > getThreshold() ? 1 : 0));
			}
