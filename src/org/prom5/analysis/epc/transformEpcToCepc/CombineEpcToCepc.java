...
/**
 * <p>Title: TransformEpcToCepc</p>
 * 
 * <p>Description: Analysis Plugin that combines two EPC process models into an EPC process model.</p>
 * 
...
public class CombineEpcToCepc {
	...
	@Analyzer(name = "Transform EPCs to a combined EPC", help = "<h1>Transform two EPC's into one EPC</h1><p>Combines the two specified EPC models into one new EPC. Focus in combining processes is on <ol><li>adding as little extra behavior as possible,</li><li>try to combine the models with low number of OR-connectors</li></ol>There is no attention on the complexity of the model (in terms of number of connectors, edges, etc)</p><br><p>Other documentation can be found in my Masters Thesis 'Combining Process Models into an Integrated Process Model'.</p>", names = {"EPC Model 1", "EPC Model 2"}, connected = false)
