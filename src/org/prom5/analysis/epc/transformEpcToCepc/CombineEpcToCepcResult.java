package org.prom5.analysis.epc.transformEpcToCepc;
...
import org.prom5.analysis.epc.epcmetrics.ControlFlow;
import org.prom5.analysis.epc.epcmetrics.Coupling;
import org.prom5.analysis.epc.epcmetrics.CrossConnectivity;
import org.prom5.analysis.epc.epcmetrics.Density;
import org.prom5.analysis.epc.epcmetrics.ICalculator;
import org.prom5.analysis.epc.epcmetrics.NumberOfANDs;
import org.prom5.analysis.epc.epcmetrics.NumberOfEvents;
import org.prom5.analysis.epc.epcmetrics.NumberOfFunctions;
import org.prom5.analysis.epc.epcmetrics.NumberOfORs;
import org.prom5.analysis.epc.epcmetrics.NumberOfXORs;
...
/**
 * <p>Title: TransformEpcToCepcResult</p>
 * 
 * <p>Description: combines TWO EPC's into an integrated EPC. Algorithm can be extended
 * in order to create an integrated C-EPC (not done yet, future work?)</p>
 * 
...
public class CombineEpcToCepcResult extends JPanel implements Provider {
	...
	private ArrayList<EPCObject> ncfNodesEpc1, ncfNodesEpc2; // non-control flow nodes
	private HashMap<Integer,Integer> mappingEpc1, mappingEpc2; // mapping of ncf node ID's to ncf nodes of new EPC ID's
	...
	public CombineEpcToCepcResult(ConfigurableEPC inputEpc1, ConfigurableEPC inputEpc2)
	{
		this.epc1 = inputEpc1;
		this.epc2 = inputEpc2;
		...
		this.paneMain = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true, leftPanel, tab);
		...
		this.model1Panel.add(new JScrollPane(this.epc1.getGrappaVisualization()), BorderLayout.CENTER);
		this.model2Panel.add(new JScrollPane(this.epc2.getGrappaVisualization()), BorderLayout.CENTER);
		this.cmodelPanel.add(new JScrollPane(this.epcNew.getGrappaVisualization()), BorderLayout.CENTER);
	...
	private void mergeModels()
	{
		...
		Message.add("Start: validate relations and add relations to new EPC", Message.TEST);
		...
		//System.out.println("new epc has " + this.epcNew.getFunctions().size() + " functions and " + this.epcNew.getEvents().size()+ 
		//		" events and " + this.epcNew.getEdges().size() + " relations and " + this.epcNew.getConfigurableObjects().size() + " configurable objects");
	 ...
	 * @param ncfNodes; list of all functions and events
	 */
	...
	/**
	 * add the connector chain relations from specified EPC to the relation matrix
	 * @param epc
	 * @param mapping
	 */
	private void addRelations(ConfigurableEPC epc, HashMap<Integer,Integer> mapping)
	{
		...
		for(Iterator<EPCObject> nI = ncfNodes.iterator(); nI.hasNext();)
		{
			...
			if(connOut != -1)
			{
				...
				{
					// reassessConnectorType ( node, current type, new type, isSplit )
					this.relationMapping[newFromId][connInOutId] = reassessConnectorType(newFromId, this.relationMapping[newFromId][connInOutId], connOut,true);
				}else{
			...
			for(Iterator<EPCObject> psI = succNodes.iterator(); psI.hasNext();)
			{
				...
				if(connIn != -1)
				{
					...
					if(currentNode2 != 0)
					{
						this.relationMapping[connInOutId][newToId] = reassessConnectorType(newToId, this.relationMapping[connInOutId][newToId], connIn, false);
					}else{
	...
	private void addRelation(EPCObject nodeFrom, EPCObject nodeTo)
	{
		...
		if(this.relationMapping[fromId][matrixSize-1] != 0 && this.relationMapping[fromId][matrixSize-1] != -1)
		{
		...
		if(this.relationMapping[matrixSize-1][toId] != 0 && this.relationMapping[matrixSize-1][toId] != -1)
		{
	...
	/**
	 * get the preceding/succeeding connector of specified node. If node has no connector, the node itself is returned
	 * @param node
	 * @param isInConnector
	...
	private EPCObject getConnector(EPCObject node, boolean isInConnector)
	{
		...
		ps = (isInConnector == false ? getPostSet(node, false) : getPreSet(node, false));
		...
		if(ps.size() == 1)
		{
			...
			if(isInConnector == false)
			{
				...
				connectorType = this.relationMapping[this.relationMapping.length-1][node.getId()];
			}
			
			EPCConnector conn = new EPCConnector( connectorType, false, this.epcNew);
	 ...
	 */
	private int getConnectorType(EPCObject node, boolean succeedingNodes, boolean isInConnector)
	{
		ArrayList<EPCObject> ps = new ArrayList<EPCObject>();
	 ...
	 * @return
	 */
	private int reassessConnectorType(int nodeId, int currentConnectorType, int newConnectorType, boolean isSplit)
	{
		switch(currentConnectorType)
		{
			...
			case AND:
				ArrayList<EPCObject> psNode1, psNode2, nodeList1, nodeList2;
				ArrayList<Integer> newNodeIDsList1, newNodeIDsList2;
				EPCObject node1, node2;
				...
				if(newConnectorType == AND)
				{
					...
					if(node1Id != -1 && node2Id != -1)
					{
						...
						newNodeIDsList1 = getNodeIDsFromMapping(psNode1, this.mappingEpc1);
						newNodeIDsList2 = getNodeIDsFromMapping(psNode2, this.mappingEpc2);
						
						if(newNodeIDsList1.containsAll(newNodeIDsList2) && newNodeIDsList2.containsAll(newNodeIDsList1))
						{
							return AND;
						}else{
							return OR;
	...
	/**
	 * validate the relations and connectors in the relation matrix. Adds connectors if a ncf node has
	 * more then one incoming or outgoing edge
	 */
	private void reassessRelationMatrix()
	{
		...
		for(int row = 0; row <= maxValueIndex; row++)
		{
			...
			if(numRelations > 1 && currentType != OR && currentType != AND && currentType != XOR)
			{
	...
	/**
	 * returns the post-set of a ncf node if connectorChained is false, if true all connector chained ncf nodes are returned
	 * @param node
	 * @param connectorChained
	 * @return
	 */
	private ArrayList<EPCObject> getPostSet(EPCObject node, boolean connectorChained )
	{
		ArrayList<EPCObject> ps = new ArrayList<EPCObject>();
		if(node == null)
		{
	 ...
	 * @return
	 */
	private ArrayList<EPCObject> getPostSet(EPCObject node, ArrayList<EPCObject> postset, boolean connectorChained )
	{
	 ...
	 * getPreSet of node
	 * @param the node
	 * @param connectorChained; true if all next functions or events need to be returned; false for all next nodes
	 * @return arraylist of all preceeding nodes
	 */
	private ArrayList<EPCObject> getPreSet(EPCObject node, boolean connectorChained )
	{
		ArrayList<EPCObject> ps = new ArrayList<EPCObject>();
		if(node == null)
		{
	...
	private ArrayList<EPCObject> getPreSet(EPCObject node, ArrayList<EPCObject> preset, boolean connectorChained )
	{
	...
	/**
	 * check EPC models for similar ncf nodes, create new ncf nodes, match ncf nodes from original EPC to new EPC 
	 * @param ncfNodeList1
	 * @param ncfNodeList2
	 * @param new EPC
	 */
	private void match(ArrayList<EPCObject> ncfNodeList1, ArrayList<EPCObject> ncfNodeList2, ConfigurableEPC epc)
	{
		...
		Checker syntacticChecker = new Checker(UISettings.getProMDirectoryPath() + "lib" +
                System.getProperty("file.separator") + "plugins" +
                System.getProperty("file.separator") + "similarity" +
                System.getProperty("file.separator"));
		
		// match ncf nodes
		for(Iterator<EPCObject> list1I = ncfNodeList1.iterator(); list1I.hasNext();)
		{
			...
			while(equivalentFound == false && list2I.hasNext())
			{
				...
				// if similar/equivalent based on....label||syntactic similarity(%) and not a dummy				
				if((l1Node.toString().startsWith("[_dummy_")==false) && (l2Node.toString().startsWith("[_dummy_")==false) &&
					(syntacticChecker.syntacticEquivalenceScore( l1Node.toString().toLowerCase(), l2Node.toString().toLowerCase()) >= SYNEQUIVSCORE))
				{
					// equivalent
				...
				else if((l1Node.toString().startsWith("[_dummy_")==true) && (l2Node.toString().startsWith("[_dummy_")==true) &&
		...
		{
			for(Iterator<EPCObject> remList2I = ncfNodeList2.iterator(); remList2I.hasNext();)
			{
				EPCObject remL2Node = (EPCObject) remList2I.next();
				this.mappingEpc2.put(remL2Node.getId(), newNcfNode(remL2Node, true));
			}
		}		
	}
	...
	private int newNcfNode(EPCObject oldNode, boolean isConfigurable)
	{
		...
		{
			EPCFunction newFunction = new EPCFunction(new LogEvent(ncfNodeLabel, ""), isConfigurable, this.epcNew);
	...
	private boolean checkIfDummiesMatch(EPCObject dummy1, EPCObject dummy2)
	{
		...
		if(isDummySubsetOf(presetD1, presetD2) || isDummySubsetOf(presetD2, presetD1))
		{
			return true;
		}else if(isDummySubsetOf(postsetD1, postsetD2) || isDummySubsetOf(postsetD2, postsetD1))
		{
			return true;
		}else{
			return false;
	...
	/**
	 * Checks if set 2 contains all elements of set 1, or if set 2 is a subset of set 1 
	 * @param set1
	 * @param set2
	 * @return true if set 2 contains all elements from set 1 or if set 2 is a subset of set 1. Else returns false
	 */
	private boolean isDummySubsetOf(ArrayList<EPCObject> set1, ArrayList<EPCObject> set2)
	{
	...
	/**
	 * add temporary dummy events to the EPC if connector node has a succeeding connector
	 * @param epc
	...
	private void removeDummies(ConfigurableEPC epc)
	{
		...
		for(Iterator<EPCObject> nodeI = ncfNodes.iterator(); nodeI.hasNext();)
		{
			...
			if(node.toString().startsWith("[_dummy_"))
			{
				...
				{
					Message.add("Execption when removing dummy nodes from EPC model " + epc.getName(), Message.ERROR);
				}
			}
		}
	}
	...
	/**
	 * add a dummy event and edges between 'from' to 'to'
	 * removes prior relation between 'from' and 'to'
	 * @param from
	 * @param to
	 */
	private void addDummy(int id, EPCObject from, EPCObject to, ConfigurableEPC epc)
	{
	 ...
	 * @param epc
	 */
	private void deleteEdge(EPCObject edgeFrom, EPCObject edgeTo, ConfigurableEPC epc)
	{
	...
	/**
	 * removes all connector nodes that have only one incoming and one outgoing edge or
	 * have one edge 
	 */
	private void removeRedundantConnectors()
	{
		...
		for( Iterator<EPCConnector> cI = connectors.iterator(); cI.hasNext();)
		{
			...
			// if # of incoming and outgoing edges is one then remove the connector and make a new edge to replace the connector
			if( con.inDegree() == 1 && con.outDegree() == 1 )
			{
				// get the pre-node and suc-node
				ArrayList<EPCObject> preset = getPreSet( con, false); // holds from
				ArrayList<EPCObject> postset = getPostSet( con, false ); // holds to
	 ...
	 */
	private int getOldNodeIdFromMapping(int newNodeId, HashMap<Integer, Integer> mapping)
	{
	 ...
	 */
	private ArrayList<Integer> getNodeIDsFromMapping(ArrayList<EPCObject> oldNodes, HashMap<Integer, Integer> mapping)
	{
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		if (this.epc1 != null) {
			objects.add(new ProvidedObject("EPC Model 1", new Object[] {this.epc1}));
		}
		if (this.epc2 != null) {
			objects.add(new ProvidedObject("EPC Model 2", new Object[] {this.epc2}));
		}
		if (this.epcNew != null) {
			objects.add(new ProvidedObject("Transformed EPC", new Object[] {this.epcNew}));
		}
