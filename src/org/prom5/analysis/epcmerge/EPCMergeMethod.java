...
import org.prom5.framework.log.rfb.AuditTrailEntryImpl;
import org.prom5.framework.log.rfb.AuditTrailEntryListImpl;
...
import org.prom5.framework.models.epcpack.EPCEdge;
import org.prom5.framework.models.epcpack.EPCEvent;
import org.prom5.framework.models.epcpack.EPCFunction;
import org.prom5.framework.models.epcpack.EPCObject;
import org.prom5.framework.ui.Message;
import org.prom5.framework.ui.Progress;
import org.prom5.mining.epcmining.EPCResult;
import org.prom5.mining.logabstraction.FSMLogRelationBuilder;
import org.prom5.mining.logabstraction.LogAbstraction;
import org.prom5.mining.logabstraction.LogAbstractionImpl;
import org.prom5.mining.logabstraction.LogRelations;
import org.prom5.mining.logabstraction.MinValueLogRelationBuilder;
import org.prom5.mining.logabstraction.TimeIntervalLogRelationBuilder;
import org.prom5.mining.partialordermining.AggregationGraphResult;
import org.prom5.mining.partialordermining.PartialOrderAggregationPlugin;
import org.prom5.mining.partialordermining.PartialOrderGeneratorPlugin;
import org.prom5.mining.partialordermining.PartialOrderMiningResult;
	...
	protected class EPCStructure {
		...
		private void calculateVisitedEdges() {
			...
			while (itFunction.hasNext()) {
				...
				Iterator<EPCEvent> itEvent=epc.getSucceedingEvents(source).iterator();
				while (itEvent.hasNext()) {
					if (visitedEdges.containsKey(source)) {
						Hashtable<EPCObject, Integer> target=visitedEdges.get(source);
		...
		private Integer getEventRelation(List<String> events, EPCObject startElement, EPCEvent startEvent, Integer relation, boolean succeedingCon) {
			Iterator<EPCObject> oIt=(succeedingCon ? epc.getSucceedingElements(startElement).iterator() : epc.getPreceedingElements(startElement).iterator());
			while (oIt.hasNext()) {
				EPCObject o=oIt.next();
				if (EPCConnector.class.isAssignableFrom(o.getClass())) {
					List<EPCEvent> sucEvent=(succeedingCon ? epc.getPreceedingEvents((EPCConnector) o) : epc.getSucceedingEvents((EPCConnector) o));
					Iterator<String> eventIt=events.iterator();
					while (eventIt.hasNext()) {
						EPCEvent eventToCheck=epc.getEvent(eventIt.next());
						if (eventToCheck!=startEvent && sucEvent.contains(eventToCheck)) {
							if (relation.intValue()==XOR && !(o.getType()== AND || o.getType()== OR)){
								relation= new Integer(XOR);
							} else if (relation.intValue()==AND && !(o.getType()==XOR || o.getType()==OR)){
					...
					getEventRelation(events, o, startEvent, relation,succeedingCon);
				}
			}
			return relation;
		}
		...
		private Integer getEventRelation(List<String> events,boolean succeedingCon) {
			...
			while (it.hasNext()) {
				...
				if (e!=null) {
					...
					relation= new Integer(getEventRelation(events, e, e, relation,succeedingCon));
				}
			}
			return relation;
		}
		...
		private List<String> getAlternativeEvents(List<String> events, EPCObject startElement, EPCEvent startEvent, List<String> listOfEvents, boolean succeedingCon) {
			Iterator<EPCObject> oIt=(succeedingCon ? epc.getSucceedingElements(startElement).iterator() : epc.getPreceedingElements(startElement).iterator());
			while (oIt.hasNext()) {
				EPCObject o=oIt.next();
				if (EPCConnector.class.isAssignableFrom(o.getClass())) {
					Iterator<EPCEvent> e= (succeedingCon ? epc.getPreceedingEvents((EPCConnector) o).iterator() : epc.getSucceedingEvents((EPCConnector) o).iterator());
					while (e.hasNext()) {
						listOfEvents.add(e.next().getIdentifier());
					}
					getAlternativeEvents(events, o, startEvent, listOfEvents, succeedingCon);
				}
			}
			return listOfEvents;
		}
		...
		private List<String> getAlternativeEvents(List<String> events, boolean succeedingCon) {
			Iterator<String> it=events.iterator();
			...
			while (it.hasNext()) {
				...
				if (e!=null) {
					...
					getAlternativeEvents(events, e, e, listOfEvents, succeedingCon);
				}
			}
			return listOfEvents;
		}
...
//					getNextObjects(NewAteList,newVisitedThisInstance,true,initialEvents.next());
		...
		protected Hashtable<String, Integer> addInitialFunctions() {
			...
			while (it.hasNext()) {
				...
				LogEvent logEvent = new LogEvent((EPCMERGEADD+"INITIAL"+id),"start");
				EPCFunction f = new EPCFunction(logEvent, epc);
				EPCEvent addedEvent = new EPCEvent((EPCMERGEADD+"INITIAL"+id), epc);
				epc.addFunction(f);
				addedObjects.add(f);
				epc.addEdge(f, event);
		...
		protected Hashtable<String, Integer> addFinalFunctions() {
			...
			while (it.hasNext()) {
				...
				LogEvent logEvent = new LogEvent((EPCMERGEADD+"FINAL"+id),"start");
				EPCFunction f = new EPCFunction(logEvent, epc);
				EPCEvent addedEvent = new EPCEvent((EPCMERGEADD+"FINAL"+id), epc);
				epc.addFunction(f);
				addedObjects.add(f);
				epc.addEdge(event,f);
				epc.addEvent(addedEvent);
		...
		private EPCObject getLeastVisitedTarget(EPCObject source) {
			...
			int novisits=-1;
			if (visitedEdges.containsKey(source)) {
				Iterator<EPCObject> itTargets=visitedEdges.get(source).keySet().iterator();
				while (itTargets.hasNext()) {
					EPCObject target= (EPCObject) itTargets.next();
					if (visitedEdges.get(source).get(target)<novisits || novisits<0) {
		...
		private AuditTrailEntryList getNextObjects(AuditTrailEntryList ateList,  Boolean foundNew, EPCObject o) {
			if (EPCFunction.class.isAssignableFrom(o.getClass())) {
				...
				setConnectorValues(o, o, true, true, (Integer) NONE, (Integer) NONE); 
			}

			if (!hasNextElement(o)) {
				try {
					outputLog.startProcessInstance(ProcessInstanceType.CHANGE_LOG, Integer.toString(instanceID), "", null);
					Iterator itATE =  ateList.iterator();
					while (itATE.hasNext()) {
						org.processmining.lib.mxml.AuditTrailEntry ate=promAte2mxmlibAte((AuditTrailEntry) itATE.next());
				...
				while (no!=null) {
					...
					try {
						...
						if (visitedEdges.containsKey(o) && visitedEdges.get(o).containsKey(no)) {
							if (visitedEdges.get(o).get(no)==0) {
								foundNew=true;
							}
						...
						AuditTrailEntryList NewAteList = ateList.cloneInstance();
		...
		private AuditTrailEntryList getNextObjects(AuditTrailEntryList ateList, List<String> visitedThisInstance, Boolean foundNew, EPCObject o) {
			...
			if (EPCFunction.class.isAssignableFrom(o.getClass())) {
				...
				setConnectorValues(o, o, true, true, (Integer) NONE, (Integer) NONE); 
				sucAlternativeEvents.addAll(epc.getSucceedingEvents((EPCFunction) o));
			}

			//saes = succeeding alternative events
//			List<EPCObject> saes = new ArrayList<EPCObject>();
			if (EPCEvent.class.isAssignableFrom(o.getClass())) {
				sucAlternativeEvents.addAll(epc.getSucceedingFunctions((EPCEvent) o));
			}
				...
				while (nextObject.hasNext()) {
					...
					if (!visited.contains(edge)) {
						try {
							...
							System.out.println("\n Not visited yet: "+no.getIdentifier());
							System.out.println(edge);
							visited.add(edge);
							if (visitedEdges.containsKey(o) && visitedEdges.get(o).containsKey(no)) {
								visitedEdges.get(o).put(no,(
...
//							System.out.println(visited.toString());
							...
							getNextObjects(NewAteList,newVisitedThisInstance,true,no);
//							second=true;
						} catch (Exception e) {
							e.printStackTrace();
						}
					} else if (visited.contains(edge) && !visitedThisInstance.contains(edge)) {
//						System.out.println("EDGE PASSED, BUT NOT IN THIS INSTANCE!!!");
						...
						if (no==getLeastVisitedTarget(o)) {
							try {
								...
								List<String> newVisitedThisInstance= new ArrayList<String>();
								newVisitedThisInstance.addAll(visitedThisInstance);
								getNextObjects(NewAteList,newVisitedThisInstance,foundNew,no);
								foundNew=false;
							} catch (Exception e) {
								e.printStackTrace();
							}
						}
					} else if (visited.contains(edge) && visitedThisInstance.contains(edge) && foundNew) {
...
//				ProcessInstanceImpl pi =new ProcessInstanceImpl("Test", ateList, new ArrayList<String>());
//				pi.setName(Integer.toString(instanceID));
		...
		protected org.processmining.lib.mxml.AuditTrailEntry promAte2mxmlibAte(AuditTrailEntry promAte) {
		...
		private  void setConnectorValues(EPCObject oo,EPCObject o, boolean init, boolean forward, Integer relationJoin, Integer relationSplit) {
...
//				System.out.println("Not Event/Function");
				if (EPCConnector.class.isAssignableFrom(o.getClass())) {
					...
					if (o.getInEdges().size()>1) {
						relationJoinNew= combineConnectorValues(connectortype, relationJoinNew);
					}
					if (o.getOutEdges().size()>1) {
						relationSplitNew= combineConnectorValues(connectortype, relationSplitNew);
					}
				}

				Iterator it = (forward ? o.getOutEdgesIterator() : o.getInEdgesIterator());
				while (it.hasNext()) {
					EPCEdge e = ((EPCEdge) it.next());
					EPCObject node = (EPCObject) (forward ? e.getDest() : e.getSource());
					setConnectorValues(oo,node, false, forward, (Integer) relationJoinNew, (Integer) relationSplitNew);

				}
				// if we found a Event or Function, a new list is started that can be used when going "up" again
			} else  {
...
//				System.out.println("Join: "+key+" "+relationJoin);
		...
		protected Hashtable<String,ArrayList<String>> getEventNames() {
			...
			while (it.hasNext()) {
				...
				Iterator<EPCEvent> itEvents= ((List<EPCEvent>) epc.getPreceedingEvents(f)).iterator();
				while (itEvents.hasNext()) {
					EPCEvent event=itEvents.next();
					Iterator<EPCFunction> targetFunctions = epc.getPreceedingFunctions(event).iterator();
					if (targetFunctions.hasNext()) {
						while (targetFunctions.hasNext()) {
							String key = targetFunctions.next().getIdentifier()+";"+f.getIdentifier();
							if (eventNames.containsKey(key)) {
								ArrayList<String> values = eventNames.get(key);
								Iterator<String> newEventNames=Arrays.asList(event.getIdentifier().split(" / ")).iterator();
				...
				itEvents= ((List<EPCEvent>) epc.getSucceedingEvents(f)).iterator();
				while (itEvents.hasNext()) {
					EPCEvent event=itEvents.next();
					Iterator<EPCFunction> targetFunctions = epc.getSucceedingFunctions(event).iterator();
					if (targetFunctions.hasNext()) {
						while (targetFunctions.hasNext()) {
							String key = f.getIdentifier()+";"+targetFunctions.next().getIdentifier();
							if (eventNames.containsKey(key)) {
								ArrayList<String> values = eventNames.get(key);
								Iterator<String> newEventNames=Arrays.asList(event.getIdentifier().split(" / ")).iterator();
								while (newEventNames.hasNext()) {
	...
	private void combineConnectorValues(EPCStructure net1, EPCStructure net2) {
		...
		while (connectors.hasMoreElements()) {
			...
			if (combinedJoinValues.containsKey(key)) {
				...
				if (connectorType==XOR && !(relationNew== AND || relationNew== OR)){
					combinedJoinValues.put(key, XOR);
				} else if (connectorType==AND && !(relationNew==XOR || relationNew==OR)){
					String name = key.substring(key.indexOf(";")+1);
					ArrayList<String> preceedingFunctions1= new ArrayList<String>();
					Iterator<EPCFunction> fIt=net1.getEPC().getAllFunctions(name).iterator();
					while (fIt.hasNext()) {
						Iterator<EPCFunction> preFunIt=net1.getEPC().getPreceedingFunctions(fIt.next()).iterator();
						while (preFunIt.hasNext()) {
							preceedingFunctions1.add(preFunIt.next().getIdentifier());
						}
					}
					ArrayList<String> preceedingFunctions2= new ArrayList<String>();
					fIt=net2.getEPC().getAllFunctions(name).iterator();
					while (fIt.hasNext()) {
						Iterator<EPCFunction> preFunIt=net2.getEPC().getPreceedingFunctions(fIt.next()).iterator();
						while (preFunIt.hasNext()) {
							preceedingFunctions2.add(preFunIt.next().getIdentifier());
						}
					}
					if (preceedingFunctions1.containsAll(preceedingFunctions2) && preceedingFunctions2.containsAll(preceedingFunctions1)) {
				...
				combinedJoinValues.put(key, net2.getJoinConnectorValues().get(key));
			}
		}
		...
		while (connectors.hasMoreElements()) {
			String key=(String) connectors.nextElement();
			combinedSplitValues.put(key, net1.getSplitConnectorValues().get(key));
		}
		...
		while (connectors.hasMoreElements()) {
			...
			if (combinedSplitValues.containsKey(key)) {
				...
				if (connectorType==XOR && !(relationNew== AND || relationNew== OR)){
					...
					Iterator<EPCFunction> itF= net1.getEPC().getAllFunctions(name).iterator();
					while (itF.hasNext()) {
						Iterator<EPCFunction> preFunIt=net1.getEPC().getSucceedingFunctions(itF.next()).iterator();
						while (preFunIt.hasNext()) {
							succeedingFunctions1.add(preFunIt.next().getIdentifier());
						}
					}
					ArrayList<String> succeedingFunctions2= new ArrayList<String>();
					itF= net2.getEPC().getAllFunctions(name).iterator();
					while (itF.hasNext()) {
						Iterator<EPCFunction> preFunIt=net2.getEPC().getSucceedingFunctions(itF.next()).iterator();
						while (preFunIt.hasNext()) {
							succeedingFunctions2.add(preFunIt.next().getIdentifier());
						}
					}
					if (succeedingFunctions1.containsAll(succeedingFunctions2) && succeedingFunctions2.containsAll(succeedingFunctions1)) {
				...
				combinedSplitValues.put(key, net2.getSplitConnectorValues().get(key));
			}
		}
	...
	public ConfigurableEPC analyse() {
		...
		Iterator<EPCFunction> functions= epc1.getEPC().getFunctions().iterator();
		...
		try {
			...
			outputLog = new LogSetSequentialImpl(outputFilter, "EPC Merge", null);
...
//		p = new ProcessImpl("Test", "Test", new DataSection(), new Vector<String>() );
...
//		Multi phase Miner:
		...
		progress.setMaximum(progress.getMaximum()+ log.numberOfInstances()*2 +1);
		PartialOrderGeneratorPlugin poGen = new PartialOrderGeneratorPlugin();
		PartialOrderMiningResult poGenResult = (PartialOrderMiningResult) poGen.mineWithProgressSet(log,logRelations,progress);
		...
		EPCResult result=(EPCResult) ag2epc.convert(aggResult.getProvidedObjects()[0]);
...
//		Replace connector values:
		...
		while (connectorsIt.hasNext()) {
			...
			List<EPCFunction> sfs = resultingEPC.getSucceedingFunctions(connector);
			List<EPCFunction> pfs = resultingEPC.getPreceedingFunctions(connector);
			...
			while (sfIt.hasNext()) {
				...
				while (pfIt.hasNext()) {
					...
					if (pf.getIdentifier().length()>6 && sf.getIdentifier().length()>6) {
						String key=pf.getIdentifier().substring(0, pf.getIdentifier().length()-7)+";"+sf.getIdentifier().substring(0, sf.getIdentifier().length()-7);
						if (combinedSplitValues.containsKey(key) && connector.getOutEdges().size()>1) {
							if(splitConnectorType==NONE && combinedSplitValues.get(key)==AND) {
								splitConnectorType=AND;
							} else if(splitConnectorType==NONE && combinedSplitValues.get(key)==XOR) {
								splitConnectorType=XOR;
							} else if(splitConnectorType==XOR && combinedSplitValues.get(key)==AND) {
								splitConnectorType=OR;
							} else if(splitConnectorType==AND && combinedSplitValues.get(key)==XOR) {
								splitConnectorType=OR;
							} else if(combinedSplitValues.get(key)==OR) {
								splitConnectorType=OR;
							}
						}
						if (connector.getInEdges().size()>1 && combinedJoinValues.containsKey(key)) {
							if(joinConnectorType==NONE && combinedJoinValues.get(key)==AND) {
								joinConnectorType=AND;
							} else if(joinConnectorType==NONE && combinedJoinValues.get(key)==XOR) {
								joinConnectorType=XOR;
							} else if(joinConnectorType==XOR && combinedJoinValues.get(key)==AND) {
								joinConnectorType=OR;
							} else if(joinConnectorType==AND && combinedJoinValues.get(key)==XOR) {
								joinConnectorType=OR;
			...
			if (connector.getOutEdges().size()>1 && splitConnectorType!=NONE) {
...
//		MOVE Events before connector
		...
		while (conIt.hasNext()) {
			...
			if (con.getInEdges().size()>1) {
				Iterator<EPCEdge> it = (Iterator<EPCEdge>) con.getOutEdgesIterator();
				while (it.hasNext()) {
					EPCEdge edge = it.next();
					EPCEvent event=(EPCEvent) edge.getDest();
					Iterator<EPCEdge> itOutEdges = (Iterator<EPCEdge>) event.getOutEdgesIterator();
					while (itOutEdges.hasNext()) {
						EPCEdge outEdge = itOutEdges.next();
						EPCObject object=(EPCObject) outEdge.getDest();
						event.setIdentifier(event.getIdentifier()+object.getIdentifier());
						resultingEPC.addEdge(con, object);
						//EPCEdge edgeAdded = resultingEPC.addEdge(con, object);

//						resultingEPC.delEdge(event, object);
					}
					Iterator<EPCEdge> itInEdges = (Iterator<EPCEdge>) con.getInEdgesIterator();
					while (itInEdges.hasNext()) {
						EPCEdge inEdge = itInEdges.next();
						EPCObject object=(EPCObject) inEdge.getSource();
						EPCEvent event2 = new EPCEvent(event.getIdentifier(), resultingEPC);
...
//		Remove beginning:
		...
		Iterator<EPCFunction> fIt = resultingEPC.getAllFunctions("f0").iterator();
		while (fIt.hasNext()) {
			...
			if (startEvent!=null && startFunction!=null){
				...
				List<EPCObject> startObjects=resultingEPC.getSucceedingElements(startFunction);
				...
				while (it.hasNext()){
					...
					if (EPCConnector.class.isAssignableFrom(nextObject.getClass())) {
						if (nextObject.getInEdges().size()>1) {
							resultingEPC.delEdge(startFunction, nextObject);
						} else {
							Iterator itOutEdges = nextObject.getOutEdgesIterator();
							while (itOutEdges.hasNext()) {
								EPCEdge e = ((EPCEdge) itOutEdges.next());
								EPCObject node = (EPCObject) e.getDest();
								resultingEPC.delEdge(nextObject, node);
							}
							resultingEPC.delConnector((EPCConnector) nextObject); 
						}
					}
...
//		Remove end:
		...
		while (fIt.hasNext()) {
			...
			if (endEvent!=null && endFunction!=null){
				...
				List<EPCObject> endObjects=resultingEPC.getPreceedingElements(endFunction);
				...
				while (it.hasNext()){
					...
					if (EPCConnector.class.isAssignableFrom(nextObject.getClass())) {
						if (nextObject.getOutEdges().size()>1) {
							...
							Iterator<EPCEdge> itInEdges = nextObject.getInEdgesIterator();
							while (itInEdges.hasNext()) {
...
//		Remove "start" from function names
		...
		while (it.hasNext()) {
			...
			if (f.getIdentifier().length()>7) {
				String identifier=f.getIdentifier().substring(0, f.getIdentifier().length()-7);
...
//		Change event names to original values:
		...
		List<EPCEvent> eventList = new ArrayList<EPCEvent>(resultingEPC.getEvents());
		Iterator<EPCEvent> itEvents=eventList.iterator();
		while (itEvents.hasNext()) {
			List<String> values= new ArrayList<String>();
			EPCEvent event=(EPCEvent) itEvents.next();
			Iterator<EPCFunction> preFunctions=resultingEPC.getPreceedingFunctions(event).iterator();
			if (preFunctions.hasNext()) {
				while (preFunctions.hasNext()) {
					EPCFunction preFunction=preFunctions.next();
					Iterator<EPCFunction> postFunctions=resultingEPC.getSucceedingFunctions(event).iterator();
					if (postFunctions.hasNext()) {
						while (postFunctions.hasNext()) {
							EPCFunction postFunction=postFunctions.next();
							String key = preFunction.getIdentifier()+";"+postFunction.getIdentifier();
							if (combinedEventNames.containsKey(key)) {
								Iterator<String> itEventNames = combinedEventNames.get(key).iterator();
								while (itEventNames.hasNext()) {
									String eventName=itEventNames.next();
									if (!values.contains(eventName)) {
										values.add(eventName);
			...
			if (values.size()>0) {
				...
				while (itValues.hasNext()) {
					if (add) {
						if (first) {
//							EPCConnector sucConnector = new EPCConnector(NONE, resultingEPC);
							Iterator<EPCEdge> inEdges=event.getInEdgesIterator();
							while (inEdges.hasNext()) {
								EPCEdge inEdge=inEdges.next();
								preConnector = new EPCConnector(NONE, resultingEPC);
								preConnectors.add(preConnector);
								resultingEPC.addConnector(preConnector);
								EPCObject preObject=(EPCObject) inEdge.getSource();
								resultingEPC.addEdge(preObject, preConnector);
								resultingEPC.addEdge(preConnector, event);
								resultingEPC.delEdge(preObject, event);
							}
							Iterator<EPCEdge> outEdges=event.getOutEdgesIterator();
							while (outEdges.hasNext()) {
								EPCEdge outEdge=outEdges.next();
								postConnector = new EPCConnector(NONE, resultingEPC);
								sucConnectors.add(postConnector);
								resultingEPC.addConnector(postConnector);
								EPCObject postObject=(EPCObject) outEdge.getDest();
						...
						EPCEvent newEvent=new EPCEvent(itValues.next(), resultingEPC);
						resultingEPC.addEvent(newEvent);
						events.add(newEvent.getIdentifier());
						Iterator<EPCConnector> itInConnector=preConnectors.iterator();
						while (itInConnector.hasNext()) {
							resultingEPC.addEdge(itInConnector.next(), newEvent);
						}
						Iterator<EPCConnector> itOutConnector=sucConnectors.iterator();
						while (itOutConnector.hasNext()) {
							resultingEPC.addEdge(newEvent,itOutConnector.next());
						}
					} else {
				...
				if (values.size()>1) {
					...
					if (preConnector!=null) {
						preConnector.setType(getConnectorValuesForEvents(events, PRECEDING));
					}

					// Succeeding connector value
					if (postConnector!=null) {
						postConnector.setType(getConnectorValuesForEvents(events, SUCCEEDING));
					}
				}
...
//Move events after connector if all preceeding events are the same.
		Iterator<EPCConnector> itConnectors=resultingEPC.getConnectors().iterator();
		while (itConnectors.hasNext()) {
			EPCConnector c= itConnectors.next();
			if (c.getInEdges().size()>1 && c.getOutEdges().size()==1) {
				boolean identical=true;
				Iterator<EPCObject> itObj =resultingEPC.getPreceedingElements(c).iterator();
				if (itObj.hasNext()) {
					...
					if (EPCEvent.class.isAssignableFrom(obj.getClass())) {
						...
						while (itObj.hasNext()) {
							...
							if ((!obj2.getIdentifier().equals(id)) || (!EPCEvent.class.isAssignableFrom(obj2.getClass()))) {
								identical=false;
								break;
							}
						}
				...
				if (identical) {
					Iterator<EPCEdge> inEdgesCon=(Iterator<EPCEdge>) c.getInEdgesIterator();
					boolean moved=false;
					while (inEdgesCon.hasNext()) {
						EPCEdge e=inEdgesCon.next();
						EPCObject preObject= (EPCObject) e.getSource();
						Iterator<EPCEdge> inEdgesEvent = (Iterator<EPCEdge>) preObject.getInEdgesIterator();
						while (inEdgesEvent.hasNext()) {
							EPCEdge inEdgeEvent=inEdgesEvent.next();
							resultingEPC.addEdge((EPCObject) inEdgeEvent.getSource(),c);
							resultingEPC.delEdge((EPCObject) inEdgeEvent.getSource(), preObject);
							resultingEPC.delEdge(preObject, c);
						}
						if (!moved) {
							Iterator<EPCEdge> outEdgesCon = (Iterator<EPCEdge>) c.getOutEdgesIterator();
							while (outEdgesCon.hasNext()) {
								EPCEdge outEdgeCon=outEdgesCon.next();
								resultingEPC.addEdge(preObject, (EPCObject)outEdgeCon.getDest());
								resultingEPC.addEdge(c, preObject);
								resultingEPC.delEdge(c, (EPCObject) outEdgeCon.getDest());
							}
							moved=true;
						} else {
							if (EPCEvent.class.isAssignableFrom(preObject.getClass())) {
								resultingEPC.delEvent((EPCEvent) preObject);
							} else if (EPCConnector.class.isAssignableFrom(preObject.getClass())) {
								resultingEPC.delConnector((EPCConnector) preObject);
							}
				...
				boolean identical=true;
				Iterator<EPCObject> itObj =resultingEPC.getSucceedingElements(c).iterator();
				if (itObj.hasNext()) {
					EPCObject obj=itObj.next();
					if (EPCEvent.class.isAssignableFrom(obj.getClass())) {
						String id=obj.getIdentifier();
				...
				if (identical) {
					...
					while (outEdgesCon.hasNext()) {
						...
						Iterator<EPCEdge> outEdgesEvent = (Iterator<EPCEdge>) postObject.getOutEdgesIterator();
						while (outEdgesEvent.hasNext()) {
							EPCEdge outEdgeEvent=outEdgesEvent.next();
							resultingEPC.addEdge(c, (EPCObject) outEdgeEvent.getDest());
							resultingEPC.delEdge(postObject, (EPCObject) outEdgeEvent.getDest());
						...
						if (!moved) {
							...
							while (inEdgesCon.hasNext()) {
								...
								resultingEPC.addEdge((EPCObject) inEdgeCon.getSource(),postObject);
								resultingEPC.addEdge(postObject,c);
								resultingEPC.delEdge((EPCObject) inEdgeCon.getSource(),c);
							}
							moved=true;
						} else {
							if (EPCEvent.class.isAssignableFrom(postObject.getClass())) {
								resultingEPC.delEvent((EPCEvent) postObject);
							} else if (EPCConnector.class.isAssignableFrom(postObject.getClass())) {
								resultingEPC.delConnector((EPCConnector) postObject);
							}
	...
	private LogRelations getLogRelations(LogReader log, Progress progress) {
		...
		// First layer: abstract from log, make direct succession and model elements
//		message("Starting log abstraction: building >                             ", 1, progress);
		...
		// Second layer: Abstract from succession and build causal and parallel relations
//		message("Starting log abstraction: building -> and ||", 2, progress);
		relations = (new MinValueLogRelationBuilder(logAbstraction, 0,
		...
		if (useFsm) {
//			message("Starting log abstraction: building -> based on FSM", 3, progress);
...
//		message("Starting log abstraction: building || based on overlap", 4, progress);
	...
	private int getConnectorValuesForEvents(List<String> events, boolean succeedingCon) {
		int relation=NONE;
		...
		} else if ((relation1.intValue()==XOR || relation1.intValue()==NONE) && !(relation2.intValue()== AND || relation2.intValue()== OR)){
			relation= XOR;
		} else if (relation1.intValue()==AND && !(relation2.intValue()==XOR || relation2.intValue()==OR) &&
				epc1.getAlternativeEvents(events, succeedingCon).containsAll(epc2.getAlternativeEvents(events, succeedingCon)) && epc2.getAlternativeEvents(events, succeedingCon).containsAll(epc1.getAlternativeEvents(events, succeedingCon))){
			relation= AND;
		} else {
			relation= OR;
		}
	...
	private void removeAddedFunctions(ConfigurableEPC epc){
		for (int i=0;i<addedInitialFunctions.size();i++ ) {
			Iterator<EPCFunction> fIt = epc.getAllFunctions(EPCMERGEADD+"INITIAL"+i+"\\nstart").iterator();
			while (fIt.hasNext()) {
				EPCFunction f=fIt.next();
				Iterator<EPCObject> connectors = (Iterator<EPCObject>) f.getPredecessors().iterator();
				while (connectors.hasNext()) {
					EPCObject connector = connectors.next();
					if (EPCConnector.class.isAssignableFrom(connector.getClass())) {
						Iterator<EPCEvent> initialEvent = (Iterator<EPCEvent>) connector.getPredecessors().iterator();
						while (initialEvent.hasNext()) {
							epc.delEvent(initialEvent.next());
						}
						epc.delConnector((EPCConnector) connector);
					} else if (EPCEvent.class.isAssignableFrom(connector.getClass())) {
		...
		for (int i=0;i<addedFinalFunctions.size();i++ ) {
			Iterator<EPCFunction> fIt = epc.getAllFunctions(EPCMERGEADD+"FINAL"+i+"\\nstart").iterator();
			while (fIt.hasNext()) {
				EPCFunction f=fIt.next();
				Iterator<EPCObject> connectors = (Iterator<EPCObject>) f.getSuccessors().iterator();
				while (connectors.hasNext()) {
					EPCObject connector = connectors.next();
					if (EPCConnector.class.isAssignableFrom(connector.getClass())) {
						Iterator<EPCEvent> finalEvent = (Iterator<EPCEvent>) connector.getSuccessors().iterator();
