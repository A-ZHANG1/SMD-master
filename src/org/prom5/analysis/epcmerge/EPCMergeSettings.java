...
import org.prom5.analysis.conformance.ConformanceAnalysisResults;
import org.prom5.analysis.conformance.ConformanceAnalysisSettings;
import org.prom5.framework.log.LogEvent;
import org.prom5.framework.log.LogReaderException;
...
import org.prom5.framework.util.BrowserLauncher;
...
public class EPCMergeSettings extends JPanel implements GuiNotificationTarget, Provider {
	/**
	 * 
	 */
	...
	private GuiPropertyStringTextarea description = new GuiPropertyStringTextarea("The EPC Merge plugin merges two EPCs into one EPC by preserving all the behavior that was possible in the two models. In addition the resulting EPC allows combining the behavior from both the two models. ");
	private GuiPropertyStringTextarea mapDescription = new GuiPropertyStringTextarea("For a successful merge it is required to specify which functions are identical and which events are identical.");
...
//		"Settings for importing " + log.getFile().getShortName() + " using " + pluginLabel, true);
		...
		try {
			eventsUI = new ImportEventsUI(eventsModel1, eventsModel2, fuzzyMatch, ImportEventsUI.EVENTS);
			functionsUI = new ImportEventsUI(functionsModel1, functionsModel2, fuzzyMatch, ImportEventsUI.FUNCTIONS);
			jbInit();
	...
	private void registerGuiActionListener() {
		...
		docsButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				BrowserLauncher.openURL("http://www.floriangottschalk.de/255.html");
			}
		});
	...
	public void execute(EPCMergeMethod method) {
		...
		// and be
		// able to abort it
		EPCMergeExecutionThread worker = new EPCMergeExecutionThread(method,this);
		try {
			worker.start();
	...
	public HashMap<EPCObject,Object[]> getMapping(boolean eventMapping) {
		...
		if (eventMapping) {
			...
			for (int i = 0; i < eventsModel2.size(); i++) {
				...
				ComboBoxEPCObject ce = (ComboBoxEPCObject) combo.getSelectedItem();
				EPCObject e = ce.getEPCObject();
				result.put(event, new Object[] {e, ((JTextField) labels.get(i)).getText()});
			}
		} else {
...
*/
	...
	private void jbInit() {
		if (step==0) {
			...
			functionsUI.setBorder(BorderFactory.createLineBorder(new Color(150, 150, 150), 1));
			...
			this.setLayout(new BorderLayout());
			eventsUI.setBorder(BorderFactory.createLineBorder(new Color(150, 150, 150), 1));
	...
	public ProvidedObject[] getProvidedObjects() {
		return new ProvidedObject[] { 
				new ProvidedObject("Merged EPC", new Object[] {resultingEPC}) 
		};
	}
...
class ImportEventsUI extends JPanel {
	...
	private EPCObject none=new EPCEvent(ComboBoxEPCObject.NONE,new ConfigurableEPC());
	private boolean fuzzyMatch;

	public ImportEventsUI(ArrayList<EPCObject> elementsModel1, ArrayList<EPCObject> elementsModel2, boolean fuzzyMatch, boolean type) {
	...
	// returned list of combo boxes is in the same order as the log events in importedEvents
	// as passed to the constructor.
	public ArrayList<ToolTipComboBox> getCombos() {
		return combos;
	}

	// returned list of label-JTextFields is in the same order as the log events in importedEvents
	// as passed to the constructor.
	...
	private void jbInit() throws Exception {
...

		{
			...
			evtInMod1.setPreferredSize(new Dimension(ml, (int) evtInMod1.getPreferredSize().getHeight()));
			if (type==EVENTS) {
			...
			evtInMod2.setPreferredSize(new Dimension((int) newCombo.getPreferredSize().getWidth(),
					(int) evtInMod2.getPreferredSize().getHeight()));
			if (type==EVENTS) {
			...
			newLabel.setPreferredSize(new Dimension(ml, (int) newLabel.getPreferredSize().getHeight()));
			...
			panel.add(p, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.WEST,
		...
		for (int i = 0; i < elementsModel2.size(); i++) {
			...
			// set the label in the textfield according the the object in the EPC
			final JTextField labelField = new JTextField(label.getText());
			labelField.setFont(label.getFont().deriveFont(Font.PLAIN));
			labelField.setPreferredSize(new Dimension(ml, (int) label.getPreferredSize().getHeight()));

			// make a new combo-list
			ToolTipComboBox newCombo = new ToolTipComboBox(epcObjects);
			newCombo.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if ( ((ToolTipComboBox) e.getSource()).getSelectedItem().equals(none)) {
						labelField.setText(label.getText());
					} else {
						labelField.setText(((ToolTipComboBox) e.getSource()).getSelectedItem().
								toString());
					}
				}
			});
			...
			for (int j = 0; !foundMatch && j < newCombo.getItemCount(); j++) {
				EPCObject comboEvent = ((ComboBoxEPCObject) newCombo.getItemAt(j)).getEPCObject();
				...
				if (elementModel2 != null) {
					if (!foundMatch && fuzzyMatch) {
						int match = StringSimilarity.similarity(toMatch, comboEventName);
...
//					logEvents[0].setLogEvent(wantedEvent);
					...
					// OK, so we could not find one yet. Let's look for the closest match
			...
			panel.add(p, new GridBagConstraints(0, i + 1, 1, 1, 0.0, 0.0, GridBagConstraints.WEST,
					GridBagConstraints.NONE, new Insets(1, 1, 1, 1), 0, 0));
