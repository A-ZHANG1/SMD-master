package org.prom5.analysis.eventdata;
...
import org.prom5.mining.prediction.SpringUtils;
/**
 * @author Ronald Crooy
 *
 */
public class EventDataAttributesAnalysis {
		...
		updatebutton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (xbox.getSelectedIndex() == 0) {
					...
					mychart = ChartFactory.createScatterPlot("Scatterplot of all values", xname, "attribute value",
					...
					mychart = ChartFactory.createXYLineChart("Average values", xname, "attribute value", data,
					...
					data = getHistrogrammedDataAttributes(els, ((Integer) BSpinner.getValue())* timesize, timesize);
					mychart = ChartFactory.createXYLineChart("Average values", xname, "attribute value", data,
							PlotOrientation.VERTICAL, true, true, false);
					mychart.setBackgroundPaint(Color.white); 
					XYPlot plot = mychart.getXYPlot();
	 ...
	 */
	private XYSeriesCollection getDataAttributes(String[] attributes, boolean byTime, double timesize) {
	...
	private IntervalXYDataset getHistrogrammedDataAttributes(String[] attributes, long barsize, long timesize) {
		...
		for (int index = 0; index < attributes.length; index++) {
			...
			if (no_intervals){
				...
				((YIntervalSeriesCollection)dataset).addSeries(histogram.getYIntervalSeries(attribute, timesize));
			}
		}
		return dataset;
	}
	...
	private class Histogram {
		...
		public YIntervalSeries getYIntervalSeries(String rowname, long factor){
			...
			double sdcount=0;double total=0; Vector<Double>totalvalues=new Vector<Double>();double avgtotal=0;
			double minus=0;double plus=0;double zero=0;
			for (Integer key : ysum.keySet()) {
				...
				for(Double onevalue:listofvalues){
					...
					sdcount++;total+=Math.pow(onevalue,2);avgtotal+=onevalue;
				...
				mydataset.add(point/factor, value,value+1.96*(sd/Math.sqrt(count.get(key))),value-1.96*(sd/Math.sqrt(count.get(key))));
			}
			double sdtotal=0;double avgsd=total/sdcount;double test=0;
			...
			//System.out.println("total -1:"+minus+" total +1:"+plus+" zero: "+zero +" total:"+sdcount);
			return mydataset;
		}
