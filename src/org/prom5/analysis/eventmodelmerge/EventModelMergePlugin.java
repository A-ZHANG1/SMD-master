...
/**
 * This analysis plugin merges a simulation model that is based on events into an
 * simulation model that is only based on activities. Therefore, the events that
 * refer to the same activity are merged into one activity. For the timing,
 * organizational and data perspective the user can decide how this merging needs to
 * take place. For the high level choices no user interaction is needed and therefore they
 * are automatically copied from the simulation model that is based on events to the
 * simulation model that is based on activities.
 *
 * @author rmans
 */
public class EventModelMergePlugin implements AnalysisPlugin {
	 ...
	 *
	 * @param analysisInputItemArray 	The list of input items necessary to
	 * 									carry out the analysis provided by the
	 * 									plugin.
	 * @return 							The JComponent to be displayed to the
	 * 									user within the plugin window.
	 */
	public JComponent analyse(AnalysisInputItem[] analysisInputItemArray) {
		...
		// since the plugin can only be invoked by the ProM tool, if the accepts method
		// of its AnalysisInputItem (deliverable by getInputItems) evaluates to true,
