...
/**
 * For the provided activityset which is based on events, a new empy activityset will be generated
 * which will be based on activities. That means that the events that refer to the same activity are
 * merged into one activity. Also a mapping will be generated for which highlevelactivities, that refer
 * to an event, are merged into which highlevelactivity that refers to an activity.
 *
 * @author rmans
 */
public class MergeActivitySetIntoActivityModel {
	...
	/**
	 * the mapping from activities in the real activity model to the activities in the activityset that refers to
	 * events and that are merged into that activity in the real activity model
	 */
	private HashMap<HLID, ArrayList<HLID>> mapping = new HashMap<HLID, ArrayList<HLID>>();

	/**
	 * Basic constructor
	 * @param activitySet HLActivitySet the activity set that needs to be merged into an activity set
	 * with only activities.
	 */
	 ...
	 * @return HLActivitySet the activity set that only refers to activities. The provided
	 * activityset is a newly created object which only contains default information. So no
	 * information is transferred from the activity set that refers to events to the activity set that
	...
	public HLActivitySet mergeActivitySetIntoActivityModel() {
		...
		while (hlActs.hasNext()) {
			...
			// check whether the name of the hlactivity ends with start, schedule or complete
			...
			for (int i=0; i<evtTypes.length; i++) {
				if (nameHLAct.endsWith(evtTypes[i].toString())) {
					...
					while (it.hasNext()) {
						...
						// TODO Anne: check whether should not be obtained from 'activitySetWithActivities'
						// --> don't understand why this is added here at all
						// Minseok changed the code. (activitySetWithEvents --> activitySetWithActivities
					...
					if (!activityExists) {
						// create a hlactivity with the activity name and add it to the model
			...
			if (!evtTypeExists) {
				...
				// create a new activity for this with the same name and add it to the model
	...
	/**
	 * Retrieves the mapping of which highlevelactivities that refer to events are merged into a
	 * highlevelactivity that refer to an activity.
	 * @return HashMap the mapping from the highlevelactivities of the activityset and that refer to activities, that is merged into
	 * an activitymodel, to the highlevel activities in the provided activityset, and that refer to an event,
	 * that needed to be merged into an activity model.
	 */
