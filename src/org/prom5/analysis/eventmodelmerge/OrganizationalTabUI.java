...
/**
 * The organizational tab of the EventModelMerge plugin presents possibilities to
 * the user for in which way he wants to merge organizational information when activities that refer
 * to events are merged into activities that only refer to activities.
 *
 * @author rmans
 */
public class OrganizationalTabUI extends JPanel implements GuiNotificationTarget {
	...
	/**
	 * At plug-in startup the information from the complete low-level activity will be used for
	 * the target model only (user may choose different strategy later on).
	 */
	private void initDefaults() {
		...
		while (it.hasNext()) {
			...
			if (values != null && values.size() > 0) {
				for(int k=0; k<values.size(); k++){
					...
					HLActivity act = myInputSimModel.getHLProcess().getActivity(actID);
					if (act.getName().endsWith("complete")) {
						HLID targetActID = entry.getKey();
						HLActivity targetAct = myFinalSimModel.getHLProcess().getActivity(targetActID);
						HLGroup group = act.getGroup();
						if(!group.getID().equals(myInputSimModel.getHLProcess().getNobodyGroupID())) {
							HLGroup clonedGroup = (HLGroup) group.clone();
							// remove "complete" from a group name
							if (clonedGroup.getName().endsWith("complete")) {
								int lastIndex = clonedGroup.getName().lastIndexOf("complete");
								clonedGroup.setName((clonedGroup.getName().substring(0,
										lastIndex - 1)).trim());
							}
		...
		Iterator<HLResource> resources = myInputSimModel.getHLProcess().getResources().iterator();
	...
	{
		// initialize 
		Iterator<HLGroup> groups = myFinalSimModel.getHLProcess().getGroups().iterator();
		...
		while (it.hasNext()) {
			...
			if (values != null && values.size() > 0) {
				...
				for (int k = 0; k < values.size(); k++) {
					...
					HLActivity act = myInputSimModel.getHLProcess().getActivity(actID);
					boolean flag = false;
					if (eventCheckBoxList.is_checkBox_schedule() && act.getName().endsWith("schedule")) {
						flag = true;
					}
					else if (eventCheckBoxList.is_checkBox_start() && act.getName().endsWith("start")) {
						flag = true;
					}
					else if (eventCheckBoxList.is_checkBox_complete() && act.getName().endsWith("complete")) {
						flag = true;
					}
		...
		while (it.hasNext()) {
			...
			if (values != null && values.size() > 0) {
				...
				for (int k = 0; k < values.size(); k++) {
					...
					//HLActivity act = values.get(k);
//					if(!groups.containsKey(act.getGroup().getName()))
//						groups.put(act.getGroup().getName(), act.getGroup());
				}
				if (groups.size() == 1) {
					HLID targetID = entry.getKey();
					HLActivity act = myFinalSimModel.getHLProcess().getActivity(targetID);
					act.setGroup(groups.keySet().iterator().next());
					//entry.getKey().setGroup(groups.get((String)groups.keySet().iterator().next()));
				}
				else {
					HLGroup newGroup = new HLGroup("", myFinalSimModel.getHLProcess());
					//HLGroup newGroup = new HLGroup("");
					Set<String> newNameSet = new HashSet<String>();
					// make a new name of group
					String[] key = (String[]) groups.keySet().toArray(new String[groups.keySet().size()]);
					for (int i = 0; i < key.length; i++) {
						HLGroup tempGroup = groups.get(key[i]);
						String[] tempString = tempGroup.getName().split(":");
						for (int j=0; j<tempString.length; j++) {
							if(!newNameSet.contains(tempString[j])) {
...
*/
	private void handleGroups(HashMap<HLID, HLGroup> groups, Entry<HLID, ArrayList<HLID>> entry) {
		// if only one group -> no merging required
		if (groups.size() == 1) {
			HLID targetID = entry.getKey();
			HLActivity targetAct = myFinalSimModel.getHLProcess().getActivity(targetID);
			HLGroup group = (HLGroup)groups.get(groups.keySet().iterator().next());
			if(group!=null) {
				   HLGroup clonedGroup = (HLGroup) group.clone();
				   targetAct.setGroup(clonedGroup.getID());
				   myFinalSimModel.getHLProcess().addOrReplace(clonedGroup);
		...
		// if more than one group -> merge resources from all groups into super group
		else {
			...
			while(itr.hasNext()) {
				HLID hlID = itr.next();
				HLGroup tempGroup = myInputSimModel.getHLProcess().getGroup(hlID);
				String[] tempString = tempGroup.getName().split(":");
				for(int j=0;j<tempString.length;j++)
					newNameSet.add(tempString[j]);
			}
			...
			Iterator<HLGroup> existingGroups = myInputSimModel.getHLProcess().getGroups().iterator();
			while (existingGroups.hasNext()) {
				HLGroup group = existingGroups.next();
				if (group.getName().equals(listNames)) {
					// exist in the inputSimModel
					HLGroup clonedGroup = (HLGroup) group.clone();
					HLActivity targetAct = myFinalSimModel.getHLProcess().getActivity(entry.getKey());
					targetAct.setGroup(clonedGroup.getID());
					myFinalSimModel.getHLProcess().addOrReplace(clonedGroup);
					flag = true;
					break;
			...
			if (!flag) {
				existingGroups = myFinalSimModel.getHLProcess().getGroups().iterator();
				while (existingGroups.hasNext()) {
					HLGroup group = existingGroups.next();
					if (group.getName().equals(listNames)) {
						// exist in the inputSimModel
			...
			if (!flag&&!listNames.equals("")) {	
				HLGroup newGroup = new HLGroup(listNames, myFinalSimModel.getHLProcess());
				existingGroups = myInputSimModel.getHLProcess().getGroups().iterator();
				while (existingGroups.hasNext()) {
					HLGroup group = existingGroups.next();
					...
					if (!group.getName().contains(":")) {
						...
						if (listNames.indexOf(group.getName()) > 0) {
							...
							Iterator<HLResource> existingResources = group.getResources().iterator();
	...
	private void jbInit() {
		...
		outmostLayer.setLayout(new BoxLayout(outmostLayer, BoxLayout.PAGE_AXIS));
		...
		currentOptionEnumerationList = new GUIPropertyListEnumeration("Merging strategies: ", null, values, new StrategyListener(), 250);
	...
	/**
	 * Listener for which strategy is selected.
	 * (e.g. "default strategy ('complete' activities)", "union of originators (all events)" )
	 *
	 * @author msong
	 */
	class StrategyListener implements GuiNotificationTarget {
		...
		public void updateGUI() {
			if (currentOptionEnumerationList.getValue().equals(DEFAULT_STRATEGY)) {
				initDefaults();
			} else if (currentOptionEnumerationList.getValue().equals(UNION_ALL_STRATEGY)) {
				unionAll();
			} else if (currentOptionEnumerationList.getValue().equals(UNION_SELECTED_STRATEGY)) {
				unionSelected();
			}
	 ...
	 * @version 1.0
	 */
	public class EventCheckBoxList extends JPanel implements GuiNotificationTarget {

		private GUIPropertyBoolean checkBox_schedule = new GUIPropertyBoolean("schedule", true, this);
		private GUIPropertyBoolean checkBox_start = new GUIPropertyBoolean("start", true, this);
		private GUIPropertyBoolean checkBox_complete = new GUIPropertyBoolean("complete", true, this);
		...
		public void updateGUI() {
			if (currentOptionEnumerationList.getValue().equals(UNION_SELECTED_STRATEGY)) {
				unionSelected();
				myGraphPanel.removeAll();
				HashSet<HLTypes.Perspective> perspectives = new HashSet<HLTypes.Perspective>();
				perspectives.add(HLTypes.Perspective.ROLES_AT_TASKS);
