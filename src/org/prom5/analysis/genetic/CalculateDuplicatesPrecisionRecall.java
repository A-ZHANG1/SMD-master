...
import org.prom5.analysis.heuristics.HeuristicsNetInputItem;
import org.prom5.framework.models.heuristics.HeuristicsNet;
import org.prom5.framework.ui.Message;
import org.prom5.mining.geneticmining.analysis.duplicates.DuplicatesEquivalent;
import org.prom5.mining.geneticmining.util.MethodsOverIndividuals;

/**
 * <p>Calculates the duplicates precision and recall of two nets. The nets must
 * have the same task labels (not necessarily the same amount of tasks!).</p>
 *
	...
	public JComponent analyse(AnalysisInputItem[] inputs) {
		...
		for (int i = 0; i < o1.length; i++) {
			if (o1[i] instanceof HeuristicsNet) {
				base = MethodsOverIndividuals.removeDanglingElementReferences((HeuristicsNet) o1[i]);
			}
		}
		for (int i = 0; i < o2.length; i++) {
			if (o2[i] instanceof HeuristicsNet) {
				mined = MethodsOverIndividuals.removeDanglingElementReferences((HeuristicsNet) o2[i]);
			}
		}
		...
		try {
			...
			result = " The values for the duplicates precision (Dp) and recall (Dr) are \n\n" +
					 "Dp = " + metrics.getPrecision() + "\n\n" +
					 "Dr = " + metrics.getRecall();
			Message.add("<DuplicatesPrecisionRecall> duplicatesPrecision=\"" + metrics.getPrecision() +
					"\" duplicatesRecall=\"" + metrics.getRecall() + "\">", Message.TEST);

		} catch (Exception ex) {
			result = "The metrics could not be calculated. Please check if both " +
					 "nets contain the same " +
					 "amount of task labels";
		}
