...
import org.prom5.mining.geneticmining.analysis.duplicates.ConnectionEquivalent;
import org.prom5.mining.geneticmining.util.MethodsOverIndividuals;

/**
 * <p>Calculates the structural precision and recall of two nets. The nets must
	...
	public JComponent analyse(AnalysisInputItem[] inputs) {
		...
		try {
			...
			result = " The values for the structural precision (Sp) and recall (Sr) are \n\n" +
					 "Sp = " + metrics.getPrecision() + "\n\n" +
					 "Sr = " + metrics.getRecall();
			Message.add("<StructuralPrecisionRecall> structuralPrecision=\"" + metrics.getPrecision() +
					"\" structuralRecall=\"" + metrics.getRecall() + "\">", Message.TEST);
