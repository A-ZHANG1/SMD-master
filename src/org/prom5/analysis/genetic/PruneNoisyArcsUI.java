	...
	public void jbInit() throws Exception {
		...
		prune.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				currentNet = fitness.calculate(new
							 HeuristicsNet[] {originalNet.copyNet()})[0];

				currentNet.disconnectArcsUsedBelowThreshold(PruneNoisyArcs.
						calculatePruningThreshold(currentNet,
						((Double) model.getValue()).doubleValue()));

				panelObject.removeAll();
				netResult = new HeuristicsNetResult(currentNet, log,
							netResult.getShowSplitJoinSemantics());
				panelObject.add(netResult.getVisualization(), BorderLayout.CENTER);
				panelObject.add(fitnessAndNoisePercentageSpace, BorderLayout.SOUTH);
				panelObject.repaint();
				panelObject.validate();

				Message.add("<PruneArcs numberOfUsedArcs=\"" +
						currentNet.getArcUsage().cardinality() + "\">", Message.TEST);
