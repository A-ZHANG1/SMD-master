...
public class GraphMatchingPlugin implements AnalysisPlugin{
	...
	public AnalysisInputItem[] getInputItems() {
		...
		AnalysisInputItem comp = new AnalysisInputItem("Document Models", 1, 1000) {
			public boolean accepts(ProvidedObject object) {
				Object[] o = object.getObjects();
	...
	public JComponent analyse(AnalysisInputItem[] inputs) {
		...
		if (algo != null){
			...
			List<String> searchModelNames = new ArrayList<String>(searchModels.size());
			List<SimpleGraph> searchGraphs = new ArrayList<SimpleGraph>(searchModels.size());
			for (ConfigurableEPC searchModel: searchModels){
				SimpleGraph searchGraph = new SimpleGraph(searchModel);
				searchGraph = searchGraph.removeVertices(searchGraph.getConnectorVertices());
				if (!algo.useEvents()){searchGraph = searchGraph.removeVertices(searchGraph.getEventVertices());}
			...
			List<SimpleGraph> docGraphs = new ArrayList<SimpleGraph>(docModels.size());
			for (ConfigurableEPC docModel: docModels){
				SimpleGraph docGraph = new SimpleGraph(docModel);					
				docGraph = docGraph.removeVertices(docGraph.getConnectorVertices());
				if (!algo.useEvents()){docGraph = docGraph.removeVertices(docGraph.getEventVertices());}
				docGraphs.add(docGraph);
				docModelNames.add(docModel.getIdentifier());
			}
			
			double similarities[][] = new double[docModels.size()][searchModels.size()];
				
			Progress progress = new Progress("Computing similarity", 0, searchModels.size() * docModels.size());
			...
			for (SimpleGraph searchGraph: searchGraphs){
				...
				for (SimpleGraph docGraph: docGraphs){ 					
					if (algo.useEvents() && (algo instanceof GraphEditDistanceAStarSim)){
						((GraphEditDistanceAStarSim)algo).setPartitions(searchGraph.getFunctionVertices(), docGraph.getFunctionVertices());
					}

					similarities[rowIndex][colIndex] = 1.0 - algo.compute(searchGraph, docGraph);
					progress.inc();
			...
			if ((searchGraphs.size()>1) || (docGraphs.size()>1)){
				...
				for (TwoVertices e: algo.bestMapping()){
					matches += "(\"" + searchGraphs.get(0).getLabel(e.v1) + "\",\"" + docGraphs.get(0).getLabel(e.v2) + "\")\n";
 				}
			}
			ui = new GraphMatchingResults(searchModelNames,docModelNames,similarities,matches);
		}else{
	...
	private void buildList(AnalysisInputItem item, ArrayList<ConfigurableEPC> list) {
