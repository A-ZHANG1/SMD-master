...
public abstract class DistanceAlgoAbstr implements DistanceAlgo {
	...
	protected double editDistance(Mapping m){
		...
		if (useepsilon){
			...
			skippedEdges = (double) totalNrEdges - 2.0 * (double) m.matchedEdges.size();
			skippedVertices = (double) totalNrVertices - 2.0 * (double) m.vertexMappingCount;			
		}
		...
		return computeScore(skippedEdges, skippedVertices, substitutedVertices, nrSubstitutions, insertedVertices, deletedVertices);
	}
	...
	protected double editDistance(Set<TwoVertices> m){
		...
		Set<Integer> skippedVerticesIn1 = new HashSet<Integer>(sg1.getVertices());
		skippedVerticesIn1.removeAll(verticesFrom1Used);
		Set<Integer> skippedVerticesIn2 = new HashSet<Integer>(sg2.getVertices());
