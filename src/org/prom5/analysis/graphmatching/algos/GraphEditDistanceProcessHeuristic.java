package org.prom5.analysis.graphmatching.algos;
...
import org.prom5.analysis.graphmatching.graph.SimpleGraph;
import org.prom5.analysis.graphmatching.graph.TwoVertices;
import org.prom5.analysis.graphmatching.led.StringEditDistance;
 ...
 */
public class GraphEditDistanceProcessHeuristic extends DistanceAlgoAbstr implements DistanceAlgo{

	/**
	 * A tuple that contains a mapping that is being evaluated by the algorithm in
	 * computeAllMappings(). See cmoputeAllMappings for details.  
	 */
	class CurrentMapping{
		...
		public CurrentMapping(Set<Integer> currVerticesSG1, Set<Integer> currVerticesSG2) {
		...
		public CurrentMapping(Set<Integer> currVerticesSG1, Set<Integer> currVerticesSG2, Set<TwoVertices> mapping, Set<Integer> verticesSG1Used, Set<Integer> verticesSG2Used) {
			this.currVerticesSG1 = currVerticesSG1;
			this.currVerticesSG2 = currVerticesSG2;
		...
		public int hashCode(){
			return	currVerticesSG1.hashCode() +
					currVerticesSG2.hashCode() +
					mapping.hashCode() +
					verticesSG1Used.hashCode() +
	...
	/**
	 * Computes all possible mappings between the two graphs. 
	 * Stores them in 'finalMappings'.
	 * (Actually it does not compute >all< mappings; it only computes mappings 
	 *  that are expected to be >optimal<. So the naming of the method is not entirely 
	 *  accurate.)
	 *  
	 *  The algorithm works as follows:
	 *  - initial step: set <current vertices> to the source vertices of the two graphs
	 *  - Recursive step: 
	 *      1. prune the set of current mappings if necessary
	 *      2. take all current mappings up to <current vertices> and extend them
	 */
	private void computeAllMappings(){
		CurrentMapping currentMapping = new CurrentMapping(sg1.sourceVertices(),sg2.sourceVertices());
			...
			if (cm.currVerticesSG1.isEmpty() || cm.currVerticesSG2.isEmpty()){
				...
				for (Integer i: cm.currVerticesSG1){
					for (Integer j: cm.currVerticesSG2){
						...
						if (StringEditDistance.similarity(sg1.getLabel(i), sg2.getLabel(j)) >= this.ledcutoff){
							newVerticesSG1Used = new HashSet<Integer>(cm.verticesSG1Used); newVerticesSG1Used.add(i);
							newVerticesSG2Used = new HashSet<Integer>(cm.verticesSG2Used); newVerticesSG2Used.add(j);
							newCurrVerticesSG1 = new HashSet<Integer>(cm.currVerticesSG1); newCurrVerticesSG1.addAll(sg1.postSet(i)); newCurrVerticesSG1.removeAll(newVerticesSG1Used);
							newCurrVerticesSG2 = new HashSet<Integer>(cm.currVerticesSG2); newCurrVerticesSG2.addAll(sg2.postSet(j)); newCurrVerticesSG2.removeAll(newVerticesSG2Used); 
							newMapping = new HashSet<TwoVertices>(cm.mapping); newMapping.add(new TwoVertices(i,j));
							nm = new CurrentMapping(newCurrVerticesSG1,newCurrVerticesSG2,newMapping,newVerticesSG1Used,newVerticesSG2Used);
							newMappings.add(nm);
						}
						
						//2. Create a mapping in which i is skipped
						newVerticesSG1Used = new HashSet<Integer>(cm.verticesSG1Used); newVerticesSG1Used.add(i);
						newVerticesSG2Used = new HashSet<Integer>(cm.verticesSG2Used);
						newCurrVerticesSG1 = new HashSet<Integer>(cm.currVerticesSG1); newCurrVerticesSG1.addAll(sg1.postSet(i)); newCurrVerticesSG1.removeAll(newVerticesSG1Used);
						newCurrVerticesSG2 = new HashSet<Integer>(cm.currVerticesSG2); 
						newMapping = new HashSet<TwoVertices>(cm.mapping);
						nm = new CurrentMapping(newCurrVerticesSG1,newCurrVerticesSG2,newMapping,newVerticesSG1Used,newVerticesSG2Used);
						newMappings.add(nm);
						
						//2. Create a mapping in which j is skipped
						newVerticesSG1Used = new HashSet<Integer>(cm.verticesSG1Used);
						newVerticesSG2Used = new HashSet<Integer>(cm.verticesSG2Used); newVerticesSG2Used.add(j);
						newCurrVerticesSG1 = new HashSet<Integer>(cm.currVerticesSG1);
						newCurrVerticesSG2 = new HashSet<Integer>(cm.currVerticesSG2); newCurrVerticesSG2.addAll(sg2.postSet(j)); newCurrVerticesSG2.removeAll(newVerticesSG2Used); 
						newMapping = new HashSet<TwoVertices>(cm.mapping);
						nm = new CurrentMapping(newCurrVerticesSG1,newCurrVerticesSG2,newMapping,newVerticesSG1Used,newVerticesSG2Used);
						newMappings.add(nm);
	 ...
	 * 'pruneTo' most viable mappings. (I.e. the 'pruneTo' mappings with the smallest
	 * edit-distance. 
	 */
	...
	/**
	 * Computes the edit distance between the two SimpleGraph instances with which 
	 * the object was instantiated.
	 * 
	 * Precondition: the algorithm only returns a useful edit distance if the graphs both
	 * have at least one 'source node' (a node with no incoming arcs.) Otherwise, Double.MAX_VALUE
	 * is returned.   
	 * 
	...
	public double compute(SimpleGraph sg1, SimpleGraph sg2){
		...
		for (TwoVertices pair: mappingWithMinimalDistance){
			System.out.println("\"" + sg1.getLabel(pair.v1) + "\" - \"" + sg2.getLabel(pair.v2) + "\"");
		}
		*/
