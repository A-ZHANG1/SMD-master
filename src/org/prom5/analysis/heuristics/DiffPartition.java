	...
	public JComponent analyse(AnalysisInputItem[] inputs) {
		...
		for (int i = 0; i < o1.length; i++) {
			if (o1[i] instanceof HeuristicsNet) {
				one = MethodsOverIndividuals.removeDanglingElementReferences((HeuristicsNet) o1[i]);
			}
		}
		for (int i = 0; i < o2.length; i++) {
			if (o2[i] instanceof HeuristicsNet) {
				two = MethodsOverIndividuals.removeDanglingElementReferences((HeuristicsNet) o2[i]);
			}
		}
		result = one.diffForSets(two);
		StringTokenizer st = new StringTokenizer(result,"\n,[]=+><() ");
		Message.add("<HNDiffSets numbefOfTokensInDiffResult=\"" +st.countTokens() + "\">", Message.TEST);
