...
/**
 * <p>Title: Summary of Structural Properties of a Heuristic Net</p>
 *
 * <p>Description: This class summarizes basic information about the structure of a
 * Heuristic net. For instance, number of labels, activities, arcs, etc.</p>
...
public class HNStructuralPropertySummary implements AnalysisPlugin {
	...
	public String getHtmlDescription() {
		return
				"This plug-ins summarizes how many activities, labels and arcs a Heuristic Net contains.";
	}

	public AnalysisInputItem[] getInputItems() {
		AnalysisInputItem[] items = {
									new AnalysisInputItem("Individual") {
	...
	public JComponent analyse(AnalysisInputItem[] inputs) {
		...
		for (int i = 0; i < o1.length; i++) {
			if (o1[i] instanceof HeuristicsNet) {
				hn = MethodsOverIndividuals.removeDanglingElementReferences((HeuristicsNet) o1[i]);
			}
		}
	...
	private String calculateStructuralPropertySummary(HeuristicsNet hn) {
		...
		for (int from = 0; from < hn.getOutputSets().length; from++) {
			if (hn.getInputSet(from).size() > 0 || hn.getOutputSet(from).size() > 0) {
		...
		propertiesSummary.append("The selected Heuristic net has the following properties: \n\n");
		propertiesSummary.append("Amount of labels: " + hn.getReverseDuplicatesMapping().length +
				"\n\n");
		propertiesSummary.append("Amount of activities: " + numberConnectedActivities + "\n\n");
		propertiesSummary.append("Amount of arc: " + numberArcs + "\n\n");

		Message.add("<HNPropertySummary numberOfLabels=\"" +
				hn.getReverseDuplicatesMapping().length + "\" numberOfActivities=\"" +
				numberConnectedActivities + "\" numberOfArcs =\""+ numberArcs + "\">",
				Message.TEST);
