...
import org.prom5.analysis.logstatistics.LogStatisticsResult;
import org.prom5.framework.log.LogReader;
import org.prom5.framework.plugin.ProvidedObject;

public class PerformanceMetricsInOntologiesMacroPlugin implements AnalysisPlugin {
	 ...
	 * The Decision Point analysis plugin requires a Petri net to derive the
	...
	public AnalysisInputItem[] getInputItems() {
		AnalysisInputItem[] items = {
				new AnalysisInputItem("SAMXML Log") {
	...
	public JComponent analyse(AnalysisInputItem[] analysisInputItemArray) {
		...
		return new PerformanceMetricsInOntologiesResult(
				LogStatisticsResult.getProcessingTimes(log),
				LogStatisticsResult.getThroughputTimes(log)
				);
	}
}
