...
public class HmmExperimenterSettings extends JPanel implements GuiNotificationTarget {
	...
	protected GUIPropertyInteger noiseLevels = new GUIPropertyInteger("Number of Noise Levels: ", config.getNoiseLevels(), 0, 100);
	protected GUIPropertyInteger traces = new GUIPropertyInteger("Number of traces per Event Log: ", config.getTraces(), 1, Integer.MAX_VALUE);
	protected GUIPropertyInteger traceLength;
	protected GUIPropertyBoolean replicate = new GUIPropertyBoolean("Run replicated experiments", config.isReplicate(), this);
	protected GUIPropertyInteger replications = new GUIPropertyInteger("Number of replications per noise level: ", config.getReplications(), 2, Integer.MAX_VALUE);
	
	public HmmExperimenterSettings(PetriNet net, LogReader log, AnalysisPlugin plugin) {
		inputLog = log;
		inputModel = net;
		algorithm = plugin;
		createGui();
	...
	private void createGui() {
		...
		if (inputLog != null) {
			...
			for (int i=0; i<inputLog.numberOfProcesses(); i++) {
				...
				traceLength = new GUIPropertyInteger("Maximum number of events per trace: ", longestTraceLength, 1, 1000000);
			}
		} else {
			traceLength = new GUIPropertyInteger("Maximum number of events per trace: ", 100, 1, 1000000);
		}
		...
		confPanel.add(GuiUtilities.packMiddleVertically((GuiUtilities.packCenterHorizontally(parameterPanel))));
	...
	/**
	 * Method to be called when pressing the "Start Analysis" 
	 * button. 
	 */
	private void runAnalysis() {
		...
		ObservationNoiseGenerator obsNoise = new ObservationNoiseGenerator(models, config);
		TransitionNoiseGenerator transNoise = new TransitionNoiseGenerator(models, config);
		ObservationNoiseEvaluator obsNoiseEval = new ObservationNoiseEvaluator(models, config);
		TransitionNoiseEvaluator transNoiseEval = new TransitionNoiseEvaluator(models, config);
		AnalysisThread obsThread = new AnalysisThread(obsNoise, obsNoiseEval);
		obsThread.start();
		AnalysisThread transThread = new AnalysisThread(transNoise, transNoiseEval);
		transThread.start();
	}
