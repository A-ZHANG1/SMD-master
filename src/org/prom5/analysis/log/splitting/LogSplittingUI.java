...
public class LogSplittingUI extends JPanel implements Provider {
	...
	private void jbInit() {
		...
		considerTimeWhenFilteringCheckBox = new JCheckBox("Use Time Interval during Log Splitting");
		JLabel timeInterval = new JLabel(" Time Interval (in seconds) ");
		timeIntervalSpinnerNumberModel = new SpinnerNumberModel(100.0, 0.0, Integer.MAX_VALUE, 10.0);
		...
		considerTimeWhenFilteringCheckBox.addActionListener(
				new ActionListener(){
					public void actionPerformed(ActionEvent e) {
						timeIntervalSpinner.setEnabled(considerTimeWhenFilteringCheckBox.isSelected());
					}
				}
			);
		...
		eventSelectionPanel.setBorder(new TitledBorder("Event Selection Options"));
		...
		List <OntologyModel> ontologies = log.getLogSummary().getOntologies().getOntologies();
		...
		conceptsToolTipComboBox = createConceptsComboBox((OntologyInCombo) ontologiesToolTipComboBox.getSelectedItem());
		...
		ontologyButton.addActionListener(
				new ActionListener(){
					public void actionPerformed(ActionEvent e) {
						ontologiesToolTipComboBox.setEnabled(ontologyButton.isSelected());
		...
		ontologiesToolTipComboBox.addActionListener(
				new ActionListener(){
					public void actionPerformed(ActionEvent e) {
						conceptsToolTipComboBox = createConceptsComboBox((OntologyInCombo) ontologiesToolTipComboBox.getSelectedItem());
						addElementsToEventSelectionPanel();
					...
					public void actionPerformed (ActionEvent e){
						...
						try {
							...
							JOptionPane.showMessageDialog(MainUI.getInstance(),
								    "Log has been sucessfully split!", "Log Splitting Result", JOptionPane.INFORMATION_MESSAGE);
						} catch (IOException exc) {
							JOptionPane.showMessageDialog(MainUI.getInstance(),
								    "Log could not be split!", "Log Splitting Result", JOptionPane.ERROR_MESSAGE);
							exc.printStackTrace();
							providedLog = null;
						}
					}
	...
	private LogReader splitLog() throws IOException {
		...
		try {
			...
			for (int processIndex=0; processIndex<log.numberOfProcesses(); processIndex++) {
				...
				persistency.startProcess(process.getName(), process.getDescription(), toListURI(process.getModelReferences()), process.getAttributes());
				//writing the process instances for this process (start)
				for(int processInstanceIndex = 0; processInstanceIndex<process.size(); processInstanceIndex++){

					ProcessInstance originalPI = process.getInstance(processInstanceIndex);

					Message.add(this.getName() + ">>>   Splitting process instance number " + (processInstanceIndex+1) + "  out of " + process.size() + "...");
					
					//splitting the original process instances based on the input parameters (start)
					List<ProcessInstance> splittedPI = splitProcessInstance(originalPI);
					for(ProcessInstance newPI : splittedPI){
						persistency.startProcessInstance(newPI.getName(), (newPI.getDescription()==null?"": newPI.getDescription()), toListURI(newPI.getModelReferences()), newPI.getAttributes());
						for(int indexATE = 0; indexATE<newPI.getAuditTrailEntryList().size(); indexATE++){
							persistency.addAuditTrailEntry( promATE2mxmlibATE(newPI.getAuditTrailEntryList().get(indexATE)) );
						}
						persistency.endProcessInstance();
					}
					//splitting the original process instances based on the input parameters (end)
				}
			...
			splittedLog = BufferedLogReader.createInstance(new DefaultLogFilter(DefaultLogFilter.INCLUDE), logFile);
	...
	private org.processmining.lib.mxml.AuditTrailEntry promATE2mxmlibATE(AuditTrailEntry promATE) {
		...
		mxmlibATE.setWorkflowModelElementModelReferences(toListURI(promATE.getElementModelReferences()));
		
		//converting the event type element and its semantic references		
		mxmlibATE.setEventType(EventType.getType(promATE.getType()));
		mxmlibATE.setEventTypeModelReferences(toListURI(promATE.getTypeModelReferences()));
		
		//converting the originator element and its semantic references		
		mxmlibATE.setOriginator(promATE.getOriginator());
		mxmlibATE.setOriginatorModelReferences(toListURI(promATE.getOriginatorModelReferences()));
	...
	private AuditTrailEntryList reduceATEBasedOnTimeInterval(AuditTrailEntryList ateList){
		
		try{
			long timeInterval = timeIntervalSpinnerNumberModel.getNumber().longValue() * 1000; //because the time is in seconds
			long timeCurrentATE = ateList.get(ateList.size()-1).getTimestamp().getTime();
			long timeFirstATE = 0;
			try{
	...
	private List<ProcessInstance> splitProcessInstance(
		...
		if(ontologyButton.isSelected()){
			ConceptModel selectedConcept = ((ConceptInCombo) conceptsToolTipComboBox.getSelectedItem()).get();
			...
			if(subConcepts.isSelected()){
				...
				setOfSplitConcepts.addAll(selectedConcept.getOntology().getSubConcepts(selectedConcept));
			}
			if(superConcepts.isSelected()){
				//also include the superconcepts
				setOfSplitConcepts.addAll(selectedConcept.getOntology().getSuperConcepts(selectedConcept));
			}
		}
		...
		try {
			...
			for(AuditTrailEntry ate: originalPI.getListOfATEs()){
				if(!foundAteWithDesiredConcept){
					...
					if(ate.getTimestamp()!= null &&  ate.getTimestamp().getTime()==timeAteWithDesiredConcept){
						newATElist.append(ate);
					}else{
						//new ATE does not have the same time of current one
						//create process instance and restart the variable newATElist
						...
						ProcessInstance newProcessInstance = new ProcessInstanceImpl(originalPI.getProcess(), newATElist, originalPI.getModelReferences());
						newProcessInstance.setName(originalPI.getName().trim() + "_" + indexNewPI++);
				...
				if((labelButton.isSelected() && (((String)labelsToolTipComboBox.getSelectedItem()).compareTo(ate.getElement()) == 0))
					||(ontologyButton.isSelected() && containsConcept(setOfSplitConcepts, extractAllModelReferencesInsideATE(ate))) ){
	...
	private boolean containsConcept (Set<ConceptModel> listOfConcepts, List <String> ateModelReferences){
		
		for(String modelReference: ateModelReferences){
			for(ConceptModel concept: listOfConcepts){
				ConceptModel modelReferenceMappedToLoadedOntology = ontologyCollection.findConceptByUriInLog(modelReference);
				if( modelReferenceMappedToLoadedOntology !=null){
						modelReference = modelReferenceMappedToLoadedOntology.getName();
				}
	...
	private ToolTipComboBox createConceptsComboBox(OntologyInCombo selectedOntology){
		Collection<ConceptModel> conceptsForThisOntology = selectedOntology.get().getConcepts();
	...
	public ProvidedObject[] getProvidedObjects() {
		ProvidedObject[] objects = new ProvidedObject[] {new ProvidedObject("Splitted Log",
								   new Object[] {providedLog})};
		return objects;
	}
