...
import org.prom5.analysis.logclustering.model.AHCTreeNode;
...
///
//	protected JScrollPane graphScrollPane;
	protected GUIPropertyDoubleTextField clusterSizeBox = new GUIPropertyDoubleTextField("Min distance",0);
	...
	public void writeToDotEach(Writer bw, AHCTreeNode temp) throws IOException
	{
	    ...
	    if(dist<=upperBoundUI)
			bw.write("cluster" + temp.getName() +
					" [shape=\"octagon\",style=\"filled\",fillcolor=\""+color+"\",label=\"Cluster "
					+ temp.getName() + "\\n" + temp.size() + "\\n" + temp.getDistance() + "\"];\n");
	...
	public void writeToDotEachArc(Writer bw, AHCTreeNode temp) throws IOException
	{
			...
			if(((AHCTreeNode)temp.getLeft()).getDistance()>lowerBoundUI
				...
				if(temp.getDistance()<=upperBoundUI)
					bw.write("cluster" + temp.getName() + " -- cluster" +
						((AHCTreeNode) temp.getLeft()).getName() + " [label=\""
						+ temp.getDistance() + "\"];\n");
		...
		if(temp.getRight()!=null&&temp.getRight()instanceof AHCTreeNode){
			if(((AHCTreeNode)temp.getRight()).getDistance()>lowerBoundUI
				...
				if(temp.getDistance()<=upperBoundUI)
					bw.write("cluster" + temp.getName() + " -- cluster" +
						((AHCTreeNode) temp.getRight()).getName() + " [label=\""
						+ temp.getDistance() + "\"];\n");
	...
	public JPanel getPanel() {
		...
		sliderUpperUI.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				upperBoundUI = (double)sliderUpperUI.getValue() / 10000.0 * rootAHCTree.getDistance();
				diameterLabel.setText("  < " + format.format(upperBoundUI) + "  ");
				if(sliderUpperUI.getValueIsAdjusting() == false) {
					Message.add("Re-clustering with threshold " + upperBoundUI + "...", Message.NORMAL);
//					cluster(threshold);
					try {
		...
		sliderLowerUI = new JSlider(JSlider.VERTICAL, 0, 10000, (int)(lowerBoundUI/rootAHCTree.getDistance()*10000));
		sliderLowerUI.setBackground(bgColor);
		sliderLowerUI.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				lowerBoundUI = (double)sliderLowerUI.getValue() / 10000.0 * rootAHCTree.getDistance();
				diameterLabel2.setText("  < " + format.format(lowerBoundUI) + "  ");
				if(sliderLowerUI.getValueIsAdjusting() == false) {
					Message.add("Re-clustering with threshold " + lowerBoundUI + "...", Message.NORMAL);
//					cluster(threshold);
					try {
						graphScrollPane.getViewport().setView(getGraphPanel());
					} catch (Exception e) {
...
//added
		...
		sliderUpper.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				upperBound = (double)sliderUpper.getValue() / 10000.0 * rootAHCTree.getDistance();
				diameterLabel3.setText("  < " + format.format(upperBound) + "  ");
				if(sliderUpper.getValueIsAdjusting() == false) {
					Message.add("Re-clustering with threshold " + upperBound + "...", Message.NORMAL);
//					cluster(threshold);
					try {
						graphScrollPane.getViewport().setView(getGraphPanel());
					} catch (Exception e) {
		...
		sliderLower = new JSlider(JSlider.VERTICAL, 0, 10000, (int)(lowerBound/rootAHCTree.getDistance()*10000));
		sliderLower.setBackground(bgColor);
		sliderLower.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				lowerBound = (double)sliderLower.getValue() / 10000.0 * rootAHCTree.getDistance();
				diameterLabel4.setText("  < " + format.format(lowerBound) + "  ");
				if(sliderLower.getValueIsAdjusting() == false) {
					Message.add("Re-clustering with threshold " + lowerBound + "...", Message.NORMAL);
//					cluster(threshold);
					try {
						graphScrollPane.getViewport().setView(getGraphPanel());
					} catch (Exception e) {
