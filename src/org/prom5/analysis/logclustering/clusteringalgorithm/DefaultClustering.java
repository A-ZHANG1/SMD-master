...
public class DefaultClustering extends ClusteringAlgorithm {
	...
	public void cluster(double maxDiameter) {
		...
		while(traceList.size() > 0) {
			Message.add("Building next generation from " + traceList.size() + " traces...", Message.NORMAL);
			...
			Message.add("...found winner with size " + nextCluster.size() + ".", Message.NORMAL);
		}
	}

	public LogCluster deriveNextCluster(List<Integer> traces, double maxDiameter) {
		ArrayList<LogCluster> nextClusters = new ArrayList<LogCluster>(traces.size());
	...
	public LogCluster buildCluster(Integer initialTrace, List<Integer> traces, double maxDiameter) {
	...
	public void writeToDot(Writer bw) throws IOException {
		...
		for(int i=1; i<clusterSet.size(); i++) {
			...
			int relationIndex = 0;
			for(int k=0; k<i; k++) {
				double maxRelation = distances.get(i,k);//agProfiles.getDistance(i,k);//comparator.getRelation(i, k);
				if(maxRelation > maxClusterRelation) {
					maxClusterRelation = maxRelation;
					relationIndex = k;
				}
			}
			bw.write("cluster" + i + " -- cluster" + relationIndex + " [label=\""
					+ maxClusterRelation + "\"];\n");
		}
		// close graph
		bw.write("}\n");
	...
	public JPanel getPanel() {
		...
		slider.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				double threshold = (double)slider.getValue() / 10000.0 * agProfiles.getSumOfNormalizationMaximum();
				diameterLabel.setText("  < " + format.format(threshold) + "  ");
				if(slider.getValueIsAdjusting() == false) {
					Message.add("Re-clustering with threshold " + threshold + "...", Message.NORMAL);
					cluster(threshold);
					try {
		...
		diameterLabel = new JLabel(format.format(0.8 * agProfiles.getSumOfNormalizationMaximum()));
