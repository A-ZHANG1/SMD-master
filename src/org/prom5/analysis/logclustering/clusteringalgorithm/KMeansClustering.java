 ...
 *
 */
public class KMeansClustering  extends ClusteringAlgorithm implements DotFileWriter {
	...
	protected GUIPropertyInteger maxIterationBox = new GUIPropertyInteger("Number of Clusters = ", 50, 1, 100);
	...
	public void build()
	{
		...
		LogClusterSet oldClusterSet = null;;// = new LogClusterSet();
...
//		   distances = agProfiles.calcuateDistance(dm, centers);
		   distances = agProfiles.calcuateDistance(getDistanceMeasure(), centers);
		   ...
		   for (int i = 0; i < distances.rows(); i++) {
			   ...
			   ((LogCluster) clusterSet.getClusters().get(index)).addTraceStat(traceList.get(i));
			   temp[index] += value;
			   // calcualte mean
		   }
	...
	{
		for(int i=0;i<clusterSize;i++){
			if(clusterSet.getClusters().get(i).size()==0) continue;
			DoubleMatrix1D tempPoint = agProfiles.calcuateCenter(((LogCluster) clusterSet.getClusters().get(i)).getTraces());
			for(int j=0;j<profileSize;j++)
				centers.set(i, j, tempPoint.get(j));
		}
	...
	public void writeToDot(Writer bw) throws IOException {
		...
		for(LogCluster cluster : clusterSet.getClusters()) {
			...
			bw.write("cluster" + clusterCounter + " [shape=\"octagon\",style=\"filled\",fillcolor=\"chartreuse3\",label=\"Cluster "
					+ clusterCounter + "\\n" + cluster.size() + "\"];\n");
			clusterCounter++;
		}
		...
		for(int i=0; i<clusterSet.size()-1; i++) {
			...
			for(int k=i+1; k<clusterSet.size(); k++) {
				if(i==k||clusterSet.getClusters().get(k).size()==0) continue;
				bw.write("cluster" + clusterCounter + " -- cluster" + clusterCounter2 + " [label=\""
					+ length.get(i,k) + "\"];\n");
				clusterCounter2++;
			}
			clusterCounter++;
