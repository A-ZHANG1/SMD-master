package org.prom5.analysis.logclustering.distancemeasure;
...
public class CorrelationCoefficientDistanceMeasure extends DistanceMeasure {
	...
	public DoubleMatrix2D calculateDistance(DoubleMatrix2D profile, DoubleMatrix2D center) {
		int profile_row = profile.rows();
		int center_row = center.rows();
		int column = profile.columns();

		DoubleMatrix2D D = DoubleFactory2D.sparse.make(profile_row, center_row, 0);
		...
		for (int i = 0; i < profile_row; i++) {
			for (int j = 0; j < center_row; j++) {
				...
				for (int k = 0; k < column; k++) {
					upper += (profile.get(i, k) - temp1) * (center.get(j, k) - temp2);
					below1 += (profile.get(i, k) - temp1) * (profile.get(i, k) - temp1);
					below2 += (center.get(j, k) - temp2) * (center.get(j, k) - temp2);
				}
