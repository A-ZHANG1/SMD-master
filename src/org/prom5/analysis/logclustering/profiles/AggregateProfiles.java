...
public class AggregateProfiles extends Profile {
	...
	public DoubleMatrix2D calcuateDistance(DistanceMeasure dm) {
		DoubleMatrix2D distance = DoubleFactory2D.dense.make(traceSize, traceSize, 0);
		for (Profile aProfile : profiles) {
			if (aProfile instanceof VectorProfile) {
				...
				if (tempProfile.getNormalizationMaximum() != 0)
					distance.assign(tempProfile.calculateDistance(dm), cern.jet.math.Functions.plus);
			}
		}
		return distance;
	}

	public DoubleMatrix2D calcuateDistance(DistanceMeasure dm, DoubleMatrix2D centers) {
		DoubleMatrix2D distance = DoubleFactory2D.dense.make(traceSize,centers.rows(), 0);

		int index = 0;
		for (Profile aProfile : profiles) {
			if (aProfile instanceof VectorProfile) {
				VectorProfile tempProfile = (VectorProfile) aProfile;
				DoubleMatrix2D tempCenter = centers.viewPart(0,index,centers.rows(),tempProfile.getProfileSize());
				index += tempProfile.getProfileSize();
				if (tempProfile.getNormalizationMaximum() != 0)
					distance.assign(tempProfile.calculateDistance(dm, tempCenter), cern.jet.math.Functions.plus);
			}
		}
		return distance;
	}
    ...
    public DoubleMatrix1D calcuateCenter(HashSet<Integer> traceSet) {
		...
		for (Profile aProfile : profiles) {
			 if (aProfile instanceof VectorProfile) {
				 VectorProfile tempProfile = (VectorProfile) aProfile;
				 DoubleMatrix1D tempMatrix = tempProfile.calcuateCenter(traceSet);
				 for(int i=k;i<k+tempMatrix.size();i++)
				 {
