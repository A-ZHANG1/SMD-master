...
/**
 * Abstract class defining the interface for a log profile
 * that contains profile of instances in a log
 *
...
public abstract class VectorProfile extends Profile {
	...
	/**;
	 * The number of profiles (e.g. number of tasks, number of originators, etc.)
	 */
	protected int profileSize;

	/**
	 * Matrix holding the actual profile as
	 * derived by the specific profile implementation.
	 */
	 ...
	 */
	protected VectorProfile(String aName, String aDescription, int traceSize, String[] profileItem) {
	...
	public DoubleMatrix2D calculateDistance(DistanceMeasure dm, DoubleMatrix2D center)
	{
	...
	*/
	public DoubleMatrix1D generateRandomPoint() {
		...
		for(int i=0; i<traceSize; i++){
			for(int j=0; j<profileSize;j++){
				if(min[j]>profile.get(i,j)) min[j]=profile.get(i,j);
				if(max[j]<profile.get(i,j)) min[j]=profile.get(i,j);
			}
		}
	...
	/**
	 * Helper method; increases the counter for a given item (class), trace,
	 * by a defined increment.
	 * @param item
	 * @param traceIndex
	 * @param increment
	 */
	protected void increaseItemBy(String item, int traceIndex, double increment) {
		...
		profile.set(traceIndex, column, profile.get(traceIndex, column) + increment);
	}
}
