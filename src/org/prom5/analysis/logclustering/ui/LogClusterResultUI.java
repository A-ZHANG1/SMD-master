...
//	private AggregateProfiles agProfiles;
	...
	public LogClusterResultUI(LogReader log, ClusteringAlgorithm aClusteringAlgorithm) {
		this.log = log;
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		ProvidedObject[] objects = new ProvidedObject[clusteringAlgorithm.size() + 2];
		...
		for(LogCluster cluster : clusteringAlgorithm.getClusters()) {
			...
			try {
				...
				{
					objects[index] = new ProvidedObject("Trace cluster " + ((AHCTreeNode)cluster).getName(),
									 new Object[] {
									 LogReaderFactory.createInstance(log, pitk)
					});
				} else
				{
					objects[index] = new ProvidedObject("Trace cluster " + (index - 1),
						new Object[] {
						LogReaderFactory.createInstance(log, pitk)
				});
