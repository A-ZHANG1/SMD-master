...
import org.prom5.analysis.logclustering.profiles.Profile;
...
/*		if(profile instanceof TraceMetric) {
			this.setBackground(COLOR_TRACE);
			profileType = "trace profile";
		} else if(profile instanceof UnaryMetric) {
			this.setBackground(COLOR_UNARY);
			profileType = "unary profile";
		} else if(profile instanceof SignificanceBinaryLogMetric ||
				profile instanceof SignificanceBinaryDerivateMetric) {
			this.setBackground(COLOR_BINSIG);
			profileType = "binary significance profile";
		} else if(profile instanceof CorrelationBinaryLogMetric) {
			this.setBackground(COLOR_BINCOR);
			profileType = "binary correlation profile";
		}*/
		...
		slider = new JSlider(JSlider.HORIZONTAL, 0, 1000,
				(int)(profile.getNormalizationMaximum() * 1000));
		slider.setOpaque(false);
		slider.addChangeListener(this);
		JLabel sliderLabel = new JLabel("weight:");
		sliderLabel.setOpaque(false);
		sliderLabel.setForeground(COLOR_FG);
		weightLabel = new JLabel(format.format(profile.getNormalizationMaximum()));
		...
		descrLabel.setFont(descrLabel.getFont().deriveFont(10.0f).deriveFont(Font.ITALIC));
	...
	public void stateChanged(ChangeEvent evt) {
		if(evt.getSource() == activeBox) {
			if(activeBox.isSelected() == true) {
				...
				profile.setNormalizationMaximum((double)slider.getValue() / 1000.0);
			} else {
