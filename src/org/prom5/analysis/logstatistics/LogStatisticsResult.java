...
import org.prom5.analysis.hierarchicaldatavisualization.HierarchicalData;
import org.prom5.analysis.hierarchicaldatavisualization.logstatistics.LogStatisticsToHierarchicalDataSet;
import org.prom5.analysis.hierarchicaldatavisualization.logstatistics.ProcessingTimeFactory;
import org.prom5.analysis.hierarchicaldatavisualization.logstatistics.ThroughputTimeFactory;
...
import org.prom5.framework.models.hlprocess.view.HLAttributeView;
...
import org.prom5.framework.util.GenericTableModelPanel;
...
public class LogStatisticsResult extends JPanel implements Provider {
	...
	/**
	 * Keeps the statistics for each process instance. 
	 * [key = name of the process instance, value = LogStatistic object]
	 */
	...
	/**
	 * Keeps simmple statistics about the times between the arrival of two new cases.
	 */
	...
	/**
	 * The activity statistics will be provided as an activity-based 
	 * simulation model (i.e., without an underlying process model).
	 */
	...
	/**@todo anne: remove this workaround - invisible button for invoking the double click table action */
	...
	/**
	 * Constructor. 
	 * Calculate and displays basic log statistics under the assumption that
	 * (1) each activity is representec by both a Start and an End event,  
	 * (2) each Start event is immediately followed by a corresponding End event, and 
	 * (3) both events have a timestamp associated.
	...
	private LogStatisticsResult(LogReader log, boolean computeStatisticsOnly) {
		...
		myGlobalStatistic = new LogStatistic("the whole log", null, myDisplaySettings);
	...
	public static HierarchicalData getProcessingTimes(LogReader log) {
		...
		return new LogStatisticsToHierarchicalDataSet(stats.myInstanceStatistics, log, new ProcessingTimeFactory());
	}
	
	public static HierarchicalData getThroughputTimes(LogReader log) {
		LogStatisticsResult stats = new LogStatisticsResult(log, true);
		return new LogStatisticsToHierarchicalDataSet(stats.myInstanceStatistics, log, new ThroughputTimeFactory());
	}
	...
	public ProvidedObject[] getProvidedObjects() {
		ProvidedObject[] objects = {
				new ProvidedObject("Processing times", new Object[] {
						new LogStatisticsToHierarchicalDataSet(myInstanceStatistics, myLogReader, new ProcessingTimeFactory())}),
				new ProvidedObject("Throughput times", new Object[] {
						new LogStatisticsToHierarchicalDataSet(myInstanceStatistics, myLogReader, new ThroughputTimeFactory())}),
				new ProvidedObject("Simulation Model with Log Analysis results No." + simulationModelCounter, 
						new Object[] {actSet}),
				new ProvidedObject("Whole Log", 
						new Object[] {myLogReader})
	   } ;
	   return objects;
	}
	...
	private void generateActivitySet() {
		...
		actSet.getHLProcess().getGlobalInfo().setName("Simulation Model with Log Analysis results No." + simulationModelCounter);
		// TODO : do not hardcode the time unit but respect user configuration
		actSet.getHLProcess().getGlobalInfo().setTimeUnit(HLTypes.TimeUnit.SECONDS);
		actSet.getHLProcess().getGlobalInfo().addPerspective(HLTypes.Perspective.TIMING_EXECTIME);
		actSet.getHLProcess().getGlobalInfo().addPerspective(HLTypes.Perspective.CASE_GEN_SCHEME);
		actSet.getHLProcess().getGlobalInfo().addPerspective(HLTypes.Perspective.DATA_VALUE_RANGE);
		
		// add actual activity values
		HashMap<String, SummaryStatistics> globalDurations = myGlobalStatistic.getActivityDurations();
		...
		HLDistribution caseArrivals = LogStatistic.getDistributionObject(arrivalStatistic);
		actSet.getHLProcess().getGlobalInfo().setCaseGenerationScheme(caseArrivals);
	}
	...
	private void calculate() {
		...
		while (it.hasNext()) {
			...
			for (String attName : pi.getAttributes().keySet()) {
				HLAttribute att = actSet.getHLProcess().findAttributeByName(attName);
				if (att == null) {
					att = new HLNominalAttribute(attName, actSet.getHLProcess());
				}
				((HLNominalAttribute) att).addPossibleValue(pi.getAttributes().get(attName));
			}
			...
			LogStatistic statistic = new LogStatistic(pi.getName(), pi.getModelReferences(), myDisplaySettings);
			statistic.initialize(myLogReader.getLogSummary().getModelElements());
			...
			while (ates.hasNext()) {
				...
				for (String attName : ate.getAttributes().keySet()) {
					...
					((HLNominalAttribute) att).addPossibleValue(ate.getAttributes().get(attName));
				}
				
				// keep only track of the first date per process instance for arrival times
					...
					if (ate.getType().equals("start")) {
						...
						if (durationReference != null) {
							...
							statistic.addActivityDurationValue(
									ate.getElement(), ate.getElementModelReferences(), ate.getOriginatorModelReferences(), duration);
							// .. and globally
							myGlobalStatistic.addActivityDurationValue(ate
									.getElement(), ate.getElementModelReferences(), ate.getOriginatorModelReferences(), duration);
						}
					}
				}
			}
	...
	/**
	 * Writes test logs to the Message.TEST output in order to make test 
	 * cases for this plugin.
	 */
	private void writeTestLog() {
		//// PLUGIN TEST START
		Message.add("<BasicLogStatistics>", Message.TEST);
		// TODO: also provide more detailed test logs as soon as the presence of the test flag
		// is checked before actually writing the test logs
		Date earliest = new Date((long)myGlobalStatistic.getTimeStampStatistics().getMin());
		Date latest = new Date((long)myGlobalStatistic.getTimeStampStatistics().getMax());
		Message.add("Earliest Timestamp: " + earliest.toString(), Message.TEST);
		Message.add("Latest Timestamp: " + latest.toString(), Message.TEST);
		Message.add("No. of measured Timestamps: " + myGlobalStatistic.getTimeStampStatistics().getN(), Message.TEST);
		Message.add("</BasicLogStatistics>", Message.TEST);
		// PLUGIN TEST END
	}
	...
	private void jbInit() {
		...
		instanceNameTable = new DoubleClickTable(new ProcessInstanceTable(), updateViewButton);
		...
		globalViewPanel.setLayout(new BoxLayout(globalViewPanel, BoxLayout.PAGE_AXIS));
		GuiPropertyStringTextarea helpText = new GuiPropertyStringTextarea("The following options allow you to specify the " + 
		...
		myGlobalContentPanel.add(myGlobalStatistic.getResultPanel(myDisplaySettings.getCurrentViewMode()));
		globalViewPanel.add(myGlobalContentPanel);
		
		// instance-wise view
		LogStatistic statistic = (LogStatistic) myInstanceStatistics.get(myInstanceIDs.get(0));
		...
		myInstanceContentPanel.add(statistic.getResultPanel(myDisplaySettings.getCurrentViewMode()));
		...
		// build tab views
		tabPane.addTab("Global Activity Statistics", null, globalViewPanel, "View basic statistics for the whole log");
		tabPane.addTab("Instance-wise Activity Statistics", null, instanceViewPanel, "View basic statistics for each process instance");
		tabPane.addTab("Attribute Statistics", null, attributesPanel, "View occurrence statistics for each data attribute");
	...
	/**
	 * Creates a GUI panel containing a list of all those attributes that are contained in this
	 * simulation model.
	 * <br>
	 * Note that the panel can be readily displayed and modifications to the displayed attributes
	 * are propagated to the internally maintained values.
	...
	public JPanel getDataAttributesPanel() {
		...
		for (HLAttribute attrib : actSet.getHLProcess().getAttributes()) {
			attrib = HLAttributeManager.autoChangeType((HLNominalAttribute) attrib);
			HLAttributeView attGui = new HLAttributeView(attrib);
			dataAttrHL.add(attGui);
		}
		resultPanel.add(new GenericTableModelPanel(dataAttrHL, "Attributes",
				"Here you can view occurrence statistics for data values for each attribute in the log.", null));
		resultPanel.add(Box.createVerticalGlue());
		return resultPanel;
	}
	...
	private void registerGuiActionListener() {
		updateViewButton.addActionListener(new ActionListener() {
			// specifies the action to be taken when a process instance has been double-clicked
	...
	/**
	 * Redraws the contents of the currently viewed instance panel according to the 
	 * current display settings.
	 */
	private void updateInstancePanel() {
		// determine current selection (only one can be selected as it has been doubleclicked)
		int[] indexArray = instanceNameTable.getSelectedRows();
		// retrieve new statistics get
		LogStatistic statistic = (LogStatistic) myInstanceStatistics.get(myInstanceIDs.get(indexArray[0]));
	...
	/**
	 * Redraws the contents of the global panel according to the 
	 * current display settings.
	 */
	...
	/**
	 * Private data structure for the table containing the process instance names.
	 */
