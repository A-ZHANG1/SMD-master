...
//CONSTRUCTORS
    /**
     * Constructs a LTLVerificationResult for the given formula tested on the given
     * log. The indices of the (in)correct instances are passed trough two
     ...
     * @param log LogReader
     * @param correctOnes ArrayList
     * @param totalCorrectOnes int
     * @param badOnes ArrayList
     * @param totalBadOnes int
     */
    public LTLVerificationResult(String formulaName, LogReader log,
                                 ArrayList correctOnes,
                                 int totalCorrectOnes,
                                 ArrayList badOnes, int totalBadOnes) {
     ...
     * @param badOnes ArrayList
     * @param totalBadOnes int
     * @param sss Substitutes
     */
    public LTLVerificationResult(String formulaName, LogReader log,
                                 ArrayList correctOnes, int totalCorrectOnes,
                                 ArrayList badOnes, int totalBadOnes,
                                 Substitutes sss) {
...
//METHODS
        ...
        if (correctIdList.length > 0) {
            try {
                a.add(new ProvidedObject(
                        "Correct instances (" + numCorrectPIs + ")",
    ...
    private void createGui(Substitutes sss) {
        ...
        html +=
                "<tr valign=top, halign=left><td>Parameters:</td><td>" +
                sss.toString() +
                "</td></tr>";
        html += "</table>";
        html += "</html>";
                ...
                if (
                    ...
                    if (me.getComponent() == correctView) {
                        ...
                        if (row >= 0) {
                            ...
                            createVisualisation(((CheckResult) correctOnes.get(
    ...
    private void createVisualisation(ProcessInstance pi,
                                     boolean correct) {

        if (correct) {

            this.piCorrect = pi;
            correctViewPane.getViewport().add(new ProcessInstanceVisualization(
                    pi).getGrappaVisualization(), null);

        } else {

            this.piBad = pi;
            badViewPane.getViewport().add(new ProcessInstanceVisualization(pi).
