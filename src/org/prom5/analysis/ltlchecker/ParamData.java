...
public class ParamData extends AbstractTableModel {
	...
	public ParamData(List<FormulaParameter> params, OntologyCollection semanticLog) {
		...
		while (i.hasNext()) {
			...
			switch (var.getType()) {
				...
				case Attribute.NUMBER: {
					...
					data[j][1] = "number";
					data[j][2] = parameter.hasDefaultValue() ? parameter.getDefaultValue() : "0.0";
				}
				break;
				...
				case Attribute.STRING: {
					...
					data[j][2] = parameter.hasDefaultValue() ? cutQuotes(parameter.getDefaultValue()) : "type a string";
				}
				break;
				...
				case Attribute.DATE: {
					...
					data[j][0] = var.getValue();
					data[j][1] = "date";
					data[j][2] = parameter.hasDefaultValue() ? cutQuotes(parameter.getDefaultValue()) : sdf.format(currentDate);
				}
				break;
			}
	...
	public void setValueAt(Object value, int row, int col) {
		...
		if (value instanceof ValueNode || correctValue((String) value, row, col)) {
	...
	public Substitutes getSubstitutes(LTLParser parser) {
		...
		if (errors.size() > 0) {
			StringBuffer msg = new StringBuffer("The following errors were found:\n\n");
			for (String s : errors) {
				msg.append("  - " + s + "\n");
			}
	...
	public SetValueNode createSetValueNode(String value, int indexInTable) {
		...
		if (strings.size() == 1) {
			...
			setval.setValue(new SetAttribute(strings.first(), Attribute.LITERAL, params.get(indexInTable).getParam()));
			return setval;
		} else {
