...
package org.prom5.analysis.ltlchecker;

import java.util.ArrayList;
import java.util.Iterator;
import java.util.List;

import org.prom5.analysis.ltlchecker.formulatree.AlwaysNode;
import org.prom5.analysis.ltlchecker.formulatree.AndNode;
import org.prom5.analysis.ltlchecker.formulatree.BiimpliesNode;
import org.prom5.analysis.ltlchecker.formulatree.CompNode;
import org.prom5.analysis.ltlchecker.formulatree.DateCompNode;
import org.prom5.analysis.ltlchecker.formulatree.DateValueNode;
import org.prom5.analysis.ltlchecker.formulatree.EventuallyNode;
import org.prom5.analysis.ltlchecker.formulatree.ExistsNode;
import org.prom5.analysis.ltlchecker.formulatree.ForallNode;
import org.prom5.analysis.ltlchecker.formulatree.FormulaNode;
import org.prom5.analysis.ltlchecker.formulatree.ImpliesNode;
import org.prom5.analysis.ltlchecker.formulatree.NexttimeNode;
import org.prom5.analysis.ltlchecker.formulatree.NotNode;
import org.prom5.analysis.ltlchecker.formulatree.NumberCompNode;
import org.prom5.analysis.ltlchecker.formulatree.NumberValueNode;
import org.prom5.analysis.ltlchecker.formulatree.OrNode;
import org.prom5.analysis.ltlchecker.formulatree.SetCompNode;
import org.prom5.analysis.ltlchecker.formulatree.SetValueNode;
import org.prom5.analysis.ltlchecker.formulatree.StringCompNode;
import org.prom5.analysis.ltlchecker.formulatree.StringValueNode;
import org.prom5.analysis.ltlchecker.formulatree.TreeNode;
import org.prom5.analysis.ltlchecker.formulatree.UntilNode;
import org.prom5.analysis.ltlchecker.formulatree.ValueNode;
import org.prom5.analysis.ltlchecker.parser.ASTFormulaCall;
import org.prom5.analysis.ltlchecker.parser.ASTQuantification;
import org.prom5.analysis.ltlchecker.parser.ASTStringList;
import org.prom5.analysis.ltlchecker.parser.Attribute;
import org.prom5.analysis.ltlchecker.parser.ConceptSetAttribute;
import org.prom5.analysis.ltlchecker.parser.DateAttribute;
import org.prom5.analysis.ltlchecker.parser.FormulaParameter;
import org.prom5.analysis.ltlchecker.parser.LTLParser;
import org.prom5.analysis.ltlchecker.parser.Node;
import org.prom5.analysis.ltlchecker.parser.NumberAttribute;
import org.prom5.analysis.ltlchecker.parser.SetAttribute;
import org.prom5.analysis.ltlchecker.parser.SimpleNode;
...
// FIELDS
	...
	/**
	 * The parser with the parse data from which the formula tree is build
	 * up.
	 */
...
// CONSTRUCTORS
	...
	public TreeBuilder(LTLParser parser, String formulaName, SetsSet ss, OntologyCollection ontologies) {
		this.parser = parser;
...
// METHODS
	 ...
	 *
	 * @param pNode The subtree from the parser of the `selected' formula.
	 * @param substs A list with substitutions, that is, a list with values
	 * for the parameters of pNode's formula.
	 * @param binder The operator of treenode to bind a valuenode to.
	 *
		...
		switch (parserNode.getType()) {
			...
			case SimpleNode.FORMULA: {
				...
				if (parserNode.jjtGetNumChildren() > 1) {
					result = build(parserNode.jjtGetChild(1), substs,
							 binder);
				} else {
					result = build(parserNode.jjtGetChild(0), substs,
							 binder);
				}
				;
			}
			...
			case SimpleNode.FORALL: {
				...
				for (SetsSetItem item : sets.getSet(attr, var.getScope())) {
					...
					svn.setValue(new SetAttribute(item.getName(), Attribute.LITERAL, var, item.getModelReferences()));
					svn.setBinder(node);

					Substitutes s = (Substitutes) substs.clone();
					s.add(var.getValue(), svn);
					node.addChild((FormulaNode) build(qNode.jjtGetChild(1), s, node));
				}
				result = node;
			}
			; break;
			...
			case SimpleNode.EQUAL: {
				...
				result = compOp(parserNode, substs, CompNode.EQUAL,
						 binder);
			}
			; break;
			...
			case SimpleNode.NOTEQUAL: {
				...
				result = compOp(parserNode, substs, CompNode.NOTEQUAL,
						 binder);
			}
			; break;
			...
			case SimpleNode.LESSOREQUAL: {
				...
				result = compOp(parserNode, substs,
						 CompNode.LESSEREQUAL, binder);
			}
			; break;
			...
			case SimpleNode.BIGGEROREQUAL: {
				...
				result = compOp(parserNode, substs,
						 CompNode.BIGGEREQUAL, binder);
			}
			; break;

			case SimpleNode.LESSER: {

				result = compOp(parserNode, substs, CompNode.LESSER,
						 binder);
			}
			; break;
			...
			case SimpleNode.BIGGER: {
				...
				result = compOp(parserNode, substs, CompNode.BIGGER,
						 binder);
			}
			; break;
			...
			case SimpleNode.PLUS: {
				...
				NumberValueNode node = new NumberValueNode(
									   NumberValueNode.PLUS);
				node.setLeftChild((NumberValueNode) build(parserNode.jjtGetChild(0),
						substs, node));
				node.setRightChild((NumberValueNode) build(parserNode.jjtGetChild(1),
						substs, node));
				result = node;
			...
			case SimpleNode.DATESTRING: {
				...
				DateValueNode node = new DateValueNode(
									 DateValueNode.VALUE);
			...
			case SimpleNode.ATTRIBUTE: {
				...
				if (parser.existsAttribute(parserNode.getName())) {
					...
					Attribute attr = parser.getAttribute(
									 parserNode.getName());
					switch (attr.getType()) {
			...
			case SimpleNode.USEFORMULA: {
				...
				List<FormulaParameter> params = parser.getParameters(useNode.getName());
				...
				while (i.hasNext() && j.hasNext()) {
					...
					if (substs.hasId(attr.getValue())) {
						...
						s.add(j.next().getParam().getValue(), substs.get(attr.getValue()));
					} else {
				...
				SimpleNode newFormula = parser.getFormula(
										useNode.getName());
	...
	private TreeNode compOp(Node pNode, Substitutes substs,
			int compType, TreeNode binder) {
		...
		switch (type) {
			...
			case Attribute.NUMBER: {
				NumberCompNode node = new NumberCompNode(
									  compType);
			...
			case Attribute.STRING: {
				StringCompNode node = new StringCompNode(
									  compType);
			...
			case Attribute.DATE: {
				DateCompNode node = new DateCompNode(
									compType);
			...
			case Attribute.SET: {
				...
				if (compType == CompNode.IN) {
					...
					valNode.setSet(((ASTStringList)
							parserNode.jjtGetChild(1)).getStrings());
