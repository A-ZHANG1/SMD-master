...
// FIELDS
    ...
    /** Lefthandside attribute of this comparison. Either a SetAttribute or a ConceptSetAttribute. */
        ...
        if (ateNr < ates.size()) {
            try {
                ...
                if (value.getType() == SetValueNode.MODEL_REFERENCE_SET) {
                	List<String> refs = attribute.modelReferences(pi, ates, ateNr);
	                switch (this.op) {
	                case CompNode.EQUAL: 
	                case CompNode.IN: 
	                ...
	                case CompNode.NOTEQUAL:
	                	...
	                	break;
                	default:
                		throw new RuntimeException("cannot use operator " + opAsString(this.op) + " on a set of concepts");
	                }
	                ...
	                case CompNode.NOTEQUAL:
	                	...
	                	break;
                	default:
                		throw new RuntimeException("cannot use operator " + opAsString(this.op) + " on a set of values");
	                }
                } else {
                	String attrValue = attribute.value(pi, ates, ateNr);
                	...
                	case CompNode.BIGGER: 
                		...
                		break;
                	default:
                		throw new RuntimeException("cannot use operator " + opAsString(this.op) + " on a single value");
                	}
                }
