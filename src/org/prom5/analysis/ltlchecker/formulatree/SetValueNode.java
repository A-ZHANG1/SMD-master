	...
	public Set<String> getModelReferencesInLog() {
		...
		for (int i = 0; i < refsInOntologies.size(); i++) {
			...
			if (i + 1 < refsInOntologies.size()) {
				includeSuper = "include-super-concepts".equals(refsInOntologies.get(i + 1));
				includeSub = "include-sub-concepts".equals(refsInOntologies.get(i + 1));
				
				if (i + 2 < refsInOntologies.size() && (includeSub || includeSuper)) {
					includeSuper = includeSuper || "include-super-concepts".equals(refsInOntologies.get(i + 2));
					includeSub = includeSub || "include-sub-concepts".equals(refsInOntologies.get(i + 2));
				}
			}
			if (ontologies == null) {
				result.add(uri);
			} else {
				result.addAll(ontologies.translateToReferenceInLog(uri, includeSuper, includeSub));
			}
		}
