...
public class MergeSimModelsPlugin implements AnalysisPlugin {
	...
	public AnalysisInputItem[] getInputItems() {
		AnalysisInputItem[] items = {
		  new AnalysisInputItem("Simulation Model", 2, Integer.MAX_VALUE) {
	...
	/*
	 *  (non-Javadoc)
	 * @see org.processmining.analysis.AnalysisPlugin#analyse(org.processmining.analysis.AnalysisInputItem[])
	 */
	public JComponent analyse(AnalysisInputItem[] inputs) {
		...
		//
		ReferenceModelChooser refChooser = new ReferenceModelChooser(activityModels);
		if (refChooser.showModal()) {
			...
			// For each of the other activity models a mapping needs to be made from the activity nodes in the
			// reference model to the highlevelactivities in the other activity models.
			// Therefore, for each other activity model a screen needs to pop on which the user can define the mapping
			Iterator<HLModel> actModelsIt = refChooser.getNotSelectedReferenceModels().iterator();
			while (actModelsIt.hasNext()) {
				...
				ReferenceModelMapping refModMapping = new ReferenceModelMapping(clonedReferenceModel, clonedActivityModel);
