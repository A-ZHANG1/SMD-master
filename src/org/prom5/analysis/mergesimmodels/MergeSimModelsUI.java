...
/**
 * Provides the GUI with which the user can indicate for each perspective from which of
 * the provided simulation models the specific information for that perspective, provided by
 * the selected simulation model, needs to be copied to the final simulation model.
 * The simulation information that is copied will be visualized in a separate panel.
 *
 * @author rmans
 */
public class MergeSimModelsUI extends JPanel implements Provider {
	...
	/**
	 * Basic constructor
	 * @param simModels ArrayList list of simulation models that need to be merged
	 */
	public MergeSimModelsUI(ArrayList<HLModel> simModels) {
		mySimModels = simModels;
		// create the final simulation model which has default simulation information
	...
	/**
	 * Construct the user interface for the merge simulation models analysis plugin.
	 */
	private void jbInit() {
		...
		myTimingPanel = new TimingTabUI(myFinalSimModel, mySimModels, myGraphPanel);
		myDataPanel = new DataTabUI(myFinalSimModel, mySimModels, myGraphPanel);
		myOrganizationalPanel = new OrganizationalTabUI(myFinalSimModel, mySimModels, myGraphPanel);
		myChoicesPanel = new ChoicesTabUI(myFinalSimModel, mySimModels, myGraphPanel);
		// build tab views
		...
		myFinalSimModel.getHLProcess().getGlobalInfo().addPerspective(HLTypes.Perspective.TIMING_EXECTIME);
		myFinalSimModel.getHLProcess().getGlobalInfo().addPerspective(HLTypes.Perspective.TIMING_WAITTIME);
		myFinalSimModel.getHLProcess().getGlobalInfo().addPerspective(HLTypes.Perspective.TIMING_SOJTIME);
		myFinalSimModel.getHLProcess().getGlobalInfo().addPerspective(HLTypes.Perspective.CASE_GEN_SCHEME);
		myFinalSimModel.getHLProcess().getGlobalInfo().addPerspective(HLTypes.Perspective.ORGANIZATIONAL_MODEL);
		myFinalSimModel.getHLProcess().getGlobalInfo().addPerspective(HLTypes.Perspective.ROLES_AT_TASKS);
		myFinalSimModel.getHLProcess().getGlobalInfo().addPerspective(HLTypes.Perspective.DATA_AT_TASKS);
		myFinalSimModel.getHLProcess().getGlobalInfo().addPerspective(HLTypes.Perspective.DATA_INITIAL_VAL);
		myFinalSimModel.getHLProcess().getGlobalInfo().addPerspective(HLTypes.Perspective.DATA_VALUE_RANGE);
		myFinalSimModel.getHLProcess().getGlobalInfo().addPerspective(HLTypes.Perspective.CHOICE_DATA);
		myFinalSimModel.getHLProcess().getGlobalInfo().addPerspective(HLTypes.Perspective.CHOICE_PROB);
		myFinalSimModel.getHLProcess().getGlobalInfo().addPerspective(HLTypes.Perspective.CHOICE_FREQ);
		myFinalSimModel.getHLProcess().getGlobalInfo().setName("Merged Simulation Model (" + simulationModelCounter + ")");
	...
	public ProvidedObject[] getProvidedObjects() {
		if (myFinalSimModel != null) {
			ProvidedObject[] objects = {
									   new ProvidedObject("Merged Simulation Model (" + simulationModelCounter + ")",
