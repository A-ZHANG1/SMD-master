...
/**
 * In this panel, the user can select one of the simulation models, that needs to
...
public class ReferenceModelChooser extends JDialog {
	...
	/** combo box showing the names of the simulation models that can be
	...
	/**
	 * Basic constructor
	 * @param activityModels List simulation models that may be selected as a reference model
	 * @throws HeadlessException
	 */
	public ReferenceModelChooser(List<HLModel> activityModels) throws HeadlessException {
	...
	private void jbInit() {
		...
		myPosRefModels = new GUIPropertyListEnumeration("", "", myActivityModels, null, 300);
		...
		String description = new String("Please choose one of the input simulation models as the reference model. " 
	 ...
	 * @return HighLevelProcess the simulation model that is assigned as reference model
	 */
	...
	/**
	 * Retrieves the highlevelprocesses that are not selected. In the case that the choosen
	 * reference model appears twice or more in the list, the first occurrence will be left out.
