package org.prom5.analysis.ontologies;
...
import org.prom5.framework.models.ontology.InvalidModelException;
...
public class AnnotateWithDefaultOntologies {
	...
	private static void createConcept(OntologyModel ontology, ConceptModel superConcept, String newConceptName, AuditTrailEntry ate, int type) throws InvalidModelException, IndexOutOfBoundsException, IOException {
	...
	private static void annotateDataSection(OntologyModel ontology, ConceptModel datafieldConcept, DataSection dataAttributes) throws InvalidModelException {
	...
	private static void createEVO(OntologyModel evo) throws InvalidModelException {
		...
		ConceptModel successfulExecutionEvent = evo.addConcept("SuccessfulExecutionEvent");
		...
		ConceptModel unsuccessfulExecutionEvent = evo.addConcept("UnsuccessfulExecutionEvent");
	...
	public static OntologySummaryResults analyse(LogReader inputLog) throws Exception {

		Progress progress = new Progress("Annotating the log " + inputLog.getFile().getShortName() + " with default ontologies.");
		
		// create a copy so we don't modify the original log
		LogReader log = LogReaderFactory.createInstance(inputLog.getLogFilter(), inputLog);

		
		OntologyModel tasks = log.getLogSummary().getOntologies().createOntology(
				"http://www.processmining.org/ontologies/TaskOntology", "TaskOntology");
		OntologyModel originators = log.getLogSummary().getOntologies().createOntology(
				"http://www.processmining.org/ontologies/OriginatorOntology", "OriginatorOntology");
		OntologyModel eventtypes = log.getLogSummary().getOntologies().createOntology(
				"http://www.processmining.org/ontologies/EVO", "EVO");
		OntologyModel PIs = log.getLogSummary().getOntologies().createOntology(
				"http://www.processmining.org/ontologies/ProcessInstanceOntology", "ProcessInstanceOntology");
		OntologyModel processes = log.getLogSummary().getOntologies().createOntology(
				"http://www.processmining.org/ontologies/ProcessOntology", "ProcessOntology");
		OntologyModel workflowlogs = log.getLogSummary().getOntologies().createOntology(
				"http://www.processmining.org/ontologies/WorkFlowLogOntology", "WorkFlowLogOntology");
		OntologyModel sources = log.getLogSummary().getOntologies().createOntology(
				"http://www.processmining.org/ontologies/SourceOntology", "SourceOntology");
		OntologyModel datafields = log.getLogSummary().getOntologies().createOntology(
				"http://www.processmining.org/ontologies/DataFieldOntology", "DataFieldOntology");
		...
		for (ProcessInstance pi : log.getInstances()) {
			if(progress != null && progress.isCanceled()){
				return new OntologySummaryResults(log.getLogSummary().getOntologies(), log);
			}
			...
			for (int index = 0; index < ates.size(); index++) {
				...
				ConceptModel eventConcept = getEVOConcept(eventtypes, ate.getType());
				
				createConcept(tasks, task, ate.getElement(), ate, ConceptModel.WORKFLOW_MODEL_ELEMENT);
				if (eventConcept != null) {
					eventConcept.addInstance(ate, ConceptModel.EVENTTYPE);
				}
				if (ates.get(index).getOriginator() != null && ate.getOriginator().trim().length() > 0) {
					createConcept(originators, originator, ate.getOriginator(), ate, ConceptModel.ORIGINATOR);
				}
				
				annotateDataSection(datafields, ate_data, ate.getDataAttributes());
				
				// we need to replace the ATE so the BufferedLogReader knows it has changed
		...
		progress.setNote("Building the graphical representation of the mined ontologies.");
		progress.setProgress(1);
		
		for (int index = 0; index < log.numberOfProcesses(); index++) {
			process.addInstance(log.getProcess(index));
			annotateDataSection(datafields, process_data, log.getProcess(index).getDataAttributes());
		}
		progress.setProgress(2);
		
		workflowlog.addInstance(log.getLogSummary().getWorkflowLog());
		annotateDataSection(datafields, log_data, log.getLogSummary().getWorkflowLog().getData());
		
		progress.setProgress(3);
		
		source.addInstance(log.getLogSummary().getSource());
		annotateDataSection(datafields, source_data, log.getLogSummary().getSource().getData());
