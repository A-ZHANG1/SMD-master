...
public class OntologySummaryResults extends JPanel implements Provider {
	...
	private void updateGraph() {
		...
		if (table.getSelectedRowCount() == 0) {
			JLabel msg = new JLabel(ontologies.isEmpty() ? "Log does not contain any ontologies." : "Please select an ontology first.");
			msg.setHorizontalAlignment(SwingConstants.CENTER);
			graphPanel.add(msg, BorderLayout.CENTER);
		} else {
			OntologyModel ontology = tableModel.getOntology(table.getSelectedRow());
			
			ModelGraphPanel gp = ontology.toModelGraph(checkShowInstances.isSelected()).getGrappaVisualization();
	...
	public ProvidedObject[] getProvidedObjects() {
		return log == null ? new ProvidedObject[0] : new ProvidedObject[] { new ProvidedObject("Annotated log", log) };
	}
