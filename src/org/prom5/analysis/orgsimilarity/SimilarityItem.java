...
/**
 * Encapsulates one item to be calculated, i.e. a org model which
 * is named after the algorithm having mined it. Provides storage
 * for named measurements (use metrics for names / keys).
 * 
 * @author Minseok Song
 */
public class SimilarityItem {
	...
	public SimilarityItem(String aName, OrgModel aModel, String aModelName) {
		name = aName + " (Originator:" + aModel.getResourceList().size() + ",OrgEntity:" + aModel.getOrgEntityList().size()   + ")";
		model = aModel;
