...
import org.prom5.analysis.orgsimilarity.ui.SimilarityUI;
...
public class SimilarityModel {
	...
	public SimilarityModel(List<SimilarityItem> simItems)
	{
		...
		for(SimilarityItem item: simItems)
		{
			ItemMatrix itemMatrix = new ItemMatrix(item.getModel(),originatorList);
	...
	public SimilarityResultTableModel getResult(int index1, int index2, String metrics)
	{
		...
		mapping(item1, item2);
		result = new SimilarityResultTableModel(item1.getOrgModel(),item2.getOrgModel(),  
				matrix, relations);
		return result;
	}
	...
	public static DoubleMatrix2D correlationcoefficient(DoubleMatrix2D m1, DoubleMatrix2D m2) {
	...
	public static DoubleMatrix2D euclidiandistance(DoubleMatrix2D m1, DoubleMatrix2D m2) {
		...
		for (int i = 0; i < row1; i++) {
			for (int j = 0; j < row2; j++) {
				...
				for (int k = 0; k < column; k++) {
					temp += (double) ((m1.get(i, k) - m2.get(j, k)) * (m1.get(i, k) - m2.get(j, k)));
				}
	...
	public static DoubleMatrix2D hammingdistance(DoubleMatrix2D m1, DoubleMatrix2D m2) {
	...
	public static DoubleMatrix2D similaritycoefficient(DoubleMatrix2D m1, DoubleMatrix2D m2) {
