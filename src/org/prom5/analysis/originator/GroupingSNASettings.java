...
import org.prom5.framework.ui.filters.GenericMultipleExtFilter;
import org.prom5.framework.util.GUIPropertyListEnumeration;
import org.prom5.framework.util.GUIPropertyString;
import org.prom5.framework.util.GuiPropertyStringList;
import org.prom5.mining.snamining.SocialNetworkResults;
import org.prom5.mining.snamining.model.SocialNetworkMatrix;

/**
 * Displays the analysis settings frame that precedes the actual
 * analysis. The user can define group and relationship between group and nodes here.
 *
...
public class GroupingSNASettings extends JPanel{
	...
	public GroupingSNASettings(GroupingSNAPlugin algorithm, AnalysisInputItem[] input, SocialNetworkMatrix snMatrix) {
		...
		groupNames = new GuiPropertyStringList("Group Name", new ArrayList<String>());
	...
	private void jbInit() {
		...
		splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT,tabPane,rightPane);
	 ...
	 private void drawRightPane() {
		 ...
		 for (int i = 0; i < mySnMatrix.getNodeNames().length; i++) {
			 uiNodesToGroup[i] = new UiNodesToGroup(new GUIPropertyString("name",
								 mySnMatrix.getNodeNames()[i]),
								 new GUIPropertyListEnumeration("group", groupNames.getAllValues()));
		 ...
		 rightPane = new JScrollPane(rightInnerPane);
 		 rightPane.setBorder(BorderFactory.createLineBorder(new Color(150, 150, 150), 1));
	 }
	...
	private void initFilePane()
	{
		...
		chooseOrgModelButton.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(ActionEvent e) {
				chooseOrgModelButton_actionPerformed(e);
			}
		});

		selectRoleUnit = new GUIPropertyListEnumeration("Grouping property", new ArrayList(Arrays.asList(new String[]{"role","org unit","mined group"})));
	...
	public String getOrgModelFileName() {
	 ...
	 private void makeRelationship2()
	 {
		 ...
		 if(selected.equals("role")) groupNameforResult = orgModel.getOrgEntityStringList(OrgEntity.ORGENTITYTYPE_ROLE);
		 else if(selected.equals("mined group")) groupNameforResult = orgModel.getOrgEntityStringList(OrgEntity.ORGENTITYTYPE_MININGRESULT);
		 else groupNameforResult = orgModel.getOrgEntityStringList(OrgEntity.ORGENTITYTYPE_ORGUNIT);
		 ...
		 for (int i = 0; i < mySnMatrix.getNodeNames().length; i++) {
			 ...
			 if(selected.equals("role")){
				 uiNodesToGroup[i] = new UiNodesToGroup(new GUIPropertyString("name",
									 mySnMatrix.getNodeNames()[i]),
									 new GUIPropertyListEnumeration("group",
									 orgModel.getOrgEntityList(res,OrgEntity.ORGENTITYTYPE_ROLE)));
			 }
			 else if(selected.equals("mined group")){
				 uiNodesToGroup[i] = new UiNodesToGroup(new GUIPropertyString("name",
					 mySnMatrix.getNodeNames()[i]),
					 new GUIPropertyListEnumeration("group",
					 orgModel.getOrgEntityList(res,OrgEntity.ORGENTITYTYPE_MININGRESULT)));
			 }
			 else{
				 uiNodesToGroup[i] = new UiNodesToGroup(new GUIPropertyString("name",
									 mySnMatrix.getNodeNames()[i]),
									 new GUIPropertyListEnumeration("group",
									 orgModel.getOrgEntityList(res,OrgEntity.ORGENTITYTYPE_ORGUNIT)));
			 }
			 imsiPane.add(uiNodesToGroup[i].getNodeName().getPropertyPanel());
			 imsiPane.add(uiNodesToGroup[i].getGroup().getPropertyPanel());
	...
	public void startAnalysis() {
		...
		if(uiNodesToGroup[0].getGroup().getValue()!=null){
			for (int i = 0; i < size; i++) {
				assignment[i] = uiNodesToGroup[i].getGroup().getValue().toString();
			}

			// spawn result window
			MainUI.getInstance().createAnalysisResultFrame(myAlgorithm, myInput,
					new SocialNetworkResults(null, mySnMatrix.groupOriginators(groupNameforResult, assignment), null));
		}
	}
	...
	class UiNodesToGroup
	{
	    ...
	    public UiNodesToGroup(GUIPropertyString nodename, GUIPropertyListEnumeration group)
		{
