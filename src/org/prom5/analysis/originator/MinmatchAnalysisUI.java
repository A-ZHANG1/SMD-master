...
/**
 * On the panel originator by task matrix is shown. The matrix show how frequent each originator executes specific tasks. <br/>
 *
 * @author Minseok Song
 * @version 1.0
 */
public class MinmatchAnalysisUI extends JPanel implements Provider {
	...
	public MinmatchAnalysisUI(SimilarityModel simModel, Mismatch2DTableModel matrix) {
	...
	public void jbInit() throws Exception {
		...
		for (int i = 1; i < table.getColumnCount(); i++) {
			DefaultTableCellRenderer renderer = new ColoredCellRenderer2(maxElement);
...
//		Message.add("<SummaryOfMatrix sumOfCells=\"" + matrix.getSumOfOTMatrix() + "\" numberOfEvents=\"" +
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		if (simModel != null) {
			objects = new ProvidedObject[] {
					new ProvidedObject("Mismatch Matrix", new Object[] {simModel, matrix})
					};
		}
		return objects;
	}
...
//		int freq = (int) Math.round(100*Double.parseDouble((value).toString()));
//		int color = maxValue <= 0 ? 0 : freq * 0xFF / maxValue;
		...
		if(String.valueOf(value).equals("")) {
			...
			if(column==0) setBackground(Color.LIGHT_GRAY);
			else if(column==table.getColumnCount()-1||row==table.getRowCount()-1) setBackground(new Color(0xFF-50, 0xFF, 0xFF-50));
