...
public class Mismatch2DTableModel extends AbstractTableModel {
	...
	protected void init(){
		...
		mismatchMatrix = DoubleFactory2D.sparse.make(orgentities1.size()+1, orgentities2.size()+1, 0);
		matchedMatrix = DoubleFactory2D.sparse.make(orgentities1.size()+1, orgentities2.size()+1, 0);
		...
		for(OrgEntity tempEntity: relations.keySet()) {
			...
			ArrayList<String> targetRes = orgModel2.getResourceList(targetEntity);
			for(String res: sourceRes){
				if(targetRes.contains(res))num++;
			}
			...
			mismatchMatrix.set(row, orgentities2.size(), mismatchMatrix.get(row, orgentities2.size())+num);
			matchedMatrix.set(row, orgentities2.size(), matchedMatrix.get(row, orgentities2.size())+sourceRes.size());
			mismatchMatrix.set(orgentities1.size(),col, mismatchMatrix.get(orgentities1.size(),col)+num);
			matchedMatrix.set(orgentities1.size(), col, matchedMatrix.get(orgentities1.size(),col)+sourceRes.size());
			mismatchMatrix.set(orgentities1.size(),orgentities2.size(), mismatchMatrix.get(orgentities1.size(),orgentities2.size())+num);
			matchedMatrix.set(orgentities1.size(), orgentities2.size(), matchedMatrix.get(orgentities1.size(),orgentities2.size())+sourceRes.size());
	...
	public Object getValueAt(int row, int col) {
		...
		if(col ==0 ) {
			...
			if(matchedMatrix.get(row, col - 1)==0) return "";
			String result = String.valueOf((int)mismatchMatrix.get(row, col - 1)) +"/"+ String.valueOf((int)matchedMatrix.get(row, col - 1));
			return result;
		}
	}
    ...
    public DoubleMatrix2D getFilteredOTMatrix(String[] filteredUsers) {
		if(mismatchMatrix == null) init();
		DoubleMatrix2D D = DoubleFactory2D.sparse.make(filteredUsers.length, orgentities2.size(), 0);

		ArrayList<String> userArrayList = new ArrayList<String>(Arrays.asList(filteredUsers));
	...
	public int getMaxElement() {
		...
		for (int row = 0; row < mismatchMatrix.rows(); row++) {
			for (int col = 0; col < mismatchMatrix.columns(); col++) {
				result = Math.max(result, (int) Math.round(mismatchMatrix.get(row, col)));
			}
		}
		return result;
	}
	...
	public void writeToTestLog() {
		if(mismatchMatrix == null) init();
			/// PLUGIN TEST START
			Message.add("<SummaryOfMatrix numberOfUsers=\""+ orgentities1.size() + "\" numberOfRows=\"" +
					String.valueOf(getRowCount()) + "\">", Message.TEST);
			Message.add("<SummaryOfMatrix numberOfTasks=\"" + orgentities2.size() + "\" numberOfColumns=\"" +
