...
public class OTMatrix2DTableModel extends AbstractTableModel {
	...
	public Object getValueAt(int row, int col) {
		if(otMatrix == null) init();
		return col == 0 ? originators.get(row) : String.valueOf(otMatrix.get(row, col - 1));
	}

	private DoubleMatrix2D makeOTMatrix(LogReader log) {
		DoubleMatrix2D D = DoubleFactory2D.sparse.make(originators.size(), tasks.size(), 0);
    ...
    public DoubleMatrix2D getFilteredOTMatrix(String[] filteredUsers) {
		if(otMatrix == null) init();
		DoubleMatrix2D D = DoubleFactory2D.sparse.make(filteredUsers.length, tasks.size(), 0);
	...
	public int getMaxElement() {
		...
		for (int row = 0; row < otMatrix.rows(); row++) {
			for (int col = 0; col < otMatrix.columns(); col++) {
				result = Math.max(result, (int) Math.round(otMatrix.get(row, col)));
			}
		}
		return result;
	}
	...
	public void writeToTestLog() {
		if(otMatrix == null) init();
			/// PLUGIN TEST START
			Message.add("<SummaryOfMatrix numberOfUsers=\""+ originators.size() + "\" numberOfRows=\"" +
					String.valueOf(getRowCount()) + "\">", Message.TEST);
			Message.add("<SummaryOfMatrix numberOfTasks=\"" + tasks.size() + "\" numberOfColumns=\"" +
