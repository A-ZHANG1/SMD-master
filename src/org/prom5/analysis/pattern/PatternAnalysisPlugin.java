package org.prom5.analysis.pattern;
...
public class PatternAnalysisPlugin extends JPanel implements Provider {
	...
	public void buildMainMenuGui()
	{
		...
		minCommonActivity = new GUIPropertyInteger("Min common activities: ", "", 3,0,100);;
		originatorPanel.add(minCommonActivity.getPropertyPanel());
		
		
		provideSingleInstance = new GUIPropertyBoolean("Provide a single instance", false); 
		originatorPanel.add(provideSingleInstance.getPropertyPanel());
		
		makePatternWithRemainings = new GUIPropertyBoolean("Make pattern with Remainings", false); 
	...
	protected void splitLogs() {
		try {			
			File outputFile = File.createTempFile("PatternAnalysisTemp", ".mxml.gz");
			...
			for (int i=0; i<inputLog.numberOfProcesses(); i++) {
				...
				for (int j=0; j<process.size(); j++) {
					...
					// one new process per process instance (potential previous processes 
					// will be lost)
					persistency.startProcess(name, description, new HashMap<String,String>());
...
//					int remainingCounter = 0;
					...
					boolean bFlag = true;
					persistency.startProcessInstance(name, "Original instance",	new HashMap<String,String>());
					for (int i1 = 0; i1< ateList.size(); i1++) {
						boolean bFind = false;
						try {
							AuditTrailEntry entry = ateList.get(i1);
							if(entry.getTimestamp()==null) continue;
							date = (entry.getTimestamp().getTime()+timeOffset.getValue()*3600000L)/ (long)86400000L;
							int num = 0;
							for (int j1 = i1+1; j1 < ateList.size(); j1++) {
								AuditTrailEntry entry2 = ateList.get(j1);
								long date2 = (entry2.getTimestamp().getTime()+timeOffset.getValue()*3600000L)/ (long)86400000L;
								if(date2==date) {
									num++;
								}
							...
							if(bFind){
								...
								persistency.startProcessInstance(name+"_pattern" + patternCounter++, "instance split up from intance " + name, 
										new HashMap<String,String>());
								for(int k=0;k<num;k++)
								{
									persistency.addAuditTrailEntry(promAte2mxmlibAte(ateList.get(i1+k)));
								}
								persistency.endProcessInstance(); // end last session instance
								i1+=num;
								if(i1<ateList.size()){
									bInstanceFind = true;
								} else {
								...
								if(bInstanceFind) {
									persistency.startProcessInstance(name+"_remaining" + patternCounter++, "instance split up from intance " + name, 
											new HashMap<String,String>());
									bInstanceFind = false;
								}
								persistency.addAuditTrailEntry(promAte2mxmlibAte(ateList.get(i1)));
							}
						} catch (IOException ex) {
							Message.add("Error while filtering instance: "+ex.getMessage(), Message.ERROR);
						} catch (IndexOutOfBoundsException ex) {
							Message.add("Error while filtering instance: "+ex.getMessage(), Message.ERROR);
						}
					}
					if(bFlag) persistency.endProcessInstance(); 
				}
			...
			patternLog = BufferedLogReader.createInstance(new DefaultLogFilter(DefaultLogFilter.INCLUDE), logFile);
		...
		try {			
			...
			for (int i=0; i<inputLog.numberOfProcesses(); i++) {
				...
				for (int j=0; j<process.size(); j++) {
					...
					long date = 0;
					for (int i1 = 0; i1< ateList.size(); i1++) {
						persistency.startProcessInstance(name+"_day"+patternCounter++, "Original instance",	new HashMap<String,String>());
						try {
							AuditTrailEntry entry = ateList.get(i1);
							persistency.addAuditTrailEntry(promAte2mxmlibAte(entry));
							...
							for (int j1 = i1+1; j1 < ateList.size(); j1++) {
								...
								long date2 = (entry2.getTimestamp().getTime()+timeOffset.getValue()*3600000L)/ (long)86400000L;
								if(date2==date) {
									persistency.addAuditTrailEntry(promAte2mxmlibAte(ateList.get(j1)));
									num++;
								} else {
									break;
								}
...
/*	protected void findSequencePattern() {
		...
		patternSet = new HashSet<HashSet<String>>();
					...
					for (int i1 = 0; i1< ateList.size(); i1++) {
						...
						try {
							...
							if(bFind){
								...
								for(int k=0;k<num;k++)
								{
									pattern.add(ateList.get(i1+k).getElement());
								}
								boolean bbFlag = true;
								int k=0;
								for(HashSet<String> aPattern:patternSet)
								{
								...
								if (bbFlag) {
									...
									ArrayList<ArrayList<AuditTrailEntry>> ptr = patternList.get(k);
									ptr.add(list);
								}								
								i1+=num;
							}
	...
	protected void makePatternLog() {
		...
		try {			
			for (int i=0; i<patternList.size(); i++) {
				ArrayList<ArrayList<AuditTrailEntry>> tempList = patternList.get(i);
				File outputFile = File.createTempFile("PatternAnalysisTemp", ".mxml.gz");
				FileOutputStream output = new FileOutputStream(outputFile);
				BufferedOutputStream out = new BufferedOutputStream(new GZIPOutputStream(output));
				LogPersistencyStream persistency = new LogPersistencyStream(out, false);
				...
				for (int j=0; j<tempList.size(); j++) {
					persistency.startProcessInstance("instance_"+j, "pattern instance", new HashMap<String,String>());
				...
				// read back again
				LogFile logFile= LogFile.getInstance(outputFile.getAbsolutePath());
				LogReader tempLog = BufferedLogReader.createInstance(new DefaultLogFilter(DefaultLogFilter.INCLUDE), logFile);
...
//		}		
					...
					for (int i1 = 0; i1< ateList.size(); i1++) {
						...
						try {
							...
							if(bFind){
								...
								if (bbFlag) {
									persistency.addAuditTrailEntry(addPattern("pattern_"+k,ateList.get(i1),"start"));
									persistency.addAuditTrailEntry(addPattern("pattern_"+k,ateList.get(i1+num),"complete"));
								}
								i1+=num;
								if(i1<ateList.size()){
	 ...
	 * @return the mxml log entry
	 */
	protected org.processmining.lib.mxml.AuditTrailEntry addPattern(String patternName, AuditTrailEntry promAte, String eventType) {
		org.processmining.lib.mxml.AuditTrailEntry mxmlibAte = new org.processmining.lib.mxml.AuditTrailEntry();
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		if(patternLogList!=null) {
			ProvidedObject[] objects = new ProvidedObject[2+patternLogList.size()];
			objects[0] = new ProvidedObject("Input Log", new Object[] {inputLog});
			objects[1] = new ProvidedObject("Pattern Log", new Object[] {patternLog});
			for(int k=0;k<patternLogList.size();k++)
			{
				objects[2+k]= new ProvidedObject("Pattern"+k+" ("+patternLogList.get(k).numberOfInstances()+")", new Object[] {patternLogList.get(k)});
			}
			return objects;
		}
		else if (patternLog != null) {
			ProvidedObject[] objects = {
					new ProvidedObject("Input Log", 
							new Object[] {inputLog}),
