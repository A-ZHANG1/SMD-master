...
package org.prom5.analysis.pdm;
...
import org.prom5.framework.models.pdm.PDMDesign;
import org.prom5.framework.models.pdm.PDMModel;
import org.prom5.framework.models.pdm.algorithms.PDMDesignReader;
...
public class PDMAnalysisUI extends JPanel implements Provider {
	...
	private JButton calculateCouplingCohesionButton = new JButton("Calculate Coupling and Cohesion");
	private JButton makePetriNetButton = new JButton("Make PetriNet");

	private JPanel textContainer; // Holds the panel with CoCoMo analysis results.
...
//					buttonsPanel.add(makePetriNetButton);
		...
		calculateCouplingCohesionButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String str2= "The Coupling/Cohesion ratio is:  " + ratio.toString();
				textContainer.add(new JLabel(str2));

				String str1= "The Process Coupling is:         " + cpl.toString();
				textContainer.add(new JLabel(str1));

				String str = "The Process Cohesion is:         " + coh.toString();
...
/*		makePetriNetButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				PDMDesignToPNPlugin plugin = new PDMDesignToPNPlugin();
				plugin.convert(design,model);
			}
		});
*/
...
//		analysisPanel.add(Box.createRigidArea(new Dimension(5,0)));
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		if (design != null) {
			objects.add(new ProvidedObject("PDM design", new Object[] {design}));
		}
	...
	protected void printTestOutput() {
		...
		Message.add("<Number of activities = " + design.getActivities().size() + " >", Message.TEST);
		Message.add("<Coupling Cohesion Ratio = " + design.calculateProcessRatio() + " >", Message.TEST);
		Message.add("</PDMAnalysis>", Message.TEST);
	}
