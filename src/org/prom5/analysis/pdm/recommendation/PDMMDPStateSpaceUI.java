...
public class PDMMDPStateSpaceUI extends JPanel implements Provider{
	...
	private JButton calculateStateSpaceButton = new JButton("Calculate statespace");
	private JButton calculateStrategyButton = new JButton("Calculate MDP strategy");

	private JPanel pdmPanel; // Holds the panel with the PDM model and the actions to be taken.
	private JScrollPane pdmContainer; // Holds the PDM.
	private JPanel actionContainer; // Holds the checkboxes and buttons for the actions that can be taken.
	...
	private JCheckBox proceedAfterRootElement = new JCheckBox("Proceed after root element", false);
	private JCheckBox failurePossible = new JCheckBox("Failure of operations possible", true);
	private JCheckBox allInputEltsAvailable = new JCheckBox("All input elements available", false);
	private JCheckBox coloredStatespace = new JCheckBox("Display colours" ,true);
	private Choice optimizationChoice = new Choice();
	private JFormattedTextField numberOfStates = new JFormattedTextField(new Integer("1000"));
	private JFormattedTextField breadthOfTree = new JFormattedTextField(new Integer("50"));
	...
	void jbInit() throws Exception {
	    ...
	    calculateStateSpaceButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				statespace = model.calculateSimpleStateSpace(proceed_after_root, failure_possible, all_inputs_available, colored_statespace, nStates, breadth);
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		if (model != null) {
			objects.add(new ProvidedObject("PDM model", new Object[] {model}));
		}
		if (statespace != null) {
			objects.add(new ProvidedObject("PDM MDP Statespace", new Object[] {statespace}));
		}
		if (strategy != null) {
			objects.add(new ProvidedObject("PDM MDP Strategy", new Object[] {strategy}));
		}
