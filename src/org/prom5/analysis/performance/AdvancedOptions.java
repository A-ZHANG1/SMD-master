...
public class AdvancedOptions extends JPanel implements GuiNotificationTarget {
	...
	private GUIPropertyBoolean isRestricted = new GUIPropertyBoolean("Restrict search depth for invisible tasks", 
			"Restricts search for sequences of invisible tasks that might enable another task during log replay", true, this);
	private GUIPropertyInteger restrictedDepth = new GUIPropertyInteger("Maximum depth: ", 
			"Corresponds to the maximum length of a sequence of invisible tasks that can be found", 1, 0, 100);
	//various labels
	private JLabel advancedLabel = new JLabel ("Advanced Settings");
	private JLabel conformanceLabel = new JLabel("<html>Values of various performance"
	...
	private JLabel transitionsLabel = new JLabel("Times in between two transitions"
									+ " are based on:");
	 ...
	 * @param settings int[]: the initial settings
	 * @param pc PerformanceConfiguration: the Performance Configuration object
	...
	private void jbInit() {
		...
		con.gridy = 0; con.gridx = 0; con.insets = new Insets(0, 0, 5, 0);
		con.anchor = GridBagConstraints.NORTHWEST;
		advancedPanel.add(advancedLabel, con);
		con.gridy = 1; con.gridx = 0; con.gridwidth = 3;
		...
		con.gridwidth = 3; con.insets = new Insets(0, 0, 5, 0);
		...
		con.gridy = 4; con.gridx = 0; con.insets = new Insets(5, 0, 0, 0);
		advancedPanel.add(placeLabel, con);
		con.gridy = 5; con.gridx = 0; con.insets = new Insets(0, 0, 5, 0);
		...
		placeBox.addItem("All measurements taken in each trace before the trace fails");
		placeBox.addItem("All measurements taken in each trace where no related transition has failed execution");
		...
		con.gridy = 6; con.gridx = 0; con.insets = new Insets(5, 0, 0, 0);
		advancedPanel.add(transitionsLabel, con);
		con.gridy = 7; con.gridx = 0; con.insets = new Insets(0, 0, 5, 0);
		...
		transitionsBox.addItem("All measurements taken in each trace before the trace fails");
		transitionsBox.addItem("All measurements taken in traces where both transitions do not fail");
		...
		con.gridy = 8; con.gridx = 0; con.insets = new Insets(5, 0, 0, 0);
		advancedPanel.add(activityLabel, con);
		con.gridy = 9; con.gridx = 0; con.insets = new Insets(0, 0, 5, 0);
		...
		activityBox.addItem("All measurements taken in each trace before the trace fails");
		activityBox.addItem("All measurements taken in traces where no transition corresponding to the activity fails");
		activityBox.addItem("All measurements taken where no transition corresponding to a metric of the activity fails");
		...
		con.gridy = 10; con.gridx = 0; con.gridwidth = 3;
		advancedPanel.add(saveTimeLabel0, con);
		con.gridy = 11; con.gridx = 0;
		advancedPanel.add(saveTimeLabel1, con);
		con.gridy = 12; con.gridx = 0;
		advancedPanel.add(saveTimeLabel2, con);
		con.gridy = 13; con.gridx = 0;
		advancedPanel.add(saveTimeLabel3, con);
		con.gridy = 14; con.gridx = 0;
		advancedPanel.add(saveTimeLabel4, con);
		
		/* NOTE: modified by Anne in order to support restricted search depth for invisible tasks 
		 also from the Performance Analysis plug-in. Now the binary choice of time saving option or not
		 translates into an integer as follows:
		 x < 0 --> the whole state space will be built (i.e., no time saving option)
		 x = 0 --> no state space will be built at all (i.e., time saving option as it used to be)
		 x > 0 --> improved time saving option building a partial state space in order to still detect 
		 sequences of invisible tasks of the length x (and therefore increase fitness of log and model). 
		  */
		if (advancedSettings[4] >= 0) {
			restrictedDepth = new GUIPropertyInteger("Maximum depth: ", 
					"Corresponds to the maximum length of a sequence of invisible tasks that can be found", 
					advancedSettings[4], 0, 100);
		} else {
			isRestricted = new GUIPropertyBoolean("Restrict search depth for invisible tasks", 
					"Restricts search for sequences of invisible tasks that might enable another task during log replay", false, this);
			restrictedDepth = new GUIPropertyInteger("Maximum depth: ", 
					"Corresponds to the maximum length of a sequence of invisible tasks that can be found", 
					1, 0, 100);
			// TODO Bugfix Anne: does not work because property object not built yet (result of memory problems 
			// in CPN export - look into further possibilities to solve this issue)
			// restrictedDepth.disable();
		}
		
		con.gridy = 15; con.gridx = 0; con.gridwidth = 1;
		advancedPanel.add(isRestricted.getPropertyPanel(), con);
		con.gridy = 15; con.gridx = 2; con.gridwidth = 1;
		con.anchor = GridBagConstraints.NORTHEAST;
		advancedPanel.add(restrictedDepth.getPropertyPanel(), con);
		
		// now that the property panels are created, we can disable the enumeration property if necessary
		...
		con.gridy = 16; con.gridx = 0; con.insets = new Insets(0, 0, 0, 0);
		con.anchor = GridBagConstraints.NORTHWEST;		
		advancedPanel.add(cancelButton, con);
		con.gridy = 16; con.gridx = 2; con.gridwidth = 1;
		con.anchor = GridBagConstraints.NORTHEAST;
	...
	private void registerGuiActionListener() {
		applyButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				/* NOTE modified by Anne in order to support restricted search depth (see above) */
