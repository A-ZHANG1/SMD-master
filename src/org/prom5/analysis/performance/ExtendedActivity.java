...
public class ExtendedActivity {
	...
	private DescriptiveStatistics waitStatistics = DescriptiveStatistics.newInstance();
	//Used to keep track of execution time measurements for this activity and for
	//calculation of statistical values(eg mean, stdev)
	private DescriptiveStatistics executionStatistics = DescriptiveStatistics.newInstance();
	//Used to keep track of sojourn time measurements for this activity and for
	//calculation of statistical values(eg mean, stdev)
	private DescriptiveStatistics sojournStatistics = DescriptiveStatistics.newInstance();
	//Used to keep track of upper bound sojourn time measurements for this activity and for
	//calculation of statistical values(eg mean, stdev). Here times can be stored
	//when a log does not contain any complete events
	private DescriptiveStatistics upperBoundSojournStatistics = DescriptiveStatistics.newInstance();
	//Used to keep track of upper bound waiting time measurements for this activity and for
	//calculation of statistical values(eg mean, stdev). Here times can be stored
	//when a log does not contain any complete events
	private DescriptiveStatistics upperBoundWaitStatistics = DescriptiveStatistics.newInstance();
	//Used to keep track of upper bound execution time measurements for this activity and for
	//calculation of statistical values(eg mean, stdev). Here times can be stored
	//when a log does not contain any complete events
	private DescriptiveStatistics upperBoundExecutionStatistics = DescriptiveStatistics.newInstance();
	//Used for calculation of inter arrival times for this activity
	private SummaryStatistics interArrivalStatistics = SummaryStatistics.newInstance();
	 ...
	 * @param aName String: name of the activity
	 */
	 ...
	 * fires.
	 *
	 * @param piName String: process instance ID in which the measurement was
	 * taken
	 * @param timestamp Date: measured timestamp
	 * @param transEnabled Date: timestamp at which the transition that calls this
	 * method was enabled
	 * @param type String: event type that occurred
	 * @param transitionFailed String: contains the event-type of the transition
	 * if it did not fire regularly
	 * @param failedBefore boolean: true in case a transition in the trace with
	 * name piName failed, before this method was called
	 *
	 */
	public void measure(String piName, Date timestamp, Date transEnabled,
		...
		if (measurements.size() == 0) {
			//no measurements taken for this activity yet, so create the first one
			ActivityMeasurement meas = new ActivityMeasurement(piName, transEnabled);
			meas.setDate(timestamp, type, transitionFailed, failedBefore);
			//add the new measurements to the measurements ArrayList
			measurements.add(meas);
		} else {
			//There already exist measurements for this activity, obtain the last measurement
			ActivityMeasurement lastMeasurement = (ActivityMeasurement)
												  measurements.get(measurements.size() - 1);
			if (!piName.equals(lastMeasurement.piName)) {
				//this trace is another one than the last, so measurement should be a new one
				ActivityMeasurement meas = new ActivityMeasurement(piName, transEnabled);
				meas.setDate(timestamp, type, transitionFailed, failedBefore);
				//add measurement to the measurements-list
				measurements.add(meas);
			} else {
				if (lastMeasurement.completeDate != null || lastMeasurement.abortDate != null) {
				    //last measurement was completed or aborted, so create a new measurement
					ActivityMeasurement meas = new ActivityMeasurement(piName, transEnabled);
					meas.setDate(timestamp, type, transitionFailed, failedBefore);
					...
					if (type.equalsIgnoreCase("schedule")) {
						...
						if (lastMeasurement.assignDate == null) {
							lastMeasurement.setDate(timestamp, type, transitionFailed, failedBefore);
						} else {
	 ...
	 * last measurement.
	 * Furthermore, if a transition in the trace piName failed execution before
	 * the transition after the last transition executed, then "end" is added
	 * to failedBeforeTransitions of the last measurement.
	 *
	 * @param piName String: Name of process instance
	 * @param timeStamp Date: Date to which the end date of the last measurement
	 * is compared and possibly set (if before the current end date)
	 * @param failedBeforeEnd boolean: true if the trace piName failed before
	 * this method was called
	 * @param failedEnd boolean: true if the transition after the last transition
	...
	public void setFinalTimeLast(String piName, Date timeStamp,
		...
		if (measurements.size() > 0) {
			//only add if at least one measurement corresponding to this activity exists
			ActivityMeasurement lastMeasurement = (ActivityMeasurement)
												  measurements.get(measurements.size() - 1);
			if (piName.equals(lastMeasurement.piName)) {
				if (lastMeasurement.endDate == null || !lastMeasurement.endDate.before(timeStamp)) {
	 ...
	 * In case information is incomplete (i.e. not schedule-start-complete present)
	 * then upper bound for these time-metrics are derived, if possible and placed
	 * in upperBoundWaitStatistics, upperBoundExecutionStatistics and
	 ...
	 * time
	 *
	 * @param piList ArrayList: Names of the process instances that are  considered
	 * @param fitOption int: selected fitness/conformance option (calculation
	 * of activity metrics is dependent on how we deal with non-conformance)
	 * @param failedInstances HashSet: set of traces that executed executed unsuccessfully or
	 * terminated unproperly
	 */
	public void calculateMetrics(ArrayList piList, int fitOption, HashSet failedInstances) {
		String[] names = (String[]) piList.toArray(new String[0]);
		Arrays.sort(names);
		//clear all previously recorded statistics
		waitStatistics.clear(); executionStatistics.clear();
		sojournStatistics.clear(); upperBoundSojournStatistics.clear();
		upperBoundWaitStatistics.clear(); upperBoundExecutionStatistics.clear();
		...
		while (lit.hasNext()) {
			...
			if (fitOption == 1 && (am.failedBeforeTransitions.contains("schedule")
					|| am.failedBeforeTransitions.contains("assign")
					|| am.failedBeforeTransitions.contains("reassign")
					|| am.failedBeforeTransitions.contains("start"))) {
				//the log trace failed before a measurement for this activity was taken
				continue;
			} else if (fitOption == 1) {
				if (am.enabledDate != null) {
	    			//add arrival date
			...
			if (fitOption == 2 && am.failedTransitions.size() > 0 &&
					!(am.failedTransitions.size() == 1 &&
						am.failedTransitions.contains("end")
						&& am.completeDate != null)) {
						//if the set of failed transitions is larger than 0 and does not
						//only contain 'end'
				continue;
			} else if (fitOption == 2) {
				if (am.enabledDate != null) {
			...
			if (fitOption == 3 && am.failedTransitions.contains("start")) {
				//in case a start-transition fails, all time-metrics are useless
				continue;
			} else if (fitOption == 3) {
				if (am.enabledDate != null) {
					//add arrival date
			...
			if ((am.scheduleDate != null) && (am.startDate != null)
					&& (am.startDate.getTime() - am.scheduleDate.getTime() >= 0)) {
				if (!(fitOption == 3 && am.failedTransitions != null
						&& (am.failedTransitions.contains("schedule")
						|| am.failedTransitions.contains("assign")
						|| am.failedTransitions.contains("reassign")))) {
					//add waiting time
					waitStatistics.addValue(am.startDate.getTime() - am.scheduleDate.getTime());
				}
			}
			if (fitOption == 1 && (am.failedBeforeTransitions.contains("complete")
					|| am.failedBeforeTransitions.contains("suspend")
					|| am.failedBeforeTransitions.contains("resume"))) {
				//the log trace failed before an execution or sojourn measurement for this activity was taken
				continue;
			}
			if (fitOption == 3 && am.failedTransitions != null
					&& (am.failedTransitions.contains("complete")
					    || am.failedTransitions.contains("suspend")
						|| am.failedTransitions.contains("resume"))) {
				//in case a complete, suspend or resume-transition fails, only waiting times can be used
				continue;
			}
			if ((am.completeDate != null) && (am.enabledDate != null)//((am.scheduleDate != null) || )
				...
				if (am.scheduleDate != null) {
					...
					sojournStatistics.addValue(am.completeDate.getTime() - am.scheduleDate.getTime());
				} else {
					//add sojourn time (as time between enabling and complete)
					sojournStatistics.addValue(am.completeDate.getTime() - am.enabledDate.getTime());
			...
			if (am.endDate != null) {
				if (!am.discardFinal && am.abortDate==null) {
					if ((am.scheduleDate != null)
							...
							&&
							!(((fitOption == 3 || fitOption == 2) &&
							(am.failedTransitions.contains("end")
							|| am.failedTransitions.contains("schedule")))
							|| (fitOption == 1 && am.failedBeforeTransitions.contains("end")))) {
						//add upper bound of sojourn time
						upperBoundSojournStatistics.addValue(am.endDate.getTime()
			...
			if ((am.startDate != null) && (am.completeDate != null)
			...
			if (am.startDate != null) {
				if (am.enabledDate != null && (am.startDate.getTime() - am.enabledDate.getTime() >= 0)) {
					//add upper bound of waiting time
					upperBoundWaitStatistics.addValue(am.startDate.getTime() - am.enabledDate.getTime());
				}
				if (am.endDate != null && (am.endDate.getTime() - am.startDate.getTime() >= 0)) {
					if (!(((fitOption == 3 || fitOption == 2)
						    && !am.discardFinal && am.abortDate==null
							&& (am.failedTransitions.contains("end")))
							|| (fitOption == 1 && am.failedBeforeTransitions.contains("end")))) {
						//add upper bound of execution time
						upperBoundExecutionStatistics.addValue(am.endDate.getTime() -
							am.startDate.getTime() - am.suspendedTime);
					}
				}
			}
		...
		if (arrivalDates.size() > 0) {
			...
			for (int i = 1; i < dates.length; i++) {
				interArrivalStatistics.addValue(dates[i].getTime() - dates[i-1].getTime());
			}
		}
	}
	...
	/**
	 * Exports all time measurements (waiting, execution, sojourn time)
	 * taken in process instances in piList and corresponding to this activity
	 * to (a comma-seperated text) file.
	 *
	 * @param piList ArrayList
	 * @param file File
	 * @param divider long
	 * @param sort String
	 * @param fitOption int
	...
	public void exportToFile(ArrayList piList, File file, long divider,
		...
		String line = "Log trace, Waiting time (" + sort + "), Execution time ("
					  + sort + "),Sojourn time (" + sort + ")\n";
		output.write(line);
		ListIterator lit = measurements.listIterator();
		while (lit.hasNext()) {
			ActivityMeasurement am = (ActivityMeasurement) lit.next();
			if (Arrays.binarySearch(names, am.piName) <= -1) {
				//no process instance in piList that corresponds to am.piName
				//or the current measurement should be ignored for other reasons
				continue;
			}
			if (fitOption == 1 && (am.failedBeforeTransitions.contains("schedule")
					|| am.failedBeforeTransitions.contains("assign")
			...
			if ((am.scheduleDate != null) && (am.startDate != null)
				...
				if (!(fitOption == 3 && am.failedTransitions != null
						...
						|| am.failedTransitions.contains("reassign")))) {
					//add waiting time
					line += ((am.startDate.getTime() - am.scheduleDate.getTime())
						* 1.0/ divider) + ",";
	    		} else {
					line += "-,";
				}
				    ...
				    line += ((am.startDate.getTime() - am.enabledDate.getTime())
							* 1.0/ divider) + ",";
			} else {
				line += "-,";
			}
			...
			if ((am.startDate != null) && (am.completeDate != null)
					&& (am.completeDate.getTime() - am.startDate.getTime() >= 0)) {
				//add execution time
				line += ((am.completeDate.getTime() - am.startDate.getTime()
						- am.suspendedTime) * 1.0 / divider) + ",";
			} else if (am.endDate != null && am.startDate != null
					&& (am.endDate.getTime() - am.startDate.getTime() >= 0)) {
				if (!(((fitOption == 3 || fitOption == 2)
						&& (am.failedTransitions.contains("end") ))
						|| (fitOption == 1 && am.failedBeforeTransitions.contains("end")))) {
					line += ((am.endDate.getTime() - am.startDate.getTime()
						- am.suspendedTime) * 1.0 / divider) + ",";
				} else {
					line += "-,";
				}
			...
			if ((am.completeDate != null) && (am.enabledDate != null)//((am.scheduleDate != null) || )
				...
				if (am.scheduleDate != null) {
					//add sojourn time (as time between schedule and complete)
					line += ((am.completeDate.getTime() - am.scheduleDate.getTime())
						* 1.0 / divider) + "\n";

				} else {
					//add sojourn time (as time between enabling and complete)
					//add sojourn time
					line += ((am.completeDate.getTime() - am.enabledDate.getTime())
							* 1.0 / divider) + "\n";
				}
				...
				if (!(((fitOption == 3 || fitOption == 2) &&
						(am.failedTransitions.contains("end")
						 || am.failedTransitions.contains("schedule")))
    					 || (fitOption == 1 && am.failedBeforeTransitions.contains("end")))) {
					 line += ((am.endDate.getTime() - am.scheduleDate.getTime())
							* 1.0 / divider) + "\n";
	...
	/**
	 * Checks whether normal time measurements can be used (sojourn time, waiting
	 * time, execution). If not, upper bounds of these time measurements are used
	 * instead (if possible).
	 */
	public void checkWhichMetricsToUse() {
		if (sojournStatistics.getN()==0 && upperBoundSojournStatistics.getN()>0) {
		...
		if (executionStatistics.getN()==0 && upperBoundExecutionStatistics.getN()>0) {
	 ...
	 * of an array of type double. Where:
	 * [0..2]: fast-slow-normal
	 *
	 * @param fastestPercentage double: percentage of 'fast' times
	 * @param slowestPercentage double: percentage of 'slow' times
	 * @return double[]
	 */
	public double[] getAvgWaitTimes(double fastestPercentage, double slowestPercentage) {
		return getAvgTimes(fastestPercentage, slowestPercentage,
				waitStatistics.getSortedValues());
	}

	/**
	 * Returns the mean execution time of this activity. Note
	 * that the method calculateTimes should be called before this method.
	 * @return double
	 */
	...
	/**
	 * Returns the average of fast, slow and normal speed execution times in form
	 * of an array of type double. Where:
	 * [0..2]: fast-slow-normal
	 *
	 * @param fastestPercentage double: percentage of 'fast' times
	...
	public double getArrivalRate() {
		double rate = 0;
		if (interArrivalStatistics.getN() > 0 && interArrivalStatistics.getMean() != 0
				&& (waitStatistics.getN() > 0 || executionStatistics.getN() > 0 ||
				sojournStatistics.getN() >0)) {
	...
	/**
	 * Returns the average of fast, slow and normal speed times of the times that
	 * occur in the inputArray. Results are returned in form of an array. Where:
	 * [0..2]: fast-slow-normal
	 *
	 * @param fastestPercentage double
	 * @param slowestPercentage double
	 * @param inputArray double[]
	 * @return double[]
	 */
	private double[] getAvgTimes(double fastestPercentage, double slowestPercentage,
			double[] inputArray) {
		long total = 0;
		int[] sizes = getSizes(fastestPercentage, slowestPercentage, inputArray.length);
		int fastSize = sizes[0],
			slowSize = sizes[1],
			middleSize = sizes[2];
	...
	/**
	 * Returns an array containing the number of measurements that are considered
	 * to be fast (place 0 in array), the number of measurements that are
	 * considered to be slow (place 1 in array) and the number of measurements
	 * that are of 'normal' speed (place 2 in array). Based on fastestPercentage,
	 * slowestPercentage and the given length.
	 *
	 * @param fastestPercentage double
	 * @param slowestPercentage double
	 * @param length int
	...
	static class ActivityMeasurement implements Cloneable {
		...
		//Date at which a transition belonging to this activity was first enabled
		public Date enabledDate;
		//Date the activity was scheduled
		public Date scheduleDate;
		//Date the activity was assigned to someone/something (not used in calculations)
		 ...
		 * @param piName String: the processID for which this measurement has been taken
		 * @param firstEnabledDate Date: the Date at which the activity was
		 * first enabled
		 */
		 ...
		 *
		 * @param timestamp Date to be set
		 * @param type String Event-type
		 * @param failedTrans String
		 * @param failedBefore boolean
		 */
		public void setDate(Date timestamp, String type, String failedTrans,
			...
			if (c.equals(type, "schedule")) {
				...
				if (suspendDate != null) {
					suspendedTime += timestamp.getTime() - suspendDate.getTime();
				}
			} else if (c.equals(type, "complete")) {

				completeDate = timestamp;
			} else if (c.equals(type, "abort") || c.equals(type, "withdraw") || c.equals(type, "skip")) {
				//@todo: not sure, if these are the right event-types, adjust if needed.
				abortDate = timestamp;
			}
			if (failedTrans != null) {
