...
import org.prom5.framework.models.petrinet.algorithms.logReplay.ReplayedPetriNet;
...
public class ExtendedPetriNet extends ReplayedPetriNet {
    ...
    /**
	 * The currentlySelectedInstances attribute is necessary because
	 * the writeToDot() method has a fixed interface,
	 * though the visualization should be able to take them into account.
	 */
	 ...
	 * that want to store diagnostic information
	 */
	public ExtendedPetriNet(PetriNet net, ArrayList caseIDs) {
		...
		while (lit.hasNext()) {
			...
			// TODO: Peter, please check whether this is in fact what you want to do? 
	...
	/**
	 * Copy Constructor mimics the clone method and will be called by sub classes
	 * when the visualization state has changed in PerformanceAnalysisResult.
	 * Note that a deep copy with respect to the Petri net elements is created, i.e.,
	 * the places, transitions and edges will be different objects afterwards.
	 * Nevertheless, the list of selected instances and the associated extended log
	 * are reassigned and do not get cloned.
	...
	/**
	 * {@inheritDoc}
	 * Produces an {@link ExtendedTransition ExtendedTransition}.
	 */
	...
	/**
	 * {@inheritDoc}
	 * Produces an {@link ExtendedPlace ExtendedPlace}.
	 */
	...
	/**
	 * {@inheritDoc}
	 * Produces an {@link ExtendedPNEdge ExtendedPNEdge}.
	 */
	...
	/**
	 * {@inheritDoc}
	 * Produces a {@link ExtendedPNEdge ExtendedPNEdge}.
	 */
			...
			while (it.hasNext()) {
				...
				if (e.isPT()) {
					...
					if (p.getOutEdges().size() > 1) {
						...
						//outgoing edge, display probability in 2 decimal digits
						double prob = e.getProbability(currentlySelectedInstances,
