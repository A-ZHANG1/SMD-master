...
public class ExtendedPlace extends ReplayedPlace {
	...
	private DescriptiveStatistics syncStatistics = DescriptiveStatistics.newInstance();
	private DescriptiveStatistics sojStatistics = DescriptiveStatistics.newInstance();
	//SummaryStatistics to obtain mean inter-arrival time
	private SummaryStatistics arrivalStatistics = SummaryStatistics.newInstance();
	//Keeps track of whether the user has set waiting time settings specifically
	 ...
	 * @param p The place (to be passed to super class).
	 * @param net The Petri net it belongs to (to be passed to super class).
	 * @param caseIDs The list of diagnostic log traces that want to be able to
	 * store results of the conformance analysis in this diagnostic place.
	 */
	 ...
	 * place and stores it.
	 * @param tokenAvailable Date the Date at which a token was first available in this place
	 * @param transEnabled Date the Date at which the transition to fire was enabled
	 * @param transFired Date the Date at which the transition fired
	 * @param trace ReplayedLogTrace the process instance in which this measurement
	 * was taken
	 */
	public void recordTimeMeasurement(Date tokenAvailable, Date transEnabled,
		...
		if (!lt.hasProperlyTerminated() || !lt.hasSuccessfullyExecuted()) {
			//the log trace has failed execution before this measurement was taken
			tm = new TimeMeasurement(trace.getName(), wait, sync, tokenAvailable, true);
		} else {
			//measurement is not disturbed by a non-fitting log trace
			tm = new TimeMeasurement(trace.getName(), wait, sync, tokenAvailable, false);
		}
	...
	/**
	 * takes the waiting time, synchronization time and the sojourn time of
	 * each timemeasurement and puts them in the sorted arrays waitArray,
	 * synchArray and sojArray respectively.
	 * @param piList ArrayList: the process instance on which the metrics
	 * @param fitOption int: the used fit option (how to deal with conformance)
	...
	public void calculateMetrics (ArrayList piList, int fitOption, HashSet failedInstances) {
	    waitStatistics.clear(); syncStatistics.clear(); sojStatistics.clear();
		...
		while (it.hasNext()) {
			...
			if (fitOption == 2) {
				...
				ListIterator transitions = getRelatedTransitions().listIterator();
				while (transitions.hasNext() && isUnaffected) {
					ExtendedTransition trans = (ExtendedTransition) transitions.next();
					if (trans.hasFailedExecution(tm.piName)) {
						isUnaffected = false;
					}
				}
				//add the times to the ArrayLists in case the trace has completed
	 ...
	 *
	 * @param piList ArrayList: the process instances
	 * @param file File: the file to which the measurements are exported
	 * @param divider long: the used time divider
	 * @param sort String: the used time sort
	 * @param fitOption int: the fit option used (how to deal with non-conformance)
	 * @throws IOException : can occur when connecting to file
	 */
	...
	public void exportToFile(ArrayList piList, File file, long divider,
		...
		String line = "Log Trace,Waiting time (" + sort + "),Synchronization time ("
	 ...
	 * @param trans ReplayedTransition: the transition that last put a token
	 * in this place
	 */
	 ...
	 *
	 * @param endTime Date: the timestamp
	 * @param trace ReplayedLogTrace: the trace that is currently being replayed
	 * @param rt ReplayedTransition: the transition that called this method
	 */
	public void removeEnablingTransition(Date endTime, ReplayedLogTrace trace, ReplayedTransition rt) {
		...
		if (lit.hasNext()) {
			...
			if (enablingTransition != null && enablingTransition.getAssociatedActivity() != null) {
				boolean failedBeforeSecond = false, secondFailed = false;
				//check whether the transition has failed in this process instance
				...
				try {
					if (!rt.getLogEvent().getModelElementName().equalsIgnoreCase(
							enablingTransition.getLogEvent().getModelElementName())) {
						// the endTime of the activity corresponding to the enablingTransition is only
						// set when rt and enablingTransition correspond to different activities
						enablingTransition.getAssociatedActivity().setFinalTimeLast(
								piName, endTime, failedBeforeSecond, secondFailed);
						if (enablingTransitions.size()>1) {
							//discard bound value for final time of enabling transition if multiple tokens in input place of rt
							enablingTransition.getAssociatedActivity().setDiscardBoundFinal();
						}
						while (lit.hasNext()) {
						    //discard bound value for all transitions if multiple tokens in input place of rt
							ExtendedTransition trans = (ExtendedTransition) lit.next();
							trans.getAssociatedActivity().setDiscardBoundFinal();
						}
	 ...
	 * @param bnds ArrayList: the bounds of the waiting time levels for this
	 *                        place.
	 * @param cols ArrayList: the colors of the waiting time levels for this
	 *                        place
	 */
	 ...
	 * times in waitStatistics. Make sure method calculateMetrics is called before
	 * this one.
	 * @return double
	 */
	 ...
	 * times in array waitStatistics (make sure method calculateMetrics is called
	 * before this one)
	 * @return double
	 */
	...
	/**
	 * Returns the mean waiting time of this place, calculated based on the
	 * times in syncStatistics. Make sure method calculateMetrics is called before
	 * this one.
	 *
	 * @return double
	 */
	 ...
	 * synchronization times in syncStatistics, make sure calculateMetrics is called
	 * before this method
	 *
	 * @return double
	 */
	 ...
	 * based on the waiting times in syncStatistics (make sure calculateMetrics is
	 * called before this method) and meanTime.
	 *
	 * @return double
	 */
	 ...
	 * sojourn times as present in sojStatistics (make sure method calculateMetrics
	...
	/**
	 * Returns the minimum sojourn time of this place, based on the
	 * sojourn times as present in sojStatistics (make sure method calculateMetrics
	 * is called before this method.
	 *
	 * @return double
	...
	/**
	 * Returns the maximum sojourn time of this place, based on the
	 * sojourn times as present in sojStatistics (make sure method calculateMetrics
	 * is called before this method.
	 *
	 * @return double
	...
	/**
	 * Returns the average of fast, slow and normal speed waiting/synch/soj times
	 * Where [0..2]: fast wait-sync-soj
	 *       [3..5]: slow wait-sync-soj
	 *       [6..8]: middle wait-sync-soj
	 *
	...
	public double[] getAvgTimes(double fastestPercentage,
		...
		waitTotal = 0; syncTotal = 0; sojTotal = 0;
	 ...
	 * slowestPercentage and waitArray (thus method calculateMetrics()
	 ...
	 *
	 * @param piList ArrayList: the process instances
	 * @param fitOption int: the fit Option used (how to deal with conformance)
	 * @param failedInstances HashSet: set of traces that executed executed unsuccessfully or
	 * terminated unproperly
	 * @return long
	 */
	public long getTotalOutEdgeFrequency(ArrayList piList, int fitOption, HashSet failedInstances) {
		String[] names = (String[]) piList.toArray(new String[0]);
		Arrays.sort(names);
	...
	/**
	 * Data structure consisting of some performance-related measurements
	 * for this place.
	 * <br>
	 * Suppose there is a transition with two input places p1 and p2, which has
	 * a firing time stamp t. The tokens in those input places also have a timestamp tj
	 * (which is derived from the transition that had produced the token): t1 and t2,
	 * respectively. Then we can define tm as the greatest timestamp of the
	...
	static class TimeMeasurement implements Cloneable {
		...
		/**
		 * The synchronization time is the time the token spends in a place until
		 * the transition becomes enabled (i.e., tm - tj).
		 */
		 ...
		 * @param piName the processID for which this measurement has been taken
		 * @param waitTime the waiting time is the time a token spends time in a
		 * place although the transition is already enabled (i.e., t - tm)
		 * @param syncTime the synchronization time is the time the token spends
		 * in a place until the transition becomes enabled (i.e., tm - tj)
		 * @param tokenAvailable the date of arrival of a token
		 * @param failed boolean
		 */
		public TimeMeasurement(String piName, long waitTime, long syncTime, Date
				tokenAvailable, boolean failed) {
			this.piName = piName;
		 ...
		 * @return a string containing the processID, the synchronization time, and then the waiting time
		 */
