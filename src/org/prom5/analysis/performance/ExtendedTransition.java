...
public class ExtendedTransition extends ReplayedTransition {
	...
	/**
	 * Map used to store the timestamp at which the transition first fired during
	 * execution of each process instance. The process name is used as search
	 * key.
	 */
	private HashMap firstFireTimes = new HashMap();
	/**
	 * Set used to store the process instances that failed execution, before this
	 * transition first fired in those instances
	 */
	private HashSet failedBefore = new HashSet();
	/**
	 * Set used for remembering those cases in which the transition was
	 ...
	 * yet exist for the current process instance.
	 * Furthermore it passes the timestamp to the ExtendedLogTrace for throughput
	 * time measurements.
	 *
	 * @param extendedTrace ReplayedLogTrace: process instance for which this
	 * measurement is done
	 * @param timestamp Date: timestamp added
	 */
	protected void takeTimeBetweenMeasurement(ReplayedLogTrace extendedTrace,
		...
		if (firstFireTimes.get(piName)==null) {
			//record the first fire time of this transition for this proces instance
	 ...
	 * sojourn time)
	 *
	 * @param trace ReplayedLogTrace: the trace that is replayed
	 * @param timestamp Date: the date at which the transition fired
	 * @param transEnabled Date: the date at which the transition was enabled
	 */
	protected void takeActivityMeasurement(ReplayedLogTrace trace, Date timestamp,
		...
		if (associatedActivity != null) {
			...
			if (!hasFailedExecution(trace.getName())) {
				...
				if (!elt.hasProperlyTerminated() || !elt.hasSuccessfullyExecuted()) {
					//the log trace has failed execution before this measurement was taken

					associatedActivity.measure(trace.getName(), timestamp, transEnabled,
							this.getLogEvent().getEventType().toLowerCase(), null, true);
				} else {
					//the log trace has not failed execution before this measurement was taken
					associatedActivity.measure(trace.getName(), timestamp, transEnabled,
					this.getLogEvent().getEventType().toLowerCase(), null, false);
				}
			} else {
				//this transition has failed execution in this trace
					associatedActivity.measure(trace.getName(), timestamp, transEnabled,
							this.getLogEvent().getEventType().toLowerCase(),
							this.getLogEvent().getEventType().toLowerCase(), true);
			}
		}
	}
	 ...
	 * @param pi The trace to be updated.
	 ...
	 * piName
	 *
	 * @param piName String: the name of the log trace
	 * @return boolean
	 */
	 ...
	 * @param activity PerformanceActivity: the activity with which this transition
	 * is to be associated
	 */
	 ...
	 * instance piName
	 * @param piName String: the name of the process instance
	 * @return Date
	 */
