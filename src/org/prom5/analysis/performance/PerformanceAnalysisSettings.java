 ...
 * analysis. The user can define performance configuration settings here, such as
 * used timesort and bounds and colors of waiting time levels.
 *
...
public class PerformanceAnalysisSettings extends JPanel{
	...
	private JPanel buttonsPanel = new JPanel(new BorderLayout()); // lower panel containing the buttons
	private JPanel infoButtonsPanel = new JPanel(); // panel containing the log summary and the documentation button
	private JButton startButton = new JButton(new PerformanceCheckAction(this));
	private JButton docsButton = new JButton("Plugin documentation..."); // shows the plugin documentation
	...
	/**
	 * Connects the GUI elements with functionality to create
	 * interaction.
	 */
	 ...
	 */
	public synchronized void threadDone(PerformanceAnalysisMethodExecutionThread thread) {
	...
	private void buildResultsFrame() {
		...
		PerformanceAnalysisGUI perfWindow = new PerformanceAnalysisGUI(
											tempSet,
											options.getManualSelected(),
											options.getBoundaries(),
											options.getColors(),
											options.getTimeDivider(),
											options.getTimesort(),
											options.getDecimalPlaces(),
											options.getAdvancedSettings().clone()
											);

		perfWindow.setVisible(true);
		// spawn result window
		MainUI.getInstance().createAnalysisResultFrame(
				                              myAlgorithm, myInput, perfWindow);
	...
	/**
	 * Invokes a seperate thread for each
	 * {@link LogReplayAnalysisMethod LogReplayAnalysisMethod}
	 * and builds the {@link PerformanceAnalysisResults PerformanceAnalysisResults}
	 * frame with the results obtained.
	...
	class PerformanceCheckAction extends AbstractAction {
		 ...
		 * @param frame the calling settings frame containing the referenced objects
		 * needed to carry out the analysis
		 */
		public PerformanceCheckAction(PerformanceAnalysisSettings frame) {
			super("<html><B>Start analysis<html>", Utils.getStandardIcon("media/Play24"));
		 ...
		 * Starts each selected {@link AnalysisMethod AnalysisMethod} in a separate
		 * {@link CustomSwingworker CustomSwingworker} and waits for their return
		 * before building the {@link ConformanceAnalysisResults ConformanceAnalysisResults}
		 * frame.
		 * Note that the {@link AnalysisConfiguration AnalysisConfiguration} object is
		 * cloned before passed to the analyis methods in order to disconnect it from
		 * the {@link ConformanceAnalysisSettings ConformanceAnalysisSettings} frame,
		 * which remains to exist (and may invoke the analysis multiple times).
		 * @param e not used
		 */
		public void actionPerformed(ActionEvent e) {
			settings.startAnalysis();
			// start each analysis method in a separate thread to monitor progress and be
			// able to abort it
			execute(new LogReplayAnalysisMethod(myNet, myLog, new PerformanceMeasurer(), new Progress("")));
		}

		/**
		 * Starts the given analyis method in a separate thread.
		 * This is necessar if one wants to be able that the method can be aborted by the user.
		 * @param method the AnalysisMethod to be performed
		 */
		public void execute(LogReplayAnalysisMethod method) {
			// start analysis method in a separate thread to monitor progress and be
			// able to abort it
			/* NOTE modified by Anne in order to support restricted search depth (see AdvancedOptions.java) */
			if (options.getAdvancedSettings()[4] >= 0) {	
				method.setMaxDepth(options.getAdvancedSettings()[4]);
			}
			PerformanceAnalysisMethodExecutionThread worker =
				new PerformanceAnalysisMethodExecutionThread(
		...
		/**
		 * Called if an analyis method runs out of memory while being executed, it
		 * propagates an error message to the user.
		 * @param err the OutOfMemoryError being passed from the caller
		 */
