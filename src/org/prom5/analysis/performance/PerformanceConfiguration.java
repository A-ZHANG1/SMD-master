...
import org.prom5.analysis.conformance.MaximumSearchDepthDiagnosis;
...
public class PerformanceConfiguration extends JPanel {
	...
	//(last parameter determines whether to restrict the search depth for the log replay. 
	...
	private JPanel placePanel = new JPanel(); //Panel containing other radiobuttons
	...
	private JRadioButton autoButton = new JRadioButton("Use auto bottleneck settings "
									 + " (let program choose bound values)", true);
	private JRadioButton ownButton = new JRadioButton("Use manual bottleneck settings "
									+ " (specify below)");

	//radio buttons + group to allow the user to select if the entered values
	//should be set for all places or only for the currently selected (if any) one
	private ButtonGroup buttonGroup2 = new ButtonGroup();
	private JRadioButton allPlacesButton = new JRadioButton("All places", true);
    private JRadioButton thisPlaceButton = new JRadioButton("The selected place only", false);
	...
	private JLabel useSettingsLabel = new JLabel("Use bottleneck settings for: ");
	...
	public PerformanceConfiguration(PetriNet net) {
		try {
			...
			int maxSearchDepth = MaximumSearchDepthDiagnosis.determineMaximumSearchDepth(net);
	 ...
	 * @param levelColors ArrayList: contains the colors corresponding to the
	 * different time levels
	 * @param bounds ArrayList: contains the bounds
	 * @param sort String: contains the timeSort in which is measured
	 * @param decimals int: number of decimal places
	 * @param settings int[]: the advanced options used
	 */
	...
	/**
	 * Initializes the GUI-panel in which the user can specify
	 * settings, e.g. level boundaries and colors
	 *
	 * @throws Exception
	 */
	private void jbInit() throws Exception {
		...
		con.gridx = 0; con.gridy = 0; con.insets = new Insets(10, 10, 0, 0);
		...
		con.gridx = 0; con.gridy = 3;con.insets = new Insets(0, 10, 0, 0);
		...
		con.gridx = 0; con.gridy = 0; //column and row
		con.gridwidth = 1; con.insets = new Insets( 0, 5, 5, 10);
		...
		con.gridx = 3; con.gridy = 0; con.gridwidth = 1;
		...
		mediumPanel.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(
							  Color.white, new Color(148, 145, 140)),
							  "Medium level settings", 0, 0,
		...
		con.gridx = 0; con.gridy = 1; con.anchor = GridBagConstraints.NORTHWEST;
		...
		con = new GridBagConstraints();
		con.gridy = 0; con.gridx = 0; con.insets = new Insets(0, 10, 0, 0);
		con.anchor = GridBagConstraints.NORTHWEST;
		generalPanel.add(timesortLabel,con);
		con.gridy = 0; con.gridx = 1; con.insets = new Insets(0, 10, 5, 70);
		con.anchor = GridBagConstraints.NORTHWEST;
		timeBox.setPreferredSize(new Dimension(120, 20));
		timeBox.setBackground(Color.WHITE);
		generalPanel.add(timeBox, con);
		con.gridy = 1; con.gridx = 0; con.insets = new Insets(0, 10, 0, 0);
		con.anchor = GridBagConstraints.NORTHWEST;
		generalPanel.add(decimalLabel,con);
		con.gridy = 1; con.gridx = 1; con.insets = new Insets(0, 10, 0, 0);
		con.anchor = GridBagConstraints.NORTHWEST;
		...
		con.gridy = 2; con.gridx = 0; con.insets = new Insets(5, 10, 5, 0);
		con.anchor = GridBagConstraints.NORTHWEST; con.gridwidth = 2;
		generalPanel.add(advancedButton, con);
		generalPanel.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(
				Color.white, new Color(148, 145, 140)), "General settings",
		...
		con.gridy = 0; con.gridx = 0; con.gridwidth = 5;
		con.insets = new Insets(0, 10, 0, 0); con.anchor = GridBagConstraints.NORTHWEST;
		bottleneckPanel.add(lowPanel, con);
		con.gridy = 1; con.gridx = 0; con.gridwidth = 5;
		bottleneckPanel.add(mediumPanel, con);
		con.gridy = 2; con.gridx = 0; con.gridwidth = 5;
		bottleneckPanel.add(highPanel, con);
		con.gridy = 3; con.gridx = 0;
		bottleneckPanel.add(placePanel, con);
		bottleneckPanel.setBorder(new TitledBorder(
				BorderFactory.createEtchedBorder(Color.white,
				new Color(148, 145, 140)), "Bottleneck settings",
				0, 0, new Font("SansSerif", Font.BOLD, 13)));
	...
	*/
	private void registerGuiActionListener(){
		...
		decimalField.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				//set the number of decimal places to the number specified by the user
		...
		lowButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				Color tempColor = JColorChooser.showDialog(MainUI.getInstance(),
								  "Select low level color", lowLevelColor);
				if (tempColor != null) {
		...
		mediumButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				//open dialog in which the user can choose a new color
				Color tempColor = JColorChooser.showDialog(MainUI.getInstance(),
								  "Select medium level color", mediumLevelColor);
				if (tempColor != null) {
		...
		highButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				//open dialog in which the user can choose a new color
				Color tempColor = JColorChooser.showDialog(MainUI.getInstance(),
								  "Select high level color", highLevelColor);
				if (tempColor != null) {
		...
		lowUpperField.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				//set lowUpper to the value filled in at lowUpperField (if valid)
		...
		advancedButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				MainUI.getInstance().createFrame("Advanced Performance Analysis options", advancedPanel);
	 ...
	 * method checks whether a place was selected and displays the placePanel
	 * if this is the case. If the place has its 'own'-settings, i.e. the user
	 * filled in settings specific to this place before, then the settings that
	 * were filled in by the user are displayed.
	 *
	 * @param selected ExtendedPlace: the selected place
	 */
	...
	/**
	 * Takes as input the timesort (seconds, minutes etc.) and returns the number
	 * through which the measurements have to be divided to get the time in the
	 * right timesort (normally time is measured in milliseconds)
	 *
	 * @param time String: the timesort
	 * @return long
	 */
	...
	/**
	 * Returns true if manual settings have been selected, else (use auto-settings)
	 * false.
	 * @return boolean
	 */
	 ...
	 * @param settings int[]: the settings to which the advanced settings are to
	 * be set
	 */
