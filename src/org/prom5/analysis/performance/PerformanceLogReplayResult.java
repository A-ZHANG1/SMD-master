 ...
 * Can be used to retrieve values for the performance metrics and to get
...
public class PerformanceLogReplayResult extends LogReplayAnalysisResult {
	...
	public PerformanceLogReplayResult(AnalysisConfiguration analysisOptions, PetriNet net,
    ...
    /**
	 * Initializes the diagnostic data structures needed to store the measurements
	 * taken during the log replay analysis.
	 */
	protected void initDiagnosticDataStructures() {
		replayedLog = new ExtendedLogReader(inputLogReader);
		replayedPetriNet = new ExtendedPetriNet(inputPetriNet,
						   replayedLog.getLogTraceIDs());
	}

//////////////////////////////METRICS-RELATED METHODS///////////////////////////

    /**
	 * Calculates the average, min ad max throughput time out of the throughput times of all
	 * traces in piList. Next to this, the arrival rate is calculated.
	 * All metrics are based on the process instances in piList only
	 *
	...
	public void calculateMetrics(ArrayList piList, int fitOption) throws Exception {
		properFrequency = 0;
		timeStats.clear(); arrivalStats.clear();
		...
		while (lit.hasNext()) {
			...
			try {
				long tp = (currentTrace.getEndDate().getTime() -
						  currentTrace.getBeginDate().getTime());
				if (fitOption == 0) {
	...
	/**
	 * Exports the throughput times of all process instances in piList to
	...
	public void exportToFile(ArrayList piList, File file, long divider,
		...
		while (lit.hasNext()) {
			ExtendedLogTrace currentTrace = (ExtendedLogTrace) lit.next();
			try {
				double tp = (currentTrace.getEndDate().getTime() -
							currentTrace.getBeginDate().getTime()) * 1.0 / divider;
				if (fitOption == 0) {
...
//////////////////////////////GET METHODS///////////////////////////////////////
	 ...
	 * times in timeStats. (make sure calculateProcessMetrics() is called
	 * before this method).
	 *
	 * @return double
	 */
	 ...
	 * in an array, where [0]: avg fast throughput time
	 *                    [1]: avg slow throughput time
	 ...
	 * considered to be fast, i.e. have a low throughput time (place 0 in array),
	 ...
	 * timeList (thus method calculateProcessMetrics() should be called before
	 * this one)
	 *
	 * @param fastestPercentage double: the percentage of measurements that
	 * is to be counted as fast
	...
	public int[] getSizes(double fastestPercentage, double slowestPercentage) {
		...
		if (sizes[0] != length) {
			...
			if ((sizes[0] + sizes[1]) > length) {
				...
				//the number of measurements in timeList (rounding could mess this up)
	...
	public double getArrivalRate() {
		...
		if (arrivalStats.getN() > 0 && arrivalStats.getMean() != 0) {
			//mean arrivalRate is 1 divided by the mean of the inter-arrival times
