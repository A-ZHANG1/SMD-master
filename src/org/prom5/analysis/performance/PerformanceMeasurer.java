...
public class PerformanceMeasurer extends Measurer {
    ...
    /**
	 * {@inheritDoc}
	 * Creates an analysis result of type {@link PerformanceLogReplayResult PerformanceLogReplayResult}.
	 */
	protected LogReplayAnalysisResult initLogReplayAnalysisResult(AnalysisConfiguration
			analysisOptions, PetriNet petriNet, LogReader log,
			LogReplayAnalysisMethod analysisMethod) {
		// initialize the result object and the analysis options
		return new PerformanceLogReplayResult(analysisOptions, petriNet, log, analysisMethod);
	}
	 ...
	 *
	 * @param pi ReplayedLogTrace
	 * @param t ReplayedTransition
	...
	protected void takePostTraceReplayMeasurement(LogReplayAnalysisResult result,
		...
		try {
			...
			while (remainingTokenChecker.hasNext()) {
				ExtendedPlace currentPlace = (ExtendedPlace) remainingTokenChecker.next();
