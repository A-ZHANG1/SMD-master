 ...
 * Most importantly, the (avg,min,max etc.) time-in-between the two transitions
 * can be calculated. Where time in between two transitions for a certain
 * process instance is the time between the first firing of the one transition
 * and the first firing of the other transition.
 *
 ...
 * occurs first, and if we should keep seperate metrics for the times transition
 * one occurs before two and the other way around
 */
public class TransitionAnalysis {
	 ...
	 * and otherTrans
	 * @param lastTrans ExtendedTransition: one of the two transitions
	 * @param otherTrans ExtendedTransition: one of the two transitions
	 */
	...
	/**
	 * Calculates the average, maximal and minimum time tokens spend between
	 * the two transitions, and the standard deviation in this time, and
	 * averages of the fastest, slowest and normal time-in-betweens.
	 * Next to this, the frequency of cases in which both transitions fire is
	 * calculated. Results of the calculations are stored in corresponding
	 * instance-variables.
	 *
	 * @param piList ArrayList: The process instances on which calculations are
	 * based
	...
	public void calculateMetrics(ArrayList piList, int fitOption, HashSet failedInstances) {
		...
		while (it.hasNext()){
			...
			if ((other.getFirstFireTime(piName) != null) &&
				...
				long transTime = Math.abs(other.getFirstFireTime(piName).getTime()
				...
				if ((fitOption == 1 && !other.hasFailedBefore(piName)) &&
					...
					//times based on measurements in traces where both transitions
	 ...
	 * @param file File: the file to which the measurements should be exported
	...
	public void exportToFile (ArrayList piList, File file, long divider,
		...
		while (it.hasNext()){
			...
			if ((other.getFirstFireTime(piName) != null) &&
				...
				double transTime =
							(Math.abs(other.getFirstFireTime(piName).getTime()
									 - last.getFirstFireTime(piName).getTime())
									 * 1.0) / divider;
				if (fitOption == 0) {
					//times based on all measurements
	...
	/**
	 * Calculates the average of the (fastestpercentage) fast traces,
	 * the (slowestPercentage) slow traces and the (100% - fastestPercentage -
	 * slowestPercentage) normal speed traces and returns these averages
	 * in an array, where [0]: avg fast time in between
	 *                    [1]: avg slow time in between
	 *                    [2]: avg middle time in between
	 * @param fastestPercentage double: the percentage of measurements that
	 * is to be counted as fast
	 * @param slowestPercentage double: the percentage of measurements that
	 ...
	 * considered to be fast, i.e. have a low time in between (place 0 in array),
	 * the number of process instances that are slow (place 1 in array) and the
	 * number of process instances that are considered to be of normal speed
	 * (place 2 in array). Based on fastestPercentage, slowestPercentage and
	 * the values in timeStats (thus method calculateTimeInBetweenMetrics()
