...
import org.prom5.analysis.performance.advanceddottedchartanalysis.model.guimap.GuiMapModel;
...
public class DottedChartModel {
	...
	{
		if(originatorCode[k]==-1) return "";
		else return originatorList.get(originatorCode[k]);
	}
	public String getEvent(int k)
	{
		if(originatorCode[k]==-1) return "";
		else return eventList.get(eventCode[k]);
	}
...
//		XLogInfo summary = XLogInfoFactory.createLogInfo(inputLog);
		logMinValue = inputLog.getLogSummary().getStartTime(inputLog.getProcess(0).getName()).getTime();
		logMaxValue = inputLog.getLogSummary().getEndTime(inputLog.getProcess(0).getName()).getTime();
		
		
//		logMinValue = summary.getLogTimeBoundaries().getStartDate().getTime();
...
//		Iterator it = inputLog.instanceIterator();
		...
		while (it.hasNext()) 
		{
			...
			while(ates.hasNext()) {
				...
				if(ate.getOriginator()==null){
					...
					originatorCode[num] = originatorList.indexOf(ate.getOriginator());
				}
				...
				guiMapModel.makeLogicalMap(TIME_LOGICAL_RELATIVE+ST_INST, logicalRelative[num], String.valueOf(num_instance), num);
				guiMapModel.makeLogicalMap(TIME_LOGICAL_RELATIVE+ST_ORIG, logicalRelative[num], String.valueOf(originatorCode[num]), num);
				guiMapModel.makeLogicalMap(TIME_LOGICAL_RELATIVE+ST_EVEN, logicalRelative[num], String.valueOf(eventCode[num]), num);
				guiMapModel.makeLogicalMap(TIME_LOGICAL_RELATIVE+ST_TASK, logicalRelative[num], String.valueOf(taskCode[num]), num);
				...
				if ( timestamp == null) {
					...
					guiMapModel.makeActualMap(TIME_ACTUAL+ST_INST, timestamp.getTime(), String.valueOf(num_instance), num);
					guiMapModel.makeActualMap(TIME_ACTUAL+ST_ORIG, timestamp.getTime(), String.valueOf(originatorCode[num]), num);
					guiMapModel.makeActualMap(TIME_ACTUAL+ST_EVEN, timestamp.getTime(), String.valueOf(eventCode[num]), num);
					guiMapModel.makeActualMap(TIME_ACTUAL+ST_TASK, timestamp.getTime(), String.valueOf(taskCode[num]), num);
					...
					if(duration<relativeTime[num]) duration = relativeTime[num];
					guiMapModel.makeRelativeTimeMap(TIME_RELATIVE_TIME+ST_INST, relativeTime[num], String.valueOf(num_instance), num);
					guiMapModel.makeRelativeTimeMap(TIME_RELATIVE_TIME+ST_ORIG, relativeTime[num], String.valueOf(originatorCode[num]), num);
					guiMapModel.makeRelativeTimeMap(TIME_RELATIVE_TIME+ST_EVEN, relativeTime[num], String.valueOf(eventCode[num]), num);
					guiMapModel.makeRelativeTimeMap(TIME_RELATIVE_TIME+ST_TASK, relativeTime[num], String.valueOf(taskCode[num]), num);
					...
					// statistics
					if(current==-1)	current = timestamp.getTime();
					else {
			...
			for(int k=num-1;k>=startnum;k--)
			{
				if(relativeTime[k]!=-1) {
					...
					if(logRelativeMaxValue<duration) logRelativeMaxValue = duration;
					break;
				}				
			}
			// logical relative time max
			if(logLogicalRelativeMaxValue<index) logLogicalRelativeMaxValue = index;
			num_instance++;
			// relative ratio
			...
			{
				if(relativeTime[k]==-1)continue;
				if(duration==0){
					relativeRatio[k] = 0;
				} else {
					relativeRatio[k] = (int)(((float)relativeTime[k]/(float)duration)*10000);
				}
				guiMapModel.makeRelativeRatioMap(TIME_RELATIVE_RATIO+ST_INST, relativeRatio[k], String.valueOf(num_instance-1), k);
				guiMapModel.makeRelativeRatioMap(TIME_RELATIVE_RATIO+ST_ORIG, relativeRatio[k], String.valueOf(originatorCode[k]), k);
				guiMapModel.makeRelativeRatioMap(TIME_RELATIVE_RATIO+ST_EVEN, relativeRatio[k], String.valueOf(eventCode[k]), k);
				guiMapModel.makeRelativeRatioMap(TIME_RELATIVE_RATIO+ST_TASK, relativeRatio[k], String.valueOf(taskCode[k]), k);
	...
	public HashMap<String,HashMap<String,ArrayList<Integer>>> getProperActualMap(String key, double milli2pixels)
	{
		...
		else if(temp<=1800000L) type = "halfhour";
		else if(temp<=3600000L) type = "hour";
		else if(temp<=86400000L) type = "day";
		else if(temp<=2592000000L) type = "week";
		return guiMapModel.getMap(key, type).getMap();
	}
	
//	public HashMap<String,HashMap<String,ArrayList<Integer>>> getDifferedProperMap(String key, double milli2pixels)
//	{
//		String type="month";
//		long temp = (long)(1/milli2pixels);
//		
...
//	}
	...
	{
		if(key.startsWith(TIME_RELATIVE_RATIO)) return getProperRelativeRatioMap(key, milli2pixels);
		else if(key.startsWith(TIME_RELATIVE_TIME)) return getProperRelativeTimeMap(key, milli2pixels);
		else if(key.startsWith(TIME_ACTUAL)) return getProperActualMap(key, milli2pixels);
		else if(key.startsWith(TIME_LOGICAL_RELATIVE)) return getLogicalMap(key);
		return null;
	}
	...
	public HashMap<String,HashMap<String,ArrayList<Integer>>> getProperRelativeRatioMap(String key, double milli2pixels)
	{
		long temp = (long)(1/milli2pixels);
		if(temp<=1L) return guiMapModel.getMap(key, "1").getMap();
		else if(temp<=5L) return guiMapModel.getMap(key, "5").getMap();
		else if(temp<=10L) return guiMapModel.getMap(key, "10").getMap();
		else if(temp<=50L) return guiMapModel.getMap(key, "50").getMap();//hourMap;
		else return guiMapModel.getMap(key, "100").getMap();//hourMap;
	}
	...
	{
		if(timeOption.equals(TIME_RELATIVE_RATIO)) return getRelativeRatioProperTimeUnit(milli2pixels);
		else if(timeOption.equals(TIME_RELATIVE_TIME)) return getRelativeProperTimeUnit(milli2pixels);
		else if(timeOption.equals(TIME_ACTUAL)) return getActualProperTimeUnit(milli2pixels);
		else if(timeOption.equals(TIME_LOGICAL_RELATIVE)) return 1;
		return 1;
	}
	...
	public long getActualProperTimeUnit(double milli2pixels)
	{
		...
		else if(temp<=60000L) return 60000L;
		else if(temp<=1800000L) return 1800000L;
		else if(temp<=3600000L) return 3600000L;
		else if(temp<=86400000L) return 86400000L;
		else if(temp<=604800000L) return 604800000L;
		return 2592000000L;
	}
...
//	}
	...
	public long getRelativeRatioProperTimeUnit(double milli2pixels)
	{
		long temp = (long)(1/milli2pixels);
		if(temp<=1L) return 1;
		else if(temp<=5L) return 5;
		else if(temp<=10L) return 10;
		else if(temp<=50L) return 50;
		else return 100;
	}
