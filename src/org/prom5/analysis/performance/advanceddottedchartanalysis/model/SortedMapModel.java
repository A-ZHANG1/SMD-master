...
import org.prom5.analysis.performance.advanceddottedchartanalysis.ui.DottedChartPanel;
...
public class SortedMapModel {
	...
	{
		minMaxModel.assignValue(DottedChartModel.ST_INST+time+instanceid,value);
		minMaxModel.assignValue(DottedChartModel.ST_ORIG+time+task,value);
		minMaxModel.assignValue(DottedChartModel.ST_EVEN+time+originator,value);
	...
	public long[] getStartTimes(String timeOption, String componentType, ArrayList<String> strList)
	{
		ArrayList<String> itemList = dcModel.getItemArrayList(componentType);
		...
		{
			values[i++] = minMaxModel.getMinValue(componentType+timeOption+itemList.indexOf(str));
		}
		return values;		
	}
	...
	public long[] getEndTimes(String timeOption, String componentType,ArrayList<String> strList)
	{
		ArrayList<String> itemList = dcModel.getItemArrayList(componentType);
		...
		for(String str:strList)
		{
			values[i++] = minMaxModel.getMaxValue(componentType+timeOption+itemList.indexOf(str));
		}
		return values;		
	}
	
	public long[] getDurations(String timeOption, String componentType, ArrayList<String> strList)
	{
		ArrayList<String> itemList = dcModel.getItemArrayList(componentType);
		...
		for(String str:strList)
		{
			values[i++] = minMaxModel.getDurationValue(componentType+timeOption+itemList.indexOf(str));
		}
		return values;		
	}
	...
	public long[] getDurations(String timeOption, String componentType)
	{
		...
		for(int i=0;i<itemList.size();i++)
		{
			values[i] = minMaxModel.getDurationValue(componentType+timeOption+i);
		}
		
		return values;		
	}
	
	public ArrayList<String> getSortedItemArrayList(String timeOption, String componentType, String sortStardard, boolean desc)
	{
		...
		{
			if(!sortedStringMap.containsKey(componentType+sortStardard+desc)) {
				originalStrList = dcModel.getItemArrayList(componentType);
				ArrayList<String> origianlSortedKeys = new ArrayList<String>(originalStrList);
				if(sortStardard.equals(DottedChartPanel.ST_START_TIME)) {
						dataArray = getStartTimes(DottedChartModel.TIME_ACTUAL, componentType);
				}
				else if(sortStardard.equals(DottedChartPanel.ST_END_TIME)) {
					dataArray = getEndTimes(DottedChartModel.TIME_ACTUAL, componentType);
				}
				else if(sortStardard.equals(DottedChartPanel.ST_DURATION)) {
					dataArray = getDurations(DottedChartModel.TIME_ACTUAL, componentType);
				}
				quicksort(origianlSortedKeys,0,(origianlSortedKeys.size()-1), sortStardard);

				sortedStringMap.put(componentType+sortStardard+false, origianlSortedKeys);
				...
				sortedStringMap.put(componentType+sortStardard+true, inversedKeys);
			}
			return sortedStringMap.get(componentType+sortStardard+desc);
		}else if(!sortedStringMap.containsKey(timeOption+componentType+sortStardard+desc)) {
			originalStrList = dcModel.getItemArrayList(componentType);
			ArrayList<String> origianlSortedKeys = new ArrayList<String>(originalStrList);
			...
			sortedStringMap.put(timeOption+componentType+sortStardard+false, origianlSortedKeys);
			
			ArrayList<String> inversedKeys = new ArrayList<String>();
			for(int k=origianlSortedKeys.size()-1;k>=0;k--)
				inversedKeys.add(origianlSortedKeys.get(k));
			sortedStringMap.put(timeOption+componentType+sortStardard+true, inversedKeys);
		}
		return sortedStringMap.get(timeOption+componentType+sortStardard+desc);
	}
	
	public HashMap<Integer,Integer> getSortedMapping(String timeOption, String str, String sort, boolean desc)
	{
				...
				sort.equals(DottedChartPanel.ST_DURATION))
		{
			if(!codeMatchMapMap.containsKey(str+sort+desc)) {
				...
				ArrayList<String> tempsortedKeys = getSortedItemArrayList(timeOption, str,sort,desc);
	...
	public int[] getSortedCode(String timeOption, String str, String sort, boolean desc)
	{
				...
				sort.equals(DottedChartPanel.ST_DURATION))
		{
			if(!sortedCodeMap.containsKey(str+sort+desc)){
				...
				HashMap<Integer,Integer> codeMatchMap = getSortedMapping(timeOption, str, sort, desc);
...
//////////quick sort
	private void quicksort(ArrayList<String> key, int left, int right, String type) {
	...
	private int partition(ArrayList<String> key, int left, int right, String type) {
		int i = left - 1;
		int j = right;
		while (true) {
			while (less(key, (++i), right, type)) // find item on left to swap
					; // a[right] acts as sentinel
			while (less(key, right, (--j), type)) // find item on right to swap
	...
	private  boolean less(ArrayList<String> keys, int i, int j, String type) {
		if (type.equals(DottedChartPanel.ST_NAME)) {
			...
			return (dataArray[originalStrList.indexOf(keys.get(i))]<dataArray[originalStrList.indexOf(keys.get(j))]);
		}
	}
