...
public class CoordinationUtil {
	 ...
	 * constructor
	 */
	public CoordinationUtil(DottedChartOptionPanel dcop, DottedChartModel dcModel) {
		this.dcop = dcop;
	...
	public void updateMilli2pixelsRatio(int width, int border) {
		...
		if(str.equals(DottedChartModel.TIME_ACTUAL)) {
			milli2pixels = (double) (width - border*2) / (double)(dcModel.getLogMaxValue() - dcModel.getLogMinValueforScreen()); // + 2 is added to extend bound
		} else if(str.equals(DottedChartModel.TIME_RELATIVE_TIME)) {
			milli2pixels = (double) (width - border*2) / (double)(dcModel.getLogRelativeMaxValue() - 0); // + 2 is added to extend bound
		} else if (str.equals(DottedChartModel.TIME_RELATIVE_RATIO)) {
			milli2pixels = (double) (width - border*2) / (double)(dcModel.getLogRelativeRatioMaxValue() - 0); // + 2 is added to extend bound
		} else if (str.equals(DottedChartModel.TIME_LOGICAL_RELATIVE)) {
			milli2pixels = (double) (width - border*2) / (double)(dcModel.getLogLogiclRelativeMaxValue() - 0); // + 2 is added to extend bound
		}
	...
	public long coord2timeMillis(double anX) {
		if(dcop.getTimeOption().equals(DottedChartModel.TIME_ACTUAL))
			return (long)(anX / getMilli2pixels()) + dcModel.getLogMinValueforScreen();
		else 
