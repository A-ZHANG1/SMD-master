...
package org.prom5.analysis.performance.advanceddottedchartanalysis.ui;
...
import org.prom5.analysis.performance.advanceddottedchartanalysis.DottedChartAnalysis;
import org.prom5.analysis.performance.advanceddottedchartanalysis.model.DottedChartModel;
 ...
 * @author Minseok Song
 */
public class DottedChartPanel extends JPanel implements MouseListener, MouseMotionListener {
...
//		colorLogBright = new Color(210, 210, 200);
	...
	public void setViewportZoomIn() {
		...
		int value = (int) (Math.log10(this.getWidth() * (d.getWidth() / width)/dca.getViewportSize().getWidth())*1000.0);
		if(value>3000) return;
		value = (int) (Math.log10(this.getHeight() * (d.getHeight() / height)/dca.getViewportSize().getHeight())*1000.0);
		if(value>3000) return;
	...
	public void adjustSlideBar() {
		int value = (int) (Math.log10(updWidth/dca.getViewportSize().getWidth())*1000.0);
	...
	public void adjustWidth(){
		...
		if (number == 0) {
			...
			if (idx < 0) dca.getDottedChartOptionPanel().changeWidthSort(0);
			else dca.getDottedChartOptionPanel().changeWidthSort(idx);
		} else if (number > 100) {
			for (idx = 0; idx < widthDividers.length; idx++)
				if (widthDividers[idx] == widthSort)
					break;
			...
			if (idx >= widthDividers.length) dca.getDottedChartOptionPanel().changeWidthSort(widthDividers.length - 1);
			else dca.getDottedChartOptionPanel().changeWidthSort(idx);
		}
	}
	...
	*/
	public void paintComponent(Graphics grx) {
		...
		if((int)gr.getClipBounds().getMinX()<coUtil.getClipL()||(int)gr.getClipBounds().getMaxX()>coUtil.getClipR()
			|| (int)gr.getClipBounds().getMinY()<coUtil.getClipU()||(int)gr.getClipBounds().getMaxY()>coUtil.getClipB()){
				modelBuffer = null;
		}
...
/*	public void paintComponent(Graphics grx) {
	...
	protected Graphics2D drawModelBuffer_Common(int imageWidth, int imageHeight, boolean isModel) {
		...
		if(isModel) {
			modelBuffer = new BufferedImage(Math.min(imageWidth,SCREENLENGTH), Math.min(imageHeight,SCREENLENGTH), BufferedImage.TYPE_INT_RGB);
	...
	protected long getLeftRight(int k)
	{
		if(dcop.getTimeOption().equals(DottedChartModel.TIME_ACTUAL)) 
			return ((coUtil.coord2timeMillis(k)+timeOffset)/dcModel.getProperTimeUnit(dcop.getTimeOption(),coUtil.getMilli2pixels()));
		if(dcop.getTimeOption().equals(DottedChartModel.TIME_RELATIVE_RATIO)||dcop.getTimeOption().equals(DottedChartModel.TIME_RELATIVE_TIME))
			return ((coUtil.coord2timeMillis(k))/dcModel.getProperTimeUnit(dcop.getTimeOption(),coUtil.getMilli2pixels()));
		if(dcop.getTimeOption().equals(DottedChartModel.TIME_LOGICAL_RELATIVE))
			return coUtil.coord2timeMillis(k);
		return 0;
	}
	
	protected void drawModelBuffer(int imageWidth, int imageHeight, boolean isModel) {
		int tempBorder;
		int compNumber = dcModel.getComponentSize(dca.getDottedChartOptionPanel().getComponentType());
		if (isModel) {
			tempBorder=BORDER;
		}
		else {
		...
		double milli2pixelsBuffer = coUtil.updateMilli2pixelsRatioBuffer(imageWidth, tempBorder);
		Graphics2D gBuf = drawModelBuffer_Common(imageWidth, imageHeight, isModel);
		int unitHeight = coUtil.getUnitHeight(imageHeight,tempBorder);

		HashMap<String,HashMap<String,ArrayList<Integer>>> timeMap = 
			dcModel.getProperTimeMap(dcop.getTimeOption()+dcop.getComponentType(),coUtil.getMilli2pixels());
		...
		int currentTop = 0;
		// currentTop is different. But the performance reason, the code is separated.
		if(dcop.getSortStandard().equals(DottedChartModel.STR_NONE))
		{
			for(long j=left; j<=right;j++){
				HashMap<String,ArrayList<Integer>> tempMap = timeMap.get(String.valueOf(j));
				if(tempMap==null) continue;
				for(String str:tempMap.keySet()){
					currentTop = coUtil.unit2CordHeight(Integer.valueOf(str), compNumber, imageHeight,tempBorder) + unitHeight / 2;
					if(currentTop<upper|| currentTop>bottom) continue;
					assignColorByItem(tempMap.get(str), gBuf);
					long temp = j*dcModel.getProperTimeUnit(dcop.getTimeOption(),coUtil.getMilli2pixels());
					//if(dcop.getTimeOption().equals(DottedChartModel.TIME_ACTUAL)) temp -= timeOffset;
					if(isModel) {
						uiUtil.paintItemHigh(coUtil.time2coord(temp) + tempBorder - coUtil.getClipL(), currentTop - upper, gBuf, ITEM_HANDLE_CIRCLE, tempMap.get(str).size());
					} else {
						uiUtil.paintItem_buffer(coUtil.time2coordOverview(temp,milli2pixelsBuffer) + tempBorder, currentTop, gBuf, ITEM_HANDLE_CIRCLE, tempMap.get(str).size());
					}
			...
			HashMap<Integer,Integer> mapping = dcModel.getSortedMapModel().getSortedMapping(dcop.getTimeOption(), dcop.getComponentType(),dcop.getSortStandard(),dcop.isDescCheckBoxSelected());
			for(long j=left; j<=right;j++){
				HashMap<String,ArrayList<Integer>> tempMap = timeMap.get(String.valueOf(j));
				if(tempMap==null) continue;
				for(String str:tempMap.keySet()){
					currentTop = coUtil.unit2CordHeight(mapping.get((int)Integer.valueOf(str)),compNumber,imageHeight,tempBorder) + unitHeight / 2;
					if(currentTop<upper|| currentTop>bottom) continue;
					assignColorByItem(tempMap.get(str), gBuf);
					long temp = j*dcModel.getProperTimeUnit(dcop.getTimeOption(),coUtil.getMilli2pixels());
					//if(dcop.getTimeOption().equals(DottedChartModel.TIME_ACTUAL)) temp -= timeOffset;
	...
	protected void drawModelBuffer_NO_SIZE(int imageWidth, int imageHeight, boolean isModel) {
		int tempBorder;
		...
		int[] intCodes = dcModel.getSortedMapModel().getSortedCode(dcop.getTimeOption(), dcop.getComponentType(),
		...
		for(int k=0;k<items.length;k++) {
			if(items[k]<clipLeftTs||items[k]>clipRightTs) continue;
			currentTop = coUtil.unit2CordHeight(intCodes[k],size,imageHeight,tempBorder) + unitHeight / 2;
			if(currentTop<upper || currentTop>bottom) continue;
			assignColorByItem(k, gBuf);
			if(isModel) {							
				uiUtil.paintItem(coUtil.time2coord(items[k]) + tempBorder - coUtil.getClipL(), currentTop - upper, gBuf, assignShapeByItem(k));
			} else {
				uiUtil.paintItem_buffer(coUtil.time2coordOverview(items[k],milli2pixelsBuffer) + tempBorder, currentTop, gBuf, assignShapeByItem(k));
			}
		}
	}
	...
	protected void paintComponentLane(Graphics g, int imageWidth, int imageHeight, boolean isModel){
		...
		Iterator<String> itr = dcModel.getSortedMapModel().getSortedItemArrayList(dcop.getTimeOption(), dcop.getComponentType(),
											dcop.getSortStandard(),dcop.isDescCheckBoxSelected()).iterator();
		...
		while (itr.hasNext())
		{
			...
			if(coUtil.unit2CordHeight(index+1, size, imageHeight, tempBorder)>bottom) break;
			index++;
			// swap colors
			tmpColor = fgColor;
			fgColor = bgColor;
...
//				g.drawString(timeStr, pixStart+2, yTop);
				...
				if(lastLable+40>=pixStart+2) continue;
				timeStr = timeStart/100 + "." + (timeStart - timeStart/100*100) + "%";
				g.drawString(timeStr, pixStart+2, yTop);
			}
			else if (dcop.getTimeOption().equals(DottedChartModel.TIME_LOGICAL)||dcop.getTimeOption().equals(DottedChartModel.TIME_LOGICAL_RELATIVE)) {
				if(lastLable+20>=pixStart+2) continue;
				timeStr = String.valueOf(timeStart);
				g.drawString(timeStr, pixStart+2, yTop);
			}
	...
	{
		colorCodes = dcModel.getCode(dca.getDottedChartOptionPanel().getColorStandard());
		colorItems = dcModel.getItemArrayList(dca.getDottedChartOptionPanel().getColorStandard());
		shapeCodes = dcModel.getCode(dca.getDottedChartOptionPanel().getShapeStandard());
		shapeItems = dcModel.getItemArrayList(dca.getDottedChartOptionPanel().getShapeStandard());
	}
	
	protected String assignShapeByItem(int k) {
		String shapeStandard = dca.getDottedChartOptionPanel().getShapeStandard();
	...
	protected void assignColorByItem(String name, Graphics g) {
		String colorStandard = dca.getDottedChartOptionPanel().getColorStandard();
	...
	protected void assignColorByItem(ArrayList<Integer> itemlist, Graphics g) {
		...
		if(colorStandard.equals(DottedChartModel.STR_NONE))
	    ...
	    else 
			{
	    		...
	    		{
	    			colorArray[k]= itemColors.getColor(colorItems.get(colorCodes[itemlist.get(k)]));
	    		}
	...
	private String hasDots_NO_SIZE(Point p1, Point p2)
	{
		...
		int[] intCodes = dcModel.getSortedMapModel().getSortedCode(dcop.getTimeOption(), dcop.getComponentType(),
											dcop.getSortStandard(),dcop.isDescCheckBoxSelected());
		long clipLeftTs  = coUtil.coord2timeMillis(Math.max(0, min_x-5-BORDER));
		long clipRightTs = coUtil.coord2timeMillis(max_x+5-BORDER);
		for(int k=0;k<items.length;k++) {
			if(items[k]<clipLeftTs||items[k]>clipRightTs) continue;
			currentTop = coUtil.unit2CordHeight(intCodes[k],size,this.getHeight(),BORDER) + unitHeight / 2;
			if(currentTop>min_y-5&&currentTop<max_y+5) {

				assignColorByItem(k, gBuf);
				if(clicked) {
					uiUtil.paintItem(coUtil.time2coord(items[k]) + BORDER - coUtil.getClipL(), currentTop, gBuf, assignShapeByItem(k));
					continue;
				} else {
					uiUtil.paintItemExt(coUtil.time2coord(items[k]) + BORDER - coUtil.getClipL(), currentTop, gBuf, assignShapeByItem(k));
				}
	...
	private String hasDots(Point p1, Point p2)
	{
		...
		for(long j=left; j<=right;j++){
			...
			for(String str:tempMap.keySet()){
				int diameter = 5;
				if(dcop.isSizeCheckBoxSelected()) diameter = uiUtil.getDiameter(tempMap.get(str).size());
				if(dcop.getSortStandard().equals(DottedChartModel.STR_NONE)) {
					currentTop = coUtil.unit2CordHeight(Integer.valueOf(str), size, this.getHeight(), BORDER) + unitHeight / 2;
				} else {
					currentTop = coUtil.unit2CordHeight(mapping.get((int)Integer.valueOf(str)),size,this.getHeight(),BORDER) + unitHeight / 2;
				}
				if(currentTop>min_y-diameter&&currentTop<max_y+diameter) {
					long temp = j*dcModel.getProperTimeUnit(dcop.getTimeOption(),coUtil.getMilli2pixels());
				//	if(dcop.getTimeOption().equals(DottedChartModel.TIME_ACTUAL)) temp -= timeOffset;
					int pointX = coUtil.time2coord(temp);
					if(pointX>=min_x-diameter-BORDER&&pointX<=max_x+diameter-BORDER) {
						assignColorByItem(tempMap.get(str), gBuf);
						if(clicked) {
							uiUtil.paintItemHigh(coUtil.time2coord(temp) + BORDER - coUtil.getClipL(), currentTop, gBuf, ITEM_HANDLE_CIRCLE, tempMap.get(str).size());
							continue;
						} else {
							uiUtil.paintItemHighExt(coUtil.time2coord(temp) + BORDER - coUtil.getClipL(), currentTop, gBuf, ITEM_HANDLE_CIRCLE, tempMap.get(str).size());
						}
	...
	{
		String str = "";
			str += dcModel.getInstance(index)+
			" | " + dcModel.getTask(index)
   ...
   public void mousePressed(MouseEvent e) {
	   ...
	   if(clicked&&dcop.getMouseMode().equals(ST_DRAG)){
			...
			p1 = e.getPoint();
			p2 = e.getPoint();
			if(p3!=null&&p4!=null) makeTooltip(p3,p4);
			repaint();
			p3=e.getPoint();
		}
		else if(dcop.getMouseMode().equals(ST_ZOOMIN)){
