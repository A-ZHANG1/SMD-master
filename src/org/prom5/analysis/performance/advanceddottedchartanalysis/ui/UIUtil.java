...
public class UIUtil {
	...
	public void paintItemHigh(int x, int y, Graphics g, String shape, int number) {
		...
		if(shape.equals(DottedChartPanel.STR_NONE)) {
			...
			g.fillOval(x-size(number), y-size(number), size(number)*2, size(number)*2);
			...
			g.fillOval(x-(5+size-1), y-(5+size-1), (5+size-1)*2+1, (5+size-1)*2+1);
	...
	public void paintItemHighExt(int x, int y, Graphics g, String shape, int number) {
		int size = size(number);
		if(shape.equals(DottedChartPanel.ITEM_HANDLE_CIRCLE)) {
			g.fillOval(x-(5+size-1), y-(5+size-1), (5+size-1)*2+1, (5+size-1)*2+1);
			g.setColor(Color.white);
			g.fillOval(x-(5+size-3), y-(5+size-3), (5+size-3)*2+1, (5+size-3)*2+1);
		} 
	}
	 ...
	 * @param g the Graphics object used for painting
	 */
	protected void paintItem_buffer(int x, int y, Graphics g, String shape, int number) {
		if(shape.equals(DottedChartPanel.STR_NONE)) {
			return;
		}
		else if(shape.equals(DottedChartPanel.ITEM_HANDLE_DOT)) {
...
//			g.fillOval(x-2, y-2, 7, 7);
			g.fillOval(x-(2+sizeBuffer(number)), y-(2+sizeBuffer(number)), (2+sizeBuffer(number))*2+1, (2+sizeBuffer(number))*2+1);
		} else if(shape.equals(DottedChartPanel.ITEM_HANDLE_RHOMBUS)) {
	...
	public Color getAverageColor(Color colorArray[])
	{
		...
		return new Color(valueRed/colorArray.length, valueGreen/colorArray.length, valueBlue/colorArray.length);		
	}
