...
public abstract class AbstractChart implements GuiNotificationTarget {
	 ...
	 * Creates a new profile with the given name and description
	 */
	public AbstractChart(String aName, String aDescription, BasicPerformanceAnalysisUI aPerformanceModel) {
	...
	protected void initTimeSort()
	{
		...
		timeList.add("months");
		timeList.add("years");
		timeUnitSort = new GUIPropertyListEnumeration("Time Unit:", "", timeList, this, 150);
	...
	protected void initMeasureSort()
	{
		...
		measureSort = new GUIPropertyListEnumeration("Measure:", "", measureList, this, 150);
	}
	...
	protected void initDimSort()
	{
		...
		dim1Sort = new GUIPropertyListEnumeration("1st Dim:", "", dim1List, this, 150);
		...
		dim2Sort = new GUIPropertyListEnumeration("2st Dim:", "", dim2List, this, 150);
	}
	...
	protected JPanel getOptionPanel()
	{
		...
		performanceSort = new GUIPropertyListEnumeration("Performance Sort:", "", performanceList, this, 150);
	...
	protected void addDate(DefaultCategoryDataset dataset, StatisticUnit su, String rowKey, String columnKey)
	{
		String sort = (String) measureSort.getValue();
		if(sort.equals("Minimum")) {
			dataset.addValue(su.getStatistics().getMin()/getTimeUnit(), rowKey, columnKey);
		} else if(sort.equals("Average")) {
			dataset.addValue(su.getStatistics().getMean()/getTimeUnit(), rowKey, columnKey);
		} else if(sort.equals("Median")) {
			dataset.addValue(su.getStatistics().getPercentile(50)/getTimeUnit(), rowKey, columnKey);
		} else if(sort.equals("Maximum")) {
			dataset.addValue(su.getStatistics().getMax()/getTimeUnit(), rowKey, columnKey);
		} else if(sort.equals("Sum")) {
			dataset.addValue(su.getStatistics().getSum()/getTimeUnit(), rowKey, columnKey);
		} else if(sort.equals("StandDev")) {
			dataset.addValue(su.getStatistics().getStandardDeviation()/getTimeUnit(), rowKey, columnKey);
		} else if(sort.equals("Frequency")) {
			dataset.addValue(su.getStatistics().getN()/getTimeUnit(), rowKey, columnKey);
		}
	}
		...
		if(!absPerformance.getName().equals(InstancePerformance.getNameCode())){
			if (dim1Sort.getValue().equals("task")) {
				if(dim2Sort.getValue().equals("task")) {
					absPerformance = performanceUI.getPerformance(TaskTPerformance.getNameCode());
					isItemA = true;
				}
				else if (dim2Sort.getValue().equals("originator")) {
					absPerformance = performanceUI.getPerformance(TaskOriginatorTPerformance.getNameCode());
					isItemA = false;
				}
				...
				if(dim2Sort.getValue().equals("task")) {
					...
					absPerformance = performanceUI.getPerformance(InstanceTPerformance.getNameCode());
				}
				isItemA = false;
				...
				if(dim2Sort.getValue().equals("task")) {
					...
					absPerformance = performanceUI.getPerformance(OriginatorTPerformance.getNameCode());
				}
				else if (dim2Sort.getValue().equals("instance")) {
					absPerformance = performanceUI.getPerformance(InstOriTPerformance.getNameCode());
				}
