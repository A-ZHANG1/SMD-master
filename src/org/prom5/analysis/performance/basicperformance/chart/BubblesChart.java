...
public class BubblesChart extends AbstractChart {
	...
	public JPanel getPanel(AbstractPerformance absPer)
	{
		...
		NormalizedMatrixSeries newSeries =
            new NormalizedMatrixSeries("frequency", absPer2D.getItems().size(),absPer2D.getSecondItems().size());
		for (int i = 0; i < absPer2D.getItems().size(); i++) {
			...
			for (int j = 0; j < absPer2D.getSecondItems().size(); j++) {
				...
				StatisticUnit su = absPer2D.getExecutionTimeSU(task+" - "+originator); 
		...
		final MatrixSeriesCollection dataset2 = new MatrixSeriesCollection(newSeries);
