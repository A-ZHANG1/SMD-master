...
public class GanttChart extends AbstractChart {
	...
	public void initGUI() {
		...
		displaySort = new GUIPropertyListEnumeration("Display:", "", displayList, this, 150);
	}
	...
	{	
		...
		if(isItemA) {
			...
			for(TaskSeries ts: performance2D.getFirstTaskSeriesMap().values())
		    {
				...
				chart = ChartFactory.createGanttChart(
						String.valueOf(ts.getKey()),  // chart title
			            label,              // domain axis label
			            "Date",              // range axis label
			            dataset,  			// data
				...
				tempChartPanel.setPreferredSize(new java.awt.Dimension(250, 160));
		        jtempPanel.add(tempChartPanel);
		    }
			if(performance2D.getSecondItems().size()<10) bLegend = true;

		} 
		else {
			label = performance2D.getItemName();
			for(TaskSeries ts: performance2D.getSecondTaskSeriesMap().values())
		    {
			...
			if(performance2D.getItems().size()<10) bLegend = true;
		}
	...
	{	
		...
		if(absPerformance==null) {
			JTextArea textarea = new JTextArea("Two different dimentions are required");
			panel.add(textarea);
		...
		number = Math.max(performance2D.getSecondTaskSeriesMap().values().size(),
					performance2D.getFirstTaskSeriesMap().values().size());
	...
	protected JPanel getOptionPanel()
	{
		...
		} else if(!absPerformance.getName().equals(InstancePerformance.getNameCode())){
			panel.add(displaySort.getPropertyPanel());
			panel.add(dim1Sort.getPropertyPanel());
			panel.add(dim2Sort.getPropertyPanel());
		}
...
//						isItemA = false;
			...
			} else if (performanceUI.isInstanceDim()&&dim1Sort.getValue().equals("instance")) {
				if(dim2Sort.getValue().equals("task")) {
					absPerformance = performanceUI.getPerformance(InstanceTaskTPerformance.getNameCode());
				} else if (dim2Sort.getValue().equals("originator")) {
...
//					absPerformance = performanceUI.getPerformance(InstanceTaskTPerformance.getNameCode());
				...
				isItemA = false;
			} else if (performanceUI.isOriginatorDim()&&dim1Sort.getValue().equals("originator")){
				if(dim2Sort.getValue().equals("task")) {
					absPerformance = performanceUI.getPerformance(TaskOriginatorTPerformance.getNameCode());
				} else if (dim2Sort.getValue().equals("originator")) {
					absPerformance = null;
