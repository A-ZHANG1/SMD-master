...
public class MeterChart extends AbstractChart implements ActionListener {
	...
	public ChartPanel drawMeterPlot(String name, double value)
	{
        ...
        getMinMaxValue();
        MeterInterval mInterval = new MeterInterval("Normal",new Range(minValue, normalValue),
        		Color.lightGray, new BasicStroke(2.0f), new Color(0, 255, 0, 64));
        MeterInterval wInterval = new MeterInterval("warning",new Range(normalValue, warningValue),
                Color.lightGray, new BasicStroke(2.0f), new Color(255, 255, 0, 64));
        MeterInterval cInterval = new MeterInterval("critical",new Range(warningValue, maxValue),
	...
	public JScrollPane getPanel2(AbstractPerformance absPer)
	{
		...
		for (String aItem:items) {
			...
			if(su!=null&&su.getStatistics().getN()!=0) {
				ChartPanel chartPanel = drawMeterPlot(aItem, getMeasrueValue(su.getStatistics()));
	...
	public JPanel drawMeterPanel(List<String> items, List<String> items2)
	{
		...
		for (String aItem: items) {
			for (String bItem:items2) {
				String key;
				if(isItemA)	key = bItem+" - "+aItem; 
				else key = aItem+" - "+bItem;
				StatisticUnit su;
				if(performanceSort.getValue().equals("Working Time")) {
					su = ((AbstractPerformance2D) absPerformance).getExecutionTimeSU(key);
				} else {
					su = ((AbstractPerformance2D) absPerformance).getWaitingTimeSU(key);
				}
				if(su!=null&&su.getStatistics().getN()!=0) {
		...
		for (String aItem: items) {
			for (String bItem:items2) {
				...
				if(su!=null&&su.getStatistics().getN()!=0) {
					ChartPanel chartPanel = drawMeterPlot(aItem+" - "+bItem, getMeasrueValue(su.getStatistics()));
					jtempPanel.add(chartPanel);
				}
			}
		}
