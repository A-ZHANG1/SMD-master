...
public class TableChart extends AbstractChart implements ActionListener{
	...
	public void itemStateChanged(ItemEvent e) {
		...
		if ((source==minCheckBox)||(source==medianCheckBox)||(source==maxCheckBox)) {
	...
	private StringBuffer makeHeader(String str) {
		...
		if(avgCheckBox.isSelected()){
			tableHeader.append(tag("Average ("+str+")", TH)); // add Average column header
		}
		if(freqCheckBox.isSelected()){
			tableHeader.append(tag("Frequency ("+str+")", TH)); // add Frequency column header
		}
		if(stdCheckBox.isSelected()){
			tableHeader.append(tag("Stadev ("+str+")", TH)); // add Standard Deviation column header
		}
		if(minCheckBox.isSelected()){
			tableHeader.append(tag("Min ("+str+")", TH)); // add Min column header
		}
		if(medianCheckBox.isSelected()){
			tableHeader.append(tag("Median ("+str+")", TH)); // add Median column header
		}
		if(maxCheckBox.isSelected()){
			tableHeader.append(tag("Max ("+str+")", TH)); // add Max column header
		}
		return tableHeader;
	}
	...
	private String makeRow(DescriptiveStatistics ds) {
		String temp="";
		if(avgCheckBox.isSelected()){
			temp += tag(String.valueOf(round2(ds.getMean()/getTimeUnit())), TD);
		}
		if(freqCheckBox.isSelected()){
			temp += tag(String.valueOf(round2(ds.getN())), TD);
		}
		if(stdCheckBox.isSelected()){
			temp += tag(String.valueOf(round2(ds.getStandardDeviation()/getTimeUnit())), TD);
		}		
		if(minCheckBox.isSelected()){
			temp += tag(String.valueOf(round2(ds.getMin()/getTimeUnit())), TD);
		}
		if(medianCheckBox.isSelected()){
			temp += tag(String.valueOf(round2(ds.getPercentile(50)/getTimeUnit())), TD);
		}
		if(maxCheckBox.isSelected()){
	...
	protected JScrollPane getGraphPanel()
	{
		...
		if(absPerformance instanceof AbstractPerformance2D) {
			content = this.getHtmlRepresentation((AbstractPerformance2D)absPerformance);
		}
		else {
