package org.prom5.analysis.performance.basicperformance.chart;
...
import org.prom5.analysis.performance.basicperformance.BasicPerformanceAnalysisUI;
...
public class XYBlockChart extends AbstractChart {
	...
	public void initGUI()
	{
		...
		measSort = new GUIPropertyListEnumeration("Measure:", "", measList, this, 150);
		ArrayList<String> lineList = new ArrayList<String>();
		lineList.add("step");
		lineList.add("curve");
		lineType = new GUIPropertyListEnumeration("Line type:", "", lineList, this, 150);
		...
		dim1Sort = new GUIPropertyListEnumeration("Display:", "", displayList, this, 150);
	}
	...
	// generating data set ///////////////////////////////////////////////////////////////////////
	private ArrayList<XYDataset> getXYBlockChartDataset(HashMap<String,TaskSeries> taskSeriesMap)
	{
		...
		for(String key : taskSeriesMap.keySet())
	    {
			...
			{
				Date currentDate = new Date(startDate.getTime() + j * getPeriodUnit());
				double num=0;
				if(!performanceUI.isConsiderHoliday()||holidays[j]) {
					Date date1 = new Date((currentDate.getTime()/getPeriodUnit())*getPeriodUnit());
					Date date2 = new Date((currentDate.getTime()/getPeriodUnit()+1)*getPeriodUnit());
					for(int i=0; i<ts.getTasks().size();i++){
						Task task = ts.get(i);
						SimpleTimePeriod stp = (SimpleTimePeriod) task.getDuration();
						
						if(stp.getStart().before(date1)&&stp.getEnd().after(date2)) {
							if(periodUnit instanceof Day) {
								num+=(double)(endTime.getValue()-startTime.getValue())*(double)3600000L/(double)getTimeUnit();
							} else {
								num+=(double)getPeriodUnit()/(double)getTimeUnit();
							}
						} else if((stp.getStart().equals(date1)||stp.getStart().after(date1))
								&&(stp.getEnd().equals(date2)||stp.getEnd().before(date2))){
							double diff = ((double)(stp.getEnd().getTime()-stp.getStart().getTime()))/(double)getTimeUnit();
							num+=Math.max(0.0, diff);
						} else if((stp.getStart().equals(date1)||stp.getStart().after(date1))
								&&stp.getStart().before(date2)){
							double diff = ((double)(date2.getTime()-stp.getStart().getTime()))/(double)getTimeUnit();
							if(getPeriodUnit()==86400000L) {
								diff = diff - (double)(24-endTime.getValue())*(double)3600000L/(double)getTimeUnit();
							}
							num+=Math.max(0, diff); // 24-6
						}
						else if((stp.getEnd().equals(date1)||stp.getEnd().after(date1))
								&&stp.getEnd().before(date2)){
							double diff = ((double)(date1.getTime()-stp.getEnd().getTime()))/(double)getTimeUnit();
							if(getPeriodUnit()==86400000L) {
								diff = diff - (double)startTime.getValue()*(double)3600000L/(double)getTimeUnit();
							}
	...
	private ArrayList<XYDataset> getXYBlockChartDataset_overall(HashMap<String,TaskSeries> taskSeriesMap)
	{
		ArrayList<XYDataset> datasetList = new ArrayList<XYDataset>();
	...
	private ArrayList<XYDataset> getXYBlockChartDataset_freq(HashMap<String,TaskSeries> taskSeriesMap)
	{
		ArrayList<XYDataset> datasetList = new ArrayList<XYDataset>();
	...
	private ArrayList<XYDataset> getXYBlockChartDataset_freq_overall(HashMap<String,TaskSeries> taskSeriesMap)
	{
		ArrayList<XYDataset> datasetList = new ArrayList<XYDataset>();
		sum = 0.0;
		TimeSeries series1, series2, series3;
	...
	protected JScrollPane getGraphPanel()
	{
		...
		if(dim1Sort.getValue().equals("overall")){
			if (measSort.getValue().equals("frequency")) {
				...
				scrollPane = new JScrollPane(getChartPanel(datasetList.get(0),null));
			}
		}else {			
			...
			for(String key: tsMap.keySet())
		    {
				...
				if (measSort.getValue().equals("frequency")) {
					chartPanel = getChartPanel(datasetList.get(k*3),datasetList.get(k*3+1),datasetList.get(k*3+2),key);
				} else {
	        ...
	        tempPanel.setPreferredSize(new java.awt.Dimension(250*3, 160*(k/3+1)));
	...
	private ChartPanel getChartPanel(XYDataset dataSet, String key)
	{
		if(lineType.getValue().equals("step")){
			chart = ChartFactory.createXYStepChart(
		            null, key, null, dataSet, PlotOrientation.VERTICAL, false, true, false);
		} else {
			chart = ChartFactory.createTimeSeriesChart(
		            null, key, null, dataSet, false, true, false); 
		}
	...
	private ChartPanel getChartPanel(XYDataset dataSet,XYDataset dataSet1,XYDataset dataSet2, String key)
	{
		if(lineType.getValue().equals("step")){
			chart = ChartFactory.createXYStepChart(
		            null, key, null, dataSet, PlotOrientation.VERTICAL, true, true,false);
		} else {
			chart = ChartFactory.createTimeSeriesChart(
		            null, key, null, dataSet, true, true, false); 
		}
	...
	protected JPanel getOptionPanel()
	{
		...
		if(performanceUI.isConsiderWorkingHour()) {
			startTime = new GUIPropertyInteger("Start time:",performanceUI.getStartHour(),0,24,this);
			endTime = new GUIPropertyInteger("End time:",performanceUI.getEndHour(),0,24,this);
		} else {
		...
		if(absPerformance!=null&&!absPerformance.getName().equals(InstancePerformance.getNameCode())){
	...
	{ 	
		if(absPerformance instanceof AbstractPerformance2D) {
			...
			if(dim1Sort.getValue().equals("overall")||dim1Sort.getValue().equals("task")) {
				absPerformance = performanceUI.getPerformance(InstanceTaskTPerformance.getNameCode());
				tsMap = ((AbstractPerformance2D)absPerformance).getSecondTaskSeriesMap();
			}
			if(dim1Sort.getValue().equals("instance")){
				absPerformance = performanceUI.getPerformance(InstanceTaskTPerformance.getNameCode());
				if(absPerformance!=null){
					tsMap = ((AbstractPerformance2D)absPerformance).getFirstTaskSeriesMap();
				} else {
					absPerformance = performanceUI.getPerformance(InstOriTPerformance.getNameCode());
					tsMap = ((AbstractPerformance2D)absPerformance).getFirstTaskSeriesMap();
				}
			}
