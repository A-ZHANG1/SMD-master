...
public class XYBlockHourChart extends AbstractChart implements GuiNotificationTarget{
	...
	public void initGUI()
	{
		...
		colorSort = new GUIPropertyListEnumeration("Color base:", "", colorList, this, 150);
		...
		dim1Sort = new GUIPropertyListEnumeration("Dim:", "", displayList, this, 150);

	}

	private ArrayList<DefaultXYZDataset> getXYBlockChartDatasetList(HashMap<String,TaskSeries> tsMap)
	{
		...
		for(String key : tsMap.keySet())
	    {
			...
			for(int i=0; i<ts.getTasks().size();i++){
				...
				int day1 = (int)((stp.getStart().getTime()/86400000L) - (startDate.getTime()/86400000L));
				int hour1 = (int)((stp.getStart().getTime() - (stp.getStart().getTime()/86400000L)*86400000L)/3600000L);
				double time1 = (double)(stp.getStart().getTime() - (stp.getStart().getTime()/3600000L)*3600000L)/((double)getTimeUnit());
				time1 = ((double)(3600000L - time1))/(double)getTimeUnit();
				int day2 = (int)((stp.getEnd().getTime()/86400000L) - (startDate.getTime()/86400000L));
				int hour2 = (int)((stp.getEnd().getTime() - (stp.getEnd().getTime()/86400000L)*86400000L)/3600000L);
				double time2 = ((double)(stp.getEnd().getTime() - (stp.getEnd().getTime()/3600000L)*3600000L))/((double)getTimeUnit());
		...
		if(endDateOverall!=null&&startDateOverall!=null) {
			duration = (int)((endDateOverall.getTime()/86400000L-startDateOverall.getTime()/86400000L))+1;
		}
		else {
			duration = -1;
		}
        ...
        for(TaskSeries ts: tsMap.values())
	    {
			for(int i=0; i<ts.getTasks().size();i++){
				...
				int day1 = (int)((stp.getStart().getTime()/86400000L) - (startDateOverall.getTime()/86400000L));
				int hour1 = (int)((stp.getStart().getTime() - (stp.getStart().getTime()/86400000L)*86400000L)/3600000L);
				double time1 = (double)(stp.getStart().getTime() - (stp.getStart().getTime()/3600000L)*3600000L)/((double)getTimeUnit());
				time1 = ((double)(3600000L - time1))/(double)getTimeUnit();
				int day2 = (int)((stp.getEnd().getTime()/86400000L) - (startDateOverall.getTime()/86400000L));
				int hour2 = (int)((stp.getEnd().getTime() - (stp.getEnd().getTime()/86400000L)*86400000L)/3600000L);
				double time2 = ((double)(stp.getEnd().getTime() - (stp.getEnd().getTime()/3600000L)*3600000L))/((double)getTimeUnit());
	...
	private ArrayList<DefaultXYZDataset> getXYBlockChartDatasetList_freq(HashMap<String,TaskSeries> tsMap)
	{
	...
	{	
		...
		if(dim1Sort.getValue().equals("overall")){
			...
			tempPanel.setPreferredSize(new java.awt.Dimension(300*2, 230*(datasetList.size()/3+1)));
		}	
	...
	private ChartPanel getChartPanel(DefaultXYZDataset dataset, int i)
	{
        ...
        if(colorSort.getValue().equals("gray")) //gray scaling
        {
            ...
            { 
          		((LookupPaintScale)paintScale).add(new Double(k+0.5), new Color(color_max-(k+1)*d));
            }
        }
