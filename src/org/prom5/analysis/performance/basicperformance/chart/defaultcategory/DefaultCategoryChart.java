...
public class DefaultCategoryChart extends AbstractChart {
	...
	public DefaultCategoryChart(String name,String desc, BasicPerformanceAnalysisUI pm) {
		super(name, desc, pm);
		init2D3DSort();
	}
	...
	public void init2D3DSort()
	{
		...
		dimList.add("2D");
		dimList.add("3D");
		dimSort = new GUIPropertyListEnumeration("Dimension:", "", dimList, this, 150);		
	}
	
	// Default Category Dataset //////////////////////////////////////////////////////////////////////
	protected DefaultCategoryDataset getDefaultCategoryDataset(AbstractPerformance2D performance2D)
	{
		...
		if(isItemA){
			if(performanceSort.getValue().equals("Working Time")){
				...
				for (String aItem: items) {
					for (String bItem: items2) {
						StatisticUnit su = performance2D.getSojournTimeSU(aItem+" - "+bItem); 
						if(su!=null&&su.getStatistics().getN()!=0) {
							addDate(dataset,su, aItem,bItem);
						}
					}
				...
				for (String aItem: items) {
					for (String bItem: items2) {
						...
						su = performance2D.getSojournTimeSU(aItem+" - "+bItem);
						if(su!=null&&su.getStatistics().getN()!=0) {
	...
	protected DefaultCategoryDataset getDefaultCategoryDataset(AbstractPerformance performance)
	{
		List<String> items = sortItems(performance.getItems(),performance.getExecutionMap());
	...
	protected List<String> sortItems(List<String> items,HashMap<String, StatisticUnit> map)
	{
		for (int i=0; i< items.size()-1;i++) {
			for( int k=i+1; k< items.size();k++) {
				if(getValue(map.get(items.get(i)))>getValue(map.get(items.get(k)))){
