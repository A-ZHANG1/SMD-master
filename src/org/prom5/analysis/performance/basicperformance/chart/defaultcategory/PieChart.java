package org.prom5.analysis.performance.basicperformance.chart.defaultcategory;
...
public class PieChart extends DefaultCategoryChart {
	...
	protected ArrayList<DefaultPieDataset> getPieDatasetList(AbstractPerformance2D performance2D)
	{
		...
		if(isItemA){
			...
			if(items.size()>20) bLegend = false;
		} else {
			if(performanceSort.getValue().equals("Working Time")){
				for (String bItem: items2) {
					DefaultPieDataset dpd = new DefaultPieDataset();
	...
	protected void addData(DefaultPieDataset dataset, StatisticUnit su, String columnKey)
	{
		String sort = (String) measureSort.getValue();
		if(sort.equals("Minimum")) {
			dataset.setValue(columnKey, su.getStatistics().getMin()/getTimeUnit());
		} else if(sort.equals("Average")) {
			dataset.setValue(columnKey, su.getStatistics().getMean()/getTimeUnit());
		} else if(sort.equals("Median")) {
			dataset.setValue(columnKey, su.getStatistics().getPercentile(50)/getTimeUnit());
		} else if(sort.equals("Maximum")) {
			dataset.setValue(columnKey, su.getStatistics().getMax()/getTimeUnit());
		} else if(sort.equals("Sum")) {
			dataset.setValue(columnKey, su.getStatistics().getSum()/getTimeUnit());
		} else if(sort.equals("StandDev")) {
			dataset.setValue(columnKey, su.getStatistics().getStandardDeviation()/getTimeUnit());
		} else if(sort.equals("Frequency")) {
			dataset.setValue(columnKey, su.getStatistics().getN()/getTimeUnit());
		}
	}
	...
	protected JScrollPane getGraphPanel()
	{
		...
		if(absPerformance instanceof AbstractPerformance2D) {
			int num=0;
			num= (isItemA)?absPerformance.getItems().size():((AbstractPerformance2D)absPerformance).getSecondItems().size();
			if (num>10) {
				...
				if(isItemA){
					...
					items = ((AbstractPerformance2D)absPerformance).getSecondItems();
				}
				int numChart=0;
				for (int i=0;i< items.size();i++) {
					if(dataset2.get(i).getItemCount()<1) continue;
					numChart++;
					boolean legend = (dataset2.get(i).getItemCount()>10)?false:true; 
					 JFreeChart chart1 = ChartFactory.createPieChart(items.get(i), dataset2.get(i), 
							 legend, false, false);
				...
				tempPanel.setPreferredSize(new java.awt.Dimension(280*3, 180*(numChart/3+1)));
				...
				if(isItemA){
					if(((AbstractPerformance2D)absPerformance).getSecondItems().size()>20)
						bLegend = false;
				} else 
				{
		...
		if(dimSort.getValue().equals("2D")) {
			chart = ChartFactory.createMultiplePieChart(
					null, dataset, order, bLegend, true, false
		        );	
