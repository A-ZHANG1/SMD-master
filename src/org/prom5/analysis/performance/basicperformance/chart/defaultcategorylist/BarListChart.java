...
public class BarListChart extends DefaultCategoryListChart {
	...
	public void initGUI() {
		...
		orientationSort = new GUIPropertyListEnumeration("Orientation:", "", orientationList, this, 150);
	}
	...
	protected JScrollPane getGraphPanel_Single()
	{
		...
		if(dimSort.getValue().equals("2D")) {
			chart = ChartFactory.createBarChart(
						null, strCategory,null,dataset, order, bLegend, true, false
	        			);
	...
	protected JScrollPane getGraphPanel_Multi()
	{
		...
		if(datasetArray.size()==1){
			...
			tempPanel.setPreferredSize(new java.awt.Dimension(250*3, 160*(numChart/3+1)));
	...
	private void addChart(JPanel panel, DefaultCategoryDataset dataset)
	{
		...
		if(dimSort.getValue().equals("2D")) {
			chart = ChartFactory.createBarChart(
					aItem, strCategory,null,dataset, order, false, true, false
	    			);
		} else {
			 chart = ChartFactory.createBarChart3D(
					aItem, strCategory,null,dataset, order, false, true, false
