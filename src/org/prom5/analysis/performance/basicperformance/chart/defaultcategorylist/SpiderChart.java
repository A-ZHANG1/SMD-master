...
public class SpiderChart extends DefaultCategoryListChart {
	...
	protected JScrollPane getGraphPanel()
	{
		...
		if(datasetArray.size()==1){
			SpiderWebPlot plot = new SpiderWebPlot(datasetArray.get(0),TableOrder.BY_ROW);
			JFreeChart jchart = new JFreeChart(absPerformance.getName(), new Font("SansSerif", Font.PLAIN, 8), plot, false);
			...
			for (int i=0;i<datasetArray.size();i++) {
				if(datasetArray.get(i).getRowCount()==0) continue;
				String aItem = (String)datasetArray.get(i).getRowKey(0); 
				SpiderWebPlot plot = new SpiderWebPlot(datasetArray.get(i),TableOrder.BY_ROW);
				JFreeChart jchart = new JFreeChart(aItem, new Font("SansSerif", Font.PLAIN, 8), plot, false);
				ChartPanel chartPanel = new ChartPanel(jchart);
				chartPanel.setPreferredSize(new java.awt.Dimension(280, 180));
				tempPanel.add(chartPanel);
			}
			tempPanel.setPreferredSize(new java.awt.Dimension(280*3, 180*(datasetArray.size()/3+1)));
