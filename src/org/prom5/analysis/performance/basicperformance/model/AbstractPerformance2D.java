package org.prom5.analysis.performance.basicperformance.model;
...
/**
 * This class defines the interface for a performance, in which for
 * a number of performance values are stored. 
 *
...
public abstract class AbstractPerformance2D extends AbstractPerformance {
	 ...
	 */
	public AbstractPerformance2D(String aName, String aNameItem1, String aNameItem2, String aDescription) {
	 ...
	 * Creates a new profile with the given name and description
	 */
	public AbstractPerformance2D(String aName, String aNameItem1, String aNameItem2, String aDescription, List<String> aItems, List<String> bItems) {
	...
	public void setRefPerformance(AbstractPerformance aRefPerformance, AbstractPerformance bRefPerformance)
	{
	...
	public void addTaskSeries(String aItem, String bItem, Date start, Date end)
	{
		...
		try {
			if(taskSeries.get(bItem)==null) {
				taskSeries.add(new Task(bItem,new SimpleTimePeriod(start,end)));
			}
			else {
				taskSeries.add(new Task(bItem+"_"+indexGannt++,new SimpleTimePeriod(start,end)));
			}
			
			TaskSeries taskSeries2 = item2SeriesMap.get(bItem);
			if(taskSeries2.get(aItem)==null) {
				taskSeries2.add(new Task(aItem,new SimpleTimePeriod(start,end)));
			}
			else {
				taskSeries2.add(new Task(aItem+"_"+indexGannt++,new SimpleTimePeriod(start,end)));
			}
		} catch (Exception ce)
		{
	...
	public void addTaskSeriesWaiting(String aItem, String bItem, Date start, Date end)
	{
	...
	public void addTaskSeriesSojourn(String aItem, String bItem, Date start, Date end)
	{
