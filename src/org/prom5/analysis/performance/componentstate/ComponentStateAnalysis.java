...
package org.prom5.analysis.performance.componentstate;
...
import org.prom5.analysis.performance.componentstate.ui.OverviewPanel;
import org.prom5.analysis.performance.componentstate.ui.SettingPanel;
import org.prom5.analysis.performance.dottedchart.ui.MetricsPanel;
...
public class ComponentStateAnalysis extends JPanel implements Provider{
	...
	private int[] selectedInstanceIndices; // indices of the process instances used
	...
	public ComponentStateAnalysis(ComponentStatePlugin algorithm, AnalysisInputItem[] input, LogReader log) {
	...
	/**
	 * Creates the thread in which the log relations are mined out from the used log
	 */
	public void analyse() {
	...
	public void jbInit() {
		...
		chartPane = new JScrollPane(componentStatePanel, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
...
//		============================================================================================//
		...
		/**
		 * split = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, chartPane, metricsPane);
...
//		 ============================================================================
		...
		settingPanel = new SettingPanel(inputLog, this, componentStatePanel, csModel);
	...
	public ProvidedObject[] getProvidedObjects() {
		try {
			if (selectedInstanceIndices != null) {
				...
				ProvidedObject[] objects = new ProvidedObject[] {new ProvidedObject("Component State",
										   new Object[] {inputLog})};
				return objects;
			}
		} catch (Exception e) {
...
////////////////////////////
	...
	public void initOptions(){
		...
		eventTypeToKeep = new ArrayList(Arrays.asList(inputLog.getLogSummary().getEventTypes()));
	...
	public void setScrollBarPosition(Point aP)
	{
		...
		if(aP.x<0) aP.x=0;
		if(aP.y<0) aP.y=0;
		if ((aP.x+chartPane.getViewport().getSize().getWidth())>componentStatePanel.getWidth())
			aP.x = (int) (componentStatePanel.getWidth() - chartPane.getViewport().getSize().getWidth());
		if ((aP.y+chartPane.getViewport().getSize().getHeight())>componentStatePanel.getHeight())
			aP.y = (int) (componentStatePanel.getHeight() - chartPane.getViewport().getSize().getHeight());
