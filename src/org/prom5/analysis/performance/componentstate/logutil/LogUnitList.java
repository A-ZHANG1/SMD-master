package org.prom5.analysis.performance.componentstate.logutil;
...
/**
 * This class represents a Data Modification Cluster (LogUnitList).
 * Data modification clusters extracted in the initial scan pass.
...
//	protected Date endDate = null;
	...
	public int size(ArrayList<String> eventTypeToKeep, ArrayList instanceIDs) {
		...
		for(Iterator itr = events.iterator();itr.hasNext();)
		{
			...
			if(eventTypeToKeep.contains(abstLogUnit.getType())&&instanceIDs.contains(abstLogUnit.getProcessInstance().getName()))
				num++;
		}
		return num;
	}
	...
	/**
	 * tests with equality to another LogUnitList
	 * WARNING: As 'AuditTrailEntry' does so far not override the 'equals()' method this
	 * method does, as well, not check for real value equality. This needs, if necessary,
	 * to be fixed in AuditTrailEntry.
	 * @param obj
	 * @return
	 */
	...
	/**
	 * deep check whether a given AbstractLogUnit is contained
	 * within this LogUnitList's event set
	 * @param entry
	 * @return
	 */
	 ...
	 */
	public Date getLeftBoundaryTimestamp(ArrayList<String> eventTypeToKeep, ArrayList instanceIDs) {
	...
	/**
	public Date getLeftBoundaryTimestampRelativeTime(ArrayList<String> eventTypeToKeep, ArrayList instanceIDs) {
		ArrayList<AbstractLogUnit> abst = getEvents();
		...
		for (Iterator itr = abst.iterator(); itr.hasNext(); ) {
			...
			if (eventTypeToKeep.contains(abstLogUnit.getType())
					&& instanceIDs.contains(abstLogUnit.getProcessInstance().getName())
					&& abstLogUnit.getRelativeTimestamp().before(startDate))
			{
				startDate = abstLogUnit.getRelativeTimestamp();
				flag = true;
			}
		}
		return (flag)?startDate:null;
	}

	public Date getLeftBoundaryTimestampRelativeRatio(ArrayList<String> eventTypeToKeep, ArrayList instanceIDs) {
		ArrayList<AbstractLogUnit> abst = getEvents();
		Date startDate = new Date(Long.MAX_VALUE);
		boolean flag = false;
		...
		for (Iterator itr = abst.iterator(); itr.hasNext(); ) {
			...
			if (eventTypeToKeep.contains(abstLogUnit.getType())
					&& instanceIDs.contains(abstLogUnit.getProcessInstance().getName())
					&& abstLogUnit.getRelativeRatio().before(startDate))
			{
				startDate = abstLogUnit.getRelativeRatio();
				flag = true;
			}
		}
		return (flag)?startDate:null;
	...
	public Date getRightBoundaryTimestampRelativeRatio(ArrayList<String> eventTypeToKeep, ArrayList instanceIDs) {
		ArrayList<AbstractLogUnit> abst = getEvents();
		...
		for (Iterator itr = abst.iterator(); itr.hasNext(); ) {
			...
			if (eventTypeToKeep.contains(abstLogUnit.getType()) &&
					instanceIDs.contains(abstLogUnit.getProcessInstance().getName())
					&& abstLogUnit.getRelativeRatio().after(endDate)) {
				endDate = abstLogUnit.getRelativeRatio();
				flag = true;
			}
		}
		return (flag) ? endDate : null;
	}

	public Date getRightBoundaryTimestampRelativeTime(ArrayList<String> eventTypeToKeep, ArrayList instanceIDs) {
		ArrayList<AbstractLogUnit> abst = getEvents();
		Date endDate = new Date(0);
		boolean flag = false;
		...
		for (Iterator itr = abst.iterator(); itr.hasNext(); ) {
			...
			if (eventTypeToKeep.contains(abstLogUnit.getType()) &&
					instanceIDs.contains(abstLogUnit.getProcessInstance().getName())
					&& abstLogUnit.getRelativeTimestamp().after(endDate)) {
...
*/
	 ...
	 */
	public Date getRightBoundaryTimestamp(ArrayList<String> eventTypeToKeep, ArrayList instanceIDs) {
		ArrayList<AbstractLogUnit> abst = getEvents();
		Date endDate = null;
