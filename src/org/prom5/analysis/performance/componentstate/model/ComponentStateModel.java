package org.prom5.analysis.performance.componentstate.model;
...
import org.prom5.analysis.performance.componentstate.ui.ComponentStatePanel;
...
//////	 define standard states for all components:
		...
		public static final String[] instanceStatesList = new String[]{STATE_RUNNING,STATE_ASSIGNED,STATE_SUSPENDED,STATE_COMPLETED,
																   STATE_MANUALSKIP,STATE_ABORTED};//, STATE_INITIAL, STATE_READY = out
		public static final String[] taskStatesList = new String[]{STATE_RUNNING,STATE_ASSIGNED,STATE_SUSPENDED,STATE_COMPLETED,
			   														STATE_MANUALSKIP,STATE_ABORTED};// , STATE_INITIAL, STATE_READY = out
		public static final String[] originatorStatesList = new String[]{STATE_WORKING,STATE_ASSIGNED,STATE_UNASSIGNED};// ,STATE_UNASSIGNED = out, as it not determined by a specific event
...
//			return new Date(0);
		...
		public ArrayList getHelpListElement(LogUnitList elementList){
			...
			//	 make copy after LogUnitList that contains all events of the element, to use it for finding the End event for a Lane
...
public HashMap getAssignedUsersMap(){
	return assignedUsersMap;
		...
		//	HashMap <String, ArrayList> origExternalMap = new HashMap<String, ArrayList>();
			...
			while (it.hasNext()) {
				...
				while (ates.hasNext()) {
					...
					// for eventTYPES, populate the list will all related events for each event type
					...
					// if there is a specific VARIABLE/DATA event, don't add to task list
					// propose these as specific data events!!!
					if (event.getType().equalsIgnoreCase("update")||event.getType().equalsIgnoreCase("define")
							||event.getType().equalsIgnoreCase("rollback")||event.getType().equalsIgnoreCase("confirm")){
					...
					if (!event.getType().equalsIgnoreCase("update")&& !event.getType().equalsIgnoreCase("define")
						...
						//	===========================================================================
						// for a process INSTANCE, add all events related...should we keep only PI specific events??
						// or, in case there is no PI specific event, only TASK ATEs...and exclude the DATA ate...

						if (instanceDateMap.get(event.getProcessInstance().getName()) == null ||
								instanceDateMap.get(event.getProcessInstance().getName()).after(ate.getTimestamp())) {
						...
						if (instanceEndDateMap.get(event.getProcessInstance().getName()) == null ||
								instanceEndDateMap.get(event.getProcessInstance().getName()).before(ate.getTimestamp())) {
					...
					//	if ((event.getType().equalsIgnoreCase("assign"))||(event.getType().equalsIgnoreCase("reassign"))){

						// must add the assigned users to the list of originators, with the related assign (or ANY other) event
						// assigned users may be set as "assignedUser" attributes for the ATE
						...
						while (iter.hasNext()) {
							...
							if ((assignedUserName != null)&&(assignedUserName != "")&&(assignedUser.contains("assignedUser"))) {
								if (originatorDateMap.get(assignedUserName) == null ||
										originatorDateMap.get(assignedUserName).after(ate.getTimestamp())) {
								...
								if (originatorEndDateMap.get(assignedUserName) == null ||
										originatorEndDateMap.get(assignedUserName).before(ate.getTimestamp())) {
								...
								if (originatorList.contains(assignedUserName)&&(originatorList.indexOf(assignedUserName) < logUnitList.length)) {
									// add the assign event to the originator events list
									logUnitList[originatorList.indexOf(assignedUserName)].addEvent(event);
								}
								else {  //add user to originator list if it is not equal to the empty String
									originatorList.add(assignedUserName);
									LogUnitList tempLogUnitList = new LogUnitList(assignedUserName);
									tempLogUnitList.addEvent(event);
									// assignedUser not added to "originators" array that has a fixed size, only to originatorList!
							...
							//============ originator external events===================

							// look for all originators related to the Task & PI of the  new event,
							// and add this event to their list, if it doesn't exist already
							...
							for (Iterator iter2 = originatorList.iterator(); iter2.hasNext();){
								...
								//	System.out.println(" csModel:  userEventsList = "+ userEventsList.size());							
								if ((userEventsList != null) && (userEventsList.size()>=1)){
									...
									if (!userEventsList.containsEvent(event)){
										...
										// add event only if it relates to same Element&PI, and its the First external event for this task
										// we don't know which one is the FIRST SELECTED external event!!!!!!..so we include all of them
										if (addExternal ){ 
									...
									}//else System.out.println("Orig related event already in userEventsList");
								}
			...
			for (int i = 0; i < originators.length; i++) {
				...
				// the assignedUsers were added to the originatorList, and already put into the originatorMap
			}
		}
	...
	/*	
		public ArrayList getExternalEvents(String userName){
			...
			LogUnitList userEventsList = null;
			ArrayList externalEvents = new ArrayList();
		...
		//	HashMap <String, ArrayList> origExternalMap = new HashMap<String, ArrayList>();
			...
			if (originatorMap.containsKey(userName)){
				userEventsList = (LogUnitList)originatorMap.get(userName);
			}
	
			///// ==============we have to verify for each event if its an external event, then to see if its the first :((
			// not good !!1
	...
	*/	
	...
	public Date getEndPITimestamp(String aInstance){
		...
		if (item != null) return item.getCurrentTimeStamp();
		else return null;
			...
			for (Iterator itSets = getItemMap().keySet().iterator(); itSets.hasNext(); ) {
				...
				for (Iterator itItm = tempLogUnitList.iterator(); itItm.hasNext(); ) {
					...
					if (eventTypeToKeep != null && (//!eventTypeToKeep.contains(item.getType()) ||
							 !instanceIDs.contains(item.getProcessInstance().getName()))) continue;
					if(arrayList.size()!=0){
		...
		public void adjustLogBoundaries(String timeOption) {
			...
			if (timeOption.equals(ComponentStatePanel.TIME_RELATIVE_RATIO)) {
				...
				/// if there is no event to display, make the boundary for 5 days = default
