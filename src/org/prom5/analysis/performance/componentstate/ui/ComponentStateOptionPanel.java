...
public class ComponentStateOptionPanel extends JPanel implements ChangeListener {
	...
	private String[]  timeOptions = {ComponentStatePanel.TIME_ACTUAL, ComponentStatePanel.TIME_RELATIVE_TIME,
									ComponentStatePanel.TIME_RELATIVE_RATIO, ComponentStatePanel.TIME_LOGICAL,
									ComponentStatePanel.TIME_LOGICAL_RELATIVE,}; // array of "time option" strings required to fill the timeOptionBox combobox with
	private JComboBox timeOptionBox = new JComboBox(timeOptions);
//	array of "relative time classification" strings required to fill the timeOptionBox combobox with
	private String[]  relativeTimeOptions = {ComponentStateAnalysis.ST_INST };//, ComponentStateAnalysis.ST_TASK, ComponentStateAnalysis.ST_ORIG, ComponentStateAnalysis.ST_EVEN}; 	
...
//	private JCheckBox checkLineBox = new JCheckBox("Show Lines");
	...
	public ComponentStateOptionPanel(ComponentStatePanel aCSPanel, ComponentStateAnalysis aCSA) {
		csPanel = aCSPanel;
		csAnalysis = aCSA;
...
//						csAnalysis.getMetricsPanel().displayPerformanceMetrics(); 	// update MetricsPanel
					}
		});
	...
	//					csAnalysis.getOverviewPanel().repaint();
		...
		timeOptionBox.addActionListener(new ActionListener() {
			    public void actionPerformed(ActionEvent e) {
					...
					if (type.equals(csPanel.getTimeOption()))return;
					csPanel.changeTimeOption();
					csPanel.repaint();
	...
	//				csAnalysis.getMetricsPanel().displayPerformanceMetrics();
		...
		relativeTimeOptionBox.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if(base.equals(csPanel.getRelativeTimeBase())) return;
					if(type.equals(ComponentStatePanel.TIME_RELATIVE_TIME)||type.equals(ComponentStatePanel.TIME_RELATIVE_RATIO)){
						csPanel.setTimeBaseHaspMap(csPanel.getComponentStateModel().getStartDateMap(base));
						csPanel.setEndTimeBaseHaspMap(csPanel.getComponentStateModel().getEndDateMap(base));
...
//			 we don't display the metrics now...====================================================
		//	csAnalysis.getOverviewPanel().repaint();
		}
