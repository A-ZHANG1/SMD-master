package org.prom5.analysis.performance.componentstate.ui;
...
import org.prom5.analysis.performance.componentstate.ComponentStateAnalysis;
import org.prom5.analysis.performance.componentstate.logutil.AbstractLogUnit;
import org.prom5.analysis.performance.componentstate.logutil.LogUnitList;
import org.prom5.analysis.performance.componentstate.model.ComponentStateModel;
import org.prom5.analysis.performance.dottedchart.ui.ColorReference;
import org.prom5.analysis.performance.dottedchart.ui.ShapeReference;
...
//	String componentState = "Initial";
	...
	public static String HANDLE_ICONS[] = {
		STR_NONE,
		ITEM_HANDLE_CIRCLE,
	...
	protected double milli2pixels = 1.0;	// the ratio between milliseconds and displayed pixels
	protected double height2pixels = 1.0;	// the ratio between heights and displayed pixels
...
//	protected boolean bDrawLine = false;
	...
	/**
	 * constructor
	 * @param aCSA ComponentStateAnalysis to be displayed
	 * @param aDcModel ComponentStateModel the componentstatemodel that includes the data structure
	 */
...
//		updateMilli2pixelsRatio();
	...
	public int getHashMapSize(){
		if(!csModel.getTypeHashMap().equals(ST_INST)) return csModel.getItemMap().size();
		else return csModel.getInstanceTypeToKeep().size();
	}
	...
	protected void assignColorByItem(AbstractLogUnit logUnit, Graphics g) {
		...
		if(colorStandard.equals(ComponentStateAnalysis.STR_NONE))
			...
			g.setColor(itemColors.getColor(logUnit.getProcessInstance().getName()));
	}



	protected String getStateShapeType (String aState){
		if ((aState.equals(ComponentStatePanel.STATE_ASSIGNED))||(aState.equals(ComponentStatePanel.STATE_RUNNING))||(aState.equals(ComponentStatePanel.STATE_SUSPENDED))
				||(aState.equals(ComponentStatePanel.STATE_WORKING))
				||(aState.equals(ComponentStatePanel.STATE_DEFINED)) || (aState.equals(ComponentStatePanel.STATE_UNCONFIRMED))){
			return ComponentStatePanel.STATE_SHAPE_LANE;
		}
		else if ((aState.equals(ComponentStatePanel.STATE_ABORTED)) || (aState.equals(ComponentStatePanel.STATE_COMPLETED)) || (aState.equals(ComponentStatePanel.STATE_MANUALSKIP))
				|| (aState.equals(ComponentStatePanel.STATE_UNASSIGNED))
				|| (aState.equals(ComponentStatePanel.STATE_UNDEFINED))
				|| (aState.equals(ComponentStatePanel.STATE_INITIAL))|| (aState.equals(ComponentStatePanel.STATE_READY))){ // this messes up painting the states!!!!!
	...
	/**
	 * convenience method
	 * for the Separator state, looks inside the DrawUnitList and removes the starting event of the corresponding Lane state
	 * so that the lane state will be finished drawing.
	 * @param aDrawList
	 * @param event
	 */
	...
	protected void updateDrawUnitList(LogUnitList aDrawList, AbstractLogUnit event){
		...
		if (aDrawList.size() >= 1){
			for (Iterator it = ((LogUnitList)aDrawList).iterator(); it.hasNext(); ) {
				...
				// for originator we also have to check if it is the same task & pi
				// for task/instance/data that are mapped as ATE, check also element name:
				if (event2.getElement().equals(event.getElement())
						&& event2.getProcessInstance().getName().equals(event.getProcessInstance().getName())){
...
//	AbstractLogUnit item = null;
	...
	while (indexEndItem < helpList.size()){
		...
		if (endEvent == null || !csModel.getInstanceTypeToKeep().contains(endEvent.getProcessInstance().getName())
							 || csModel.getEventTypeToKeep()==null ) {

			indexEndItem++;
		}

		// if not originator, return the next event, that is in selected list.
				...
				}else if (endEvent.getProcessInstance().getName().equals(startEvent.getProcessInstance().getName())
						&& endEvent.getElement().equals(startEvent.getElement())){
		...
		// if originator, then get next event in selected orig||task event....
		else if (csModel.getTypeHashMap().equals(ST_ORIG)){
			...
			if (endEvent.getProcessInstance().getName().equals(startEvent.getProcessInstance().getName())
				&& endEvent.getElement().equals(startEvent.getElement())){

				if (csModel.getEventTypeToKeep().contains(endEvent.getType())){

					state = getStateByEvent(endEvent, ST_ORIG);
					if ((state.equals(ComponentStatePanel.STATE_WORKING)&& endEvent.getOriginator().equals(key))
						||(state.equals(ComponentStatePanel.STATE_ASSIGNED)&& usersList!= null && usersList.contains(key))){

						//System.out.println("NEXT: "+endEvent.getType()+ " = END event found -> for ORIG selected event= "+startEvent.getType());
						return endEvent.getCurrentTimeStamp();
					}
					else if ((csModel.getSelectedEventsForStates(ComponentStateModel.ST_TASK) != null)
							&& (csModel.getSelectedEventsForStates(ComponentStateModel.ST_TASK).containsKey(endEvent.getType()))){

						//System.out.println("NEXT: "+endEvent.getType()+ " = END external -> for ORIG selected event= "+startEvent.getType());
						return endEvent.getCurrentTimeStamp();
					}
					else indexEndItem++;
					...
					//	System.out.println("NEXT: "+endEvent.getType()+ " = END external(not sel) -> for ORIG selected event= "+startEvent.getType());
					return endEvent.getCurrentTimeStamp();
				}
				else indexEndItem++;
		...
		else indexEndItem++; // skip
	}
	// if the state generated by startEvent doesn't have an end event in this list,
	// we have to return the final timestamp of the PI.
	// System.out.println("NEXT: for "+startEvent.getType()+ " END event NOT found -> return PI end timestamp");
	
	
	// start chang : by msong
	if(timeOption.equals(TIME_RELATIVE_TIME))
		return new Date(csModel.getEndDateMap(ST_INST).get(startEvent.getProcessInstance().getName()).getTime() - csModel.getStartDateMap(ST_INST).get(startEvent.getProcessInstance().getName()).getTime());
...
//	AbstractLogUnit item = null;
	...
	while (indexEndItem < helpList.size()){
		...
		if (nextEvent == null || !csModel.getInstanceTypeToKeep().contains(nextEvent.getProcessInstance().getName())
							 || csModel.getEventTypeToKeep()==null ) {
		...
		// if originator, then get next event in selected orig||task event....
			...
			if (csModel.getEventTypeToKeep().contains(nextEvent.getType())){
					...
					if ((state.equals(ComponentStatePanel.STATE_WORKING)&& nextEvent.getOriginator().equals(key))
						||(state.equals(ComponentStatePanel.STATE_ASSIGNED)&& usersList!= null && usersList.contains(key))){
					...
					// else it can be an external event
							...
							&& (csModel.getSelectedEventsForStates(ComponentStateModel.ST_TASK).containsKey(nextEvent.getType()))){
...
//	AbstractLogUnit item = null;
	...
	protected void updateMilli2pixelsRatio() {
		milli2pixels = (double) (this.getWidth()- border*2) / (double)(csModel.getLogBoundaryRight().getTime() - csModel.getLogBoundaryLeft().getTime()); // + 2 is added to extend bound
	 ...
	 * @param aZoom fraction of the log to be viewable (within (0,1] !)
	 */
	...
	public Point zoomInViewPort() {
		...
		p2=null;
		adjustSlideBar();
		return new Point((int) (pos_x * d.getWidth() / width),
				(int) (pos_y * d.getHeight() / height));
	}
	...
	/**
	 * Convenience method.
	 * Transforms a timestamp (milliseconds) into the corresponding
	 * horizontal position within the viewport.
	 * @param aTimeMillis
	 * @return
	 */
	 ...
	 * into the correspondig timestamp (Date) in the log space.
	 * @param anX
	 * @return
	 */
	 ...
	 * convenience method.
	 * transforms a given horizontal coordinate within the viewport
	 * into the correspondig timestamp (milliseconds) in the log space.
	 * @param anX
	 * @return
	 */
	...
	public void adjustWidth(){
		...
		if (number == 0) {
			...
			if (idx < 0) csOptionPanel.changeWidthSort(0);
			else csOptionPanel.changeWidthSort(idx);
		} else if (number > 100) {
			for (idx = 0; idx < widthDividers.length; idx++)
				if (widthDividers[idx] == widthSort)
					break;
			...
			if (idx >= widthDividers.length) csOptionPanel.changeWidthSort(widthDividers.length - 1);
			else csOptionPanel.changeWidthSort(idx);
		}
	}
	 ...
	 * one timeframe is delimited by d1Left on the right and d1Right on the right,
	 * this is the reference frame. the second timeframe is delimited by d2Left/Right.
	 * This method checks, if any part of the second timeframe is contained within
	 * the first timeframe.
	 * @param d1Left
	 * @param d1Right
	 * @param d2Left
	...
	protected void calculateCurrentTime() {
		...
		for (Iterator itSets = csModel.getItemMap().keySet().iterator(); itSets.hasNext(); ) {
			...
			if (csModel.getTypeHashMap().equals(ST_INST) && !csModel.getInstanceTypeToKeep().contains(key))
				continue;
			...
			for (Iterator itItm = tempLogUnit.iterator(); itItm.hasNext(); ) {
				...
				if (csModel.getEventTypeToKeep() != null && ( //!csModel.getEventTypeToKeep().contains(item.getType()) ||
					 !csModel.getInstanceTypeToKeep().contains(item.getProcessInstance().getName())))
					continue;
				...
				if (timeOption.equals(TIME_ACTUAL)) {
					...
					if (relativeTimeBase.equals(ST_INST))
						...
						item.setCurrentTimeStampRelative_Others((Date) timeBaseHashMap.get(item.
								getElement()));
					else if (relativeTimeBase.equals(ST_ORIG)) {
						if (timeBaseHashMap.containsKey(item.getOriginator()))
							item.setCurrentTimeStampRelative_Others((Date) timeBaseHashMap.get(
									item.getOriginator()));
					...
					if (relativeTimeBase.equals(ST_INST))
						...
						item.setCurrentTimeStampRelativeRatio_Others((Date) timeBaseHashMap.get(
								item.getElement()),
								(Date) endTimeBaseHashMap.get(item.getElement()));
					else if (relativeTimeBase.equals(ST_ORIG)){
						if (timeBaseHashMap.containsKey(item.getOriginator()))
							item.setCurrentTimeStampRelativeRatio_Others((Date) timeBaseHashMap.
	 ...
	 * paints a log item handle visualization.
...
//			 display element name
			...
			//	g.drawString(dimName, pixStart+5, currentTop+20);  // from init lanes !!
	    	}
...
//		 calculate common coordinates :
		...
		// calculate the color array for states of activities/originators/data in eventsList:
		if (eventsList.size() >= 1){			
			...
			for (Iterator it = ((LogUnitList)eventsList).iterator(); it.hasNext(); ) {
			 ...
			 if (csa.getSettingPanel().combineColor()){
				...
				// System.out.println("#=== color combined from: " + stateColors.length + "  diff colors...");
...
/*
		if (false);
		else if (shape.equals(ComponentStatePanel.STATE_ABORTED))
		{
			g.fillOval(x-2, y-2, 4, 4);
		}
		else if (shape.equals(ComponentStatePanel.STATE_COMPLETED))
		{
			int y0, x0;
...
//		g.fillPolygon(triX, triY, 3);
			...
			else if (shape.equals(ComponentStatePanel.STATE_MANUALSKIP))
		{
			int rhombX[] = {x, x-5, x, x+5};
			int rhombY[] = {y-5, y, y+5, y};
			g.fillPolygon(rhombX, rhombY, 4);
		}
		else if (shape.equals(ComponentStatePanel.STATE_UNASSIGNED))
		{
			g.drawRect(x-5, y-5, 10, 10);
		}
		else if (shape.equals(ComponentStatePanel.STATE_UNDEFINED))
		{
			g.drawRoundRect(x-5, y-5, 10, 10,2,2);
		}
		else if (shape.equals(ComponentStatePanel.STATE_INITIAL))
		{
			g.drawOval(x-5, y-5, 11, 11);
		}
		else if (shape.equals(ComponentStatePanel.STATE_READY))
		{
			g.drawOval(x-5, y-5, 11, 11);
		}
	*/
	...
	public ArrayList orderPIEndTimestampList(LogUnitList tempDrawList){
		...
		if (tempDrawList != null && tempDrawList.size()>0){
			...
			for (Iterator  iter = tempDrawList.iterator(); iter.hasNext();){
				...
				// endPIDate = csModel.getEndDateMap(ST_INST).get(event.getProcessInstance().getName());  // end time PI
				// endPIDate = csModel.getEndPITimestamp(event.getProcessInstance().getName());
		...
		else return null; // when tempDrawList is null or empty
	}
...
public void paintInterval(LogUnitList tempDrawList, String key, Date itemTimestamp, Date nextItemTimestamp, int height, int top, Graphics gr){
	...
	if (tempDrawList != null && tempDrawList.size()>0 && orderedPIList != null){
		...
		int index = 0;
		// while endPI of the events in tempDrawList are before the nextItemTime, paint and remove them from list		
		while (index < orderedPIList.size() && !((Date)orderedPIList.get(index)).after(nextItemTimestamp)){		
			...
			for (Iterator  iter = tempDrawList.iterator(); iter.hasNext();){
				...
				//	System.out.println("paintInterval >>>>>>>> paint prev ="+ prevDate.toString()+" - endPI = "+ endPIDate.toString());
	...
	//	System.out.println("paintInterval >> paint prev ="+ prevDate.toString()+" - nextItemTimestamp = "+ nextItemTimestamp.toString());
	}			
			...
			if(timeOption.equals(TIME_LOGICAL)||timeOption.equals(TIME_LOGICAL_RELATIVE))
			{
			    ...
			    while(flag && tempDrawList.size()>0){
					...
					if(endTempDate.before(secondEvent.getActualTimeStamp())){
						...
						tempDrawList.removeEvent(event);
						//System.out.println("removed  = " + event.getElement() +" "+event.getType()); 
	...
	if (tempDrawList != null && tempDrawList.size()>0){
		...
		for (Iterator ite = tempDrawList.iterator(); ite.hasNext();){
			...
			if (item.getElement().equals(event.getElement())
					&& item.getProcessInstance().equals(event.getProcessInstance())
					&& (evListForExternalEvents.isEmpty() || !evListForExternalEvents.contains(item))){
	...
	public void paintComponent(Graphics grx) {
		...
		if (csModel.getItemMap().size() > 0) {
			...
			for (Iterator itSets = csModel.getItemMap().keySet().iterator(); itSets.hasNext(); ) {
				...
				// initially paint the originator lane as UNASSIGNED, for all instances selected = INIT
				...
				// list to store events that have already an external event for originators
				// ArrayList evListForExternalEvents = new ArrayList();
			
				
				

			//	System.out.println("PAINT: element to be analyzed: *** "+ key+ " ***");
				...
				for (Iterator itItm = sortedElementList.iterator(); itItm.hasNext(); ) {
					...
					//	 if the event was not selected, skip....only for Task/Inst/Data
					//   orig can have external events to behave as separators!!
					if(!csModel.getTypeHashMap().equals(ST_ORIG)&& csModel.getEventTypeToKeep()!=null
							&& !csModel.getEventTypeToKeep().contains(item.getType())) continue;

					
									
					// if event not selected for task or originator, skip
					if (csModel.getTypeHashMap().equals(ST_ORIG)
							&& (((csModel.getEventTypeToKeep()!=null && !csModel.getEventTypeToKeep().contains(item.getType())))
							&& (csModel.getSelectedEventsForStates(ComponentStateModel.ST_TASK) != null
								&& !csModel.getSelectedEventsForStates(ComponentStateModel.ST_TASK).containsKey(item.getType())))){
					
					//	System.out.println("PAINT SKIP Orig  event= "+item.getType()+ " and Orig= "+item.getOriginator());
						continue;
					...
					//	remove outdated events, for which their instances just finished
					...
					if (csModel.getTypeHashMap().equals(ST_ORIG) ){
						...
						//	System.out.println("PAINT ORIG  event= "+item.getType()+ " and Orig= "+item.getOriginator());
						...
						if (csModel.getEventTypeToKeep().contains(item.getType())
								&&(item.getOriginator().equals(key) || (usersList!= null && usersList.contains(key)))){
							
							// this may return an ASSIGN event with Orig = user, but not in assignedUser list!! - it will be filtered later on..
							componentState = getStateByEvent(item, ComponentStateModel.ST_ORIG);
							
						}else externalEvent = true; // // this can be also a lane that acts like a separator for Originator state
					}
				...
				//	System.out.println(" -- current event: = "+ item.getType());
					
					
					// ========================== treat external event for Originator
					if (externalEvent){
						//System.out.println(" -- evaluate Orig EXTERNAL event......"+item.getType()+ " / "+ item.getElement());
						...
						if (tempDrawList != null && tempDrawList.size() > 0){
							...
							//	nextItemTimestamp = getNextEventTimestamp(item, key, helpList, tempDrawList, evListForExternalEvents);
							...
							if(timeOption.equals(TIME_ACTUAL)||timeOption.equals(TIME_RELATIVE_TIME)||timeOption.equals(TIME_RELATIVE_RATIO))
								paintInterval(tempDrawList, key, item.getCurrentTimeStamp(), nextItemTimestamp, unitHeight, currentTop - unitHeight/2, gr);
							else 
								paintInterval(tempDrawList, key, item, getNextEvent(item, key, helpList), unitHeight, currentTop - unitHeight/2, gr);
					...
					if ((stateShapeType != null)&& stateShapeType.equals(ComponentStatePanel.STATE_SHAPE_SEPARATOR)){
							...
							// remove  previous event for the instance & task corresponing to this separator
							updateDrawUnitList(tempDrawList, item);
							
							// if the event happens at a later time time than the time interval in the main view
							// we skip it, cause it's not affecting the current states
							if (item.getCurrentTimeStamp().getTime()>clipRightTs2) continue;
							...
							if (tempDrawList != null && tempDrawList.size() > 0){								
								...
								//System.out.println("PAINT SEP interval: currentTime= "+item.getCurrentTimeStamp().toString()+ " has NEXTevent timestamp: = "+ nextItemTimestamp.toString());
								if(timeOption.equals(TIME_ACTUAL)||timeOption.equals(TIME_RELATIVE_TIME)||timeOption.equals(TIME_RELATIVE_RATIO))
									paintInterval(tempDrawList, key, item.getCurrentTimeStamp(), nextItemTimestamp, unitHeight, currentTop - unitHeight/2, gr);
								else 
									paintInterval(tempDrawList, key, item, getNextEvent(item, key, helpList), unitHeight, currentTop - unitHeight/2, gr);
							...
							//	paint the separator itself: if item is not in the main view, skip drawing it
					...
					//	 ========================= paint Lane======================================
							...
							// take out the events of the same instance and element, and add new event --> keep only last state for each instance!
							updateDrawUnitList(tempDrawList, item);

							// if the new state is not visible, then it's not added to the Drawlist
							// the lane is painted based on existing states;
							if (item.getCurrentTimeStamp().getTime()>clipRightTs2) continue;

							boolean addState = false;  // to decide when to add a state to tempDrawList
							


							if ((csModel.getTypeHashMap().equals(ST_TASK))||(csModel.getTypeHashMap().equals(ST_INST))
									|| csModel.getTypeHashMap().equals(ST_DATA)){
								addState = true;
							}
							if (csModel.getTypeHashMap().equals(ST_ORIG)) {

								// the new state is added only if it's the same originator or assignedUser
								// not added if represents an external END event for existing state

								//if (csModel.getEventTypeToKeep()!=null && csModel.getEventTypeToKeep().contains(item.getType())
								if (csModel.getEventTypeToKeep().contains(item.getType())
										&&((componentState.equals(ComponentStatePanel.STATE_WORKING)&& item.getOriginator().equals(key))
										||(componentState.equals(ComponentStatePanel.STATE_ASSIGNED)&& usersList!= null && usersList.contains(key)))){
									addState = true;
									//	System.out.println("###=== at LANE type, new ORIG state added and tempDrawList has now: "+ tempDrawList.size()+ " events");
								}
							}

							if (addState){

								// System.out.println("PAINT== LANE ADD to tempDrawList, current item event: -"+ item.getType() + "- with state: -"+ componentState+ "-.....");
								...
								// if the nextItem of a lane passed already, skip drawing the lane
								// the state is not valid anymore, so it's not affecting the current view
								if (nextItemTimestamp == null || nextItemTimestamp.getTime() < clipLeftTs2) continue;
	...
	//	bDrawLine = csa.getSettingPanel().isDrawLine();
		...
		if (csModel.getItemMap().size() > 0) {
			...
			for (Iterator itSets = csModel.getItemMap().keySet().iterator(); itSets.hasNext(); ) {
				...
				// iterate through items
				for (Iterator itItm = ((LogUnitList) csModel.getItemMap().get(key)).iterator();
									  itItm.hasNext(); ) {
					item = (AbstractLogUnit) itItm.next();
					...
					// to do
				//	if (bDrawLine && item.getType().equals(csa.getSettingPanel().getStartEvent()))
				//		tempUnitList.addEvent(item);
					assignColorByItem(item, gr);
					clipL = 0;
					clipR = width;

					double milli2pixels = (double) ((width)-10) / (double)(csModel.getLogBoundaryRight().getTime() - csModel.getLogBoundaryLeft().getTime()); // + 2 is added to extend bound

					long clipLeftTs2 = coord2timeMillis_buffer(clipL,milli2pixels);
					long clipRightTs2 = coord2timeMillis_buffer(clipR,milli2pixels);
					//  if line is added
					/*
					if (bDrawLine && item.getType().equals(csa.getSettingPanel().getEndEvent())) {
						for (Iterator itr = tempUnitList.iterator(); itr.hasNext(); ) {
							...
							if (item2.getElement().equals(item.getElement()) &&
								...
								paintItemLine_buffer(time2coord_buffer(item2.getCurrentTimeStamp(),milli2pixels) + 3,
					...
					*/
					...
					if (item.getCurrentTimeStamp().getTime() < clipLeftTs2 ||
						...
						continue;
					// paint an item
					this.paintItem_buffer(time2coord_buffer(item.getCurrentTimeStamp(),milli2pixels)+3, currentTop, gr,
...
//////////////////////MOUSE LISTENER AND TOOLTIP METHODS/////////////////
	...
	public void mouseMoved(MouseEvent e) {
		...
		for (Iterator itSets = csModel.getItemMap().keySet().iterator(); itSets.hasNext(); ) {
			...
			index++;			
			currentTop = unit2Cord(index) + unitHeight / 2;	
			if (currentTop - unitHeight / 2 >= p.getY() || p.getY() >= currentTop + unitHeight / 2)
				continue;
			...
			for (Iterator itItm = tempLogUnit.iterator(); itItm.hasNext(); ) {
				...
				if ((stateShapeType != null)&& stateShapeType.equals(ComponentStatePanel.STATE_SHAPE_SEPARATOR)){
					...
					if (x <= p.getX() && p.getX() <= x2) {
						...
						//	we check if the new state was added to the tempDrawList:
						else if (csModel.getTypeHashMap().equals(ST_ORIG)) {
