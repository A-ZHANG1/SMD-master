...
public class OverviewPanel extends JPanel implements MouseMotionListener, MouseListener {
	...
	/**
	 * constructor
	 * @param aItemSets HashMap representing Item (start) of the log to be visualized
	...
	public void paintComponent(Graphics grx) {
		...
		grx.drawImage(csa.getComponentStatePanel().getBufferedImage(), 0, 0, this);
		drawBox(grx);
	}
	...
	public void mousePressed(MouseEvent e) {
		Point p = e.getPoint();
		if((p.x>=x1)&&(p.x<=x1+width)&&(p.y>y1)&&(p.y<=y1+height)){
			clicked = true;
			x1_o = x1;
			y1_o = y1;
			p_x = (int) p.x;
