...
import org.prom5.analysis.performance.dottedchart.model.ExtendedLogTable;
...
public class SettingPanel extends JPanel {
	...
	protected JButton applySelectedInstancesButton = new JButton("Use Selected Instances");
	...
	private JCheckBox checkPrioritizeColorBox = new JCheckBox(" Prioritize colors ");
	...
	protected JLabel colorsettingLabel = new JLabel("Change colors by pressing buttons");
	...
	private JRadioButton prioritizeColorBut = new JRadioButton("Prioritize colors");
	...
	public SettingPanel(LogReader aInputLog, ComponentStateAnalysis aComponentStateAnalysis, ComponentStatePanel aComponentStatePanel, ComponentStateModel aCsModel) {
		...
		// for states - get selected events map for the chosen element type: might be null first time ???
		selectedEventsMap = csModel.getSelectedEventsForStates(componentStatePanel.getComponentStateOptionPanel().getComponentType());
	...
	public void initSettingPanel()
	{
		...
		processInstanceIDsTable = new DoubleClickTable(new ExtendedLogTable(inputLog,instanceIDs), null);
	...
	protected void initEventTypeSelection(){
		...
		for (Iterator itSets = csModel.getItemMap(ComponentStateAnalysis.ST_EVEN).keySet().iterator(); itSets.hasNext(); ) {				
		...
		// for tasks and PI
		String[] statesList = csModel.getElementStatesList(ComponentStateAnalysis.ST_TASK);
		int i = 0;	
		while (i < statesList.length){
			...
			for(int j=0;j<listModel.getSize();j++){
				...
				// for running
				if(tempEvent.equalsIgnoreCase("start")&& tempState.equalsIgnoreCase("running")){
				...
				// for complete
				else if(tempEvent.equalsIgnoreCase("complete")&& tempState.equalsIgnoreCase("completed")){
				...
				// for assigned  -- in case of the originator,the SUSPEND event also determines this state
				else if((tempEvent.equalsIgnoreCase("assign")||tempEvent.equalsIgnoreCase("reassign"))&& (tempState.equalsIgnoreCase("assigned"))){
				...
				// for suspended
				else if (tempEvent.equalsIgnoreCase("suspend")&& tempState.equalsIgnoreCase("suspended")){
				...
				// for aborted
				else if ((tempEvent.equalsIgnoreCase("ate_abort")||tempEvent.equalsIgnoreCase("pi_abort")||tempEvent.equalsIgnoreCase("withdraw"))
				...
				// for manual skipped
				else if(tempEvent.equalsIgnoreCase("manualskip")&& tempState.equalsIgnoreCase("manualskipped")){
			...
			int priority = -1;
			if(tempState.equals(csModel.STATE_RUNNING)|| tempState.equals(csModel.STATE_WORKING)|| tempState.equals(csModel.STATE_DEFINED)){
				priority = 1;
				tempColor = new Color(50, 200, 50);		//  green 
			}else if(tempState.equals(csModel.STATE_ASSIGNED)|| tempState.equals(csModel.STATE_UNCONFIRMED)){
				priority =2;				
				tempColor = Color.YELLOW; //
			}else if(tempState.equals(csModel.STATE_SUSPENDED)|| tempState.equals(csModel.STATE_UNDEFINED)){
				priority = 3;
		...
		csModel.setStatePriorityMap(ComponentStateAnalysis.ST_TASK, statePriorityMap);
		csModel.setStatePriorityMap(ComponentStateAnalysis.ST_INST, statePriorityMap);
		...
		csModel.setSelectedEventsForStates(ComponentStateAnalysis.ST_TASK, selectedEventsMap);
		csModel.setSelectedEventsForStates(ComponentStateAnalysis.ST_INST, selectedEventsMap);
	//	System.out.println("<- set init csModel.setSelectedEventsForStates for type TASK/INST");
		...
		String tempEvt = null;
		for (Iterator itSets = selectedEventsMap.keySet().iterator(); itSets.hasNext(); ) {
		...
		// for originator
		statesList = csModel.getElementStatesList(ComponentStateAnalysis.ST_ORIG);
		i = 0;	
		...
		while (i < statesList.length){
			...
			for(int j=0;j<listModel.getSize();j++){
				...
				//	working				
				if(tempEvent.equalsIgnoreCase("start")&& tempState.equalsIgnoreCase("working")){
					selections.add(j);
					k++;
					
				}
				//	 for assigned  -- in case of the originator,the SUSPEND event also determines this state
				else if((tempEvent.equalsIgnoreCase("assign")||tempEvent.equalsIgnoreCase("reassign") || tempEvent.equalsIgnoreCase("suspend"))&& (tempState.equalsIgnoreCase("assigned"))){
					selections.add(j);
					k++;	
...
//			 set PRIORITY for this state
		...
		//	save default selection
		csModel.setStatePriorityMap(ComponentStateAnalysis.ST_ORIG, statePriorityMap);
		...
		//	for DATA
		statesList = csModel.getElementStatesList(ComponentStateAnalysis.ST_DATA);
		i = 0;	
	//	allStatesSelections.clear();
		while (i < statesList.length){
			...
			for(int j=0;j<listModel.getSize();j++){
				...
				//	for update
				if(tempEvent.equalsIgnoreCase("update")&& tempState.equalsIgnoreCase("defined")){
					selections.add(j);
					k++;					
				}
			}
...
//			 set PRIORITY for this state
		...
		// save default selection
		csModel.setStatePriorityMap(ComponentStateAnalysis.ST_DATA, statePriorityMap);		
	...
	public void initStateSettings(){
		...
		stateMainPanel.setLayout(new BoxLayout(stateMainPanel, BoxLayout.Y_AXIS));
	...
	//	System.out.println("init State Panel");	
		...
		String type = componentStatePanel.getComponentStateOptionPanel().getComponentType();
		//System.out.println("Setting Panel for type - " + type);
		
		

		if (type.equals(ComponentStateAnalysis.STR_NONE)||type.equals(ComponentStateAnalysis.ST_EVEN))// have to eliminate the EVEN type!!
			return null;
		...
		statePanel.setPreferredSize(new Dimension(600, (statesList.length+ 2) * 80));
		statePanel.setMaximumSize(new Dimension(600, (statesList.length+ 2) * 80));
		...
		//panelStateElementLabel.setLayout(new GridLayout(panelStateElementLabel, BoxLayout.X_AXIS));
	...
	//	tempEventTypeButton = new JList[csModel.getItemMap(ComponentStateAnalysis.ST_EVEN).keySet().size()];
		...
		while (i < statesList.length){
			...
			// we could make static colors for states...to make clear the combination...
		...
		//	tempColorButton[i].setPreferredSize(new Dimension(80, 20));
			...
			}else tempColorButton[i].setBackground(colorReference.getColor(tempState));	
			...
			tempColorButton[i].addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					Color newColor = JColorChooser.showDialog(
									 tempButton, "Choose Background Color",
									 tempButton.getBackground());
					if (newColor != null) {
			...
			stateColorMap.put(statesList[i], tempColorButton[i].getBackground());  /// can this be null????
			...
			tempEventTypeButton[i].setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
			...
			if (type.equals(ComponentStateAnalysis.ST_TASK)||type.equals(ComponentStateAnalysis.ST_INST)){
				selectedIndices = (int[])(taskIndicesSelectionMap.get(tempState));
			}else if (type.equals(ComponentStateAnalysis.ST_ORIG)){
				selectedIndices = (int[])(origIndicesSelectionMap.get(tempState));
			}else if (type.equals(ComponentStateAnalysis.ST_DATA)){
				selectedIndices = (int[])(dataIndicesSelectionMap.get(tempState));
			}
			
			int sel = 0;
			for (int j=0; j<selectedIndices.length; j++ ){
				int val = selectedIndices[j];				
				if (val <listModel.getSize()) sel++;
			}				
			...
			for (int j=0; j< tempEventTypeButton[i].getSelectedIndices().length; j++){
				...
				//System.out.println(" selected ev put in the map: " + tpEvent);
				selectedEventsMap.put(tpEvent, statesList[i]); // this can overwrite, so must check consist!!!!
			}
			
			// user action ...

			tempEventTypeButton[i].addListSelectionListener(new ListSelectionListener() {
			      public void valueChanged(ListSelectionEvent evt) {
			        if (evt.getValueIsAdjusting() == false)   // user stoped manipulating the list
			        {
			        	
			        	String type = componentStatePanel.getComponentStateOptionPanel().getComponentType();
			        	JList list = (JList)evt.getSource();
			        	String nameState = list.getName();
			        	//System.out.println("current state: " + nameState);
			            ...
			            for (int j=0; j< selectedValues.length; j++){
			            	//System.out.println("CHANGE - selected values: " + selectedValues[j].toString());
			            	vals.add(selectedValues[j].toString());
			            }	
			            // remove deselected items from the same list map
			            for (Iterator it = selectedEventsMap.keySet().iterator(); it.hasNext();){
			            	String tmpEvent = (String)it.next();
			            	if (nameState.equals(selectedEventsMap.get(tmpEvent))&& (!vals.contains(tmpEvent))){
			            		selectedEventsMap.remove(tmpEvent);
			            		// redefine the iterator!!!!!
			            		it = selectedEventsMap.keySet().iterator();
			            		//System.out.println("CHANGE - remove from selectedEventMap: " + tmpEvent+ " for state "+ nameState);
			            	}
			            }			            
			            for (int j=0; j<selections.length; j++){
			            	String tmpEvent = (String)listModel.get(selections[j]);			            	
			            	if ((selectedEventsMap.containsKey(tmpEvent))&& (!nameState.equals(selectedEventsMap.get(tmpEvent)))){			            		
			            		list.removeSelectionInterval(selections[j], selections[j]); // remove selection index
			            		//System.out.println("CHANGE - remove selected index: " + tmpEvent);
			            		// set a bg color for this index?!
			            	}else if (!selectedEventsMap.containsKey(tmpEvent)){
			            		selectedEventsMap.put(tmpEvent, nameState);   
			            		//System.out.println("CHANGE - add to selectedEventMap: " + tmpEvent+ " with new state " + nameState);
			            ...
			            if (type.equals(ComponentStateAnalysis.ST_TASK)||type.equals(ComponentStateAnalysis.ST_INST)){
	        				taskIndicesSelectionMap.put(nameState, list.getSelectedIndices());
	        			}else if (type.equals(ComponentStateAnalysis.ST_ORIG)){
	        				origIndicesSelectionMap.put(nameState, list.getSelectedIndices());
	        			}else if (type.equals(ComponentStateAnalysis.ST_DATA)){
	        				dataIndicesSelectionMap.put(nameState, list.getSelectedIndices());
	        			}
			          //  System.out.println("Value changed from first index " + evt.getFirstIndex() + " to last index " + evt.getLastIndex());
			        } 
			...
			if (statePriorityMap.get(tempState) != null){
				tempPriorityButton[i].setSelectedItem(statePriorityMap.get(tempState));
				//System.out.println("priority set " + statePriorityMap.get(tempState)+ "for state "+ tempState);
			}
		...
		//	tempColorButton[i].setAlignmentY(TOP_ALIGNMENT);
			...
			if (type.equals(ComponentStateAnalysis.ST_ORIG) && tempState.equals(csModel.STATE_UNASSIGNED)){
...
//		 STATE panel
		...
		String[] statesList = csModel.getElementStatesList(type);	
		//System.out.println("State Panel - number of elements: "+ statesList.length);
		
		// initializing the new STATE panel:
		JPanel statePanel = new JPanel(new GridLayout(statesList.length + 1, 1));		
		statePanel.setPreferredSize(new Dimension(600, (statesList.length+ 1) * 80));
		statePanel.setMaximumSize(new Dimension(600, (statesList.length+ 1) * 80));
		statePanel.setBorder(BorderFactory.createEtchedBorder());
		statePanel.setAlignmentX(LEFT_ALIGNMENT);
	...
	//	tempEventTypeButton = new JList[csModel.getItemMap(ComponentStateAnalysis.ST_EVEN).keySet().size()];
		...
		while (i < statesList.length){
			...
			tempColorButton[i] = new JButton("change color");
		//	tempColorButton[i].setPreferredSize(new Dimension(80, 20));
			tempColorButton[i].setBackground(colorReference.getColor(tempState));	
			tempColorButton[i].setName(statesList[i]);
			tempColorButton[i].setActionCommand(tempState);
			tempColorButton[i].addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
			...
			//  set available colors for states
			stateColorMap.put(statesList[i], tempColorButton[i].getBackground());  /// can this be null????
			//System.out.println("<<< state button color is set for state -"+ statesList[i]);
			...
			for (int j=0; j<selectedIndices.length; j++ ){
				int val = selectedIndices[j];
				//System.out.println("<> value for indice to be selected: "+ val);
				if (val <listModel.getSize()) sel++;
			}				
		
			if (selectedIndices != null && selectedIndices.length != 0){
				tempEventTypeButton[i].setSelectedIndices(selectedIndices);
				//System.out.println("dimension is "+ selectedIndices.length + " for indices selected for state - "+ tempState);
			}	
	...
	private void initColorPanelOLD() {
		...
		String type = componentStatePanel.getComponentStateOptionPanel().getColorStandard();
...
//***
	...
	public void changeStatePanel() {
		...
		if ((tempContainer.getComponentCount() != 0)&& (tempContainer.getComponent(0)!= null))
			{
		...
		if (this.initStatePanel()!= null){
			...
			//	 update the eventTypeToKeep list from the priviously selected events for the new component type
			HashMap eventsMap = csModel.getSelectedEventsForStates(componentStatePanel.getComponentStateOptionPanel().getComponentType());			
			// put in eventTypeToKeep the available selected events
			ArrayList<String> atr = new ArrayList<String>();
			String tempEvent = null;
			for (Iterator itSets = eventsMap.keySet().iterator(); itSets.hasNext(); ) {
	...
	private void registerGUIListener() {
		...
		applyEventTypeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < eventTypeToKeep.size(); i++) {
					if (checks[i].isSelected()) atr.add((String) eventTypeToKeep.get(i));
				}
					componentStatePanel.changeEventTypeToKeep(atr);			
		...
		applySelectedInstancesButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				componentStateAnalysis.setSelectedInstanceIndices(getSelectionStatus());		
		...
		applyStateButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				componentStatePanel.changeSelectedEventsForStates(type, selectedEventsMap);
