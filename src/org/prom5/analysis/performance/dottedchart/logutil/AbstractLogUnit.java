...
package org.prom5.analysis.performance.dottedchart.logutil;
...
public class AbstractLogUnit implements Comparable {
	 ...
	 * @param aProcessInstance the process instance where this abstract event is contained
	 * @param aStartEvent left boundary atomic event
	 * @param anEndEvent right boundary atomic event
	 */
	 ...
	 * @return
	 */
	public static AbstractLogUnit create(ProcessInstance aProcessInstance, AuditTrailEntry aLogEvent) {
	...
	public Date getRelativeTimestamp() {
		return new Date(logEvent.getTimestamp().getTime() - ((AuditTrailEntry)processInstance.getAuditTrailEntryList().iterator().next()).getTimestamp().getTime());
	}
	...
	public Date getRelativeRatio() {
		...
		try{
			over = logEvent.getTimestamp().getTime() -
						((AuditTrailEntry) processInstance.getAuditTrailEntryList().iterator().next()).
						getTimestamp().getTime();
			end = ((AuditTrailEntry) processInstance.getAuditTrailEntryList().get(
					   processInstance.getAuditTrailEntryList().size() - 1)).getTimestamp().getTime() -
					   ((AuditTrailEntry) processInstance.getAuditTrailEntryList().iterator().next()).
					   getTimestamp().getTime();
		}
		catch(Exception ce){ };
	...
	public Date getRelativeRatio(Date startDate, Date endDate) {
		return new Date((long)((float)(logEvent.getTimestamp().getTime() - startDate.getTime())/(float)(endDate.getTime() - startDate.getTime())*10000));
	}
...
//		currentDate = new Date(logEvent.getTimestamp().getTime() - ((AuditTrailEntry)processInstance.getAuditTrailEntryList().iterator().next()).getTimestamp().getTime());
//		relativeDate = currentDate;
		Date tempDate = null;
		Iterator itr = processInstance.getAuditTrailEntryList().iterator();
		while(itr.hasNext()){
			tempDate = ((AuditTrailEntry)itr.next()).getTimestamp();
			if(tempDate!=null) {
				currentDate = new Date(logEvent.getTimestamp().getTime() - tempDate.getTime());
				relativeDate = currentDate;
				return;
			}
		}
	...
	public void setCurrentTimeStampRelative_Others(Date aDate) {
		currentDate =  new Date(logEvent.getTimestamp().getTime() - aDate.getTime());
		relativeDate = currentDate;
	}
	...
	public void setCurrentTimeStampRelativeRatio_Others(Date startDate, Date endDate) {
		currentDate =  new Date((long)((float)(logEvent.getTimestamp().getTime() - startDate.getTime())/(float)(endDate.getTime() - startDate.getTime())*10000));
		relativeDate = currentDate;
	}
	...
	public int compareTo(Object arg0) {
		if(currentDate==null||((AbstractLogUnit)arg0).getCurrentTimeStamp()==null) {
			return 0;
		}
		if((currentDate.getTime() - ((AbstractLogUnit)arg0).getCurrentTimeStamp().getTime())>0) return 1;
		if((currentDate.getTime() - ((AbstractLogUnit)arg0).getCurrentTimeStamp().getTime())<0)	return -1;
		else return 0;
	}
	...
	public String toString() {
		return "AbstractEvent '" + logEvent.getElement() + "' (" + logEvent.getTimestamp() + ")";
	}
}
