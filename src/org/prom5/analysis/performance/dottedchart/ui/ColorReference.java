 ...
 * ColorRepository.
 *
 * For assigning colors to keys automatically, and preserving
 * these associations over time.
 * Keeps string ids assigned to colors and preserves mappings.
 * Provides a set of 10 standard colors for mappings, when full
 * it will return random colors.
 *
 * @author Minseok Song
 */
public class ColorReference {
	...
	public Color getColor(String key) {
		if(mappings.containsKey(key)) {
			...
			// create random color
			Color random = new Color(rnd.nextInt(245), rnd.nextInt(245), rnd.nextInt(245));
	...
	private void changeColor(Color color){
		...
		while (itr.hasNext()){
			...
			if (mappings.get(key).equals(color)) {
				mappings.remove(key);
				Color random = new Color(rnd.nextInt(245), rnd.nextInt(245), rnd.nextInt(245));
				mappings.put(key, random);
			}
...
//		freeColor(key);
	...
	public void writeToXML(Writer bw) throws IOException {
		...
		while (itr.hasNext()){
			String key = (String) itr.next();
			bw.write("<ColorUnit>\n<Key>"+key+"</Key>\n");
			bw.write("<ColorRGB>"+((Color) mappings.get(key)).getRGB()+"</ColorRGB>\n</ColorUnit>");
		}
	...
	public void readFile(InputStream input){
			...
			try {
				...
				for (int i = 0; i < children.size(); i++) {
					...
					if (element != null)
						if ("ColorUnit".equals(element.getName())){
							String key = element.getChildText("Key");
							int colorRGB = (int) Integer.valueOf(element.getChildText("ColorRGB"));
							mappings.remove(key);
							mappings.put(key,new Color(colorRGB));
						}
				}
