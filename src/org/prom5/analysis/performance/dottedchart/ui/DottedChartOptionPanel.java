...
public class DottedChartOptionPanel extends JPanel implements ChangeListener, ItemListener {
	...
	private String[] widthSorts = {"L-1", "L-10", "L-100", "L-500", "seconds", "minutes", "hours", 	// width option
								 "days", "weeks", "months", "years"};
	private long[] widthDividers = {1, 10, 100, 500, 1000, 60000, 3600000L, 86400000L, 604800000L,
	...
	private String[]  timeOptions = {DottedChartPanel.TIME_ACTUAL, DottedChartPanel.TIME_RELATIVE_TIME,
									DottedChartPanel.TIME_RELATIVE_RATIO, DottedChartPanel.TIME_LOGICAL,
									DottedChartPanel.TIME_LOGICAL_RELATIVE,}; // array of "time option" strings required to fill the timeOptionBox combobox with
	private JComboBox timeOptionBox = new JComboBox(timeOptions);
	private String[]  relativeTimeOptions = {DottedChartAnalysis.ST_INST, DottedChartAnalysis.ST_TASK, DottedChartAnalysis.ST_ORIG, DottedChartAnalysis.ST_EVEN}; 	//array of "relative time classification" strings required to fill the timeOptionBox combobox with
	...
	public DottedChartOptionPanel(DottedChartPanel aDcPanel, DottedChartAnalysis aDCA) {
		dcPanel = aDcPanel;
		dcAnalysis = aDCA;
...
//		this.add(checkLineBox);
		...
		zoomSliderX = new JSlider(JSlider.VERTICAL, 0, 3000, 0); // zooms in view
		...
		zoomSliderY = new JSlider(JSlider.VERTICAL, 0, 3000, 0); // zooms in view
	...
	private void initializeComponentBox() {
		...
		boolean flag = false;
		if(dcAnalysis.getDottedChartModel().getItemMap(DottedChartModel.ST_TASK).size()>0){
		...
		if(dcAnalysis.getDottedChartModel().getItemMap(DottedChartModel.ST_ORIG).size()>0){
		...
		if(dcAnalysis.getDottedChartModel().getItemMap(DottedChartModel.ST_INST).size()>0){
		...
		if(dcAnalysis.getDottedChartModel().getItemMap(DottedChartModel.ST_EVEN).size()>0){
		...
		if(dcAnalysis.getDottedChartModel().getItemMap(DottedChartModel.ST_DATA).size()>0){
	...
	private void registerGUIListener() {
		componentBox.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						dcAnalysis.getDottedChartModel().sortKeySet(getSortStandard(),isDescCheckBoxSelected());
						dcPanel.repaint();
						dcAnalysis.getOverviewPanel().setDrawBox(true);
						dcAnalysis.getMetricsPanel().displayPerformanceMetrics(); 	// update MetricsPanel
					}
		});
		...
		timeOptionBox.addActionListener(new ActionListener() {
			    public void actionPerformed(ActionEvent e) {
					...
					if (type.equals(dcPanel.getTimeOption()))return;
		...
		relativeTimeOptionBox.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if(base.equals(dcPanel.getRelativeTimeBase())) return;
					if(type.equals(DottedChartPanel.TIME_RELATIVE_TIME)||type.equals(DottedChartPanel.TIME_RELATIVE_RATIO)){
						dcPanel.setTimeBaseHaspMap(dcPanel.getDottedChartModel().getStartDateMap(base));
						dcPanel.setEndTimeBaseHaspMap(dcPanel.getDottedChartModel().getEndDateMap(base));
	...
	public void stateChanged(ChangeEvent e) {
		...
		if((e.getSource()==zoomSliderX)){
			double zoom = StrictMath.pow(10.0, ((double)zoomSliderX.getValue() / 1000.0)) ;
			...
			double zoom = StrictMath.pow(10.0, ((double)zoomSliderY.getValue() / 1000.0)) ;
