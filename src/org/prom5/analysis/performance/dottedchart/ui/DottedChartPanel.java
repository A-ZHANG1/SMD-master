...
package org.prom5.analysis.performance.dottedchart.ui;
...
import org.prom5.analysis.performance.dottedchart.DottedChartAnalysis;
...
import org.prom5.analysis.performance.dottedchart.model.DottedChartModel;
...
public class DottedChartPanel extends JPanel implements MouseListener, MouseMotionListener  {
	...
	public void changeComponentType()
	{
		...
		if(timeOption.equals(TIME_ACTUAL)||timeOption.equals(TIME_RELATIVE_TIME)||timeOption.equals(TIME_RELATIVE_RATIO)) dcModel.calculateStatistics();
		else if(timeOption.equals(TIME_LOGICAL)) dcModel.calculateStatisticsLogical();
		else {
	...
	public int getHashMapSize(){
		if(!dcModel.getTypeHashMap().equals(ST_INST)) return dcModel.getItemMap().size();
		else return dcModel.getInstanceTypeToKeep().size();
	}
	...
	protected void updateMilli2pixelsRatio() {
		milli2pixels = (double) (this.getWidth()- border*2) / (double)(dcModel.getLogBoundaryRight().getTime() - dcModel.getLogBoundaryLeft().getTime()); // + 2 is added to extend bound
	...
	public void setViewportZoomIn() {
		...
		int value = (int) (Math.log10((double)this.getWidth() * (d.getWidth() / width)/(double)dca.getViewportSize().getWidth())*1000.0);
		if(value>3000) return;
		value = (int) (Math.log10((double)this.getHeight() * (d.getHeight() / height)/(double)dca.getViewportSize().getHeight())*1000.0);
		if(value>3000) return;
	...
	protected int time2coord(Date aTimestamp) {
		return (int)((aTimestamp.getTime()- dcModel.getLogBoundaryLeft().getTime()) * milli2pixels);
	}
	...
	protected long coord2timeMillis(double anX) {
		return (long)((double)anX / milli2pixels) + dcModel.getLogBoundaryLeft().getTime();
	}
	...
	public void adjustWidth(){
		...
		if (number == 0) {
			...
			if (idx < 0) dcOptionPanel.changeWidthSort(0);
			else dcOptionPanel.changeWidthSort(idx);
		} else if (number > 100) {
			for (idx = 0; idx < widthDividers.length; idx++)
				if (widthDividers[idx] == widthSort)
					break;
			...
			if (idx >= widthDividers.length) dcOptionPanel.changeWidthSort(widthDividers.length - 1);
			else dcOptionPanel.changeWidthSort(idx);
		}
	}
	...
	protected void calculateCurrentTime() {
		...
		// paint items
		if (dcModel.getItemMap().size() <= 0)  return;
	...
	protected void paintHighligtedItem(int x, int y, Graphics g, String shape) {
		...
		if (shape.equals(STR_NONE)) {
			...
			if(!color.equals(Color.red)) g.setColor(Color.red);
	...
	protected void paintComponentLane(Graphics g){
		...
		double percentileL = dcModel.getTimeStatistics().get(0).getPercentile(dca.getSettingPanel().getPercentileforInstanceL());
		double percentileU = dcModel.getTimeStatistics().get(0).getPercentile(dca.getSettingPanel().getPercentileforInstanceU());
		...
		while (itr.hasNext())
		{
				...
				long tempDuration;
				try{
					tempDuration = (tempList.getRightBoundaryTimestamp(dcModel.getEventTypeToKeep(), dcModel.getInstanceTypeToKeep()).getTime() -
							tempList.getLeftBoundaryTimestamp(dcModel.getEventTypeToKeep(), dcModel.getInstanceTypeToKeep()).getTime());
				} catch (Exception ce)
				{
					tempDuration = 0;
				}
	
				if(dcModel.getTypeHashMap().equals(ST_INST)&&!dcModel.getInstanceTypeToKeep().contains(dimName)) continue;
				g.setColor(bgColor);
				...
				// for bottleneck
				if(dcOptionPanel.getComponentType().equals(DottedChartPanel.ST_INST)&&bBottleneckforInstances&&tempDuration>=percentileL&&tempDuration<=percentileU) g.setColor(Color.red);
				else g.setColor(Color.black);
		...
		for(long timeStart=dcModel.getLogBoundaryLeft().getTime(); timeStart<clipRightTs; timeStart+=dcOptionPanel.getWidthDivider()) {
		...
		if (dcModel.getItemMap().size() > 0) {
			...
			for (Iterator itSets = dcModel.getSortedKeySetList().iterator(); itSets.hasNext(); ) {
				...
				if(dcOptionPanel.getComponentType().equals(DottedChartPanel.ST_INST)&& bBottleneckforInstances){
					...
					double percentile2L = dcModel.getTimeStatistics().get(0).getPercentile(dca.getSettingPanel().
										 getPercentileforInstanceL());
					double percentile2U = dcModel.getTimeStatistics().get(0).getPercentile(dca.getSettingPanel().
										 getPercentileforInstanceU());
					long tempDuration = (tempUnitList2.getRightBoundaryTimestamp(dcModel.getEventTypeToKeep(),
										dcModel.getInstanceTypeToKeep()).getTime() -
										tempUnitList2.getLeftBoundaryTimestamp(dcModel.getEventTypeToKeep(),
										dcModel.getInstanceTypeToKeep()).getTime());
					if (dcOptionPanel.getComponentType().equals(DottedChartPanel.ST_INST) &&
							bBottleneckforInstances && tempDuration >= percentile2L && tempDuration <= percentile2U) bInstances = true;
				}
				// end for bottleneck ////////
				...
				for (Iterator itItm = ((LogUnitList) dcModel.getItemMap().get(key)).iterator(); itItm.hasNext(); ) {
					item = (AbstractLogUnit) itItm.next();
					if(dcModel.getEventTypeToKeep()!=null&&(!dcModel.getEventTypeToKeep().contains(item.getType())
						||!dcModel.getInstanceTypeToKeep().contains(item.getProcessInstance().getName()))) continue;
					...
					if (bDrawLine&&item.getType().equals(dca.getSettingPanel().getEndEvent())) {
						for (Iterator itr = tempUnitList.iterator(); itr.hasNext(); ) {
							...
							if (item2.getElement().equals(item.getElement())&&item2.getProcessInstance().equals(item.getProcessInstance())) {
								paintItemLine(time2coord(item2.getCurrentTimeStamp()) + border,
										currentTop, time2coord(item.getCurrentTimeStamp()) + border,
										gr);
					...
					if(item.getCurrentTimeStamp()==null||item.getCurrentTimeStamp().getTime()<clipLeftTs2||item.getCurrentTimeStamp().getTime()>clipRightTs2) continue;

					// for botteleneck
					if(dcOptionPanel.getComponentType().equals(DottedChartPanel.ST_INST)&& bBottleneck&&olditem!=null&&(item.getCurrentTimeStamp().getTime()-olditem.getCurrentTimeStamp().getTime())>=percentileL
							&& (item.getCurrentTimeStamp().getTime()-olditem.getCurrentTimeStamp().getTime())<=percentileU) {
						paintItemLineBottleneck(time2coord(olditem.getCurrentTimeStamp()) + border,
								currentTop, time2coord(item.getCurrentTimeStamp()) + border,
								gr);
					...
					// paint an item
					if(bInstances) {
						this.paintHighligtedItem(time2coord(item.getCurrentTimeStamp()) + border, currentTop,
								gr,	assignShapeByItem(item));
						if(flag) {
							this.addExportingList(item);
							flag=false;
						}
							...
							this.paintItem(time2coord(item.getCurrentTimeStamp()) + border,
	...
	public void generateBufferedImage(int width, int height) {
		...
		if (dcModel.getItemMap().size() > 0) {
			...
			for (Iterator itSets = dcModel.getSortedKeySetList().iterator(); itSets.hasNext(); ) {
				...
				// iterate through items
				for (Iterator itItm = ((LogUnitList) dcModel.getItemMap().get(key)).iterator();
									  itItm.hasNext(); ) {
					item = (AbstractLogUnit) itItm.next();
					if (dcModel.getEventTypeToKeep() != null &&
							(!dcModel.getEventTypeToKeep().contains(item.getType())
							||
							!dcModel.getInstanceTypeToKeep().contains(item.getProcessInstance().getName())))
						continue;
					// to do
					...
					double milli2pixels = (double) ((width)-10) / (double)(dcModel.getLogBoundaryRight().getTime() - dcModel.getLogBoundaryLeft().getTime()); // + 2 is added to extend bound
						...
						if (item2.getElement().equals(item.getElement())&&item2.getProcessInstance().equals(item.getProcessInstance())) {
							...
							if (x1 + 5 <= p.getX() && p.getX() <= x2 - 5) {
								...
								str += item.getProcessInstance().getName() + " | " +
										...
										+ " |(start)" + item.getTimestamp() + "-(complete)" + item2.getTimestamp() + "<br>";
								tempUnitList.removeEvent(item2);
							}
							break;
						}
