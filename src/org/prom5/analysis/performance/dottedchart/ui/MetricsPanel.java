...
import org.prom5.analysis.performance.dottedchart.model.OneMetricTableModel;
import org.prom5.analysis.performance.dottedchart.model.OverallMetricTableModel;
...
public class MetricsPanel extends JPanel{
	...
	private String[] timeSorts = {"milli", "seconds", "minutes", "hours", //array of "time sort" strings required to fill the timeBox combobox with
	...
	public void displayPerformanceMetrics() {
		...
		try{
			    ...
			    if(i!=0)  key = sortedTitleList.get(i-1);
				else {
					key = aTitles.get(0);
					currentDS = aList.get(i);
				}

				if(i>0&&dcModel.getTypeHashMap().equals(DottedChartPanel.ST_INST)&&!dcModel.getInstanceTypeToKeep().contains(key)) continue;
				size++;

				if(i>0){
					for(int j=1;j<aTitles.size();j++)
					{
						if(aTitles.get(j).equals(key)) currentDS = aList.get(j);
					}
				}
				...
				if(type.equals(DottedChartPanel.TIME_ACTUAL)){
					if (i == 0) {
						...
						table.setValueAt(DateFormat.getInstance().format(dcModel.
				...
				} else if(type.equals(DottedChartPanel.TIME_LOGICAL)||type.equals(DottedChartPanel.TIME_LOGICAL_RELATIVE)){
					if (i == 0) {
				...
				if (i==0) frequencyLabel = new JLabel("# of components: " + currentDS.getN());
				else frequencyLabel = new JLabel("# of dots: " + dcModel.getNumberOfLogUnits(key));
	...
	private String formatDate(Date time) {
		...
		long minutes = (timeStart - days * 24 * 60 * 60 * 1000 -
					   hours * 60 * 60 * 1000) / 1000 / 60;
		long seconds = (timeStart - days * 24 * 60 * 60 * 1000 - hours * 60 * 60 * 1000 -
					   minutes * 60 * 1000) / 1000;
		return String.valueOf(days + "days:" + hours + ":" + minutes + ":" + seconds);
	}

	private String formatRatio(Date time) {
		long timeStart = time.getTime();
		return String.valueOf(timeStart / 100 + "." + (timeStart - timeStart / 100 * 100) + "%");
	}
}
