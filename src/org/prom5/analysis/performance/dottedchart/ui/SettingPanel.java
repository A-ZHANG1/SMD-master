...
public class SettingPanel extends JPanel {
	...
	// for bottleneck
	private JCheckBox checkBottleneckBox = new JCheckBox("Filter based on interval");
	private GUIPropertyInteger percentileBoxL = new GUIPropertyInteger("lower bound (percentile)",90,1,100);
	private GUIPropertyInteger percentileBoxU = new GUIPropertyInteger("upper bound (percentile)",100,1,100);
	private JCheckBox checkBottleneckBoxforInstance = new JCheckBox("Filter based on instances duration");
	private GUIPropertyInteger percentileBoxforInstanceL = new GUIPropertyInteger("lower bound (percentile)",90,1,100);
	private GUIPropertyInteger percentileBoxforInstanceU = new GUIPropertyInteger("upper bound (percentile)",100,1,100);
	...
	public SettingPanel(LogReader aInputLog, DottedChartAnalysis aDottedChartAnalysis, DottedChartPanel aDottedChartPanel, DottedChartModel aDcModel) {
	...
	public void initSettingPanel()
	{
		...
		for(int i=0;i<eventTypeToKeep.size();i++){
			...
			if(temp.equals("start"))fromEventComboBox.setSelectedIndex(i);
			if(temp.equals("complete"))toEventComboBox.setSelectedIndex(i);
		}
	...
	private void initColorPanel() {
		...
		String type = dottedChartPanel.getDottedChartOptionPanel().getColorStandard();
	...
	private void registerGUIListener() {
		applyEventTypeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				dottedChartAnalysis.getMetricsPanel().displayPerformanceMetrics();
				dottedChartAnalysis.getOverviewPanel().setDrawBox(true);
				dottedChartAnalysis.getDottedChartModel().sortKeySet(dottedChartPanel.getDottedChartOptionPanel().getSortStandard(),
						dottedChartPanel.getDottedChartOptionPanel().isDescCheckBoxSelected());
		...
		applySelectedInstancesButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				dottedChartAnalysis.setSelectedInstanceIndices(getSelectionStatus());
				dottedChartAnalysis.getMetricsPanel().displayPerformanceMetrics();
				dottedChartPanel.changeLogical(false);
