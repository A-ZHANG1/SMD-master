...
public class ShapeReference {
	...
	public static String[] shapeList = new String[]{ITEM_HANDLE_CIRCLE, ITEM_HANDLE_TRIANGLE, ITEM_HANDLE_BOX, ITEM_HANDLE_RHOMBUS, ITEM_HANDLE_ROUND_BOX,
									   ITEM_HANDLE_DRAW_CIRCLE, ITEM_HANDLE_DRAW_TRIANGLE, ITEM_HANDLE_DRAW_BOX,  ITEM_HANDLE_DRAW_ROUND_BOX, ITEM_HANDLE_DRAW_RHOMBUS};
...
//	protected Random rnd = null;
	 ...
	 * @param key the key to map a color to
	 ...
	 * @param key previously assigned key
	 */
...
/*
 /**
	...
	/*protected void paintItem(int x, int y, Graphics g) {
		if(itemHandle.equals(DottedChartPanel.ITEM_HANDLE_NONE)) {
			return;
		}
		if(itemHandle.equals(DottedChartPanel.ITEM_HANDLE_BOX)) {
			g.fillRect(x-5, y-5, 11, 11);
		} else if(itemHandle.equals(DottedChartPanel.ITEM_HANDLE_CIRCLE)) {
			g.fillOval(x-5, y-5, 11, 11);
		} else if(itemHandle.equals(DottedChartPanel.ITEM_HANDLE_RHOMBUS)) {
			int rhombX[] = {x, x-5, x, x+5};
			int rhombY[] = {y-5, y, y+5, y};
			g.fillPolygon(rhombX, rhombY, 4);
		} else if(itemHandle.equals(DottedChartPanel.ITEM_HANDLE_TRIANGLE)) {
			int triX[] = {x, x-5, x+5};
