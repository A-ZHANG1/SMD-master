...
import org.prom5.analysis.hierarchicaldatavisualization.HierarchicalDataVisualizationResult;
import org.prom5.analysis.originatoravailability.OriginatorAvailability;
import org.prom5.analysis.originatoravailability.ShiftTimeIntervals;
...
public class ExecutionTimesResultUI extends JPanel implements Provider{
	...
	public void jbInit() throws Exception {
		...
		useOntologies = new JComboBox(new String[] { DON_T_USE_ONTOLOGIES, USE_TASK_ONTOLOGY, USE_ORIGINATOR_ONTOLOGY});
		...
		start.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				OriginatorAvailability availability = new OriginatorAvailability(log, new ShiftTimeIntervals(hoursPerShift.getNumber().intValue()));
				
				availabilityModel = availability.toTableModel();
				executionTimesresult = new ExecutionTimesResult(log, availability, 
						USE_TASK_ONTOLOGY.equals(useOntologies.getSelectedItem()),
						USE_ORIGINATOR_ONTOLOGY.equals(useOntologies.getSelectedItem()));
				
				stat = new JComboBox(new String[] { "Average", "Sum", "Frequency", "Standard deviation", "Variance", "Minimum", "Maximum" });
				...
				measurementsPanel.add(new JScrollPane(table), BorderLayout.CENTER);
				...
				for (int i = 1; i < availabilityTable.getColumnCount(); i++) {
					...
					renderer.setHorizontalAlignment(SwingConstants.RIGHT);
					availabilityTable.getColumnModel().getColumn(i).setCellRenderer(renderer);
				}
				
				hierarchicalDataVisualization = new HierarchicalDataVisualizationResult(
						createHierarchicalData(
								executionTimesresult.getRawMeasurements(),
								USE_TASK_ONTOLOGY.equals(useOntologies.getSelectedItem()),
								USE_ORIGINATOR_ONTOLOGY.equals(useOntologies.getSelectedItem()))
						);
				
				JTabbedPane tabs = new JTabbedPane();
				tabs.addTab("Graphical View of Execution Times", hierarchicalDataVisualization);
				tabs.addTab("Originator vs Task", measurementsPanel);
				tabs.addTab("Task Statistics", new JScrollPane(taskTable));
				tabs.addTab("Originator Availability", new JScrollPane(availabilityTable));
	...
	protected HierarchicalData createHierarchicalData(ExecutionTimes executionTimes, boolean useTaskOntologies, boolean useOriginatorOntologies) {
		return new HierarchicalExecutionTimesInOntologies(log, executionTimes, useTaskOntologies, useOriginatorOntologies);
	}
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		if (hierarchicalDataVisualization != null) {
			ProvidedObject[] hdv = hierarchicalDataVisualization.getProvidedObjects();
			
			if (hdv.length > 0) {
				objects.add(new ProvidedObject("Table with raw numbers", hdv[0].getObjects()));
			}
		}
		if (stat != null && stat.getSelectedItem() != null && model != null) {
			objects.add(new ProvidedObject(stat.getSelectedItem() + " of Originator vs Task Matrix", model)); 
		}
		if (taskModel != null) {
			objects.add(new ProvidedObject("Task Statistics Matrix", taskModel));
		}
		if (availabilityModel != null) {
			objects.add(new ProvidedObject("Originator Availability", availabilityModel));
		}
...
class ColoredCellRenderer extends DefaultTableCellRenderer {
	...
	public Component getTableCellRendererComponent(JTable table, Object value,  boolean isSelected, boolean hasFocus, int row,  int column) {
		...
		setBackground(value.toString().equals("true") ? new Color(0, 0xFF, 0) : new Color(0xFF, 0xFF, 0xFF));
		return this;
	}
}
