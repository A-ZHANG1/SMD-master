package org.prom5.analysis.performance.fsmanalysis;
...
public class FSMAnalysisPlugin extends JPanel implements Provider {
	...
	public boolean analysis(LogReader log) {
	   FsmSettings settings = ((FsmMinerPayload)acceptedFSM.getStartState().getPayload()).getSettings(); 
            ...
            for (ProcessInstance pi : log.getInstances()) {
                ...
                for (int i = 0; i < atel.size(); i++) {
                    ...
                    // for node
                    if (i == 0) {
                    	if(fsmStatistics.getRemainingMap().get(fromPayload.toString())!=null) {
                    		if(ate.getTimestamp()!=null)
                    		 fsmStatistics.getRemainingMap().get(fromPayload.toString()).addValue(endTime-ate.getTimestamp().getTime());
                    	}
                    	if(fsmStatistics.getElapsedMap().get(fromPayload.toString())!=null)
                    		fsmStatistics.getElapsedMap().get(fromPayload.toString()).addValue(0);
                    	if(fsmStatistics.getSojournMap().get(fromPayload.toString())!=null)
                    		fsmStatistics.getSojournMap().get(fromPayload.toString()).addValue(0);
                    	// for edge
                    	String key = fromPayload.toString()+":"+toPayload.toString();
                    ...
                    if(fsmStatistics.getElapsedMap().get(toPayload.toString())!=null) {
                    	if(ate.getTimestamp()!=null) {
                    		fsmStatistics.getElapsedMap().get(toPayload.toString()).addValue(ate.getTimestamp().getTime()-startTime);
                    	}
                    }
                    
            	    if( fsmStatistics.getRemainingMap().get(toPayload.toString())!=null) {
            	    	if(ate.getTimestamp()!=null) {
            	    		fsmStatistics.getRemainingMap().get(toPayload.toString()).addValue(endTime-ate.getTimestamp().getTime());
            	    	}
                   	}
                   	if ((i!= atel.size()-1)&&(fsmStatistics.getSojournMap().get(toPayload.toString())!=null)) {
                   		if(ate.getTimestamp()!=null&&atel.get(i+1).getTimestamp()!=null) {
                   			fsmStatistics.getSojournMap().get(toPayload.toString()).addValue(atel.get(i+1).getTimestamp().getTime()-ate.getTimestamp().getTime());
                   		}
                   	} else {
                   		if(fsmStatistics.getSojournMap().get(toPayload.toString())!=null)
                   		fsmStatistics.getSojournMap().get(toPayload.toString()).addValue(0);
                   	}
                   	...
                   	if(fsmStatistics.getEdgeMap().get(key)!=null){
	                   	if (i == 0) {
	                   		...
	                   		if(ate.getTimestamp()!=null&&atel.get(i-1).getTimestamp()!=null) {
	                   			fsmStatistics.getEdgeMap().get(key).addValue(ate.getTimestamp().getTime() - atel.get(i-1).getTimestamp().getTime());
	                   		}
	                   	}    
                   	}
               }
   ...
   private void buildDS(AcceptFSM graph) {
		...
		{			
			...
			fsmStatistics.getElapsedMap().put(((FSMState)as).getLabel(), DescriptiveStatistics.newInstance());
			fsmStatistics.getRemainingMap().put(((FSMState)as).getLabel(), DescriptiveStatistics.newInstance());
			fsmStatistics.getSojournMap().put(((FSMState)as).getLabel(), DescriptiveStatistics.newInstance());
		}
		
		for(Object obj: graph.getEdges())
		{
			ModelGraphEdge edg = (ModelGraphEdge)obj;		
	   		Message.add(edg.getId()+","+((FSMTransition)edg).getCondition(), Message.DEBUG);
	   		fsmStatistics.getEdgeMap().put(makeKey(edg), DescriptiveStatistics.newInstance());
		}
	}
   ...
   public void initAnalysisMenuUI() {
	...
	public Map<String,LogReader> getLogReaders() {
		if(logReaders==null) updateFrameworkResources();
		return logReaders;
	}
				...
				for(ProvidedObject providedObject : providedObjects) {
					for(Object object : providedObject.getObjects()) {
					    if(object instanceof LogReader) {
							logReaders.put(frame.getTitle() + " - " + providedObject.getName(), (LogReader)object);
						}
					}
				}
			}
	...
	public ProvidedObject[] getProvidedObjects() {
		if(ui!=null) return ui.getProvidedObjects();
		return null;
	}
}
