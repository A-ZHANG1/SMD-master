...
public class FSMPerformanceAnalysisUI extends JPanel implements GuiNotificationTarget {
	...
	public void initGraphMenu()
	{
		...
		{
			if(overallMax<getData(ds)) overallMax = getData(ds); 
		}
		
		overallEdgeMax = 0.0;
		for(DescriptiveStatistics ds :edgeInterMap.values())
		{
			if(overallEdgeMax<getData(ds)) overallEdgeMax = getData(ds); 
		}
		findCriticalPath();
		adjustTimeScale();
	...
	private void restoreAcceptFSM()
	{
		 for(ModelGraphVertex as: acceptFSM.getVerticeList())
		   {
			 ((FSMState)as).setLabel(getActualLabel(((FSMState)as).getLabel()));
	...
	private void adjustTimeScale()
    {
   	   for(ModelGraphVertex as: acceptFSM.getVerticeList())
	   {
		   ...
		   if(getTimeMap().get(key)!=null) {
			   String value = String.valueOf("sojourn = "+ getSubString(String.valueOf(getData((DescriptiveStatistics)suSojournMap.get(key))/(double)getTimeUnit())));
			   value += String.valueOf("\\n elapsed = "+getSubString(String.valueOf(getData(suElapsedMap.get(key))/(double)getTimeUnit())));
			   value += String.valueOf("\\n remaining = "+getSubString(String.valueOf(getData(suRemainingMap.get(key))/(double)getTimeUnit())));
   		   ...
   		   if(true) {
			   if(criticalEdge.contains(edg)) {
	   		   ...
	   		   if(edgeInterMap.get(key)!=null) {
				   ...
				   if(tempValue<overallEdgeMax*(((double)min.getValue())/100.0)) {
					   edg.setDotAttribute("color","blue");
				   } else if(tempValue<overallEdgeMax*(((double)max.getValue())/100.0)) {
					   edg.setDotAttribute("color","yellow");
				   } else {
					   edg.setDotAttribute("color","red");
				   }
...
//	}
	...
	private void findCriticalPath()
	{
		...
		for(FSMState st: fsms)
		{
			String key = getActualLabel((st).getLabel());
			if(suElapsedMap.get(key)!=null) {
				if(getData(suElapsedMap.get(key))>=pathMax*(((double)max.getValue())/100.0)) {
		...
		for(FSMState st: fsms)
		{
			String key = getActualLabel((st).getLabel());
			if(suElapsedMap.get(key)!=null) {
				if(getData(suElapsedMap.get(key))>=pathMax*(((double)min.getValue())/100.0)
						&& getData(suElapsedMap.get(key))<pathMax*(((double)max.getValue())/100.0)) {
					inbtwnState.add(st);
					for(ModelGraphEdge edge: getAllEdgesTo(st)) {
						if(!criticalEdge.contains(edge)) inbtwnEdge.add(edge);
					}
				}
			}
		}
	...
	public boolean isInPath(ModelGraphVertex v1, ModelGraphVertex v2, HashSet<ModelGraphVertex> vs)
	{
		Iterator<ModelGraphEdge> it =  v1.getOutEdgesIterator();
		while (it.hasNext()) {
			ModelGraphEdge e =  it.next();
			if (e.getDest() == v1) continue;
			if(vs.contains(e.getDest())) return false;
			if (e.getDest() == v2) {
				return true;
			} else {
				vs.add(v1);
				if(e.getDest().getOutEdges()!=null&&e.getDest().getOutEdges().size() > 0)
					if(isInPath(e.getDest(), v2, vs)) return true;
			}
		}
		return false;
	}
