...
public class FSMEvaluationAnalysisUI extends JPanel implements GuiNotificationTarget, Provider {
	...
	public FSMEvaluationAnalysisUI(AcceptFSM fsm, FSMEvaluationStatistics fsmStat) {
		acceptFSM = fsm;
	...
	public void initGraphMenu()
	{
		...
		{
			if(overallMax<ds.getSum()) overallMax = ds.getSum(); 
		}
		adjustTimeScale();
	...
	private void adjustTimeScale()
    {
   	   for(ModelGraphVertex as: acceptFSM.getVerticeList())
	   {
		   ...
		   if(getTimeMap().get(key)!=null) {
			   String value = String.valueOf("\\n MSE = "+getSubString(String.valueOf(suMSEMap.get(key).getMean())));
			   value += String.valueOf("\\n Frequency = "+String.valueOf(suMSEMap.get(key).getN()));
			   StringBuffer tempBuffer = new StringBuffer();
			   tempBuffer.append(tag(key,TD));
			   tempBuffer.append(tag(getSubString(String.valueOf(suMAEMap.get(key).getMean())),TD));
			   tempBuffer.append(tag(getSubString(String.valueOf(Math.sqrt(suMSEMap.get(key).getMean()))),TD));
			   tempBuffer.append(tag(getSubString(String.valueOf(suMAPEMap.get(key).getMean())),TD));
			   tempBuffer.append(tag(String.valueOf(suMAEMap.get(key).getN()),TD));
			   ...
			   if(getTimeMap().get(key).getSum()>=0.0){
				   ...
				   if(tempValue<overallMax*(((double)min.getValue())/100.0)) {
					   as.setDotAttribute("color","blue");
				   } else if(tempValue<overallMax*(((double)max.getValue())/100.0)) {
	   ...
	   tempBuffer.append(tag(getSubString(String.valueOf(fsmStatistics.getOverallMAE().getMean())),TD));
	   tempBuffer.append(tag(getSubString(String.valueOf(fsmStatistics.getOverallRMSE().getMean())),TD));
	   tempBuffer.append(tag(getSubString(String.valueOf(fsmStatistics.getOverallMAPE().getMean())),TD));
	   tempBuffer.append(tag(String.valueOf(fsmStatistics.getOverallMAPE().getN()),TD));
	   sb.append(tag(tempBuffer.toString(),TR));
	   tempBuffer = new StringBuffer();
	   tempBuffer.append(tag("overall(aggregated mean)",TD));
	   tempBuffer.append(tag(getSubString(String.valueOf(fsmStatistics.getOverallMAEAggre())),TD));
	   tempBuffer.append(tag(getSubString(String.valueOf(fsmStatistics.getOverallRMSEAggre())),TD));
	   tempBuffer.append(tag(getSubString(String.valueOf(fsmStatistics.getOverallMAPEAggre())),TD));
	   tempBuffer.append(tag(String.valueOf(fsmStatistics.getNumber()),TD));
	   sb.append(tag(tempBuffer.toString(),TR));
	   sb.append("</table></body></html>");
	   Message.add("Overall MAE value = " + fsmStatistics.getOverallMAE().getSum(), Message.NORMAL);
	   Message.add("Overall RMSE value = " + fsmStatistics.getOverallRMSE().getSum(), Message.NORMAL);
	   Message.add("Overall MAPE value = " + fsmStatistics.getOverallMAPE().getSum(), Message.NORMAL);
   }
...
//	}
	...
	private String getSubString(String str)   {
		...
		if(d>=0.000001&&d<=100000||d==0) return str.substring(0, Math.min(str.length(), 7));;
	...
	public HashSet<ModelGraphEdge> getAllEdgesTo(ModelGraphVertex v2) {
		...
		for(Object obj: acceptFSM.getEdges())
		{
			...
			if(isInPath(edge,v2)) s.add(edge);
		}
		return s;
	}
	...
	public boolean isInPath(ModelGraphVertex v1, ModelGraphVertex v2)
	{
		...
		while (it.hasNext()) {
			...
			if (e.getDest()==v1) continue;
			if (e.getDest() == v2) {
				return true;
			} else {
				if(e.getDest().getOutEdges()!=null&&e.getDest().getOutEdges().size() > 0)
					if(isInPath(e.getDest(), v2)) return true;
			}
		}
		return false;
	}
	...
	public boolean isInPath(ModelGraphEdge e1, ModelGraphVertex v2)
	{
		...
		if (e1.getDest() == v2) {
			...
			if(e1.getSource()!=e1.getDest()&&e1.getDest().getOutEdges()!=null
	...
	public ProvidedObject[] getProvidedObjects() {
		return new ProvidedObject[] {
				new ProvidedObject("Error Statistics", suMSEMap),
		};
	}
}
