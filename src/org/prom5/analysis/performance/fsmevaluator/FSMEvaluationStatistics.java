...
import org.prom5.mining.fsm.FsmHorizonSettings;
import org.prom5.mining.fsm.FsmMinerPayload;
import org.prom5.mining.fsm.FsmSettings;
...
public class FSMEvaluationStatistics extends FSMStatistics{
	...
	public DescriptiveStatistics getOverallRMSE()
	{
		...
		for(String str:suMSEMap.keySet()) {
			if(String.valueOf(suMSEMap.get(str).getMean()).equals("NaN")) continue;
			overAllMSEData.addValue(Math.sqrt(suMSEMap.get(str).getMean()));
			Message.add("Error value = " + suMSEMap.get(str).getMean(), Message.NORMAL);
		}
	...
	public DescriptiveStatistics getOverallMAE()
	{
		...
		for(String str:suMAEMap.keySet()) {
			if(String.valueOf(suMAEMap.get(str).getMean()).equals("NaN")) continue;
			overAllMAEData.addValue(suMAEMap.get(str).getMean());
			Message.add("Error value = " + suMAEMap.get(str).getMean(), Message.NORMAL);
		}
	...
	public DescriptiveStatistics getOverallMAPE()
	{
		...
		for(String str:suMAPEMap.keySet()) {
			if(String.valueOf(suMAPEMap.get(str).getMean()).equals("NaN")) continue;
			overAllMAPEData.addValue(suMAPEMap.get(str).getMean());
			Message.add("Error value = " + suMAPEMap.get(str).getMean(), Message.NORMAL);
		}
	...
	public double getOverallRMSEAggre()
	{
		...
		for(String str:suMSEMap.keySet()) {
			if(String.valueOf(suMSEMap.get(str).getMean()).equals("NaN")) continue;
			overAllMSEaggre += Math.sqrt(suMSEMap.get(str).getMean()) * suMSEMap.get(str).getN();
	...
	public double getOverallMAEAggre()
	{
		...
		for(String str:suMAEMap.keySet()) {
			if(String.valueOf(suMAEMap.get(str).getMean()).equals("NaN")) continue;
			overAllMAEaggre += suMAEMap.get(str).getMean() * suMAEMap.get(str).getN();
	...
	public double getOverallMAPEAggre()
	{
		...
		for(String str:suMAPEMap.keySet()) {
			if(String.valueOf(suMAPEMap.get(str).getMean()).equals("NaN")) continue;
			overAllMAPEaggre += suMAPEMap.get(str).getMean() * suMAPEMap.get(str).getN();
	...
	public void analysis(LogReader log, AcceptFSM fsm, FSMStatistics fsmStatistics, String estimator) {
		this.fsmStatistics = fsmStatistics;
		this.estimator = estimator;
		   FsmSettings settings = ((FsmMinerPayload)fsm.getStartState().getPayload()).getSettings(); 
	        try {
//	        	Progress progress = null;
	            int progressCtr = 0;
	            if (settings.hasGUI()) {
	                // First count how many stpes we have to do for the progress bar.
	                progressCtr = 0;
...
//	                                        progressCtr - 1);
	            ...
	            for (ProcessInstance pi : log.getInstances()) {
	                ...
	                for (int i = 0; i < atel.size(); i++) {
	                    ...
	                    /**
	                     * An AuditTrailEntry corresponds to a transition in the FSM.
	                     * First, construct the payload of the state preceding the transition.
	                     */
	                    ...
	                    for (int mode = 0; mode < FsmMinerPayload.LAST; mode++) {
	                        mineBwd(atel, i, 1,
	                                settings.getHorizonSettings(true, mode),
	                                fromPayload, mode);
	                        mineFwd(atel, i, 0,
	                                settings.getHorizonSettings(false, mode),
	                                fromPayload, mode);
	                    }
	                    ...
	                    if (settings.getUseAttributes()) {
	                        for (int j = 0; j < i; j++) {
	                            ...
	                            for (String attribute : dataSection.keySet()) {
	                                if (settings.getAttributeSettings().containsKey(
	                                        attribute)) {
	                                    String cluster = settings.
	                                            getAttributeSettings().get(
	                                            attribute).get(dataSection.get(
	                                            attribute));
	                                    if (cluster != null) {
	                    ...
	                    /**
	                     * Second, in a similar way, create the payload of the state succeding the transition.
	                     */
	                    ...
	                    if (i == 0) {
	                    	if(getMSEMap().get(fromPayload.toString())!=null) {
	                    		if(ate.getTimestamp()!=null){
		                    		double remaingTime = endTime-ate.getTimestamp().getTime();
		                    		double remaingTimeMean = getEstimator(fsmStatistics.getRemainingMap().get(fromPayload.toString()));
		                    		if(estimator.equals("HEURISTIC1")){
		                    			remaingTimeMean = fsmStatistics.getRemainingMap().get("[{}]").getMean()/2;
		                    		} else if(estimator.equals("HEURISTIC2")){
		                    			double elapsedTime = ate.getTimestamp().getTime() - startTime;
		                    			double heurst = fsmStatistics.getRemainingMap().get("[{}]").getMean();
		                    			remaingTimeMean = Math.max(0.0,heurst-elapsedTime);
		                    		}
		                    		getMSEMap().get(fromPayload.toString()).addValue(getMSE(remaingTimeMean,remaingTime)); 
		                    		getMAEMap().get(fromPayload.toString()).addValue(getMAE(remaingTimeMean,remaingTime)); 
		                    		getMAPEMap().get(fromPayload.toString()).addValue(getMAPE(remaingTimeMean,remaingTime)); 
	                    		}
	                    	}
	                    }
	                    ...
	                    if(getMSEMap().get(toPayload.toString())!=null) {
	                    	if(ate.getTimestamp()!=null){
	                    		double remaingTime = endTime-ate.getTimestamp().getTime();
	                    		double remaingTimeMean = getEstimator(fsmStatistics.getRemainingMap().get(toPayload.toString()));
	                    		if(estimator.equals("HEURISTIC1")){
	                    			remaingTimeMean = fsmStatistics.getRemainingMap().get("[{}]").getMean()/2;
	                    		}else if(estimator.equals("HEURISTIC2")){
	                    			double elapsedTime = ate.getTimestamp().getTime() - startTime;
	                    		...
	                    		getMSEMap().get(toPayload.toString()).addValue(getMSE(remaingTimeMean,remaingTime)); 
	                    		getMAEMap().get(toPayload.toString()).addValue(getMAE(remaingTimeMean,remaingTime)); 
	                    		getMAPEMap().get(toPayload.toString()).addValue(getMAPE(remaingTimeMean,remaingTime));
	                    	}
	                    }
	               }
	            }
	   ...
	   {
		   if(estimator.equals("MEAN")) return ds.getMean();
		   else if(estimator.equals("MEDIAN")) return ds.getPercentile(50);
		   else if(estimator.equals("MIN")) return ds.getMin();
		   else if(estimator.equals("MAX")) return ds.getMax();
		   return 0.0;
	   }
	   
	   /**
	    * Collect infromation for the payload of a given state by taking backwards step.
	    * @param atel AuditTrailEntryList the list of audti trail entries for this process instance.
	    * @param i int the index of the current audit trail entry in this list.
	    * @param offset int offset from i to start taking steps.
	    * @param settings FsmHorizonSettings the settings to use.
	    * @param payload FsmMinerPayload the payload to store the results in.
	    * @param mode int whether to collect model element info, originator info, or event type info.
	    */
	   private static void mineBwd(AuditTrailEntryList atel, int i, int offset,
	                               FsmHorizonSettings settings,
	                               FsmMinerPayload payload, int mode) {
	           ...
	           for (int j = i - offset; horizon != 0 && j >= 0; j--) {
	               try {
	                   ...
	                   if (settings.getFilter().contains(s)) {
	                       // Not filtered out, add to payload if number of steps not exceeded.
	   ...
	   /**
	    * Collect infromation for the payload of a given state by taking forward steps.
	    * @param atel AuditTrailEntryList the list of audti trail entries for this process instance.
	    * @param i int the index of the current audit trail entry in this list.
	    * @param offset int offset from i to start taking steps.
	    * @param settings FsmHorizonSettings the settings to use.
	   ...
	   public void buildDS(AcceptFSM graph, long timeunit) {
		   this.timeunit = timeunit;
			...
			{			
				Message.add(as.getId()+","+((FSMState)as).getLabel(), Message.DEBUG);
