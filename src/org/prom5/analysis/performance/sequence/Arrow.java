...
public class Arrow {
	 ...
	 * @param sourceBlock DataElementBlock
	 */
	 ...
	 * @param destinationBlock DataElementBlock
	 */
	...
	public boolean isOnLine (Point p, HashMap lifeLines, double scale) {
		...
		if (sourceLine != null && destLine != null) {
			...
			Line2D l = new Line2D.Double(Math.round(startX * scale), Math.round(startAt * scale),
					   Math.round(endX * scale), Math.round(endAt * scale));
