...
public class FilterOptions extends JPanel {
	 ...
	 * @param log LogReader                   : the log out of which process instances are obtained
	 * @param selectedInstanceIndices int[]   : the indices of the process instances that are currently
     *               	                        selected (initially all selected)
	 * @param sequences ArrayList              : the existing sequences
	 * @param patterns ArrayList              : the existing patterns
	 * @param timeSort String                 : the time sort used
	 * @param timeDivider long                : the time divider used
	 * @param patternDrawn boolean            : if true, pattern options should be available
	 * @param psa PerformanceSequenceAnalysis : the object from which this constructor was called
	 */
	public FilterOptions(LogReader log, int[] selectedInstanceIndices, ArrayList sequences,
			ArrayList patterns, String timeSort, long timeDivider, boolean patternDrawn,
	...
	private void jbInit(boolean patternDrawn) throws Exception {
		...
		if (sequences != null && sequences.size()>0) {
			...
			gbc.gridx = 6; gbc.gridy = 3; gbc.gridwidth = 1;
			searchPanel.add(l3, gbc);
			gbc.gridx = 7; gbc.gridy = 3; gbc.gridwidth = 1;
			...
			gbc.gridx = 8; gbc.gridy = 3; gbc.gridwidth = 1;
			l5.setText(" " + timeSort);
			searchPanel.add(l5, gbc);
		}
		if (patternDrawn && patterns != null && patterns.size()!=0) {
			bg.add(r2); bg.add(r3); bg.add(r4);
			...
			gbc.gridx = 3; gbc.gridy = 5; gbc.gridwidth = 1;
			...
			gbc.gridx = 5; gbc.gridy = 6; gbc.gridwidth = 1;
			gbc.anchor = GridBagConstraints.WEST;
			searchPanel.add(selectionBox1, gbc);
			gbc.gridx = 6; gbc.gridy = 6; gbc.gridwidth = 1;
			searchPanel.add(l3, gbc);
			gbc.gridx = 7; gbc.gridy = 6; gbc.gridwidth = 1;
		...
		gbc.gridx = 2; gbc.gridy = 7; gbc.gridwidth = 4;
	...
	private void selectInstances(int[] indices) {
		...
		processInstanceIDsTable.getSelectionModel().removeSelectionInterval(0, processInstanceIDsTable.getRowCount()-1);
	...
	/**
	 * Updates the current selection status of the table containing the process instances.
	 * This means that instances that meet the requirements of the filled in options
	 * get selected
	 */
	private void updateSelectionStatus() {
		...
		if (r0.isSelected()) {
			...
			for (int i = 0; i < sequences.size(); i++) {
				...
				try {
					if ((selectionBox0.getSelectedIndex() == 0
							&& sequence.getThroughputTime() > throughputBound * timeDivider)
							|| (selectionBox0.getSelectedIndex() == 1
							&& sequence.getThroughputTime() < throughputBound * timeDivider)) {
			...
			for (int i = 0; i < patterns.size(); i++) {
				...
				if (r2.isSelected()) {
					...
					try {
						...
						if ((selectionBox1.getSelectedIndex() == 0
								&& pattern.getMeanThroughputTime() > throughputBound * timeDivider)
								|| (selectionBox1.getSelectedIndex() == 1
								&& pattern.getMeanThroughputTime() < throughputBound * timeDivider)) {
							piNames.addAll(pattern.getPiNames());
						}
	 ...
	 */
/*8	private ArrayList getSelectedInstances() {
		ArrayList selectedInstances = new ArrayList();
		int[] indexArray = getSelectionStatus();
		for (int i = 0; i < indexArray.length; i++) {
			selectedInstances.add(log.getInstance(indexArray[i]));
		}
