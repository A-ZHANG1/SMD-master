...
public class Pattern {
	 ...
	 * the first sequence of the pattern.
	 * (sorted first on begin time, then on end time, then on data-element)
	 */
	private ArrayList sortedDataEltBlocks;
	/**
	 * if strict-equivalent pattern type selected:
	 * ArrayList containing the data-element blocks in the order they appear in
	 * the first sequence of the pattern.
	 * (sorted first on end time, then on begin time, then on data-element)
	 */
	 ...
	 */
	private DescriptiveStatistics timeStatistics = DescriptiveStatistics.newInstance();
	/**
	...
	public Pattern(Sequence sq) {
		...
		for (int i=0; i < sequenceBlocks.size(); i++) {
			...
			PatternBlock patBlock = new PatternBlock(beginTime, endTime, block.getDataElement());
		...
		for (int i=0; i < arrows.size(); i++) {
			...
			PatternArrow patArrow = new PatternArrow(beginTime, endTime, arrow.getSource(), arrow.getDestination());
	...
	/**
	 * Checks whether same arrows appear and whether they appear in the same order.
	 * If this is the case, then: 'the arrows are equal' and true is returned.
	 * @param firstArrows ArrayList
	 * @param secondArrows ArrayList
	 * @param isStrict boolean
	 * @return boolean
	 */
	private boolean equalArrows(ArrayList firstArrows, ArrayList secondArrows, boolean isStrict) {
		if (firstArrows.size()!=secondArrows.size()) {
			...
			for (int i=0; i < firstArrows.size(); i++) {
				...
				if (!firstArrow.getSource().equals(secondArrow.getSource())
					...
					if (
						firstArrow.getSourceBlock().getSimilarIndex() != secondArrow.getSourceBlock().getSimilarIndex()
						|| firstArrow.getDestinationBlock().getSimilarIndex() != secondArrow.getDestinationBlock().getSimilarIndex()) {
	 ...
	 * @param parts0 ArrayList : sorted array with data-element blocks
	 * @param parts1 ArrayList : sorted array with data-element blocks
	 * @param isStrict boolean : true if strict-equivalence is used
	 * @return boolean
	 */
	private boolean equalParts (ArrayList parts0, ArrayList parts1, boolean isStrict) {
			...
			for (int i=0; i<parts0.size(); i++) {
				...
				if (!firstBlock.getDataElement().equals(secondBlock.getDataElement())) {
					...
					for (int j=0; j<parts0.size(); j++) {
						DataElementBlock afterFirst = (DataElementBlock) parts0.get(j);
						SequenceBlock afterSecond = (SequenceBlock) parts1.get(j);
						if (firstBlock instanceof PatternBlock && afterFirst instanceof PatternBlock) {
							if (((PatternBlock)afterFirst).getAverageBeginTime()
									> ((PatternBlock)firstBlock).getAverageEndTime()
									&& !(afterSecond.getBeginTimestamp().getTime()
									>= secondBlock.getEndTimestamp().getTime())) {
								//blocks don't appear in the same order
								return false;
							}
						}
						else if (firstBlock instanceof SequenceBlock && afterFirst instanceof SequenceBlock) {
							if (((SequenceBlock)afterFirst).getBeginTimestamp().getTime()
									> ((SequenceBlock)firstBlock).getBeginTimestamp().getTime()
									&& !(afterSecond.getBeginTimestamp().getTime()
									>= secondBlock.getEndTimestamp().getTime())) {
								//blocks don't appear in the same order
								return false;
	...
	/**
	 * Returns the average time between the beginning of a sequence of this pattern
	 * and the Timestamp at which data-element part with number 'number' starts
	 * @param number int
	 * @return double
	 */
	...
	/**
	 * Returns the average time between the beginning of a sequence of this pattern
	 * and the Timestamp at which arrow number 'number' began and ended
	 * @param number int
	 * @return double
	 */
	...
	public void drawPattern(HashMap lifeLines, int startY, Graphics2D g) {
		...
		for (int i = 0; i < sortedDataEltBlocks.size(); i++) {
			DataElementBlock block = (DataElementBlock) sortedDataEltBlocks.get(i);
			try {
				block.drawBlock(((LifeLine) lifeLines.get(block.getDataElement())).getMiddle() - 10,
						color, g);
