...
public class PatternDiagram extends JPanel implements MouseMotionListener {
	...
	/**
	 * map in which the key is frequence of occurence of patterns
     * and value the set of patterns in the diagram that have this
	 * frequency
	 */
	...
	/**
	 * Returns a sorted array, containing the (sorted) frequencies of the patterns
	 * in the diagram
	 * @return int[]
	 */
	 ...
	 * @param patterns HashMap     : map in which the key is frequence of occurence of patterns
	 *                               and value the set of patterns in the diagram that have this
	 *                               frequency
	 * @param dataElts ArrayList   : list containing the data-elements in this diagram
	 * @param timeDivider long     : the timedivider used
	 * @param scale float          : the scale at which the diagram is to be drawn
	 * @param timePerPixel double  : the time per pixel
	 */
	public void initializePaint(HashMap patterns, ArrayList dataElts, long timeDivider,
	...
	public void paintComponent(Graphics g) {
		...
		if (paintDiagram) {
			...
			while (it.hasNext()) {
				try {
					LifeLine lifeLine = (LifeLine) lifeLines.get((String) it.next());
			...
			for (int j = sortedArray.length - 1; j >= 0; j--) {
				...
				while (itr.hasNext()) {
					...
					if (visible.getMinY() <= endY * scale && (startY - 5)* scale <= visible.getMaxY()) {
	...
	/**
	 * Check if the cursor is on a data-element block or on an arrows, if
	 * so display, information about the pattern that the arrow or data-element
	 * belongs to
	 * @param e MouseEvent
	 */
	public void mouseMoved(MouseEvent e) {
		...
		if (patterns.size()>0 && tooltipsOn) {
			...
			while (itr.hasNext() && !onPattern) {
				...
				ArrayList sortedDataElementBlocks = pattern.getSortedDataElementBlocks();
				for (int i = 0; i < sortedDataElementBlocks.size(); i++) {
					PatternBlock block = (PatternBlock) sortedDataElementBlocks.get(i);
					if (block.isInBlock(p, lifeLines, scale)) {
						str += "Pattern " + pattern.getPatternNumber()
					...
					for (int i = 0; i < arrowList.size(); i++) {
						...
						if (arrow.isOnLine(p, lifeLines, scale)) {
							...
							str += "Pattern " + pattern.getPatternNumber()
									...
									+ ((arrow.getEndAt() - arrow.getStartAt()) * timePerPixel) / timeDivider
									+ "<br>";
							onArrow = true;
						}
					}
