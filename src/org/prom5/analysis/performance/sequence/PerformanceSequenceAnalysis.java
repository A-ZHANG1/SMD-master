...
package org.prom5.analysis.performance.sequence;
...
public class PerformanceSequenceAnalysis extends JPanel implements Provider{
	...
	/**
	 * list to store the names of data-elements of the selected data-element/component type
	 */
...
//	private JLabel detailLabel = new JLabel("100.0%");
	...
	private String[] timeSorts = {"milliseconds", "seconds", "minutes", "hours",
								 "days", "weeks", "months", "years"};
	private long[] dividers = {1, 1000, 60000, 3600000L, 86400000L, 604800000L,
							  2592000000L, 31536000000L};
...
//	private JSlider detailSlider = new JSlider(JSlider.VERTICAL, 5, 1000, 100);
	public PerformanceSequenceAnalysis(PerformanceSequenceDiagramPlugin algorithm,
...
//		detailSlider.setMajorTickSpacing(50);
		...
		gbc.gridx=1; gbc.gridy=0;gbc.gridheight=5;
...
//		prettyPanel.add(detailSlider, gbc);
		gbc.gridx=0; gbc.gridy=6;gbc.gridheight=1;
...
//		prettyPanel.add(detailLabel, gbc);
		...
		patternDiagramPanel.setMinimumSize(new Dimension(width - 200, height - 10));
		...
		split = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, patternPane, metricsPane);
	...
	private void registerGUIListener() {
		//add componentlistener, which resizes the panes in which the diagrams are drawn,
		//when the window is resized
		this.addComponentListener(new ComponentListener() {
			public void componentResized(ComponentEvent e) {
				//resize panes as well
				int height = getHeight() - getInsets().bottom - getInsets().top - 50;
				int width = getWidth() - getInsets().left - getInsets().right - 200;
				sequencePane.setPreferredSize(new Dimension(width, height - 10));
				patternPane.setPreferredSize(new Dimension(width - 200, height - 10));
			}
		...
		menuItem4.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (patternSelected) {
					patternDiagramPanel.setTimePerPixel(patternDiagramPanel.getTimePerPixel() * 0.666666666);
					showPatternDiagram();
					patternPane.doLayout();
					int x = Math.max(0, (int) (xPos
							- 0.5 * patternPane.getViewport().getWidth()));
					int y = Math.max(0, (int) (yPos * 1.5
							- 0.5 * patternPane.getViewport().getHeight()));
					patternPane.getViewport().setViewPosition(new Point(x, y));
				} else {
					sequenceDiagramPanel.setTimePerPixel(sequenceDiagramPanel.getTimePerPixel() * 0.666666666);
					showFullDiagram();
					sequencePane.doLayout();
					int x = Math.max(0, (int) (xPos
							- 0.5 * sequencePane.getViewport().getWidth()));
					int y = Math.max(0, (int) (yPos * 1.5
							- 0.5 * sequencePane.getViewport().getHeight()));
		...
		menuItem5.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (patternSelected) {
					patternDiagramPanel.setTimePerPixel(patternDiagramPanel.getTimePerPixel() * 1.5);
					showPatternDiagram();
					int x = Math.max(0, (int) (xPos
							- 0.5 * patternPane.getViewport().getWidth()));
					int y = Math.max(0, (int) (yPos * 0.666666666
							- 0.5 * patternPane.getViewport().getHeight()));
					patternPane.getViewport().setViewPosition(new Point(x, y));
					patternPane.doLayout();
				} else {
					sequenceDiagramPanel.setTimePerPixel(sequenceDiagramPanel.getTimePerPixel() * 1.5);
					showFullDiagram();
					int x = Math.max(0, (int) (xPos
							- 0.5 * sequencePane.getViewport().getWidth()));
					int y = Math.max(0, (int) (yPos * 0.666666666
...
//				detailSlider.setValue(((Double) (oldValue * 0.666666666)).intValue());
//				detailLabel.setText((oldValue * 0.666666666) + "%");
			}
		});
		/**
		 * Add ChangeListener to scaleSlider, so it can be used for zooming purposes
		 */
		scaleSlider.addChangeListener(new ChangeListener() {
		    public void stateChanged(ChangeEvent e) {
				...
				xPos = Math.round(currentPanel.getVisibleRect().getMinX() + (currentPanel.getVisibleRect().getWidth() * 0.5F));
				yPos = Math.round(currentPanel.getVisibleRect().getMinY() + (int) (currentPanel.getVisibleRect().getHeight() * 0.5F));
...
/*		detailSlider.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				...
				if (patternSelected) {
					...
					xPos = Math.round(currentPanel.getVisibleRect().getMinX() + (currentPanel.getVisibleRect().getWidth() * 0.5F));
					yPos = Math.round(currentPanel.getVisibleRect().getMinY() + (int) (currentPanel.getVisibleRect().getHeight() * 0.5F));
					oldTpp = patternDiagramPanel.getTimePerPixel();
					patternDiagramPanel.setTimePerPixel(originalTppPattern / (detailSlider.getValue() / 100.0F));
					showPatternDiagram();
					patternPane.doLayout();
					int x = Math.max(0, (int) (xPos
							- 0.5 * patternPane.getViewport().getWidth()));
					int y = Math.max(0, (int) (yPos * oldTpp/patternDiagramPanel.getTimePerPixel()
							- 0.5 * patternPane.getViewport().getHeight()));
					patternPane.getViewport().setViewPosition(new Point(Math.round(x), y));
				} else {
					currentPanel = sequenceDiagramPanel;
					xPos = Math.round(currentPanel.getVisibleRect().getMinX() + (currentPanel.getVisibleRect().getWidth() * 0.5F));
					yPos = Math.round(currentPanel.getVisibleRect().getMinY() + (int) (currentPanel.getVisibleRect().getHeight() * 0.5F));
					oldTpp = sequenceDiagramPanel.getTimePerPixel();
					sequenceDiagramPanel.setTimePerPixel(originalTppFull / (detailSlider.getValue() / 100.0F));
					showFullDiagram();
					sequencePane.doLayout();
					int x = Math.max(0, (int) (xPos
							- 0.5 * sequencePane.getViewport().getWidth()));
					int y = Math.max(0, (int) (yPos * oldTpp/sequenceDiagramPanel.getTimePerPixel()
							- 0.5 * sequencePane.getViewport().getHeight()));
					sequencePane.getViewport().setViewPosition(new Point(x, y));
				}
				DecimalFormat df = new DecimalFormat("0.00");
...
*/
	...
	/**
	 * Zooms visible diagram to the position of the mouse.
	 * oldscale/scale determines in which direction (in or out) is zoomed
	...
	private void zoom (float oldScale) {
		...
		if (patternSelected) {
			showPatternDiagram();
			patternPane.doLayout();
			int x = Math.max(0, (int) (xPos * scale/oldScale
					- 0.5 * patternPane.getViewport().getWidth()));
			int y = Math.max(0, (int) (yPos * scale/oldScale
					- 0.5 * patternPane.getViewport().getHeight()));
			patternPane.getViewport().setViewPosition(new Point(x, y));
	...
	private void createFilterOptionsScreen() {
		...
		FilterOptions fo = new FilterOptions(inputLog, selectedInstanceIndices, sequences,
						   patterns, timeSort, timeDivider, patternDrawn, this);
	 ...
	 * @param selectedInstanceIndices int[]
	 */
	...
	private void initializeComponentBox() {
		...
		if (log.hasNext()) {
			...
			try {
				...
				while (ate.getTimestamp()==null && index!=pi.getAuditTrailEntryList().size()) {
	...
	/**
	 * Called when the thread, in which the log relations are mined out of the log,
	 * is done. Calls methods to initialize the GUI
	 */
	...
	public void mineRelations() {
		...
		Progress progress = new Progress("Mining " + inputLog.getFile().getShortName() + " to derive log relations");
	...
	private void construct() {
		...
		patternDrawn = false;
		Date endDate = null;
		patterns.clear(); sequences.clear(); dataElts.clear();
		if (relations!=null) {
			...
			while (instances.hasNext()) {
				...
				if (selectedIndex < selectedInstanceIndices.length
					...
					while (it.hasNext()) {
						...
						int eventNumber = events.findLogEventNumber(ate.getElement(), ate.getType());
						...
						if (numberToAteMap.get(eventNumber) != null) {
							...
							belongingAtes = (ArrayList) numberToAteMap.get(eventNumber);
						}
						...
						for (int i = 0; i < number; i++) {
							...
							if (!((relations.getOneLengthLoopsInfo().get(eventNumber) > 0)
									&& num == eventNumber)) {
								if (causalMatrix.get(num, eventNumber) >
										noiseRatio * inputLog.numberOfInstances()) {
									//Check if num is the closest causal precessor of eventNumber
									boolean isClosest = true;
									noPrecessor = false;
									for (int j = i + 1; j < number && isClosest; j++) {
										int k = ((Integer) ateNumbers.get(j)).intValue();
										if (((causalMatrix.get(num,
												k) > noiseRatio * inputLog.numberOfInstances())
												&& (causalMatrix.get(k,
												eventNumber) > noiseRatio * inputLog.numberOfInstances()))
												|| k == num) {
									...
									if (isClosest) {
										...
										ArrayList ates0 = (ArrayList) numberToAteMap.get(num);
										ArrayList ates1 = (ArrayList) numberToAteMap.get(
												eventNumber);
										rl[0] = (AuditTrailEntry) ates0.get(ates0.size() - 1);
										rl[1] = (AuditTrailEntry) ates1.get(ates1.size() - 1);
										if (rl[0].getTimestamp() != null && rl[1].getTimestamp() != null) {
								...
								try {
									rl[0] = (AuditTrailEntry) ates0.get(ates0.size() - 1);
									rl[1] = (AuditTrailEntry) ates0.get(ates0.size() - 2);
									if (rl[0].getTimestamp() != null && rl[1].getTimestamp() != null) {
										relationList.add(rl);
									}
								} catch (ArrayIndexOutOfBoundsException e) {
									//Can occur if ates0.size() is 0 or 1
									Message.add("Exception occurred during sequence diagram build");
								}
							}
						}
						if (noPrecessor) {
						...
						if (dataEltType.equalsIgnoreCase("Task ID")) {
							...
							//get data-element which can be found in the data-part of the audit trail entry
							current = (String) ate.getAttributes().get(dataEltType);
						}
						if (current != null) {
							//keep track of the data elements that occurred
							String[] elts = (String[]) dataElts.toArray(new String[0]);
							Arrays.sort(elts);
						...
						if (ate.getTimestamp() != null) {
							...
							if (firstDate == null || firstDate.after(ate.getTimestamp())) {
								firstDate = ate.getTimestamp();
							}
						}
					}
					//create sequence
					Sequence seq = new Sequence(firstDate, endDate, pi.getName());
					...
					seq.initializeSequence(relationList, dataEltType, strictButton.isSelected());
	 ...
	 * When a match is found, the sequence is added to that pattern.
	 * If no match is found, a new pattern is created based on the sequence
...
//			originalTppFull = dps / 20.0;
		...
		int height = Math.max(((Double) (((duration / sequenceDiagramPanel.getTimePerPixel()) + 100) * scale)).intValue(),  //Math.round(200 * scale + sequences.size() * 20 * scale),
		...
		sequenceDiagramPanel.initialize(sequences, dataElts, timeSort, timeDivider, scale, beginTotal, duration);
...
//			originalTppPattern = timePerPixel;
		...
		if (timePerPixel > 0) {
			...
			height = Math.round(
					 (screenHeight.floatValue() + 50 * patterns.size() + 60) * scale);
		}
		...
		for (int j = sortedArray.length - 1; j >= 0; j--) {
			try {
				...
				while (itr.hasNext()) {
					...
					table.setValueAt(formatString(current.getMeanThroughputTime() / timeDivider, 5), 0, 1);
					table.setValueAt(formatString(current.getMinThroughputTime() / timeDivider, 5), 1, 1);
					table.setValueAt(formatString(current.getMaxThroughputTime() / timeDivider, 5), 2, 1);
					table.setValueAt(formatString(current.getStdevThroughputTime() / timeDivider, 5), 3, 1);
					...
					JLabel patternLabel = new JLabel("Pattern " + patternNumber++ + ":");
					patternLabel.setAlignmentX(LEFT_ALIGNMENT);
					JLabel frequencyLabel = new JLabel("Frequency: " + current.getFrequency());
				...
				if (patterns.size()==1) {
					metricsPanel.add(Box.createRigidArea(new Dimension(5,280)));
				}
				if (patterns.size()==2) {
					metricsPanel.add(Box.createRigidArea(new Dimension(5,50)));
				}
			} catch (NullPointerException ex) {
				//can occur when patternMap does not contain a pattern with this
				//frequency
			}
		}
			...
			if (selectedInstanceIndices!=null) {
				...
				ProvidedObject[] objects = {
					   ...
					   new ProvidedObject("Log Selection", new Object[] {
						   LogReaderFactory.createInstance(inputLog, selectedInstanceIndices)})
					   } ;
			   return objects;
		   } else {
			   ProvidedObject[] objects = {
