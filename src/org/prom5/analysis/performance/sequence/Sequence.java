...
/**
 * Represents a sequence, i.e. the transfer of work between data-element instances
...
public class Sequence {
	...
	/*
	 * Contains a sorted list data-element blocks, that is sorted on if flexible-equivalent:
	 * data-element name (alphabetically), in case of multiple blocks of the same
	 * data-element name then also on begin time, if still multiples, then also
	 * on end time. (Sorted ascendingly)
	 * if strict-equivalent:
	 * on begin time, end time and finally on data-element
	 */
	...
	/**
	 * This method initializes the sequence, and sorts the arrows and data-element
	 * blocks in such a manner that it can be easily compared with other sequences
	 * @param relationList ArrayList:  contains relations (arraylist of 2 audit trail
	 ...
	 *                                 second ate.
	 * @param dataEltType String    :  the data-element type that is used
	 * @param strict boolean        :  true if 'strict' patterns used, false if
	 *                                 'flexible'.
	 */
	public void initializeSequence(ArrayList relationList, String dataEltType,
		...
		while (rels.hasNext()) {
			...
			if (first!=null && second !=null && !first.equals(second)) {
				...
				SequenceArrow arrow = new SequenceArrow(rel[0].getTimestamp(), rel[1].getTimestamp(), first, second);
				arrowList.add(arrow);
	...
	/**
	 * Sorts arrowList on source data-element name, destination data-element name
	 * begin time and end time (in that order)
	 */
		...
		while (lit.hasNext()) {
			...
			if (prts!=null) {
				...
				while (elts.hasNext()) {
					...
					String dest = (String) ((Arrow) originalList.get(ind)).getDestination();
				...
				while (lit0.hasNext()) {
					...
					if (ars!=null) {
						...
						while (arcs.hasNext()) {
							...
							Date beginTime = (Date) ((SequenceArrow) originalList.get(ind)).getBeginTimestamp();
						...
						while (lit1.hasNext()) {
							...
							if (indices != null) {
								...
								while (iterator.hasNext()) {
									...
									Date endTime = (Date) ((SequenceArrow) originalList.get(ind)).
											getEndTimestamp();
									...
									if (endTimeToArrows.containsKey(dest)) {
										endSet = (HashSet) endTimeToArrows.get(endTime);
									}
									endSet.add(ind);
								...
								while (lit2.hasNext()) {
									...
									HashSet endSet = (HashSet) endTimeToArrows.get(endTime);
									Iterator it7 = endSet.iterator();
									while (it7.hasNext()) {
										int ind = ((Integer) it7.next()).intValue();
										//place arrow in new (sorted) location in arrowList
										arrowList.set(index++, (Arrow) originalList.get(ind));
									}
								}
							}
						}
	...
	/**
	 * Sorts arrowList on
	 * begin time, end time source data-element name, destination data-element name(in that order)
	 */
	public void	sortArrowListStrict() {
		...
		HashMap beginTimeToArrows = new HashMap();
		for (int i=0; i<arrowList.size(); i++) {
			Date beginTime = (Date) ((SequenceArrow) originalList.get(i)).getBeginTimestamp();
			if (!beginTimeList.contains(beginTime)) {
				beginTimeList.add(beginTime);
			}
			HashSet beginTimeSet = new HashSet();
		...
		while (lit1.hasNext()) {
			...
			if (indices != null) {
				...
				while (iterator.hasNext()) {
					int ind = ((Integer) iterator.next()).intValue();
					Date endTime = (Date) ((SequenceArrow) originalList.get(ind)).getEndTimestamp();
					if (!endTimeList.contains(endTime)) {
						endTimeList.add(endTime);
					}
				...
				while (lit2.hasNext()) {
					...
					if (endTimes != null) {
						...
						while (prts.hasNext()) {
							...
							String source = (String)((Arrow) originalList.get(ind)).getSource();
							if (!sources.contains(source)) {
								sources.add(source);
							}
							HashSet arrowSet = new HashSet();
						...
						while (srcs.hasNext()) {
							...
							HashSet sourceSet = (HashSet)sourceToArrows.get(source);
							if (sourceSet != null) {
								...
								while (lit0.hasNext()) {
									...
									HashSet dstSet = (HashSet) destToArrows.get(dst);
	...
	/**
	 * Creates and initializes the data-element blocks that are part of this sequence,
	 * stores them in the sortedDataEltBlocks list and sorts this list.
	 * @param dataEltType String
	 * @param ateToSetMap HashMap
	 * @param sourceAteToArrow HashMap
	 * @param destAteToArrow HashMap
			...
			if (!testSet.contains(currentSet)) {
				...
				if (firstDate != null && secondDate != null) {
					...
					if (blockToSetOfSimilar.containsKey(dataElement)) {
						similarBlocks = (HashSet) blockToSetOfSimilar.get(dataElement);
	...
	/**
	 * This methods sorts the sortedDataEltBlocks ascendingly on
	 * data-element, in case of more data-element blocks of same data-element also on,
	 * begin time ascendingly
	 * @param blockSet HashSet : set containing the blocks in this sequence
	 */
		...
		while (lit.hasNext()) {
			...
			if (parts!=null) {
				...
				while (eltparts.hasNext()) {
					...
					Date beginTime = (Date) ((SequenceBlock) originalList.get(ind)).getBeginTimestamp();
			    ...
			    while (lit0.hasNext()) {
				    ...
				    if (prts!=null) {
					    ...
					    while (blocks.hasNext()) {
							...
							DataElementBlock block = (DataElementBlock) originalList.get(ind);
	...
	private void sortBlocksStrictBegin(HashSet blockSet) {
		...
		while (lit.hasNext()) {
			...
			if (parts!=null) {
				...
				while (eltparts.hasNext()) {
					int ind = ((Integer) eltparts.next()).intValue();
					Date endTime = (Date) ((SequenceBlock) originalList.get(ind)).getEndTimestamp();
				...
				while (lit0.hasNext()) {
					...
					if (prts!=null) {
						...
						while (elts.hasNext()) {
							int ind = ((Integer) elts.next()).intValue();
							String elt = (String) ((DataElementBlock) originalList.get(ind)).getDataElement();
						...
						while (lit1.hasNext()) {
							int ind = ((Integer) eltToPartMap.get((String) lit1.next())).intValue();
							DataElementBlock block = (DataElementBlock) originalList.get(ind);
			...
			if (parts!=null) {
				...
				while (lit0.hasNext()) {
					...
					if (prts!=null) {
						...
						while (lit1.hasNext()) {
							int ind = ((Integer) eltToPartMap.get((String) lit1.next())).intValue();
							sortedOnEndDataEltBlocks.add(index++, (DataElementBlock) originalList.get(ind));
						}
					}
				}
			}
	...
	/**
	 * Returns the list of data-element blocks, sorted on end time, begin time and
	 * finally on data-element name.
	 * @return ArrayList
	 */
	...
	public double getBeginPositionBlock(int num) {
		try {
			Date bDate = (Date) ((SequenceBlock) sortedDataEltBlocks.get(num)).getBeginTimestamp();
	...
	/**
	 * Initializes the data-element blocks of the sequence so they can easily
	 * be drawn
	 * @param timePerPixel double : time per pixel
	 * @param firstDate Date      : the starting date of the first sequence in the sequence diagram
	 */
	public void initializeDrawSequence(double timePerPixel, Date firstDate) {
		...
		for (int i = 0; i < arrowList.size(); i++) {
			...
			if (firstDate != null && beginTimestamp != null && endTimestamp != null) {
				...
				double startAt = 60 + (beginTimestamp.getTime() -
								 firstDate.getTime()) / timePerPixel;
				arrow.setStartAt(startAt);
				double endAt = 60 + (endTimestamp.getTime() -
							   firstDate.getTime()) / timePerPixel;
	...
	public void drawSequence(HashMap lifeLines, Graphics2D g) {
		...
		while (lit.hasNext()) {
			...
			try {
				block.drawBlock(
						((LifeLine) lifeLines.get(block.getDataElement())).getMiddle() - 10,
						thisColor, g);
	...
	/**
	 * Draws a rectangle of width 20, height length and starting point (startX,startY)
	 * in the northwest corner of the rectangle. In case logicSteps is true, the
	 * height is 10.
	 * @param startX double
	 * @param startY double
	 * @param length double
	 * @param logicSteps boolean
	 * @param g Graphics2D
	 */
