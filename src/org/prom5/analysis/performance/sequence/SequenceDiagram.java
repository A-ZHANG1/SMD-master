...
//	private Date lastDate;
	 ...
	 * @param sequences ArrayList
	 * @param dataElts ArrayList
	 * @param timeSort String
	 * @param timeDivider long
	 * @param scale float
	 * @param beginDate Date
	 * @param duration double
	 */
	  public void initialize(ArrayList sequences, ArrayList dataElts, String timeSort,
			long timeDivider, float scale, Date beginDate, double duration) {
		paintDiagram = false;
		...
		if (paintDiagram) {
			...
			while (lit.hasNext()) {
				...
				if (visible.getMinY() <= current.getEndY() * scale
						&& (current.getStartY() - 5) * scale <= visible.getMaxY()) {
	...
	private void drawTimeAxis(Graphics2D g, int height, Date beginDate) {
		...
		int newHeight = ((Float) (height / scale)).intValue();//Math.round(Math.max(height, height / scale));
		//obtain step-size on axis (time) and distance in pixels of a step
		ArrayList stepList = obtainStep(new BigDecimal(1), timePerPixel, Double.MAX_VALUE);
		//step-size
	...
	/**
	 * Returns the best step size for the time-axis and the number of pixels of a step
	 *
	 * @param n BigDecimal
	 ...
	 * @return ArrayList
	 */
	private ArrayList obtainStep(BigDecimal n, double timePerPixel, double distance) {
		ArrayList biggerList = obtainStepBiggerOne(n, timePerPixel, distance);
		if (((BigDecimal) biggerList.get(1)).doubleValue() > 1) {
			return biggerList;
		} else {
			ArrayList smallerList = obtainStepSmallerOne(n, timePerPixel, distance);
			return smallerList;
		}
	}
	/**
	 ...
	 * @return ArrayList
	 */
	private ArrayList obtainStepBiggerOne(BigDecimal n, double timePerPixel, double distance) {
		...
		if (Math.abs(dist3) < Math.abs(dist)  && list3 != null && list3.size()>0) {
	 ...
	 * @return ArrayList
	 */
	private ArrayList obtainStepSmallerOne(BigDecimal n, double timePerPixel, double distance) {
		ArrayList stepList = new ArrayList();
		BigDecimal i = n;
		//time Per pixel is in milliseconds
		double dist = n.doubleValue() *timeDivider / timePerPixel - 25;
		...
		if (visibleSequences.size()>0 && tooltipsOn) {
			...
			while (it.hasNext()) {
				...
				if (Math.round(seq.getStartY()*scale) <= p.getY() + 2
					...
					ArrayList sortedDataElementBlocks = seq.getSortedDataEltBlocks();
					for (int i = 0; i < sortedDataElementBlocks.size() && !onSequence; i++) {
						SequenceBlock block = (SequenceBlock) sortedDataElementBlocks.get(i);
						if (block.isInBlock(p, lifeLines, scale)) {
							str += "Process instance: " + seq.getPiName()
									+ "&nbsp&nbsp&nbsp |&nbsp&nbsp&nbsp Throughput time:" + seq.getThroughputTime() / timeDivider
					...
					if (!onSequence) {
						...
						for (int i = 0; i < arrowList.size(); i++) {
							...
							if (arrow.isOnLine(p, lifeLines, scale)) {
								str += "Process instance: " + seq.getPiName()
										+ "&nbsp&nbsp&nbsp |&nbsp&nbsp&nbsp Throughput time:" + seq.getThroughputTime() / timeDivider
										+ "&nbsp&nbsp&nbsp |&nbsp&nbsp&nbsp Time between "+ arrow.getSource() + " and " +
