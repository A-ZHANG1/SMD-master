...
import org.prom5.framework.models.petrinet.algorithms.PlaceInvariantCalculator;
import org.prom5.framework.models.petrinet.algorithms.ReachabilityGraphBuilder;
import org.prom5.framework.models.petrinet.algorithms.TransitionInvariantCalculator;
...
public class PetriNetAnalysisUI extends JPanel implements Provider {
	...
	private JButton calculateInvariantsButton = new JButton("Calculate Invariants");
	private JButton calculateReachabilityButton = new JButton("Show reachability graph");
	private JButton calculateCoverabilityButton = new JButton("Show coverability graph");
	private JButton calculateRestrictedCoverabilityButton = new JButton("Show restricted coverability graph");
	...
	void jbInit() throws Exception {
		...
		calculateReachabilityButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				validate();
				repaint();
				Message.add("<PetriNetAnalysis nofReachableStates=\"" + reachability.getVerticeList().size() +
		...
		calculateCoverabilityButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				InitialPlaceMarker.mark(net, 1);
				remove(netContainer);
				coverability = CoverabilityGraphBuilder.build(net, -1, true, true);
				...
				validate();
				repaint();
				Message.add("<PetriNetAnalysis nofCoverableStates=\"" + coverability.getVerticeList().size() +
						"\" nofEdges=\"" + coverability.getNumberOfEdges() +
						"\">",
						Message.TEST);
			}
		});
		calculateRestrictedCoverabilityButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				InitialPlaceMarker.mark(net, 1);
				remove(netContainer);
				restrictedcoverability = CoverabilityGraphBuilder.build(net, -1, true, false);
				...
				validate();
				repaint();
				Message.add("<PetriNetAnalysis nofCoverableStates=\"" + restrictedcoverability.getVerticeList().size() +
						"\" nofEdges=\"" + restrictedcoverability.getNumberOfEdges() +
						"\">",
						Message.TEST);
			}
		});
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		if (restrictedcoverability != null) {
			objects.add(new ProvidedObject("Restricted coverability graph", new Object[] {restrictedcoverability}));
		}
