...
public class StateSpaceCalculatorUI extends JPanel implements Provider {
	...
	public StateSpaceCalculatorUI(FSM statespace) {
		...
		try {
			minimalRegions = statespace.calculateMinimalRegions(statespace.getEdgeObjects());
			Message.add("<MinimalRegionsFound "+minimalRegions.size()+">",Message.TEST);
			jbInit();
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	void jbInit() throws Exception {
		...
		showAllRegions.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (allRegions == null) {
					allRegions = statespace.calculateAllRegions(statespace.getEdgeObjects());
				}
				updateTable(allRegions);
				Message.add("<AllRegionsFound "+minimalRegions.size()+">",Message.TEST);
			}
		});
		...
		graphPanel.addGrappaListener(new GrappaAdapter() {
			...
			protected JMenuItem getCustomMenu(Subgraph subg, Element elem, GrappaPoint pt,
				...
				if ((elem != null) && (elem.object instanceof ModelGraphEdge) &&
					...
					JMenuItem preRegionMenuItem = new JMenuItem("Show pre-regions of " + t.toString());
					preRegionMenuItem.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							updateTable(statespace.getPreRegions(minimalRegions, t));
						}
					});
					JMenuItem postRegionMenuItem = new JMenuItem("Show post-regions of " +
							t.toString());
					postRegionMenuItem.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							updateTable(statespace.getPostRegions(minimalRegions, t));
						}
					});
		...
		updateGraphButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				HashSet region = (HashSet) table.getValueAt(table.getSelectedRow(),
		...
		splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, tablePanel, graphPanel);
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		if (allRegions != null) {
			objects.add(new ProvidedObject("Statespace with all regions", new Object[] {statespace,
					allRegions}));
		}
