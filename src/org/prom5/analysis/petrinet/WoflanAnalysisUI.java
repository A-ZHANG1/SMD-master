 ...
 * All rights reserved.
 */
package org.prom5.analysis.petrinet;
...
import org.prom5.framework.models.petrinet.algorithms.TPNWriter;
import org.prom5.framework.models.petrinet.algorithms.Woflan;
...
/**
 * <p>Title: WoflanAnalysisUI</p>
 * <p>Description: Woflan Analysis Plugin User Interface</p>
 * <p>Copyright: Copyright (c) 2005 Eric Verbeek</p>
 * <p>Company: Technische Universiteit Eindhoven</p>
 * @author Eric Verbeek
 * @version 1.0
 */

public class WoflanAnalysisUI extends JPanel implements Provider, TreeSelectionListener {

	private PetriNet net;
	private DefaultMutableTreeNode root; // Holds the root node in the tree with analysis results.
	private JTree treeContainer; // Holds the tree with analysis results.
	private JScrollPane netContainer; // Holds the net and visualizes selected analysis results.
	...
	private DefaultMutableTreeNode netProperties; // Holds the Net Properties node.
	private DefaultMutableTreeNode netDiagnosis; // Holds the Net Diagnosis node.
	 ...
	 * @param net The Petri net that is to be analysed by Woflan
	 */
	...
	void jbInit() throws Exception {
		...
		finishButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				while (dfe.hasMoreElements()) {
					...
					if (text.startsWith("Place ") || text.startsWith("Transition ")) {
						// nothing
					} else {
		...
		try {
			...
			BufferedWriter bw = new BufferedWriter(new FileWriter(tpnFile, false));
			...
			System.err.println("Error while running Woflan: " + ex.getMessage());
		}
		if (woflanNet != 0) {
	...
	private void getBasicNetProperty(int info, int subinfo,
			String label, DefaultMutableTreeNode parent) {
		int n, i;
		...
		parent.add(node);
		for (i = 0; i < n; i++) {
			node.add(new DefaultMutableTreeNode(getName(woflan.Info(woflanNet, subinfo, i, 0))));
		}
	}

	/**
	 * Create the "Net Properties" section in the Analysis tree.
	 * @param woflan Handle to Woflan itself.
	 * @param woflanNet The reference to the Petri net at hand.
	 */
	private void buildNetProperties() {
		...
		getBasicNetProperty(woflan.InfoNofP, woflan.InfoPName,
				"Places", netProperties);
		getBasicNetProperty(woflan.InfoNofT, woflan.InfoTName,
				"Transitions", netProperties);
	}

	private void buildWFNetDiagnosis() {
		int nofSrcP, nofSnkP, nofSrcT, nofSnkT, nofUncN, nofSncN;
		DefaultMutableTreeNode step;
		nofSrcP = Integer.parseInt(woflan.Info(woflanNet, woflan.InfoNofSrcP, 0, 0));
		nofSnkP = Integer.parseInt(woflan.Info(woflanNet, woflan.InfoNofSnkP, 0, 0));
		nofSrcT = Integer.parseInt(woflan.Info(woflanNet, woflan.InfoNofSrcT, 0, 0));
		nofSnkT = Integer.parseInt(woflan.Info(woflanNet, woflan.InfoNofSnkT, 0, 0));
		woflan.Info(woflanNet, woflan.SetSUnc, 0, 0);
		nofUncN = Integer.parseInt(woflan.Info(woflanNet, woflan.InfoNofUncN, 0, 0));
		nofSncN = Integer.parseInt(woflan.Info(woflanNet, woflan.InfoNofSncN, 0, 0));
		if (nofSrcP == 1 && nofSnkP == 1 && nofSrcT == 0 && nofSnkT == 0 && nofUncN == 0 &&
				nofSncN == 0) {
			isWFNet = 2;
	...
	private void buildSCoverableDiagnosis() {
		...
		nofNot = Integer.parseInt(woflan.Info(woflanNet, woflan.InfoNofNotSCom, 0, 0));
		if (nofNot == 0) {
			isSCoverable = 2;
			...
			DefaultMutableTreeNode node;
			int i, j;
			nof = Integer.parseInt(woflan.Info(woflanNet, woflan.InfoNofSCom, 0, 0));
			for (i = 0; i < nof; i++) {
				nofN = Integer.parseInt(woflan.Info(woflanNet, woflan.InfoSComNofN, i, 0));
				node = new DefaultMutableTreeNode("S-component " + i + " [" + nofN + "]");
				step.add(node);
				for (j = 0; j < nofN; j++) {
					node.add(new DefaultMutableTreeNode(getName(woflan.Info(woflanNet,
							woflan.InfoSComNName, i, j))));
				}
			}
			...
			nofC = Integer.parseInt(woflan.Info(woflanNet, woflan.InfoNofNFCC, 0, 0));
			nofPT = Integer.parseInt(woflan.Info(woflanNet, woflan.InfoNofPTH, 0, 0));
			nofTP = Integer.parseInt(woflan.Info(woflanNet, woflan.InfoNofTPH, 0, 0));
	...
	private void buildPIDiagnosis() {
		...
		nofNot = Integer.parseInt(woflan.Info(woflanNet, woflan.InfoNofNotSPIn, 0, 0));
		if (nofNot == 0) {
			isPICoverable = 2;
		...
		int i, j;
		nof = Integer.parseInt(woflan.Info(woflanNet, woflan.InfoNofSPIn, 0, 0));
		for (i = 0; i < nof; i++) {
			nofN = Integer.parseInt(woflan.Info(woflanNet, woflan.InfoSPInNofP, i, 0));
			node = new DefaultMutableTreeNode("P-invariant " + i + " [" + nofN + "]");
			step.add(node);
			for (j = 0; j < nofN; j++) {
				node2 = new DefaultMutableTreeNode(getName(woflan.Info(woflanNet,
						woflan.InfoSPInPName, i, j)));
				node.add(node2);
				node2.add(new DefaultMutableTreeNode("Weight " + woflan.Info(woflanNet,
	...
	private void buildClusterDiagnosis(DefaultMutableTreeNode parent) {
		...
		parent.add(node1);
		for (i = 0; i < nofC; i++) {
			nofN = Integer.parseInt(woflan.Info(woflanNet, woflan.InfoNFCCNofN, i, 0));
			node2 = new DefaultMutableTreeNode("Non-free-choice cluster" + i + " [" + nofN + "]");
			node1.add(node2);
			for (j = 0; j < nofN; j++) {
				node2.add(new DefaultMutableTreeNode(getName(woflan.Info(woflanNet,
						woflan.InfoNFCCNName, i, j))));
			}
		}
	}

	private void buildHandleDiagnosis(boolean isPT, DefaultMutableTreeNode parent) {
		...
		for (i = 0; i < nof; i++) {
			...
			node2.add(node3);
			for (j = 0; j < nof1; j++) {
				node3.add(new DefaultMutableTreeNode(getName(woflan.Info(woflanNet, infoN1Name, i,
						j))));
			}
			...
			node2.add(node3);
			for (j = 0; j < nof2; j++) {
				node3.add(new DefaultMutableTreeNode(getName(woflan.Info(woflanNet, infoN2Name, i,
						j))));
			}
		}
	}

	private void buildSequenceDiagnosis(boolean isUnbounded, DefaultMutableTreeNode parent) {
		int nofSeq, nofT;
		...
		for (i = 0; i < nofSeq; i++) {
			...
			node2 = new DefaultMutableTreeNode(s + " sequence " + i + " [" + nofT + "]");
			node.add(node2);
			for (j = 0; j < nofT; j++) {
				node2.add(new DefaultMutableTreeNode(getName(woflan.Info(woflanNet, infoName, i, j))));
			}
		}
	...
	private void buildBoundedDiagnosis() {
		...
		nofNot = Integer.parseInt(woflan.Info(woflanNet, woflan.InfoNofUnbP, 0, 0));
		if (nofNot == 0) {
			isBounded = 2;
	...
	private void buildNonDeadDiagnosis() {
		...
		nofNot = Integer.parseInt(woflan.Info(woflanNet, woflan.InfoNofDeadT, 0, 0));
		if (nofNot == 0) {
			isNonDead = 2;
	...
	private void buildLiveDiagnosis() {
		...
		//woflan.Info(woflanNet, woflan.SetNLive, 0, 0);
		nofNot = Integer.parseInt(woflan.Info(woflanNet, woflan.InfoNofNLiveT, 0, 0));
		if (nofNot == 0) {
			isLive = 2;
	...
	private void buildNetDiagnosis(int nofSteps) {
		...
		while (i > 0 && woflanNet != 0) { // Do something as long as soundness is unknown.
			i--;
			if (isSound != 0 && woflanNet != 0) {
				...
				// No WF-net, hence soundness not defined (for sake of simplicity say it is not sound).
				isSound = 1;
				i++;
			} else if (isWFNet == 2 && isSCoverable == 0) { // WF-net, unknown whether S-Coverable.
				// Check whether S-coverable
				buildSCoverableDiagnosis();
			} else if (isSCoverable == 1 && isPICoverable == 0) { // No S-Cover, check whether PI-cover
				// Check whether BPI-coverable
				buildPIDiagnosis();
			} else if (isPICoverable == 1 && isBounded == 0) { // Check boundedness
				// Check whether bounded
				buildBoundedDiagnosis();
			} else if ((isSCoverable == 2 || isPICoverable == 2) && isBounded == 0) {
				isBounded = 2;
				i++;
			} else if (isBounded == 2 && isNonDead == 0) { // Bounded: check whether dead transitions exist
				// Check whether 0-live
				buildNonDeadDiagnosis();
			} else if (isBounded == 2 && isNonDead == 2 && isLive == 0) { // Bounded, no dead transitions: check liveness
				// Check whether live
				buildLiveDiagnosis();
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		if (net != null) {
			objects.add(new ProvidedObject("Original Petri Net", new Object[] {net}));
		}
		if (reachability != null) {
			objects.add(new ProvidedObject("Reachability graph", new Object[] {reachability}));
		}
