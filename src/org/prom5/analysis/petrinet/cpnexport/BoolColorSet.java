...
/**
 * Defines a bool color set for cpn tools. For a bool color set it also
...
public class BoolColorSet extends CpnColorSet {
	...
	/**
	 * Default constructor
	 * @param name String the name of the bool color set
	 */
	...
	public void write(BufferedWriter bw) throws IOException {
		bw.write("\t\t\t<color id=\"" + ManagerID.getNewID() + "\">\n" +
					"\t\t\t\t<id>" + CpnUtils.getCpnValidName(myNameColorSet) + "</id>\n");
		if (myTimed) {
			bw.write("\t\t\t\t<timed/>\n");
		}
		bw.write("\t\t\t\t<alias>\n" +
					"\t\t\t\t\t<id>" + "BOOL" + "</id>\n" +
					"\t\t\t\t</alias>\n" +
					"\t\t\t\t<layout>" + "colset " + CpnUtils.getCpnValidName(myNameColorSet) + " = "
					 + "BOOL");
					 if (myTimed) {
						 bw.write(" timed");
					 }
	 ...
	 * @param timed boolean <code>true</code> when this bool color set
	 * needs to be timed, <code>false</code> otherwise.
	 */
