...
public class ColoredEdge extends PNEdge {
	...
	/**
	 * The constructor creates a ColoredEdge from a ColoredTransition
	 * to a ColoredPlace.
	 *
	 * @param source the transition to be connected to this arc as a source node
	 * @param target the place to be connected to this arc as a target node
	 */
	...
	/**
	 * The constructor creates a ColoredEdge from a ColoredPlace
	 * to a ColoredTransition.
	 *
	 * @param source the place to be connected to this arc as a source node
	 * @param target the transition to be connected to this arc as a target node
	 */
	...
	public void write(BufferedWriter bw) throws java.io.IOException {
 	...
 	if (this.isPT()) {
		...
		placeID = place.getCpnID();
		transID = trans.getCpnID();
		ManagerXml.getXCoordinateArcInscription(place.getXCoordinate(), trans.getXCoordinate());
	} else {
		...
		if (this.getBendPoints().size() > 0) {
			ManagerXml.writeArcTag(bw, orientation, transID, placeID,
				ManagerXml.getXCoordinateArcInscription(place.getXCoordinate(), trans.getXCoordinate()),
				ManagerXml.getYCoordinateArcInscription(place.getYCoordinate(), trans.getYCoordinate()),
				this.getArcInscription(), this.getBendPoints());
        }
		else {
	     ManagerXml.writeArcTag(bw, orientation, transID, placeID,
		 ManagerXml.getXCoordinateArcInscription(place.getXCoordinate(), trans.getXCoordinate()),
		 ManagerXml.getYCoordinateArcInscription(place.getYCoordinate(), trans.getYCoordinate()),
		 this.getArcInscription(), null);
        }
	...
	/**
	 * Sets the arc inscription for this arc (The inscription has to be according
	 * to the CPN tools syntax).
	 * @param inscript the arc inscription for this arc.
	 */
	 ...
	 * @param bendPoint ArrayList the first element in the array needs to be value
	...
	/**
	 * indicate whether the arc is doubleheaded
	 * USE ONLY FOR CPN FILE GENERATION
	 * @param doubleHeaded boolean
	 */
