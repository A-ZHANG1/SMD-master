...
import org.prom5.analysis.redesign.ui.RedesignAnalysisUI;
...
import org.prom5.framework.models.hlprocess.expr.HLExpressionManager;
import org.prom5.framework.models.hlprocess.expr.operator.HLAndOperator;
 ...
 *
 * It holds a reference to a simulation model {@link HLPetriNet} and a translator object
 * {@link HLToCPNTranslator} that is able to translate the given simulation information to
 * this Coloured Petri net representation of the simulation model.
 *
 * @author Anne Rozinat
 * @author Ronny Mans
 */
public class ColoredPetriNet extends PetriNet {
	...
	/** the instanceID of the page in CPN on which the Simulated PetriNet is place. */
	...
	/** the translator (high level information to Cpn specific information) for this process. */
	private HLToCPNTranslator translator;
	/** Indicator for determining whether the petri net model has been written to the cpn-file. */
	...
	/**
	 * Constructs a ColoredPetriNet out from a given HLPetri net
	 * (i.e., a PetriNet-based simulation model).
	 * @param simModel the PetriNet-based simulation model
	 */
	 ...
	 * @param net the Petri net that is re-established in the simulated net
	 */
	 ...
	 * <p>Replaces ordinary transition, places, and edges by simulated transitions, 
	 * places, and edges.
	...
	/**
	 * Updates the highLevelPN, so that all instances of Transition are replaced by the corresponding
	 * ColoredTransition object and that all instances of Place are replaced by the corresponding
	 * ColoredPlace object
	 * @param highLevelPN HighLevelProcess
	 */
	...
	/**
	 * Expands the data expressions according to the order ranks in XOR semantics (like in YAWL).
	 * First generates exclusive expressions in ascending rank order, and then makes the default
	 * expresson to be a complement of all the other expressions. 
	 * <p>If this should become not always desirable (as e.g., also OR splits - which have 
	 * different order rank evaluation semantics - are supported), then include an option in 
	 * CPN Export configuration, or move to the conversion. 
	 */
	...
	/**
	 * Returns the object containing the high-level information referring
	 * to this process.
	 ...
	 * @param hlpn HLPetriNet the high level process
	 */
	 ...
	 * @return String the cpnID for this model. "" is returned if no such
	 * cpnID exists.
	 */
	 ...
	 * @param cpnID String the cpnID to be assigned to this model.
	 */
	...
	/**
	 * Adds a pageInstanceID for one or more pages in CPN on which the Petri Net will be
	 * located.
	 * @param pageInstanceID String the pageInstanceID.
	 */
	 ...
	 * No cpnIDs are generated for any places and transitions in a model that is a
	 * submodel of any transition in this model.
	 */
	 ...
	 * <br>
	 * Note that this is the starting point to invoke the export procedure as it,
	 * e.g., writes the header information of the file. For actually writing the page
	 * containing the transitions and places of this Petri net the
	 * {@link #write write} procedure is invoked.
	 *
	 * @param bw the output stream handle to actually write into the file
	 */
	public void writeToFile(BufferedWriter bw, BufferedWriter currentStateWriter, String smlName) {
		try {
			translator.reset(); // needed to enable multiple exports from the same data structure
			ManagerID.reset(); // not needed but nicer to make the generated files more comparable
			...
			boolean isTimed = ManagerConfiguration.getInstance().isTimePerspectiveEnabled();
			// write to the file which perspectives are enabled
			Message.add("TimePerspectiveEnabled: " + isTimed, Message.TEST);
			Message.add("DataPerspectiveEnabled: " + ManagerConfiguration.getInstance().isDataPerspectiveEnabled(), Message.TEST);
			Message.add("ResourcePerspectiveEnabled: " + ManagerConfiguration.getInstance().isResourcePerspectiveEnabled(), Message.TEST);
			Message.add("ActivityLoggingEnabled: " + ManagerConfiguration.getInstance().isActivityLoggingEnabled(), Message.TEST);
			Message.add("ThroughputTimeMonitorEnabled: " + ManagerConfiguration.getInstance().isThroughputTimeMonitorEnabled(), Message.TEST);
			Message.add("ResourceMonitorEnabled: " + ManagerConfiguration.getInstance().isResourcesMonitorEnabled(), Message.TEST);
			...
			ManagerXml.writeControlFlowDec(bw, translator.variableTranslator.getCpnVarForCaseId()
					,translator.colorsetTranslator.getColorSetCaseID(), isTimed);
			// write Redesign declarations (fixed)
			if (isRedesign) {
			//	ManagerXml.writeRedesignDec(bw, "C:/RedesignAnalysis/currentSimSettings");
				ManagerXml.writeRedesignDec(bw,RedesignAnalysisUI.locationForCurrentSimSettingsInCPNExport);
			}
			// write Resource and group declarations (do this always in redesign mode)
			if ((ManagerConfiguration.getInstance().isResourcePerspectiveEnabled()) &&
				...
				colorSetGroups.addAll(translator.colorsetTranslator.getColorSetsGroups());
				colorSetGroups.add(translator.colorsetTranslator.getColorSetForResourcesPlace());
				ArrayList<CpnVarAndType> cpnVarsGroups = new ArrayList<CpnVarAndType>();
				cpnVarsGroups.addAll(translator.variableTranslator.getCpnVarsForGroups());
				cpnVarsGroups.add(translator.variableTranslator.getCpnVarForGroupAllResources());
				ManagerXml.writeResourceDec(bw, cpnVarsGroups, colorSetGroups);
				// write to the test file
				Message.add("number of cpn vars written for the groups [" + cpnVarsGroups.size() + "]", Message.TEST);
				Message.add("number of color sets written for the groups [" + colorSetGroups.size() + "]", Message.TEST);
			}
			...
			if (ManagerConfiguration.getInstance().isDataPerspectiveEnabled() &&
				...
				cpnVarsDataAttr.add(translator.variableTranslator.getCpnVarForDataAttributes());
				cpnVarsDataAttr.add(translator.variableTranslator.getCpnVarForModifiedDataAttributes());
				ManagerXml.writeDataDec(bw, cpnVarsDataAttr,
						translator.colorsetTranslator.getColorSetDataAttributes(),
						translator.colorsetTranslator.getColorSetsSeparateDataAttributes());
				...
				if (process.getAttributes().size() > 0) {
					...
					Iterator<HLAttribute> dataAttrs = process.getAttributes().iterator();
					while (dataAttrs.hasNext()) {
						HLAttribute dataAttr = dataAttrs.next();
						CpnFunction cpnFunc = translator.generateRandomFunctionForDataAttribute(dataAttr);
						dataAttrCpnFunc.add(cpnFunc);
					}
					if (dataAttrCpnFunc.size() > 0) {
						ManagerXml.writeRandomValuesFunctions(bw, dataAttrCpnFunc);
					}
				}
				// write to the test file
				Message.add("number of separate color sets written for the data attributes [" + translator.colorsetTranslator.getColorSetsSeparateDataAttributes().size() + "]", Message.TEST);
			}
			// write information for the start timestamp of the case (do this always in redesign mode)
			if (ManagerConfiguration.getInstance().isThroughputTimeMonitorEnabled() || isRedesign) {
				ManagerXml.writeStartCase(bw, translator.variableTranslator.getCpnVarForStartCase(),
						translator.colorsetTranslator.getColorSetStartCase());
			}
			// write combined declarations for control flow, data declarations, resources
			// when needed or when in redesign mode
			if (isRedesign) {
				ManagerXml.writeProductColorSetsForPlaces(bw, translator.colorsetTranslator.getProductColorSetsForPlaces(),true);
			} else {
			ManagerXml.writeProductColorSetsForPlaces(bw, translator.colorsetTranslator.getProductColorSetsForPlaces(),
					isTimed);
			}
			// write product color set
			Message.add("number of special product color sets for places [" + translator.colorsetTranslator.getProductColorSetsForPlaces().size() + "]", Message.TEST);

			// write the probabilityvariable, and maybe some other variables that might be needed
			...
			while (choices.hasNext()) {
				...
				if (choice.getChoiceConfiguration().equals(ChoiceEnum.PROB)) {
					ManagerXml.writePossibilityDependencies(bw,
						translator.variableTranslator.getCpnVarsForProbDep());
					break;
				}
			}
			...
			if (ManagerConfiguration.getInstance().isActivityLoggingEnabled()) {
				ManagerXml.writeLogDeclarations(bw,
						ManagerConfiguration.getInstance().getYearOffset(), 
						process.getGlobalInfo().getTimeUnit());
			}
			...
			Message.add("number of fusion places [" + translator.getFusionPlaces().size() + "]", Message.TEST);
			// write the instances
			...
			// generate a monitor for the lowest level functions of the process page
			if (ManagerConfiguration.getInstance().isActivityLoggingEnabled()) {
				// use a list for keeping the names that already have been used for a monitor
				ArrayList<String> usedNamesMonitors =  new ArrayList<String>();
				environment.writeMonitors(bw, usedNamesMonitors);
				Message.add("Activity monitors have been written", Message.TEST);
			}
			// write resources monitor (if needed)
			if (ManagerConfiguration.getInstance().isResourcePerspectiveEnabled() &&
						process.getResources().size() > 0 &&
						ManagerConfiguration.getInstance().isResourcesMonitorEnabled()) {
					// write the monitor that keeps track of the number of resources in the resource place
					ManagerXml.writeMarkingSizeMonitorResources(bw,
							this.translator.getInfoForMonitoringResources());
			...
			if (ManagerConfiguration.getInstance().isThroughputTimeMonitorEnabled() && !isRedesign) {
				String dataVar = "";
				if (process.getAttributes().size() > 0 &&
						ManagerConfiguration.getInstance().isDataPerspectiveEnabled()) {
					dataVar = translator.variableTranslator.getCpnVarForDataAttributes().getVarName();
				}
				ManagerXml.writeThroughputTimeMonitor(bw,
						translator.getCpnIdMonitorTroughputTime(),
						translator.getPageinstancesMonitorThroughputTime(), dataVar,
						this.getHighLevelProcess().getHLProcess().getGlobalInfo().getTimeUnit());
				Message.add("Throughput time monitor has been written", Message.TEST);
			}
	 ...
	 * @param bw the output stream handle to write into the file
	 * @param topTransition the top transition of the subPage. Null if no such
	 * top transition exists
	 */
	public void write(BufferedWriter bw, ColoredTransition topTransition) {
		try {
			this.Test(this.getIdentifier());
			// write this Petri net level as one page and invoke the writing of potential subpages
...
//				}
			...
			// writing subpages, you should not start with writing another page before
			...
			while (transitionsSub.hasNext()) {
				ColoredTransition transition = (ColoredTransition) transitionsSub.next();
				if (transition.getSubpage() != null && !transition.getSubpage().isAlreadyWrittenToCPN() ) {
					// create the subpage
	 ...
	 * @return boolean <code>true</code> when this colored petri net is already written to
	 * the cpn file. <code>false</code> otherwise.
	 */
	...
	/**
	 * Retrieves the translator that translator that translates the simulation information
	 * together with the ordinary petri net to a colored petri net which on its turn can be exported
	 * to cpn.
	...
	/**
	 * Sets the translator that can translate the simulation information together with the ordinary
	 * petri net to a colored petri net which on its turn can be exported to cpn.
	 * @param translator HLToCPNTranslator the translator.
	 */
	...
	/**
	 * The monitors for the logging functionality in cpn-tools are written.
	 * It is assumemed that there does not exist a transition with identifier "Init" on
	 * the process page
	 * @param bw BufferedWriter the output stream handle to write into the file
	 * @param usedNamesMonitors the list of names that have already been used for a monitor
	 * @throws IOException
	 */
