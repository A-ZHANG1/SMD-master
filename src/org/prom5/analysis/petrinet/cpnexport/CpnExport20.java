 ...
 * @author Ronny Mans
 * @author Mariska Netjes (adding possibility to remove user save for CpnExportSettings for redesign mode)
 */
public class CpnExport20 implements AnalysisPlugin {
	
	/**
	 * Specifies the name of the plug-in.
	 * This is used for, e.g., labelling the corresponding menu item or the
		...
		for (int i = 0; i < o.length; i++) {
			if (o[i] instanceof PetriNet  || o[i] instanceof HLPetriNet) {
				if ((o[i] instanceof PetriNet) == true) {
					...
					// if the the provided object is a (PetriNet-based) simulation model..
					// ... clone the passed simulation model (i.e., so that the provided object remains
					// in the framework in the state it was before calling this export plugin)
					HLPetriNet cloned = (HLPetriNet) ((HLPetriNet) o[i]).clone();
