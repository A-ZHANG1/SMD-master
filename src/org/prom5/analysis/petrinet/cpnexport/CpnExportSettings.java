...
public class CpnExportSettings extends JPanel {
	...
	/**
	 * Required for a serializable classes (generated quickfix).
	 * Not directly used.
	 */
	...
	private ColoredPetriNet mySimulatedPN; /** The simulated PetriNet to be exported to CPN Tools */
	private HLPetriNet hlProcess; /** The high-level Petri net simulation model */
	private HLProcessGui hlProcessGui;
	// user interface related attributes
	private JButton exportButton = new AutoFocusButton("Export"); /** invoking the actual export */
	private JButton saveButton = new AutoFocusButton("Save"); /** invoking the actual export */
	private FlatTabbedPane tabPane = new FlatTabbedPane("CPN Export Settings", new Color(200, 200, 200), new Color(120, 120, 120, 200), new Color(200, 200, 200));
	private JButton docsButton = new SlickerButton("Help..."); // shows the plugin documentation
	
	private boolean saveDialogue; /** Normally, there is a save dialogue, in redesign mode this is disabled, i.e., false. */
	
	/**
	 * Creates the settings frame for the CPN Tools export.
	 * @param netToExport the high-level Petri net simulation model to be exported to CPN Tools
	 */
	 ...
	 * Creates the settings frame for the CPN Tools export.
	 * @param netToExport the high-level Petri net simulation model to be exported to CPN Tools
	 * @param saveOn boolean indicating if a save dialogue is needed, true: it is, false: redesign mode, no save dialogue.
	 */
	public CpnExportSettings(CpnExport20 algorithm, ColoredPetriNet netToExport, boolean saveOn) {
	...
	private void jbInit() {
		...
		tabPane.addTab("Configure Simulation Model", ManagerConfiguration.getInstance().getPanel());
		tabPane.addTab("Edit Process Details", hlProcessGui.getPanel());
		tabPane.addTab("Adjust Layout", ManagerLayout.getInstance().getPanel());
		} else {
		...
		// add buttons
		GradientPanel buttonsPanel = new GradientPanel(new Color(120, 120, 120), new Color(90, 90, 90));
		...
		GradientPanel gradient = new GradientPanel(new Color(50, 50, 50), new Color(30, 30, 30));
				...
				if (filename != null && !filename.equals("")) {
					...
					try {
						...
						if (smlName != null) {
							...
							BufferedWriter smlWriter = new BufferedWriter(new OutputStreamWriter(sml));
							mySimulatedPN.writeToFile(outWriter, smlWriter, smlName);
							smlWriter.close();
						} else {
						...
						outWriter.close();
						Message.add("<CPNexport/>", Message.TEST);
						Message.add("Export of CPN model finished.", Message.NORMAL);
	...
	String filename = RedesignAnalysisUI.locationForCurrentSimModels + "//" + hlProcess.getPNModel().getIdentifier()+".cpn";
	try {
