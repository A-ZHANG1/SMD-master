...
import org.prom5.framework.models.hlprocess.att.HLBooleanDistribution;
...
import org.prom5.framework.models.hlprocess.expr.operator.HLNotOperator;
import org.prom5.framework.models.hlprocess.expr.operator.HLOrOperator;
...
/**
 * Contains static utility methods needed for generating 
 * CPN models.
 */
public class CpnUtils {
	 ...
	 * @return String a name which only may contain letters, numbers, the _ (underscore) character,
	 * the ' (apostrophe) character and which has to start with a letter.
	 */
	public static String getCpnValidName(String name) {
		if (name != null && name.equals("") == false) {
			...
			// CPN does not allow names to contain spaces, and other special characters
	...
	/**
	 * Replaces every character other than a letter, a number, an apostrophe, or an underscore
	 * with an underscore. 
	 ...
	 * directly used in cpn. The trick in which you read such a double from a string
	 * is allowed.
	 * @param number double the number for which we want to check whether it contains an
	 * E with a minus sign
	 * @return String In case that the number contains an E with a minus sign, a string
	 * is returned so that in cpn the value is read from a string. Otherwise, the string
	 ...
	 * distribution.
	 * @param dist the distribution for which the CPN distribution is requested
	 * @return the CPN string that can be used to represent the given distribution
	 */
	public static String getCpnDistributionFunction(HLDistribution dist) {
		String result = "";
		if (dist instanceof HLBernoulliDistribution) {
			result = "bernoulli(" + Double.toString(((HLBernoulliDistribution)dist).getProbability()) + ")";
		} else if (dist instanceof HLBinomialDistribution) {
			result = "binomial(" + ((HLBinomialDistribution)dist).getNumberOfExperiments() 
			+ "," + ((HLBinomialDistribution)dist).getProbability() + ")";
		} else if (dist instanceof HLChiSquareDistribution) {
			result = "round(chisq(" + ((HLChiSquareDistribution)dist).getDegreesFreedom() + "))";
			...
			result = "round(erlang(" + ((HLErlangDistribution)dist).getEmergenceOfEvents() 
			+ "," + negativeEnotationCheck(((HLErlangDistribution)dist).getIntensity()) + "))";
		} else if (dist instanceof HLExponentialDistribution) {
			result = "round(exponential(" + negativeEnotationCheck(((HLExponentialDistribution)dist).getIntensity()) + "))";
		} else if (dist instanceof HLGeneralDistribution) {
			result = getCpnDistributionFunction(((HLGeneralDistribution)dist).getBestDistribution());
		} else if (dist instanceof HLNormalDistribution) {
			result = "round(normal(" + negativeEnotationCheck(((HLNormalDistribution)dist).getMean()) + ","
			+ negativeEnotationCheck(((HLNormalDistribution)dist).getVariance()) + "))";
		} else if (dist instanceof HLPoissonDistribution) {
			result = "poisson(1.0 / " + negativeEnotationCheck(((HLPoissonDistribution)dist).getIntensity()) + ")";
		} else if (dist instanceof HLStudentDistribution) {
			result = "round(student(" + ((HLStudentDistribution)dist).getDegreesFreedom() + "))";
		} else if (dist instanceof HLUniformDistribution) {
			result = "round(uniform(" + negativeEnotationCheck(((HLUniformDistribution)dist).getMin()) + ","
			+ negativeEnotationCheck(((HLUniformDistribution)dist).getMax()) + "))";
		} else {
			Message.add("Distribution type not found while creating CPN Tools compliant distribution string");
		}
		return result;
	}
	 ...
	 * @param dist the boolean distribution for which the CPN distribution is requested
	 * @param attName the name of the boolean attribute for which the function is generated
	 * @return the CPN string that can be used to represent the given distribution
	 */
	public static String getCpnDistributionFunction(HLBooleanDistribution dist, String attName) {
		String result = "";
		result = "get" + attName + "Value(bernoulli(" + dist.getProbability() + ")) and " +
		// translate 1 and 0 value into boolean true and false, respectively
		"get" + attName + "Value(i:int) = if (i=1) then true else false";
		return result;
	}
	 ...
	 * @param dist the nominal distribution for which the CPN distribution is requested
	 * @param attName the name of the nominal attribute for which the function is generated
	 * @return the CPN string that can be used to represent the given distribution
	 */
	public static String getCpnDistributionFunction(HLNominalDistribution dist, String attName) {
		String result = "";
		result = "get" + attName + "Value(discrete(1," + dist.getSumOfAllFrequencies() + ")) and " +
		// defines a second function retrieving the nominal value for the random int value in the same row
		...
		for (String val : dist.getValues()) {
			...
			if (counter < dist.getValues().size()) {
				result = result + "\nif (" + accumulatedFreq + "<=i andalso i<=" + (accumulatedFreq+freqVal-1) 
								+ ") then " + val + " else (";
			} else {
	...
	/**
	 * Generates a CPN-compliant string for the given data dependency, which can be 
	 * directly set as guard for the respective CPN transition.
	 * <p> This means that the expression is logically composed of AND (CPN: andalso) and OR (CPN: orelse)
	 * connectors over data attribute statements (CPN: #attributeName data).
	 * @param expr the high level expression to be converted
	...
	/*
	 * Dispatches the expression elements based on their type.
	 * TODO: check how overloading can be put to work or refactor 
	 * (overwrite evaluateToString() method of operators instead)
	 */
	...
	/*
	 * Attribute values will be accessed in the record type of the 
	 * data variable, e.g., "#Amount data". 
	 */
	...
	/*
	 * Values and attributes are expected to be leaf nodes in expressions, 
	 * i.e., no other expressions need to be evaluated below. 
	 */
	...
	/*
	 * Evaluates an AND operator into "(..) andalso (..) andalso (..)", whereas the 
	 * content within the brackets is determined by the sub expressions.
	 */
	private static String getCpnExpression(HLAndOperator op) {
		...
		if (op.getExpressionNode().getChildCount() == 0) {
			...
			HLExpressionElement childExpr = (HLExpressionElement) ((DefaultMutableTreeNode) 
					op.getExpressionNode().getChildAt(0)).getUserObject();
			return childExpr.evaluateToString();
		} else {
			for (int i=0; i<op.getExpressionNode().getChildCount(); i++) {
				HLExpressionElement childExpr = (HLExpressionElement) ((DefaultMutableTreeNode) 
						op.getExpressionNode().getChildAt(i)).getUserObject();
				if (result == "") {
					result = result + "(" + getCpnExpression(childExpr) + ")";
				} else {
					result = result + " andalso " + "(" + getCpnExpression(childExpr) + ")";
				}
			}
			return result;
		}
		...
		if (op.getExpressionNode().getChildCount() == 0) {
			...
			for (int i=0; i<op.getExpressionNode().getChildCount(); i++) {
				...
				if (result == "") {
					...
					result = result + " orelse " + "(" + getCpnExpression(childExpr) + ")";
				}
			}
			return result;
		}
	...
	private static String getCpnExpression(HLGreaterOperator op) {
		...
		HLExpressionElement second = (HLExpressionElement) ((DefaultMutableTreeNode) 
				op.getExpressionNode().getChildAt(1)).getUserObject();
