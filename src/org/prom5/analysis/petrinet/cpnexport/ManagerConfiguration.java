 ...
 * exported to the cpn file (data, resources, timing information, logging functions). Furthermore, it should
 * ensure that there are conflicts with regard to the exported simulation model (e.g. that there resources included
 * in the model and that at the same time waiting times are included in the model).
 *
 * @author arozinat
 * @author rmans
 */
public class ManagerConfiguration {
	 ...
	 */
	private GUIPropertyBoolean myDataPerspectiveSelected = new GUIPropertyBoolean("  Data Perspective  ", 
			"Includes simulation information about data attributes and choices based on data", false);
	private GUIPropertyBoolean myTimePerspectiveSelected = new GUIPropertyBoolean("  Time Perspective  ", 
			"Includes simulation information about the timing of activities and the case generation scheme", false, new TimePerspectiveListener());
	private GUIPropertyBoolean myResourcePerspectiveSelected = new GUIPropertyBoolean("  Resource Perspective  ", 
			"Includes simulation information about groups and resources", false, new ResourcePerspectiveListener());
	 ...
	 */
	protected GUIPropertyInteger myYearOffset = new GUIPropertyInteger(" Year Offset (starting from 1970):", 
			"The year offset that will be used for logging", 38, 0, 1000, null, 100, true);
	private GuiPropertyListRadio PushPullProperty;
	private GUIPropertyBoolean myActivityLoggingMonitor = new GUIPropertyBoolean(" MXML logging", 
			"Generates monitors that create execution logs during the simulation of the CPN model", false, new LoggingOptionListener());
	private GUIPropertyBoolean myCurrentStateOption = new GUIPropertyBoolean(" Current state support", 
			"Generates CPN model with inscriptions for loading a current state file", false, null);
	private GUIPropertyBoolean throughputTimeMonitor = new GUIPropertyBoolean(" Throughput time monitor ", 
			"Generates a monitor for monitoring the throughput time", false, null);
	private GUIPropertyBoolean resourcesMonitor = new GUIPropertyBoolean(" Resource availability monitor ", 
			"Generates a monitor for monitoring the availability of resources", false, null);
	 ...
	 * configuration manager object (Singleton pattern).
	 * Use {@link #getInstance()} instead to retrieve the single object available.
	 */
	private ManagerConfiguration() {
		...
		timeOptionProperty = new GuiPropertyListRadio("", null, timeOptions, new TimeOptionsPerspectiveListener());
		// initially time dimension is deselected (and therefore all sub-options too)
		...
		myWaitingRatio = new GUIPropertyInteger("  percent of the waiting time", 100, 0, 100);
		// define the guipropertylistradio for chosing between push and pull when the waiting+execution time option and the
	 ...
	 * @return boolean <code>true</code> when the data perspective is enabled and
	 * <code>false</code> when the data perspective is disabled
	 */
	 ...
	 * @return boolean <code>true</code> when the time perspective is enabled and
	 * <code>false</code> when the time perspective is disabled
	 */
	...
	public boolean isOnlyExecutionTimeEnabled() {
		return myTimePerspectiveSelected.getValue() && timeOptionProperty.getValue().equals(textExecutionTimeOption);
	}
	...
	public boolean isOnlyWaitingAndExecutionTimeEnabled() {
		return myTimePerspectiveSelected.getValue() && timeOptionProperty.getValue().equals(textWaitingTimeAndExecutionTimeOption);
	}
	...
	public boolean isOnlySojournTimeEnabled() {
		return myTimePerspectiveSelected.getValue() && timeOptionProperty.getValue().equals(textSojournTimeOption);
	}
	
	/**
	 * Retrieves the year offset that indicates when the process starts.
	 * <br> 
	 * For example used for logging timestamps during simulation. The generated logging monitors would start to
	 * log at the Unix zero timestamp (i.e. 1970). 
	 ...
	 * @return boolean <code>true</code> when the resource perspective is enabled and
	 * <code>false</code> when the resource perspective is disabled
	 */
	...
	public boolean isPushEnabled() {
		return PushPullProperty.isEnabled(textPush) && PushPullProperty.getValue().equals(textPush);
	}

	public boolean isPullEnabled() {
		return PushPullProperty.isEnabled(textPull) && PushPullProperty.getValue().equals(textPull);
	}

	public boolean isThroughputTimeMonitorEnabled() {
		return throughputTimeMonitor.isEnabled() && throughputTimeMonitor.getValue();
	}
	...
	public JPanel getPanel() {
		if (outmostLayer == null) {
			...
			configurationOptionsInnerLayer.setLayout(new BoxLayout(configurationOptionsInnerLayer, BoxLayout.Y_AXIS));
			configurationOptionsInnerLayer.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
			configurationOptionsInnerLayer.setBackground(bgColor); 	
			configurationOptionsInnerLayer.add(myDataPerspectiveSelected.getPropertyPanel());
			configurationOptionsInnerLayer.add(Box.createRigidArea(new Dimension(0, 15)));
			configurationOptionsInnerLayer.add(myTimePerspectiveSelected.getPropertyPanel());
			configurationOptionsInnerLayer.add(timeOptionProperty.getPropertyPanel());
			// continue with the remaining boolean properties
			configurationOptionsInnerLayer.add(Box.createRigidArea(new Dimension(0, 15)));
			configurationOptionsInnerLayer.add(myResourcePerspectiveSelected.getPropertyPanel());
			configurationOptionsInnerLayer.add(Box.createRigidArea(new Dimension(0, 5)));
			configurationOptionsInnerLayer.add(myWaitingRatio.getPropertyPanel());
			// add the radio buttons for selecting push and pull
			configurationOptionsInnerLayer.add(PushPullProperty.getPropertyPanel());
			...
			extrasOptionsInnerLayer.setLayout(new BoxLayout(extrasOptionsInnerLayer, BoxLayout.Y_AXIS));
			extrasOptionsInnerLayer.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
			extrasOptionsInnerLayer.setBackground(bgColor); 			
			extrasOptionsInnerLayer.add(myCurrentStateOption.getPropertyPanel());
			extrasOptionsInnerLayer.add(Box.createRigidArea(new Dimension(0, 15)));
			extrasOptionsInnerLayer.add(myActivityLoggingMonitor.getPropertyPanel());
			JPanel loggingOptions = new JPanel();
			loggingOptions.setLayout(new BoxLayout(loggingOptions, BoxLayout.X_AXIS));
			...
			extrasOptionsInnerLayer.add(throughputTimeMonitor.getPropertyPanel());
			...
			jointPanel.add(GuiUtilities.configureAnyScrollable(leftPanel, "Perspectives", 
					"Select which perspectives should be included in the simulation model.", bgColor));
			jointPanel.add(GuiUtilities.configureAnyScrollable(rightPanel, "Extras", 
					"The following additional functions can be generated as a supplement.", bgColor));
	...
	/**
	 * Class handling the selection state changes due to activity 
	 * logging option. 
	 */
		...
		public void updateGUI() {
			if (myTimePerspectiveSelected.getValue()) {
				if (myResourcePerspectiveSelected.getValue()) {
					...
					timeOptionProperty.enable(textWaitingTimeAndExecutionTimeOption);
					timeOptionProperty.disable(textSojournTimeOption);
					// select the execution time option
					timeOptionProperty.setSelected(textExecutionTimeOption, true);
				...
				PushPullProperty.disable(textPush);
				PushPullProperty.disable(textPull);
				// again enable the resource perspective in the case that this had
				// been disabled
	...
	class ResourcePerspectiveListener implements GuiNotificationTarget {
		...
		public void updateGUI() {
			if (myResourcePerspectiveSelected.isEnabled()) {
				if (myResourcePerspectiveSelected.getValue()) {
					// disable the waiting time + execution time and sojourn time radio buttons
					timeOptionProperty.disable(textSojournTimeOption);
					// in the case that the timing perspective is enabled make sure that that the
					// execution time radio button is selected
					if (myTimePerspectiveSelected.getValue()&&timeOptionProperty.getValue().equals(textSojournTimeOption)) {
						timeOptionProperty.setSelected(textExecutionTimeOption, true);
					}
					if (myTimePerspectiveSelected.getValue()&&timeOptionProperty.getValue().equals(textWaitingTimeAndExecutionTimeOption)) {
					...
					if (myTimePerspectiveSelected.getValue()) {
						// enable the waiting time + execution time and sojourn time radio buttons
	...
	/**
	 * Class handling the selection state change of the radio buttons for the time options
	 */
	class TimeOptionsPerspectiveListener implements GuiNotificationTarget {

		public void updateGUI() {
			if ( (timeOptionProperty.getValue().equals(textSojournTimeOption) &&
					timeOptionProperty.isEnabled(textSojournTimeOption) ) ) {
			...
			if ( timeOptionProperty.getValue().equals(textWaitingTimeAndExecutionTimeOption) &&
				timeOptionProperty.isEnabled(textWaitingTimeAndExecutionTimeOption)) {
			     if (!myResourcePerspectiveSelected.isEnabled()) {
					 myResourcePerspectiveSelected.enable();
			     }
				 if (myResourcePerspectiveSelected.getValue()) {
