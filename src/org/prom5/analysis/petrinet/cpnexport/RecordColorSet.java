...
public class RecordColorSet extends CpnColorSet {
	...
	/**
	 * constructor
	 * @param name String the name of this record colorset
	 */
	 ...
	 * @param nameRecord String the name of the record.
	 * @param csNameRecord String the name of the color set.
	 */
	...
	public void write(BufferedWriter bw) throws IOException {
		...
		if ((recordsString.length() -2) >= 0) {
			recordsString = recordsString.substring(0, recordsString.length()-2);
		}
		bw.write("\t\t\t\t</record>\n" +
				"\t\t\t\t<layout>" + "colset " + CpnUtils.getCpnValidName(myNameColorSet) + " = record "
				+ recordsString + " declare set;</layout>\n" +
				"\t\t\t</color>");
	}
