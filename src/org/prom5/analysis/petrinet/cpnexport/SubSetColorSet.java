...
public class SubSetColorSet extends CpnColorSet {
	...
	/**
	 * Constructor
	 * @param nameColorSet String the name of the subset color set
	 * @param subSetName String the type of all the values in the subset color set.
	 */
	...
	/**
	 * Retrieves an initial marking based on the possible values that have
	 * been declared for this subset color set.
	...
	public void write(BufferedWriter bw) throws IOException {
		...
		bw.write("\t\t\t\t\t<color id=\"" + ManagerID.getNewID() + "\">\n" +
				"\t\t\t\t\t\t<id>" + CpnUtils.getCpnValidName(getNameColorSet()) + "</id>\n" +
				"\t\t\t\t\t\t<subset>\n" +
				"\t\t\t\t\t\t\t<id>" + CpnUtils.getCpnValidName(mySubSetName) + "</id>\n" +
				"\t\t\t\t\t\t\t<with>\n");
		bw.write("\t\t\t\t\t\t\t\t<ml>" + mlPart + "</ml>\n");
		bw.write("\t\t\t\t\t\t\t</with>\n" +
				"\t\t\t\t\t\t</subset>\n" +
				"\t\t\t\t\t\t<layout>" + "colset " + CpnUtils.getCpnValidName(myNameColorSet) + " = subset " +
				CpnUtils.getCpnValidName(mySubSetName) + " with " + mlPart + ";</layout>\n" +
				"\t\t\t\t\t</color>\n");
	}
