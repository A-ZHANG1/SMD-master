...
public class ControlFlow implements ICalculator {
	...
	public String Calculate() {
		...
		for(int count = 0; count < i; count ++){
			Transition transition = (Transition) net.getTransitions().get(count);
			if(isAND(transition)){
				result++;
	...
	public String VerifyBasicRequirements() {
		...
		}// verify if there are at least two places(one for the begining of the net and the other for the end of it),
