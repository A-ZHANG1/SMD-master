...
public class NumberOfANDSplit implements ICalculator {
	private PetriNet net;
	private final static String TYPE = "Size";
	private final static String NAME = "AND-Splits";


	public NumberOfANDSplit(PetriNet net) {
		super();
		this.net = net;
	}
	...
	public String Calculate() {
		...
		for(int o = 0 ; o<i ; o++){
			...
			if(!(inEdges == null)){
				if (!(outEdges == null)){
					if(transition.getOutEdges().size()>1 && transition.getInEdges().size()>0){
					result++;
					}
				}
			}
		}
		Message.add("\t<NumOfAndSplits value=\""+result+"\"/>", Message.TEST);
		return ""+result;
	}
