...
public class PNComplexityCalculatorUI extends JPanel implements Provider {
	...
	public PNComplexityCalculatorUI(PetriNet net) {
		...
		Message.add("<PetriNetComplexityCalculator " +
				"NumOfArcs=\""+net.getEdges().size()+"\" " +
				"NumOfPlaces=\""+net.getPlaces().size()+"\" "+
				"NumOfTransitions=\""+net.getTransitions().size()+"\" >", Message.TEST);
		try {
			jbInit();
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	private String[][] getValues(){
		...
		if(!calc.VerifyBasicRequirements().startsWith(".")){
			String text = this.ErrorTextArea.getText();
			this.ErrorTextArea.setText(text + "\n[Size]"+calc.VerifyBasicRequirements());
	...
	public ProvidedObject[] getProvidedObjects() {
        ...
        new ProvidedObject("Petri net", new Object[]{net}),
        };
    }
}
