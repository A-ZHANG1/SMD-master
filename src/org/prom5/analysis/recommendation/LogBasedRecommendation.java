 ...
 * @version 1.0
 */
public class LogBasedRecommendation extends SingleProcessAcceptor implements RecommendationProvider {
	 ...
	 *
	 * @param log LogReader
	 * @param startServerImmediately boolean
	 * @return JComponent
	 * @todo Implement this
	 *   org.processmining.analysis.recommendation.basic.SingleProcessAcceptor
	 *   method
	 */
	protected JComponent analyse(BufferedLogReader log, boolean startServerImmediately ) {
	...
	public RecommendationResult getRecommendation(RecommendationQuery query) throws
			IOException, Exception {
		ui.writeQuery(query);
		// Message.add("received query with id " + query.getId());
		RecommendationResult result = ui.getContributor().generateRecommendations(query,process);
	...
	/* (non-Javadoc)
	 * @see org.processmining.framework.models.recommendation.RecommendationProvider#handleCompletedExecution(org.processmining.framework.log.ProcessInstance)
	 */
	public void handleCompletedExecution(ProcessInstance instance)
		...
		Message.add("Added instance "+ instance.getProcess()+ "."+instance.getName()+" to log, which now contains "+log.numberOfInstances()+" instances");
