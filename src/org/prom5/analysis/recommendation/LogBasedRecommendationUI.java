...
import org.prom5.analysis.recommendation.contrib.NoFactoryException;
...
import org.prom5.framework.ui.PluginComboItem;
...
public class LogBasedRecommendationUI extends JPanel implements Provider {
    ...
    public LogBasedRecommendationUI(final LogReader log,
        ...
        this.log = log;
        Iterator<Plugin>
                it = ScaleCollection.getInstance().getPlugins().iterator();
        while (it.hasNext()) {
        ...
        { //Scale part
            ...
            scales.setPreferredSize(new Dimension(200,
                                                  (int) scales.getPreferredSize().
                                                  getHeight()));
            selPanel.add(scales);
        }
        { //Contributor part
        ...
        topPanel.add(selPanel);
        {
            ...
            portField.setPreferredSize(new Dimension(50,
            ...
            startButton.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    logBasedContributor = (LogBasedContributor)
                                          ((PluginComboItem) contributors.
                                           getSelectedItem()).
                                          getPlugin();

                    ProcessInstanceScale scale = (ProcessInstanceScale) ((
                            PluginComboItem) scales.
                    ...
                    RecommendationServiceHandler handler = new
     ...
     * writeResult
     *
     * @param result RecommendationResult
     */
    public void writeResult(final RecommendationResult result) {
        if (writeResults.isSelected()) {
            ...
            SwingUtilities.invokeLater(new Runnable() {
                public void run() {
                    tableData.setResultCaret(result.getQueryId(),
                                             resultsPane.getCaretPosition());
                    resultsPane.setCaretPosition(resultsPane.getText().length());
                }
            });
        }
    }
    ...
    public void writeQuery(final RecommendationQuery query) {
        if (writeResults.isSelected()) {
            ...
            SwingUtilities.invokeLater(new Runnable() {
                public void run() {
                    tableData.setQueryCaret(query.getId(),
                                            queryPane.getCaretPosition());
    ...
    public void requestRestart(String contributor, String scaleName) throws
            Exception {
        for (int i = 0; i < contributors.getItemCount(); i++) {
            LogBasedContributor contributor_i = (LogBasedContributor)
                                                ((PluginComboItem) contributors.
                                                 getItemAt(i)).
                                                getPlugin();
        ...
        for (int i = 0; i < scales.getItemCount(); i++) {
            ProcessInstanceScale scale_i = (ProcessInstanceScale)
                                           ((PluginComboItem) scales.getItemAt(
                    i)).
                                           getPlugin();
