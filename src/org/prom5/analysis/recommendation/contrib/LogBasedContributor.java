 ...
 *
 */
package org.prom5.analysis.recommendation.contrib;
...
import org.prom5.framework.models.logabstraction.LogAbstraction;
...
/**
 * This abstract class may be used as a starting point for contributor implementations
 * which cluster a set of process instances. It provides a method for calculating the
 * @author christian
 *
 */
public abstract class LogBasedContributor extends RecommendationContributor {
	...
	public RecommendationResult generateFilteredRecommendations(RecommendationQuery query,
			String process) {
		ProcessInstance pi = query.toProcessInstance(process);
		// No log is present, since this is the partial trace. Note that we
		// pass "null" as the LogReader, since this instance is not going to be weight
		LogAbstraction incompletePIAbstraction = logAbstractionFactory.getAbstraction(null, pi,
												 scale);

		double partialTraceWeight = scale.weigh(pi);
		RecommendationResult result = new RecommendationResult(query.getId(), partialTraceWeight);
		...
		// provided that these events are a possible continuation in that abstraction.

		for (LogAbstraction abstraction : logAbstractions) {
			abstractionEnabledEvents = abstraction.getPossibleNextLogEvents(
										  incompletePIAbstraction);
			
			// Now, do not consider this abstraction, if the intersection between
			// enabledEvents and recommendationEnabledEvents is empty
			
			//SortedSet<LogEvent> intersection = new TreeSet<LogEvent>(abstractionEnabledEvents);
			//intersection.retainAll(enabledEvents);
			//if (intersection.isEmpty()) {
			//	continue;
			//}
			
			// There is at least one element both in the set of enabled events, as well as
			// in the set of abstractionEnabledEvents
			// Message.add("Case "+abstraction);
			for (LogEvent evt : enabledEvents) {
				// The event evt is enabled
				Recommendation rec = abstraction.getRecommendation(evt, incompletePIAbstraction, enabledEvents);
...
/*		for (LogEvent e : enabledEvents) {
			Recommendation r = new Recommendation();
			r.setDontWeight(sumW);
			r.setDontExpectedValue(sumE / sumW);
			r.setDontExpectedSquaredValue(sumE2 / sumW);
			r.setTask(e.getModelElementName());
...
*/
	...
	private void update(List<Recommendation> recommendations, Recommendation toAdd,
			SortedSet<LogEvent> abstractionEnabledEvents) {
		...
		for (int i = 0; i < recommendations.size(); i++) {
			...
			if (rec.equals(toAdd)) {
				...
				if (!abstractionEnabledEvents.contains(toAdd.getLogEvent())) {
					// It is not possible in the abstraction currently being processed that
					// the logEvent proposed by toAdd is executed, hence it has influence on the don't
