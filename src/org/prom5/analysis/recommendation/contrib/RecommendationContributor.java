...
import org.prom5.framework.log.filter.AbstractLogFilter;
import org.prom5.framework.log.filter.DefaultLogFilter;
import org.prom5.framework.models.logabstraction.LogAbstraction;
import org.prom5.framework.models.logabstraction.LogAbstractionFactory;
...
import org.prom5.framework.models.recommendation.compat.RecommendationQueryFilter;
...
public abstract class RecommendationContributor implements Plugin {
	...
	/**
	 * The scale which is used to calculate the weight of this contributor
	 * based on the contained process instances' weights as their sum.
	 */
	protected ProcessInstanceScale scale;

	/**
	 * @return The scale used for weighing the contained
	 * process instances.
	 */
	...
	/**
	 * Sets the scale used for weighing this contributor based
	 * on the contained process instances.
	 * @param aScale a process instance scale.
	 */
	...
	public void initialize(LogReader log, ProcessInstanceScale scale) throws NoFactoryException {
	...
	/**
	 * logAbstractor that takes care of the abstraction of the entire log, as well
	 * as any offered query
	 */
	...
	public RecommendationResult generateRecommendations(RecommendationQuery query,
			String process) {
		return generateFilteredRecommendations(filter.filter(query, process), process);
	}
	 ...
	 * the body of the html page, so the <code>html</code>, <code>head</code> and
	 * <code>body</code> tags should <b>not</b> be used.
	 ...
	 * folder and in this case it can be obtained by calling <code>PluginDocumentationLoader.load(this)</code>
	 *
