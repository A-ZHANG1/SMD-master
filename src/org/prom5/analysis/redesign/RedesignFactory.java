...
/**
 * Defines the possible redesigns that can be made.
 * <br>
 * There are eight types of redesigns: parallel, sequential, group,
 * add task, remove task, add constraint, remove constraint, copy model.
 */
...
public class RedesignFactory {
    ...
    public static String[] getAllRedesignTypes() {
        return new String[] {"Parallel Redesign", "Sequence Redesign", "Group redesign",
          "Add Task", "Remove Task", "Add Constraint", "Remove Constraint", "Copy model"};
    }
  ...
  return false;
        ...
        case SEQUENCE_INDEX:
         for (PNNode node : net.getNodes()) {
          ...
          if(hasSuccessors(nodes)) {
           ...
           if (hasSuccessors(succNodes)) {
            getAcyclicMGComponents(model, components, nodes, succNodes);
           }
          }
          /**
           * clean up for next node
     ...
     * @param indexRedesignType int Redesign type.
     * @param model HLPetriNet Model to be redesigned.
     * @param transitions List<Transition> the transitions to redesign, can be all transitions.
     * @returns comp Component The component including the nodes to be redesigned. 
     */
    public static Component getComponent(int indexRedesignType, 
      HLPetriNet model, List<Transition> transitions) throws Exception {
    Component comp = null;
    /**
  * all transitions in the model are selected for redesign.
  * Find the maximal component in the model.
  */
 if (transitions.size() == model.getPNModel().getTransitions().size()) {
     comp = getMaximalComponent(indexRedesignType, model, transitions);
    } else {
     /**
      * some transitions in the model are selected for redesign.
      * Find the minimal component that encloses these transitions.
      */
 ...
 return comp;
     ...
     * @param indexRedesignType int Redesign type.
     * @param model HLPetriNet Model to be redesigned.
     * @param transitions List<Transition> the transitions to redesign.
     * @returns comp Component The process part that should be redesigned. 
     * Includes the transitions
     */
    public static Component getMinimalComponent(int indexRedesignType, HLPetriNet model, List<Transition> transitions)  throws Exception {
     Component comp = null;
        ...
        case PARALLEL_INDEX:
         ...
         for (Component c : components) {
          if (c.getTransitions().equals(transitions)) {
           /**
            * the selected transitions form a component, return component
            */
           comp = c;
           break;
          } else if (c.getTransitions().containsAll(transitions)) {
           /**
            * collect the components that include all selected transitions
            */
           inclComps.add(c);
          }
         }
         /**
          * find the minimal component of the components that 
          * include all selected transitions
          */
         if (inclComps.size() == 0) {
          Message.add("There are no components that include all selected transitions.",
                         Message.ERROR);
                        JOptionPane.showMessageDialog(null, "It is not possible to apply " +
    ...
    /**
     * Returns the maximal component that can be found in the set of transitions.
     * @param indexRedesignType int Redesign type.
     * @param model HLPetriNet Model to be redesigned.
     * @param transitions List<Transition> the transitions to redesign, can be all transitions.
     * @returns comp Component The process part that should be redesigned. 
     ...
     */
    public static Component getMaximalComponent(int indexRedesignType, HLPetriNet model, List<Transition> transitions)  throws Exception {
     Set<Component> components = getAllComponents(indexRedesignType, model);
     Component comp = null;
     Set<Component> inclComps = new HashSet<Component>();
     /**
     ...
     if (!components.isEmpty()) {
          ...
          */
         if (inclComps.size() == 0) {
          Message.add("There are no components for this transformation rule.",
                         Message.ERROR);
                        JOptionPane.showMessageDialog(null, "It is not possible to apply " +
     ...
     * @param indexRedesignType int Redesign type.
     * @param model HLPetriNet Model to be redesigned.
     * @param comp Component The component including the transitions to be redesigned.
     * @return alt HLPetriNet Result of a redesign with the selected redesign type.
     * Returns alt = null if the redesign fails, this should be handled where
     * this method is called.
     */
    public static HLPetriNet getRedesign(int indexRedesignType, HLPetriNet model, 
      Component comp) throws Exception {
     HLPetriNet alt = null;
     /**
      * perform the redesign for the selected redesign type with the
      * created component.
      * If there is no component, HLPetriNet alt is null, this should
      * be handled where this method is used.
      */
        if (comp != null) {
     Transformation trans = new Transformation(model, comp);
        ...
        case PARALLEL_INDEX:
            if (comp.isProperComponent()) {
                ...
                Message.add("The selected transitions do not form a proper component",
                 Message.ERROR);
                JOptionPane.showMessageDialog(null, "It is not possible to apply " +
                "the selected transformation rule on the current selection of transitions.");
             /**
              *  note that alt remains null, is handled in RedesignAnalysisUI.
              */
            }
            break;
        case SEQUENCE_INDEX:
            if (comp.isAcyclicMGComponent()) {
                ...
                Message.add("The selected transitions do not form an acyclic MG component",
                            Message.ERROR);
                JOptionPane.showMessageDialog(null, "It is not possible to apply " +
                "the selected transformation rule on the current selection of transitions.");
             // note that alt remains null, is handled in RedesignAnalysisUI.
        ...
        case GROUP_INDEX:
         break;
        case ADDTASK_INDEX:  
         //give warning if input data is not available, provide possibility to cancel
         //It is possible to continue, because yet another task may be added to create the input
            if (!comp.isComponent()) {
             Message.add("Please be aware that not all input data elements " +
                "for the added node are available", Message.WARNING);
             // provide user with a message asking (s)he is sure
             final JOptionPane optionPane = new JOptionPane("Please be aware that not all " +
            ...
            break;
        case REMOVETASK_INDEX:
         //give warning if necessary data will be removed, provide possibility to cancel
         //It is possible to continue, because these data may be added to other tasks
            if (!comp.isComponent()) {
                Message.add("Please be aware that data elements necessary for the " +
                "execution of other tasks will be removed", Message.WARNING);
                // provide user with a message asking (s)he is sure
                final JOptionPane optionPane = new JOptionPane("Please be aware that data elements necessary for the " +
        ...
        case ADDCONSTRAINT_INDEX:
         if (comp.netIncludesNodes() && comp.isTwoTransitions()) {
           ...
           * constraint can be added. Decide which transition is the first,
           * the constraint is added from the first to the second transition.
           */
          String t1 = comp.getNodeList().get(0).getIdentifier();
          String t2 = comp.getNodeList().get(1).getIdentifier();
          final JOptionPane optionPane = new JOptionPane("You have selected the transitions " +
                t1 + " and " + t2 + ". Would you like transition " + t1 +
                " to be the start point of the constraint?",
                JOptionPane.QUESTION_MESSAGE, JOptionPane.YES_NO_OPTION);
                final JDialog dialog = new JDialog(MainUI.getFrames()[0], "Question", true);
                dialog.setContentPane(optionPane);
                optionPane.addPropertyChangeListener(new PropertyChangeListener() {
                     public void propertyChange(PropertyChangeEvent e) {
                            String prop = e.getPropertyName();
          ...
          Message.add("Exactly two transitions have to be selected for the " +
                  "execution of add constraint",
                            Message.ERROR);
                JOptionPane.showMessageDialog(null, "Please select two transitions.");
             // note that alt remains null, is handled in RedesignAnalysisUI.
         }
         break;
        case REMOVECONSTRAINT_INDEX:
        ...
        case COPYMODEL_INDEX:
         if (comp.netEqualsNodes()) {
          ...
          alt = trans.createCopy();
                ...
                Message.add("The complete model has to be selected for the " +
                  "execution of copy model",
                            Message.ERROR);
                JOptionPane.showMessageDialog(null, "Please select the complete " +
