package org.prom5.analysis.redesign;
...
import org.prom5.framework.models.hlprocess.HLGlobal;
...
import org.prom5.framework.models.hlprocess.expr.HLDataExpression;
import org.prom5.framework.models.hlprocess.hlmodel.HLPetriNet;
import org.prom5.framework.models.hlprocess.pattern.Component;
import org.prom5.framework.models.petrinet.PNEdge;
import org.prom5.framework.models.petrinet.PNNode;
...
public class Transformation {
     ...
     * 
     * @param aModel HLPetriNet the input model which is being redesigned
     * @param aNodes Collection<PNNode> the nodes selected in the input model
     */
    ...
    /**
     * As a starting point for the parallel and sequence redesign a graph
     * with only vertices, i.e., no edges are between them, is used.
     * The graph is created with the transitions selected in the input model.
     * 
     * @return graph ModelGraph the resulting graph with only vertices
     */
    public ModelGraph createInitialGraphNodes() {
        /**
         * Create an empty graph and add the transitions as vertices.
         * Only transitions that produce output data are included.
         */ 
     ...
     * Relations are added between the vertices in the initial graph based on the
     * data dependencies.
     * 
     * @param graph ModelGraph the input graph without edges
     * @return graph ModelGraph the resulting parallel graph
     */
    public ModelGraph createParallelGraph(ModelGraph graph) {
        /**
         *  Add arcs between nodes that have a dependency.
         * The result is that nodes without dependencies are placed in parallel.
         */ 
        if (graph.getVerticeList().size() > 1) {
            for (ModelGraphVertex v1 : graph.getVerticeList()) {
                for (ModelGraphVertex v2 : graph.getVerticeList()) {
                    HLActivity hlAct1 = subNet.getHLProcess().findActivityByName(v1.getIdentifier());
                    HLActivity hlAct2 = subNet.getHLProcess().findActivityByName(v2.getIdentifier());
        ...
        /**
         *  Add articifical start / end node, if the graph starts / ends 
         *  with multiple nodes.
         */
        addStartNode(graph);
        addEndNode(graph);
        ...
        /**
         * Transitively reduces this graph, i.e. removes an edge if there is another path between
         * two nodes. The result is unique for a-cyclic graphs
         * See ModelGraph
         */
    ...
    /**
     * The parallel redesign is created by a number of methods. First, the initial
     * graph with only vertices is created, from this a parallel graph is made.
     * Then, a parallel petri net is created, followed by a parallel HLPetriNet.
     * The HLPetriNet is incorporated in the input model, this creating an
     * alternative model.
    ...
    public ModelGraph createSequentialGraph(ModelGraph graph) {
        ...
        if (sortedNodes.get(sortedNodes.size() - 1).toString().contains("addedEndNode")) {
            seqGraph.removeVertex((ModelGraphVertex) sortedNodes.get(sortedNodes.size() - 1));
    ...
    /**
     * The sequence redesign is created by a number of methods. First, the initial
     * graph with only vertices is created, from this a sequential graph is made.
     * Then, a petri net is created, followed by a HLPetriNet.
     * This HLPetriNet is incorporated in the input model, this creating an
     * alternative model.
     * @return alt HLPetriNet the alternative model.
     */
     ...
     * 
     * @param graph ModelGraph input graph that is translated to a Petri net
     * @return process PetriNet the resulting Petri net
     */
    public PetriNet createPetriNet(ModelGraph graph) {
        /**
         *  A place is added between each two nodes that are connected in 
         *  the graph.
         */
        ...
        for (ModelGraphVertex vertex : graph.getVerticeList()) {
            ...
            /**
             *  add log event; necessary to create visible transitions 
             *  for PetriNet.
             */
            if (!(t.getIdentifier().contains("addedStartNode") || t.getIdentifier().contains("addedEndNode"))) {
        ...
        for (ModelGraphVertex v1 : graph.getVerticeList()) {
            for (ModelGraphVertex v2 : graph.getVerticeList()) {
                if (graph.getEdgesBetween(v1, v2).size() > 0) {
                    Transition act1 = process.findTransition(v1.getIdentifier());
                    Transition act2 = process.findTransition(v2.getIdentifier());
                    Place newPlace = new Place("place_" + act1.getIdentifier() + "_" + act2.getIdentifier(), process);
     ...
     * Petri net
     * @param net PetriNet the Petri net that is translated to a HLPetriNet
     * @param hlNet HLPetriNet the HLPetriNet which HL info is used in the translation
    ...
    public HLPetriNet createHLPetriNet(PetriNet net, HLPetriNet hlNet) {
        ...
        for (ModelGraphVertex oldVertex : hlNet.getActivityNodes()) {
            for (ModelGraphVertex newVertex : copyNet.getGraphNodes()) {
                if (oldVertex.getIdentifier().equalsIgnoreCase(newVertex.getIdentifier())) {
                    HLActivity clonedAct = (HLActivity) hlNet.findActivity(oldVertex).clone();
        ...
        for (ModelGraphVertex oldVertex : hlNet.getChoiceNodes()) {
            ...
            for (ModelGraphVertex newVertex : copyNet.getChoiceNodes()) {
                if (newVertex.getIdentifier().equalsIgnoreCase(oldVertex.getIdentifier())) {
                    HLChoice newChoice = copyNet.findChoice(newVertex);
                    for (Edge newEdge : newVertex.getOutEdges()) {
                        // Attempt to make this work when target has been removed during redesign.
//                     //   for (Edge oldEdge : oldVertex.getOutEdges()) {
//                            Transition newTrans = (Transition) newEdge.getHead();
//                            HLActivity newTarget = copyNet.findActivity(newTrans);
//                         //   Transition oldTrans = (Transition) oldEdge.getHead();
//                            HLActivity posTarget = hlNet.findActivity(newTrans);
//                            if (oldVertex.getSuccessors().contains(newTrans)) { //kan niet!!
//                            // if (posTarget.getID().equals(newTarget.getID()) {
//                            // set choice target
//                            HLCondition newCondition = newChoice.addChoiceTarget(newTarget.getID());
//                                HLCondition oldCondition = oldChoice.getCondition(oldTarget.getID());
//                                HLDataExpression expression = oldCondition.getExpression();
//                                if (expression != null) {
...
//                  //      }
                        ...
                        for (Edge oldEdge : oldVertex.getOutEdges()) {
                            ...
                            if (oldTarget.getID().equals(newTarget.getID())) {
                                ...
                                try {
                                    /**
                                     *  old target may differ from new target, has to be improved.
                                     */
                                    HLCondition oldCondition = oldChoice.getCondition(oldTarget.getID());
                                    HLDataExpression expression = oldCondition.getExpression();
                                    if (expression != null) {
                                    ...
                                    Message.add("Error during adaptation of transition targets. Target " +
                                                newTrans.getIdentifier() + " is not the same as the target of the input model", Message.ERROR);
                                }
                            }
                        }
                    }
        ...
        copyNet.getHLProcess().getGlobalInfo().setCaseGenerationScheme(copyGlobal.getCaseGenerationScheme());
        copyNet.getHLProcess().getGlobalInfo().setTimeUnit(copyGlobal.getTimeUnit());
        /**
         ...
         */  
        // update nobody and anybody ID as activities will still point to nobody and anybody IDs   
        // from the source hlModel (cloning of IDs is not needed)  
        copyNet.getHLProcess().removeGroupWithoutAffectingAssigmnets(copyNet.getHLProcess().getNobodyGroupID());  
        copyNet.getHLProcess().setNobodyGroupID(hlNet.getHLProcess().getNobodyGroupID());  
        copyNet.getHLProcess().removeGroupWithoutAffectingAssigmnets(copyNet.getHLProcess().getAnybodyGroupID());  
        copyNet.getHLProcess().setAnybodyGroupID(hlNet.getHLProcess().getAnybodyGroupID());  
        // now clone the real group objects  
        Iterator<HLGroup> groups = hlNet.getHLProcess().getAllGroups().iterator();  
    ...
    /**
     * Replaces the sub net formed by the selected nodes by an 
     * alternative HLPetriNet. 
     * It is assumed that both nets fulfill the requirements that are necessary
     * to return a sound model.
     * @param model HLPetriNet the original model of which a part is replaced
     * @param nodes the selected nodes in the original model that are replaced
     * @param altPart PetriNet the replacing part
     * @return altModel HLPetriNet the resulting model
     */
    public HLPetriNet replace(HLPetriNet model, List<PNNode> nodes, PetriNet altPart) {
        /** 
        ...
        * and replaced by the created alternative part.
        ...
        */
        ...
        for (PNNode node : altNet.getNodes()) {
            for (PNNode selNode : nodes) {
                if (node.getIdentifier().equalsIgnoreCase(selNode.getIdentifier())) {
        ...
        /**
         *  Now, we are adding all relevant edges.
         *  First, note that the edges from the original model that should 
         *  remain are still there.
         * Then, add the edges from the alternative part.
         */
        ...
        while (edges.hasNext()) {
            ...
            if (edge.isPT()) {
                ...
                Transition myTransition = (Transition) altNet.findTransition(t.getIdentifier());
         ...
         */
        /**
         *  First, connect the source of the alternative part with the preceding part of the altNet
         */
        for (PNNode node : model.getPNModel().getNodes()) {
            /**
             *  find the source node of the component
             */
            if (subNet.getPNModel().getSource().getIdentifier().equalsIgnoreCase(node.getIdentifier())) {
            	/**
            	 * check if subNet starts with place.
            	 * if so, add place before source of altNet.
            	 * also add it to altPart, to give it a matching source
            	 */   	
            	if (node instanceof Place) {
            		Transition firstTransition = (Transition) altPart.getSource();
            		Place newPlace = new Place(subNet.getPNModel().getSource().getIdentifier(), altNet);
            		altNet.addAndLinkPlace(newPlace);
            		altPart.addAndLinkPlace(newPlace);
                    PNEdge edge = new PNEdge(newPlace, altNet.findTransition(firstTransition.getIdentifier()));
                    altNet.addAndLinkEdge(edge, newPlace, altNet.findTransition(firstTransition.getIdentifier()));
                    altPart.addAndLinkEdge(edge, newPlace, altNet.findTransition(firstTransition.getIdentifier()));
                }
            	/**
            	 * only connect with preceding part if source node has predecessors
            	 */
            	if (!(node.getPredecessors().size() == 0)) {
                /**
                 *  get the predecessors of this source node, they have to be connected with the alternative part.
                 */
                ...
                for (PNNode pred : preds) {
                     ...
                     */
                    if (altNet.findTransition(pred.getIdentifier()) != null && altNet.findTransition(altSource.getIdentifier()) != null) {
                        Place newPlace = new Place("place_" + pred.getIdentifier() + "_" + altSource.getIdentifier(), altNet);
                        altNet.addAndLinkPlace(newPlace);
                        PNEdge edge1 = new PNEdge(altNet.findTransition(pred.getIdentifier()), newPlace);
                        PNEdge edge2 = new PNEdge(newPlace, altNet.findTransition(altSource.getIdentifier()));
                        altNet.addAndLinkEdge(edge1, altNet.findTransition(pred.getIdentifier()), newPlace);
                        altNet.addAndLinkEdge(edge2, newPlace, altNet.findTransition(altSource.getIdentifier()));

                    } else {
                        /**
                         *  pred is place and altSource is transition
                         */
                        if (altNet.findPlace(pred.getIdentifier()) != null && altNet.findTransition(altSource.getIdentifier()) != null) {
                            PNEdge edge = new PNEdge(altNet.findPlace(pred.getIdentifier()), altNet.findTransition(altSource.getIdentifier()));
                            altNet.addAndLinkEdge(edge, altNet.findPlace(pred.getIdentifier()), altNet.findTransition(altSource.getIdentifier()));
                        } else {
                            /**
                             *  pred is transition and altSource is place
                             */
                            if (altNet.findTransition(pred.getIdentifier()) != null && altNet.findPlace(altSource.getIdentifier()) != null) {
                                PNEdge edge = new PNEdge(altNet.findTransition(pred.getIdentifier()), altNet.findPlace(altSource.getIdentifier()));
                                altNet.addAndLinkEdge(edge, altNet.findTransition(pred.getIdentifier()), altNet.findPlace(altSource.getIdentifier()));
                            }
                        }
                    }
                }
        ...
        /**
         *  Then, connect the sink of the alternative part with the succeeding part of the altNet
         */
        for (PNNode node : model.getPNModel().getNodes()) {
            /**
             *  find the sink node of the component
             */
            if (subNet.getPNModel().getSink().getIdentifier().equalsIgnoreCase(node.getIdentifier())) {
            	/**
            	 * check if subNet ends with place.
            	 * if so, add place before sink of altNet.
            	 * also add it to altPart, to give it a matching source
            	 */
            	if (node instanceof Place) {
            		Transition lastTransition = (Transition) altPart.getSink();
            		Place newPlace = new Place(subNet.getPNModel().getSink().getIdentifier(), altNet);
            		altNet.addAndLinkPlace(newPlace);
            		altPart.addAndLinkPlace(newPlace);
                    PNEdge edge = new PNEdge(altNet.findTransition(lastTransition.getIdentifier()), newPlace);
                    altNet.addAndLinkEdge(edge, altNet.findTransition(lastTransition.getIdentifier()), newPlace);
                    altPart.addAndLinkEdge(edge, altNet.findTransition(lastTransition.getIdentifier()), newPlace);
                }
            	/**
            	...
            	/**
            	 *  get the successors of this sink node, they have to be connected with the alternative part
            	 */
                ...
                for (PNNode suc : sucs) {
                     ...
                     *  if both are transitions a place has to be added
                     */
                    if (altNet.findTransition(suc.getIdentifier()) != null && altNet.findTransition(altSink.getIdentifier()) != null) {
                        Place newPlace = new Place("place_" + altSink.getIdentifier() + "_" + suc.getIdentifier(), altNet);
                        altNet.addAndLinkPlace(newPlace);
                        PNEdge edge1 = new PNEdge(newPlace, altNet.findTransition(suc.getIdentifier()));
                        PNEdge edge2 = new PNEdge(altNet.findTransition(altSink.getIdentifier()), newPlace);
                        altNet.addAndLinkEdge(edge1, newPlace, altNet.findTransition(suc.getIdentifier()));
                        altNet.addAndLinkEdge(edge2, altNet.findTransition(altSink.getIdentifier()), newPlace);

                    } else {
                        /**
                         *  suc is place and altSource is transition
                         */
                        if (altNet.findPlace(suc.getIdentifier()) != null && altNet.findTransition(altSink.getIdentifier()) != null) {
                            PNEdge edge = new PNEdge(altNet.findTransition(altSink.getIdentifier()), altNet.findPlace(suc.getIdentifier()));
                            altNet.addAndLinkEdge(edge, altNet.findTransition(altSink.getIdentifier()), altNet.findPlace(suc.getIdentifier()));
                        } else {
                            /**
                             *  pred is transition and altSource is place
                             */
                            if (altNet.findTransition(suc.getIdentifier()) != null && altNet.findPlace(altSink.getIdentifier()) != null) {
                                PNEdge edge = new PNEdge(altNet.findPlace(altSink.getIdentifier()), altNet.findTransition(suc.getIdentifier()));
                                altNet.addAndLinkEdge(edge, altNet.findPlace(altSink.getIdentifier()), altNet.findTransition(suc.getIdentifier()));
                            }
                        }
                    }
                }
        ...
        /**
         * The low-level Petri net is finished.
         * create a HLPetrinet from it
         * note that with param model addedStartNode and addedEndNode are not 
         * added to the HLActivities of altModel.
         */
     ...
     * @param process ModelGraph the graph under consideration
    ...
    public ModelGraph addEndNode(ModelGraph process) {
        /**
         * add a new end node if there are multiple end nodes and connect
         * the end nodes to the new node.
         * */
    ...
    /**
     * The input model is copied and a constraint, in the form of a 
     * HLAttribute, is added between the two selected transitions.
     * @param firstTransitionIdentifier String the identifier of the first transition
     * @param secondTransitionIdentifier String the identifier of the second transition
     * @param ConstraintName String the name of the constraint
     * @return alt HLPetriNet the resulting model
     */
    public HLPetriNet addConstraint(String firstTransitionIdentifier,
    	 ...
    	 */
    	HLAttribute constraint = new HLNominalAttribute(constraintName, alt.getHLProcess());
    	alt.getHLProcess().addOrReplace(constraint);
    	/**
    	 * add attribute to the transitions
    	 * For the first transition, start point of the constraint, 
    	 * an output data attribute is added, 
    	 * for the second an input data attribute. The result is the
    	 * constraint that the first transition has to be placed before the second.
    	 */
    		HLActivity firstAct = alt.getHLProcess().findActivityByName(firstTransitionIdentifier);
    		firstAct.addOutputDataAttribute(constraint.getID());
    		HLActivity secAct = alt.getHLProcess().findActivityByName(secondTransitionIdentifier);
