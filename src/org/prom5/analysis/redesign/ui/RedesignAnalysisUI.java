package org.prom5.analysis.redesign.ui;
...
import org.prom5.analysis.petrinet.cpnexport.ColoredPetriNet;
import org.prom5.analysis.petrinet.cpnexport.CpnExport20;
import org.prom5.analysis.petrinet.cpnexport.CpnExportSettings;
import org.prom5.analysis.petrinet.cpnexport.CpnUtils;
import org.prom5.analysis.petrinet.cpnexport.ManagerConfiguration;
import org.prom5.analysis.redesign.RedesignFactory;
import org.prom5.analysis.redesign.ui.petri.PetriNetGraph;
import org.prom5.analysis.redesign.ui.petri.PetriNetTransition;
...
import org.prom5.framework.ui.ComponentFrame;
...
public class RedesignAnalysisUI extends JPanel implements Provider {
	...
	private static final long serialVersionUID = 1L;
		...
		/**
		 * Specifies the redesign type.
		 * The redesign type determines the specific transformation rule, e.g., parallel.
		 * @return enumeration of the redesign types
		 */
		public enum RedesignType {
			Parallel, Sequence, Group, AddTask, RemoveTask,
			AddConstraint, RemoveConstraint, CopyModel;
		
        /**
    	 * Specifies the various Key Performance Indicators.
    	 * The kpi type determines which KPI is used when simulating, e.g., throughput time.
    	 * @return enumeration of the kpis.
    	 */} ;
    	 public enum kpiType {
    		 ThroughputTime, WaitingTime, ResourceUtilization, InventoryCosts, 
    		 CustomerSatisfaction, LaborFlexibility;
             }
        ...
        public RedesignAnalysisUI(HLPetriNet net) {
         ...
         e.printStackTrace();
            ...
            /**
             * create the current simulation settings folder 
             * (CPN models look for these settings on a fixed location)
             */
         ...
         try {
          ...
          e.printStackTrace();
            ...
            /**
             * create first simulation model, i.e., the CPN model of the original HLPetriNet
             */
        ...
        /**
      * Specifies the UI of the plug-in.
      * The upper part of the UI contains the tree of alternatives models and
      * the simulation functionality.
      * The lower part of the UI contains the selected model and the redesign
      * possibilities.
      * Initially, no model is selected in the tree.
      */
        private void constructUI() {
          ...
          */
            ...
            /**
          * Create the upper part of the UI and the control panel for simulation buttons.
          */
          ...
          */
            ...
            /**
          * Add the tree view to the content pane.
          * This finishes the upper part of the UI.
          */
            this.treeView = upperView;
            contentPane.add(this.treeView, BorderLayout.NORTH);
            /**
          * Create the lower part of the UI.
          * Note that this is for the initial UI, i.e., no model is selected.
          */
        ...
        /**
      * When a model is selected in the upper part of the UI, the lower part
      * of the UI has to show the selected model.
      */
       ...
       //     updateButton(redesignButton, model, pnGraph);
            ...
            public void actionPerformed(ActionEvent event) {
              /**
               *  get the selected nodes and the selected redesign type.
               *  NB. If no nodes are selected, then getSelectedPetriNetNodes() returns
      ...
      analysisButton.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent event) {
                  ...
                  */
                    ...
                    /**
                     * perform some actions after editing, 
                     * more precisely, when the frame is closing
                     */
              ...
              /** 
               * save of case gen. scheme to file, i.e., overwrite the arrival rate file
               */
           ...
           try {
            ...
            CpnUtils.getCpnDistributionFunction(model.getHLProcess().getGlobalInfo().getCaseGenerationScheme()) + ";");
            dout.close();
      ...
      /**
        ...
        /**
         * Determine all components on which the selected redesign type
         * can be applied. If any transitions in the model are selected, these
         * transitions need to be included in the resulting components.
         * @param type RedesignType the selected redesign type
         * @param selectedTransitions List<Transition> the selected transitions
         * @param model HLPetriNet the input model for redesign
         * @return comps Set<Component> set of all components, if no transitions selected
         * @return selComps Set<Component> set of components including all selected transitions        
         */
        public Set<Component> getComponentsToDisplay(RedesignType type, 
         ...
         /**
          * if no Transitions are selected, selectedTransitions includes all Transitions.
          * All components in the model are returned
          */
         ...
         * components
         * @param comps Set<Component> the components to display
         * @param model HLPetriNet the input model for redesign
         * @param pnGraph PetriNetGraph the visualization of the PetriNet in model
         */
...
//        	showModel(model);
         ...
         /**
          * then, highlight nodes
          * nodes that are selected: green
          * nodes that are in a component enclosing all selected nodes: yellow
          * other nodes: red
          */
         if (!comps.isEmpty()) {
           ...
           if (comp.getTransitions().size() != 0) {
                 for (Transition node : comp.getTransitions()) {
                  ...
                  if (model.getPNModel().getTransitions().contains(node)) {
                   ...
                   /**
                    * node is in component, if selected then set to IsSelectedInComponent
                    */
                   if (t.isSelected()) {
                     t.setIsSelectedInComponent(true);
                   /**
                    * else, add to the set of yellow nodes, it is in a component, but not selected.
                    */
                   } else {
                ...
                t.setToBeSelectedInComponent(true);
                 ...
                 /**
                  * set remaining nodes, i.e., the nodes not in a component to NotToBeSelectedInComponent.
                  * This are the nodes that are not in allIncludedNodes
                  */
        ...
        /**
         * highlight the transitions in the model that are included in a 
         * component
         * @param type RedesignType the selected redesign type
         * @param model HLPetriNet the input model for redesign
         * @param pnGraph PetriNetGraph the visualization of the PetriNet in model
         ...
         * @param comp Component the given component
         * @param model HLPetriNet the input model for redesign
         * @param pnGraph PetriNetGraph the visualization of the PetriNet in model
         */
...
//     showModel(model);
         ...
         * @param button SlickerButton the involved button
         */
   ...
   //      showModel(model);
        ...
        /**
         * Determines the minimal component enclosing the selected nodes 
         * and checks if the selected nodes enclose all transitions in the component,
         * i.e., the component does not enclose more transitions than the selected ones.
         * @param indexRedesignType int Redesign type.
         * @param model HLPetriNet Model to be redesigned.
         * @param nodes List<PNNode> the nodes to redesign, can be all nodes.
         * @returns true Boolean returned if component encloses exactly the selected transitions. 
      */
      ...
      * @param model HLPetriNet the input model of the redesign
      * @param type RedesignType the selected transformation rule
      * @param comp Component the component including the nodes to be redesigned
      */
        private void performRedesign(HLPetriNet model, RedesignType type, Component comp) {
            try {
             /**
              *  create redesign model and add it to a (new) node in the alternatives tree
              */
                ...
                /**
                 *  give the redesign model a unique ID, which is the modelID created with the redesign node creation.
                 */
                ...
                /**
                 *  trigger selection of active model, i.e., the alternative model.
                 */
        ...
        /**
      * Specifies the removal of an alternative model from the tree.
      * Note that the original model can not be removed.
      */
        private void removeSelectedNode() {
         ...
         /**
          * if one model is selected, provide user with a message asking (s)he is sure
          */
         ...
         if (value == JOptionPane.OK_OPTION) {
          ...
          /**
           * but first, remove related CPN models from currentSimModels folder
           */
      ...
      */
        private void inOrExcludeNodeForSimulation() {
          ...
          */
          ...
          /**
           *  original model can not be excluded, has to remain on selectedForSimulation
           */
          if (redesignGraph.getRedesignNodesForSimulation().contains(nodeToSim) &&
      ...
      */
        private void performSimulation() {
         ...
         String dirDest = folderPref + "\\" + "experiment_" + experimentCounter;
         /**
          * create a file for the means of all nodes to calculate statistics
         ...
         /**
          * copy the folder with the current simulation settings
         ...
         try {
          ...
          e.printStackTrace();
            ...
            /**
             * then, copy current simulation setting files to the created simSettings folder
             */
            File oldSettingFolder = new File(locationForCurrentSimSettings);
            copyFiles(oldSettingFolder,newSettingFolder);
         
            /**
             * set boolean isSimulated to false for all models to distinguish last experiment from previous experiments
             */
...
//             */
//            ArrayList<RedesignNode> nodesToSimulate = new ArrayList<RedesignNode>();
//            /**
//             *  all models are simulated if no models or only the original model are selected for simulation.
//             */
...
//            }
//            /**
//             * copy the simulation model from the currentSimModels folder to the result folder
//             */
//            copyOneFile(locationForCurrentSimModels + "\\" + "redesign" + i + ".cpn", nodeDest + "\\" + "redesign" + i + ".cpn");
//         /**
//          * run simulations for the specified number of sub runs
//          */
//         SimSettingsUI ui = new SimSettingsUI(originalNet.getHLProcess().getGlobalInfo(), locationForCurrentSimSettings);
...
//          */
//         String locDest = locationForCurrentSimSettings + "\\" + "valFolder.sml"; 
//            FileWriter out = null;
//          try {
//           out = new FileWriter(locDest);
//          // an example: out.write("val FOLDER = \"C:/RedesignAnalysis/experiment_1/Original_0/sim_1\""); //
//           out.write("val FOLDER = \"" + folderPrefCPN + "/experiment_" + experimentCounter + "/" + node.label()[0] + "/sim_" + j + "\"");
//           out.close();
//           } catch (IOException ex) {
//            ex.printStackTrace();
//          }
...
//          */
//         Simulation sim = new Simulation(locationForCurrentSimModels + "\\" + "redesign" + i + ".cpn");
...
//         
//         String[] s = {node.label()[0], "tpt: mean = " + meanOut, "conf95 = (" + lowerOut + " ; " + upperOut + ")"};
//         node.setLabel(s);
...
//          */
//         for (RedesignNode simNode : redesignGraph.getSimulatedRedesignNodes()) {
...
//            });
      ...
      * @see CPN Export
      * @param net HLPetriNet the net from which a simulation model is created.
      */
      ...
      * 
      * @param oldFolder File the folder from which files have to be copied
      * @param newFolder File the folder to which files are copied
      */
      ...
      * 
      * @param oldPath String the old path of the file
      * @param newPath String the new path of the file
      */
        ...
        /**
      * Specifies the switch from the view on the lower part of the UI 
      * when no model is selected to the view on a selected model.
      */
        ...
        /** (non-Javadoc)
         * @see org.prom5.framework.plugin.Provider#getProvidedObjects()
         */
