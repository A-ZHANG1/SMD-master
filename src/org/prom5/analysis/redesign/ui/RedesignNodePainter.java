...
//    private static Color blue2 = new Color(0, 255, 255);
	 ...
	 * java.awt.Graphics2D, org.deckfour.gantzgraf.model.GGNode)
	 * specific drawing of the nodes: green filling (if simulated), faded, 
	 * thick and normal lining
	 */
	public void paint(GGDimension canvasDimension, Graphics2D g2d, GGNode node) {
		...
		/**
		 *  specific drawing of the nodes:
		 *  thick border width if the node is selected in the graph
		 * green filling if simulated otherwise no filling, 
		 * normal border color if selected for simulation otherwise faded border. 
		 */  
		
		RedesignNode redesignNode = (RedesignNode) node;
		
		/**
		 *  node is selected in the graph: thick border width
		 *  other drawing parameters are determined in the following if clauses
		 */
		if (redesignNode.isSelected()) {
		BasicStroke stroke = new BasicStroke(canvasDimension.getBorderWidth() * borderWidth * 3,
		...
		if (redesignNode.isSimulated() && redesignNode.getModelID() == 0) {
			...
			/**
			 *  original node is always selected for simulation: normal lining and text colors
			 */
			colorBorder = border;
			colorText = text;
		}
		 ...
		 */
		else if (redesignNode.isSimulated() && redesignNode.isBetterPerforming()) {
			/**
			 *  fill background node with green, intensity depends on performance
			 */
			setColorBackgroundForBetter(redesignNode);
			/**
			 *  node is selected for simulation: normal lining and text colors
			 */
			if (redesignNode.isSelectedForSimulation()) {
				colorBorder = border;
				colorText = text;
			/**
			 * node is not selected for simulation: faded lining and text
			 */
		 ...
		 */
		else if (redesignNode.isSimulated() && redesignNode.isEqualPerforming()) {
			/**
		 ...
		 */
		else if (redesignNode.isSimulated() && redesignNode.isWorsePerforming()) {
			/**
			 *  fill background node with red, intensity depends on performance
			 */
		...
		/**
		 * node is not selected in the graph: normal border width
		 * other parameters are defined in the following if clauses
		 */
		if (!redesignNode.isSelected()) {
		BasicStroke stroke = new BasicStroke(canvasDimension.getBorderWidth() * borderWidth,
				BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
		g2d.setStroke(stroke);
		 ...
		 * paint background
		 */
		GradientPaint gradient = new GradientPaint(x, y, colorBackground, x, y + height, colorBackground, false);
		 ...
		 * paint border
		 */
		gradient = new GradientPaint(x, y, colorBorder, x, y + height, colorBorder, false);
		g2d.setPaint(gradient);
		g2d.draw(shape);
	}
	 ...
	 * @see org.deckfour.gantzgraf.painter.GGNodeShapePainter#paint(org.deckfour.gantzgraf.layout.GGDimension, 
	 * java.awt.Graphics2D, org.deckfour.gantzgraf.model.GGNode)
	 * specific drawing of the nodes: no filling, dashed, thick and normal lining
	 */
	public void paint1(GGDimension canvasDimension, Graphics2D g2d, GGNode node) {
		...
		if (redesignNode.isSelectedForSimulation() && !redesignNode.isSelected()) {
			BasicStroke stroke = new BasicStroke(canvasDimension.getBorderWidth() * borderWidth,
					BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
			g2d.setStroke(stroke);
			...
			BasicStroke stroke = new BasicStroke(canvasDimension.getBorderWidth() * borderWidth * 3,
					BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 0, dash, 0);
			g2d.setStroke(stroke);
		...
		// paint background
		GradientPaint gradient = new GradientPaint(x, y, colorBgTop, x, y + height, colorBgBottom, false);
		g2d.setPaint(gradient);
		g2d.fill(shape);
		//paint text
		paintText(g2d, node.label(), x, y, width, height, fontSize, colorText);
		// paint border
		gradient = new GradientPaint(x, y, colorBorderTop, x, y + height, colorBorderBottom, false);
		g2d.setPaint(gradient);
		g2d.draw(shape);
	}
	 ...
	 * @see org.deckfour.gantzgraf.painter.GGNodeShapePainter#paint(org.deckfour.gantzgraf.layout.GGDimension, 
	 * java.awt.Graphics2D, org.deckfour.gantzgraf.model.GGNode)
	 * specific drawing of the nodes: white, blue or green filling, normal and thick lining
	 */
	public void paint2(GGDimension canvasDimension, Graphics2D g2d, GGNode node) {
		...
		// specific drawing of the nodes: white and blue filling, normal and thick lining
		RedesignNode redesignNode = (RedesignNode) node;
		...
		if (redesignNode.isSelectedForSimulation() && !redesignNode.isSelected()) {
			BasicStroke stroke = new BasicStroke(canvasDimension.getBorderWidth() * borderWidth,
					BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
			g2d.setStroke(stroke);
			GradientPaint gradient = new GradientPaint(x, y, blue, x, y + height, blue, false);
			g2d.setPaint(gradient);
			g2d.fill(shape);
			//node for simulation and selected: blue filling, thick normal lining
		} else if (redesignNode.isSelectedForSimulation() && node.isSelected()) {
			BasicStroke stroke = new BasicStroke(canvasDimension.getBorderWidth() * borderWidth * 3,
					BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
			g2d.setStroke(stroke);
			...
			g2d.setPaint(gradient);
			g2d.fill(shape);
			// node is selected, but not for simulation: no filling, thick lining
		} else if(!redesignNode.isSelectedForSimulation() && node.isSelected()) {
			BasicStroke stroke = new BasicStroke(canvasDimension.getBorderWidth() * borderWidth * 3,
					BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
			g2d.setStroke(stroke);
		...
		paintText(g2d, node.label(), x, y, width, height, fontSize, colorText);
		// paint border
		GradientPaint gradient = new GradientPaint(x, y, colorBorderTop, x, y + height, colorBorderBottom, false);
		g2d.setPaint(gradient);
		g2d.draw(shape);
	}
