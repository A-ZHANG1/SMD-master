...
import org.prom5.framework.util.GUIPropertyDoubleTextField;
...
/**
 * Creates a gui representation for the simulation settings of the 
 * CPN model.
 * <br>
...
public class SimSettingsUI implements GuiDisplayable, GuiNotificationTarget {
	...
	/** the global process information taken from the original model
	 * Maybe it is better to read the file with simulation settings 
	 * to always have the latest setting presented */
	...
	/**
	 * (non-Javadoc)
	 * @see org.prom5.framework.util.GuiDisplayable#getPanel()
	 */
	...
	/**
	 * Creates a panel with the option to set the simulation settings.
	 * The possible settings are the number of cases, the number of sub runs,
	 * and the case generation scheme.
	 */
	private void createPanel() {
		...
		String description = new String("Here you can specify the simulation settings of the processes. " +
		...
		numberOfCases = new GUIPropertyDoubleTextField("Number of cases per run", 
				null, cVal, 0.0, 1000.0);		
		...
		numberOfSubRuns = new GUIPropertyDoubleTextField("Number of sub runs per simulation", 
				null, rVal, 0.0, 100.0);
		...
		listEnumForTimeUnit = new GUIPropertyListEnumeration("Time unit:",
				...
				TimeUnit.getAllTypes(), new TimeUnitChangeListener());
	    /**
		...
		resultPanel.add(Box.createHorizontalGlue());
		outmostPanel.add(GuiUtilities.getSimpleScrollable(resultPanel, bgColor), BorderLayout.CENTER);
	...
	private void createCaseGenerationPanel() {
		if (caseGenerationPanel == null) {
			...
			caseGenerationPanel.setLayout(new BoxLayout(caseGenerationPanel, BoxLayout.PAGE_AXIS));
		...
		HLDistributionGui distGui = HLDistributionGuiManager.getDistributionGui(global.getCaseGenerationScheme(), this);
	 ...
	 * @param simDest String the location of the file
	 * @param d HLDistribution the case generation scheme
	 */
	public void createSettingFiles(String simDest, HLDistribution d) {
		/**
		 * simDest is the name of the folder to write to, this folder should have been created.
		 */
        /**
         * create the number of cases file
         * the number is written into a function that is used in simulation
         */
		...
		try {
         ...
         FileWriter lout = null;
     		try {
     			...
     			//Note that the following value will be overwritten before simulation.
	...
	/**
	 * write the settings entered in the UI to the files, 
	 * assumes files are already there.
	 */
	...
	/**
	 * reads the number of cases from file
	 * the number in file is read as a string and translated to a double.
	...
	/**
	 * reads the number of sub runs from file
	 * the number in file is read as a string and translated to a double.
	 ...
	 * @param fileName String the location to write to.
	 */
	public void copySettingsToOneFile(String fileName) {
		...
		try {
			...
			//out.write("The arrival rate: " + CpnUtils.getCpnDistributionFunction(readArrivalRateFromFile()) + "\n");
	...
	/**
	 * (non-Javadoc)
	 * @see org.prom5.framework.util.GuiNotificationTarget#updateGUI()
	 * Update after time unit change as this leads to a re-calculation of time-related properties
	 * (values need to be converted to match new time unit).
	 */
	public void updateGUI() {
		// update attributes that might have changed
		global.setTimeUnit((TimeUnit) listEnumForTimeUnit.getValue());
		
		// also update the gui as, for example, the distribution type might have changed
		createCaseGenerationPanel();
	...
	/**
	 * Invokes the conversion of all time-related values and issues a redraw of the 
	 * global GUI. 
	 */
	...
	/**
	 * Listener for time unit change as this leads to a re-calculation of time-related properties
	 * (values need to be converted to match new time unit).
	 */
