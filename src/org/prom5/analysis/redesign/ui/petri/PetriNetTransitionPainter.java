package org.prom5.analysis.redesign.ui.petri;
...
public class PetriNetTransitionPainter extends GGNodeRoundRectanglePainter {
	 ...
	 * specific drawing of the nodes: black if invisible task, thick if selected, 
	 * else normal lining
	 */
	public void paint(GGDimension canvasDimension, Graphics2D g2d, GGNode node) {
		...
		if (t.isSelectedInComponent() && !t.original().isInvisibleTask()){
			/**
			 *  transition is selected and included in a component: green background
			 */
			BasicStroke stroke = new BasicStroke(canvasDimension.getBorderWidth() * borderWidth * 1,
			...
			colorBorder = border;
			colorText = text;
		} else if (t.toBeSelectedInComponent() && !t.original().isInvisibleTask()){
			/**
			 *  transition is not yet selected and included in a component,
			 *  but it is possible to add it: yellow background
			 */
			BasicStroke stroke = new BasicStroke(canvasDimension.getBorderWidth() * borderWidth * 1,
					BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
			g2d.setStroke(stroke);
			...
			colorBorder = border;
			colorText = text;
		} else if (t.notToBeSelectedInComponent() && !t.original().isInvisibleTask()){
			/**
			...
			/**
			 *  transition is invisible: black node with white letters
			 *  transition is selected and included in a component: green border
			 */
			 ...
			 *  transition is invisible: black node with white letters
			 *  transition is not yet selected and included in a component,
			 *  but it is possible to add it: yellow border
			 */
			BasicStroke stroke = new BasicStroke(canvasDimension.getBorderWidth() * borderWidth * 3,
					BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
			g2d.setStroke(stroke);
			...
			/**
			 *  transition is invisible: black node with white letters
			 *  transition cannot be included in this component: red border
			 */
			BasicStroke stroke = new BasicStroke(canvasDimension.getBorderWidth() * borderWidth * 3,
					BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
			g2d.setStroke(stroke);
		...
		/**
		 *  transition not set with regard to component selection: basic layout
		 */
