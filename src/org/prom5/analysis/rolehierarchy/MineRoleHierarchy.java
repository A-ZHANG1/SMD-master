...
import org.prom5.mining.NoMiningSettings;
...
public class MineRoleHierarchy {
	...
	public static JPanel mine(LogReader inputLog) throws Exception {
		return new RoleHierarchyResult(inputLog, mineRoleHierarchy(inputLog, 0, 0));
	}
			
	public static RoleHierarchy mineRoleHierarchy(LogReader log, int absoluteThreshold, double relativeThreshold) throws Exception {
		...
		if (!ontologies.getOntologies().isEmpty()) { // skip reading the whole log if we know we're not going to find any annotations
				...
				for (int index = 0; index < ates.size(); index++) {
					...
					for (String uri : ate.getElementModelReferences()) {
						...
						if (concept != null) {
							...
							taskOntologies.put(taskOntology.getName(), taskOntology);
						}
					}
					...
					if (ate.getOriginator() != null && ate.getOriginator().length() > 0) {
						...
						if (!originator2task.containsKey(originatorName)) {
							...
							visibleTasks.put(originatorName, new HashSet<String>());
						}
						for (String uri : ate.getElementModelReferences()) {
							ConceptModel concept = ontologies.findConceptByUriInLog(uri);
							if (concept != null) {
								originator2task.get(originatorName).add(concept.getName());
								visibleTasks.get(originatorName).add(concept.getName());
								
								for (ConceptModel superConcept : concept.getOntology().getSuperConcepts(concept)) {
									originator2task.get(originatorName).add(superConcept.getName());
								}
							}
						}
					}
		...
		if (!taskOntologies.isEmpty()) {
			...
			if (taskOntologies.size() > 1) {
				OntologyModel[] possibleOntologies = taskOntologies.values().toArray(new OntologyModel[0]);
				selectedTaskOntology = (OntologyModel) JOptionPane.showInputDialog(
				                    MainUI.getInstance(),
				                    ...
				                    null,
				                    possibleOntologies,
				                    possibleOntologies[0]);
				if (selectedTaskOntology == null) {
					// user cancelled
					return null;
				}
			} else {
				selectedTaskOntology = taskOntologies.values().iterator().next();
			}
			...
			nodeLabelFormatter = new NodeLabelFormatter() {
				public String getLabel(Set<String> nodeNames, boolean useShortLabels) {
					List<String> conceptNames = Func.map(nodeNames, new Func.Fun1<String, String>() {
						public String apply(String uri) {
							return OntologyModel.getConceptPart(uri); 
						}
					});
					if (useShortLabels) {
						return Func.joinAtMost(RoleHierarchy.MIN_NUMBER_LABELS_PER_NODE, conceptNames, "", "_et_al");
					} else {
						return Func.joinAtMost(RoleHierarchy.MAX_NUMBER_LABELS_PER_NODE, conceptNames, "\\n", "... and ", " more ...");
					}
				}
			};
		} else {
			...
			// no task ontology found, so we use the 'default task ontology': simply a single node for every task
			// and we use the originator by task matrix to build the originator2task map
			
			taskGraph = new ModelGraph("Tasks");
			for (String taskName : log.getLogSummary().getModelElements()) {
				ModelGraphVertex node = taskGraph.addVertex(new ModelGraphVertex(taskGraph));
			...
			for (String originator : log.getLogSummary().getOriginators()) {
				Map<LogEvent, Integer> events = log.getLogSummary().getEventsForOriginator(originator);
					...
					for (Map.Entry<LogEvent, Integer> event : events.entrySet()) {
						...
						if (nodeName != null) {
							if (!originator2task.containsKey(originator)) {
								...
								visibleTasks.put(originator, new HashSet<String>());
							}
							originator2task.get(originator).add(nodeName, event.getValue());
		...
		RoleHierarchy hierarchy = new RoleHierarchy(taskGraph.getName(), visibleTasks, nodeLabelFormatter);
		
		mineNodeSet(taskGraph.getVerticeList(), originator2task, hierarchy, absoluteThreshold, relativeThreshold);
		return hierarchy;
	}

	private static void mineNodeSet(List<ModelGraphVertex> nodes, Map<String, TaskSet> originator2task, RoleHierarchy hierarchy,
			int absoluteThreshold, double relativeThreshold) {
		Map<String, TaskSet> localOTMatrix = new HashMap<String, TaskSet>();
		
		// Step 1: construct the originator by task matrix for the given nodes only
		for (Map.Entry<String, TaskSet> item : originator2task.entrySet()) {
			...
			for (ModelGraphVertex node : nodes) {
				if (tasks.contains(node.getIdentifier()) &&  satisfiesThresholds(tasks, node.getIdentifier(), absoluteThreshold, relativeThreshold)) {
					if (!localOTMatrix.containsKey(originator)) {
						localOTMatrix.put(originator, new TaskSet());
					}
					localOTMatrix.get(originator).add(node.getIdentifier(), tasks.getFrequency(node.getIdentifier()));
				}
			}
		}
	...
	private static boolean satisfiesThresholds(TaskSet tasks, String taskName, int absoluteThreshold, double relativeThreshold) {
		int freq = tasks.getFrequency(taskName);
		
		if (freq < absoluteThreshold) {
			return false;
		}
		if (relativeThreshold > 0 && freq < tasks.getTotalFrequency() * (relativeThreshold / 100.0)) {
			return false;
		}
		return true;
	}
