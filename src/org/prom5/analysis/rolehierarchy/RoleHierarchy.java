...
import org.prom5.framework.models.ModelGraphCluster;
...
import org.prom5.framework.util.Func;
...
public class RoleHierarchy {
	...
	public final static NodeLabelFormatter DEFAULT_FORMATTER = new NodeLabelFormatter() {
		public String getLabel(Set<String> nodeNames, boolean useShortLabels) {
			if (useShortLabels) {
				return Func.joinAtMost(MIN_NUMBER_LABELS_PER_NODE, nodeNames, "", "_et_al");
			} else {
	...
	public RoleHierarchy(String name, Map<String, Set<String>> visibleTasks, NodeLabelFormatter nodeLabelFormatter) {
		this.name = name;
	...
	public int getMaximumFrequencyForAnyOriginator() {
		...
		for (Map.Entry<String, TaskSet> item : originator2taskset.entrySet()) {
			for (Map.Entry<String, Integer> task : item.getValue().entrySet()) {
				if (visibleTasks.containsKey(item.getKey()) && visibleTasks.get(item.getKey()).contains(task.getKey())) {
	...
	/**
	 * Creates an ontology from this role hierarchy and returns it. This method will return null if the ontology could not be created.
	 * @param useTasks True to use tasks as concept names, false to use originators as concept names
	...
	public OntologyModel toOntology() {
		try {
			...
			OntologyModel ontology = new OntologyModel(toModelGraph(false, false, true), 
					"http://www.processmining.org/ontologies/RoleHierarchy-" + counter ++, 
					"RoleHierarchy");
			for (Set<String> originatorSet : taskset2originatorset.values()) {
				String conceptName = DEFAULT_FORMATTER.getLabel(originatorSet, true);
	...
	private ModelGraph toModelGraph(boolean annotate, boolean showArtificalRoot, boolean useShortLabels) {
		...
		// create a node for every set of tasks, and annotate that node with the set of originators which perform that set of tasks
		for (Map.Entry<Set<String>, Set<String>> item : taskset2originatorset.entrySet()) {
			ModelGraphVertex node;
			Set<String> tasks = item.getKey();
			Set<String> originators = item.getValue();
			
			node = createNode(result, tasks, originators, "box", "cadetblue1", originatorFormatter, false, useShortLabels);
			nodes.put(tasks, node);
			annotations.put(tasks, new HashSet<String>(tasks));			
		}
		
		// create an edge from node A to node B if the task set of A is a subset of the task set of B
		for (Map.Entry<Set<String>, ModelGraphVertex> a : nodes.entrySet()) {
			...
			for (Map.Entry<Set<String>, ModelGraphVertex> b : nodes.entrySet()) {
				...
				if (tasksB.size() > tasksA.size() && tasksB.containsAll(tasksA)) {
					...
					for (Map.Entry<Set<String>, ModelGraphVertex> c : nodes.entrySet()) {
						Set<String> tasksC = c.getKey();
						
						if (tasksB.size() > tasksC.size() && tasksB.containsAll(tasksC) &&
								tasksC.size() > tasksA.size() && tasksC.containsAll(tasksA)) {
							// we've found a node C for which tasksA < tasksC < tasksB, so A is not a largest possible subset of B
							isLargestSubset = false;
							break;
						}
					}
					if (isLargestSubset) {
						ModelGraphEdge edge = result.addEdge(a.getValue(), b.getValue());
		...
		if (showArtificalRoot) {
			...
			if (rootNodes.size() > 1) {
				...
				ModelGraphVertex root = result.addVertex(new ModelGraphVertex(result));
		...
		if (annotate) {
			...
			for (Set<String> tasks : taskset2originatorset.keySet()) {
				if (!annotations.get(tasks).isEmpty()) {
					...
					annotation = createNode(result, tasks, annotations.get(tasks), "ellipse", "burlywood1", nodeLabelFormatter, false, useShortLabels);
	
					ModelGraphEdge edge = result.addEdge(node, annotation);
					edge.setDotAttribute("color", "gray");
					edge.setDotAttribute("constraint", "false");
					
					ModelGraphCluster cluster = new ModelGraphCluster("c" + id++);
	...
	private ModelGraphVertex createNode(ModelGraph graph, Set<String> tasks, Set<String> originators, String shape, String color, 
			NodeLabelFormatter formatter, boolean showTasks, boolean useShortLabels) {
		...
		if (showTasks) {
			...
			for (String originator : originators) {
				for (String task : tasks) {
					if (visibleTasks.containsKey(originator) && visibleTasks.get(originator).contains(task)) {
						labels.add(task);
					}
				}
			}
		...
		node.setDotAttribute("label", formatter.getLabel(labels, useShortLabels));
	...
	public TableModel getLocalOTMatrix(Set<String> taskSet) {
		return new LocalOTMatrix(originator2taskset, taskset2originatorset.get(taskSet), visibleTasks, nodeLabelFormatter);
	}

	public TableModel getFullOTMatrix() {
		return new LocalOTMatrix(originator2taskset, originator2taskset.keySet(), visibleTasks, nodeLabelFormatter);
	}
}
...
class LocalOTMatrix extends AbstractTableModel {
	...
	public LocalOTMatrix(Map<String, TaskSet> originator2taskset, Set<String> originatorSet, Map<String, Set<String>> visibleTasks, NodeLabelFormatter nodeLabelFormatter) {
		Set<String> taskSet = new HashSet<String>();
		
		for (Map.Entry<String, TaskSet> item : originator2taskset.entrySet()) {
			if (originatorSet.contains(item.getKey())) {
				for (String task : item.getValue().getTasks()) {
					if (visibleTasks.containsKey(item.getKey()) && visibleTasks.get(item.getKey()).contains(task)) {
						taskSet.add(task);
					}
				}
			}
		...
		this.table = DoubleFactory2D.sparse.make(origMapping.size(), taskMapping.size(), 0);
		this.nodeLabelFormatter = nodeLabelFormatter;
		...
		for (Map.Entry<String, TaskSet> item : originator2taskset.entrySet()) {
			...
			if (origIndex != null) {
				for (Map.Entry<String, Integer> taskWithFreq : item.getValue().entrySet()) {
					Integer taskIndex = taskMapping.get(taskWithFreq.getKey());
					
					if (taskIndex != null) {
						this.table.set(origIndex, taskIndex, taskWithFreq.getValue());
					}
				}
			}
		}
