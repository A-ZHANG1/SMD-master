...
public class RoleHierarchyResult extends JPanel implements Provider, GuiNotificationTarget {
	...
	public RoleHierarchyResult(LogReader log, RoleHierarchy hierarchy) {
		...
		this.maxAbsoluteFrequency = hierarchy.getMaximumFrequencyForAnyOriginator();
		initGUI();
	}
	...
	private void initGUI() {
		...
		showTasks.addActionListener(new ActionListener() { public void actionPerformed(ActionEvent e) { updateGraph(); } });
		
		absoluteThreshold = new GUIPropertyInteger("", 0, 0, maxAbsoluteFrequency, this);
		relativeThreshold = new GUIPropertyDouble("", 0, 0, 100, 1, this);
		
		JPanel thresholdsPanel = new JPanel(new GridBagLayout());
		thresholdsPanel.add(new JLabel("Absolute (0-" + maxAbsoluteFrequency + ")"), new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0,
				GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		thresholdsPanel.add(absoluteThreshold.getPropertyPanel(), new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0,
				GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
		thresholdsPanel.add(new JLabel("Relative (0-100%)"), new GridBagConstraints(0, 2, 1, 1, 0.0, 0.0,
				GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		thresholdsPanel.add(relativeThreshold.getPropertyPanel(), new GridBagConstraints(0, 3, 1, 1, 0.0, 0.0,
				GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
		thresholdsPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), "Thresholds"));
		
		JPanel settings = new JPanel(new GridBagLayout());
		settings.add(showTasks, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0,
				GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		settings.add(thresholdsPanel, new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0,
		...
		JSplitPane contentPanel = new JSplitPane(JSplitPane.VERTICAL_SPLIT, settingsAndGraphPanel, tablePanel);
	...
	protected void updateThresholds() {
		...
		if (0 <= absolute && 0.0 <= relative && relative <= 100.0) {
			try {
				hierarchy = MineRoleHierarchy.mineRoleHierarchy(log, absolute, relative);
			} catch (Exception e) {
				e.printStackTrace();
			...
			JOptionPane.showMessageDialog(MainUI.getInstance(), 
					...
					"relative threshold should be a number between 0 and 100 (inclusive).", "Invalid threshold value", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	protected void updateGraph() {
		...
		ModelGraphPanel graph = hierarchy.toModelGraph(annotate).getGrappaVisualization();
	...
	void updateOTMatrix(Set<String> taskSet) {
		...
		if (otMatrix.getColumnCount() > 0) {
			...
			for (int i = 1; i < otMatrix.getColumnCount(); i++) {
				otMatrix.getColumnModel().getColumn(i).setCellRenderer(otCellRenderer);
			}
		}
	...
	public ProvidedObject[] getProvidedObjects() {
		return new ProvidedObject[] {
				...
				new ProvidedObject("Role hierarchy Graph", hierarchy.toModelGraph(showTasks.isSelected())),
				new ProvidedObject("Originator by task matrix", otMatrix.getModel()),
	...
	public void grappaClicked(Subgraph subg, Element elem, GrappaPoint pt, int modifiers, int clickCount, GrappaPanel panel) {
		...
		if ((modifiers & InputEvent.BUTTON1_MASK) > 0 && clickCount == 1) {
			if (elem != null &&
				...
				result.updateOTMatrix((Set<String>) ((ModelGraphVertex) elem.object).object2);
			} else {
	...
	public void setModel(TableModel model) {
		...
		for (int row = 0; row < model.getRowCount(); row++) {
			for (int col = 1; col < model.getColumnCount(); col++) {
				maxValue = Math.max(maxValue, (Integer) model.getValueAt(row, col));
			}
		}
	}
