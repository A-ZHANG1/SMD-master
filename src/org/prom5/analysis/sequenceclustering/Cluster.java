...
public class Cluster {
	...
	public Cluster(int index, int numberEvents, LogReader log, LogReader originalLog, SCAlgorithm sc, double minEventSupport, double maxEventSupport, boolean preprocessed) {
	...
	public void buildCluster() {
		...
		for(int i=0; i<originalLog.getLogSummary().getLogEvents().size(); i++) {
			eventOccurence = round(((((double) (originalLog.getLogSummary().getLogEvents().get(i).getOccurrenceCount())) /
					(originalLog.getLogSummary().getNumberOfAuditTrailEntries())) * 100), 3);
	...
	public void buildInstancesTypes() {
		...
		if(this.getLog().getInstances().size() > 0) {
			...
			for(int i=0; i<this.getLog().getInstances().size(); i++){
				for(int j=0; j<this.getLog().getInstances().get(i).getAuditTrailEntryList().size(); j++) {
					try {
						sequences.set(i, sequences.get(i).concat(this.getLog().getInstances().get(i).getAuditTrailEntryList().get(j).getElement()));
			...
			for(int i=0; i<this.getLog().getInstances().size(); i++) {
				for(int j=0; j<instancesTypesString.size(); j++) {
					if(sequences.get(i).equals(instancesTypesString.get(j))) {
						...
						typesInstancesList.set(j, typesInstancesList.get(j)+ ", " + this.getLog().getInstances().get(i).getName());
					}
				}	
				if(!exists) {
					...
					typesInstancesList.add("" + this.getLog().getInstances().get(i).getName());
				}
				exists = false;
			}
			...
			for(int i=0; i<instancesTypesString.size(); i++) {
				if(typesOccurrences.get(i) > 1)
					instancesTypesName.set(i, typesOccurrences.get(i) + " Instances: " + typesInstancesList.get(i));
				else
					instancesTypesName.set(i, typesOccurrences.get(i) + " Instance: " + typesInstancesList.get(i));
			}
	...
	/**
	 * Recalculates the matrix that represents the Markov chain, according to the thresholds
	 * @param nodeSignificance
	 * @param edgeSignificance
	 * @param nodeInvertedSignificance
	 * @param edgeInvertedSignificance
	 */
	public void recalculateMatrix(double nodeSignificance, double edgeSignificance, double nodeInvertedSignificance, double edgeInvertedSignificance) {
		int iteration = 0;
		
		//remove edges below minimum threshold or above maximum threshold
		for(int i=0; i<numLogEvents+2; i++) {
			for(int j=0; j<numLogEvents+2; j++) {
				if(auxMatrix[i][j] < edgeSignificance || auxMatrix[i][j] > edgeInvertedSignificance) {
					auxMatrix[i][j] = 0.0;
		...
		while(auxMatrixChanged() || iteration == 0) {
			...
			for(int i=1; i<numLogEvents+1; i++) {
				if ( (!isReachable(i)) || 
						((log.getLogSummary().getInstancesForEvent(log.getLogSummary().getLogEvents().get(i-1)).size() * 1.0 / log.numberOfInstances()) < nodeSignificance) || 
						((log.getLogSummary().getInstancesForEvent(log.getLogSummary().getLogEvents().get(i-1)).size() * 1.0 / log.numberOfInstances()) > nodeInvertedSignificance)) {
					removeNode(i);
				}
			}
	 ...
	 * @param edgeInvertedSignificance
	 */
	public void writeToDot(Writer bw, double nodeSignificance, double edgeSignificance, double nodeInvertedSignificance, double edgeInvertedSignificance) {
		
		clusterModel = new ClusterModel(bw, ID, scAlg, nodeSignificance, edgeSignificance, nodeInvertedSignificance, edgeInvertedSignificance);
		
	}
	
	public ProvidedObject getProvidedObject(){
		return new ProvidedObject(name,
				new Object[] {
				getLog()
		});
	}
