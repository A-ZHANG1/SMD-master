...
public class ClusterModel implements DotFileWriter {
	...
	public ClusterModel(Writer bw, int index, SCAlgorithm sc, double nodeSignificance, double edgeSignificance, double nodeInvertedSignificance, double edgeInvertedSignificance) {
		this.index = index;
		this.scAlg = sc;
	...
	public void writeToDot(Writer bw) throws IOException {
		...
		if(cluster.getLog().getInstances().size() > 0) {
			...
			bw.write("digraph G {rankdir=LR; " + "\"" + "in" + "\"" + "[shape=doublecircle]; " + "\"" + "out" + "\"" + "[shape=doublecircle];");
			
			cluster.recalculateMatrix(nodeSignificance, edgeSignificance, nodeInvertedSignificance, edgeInvertedSignificance);
			
			for(int i=1; i<cluster.numLogEvents+1; i++) {
				if(cluster.isReachable(i)) {
					grayNodeValue = (int) (100 - ((cluster.log.getLogSummary().getInstancesForEvent(cluster.log.getLogSummary().getLogEvents().get(i-1)).size() * 1.0 / cluster.log.numberOfInstances()) * 100));
					...
					bw.write("\"" + cluster.log.getLogSummary().getLogEvents().get(i-1).getModelElementName() + "\"" + "[shape=ellipse,fontcolor=" + fontColor + ",style=filled,fillcolor=gray" + grayNodeValue + "];\n");
				}
			}
			...
			for(int i=0; i<cluster.numLogEvents+1; i++) {
				for(int j=1; j< cluster.numLogEvents+2; j++) {
					...
					if(cell != 0) {
						if(i == 0) {
							bw.write("\"" + "in" + "\"" + " -> ");
							bw.write("\"" + cluster.log.getLogSummary().getLogEvents().get(j-1).getModelElementName() + "\"\n");
							draw = true;
						}
						else if (i != 0 && j == cluster.numLogEvents+1) {
							bw.write("\"" + cluster.log.getLogSummary().getLogEvents().get(i-1).getModelElementName() + "\"");
							bw.write(" -> " + "\"" + "out" + "\"" + "\n");
							draw = true;
						}
						else if(i != 0 && j != cluster.numLogEvents+1){
						...
						if(cell > 0.05)
							...
							grayEdgeValue = 95;
						if(draw) {
							bw.write("[label=" + "\"" + cell + "\"" + "," + "color=gray" + grayEdgeValue + "];\n");
						}
					}
				}
			}
