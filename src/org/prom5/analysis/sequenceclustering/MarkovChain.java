...
public class MarkovChain extends JPanel {
	...
	protected JLabel edgeLabel, edgeInvertedLabel, nodeLabel, nodeInvertedLabel;
	protected JSlider edgeSlider, edgeInvertedSlider, nodeSlider, nodeInvertedSlider;
	...
	public MarkovChain(LogReader log, int numberClusters, SCAlgorithm scAlg) {
		...
		clustersScrollPane.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
		...
		lowerNodePanel.setLayout(new BoxLayout(lowerNodePanel, BoxLayout.X_AXIS));
		...
		lowerEdgePanel.setLayout(new BoxLayout(lowerEdgePanel, BoxLayout.X_AXIS));
	...
	public void updateUI(int index) {
		try {
			graphScrollPane.getViewport().setView(getGraphPanel(index, nodeSignificance, edgeSignificance, nodeInvertedSignificance, edgeInvertedSignificance));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

	public JPanel getGraphPanel(int index, double nodeSignificance, double edgeSignificance, double nodeInvertedSignificance, double edgeInvertedSignificance) throws Exception {
		File dotFile = File.createTempFile("pmt", ".dot");
		BufferedWriter bw = new BufferedWriter(new FileWriter(dotFile, false));
		scAlgorithm.clusterList.get(index).writeToDot(bw, nodeSignificance, edgeSignificance, nodeInvertedSignificance, edgeInvertedSignificance);
		bw.close();
	...
	protected void updateView() {
			...
			for(int i=0; i<clusterNames.length; i++) {
				clusterNames[i] = scAlgorithm.clusterList.get(i).getName() + "  (" + scAlgorithm.clusterList.get(i).instancesToKeep.size() + " Instances)";
