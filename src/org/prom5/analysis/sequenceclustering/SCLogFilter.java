...
public class SCLogFilter extends LogFilter {
				...
				if(ateList.size() == 0) {
					...
					if(flag == 0) {
						...
						for(int i=0; i<ateList.size(); i++) {
							...
							currentEvent = new LogEvent(currentAte.getElement(), currentAte.getType());
							...
							if(!(currentAte.getType().equals("complete"))) {
								...
								if(i < 1)
									...
									i = i - 2;
							} else if((i<ateList.size()-1 && currentAte.getElement().equals(ateList.get(i+1).getElement())) &&
									(i<ateList.size()-1 && currentAte.getType().equals(ateList.get(i+1).getType())))
							{
								ateList.remove(i+1);
								i--;
							}
						...
						if (ateList.size() < minSequenceSize || ateList.size() > maxSequenceSize) {
							return false;
						}
	...
	protected void writeSpecificXML(BufferedWriter output) throws IOException {
		...
		for(LogEvent event : this.filterEvents) {
			output.write("<filterEvent element=\"" + event.getModelElementName() +
		...
		for(int i=0; i<logFilterSpecificNode.getChildNodes().getLength(); i++) {
			...
			if(node.getNodeName().equalsIgnoreCase("removeType") == true) {
				...
				if(element != null && type != null) {
					LogEvent event = new LogEvent(element.getNodeValue().trim(), type.getNodeValue().trim());
