package org.prom5.analysis.sessionanalysis;
...
import org.prom5.analysis.sessionanalysis.SessionOptions.DisplayType;
...
public class SessionAnalysisResult extends JPanel implements Provider, GuiNotificationTarget {
	...
	protected GUIPropertyListEnumeration timeUnitProperty = new GUIPropertyListEnumeration(
			"Time unit: ", "Determine time unit to be used for analysis", Arrays.asList(TimeUnit.values()), null, 200);
	protected GUIPropertyDoubleTextField timeoutProperty = new GUIPropertyDoubleTextField(
			"Timeout (to split sessions): ", "Time out to be applied to split up sessions (interpreted in the given time unit)", 1, null, 200);

	/**
	 * Default constructor.
	 * @param log the log to be analyzed for sessions
	 * @param algorithm the SessionAnalysisPlugin (for displaying documentation)
	 */
	...
	protected JPanel constructConfigurationPanel() {	
		...
		confPanel.add(GuiUtilities.packCenterHorizontally(GuiUtilities.packMiddleVertically(GuiUtilities.packCenterHorizontally(optPanel))));
	...
	protected void splitSessions() {
		try {			
			...
			File outputFile = File.createTempFile("SessionAnalysisTemp", ".mxml.gz");
				...
				for (int j=0; j<process.size(); j++) {
					...
					for (int k=0; k<ateList.size(); k++) {
						...
						if (currentTimeStamp == null) {
							...
							persistency.startProcessInstance("Session" + sessionCounter, "Session split up from intance " + name, 
									sessionAtt);
							
						} else if (currentTimeStamp.getTime() - previousTimeStamp.getTime() > (timeoutInMilliseconds)) {
			...
			sessionLog = BufferedLogReader.createInstance(new DefaultLogFilter(DefaultLogFilter.INCLUDE), logFile);
		...
		if (sessionLog != null) {
			try {
				...
				for (int i=0; i<sessionLog.numberOfProcesses(); i++) {
					...
					SummaryStatistics sessionTimes = SummaryStatistics.newInstance();
					for (int j=0; j<process.size(); j++) {
						instance = process.getInstance(j);
						ateList = instance.getAuditTrailEntryList();
						firstAte = ateList.get(0).getTimestamp();
						lastAte = ateList.get(ateList.size() - 1).getTimestamp();
						double sessionTime = (double) (lastAte.getTime() - firstAte.getTime());
						double conversionValue = 1.0;
						if (timeUnit != TimeUnit.MILLISECONDS) {
							conversionValue = timeUnit.getConversionValue() * 1000;
						}
						sessionTimes.addValue(sessionTime/conversionValue); // record in seconds
					}
	...
	/**
	 * Creates a graphical representation of the currently selected result type and 
	 * returns the panel containing this graphical representation. 
	 * @return the graph panel
	 */
	protected JPanel visualizeSessions() {
		DisplayType type = (DisplayType) myDisplayOptions.myDisplayProperty.getValue();
		TimeUnit timeUnit = (TimeUnit) timeUnitProperty.getValue();
		...
		for (int i = 0; i < keys.length; i++) {
			// get statistics 
			SummaryStatistics statistic = (SummaryStatistics) sessionStatistics.get(keys[i]);
	...
	protected JPanel constructResultPanel() {
				...
				if(saveDialog.showSaveDialog(MainUI.getInstance())==JFileChooser.APPROVE_OPTION) {
					...
					try {
						...
						JOptionPane.showMessageDialog(MainUI.getInstance(), 
								"Session statistics have been saved\nto CSV file!", "Session statistics saved.", 
		...
		resultPanel.add(GuiUtilities.packCenterHorizontally(myDisplayOptions.getPanel()));
	...
	/**
	 * Creates a CSV representation of the Session metrics (to be opened in MSExcel). 
	...
	protected String getCsvRepresentation() {
		...
		for (int i = 0; i < keys.length; i++) {
			...
			for (int j=0; j<allTypes.length; j++) {
				double duration = SessionOptions.getStatisticValue(statistic, allTypes[j]);
				sb.append(duration);
