...
import org.prom5.analysis.ltlchecker.Substitutes;
...
public class SemanticParamTable extends ParamTable {
	...
	public Substitutes getSubstitutes(LTLParser parser) {
		for (int i = 0; i < settings.size(); i++) {
			if (settings.get(i).isASet()) {
				...
				if (value == null) {
					JOptionPane.showMessageDialog(MainUI.getInstance(), "Please enter a value for parameter " + settings.get(i).getParameterName() + ".");
					return null;
				}
	...
	private void addPanel(JPanel panel, int index) {
					...
					if (ref.length() > 0) {
						if (ref.startsWith("@")) {
							if (!foundFirstModelRef) {
								initialOntology = OntologyModel.getOntologyPart(ref.substring(1));
								initialConcept = OntologyModel.getConceptPart(ref.substring(1));
								value = "";
								foundFirstModelRef = true;
							} else if (!foundSecondModelRef && "@include-sub-concepts".equals(ref)) {
								initialIncludeSub = true;
							} else if (!foundSecondModelRef && "@include-super-concepts".equals(ref)) {
		...
		JRadioButton instanceButton = new JRadioButton("Instance:", !valueIsConcept);
	    ...
	    if (isSet) {
	    	...
	    	for (OntologyModel ontology : log.getOntologies()) {
	    		...
	    		if (OntologyModel.getOntologyPart(ontology.getName()).equals(initialOntology)) {
	    			ontologyIndex = i;
	    		}
	    		i++;
	    	}
	    	...
	    	panel.add(new JLabel("Concept:"), defaultConstraints(4, OFFSET + 1));
	    	...
	    	final JCheckBox superConcepts = new JCheckBox("Include super concepts", initialIncludeSuper);
	    	superConcepts.setEnabled(valueIsConcept);
	    	panel.add(superConcepts, defaultConstraints(6, OFFSET + 1));

	    	final JCheckBox subConcepts = new JCheckBox("Include sub concepts", initialIncludeSub);
	    	subConcepts.setEnabled(valueIsConcept);
	    	panel.add(subConcepts, defaultConstraints(6, OFFSET + 2));

	    	JRadioButton ontologyButton = new JRadioButton("Ontology:", valueIsConcept);
	    	...
	    	settings.add(new Setting(index, name, instanceText, ontologies, concepts, instanceButton, superConcepts, subConcepts, log));
	    } else {
		...
		panel.add(line, new GridBagConstraints(0, OFFSET + NUMROWS - 1, 7, 1, 1.0, 1.0, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(5,5,5,5), 0, 0));

	}

	private void updateConcepts(JComboBox ontologies, JComboBox concepts, String selectedConceptName) {
	...
	private GridBagConstraints defaultConstraints(int x, int y, int spanX, int spanY) {
		return new GridBagConstraints(x, y, spanX, spanY, 1.0, 1.0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0,0,0,0), 0, 0);
	}
}
...
class Setting {
	...
	public Setting(int index, String name, JTextField instanceText, JComboBox ontologies, JComboBox concepts, JRadioButton instanceButton,
			JCheckBox superConcepts, JCheckBox subConcepts, OntologyCollection ontologyCollection) {
		this.index = index;
		this.name = name;
	...
	public SetValueNode getValue(ParamData data) {
		...
		} else if (ontologies.getSelectedIndex() >= 0 && concepts.getSelectedIndex() >= 0){
			ConceptModel selectedConcept = ((ConceptInCombo) concepts.getSelectedItem()).get();
			List<String> concepts = new ArrayList<String>();
			SetValueNode setval = new SetValueNode(SetValueNode.MODEL_REFERENCE_SET, ontologyCollection);
			
			concepts.add(selectedConcept.getName());
			if (superConcepts.isSelected()) { concepts.add("include-super-concepts"); }
			if (subConcepts.isSelected()) { concepts.add("include-sub-concepts"); }
...
//	private List<ConceptModel> getConcepts(OntologyModel ontology, ConceptModel concept, boolean includeSuper, boolean includeSub) {
...
//	}
//	
//	private List<String> translateConceptsToURIsInLog(OntologyModel ontology, List<ConceptModel> concepts) {
...
//	}
//	
//	private List<String> translateConceptsToURIsInOntology(OntologyModel ontology, List<ConceptModel> concepts) {
//		List<String> result = new ArrayList<String>();
//
//		for (ConceptModel c : concepts) {
