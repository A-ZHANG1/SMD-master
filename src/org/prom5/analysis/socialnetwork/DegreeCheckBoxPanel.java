...
public class DegreeCheckBoxPanel extends JPanel implements ItemListener {
	...
	public DegreeCheckBoxPanel(SocialNetworkAnalysisUI parentClass, boolean[] selectedDegreeOptions) {
		...
		for (int i = 0; i < degreeCentralityOptionsCheckBoxes.length; i++) {
			degreeCentralityOptionsCheckBoxes[i] = new JCheckBox(SocialNetworkAnalysisUI.DEGREE_CENTRALITY_OPTIONS[i]);
			degreeCentralityOptionsCheckBoxes[i].addItemListener(this);
			degreeCentralityOptionsCheckBoxes[i].setSelected(this.selectedDegreeOptions[i]);
		}
	...
	public void itemStateChanged(ItemEvent e) {
		...
		if (e.getStateChange() == ItemEvent.DESELECTED) {
			...
			if(index==3) degreeCentralityOptionsCheckBoxes[2].setEnabled(true);
		} else {
			selectedDegreeOptions[index]=true;
			if(index==3) degreeCentralityOptionsCheckBoxes[2].setEnabled(false);
		}
