...
import org.prom5.framework.log.rfb.LogSummaryFormatter;
...
public class PersonalityData extends LogSummaryFormatter {
	...
	private HashMap<String, Integer> processes; // NROfProcessInstances, firstPInstance 
	...
	public PersonalityData(LogReader log0, LogReader log1, LogReader log2, LogReader log3, OrgModel model) {
		super();
	...
	public Date getStartTimeFromData() {
		...
		for (int j = 0; j < log.length; j++){
			for (int i = 0; i < log[j].numberOfProcesses(); i++) {
				Date timeStart = this.summary[j].getStartTime(log[j].getProcess(i)
						.getName());
				if (timeStart != null) {
					if (startTime != null) {
	...
	public Date getEndTimeFromData() {
		...
		for (int j = 0; j < log.length; j++){
			for (int i = 0; i < log[j].numberOfProcesses(); i++) {
				Date timeEnd = this.summary[j].getEndTime(log[j].getProcess(i).getName());
				if (timeEnd != null) {
					if (endTime != null) {
	...
	public LogReader getProcess(String processName) {
		if (processes.get(processName) != null)
			...
			return null;
		/*
		if (isProcessAvailable(processName)) {
			if (!filteredProcess.containsKey(processName)) {
				DefaultLogFilter filter = new DefaultLogFilter(
						DefaultLogFilter.DISCARD);
				filter.setProcess(processName);
				String[] list = summary.getEventTypes();
				for (int i = 0; i < list.length; i++) {
					filter.filterEventType(list[i], DefaultLogFilter.DISCARD);
				}
				try {
					// TODO fix this
					LogReader filteredLog = LogReaderFactory.createInstance(filter, log);
					// fL.data = null heeft iets met asynchroon te maken.
					filteredProcess.put(processName, filteredLog);
					return filteredLog;
				} catch (Exception e) {
					// TODO Auto-generated catch block
					System.out.println("Unable to filter log");
					e.printStackTrace();
					return null;
				}
			} else {
				return filteredProcess.get(processName);
			}
		} else {
			return null;
		} //*/
