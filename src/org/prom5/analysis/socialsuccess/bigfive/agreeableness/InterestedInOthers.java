...
public class InterestedInOthers extends Behaviour {
	...
	public void analyseAllUsers(Date startTime, Date endTime) {
		...
		if (trait.getData().isProcessAvailable(PersonalityData.USER_ACTION)) {
			...
			while (instance.hasNext()) {
				...
				while (audits.hasNext()) {
					...
					if (startTime.before(au.getTimestamp())
						...
						int profileID = getIntFromUID(au.getDataAttributes().get("userID"));
						int userID = getIntFromUID(uid);
						if (userID != profileID) {
							if (viewedProfilesPerUser.containsKey(uid)) {
								viewedProfilesPerUser.put(uid, viewedProfilesPerUser.get(uid) + 1);
							} else {
			...
			Iterator<String> it = viewedProfilesPerUser.keySet().iterator();
			while (it.hasNext()){
				String user = it.next();
				scorePerUser.put(user, Math.log1p(viewedProfilesPerUser.get(user)));				
			}*/
	...
	public int getIntFromUID(String uid){
		if (uid.contains("-")){
			...
			return Integer.parseInt(uid.subSequence(0, uid.indexOf("-")).toString());
