package org.prom5.analysis.socialsuccess.bigfive.agreeableness;
				...
				if (beginTime.before(au.getTimestamp())
					...
					if (au.getName().equalsIgnoreCase("VOTE")
						...
						if (au.getDataAttributes().containsKey("Actor")
							...
							String uid = au.getDataAttributes().get("Actor");
							Integer vote = Integer.parseInt(au.getDataAttributes().get("Vote"));
							// die niet de auteur van de node is
							    // && vote is > 50%
							if (!uid.equals(nodeCreator) && vote.compareTo(50) == 1) {
...
/*
	@SuppressWarnings("unchecked")
	private void analayseAllUsersExt(LogReader log, Date beginTime, Date endTime) {
		Iterator<ProcessInstance> instance = log.instanceIterator();
		while (instance.hasNext()) {
			ProcessInstance p = instance.next();
			String nodeCreator = p.getAttributes().get("CreatedBy");
			...
			while (audits.hasNext()) {
				...
				if (beginTime.before(au.getTimestamp())
					...
					if (au.getName().equalsIgnoreCase("CREATE")
							&& au.getType().equalsIgnoreCase("complete")) {
						// die door iemand gedaan zijn
						if (au.getDataAttributes().containsKey("GenericType")
								&& au.getDataAttributes().get("GenericType").equals("comments")) {
							String uid = au.getOriginator();
							if (repliesPerUser.containsKey(uid)) {
								Double r = repliesPerUser.remove(uid);
								repliesPerUser.put(uid, r + 1);
							} else {
								repliesPerUser.put(uid, 1.);
							}
						}
					}
					// Tel het aantal stemmen
					if (au.getName().equalsIgnoreCase("VOTE")
							&& au.getType().equalsIgnoreCase("complete")) {
						// die door iemand zijn gedaan
						if (au.getDataAttributes().containsKey("Node_By")) {
							String uid = au.getDataAttributes().get("Node_By");
							Integer vote = Integer.parseInt(au.getDataAttributes().get("Vote"));
							// die niet de auteur van de node is
						    // && vote is > 50%
							if (!uid.equals(nodeCreator)  && vote.compareTo(50) == 1) {
