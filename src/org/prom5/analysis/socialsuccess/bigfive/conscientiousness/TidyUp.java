...
public class TidyUp extends Behaviour {
	...
	/*	if (trait.getData().isProcessAvailable(SocialSuccessData.USER_ACTION)) {
			// welke users hebben er een tidyUp functionaliteit gebruikt
			// welke users hebben hoeveel wijzigingen aan hun posts gemaakt
			LogReader log = trait.getData().getProcess(
					SocialSuccessData.USER_ACTION);
			Iterator<ProcessInstance> instance = log.instanceIterator();
			while (instance.hasNext()) {
				ProcessInstance p = instance.next();
				String uid = p.getAttributes().get("User_ID");
				...
				while (audits.hasNext()) {
					...
					if (beginTime.before(au.getTimestamp())
						...
						// tidy - up
						if (au.getName().equalsIgnoreCase("TIDY")
								&& au.getType().equalsIgnoreCase("complete")) {
							if (scorePerUser.containsKey(uid)) {
								Double v = scorePerUser.remove(uid);
								scorePerUser.put(uid, v + TIDY_CHANGE_RATIO);
							} else {
								scorePerUser.put(uid, TIDY_CHANGE_RATIO);
							}
						}
						if (au.getName().equalsIgnoreCase("FORM")
								&& au.getType().equalsIgnoreCase("complete")
								&& au.getDataAttributes().containsKey(
										"form_action")
								&& au.getDataAttributes().get("form_action")
										.equalsIgnoreCase("edit")) {
							if (scorePerUser.containsKey(uid)) {
								Double v = scorePerUser.remove(uid);
								scorePerUser.put(uid, v + 1);
							} else {
								scorePerUser.put(uid, 1.);
							}
						}
					}
				}
			}
		} else */ if (trait.getData().isProcessAvailable(PersonalityData.NODE_ACTION_REFER_ORIG)
				|| trait.getData().isProcessAvailable(PersonalityData.NODE_ACTION_USER_ORIG)) {
			LogReader log;
			...
			while (instance.hasNext()) {
				ProcessInstance p = instance.next();
				Iterator<AuditTrailEntry> audits = p.getAuditTrailEntryList().iterator();
				if (p.getAttributes().containsKey("GenericType")
					...
					while (audits.hasNext()) {
						...
						if (beginTime.before(au.getTimestamp())
								&& endTime.after(au.getTimestamp())) {
							if ((au.getName().equalsIgnoreCase("EDIT") 
									|| au.getName().equalsIgnoreCase("LAST_EDIT"))
									&& au.getType().equalsIgnoreCase("complete")
									&& au.getAttributes().containsKey("Actor")) {
								String editor = au.getAttributes().get("Actor");
								if (creator.equals(editor)){
									if (au.getName().equalsIgnoreCase("LAST_EDIT")){
										if (lastEditPerUser.containsKey(editor)) {
											lastEditPerUser.put(editor, lastEditPerUser.get(editor)+1);
										} else {
											lastEditPerUser.put(editor,1.);
										}
									} else {
										if (editsPerUser.containsKey(editor)) {
											editsPerUser.put(editor, editsPerUser.get(editor)+1);
										} else {
			...
			while (it.hasNext()){
				String user = it.next();
				if (editFound){
					scorePerUser.put(user, editsPerUser.get(user) / nodesPerUser.get(user));
				} else {
					scorePerUser.put(user, lastEditPerUser.get(user) / nodesPerUser.get(user));
				}
			}
		} else {
