package org.prom5.analysis.socialsuccess.bigfive.extraversion;
...
public class StartConversations  extends Behaviour {
	...
	public void analyseAllUsers(Date beginTime, Date endTime) {
		...
		if (trait.getData().isProcessAvailable(PersonalityData.NODE_ACTION_REFER_ORIG) ||
			...
			while (instance.hasNext()) {
				...
				try {
					if (p.getAttributes().containsKey("GenericType")
						...
						if ((p.getDataAttributes().containsKey("NROfComments")
								&& Integer.parseInt(p.getDataAttributes().get("NROfComments")) > 0)
								||
							(p.getDataAttributes().containsKey("ContainsComments")
										&& Integer.parseInt(p.getDataAttributes().get("ContainsComments")) > 0))
						{
							if (numberOfNodesWithRepliesPerUser.containsKey(uid)) {
								Integer v = numberOfNodesWithRepliesPerUser.get(uid);
			...
			Iterator<String> it = numberOfNodesWithRepliesPerUser.keySet().iterator();
			while (it.hasNext()){
				String uid = it.next();
				Double r;
				if (!numberOfNodesWithRepliesPerUser.containsKey(uid)){
					...
					r = ((double)numberOfNodesWithRepliesPerUser.get(uid) / (double)numberOfNodesPerUser.get(uid));
				}
