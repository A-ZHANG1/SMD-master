			...
			while (instance.hasNext()) {
				...
				while (audits.hasNext()) {
					...
					if (beginTime.before(au.getTimestamp())
						...
						if (prevEntry != null
								&& prevEntry.getName().equalsIgnoreCase(au.getName()) 
								&& prevEntry.getType().equalsIgnoreCase(au.getType())) {
							if (impatientPerUser.containsKey(uid)) {
								impatientPerUser.put(uid, impatientPerUser.get(uid) + 1);
							} else {
						...
						// deze actie voldoet  niet, dan moet vorige actie variabele
