package org.prom5.analysis.socialsuccess.bigfive.neuroticism;
...
public class OverwhelmedByEmotions extends Behaviour {
	...
	public void analyseAllUsers(Date beginTime, Date endTime) {
		// process 'NODES+TAGS', 'ACTION-USER_ORIG' of 'ACTION-REFER_ORIG' is nodig
		LogReader log = null; 
		...
		if (log != null) {
				...
				try {
					if (d.containsKey("GenericType")
						&& (d.get("GenericType").equals("node")
						  || d.get("GenericType").equals("reply"))
						&& d.containsKey("TimestampCreate")
						&& beginTime.before(fd.parse(d.get("TimestampCreate")))
						&& d.containsKey("Text")
						&& d.containsKey("CreatedBy")) {
							...
							// klinkers matchen
							Pattern klinkers = Pattern.compile(".*(e{4,5}|u{4,5}|i{4,5}|o{4,5}|a{4,5}|y{4,5}).*");
							Pattern klinkers2 = Pattern.compile(".*(e{6,}|u{6,}|i{6,}|o{6,}|a{6,}|y{6,}).*");
							...
							// leestekens herhalen
							Pattern leestekens = Pattern.compile(".*(!{4,5}|\\.{4,5}|\\?{4,5}).*");
							Pattern leestekens2 = Pattern.compile(".*(!{6,}|\\.{6,}|\\?{6,}).*");
							...
							// Smilies
							Pattern smilies = Pattern.compile(".*(:-?\\)|:-?d|:-?\\().*");
							...
							// BrEeZaH
							Pattern breezah = Pattern.compile(".*(\\p{Lu}[\\p{L}&&[^\\p{Lu}]]){2,}[\\p{Lu}]?(\\p{Space}|\\z).*");
							...
							Pattern capitals = Pattern.compile(".*(ECHT|NU|NOW|UNLIMITED|DELICIOUS|FREE).*");
							...
							if (score > 0){
								if (nodesPerCreator.containsKey(author)) {
									nodesPerCreator.put(author, nodesPerCreator.get(author) + 1);
									overwhelmedPerCreator.put(author, 
											overwhelmedPerCreator.get(author) + score);
								} else {
			...
			while (it.hasNext()){
				String user = it.next();
				scorePerUser.put(user, 
						overwhelmedPerCreator.get(user)	/ nodesPerCreator.get(user));
			}
