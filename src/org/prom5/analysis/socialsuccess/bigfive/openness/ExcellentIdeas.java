package org.prom5.analysis.socialsuccess.bigfive.openness;
...
public class ExcellentIdeas extends Behaviour {
	...
	public void analyseAllUsers(Date beginTime, Date endTime) {
		if (trait.getData().isProcessAvailable(PersonalityData.USER_USER_RELATION)){
			LogReader log = trait.getData().getProcess(PersonalityData.USER_USER_RELATION);
			...
			while (it.hasNext()) {
				...
				while (audits.hasNext()) {
					...
					if (endTime.after(au.getTimestamp()) 
						...
						String subscriber = au.getDataAttributes().get("subscriber");
			...
			while (userIt.hasNext()){
				...
				scorePerUser.put(uid, (double)subscribersPerUser.get(uid).size());
			}
