...
public class KMeans1D extends KMeansnD{
	...
	{
		ArrayList<HashMap<String, Double>> returnClusters = new ArrayList<HashMap<String, Double>>(this.k);
	...
	private void initializePickValues() throws InvalidDimensionsException {
		...
		for (int i = 0; i < k; i++) {
			...
			int index = (int)Math.floor(((double)i)*(((double)uniqueValues.size() - 1) / (this.k - 1)));
...
class kMeansPoint1D extends kMeansPointnD {
	...
	public kMeansPoint1D(String _user, Vector<Double> _value) throws InvalidDimensionsException {
		super(_user, 1);
