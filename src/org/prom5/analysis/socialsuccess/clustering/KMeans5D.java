...
public class KMeans5D extends KMeansnD{
	...
	public KMeans5D(int _k, HashMap<String, Vector<Double>> scores) {
		try {
			...
			while (users.hasNext()) {
				...
				this.kMeansPoints.add(new kMeansPoint5D(user, scores.get(user)));
			}
	...
	{
		ArrayList<TreeMap<String, Vector<Double>>> returnClusters = new ArrayList<TreeMap<String, Vector<Double>>>(this.k);
		...
		for (int i = 0; i < this.k; i++){
			...
			returnClusters.get(i).put(MEAN_POINT, clusters[i].getMean().getValue());
		}
...
class kMeansPoint5D extends kMeansPointnD {
	...
	public kMeansPoint5D(String _user, Vector<Double> _value) throws InvalidDimensionsException {
		super(_user, 5);
