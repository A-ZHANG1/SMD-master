package org.prom5.analysis.socialsuccess.clustering;
...
abstract public class KMeansnD {
	...
	final public void runKMeans() {
		...
		try {
			...
			// System.out.println("Clusters found after " + this.nIterations + " iterations.");
	 ...
	 * @throws InvalidDimensionsException 
	 */
	final protected void assignToCluster(kMeansPointnD dp) throws InvalidDimensionsException {
		int oldSize; double minDistance;
		int optimalCluster = dp.getClusterNumber();
		if (optimalCluster >= 0){
			minDistance = 
				dp.distance(this.clusters[optimalCluster].getMean());
	...
	final protected double getSchwarzCriterion() throws InvalidDimensionsException{
	...
	{
		double minValue = Double.MAX_VALUE; int optK = 0;
		try {
...
class Cluster {
	...
	final public double getDistortion(ArrayList<kMeansPointnD> points) throws InvalidDimensionsException{
		double csum = 0.;
