...
public class ClustersUI extends JPanel {
	...
	public void LogPreviewUI() {
		...
		FlatTabbedPane tabbedPane = new FlatTabbedPane("Groups", 
				new Color(240, 240, 240, 230), 
				new Color(180, 180, 180, 120), 
	...
	private void userSelectionChanged() {
		...
		String[] users = new String[selectedIndices.length + selectedClusters.length];
	...
	protected void updateClusters() {
		...
		if(clusters != null && clusters.size() > 0) {
			...
			for (int i = 0; i < clusters.size(); i++){
				...
				clusteNames[i] = "Cluster " + i + " (# "+(clusters.get(i).size()-1)+")";
			}
