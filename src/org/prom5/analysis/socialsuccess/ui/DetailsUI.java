...
import org.prom5.analysis.socialsuccess.ui.summary.SummaryUI;
		...
		if(users.length == 0) {
			...
			for (int i = 0; i < users.length; i++){
				...
				for (int j = 0; j < sc.size(); j++){
					text += "<td>" + pf.sprintf(((sc.get(j) - BigFive.additionConst)/(BigFive.additionConst*4.))*100) + " % </td>";
				}
				text += "</tr>";
			}
			text += "</table>";
	...
	private String getDetails(String user) {
		...
		for (int i = 0; i < s.length; i++){
			...
			for (int j = 0; j < s[i].length; j++){
				if (s[i][j] != null)
					text += "<td>" + pf.sprintf(((s[i][j])/(4*BigFive.additionConst))*100) + " % </td>";
				else 
					text += "<td>&nbsp;</td>";
			}
