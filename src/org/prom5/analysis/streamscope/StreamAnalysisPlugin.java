 ...
 *
 */
public class StreamAnalysisPlugin extends JPanel implements AnalysisPlugin, Provider {
	...
	public void setup(final LogReader log) {
		Thread setupThread = new Thread() {
			public void run() {
				progressPanel.setMinMax(0, (log.getLogSummary().getNumberOfAuditTrailEntries() / 3000));// +
	...
	public JComponent analyse(AnalysisInputItem[] inputs) {
		...
		if(log != null) {
			...
			// error!
			throw new AssertionError("analysis input items do not contain a log reader instance!");
		}
	}
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		ProvidedObject filteredLogObject =
			new ProvidedObject("Filtered log (projected on stream scope)",
				   new Object[] {
					streamLogView.getFilteredLog()
		});
