...
package org.prom5.analysis.summary;
...
public class ExtendedLogSummary implements LogSummary {
	...
	public void addProcessInstance(ProcessInstance pi) {
		...
		while (it.hasNext() && ((process == null) || (!process.getName().equals(processName)))) {
			process = (InfoItem) it.next();
		}

		numSimilarInstances = MethodsForWorkflowLogDataStructures.getNumberSimilarProcessInstances(
							  pi);
		...
		while (ates.hasNext()) {
			...
			if (e == null) {
				// add LogEvent
				logEvents.add(new LogEvent(ate.getElement(), ate.getType(), numSimilarInstances));
			} else {
		...
		int piFrequency = MethodsForWorkflowLogDataStructures.getNumberSimilarProcessInstances(
						  pi);
		...
		for (int i = 0; i < ates.size(); i++) {
			...
			insert(mapOriginators, ates.getEntry(i).getOriginator(), piFrequency);
		}
		...
		if (end != null) {
			if (!endTimes.containsKey(process) ||
					endTimes.get(process) == null) {
...
//process instances (PIs)
		...
		sb.append(tag(tag("Number of PIs: ",
				BOLD) + getNumberOfProcessInstances(), PAR));
		sb.append(tag(tag("Number of different PIs: ",
				BOLD) + getNumberOfUniqueProcessInstances(), PAR));
		sb.append(tag("", PAR));
		sb.append("<table border=\"1\">");
		sb.append(tag((tag("Frequency", TH) + tag("Percentage",
				TH) + tag("PIs with this frequency/percentage", TH)), TR));
...
//audit trail entries (ATEs)
		...
		sb.append(tag(tag("Number of ATEs: ", BOLD) + getNumberOfAuditTrailEntries(), PAR));
		sb.append(tag(tag("Number of different ATEs: ", BOLD) + mapAtes.size(), PAR));
		sb.append(tag(tag("ATEs: ", BOLD) + orderSet(mapAtes.keySet(), false), PAR));
		sb.append(tag("", PAR));
		sb.append("<table border=\"1\">");
		sb.append(tag((tag("Frequency", TH) + tag("Percentage",
				TH) + tag("ATEs with this frequency/percentage", TH)), TR));
...
//start ATEs
		...
		sb.append(tag(tag("Number of different start ATEs: ", BOLD) + mapStartAtes.size(), PAR));
		sb.append(tag(tag("Start ATEs: ", BOLD) + orderSet(mapStartAtes.keySet(), false), PAR));
		sb.append(tag("", PAR));
		sb.append("<table border=\"1\">");
		sb.append(tag((tag("Frequency", TH) + tag("Percentage",
				TH) + tag("Start ATEs with this frequency/percentage", TH)), TR));
...
//end ATEs
		...
		sb.append(tag(tag("Number of different end ATEs: ", BOLD) + mapEndAtes.size(), PAR));
		sb.append(tag(tag("End ATEs: ", BOLD) + orderSet(mapEndAtes.keySet(), false), PAR));
		sb.append(tag("", PAR));
		sb.append("<table border=\"1\">");
		sb.append(tag((tag("Frequency", TH) + tag("Percentage",
				TH) + tag("End ATEs with this frequency/percentage", TH)), TR));
...
//Originators
		...
		sb.append(tag(tag("Number of different originators: ", BOLD) + mapOriginators.size(), PAR));
		sb.append(tag(tag("Originators: ", BOLD) + orderSet(mapOriginators.keySet(), false), PAR));
		sb.append(tag("", PAR));
		sb.append("<table border=\"1\">");
		sb.append(tag((tag("Frequency", TH) + tag("Percentage",
				TH) + tag("Originators with this frequency/percentage", TH)), TR));
	...
	private String convertToOrderedString(HashMap hash, double sum) {
		...
		for (int i = 0; i < keys.length; i++) {
			...
			string.append(tag(tag(keys[i] + "", TD) + tag(percentFormatter.format(percent),
					TD) + tag((String) hash.get(new Integer(keys[i])),
					TD),
					TR));
		}
	...
	private String orderSet(Set set, boolean breakLine) {
		...
		if (!set.isEmpty()) {
			//Building the text...
			//The break lines "<br>" are included to limit the cell width. Otherwise,
			//the exception "javax.swing.text.StateInvariantError: infinite loop in formatting"
			//will be raised by the "FlowView" class when the result has to be shown.
			...
			value = new StringBuffer(value.substring(0, value.lastIndexOf(separator)));
		}
	...
	public Date getStartTime(String process) {
		InfoItem proc = null;
		Iterator it = processes.iterator();
		while (it.hasNext() && ((proc == null) || (!proc.getName().equals(process)))) {
