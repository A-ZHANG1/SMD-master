...
import org.prom5.analysis.traceclustering.distance.DistanceMatrix;
import org.prom5.analysis.traceclustering.model.AHCTreeNode;
...
import org.prom5.analysis.traceclustering.ui.DendroGramUI;
...
public class AHCAlgorithm extends ClusteringAlgorithm implements GuiNotificationTarget {
	...
	protected void build()
	{
		...
		for (int i = 0; i < traceSize; i++) {
			...
			AHCTreeNode tempCluster = new AHCTreeNode(input.getLog(),"Cluster " + clusterCounter++, temp);
			tempCluster.addTrace(i);
			listAHCTree.add(tempCluster);
			if(i%100==0) {
				progress.setProgress(step++);
				progress.setNote((i/100) + " of " + (traceSize/100) +"steps (initialzing clusters) ...");
			}
		}
	...
	private void findNearestCluster_SingleLinkage() {
		...
		AHCTreeNode aAHCTreeNode = new AHCTreeNode(input.getLog(), "Cluster " + clusterCounter++, (AHCTreeNode) aNode, (AHCTreeNode) bNode, minDistance);
	...
	private void findNearestCluster_CompleteLinkage() {
		...
		AHCTreeNode aAHCTreeNode = new AHCTreeNode(input.getLog(), "Cluster " + clusterCounter++,
	...
	private void findNearestCluster_AverageLinkage() {
		...
		for (int i = 0; i < listAHCTree.size()-1; i++) {
			...
			for (int j = i + 1; j < listAHCTree.size(); j++) {
				...
				tempDistanceSum = tempDistanceSum/(double)(aTempNode.size()*bTempNode.size());
