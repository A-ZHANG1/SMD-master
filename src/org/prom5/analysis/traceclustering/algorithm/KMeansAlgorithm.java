 ...
 * @author Minseok Song
 */
public class KMeansAlgorithm  extends ClusteringAlgorithm implements DotFileWriter, GuiNotificationTarget {
	...
	protected GUIPropertyInteger numberOfRunsBox = new GUIPropertyInteger("Number of Runs = ", 50, 1, 100);
	protected GUIPropertyInteger clusterSizeBox = new GUIPropertyInteger("Number of Clusters = ", 3, 1, 100);
	protected GUIPropertyInteger maxIterationBox = new GUIPropertyInteger("Number of Iterations = ", 50, 1, 1000);
	protected GUIPropertyListEnumeration selectResultBox;
	protected GUIPropertyBoolean hideNullCluster = new GUIPropertyBoolean("Hide null clusters",true, this);
	...
	public void build()
	{
		...
		if(selectResultBox.getValue().equals(ST_FREQUENCY)) {
			...
			Message.add("K-means clustering: frequency = " + max, Message.NORMAL);
		}
		else
		{
			Message.add("K-means clustering: distance = " + temp_sum, Message.NORMAL);
		}
	}
	...
	private ClusterSet runKmean()
	{
		...
		for (int k = 0; k < maxIterationBox.getValue(); k++) {
			...
			for (int i = 0; i < distances.rows(); i++) {
				...
				// assign cluster
				((Cluster) clusters.getClusters().get(index)).addTrace(traceList.get(i));
	...
	private void initCenters()
	{
		...
		for(int i=0;i<clusterSize;i++){
			...
			{
				instancePoints[i].set(agProfiles.getItemKey(j), agProfiles.getRandomValue(j));
			}
		}
	}
	...
	private void calcualteDistance()
	{
		...
		for(int i=0;i<traceSize;i++)
			for(int j=0;j<clusterSize;j++)
				distances.set(i,j,distanceMeasures.getDistance(agProfiles.getPoint(i),instancePoints[j]));
	}
...
//	}
	...
	protected void setupGUI()
	{
		...
		selectResultBox = new GUIPropertyListEnumeration("Choose a cluster based on =",null,values2,null,230);
...
//		double[] centers = calculateIntraClusterDistance(clusters);
		...
		bw.write(
				"graph G { ranksep=\".3\"; compound=true; fontsize=\"8\"; remincross=true; margin=\"0.0,0.0\"; ");
		...
		for (Cluster cluster : clusters.getClusters()) {
			...
			bw.write(cluster.getName().replace(" ",
					"") + " [shape=\"octagon\",style=\"filled\",fillcolor=\"chartreuse3\",label=\""
					+ cluster.getName() + "\\n" + cluster.size() + "\\n");// + centers[clusterCounter-1] + " - \"];\n");
		}
		// todo
/*		DoubleMatrix2D length = calculateInterClusterDistance(clustersforOthers);
		// write edges
		for (int i = 0; i < clusters.size() - 1; i++) {
			Cluster cluster1 = clusters.getClusters().get(i);
			if (hideNullCluster.getValue() == true && clusters.getClusters().get(i).size() == 0)
				continue;
			for (int k = i + 1; k < clusters.size(); k++) {
				if (hideNullCluster.getValue() == true && clusters.getClusters().get(k).size() == 0)
					continue;
				Cluster cluster2 = clusters.getClusters().get(k);
				bw.write(cluster1.getName().replace(" ",
						"") + " -- " + cluster2.getName().replace(" ", "") + " [label=\""
						+ length.get(i, k) + "\"];\n");
			}
		}*/
		// close graph
