...
import org.prom5.analysis.traceclustering.ui.DisjointClusterView;
...
public class QualityThresholdAlgorithm extends ClusteringAlgorithm {
	...
	public QualityThresholdAlgorithm() {
		super("Quality threshold clustering",
	...
	protected void setupGui() {
		...
		Thread setupThread = new Thread() {
			public void run() {
				...
				watch.stop();
				Message.add("Calculated distance matrix in " + watch.formatDuration());
				watch.start();
				// start clustering with 0.5
				cluster(0.5);
				watch.stop();
				Message.add("Clustering performed in " + watch.formatDuration());
			}
		};
		setupThread.start();
	}
	...
	public void cluster(double maxDiameter) {
		...
		Thread clusterThread = new Thread() {
			public void run() {
				...
				clusters = new ClusterSet(input.getLog(),(AggregateProfile)input.getProfile());
				while(tmpClusters.size() > 1) {
					...
					int clustered = input.getLog().numberOfInstances() - tmpClusters.size();
					progress.setProgress(clustered);
					progress.setNote(clustered + " of " + input.getLog().numberOfInstances() + " traces clustered...");
				}
				clusters.getClusters().addAll(tmpClusters.getClusters());
				//view.getViewport().setView(new DisjointClusterSetUI(clusters, input));
				DisjointClusterView clusterView = new DisjointClusterView(clusters, input);
				rootPanel.remove(view);
				view = clusterView;
				rootPanel.add(view, BorderLayout.CENTER);
				try {
	...
	protected Cluster generateCluster(Cluster seed, ClusterSet clusters, double diameter) {
	...
	protected class ClusterPointer implements Comparable<ClusterPointer> {
		...
		protected ClusterPointer(Cluster aCluster, Cluster aReferenceCluster, DistanceMatrix aDistanceMatrix) {
			cluster = aCluster;
			int clusterIndex = cluster.getTraceIndices().get(0);
			int referenceIndex = aReferenceCluster.getTraceIndices().get(0);
			distanceToReference = aDistanceMatrix.get(clusterIndex, referenceIndex);
		}
