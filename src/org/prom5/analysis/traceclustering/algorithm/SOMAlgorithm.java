...
import org.prom5.analysis.traceclustering.ui.SOMUI;
...
public class SOMAlgorithm  extends ClusteringAlgorithm implements GuiNotificationTarget {
	...
	private ClusterSet runSOM()
	{
		...
		for(int i=0;i<traceSize;i++){
			...
			progress.setNote( i + " of " + traceSize +" clustering steps (allocating instances) ...");
		...
		for (int i = 0; i < clusterXDim; i++)
			for (int j = 0; j < clusterYDim; j++)
				clusterArray[i][j].setName(clusterArray[i][j].getName()+"("+clusterArray[i][j].size()+")");
		return clusters;
	}
	...
	private void initSOM()
	{
		...
		for (int i = 0; i < clusterXDim; i++)
			for (int j = 0; j < clusterYDim; j++) {
				clusterArray[i][j] = new Cluster(input.getLog(),"Cluster("+j+","+i+")"); // name --> Cluster (y,x)
...
/*	private void calcuateEstimatedRadius() {
		// calculating real paretoradius
		int n= agProfiles.numberOfInstances();
		progress.setNote("estmating radius for P-matrix...");
		progress.setMinMax(0,n*(n-1)/2);
		progress.setProgress(0);
		double optimalMedian = 0.2013 * n;
		
		// calculating distances between every example
		double[] distances = new double[n * n];
		double[] distances2 = new double[n * n];
		int k=0;
		for (int i = 0; i < n; i++) {
			for (int j = i+1; j < n; j++) {
				distances[i * n + j] = distanceMeasures.getDistance(agProfiles.getPoint(i),agProfiles.getPoint(j));
				distances[j * n + i] = distances[i * n + j];
				distances2[i * n + j] = distances[i * n + j];
				distances2[j * n + i] = distances[i * n + j];
				k++;
				progress.setProgress(k);
			}
		}
	...
	// do not use because of poor performance 	
	private double[][] getPMatrix() {
		int n= agProfiles.numberOfInstances();
		// generating P Matrix
		double[][] pMatrix = new double[clusterXDim][clusterYDim];
		for (int i = 0; i < clusterXDim; i++) {
			for (int j = 0; j < clusterYDim; j++) {
				InstancePoint ip = pointArray[i][j];
				int neighbours = 0;
				for (int x = 0; x < n; x++) {
					double distance = distanceMeasures.getDistance
					(agProfiles.getPoint(x),pointArray[i][j]);
					if (distance < estimatedRadius) {
						neighbours++;
					}
				}
...
*/
	...
	private void calcualteDistance(int index, double weight)
	{
		...
		for (int j = 0; j < clusterXDim; j++)
			for (int k = 0; k < clusterYDim; k++) {
				double tempDistance = distanceMeasures.getDistance(agProfiles.getPoint(index),
									  pointArray[j][k]);
	...
	protected void setupGUI()
	{
		...
		numberOfTrainingBox = new GUIPropertyInteger("Number of training = ", Math.round(agProfiles.numberOfInstances()/10+1), 1, agProfiles.numberOfInstances());
