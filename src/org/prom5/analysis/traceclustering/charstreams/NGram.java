...
//		Logger.printCall("In Constructor NGram");
		...
		if(!new File("C:\\Temp\\ProM\\NGrams\\"+n).exists()){
			...
			if(!success){
				MainUI.getInstance().showGlassDialog("File Creation Error", "Couldn't create a temporary directory in C:\\Temp; Can't proceed");
			}
		}
...
//			System.out.println(currentStream);
	...
	public void writeNGramsToFile(String fileName){
		...
		try {
			...
			TablePrinter tablePrinter = new TablePrinter(p,(nGramSize+1) * encodingLength,3,10);
			while (it.hasNext()) {
				currentGram = it.next();
