...
public class NGramSubstitution {
	 ...
	 * 
	 */
	public NGramSubstitution(int nGramSize, int encodingLength, String nGramDir, String nGramFile, HashMap<String, String> itemCharMap) {
		...
		try {
			...
			while ((currentLine = reader.readLine()) != null) {
				...
				if (!gramSymbols[2].equals(prefixSuffix)) {
					leftGivenRightIndexI = symbols.indexOf(gramSymbols[1]); // hard coded for 3
			...
			System.out.println("IOException while reading nGrams file " + nGramDir
					+ "\\" + nGramFile);
			System.exit(0);
		}
	}
...
//		Logger.printCall("Entering computeIndelScores");
		...
		for(int i = 0; i < noSymbols; i++){
			for(int j = 0; j < noSymbols; j++){
				if(countRightGivenLeft[j][i] > 0 && probSymbol[i] > 0 && probSymbol[j] > 0){
					numer = countRightGivenLeft[j][i]/((float)normSymbolScore[i]);
					indelScore = Math.round(new Double(Math.log(numer/(probSymbol[i]*probSymbol[j]))/Math.log(2.0)).floatValue()*2)/10;
//					Logger.println(symbols.get(j)+"@"+symbols.get(i)+" "+indelScore);
					insRightGivenLeft.put(symbols.get(j)+"@"+symbols.get(i),indelScore);
				}
			}
		}
		printScoringMatrixToFile(insRightGivenLeft,"indelRightGivenLeft0.2.txt");
		printScoringMatrixSymbolToFile(insRightGivenLeft,"indelSymbolRightGivenLeft0.2.txt");
		...
		for(int i = 0; i < noSymbols; i++){
			for(int j = 0; j < noSymbols; j++){
				if(countLeftGivenRight[i][j] > 0 && probSymbol[i] > 0 && probSymbol[j] > 0){
					numer = countLeftGivenRight[i][j]/((float)normSymbolScore[i]);
					indelScore = Math.round(new Double(Math.log(numer/(probSymbol[i]*probSymbol[j]))/Math.log(2.0)).floatValue()*2)/10;
					insLeftGivenRight.put(symbols.get(i)+"@"+symbols.get(j),indelScore);
				}
			}
		}
		printScoringMatrixToFile(insLeftGivenRight,"indelLeftGivenRight0.2.txt");
		printScoringMatrixSymbolToFile(insLeftGivenRight,"indelSymbolLeftGivenRight0.2.txt");
			...
			for (int j = 0; j < noSymbols; j++) {
				if (i != j) {
					if (normSubCount[i][j] > 0 && probSymbol[i] > 0
						...
						temp = Math.round(new Double((Math.log(normSubCount[i][j]
						    										/ (2*probSymbol[i] * probSymbol[j])))
						    										/ Math.log(2.0)).floatValue() * 2);
//						if(temp == 0)
//							System.out.println(symbols.get(i) + "@" + symbols.get(j)+" "+2*probSymbol[i]*probSymbol[j]+" "+normSubCount[i][j]);
						scoringMatrix.put(symbols.get(i) + "@" + symbols.get(j), temp);
					}
				}
			}
		}
...
//		Logger.printReturn("Exiting computeSubstitutionScores");
	...
	private void printScoringMatrixSymbolToFile(HashMap<String,Integer> scoreMap, String fileName){
//		Logger.printCall("Entering printScoringMatrixSymbolToFile");
		
		try{
//			FileOutputStream out = new FileOutputStream(charStreamInputDir+"\\"+map+"\\"+fileName);
			FileOutputStream out = new FileOutputStream("C:\\Temp\\ProM\\NGrams\\3\\"+fileName);
			PrintStream p = new PrintStream(out);
			
			TablePrinter tablePrinter  = new TablePrinter(p,2*encodingLength+1,2,5);
			...
			System.out.println("IOException when writing symbol score matrix to file: "+fileName);
		}
//		Logger.printReturn("Entering printScoringMatrixSymbolToFile");
	}
	
	private void printScoringMatrixToFile(HashMap<String,Integer> scoreMap, String fileName){
...
//			FileOutputStream out = new FileOutputStream(charStreamInputDir+"\\"+map+"\\"+fileName);
			...
			while (it.hasNext()) {
				...
				tablePrinter.print(commands.get(symbols.indexOf(commandPair[0])));
				tablePrinter.print(commands.get(symbols.indexOf(commandPair[1])));
...
//						+ "," + scoringMatrix.get(currentPair));
			...
			System.out.println("IOException while writing scoring matrix to File");
			System.exit(0);
		}
