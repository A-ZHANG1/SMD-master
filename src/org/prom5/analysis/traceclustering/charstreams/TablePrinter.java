...
public class TablePrinter{
    ...
    public TablePrinter (PrintStream stream, Integer... widths) {
        ...
        for (int i = 0; i < widths.length; i ++) {
            if (widths[i] < 0) {
                throw new IllegalArgumentException ("negative widths are forbidden");
            } else {
                sum += widths[i];
...
//        if (sum > 200) {
//            throw new IllegalArgumentException ("the widths cannot exceed 80 characters");
//        }
    ...
    public void print (String value) {
        if (column == columnWidths.size ()) {
            throw new RuntimeException ("all columns in this row have been printed - use newLine");
        }
        ...
        if (length > width) {
            throw new IllegalArgumentException ("the value's length "+length+" exceeds this column's width");
        } else {
            if (column > 0) {
                stream.print (" ");
            }
