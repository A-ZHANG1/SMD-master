...
package org.prom5.analysis.traceclustering.charstreams;
...
//		Logger.printCall("Entering findPrimitiveRepeats");
		...
		// For example if there is a tandem repeat of 30 length involving only 2 alphabets
		// then the repeat would be of 15 length; in the above logic, 15%2 != 0 and the 
		// entire 15 length string would have been placed as a primitive repeat; however it would 
		// have been a tandem array with alpha^k
		// Process only odd lengthed primitive repeat
		
		//simple logic as follows:
		// get all primitive repeats involving the same alphabet that are of lengths less than the current primitive repeat
		// check if the current primitive repeat is a multiple of the shorter primitive; then replace the current one with the shorter one
		...
		while(it.hasNext()){
			...
			currPrimitiveRepeatLength = currPrimitiveRepeat.length()/encodingLength;
			repeatAlphabet = new HashSet<String>();
			for(int i = 0; i < currPrimitiveRepeatLength; i++){
				repeatAlphabet.add(currPrimitiveRepeat.substring(i*encodingLength,(i+1)*encodingLength));
			}
		...
		while(it.hasNext()){
			...
			if(currPrimitiveRepeatLength %2 != 0){
				...
				activeRepeatAlphabet = primitiveRepeatAlphabetMap.get(currPrimitiveRepeat);
				
				//get the shortest primitive repeat with the same repeat alphabet
				minPrimitiveRepeat = currPrimitiveRepeat;
				it1 = primitiveRepeatAlphabetMap.keySet().iterator();
				while(it1.hasNext()){
					tempPrimitiveRepeat = it1.next();
					//check if currPrimitiveRepeat and tempPrimitiveRepeat start with the same char
					if(!currPrimitiveRepeat.substring(0,encodingLength).equals(tempPrimitiveRepeat.substring(0,encodingLength)))
							continue;
					if(!primitiveRepeatAlphabetMap.containsKey(tempPrimitiveRepeat)){
//						Logger.println("PR: "+tempPrimitiveRepeat);
						System.exit(0);
					}
					tempAlphabet = primitiveRepeatAlphabetMap.get(tempPrimitiveRepeat); 
					if(tempAlphabet.size() == activeRepeatAlphabet.size() && tempAlphabet.containsAll(activeRepeatAlphabet)){
						if(tempPrimitiveRepeat.length() < minPrimitiveRepeat.length())
							minPrimitiveRepeat = tempPrimitiveRepeat;
					}
				}
				
				//check if the curr primitive repeat is a multiple occurrence of minPR
				tempStr = "";
				int noOccurrences = currPrimitiveRepeat.length()/minPrimitiveRepeat.length();
				for(int i = 0; i < noOccurrences; i++)
					tempStr += minPrimitiveRepeat;
				if(currPrimitiveRepeat.equals(tempStr)){
					repeatPrimitiveRepeatMap.put(currRepeat, minPrimitiveRepeat);
				}
			}
		}
...
//		Logger.printReturn("Exiting findRepeatsAtPos");
	...
	/*
	 * This module determines all maximal repeats (not necessarily tandem)
	 * in the sequence The algorithm is of Gusfield's; Finding
	 * leftDiverseNodes Returns a HashSet of maximal Repeats;
	 * 
	 * Assumes that the parent method calling this would have invoked findLeftDiverseNodes();
	 */
...
//			System.out.println("Looks like findLeftDiverseNodes() is not invoked");
	 ...
	 * A left diverse internal node v represents a super maximal repeat alpha if and only if all of v's
	 * children are leaves, and each has a distinct left character
	 * 
	 * Assumes that the parent method calling this would have already invoked findLeftDiverseNodes();
	 */
...
//			System.out.println("Looks like findLeftDiverseNodes() is not invoked");
		...
		for (int i = 0; i < noLeftDiverseNodes; i++) {
			...
			if (j == noChildren) {
				...
				if (leftSymbolSet.size() == noChildren) {
					...
					// different
					superMaximalRepeats.add(sequence.substring(node.pathPosition
									* encodingLength, (node.edgeLabelEnd + 1)
									* encodingLength));
				}
			}
	...
	/*
	 * This method determines the set of leaves of any given node;
	 * Returns the set of leaves as an array list
	 */
