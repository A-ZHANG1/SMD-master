package org.prom5.analysis.traceclustering.distance;

import java.util.HashMap;
import java.util.Vector;

import org.prom5.analysis.traceclustering.charstreams.NGram;
import org.prom5.analysis.traceclustering.charstreams.NGramSubstitution;
...
public class GenericEditDistance extends DistanceMetric {
	...
	public GenericEditDistance(){
		super("Generic Edit Distance", "Generic Edit Distance-costs for substitution and indel operations automatically derived");
	}
	...
	public DistanceMatrix getDistanceMatrix(Profile profile) {
		...
		encodingLength = ((ActivityCharStreamProfile)profile).getEncodingLength();
		Vector<String> charStreams = ((ActivityCharStreamProfile)profile).getCharStreams();
		
		//Process charStreams
		//1. Generate N Grams
		new NGram(3,encodingLength,charStreams);
		NGramSubstitution ngramSubstitution = new NGramSubstitution(3, encodingLength,"C:\\Temp\\ProM\\NGrams\\3", "allNGrams.out", ((ActivityCharStreamProfile)profile).getActivityCharMap());
	...
	protected void computeEditDistance(Vector<String> charStreams, DistanceMatrix distanceMatrix){
			...
			for(int j = 0; j < i; j++){
				...
				if(seq1.equals(seq2)){
					...
					if(bestScore < 0){
						...
						distanceNorm1 = (float) (lengthSeq1 + lengthSeq2) / (float) bestScore;
					}
				}
	...
	protected int globalAlignLinearSpace(String seq1, String seq2){
		...
		for(int j = 2; j <= lengthSeq2; j++){
			...
			leftSymbol = seq2.substring((j-2)*encodingLength, (j-1)*encodingLength);
			currentSymbol = seq2.substring((j-1)*encodingLength, j*encodingLength);
		...
		for(int i = 1; i <= lengthSeq1; i++){
			s = S[0];
			currentISymbol = seq1.substring((i-1)*encodingLength, i*encodingLength);
			indelIValue = gapCost;
			if(i > 1){
				leftISymbol = seq1.substring((i-2)*encodingLength, (i-1)*encodingLength);
			...
			for(int j = 1; j <= lengthSeq2; j++){
				currentJSymbol = seq2.substring((j-1)*encodingLength, j*encodingLength);
				indelJValue = gapCost;
				if(j > 1){
					leftJSymbol = seq2.substring((j-2)*encodingLength, (j-1)*encodingLength);
					indelObj = indelScore.get(leftJSymbol+"@"+currentJSymbol);
					if(indelObj != null)
						indelJValue = indelObj.intValue();
				}
				
				subScore = gapCost;
				subObj = substitutionScore.get(currentISymbol+"@"+currentJSymbol);
				if(subObj != null)
