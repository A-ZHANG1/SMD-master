...
public class LevenshteinDistance extends DistanceMetric {
	...
	public LevenshteinDistance(){
		super("Levenshtein Edit Distance", "Levenshtein Edit Distance-unit cost model for substitution and indel operations");
	}
	...
	public DistanceMatrix getDistanceMatrix(Profile profile) {
		...
		computeLevenshteinDistanceLinearSpace(((ActivityCharStreamProfile)profile).getCharStreams());
		
		return distanceMatrix;
	}

	protected void computeLevenshteinDistanceLinearSpace(Vector<String> charStreams){
		...
		for(int i = 0; i < noCharStreams; i++){
			...
			for(int j = 0; j < i; j++){
				...
				if(seq1.equals(seq2)){
					...
					if(lengthSeq1 == 0){
						distanceNorm1 = lengthSeq2/((float)(lengthSeq1+lengthSeq2));
					}else if(lengthSeq2 == 0){
						distanceNorm1 = lengthSeq1/((float)(lengthSeq1+lengthSeq2));
					}
					...
					for(int m = 1; m <= lengthSeq1; m++){
						sI = seq1.substring((m-1)*encodingLength, m*encodingLength);
						
						for(int n = 1; n <= lengthSeq2; n++){
							tJ = seq2.substring((n-1)*encodingLength, n*encodingLength);
							...
							d[m][n] = Minimum (d[m-1][n]+1, d[m][n-1]+1, d[m-1][n-1] + cost);
						}
					}
					
					distanceNorm1 = d[lengthSeq1][lengthSeq2]/((float)(lengthSeq1+lengthSeq2));
				}
				distanceMatrix.set(i, j, distanceNorm1);
			}
		}
