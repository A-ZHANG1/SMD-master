...
public class RPProfile extends AbstractPreProcessor {
	...
	public RPProfile(LogReader log) throws IndexOutOfBoundsException, IOException {
		super("Random Projection", "Apply Random Projection to the profiles", log);
	}
		...
		try{
			...
			for (int i=0;i< log.numberOfInstances(); i++) {
				...
				for (int j = 0; j < aggregateProfile.numberOfItems(); j++) {
					String name = CpnUtils.replaceSpecialCharacters(aggregateProfile.getItemKey(j));
					Attribute wekaAtt = data.attribute(name);
					if (wekaAtt != null) {
						double doubleAttValue = (new Double(aggregateProfile.getValue(i,j))).doubleValue();
...
/*			
			for(int i=0;i<data.numAttributes();i++) {
				double db[] = data.attributeToDoubleArray(i);
				System.out.print("{");
