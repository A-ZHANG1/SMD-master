...
import org.prom5.analysis.traceclustering.charstreams.UkkonenSuffixTree;
...
public class ActivityPatternAlphabetsProfile extends ActivityCharStreamProfile {
	public ActivityPatternAlphabetsProfile(LogReader log) throws IndexOutOfBoundsException, IOException{
		super("Activity Pattern Alphabets Profile", "Compares patterns with flexible ordering in process instances", log);
		buildProfile(log);
	}
	...
	protected void buildProfile(LogReader log) throws IndexOutOfBoundsException, IOException {
		...
		for(String currentRepeat: maximalRepeats){
			...
			for(int i = 0; i < currentRepeatSplit.length; i++){
				currentRepeatSplitLength = currentRepeatSplit[i].length()/encodingLength;
				if(currentRepeatSplitLength > 0){
					currentRepeatAlphabet = new HashSet<String>();
					for(int j = 0;  j < currentRepeatSplitLength; j++)
						currentRepeatAlphabet.add(currentRepeatSplit[i].substring(j*encodingLength, (j+1)*encodingLength));
					patternPatternAlphabetMap.put(currentRepeatSplit[i], currentRepeatAlphabet);
		...
		for(int i = 0; i < noCharStreams; i++){
			...
			for(String currentRepeat: patternPatternAlphabetMap.keySet()){
				currentRepeatAlphabet = patternPatternAlphabetMap.get(currentRepeat);
				if(currentCharStream.equals(currentRepeat)){
					incrementValue(i, currentRepeatAlphabet.toString(), 1.0/currentCharStreamLength);
				}else if(currentCharStream.contains(currentRepeat)){
					count = currentCharStream.split(currentRepeat).length > 1 ? currentCharStream.split(currentRepeat).length-1 : 1;
					incrementValue(i, currentRepeatAlphabet.toString(), count/currentCharStreamLength);
				}
			}
		}	
