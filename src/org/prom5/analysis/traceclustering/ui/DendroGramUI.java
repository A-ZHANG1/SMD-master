...
//		g2d.setColor(Color.BLACK);
		...
		int recomRasterSize = Math.max(ratioX, 10);
...
//		if(recomRasterSize != this.rasterSize || this.buffer == null) {
			...
			gBuf.fillRect(0, 0 , bufferSizeX, bufferSizeY);
			...
			// write the name of instances
//			try{
//				for(int i=0;i<numberOfTraces;i++){
//					gBuf.setColor(Color.black);
//					gBuf.drawString(rootNode.getFilteredLog().getInstance(i).getName(), i*rasterSize, bufferSizeY+border * 2);
//				}
//			} catch(Exception ce){
...
//			}
			...
			for(int i=1;i<numberOfTraces;i++){
				gBuf.drawLine(i*rasterSize, bufferSizeY, i*rasterSize, bufferSizeY+ border * 3);
			}
...
//		}
	...
	protected void drawLines(AHCTreeNode node,int pivotX, Graphics g)
	{
	   ...
	   if(node.getDistance()>cutPoint) g.setColor(Color.black);
	   ...
	   if(node.getDistance()!=0){
		   if ((x1 + 2 + 100) > bufferSizeX)
			   g.drawString(format.format(node.getDistance()), x1 + 2 - 40, y + 13);
		   else
			   g.drawString(format.format(node.getDistance()), x1 + 2, y + 13);
	   }

	   if(node.getDistance()>=cutPoint&&((AHCTreeNode)node.getLeft()).getDistance()<cutPoint){
		   changeCurrentColor(g);
		   g.fillRect(pivotX, bufferSizeY, rasterSize*((AHCTreeNode)node.getLeft()).size(), border * 3);
		   numberOfClusters++;
	   }
	   drawLines(leftNode,pivotX,g);
	   if(node.getDistance()>=cutPoint&&((AHCTreeNode)node.getRight()).getDistance()<cutPoint){
		   changeCurrentColor(g);
		   g.fillRect(pivotX+(int)(leftNode.size()*rasterSize), bufferSizeY , rasterSize*((AHCTreeNode)node.getRight()).size(), border * 3);
		   numberOfClusters++;
	   }
	...
	{
		Color tempColor = colorReference.getColor("Cluster "+numberOfClusters);
