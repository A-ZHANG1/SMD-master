package org.prom5.analysis.tracediff;
...
import org.prom5.analysis.tracediff.EventDiff.DiffType;
import org.prom5.framework.log.AuditTrailEntry;
import org.prom5.framework.ui.slicker.logdialog.LogPreviewUI;

/**
 * Renderer displaying the diff of two events in one list cell. 
 * Each element in the list consists of two aligned audit trail 
 * entries and their type of difference at this position. 
 * 
 * @see DiffedAuditTrailEntryListModel
 * 
 * @author Anne Rozinat (a.rozinat at tuel.nl)
 */
public class TraceDiffCellRenderer extends GradientPanel implements ListCellRenderer {
	...
	/**
	 * Create cell renderer that can be used in a list providing 'EventDiff' objects.
	 * @see EventDiff
	 */
	...
	public Component getListCellRendererComponent(JList list, Object value,
		...
		if (isSelected) {
			...
			this.setColors(LogPreviewUI.COLOR_LIST_BG, LogPreviewUI.COLOR_LIST_BG_LOWER);
		}
		...
		if (evDiff.getType() == DiffType.both) {
			...
			leftPanel.setColors(LogPreviewUI.COLOR_LIST_SELECTION_BG, LogPreviewUI.COLOR_LIST_SELECTION_BG_LOWER);
			rightPanel.setOpaque(true);
			rightPanel.setColors(LogPreviewUI.COLOR_LIST_SELECTION_BG, LogPreviewUI.COLOR_LIST_SELECTION_BG_LOWER);
		} else if (evDiff.getType() == DiffType.left) {
			leftPanel.setOpaque(true);
			leftPanel.setColors(COLOR_DIFFED_CELL, COLOR_DIFFED_CELL_LOWER);
			rightPanel.setColors(LogPreviewUI.COLOR_LIST_BG, LogPreviewUI.COLOR_LIST_BG_LOWER);
			rightPanel.setOpaque(false);
		} else if (evDiff.getType() == DiffType.right) {
			leftPanel.setColors(LogPreviewUI.COLOR_LIST_BG, LogPreviewUI.COLOR_LIST_BG_LOWER);
	 ...
	 * @param panel the event cell panel to be filled
	 * @param ate the event to be represented (can be null)
	 * @param ateIndex the original index of the given audit trail entry to be displayed 
	 */
	private void fillEventContent(EventPanel panel, AuditTrailEntry ate, int ateIndex) {
		if (ate != null) {
	...
	/**
	 * Custom component painting the diff symbol to be displayed between 
	 * two diffed events in each row. 
	 * 
