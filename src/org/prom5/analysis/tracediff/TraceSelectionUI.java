...
public class TraceSelectionUI extends JPanel {
	 ...
	 * notifcation target provided, then it will be invoked upon pressing the start 
	 * button. 
	 * @param aLog the log from which two traces should be selected 
	 * @param aTarget a target to notify when selection is completed (can be null)
	 */
	...
	public String getLeftSelection() {
		return (String) leftList.getModel().getElementAt(leftList.getSelectedIndex());
	}
	
	public String getRightSelection() {
		return (String) rightList.getModel().getElementAt(rightList.getSelectedIndex());
	}
