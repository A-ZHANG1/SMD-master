...
import org.prom5.mining.partialordermining.AggregationGraph;
...
public class AggregationGraphToEPC implements ConvertingPlugin {
	...
	public MiningResult convert(ProvidedObject original) {
		...
		LogReader log = null;
		while (((g == null) || (log == null)) && (i < original.getObjects().length)) {
	...
	private EPC toEPC(AggregationGraph aggregated) {
		...
		while (it.hasNext()) {
			...
			Object o = v.object;
			EPCFunction f = epc.addFunction(o == null ? new EPCFunction((LogEvent)null, epc) :
							new EPCFunction((LogEvent) o, epc));
			if (o == null) {
			...
			if (v.inDegree() > 1) {
				...
				if (s == v.getValue()) {
					// Introduce XOR-join
					in.put(f, epc.addConnector(new EPCConnector(EPCConnector.XOR, epc)));
				} else if (s == (v.inDegree() * v.getValue())) {
					// Introduce AND-join
					in.put(f, epc.addConnector(new EPCConnector(EPCConnector.AND, epc)));
				} else {
					// Introduce OR-join
					in.put(f, epc.addConnector(new EPCConnector(EPCConnector.OR, epc)));
				}
			}
			if (v.outDegree() > 1) {
				...
				if (s == v.getValue()) {
					// Introduce XOR-split
					out.put(f, epc.addConnector(new EPCConnector(EPCConnector.XOR, epc)));
				} else if (s == (v.outDegree() * v.getValue())) {
					// Introduce AND-split
					out.put(f, epc.addConnector(new EPCConnector(EPCConnector.AND, epc)));
				} else {
					// Introduce OR-split
					out.put(f, epc.addConnector(new EPCConnector(EPCConnector.OR, epc)));
				}
		...
		// Build the EPC by translating edges to events and connecting them with connectors
		// and functionds
		...
		while (it.hasNext()) {
			...
			/*
			   if (source == null) {
				// initial edge.
				if (n1.outDegree() == 1) {
				 // Only one outgoing arc at the start event
				 if (in.get(dest) == null) {
				  epc.addEdge(start, dest);
				 } else {
				  epc.addEdge(start, (EPCConnector) in.get(dest));
				 }
				} else {
				 // Multiple outgoing arcs from the start event
				 if (out.get(start) == null) {
				  double s = sumOverEdgeValues(n1.getOutEdgesIterator());
				  if (s == n1.getValue()) {
				   out.put(start,
					 epc.addConnector(new EPCConnector(EPCConnector.XOR,
					 epc)));
				  } else if (s == n1.getValue() * n1.outDegree()) {
				   out.put(start,
					 epc.addConnector(new EPCConnector(EPCConnector.AND,
					 epc)));
				  } else {
				   out.put(start,
					 epc.addConnector(new EPCConnector(EPCConnector.OR, epc)));
				  }
				  epc.addEdge(start, (EPCConnector) out.get(start));
				 }
				 if (in.get(dest) == null) {
				  epc.addEdge((EPCConnector) out.get(start), dest);
				 } else {
				  epc.addEdge((EPCConnector) out.get(start), (EPCConnector) in.get(dest));
				 }
				}
				continue;
			   }
			   ...
			   if (dest == null) {
				if (n2.inDegree() == 1) {
				 // Only one Incoming arc at final event
				 if (out.get(source) == null) {
				  epc.addEdge(source, end);
				 } else {
				  epc.addEdge((EPCConnector) out.get(source), end);
				 }
				} else {
				 // Multiple Incoming arcs at final event
				 if (in.get(end) == null) {
				  double s = sumOverEdgeValues(n2.getInEdgesIterator());
				  if (s == n2.getValue()) {
				   in.put(end, epc.addConnector(new EPCConnector(EPCConnector.XOR,
					 epc)));
				  } else if (s == n2.getValue() * n2.inDegree()) {
				   in.put(end, epc.addConnector(new EPCConnector(EPCConnector.AND,
					 epc)));
				  } else {
				   in.put(end, epc.addConnector(new EPCConnector(EPCConnector.OR,
					 epc)));
				  }
				  epc.addEdge((EPCConnector) in.get(end), end);
				 }
				 if (out.get(source) == null) {
				  epc.addEdge(source, (EPCConnector) in.get(end));
				 } else {
				  epc.addEdge((EPCConnector) out.get(source), (EPCConnector) in.get(end));
				 }
				}
			...
			if (in.get(dest) == null) {
				...
				if (out.get(source) == null) {
					...
					epc.addEdge((EPCConnector) out.get(source), (EPCConnector) in.get(dest));
				}
			}
		}
