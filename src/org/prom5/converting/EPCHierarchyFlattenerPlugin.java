...
import org.prom5.framework.models.epcpack.algorithms.EPCCopier;
...
public class EPCHierarchyFlattenerPlugin implements ConvertingPlugin {
	...
	public MiningResult convert(ProvidedObject original) {
		...
		while (it.hasNext()) {
			...
			// now for each of these preceeding events, add an extra and-split connector
			it2 = orgPreEvt.iterator();
			while (it2.hasNext()) {
				EPCEvent e = (EPCEvent) it2.next();
				EPCObject dest = (e.outDegree() > 0 ? (EPCObject) e.getSuccessors().iterator().next() : null);
				EPCConnector and = newEPC.addConnector(new EPCConnector(EPCConnector.AND, newEPC));
				if (dest != null) {
			...
			// now for each of these succeeding events, add an extra xor-join connector
			it2 = orgSucEvt.iterator();
			while (it2.hasNext()) {
				EPCEvent e = (EPCEvent) it2.next();
				EPCObject src = (e.inDegree() > 0 ? (EPCObject) e.getPredecessors().iterator().next() : null);
				EPCConnector xor = newEPC.addConnector(new EPCConnector(EPCConnector.XOR, newEPC));
				if (src != null) {
			...
			while (it2.hasNext()) {
				...
				if (fun instanceof EPCSubstFunction) {
					newFun = newEPC.addFunction(new EPCSubstFunction(fun.getLogEvent(),
							 fun.isConfigurable(), newEPC, ((EPCSubstFunction) fun).getSubstitutedEPC()));
				} else {
					newFun = newEPC.addFunction(new EPCFunction(fun.getLogEvent(),
							 fun.isConfigurable(),
							 newEPC));
				}
			...
			while (it2.hasNext()) {
				...
				mapping.put(c.getIdKey(), newEPC.addConnector(new EPCConnector(c.getType(), newEPC)));
			}
			...
			while (it2.hasNext()) {
				...
				if (evt.inDegree() == 0) {
					...
					newEvt = newEPC.addEvent(new EPCEvent(evt.getIdentifier(), newEPC));
				}
				if (newEvt == null) {
					// some initial or final event doesn't match
					Message.add("Some substitution function does not have the same initial and final events as the substituted EPC, aborting!");
					return null;
				}
			...
			while (it2.hasNext()) {
				...
				EPCObject source = (EPCObject) mapping.get(e.getSource().getIdKey());
				EPCObject dest = (EPCObject) mapping.get(e.getDest().getIdKey());
		...
		while (it.hasNext()) {
			...
			if (c.inDegree() == 0 || c.outDegree() == 0) {
				...
				EPCObject source = (EPCObject) c.getPredecessors().iterator().next();
				EPCObject dest = (EPCObject) c.getSuccessors().iterator().next();
				newEPC.delConnector(c);
				newEPC.addEdge(source, dest);
				it = newEPC.getConnectors().iterator();
			}
	...
	public boolean accepts(ProvidedObject original) {
		...
		while (!(epcHier) && (i < original.getObjects().length)) {
			epcHier |= (original.getObjects()[i] instanceof EPCHierarchy) &&
					(((EPCHierarchy) original.getObjects()[i]).getSelectedNode() instanceof
					EPC);
			i++;
		}
		return epcHier;
