...
public class EPCToPetriNetConverterPlugin implements ConvertingPlugin {
	...
	public MiningResult convert(ProvidedObject original) {
		...
		if (!b) {
			...
			return new PetriNetResult((LogReader) original.getObjects()[i - 1], reduced);
		}
	}

	/**
	 * Converts the given EPC model into a Petri net model.
	 * @param model the EPC model to convert
	...
	public PetriNet convert(ConfigurableEPC baseEPC) {
		...
		PetriNet petrinet = EPCToPetriNetConverter.convert(baseEPC, new HashMap(), functionActivityMapping, xorconnectorChoiceMapping);

		HashSet visible = new HashSet();

		//HV: The next block is taken care of by the functionActivityMapping below.
		/*
...
//			if (t.getLogEvent() != null) {
				// Add transitions with LogEvent (i.e. referring to functions)
				visible.add(t);
			}
		}*/
		...
		HashMap pnMap = new HashMap(); // Used to map pre-reduction nodes to post-reduction nodes.
		PetriNet reduced = pnred.reduce(petrinet, pnMap);

		if (reduced != petrinet) {
			// Update both mappings from pre-reduction nodes to post-reduction nodes.
			HashMap<EPCFunction,
					Transition> newFunctionActivityMapping = new HashMap<EPCFunction, Transition>();
			...
			HashMap<EPCConnector,
					Place> newXorconnectorChoiceMapping = new HashMap<EPCConnector, Place>();
	...
	/**
	 * Returns a mapping of the input epc functions to the
	 * corresponding transitions in the converted model.
