...
import org.prom5.converting.protos.ProtosToPetriNet;
import org.prom5.converting.protos.ProtosToProtos;
...
import org.prom5.framework.models.hlprocess.hlmodel.HLProtos;
...
public class HLProtosToHLPetriNet implements ConvertingPlugin {
	...
	private void copyHighLevelInformation(HLProtos hlProtos,
		...
		// first, remove the automatically created nobody and anybody groups in the target hlModel
		hlPetriNet.getHLProcess().removeGroupWithoutAffectingAssigmnets(hlPetriNet.getHLProcess().getNobodyGroupID());
		hlPetriNet.getHLProcess().removeGroupWithoutAffectingAssigmnets(hlPetriNet.getHLProcess().getAnybodyGroupID());
		// then, update nobody and anybody ID as activities will still point to nobody and anybody IDs 
		// from the source hlModel (cloning of IDs is not needed)
		hlPetriNet.getHLProcess().setNobodyGroupID(hlProtos.getHLProcess().getNobodyGroupID());
		hlPetriNet.getHLProcess().setAnybodyGroupID(hlProtos.getHLProcess().getAnybodyGroupID());
		// now, clone the real group objects (including nobody and anybody of source hlModel)
		Iterator<HLGroup> groups = hlProtos.getHLProcess().getAllGroups().iterator();
