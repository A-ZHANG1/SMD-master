...
/**
 * <p>Title: Heuristics Net to Petri Net</p>
 *
 * <p>Description: This class converts a <code>HeuristicsNet</code> to a <code>PetriNet</code>.</p>
 *
 *
...
public class HNetToPetriNetConverter implements ConvertingPlugin {
	...
	public MiningResult convert(ProvidedObject original) {
		...
		for (int i = 0; i < original.getObjects().length; i++) {
			hNet = (original.getObjects()[i] instanceof HeuristicsNet ?
				   (HeuristicsNet) original.getObjects()[i] :
				   hNet);
		...
		//associated to it.
		PetriNetResult reducedResult = (PetriNetResult) reducedPetriNet.convert(new ProvidedObject(
									   "Petri net", new Object[] {pn}));
	 ...
	 * If you are not sure if this is the case, you can use the method
	 * removeDanglingElementReferences in class org.processmining.mining.geneticmining.util.MethodsOverIndividuals
	 * to remove dangling element references from the INPUT/OUPUT sets of the heuristics net.
	 * @return a Petri net based on the naive mapping from heuristics net to Petri net.
	 */
	public PetriNet toPetriNet(HeuristicsNet hNet) {
		...
		//adding start place
		Place uniqueStartPlacePetriNet = pn.addPlace("UniqueStartPlacePetriNet");
		Place uniqueEndPlacePetriNet = pn.addPlace("UniqueEndPlacePetriNet");

		//adding places and tasks
		for (int i = 0; i < hNet.size(); i++) {
			if ((hNet.getInputSet(i).size() > 0) || (hNet.getOutputSet(i).size() > 0)) {
				//adding input places
				...
				visibleTask = new Transition(hNet.getLogEvents().getEvent(hNet.
							  getDuplicatesMapping()[i]), pn);
				...
				for (int iUnionOutputSubsets = 0; iUnionOutputSubsets < subset.size();
											   iUnionOutputSubsets++) {
					//adding invisible (or silent) tasks
					name = "t" + i + "t" + subset.get(iUnionOutputSubsets);
					pn.addTransition(new Transition(new LogEvent(name, ""), pn));
				}
			}
		}
		...
		for (int i = 0; i < hNet.size(); i++) {
			if ((hNet.getInputSet(i).size() > 0) || (hNet.getOutputSet(i).size() > 0)) {
				addInputArcs(pn, i, hNet.getInputSet(i),
						hNet.getLogEvents().getEvent(hNet.getDuplicatesMapping()[
						i]),
						invisibleTasks,
						uniqueStartPlacePetriNet);
				addOutputArcs(pn, i, hNet.getOutputSet(i),
						hNet.getLogEvents().getEvent(hNet.getDuplicatesMapping()[
						i]),
						invisibleTasks,
						uniqueEndPlacePetriNet);
			}
		}
	...
	private static Transition getTransition(ArrayList listOfTransitions, int transitionID) {
		Transition t = null;
	...
	private static void addInputArcs(PetriNet pn, int task, HNSet inputSet, LogEvent le,
			HashSet invisibleTasks, Place uniqueStartPlacePetriNet) {
	...
	private static void addOutputArcs(PetriNet pn, int task, HNSet outputSet, LogEvent le,
			HashSet invisibleTasks, Place uniqueEndPlacePetriNet) {
		StringBuffer sb = null;
		HNSubSet subset = null;
		Place p = null;
		Transition t = null;
