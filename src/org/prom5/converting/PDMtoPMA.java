...
public class PDMtoPMA implements ConvertingPlugin
{
	...
	public PetriNet convert(PDMModel model) {
		...
		PDMDataElement root = model.getRootElement();   // the "root element of the PDM
    	...
    	// Wil's algorithm to transform a PDM into a PN!!! See literature: Computers in Industry (39),
	    // pp. 97-111, On the automatic generation of workflow processes based on product structures.


	    // Wil's algorithm STEP 1: construct the basic net PN

		// Create an input place for the "root" element; a new place object is created,
		// then this object is addes to the set of places (defined above) and the place
	    // is addes to the PetriNet that will be returned when this method has finished.
        ...
        // Create an edge from the input place to the transition of the "root" element
	    PNEdge edgeIn = new PNEdge(inRoot,rootTransition);
		result.addEdge(inRoot,rootTransition);
		edges.add(edgeIn);
		// Create an edge frome the transition to the output place of the "root" element
		...
		// Determine the intersection of T, the transitions, and C, the data elements in the PDM
	    ...
	    while (!(intersectionTC.isEmpty())) {
			// Put all elements from the intersection in an array so we can walk through it
			// translate the multiset containing the intersecting elements to an array. Be careful: one transition can occur more than once!
		    ...
		    while (v.hasNext()) {
				...
				PDMDataElement data2 = model.getDataElement(trans.getIdentifier());
				// Find the preceeding elements of this data element
				HashMap precs = model.getPrecedingElements(data2);
				// If there are non, then the transition is relabeled and go to STEP 2 again to select another element
				if ((precs.isEmpty())) {
					// relabel the transition 'trans' to 'produce_trans'
					Transition t = new Transition("produce_"+trans.getIdentifier(), result);
					result.addTransition(t);
					trsVector.add(t);
                    // do some administration to make the edges go to the right object:
					// add the original arcs from input and output places of trans to produce_trans
				...
				// Replace transition 'data2' by a subnet
					...
					// (transition trans is replaced by prepare_data2 and produce_data2 transitions)
					Transition t3 = new Transition("prepare_"+trans.getIdentifier(), result);
					result.addTransition(t3);
					trsVector.add(t3);
					Transition t4 = new Transition("produce_"+trans.getIdentifier(), result);
				    result.addTransition(t4);
				    trsVector.add(t4);

	                // Add the edges that flow into trans to flow into prepare_data2 and remove the edges to trans
					...
					// Add the edges that flow out of trans to flow out of produce_data2 and remove the edges to trans
	                HashSet edges3 = (HashSet) edges.clone();
	                ...
	                // Check how many operations can produce data element 'data2' produced by trans
					HashSet ops = model.getOperationsWithOutputElement(data2);
					// There are more than one operations in the set 'ops' when there are alternative paths to produce data element 'data2'
					if (ops.size()==1){

						// Create an output place for every data element in 'precs'
						// Create an input place for every data element in 'precs'
						...
						for (int k = 0; k < precel.length; k++) {
							...
							Place output = new Place("out_" + dataElt.getID(), result);
							result.addPlace(output);
							places.put("out_" + dataElt.getID(), output);
							Place input = new Place("in_" + dataElt.getID(), result);
							result.addPlace(input);
							places.put("in_" + dataElt.getID(), input);
							Transition t2 = new Transition(dataElt.getID(), result);
					...
					// There are more than one operations in the set 'ops' when there are alternative paths to produce data element 'data2'
							...
							Place place5 = new Place("in_" + precs.toString(), result);
							Place place6 = new Place("out_" + precs.toString(), result);
							...
							// And, the edges from the prepare_y to the input place and from the output place to produce_y
							...
							// create the structure for the input elements of the operation
							// Create prepare_data2 and produce_data2 transitions
							Iterator it6= ops.iterator();
							while (it6.hasNext()) {
								PDMOperation op = (PDMOperation) it6.next();
								HashMap input = op.getInputElements();
								Transition t6 = getTransitionWithID(trsVector,op.getID());
								if (input.size() == 1){
									// transition t6 is replaced by the a transition producing the only input element of the operation t6
									Object[] ar = input.values().toArray();
									for (int i=0; i<ar.length; i++){
										PDMDataElement data = (PDMDataElement) ar[i];
										Transition t9 = new Transition(data.getID(),result);
										result.addTransition(t9);
										trsVector.add(t9);
										// Add the edges that flow into t6 to flow into prepare_t6 and remove the edges to t6
										...
										// Add the edges that flow out of t6 to flow out of produce_t6 and remove the edges to t6
										...
										while (it12.hasNext()) {
											...
											if (edge1.getSource().equals(t6)) {
												Place place2 = (Place) edge1.getDest();
												PNEdge edge2 = new PNEdge(t9, place2);
									...
									// (transition t6 is replaced by prepare_data2 and produce_data2 transitions)
									Transition t9 = new Transition("prepare__" + t6.getIdentifier(),
											result);
									result.addTransition(t9);
									trsVector.add(t9);
									Transition t10 = new Transition("produce__" + t6.getIdentifier(),
											result);
									...
									for (int i = 0; i < in.length; i++) {
										// Add a transition an an input and output place for each data element
										PDMDataElement data3 = (PDMDataElement) in[i];
										Place in2 = new Place("in_" + data3.getID(), result);
										places.put("in_" + data3.getID(), in2);
										result.addPlace(in2);
										// Create an output place for the data element
										Place out2 = new Place("out_" + data3.getID(), result);
										places.put("out_" + data3.getID(), out2);
										result.addPlace(out2);
										// Create a transition for the data element
										Transition t7 = new Transition(data3.getID(), result);
										result.addTransition(t7);
										trsVector.add(t7);
//										System.out.println(transitions.values().toString());

										// Create an edge from the input place to the transition of the data element
										PNEdge edgeIn2 = new PNEdge(in2, t7);
										result.addEdge(in2, t7);
										edges.add(edgeIn2);
										// Create an edge from the transition to the output place of the data element
										PNEdge edgeOut2 = new PNEdge(t7, out2);
										result.addEdge(t7, out2);
										edges.add(edgeOut2);
										// Create an edge from the prepare to input place
...
//	    Object[] trs = transitions.values().toArray();
		...
		// Add a fake Logevent to make the identifiers of the transitions visible in the graph
...
//			if (!(trid.startsWith("prepare__")) && !(trid.startsWith("produce__"))) {
...
//			}
	...
	/**
	 * Returns the identifier (or name) of the transition on place i in the vector
	 * @param vector Vector
	 * @param i int
	 * @return String
	 */
...
private Transition getTransitionWithID(Vector vector, String id){
	 ...
	 *
	 * @param original The original mining result
