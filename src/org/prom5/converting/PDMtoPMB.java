...
//		Ps.addToken(to);
	...
	public PetriNet calculate(PDMModel model, PetriNet result, PDMDataElement ie, Place in, Place out) {
		// model contains the data of the PDM model
		// result contains the translation to the PetriNet, as far as it is ready yet
		// ie is the element considered now in the algorithm
		// in and out are the places inbetween which the translation of ie and its children will be placed
		Object[] opArray = model.getOperationsWithOutputElement(ie).toArray();

		for (int j=0; j<opArray.length; j++){
			// Each operation (i, cs) with (i==ie) will now be added to the Petri net,
			...
			if (inputs.isEmpty()) {
				...
				// cs = 1. Therefore it is enough to add a single transition, that will be preceded
				...
				for (int k = 0; k < ins.length; k++) {
					...
					result = calculate(model, result, (PDMDataElement) ins[k], in, P1);
				}
			} else if (inputs.size() > 1) {
				// cs > 1. Therefore the translation of the elements preceding the current element
				// will be nested. Thus, as a nesting T(i, cs)_init and T(i,cs) are created.
				Transition t2init = new Transition("T_" + op.getID() + "_init", result);
				result.addTransition(t2init);
				LogEvent fake2 = new LogEvent(t2init.getIdentifier(), "complete");
				...
				Object[] ins = inputs.values().toArray();
				for (int k = 0; k < ins.length; k++) {
					// recursively the translation of all elements preceding the current element
					// will be translated.
					PDMDataElement data2 = (PDMDataElement) ins[k];

					Place P2init = new Place("P_" + data2.getID() + "_init", result);
					...
					result = calculate(model, result, (PDMDataElement) ins[k], P2init, P2);
				}
			}
		}
		return result;
