...
public class PDMtoPMD implements ConvertingPlugin
{
	...
	public YAWLModel convert(PDMModel model) {
	    ...
	    YAWLDecomposition decomp = new YAWLDecomposition("main", "true", "NetFactsType");
    	result.addDecomposition("main", decomp);

	    // HV: Keep track whether decompositions are required for task, router, and start.
	    boolean hasTask = false, hasRouter = false, hasStart = false;

		// Create an input place, a transition, and an output place for every data element in the PDM, plus a control input place
		...
		for (int i=0; i<dataArray.length; i++){
			...
			hasTask = true; // HV: At least one task requires the task decomposition.
...
//			t.setLogEvent(fake);
			...
			edges.add(e1);
			decomp.addEdge(p1.getIdentifier(),t.getIdentifier(),false,"true",null);
			YAWLEdge e2 = new YAWLEdge(t,p2);
			edges.add(e2);
			decomp.addEdge(t.getIdentifier(),p2.getIdentifier(),false,"true",null);
			YAWLEdge e3 = new YAWLEdge(p3,t);
			edges.add(e3);
			decomp.addEdge(p3.getIdentifier(),t.getIdentifier(),false,"true",null);
		...
		for (int j=0; j<opArray.length; j++){
			...
			if (!(inputs.isEmpty())) {
				...
				hasRouter = true; // HV: At least one task requires the router decomposition.
				// Also add an initial input/control place to every invisible transition to make sure it can only fire once
				YAWLCondition c = new YAWLCondition(result, 2, "C_" + op.getID());
				places.put(c.getIdentifier(), c);
				decomp.addCondition(c.getIdentifier());
				YAWLEdge e = new YAWLEdge(c, t);
				edges.add(e);
				decomp.addEdge(c.getIdentifier(), t.getIdentifier(), false, "true", null);

				// An edge from every input element to the invisible transition (AND back)
				Object[] ins = inputs.values().toArray();
				for (int k = 0; k < ins.length; k++) {
					PDMDataElement data1 = (PDMDataElement) ins[k];
					YAWLCondition p = (YAWLCondition) places.get("out_" + data1.getID());
					YAWLEdge e1 = new YAWLEdge(p, t);
					YAWLEdge e2 = new YAWLEdge(t, p);
					edges.add(e1);
					edges.add(e2);
					decomp.addEdge(p.getIdentifier(), t.getIdentifier(), false, "true", null);
					decomp.addEdge(t.getIdentifier(), p.getIdentifier(), false, "true", null);
				}
				...
				for (int m = 0; m < outs.length; m++) {
					...
					YAWLCondition p2 = (YAWLCondition) places.get("in_" + data1.getID());
					YAWLEdge e2 = new YAWLEdge(t, p2);
					edges.add(e2);
					decomp.addEdge(t.getIdentifier(), p2.getIdentifier(), false, "true", null);
				}
		...
		hasStart = true; // HV: At least one task requires the start decomposition.
		decomp.addEdge(start.getIdentifier(),tstart.getIdentifier(),false,"true",null);
		...
		for (int ii=0; ii<leafArray.length; ii++) {
			...
			edges.add(e3);
			decomp.addEdge(tstart.getIdentifier(),p3.getIdentifier(),false, "true", null);
		}

	    // Add edges from start transition to all enabling places to the transitions
	    ...
	    for (int ii=0; ii<pls2.length; ii++) {
		   ...
		   if (cc.startsWith("C_")) {
			  ...
			  edges.add(e3);
	          decomp.addEdge(tstart.getIdentifier(), c.getIdentifier(), false, "true", null);
           }
	    }

	   // Add resetEdges from all places to the root element, except from the start place and out_root places.
	   ...
	   for (int jj = 0; jj<pls.length; jj++) {
		   ...
		   if ((!p4.equals(start)) && (!p4.equals(places.get("out_"+root.getID())))) {
	   ...
	   if (hasTask) {
		   result.addDecomposition("task", new YAWLDecomposition("task", "false", "WebServiceGatewayFactsType"));
	   }
	   if (hasRouter) {
		   result.addDecomposition("router", new YAWLDecomposition("router", "false", "WebServiceGatewayFactsType"));
	   }
	   if (hasStart) {
		   result.addDecomposition("start", new YAWLDecomposition("start", "false", "WebServiceGatewayFactsType"));
	   }
	   ...
	   return result;
		...
		while (it.hasNext()) {
			...
			Message.add("<Number of YAWL nodes = " + nodes.size()+ " >", Message.TEST);
			Message.add("<Number of YAWL edges = " + edges.size()+ " >", Message.TEST);
		}
		Message.add("</PDMtoPM Delta>", Message.TEST);
	}
