...
//		Ps.addToken(to);
		...
		result = calculate(model, result, places, transitions, edges, FullPath, done);
		...
		for (int l = 0 ; l < placeloop.length; l++) {
			...
			if (placename.contains(rootname) == true) {
				...
				// to prevent the rootname being a substring of another element's name
				if ((placename.contains("{" + rootname + ", ") == true)
	...
	public PetriNet calculate(PDMModel model, PetriNet result, HashMap places, HashMap transitions, HashSet edges, HashMap todo, HashMap done) {
		...
		for (int j=0; j<opArray.length; j++){
			...
			if ((issubset(todo, outputs)) && (issubset(done, inputs))) {
				// Operation opArray[j] (i, cs) with i element todo and cs subset done is found
