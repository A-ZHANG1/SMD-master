 ...
 * <p>Company:  TU/e and Pallas Athena </p>*
 *
...
//		Ps.addToken(to);
		...
		for (int c = 0; c < pathsArray.length; c++) {
			...
			// Only a last "empty" transition to the end place P_f needs to be added.
...
// It can be used since Operation.getOutputElements().size() == 1.
/**/
/**/
			...
			for (int ipath = 0; ipath < pathsort.length; ipath++) {
				...
				if (ipath < (pathsort.length - 1)) {ppathname = ppathname + ", ";}
			}
			ppathname = ppathname + "}";
	...
	public PetriNet calculate(PDMModel model, PetriNet result, HashMap places, HashMap transitions, HashSet edges, HashMap todo, HashMap done) {
		...
		for (int j=0; j<opArray.length; j++){
			...
			if (issubset(done, inputs)) {
				// Operation opArray[j] (i, cs) element of todo and with cs subset of done is found

				// First create todo_after = (todo \ i) and done_after = (done u i)
...
/**/
				...
				// todo_after = (todo \ i) and done_after = (done u i) are now created.

				// Check to see if place P_done_after exists, and if not create it!
				...
				for (int aftersort = 0; aftersort < sort_after.length; aftersort++) {
					...
					if (aftersort < (sort_after.length - 1)) {pdoneaftername = pdoneaftername + ", ";}
				}
				pdoneaftername = pdoneaftername + "}";

				// Then check if the place already exists, and if not, create it.
				Place pdoneafter;
				...
				for (int s1 = 0; s1 < sort.length; s1++) {
					...
					if (s1 < (sort.length - 1)) {pdonename = pdonename + ", ";}
				}
				pdonename = pdonename + "}";
				...
				if ((transitions.containsKey("T_" + op.getID())) == false) {
					// The connecting transition does not exist, so there is no connection!
					connected = false;
				} else {
					// The connecting transition exists. But is this true for the edges?
					...
					if (econ1exists && econ0exists) {
						// Both edge (pdone -> tconnected) and (tconnected -> pdoneafter) exist
						// Nothing happenes.
					} else {
						// P_done and P_done_after are not connected via transition T_op
						connected = false;
					}
				}
	...
	public Object[] bubblesort(Object[] tosort) {
		// Sort the array tosort using a simple (though inefficient) sorting algorithm.
		Boolean sorting;
		...
		do {
			...
			for (int s0 = 0; s0 < upperlimit; s0++) {
				if (tosort[s0].toString().compareTo(tosort[s0 + 1].toString()) < 0) {
					...
					// However, since dealing with an array, not a set, duplicates are possible.
	...
	public HashMap getOpaths(PDMModel model, PDMDataElement ie) {
		...
		// A path is exactly the set of operations needed to create ie (not a unique set!).
		...
		if (opArray.length == 0) {
			/**/
			/**/
			// This code will never be executed. By implementation, each information element
