...
/**
 * <p>Title: Liveness and boundedness preserving reduction for Petri nets</p>
 * <p>Description: Reduces a Petri net given a well-known set of liveness and boundedness preserving rules.</p>
 * <p>T. Murata. Petri nets: Properties, analysis and applications. <em>Proceedings of the IEEE</em>, 77(4):541-580, April 1989.</p>
...
public class PetriNetReduction implements ConvertingPlugin {
    ...
    /**
     * The set of nodes not to reduce. These nodes are 'sacrosanct' for the reduction rules.
     * Nodes are kept in an ArrayList instead of HashSet for usability reasons.
     * The hashCode of a Transition object depends on the PetriNet it is contained
     * in and can therefore change AFTER it has been added to the sacroSanctNodes.
     */
     ...
     * @param object The given objects.
     * @return The reduced Petri net (the original Petri net if no reductions could be applied!)
     */
     ...
     * @param nodes The set of nodes not to reduce.
     */
     ...
     * @param providedPN The given Petri net.
    ...
    public PetriNet reduce(PetriNet originalPN, HashMap pnMap) {
        ...
        Message.add("Irreducible: "+ sacroSanctNodes.toString(), Message.DEBUG);
        ...
        while (count > 0 && reducedPN != providedPN) { // Some reductions could be applied, try again.
        ...
        // Note that if no reductions could be applied at all, then reducedPN equals providedPN.
        for (Object key: workMap.keySet()) {
    ...
    /*
     * The reduction rules cannot reduce a vertex with incident edges with weight exceeding 1.
     * Therefore, we need to test explicitly whether such edge exists.
     */

    /**
     * Checks whether all outgoing edges have weight 1.
     *
     * @param vertex ModelGraphVertex The given vertex
     * @return boolean Whether all outgoing edges have weight 1.
     */
    private boolean hasUniqueSuccessors(ModelGraphVertex vertex) {
        return vertex.getSuccessors().size() == (vertex.getOutEdges() == null ? 0 : vertex.getOutEdges().size());
    }
    ...
    private boolean hasUniquePredecessors(ModelGraphVertex vertex) {
        return vertex.getPredecessors().size() == (vertex.getInEdges() == null ? 0 : vertex.getInEdges().size());
    }
    ...
    public PetriNet reduceOnce(PetriNet providedPN, HashMap pnMap) {
        ...
        HashMap reducedTransitions = new HashMap() {
            public Object put(Object key, Object object) {
                Message.add("Map " + key.toString() + " to " + object.toString(), Message.DEBUG);
        ...
        while (it.hasNext()) {
            Place place = (Place) it.next();
            if ((notReducedPlaces.contains(place) || sacroSanctNodes.contains(place)) && // place not reduced
                    hasUniqueSuccessors(place) &&
                    place.getSuccessors().size() == 1) { // place has only one output transition
                Transition transition = (Transition) place.getSuccessors().iterator().next(); // Get the output transition
                if (notReducedTransitions.contains(transition) && // output transition is not reduced
                        hasUniquePredecessors(transition) &&
                        transition.getPredecessors().size() == 1 && // output transition has only one input place
                        hasUniqueSuccessors(transition) &&
                        transition.getSuccessors().size() == 1) { // output transition has only one output place
                    Place otherPlace = (Place) transition.getSuccessors().iterator().next(); // Get output place
                    if ((notReducedPlaces.contains(otherPlace) ||
                            (!sacroSanctNodes.contains(place) && sacroSanctNodes.contains(otherPlace))
                        ) &&
                        // if place is sacrosanct, then otherPlace is not allowed to have additional inputs.
                        (!sacroSanctNodes.contains(place) || (hasUniquePredecessors(otherPlace) && otherPlace.getPredecessors().size() == 1))
                    ) {
                        Message.add("FSP(" + place.toString() + "," + transition.toString() +
        ...
        while (it.hasNext()) {
            Transition transition = (Transition) it.next();
            if ((notReducedTransitions.contains(transition) || sacroSanctNodes.contains(transition)) && // transition not reduced
                    hasUniquePredecessors(transition) &&
                    transition.getPredecessors().size() == 1) { // transition has only one input place
                Place place = (Place) transition.getPredecessors().iterator().next(); // Get the input place
                if (notReducedPlaces.contains(place) && // output place not reduced
                        hasUniquePredecessors(place) &&
                        place.getPredecessors().size() == 1 && // input place has only one input transition
                        hasUniqueSuccessors(place) &&
                        place.getSuccessors().size() == 1) { // input place has only one output transition
                    Transition otherTransition = (Transition) place.getPredecessors().iterator().
                                                 next(); // Get input transition
                    if ((notReducedTransitions.contains(otherTransition) ||
                            (!sacroSanctNodes.contains(transition) && sacroSanctNodes.contains(otherTransition))
                        ) &&
                        (!sacroSanctNodes.contains(transition) || (hasUniqueSuccessors(otherTransition) && otherTransition.getSuccessors().size() == 1))
                    ) {
                        Message.add("FST(" + transition.toString() + "," + place.toString() +
                                "," + otherTransition.toString() + ")", Message.DEBUG);
                        if (sacroSanctNodes.contains(transition)) {
        ...
        while (it.hasNext()) {
            ...
            if ((notReducedPlaces.contains(place) || sacroSanctNodes.contains(place)) && // place not reduced
                    hasUniqueSuccessors(place) &&
                    place.getSuccessors().size() >= 1 && // place has at least one output transition
                    hasUniquePredecessors(place) &&
                    place.getPredecessors().size() >= 1) { // place has at least one input transition
                Transition transition = (Transition) place.getPredecessors().iterator().next(); // Get input transition
                Iterator it2 = transition.getSuccessors().iterator();
                while (it2.hasNext()) {
                    Place otherPlace = (Place) it2.next(); // For every output place of the input transition
                    if (place != otherPlace && // other place is not place
                            (notReducedPlaces.contains(otherPlace) ||
                             (!sacroSanctNodes.contains(place) && sacroSanctNodes.contains(otherPlace))
                            ) && // other place not reduced
                            otherPlace.getSuccessors().equals(place.getSuccessors()) && // other place has same output transitions
                            otherPlace.getPredecessors().equals(place.getPredecessors())) { // other place has same input transitions
                        Message.add("FPP(" + place.toString() + "," + transition.toString() +
                                "," + otherPlace.toString() + ")", Message.DEBUG);
                        if (sacroSanctNodes.contains(place)) {
        ...
        while (it.hasNext()) {
            Transition transition = (Transition) it.next();
            if (notReducedTransitions.contains(transition) && // transition not reduced
                    hasUniqueSuccessors(transition) &&
                    transition.getSuccessors().size() >= 1 && // transition has at least one output place
                    hasUniquePredecessors(transition) &&
                    transition.getPredecessors().size() >= 1) { // transition has at least one input place
                Place place = (Place) transition.getPredecessors().iterator().next(); // Get input place
                Place otherPlace = (Place) transition.getSuccessors().iterator().next(); // Get output place
                Iterator it2 = place.getSuccessors().iterator();
                while (it2.hasNext()) {
                    Transition otherTransition = (Transition) it2.next(); // For every output transition of the input place
                    if (transition != otherTransition && // other transition is not transition
                            !sacroSanctNodes.contains(otherTransition) && // other transition is not sacro sanct
                            otherTransition.getSuccessors().equals(transition.getSuccessors()) && // other transition also has only one output place
                            otherTransition.getPredecessors().equals(transition.getPredecessors())) { // other transition also has only one input place
                        Message.add("FPT(" + transition.toString() + "," + place.toString() +
                                "," + otherTransition.toString() + ")", Message.DEBUG);
                        // Reduce other transition to itself
                        notReducedTransitions.remove(otherTransition);
                        reducedTransitions.put(otherTransition, otherTransition);
                        nothingReduced = false;
                    }
                }
            }
        ...
        while (it.hasNext()) {
            ...
            if (notReducedPlaces.contains(place) && // place not reduced
                    ...
                    place.getSuccessors().size() == 1 && // place has only one output transition
                    hasUniquePredecessors(place) &&
                    place.getPredecessors().size() == 1 && // place has only one input transition
                    place.getNumberOfTokens() > 0) { // place contains tokens
                if (place.getSuccessors().iterator().next() ==
                        place.getPredecessors().iterator().next()) { // input transition equals output transition
        ...
        while (it.hasNext()) {
            ...
            if (notReducedTransitions.contains(transition) && // transition not reduced
                    hasUniqueSuccessors(transition) &&
                    transition.getSuccessors().size() == 1 && // transition has only one output place
                    hasUniquePredecessors(transition) &&
                    transition.getPredecessors().size() == 1) { // transition has only one input place
                if (transition.getSuccessors().iterator().next() ==
                        transition.getPredecessors().iterator().next()) { // input place equals output place
                    // Reduce transition onto itself
                    Message.add("ELT(" + transition.toString() + ")", Message.DEBUG);
                    notReducedTransitions.remove(transition);
                    reducedTransitions.put(transition, transition);
                    nothingReduced = false;
                }
        ...
        while (it.hasNext()) {
            Transition transition = (Transition) it.next();
            Transition mappedTransition = reducedPN.addAndLinkTransition(new Transition(transition));
            transitions.put(transition, mappedTransition);
            pnMap.put(transition, mappedTransition);
        }
                ...
                while (it2.hasNext()) {
                    ...
                    if (reducedPlaces.containsKey(place)) {
                        ...
                        while (reducedPlaces.containsKey(mappedPlace) &&
                                (mappedPlace != reducedPlaces.get(mappedPlace))) {
                            mappedPlace = (Place) reducedPlaces.get(mappedPlace);
                        }
                    } else {
                        mappedPlace = place;
                    }
                    ...
                    if (reducedTransitions.containsKey(transition)) {
                        mappedTransition = (Transition) reducedTransitions.get(transition);
                        while (reducedTransitions.containsKey(mappedTransition) &&
                                (mappedTransition != reducedTransitions.get(mappedTransition))) {
                            mappedTransition = (Transition) reducedTransitions.get(mappedTransition);
                        }
                    } else {
                        mappedTransition = transition;
                    }
                    Message.add("Transition " + transition.getIdentifier() + "->" +
                            mappedTransition.getIdentifier(), Message.DEBUG);
                    // Add transferred edge, if needed
                    if ((!reducedPlaces.containsKey(mappedPlace) ||
                            mappedPlace != reducedPlaces.get(mappedPlace)) &&
                            (!reducedTransitions.containsKey(mappedTransition) ||
                            mappedTransition != reducedTransitions.get(mappedTransition))) {
            ...
            if (place.getInEdges() != null) {
                ...
                while (it2.hasNext()) {
                    ...
                    if ((!reducedPlaces.containsKey(mappedPlace) ||
                            ...
                            (!reducedTransitions.containsKey(mappedTransition) ||
                            mappedTransition != reducedTransitions.get(mappedTransition))) {
                        reducedPN.addEdge((Transition) transitions.get(mappedTransition),
                                (Place) places.get(mappedPlace));
                    }
                }
            }
        }
