...
import org.prom5.framework.models.heuristics.HNSet;
import org.prom5.framework.models.heuristics.HNSubSet;
...
import org.prom5.importing.heuristicsnet.HeuristicsNetResultWithLogReader;
...
/**
 * <p>Title: Petri net to Heuristic net</p>
 *
 * <p>Description: This class converts Petri nets to Heuristic nets. The Petri net should have a single
 * start/end transition because a Heuristic net has a single start/end task</p>.
 * If this is not the case, one of the start/end transitions of the Petri net will be randomly
 * chosen to become the single start/end taks of the converted Heuristic net. Note that this
 * may lead to undesirable side-effects.
 *
 * <p><b>IMPORTANT!!!</b> This plug-in depends on the <code>PetriNetReduction</code> plug-in! The
...
public class PetriNetToHeuristicNetConverter implements ConvertingPlugin {
	...
	public MiningResult convert(ProvidedObject original) {
		...
		for (int i = 0; i < original.getObjects().length; i++) {
			petriNet = (original.getObjects()[i] instanceof PetriNet ?
					   (PetriNet) original.getObjects()[i] :
					   petriNet);
			log = ((original.getObjects()[i] instanceof LogReader) ?
				  ((LogReader) original.getObjects()[i]) :
				  log);
	 ...
	 * @param petriNet Petri net to reduce.
	 * @return PetriNet Reduced Petri net.
	 */

	public static PetriNet removeUnnecessaryInvisibleTasksFromPetriNet(PetriNet petriNet) {
	...
	public HeuristicsNet toHeuristicsNet(PetriNet pn) throws Exception {
		...
		//auxiliary structure that keeps track of the incidence matrices of the PN
		DoubleMatrix2D incidencenMatrix = pn.getIncidenceMatrix();
		IdentifyElementsIncidenceMatrices matrix = new IdentifyElementsIncidenceMatrices(pn.
				getInputArcsTransitionsIncidenceMatrix(),
				pn.getOutputArcsTransitionsIncidenceMatrix());

		//Checking if there are invisible transitions with multiple input/output places
		//If so, these invisible transitions are converted to visible one
		convertInvisibleTransitionsWithMultipleInputOutputPlacesToVisibleOnes(pn, matrix);

		//Getting the set of visiblet tasks in the PN.
		//These tasks are the tasks of the HN.
		ArrayList visibleTasks = pn.getVisibleTasks();

		Multiset ms = new HashMultiset(); //used to build an event log with the elements that are actually linked to a transition in the PN
		...
		for (int i = 0; i < events.size(); i++) {
			ArrayList transitionsLinkedToEvent = pn.findTransitions(events.get(i));
			for (int j = 0; j < transitionsLinkedToEvent.size(); j++) {
				mappingBetweenTransitionsInPNandHN.put((Transition) transitionsLinkedToEvent.get(j),
						indexDuplicatesMapping);
				duplicatesMapping[indexDuplicatesMapping++] = i;

			}
		}
		HNSubSet[] reverseDuplicatesMapping = HeuristicsNet.buildReverseDuplicatesMapping(
											  duplicatesMapping);
		hn = new HeuristicsNet(events, duplicatesMapping, reverseDuplicatesMapping);
		...
		for (int indexStartPlace = 0; indexStartPlace < startPlaces.size(); indexStartPlace++) {
			HNSubSet outputTransitions = matrix.getOutputTransitions(startPlaces.get(
										 indexStartPlace));
			for (int indexStartTransition = 0; indexStartTransition < outputTransitions.size();
											indexStartTransition++) {
				Transition t = (Transition) pn.getTransitions().get(outputTransitions.get(
							   indexStartTransition));
				startTransitions.add(findEquivalentTaskInHN(t, mappingBetweenTransitionsInPNandHN));
			}
		}
		...
		for (int indexEndPlace = 0; indexEndPlace < endPlaces.size(); indexEndPlace++) {
			HNSubSet inputTransitions = matrix.getInputTransitions(endPlaces.get(indexEndPlace));
			for (int indexEndTransition = 0; indexEndTransition < inputTransitions.size();
										  indexEndTransition++) {
				Transition t = (Transition) pn.getTransitions().get(inputTransitions.get(
							   indexEndTransition));
				endTransitions.add(findEquivalentTaskInHN(t, mappingBetweenTransitionsInPNandHN));
			}
		}
		//setting the start tasks with the correct codes for the mapped HN
		...
		//for these transitions. If the transitions are invisible, look further to get
		//the visible transitions that are connected to these invisible transitions.

		HNSubSet[] inputVisibleTransitionsForPlaceInPN = new HNSubSet[incidencenMatrix.columns()];
		HNSubSet[] outputVisibleTransitionsForPlaceInPN = new HNSubSet[incidencenMatrix.columns()];
		for (int placeInPN = 0; placeInPN < incidencenMatrix.columns(); placeInPN++) {
			inputVisibleTransitionsForPlaceInPN[placeInPN] = toVisibleInputTransitions(matrix.
					getInputTransitions(
					placeInPN),
					pn, events, matrix, mappingBetweenTransitionsInPNandHN);
			outputVisibleTransitionsForPlaceInPN[placeInPN] = toVisibleOutputTransitions(matrix.
		...
		for (int transitionCode = 0; transitionCode < transitionsInPN.size(); transitionCode++) {
			//for every visible transition, get its input places and output places
			Transition transitionInPN = (Transition) transitionsInPN.get(transitionCode);
			if (!transitionInPN.isInvisibleTask()) {
				//retrieving the input places of this visible transition...
				HNSubSet inputPlacesTransitionInPN = matrix.getInputPlaces(transitionCode);
				//identifying the respective code of this transition in the heuristics net
				int correspondingTaskInHN = findEquivalentTaskInHN(transitionInPN,
											mappingBetweenTransitionsInPNandHN);
				//adding the input elements of every input places of the transition in the PN
				//as input subsets of the corresponding task in the HN
				for (int i = 0; i < inputPlacesTransitionInPN.size(); i++) {
					insertInHeuristicNetSets(hn.getInputSets(), correspondingTaskInHN,
							inputVisibleTransitionsForPlaceInPN[inputPlacesTransitionInPN.get(i)]);
				}

				//retrieving the output places of this visible transition...
				HNSubSet outputPlacesTransitionInPN = matrix.getOutputPlaces(transitionCode);
				//adding the input elements of every input places of the transition in the PN
				//as input subsets of the corresponding task in the HN
				for (int i = 0; i < outputPlacesTransitionInPN.size(); i++) {
					insertInHeuristicNetSets(hn.getOutputSets(), correspondingTaskInHN,
							outputVisibleTransitionsForPlaceInPN[outputPlacesTransitionInPN.get(
							i)]);
				}
	 ...
	 * @param pn PetriNet
	 */
	private void convertInvisibleTransitionsWithMultipleInputOutputPlacesToVisibleOnes(PetriNet pn,
		...
		if (startPlaces.size() > 0) {
			...
			for (int i = 0; i < startPlaces.size(); i++) {
				HNSubSet outputTransitionsPlace = matrix.getOutputTransitions(startPlaces.get(i));
				for (int j = 0; j < outputTransitionsPlace.size(); j++) {
					Transition outputTransition = pn.getTransitions().get((outputTransitionsPlace.
												  get(j)));
					if (outputTransition.isInvisibleTask()) {
						hasInvisibleTransitionsConnectedToStartPlaces = true;
						outputTransition.setIdentifier(outputTransition.getIdentifier());
						outputTransition.setLogEvent(new LogEvent(outputTransition.getIdentifier(),
			...
			String message =
					"No starting tasks have been detected! \n" +
		...
		for (int i = 0; i < endPlaces.size(); i++) {
			HNSubSet inputTransitionsPlace = matrix.getInputTransitions(endPlaces.get(i));
			for (int j = 0; j < inputTransitionsPlace.size(); j++) {
				Transition inputTransition = pn.getTransitions().get((inputTransitionsPlace.get(j)));
				if (inputTransition.isInvisibleTask()) {
					hasInvisibleTransitionsConnectedToEndPlaces = true;
					inputTransition.setIdentifier(inputTransition.getIdentifier());
					inputTransition.setLogEvent(new LogEvent(inputTransition.getIdentifier(),
							LogStateMachine.COMPLETE));
				}
			}
		}
		...
		//invisible transitions with multiple input/output places are going to be made visible
		...
		for (int i = 0; i < transitions.size(); i++) {
			...
			if (t.isInvisibleTask()) {
				...
				if (t.inDegree() > 1 || t.outDegree() > 1) {
					...
					t.setLogEvent(new LogEvent(t.getIdentifier(), LogStateMachine.COMPLETE));
				}
			}
		}
		...
		if (hasInvisibleTransitionsConnectedToStartPlaces ||
			...
			String message =
					"Some of the invisible transitions in this Petri net are going to \n" +
...
//			showWarningMessageDialog(message);
	...
	private void insertInHeuristicNetSets(HNSet[] hnetSets, int taskToModifySets,
			HNSubSet tasksToAddToSets) {
	...
	private HNSubSet toVisibleInputTransitions(HNSubSet setTransitionCodes, PetriNet pn,
			LogEvents events,
			IdentifyElementsIncidenceMatrices matrix,
		...
		for (int i = 0; i < setTransitionCodes.size(); i++) {
			...
			if (t.isInvisibleTask()) {
				...
				HNSubSet inputPlacesInvisibleTransition = matrix.getInputPlaces(transitionCode);

				for (int j = 0; j < inputPlacesInvisibleTransition.size(); j++) {
					visibleTransitions.addAll(toVisibleInputTransitions(matrix.getInputTransitions(
							inputPlacesInvisibleTransition.get(j)), pn, events, matrix,
							mappingBetweenTransitionsInPNandHN));
				}

			} else { //transition is visible task. Add to set of visible transitions
				visibleTransitions.add(findEquivalentTaskInHN(t, mappingBetweenTransitionsInPNandHN));
			}
		}
	...
	private HNSubSet toVisibleOutputTransitions(HNSubSet setTransitionCodes, PetriNet pn,
			LogEvents events,
			IdentifyElementsIncidenceMatrices matrix,
			Hashtable mappingBetweenTransitionsInPNandHN) throws Exception {
		...
		for (int i = 0; i < setTransitionCodes.size(); i++) {
			...
			if (t.isInvisibleTask()) {
				...
				HNSubSet outputPlacesInvisibleTransition = matrix.getOutputPlaces(
						transitionCode);

				for (int j = 0; j < outputPlacesInvisibleTransition.size(); j++) {
					visibleTransitions.addAll(toVisibleOutputTransitions(matrix.
							getOutputTransitions(
							outputPlacesInvisibleTransition.get(j)), pn, events, matrix,
							mappingBetweenTransitionsInPNandHN));
				}
			} else { //transition is visible task. Add to set of visible transitions
				visibleTransitions.add(findEquivalentTaskInHN(t, mappingBetweenTransitionsInPNandHN));
...
class IdentifyElementsIncidenceMatrices {
	...
	public IdentifyElementsIncidenceMatrices(DoubleMatrix2D incidenceMatrixForInputArcsTransitions,
	...
	public HNSubSet getStartPlaces() {
		...
		for (int place = 0; place < incidenceMatrixForOutputArcsTransitions.columns(); place++) {
			isStartPlace = true;
			for (int transition = 0; transition < incidenceMatrixForOutputArcsTransitions.rows();
								  transition++) {
	...
	public HNSubSet getEndPlaces() {
		...
		for (int place = 0; place < incidenceMatrixForInputArcsTransitions.columns(); place++) {
			isEndPlace = true;
			for (int transition = 0; transition < incidenceMatrixForInputArcsTransitions.rows();
								  transition++) {
