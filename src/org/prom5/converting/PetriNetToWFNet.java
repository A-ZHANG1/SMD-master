...
/**
 * <p>Title: PetriNetToWFNet</p>
 *
 * <p>Description: Converts a PetriNet object into a WFNet object (by specifying the source place and the sink place).
 * Additional source and sink places will be left out.</p>
 *
	...
	public WFNet convert(PetriNet source) {
		...
		if (sourcePlaces.size() > 1) {
			for (Place place: sourcePlaces) {
				Transition transition = new Transition(prefix + "i_" + place.getIdentifier(), target);
		...
		if (sinkPlaces.size() > 1) {
			for (Place place: sinkPlaces) {
				Transition transition = new Transition(prefix + place.getIdentifier() + "_o", target);
				target.addTransition(transition);
