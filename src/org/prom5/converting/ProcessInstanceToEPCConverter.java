	...
	public MiningResult convert(ProvidedObject original) {
		...
		while ((pi == null) && (k < original.getObjects().length)) {
			if ((original.getObjects()[k] instanceof ProcessInstance) &&
					(((ProcessInstance) original.getObjects()[k]).getAttributes().containsKey(
					ProcessInstance.ATT_PI_PO) &&
					((ProcessInstance) original.getObjects()[k]).getAttributes().get(
		...
		while (i.hasNext()) {
			...
			EPCFunction f = new EPCFunction(new LogEvent(ate.getElement(), ate.getType()), net);
			net.addFunction(f);
		...
		while (i.hasNext()) {
			...
			if (!st.hasMoreTokens()) {
				...
				net.addEdge((EPCFunction) mapping.get(ate.getAttributes().get(ProcessInstance.
						ATT_ATE_ID)), e);
			} else {
		...
		net = ConnectorStructureExtractor.extract(net,true,false,false,false,false,false,false,true,false,false);
	...
	public boolean accepts(ProvidedObject original) {
		...
		while (!b && (i < original.getObjects().length)) {
			b |= (original.getObjects()[i] instanceof ProcessInstance) &&
					(((ProcessInstance) original.getObjects()[i]).getAttributes().containsKey(
					ProcessInstance.ATT_PI_PO) &&
					((ProcessInstance) original.getObjects()[i]).getAttributes().get(
					ProcessInstance.ATT_PI_PO).equals("true"));
			i++;
		}
		return b;
