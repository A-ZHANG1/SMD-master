...
public class ProcessInstanceToPetriNetConverter implements ConvertingPlugin {
	...
	public MiningResult convert(ProvidedObject original) {
		...
		while (i.hasNext()) {
			...
			Transition t = new Transition(new LogEvent(ate.getElement(), ate.getType()), net);
			net.addTransition(t);
		...
		while (i.hasNext()) {
			...
			while (st.hasMoreTokens()) {
				...
				net.addEdge((Transition) mapping.get(ate.getAttributes().get(ProcessInstance.ATT_ATE_ID)), p);
				net.addEdge(p, t);
		...
		if (start.outDegree() > 1) {
			...
			Transition st = net.addTransition(new Transition((LogEvent)null, net));
