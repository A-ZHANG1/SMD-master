...
/**
 * <p>Title: ProtosToEPC</p>
 *
 * <p>Description: Converts a Protos subprocess into an EPC model.</p>
 *
...
public class ProtosToEPC implements ConvertingPlugin {
	 ...
	 * @param object ProvidedObject Should contain a Protos subprocess.
	 ...
	 * @param object ProvidedObject The provided objects.
	...
	public ConfigurableEPC result(ProtosSubprocess process) {
		...
		// Convert every activity into a chain of a join connector, an event, a function, and a split connector.
		...
		for (ProtosFlowElement activity : activities) {
			...
			EPCFunction function = new EPCFunction(new LogEvent(activity.getName(), "complete"), epc);
			epc.addFunction(function);
			EPCEvent event = new EPCEvent("status change to " + activity.getName(), epc);
			epc.addEvent(event);
			...
			// Add the connectors as point of entry and point of exit for this chain.
			pointsOfEntry.put(activity.getID(), joinConnector);
			pointsOfExit.put(activity.getID(), splitConnector);
		}

	    // Convert every status into a chain of a join connector and a split conenctor.
		HashSet<ProtosFlowElement> statuses = process.getStatuses();
		for (ProtosFlowElement status: statuses) {
			// Similar to activities, except for the fact that we do not add an event and a function, and that
	    ...
	    // Some connectors will have no outgoing edges. Add a final event to these connectors.
	    ...
	    // Reduce the connectors as much as possible, while leaving the events and functions in tact.
		...
		Message.add("<ProtosToEPC nofFunctions=\"" + nofFunctions +
				"\" nofEvents=\"" + nofEvents +
				"\" nofOrs=\"" + nofConnectors[EPCConnector.OR] +
				"\" nofAnds=\"" + nofConnectors[EPCConnector.AND] +
				"\" nofXors=\"" + nofConnectors[EPCConnector.XOR] +
				"\" nofEdges=\"" + nofEdges +
				"\"/>", Message.TEST);
