...
/**
 * <p>Title: Labeled WF Net to EPC convertor</p>
 *
 * <p>Description: Takes a labeled WF-net and converts it into an EPC. This convertor heavily depends on the convertor form labeled WF nets to YAWL models.</p>
 *
 * <p>Copyright: Copyright (c) 2004 Eric Verbeek</p>
 *
 * <p>Company: TU/e</p>
 *
...
public class WFNetToEPC implements ConvertingPlugin {
	...
	public MiningResult convert(ProvidedObject object) {
		...
		for (Transition transition : providedPN.getTransitions()) {
			if (transition.getPredecessors().size() != (transition.getInEdges() != null ? transition.getInEdges().size() : 0)) {
				hasArcWeights = true;
			} else if (transition.getSuccessors().size() != (transition.getOutEdges() != null ? transition.getOutEdges().size() : 0)) {
				hasArcWeights = true;
			}
		}
		if (hasArcWeights) {
			Message.add("Plug-in \"" + getName() + "\"", Message.WARNING);
			Message.add("  Petri-net \"" + providedPN.getName() + "\" contains arc weights that exceed one.", Message.WARNING);
			Message.add("  These arc weights will be ignored by this plug-in, that is, assumed to be one.", Message.WARNING);
		}
	 ...
	 * @param element ModelGraphVertex THe given object.
	 ...
	 * @param net PetriNet The given Petri net.
	...
	public ConfigurableEPC convert(PetriNet net) {
		...
		for (Transition transition: net.getTransitions()) {
			if (!transition.isInvisibleTask()) {
				...
				// A YAWL task is broken down into a join connector, an event, a function, and a split connector.
				EPCFunction function = new EPCFunction(transition.getLogEvent(), false, epc);
				if (transition.getLogEvent() == null) {
					function.setIdentifier(transition.getIdentifier());
				}
				epc.addFunction(function);
				EPCEvent event = new EPCEvent("Status change to " + getName(transition), epc);
				epc.addEvent(event);
				...
				if (transition.getSuccessors().isEmpty()) {
					EPCEvent completedEvent = new EPCEvent("Completed " + getName(transition), epc);
		...
		while (!transitions.isEmpty()) {
			...
			// A YAWL router is broken down into a join connector and a split connector.
			EPCConnector joinConnector = null;
			String joinType = getType(transition, joinSphere, true);
			if (joinType.compareTo("and") == 0) {
				joinConnector = new EPCConnector(EPCConnector.AND, epc);
		...
		/*
		 ...
		 HashSet joinSphere = (HashSet) joinSpheres.get(transition);
		 HashSet splitSphere = (HashSet) splitSpheres.get(transition);
		 decomposition.addTask(getName(transition), getType(transition, joinSphere, true), getType(transition, splitSphere, false), "Task");
		   }
		 */
		...
		for (Place place: places) {
			if (place.getPredecessors().isEmpty()) {
				...
				if (place.getSuccessors().size() == 1) {
					Transition transition = (Transition) place.getSuccessors().iterator().next();
					if (!transition.isInvisibleTask()) {
				...
				if (needStartEvent) {
					...
					epc.addEvent(event);
					epc.addEdge(event, function);
					EPCConnector splitConnector = new EPCConnector(EPCConnector.XOR, epc);
					epc.addConnector(splitConnector);
					epc.addEdge(function, splitConnector);
					pointsOfExit.put(place, splitConnector);
				}
			} else if (place.getSuccessors().isEmpty()) {
				EPCConnector joinConnector = new EPCConnector(EPCConnector.XOR, epc);
				epc.addConnector(joinConnector);
		...
		for (Object joinTransition: joinSpheres.keySet()) {
			for (Object splitTransition: splitSpheres.keySet()) {
				...
				if (connect(splitSphere, joinSphere)) {
					EPCConnector pointOfExit = (EPCConnector) pointsOfExit.get(splitTransition);
					EPCConnector pointOfEntry = (EPCConnector) pointsOfEntry.get(joinTransition);
					if (pointOfExit != null && pointOfEntry != null) {
						...
						pointsOfEntry.put((PNNode)joinTransition, pointOfEntry);
					}
				}
				if (joinTransition == splitTransition) {
		...
		for (Object transition: splitSpheres.keySet()) {
			for (Place place: places) {
				...
				HashSet<PNNode> joinSphere = (HashSet<PNNode>) joinSpheres.get(transition);
				HashSet<PNNode> splitSphere = (HashSet<PNNode>) splitSpheres.get(transition);
				if (connect(splitSphere, placeSphere)) {
					EPCConnector pointOfExit = (EPCConnector) pointsOfExit.get(transition);
					EPCConnector pointOfEntry = (EPCConnector) pointsOfEntry.get(place);
					if (pointOfExit != null && pointOfEntry != null) {
				...
				if (connect(placeSphere, joinSphere)) {
					EPCConnector pointOfExit = (EPCConnector) pointsOfExit.get(place);
					EPCConnector pointOfEntry = (EPCConnector) pointsOfEntry.get(transition);
					if (pointOfExit != null && pointOfEntry != null) {
		...
		// Remove any source connect. because we did not add a new start event (and function), any original start event will generate a source connector.
		...
		return ConnectorStructureExtractor.extract(epc, true, false, false, false, false, false, false, true, false, false);
		//return epc;
	}

	/**
	 * Construct the join/split sphere for the given transition in the given Petri net.
	 * @param net PetriNet The given Petri net.
	 * @param labeledTransition Transition The given transition.
	 * @param isJoin boolean Whether to construct the join sphere (true) or a split sphere (false).
	 * @return HashSet
	 */
	private HashSet<PNNode> getSphere(PetriNet net, Transition labeledTransition, boolean isJoin) {
	 ...
	 * @param sphereTransition Transition The given transition.
	 * @param sphere HashSet Its join/split sphere (depends on isJoin).
	 * @param isJoin boolean Whether the join split is requested (true) or the split type (false).
	 * @return String
	 */
	public String getType(Transition sphereTransition, HashSet<PNNode> sphere, boolean isJoin) {
	 ...
	 * @param splitSphere HashSet The split sphere of 'one YAWL object'.
	 * @param joinSphere HashSet The split sphere of 'another YAWL object'.
	 * @return boolean Whether there should be an edge.
	 */
	private boolean connect(HashSet<PNNode> splitSphere, HashSet<PNNode> joinSphere) {
