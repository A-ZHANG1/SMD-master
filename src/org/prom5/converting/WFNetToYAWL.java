...
/**
 * <p>Title: Labeled WF Net to YAWL model convertor</p>
 *
 * <p>Description: Takes a labeled WF-net and converts it into a YAWL model.</p>
 *
...
public class WFNetToYAWL implements ConvertingPlugin {
	 ...
	 * @return String X\tY, where X is the object's name and Y makes the name unique. Y will be stripped from the name at an appropriate time, leaving X.
	 */
	public String getName(ModelGraphVertex element) {
		String name = element.getIdentifier() + '\t' + String.valueOf(element.getId());
		return name;
	}
	...
	public YAWLModel convert(PetriNet net) {
		String uri = net.getIdentifier().replaceAll(" ", ""); // Remove spaces, as these ar enot allowed in an URI.
		...
		YAWLDecomposition decomposition = new YAWLDecomposition("labeledWFNet", "true",
										  "NetFactsType");
		model.addDecomposition("labeledWFNet", decomposition);

		// Add the decomposition that will be used for all transitions
		YAWLDecomposition taskDecomposition = new YAWLDecomposition("Task", "false",
		...
		while (it.hasNext()) {
			...
			if (!transition.isInvisibleTask()) {
				HashSet<PNNode> joinSphere = getSphere(net, transition, true, isSimple);
				HashSet<PNNode> splitSphere = getSphere(net, transition, false, isSimple);
				...
				decomposition.addTask(getName(transition), getType(transition, joinSphere, true),
						getType(transition, splitSphere, false), "Task", transition.getLogEvent());
			}
		}
		...
		while (!transitions.isEmpty()) {
			...
			transitions.removeAll(splitSphere);
			decomposition.addTask(getName(transition), getType(transition, joinSphere, true),
					getType(transition, splitSphere, false), "Router", null);
			needsRouter = true;
		}
		if (needsRouter) {
			YAWLDecomposition routerDecomposition = new YAWLDecomposition("Router", "false",
													"WebServiceGatewayFactsType");
			...
			while (it2.hasNext()) {
				...
				if (connect(splitSphere, joinSphere)) {
					...
					if (map.containsKey(getName(splitTransition))) {
						...
						decomposition.addEdge(
								getName(splitTransition),
								getName(joinTransition),
								false,
								"true",
						...
						decomposition.addEdge(
								getName(splitTransition),
								getName(joinTransition),
								splitType.equals("xor") || splitType.equals("or"),
								splitType.equals("or") ? "true" : null,
								splitType.equals("xor") ? ordering : null);
					}
	 ...
	 * @return HashSet
	 */
	public static HashSet<PNNode> getSphere(PetriNet net, Transition labeledTransition, boolean isJoin, boolean isSimple) {
		...
		while (!done) {
			...
			} while (it.hasNext() && (!isSimple || !labeledTransition.isInvisibleTask())) {
				transition = (Transition) it.next();
				if (!sphere.contains(transition) && transition.isInvisibleTask()) {
					if (isJoin) {
	 ...
	 * @return String
	 */
	public static String getType(Transition sphereTransition, HashSet<PNNode> sphere, boolean isJoin) {
		...
		while (it.hasNext() && (isNone || isAnd || isXor)) {
			...
			if (object instanceof Place) {
				...
				if (place.getPredecessors().size() != 1) { // If place with multiple inputs
				...
				if (place.getSuccessors().size() != 1) { // If place has multiple outputs
				...
				if ((transition != sphereTransition || isJoin) && // Ignore inputs of sphere transition if join sphere
						transition.getPredecessors().size() != 1) { // If transition has multiple inputs
				...
				if ((transition != sphereTransition || !isJoin) && // Ignore outputs of sphere transition if split sphere
						transition.getSuccessors().size() != 1) { // If transition has multiple outputs
	 ...
	 */
	public static boolean connect(HashSet<PNNode> splitSphere, HashSet<PNNode> joinSphere) {
