...
public class YAWLToEPC implements ConvertingPlugin {
	 ...
	 * @param model the YAWL model to convert
	...
	public ConfigurableEPC convert(YAWLModel model) {
		...
		while (it.hasNext()) {
			...
			if (decomposition.isRoot()) {
				...
				addDecomposition(epc, model, "/", decomposition, true, ancestors);
				ancestors.remove(decomposition.getName());
			}
		}

		HashMap epcMap = new HashMap();
		epc = ConnectorStructureExtractor.extract(epc, true, false, false, false, false, false, false, true, false, false, epcMap);

	    //HV: Possibly, mapped object were removed during the previous reduction phase. Remove these mapped objects.
		...
		for (YAWLCondition condition: conditionChoiceMapping.keySet()) {
			...
			if (epcMap.keySet().contains(oldConnector.getIdKey())) {
				EPCConnector newConnector = (EPCConnector) epcMap.get(oldConnector.getIdKey());
		...
		for (YAWLTask task: taskActivityMapping.keySet()) {
			...
			if (epcMap.keySet().contains(oldFunction.getIdKey())) {
				EPCFunction newFunction = (EPCFunction) epcMap.get(oldFunction.getIdKey());
		...
		while (it.hasNext()) {
			...
			if (connector.getPredecessors().isEmpty() || connector.getSuccessors().isEmpty()) {
	...
	private void addDecomposition(ConfigurableEPC epc, YAWLModel model, String parentId,
			YAWLDecomposition decomposition, boolean isRoot, HashMap ancestors) {
			...
			if (object instanceof YAWLCondition) {
				...
				if (condition.getPredecessors().isEmpty()) {
					...
					if (condition.getSuccessors().size() == 1) {
						YAWLTask task = (YAWLTask) condition.getSuccessors().iterator().next();
					...
					//Message.add("put entry " + parentId + "[]", Message.DEBUG);
					pointsOfExit.put(parentId + "/" + String.valueOf(condition.getId()),
							splitConnector);
					//Message.add("put exit " + parentId + "/" + String.valueOf(condition.getId()), Message.DEBUG);
					...
					pointsOfEntry.put(parentId + "/" + String.valueOf(condition.getId()),
							joinConnector);
					//Message.add("put entry " + parentId + "/" + String.valueOf(condition.getId()), Message.DEBUG);
					pointsOfExit.put(parentId + "[]", joinConnector);
					//Message.add("put exit " + parentId + "[]", Message.DEBUG);
					if (isRoot) {
					...
					//HV: Remember that this condition is mapped onto this split connector.
		...
		while (it.hasNext()) {
			...
			if (object instanceof YAWLTask) {
				...
				if (model.isComposite(task.getDecomposition()) &&
					...
					YAWLDecomposition taskDecomposition = model.getDecomposition(task.
							getDecomposition());
					ancestors.put(task.getDecomposition(), taskId + "[]");
					addDecomposition(epc, model, taskId, taskDecomposition, false, ancestors);
					ancestors.remove(task.getDecomposition());
					String decompositionId = taskId + "[]";
					EPCConnector pointOfEntry = pointsOfEntry.get(decompositionId);
					//Message.add("get entry " + decompositionId + (pointOfEntry == null ? " failed" : " succeeded"), Message.DEBUG);
					EPCConnector pointOfExit = pointsOfExit.get(decompositionId);
					//Message.add("get exit " + decompositionId + (pointOfExit == null ? " failed" : " succeeded"), Message.DEBUG);
					epc.addEdge(joinConnector, pointOfEntry);
					epc.addEdge(pointOfExit, splitConnector);

					//HV: Remember that this task is mapped onto this splitConnector.
					//HV: Composite task is no tmapped onto a single function.
					xortaskChoiceMapping.put(task, splitConnector);
				} else {
					EPCEvent event = new EPCEvent("status change to " + task.getIdentifier(), epc);
					EPCFunction function = new EPCFunction(task.getLogEvent(), epc);
					...
					//HV: Remember that this task is mapped onto this function and this splitConnector.
		...
		while (it.hasNext()) {
			...
			if (object instanceof YAWLTask) {
				...
				//Message.add("get entry " + taskId + (pointOfEntry == null ? " failed" : " succeeded"), Message.DEBUG);
				Iterator it2 = task.getPredecessors().iterator();
				while (it2.hasNext()) {
					Object object2 = it2.next();
					if (decomposition.hasNormalEdges((YAWLNode) object2, (YAWLNode) object)) {
						String object2Id;
						if (object2 instanceof YAWLTask) {
							YAWLTask task2 = (YAWLTask) object2;
							object2Id = parentId + "/" + String.valueOf(task2.getId());
						} else {
							YAWLCondition condition2 = (YAWLCondition) object2;
							object2Id = parentId + "/" + String.valueOf(condition2.getId());
						}
						pointOfExit = pointsOfExit.get(object2Id);
						//Message.add("get exit " + object2Id + (pointOfExit == null ? " failed" : " succeeded"), Message.DEBUG);
						epc.addEdge(pointOfExit, pointOfEntry);
						//Message.add(pointOfExit.toString() + " -> " + pointOfEntry.toString());
					}
				}

				pointOfExit = pointsOfExit.get(taskId);
				//Message.add("get exit " + taskId + (pointOfExit == null ? " failed" : " succeeded"), Message.DEBUG);
				it2 = task.getSuccessors().iterator();
				while (it2.hasNext()) {
					Object object2 = it2.next();
					if (decomposition.hasNormalEdges((YAWLNode) object, (YAWLNode) object2)) {
						String object2Id;
						if (object2 instanceof YAWLTask) {
							YAWLTask task2 = (YAWLTask) object2;
							object2Id = parentId + "/" + String.valueOf(task2.getId());
						...
						//Message.add("get entry " + object2Id + (pointOfEntry == null ? " failed" : " succeeded"), Message.DEBUG);
						epc.addEdge(pointOfExit, pointOfEntry);
						//Message.add(pointOfExit.toString() + " -> " + pointOfEntry.toString());
					}
				}
