...
package org.prom5.converting.epc2transitionsystem;
...
public class EPCMarking implements Cloneable {
	...
	public EPCMarking(ConfigurableEPC Epc, Vector<EPCEdge> negativeArcs, Vector<EPCEdge> positiveArcs){
		EPC = Epc;
	...
	public void propagateDeadThenWaitContext(){
		...
		while (!deadCandidates.isEmpty()){
			...
			if (node instanceof EPCFunction || node instanceof EPCEvent){
					...
					while (outs.hasNext()){
						...
						if (Context.get(edge)== WAIT &&
						   ...
						   (!negativeTokens.contains(edge))){
							setDeadContext(edge);
							if (!deadCandidates.contains((EPCObject)edge.getDest())){
				...
				//String type = node.toString().toLowerCase().substring(0,node.toString().indexOf("-")).trim();
				if (node instanceof EPCConnector && (((EPCConnector) node).getType() == EPCConnector.AND) ){
					int inContext = Context.get(ins.next());
					while (ins.hasNext()){
						int nextContext = Context.get(ins.next());
						if (nextContext != inContext){
				...
				if (inContext == WAIT){
					while (outs.hasNext()){
						...
						if (Context.get(edge)== DEAD &&
						   ...
						   (!negativeTokens.contains(edge))){
							setWaitContext(edge);
							if (!waitCandidates.contains((EPCObject)edge.getDest())){
			...
			for (EPCObject node:negativeCandidates){
				...
				if (isenabled){
					...
					while (outedges.hasNext()){
						newNegativeCandidates.add((EPCObject)outedges.next().getDest());
					}
					hasfired = true;
				}
			}
	...
	public ArrayList<EPCMarking> propagatePositiveTokens(TransitionSystem TS,EPCMarking old){
		...
		while (!positiveCandidates.isEmpty()){
			...
			String type = node.toString().toLowerCase().substring(0,indexof).trim();
			if (node instanceof EPCFunction || node instanceof EPCEvent ||
			...
			if (node instanceof EPCConnector && type.equals("or") ){
				...
				while (ins.hasNext()){
					...
					if (getState(nextedge) == EMPTY && Context.get(nextedge) == WAIT){
						isenabled = false;
						break;
					}
				}
		...
		if (isAnyEnabled == false) {
			TransitionSystemVertexSet newone = new TransitionSystemVertexSet(old.toHashSet(),"",TS);
	...
	public ArrayList<EPCMarking> firePositive(EPCObject node,TransitionSystem TS,EPCMarking old){
		...
		//String type = node.toString().toLowerCase().substring(0,indexof).trim();
		if (node instanceof EPCFunction || node instanceof EPCEvent ||
				(node instanceof EPCConnector && (((EPCConnector) node).getType() == EPCConnector.AND)) ){
			while (ins.hasNext()){
				EPCEdge nextedge = (EPCEdge) ins.next();
			...
			TR = new EPCTransitionRelation(old.toHashSet(),nextMarking.toHashSet(),label);
			if (TR.addToTransitionSystem(TS) ){
				newMarkings.add(nextMarking);
			}
		}
		if (node instanceof EPCConnector && (((EPCConnector) node).getType() == EPCConnector.XOR)){
			while (ins.hasNext()){
				EPCEdge nextedge = (EPCEdge) ins.next();
				nextMarking.removeNegative(nextedge);
				nextMarking.setDeadContext(nextedge);
			...
			while (ins2.hasNext()){
				...
				if (nextMarking.getState(nextedge)==POSITIVE){
					...
					while (outs2.hasNext()){
						...
						TR = new EPCTransitionRelation(old.toHashSet(),thisMarking.toHashSet(),label);
						if (TR.addToTransitionSystem(TS) ){
		...
		if (node instanceof EPCConnector && (((EPCConnector) node).getType() == EPCConnector.OR)){
			while (ins.hasNext()){
				EPCEdge nextedge = (EPCEdge) ins.next();
				nextMarking.removeNegative(nextedge);
				nextMarking.removePositive(nextedge);
