package org.prom5.converting.fsm;
...
import org.prom5.framework.models.fsm.FSMPayload;
...
/**
 * <p>Title: FSsmModificationConvertor</p>
 *
 * <p>Description: Several conversions for FSMs that might be nice to use after having mined an FSM.</p>
 *
...
public class FsmModificationConvertor {
    private static HashMap<AcceptFSM,
                           FsmModificationSettings> settingsMap = new
            HashMap<AcceptFSM, FsmModificationSettings>();

    /**
     * Not implemented yet. This conversion plug-in is embedded into the FSM mining plug-in.
     * @param fsm AcceptFSM
     * @return JComponent
     */
     ...
     * @param fsm AcceptFSM The given FSM.
     * @param settings FsmModificationSettings The given settings.
    ...
    public AcceptFSM convert(AcceptFSM fsm, FsmModificationSettings settings) {
        ...
        /**
         * Repeat converting the FSM until no conversion yields results any more.
         */
        while (!done) {
            done = true;
            /**
             * If applicable, remove self loops. Reduces number of edges on actual removal.
             */
            if (settings.getUse(FsmModificationSettings.KILLSELFLOOPSTRATEGY)) {
                ...
                if (oldSize != fsm.getNumberOfEdges()) {
                    Message.add("Removed " + (oldSize - fsm.getNumberOfEdges()) + " edges");
                    done = false;
                }
            }
            /**
             * If applicable, improve the diamond structure. Increases number of edges on actual improvement.
             */
            if (settings.getUse(FsmModificationSettings.EXTENDSTRATEGY)) {
                oldSize = fsm.getNumberOfEdges();
                fsm = improveDiamondStructure(fsm);
                if (oldSize != fsm.getNumberOfEdges()) {
                    Message.add("Added " + (fsm.getNumberOfEdges() - oldSize) + " edges");
                    done = false;
                }
            }
            /**
             * If applicable, merge states if outputs are identical. Reduces number of states on actual marge.
             */
            if (settings.getUse(FsmModificationSettings.MERGYBYOUTPUTSTRATEGY)) {
                ...
                if (oldSize != fsm.getVerticeList().size()) {
                    Message.add("Removed " + (oldSize - fsm.getVerticeList().size()) + " nodes");
                    done = false;
                }
            }
            /**
             * If applicable, merge states if inputs are identical. Reduces number of states on actual merge.
             */
        ...
        for (Object object : fsm.getEdges()) {
            ...
            if (fromState != toState) {
                ...
                if (fsm.getStartStates().contains(toState)) {
                	...
                	newFsm.addTransition(newFromState, newToState,
    ...
    public AcceptFSM improveDiamondStructure(AcceptFSM fsm) {
        ...
        /**
         * Second, check whether we can improve the diamond structure by adding edges.
         */
        for (Object object : fsm.getEdges()) {
            FSMTransition transition = (FSMTransition) object;
            FSMState fromState = (FSMState) transition.getSource();
            ...
            /**
             * fromState: state from which we can do actions A and B (A might be identical to B).
             * toState: state we reach after doing B in fromState.
             * toState2: state we reach after doing A in fromState.
             * toState3: state we reach after doing B in toState2.
             * If we cannot reach toState3 from toState by doing an A, an A edge will be added from toState to toState3.
             */
            for (Object object2 : fromState.getOutEdges()) {
                ...
                for (Object object3 : toState2.getOutEdges()) {
                    ...
                    if (!transition.getCondition().equals(transition3.getCondition())) {
                        continue;
                    }
                    ...
                    for (Object object4 :
                        ...
                        if (transition4.getDest() == toState3) {
                            if (transition4.getCondition().equals(transition2.getCondition())) {
                                found = true;
                            }
                        }
                    }
                    if (!found) {
                        newFsm.addTransition(toState, toState3, transition2.getCondition());
                    }
                }
            }
        }
    ...
    /**
     * Returns a copy of the given FSM with all state with identical outputs merged.
     * @param fsm AcceptFSM
     * @return AcceptFSM
     */
    public AcceptFSM mergeByOutput(AcceptFSM fsm) {
        ...
        for (ModelGraphVertex vertex : fsm.getVerticeList()) {
            ...
            if (outputMap.containsKey(outgoing.toString())) {
                ...
                FSMPayload payload = newState.getPayload().merge(state.
                        getPayload());
                newState.setPayload(payload);
            } else {
                newState = new FSMState(newFsm, state.getLabel(),
                                        state.getPayload());
    ...
    /**
     * Returns a copy of the given FSM with all state with identical inputs merged.
     * @param fsm AcceptFSM
     * @return AcceptFSM
     */
