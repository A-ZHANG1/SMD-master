...
/**
 * <p>Title: oWFNetToWFNet</p>
 *
 * <p>Description: Converts an open WF ne tinto a WF net, by leaving out the communication places.
 * Note that according to the Berlin guys an open WF net is allowed to have multiple sink places.
 * However, we assume that an open WF net has only one sink place.</p>
 *
...
public class oWFNetToWFNet implements ConvertingPlugin {
	...
	public WFNet convert(oWFNet source) {
		...
		if (source.getSourcePlace() != null) {
			Place sourcePlace = new Place(source.getSourcePlace().getName(), target);
