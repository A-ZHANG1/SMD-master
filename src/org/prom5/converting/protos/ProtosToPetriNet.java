package org.prom5.converting.protos;
...
public class ProtosToPetriNet {
    /*
     * Prefixes to be used for transitions added for XOR-split and XOR-join translation
     */
    ...
    @Converter(name = "Protos: Convert to Petri net",
               help = "http://prom.win.tue.nl/research/wiki/online/protos2pn")
    ...
    /**
     * Convert the given Protos model to a Petri net.
     * Precondition: All implicit statusses have been added to the Protos model.
     * @param model Protos The given Protos model.
     * @param map HashMap The map to store the Protos-PN relations in.
     * Protos statusses are mapped onto a place. Protos Activities are mapped onto transitions.
     * Protos activities with XOR split are split up to include the choice behavior in the Petri net.
     * @return PetriNet
     */
    public static PetriNet convert(
        ...
        for (ProtosSubprocess process : model.getSubprocesses()) {
            if (process.isRoot()) {
                ...
                for (ProtosFlowElement act : process.getActivities()) {
                    ...
                    task.setLogEvent(new LogEvent(task.getIdentifier(), "complete"));
                ...
                for (ProtosProcessArc arc : process.getArcs()) {
                    ProtosFlowElement fromNode = (ProtosFlowElement) process.getFlowElement(arc.getSource());
                    ProtosFlowElement toNode = (ProtosFlowElement) process.getFlowElement(arc.getTarget());

                    if (fromNode.isStatus() && toNode.isActivity() && toNode.getJoinType() == YAWLTask.AND) {
                        ...
                        petriNet.addAndLinkEdge(pnEdge, pointOfExit, pointOfEntry);
                        map.put(arc, pnEdge);
                    }

                    if (fromNode.isActivity() && toNode.isStatus() && fromNode.getSplitType() == YAWLTask.AND) {
                ...
                /*
                 * For activities with an XOR-split and/or XOR-join semantics additional
                 * transitions and connections are added to include the XOR behavior.
                 */
                for (ProtosFlowElement act : process.getActivities()) {
                    if (act.getJoinType() == YAWLTask.XOR) {
                     // First, create a new place where the actual join is made in the petrinet.
                     Place joinPlace = new Place(JOIN_STRING + "_" + act.getIdentifier(), petriNet);
                     petriNet.addAndLinkPlace(joinPlace);
                     // connect task corresponding to node with XOR-join to the join place.
                    ...
                    if (act.getSplitType() == YAWLTask.XOR) {
                        // First, create a new place where the actual split is made in the petrinet.
                        Place splitPlace = new Place(SPLIT_STRING + "_" + act.getIdentifier(), petriNet);
                        petriNet.addAndLinkPlace(splitPlace);
                        // connect task corresponding to node with XOR-split to the split place.
                        Transition trans = (Transition) map.get(act);
                     ...
                     */
                    if (act.getSplitType() == YAWLTask.OR || act.getJoinType() == YAWLTask.OR) {
                        Message.add("The Protos activity with ID: " + act.getID().toString() +
                                    "has OR semantics which is not supported by this converter.", Message.ERROR);
                    }
                }
            }
        }
        ...
        /*
         * Finally, add a place to the start and end transitions
         * if the process starts / ends with transition(s).
         */
        for (ModelGraphVertex startVertex : petriNet.getStartNodes()) {
            ...
            if (petriNet.findTransition(startNode.getIdentifier()) != null) {
                Place startPlace = new Place("START_" + startNode.getIdentifier(), petriNet);
        ...
        for (ModelGraphVertex endVertex : petriNet.getEndNodes()) {
            ...
            if (petriNet.findTransition(endNode.getIdentifier()) != null) {
                Place endPlace = new Place("END_" + endNode.getIdentifier(), petriNet);
     ...
     * @param map HashMap
     */
    private static void convertXORsplit(ProtosFlowElement act, ProtosSubprocess process,
                                        Place splitPlace, PetriNet petriNet, HashMap map) {
        int rank = 1;
      ...
      for (Object object : getNormalSuccessors(process, act)) {
          ...
          Transition splitTransition = new Transition(SPLIT_STRING + rank + "_" + act.getIdentifier(), petriNet);
     ...
     * @param map HashMap
     */
    private static void convertXORjoin(ProtosFlowElement node, ProtosSubprocess process,
                                       Place joinPlace, PetriNet petriNet, HashMap map) {
        int rank = 1;
        ...
        for (Object object : getNormalPredecessors(process, node)) {
            ...
            if (status.isStatus()) {
                ...
                Place place = (Place) map.get(status);
                //create new transition.
                Transition joinTransition = new Transition(JOIN_STRING + rank + "_" + node.getIdentifier(), petriNet);
     ...
     * @param process ProtosSubprocess The subprocess containing the given node.
     * @param node ProtosFlowElement The given node.
     * @return HashSet
     */
    private static HashSet<ProtosFlowElement> getNormalSuccessors(ProtosSubprocess process, ProtosFlowElement node) {
        ...
        for (Object object : process.getArcs()) {
            if (object instanceof ProtosProcessArc) {
                ...
                ProtosFlowElement fromNode = (ProtosFlowElement) edge.getEdge().getSource();
                ProtosFlowElement toNode = (ProtosFlowElement) edge.getEdge().getDest();
                if (fromNode == node) {
                    nodes.add(toNode);
                }
            }
     ...
     * @return HashSet
     */
    private static HashSet<ProtosFlowElement> getNormalPredecessors(ProtosSubprocess process, ProtosFlowElement node) {
        HashSet<ProtosFlowElement> nodes = new HashSet<ProtosFlowElement>();
        for (Object object : process.getArcs()) {
            if (object instanceof ProtosProcessArc) {
                ProtosProcessArc edge = (ProtosProcessArc) object;
