...
public class ProtosToProtos {
    ...
    public static ProtosResult addImplicitConditions(ProtosModel model) {
        ...
        ProtosResult result = new ProtosResult(null, addImplicitConditions(model, map));
        return result;
    }

    /**
     * Make a copy of the given Protos model that contains all implicit conditions / statusses.
     * @param model ProtosModel The Protos model to copy.
     * @param map HashMap The map to fill (every Protos object will be mapped onto its copy in this map).
     * @return ProtosModel The copy of the Protos model.
     */
    public static ProtosModel addImplicitConditions(ProtosModel model, HashMap map) {
        ...
        for (ProtosSubprocess process : model.getSubprocesses()) {
            ProtosSubprocess newSubprocess = new ProtosSubprocess(process.getName());
     ...
     * @param model ProtosModel The Protos model.
     * @param process ProtosSubprocess The subprocess to copy.
    ...
    private static void addImplicitConditions(
        ...
        for (ProtosFlowElement node : process.getFlowElements()) {
            if (node.isStatus()) {
                ...
                ProtosFlowElement copyOfStatus = copyOfSubprocess.addStatus(status.getID(), status.getName());
                map.put(status, copyOfStatus);
            } else if (node.isActivity()) {
                ProtosFlowElement activity = (ProtosFlowElement) node;
                ProtosFlowElement copyOfActivity = copyOfSubprocess.addActivity(activity);
                map.put(activity, copyOfActivity);
            }
        }
        /*
         * Third, copy all arcs, but introduce a Status for an Activity-Activity edge.
         */
        for (Object object : process.getArcs()) {
            ProtosProcessArc arc = (ProtosProcessArc) object;
            ProtosFlowElement fromNode = (ProtosFlowElement) map.get( (ProtosFlowElement) process.getFlowElement(arc.getSource()));
            ProtosFlowElement toNode = (ProtosFlowElement) map.get( (ProtosFlowElement) process.getFlowElement(arc.getTarget()));
            if ((fromNode.isActivity()) && (toNode.isActivity())) {
                ...
                String constructedName = "place_" + fromNode.getName() + "_"
                                         + toNode.getName();
                ProtosFlowElement status = copyOfSubprocess.addStatus(constructedName, constructedName);
                /*
                 * Create an arc from the fromNode to the new status.
                 */
                ProtosProcessArc copyOfArc = (ProtosProcessArc) copyOfSubprocess.addArc(fromNode.getID(), status.getID());
                map.put(arc, copyOfArc);
                /*
                 * Create an arc from the new status to the toNode.
                 */
               ProtosProcessArc extraArc = (ProtosProcessArc) copyOfSubprocess.addArc(status.getID(), toNode.getID());
               map.put(arc, extraArc);
            } else {
                /*
                 * Copy the original edge.
                 */
                ProtosProcessArc copyOfArc = (ProtosProcessArc) copyOfSubprocess.addArc(fromNode.getID(), toNode.getID());
                map.put(arc, copyOfArc);
            }
        }
