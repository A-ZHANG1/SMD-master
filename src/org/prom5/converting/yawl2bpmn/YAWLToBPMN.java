...
package org.prom5.converting.yawl2bpmn;
...
import org.prom5.framework.models.bpmn.BpmnEdge;
import org.prom5.framework.models.bpmn.BpmnEdgeType;
import org.prom5.framework.models.bpmn.BpmnEvent;
import org.prom5.framework.models.bpmn.BpmnEventTriggerType;
import org.prom5.framework.models.bpmn.BpmnEventType;
import org.prom5.framework.models.bpmn.BpmnGateway;
import org.prom5.framework.models.bpmn.BpmnGatewayType;
import org.prom5.framework.models.bpmn.BpmnGraph;
import org.prom5.framework.models.bpmn.BpmnObject;
import org.prom5.framework.models.bpmn.BpmnProcessModel;
import org.prom5.framework.models.bpmn.BpmnSubProcess;
import org.prom5.framework.models.bpmn.BpmnSwimLane;
import org.prom5.framework.models.bpmn.BpmnSwimPool;
import org.prom5.framework.models.bpmn.BpmnSwimType;
import org.prom5.framework.models.bpmn.BpmnTask;
import org.prom5.framework.models.bpmn.BpmnTaskType;
import org.prom5.framework.models.bpmn.BpmnXmlTags;
...
import org.prom5.framework.models.yawl.bpmn.YAWLDecompositionBPMN;
import org.prom5.framework.plugin.ProvidedObject;
import org.prom5.mining.MiningResult;
import org.prom5.mining.bpmnmining.BpmnResult;
...
public class YAWLToBPMN implements ConvertingPlugin {
	...
	public MiningResult convert(ProvidedObject object) {
		...
		for (int i = 0; i < object.getObjects().length; i++) {
			if (providedYAWL == null && object.getObjects()[i] instanceof YAWLModel) {
	...
	private void addDecomposition(YAWLModel model, BpmnProcessModel bpm,
			...
			String parentId, boolean isAdhoc) {
		//if a task join or split code has property of or, xor, and
		//then, a gateway will be added, the gateway id  change the original id to connect with other nodes.
		...
		while (nodes.hasNext()) {
			...
			String lane = (String) node.getAttributeValue(BpmnXmlTags.BPMN_POOLLANE);
			if (node instanceof YAWLTask) {
				YAWLTask task = (YAWLTask) node;
				String taskType = (String) task.getAttributeValue(BpmnXmlTags.
								  BPMN_TASKTYPE);
				...
				if (strDecomp == null || strDecomp.equals("") || objDecomp == null ||
					...
					if (taskType.equals(BpmnXmlTags.BPMN_TASK)) {
						...
						String strLoopType = (String) task.getAttributeValue(
											 BpmnXmlTags.BPMN_PROP_LOOPTYPE);
						bpmnTask.setProperty(BpmnXmlTags.BPMN_PROP_LOOPTYPE,
								strLoopType);
						String strTimingType = (String) task.getAttributeValue(
											   BpmnXmlTags.BPMN_PROP_TIMING);
						bpmnTask.setProperty(BpmnXmlTags.BPMN_PROP_TIMING,
								strTimingType);
						String strcompensationactivity = (String) task.
								getAttributeValue(BpmnXmlTags.
								BPMN_PROP_COMPENSATIONACTIVITY);
						bpmnTask.setProperty(BpmnXmlTags.
								BPMN_PROP_COMPENSATIONACTIVITY,
								strcompensationactivity);
						String strtranaction = (String) task.getAttributeValue(
											   BpmnXmlTags.BPMN_PROP_TRANSACTION);
						...
						if (splitType == YAWLTask.AND) {
							...
							String subType = (String) task.getAttributeValue(
											 BpmnXmlTags.BPMN_SUBTYPE);
							if (subType == null) {
								bgt = null;
							} else if (subType.equals(BpmnXmlTags.BPMN_GWT_COMPLEX)) {
					...
					} else if (taskType.equals(BpmnEventTriggerType.Cancel.toString()) ||
							taskType.equals(BpmnEventTriggerType.Exception.
							toString()) ||
							taskType.equals(BpmnEventTriggerType.Error.toString())) {
						ArrayList<String> lanes = new ArrayList<String>();
						Iterator<YAWLNode> preds = task.getPredecessors().iterator();
						while (preds.hasNext()) {
							YAWLNode pred = preds.next();
							String aLane = (String) pred.getAttributeValue(
										   BpmnXmlTags.BPMN_POOLLANE);
							if (!lanes.contains(aLane)) {
								lanes.add(aLane);
							}
						}
						Iterator<YAWLNode> succs = task.getSuccessors().iterator();
						while (succs.hasNext()) {
							YAWLNode succ = succs.next();
							String aLane = (String) succ.getAttributeValue(
										   BpmnXmlTags.BPMN_POOLLANE);
							if (!lanes.contains(aLane)) {
								lanes.add(aLane);
							}
						}
						if (lanes.size() == 0 || lanes.size() == 1 && lanes.get(0) == null) {
							...
							//trigger type
							bpmnEvent.setTrigger(BpmnEventTriggerType.valueOf(
									taskType));
							bpmnEvent.setpid(parentId);
							bpm.addNode(bpmnEvent);
						} else {
							//converting a yawl model converted from a bpmn model
							//create a bpmn event for each lane and connect them correctly by edges
							int n = 0;
							for (String aLane : lanes) {
								...
								while (preds.hasNext()) {
									...
									String theLane = (String) pred.
											getAttributeValue(BpmnXmlTags.
											BPMN_POOLLANE);
									if (theLane == null && aLane == null ||
											(theLane != null && aLane != null &&
											theLane.equals(aLane))) {
										//add a bpmn edge
										String fromId = pid + "_" + pred.getId();
								...
								while (succs.hasNext()) {
									...
									if (theLane == null && aLane == null ||
										...
										BpmnEdge edge = new BpmnEdge(eventId, toId);
					...
					String strAdhoc = (String) task.getAttributeValue(BpmnXmlTags.
									  BPMN_PROP_ADHOC);
					...
					String strLoopType = (String) task.getAttributeValue(
										 BpmnXmlTags.BPMN_PROP_LOOPTYPE);
					bsp.setProperty(BpmnXmlTags.BPMN_PROP_LOOPTYPE, strLoopType);
					String strTimingType = (String) task.getAttributeValue(
										   BpmnXmlTags.BPMN_PROP_TIMING);
					bsp.setProperty(BpmnXmlTags.BPMN_PROP_TIMING, strTimingType);
					String strcompensationactivity = (String) task.
							getAttributeValue(BpmnXmlTags.
							BPMN_PROP_COMPENSATIONACTIVITY);
					bsp.setProperty(BpmnXmlTags.BPMN_PROP_COMPENSATIONACTIVITY,
							strcompensationactivity);
					String strexpanded = (String) task.getAttributeValue(
										 BpmnXmlTags.BPMN_PROP_EXPANDED);
					bsp.setProperty(BpmnXmlTags.BPMN_PROP_EXPANDED, strexpanded);
					String strtranaction = (String) task.getAttributeValue(
										   BpmnXmlTags.BPMN_PROP_TRANSACTION);
					bsp.setProperty(BpmnXmlTags.BPMN_PROP_TRANSACTION,
							strtranaction);
					//add decomposition recursively
					addDecomposition(model,
							((BpmnSubProcess) bpmnTask).getProcessModel(),
							objDecomp, id, bAdhoc);
					bsp.buildGraph();
				}
				if (bpmnTask != null) {
					...
					if ((taskType.equals(BpmnXmlTags.BPMN_TASK) ||
							taskType.equals(BpmnXmlTags.BPMN_SUBPROCESS)) && !isAdhoc) {
						...
						if (gateway != null) {
							...
							gateway.setLane(lane);
							bpm.addNode(gateway);
							//store the new name for its predcessors,in future, every node connect to the task
						...
						if (gateway != null) {
							...
							gateway.setLane(lane);
							bpm.addNode(gateway);
							//store the new name for its successors,in future, every node connect to the task
							// will change to connect with the gateway.
				...
				String nodeType = (String) condition.getAttributeValue(BpmnXmlTags.
								  BPMN_NODETYPE);
				if (nodeType == null) {
					nodeType = BpmnXmlTags.BPMN_ORIGINAL;
				}
				if (nodeType.equals(BpmnXmlTags.BPMN_ORIGINAL)) {
					String eventType = (String) condition.getAttributeValue(
									   BpmnXmlTags.BPMN_EVENTTYPE);
					//a BpmnEvent
					BpmnEvent bpmnEvent = new BpmnEvent(id);
					if (eventType == null) {
						int conditionType = 2;
					...
					String triggerType = (String) condition.getAttributeValue(
										 BpmnXmlTags.BPMN_PROP_TRIGGER);
					if (triggerType != null) {
						bpmnEvent.setTrigger(BpmnEventTriggerType.valueOf(
								triggerType));
					}
		...
		while (nodes.hasNext()) {
			...
			String[] pools = (decomposition instanceof YAWLDecompositionBPMN) ?
							 ((YAWLDecompositionBPMN) decomposition).getPools() :
							 (new String[0]);
			...
			//lane
			String[] lanes = (decomposition instanceof YAWLDecompositionBPMN) ?
							 ((YAWLDecompositionBPMN) decomposition).getLanes() :
							 (new String[0]);
		...
		for (YAWLEdge edge : alEdges) {
			...
			String edgeType = (String) edge.getAttributeValue(BpmnXmlTags.
							  BPMN_EDGETYPE);
			if (!edge.isNormal()) {
				if (edgeType == null || !edgeType.equals(BpmnXmlTags.BPMN_MANUAL)) {
					...
					if (event == null) {
						...
						bpm.addEdge(anEdge);
						//set the reset event's lane with its predecessor's lane
			...
			// need to update YawlEdge funciton of isDeafultEdge , then you can check whether an edge is default edge.
			...
			else if(edgeType != null && edgeType.equals(BpmnXmlTags.BPMN_MANUAL) && fromId.equals(toId))
			{
				//remove the self-loop edge for an event of CANCEL/ERROR/EXCEPTION
		...
		for (YAWLEdge edge : alEdges) {
			...
			String strEdgeType = (String) edge.getAttributeValue(BpmnXmlTags.
								 BPMN_EDGETYPE);
			if (strEdgeType != null) {
			...
			String strCondition = (String) edge.getAttributeValue(BpmnXmlTags.
								  BPMN_PROP_CONDITION);
			if (strCondition == null) {
				bpmnEdge.setProperty(BpmnXmlTags.BPMN_PROP_CONDITION,
						edge.getPredicate());
			} else {
				bpmnEdge.setProperty(BpmnXmlTags.BPMN_PROP_CONDITION, strCondition);
			}
			String strDefault = (String) edge.getAttributeValue(BpmnXmlTags.
								BPMN_PROP_DEFAULT);
			if (strDefault != null && strDefault.equals("true")) {
				bpmnEdge.setProperty(BpmnXmlTags.BPMN_PROP_DEFAULT, strDefault);
			}
			String strMessage = (String) edge.getAttributeValue(BpmnXmlTags.
								BPMN_PROP_MESSAGE);
