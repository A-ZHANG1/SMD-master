...
package org.prom5.converting.yawl2pn;
...
import org.prom5.converting.yawl2yawl.YawlToYawl;
import org.prom5.converting.yawl2yawl.YawlToYawlSettings;
...
import org.prom5.framework.models.yawl.YAWLEdge;
...
public class YawlToPetriNet {
     ...
     */
    private static final String COND_STRING = "COND_";   // for places corresponding to conditions.
    private static final String JOIN_STRING = "JOIN_";   // for transitions corresponding to joins.
    private static final String PRE_STRING = "PRE_";     // for places signalling that a task has started.
    private static final String TASK_STRING = "TASK_";   // for transitions corresponding to tasks.
    private static final String POST_STRING = "POST_";   // for places signalling that a has has completed.
    private static final String SPLIT_STRING = "SPLIT_"; // for transitions corresponding to splits.
     ...
     * @param model YAWLModel The given YAWL model.
     * @param settings YawlToPetriNetSettings The settings to use during the conversion.
     * @param map HashMap The map to store the YAWL-PN relations in. YAWL conditions are mapped onto a place.
     * YAWL tasks, however, are mapped onto an array list where:
     * - the first element contains the set of join transitions,
     * - the second element is the in-between (or busy) place, and
     * - the third element contains the set of split transitions.
     * Reset edges are ignored by this conversion.
     * @return PetriNet
     */
    public static PetriNet convert(
            YAWLModel model,
            YawlToPetriNetSettings settings,
            HashMap map) {
        ...
        if (settings.get(YawlToPetriNetSettings.REDUCE_PN)) {
             ...
             */
            /*
             * For mapping object from the original Petri net to the reduced Petri net.
             */
            HashMap reductionMap = new HashMap();
            /*
             * Create a reduced Petri net, leaving the non-reducable nodes alone.
             */
            ...
            for (Object key: map.keySet()) {
                ...
                if (value instanceof Place) {
                    ...
                    if (list.size() == 3) {
                        ...
                        /*
                         * Join place is non-reducable, hence it should be in the reduction map.
                         */
                        ...
                        if (settings.get(YawlToPetriNetSettings.CREATE_CLUSTERS)) {
                            /*
                             * Add a cluster for this task. Improves recognition.
                             */
                        ...
                        /*
                         * Join place and task transition are non-reducable, hence they should be in the reduction map.
                         */
                        Place joinPlace = (Place) reductionMap.get(list.get(1));
                        Transition taskTransition = (Transition) reductionMap.get(list.get(2));
                        Place splitPlace;
    ...
    /**
     * Convert the given YAWL model to a Petri net.
     * Precondition: All implicit conditions have been added to the YAWL model.
     * @param model YAWLModel The given YAWL model.
     * @param settings YawlToPetriNetSettings The settings to use during the conversion.
     * @param map HashMap The map to store the YAWL-PN relations in. YAWL conditions are mapped onto a place.
     ...
     * - the second element is the in-between (or busy) place, and
     * - the third element contains the set of split transitions.
     * @param nonReducableNodes ArrayList The list to stroe non-reducable (sacrosanct) nodes in. These nodes will
     * 'survive' the reduction.
     * Reset edges are ignored by this conversion.
     * @return PetriNet
     */
    private static PetriNet convert(
            YAWLModel model,
            YawlToPetriNetSettings settings,
            HashMap map,
            ...
            if (decomposition.isRoot()) {
                ...
                for (YAWLNode node : decomposition.getNodes()) {
                    if (node instanceof YAWLCondition) {
                        ...
                        Place place = new Place(COND_STRING + condition.getIdentifier(),
                                                petriNet);
                ...
                /*
                 * Second, convert the transitions to triplets (join transitions, place, split transitions).
                 * Assume that the predecessors and successor of the tasks have already been converted.
                 */
                for (YAWLNode node : decomposition.getNodes()) {
                    if (node instanceof YAWLTask) {
                         ...
                         */
                        if (settings.get(YawlToPetriNetSettings.CREATE_TASK_TRANSITION)) {
                            joinPlace = new Place(PRE_STRING + task.getIdentifier(), petriNet);
                            petriNet.addAndLinkPlace(joinPlace);
                            splitPlace = new Place(POST_STRING + task.getIdentifier(), petriNet);
                            petriNet.addAndLinkPlace(splitPlace);
                            taskTransition = new Transition(TASK_STRING + task.getIdentifier(),
                                    petriNet);
                            if (settings.get(YawlToPetriNetSettings.CREATE_LOG_EVENTS)) {
                                taskTransition.setLogEvent(new LogEvent(task.
                                        getIdentifier(), COMPLETE_STRING));
                            }
                            petriNet.addAndLinkTransition(taskTransition);
                            PNEdge joinEdge = new PNEdge(joinPlace, taskTransition);
                            ...
                            /*
                             * The split place should not be reduced, as a busy YAWL task will be mapped onto
                             * a token in this place.
                             */
                            //nonReducableNodes.add(splitPlace);
                        } else {
                            joinPlace = new Place(PRE_STRING + task.getIdentifier(),
                                                  petriNet);
                            petriNet.addAndLinkPlace(joinPlace);
                         ...
                         */
                        HashSet<Transition>
                                joinTransitions = new HashSet<Transition>();
                        switch (task.getJoinType()) {
                        case YAWLTask.NONE:
                        case YAWLTask.AND: {
                            convertAndJoin(task, decomposition, petriNet, joinPlace,
                                           joinTransitions, map);
                            break;
                        }
                        case YAWLTask.XOR: {
                            convertXorJoin(task, decomposition, petriNet, joinPlace,
                                           joinTransitions, map);
                            break;
                        }
                        case YAWLTask.OR: {
                            convertOrJoin(task, decomposition, petriNet, joinPlace,
                                          joinTransitions, map);
                            break;
                        }
                        }
                         ...
                         */
                        HashSet<Transition>
                                splitTransitions = new HashSet<Transition>();
                        switch (task.getSplitType()) {
                        case YAWLTask.NONE:
                        case YAWLTask.AND: {
                            convertAndSplit(task, decomposition, petriNet, splitPlace,
                                            splitTransitions, map);
                            break;
                        }
                        case YAWLTask.XOR: {
                            convertXorSplit(task, decomposition, petriNet, splitPlace,
                                            splitTransitions, map, nonReducableNodes);
                            break;
                        }
                        case YAWLTask.OR: {
                            convertOrSplit(task, decomposition, petriNet, splitPlace,
                                           splitTransitions, map, nonReducableNodes);
                            break;
                        }
                        }
     ...
     * @param task YAWLTask The task with the AND-join.
     * @param decomposition YAWLDecomposition The decomposition the task belongs to.
     * @param petriNet PetriNet The resulting Petri Net.
     * @param taskPlace Place The 'busy' place for the YAWL task.
     * @param joinTransitions HashSet The set of join transitions created so far.
     * @param map HashMap Map with YAWL-PN relations.
     */
    private static void convertAndJoin(YAWLTask task, YAWLDecomposition decomposition,
                                       PetriNet petriNet, Place taskPlace,
                                       HashSet<Transition> joinTransitions,
            HashMap map) {
        /*
        ...
        Transition joinTransition = new Transition(JOIN_STRING + inputs + "_" + task.getIdentifier(),
                petriNet);
     ...
     * @param task YAWLTask The task with the XOR-join.
     * @param decomposition YAWLDecomposition The decomposition the task belongs to.
     * @param petriNet PetriNet The resulting Petri Net.
     * @param taskPlace Place The 'busy' place for the YAWL task.
     * @param joinTransitions HashSet The set of join transitions created so far.
     * @param map HashMap Map with YAWL-PN relations.
     */
    private static void convertXorJoin(YAWLTask task, YAWLDecomposition decomposition,
                                       PetriNet petriNet, Place taskPlace,
                                       HashSet<Transition> joinTransitions,
            HashMap map) {
        /*
        ...
        for (Object object : getNormalPredecessors(decomposition, task)) {
            if (object instanceof YAWLCondition) {
                ...
                inputs += "1";
                for (int i = rank + 1; i < getNormalPredecessors(decomposition, task).size(); i++) {
                    inputs += "0";
                }
                Transition joinTransition = new Transition(
                        JOIN_STRING + inputs + "_"+ task.getIdentifier(), petriNet);
                petriNet.addAndLinkTransition(joinTransition);
                joinTransitions.add(joinTransition);
                /*
     ...
     * @param task YAWLTask The task with the OR-join.
     * @param decomposition YAWLDecomposition The decomposition the task belongs to.
     * @param petriNet PetriNet The resulting Petri Net.
     * @param taskPlace Place The 'busy' place for the YAWL task.
     * @param joinTransitions HashSet The set of join transitions created so far.
     * @param map HashMap Map with YAWL-PN relations.
     */
    private static void convertOrJoin(YAWLTask task, YAWLDecomposition decomposition,
                                      PetriNet petriNet, Place taskPlace,
                                      HashSet<Transition> joinTransitions,
            HashMap map) {
     ...
     * @param joinTransitions HashSet The set of join transitions created so far.
     * @param map HashMap Map with YAWL-PN relations.
     * @param predecessors Collection Predecessors of this task still to choose from.
     * @param conditions HashSet Chosen predecessors for the current branch.
     */
    private static void convertOrJoin(YAWLTask task,
            ...
            HashSet<YAWLCondition> conditions,
            String inputs) {
        if (predecessors.isEmpty()) {
            /*
             * All choices made in this branch. Create a transition for the chosen predecessors,
             * provided that any predecessor has been chosen.
             */
             ...
             */
            YAWLCondition condition = (YAWLCondition) predecessors.iterator().next();
            predecessors.remove(condition);
            /*
             * First, start a branch where this predecessor has not been chosen.
             */
            convertOrJoin(task, petriNet, taskPlace, joinTransitions, map, predecessors, conditions, inputs + "0");
            /*
             * Second, start a branch where this predecessor has been chosen.
             */
            conditions.add(condition);
            convertOrJoin(task, petriNet, taskPlace, joinTransitions, map, predecessors, conditions, inputs + "1");
     ...
     * See convertAndJoin.
     * @param task YAWLTask
     * @param decomposition YAWLDecomposition
     * @param petriNet PetriNet
     * @param taskPlace Place
     * @param splitTransitions HashSet
     * @param map HashMap
     */
    private static void convertAndSplit(YAWLTask task, YAWLDecomposition decomposition,
                                   PetriNet petriNet, Place taskPlace,
                                   HashSet<Transition> splitTransitions, HashMap map) {
        String outputs = "";
        for (int i =0; i < getNormalSuccessors(decomposition, task).size(); i++) {
            outputs += "1";
        }
        Transition splitTransition = new Transition(SPLIT_STRING + outputs + "_" + task.getIdentifier(),
                                                    petriNet);
     ...
     * @param map HashMap
     */
    private static void convertXorSplit(YAWLTask task, YAWLDecomposition decomposition,
                                       PetriNet petriNet, Place taskPlace,
                                       HashSet<Transition> splitTransitions,
           HashMap map,
           ArrayList nonReducableNodes) {
        int rank = 0;
        ...
        for (Object object : getNormalSuccessors(decomposition, task)) {
            if (object instanceof YAWLCondition) {
                ...
                outputs += "1";
                for (int i = rank + 1; i < getNormalSuccessors(decomposition, task).size(); i++) {
                    outputs += "0";
                }
                Transition splitTransition = new
                        Transition(SPLIT_STRING + outputs + "_" + task.getIdentifier(),
                        petriNet);
                petriNet.addAndLinkTransition(splitTransition);
                splitTransitions.add(splitTransition);
        ...
        if (nonReducableTemp.size() > 1) {
            /*
             * An explicit choice. Make sure this moment of choice does not get lost in the reduction.
             */
     ...
     * @param map HashMap
     */
    private static void convertOrSplit(YAWLTask task, YAWLDecomposition decomposition,
                                       PetriNet petriNet, Place taskPlace,
                                       HashSet<Transition> splitTransitions,
            HashMap map,
            ArrayList nonReducableNodes) {
     ...
     * @param splitTransitions HashSet
     * @param map HashMap
     * @param successors Collection
     * @param conditions HashSet
     */
    private static void convertOrSplit(YAWLTask task,
                                      PetriNet petriNet, Place taskPlace,
                                      HashSet<Transition> splitTransitions,
           HashMap map, Collection successors, HashSet<YAWLCondition> conditions,
           ArrayList nonReducableNodes,
           String outputs) {
        if (successors.isEmpty()) {
            ...
            YAWLCondition condition = (YAWLCondition) successors.iterator().next();
    ...
    /**
     * Get all nodes that are predecessor through normal edges for the given node.
     * @param decomposition YAWLDecomposition The decomposition containing the given node.
     * @param node YAWLNode The given node.
     * @return HashSet
     */
    private static HashSet<YAWLNode> getNormalPredecessors(YAWLDecomposition decomposition, YAWLNode node) {
     ...
     * @return HashSet
     */
    private static HashSet<YAWLNode> getNormalSuccessors(YAWLDecomposition decomposition, YAWLNode node) {
        HashSet<YAWLNode> nodes= new HashSet<YAWLNode>();
        for (Object object: decomposition.getEdges()) {
            if (object instanceof YAWLEdge) {
                YAWLEdge edge = (YAWLEdge) object;
