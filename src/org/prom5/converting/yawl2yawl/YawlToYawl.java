...
package org.prom5.converting.yawl2yawl;
...
import org.prom5.mining.yawlmining.YAWLResult;

public class YawlToYawl {

    /*
     * Prefixes to be used when a composite taks sis split into a start and end task.
     */
    ...
    /*
     * It seems a bad diea to map object to objects. Better is to map ids to objects.
     * However, when flattening a model, objects from multiple graphs have to mapped onto
     * object from a singel graph, and the ids or object from different graphs are not
     * necessarily unique. For this reason, we introduced a tempId for the YAWL objcts.
     * When mapping a YAWL object to some object, a fresh id is generated, which is
     * stored with the YAWL object. Next, the id is mapped onto the given object.
     */
    private static Integer id = 1;

    /**
     * Get the unique Id from a node name. If a node name has from X_Y_Z, then Z is the unique Id.
     * Note that the Id is only unique within the node's graph.
     ...
     * @param decomposedTask ArrayList The context of the current decomposition.
     * @return String The engine path.
     */
    private static String getPath(
            ArrayList<YAWLTask> decomposedTask) {
        String path = "";
    ...
    public static YAWLResult addImplicitConditions(YAWLModel model) {
        ...
        YAWLResult result = new YAWLResult(null, addImplicitConditions(model, settings, map));
        return result;
    }

    /**
     * Make a copy of the given YAWL model that contains all implicit conditions.
     * @param model YAWLModel The YAWL model to copy.
     * @param settings YawlToYawlSettings The settings to use while copying.
     * @param map HashMap The map to fill (every YAWL object will be mapped onto its copy in this map).
     * @return YAWLModel The copy of the YAWL model.
     */
    public static YAWLModel addImplicitConditions(
            YAWLModel model,
            YawlToYawlSettings settings,
            HashMap map) {
        ...
        for (YAWLDecomposition decomposition : model.getDecompositions()) {
            YAWLDecomposition newDecomposition
                    = new YAWLDecomposition(decomposition);
     ...
     * @param model YAWLModel The YAWL model.
     * @param decomposition YAWLDecomposition The decomposition to copy.
     * @param settings YawlToYawlSettings The settings to use while copying.
     * @param map HashMap Maps every object to its copy.
     */
    private static void addImplicitConditions(
            YAWLModel model,
         ...
         */
        YAWLDecomposition copyOfDecomposition = (YAWLDecomposition) map.get(decomposition);
        /*
         * Second, copy all nodes.
         * Take not of all Ids, as we will have to generate an Id for every implicit condition later on.
         */
        //Integer maxId = 0;
        for (YAWLNode node : decomposition.getNodes()) {
            ...
            if (node instanceof YAWLCondition) {
                ...
                YAWLCondition copyOfCondition = copyOfDecomposition.addCondition(condition);
        ...
        /*
         * Third, copy all edges, but introduce a Condition for a Task-Task edge.
         */
        for (Object object: decomposition.getEdges()) {
            ...
            if ((fromNode instanceof YAWLTask) && (toNode instanceof YAWLTask) && edge.isNormal()) {
                /*
                ...
                YAWLCondition condition = copyOfDecomposition.addCondition(constructedName);
                //condition.setIdentifier(constructedName);
                /*
                 * Create an edge from the fromNode to the new condition. Copy the old edge data.
                 */
                YAWLEdge copyOfEdge = copyOfDecomposition.addEdge(fromNode, condition, edge);
                map.put(edge, copyOfEdge);
                /*
                 * Create an edge from the new condition to the toNode. Create default edge data.
                 */
                YAWLEdge dummyEdge = new YAWLEdge(condition, toNode, false, null, "0");
                YAWLEdge extraEdge = copyOfDecomposition.addEdge(condition, toNode, dummyEdge);
                decomposition.removeEdge(dummyEdge);
            } else {
                /*
                 * Copy the original edge.
                 */
                YAWLEdge copyOfEdge = copyOfDecomposition.addEdge(fromNode, toNode, edge);
                map.put(edge, copyOfEdge);
            }
        }
    }
    ...
    public static YAWLResult flattenModel(YAWLModel model) {
        HashMap map = new HashMap();
        YawlToYawlSettings settings = new YawlToYawlSettings();
        YAWLResult result = new YAWLResult(null, flattenModel(model, settings, map));
        return result;
    }

    /**
     * Make a flat copy of the given YAWL model, starting from the root decomposition.
     * @param model YAWLModel The given YAWL model.
     * @param settings YawlToYawlSettings The settings to use while copying.
     * @param map HashMap Maps every YAWL object onto its copy. Composite tasks are mapped onto
     * an array list containing a start task and an end task.
     * @return YAWLModel The falt copy of the YAWL model.
     */
    public static YAWLModel flattenModel(
            YAWLModel model,
            YawlToYawlSettings settings,
            HashMap map) {
        YAWLModel newModel = new YAWLModel(model);
     ...
     * @param model YAWLModel The YAWL model.
     * @param decomposition YAWLDecomposition The decomposition to flat-copy.
     * @param decomposedTask ArrayList The context of the current decomposition.
     * @param settings YawlToYawlSettings The settings to use while flat-copying.
     * @param map HashMap Maps every object to its copy.
     * @param inputMap HashMap Maps every composed task to its input condition.
     * @param outputMap HashMap Likewise, output condition.
     */
    private static void flattenDecomposition(
            YAWLModel model,
            YAWLDecomposition decomposition,
            ArrayList<YAWLTask> decomposedTask,
            YawlToYawlSettings settings,
            HashMap map,
            HashMap<YAWLTask, YAWLCondition> inputMap,
        ...
        for (YAWLNode node : decomposition.getNodes()) {
            if (node instanceof YAWLCondition) {
                ...
                YAWLCondition copyOfCondition = copyOfDecomposition.addCondition(
                        condition);
                ...
                if (decomposedTask.size() > 0) {
                    ...
                    if (copyOfCondition.isInputCondition()) {
                        inputMap.put(decomposedTask.get(decomposedTask.size() - 1),
                                     copyOfCondition);
                        copyOfCondition.normalize();
                    } else if (copyOfCondition.isOutputCondition()) {
                        outputMap.put(decomposedTask.get(decomposedTask.size() - 1),
                                      copyOfCondition);
                        copyOfCondition.normalize();
                    }
                }
                ...
                if (model.isComposite(task.getDecomposition())) {
                     ...
                     */
                    YAWLDecomposition taskDecomposition = model.getDecomposition(task.getDecomposition());
                    map.put(taskDecomposition, copyOfDecomposition);
                    decomposedTask.add(task);
                    flattenDecomposition(model, taskDecomposition, decomposedTask, settings, map, inputMap, outputMap);
                    decomposedTask.remove(task);
                    /*
                     * Last, add edges to connect the decomposition.
                     */
                    YAWLEdge dummyEdge = new YAWLEdge(copyOfTask0, inputMap.get(task), false, null, "0");
                    YAWLEdge copyOfEdge0 = copyOfDecomposition.addEdge(copyOfTask0, inputMap.get(task), dummyEdge);
                    YAWLEdge copyOfEdge1 = copyOfDecomposition.addEdge(outputMap.get(task), copyOfTask1, dummyEdge);
                    decomposition.removeEdge(dummyEdge);
                } else {
                    String oldID = task.getID();
                    String prefix = getPath(decomposedTask);
        ...
        for (Object object: decomposition.getEdges()) {
            ...
            if (map.get((YAWLNode) edge.getSource()) instanceof ArrayList) {
                 ...
                 */
                ArrayList<YAWLTask> startEndTask = (ArrayList<YAWLTask>) map.get((YAWLNode) edge.getSource());
            ...
            if (map.get((YAWLNode) edge.getDest()) instanceof ArrayList) {
                 ...
                 */
                ArrayList<YAWLTask> startEndTask = (ArrayList<YAWLTask>)
                        map.get((YAWLNode) edge.getDest());
                ...
                if (!edge.isNormal()) {
                    ...
                    addResetEdges(model, copyOfDecomposition, fromNode, resetTask, edge, map);
                }
            } else {
     ...
     * @param model YAWLModel The given model.
     * @param copyOfDecomposition YAWLDecomposition The decomposition to add the reset edges to.
     * @param fromNode YAWLNode The source node of the reset edge.
     * @param resetTask YAWLTask The composite task which is the target of the original reset edge.
     * @param edge YAWLEdge The original reset edge.
     * @param map HashMap Maps every YAWL object onto its copy.
     */
    private static void addResetEdges(YAWLModel model, YAWLDecomposition copyOfDecomposition, YAWLNode fromNode, YAWLTask resetTask, YAWLEdge edge, HashMap map) {
        ArrayList<YAWLTask> compositeTask = (ArrayList<YAWLTask>) map.get(resetTask);
        ...
        for (YAWLNode resetNode : resetDecomposition.getNodes()) {
            if (map.get(resetNode) instanceof ArrayList) {
                 ...
                 */
                addResetEdges(model, copyOfDecomposition, fromNode, (YAWLTask) resetNode, edge, map);
            } else {
                copyOfDecomposition.addEdge(fromNode, (YAWLNode) map.get(resetNode), edge);
            }
        }
    }
}
