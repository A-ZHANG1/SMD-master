...
public class AmlExport implements ExportPlugin {
	...
	public String writeHierarchyRecursive(Object obj, EPCHierarchy hierarchy) {
		...
		if (obj instanceof String){
			...
			while (children.hasNext()){
				...
				if (child instanceof ModelHierarchyDirectory){
					childstring = childstring + writeHierarchyRecursive(child,hierarchy);
				}
			...
			return "<Group Group.ID='"+
				AmlIdFormat(currentGroup++,GROUP)+"'>\n" +
            	"<AttrDef AttrDef.Type='AT_NAME'>\n"+
            	"<AttrValue LocaleId='&LocaleId.USen;'>"+obj+"</AttrValue>\n"+
            	"<AttrValue LocaleId='&LocaleId.DEde;'>"+obj+"</AttrValue>\n"+
            	"</AttrDef>\n"+
				childstring +
			   "</Group>\n";
		}
		if (obj instanceof ModelHierarchyDirectory){
			...
			return "<Group Group.ID='"+
				AmlIdFormat(currentGroup++,GROUP)+"'>\n" +
            	"<AttrDef AttrDef.Type='AT_NAME'>\n"+
            	"<AttrValue LocaleId='&LocaleId.USen;'>"+obj.toString()+"</AttrValue>\n"+
            	"<AttrValue LocaleId='&LocaleId.DEde;'>"+obj.toString()+"</AttrValue>\n"+
            	"</AttrDef>\n"+
				childstring +
			   "</Group>\n";
		}
	...
	public String writeEPC(ConfigurableEPC epc) {
		...
		while (nodes.hasNext()){
			...
			if (node instanceof EPCConnector) {
				...
				String type = c.toString().toUpperCase().substring(0,c.toString().indexOf("-")).trim();
			...
			if (node instanceof EPCFunction) {
				...
				for (int i = 0; i < f.getNumInfSysObjects(); i++) {
					LabelType.put(f.getInfSysObject(i).getLabel(),"OT_APPL_SYS");
				}
			...
			while (successors.hasNext()){
				...
				String toObjDef = AmlIdFormat(allnodes.indexOf(succ)+1,OBJDEF);
				String toObjOcc = AmlIdFormat(allnodes.indexOf(succ)+1,OBJOCC);
				String cxnDefType = "";
				if (node instanceof EPCFunction){
				...
				cxnDef = cxnDef +
                    "<CxnDef CxnDef.ID='" + AmlIdFormat(currentDef,CXNDEF) + "'\n "+
                    "CxnDef.Type='" + cxnDefType + "'\n "+
                    "ToObjDef.IdRef='" + toObjDef + "'>\n"+
                    "</CxnDef>\n";
				cxnDefIdRefs = cxnDefIdRefs + AmlIdFormat(currentDef++,CXNDEF) + " ";
				cxnOcc = cxnOcc +
                	"<CxnOcc CxnOcc.ID='" + AmlIdFormat(currentDef,CXNOCC) + "'\n "+
                    "CxnDef.IdRef='" + AmlIdFormat((currentDef-1),CXNDEF) + "'\n "+
			...
			if (node instanceof EPCSubstFunction) {
				...
				if (sf.getSubstitutedEPC() != null && EPCs.indexOf(sf.getSubstitutedEPC()) > -1) {
					definitionlist = definitionlist +
						"LinkedModels.IdRefs='" + AmlIdFormat((EPCs.indexOf(sf.getSubstitutedEPC())+1),MODEL) + "'\n ";
				}
			}
			if (cxnDefIdRefs.length()>0){
				definitionlist = definitionlist +
				"ToCxnDefs.IdRefs='" + cxnDefIdRefs + "'\n ";
			}
			definitionlist = definitionlist +
				"SymbolNum='" + symbolNum + "'>\n"+
	            "<AttrDef AttrDef.Type='AT_NAME'>\n"+
                "<AttrValue LocaleId='&LocaleId.USen;'>" + ConfigurableEPC.editName(node.getIdentifier()) + "</AttrValue>\n"+
                "<AttrValue LocaleId='&LocaleId.DEde;'>" + ConfigurableEPC.editName(node.getIdentifier()) + "</AttrValue>\n"+
                "</AttrDef>\n"+
                cxnDef +
                "</ObjDef>\n";
			elements = elements +
            	"<ObjOcc ObjOcc.ID='" + AmlIdFormat(allnodes.indexOf(node)+1,OBJOCC) + "'\n " +
                "ObjDef.IdRef='" + AmlIdFormat(allnodes.indexOf(node)+1,OBJDEF) + "'\n ";
			if (cxnOccIdRefs.length()>0){
				elements = elements +
                "ToCxnOccs.IdRefs='" + cxnOccIdRefs + "'\n ";
			}
			elements = elements +
                "SymbolNum='" + symbolNum + "'\n "+
                "Active='YES'\n Shadow='YES'\n Visible='YES'\n Zorder='1'\n Hints='0'>\n" +
		...
		elements =
			"<Model Model.ID='" + AmlIdFormat((EPCs.indexOf(epc)+1),MODEL) + "' Model.Type='MT_EEPC'>\n" +
            "<Flag>180</Flag>\n" +
            "<AttrDef AttrDef.Type='AT_NAME'>\n" +
            "<AttrValue LocaleId='&LocaleId.USen;'>" + epc.getIdentifier() + "</AttrValue>\n" +
            "<AttrValue LocaleId='&LocaleId.DEde;'>" + epc.getIdentifier() + "</AttrValue>\n" +
            "</AttrDef>\n"+
            elements +
            "</Model>\n";
		return elements;
	...
	private String AmlIdFormat(int input,int AmlElement){
		...
		switch (AmlElement) {
		case GROUP:  in = "Group."+in+"k--";break;
		case MODEL:  in = "Model."+in+"u--";break;
		case OBJDEF: in = "ObjDef."+in+"p--";break;
		case CXNDEF: in = "CxnDef."+in+"q--";break;
		case OBJOCC: in = "ObjOcc."+in+"x--";break;
		case CXNOCC: in = "CxnOcc."+in+"y--";break;
		}
		return in;
	}
	...
	private void writeAMLHeader(OutputStream out) {
		...
		writeLn("<Header-Info\n CreateTime='"+TimeParser.format(cal.getTime())+"'",out);
		writeLn("CreateDate='"+DateParser.format(cal.getTime())+"'",out);
		writeLn("DatabaseName='ProM-Data'\n UserName='ProM'\n ArisExeVersion='61'\n />",out);
		writeLn("<Language LocaleId='&LocaleId.USen;' Codepage='&Codepage.USen;'>\n <LogFont \n FaceName='Arial'\n Height='-13'\n Width='0'\n Escapement='0'\n Orientation='0'\n Weight='400'\n CharSet='0'\n  OutPrecision='0'\n ClipPrecision='0'\n Quality='0'\n PitchAndFamily='0'\n Color='0'/>\n </Language>",out);
		writeLn("<Language LocaleId='&LocaleId.DEde;' Codepage='&Codepage.DEde;'>\n <LogFont \n FaceName='Arial'\n Height='-13'\n Width='0'\n Escapement='0'\n Orientation='0'\n Weight='400'\n CharSet='0'\n  OutPrecision='0'\n ClipPrecision='0'\n Quality='0'\n PitchAndFamily='0'\n Color='0'/>\n </Language>",out);
		writeLn("<Database>\n <AttrDef AttrDef.Type='AT_CREATOR'>\n <AttrValue LocaleId='&LocaleId.USen;'>ProM</AttrValue>\n </AttrDef>\n <AttrDef AttrDef.Type='AT_CREAT_TIME_STMP'>",out);
		writeLn("<AttrValue LocaleId='&LocaleId.USen;'>"+TimeParser.format(cal.getTime())+";"+Date2Parser.format(cal.getTime())+"</AttrValue>",out);
		writeLn("<AttrValue LocaleId='&LocaleId.DEde;'>"+TimeParser.format(cal.getTime())+";"+Date2Parser.format(cal.getTime())+"</AttrValue>",out);
		writeLn("</AttrDef>",out);
