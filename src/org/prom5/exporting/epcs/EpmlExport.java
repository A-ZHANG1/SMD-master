...
package org.prom5.exporting.epcs;
...
//	private Vector directory = new Vector();
	...
	public String writeHierarchyRecursive(Object obj, EPCHierarchy hierarchy) {
		...
		if (obj instanceof String){
			...
			while (children.hasNext()){
				childstring = childstring + writeHierarchyRecursive(children.next(),hierarchy);
			}
	...
	public String writeEPC(ConfigurableEPC epc) {
		...
		while (nodes.hasNext()){
			...
			if (node instanceof EPCFunction) {
				...
				if (f.visualObject != null) {
					int fwidth = (int) (((Double) f.visualObject.getAttributeValue(GrappaConstants.WIDTH_ATTR)).doubleValue());
					int fheight = (int) (((Double) f.visualObject.getAttributeValue(GrappaConstants.HEIGHT_ATTR)).doubleValue());
					int fcX = (int) f.visualObject.getCenterPoint().getX();
					int fcY = (int) f.visualObject.getCenterPoint().getY(); // invert the y axis (everything would be upside down otherwise)
					fwidth = 60;
					fheight = 20;
					int ftopX = fcX - (fwidth/2);
					int ftopY = fcY - (fheight/2); // invert the y axis (everything would be upside down otherwise)
					graphics = "<graphics>\n<position height='"+fheight+"' width='"+fwidth+"' x='"+ftopX+"' y='"+ftopY+"'/>\n</graphics>\n";
				}
				definitionlist = definitionlist +
					"<definition defId='" + currentDef + "'/>\n";
				elements = elements +
					"<function id='" +
					(allnodes.indexOf(f)+1) +
					"' defRef='" +
					currentDef++ +
					"'>\n";
				...
				for (int i = 0; i < f.getNumDataObjects(); i++) {
					elements = elements +
					"<relation id='"+(currentId++)+"' from='"+(allnodes.indexOf(f)+1)+"' to='"+(currentId)+"'/>\n"+
					"<dataField id='"+(currentId++)+
					"' defRef='" +
					(currentDef++) +
					"'>\n"+
					"<name>\n" +
					f.getDataObject(i).getLabel() +
					"</name>\n"+
					"</dataField>\n";
					definitionlist = definitionlist +
					"<definition defId='" + currentDef + "'/>\n";
			...
			if (node instanceof EPCEvent) {
				...
				if (e.visualObject != null) {
					int ewidth = (int) (((Double) e.visualObject.getAttributeValue(GrappaConstants.WIDTH_ATTR)).doubleValue());
					int eheight = (int) (((Double) e.visualObject.getAttributeValue(GrappaConstants.HEIGHT_ATTR)).doubleValue());
					int ecX = (int) e.visualObject.getCenterPoint().getX();
					int ecY = (int) e.visualObject.getCenterPoint().getY(); // invert the y axis (everything would be upside down otherwise)
					ewidth = 60;
					eheight = 20;
					int etopX = ecX - (ewidth/2);
					int etopY = ecY - (eheight/2); // invert the y axis (everything would be upside down otherwise)
					graphics = "<graphics>\n<position height='"+eheight+"' width='"+ewidth+"' x='"+etopX+"' y='"+etopY+"'/>\n</graphics>\n";
 				}
				definitionlist = definitionlist +
					"<definition defId='" + currentDef + "'/>\n";
				elements = elements +
					"<event id='" +
					(allnodes.indexOf(e)+1) +
					"' defRef='" +
					currentDef++ +
					"'>\n";
				elements = elements + graphics;
			...
			if (node instanceof EPCConnector) {
				...
				String type = c.toString().toLowerCase().substring(0,c.toString().indexOf("-")).trim();
				String graphics = "";
				if (c.visualObject != null) {
					int cwidth = (int) (((Double) c.visualObject.getAttributeValue(GrappaConstants.WIDTH_ATTR)).doubleValue());
					int cheight = (int) (((Double) c.visualObject.getAttributeValue(GrappaConstants.HEIGHT_ATTR)).doubleValue());
					int ccX = (int) c.visualObject.getCenterPoint().getX();
					int ccY = (int) c.visualObject.getCenterPoint().getY(); // invert the y axis (everything would be upside down otherwise)
					cwidth = 20;
					cheight = 20;
					int ctopX = ccX - (cwidth/2);
					int ctopY = ccY - (cheight/2); // invert the y axis (everything would be upside down otherwise)
					graphics = "<graphics>\n<position height='"+cheight+"' width='"+cwidth+"' x='"+ctopX+"' y='"+ctopY+"'/>\n</graphics>\n";
				}
				elements = elements +
					"<"+type+
					" id='" +
			...
			while (successors.hasNext()){
				...
				elements = elements +
					writeArc(node,next,allnodes.indexOf(node)+1, allnodes.indexOf(next)+1, currentId++);
			}
		}
		...
		return "<epc epcId='" +
				(EPCs.indexOf(epc)+1) + "' name='" + epc.getIdentifier() + "'>\n" +
				elements +
			   "</epc>\n";
	}

	public String writeArc(ModelGraphVertex snode, ModelGraphVertex tnode, int source, int target, int id) {
		int height = 0;
		int cX = 0;
		int cY = 0;
		int topY = 0;
		if (snode instanceof EPCConnector && snode.visualObject != null) {
			...
			height = (int) (((Double) c.visualObject.getAttributeValue(GrappaConstants.HEIGHT_ATTR)).doubleValue());
			...
			if (snode instanceof EPCEvent && snode.visualObject != null) {
				...
				height = (int) (((Double) e.visualObject.getAttributeValue(GrappaConstants.HEIGHT_ATTR)).doubleValue());
				...
				if (snode instanceof EPCFunction && snode.visualObject != null) {
					...
					height = (int) (((Double) f.visualObject.getAttributeValue(GrappaConstants.HEIGHT_ATTR)).doubleValue());
		...
		if (tnode instanceof EPCConnector && tnode.visualObject != null) {
			...
			theight = (int) (((Double) c.visualObject.getAttributeValue(GrappaConstants.HEIGHT_ATTR)).doubleValue());
			...
			if (tnode instanceof EPCEvent && tnode.visualObject != null) {
				...
				theight = (int) (((Double) e.visualObject.getAttributeValue(GrappaConstants.HEIGHT_ATTR)).doubleValue());
				...
				if (tnode instanceof EPCFunction && tnode.visualObject != null) {
					...
					theight = (int) (((Double) f.visualObject.getAttributeValue(GrappaConstants.HEIGHT_ATTR)).doubleValue());
