...
public class FSMPayloadExport {
	...
	@Exporter(
			name = "Payload-attributed FSM file",
			help = "http://www.win.tue.nl/~hverbeek/doku.php?id=projects:prom:plug-ins:export:fsm2pfsm",
			extension = "fsm"
	)

	/**
	 * Exports the given FSM to a fsm file. If possible, the payload information
	 * stored at the FSM states is used to generate attribute information.
	...
	public static void FSMExport(FSM fsm, OutputStream out) throws IOException {
		...
		if (!fsm.getVerticeList().isEmpty()) {
			// Not empty, get first state to retrieve the settings used to obtain
			// the FSM.
			FSMState state = (FSMState) fsm.getVerticeList().get(0);
			// If the payload is of class FsmMinerPayload, then we can use this payload
			...
			if (state.getPayload() instanceof FsmMinerPayload) {
				...
				// binary FSM export (which codes all states using a minimal set of
				// boolean attrbiutes.)
				Message.add("No suitable payload found, reverting to binary-attributed export");
	 ...
	 * @param fsm FSM the FSM with payload information.
	 * @param out OutputStream the stream to export to.
	 * @param settings FsmSettings the settings used to generate the FSM. These
	 * settigns determine which attributes should be there. Not in every state
	 * every selected attribute is defined.
	 * @throws IOException
	 */
	private static void export(FSM fsm, OutputStream out, FsmSettings settings) throws IOException {

		/**
		 * Create two maps to store all attribute info. The first map, sysMap, will
		 * contain system information (model element, originator, even type), whereas
		 * the second map (dataMap) will contain log-based attribute info.
		 */
		...
		for (int mode = 0; mode < FsmMinerPayload.LAST; mode++) {
			FsmHorizonSettings horizonSettings = settings.getHorizonSettings(true,mode);
		...
		/**
		 * For the FSM file format, we need to know the index of values. Therefore, we
		 * need to store the constructed informationis a slightly different way.
		 */
		// First, copy all attribute names into an array.
		String[] attributes = new String[sysMap.keySet().size() + dataMap.keySet().size()];
 		int index = 0;
	    ...
	    // Second, copy all attrbiute values etc. into a map of arrays and a map of maps (the inverted first map).
	    ...
	    // Frist, write the attribute info. For sake of simplicity, all have type "stirng".
		...
		bw.write("---\n");
		// Second, write the states. A state corresponds to a value index for every attribute.
		...
		for (ModelGraphVertex vertex : fsm.getVerticeList()) {
			...
			for (index = 0; index < attributes.length; index++) {
				...
				if (attribute.startsWith("Model element, backward")) {
					...
					value = payload.getAttributePayload().get(attribute.replaceFirst("Attribute ", ""));
					if (value == null) {
						value = "<undefined>";
					}
				}
		...
		for (Object object: fsm.getEdges().toArray()) {
			...
			bw.write(String.valueOf(stateIndices.get(edge.getSource())) + " " + String.valueOf(stateIndices.get(edge.getDest())) + " \"" + transition.getCondition() + "\"\n");
		}
