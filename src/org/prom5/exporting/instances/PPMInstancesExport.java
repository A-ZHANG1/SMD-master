...
package org.prom5.exporting.instances;
...
import org.prom5.mining.instancemining.InstanceEPCBuilder;
...
public class PPMInstancesExport implements ExportPlugin {
	...
	public void export(final ProvidedObject object, final OutputStream output) throws IOException {
		...
		SwingWorker w = new SwingWorker() {
			public Object construct() {
				...
				PPMInstancesExportUI ui = new PPMInstancesExportUI(MainUI.getInstance(),
										  "Specify type and typegroup");
				if (!ui.showModal()) {
					return null;
				}
				...
				synchronized (log) {
					...
					while ((logIt.hasNext()) && (!p.isCanceled())) {
						...
						// Message.add("Writing instance:" +pi.getName()+" ATE's: "+pi.getAuditTrailEntries().size());
	...
	private void writeInstance(OutputStream out, ProcessInstance pi) throws IOException {

		InstanceEPC epc = (InstanceEPC) ieb.build(pi);

		writeLn("<graph id=\"" + procType + epc.getIdentifier() + "\" xml:lang=\"en\">", out);
		writeLn("   <attribute type=\"AT_ID\">" + procType + epc.getIdentifier() + "</attribute>",
				out);
		writeLn("   <attribute type=\"AT_EPK_KEY\">" + procType + epc.getIdentifier() +
				"</attribute>", out);
		writeLn("   <attribute type=\"AT_PROCTYPEGROUP\">" + procTypeGroup + "</attribute>", out);
		writeLn("   <attribute type=\"AT_PROCTYPE\">" + procType + "</attribute>", out);
		...
		while (dataKeys.hasNext()) {
			...
			String s = (String) key;
			writeLn("   <attribute type=\"AT_" + key + "\">" + (String) data.get(key) +
					"</attribute>", out);
		}
	...
	public String getHtmlDescription() {
		return "<p> <b>Plug-in: PPM instances export</b>"+
				...
				org.prom5.framework.util.Constants.get_BVD_URLString("making_instance_nets", "this page")+
				". "
				;
	}
}
