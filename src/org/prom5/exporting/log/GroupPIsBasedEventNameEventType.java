...
import org.prom5.exporting.log.util.FilterPerWorkflowModelElementAndEvent;
		...
		SwingWorker w = new SwingWorker() {
			public Object construct() {
				...
				if (log.numberOfProcesses() <= 0) {
					...
					Message.add("<GroupLog(sameSequences)  numberTraceInLogBeforeGrouping=\"" +
	...
	private Iterator filterDuplicateATEs(LogReader logReader) {
		...
		while (logReaderIterator.hasNext()) {
			...
			if (filteredProcessInstances.contains(myPi)) {
				...
				aux.increaseNumberSimilarPIs(MethodsForWorkflowLogDataStructures.
						getNumberSimilarProcessInstances(pi));
