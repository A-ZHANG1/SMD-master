...
import org.prom5.exporting.log.util.FilterSameSetWmeEventFollowsRelation;
import org.prom5.exporting.log.util.MyProcessInstance;
		...
		SwingWorker w = new SwingWorker() {
			public Object construct() {
				...
				if (log.numberOfProcesses() <= 0) {
					...
					int numberTraceInLogBeforeGrouping = log.numberOfInstances();
					...
					while ((iteratorPIs.hasNext()) && (!p.isCanceled())) {
						ProcessInstance pi = ((MyProcessInstance) iteratorPIs.next()).getPI();
						...
						listOfFrequencyOfTraces.add(new Integer(MethodsForWorkflowLogDataStructures.
					...
					//for testing purposes (begin)
					Integer[] orderedElementsInList = new Integer[listOfFrequencyOfTraces.size()];
					orderedElementsInList = (Integer[]) listOfFrequencyOfTraces.toArray(
											orderedElementsInList);
					...
					for (int i = 0; i < orderedElementsInList.length; i++) {
						stringOrderedElementsInList.append(" ").append(orderedElementsInList[i]);
						totalNumTracesGroupedLog += orderedElementsInList[i];
					}
					stringOrderedElementsInList.append(" ]");

					Message.add("<GroupLog(sameFollowsRelation)  numberTraceInLogBeforeGrouping=\"" +
							numberTraceInLogBeforeGrouping +
							"\" numberUniqueTraceInLogAfterGrouping=\"" +
							numberUniqueTraceInLogAfterGrouping +
							"\" totalNumberTraceInLogAfterGrouping=\"" + totalNumTracesGroupedLog +
							"\" frequenciesOfTracesInTheLog=\"" +
							stringOrderedElementsInList.toString() + "\">",
							Message.TEST);
	...
	private Iterator filterDuplicateATEs(LogReader logReader) {
		...
		while (logReaderIterator.hasNext()) {
			...
			if (filteredProcessInstances.contains(newPi)) {
				FilterSameSetWmeEventFollowsRelation existingPi = (
						FilterSameSetWmeEventFollowsRelation) filteredProcessInstances.get(newPi);

				existingPi.increaseNumberSimilarPIs(MethodsForWorkflowLogDataStructures.
						getNumberSimilarProcessInstances(pi));
				existingPi.addGroupedPiIdentifier(newPi.getPI().getName());
				if (newPi.getPI().getAuditTrailEntryList().size() >
						existingPi.getPI().getAuditTrailEntryList().size()) {

					//making the newPi look like the existing one, but keeping the audit
					//trail entries of the new Pi.


					newPi.getPI().setDataAttributes(existingPi.getPI().getDataAttributes());
					newPi.getPI().setDescription(existingPi.getPI().getDescription());
