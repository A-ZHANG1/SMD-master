...
package org.prom5.exporting.log;
...
import org.prom5.exporting.ontologies.SaveOntologiesDialog;
...
import org.prom5.framework.util.OutputStreamWithFilename;
...
public class SAMXMLLogExport implements ExportPlugin {
	...
	public synchronized void export(final ProvidedObject object, final OutputStream output) throws IOException {
		LogReader log = null;
		Object[] o = object.getObjects();
		...
		if (log == null) {
			throw new IOException("Internal error: no log found in provided objects");
		}
	...
	public void export(final LogReader log, final OutputStream output) throws IOException {
	...
	public void writeHeader(OutputStream output, LogReader log) {
		...
		writeLn("", "<WorkflowLog " +
				...
				"\tdescription=\"Exported by ProM framework from " + convertLine(log.getLogSummary().getWorkflowLog().getDescription()) + "\" " +
				getModelReferences(log.getLogSummary().getWorkflowLog().getModelReferences()) + 
				">", output);
		...
		if (!data.keySet().isEmpty()) {
			...
			while (it.hasNext()) {
				...
				writeLn("\t\t", "<Attribute name=\"" + convertLine(key) + "\"" + refs + ">" +
						convertLine(value) + "</Attribute>", output);
			}
			writeLn("\t", "</Data>", output);
		}
		...
		writeLn("\t", "<Source " +
				"program=\"" + convertLine(log.getLogSummary().getSource().getData().get("program")) + "\" " +
				getModelReferences(log.getLogSummary().getSource().getModelReferences()) +
				">",
				output);
		...
		writeLn("\t", "<Process " +
				"id=\"" + convertLine(log.getLogSummary().getProcesses()[0].getName()) + "\" " +
				"description=\"" + convertLine(log.getLogSummary().getProcesses()[0].getDescription()) + "\" " +
	...
	public void writeProcessInstance(ProcessInstance pi, OutputStream output) {
		writeLn("\t\t", "<ProcessInstance " +
				"id=\"" + convertLine(pi.getName()) + "\" " +
				"description=\"" + convertLine(pi.getDescription()) + "\" " +
				getModelReferences(pi.getModelReferences()) +
				">", output);
		...
		while (ates.hasNext()) {
			...
			writeLn("\t\t\t\t", "<WorkflowModelElement" + getModelReferences(ate.getElementModelReferences()) + ">" + convertLine(ate.getElement()) +
					"</WorkflowModelElement>", output);

			if (ate.getType().startsWith("unknown")) {
				writeLn("\t\t\t\t", "<EventType " + getModelReferences(ate.getTypeModelReferences()) + "unknowntype=\"" +
						convertLine(ate.getType().replaceFirst("unknown:", "")) +
						"\">unknown</EventType>", output);
			} else {
				writeLn("\t\t\t\t", "<EventType" + getModelReferences(ate.getTypeModelReferences()) + ">" + convertLine(ate.getType()) + "</EventType>", output);
			}
			...
			if (ate.getOriginator() != null && ate.getOriginator().length() > 0) {
				writeLn("\t\t\t\t", "<Originator" + getModelReferences(ate.getOriginatorModelReferences()) + ">" +
	...
	private void writeDataSection(String indent, DataSection data, OutputStream out) {
		if (!data.keySet().isEmpty()) {
			...
			while (it.hasNext()) {
				...
				String value = (String) data.get(key);
				String refs = getModelReferences(data.getModelReferences(key));
				writeLn(indent, "\t<Attribute " +
						"name=\"" + convertLine(key) + "\" " +
						refs + ">" +
						convertLine(value) +
						"</Attribute>", out);
			}
	...
	public String getHtmlDescription() {
		return "<p> <b>Plug-in: SA-MXML log export</b>"+
				...
				org.prom5.framework.util.Constants.get_BVD_URLString("mining_meta_model","this paper");
	}
