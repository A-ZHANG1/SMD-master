...
/**
 * <p>Title: Filter per Workflow Model Element and Event</p>
 * <p>Description: This class stores a process instance. Its equals method is
 * based on a projection from the "workflow model element" and "event type" fields.</p>
...
public class FilterPerWorkflowModelElementAndEvent implements MyProcessInstance {
	...
	/**
	 * Checks of a process instance is equal to the process instance that is stored
	 * at the current object. Note that a process instance is EQUAL to another one whenever all the
	 * workflow model elements, and their correponding event types, appear in the same order
	 * at both process instances.
	...
	public boolean equals(Object obj) {
		...
		FilterPerWorkflowModelElementAndEvent toCompare = (FilterPerWorkflowModelElementAndEvent)
				obj;
		...
		for (int i = 0; i < processInstance.getAuditTrailEntryList().size(); i++) {
			try {
				if (!processInstance.getAuditTrailEntryList().get(i).getElement().equals(toCompare.
						getPI().getAuditTrailEntryList().get(i).getElement())
						||
						!processInstance.getAuditTrailEntryList().get(i).getType().equals(toCompare.
						getPI().getAuditTrailEntryList().get(i).getType())) {
					return false;
				}
			} catch (Exception e) {
				//could not compare...
	...
	public int hashCode() {
		...
		for (int i = 0; i < processInstance.getAuditTrailEntryList().size(); i++) {
			try {
				hashCode += processInstance.getAuditTrailEntryList().get(i).getElement().hashCode() +
