...
package org.prom5.exporting.log.util;
...
/**
 * <p>Title: Filter per Same Set of Workflow Model Elements and Events with Follows Relation</p>
 * <p>Description: This class stores a process instance. Any other process instance X is
 * equal to the stored process instance Y whenever X and Y have the same set of follows relation with
 * respect to workflow
 * model elements and event types.</p>
...
public class FilterSameSetWmeEventFollowsRelation implements MyProcessInstance {
	...
	public FilterSameSetWmeEventFollowsRelation(ProcessInstance pi, int numberSimilarPIS) {
		if (pi != null) {
			...
			try {
				...
				System.err.println("Could not create an AuditTrailEntryListImpl in constructor of the class org.processmining.exporting.log.util.FilterPerWorkflowModelElementAndEvent!");
			}

			if (pi.getAttributes().containsKey(FilterPerWorkflowModelElementAndEvent.IDENTIFIERS)) {
	...
	public static TreeSet buildFollowsRelations(ProcessInstance pi) {
		...
		if (ates.size() > 1) {
			...
			for (int i = 1; i < ates.size(); i++) {
				try {
					relation = ates.get(i - 1).getElement().trim() + ates.get(i - 1).getType().trim() +
							   " > " +
							   ates.get(i).getElement().trim() + ates.get(i).getType().trim();

					followsRelations.add(relation);
				} catch (Exception e) {
					//do nothing because the "for" loop makes sure that we do not go beyond the boundaries of the list!
				}
			}
		}
	...
	private void setNumberSimilarPIs(String number) {
		processInstance.getAttributes().put(MethodsForWorkflowLogDataStructures.
				NUM_SIMILAR_INSTANCES,
				number);
	...
	/**
	 * Checks if a process instance Y is equal to the process instance X that is stored
	 * at the current object. Note that Y is EQUAL to X whenever the set of follows relations for all the
	 * workflow model elements, and their correponding event types, are the same
	 * at both process instances X and Y.
	 * @param obj object that contains a process instance to be compared to the process instance in the current object.
	 * @return true if the process instances are the same, false otherwise.
	 */
	public boolean equals(Object obj) {

		FilterSameSetWmeEventFollowsRelation toCompare = (
				FilterSameSetWmeEventFollowsRelation) obj;

		if (toCompare == null) {
			return false;
		}

		return this.setFollowsRelations.equals(toCompare.getSetFollowsRelations());
	 ...
	 * @return the hashcode returned by the hashcode method as defined in TreeSet.
	 */
	 ...
	 *
	 * @return boolean True if the pi was successfully set. False otherwise. A pi is successfully
	 * set if it contains the same set of follows relations as the current pi of this object.
	 */
	...
	public void addGroupedPiIdentifier(String idToAppend) {
		if (processInstance.getAttributes().containsKey(FilterPerWorkflowModelElementAndEvent.
				IDENTIFIERS)) {
			...
			processInstance.getAttributes().put(FilterPerWorkflowModelElementAndEvent.IDENTIFIERS,
					identifiers);
		} else {
			processInstance.getAttributes().put(FilterPerWorkflowModelElementAndEvent.IDENTIFIERS,
					idToAppend);
