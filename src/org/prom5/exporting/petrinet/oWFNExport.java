...
/**
 * <p>Title: oWF net export plug-in</p>
 *
 * <p>Description: Exports an oWF net to file, as suggested by Niels Lohmann.</p>
 *
...
public class oWFNExport implements ExportPlugin {
	...
	public void export(ProvidedObject object, OutputStream output) throws IOException {
		...
		for (int i = 0; i < o.length; i++) {
			if (o[i] instanceof PetriNet) {
				...
				Message.add("<oWFNExport nofLines=\"" + nofLines + "\" nofChars=\"" + nofChars + "\"/>", Message.TEST);
				return;
			}
		}
	}
	...
	public String write(oWFNet owfn) {
		...
		result += "  "+ owfn.getSourcePlace().getIdentifier().replaceAll(" ", "_") + ": 1;\n";

		// Fifth, the final condition.
		result += "FINALCONDITION\n";
		result += "  (" + owfn.getSinkPlace().getIdentifier().replaceAll(" ", "_") + " = 1) AND ALL_OTHER_PLACES_EMPTY;\n\n";

	    // Sixth, the transitions.
		for (Transition transition: owfn.getTransitions()) {
			result += "TRANSITION " + transition.getIdentifier().replaceAll(" ", "_") + "\n";
			result += "  CONSUME ";
			...
			while (it2.hasNext()) {
				...
				if (inputs.keySet().contains(place.getIdentifier())) {
					inputs.put(place.getIdentifier(), inputs.get(place.getIdentifier()) + 1);
				} else {
					inputs.put(place.getIdentifier(), 1);
				}
			}
			for (String input: owfn.getInputs().keySet()) {
				// owfn.getInputs().get(input).contains(transition) seems not to work (?).
			...
			for (String input: inputs.keySet()) {
				result += sep + input.replaceAll(" ", "_") + ": " + inputs.get(input);
				sep = ", ";
			}
			result += ";\n";
			...
			while (it2.hasNext()) {
				...
				if (outputs.keySet().contains(place.getIdentifier())) {
					outputs.put(place.getIdentifier(), outputs.get(place.getIdentifier()) + 1);
				} else {
			...
			for (String output: outputs.keySet()) {
				result += sep + output.replaceAll(" ", "_") + ": " + outputs.get(output);
				sep = ", ";
			}
			result += ";\n\n";
		}
