package org.prom5.exporting.sna;
...
public class NetMinerExport implements ExportPlugin {
			...
			public boolean accepts(ProvidedObject object) {
					...
					for (int i = 0; i < o.length; i++) {
							if (o[i] instanceof SocialNetworkMatrix || o[i] instanceof OTMatrix2DTableModel) {
									return true;
							}
					}
					return false;
			...
			public void export(ProvidedObject object, OutputStream output) throws IOException {
					...
					if (snMatrix != null)
					{
						...
						if (orgUnitMatrix != null) numAFF++;

						roleMatrix = snMatrix.getRoleMatrix();
						if (roleMatrix != null) numAFF++;
					}

					if(numAFF!=0) bw.write("\nAFF#="+numAFF+"\n");

					if(otMatrix != null)
							writeOTPart(tasks, users, otMatrix, bw);

	                if(orgUnitMatrix!= null)
						writeAffiliatedMatrix("OrgUnitAssignment", (String[])snMatrix.getOrgUnitName().toArray(new String[snMatrix.getOrgUnitName().size()]), users.length,
					                          orgUnitMatrix,bw);

				    if(roleMatrix!= null)
						writeAffiliatedMatrix("RoleAssignment", (String[])snMatrix.getRoleName().toArray(new String[snMatrix.getRoleName().size()]), users.length,
					                          roleMatrix,bw);
	        ...
	        private  void writeSnaPart(String[] users, DoubleMatrix2D matrix, BufferedWriter bw) throws IOException{

				int matrixSize = users.length;

				bw.write("NTF=2.4\nTITLE=EXPORTED\n");
				bw.write("N=" + String.valueOf(matrixSize) + "\n");
				Message.add("<SummaryOfNetMiner networkSize=\""+ String.valueOf(matrixSize) + "\">", Message.TEST);
				bw.write("NODELIST=");

				for (int j = 0; j < users.length; j++) {
					if (users[j].equals("")) bw.write("noname,");
					else bw.write(users[j] + ",");
				}
				bw.write("\n\nADJ#=1\n\n");
				...
				bw.write("\n/DATA\n\n");
				Message.add("<SummaryOfNetMiner writeSocialNetwork=\"OK\">", Message.TEST);
			}

			private  void writeOTPart(String[] tasks, String[] users, OTMatrix2DTableModel matrix,  BufferedWriter bw) throws IOException{

	           bw.write("AFFNAME=OTMmatrix\n");
			   bw.write("CAT#=" + String.valueOf(matrix.getColumnCount()-1) + "\n");
			   Message.add("<SummaryOfNetMiner OTMatrixSize=\""+ String.valueOf(matrix.getColumnCount()-1) + "\">", Message.TEST);
			   bw.write("CATNAME=");
			   ...
			   for (int j = 0; j < matrix.getRowCount(); j++) {
				   if(users[i].endsWith((String)matrix.getValueAt(j,0))) {
					   for (int k = 0; k < tasks.length; k++)
                        ...
                        bw.write("\n");
						if(i<users.length-1) i++;
						else break;
				   }

			   }
			   bw.write("/DATA\n\n");
			   Message.add("<SummaryOfNetMiner writeOriginatorByTaskMatrix=\"OK\">", Message.TEST);
		   }

		   private  void writeAffiliatedMatrix(String AfMatrixName,String[] catNAME, int numOfUsers, DoubleMatrix2D matrix,  BufferedWriter bw) throws IOException{

						  bw.write("\nAFFNAME="+AfMatrixName+"\n");
						  bw.write("CAT#=" + catNAME.length + "\n");
						  Message.add("<SummaryOfNetMiner AffMatrixSize=\""+ String.valueOf(catNAME.length) + "\">", Message.TEST);
			   			  bw.write("CATNAME=");
						  ...
						  Message.add("<SummaryOfNetMiner writeAffiliatedMatrix=\"OK\">", Message.TEST);
		   }
}
