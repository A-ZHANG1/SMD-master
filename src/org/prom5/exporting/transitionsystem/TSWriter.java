package org.prom5.exporting.transitionsystem;
...
public class TSWriter
{
	public static void writeToPetrify(TransitionSystem ts, Writer bw) throws IOException
	{
		...
		// Eric: Keep track of sources and destination to determine which is the start node (should be source but not destination)
		...
		// Eric: Odd construct: Write to temp file, read from temp file, write again.
		int c;
		while ((c = tr.read()) != -1)
	        bw.write(c);
		tr.close();
		...
		// Eric: Remove all sources that are destinations as well. Should leave us the start node.
	...
	{
		// Eric. Remove everything from the \n. Petrify deos not like backslashes.
