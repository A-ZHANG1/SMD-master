...
import org.prom5.framework.log.filter.LogFilterParameterDialog;
 ...
 * The <code>filter</code> method of the selected log filter is called for each process instance.
 * This <code>filter</code> method can then modify the contents of the given process instance.
 * For example, certain audit trail entries can be deleted.
 * The process instance is completely ignored (discarded) if the <code>filter</code> returns false.
 *
 * The LogFilter objects can be stacked, which means that they are capable of calling eachothers
 * <code>filter</code> method. Note that each LogFilter should call the <code>filter</code> method
...
public abstract class LogFilter {
	...
	/**
	 * A integer that can have three values:
	 * FAST, AVERAGE, SLOW.
	 * This should be set in the constructor (default is SLOW) to tell how
	 * computationally intensive this filter is. FAST should be used if the
	 * computation time of the <code>filter</code> method does not depend on the
	 ...
	 * When filtering, the <code>filter(ProcessInstance pi)</code> method should
	...
	/**
	 * Provides access to the lower level log filter used
	 * by this instance.
	 * @return
	 */
	 ...
	 * further filtering is necessary.
	 *
	 * @param lowLevelFilter the filter that should be checked first, to see
	 * if the instance can be discared anyway. Note that this filter can be null.
	 * @param load the load of this LogFilter (i.e. SLOW, AVERAGE or FAST)
	 * @param name The name of the LogFilter
	 */
	 ...
	 * Note that the implementing classes can assume that the instance is not empty,
	 * i.e. instance.isEmpty() == false.
	 *
	 * @param instance the process instance to filter
	 * @return true if the whole process instance passes the filter,
	 *         false if the process instance should be discarded.
	 */
	protected abstract boolean doFiltering(ProcessInstance instance);

	/**
	 * Filters a single process instance.
	 * The process instance can be modified. This method should start with the
	 * following code:
	 ...
	 *
	 * @param name new name of this LogFilter
	 */
	...
	/**
	 * Method to tell whether this LogFilter changes the log or not. This method is
	 * used in the <code>compare</code> method.
	 *
	 * @return boolean True if this LogFilter changes the process instance in the
	 * <code>filter()</code> method. False otherwise.
	 */
	 ...
	 *
	 * @return boolean True if this LogFilter changes the process instance in the
	 * <code>filter()</code> method, or the lower level filter does. False otherwise.
	 */
	public final boolean changesLog() {
		return thisFilterChangesLog() || (filter == null ? false : filter.changesLog());
	}
	...
	public final String getHelp() {
		return (filter == null ? "" : "" + filter.getHelp() + "<p>") +
				"<i>" + getName() + ":</i><br>" + getHelpForThisLogFilter();
	}

	/**
	 * Returns the help for this LogFilter as HTML text, but
	 * without the <html> and </html> tags!
	 * @return the help as string
	 */
	...
	/**
	 * Write the inside of the <FilterSpecific> tag in the XML export file to the
	 * OutputStream output.
	 *
	 * @param output OutputStream
	 */
	protected abstract void writeSpecificXML(BufferedWriter output) throws IOException;

	/**
	 * Read the inside of the <FilterSpecific> tag in the XML export file from the
	 * InputStream input.
	 *
	 * @param input InputStream
	 */
	protected abstract void readSpecificXML(Node logFilterSpecifcNode) throws IOException;
	...
	public void writeXML(BufferedWriter output) throws IOException {
		output.write("<LogFilter load=\"" + load + "\" name=\"" + name + "\" class=\"" +
				getClass().getName() + "\">\n");
	 ...
	 * @param input InputStream
	 */
	public static LogFilter readXML(Node logFilterNode) throws IOException, ClassNotFoundException,
			IllegalAccessException, InstantiationException {
		// Assume logFilterNode is the Node containing LogFilter;
		String className = logFilterNode.getAttributes().getNamedItem("class").getNodeValue();
		LogFilter result = LogFilterCollection.getInstance().get(className).getClass().newInstance();
//		LogFilter result = (LogFilter) Class.forName(className, true,
//						   Thread.currentThread().getContextClassLoader()).newInstance();

		result.load = Integer.parseInt(logFilterNode.getAttributes().getNamedItem("load").
					  getNodeValue());
		result.name = logFilterNode.getAttributes().getNamedItem("name").getNodeValue();

		for (int i = 0; i < logFilterNode.getChildNodes().getLength(); i++) {
			if (logFilterNode.getChildNodes().item(i).getNodeName().equals("FilterSpecific")) {
				// Read the nodeSpecific part of this LogFilter
				result.readSpecificXML(logFilterNode.getChildNodes().item(i));
			} else if (logFilterNode.getChildNodes().item(i).getNodeName().equals("LogFilter")) {
				result.setLowLevelFilter(LogFilter.readXML(logFilterNode.getChildNodes().item(i)));
			}
		}
