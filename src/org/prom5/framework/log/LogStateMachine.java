...
public class LogStateMachine {
    ...
    public final static String[] STATE_TYPES = {
                                               INITIALIZED, SCHEDULED, ASSIGNED,
                                               STARTED, COMPLETED,
                                               SUSPENDED, ABORTED};
    ...
    /**
     * Return true if the given string equals any of the constants
     * SCHEDULE, ASSIGN, REASSIGN, START, AUTOSKIP,
     * MANUALSKIP, PI_ABORT, ATE_ABORT, WITHDRAW, COMPLETE,
     * SUSPEND, RESUME
     *
     * @param evt String possible event name to check
    ...
    private LogStateMachine() {
        ...
        occursAfter.put(SCHEDULE, new String[] {ASSIGN, REASSIGN, MANUALSKIP,
                        WITHDRAW, PI_ABORT,
                        START, RESUME, SUSPEND, COMPLETE, ATE_ABORT});
        occursAfter.put(ASSIGN, new String[] {REASSIGN, MANUALSKIP, WITHDRAW,
                        PI_ABORT,
                        START, RESUME, SUSPEND, COMPLETE, ATE_ABORT});
        occursAfter.put(REASSIGN, new String[] {REASSIGN, MANUALSKIP, WITHDRAW,
                        PI_ABORT,
                        START, RESUME, SUSPEND, COMPLETE, ATE_ABORT});
        ...
        /*
         ...
         SMNode schedule = new SMNode(), assign = new SMNode(), start = new SMNode();
         SMNode suspend = new SMNode(), autoskip = new SMNode(), pi_abort = new SMNode();
           root = new SMNode();
           root.put(SCHEDULE, schedule);
           root.put(AUTOSKIP, autoskip);
           root.put(PI_ABORT, pi_abort);
           schedule.put(ASSIGN, assign);
           schedule.put(WITHDRAW, pi_abort);
           schedule.put(PI_ABORT, pi_abort);
           schedule.put(MANUALSKIP, autoskip);
           assign.put(REASSIGN, assign);
           assign.put(START, start);
           assign.put(PI_ABORT, pi_abort);
           assign.put(WITHDRAW, pi_abort);
           assign.put(MANUALSKIP, autoskip);
           start.put(SUSPEND, suspend);
           start.put(PI_ABORT, pi_abort);
           start.put(ATE_ABORT, pi_abort);
           start.put(COMPLETE, autoskip);
           suspend.put(RESUME, start);
           suspend.put(ATE_ABORT, pi_abort);
           suspend.put(PI_ABORT, pi_abort);
         */
    }
    ...
    public boolean mayEventuallyOccurAfter(String eventBefore,
                                           String eventAfter) {
