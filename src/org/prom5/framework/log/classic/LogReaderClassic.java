...
package org.prom5.framework.log.classic;
...
import org.prom5.analysis.summary.ExtendedLogSummary;
 ...
 * <p>
 * This class allows a workflow log to be read by reading one process instance at a time.
 * A log is explicitly not read into memory completely, to allow very large logs to be read.
 * <p>
 * A log file will be opened when it is first needed,
 * i.e. when <code>next()</code>, <code>hasNext()</code>,
 * <code>reset()</code> or <code>getLogSummary()</code> is first called.
 ...
 * This summary information is available by calling the <code>getLogSummary()</code> method.
 *
...
public class LogReaderClassic extends LogReader {
	...
	/**
	 * Derive a new log reader, based on the given log reader and
	 * filtered by the given filter.
	 * If the filter parameter is null, no filtering is done.
	 * Note that the log file is not actually opened for reading yet.
	 * @param filter the filter to use for filtering the log file
	 * @param inputFile the log file to read
	 */
	 ...
	 * but only give the instances with the given indices.
	 * Note that the log file is not actually opened for reading yet.
	 * Note also that the int[] processInstancesToKeep should be sorted.
	 *
	 * @param oldLogReader the original log reader
	 * @param pitk the process instances to keep
	 *
	...
	protected LogReaderClassic(LogReader oldLogReader, int[] pitk) {
		...
		// Now, we need to update the list of indices we need to keep, with the old list we have
	...
	public String toString() {
		return "LogReader: " + getLogSummary().getNumberOfProcessInstances() + " instances from " +
				inputFile;
	}
	...
	private void initialize() {
		if (!initialized) {
			...
			currentIndex = -1;
			indexInKeepList = 0;
			nextIndexToKeep = (processInstancesToKeep == null ? 0 : processInstancesToKeep[0]);
			...
			try {
				...
				summary.setWorkflowLog(new InfoItem("IO Exception occurred", ex.getMessage(), null, null));
			} catch (XmlPullParserException ex) {
				summary.setWorkflowLog(new InfoItem("XML Exception occurred", ex.getMessage(), null, null));
			}
			reset();
	...
	private void fetchNext() {
		...
		do {
			do {
				...
				filterResult = current != null
							   ? (filter == null ? true : filter.filter(current))
							   : false;
			} while (current != null && (filterResult == false || current.isEmpty()));
			currentIndex++;
	...
	private boolean atEndTag(String name) throws XmlPullParserException {
		return parser.getEventType() == XmlPullParser.END_TAG && name.equals(parser.getName());
	}

	private boolean atStartTag(String name) throws XmlPullParserException {
		return parser.getEventType() == XmlPullParser.START_TAG && name.equals(parser.getName());
	}
	...
	private void parseWorkflowLog() throws XmlPullParserException, IOException {
		...
		while (!(eventType == XmlPullParser.END_DOCUMENT) && !atEndTag("WorkflowLog")) {
			if (atStartTag("Data")) {
		...
		summary.setWorkflowLog(new InfoItem("", curWorkflowLog, curWorkflowLogData, null));
	}
	...
	private void parseProcess() throws XmlPullParserException, IOException {
		...
		curProcessName = parser.getAttributeValue(null, "id");
		while (!(eventType == XmlPullParser.END_DOCUMENT) && !atEndTag("Process")) {
			if (atStartTag("Data")) {
				parseData(curProcessData);
			} else if (atStartTag("ProcessInstance")) {
				if (!processAdded) {
					summary.addProcess(new InfoItem(curProcessName, curProcessDesc, curProcessData, null));
					processAdded=true;
				}
				ProcessInstance pi = parseProcessInstance();

				if ((pi != null) && (filter == null || filter.filter(pi)) && (!pi.isEmpty())) {
					currentIndex++;

					if ((processInstancesToKeep == null) || (currentIndex == nextIndexToKeep)) {
	...
	private ProcessInstance parseProcessInstance() throws XmlPullParserException, IOException {
		...
		return new ProcessInstanceClassic(curProcessName, name, descr, data, ate);
	}
	...
	private void parseData(Map data) throws XmlPullParserException, IOException {
		while (!atEndTag("Data")) {
			if (atStartTag("Attribute")) {
				...
				data.put(name == null ? "" : name.trim(), value == null ? "" : value.trim());
			}
			parser.next();
		}
	}
	...
	private AuditTrailEntry parseAuditTrailEntry() throws XmlPullParserException, IOException {
		...
		while (!atEndTag("AuditTrailEntry")) {
			if (atStartTag("Data")) {
				...
				String unknownType = parser.getAttributeValue(null, "unknowntype");
		...
		return new AuditTrailEntryClassic(element, type, timestamp, originator, data);
	}
