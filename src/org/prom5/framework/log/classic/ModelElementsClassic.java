...
public class ModelElementsClassic implements ModelElements, Cloneable {
	...
	public ModelElementsClassic(ProcessInstance pi) {
		...
		while (ates.hasNext()) {
			...
			if (lastEntry == null || !sm.mayEventuallyOccurAfter(lastEntry.getType(), ate.getType())) {
	...
	/* (non-Javadoc)
	 * @see org.processmining.framework.log.classic.ModelElements#find(java.lang.String)
	 */
	...
	public ModelElement last() {
		return elements.size() > 0
				? (ModelElement) elements.get(elements.size() - 1)
				: null;
	}
