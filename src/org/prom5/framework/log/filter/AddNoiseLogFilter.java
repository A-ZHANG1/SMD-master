...
import org.prom5.framework.util.GUIPropertyFloat;
import org.prom5.framework.util.GUIPropertyLong;
import org.w3c.dom.Node;

/**
 * <p>Title: AddNoiseLogFilter</p>
 *
 * <p>Description: This class adds noise to a log. There are 5 possible noise types:
 * <i>head</i>, <i>body</i>, <i>tail</i>, <i>swap</i> and <i>remove</i>. The
 * noise types <i>head</i>, <i>body</i> and <i>tail</i> respectively remove
 * <i>at most!</i> the first, second or third 1/3  of a process instance. The noise type <i>swap</i> randomly
 ...
 * <p>NOTE: This class does not treat input logs in which the process instances are grouped!</p>
...
//noise types
	...
	private AddNoiseLogFilter(boolean[] selectedNoiseTypes, float noisePercentage, long seed) {
		super(LogFilter.MODERATE, "Add Noise Log Filter");
	...
	private static int[] getSelectedNoiseTypesIndeces(boolean[] types) {
         ...
         * @return true if the whole process instance passes the filter, false
         *   if the process instance should be discarded.
         */
        protected boolean doFiltering(ProcessInstance instance) {

                Random r = new Random(seed + instance.toString().hashCode());

                if (r.nextDouble() < noisePercentage && selectedNoiseTypesIndeces.length > 0) {

                        AuditTrailEntryList entries = instance.getAuditTrailEntryList();
                        double oneThird = entries.size() / 3.0;

                        switch (selectedNoiseTypesIndeces[safeNextInt(r, selectedNoiseTypesIndeces.length)]) {

                                case HEAD_INDEX:

                                        //first, advancing the pointer so that the whole head is not removed in all cases
                                        ...
                                        int sizeSubstringToRemoveFromHead = safeNextInt(r,((int) oneThird) -
                                                        startToRemoveHeadAtPosition + 1); //because it is exclusive...
                                ...
                                case BODY_INDEX:
                                        ...
                                        //first, advancing the pointer so that the whole body is not removed in all cases
                                        //the body goes from position 1/3 until position ((2/3) - 1)
                                        int startToRemoveBodyAtPosition = safeNextInt(r,(int) oneThird) + (int) oneThird;

                                        //the actual removal happens here
                                        int sizeSubstringToRemoveFromBody = safeNextInt(r,(int) oneThird -
                                                        (startToRemoveBodyAtPosition - (int) oneThird) + 1);
                                ...
                                case TAIL_INDEX:
                                        ...
                                        //the actual removal happens here
                                        int sizeSubstringToRemoveFromTail = safeNextInt(r,(int) oneThird -
                                                        (startToRemoveTailAtPosition - (int) (2 * oneThird)) + 1);
	...
	public LogFilterParameterDialog getParameterDialog(LogSummary summary) {
		return new LogFilterParameterDialog(summary, AddNoiseLogFilter.this) {
			...
			protected JPanel getPanel() {
				//noise percentage
				noisePercentageProperty = new GUIPropertyFloat("Noise Percentage",
				...
				seedProperty = new GUIPropertyLong("Seed",
				...
				settingsPanel.add(seedProperty.getPropertyPanel(), BorderLayout.SOUTH);
			...
			public LogFilter getNewLogFilter() {
				return new AddNoiseLogFilter(selectedNoiseTypes,
						noisePercentageProperty.getValue(),
	...
	protected void writeSpecificXML(BufferedWriter output) throws IOException {
		output.write("<selectedNoiseTypes>" + toStringBooleanArray(selectedNoiseTypes) +
				"</selectedNoiseTypes>\n");
		output.write("<noisePercentage>" + noisePercentage + "</noisePercentage>\n");
	...
	protected void readSpecificXML(Node logFilterSpecifcNode) throws IOException {
		for (int i = logFilterSpecifcNode.getChildNodes().getLength() - 1; i >= 0; i--) {
			...
			if (n.getNodeName().equals("selectedNoiseTypes")) {
				selectedNoiseTypes = fromStringToBooleanArray(n.getFirstChild().getNodeValue());
			} else if (n.getNodeName().equals("noisePercentage")) {
				noisePercentage = Float.parseFloat(n.getFirstChild().getNodeValue());
...
class NoiseTypesCheckBoxPanel extends JPanel implements ItemListener {
	...
	public NoiseTypesCheckBoxPanel(AddNoiseLogFilter parentClass) {
		...
		for (int i = 0; i < noiseTypesCheckBoxes.length; i++) {
			noiseTypesCheckBoxes[i] = new JCheckBox(AddNoiseLogFilter.NOISE_TYPES[i]);
			noiseTypesCheckBoxes[i].addItemListener(this);
			noiseTypesCheckBoxes[i].setSelected(this.parentClass.getSelectedNoiseTypes()[i]);
		}
		...
		noiseTypesLabel.setToolTipText("Select the types of noise to be inserted in the log.");
