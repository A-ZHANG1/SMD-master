...
import org.prom5.framework.ui.DefaultLogFilterUI;
 ...
 * 1) INCLUDE: Include all AuditTrailEntries
 * 2) DISCARD: Ignore the AuditTrailEntry, but not the ProcessInstance
 * 3) DISCARD_INSTANCE: Ignore the whole ProcessInstance if a certain LogEvent is found.
 *
...
public class DefaultLogFilter extends LogFilter {
	...
	protected boolean doFiltering(ProcessInstance instance) {
		...
		for (int i = entries.size() - 1; i >= 0; i--) {
			...
			try {
				entry = entries.get(i);
				int action = events.containsKey(entry.getType())
							 ? ((Integer) events.get(entry.getType())).intValue()
							 : defaultBehavior;
	...
	public LogFilterParameterDialog getParameterDialog(LogSummary summary) {
		return new LogFilterParameterDialog(summary, this) {
			...
			protected JPanel getPanel() {
				ui = new DefaultLogFilterUI(summary, false, process, (HashMap) events);
				return ui;
			}
	...
	protected void writeSpecificXML(BufferedWriter output) throws IOException {
		output.write("<defaultBehavior>" + defaultBehavior + "</defaultBehavior>\n");
	...
	protected void readSpecificXML(Node logFilterSpecifcNode) throws IOException {
		...
		for (int i = logFilterSpecifcNode.getChildNodes().getLength() - 1; i >= 0; i--) {
			...
			if (n.getNodeName().equals("defaultBehavior")) {
				defaultBehavior = Integer.parseInt(n.getFirstChild().getNodeValue());
			} else if (n.getNodeName().equals("process")) {
				process = n.getFirstChild().getNodeValue();
			} else if (n.getNodeName().equals("rule")) {
				String key = n.getAttributes().getNamedItem("event").getNodeValue();
