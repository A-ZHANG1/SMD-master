...
public class EndLogFilter extends LogFilter {
	 ...
	 *   if the process insstance should be discarded
	...
	protected boolean doFiltering(ProcessInstance instance) {
		String lastElementInstance = instance.getAuditTrailEntries().last().getElement();
		String lastElementTypeInstance = instance.getAuditTrailEntries().last().getType();
	...
	public LogFilterParameterDialog getParameterDialog(LogSummary summary) {
		return new LogFilterParameterDialog(summary, EndLogFilter.this) {
			...
			protected JPanel getPanel() {
				...
				for (int i = 0; i < checks.length; i++) {
					p.add(checks[i]);
					if ((endEvents != null) && (endEvents.contains(checks[i].getLogEvent()))) {
