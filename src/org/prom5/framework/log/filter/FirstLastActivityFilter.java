...
/**
 * This filter retains only the first and last occurrence of any activity in the process instance
 * 
 * @author (Idea Proposed by: Prof. Wil van der Aaalst; Implemented by: jcbose (R. P. Jagadeesh Chandra 'JC' Bose))
 */
...
public class FirstLastActivityFilter extends LogFilter {
	...
	protected boolean doFiltering(ProcessInstance instance) {
		...
		while(it.hasNext()){
			currentAuditTrailEntry = it.next();
			LogEvent e = eventsToFilter.findLogEvent(currentAuditTrailEntry.getElement(), currentAuditTrailEntry.getType());
			if(e != null && !activityFirstOccurrenceMap.containsValue(currentIndex) && !activityLastOccurrenceMap.containsValue(currentIndex)){
				it.remove();
			}else if(e != null && activityFirstOccurrenceMap.containsValue(currentIndex) && activityLastOccurrenceMap.containsValue(currentIndex)){
				//only one occurrence of the activity; change the type name to start/complete
				currentAuditTrailEntry.setType("Start/Complete");
			}else if(e != null && activityFirstOccurrenceMap.containsValue(currentIndex)){
				currentAuditTrailEntry.setType("Start");
			}else if(e != null && activityLastOccurrenceMap.containsValue(currentIndex)){
...
//		System.out.println(initialSize+ " @ "+entries.size());
	...
	public LogFilterParameterDialog getParameterDialog(LogSummary summary) {
		return new LogFilterParameterDialog(summary,FirstLastActivityFilter.this){
				...
				deSelectAllButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						for(int i = 0; i < checks.length; i++){
							if((eventsToFilter != null) && eventsToFilter.contains(checks[i].getLogEvent()))
