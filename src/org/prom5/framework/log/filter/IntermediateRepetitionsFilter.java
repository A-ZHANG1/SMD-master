...
/**
 * Filter removes all directly succeding log events that are repetitions (i.e., occurrences of the
 * same log event) but the first and the last event. The first occurrence is made a Start event, the 
 * last occurrence is made an End event. 
 * 
 * @author arozinat
 */
public class IntermediateRepetitionsFilter extends LogFilter {
	 ...
	 *
	 * @return boolean True if this LogFilter changes the process instance
	 *   in the <code>filter()</code> method. False otherwise.
...
// 		This is handled by the filter method of LogFilter and does not belong here
//		if ((filter != null) && !filter.filter(instance) || instance.isEmpty()) {
//			return false;
//		}
		...
		try {
			...
			int index = 0;
			boolean isFirst = true; // indicates when a new series of repetitions has started
			...
			while (index < entries.size()) {
				...
				if (index + 1 < entries.size()) {
					...
					if (current.getElement().equals(next.getElement())) {
						///event type should be ignored so that first start and last complete event are
						///used for the activity mapping
						//	&& current.getType().equals(next.getType())) {
						
						if (isFirst == true) {
							// make start event if is first ate in series of repetitions
							...
							// remove as is intermediate ate in series of repetitions
			...
			Message.add("An error occurred during filtering process instance " + instance.getName());
			ex.printStackTrace();
	 ...
	 * @throws Exception
	 */
	private int finishActivity(boolean isFirst, AuditTrailEntry current, int index, 
			AuditTrailEntryList entries) throws Exception {
	...
	public LogFilterParameterDialog getParameterDialog(LogSummary summary) {
		return new LogFilterParameterDialog(summary, IntermediateRepetitionsFilter.this) {
