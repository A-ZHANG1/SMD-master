...
/**
 * <p>Title: LogAttributeReplacementFilter</p>
 *
 * <p>Description:
 * This class make it possible to replace activity names or originators by other attributes (e.g. activity name, role, etc.)
 * </p>
 *
 * @author Minseok Song
 * @version 1.0
...
//	private GUIPropertyBoolean assignOrgModelChecked = new GUIPropertyBoolean("Assign Org Model", false);
	...
	public LogAttributeReplacementFilter(String repFrom, String repTo, JTextField orgModelFile) {
		super(LogFilter.MODERATE, "Replacement Filter");
		this.repFrom = repFrom;
		this.repTo = repTo;
...
//		assignOrgModelChecked.setSelected(true);
		...
		try {
			if (repFrom.equals("Originator")) {
				if (repTo.equals("Task")) {
					...
					for (int i = 0; i < entries.size(); i++) {
						AuditTrailEntry entry = entries.get(i);
						entry.setOriginator(entry.getElement() + "_" + entry.getOriginator());
						entries.replace(entry, i);
					}
					...
					for (int i = 0; i < entries.size(); i++) {
						AuditTrailEntry entry = entries.get(i);
						List<String>
								list = orgModel.getOrgEntityList(entry.getOriginator(),
				...
				if (repTo.equals("Originator")) {
					...
					for (int i = 0; i < entries.size(); i++) {
						AuditTrailEntry entry = entries.get(i);
						entry.setElement(entry.getElement() + "_" + entry.getOriginator());
						entries.replace(entry, i);
					}
				} else if (repTo.equals("Role") && useOrgModel) {
					for (int i = 0; i < entries.size(); i++) {
	...
	public LogFilterParameterDialog getParameterDialog(LogSummary summary) {
		return new LogFilterParameterDialog(summary, LogAttributeReplacementFilter.this) {
			...
			public LogFilter getNewLogFilter() {
				if (assignOrgModelChecked.getValue()) {
					return new LogAttributeReplacementFilter(fromList.getSelectedItem().toString(),
							toList.getSelectedItem().toString(), orgModelFile);
				}
				return new LogAttributeReplacementFilter(fromList.getSelectedItem().toString(),
						toList.getSelectedItem().toString());
			}
			...
			protected JPanel getPanel() {
				...
				String[] valuesTo = {"Task", "Originator", "Task+Originator", "Role", "Org Unit"};
				...
				assignOrgModelChecked = new GUIPropertyBoolean("Assign Org Model", false);
			...
			public boolean isAssignOrgModelChecked() {
				return assignOrgModelChecked.getValue() && (orgModelFile.getText() != "");
			}
	...
	protected void writeSpecificXML(BufferedWriter output) throws IOException {
		...
		if (useOrgModel) {
			output.write("<logReplaceOrgFile>" + orgModelFileName + "</logReplaceOrgFile>\n");
		}
	}
