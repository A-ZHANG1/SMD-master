 ...
 *
 * During construction, a list of events to keep has to be provided, together with
 * another LogFilter. Then afterwards, using
 * the method filterEventType(), specific events are filtered.
 *
...
public class LogEventLogFilter extends LogFilter {
	...
	// 29 March rmans, changed so that from the inheriting classes (LogEventLogFilterEnh)
	...
	protected boolean doFiltering(ProcessInstance instance) {
		...
		while (entries.hasNext()) {
			...
			LogEvent e = eventsToKeep.findLogEvent(entry.getElement(), entry.getType());
