...
import org.prom5.framework.log.rfb.LightweightLogSummary;
 ...
 * Furthermore, it is possible to filter for tasks of which the frequency (in percentages) in the log
 * is less than a given percentage and you can filter for tasks for
...
public class LogEventLogFilterEnh extends LogEventLogFilter {
	...
	/**
	* default value of the percentage to filter for tasks of which
	* the frequency (in percentages) in the log
	* is less than a given percentage
	*/
	private double percentageTask = 0.0;

	/**
	 * default value of the percentage for filtering for tasks of which
	 * the freqency (in percentages) of in how many different process instances they occur is less than
	 * a given percentage.
	 */
	 ...
	 * provided.
	 * @param eventsToKeep LogEvents the logevents that have to be kept after filtering.
	 */
	...
	/**
	 * Constructor. The events that have to be kept after filtering can be provided.
	 * Also, the percentage values for filtering for tasks of which the frequency is less than
	 * a given percentage and for filtering for tasks for
	 * which the freqency (in percentages) of in how many different process instances they occur is less than
	 * a given percentage can be provided.
	 * @param eventsToKeep LogEvents the logevents that have to be kept after filtering.
	 * @param percTask double the percentage for filtering for tasks of which the frequency is less than
	 * a given percentage
	 * @param percPI double the percentage for filtering for tasks of which the
	 * frequency in how many many different process instances they occur is less than
	 * a given percentage
	 */
	public LogEventLogFilterEnh(LogEvents eventsToKeep, double percTask, double percPI) {
		super(LogFilter.MODERATE, "Enhanced event log filter", eventsToKeep);
	...
	public LogFilterParameterDialog getParameterDialog(LogSummary summary) {
		return new LogFilterParameterDialog (summary, LogEventLogFilterEnh.this) {
			...
			protected JPanel getPanel() {
				...
				textPanel.add(new JLabel("The Calculate button needs to be clicked to calculate which tasks need to be selected!!!"), java.awt.BorderLayout.CENTER);
				textPanel.add(new JLabel("Clicking the OK button only accepts, but nothing is again calculated!!!!"), java.awt.BorderLayout.SOUTH);
			...
			private double getPercentagePI(LogEvent evt) {
				...
				if (value != null) {
					// calculate frequency
					returnPercent = (((Integer) value).doubleValue() /
					new Double(numberPI).doubleValue()) * 100;
				}
				...
				while (instIterator.hasNext()) {
					...
					while (ates.hasNext()) {
						...
						if (ate.getElement().trim().equals(evt.getModelElementName().trim()) &&
							...
							returnFrequency += MethodsForWorkflowLogDataStructures.getNumberSimilarProcessInstances(pi);
						}
					}
				}
	...
	protected void writeSpecificXML (BufferedWriter output) throws IOException {
		output.write("<percentageTask>" + this.percentageTask + "</percentageTask>\n");
		output.write("<percentagePI>" + this.percentagePI + "</percentagePI>\n");
		output.write("<selectedItemComboBox>" + this.selectedItemComboBox + "</selectedItemComboBox>\n");
	...
	protected void readSpecificXML (Node logFilterSpecifcNode) throws IOException {
		...
		for (int i = 0; i < logFilterSpecifcNode.getChildNodes().getLength(); i++) {
			...
			if (n.getNodeName().equals("logEvent")) {
				...
				int occ = Integer.parseInt(n.getAttributes().getNamedItem("numsim").getNodeValue());
				eventsToKeep.add(new LogEvent(name, type, occ));
			}
			else if (n.getNodeName().equals("percentageTask")) {
				percentageTask = Double.parseDouble(n.getFirstChild().getNodeValue());
			}
			else if (n.getNodeName().equals("percentagePI")) {
				percentagePI = Double.parseDouble(n.getFirstChild().getNodeValue());
			}
