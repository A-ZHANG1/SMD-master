...
import org.prom5.framework.ui.ProMHTMLEditorKit;
...
public abstract class LogFilterParameterDialog extends JDialog {
	...
	public LogFilterParameterDialog(LogSummary summary, LogFilter filter) {
		...
		abstrPanel = getPanel();
		this.getContentPane().add(new JScrollPane(abstrPanel), BorderLayout.CENTER);
	...
	public void showHelpDialog(LogFilter f) {
		...
		final JDialog d = new JDialog(MainUI.getInstance(), "Log Filter Help", true);
		...
		JScrollPane sp = new JScrollPane(jtp, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
		...
		d.setBounds(Math.max(0, (screenSize.width - width) / 2),
				Math.max(0, (screenSize.height - height) / 2),
				width, height);
		d.setSize(width, height);
		d.setLocation(Math.max(0, (screenSize.width - width) / 2),
				Math.max(0, (screenSize.height - height) / 2));
	...
	/**
	 * Returns a settings panel of the LogFilter filter. Note that
	 * this should be implemented in such a way that settings that have been
	 * made once are reflected when it is called again!
	 *
	 * @return JPanel
	 */
