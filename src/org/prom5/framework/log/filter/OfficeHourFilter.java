 ...
 *
 * During construction, a list of originators to keep has to be provided, together with
 * another LogFilter. Then afterwards, using
 * the method doFiltering(), specific originators are filtered.
 *
...
public class OfficeHourFilter extends LogFilter {
	...
	public OfficeHourFilter(long offset, int start, int end, 
			int timezoneOffset, boolean holiday){//GUIPropertyBoolean[] dateUI) {
		super(LogFilter.MODERATE, "Office Hour Filter");
		this.offset = offset;
	...
	/**
	 * The log is changed, it events are present in the processinstance that
	 * are not in the list given in the constructor.
	 * @return boolean true
	 */
	...
	protected String getHelpForThisLogFilter() {
		return
				"Removes all AuditTrailEntries from the log that do not correspond " +
	...
	public LogFilterParameterDialog getParameterDialog(LogSummary summary) {
		return new LogFilterParameterDialog(summary, OfficeHourFilter.this) {
			...
			public LogFilter getNewLogFilter() {
				long offset = summary.getStartTime(summary.getProcesses()[0].getName()).getTime()
					+timeZone.getValue()*3600000L;
//				offset = (offset+3600000L)/(long)86400000L*(long)86400000L;
				return new OfficeHourFilter(offset,startValue.getValue(),targetValue.getValue(), 
						timeZone.getValue(), holidayUI.getValue());//date);
			}
			...
			protected JPanel getPanel() {
				...
				holidayUI = new GUIPropertyBoolean("Considering weekends", false);
				GregorianCalendar calendar = new GregorianCalendar();
				int k = (calendar.getTimeZone().getDefault().getRawOffset() + calendar.getTimeZone().getDSTSavings())/3600000;
				timeZone  = new GUIPropertyInteger("Timezone Offset: ", "", k,0,24);;
...
/*	protected long workingTime(AuditTrailEntry ate)
	{
		...
		long day_duration = day/(long)86400000L - offset/(long)86400000L;
		long hour_offset = day - day/(long)86400000L*(long)86400000L +3600000L - 3600000L*startTime;
		long hour_diff = Math.min(Math.max(hour_offset,0),3600000L*(endTime-startTime));
		diff = day_duration*3600000L*(endTime-startTime) + hour_diff;
		if(holiday) diff -= numHoliday(ate)*3600000L*(endTime-startTime);
		return diff;
	}
*/
	protected long workingTime(AuditTrailEntry ate)
	{
		...
		long adjustedEnd = Math.max(end%(long)86400000L-3600000L*startTime,0);
		adjustedEnd = Math.min(adjustedEnd, 3600000L*(endTime-startTime));
		long adjustedBegin = Math.max(start%(long)86400000L-3600000L*startTime,0);
		adjustedBegin = Math.min(adjustedBegin, 3600000L*(endTime-startTime));
		...
		if(duration==0) {
			...
			diff += 3600000L*(endTime-startTime)- adjustedBegin + adjustedEnd;
		}
		return diff;
	}
	...
	protected long workingTimeHoliday(AuditTrailEntry ate)
	{
		...
		if(duration==0) {
			...
			long temp = start;
			diff = 0;
			if(!isWeekend(start)) diff += 3600000L* (endTime - startTime) - adjustedBegin;
			for(int i=1;i<duration;i++)
			{
				temp +=86400000L;
				if(!isWeekend(temp)) diff+= 3600000L*(endTime-startTime);
			}
			if(!isWeekend(end)) {
				diff +=  adjustedEnd;
			}
	...
	protected int numHoliday(AuditTrailEntry ate)
	{
		...
		if(duration>=2) {
			...
			for(int i=1;i<duration-1;i++)
			{
				...
				if(date==Calendar.SATURDAY||date==Calendar.SUNDAY) numHoliday++;
			}
		} 
		return numHoliday;
	}
	 ...
	 */
	protected void readSpecificXML(Node logFilterSpecifcNode) throws
			IOException {
