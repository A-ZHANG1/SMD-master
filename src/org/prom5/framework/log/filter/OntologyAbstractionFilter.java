package org.prom5.framework.log.filter;
...
public class OntologyAbstractionFilter extends LogFilter {
	...
	public OntologyAbstractionFilter(List<List<String>> selection, List<ConceptSelection> conceptPriority, boolean usePriority, 
			boolean replaceInstancesElement, boolean replaceInstancesType, boolean replaceInstancesOriginator,
			boolean removePI, boolean removeWfme, boolean removeType, boolean removeDataAttr, boolean removeOriginator) {
		this();
	...
	protected boolean doFiltering(ProcessInstance instance) {
		instance.setModelReferences(getUris(replace(instance.getModelReferences())));
		...
		try {
			...
			while (i < ateList.size()) {
				...
				Map<String, String> elementRefs = replace(ate.getElementModelReferences());
				Map<String, String> typeRefs = replace(ate.getTypeModelReferences());
				Map<String, String> originatorRefs = replace(ate.getOriginatorModelReferences());
				
				boolean shouldRemove = false;
				shouldRemove = shouldRemove || ((removeWfme || replaceInstancesElement) && elementRefs.isEmpty());
				shouldRemove = shouldRemove || ((removeType || replaceInstancesType) && typeRefs.isEmpty());
				...
				if (replaceInstancesOriginator) {
					if (originatorRefs.isEmpty()) {
						...
						ate.setOriginator(getReplacementString(originatorRefs.values()));
					}
				}
	...
	private OntologyModel getOntology(String uriInLog) {
		...
		if (model == null) {
			try {
				model = new OntologyModel(ontologyUri, new HashMap<String, Set<String>>(0));
				ontologies.put(ontologyUri, model);
			} catch (MalformedURLException e) {
				Message.add(ontologyUri + ": malformed url: " + e.getMessage(), Message.ERROR);
			} catch (IOException e) {
				Message.add(ontologyUri + ": could not read ontology: " + e.getMessage(), Message.ERROR);
			} catch (ParserException e) {
				Message.add(ontologyUri + ": could not parse ontology: " + e.getMessage(), Message.ERROR);
			} catch (InvalidModelException e) {
				Message.add(ontologyUri + ": invalid model: " + e.getMessage(), Message.ERROR);
			} catch (InconsistencyException e) {
				Message.add(ontologyUri + ": inconsistency error: " + e.getMessage(), Message.ERROR);
			} catch (InternalReasonerException e) {
				Message.add(ontologyUri + ": internal ontology reasoner error: " + e.getMessage(), Message.ERROR);
			} catch (org.prom5.framework.models.ontology.InvalidModelException e) {
				Message.add(ontologyUri + ": invalid model: " + e.getMessage(), Message.ERROR);
			}
		}
		return model;
	...
	private Map<String, String> replace(List<String> modelReferences) {
		...
		if (usePriority && !newConcepts.isEmpty()) {
			...
			for (Map.Entry<String, ConceptModel> item : newConcepts.entrySet()) {
				int index = conceptPriority.indexOf(new ConceptSelection(item.getValue().getName(), true));
				if (index < priority) {
					priority = index;
					highest = item;
				}
		...
		for (Map.Entry<String, ConceptModel> item : newConcepts.entrySet()) {
			...
			result.put(uriInLog, 
					OntologyModel.getConceptPart(concept.getOntology().getName()) +
	...
	private Map<String, ConceptModel> expand(List<String> modelReferences) {
		...
		for (String uri : modelReferences) {
			...
			if (model != null) {
				...
				if (concept != null) {
					...
					if (conceptPriority.contains(new ConceptSelection(conceptUri, true))) {
						result.put(uri, concept);
					} else {
						// we have to replace the concept by its super concepts which are in the priority list
						// and optionally only keeping the highest-priority concept
						...
						while (!todo.isEmpty()) {
							...
							for (ConceptModel superConcept : model.getAllDirectSuperConcepts(c)) {
								String superConceptUri = superConcept.getName();
								
								if (conceptPriority.contains(new ConceptSelection(superConceptUri, true))) {
									result.put(model.getConceptURIInLog(superConcept), superConcept);
								} else {
		...
		if (result.size() > 1) {
			// check whether resulting concepts are super concepts of other resulting concepts
			...
			for (Map.Entry<String, ConceptModel> item : result.entrySet()) {
				OntologyModel model = getOntology(OntologyModel.getOntologyPart(item.getKey()));
				
				for (ConceptModel subconcept : model.getSubConcepts(item.getValue())) {
					if (!subconcept.getName().equals(item.getValue().getName()) && conceptUris.contains(subconcept.getName())) {
	...
	private void replace(DataSection dataAttributes) {
		...
		for (String name : dataAttributes.keySet()) {
			List<String> newRefs = getUris(replace(dataAttributes.getModelReferences(name)));
	...
	protected String getHelpForThisLogFilter() {
		return
			"Abstracts concepts in the log file. " +
	...
	protected void readSpecificXML(Node logFilterSpecifcNode) throws IOException {
		...
		for (int i = 0; i < logFilterSpecifcNode.getChildNodes().getLength(); i++) {
			...
			if (n.getNodeName().equals(USE_PRIORITY_NODE)) {
				usePriority = n.getAttributes().getNamedItem(VALUE_ATTRIBUTE).getNodeValue().equals(TRUE);
			} else if (n.getNodeName().equals(REPLACE_ELEMENT_NODE)) {
				replaceInstancesElement = n.getAttributes().getNamedItem(VALUE_ATTRIBUTE).getNodeValue().equals(TRUE);
			} else if (n.getNodeName().equals(REPLACE_TYPE_NODE)) {
				replaceInstancesType = n.getAttributes().getNamedItem(VALUE_ATTRIBUTE).getNodeValue().equals(TRUE);
			} else if (n.getNodeName().equals(REPLACE_ORIGINATOR_NODE)) {
				replaceInstancesOriginator = n.getAttributes().getNamedItem(VALUE_ATTRIBUTE).getNodeValue().equals(TRUE);
			} else if (n.getNodeName().equals(REMOVE_PI_NODE)) {
				removePI = n.getAttributes().getNamedItem(VALUE_ATTRIBUTE).getNodeValue().equals(TRUE);
			} else if (n.getNodeName().equals(REMOVE_WFME_NODE)) {
				removeWfme = n.getAttributes().getNamedItem(VALUE_ATTRIBUTE).getNodeValue().equals(TRUE);
			} else if (n.getNodeName().equals(REMOVE_TYPE_NODE)) {
				removeType = n.getAttributes().getNamedItem(VALUE_ATTRIBUTE).getNodeValue().equals(TRUE);
			} else if (n.getNodeName().equals(REMOVE_DATA_ATTR_NODE)) {
				removeDataAttr = n.getAttributes().getNamedItem(VALUE_ATTRIBUTE).getNodeValue().equals(TRUE);
			} else if (n.getNodeName().equals(REMOVE_ORIGINATOR_NODE)) {
				removeOriginator = n.getAttributes().getNamedItem(VALUE_ATTRIBUTE).getNodeValue().equals(TRUE);
				...
				for (int sel = 0; sel < n.getChildNodes().getLength(); sel++) {
					...
					if (conceptnode.getNodeName().equals(CONCEPT_NODE)) {
						boolean selected = conceptnode.getAttributes().getNamedItem(SELECTED_ATTRIBUTE).getNodeValue().equals(TRUE);
						conceptPriority.add(new ConceptSelection(conceptnode.getTextContent().trim(), selected));
					}
				}
	...
	protected void writeSpecificXML(BufferedWriter output) throws IOException {
		output.write("<" + USE_PRIORITY_NODE + " " + VALUE_ATTRIBUTE + "=\"" + (usePriority ? TRUE : FALSE) + "\" />\n");
		output.write("<" + REPLACE_ELEMENT_NODE + " " + VALUE_ATTRIBUTE + "=\"" + (replaceInstancesElement ? TRUE : FALSE) + "\" />\n");
		output.write("<" + REPLACE_TYPE_NODE + " " + VALUE_ATTRIBUTE + "=\"" + (replaceInstancesType ? TRUE : FALSE) + "\" />\n");
		output.write("<" + REPLACE_ORIGINATOR_NODE + " " + VALUE_ATTRIBUTE + "=\"" + (replaceInstancesOriginator ? TRUE : FALSE) + "\" />\n");
		output.write("<" + REMOVE_PI_NODE + " " + VALUE_ATTRIBUTE + "=\"" + (removePI ? TRUE : FALSE) + "\" />\n");
		output.write("<" + REMOVE_WFME_NODE + " " + VALUE_ATTRIBUTE + "=\"" + (removeWfme ? TRUE : FALSE) + "\" />\n");
		output.write("<" + REMOVE_TYPE_NODE + " " + VALUE_ATTRIBUTE + "=\"" + (removeType ? TRUE : FALSE) + "\" />\n");
		output.write("<" + REMOVE_DATA_ATTR_NODE + " " + VALUE_ATTRIBUTE + "=\"" + (removeDataAttr ? TRUE : FALSE) + "\" />\n");
		output.write("<" + REMOVE_ORIGINATOR_NODE + " " + VALUE_ATTRIBUTE + "=\"" + (removeOriginator ? TRUE : FALSE) + "\" />\n");
		output.write("<" + SELECTION_NODE + ">\n");
		for (List<String> path : selection) {
			output.write("  <" + PATH_NODE + ">\n");
			for (String item : path) {
				output.write("    <" + ITEM_NODE + ">" + item + "</" + ITEM_NODE + ">\n");
			}
			output.write("  </" + PATH_NODE + ">\n");
		}
		output.write("</" + SELECTION_NODE + ">\n");
		...
		for (ConceptSelection concept : conceptPriority) {
			output.write("  <" + CONCEPT_NODE + " " + SELECTED_ATTRIBUTE + "=\"" + (concept.isSelected() ? TRUE : FALSE) + "\">" + 
					concept.getConceptURI() + 
					"</" + CONCEPT_NODE + ">\n");
		}
		output.write("</" + CONCEPTS_NODE + ">\n");
	}
