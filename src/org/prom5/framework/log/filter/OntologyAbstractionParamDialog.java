...
import org.prom5.framework.models.ui.treecomponent.InitialSelectionCallback;
...
import org.prom5.framework.models.ui.treecomponent.SelectableNode;
import org.prom5.framework.models.ui.treecomponent.SelectionChangeListener;
import org.prom5.framework.models.ui.treecomponent.VertexNode;
...
public class OntologyAbstractionParamDialog extends LogFilterParameterDialog {
	...
	public OntologyAbstractionParamDialog(LogSummary summary, OntologyAbstractionFilter filter) {
	...
	private boolean getInitialReplaceInstancesElement() {
		return ((OntologyAbstractionFilter) this.filter).getReplaceInstancesElement();
	}
	
	private boolean getInitialReplaceInstancesType() {
		return ((OntologyAbstractionFilter) this.filter).getReplaceInstancesType();
	}
	
	private boolean getInitialReplaceInstancesOriginator() {
		return ((OntologyAbstractionFilter) this.filter).getReplaceInstancesOriginator();
	}
	...
	public LogFilter getNewLogFilter() {
		...
		return new OntologyAbstractionFilter(
				tree.getSelection(), 
				model.getConceptPriorityList(), 
				usePriorities.isSelected(), 
				replaceInstancesElement.isSelected(),
				replaceInstancesType.isSelected(),
				replaceInstancesOriginator.isSelected(),
				removePI.isSelected(),
				removeWfme.isSelected(),
				removeType.isSelected(),
				removeDataAttr.isSelected(),
	...
	private JPanel createRemoveInstancesPanel() {
		...
		removePI = new JCheckBox("Process instance, if it has no model references");
		removePI.setSelected(getInitialRemovePI());
		removeWfme = new JCheckBox("Audit trail entry, if its workflow model element has no model references");
		removeWfme.setSelected(getInitialRemoveWfme());
		removeType = new JCheckBox("Audit trail entry, if its event type has no model references");
		removeType.setSelected(getInitialRemoveType());
		removeDataAttr = new JCheckBox("Data attribute, if it has no model references");
		removeDataAttr.setSelected(getInitialRemoveDataAttr());
		removeOriginator = new JCheckBox("Originator, if it has no model references");
	...
	private JPanel createReplaceInstancesPanel() {
		...
		replaceInstancesElement.setSelected(getInitialReplaceInstancesElement());
		...
		replaceInstancesOriginator.setSelected(getInitialReplaceInstancesOriginator());
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				// we have to call updateConceptList from the Swing event thread to avoid multithreading issues
				updateConceptList();
			}
		});
	...
	protected void updateConceptList() {
		listPanel.setVisible(replaceInstancesElement.isSelected() || replaceInstancesType.isSelected() || replaceInstancesOriginator.isSelected());
	}

	private JPanel createConceptList(List<OntologyModel> ontologies) {
		conceptList = new JList(new ConceptListModel(ontologies, getInitialConceptPriority()));
		conceptList.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
		...
		usePriorities = new JRadioButton("Replace instances only with the highest priority concept");
		usePriorities.setSelected(getInitialUsePriority());
	    
		dontUsePriorities = new JRadioButton("Replace instances with the concatenation of all their concepts");
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				// we have to call this from the Swing event thread to avoid multithreading issues
		...
		usePrioPanel.add(new JLabel("4) When replacing instances which have more than one concept:"), BorderLayout.NORTH);
	...
	private JPanel createOntologyTree(List<OntologyModel> ontologies) {
		InitialSelectionCallback initialSelection = new InitialSelectionCallback() {
			public boolean isInitiallySelected(List<String> path) {
				List<List<String>> sel = ((OntologyAbstractionFilter) filter).getSelection();
		...
		tree = new ModelGraphTreeComponent(models, initialSelection, listener, new NameProvider() {
			public String getName(Object vertex) {
				return OntologyModel.getConceptPart(((ModelGraph) vertex).getIdentifier());
			}
		}, new NameProvider() {
			public String getName(Object vertex) {
				return OntologyModel.getConceptPart(((ModelGraphVertex) vertex).getIdentifier());
			}
		});
...
class ConceptSelection {
	...
	public boolean equals(Object other) {
		...
		return conceptUri.equals(otherSelection.conceptUri) && selected == otherSelection.selected;
	}
...
class ConceptListModel extends AbstractListModel {
	...
	public ConceptListModel(List<OntologyModel> ontologies, List<ConceptSelection> initialConceptPriority) {
		...
		if (initialConceptPriority != null) {
			...
			for (ConceptSelection selection : initialConceptPriority) {
				...
				Iterator<ConceptListNode> nodes = conceptPriorityList.iterator();
	...
	public List<ConceptSelection> getConceptPriorityList() {
		...
		for (ConceptListNode node : conceptPriorityList) {
			result.add(new ConceptSelection(node.getConcept().getName(), node.isSelected()));
		}
		return result;
	}
	...
	private void select(VertexNode node) {
		for (ConceptListNode concept : conceptPriorityList) {
			if (concept.getConcept().getName().equals(node.getVertex().getIdentifier())) {
...
class ConceptListNode extends SelectableNode {
	...
	public String getName() {
		return concept.getOntology().getShortName() + OntologyModel.ONTOLOGY_SEPARATOR + toString();
	}
