...
class ProcessInstanceNameFilterParameterDialog extends LogFilterParameterDialog {
	...
	public ProcessInstanceNameFilterParameterDialog(LogSummary summary, ProcessInstanceNameLogFilter filter) {
	...
	protected JPanel getPanel() {
		...
		JLabel label =new JLabel("Please specify the regular expression you want to use for filtering process instance names:");
		panel.add(label,
				new GridBagConstraints(0, 0,1, 1, 0.0, 0.0, GridBagConstraints.WEST,
				GridBagConstraints.NONE, new Insets(1, 1, 1, 1), 0, 0));
		regex.setPreferredSize(new Dimension((int)label.getPreferredSize().getWidth(), (int)regex.getPreferredSize().getHeight() ));
		panel.add(regex,
				new GridBagConstraints(0,1,1, 1, 0.0, 0.0, GridBagConstraints.WEST,
				GridBagConstraints.NONE, new Insets(1, 1, 1, 1), 0, 0));
		JLabel help = new JLabel("<html><br>All process instances whose name matches the given regular expression will be filtered out.<br>" +
		...
		panel.add(help,
				...
				GridBagConstraints.NONE, new Insets(1, 1, 1, 1), 50, 0));
