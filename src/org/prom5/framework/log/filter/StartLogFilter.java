 ...
 *
 * During construction, the modelElementName and EventType of the initial event should
 * be given, together with another filter. Then, ProcessInstances are ignored if
 * their first event does not match the given event.
 *
 * <p>Title: </p>
 *
 * <p>Description: </p>
...
public class StartLogFilter extends LogFilter {
	...
	protected boolean doFiltering(ProcessInstance instance) {
		String firstElementInstance = instance.getAuditTrailEntries().first().getElement();
		String firstElementTypeInstance = instance.getAuditTrailEntries().first().getType();
		...
		return new LogFilterParameterDialog(summary, StartLogFilter.this) {
			...
			protected JPanel getPanel() {
				...
				for (int i = 0; i < size; i++) {
					checks[i] = new LogEventCheckBox(summary.getLogEvents().getEvent(i));
				...
				for (int i = 0; i < checks.length; i++) {
					p.add(checks[i]);
					if ((startEvents != null) && (startEvents.contains(checks[i].getLogEvent()))) {
