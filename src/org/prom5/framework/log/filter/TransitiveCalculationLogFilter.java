...
public abstract class TransitiveCalculationLogFilter extends LogFilter {
	...
	protected boolean doFiltering(ProcessInstance pi) {
		...
		try {
			...
			for (int i = 0; i < ates.size(); i++) {
				...
				mapping.put((String) ate.getAttributes().get(ProcessInstance.ATT_ATE_ID),
						new Integer(i));
			}

			int[][] adj = new int[mapping.keySet().size()][mapping.keySet().size()];
			int N = 2 * adj.length;
			...
			while (it.hasNext()) {
				...
				int ateID = mapping.get(ate.getAttributes().get(ProcessInstance.ATT_ATE_ID)).
							intValue();
				adj[ateID][ateID] = 0;
				...
				/*
				 if (ate.getAttributes().get(ProcessInstance.ATT_ATE_PRE).equals("")) {
				  extendPO(ate, mapping, pi.getAuditTrailEntryList(), new HashSet());
				 }
				 */
			}
			doCalculation(adj, N);
			...
			for (int i = 0; i < adj.length; i++) {
				...
				for (int j = 0; j < adj.length; j++) {
					if (adj[i][j] > 0 && adj[i][j] < N) {
						postset.add(ates.get(j).getAttributes().get(ProcessInstance.ATT_ATE_ID));
					}
					if (adj[j][i] > 0 && adj[j][i] < N) {
						preset.add(ates.get(j).getAttributes().get(ProcessInstance.ATT_ATE_ID));
					}
				}
	...
	private void dealWithError(Exception e) {
		Message.add("Problem while transitively closing pi: " +
				e.getMessage(), Message.ERROR);
	}

	private HashSet<String> getPreSet(AuditTrailEntry ate) {
		String pre = (String) ate.getAttributes().get(ProcessInstance.ATT_ATE_PRE);
	...
	private HashSet<String> getPostSet(AuditTrailEntry ate) {
		String post = (String) ate.getAttributes().get(ProcessInstance.ATT_ATE_POST);
	...
	public LogFilterParameterDialog getParameterDialog(LogSummary summary) {
		return new LogFilterParameterDialog(summary, TransitiveCalculationLogFilter.this) {
			...
			public LogFilter getNewLogFilter() {
				try {
					return TransitiveCalculationLogFilter.this.getClass().newInstance();
				} catch (IllegalAccessException ex) {
					Message.add("ERROR in instantiating new TransitiveCalculationFilter",
							Message.ERROR);
					return null;
	...
	protected void writeSpecificXML(BufferedWriter bufferedWriter) throws IOException {
	}
