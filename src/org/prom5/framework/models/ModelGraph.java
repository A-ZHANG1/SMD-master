...
package org.prom5.framework.models;
...
import org.prom5.framework.util.Dot;
...
/**
 * Encapsulates a general graph, and it is used as a base class for all types of graphs
 * in the processmining project.
 * <p>
 * Since it extends <code>Graph</code>, all functionality
 * of the GRAPPA graph library is available.
 * <p>
 ...
 * <code>writeToDot</code> method is layed out with the dot.exe tool.
 * The output of dot.exe is then visualized using the Grappa package.
 *
 * Note that the visualization depends on the <code>writeToDot</code>
 * method. Subclasses that want to customize the visualization should
 * override the <code>writeToDot</code> method and customize the DOT
 * representation, instead of overriding the <code>getGrappaVisualization</code> method.
 */
 ...
 * 20071122, hverbeek: Red (working on writeToDot)
 */
public class ModelGraph extends Graph implements DotFileWriter, DotFileAttributeWriter {
    ...
    /**@todo anne: this should be reset to private access as soon as the
     * cloning problem is resolved for PetriNet (see, e.g., PetriNet.addAndLinkTransition) */
    protected ArrayList vertices;
    private ArrayList edges;

    /*
     * September 27, 2007, Eric Verbeek
     * Added dummy vertices and dummy edges. These vertices and edges are only used when writing to Dot.
     */
    ...
    /*
     * September 27, 2007, Eric Verbeek
     * Added attributes used when writing to Dot.
     */
    ...
    /*
     * September 5, 2005, Eric Verbeek
     * Added weightedArcs attribute. If set, multiple arcs between two nodes should
     * be allowed. If not set, multiple arcs should not be allowed. By default, it
     * is not set. Any PetriNet will set it on construction.
     */
    ...
    /**
     * September 5, 2005, Eric Verbeek
     * Sets whether multiple arcs should be allowed or not.
     * @param w specifies whether multiple arcs should be allowed.
     */
    ...
    /**
     * September 5, 2005, Eric Verbeek
     * Gets whether multiple arcs should be allowed or not.
    ...
    /**
     * Added by Mariska Netjes
     * Returns the first source found in the nodes of this model graph
     *
    ...
    /**
     * Added by Mariska Netjes
     * Returns the first sink found in the nodes of this model graph
     *
    ...
    /*
     public boolean isIsomorphic(ModelGraph G, ArrayList mapping, ArrayList exclude) {
      // mapping is assumed to be empty and if the result is true,
      // it is filled with Pairs of Vertices, giving an isomorphic mapping, which
      // is not allready contained in exclude.

      return false;
     }

     public boolean isIsomorphic(ModelGraph G, ArrayList mapping) {
      return isIsomorphic(G, mapping, new ArrayList());
     }
     */

    /**
     * September 5, 2005, Eric Verbeek
     * OK for multiple arcs.
     *
    ...
    /**
     * September 5, 2005, Eric Verbeek
     * Renamed from hasEdge to getFirstEdge
     *
     ...
     */
    public ModelGraphEdge getFirstEdge(ModelGraphVertex source, ModelGraphVertex destination) {
     ...
     * If such an edge already exists, the ModelGraph's weightedArc attribute specifies
     * whether a duplicate edge is added (true) or not (false).
     * @param source the first node
     * @param destination the second node
     * @return the created edge (could be null if weightedArc is not set!)
     */
    public ModelGraphEdge addEdge(ModelGraphVertex source, ModelGraphVertex destination) {
        /* September 5, 2005, Eric Verbeek
         * Check the weightedArc attribute. If set, add the edge even if sich an
         * edge already exists.
         */
        if (!weightedArcs) {
            ModelGraphEdge e = getFirstEdge(source, destination);
    ...
    public void writeToDot(Writer bw) throws IOException {
        ...
        for (ModelGraphEdge edge : dummyEdges) {
            bw.write("\tnode" + edge.getTail().getIdKey() + "-> node" + edge.getHead().getIdKey());
            bw.write(" [");
    ...
    public void writePreambleAttributes(Writer bw) throws IOException {
        for (String attribute : graphAttributes.keySet()) {
            bw.write("\t" + attribute + "=\"" + graphAttributes.get(attribute).replaceAll("\"", "\\\\\"") + "\";\n");
        }
        String prefix = "\tedge [";
        for (String attribute : edgeAttributes.keySet()) {
            bw.write(prefix + attribute + "=\"" + edgeAttributes.get(attribute).replaceAll("\"", "\\\\\"") + "\"");
            prefix = ",";
        }
        if (prefix == ",") {
            bw.write("];\n");
        ...
        for (String attribute : nodeAttributes.keySet()) {
            bw.write(prefix + attribute + "=\"" + nodeAttributes.get(attribute).replaceAll("\"", "\\\\\"") + "\"");
            prefix = ",";
        }
        if (prefix == ",") {
            bw.write("];\n");
     ...
     * @param bw Writer
     * @param vertex ModelGraphVertex
     * @throws IOException
     *
     * Code rating: Red
     * Review rating: Red
     */
    public void writeVertexAttributes(Writer bw, ModelGraphVertex vertex) throws IOException {
     ...
     * @param bw Writer
     * @param edge ModelGraphEdge
     * @throws IOException
     *
     * Code rating: Red
     * Review rating: Red
     */
    public void writeEdgeAttributes(Writer bw, ModelGraphEdge edge) throws IOException {
     ...
     * @param bw Writer
     * @param cluster ModelGraphCluster
     * @throws IOException
     *
     * Code rating: Red
     * Review rating: Red
     */
    public void writeClusterAttributes(Writer bw, ModelGraphCluster cluster) throws IOException {
        String prefix = "\t\t";
        for (String attribute : cluster.getAttributes().keySet()) {
            bw.write(prefix + attribute + "=\"" + cluster.getAttributes().get(attribute).replaceAll("\"", "\\\\\"") + "\";\n");
            prefix = "\t\t,";
        }
    }
     ...
     * Note that the visualization is created from the DOT representation that is
     * created with the <code>writeToDot</code> method.
     * Subclasses that want to customize the visualization should override the
     * <code>writeToDot</code> method instead of this one.
     *
    ...
    public final ModelGraphPanel getGrappaVisualization() {
        ...
        try {
            ...
            Message.add("Parsing DOT-file: " + dotFile.getAbsolutePath(), Message.DEBUG);
    ...
    /**
     * Returns a mapping of all nodes and edges of the graph
     * given in subgraph onto the original nodes and edges in the ModelGraph.
     * The mapping is stored in the Object field.
     */
     ...
     * Note that cloning on this level is not supported but that the lists of
     * vertices and edges are reset to the empty list, respectively, in order
     * to properly support cloning for deriving subclasses.
    ...
    protected Object clone() {
        ...
        // reset referenced lists to support reconstruction for deriving subclasses
     ...
     * set of nodes, such that for each objects in the given list of
     * objects holds that:
     * 1) All edges corresponding to that object have a source in the region
     * and a destination outside of the region
     * 2) All edges corresponding to that object have a source outside of the
     * region and a destination inside the region
     * 3) All edges corresponding to that object have a source in the region
     * and a destination inside the region
     * 4) All edges corresponding to that object have a source outside of the
     * region and a destination outside of the region
     *
     * @param objects The list of objects to which edges can refer
     * @return An ArrayList<HashSet>, containing HashSets of states in the statespace.
     */
    public RegionList calculateMinimalRegions(Collection objects) {
        synchronized (vertices) {
            RegionList result = new RegionList();
            calculateRegions(result, new HashSet(0), vertices.size() - 1, vertices, true,
                             vertices.size(), objects);
            return result;
        }
    }

    /**
     * This method calculates all regions of this graph. A region is a
     * set of nodes, such that for each objects in the given list of
     * objects holds that:
     * 1) All edges corresponding to that object have a source in the region
     * and a destination outside of the region
    ...
    /**
     * This method calculates all non-complementary regions of this graph. A region is a
     * set of nodes, such that for each objects in the given list of
     * objects holds that:
     * 1) All edges corresponding to that object have a source in the region
     * and a destination outside of the region
    ...
    /**
     * This method calculates all regions of this graph that contain at most <code>size</code> states. A region is a
     * set of nodes, such that for each objects in the given list of
     * objects holds that:
     * 1) All edges corresponding to that object have a source in the region
     * and a destination outside of the region
     ...
     *
     * @param objects The list of objects to which edges can refer
     * @param size the maximum size of the region
     * @return An ArrayList<HashSet>, containing HashSets of states in the statespace.
     */
    public RegionList calculateRegionsMaxSize(Collection objects, int size) {
        synchronized (vertices) {
            RegionList result = new RegionList();
            result.add(new Region(new HashSet(), new HashSet()));
            calculateRegions(result, new HashSet(0), vertices.size() - 1, vertices, false, size,
                             objects);
            return result;
        }
    }

    private void calculateRegions(RegionList result, HashSet<ModelGraphVertex> currentSet,
            int currentIndex, final ArrayList states, boolean minimalOnly, int comp,
        ...
        for (int i = currentIndex; i >= 0; i--) {
            ...
            if (r != null) {
                if (!minimalOnly) {
                    calculateRegions(result, newSet, i - 1, states, minimalOnly, comp, objects);
                    result.add(r);
                } else {
     ...
     * for each object in the list objects:
     * 1) All edges corresponding to that object have a source in the region
     * and a destination outside of the region
     * 2) All edges corresponding to that object have a source outside of the
     * region and a destination inside the region
     ...
     * region and a destination outside of the region
     *
     * @param possibleRegion HashSet
     * @param objects the arraylist containing the objects to which edges can refer
     * @return Region, null if this set is not a region, an object otherwise
     */
    private Region isRegion(HashSet<ModelGraphVertex> possibleRegion, Collection objects) {
    ...
    /**
     * Returns a HashSet containing all objects that appear on edges, i.e. through
     * the edge.object method.
     *
     * @return HashSet
     */
    ...
    /**
     * This method returns a subset of the given set of regions, which represents
     * the PreRegions of the given object.
     *
     * @param regions ArrayList
     * @param t Object
     * @return ArrayList
     */
    ...
    private void removeDirectSuc(ModelGraphVertex v, ModelGraphVertex w, HashSet<Long> done, HashSet<Long> removed) {
     ...
     * @param tag String The tag to use for the indicators.
     */
