package org.prom5.framework.models.activitygraph;
...
/**
 * An activity graph is a graph with only activities in it. So, there doesn't exists any
...
//	/** the mapping for an object to its corresponding boxID and which is needed for DOT. */
...
//	private HLActivitySet hlActivitySet = null;
	 ...
	 * @param av ActivityVertex the activity vertex to be added to the graph
	 ...
	 * @param av ActivityVertex the activity vertex to be removed from the activity graph
	 */
	public void delActivityVertex(ModelGraphVertex av) {
	 ...
	 /**
	  * When a activity graph is asked for its visualization, a temporary dot file is
	  * written and afterwards read by grappa to convert it into a java frame.
	  * For the activity graph, only the activities will be displayed in the picture.
	  * @param bw Writer
	  * @throws IOException
	  */
	 ...
	 * Note that the activity vertex is expected not to be associated to this activity graph
	 * already and this link will be established. If that link already has been established use
	 * {@link addActivityVertex(ActivityVertex av) addActivityVertex(ActivityVertex av))} instead.
	 * @param av ActivityVertex the activity vertex to be added
...
//
//	/**
//	 * Makes a deep copy of the object, i.e., reconstructs the activity graph with
//	 * cloned activity vertices. Note that this method needs to be extended as soon as
//	 * there are attributes added to the class which are not primitive or immutable.
...
//	 * @param perspectivesToShow Set the perspectives that need to be shown in the activity graph
//	 * @return ModelGraphPanel the visualization of the activity graph
//	 */
//	public ModelGraphPanel getPanel(Set<HighLevelTypes.Perspective> perspectivesToShow) {
