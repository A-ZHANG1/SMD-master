 ...
 * <p>Title: BPEL</p>
 *
 * <p>Description: CLass for BPEL models</p>
 *
...
public class BPEL extends ModelGraph {
	...
	/**
	 * Needed to be able to create elements. (To create an element, we need a document.)
	 */
	...
	/**
	 * Whether the BPEL model was constructed from a document. Otherwise, it is constructed from scratch.
	 */
	...
	public BPEL(String name) {
		...
		try {
			...
			// FindBugs sais: Write to static field from instance method
			staticDocument = document = staticImplementation.createDocument("http://schemas.xmlsoap.org/ws/2003/03/business-process/", "process", null);
			// Set the name attribute of the root element.
			document.getDocumentElement().setAttribute(BPELConstants.stringName, name);
		} catch (Exception ex) {
			Message.add("Error while constructing document: " + ex.getMessage(), Message.ERROR);
		}
	 ...
	 * @param process BPELProcess The process.
	 *
	...
	public void setProcess(BPELProcess process) {
		...
		if (!fromDocument && process.getActivity() != null) {
			document.getDocumentElement().appendChild(process.getActivity().getElement());
		}
	}
	...
	public String getName() {
		return document.getDocumentElement().getAttribute(BPELConstants.stringName);
	}
