...
package org.prom5.framework.models.bpel;
...
import org.prom5.framework.models.bpel.visit.BPELVisitable;
...
/**
 * <p>Title: BPEL activity</p>
 *
 * <p>Description: Superclass for any BPEL activity</p>
 *
...
public abstract class BPELActivity implements BPELVisitable {
	 ...
	 * @param tagName String The given tag name.
	 ...
	 * @param isUnique boolean Whether the name should be unique w.r.t. its sibling activities.
	...
	public String getName(boolean isUnique) {
		...
		if (isUnique) {
			...
			name += "[" + i;
			// In case of a switch or pick, the sibling activities can also be nephews, in which case the 'brother' number might not be unique.
	 ...
	 * @param name String The given tag name.
	 * @return int 1 if the underlying element has the given tag name, otherwise 0.
	 */
	...
	public void appendSource(String name) {
		sourcesOK = false;
		Element source = element.getOwnerDocument().createElement(BPELConstants.stringSource);
	...
	public void appendTarget(String name) {
		targetsOK = false;
		Element target = element.getOwnerDocument().createElement(BPELConstants.stringTarget);
			...
			for (int i = 0; i < n; i++) {
				...
				if (node instanceof Element) {
					...
					if (BPELConstants.isSource(element.getTagName())) {
						sources.add(element.getAttribute(BPELConstants.stringLinkName));
					}
				}
			}
		}
	 ...
	 * If some link is both an incoming and an outgoing link of the activity and all its descendants, then it will not be part of the list.
	 * @return ArrayList A list of all outgoing links.
	 */
			...
			for (int i = 0; i < n; i++) {
				...
				if (node instanceof Element) {
					...
					if (BPELConstants.isTarget(element.getTagName())) {
						targets.add(element.getAttribute(BPELConstants.stringLinkName));
					}
				}
			}
		}
	...
	public void addLinksToModelGraph(BPEL model, HashSet<BPELActivity> activities) {
		...
		for (BPELActivity sourceActivity : activities) {
			HashSet<String> set = new HashSet<String>(sourceActivity.getFamilySources());
			sourceMap.put(sourceActivity, set);
			allSources.addAll(set);
		}
		// Get the incoming links
		for (BPELActivity targetActivity : activities) {
			HashSet<String> set = new HashSet<String>(targetActivity.getFamilyTargets());
		...
		for (BPELActivity activity : activities) {
			for (String link : sourceMap.get(activity)) {
				ModelGraphEdge edge = new ModelGraphEdge(activity.vertex, allLinkVertices.get(link));
				model.addDummy(edge);
		...
		for (BPELActivity activity : activities) {
			for (String link : targetMap.get(activity)) {
				ModelGraphEdge edge = new ModelGraphEdge(allLinkVertices.get(link), activity.vertex);
				model.addDummy(edge);
				edge.setDotAttribute("color", "blue");
			}
		}
	...
	/**
	 * Clone links.
	 * @param activity BPELActivity The activity to clone the links from.
	 */
