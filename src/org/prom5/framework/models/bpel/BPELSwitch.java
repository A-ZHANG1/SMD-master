...
/**
 * <p>Title: BPELSwitch</p>
 *
 * <p>Description: Class for a BPEL switch activity</p>
 *
...
public class BPELSwitch extends BPELStructured {
	...
	public BPELSwitch cloneActivity() {
		BPELSwitch clone = new BPELSwitch(element.getAttribute(BPELConstants.stringName));
		clone.cloneLinks(this);
		for (BPELActivity activity: getActivities()) {
			clone.appendChildActivity(cases.get(activity), activity.cloneActivity());
		}
		return clone;
	}
	 ...
	 * @param condition String The condition. If null, then otherwise, else case.
	 * @param activity BPELActivity The child activity.
	 */
	public void appendChildActivity(String condition, BPELActivity activity) {
		activitiesOK = false;
		boolean isOtherwise = condition == null || condition.compareToIgnoreCase("true") == 0;
		// Create a case element, unless the condition is null, in which case we create an otherwise element.
		Element caseElement = BPEL.staticDocument.createElement(isOtherwise ? BPELConstants.stringOtherwise : BPELConstants.stringCase);
		...
		// The element of the given activity will be a child of the case/otherwise element.
		caseElement.appendChild(activity.getElement());
		// Map the element onto the activity.
		map.put(activity.getElement(),activity);
		// Map the activity onto its condition. Use "true" in case of otherwise element.
		if (!isOtherwise) {
