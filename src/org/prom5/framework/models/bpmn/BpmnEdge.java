 ...
 * connection between flow objects, including sequence flow and message flow
 */
public class BpmnEdge extends BpmnElement
{
  ...
  /**
   * @param fromGatewayPosition the fromGatewayPosition to set
   */
  ...
  if(!isUseful)
  {
    strBuf.append("\t" + srcNodeId + " -> " + dstNodeId + "[arrowtail=\"none\",arrowhead=\"none\",color=lightgrey];" + "\n");
    strBuf.append("\n");
    ...
    else if (this.getType() == BpmnEdgeType.Flow)
    {
      ...
      {
        label = getMessage();
    strBuf.append("\t" + srcNodeId + " -> " + dstNodeId + "[label=\"" + label + "\"];\n");
      }
      else if (getCondition() != null)
      {
        ...
        {
          // condition flow, the state "getcondition()is empty" is not possible happen
          strBuf.append("\t" + srcNodeId + " -> " + dstNodeId +
      ...
      {
        strBuf.append("\t" + srcNodeId + " -> " + dstNodeId + "[arrowtail=\"tee\"];\n");
      }
      // flow without message and condition
      else
      {
        strBuf.append("\t" + srcNodeId + " -> " + dstNodeId + "[label=\"\"];" + "\n");
      }
    }
    strBuf.append("\n");
    return strBuf.toString();
  ...
  protected void outputProperties(StringBuffer sb)
  {
    ...
    outputProperty(sb, BpmnXmlTags.BPMN_PROP_GATEWAYPOSITION, this.fromGatewayPosition);
