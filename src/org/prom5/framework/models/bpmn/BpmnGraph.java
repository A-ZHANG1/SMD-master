...
public class BpmnGraph extends ModelGraph
{
  ...
  public String toDotString(boolean isSubProcess)
  {
    ...
    else
    {
      ...
      strBuf.append("edge [arrowsize=\"0.7\",fontname=\"Arial\",fontsize=\"8\"];\n");
      strBuf.append("node [height=\".2\",width=\".2\",fontname=\"Arial\",fontsize=\"8\"];\n");
    }
    nodeMapping.clear();
    ...
    while (it.hasNext())
    {
     ...
     String parentid = v.getBpmnObject().getpid();
      if (!isSubProcess && parentid == null  || isSubProcess && parentid == this.parentid)
  ...
  public boolean getPreds(ArrayList<String> alPred, String nodeId)
  {
    ...
    if(bo == null)
    {
      ...
      for (BpmnSubProcess subProc : subProcesses)
      {
        boolean isFound = subProc.getSubGraph().getPreds(alPred, nodeId);
        // if found, got it
        if (isFound)
        {
          return true;
  ...
  {
    //enumerate all vertexs and edges in mainProcess and add them to this model
    ...
    {
      BpmnGraphVertex v = (BpmnGraphVertex) it.next();
      if (v.bpmnObject instanceof BpmnSwimPool || v.bpmnObject instanceof BpmnSwimLane)
      {
...
// For BPMN to YAWL,find the from and to node of a edge, because in bpmn, we allow that
// an edge can point into an element of subprocess, but this attribute is not allowed in yawl
// so, we need to choice the common predecessor of the from and edge node of the edge.
