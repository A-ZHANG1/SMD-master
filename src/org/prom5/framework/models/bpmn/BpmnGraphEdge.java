...
public class BpmnGraphEdge extends ModelGraphEdge implements BpmnDotOutput {
  ...
  public BpmnGraphEdge(BpmnGraphVertex source, BpmnGraphVertex destination, BpmnEdge edge) {
  ...
  public String toDotString() {
    ...
    if (BpmnUtils.isExpandedSubGraph(srcObj)) {
      ...
      String clusterId = ((BpmnSubProcess) srcObj).getSubGraph().getClusterId();
      String oldId = srcObj.getId();
      int startIdx = edgeStrBuf.indexOf(oldId);
      if (startIdx >= 0) {
        int endIdx = startIdx + oldId.length();
        String newId = ((BpmnSubProcess) srcObj).getProcessModel().getOutEdgeTailId();
    ...
    if (BpmnUtils.isExpandedSubGraph(dstObj)) {
      ...
      String clusterId = ((BpmnSubProcess) dstObj).getSubGraph().getClusterId();
      String oldId = dstObj.getId();
      int startIdx = edgeStrBuf.lastIndexOf(oldId);
      if (startIdx >= 0) {
        int endIdx = startIdx + oldId.length();
        String newId = ((BpmnSubProcess) dstObj).getProcessModel().getInEdgeHeadId();
        if (newId != null && !newId.equals("")) {
          edgeStrBuf.replace(startIdx, endIdx, newId);
        }
      }
