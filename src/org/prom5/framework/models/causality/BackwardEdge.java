...
public class BackwardEdge  {
	...
	public BackwardEdge(HashSet<ModelGraphVertex> sources, ModelGraphVertex destination, CausalFootprint footprint) {
		this.sources = sources;
		this.destination = destination;
		this.footprint=footprint;
	}

	public boolean equals(HashSet<ModelGraphVertex> sources, ModelGraphVertex destination) {
		return this.sources.equals(sources) && this.destination.equals(destination);
	}
	...
	public static BackwardEdge readXML(Node edgeNode,
			HashMap<Long,ModelGraphVertex> stored2retrieved, CausalFootprint footprint) throws IOException, ClassNotFoundException,
		...
		for (int i = 0; i < edgeNode.getChildNodes().getLength(); i++) {
			...
			if (node.getNodeName().equals("destination")) {
				...
				for (int j = 0; j < node.getChildNodes().getLength(); j++) {
					if (node.getChildNodes().item(j).getNodeName().equals("vertex")) {
						edge.destination = footprint.readVertexXML(node.getChildNodes().item(j),
										   footprint, stored2retrieved);
					}
				}
			} else if (node.getNodeName().equals("sources")) {
				edge.sources = new HashSet<ModelGraphVertex>(node.getChildNodes().getLength());
				for (int j = 0; j < node.getChildNodes().getLength(); j++) {
					if (node.getChildNodes().item(j).getNodeName().equals("vertex")) {
						edge.sources.add(footprint.readVertexXML(node.getChildNodes().item(j),
								footprint, stored2retrieved));
					}
				}
			}
