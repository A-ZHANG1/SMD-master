package org.prom5.framework.models.causality;
...
public class CausalFootprint extends ModelGraph implements Serializable {
    ...
    private ArrayList<ForwardEdge> forwardEdges = new ArrayList();
    private ArrayList<BackwardEdge> backwardEdges = new ArrayList();
    ...
    void writeVertexXML(ModelGraphVertex v, BufferedWriter output) throws
            IOException {
        ...
        if (v != source && v != target) {
            ...
            output.write("objectclass=\"" +
                         StringNormalizer.
                         escapeXMLCharacters(base.getClass().getName()) + "\" ");
    ...
    /**
     * Read the inside of the <CausalFootprint> tag in the XML export file from the
     * InputStream input.
     *
     * @param input InputStream
     */
    public static CausalFootprint readXML(Node causalFootprintNode,
                                          ModelGraph baseModel) throws
        ...
        for (int i = 0; i < causalFootprintNode.getChildNodes().getLength(); i++) {
            if (causalFootprintNode.getChildNodes().item(i).getNodeName().
                ...
                result.readForwardEdgesXML(causalFootprintNode.getChildNodes().
                                           item(i),
                                           stored2retieved);
            } else if (causalFootprintNode.getChildNodes().item(i).getNodeName().
                       equals(
                               "backwardedges")) {
                // Read the nodeSpecific part of this LogFilter
     ...
     * @param input InputStream
     */
    void readForwardEdgesXML(Node forwardEdgeNode, HashMap<Long,
                             ModelGraphVertex> stored2retrieved) throws
            IOException,
     ...
     * @param input InputStream
     */
    void readBackwardEdgesXML(Node backwardEdgeNode, HashMap<Long,
                              ModelGraphVertex> stored2retrieved) throws
            IOException,
            ClassNotFoundException,
            IllegalAccessException, InstantiationException {
    ...
    HashMap<Long, ModelGraphVertex> readVerticesXML(Node verticesNode,
            ModelGraph baseModel) throws IOException,
            ClassNotFoundException,
            IllegalAccessException, InstantiationException {
        HashMap<Long, ModelGraphVertex>
                result = new HashMap<Long, ModelGraphVertex>();
     ...
     * @param destinations the set of destination nodes
    ...
    /**
     * adds a node in the graph with a reference to the original object in the base
     * graph
     *
     * @param v ModelGraphVertex
    ...
    /**
     * Returns the vertex in the basegraph that is referenced by this vertex in the
     * causal structure
     *
     * @param vertex ModelGraphVertex referring to the object in this graph
    ...
    /**
     * Returns the vertex in the causalStructure that is referenced by this vertex in the
     * basegraph
     *
     * @param vertex ModelGraphVertex referring to the object in the base graph
    ...
    /**
     * Tells whether or not this graph holds a reference to a given
     * vertex in the base graph.
     *
     * @param baseVertex ModelGraphVertex referring to the vertex in the base graph
     * @return true if this graph has a reference to the baseVertex, false otherwise.
     */
    ...
    public void removeObsoleteEdges(CancelationComponent cancel) {
        ...
        while (it.hasNext() && !cancel.isCanceled()) {
            ...
            if (getStrictSmallerEdge(e.sources, e.destination)!=null) {
                ((HashSet) node2backward.get(e.destination.getIdKey())).remove(
                        e);
                it.remove();
            }
        }
    ...
    /**
     * This method tells the causal footprint whether the dot visualization should
     * only write the output for the selected node.
     *
     * @param selection ModelGraphVertex This vertex will be the center of the
     * written output. If the whole graph needs to be shown, set this to null
     */
...
//		HashMap nodeMapping = new HashMap();
    ...
    private int writeDotEdgesForVertex(Writer bw, ModelGraphVertex v, int num) throws
            IOException {
    ...
    private int writeBackwardEdge(Writer bw, BackwardEdge edge,
        ...
        while (it3.hasNext()) {
            ...
            bw.write("node" + source.getId() + " -> rel" + num +
