...
public class ForwardEdge {
	...
	public ForwardEdge(ModelGraphVertex source, HashSet<ModelGraphVertex> destinations,
	...
	public boolean equals(ModelGraphVertex source, HashSet<ModelGraphVertex> destinations) {
		return this.source.equals(source) && this.destinations.equals(destinations);
	}
	...
	public static ForwardEdge readXML(Node edgeNode,
			...
			CausalFootprint footprint) throws IOException, ClassNotFoundException,
				...
				for (int j = 0; j < node.getChildNodes().getLength(); j++) {
					if (node.getChildNodes().item(j).getNodeName().equals("vertex")) {
						edge.source = footprint.readVertexXML(node.
									  getChildNodes().item(j),footprint, stored2retrieved);
					}
				}
			} else if (node.getNodeName().equals("destinations")) {
				edge.destinations = new HashSet<ModelGraphVertex>(node.getChildNodes().getLength());
				for (int j = 0; j < node.getChildNodes().getLength(); j++) {
					if (node.getChildNodes().item(j).getNodeName().equals("vertex")) {
