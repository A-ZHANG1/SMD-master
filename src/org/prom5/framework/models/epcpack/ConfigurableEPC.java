...
public class ConfigurableEPC extends ModelGraph {
    ...
    public String isValidEPC() {
        ...
        if (message.length() > 0) {
            message = "<html>Problems found in EPC: " + getIdentifier() +
                      ": <br>" + message +
    ...
    public EPCEdge addEdge(EPCObject source, EPCObject destination) {
        if (enforceValidity) {
            ...
            /*			if ((source instanceof EPCFunction) && ((destination instanceof EPCEvent) ||(destination instanceof EPCConnector)) && (source.outDegree() != 0)) {
             return null;
               }
             */
            ...
            if ((destination instanceof EPCFunction) &&
                ((source instanceof EPCEvent) ||
                 (source instanceof EPCConnector)) &&
                (destination.inDegree() != 0)) {
                return null;
            }
            // The number of incoming or outgoing arcs at each connector should be at most 1
    ...
    public void writeToDot(Writer bw) throws IOException {
        ...
        bw.write(
                "node [height=\".2\",width=\"1\",fontname=\"Arial\",fontsize=\"8\"];\n");
        ...
        for (int i = 0; i < connectors.size(); i++) {
            ...
            bw.write("node" + connector.getId() +
                     " [shape =\"circle\",fixedsize=\"true\",height=\"0.25\",width=\"0.25\"," +
                     "style=\"filled" + (connector.isConfigurable() ? ",bold" :
                                         "") +
...
//					(destination instanceof EPCEvent)) {
            ...
            bw.write("node" + source.getId() + " -> node" + destination.getId() +
                     " [label=\"" +
...
//			}
    ...
    /**
     * applyConfiguration applies the configuration of this configurable epc, thus
     * yielding a new EPC, note that config.isComplete() should be true. If not,
     * null is returned.
     *
     * @param config ConfigurableEPCConfiguration
    ...
    /**
     * applyConfiguration applies the current configuration of this configurable epc,
     * thus yielding a new EPC. The current configuration is the one last added by the
     * addConfiguration method. If there is no such configuration, null is returned.
     * Note that the configuration should be complete. If not,
     * null is returned.
     *
     * @param config ConfigurableEPCConfiguration
     * @return EPC the newly configured EPC.
    ...
    public EPC applyCurrentConfiguration() {
        if (configurations.size() > 0) {
            return applyConfiguration((ConfigurableEPCConfiguration)
                                      configurations.get(
    ...
    /**
     * Get a list of events preceding a function of an EPC, either directly or via connectors.
     *
     * @param EPCFunction A function of the EPC
     * @return ArrayList<EPCEvent> A list of all EPC Events that preceed the function either directly or via connectors.
     */
    public ArrayList<EPCEvent> getPreceedingEvents(EPCFunction f) {
            ArrayList list = new ArrayList();
            addSurroundingObjects(f, list, EPCEvent.class, EPCFunction.class, true, false);
            return list;
    }

    /**
     * Get a list of events preceding a connector of an EPC, either directly or via other connectors and functions.
     *
     * @param EPCConnector A connector of the EPC
     * @return ArrayList<EPCEvent> A list of all EPC Events that preceed the connector either directly or via other connectors and functions.
     */
    public ArrayList<EPCEvent> getPreceedingEvents(EPCConnector c) {
            ArrayList list = new ArrayList();
            addSurroundingObjects(c, list, EPCEvent.class, EPCEvent.class, true, false);
            return list;
    }

    /**
     * Get a list of events preceding an event of an EPC via connectors and functions.
     *
     * @param EPCEvent An event of the EPC
     * @return ArrayList<EPCEvent> A list of all EPC Events that preceed the event via connectors and functions.
     */
    public ArrayList<EPCEvent> getPreceedingEvents(EPCEvent e) {
            ArrayList list = new ArrayList();
            addSurroundingObjects(e, list, EPCEvent.class, EPCEvent.class, true, false);
            return list;
    }

    /**
     * Get a list of events succeeding a function of an EPC, either directly or via connectors.
     *
     * @param EPCFunction A function of the EPC
     * @return ArrayList<EPCEvent> A list of all EPC Events that succeed the function either directly or via connectors.
     */
    public ArrayList<EPCEvent> getSucceedingEvents(EPCFunction f) {
            ArrayList list = new ArrayList();
            addSurroundingObjects(f, list, EPCEvent.class, EPCFunction.class, true, true);
            return list;
    }

    /**
     * Get a list of events succeeding a connector of an EPC, either directly or via other connectors and functions.
     *
     * @param EPCConnector A connector of the EPC
     * @return ArrayList<EPCEvent> A list of all EPC Events that succeed the connector either directly or via other connectors and functions.
     */
    public ArrayList<EPCEvent> getSucceedingEvents(EPCConnector c) {
            ArrayList list = new ArrayList();
            addSurroundingObjects(c, list, EPCEvent.class, EPCEvent.class, true, true);
            return list;
    }

    /**
     * Get a list of events succeeding an event of an EPC via connectors and functions.
     *
     * @param EPCEvent An event of the EPC
     * @return ArrayList<EPCEvent> A list of all EPC Events that succeed the event via connectors and functions.
     */
    public ArrayList<EPCEvent> getSucceedingEvents(EPCEvent e) {
            ArrayList list = new ArrayList();
            addSurroundingObjects(e, list, EPCEvent.class, EPCEvent.class, true, true);
            return list;
    }


    /**
     * Provides a list of objects that directly succeed the given object of the EPC in the EPC.
     *
     * @param EPCObject An object of the EPC
     * @return ArrayList<EPCFunction> A list of all EPC objects succeeding the given EPC object.
     */
    ...
    /**
     * Provides a list of objects that directly preceed the given object of the EPC in the EPC.
     *
     * @param EPCObject An object of the EPC
     * @return ArrayList<EPCFunction> A list of all EPC objects preceeding the given EPC object.
     */
    ...
    /**
     * Get a list of functions preceeding an event of an EPC either directly or via connectors.
     *
     * @param EPCEvent An event of the EPC
     * @return ArrayList<EPCFunction> A list of all EPC functions that preceed the function either directly or via connectors.
     */
    public ArrayList<EPCFunction> getPreceedingFunctions(EPCEvent e) {
            ArrayList<EPCFunction> list = new ArrayList<EPCFunction>();
            addSurroundingObjects(e, list, EPCFunction.class, EPCEvent.class, true, false);
            return list;
    }

    /**
     * Get a list of functions preceeding a connector of an EPC either directly or via other connectors and events.
     *
     * @param EPCConnector A connector of the EPC
     * @return ArrayList<EPCFunction> A list of all EPC functions that preceed the function either directly or via other connectors and events.
     */
    public ArrayList<EPCFunction> getPreceedingFunctions(EPCConnector c) {
            ArrayList list = new ArrayList();
            addSurroundingObjects(c, list, EPCFunction.class, EPCFunction.class, true, false);
            return list;
    }

    /**
     * Get a list of functions preceeding a function of an EPC via connectors and events.
     *
     * @param EPCFunction A function of the EPC
     * @return ArrayList<EPCFunction> A list of all EPC functions that preceed the function via connectors and events.
     */
    public ArrayList<EPCFunction> getPreceedingFunctions(EPCFunction f) {
            ArrayList list = new ArrayList();
            addSurroundingObjects(f, list, EPCFunction.class, EPCFunction.class, true, false);
            return list;
    }

    /**
     * Get a list of functions succeeding an event of an EPC either directly or via connectors.
     *
     * @param EPCEvent An event of the EPC
     * @return ArrayList<EPCFunction> A list of all EPC functions that succeed the function either directly or via connectors.
     */
    public ArrayList<EPCFunction> getSucceedingFunctions(EPCEvent e) {
            ArrayList list = new ArrayList();
            addSurroundingObjects(e, list, EPCFunction.class, EPCEvent.class, true, true);
            return list;
    }

    /**
     * Get a list of functions succeeding a connector of an EPC either directly or via other connectors and events.
     *
     * @param EPCConnector A connector of the EPC
     * @return ArrayList<EPCFunction> A list of all EPC functions that succeed the function either directly or via other connectors and events.
     */
    public ArrayList<EPCFunction> getSucceedingFunctions(EPCConnector c) {
            ArrayList list = new ArrayList();
            addSurroundingObjects(c, list, EPCFunction.class, EPCFunction.class, true, true);
            return list;
    }

    /**
     * Get a list of functions succeeding a function of an EPC via connectors and events.
     *
     * @param EPCFunction A function of the EPC
     * @return ArrayList<EPCFunction> A list of all EPC functions that succeed the function via connectors and events.
     */
    public ArrayList<EPCFunction> getSucceedingFunctions(EPCFunction f) {
            ArrayList list = new ArrayList();
            addSurroundingObjects(f, list, EPCFunction.class, EPCFunction.class, true, true);
            return list;
    }

    private void addSurroundingObjects(EPCObject o, ArrayList list,
                                       Class searchFor,
                                       Class doNotCross, boolean init,
                                       boolean forward) {
        if (!init && searchFor.isAssignableFrom(o.getClass())) {
            list.add(o);
        } else if (init || !doNotCross.isAssignableFrom(o.getClass())) {
            Iterator it = (forward ? o.getOutEdgesIterator() :
                           o.getInEdgesIterator());
            while (it.hasNext()) {
                EPCEdge e = ((EPCEdge) it.next());
                EPCObject node = (EPCObject) (forward ? e.getDest() :
                                              e.getSource());
    ...
    public void copyAllFrom(ConfigurableEPC epc, HashMap org2new) {
        ...
        while (it.hasNext()) {
            ...
            if (f instanceof EPCFunction) {
                newF = addFunction(new EPCFunction(f.getLogEvent(),
                        f.isConfigurable(), this));
            } else {
                newF = addFunction(new EPCSubstFunction(f.getLogEvent(),
                        f.isConfigurable(), this,
                                   ((EPCSubstFunction) f).getSubstitutedEPC()));
            }
        ...
        while (it.hasNext()) {
            ...
            org2new.put(f.getIdKey(), addEvent(new EPCEvent(f.getIdentifier(), this)));
        }
        it = epc.getConnectors().iterator();
        while (it.hasNext()) {
            EPCConnector f = (EPCConnector) it.next();
            org2new.put(f.getIdKey(), addConnector(new EPCConnector(f.getType(), this)));
        }
    ...
    public void writePPMImport(OutputStream out) throws IOException {
        ...
        while (it.hasNext()) {
            ...
            write("         <attribute type=\"AT_OBJNAME_INTERN\">" +
                  StringNormalizer.escapeXMLCharacters(editName(o.getIdentifier())) +
                  "</attribute>\n", out);
            ...
            if (o.object2 instanceof AuditTrailEntry) {
                ...
                while (dataKeys.hasNext()) {
                    ...
                    s.replaceAll(" ", "_");
                    write("         <attribute type=\"AT_" + s + "\">" +
                          StringNormalizer.escapeXMLCharacters((String) data.
                            get(key)) +
                          "</attribute>\n", out);
                }
            }
