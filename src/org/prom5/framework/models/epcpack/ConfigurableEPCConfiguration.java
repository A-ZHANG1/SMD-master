...
public class ConfigurableEPCConfiguration  {
	...
	/**
	 * Returns true if applying this configuration to its EPC yields an EPC where
	 * no object is configured anymore.
	 *
	 * @return boolean
	 */
	public boolean isComplete() {
		...
		// if the collection changes when one object is not present in the configMapping
		// return false
	...
	public Integer addConfigItem(EPCConfigurableObject object, Integer configuration) {
	...
	public Integer addConfigItem(EPCConfigurableObject object, Integer configuration, EPCObject sequenceObject) {
		if (!object.isConfigurable()) {
			return null;
		}
		if ( (configuration.equals(OR_AS_SEQ) ||configuration.equals(XOR_AS_SEQ)) && sequenceObject != null) {
			eraseConfig(object);
			configMapping.put(new Integer(object.getId()),configuration);
