 ...
 * <p>Title: </p>
 *
 * <p>Description: This class builds a fragment of an EPC, based on a visible function f.
 * The EPC contains the spanning tree of the original epc starting in f upto the preceeding events.
 * The EPC contains the spanning tree of the original epc starting in f downto the succeeding events.
 * </p>
 *
...
public class EPCFragment extends ConfigurableEPC {
	...
	public EPCFragment(EPCFunction function) {
		...
		EPCFunction f = addFunction(new EPCFunction(function.getLogEvent(), function.isConfigurable(), this));
	...
	private void constructBack(EPCObject o, boolean invis) {
		if (o instanceof EPCEvent) {
			...
			while (it.hasNext()) {
				EPCObject p = (EPCObject) it.next();
				constructBack(p, false);
				addEdge((EPCObject) org2frag.get(p.getIdKey()), (EPCObject) org2frag.get(o.getIdKey()));
			}
			return;
			...
			while (it.hasNext()) {
				EPCObject p = (EPCObject) it.next();
				constructBack(p, false);
				addEdge((EPCObject) org2frag.get(p.getIdKey()), (EPCObject) org2frag.get(f.getIdKey()));
			}
			return;
		}
	}
	...
	private void constructForward(EPCObject o, boolean invis) {
		if (o instanceof EPCEvent) {
			...
			while (it.hasNext()) {
				EPCObject p = (EPCObject) it.next();
				constructForward(p, false);
				addEdge((EPCObject) org2frag.get(o.getIdKey()), (EPCObject) org2frag.get(p.getIdKey()));
			}
			return;
		} else if (o instanceof EPCConnector) {
			EPCConnector f = (EPCConnector) o;
			...
			while (it.hasNext()) {
				EPCObject p = (EPCObject) it.next();
				constructForward(p, false);
				addEdge((EPCObject) org2frag.get(f.getIdKey()), (EPCObject) org2frag.get(p.getIdKey()));
			}
			return;
		}
	}
