...
public class EPCFunction extends EPCConfigurableObject implements LogEventProvider, DOTCodeWriter {
	...
	public EPCFunction(LogEvent logModelElement, ConfigurableEPC epc) {
		...
		/* for testing purposes only:
		   this.addDataObject(new EPCDataObject("test Data", this));
		   this.addInfSysObject(new EPCInfSysObject("test InfSys", this));
		   this.addOrgObject(new EPCOrgObject("test Org", this));
		   this.addDataObject(new EPCDataObject("test Data 2", this));
		   this.addInfSysObject(new EPCInfSysObject("test InfSys 2", this));
		   this.addOrgObject(new EPCOrgObject("test Org 2", this));
		 /**/
	}
	...
	public void writeDOTCode(Writer bw, HashMap nodeMapping) throws IOException {
		bw.write("node" + getId() +
				" [shape =\"box\",style=\"filled" + (isConfigurable() ? ",bold" :
	...
	private void writeAdditionalDotCode(Writer bw, HashMap nodeMapping, ArrayList additions,
		...
		for (int i = 0; i < additions.size(); i++) {
			...
			if (i > 0) {
				bw.write("node" + ((DOTCodeWriter) additions.get(i - 1)).getId() + " -> node" +
						o.getId() + " [style=\"invis\"];\n");
			}
		}
		bw.write("}\n");
		DOTCodeWriter o = (DOTCodeWriter) additions.get(0);
		bw.write("node" + getId() + " -> node" + o.getId() +
