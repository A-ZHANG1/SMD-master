...
public class ConnectorStructureExtractor {
	...
	public synchronized static ConfigurableEPC extract(ConfigurableEPC baseEPC, boolean singleInSingleOut
			, boolean sameSplitAndJoin
			, boolean splitWithOrJoin
			, boolean sameSplitAndSplit
			, boolean sameJoinAndJoin
			, boolean xorLoop
			, boolean orLoop
			) {
		return extract(baseEPC, singleInSingleOut, sameSplitAndJoin, splitWithOrJoin,
				sameSplitAndSplit, sameJoinAndJoin, xorLoop, orLoop, true, true, true);
	}

	// Sometimes, we only need to extract single-in single-out connectors.
	// For this reason, I've extended the extract method with three flags: connectors, functions, and events.
	// - Eric.
	public synchronized static ConfigurableEPC extract(ConfigurableEPC baseEPC, boolean singleInSingleOut
			, boolean sameSplitAndJoin
			, boolean splitWithOrJoin
		...
		return extract(baseEPC, singleInSingleOut, sameSplitAndJoin, splitWithOrJoin,
				sameSplitAndSplit, sameJoinAndJoin, xorLoop, orLoop, connectors, functions, events, new HashMap());
	}
	...
	public synchronized static ConfigurableEPC extract(ConfigurableEPC baseEPC, boolean singleInSingleOut
		...
		do {
			...
			if (singleInSingleOut) {
				b = b || removeSingleInputSingleOutput(cs, connectors, functions, events);
			}
		...
		while (it.hasNext()) {
			...
			while (it2.hasNext()) {
				...
				if ((c.outDegree() > 1) && (c2.outDegree() > 1) &&
					...
					EPCEdge inEdge = (EPCEdge) c2.getInEdges().iterator().next();
					Iterator it3 = c2.getOutEdges().iterator();
					while (it3.hasNext()) {
						EPCEdge e = (EPCEdge) it3.next();
						EPCEdge e2 = (EPCEdge) cs.addEdge(new EPCEdge(c, (EPCObject) e.getDest()));
		...
		while (it.hasNext()) {
			...
			while (it2.hasNext()) {
				...
				if ((c.inDegree() > 1) && (c2.inDegree() > 1) &&
					...
					EPCEdge outEdge = (EPCEdge) c2.getOutEdges().iterator().next();
					Iterator it3 = c2.getInEdges().iterator();
					while (it3.hasNext()) {
						EPCEdge e = (EPCEdge) it3.next();
						EPCEdge e2 = (EPCEdge) cs.addEdge(new EPCEdge((EPCObject) e.getSource(), c));
						e2.object2 = e.object2;
						((HashSet) e2.object2).addAll((HashSet) outEdge.object2);
					}
					// Remove c2
					((HashSet) c.object2).addAll((HashSet) c2.object2);
					cs.delConnector(c2);
	...
	private static boolean removeXorJoinXorSplitLoops(ConfigurableEPC cs) {
		...
		while (it.hasNext()) {
			...
			if ((e.getValue() > 1) || (e2.getValue() > 1) || (e.getSource().inDegree() <= 1)) {
				continue;
			}
			EPCConnector c1 = (EPCConnector) e.getSource();
			EPCConnector c2 = (EPCConnector) e.getDest();

			if ((c1.getType() == c2.getType()) && (c1.getType() == EPCConnector.XOR)) {
				result = true;
	...
	private static boolean removeOrJoinAnySplitLoops(ConfigurableEPC cs) {
		...
		while (it.hasNext()) {
			...
			if ((c1.getType() == EPCConnector.OR) && (c1.getType() != EPCConnector.AND)) {
				result = true;
	...
	private static boolean removeSingleInputSingleOutput(ConfigurableEPC cs, boolean connectors,
			boolean functions, boolean events) {
		Iterator it = cs.getVerticeList().iterator();
		boolean result = false;
		while (it.hasNext()) {
			...
			if ((c instanceof EPCConnector && connectors) ||
				...
				if ((c.inDegree() == 1) && (c.outDegree() == 1)) {
					...
					a.addAll((HashSet) ((EPCEdge) c.getInEdges().iterator().next()).object2);
					a.addAll((HashSet) ((EPCEdge) c.getOutEdges().iterator().next()).object2);
