...
package org.prom5.framework.models.epcpack.algorithms;
...
public class EPCToPetriNetConverter {
	 ...
	 *
	 * @param baseEPC EPC
	 * @param connectorMapping HashMap
	 * @return PetriNet
	 */
	public static PetriNet convert(ConfigurableEPC baseEPC, HashMap connectorMapping) {
		return convert(baseEPC, connectorMapping, null, null);
    }

	//HV: Extend this method with two mappings: one from functions to transitions and one from xor connectors to places.
	//HV: Both may be null.
    public static PetriNet convert(ConfigurableEPC baseEPC, HashMap connectorMapping,
			HashMap<EPCFunction, Transition> functionMapping, HashMap<EPCConnector, Place> xorMapping) {
	    ...
	    //HV 08-01024: Places get merged somewhere late in the conversion. As a result, places mapped onto might need updating.
		...
		while (fi.hasNext()) {
			...
			counter++;
			Transition t = petrinet.addTransition(new Transition(f.getLogEvent(), petrinet));
			...
			if (f.inDegree() > 0) {
				outmapping.put(getInEdgeObjectIterator(f).next().getIdKey(), p1);
			}
			if (f.outDegree() > 0) {
				inmapping.put(getOutEdgeObjectIterator(f).next().getIdKey(), p2);
			}
		}
		...
		while (ci.hasNext()) {
			...
			//HV 08-01-22 Maps c to an empty list of transitions? What's the use?
			// THis is not mapped onto an empty list. The list is filled afterwards.
			...
			if (c.getType() == EPCConnector.AND) {
				...
				if ((c.inDegree() == 1)) {
					...
					outmapping.put(getInEdgeObjectIterator(c).next().getIdKey(), inPlace);

					// Connectors are mapped onto lists of transitions. Please check code before changing
					// it! This was essential for EPC verification!
//					connectorMapping.put(c, inPlace); //HV 08-01-22

					counter++;
					Transition t = petrinet.addTransition(new Transition("t" + counter, petrinet));
					t.object = c;
					counter++;
					transitions.add(t);
				...
				if ((c.outDegree() == 1)) {
					Place outPlace = petrinet.addPlace("p" + counter);
					counter++;
					inmapping.put(getOutEdgeObjectIterator(c).next().getIdKey(), outPlace);
...
//					connectorMapping.put(c, inPlace); //HV 08-01-22
					...
					//HV: Remember that this xor connector is mapped onto this place and v.v.
					if (xorMapping != null) {
					...
					Iterator<Edge> it = getOutEdgeObjectIterator(c);
					while (it.hasNext()) {
						Transition t = petrinet.addTransition(new Transition("t" + counter,
									   petrinet));
						transitions.add(t);
						counter++;
						t.object = c;
...
//					connectorMapping.put(c, outPlace); //HV 08-01-22
		...
		while (ied.hasNext()) {
			...
			// to be removed
			if (inmapping.get(e.getIdKey()) == null || outmapping.get(e.getIdKey()) == null) {
			...
			if (p2.object instanceof EPCEvent) {
				...
				// Check whether the place removed (p1) was mapped onto. If so, map onto p2 instead.
				...
				// Check whether the place removed (p2) was mapped onto. If so, map onto p1 instead.
...
//		return  node.getInEdges().iterator(); // TODO: original code
...
//		return  node.getOutEdges().iterator(); // TODO: original code
