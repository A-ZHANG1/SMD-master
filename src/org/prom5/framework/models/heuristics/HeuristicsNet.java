...
package org.prom5.framework.models.heuristics;
...
/**
 *
 * <p>Title: Heuristics Net</p>
 *
 * <p>Description: </p>
 *
 * <p>Copyright: Copyright (c) 2004</p>
 *
 * <p>Company: Eindhoven University of Technology (TU/e)</p>
 *
...
public class HeuristicsNet implements Comparable, DotFileWriter {
	 ...
	 * @param event LogEvents event log.
	 */
	 ...
	 * @param event LogEvents event log.
	 * @param duplicatesMapping int[] The mapping from the tasks in the net to the code in the event log.
	 * @param reverseDuplicatesMapping HNSubSet[] The mapping from the codes in the log to the tasks in the net.
	 */
	...
	/**
	 * The duplicatesActualFiring variable keeps track of the duplicate tasks that are actually
	 * fired during the parsing of a log. The duplicates that do not fire are not shown to the
	 * end user.
	 */
	...
	/**
	 * The arcUsage variable keeps track of how often the arcs are actually
	 * used during the parsing of a log by this individual.
	 */
	...
	public String diffForSets(HeuristicsNet netToCompare) {
		...
		if (this.size != netToCompare.size()) {
			...
			//really check if the nets have the same structure in terms of labels

			//input sets
			TreeSet thisInput = toStringRepresentation(this.getInputSets(),
								this.getLogEvents(),
								this.getDuplicatesMapping());
			TreeSet toCompareInput = toStringRepresentation(netToCompare.getInputSets(),
									 netToCompare.getLogEvents(),
									 netToCompare.getDuplicatesMapping());
			...
			TreeSet thisOutput = toStringRepresentation(this.getOutputSets(),
								 this.getLogEvents(),
								 this.getDuplicatesMapping());
			TreeSet toCompareOutput = toStringRepresentation(netToCompare.getOutputSets(),
									  netToCompare.getLogEvents(),
									  netToCompare.getDuplicatesMapping());
	...
	private TreeSet toStringRepresentation(HNSet[] set, LogEvents le, int[] duplicatesMapping) {
		TreeSet entries = new TreeSet();

		for (int i = 0; i < set.length; i++) {
			String entry = "Element = " + le.getEvent(duplicatesMapping[i]).getModelElementName() +
						   " (" +
			...
			for (int j = 0; j < set[i].size(); j++) {
				...
				for (int k = 0; k < subset.size(); k++) {
					String label = le.getEvent(duplicatesMapping[subset.get(k)]).
								   getModelElementName() + " (" +
								   le.getEvent(duplicatesMapping[subset.get(k)]).getEventType() +
								   ") ";
	...
	public void increaseArcUsage(int element, HNSubSet usedInputElements, int amount) {
		for (int inputElementPosition = 0; inputElementPosition < usedInputElements.size();
										inputElementPosition++) {
			arcUsage.setQuick(usedInputElements.get(inputElementPosition), element,
					(arcUsage.getQuick(usedInputElements.get(inputElementPosition),
					element) + amount));
		}
	 ...
	 * @param events LogEvents new events to be associated to this net.
	 */
	 ...
	 * @param duplicatesMapping int[] the array of int that contains the duplicates mapping.
	...
	public static HNSubSet[] buildReverseDuplicatesMapping(int[] duplicatesMapping) {
		...
		for (int i = 0; i < duplicatesMapping.length; i++) {
			if (duplicatesMapping[i] >= 0 && duplicatesMapping[i] < reverseDuplicatesMapping.length) {
	...
	public String toString() {
		...
		for (int i = 0; i < size(); i++) {
			...
			sb.append(INPUT_SETS_HEADER).append(": ").append(this.inputSets[i].toString()).append(
					"\n");
			sb.append(OUTPUT_SETS_HEADER).append(": ").append(this.outputSets[i].toString()).append(
					"\n");
		}
		return sb.toString();
	}
	...
	public String toStringWithEvents() {
		...
		for (int i = 0; i < size(); i++) {
			sb.append("\n\n").append(WME_HEADER).append(" ").append(WME_NAME_DELIMITER).append(
					events.getEvent(duplicatesMapping[i]).getModelElementName()).append(" (id = ").
					append(
					i).append(") ").append(" (").append(events.getEvent(duplicatesMapping[i]).
					getEventType())
	...
	private void buildVisualPresentation(StringBuffer sb, HNSet set, LogEvents events) {
		...
		if (set != null) {
			...
			for (int i = 0; i < set.size(); i++) {
				...
				for (int j = 0; j < subset.size(); j++) {
					...
					sb.append(" ").append(WME_NAME_DELIMITER).append(events.getEvent(element).
							getModelElementName()).append(" (id = ").append(
							subset.get(j)).append(") ").append(" (").append(events.getEvent(element).
							getEventType())
			...
			for (i = 0; i < set.size(); i++) {
				...
				for (j = 0; j < subset.size(); j++) {
					if (nonDanglingTasks[subset.get(j)] >= 0) {
						convertedSet.append(nonDanglingTasks[subset.get(j)]).append(OR_SEPARATOR);
					}
				}
				if (j > 0) {
	...
	public void toFile(OutputStream output) throws IOException {
		...
		if (thereAreNonDanglingTasks) {
			...
			for (int i = 0; i < startTasks.size(); i++) {
				line.append(nonDanglingTasks[startTasks.get(i)]).append(this.SETS_SEPARATOR);
			}
			line.append("\n");
			...
			for (int i = 0; i < endTasks.size(); i++) {
				line.append(nonDanglingTasks[endTasks.get(i)]).append(this.SETS_SEPARATOR);
			}
			line.append("\n");
			...
			for (int i = 0; i < reverseDuplicatesMapping.length; i++) {
				...
				line.append(le.getModelElementName() + this.EVENT_SEPARATOR + le.getEventType());
				...
				for (int j = 0; j < reverseDuplicatesMapping[i].size(); j++) {
					if (nonDanglingTasks[reverseDuplicatesMapping[i].get(j)] >= 0) {
						line.append(nonDanglingTasks[reverseDuplicatesMapping[i].get(j)]);
			...
			for (int i = 0; i < inputSets.length; i++) {
				if (nonDanglingTasks[i] >= 0) {
					...
					line.append(SETS_SEPARATOR);
					line.append(convertToFileFormat(inputSets[i], nonDanglingTasks));
					line.append(SETS_SEPARATOR);
					line.append(convertToFileFormat(outputSets[i], nonDanglingTasks));
					line.append("\n");
				}
			}
		}
	...
	protected Object clone() {
		...
		System.arraycopy(this.duplicatesMapping, 0, dmCopy, 0, this.duplicatesMapping.length);
		...
		//creating the copy...
		copy = new HeuristicsNet(events, dmCopy,
			   rdmCopy);
	...
	public int hashCode() {
		int hashCode = 0;
		for (int i = 0; i < size(); i++) {
			hashCode += ((this.getInputSet(i).hashCode() + this.getOutputSet(i).hashCode()) * 31 ^
					(size - i + 1));
		}
		return hashCode;
	}
	 ...
	 * @param index  workflow model element whose INPUT set must the searched.
	 * @param element  element to find in the INPUT subsets.
	 ...
	 * @param index  workflow model element whose OUTPUT set must the searched.
	 * @param element  element to find in the OUTPUT subsets.
	 * @return a TreeSet that contains 'element'. Otherwise, null.
	 */
	...
	/*
	 * Retrieves a subset from an INPUT/OUTPUT set of "index". This subset contains "element".
	 * Returns null if there is not such index's TreeSet with element.
	 */
	 ...
	 * @param index  workflow model element whose INPUT set must the searched.
	 * @param element  element to find in the INPUT subsets.
	 * @return a HashSet with the subsets that contain 'element'. Otherwise, null.
	 */
	...
	/*
	 * Retrieves  subsets from an INPUT/OUTPUT set of "index". These subsets contain "element".
	 * Returns null if there is not such subsets with element.
	 */
	...
	public void writeToDot(Writer bw) throws IOException {
		...
		//visualization of the heuristics net would be the one to be exported to DOT.
	...
	public void writeToDotWithoutSplitJoinSemantics(Writer bw) throws IOException {
		//correcting individual for visual presentation
		//two individuals with different genotype can have the same phenotype
		//HeuristicsNet phenotype = MethodsOverIndividuals.removeDanglingElementReferences((HeuristicsNet)this.clone());

		bw.write("digraph G {\n");
		bw.write("  size=\"6,10\"; fontname=\"Arial\"; fontsize=\"12\";\n");
		bw.write("  node [shape=\"box\",fontname=\"Arial\",fontsize=\"12\"];\n");
		//bw.write("  node [shape=\"rect\",fontname=\"Arial\",fontsize=\"12\",style=filled, fillcolor=red];\n");

		// write nodes
		for (int i = 0; i < size(); i++) {
			if (((inputSets[i].size() > 0) || (outputSets[i].size() > 0)) ||
					((this.getStartTasks().contains(i)) || (this.getEndTasks().contains(i)))) {

				bw.write("E" + i +
						" [label=\"" +
						events.getEvent(duplicatesMapping[i]).getModelElementName().replace('"',
						'\'') + /*" " + i +*/ "\\n(" +
						events.getEvent(duplicatesMapping[i]).getEventType().replace('"', '\'') +
		...
		for (int from = 0; from < size(); from++) {
			...
			for (int iSet = 0; iSet < set.size(); iSet++) {
				...
				bw.write("E" + from + " -> E" + to +
						" [style=\"filled\", label=\"  " + (int) arcUsage.get(from,
						to) + "\"];\n");
			}
		}
		bw.write("}\n");
	...
	public void writeToDotWithSplitJoinSemantics(Writer bw) throws IOException {
		...
		bw.write("digraph G {\n");
		bw.write("  size=\"6,10\"; fontname=\"Arial\"; fontsize=\"12\";\n");
		bw.write("  node [shape=\"record\",fontname=\"Arial\",fontsize=\"12\"];\n");
		//bw.write("  node [shape=\"rect\",fontname=\"Arial\",fontsize=\"12\",style=filled, fillcolor=red];\n");
		// write nodes
		for (int i = 0; i < size(); i++) {
			if (((inputSets[i].size() > 0) || (outputSets[i].size() > 0)) ||
				...
				if (inputSets[i].size() > 0) {
					...
					for (int j = 0; j < inputSets[i].size(); j++) {
						...
						bw.write("<" + toInputDotName(inputSets[i].get(j).toString()) + ">  XOR ");
					}
					bw.write("} | ");
				}
				bw.write(events.getEvent(duplicatesMapping[i]).getModelElementName().replace('"',
						'\'') + " \\n(" +
						events.getEvent(duplicatesMapping[i]).getEventType().replace('"', '\'') +
						")\\n " + duplicatesActualFiring[i]);
				if (outputSets[i].size() > 0) {
					...
					for (int j = 0; j < outputSets[i].size(); j++) {
						...
						bw.write("<" + toOutputDotName(outputSets[i].get(j).toString()) + "> XOR ");
					}
					bw.write("}}");
				}
		...
		for (int from = 0; from < size(); from++) {
			//Iterator set = phenotype.getAllElementsOutputSet(from).iterator();
			for (int outSubsetIndex = 0; outSubsetIndex < outputSets[from].size(); outSubsetIndex++) {
				HNSubSet outSubset = outputSets[from].get(outSubsetIndex);
				for (int outSubsetElementIndex = 0; outSubsetElementIndex < outSubset.size();
												 outSubsetElementIndex++) {
					int to = outSubset.get(outSubsetElementIndex);
					HNSet inputSubsetsElementWithFrom = getInputSetsWithElement(to, from);
					for (int k = 0; k < inputSubsetsElementWithFrom.size(); k++) {
						bw.write("E" + from + ":" + toOutputDotName(outSubset.toString()) +
								" -> E" + to + ":" +
								toInputDotName(inputSubsetsElementWithFrom.get(k).toString()) +
								" [style=\"filled\", label=\"  " + (int) arcUsage.get(from,
								to) + "\"];\n");
					}
				}
	...
	private String toDotName(String s) {
		return "p" + s.trim().replace('[', 'a').replace(']', 'b').replace(',', 'c');
	}

	/*
	 * This method disconnects from the tasks that are not used during the parsing of a log.
	 * The main idea is to remove duplicates that are not used.
	 */
	private void disconnectUnusedTasks() {
		...
		for (int iUnfiredElements = 0; iUnfiredElements < unfiredElements.size();
									iUnfiredElements++) {
	 ...
	 * threshold (inclusive). In other words, only the arcs that are used more times
	...
	/**
	 * This method builds a phenotype that disconnect the
	 * tasks and arcs that are not used during the parsing
	 * of an event log. <p> <b> NOTE: This method should be used with care
	 * because at the starting of a parsing,
	 * no elements (tasks) have been used.</b>
	 */
...
class DuplicateTasksHeuristicsNetModelGraph extends ModelGraph {
	...
	public DuplicateTasksHeuristicsNetModelGraph(HeuristicsNet net, boolean showSplitJoinSemantics) {
