...
public class MarkingHeuristicsNet {
	 ...
	 * @throws java.lang.NullPointerException whenever the net has disconnected elements. An element is disconnected
	 * when its INPUT or OUTPUT set is null.
	 */
	public MarkingHeuristicsNet(HeuristicsNet net,
			Random generator) throws NullPointerException {
	...
	/**
	 * Removes one token from the marked output places (subsets) of the tasks in <i>tasks</i>
	 * that contain <i>element</i>.
	...
	/**
	 * @throws java.lang.NullPointerException when inElement is not in the hash. I.e, inElement has OUT=[]
	 * @throws java.lang.NumberFormatException when outElement does not belong to the OUT set of inElement.
	 */
	...
	private void setNumberTokens(int inElement, HNSubSet outElement,
			Integer numTokens) throws NullPointerException, NumberFormatException {
	...
	private void addTokensOutputPlaces(int element) {
		...
		/*
		 * Pseudo-code:
		 * 1. Retrieve all elements in the OUT set of 'element'
		 * 2. Increase the number of tokens for every output element
		 */
	...
	/**
	 * Fires a element even if it is not enabled. When the element has duplicates, it
	 * looks ahead to set which duplicate to fire.
	...
	public int fire(int element, ProcessInstance pi, int elementPositionInPi) {
		...
		if ((hNet.getReverseDuplicatesMapping()[element]).size() == 1) {
			...
			if (duplicates.size() > 0) {
				if (duplicates.size() == 1) {
					...
					HNSubSet unionMappedToATEsCode = getAllOutputElementsOfDuplicates(
							duplicates);
					...
					for (int i=elementPositionInPi+1; i<ATEntriesList.size(); i++){
						try {
							...
							elementInATE = this.hNet.getLogEvents().findLogEventNumber(ATEntry.
										   getElement(),
										   ATEntry.getType());
				...
				//because no duplicate is enabled, a random one is chosen to fire...
				elementDuplicates = (hNet.getReverseDuplicatesMapping()[element]).get(generator.
									nextInt(hNet.getReverseDuplicatesMapping()[element].size()));
			}
		...
		hNet.increaseArcUsage(bestCombination.getElementToFire(), bestCombination.getTasks(),
				MethodsForWorkflowLogDataStructures.getNumberSimilarProcessInstances(pi));
...
//>>>>>>>>>>>>>>>>>> Hint!!! I think that's why I don't run into problems...
			...
			if (inputSet.size() > 0) {
				...
				bCombination = findBestCombination(bCombination, inputSet, rootTask,
							   combination,
							   treatedTasks);
			}
		}
	...
	private CombinationTasksToFire findBestCombination(
			CombinationTasksToFire bCombination, HNSet inputSet,
			int rootTask, CombinationTasksToFire combination,
			...
			if (inputSet.size() == 0) {
				...
				for (int iInputSet = 0; iInputSet < inputSet.size(); iInputSet++) {
					...
					while (subset.size() > 0) {
						...
						bCombination = findBestCombination(bCombination, inputSet.deepCopy(),
									   task, combination.copy(), treatedTasks.deepCopy());
	...
	/*
	 * Checks if a element is enabled at the current marking.
	 * <p> <b>Note:</b> The element MUST be in the Element.
	 * @param element element/transition to check
	 * @return true if the element is enabled at the current marking. false otherwise.
	 */
...
class MappingToSubsets {
	...
	/**
	 * Returns the output subsets of <i>task</i> that contains <i>taskInSubset</i>
	 * @return a HashSet with the subsets. Returns null if there
	 * are no subsets.
	 */
...
class CombinationTasksToFire {
	...
	/**
	 * This method sets the element that is going to fire for this combination of tasks.
	 * This method is useful when dealing with duplicate tasks because we precisely know
	 * which duplicate requires this combination.
	 * Note: This method is useful when counting arc usage for an individual!
	 */
	...
	/**
	 * This method returns the element that is going to fire for this combination of tasks.
	 * This method is useful when dealing with duplicate tasks because we precisely know
	 * which duplicate requires this combination.
	 * Note: This method is useful when counting arc usage for an individual!
	...
	/**
	 * Sets the stored number of missing tokens. The number of missing
	 * tokens ranges from 0 to maximum integer. So, if <i>newNumberMissingTokens</i>
	 * is smaller than 0, the number of missing tokens is automatically
	 * set to 0.
	...
	/**
	 * Returns the String representation of CombinationTasksToFire.
	 * The format is "[set of tasks to fire] : number_of_missing_tokens"
	 * @return
	 */
