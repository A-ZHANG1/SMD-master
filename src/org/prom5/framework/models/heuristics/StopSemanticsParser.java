...
public class StopSemanticsParser implements HeuristicsParser {
	...
	public boolean parse(ProcessInstance pi) {
		...
		while (ATEntriesListIterator.hasNext()) {
			...
			element = logEvents.findLogEventNumber(ATEntry.getElement(), ATEntry.getType());
			try {
				arcUsageBeforeFiring = net.getArcUsage();
				duplicatesActualFiringBeforeFiring = net.getDuplicatesActualFiring();
			...
			if (numTokens > 0) {
				...
				numUnparsedElements = pi.getAuditTrailEntryList().size() - numParsedElements;
				...
				net.setDuplicatesActualFiring(duplicatesActualFiringBeforeFiring);
				return false;
			}
			numParsedElements++;
