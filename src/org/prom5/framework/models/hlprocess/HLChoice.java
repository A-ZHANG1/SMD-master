...
/**
 * Holds information with respect to a decision point in the process.
 * <br>
 * Note that a choice is defined independently of any concrete choice node in the 
 * context of a process model (e.g., an XOR connector in an EPC model). It only 
 * specifies a set of target activities that might be enabled by this choice, and 
 * enabling conditions for each of them. 
 * <br>
 * If the choice is to be connected to a specific node in a process model, then this 
 * can be done in the high level process implementation for the corresponding 
 * process model type (e.g., a Place for the HighLevelPetriNet).
 * 
 * @see HLCondition
...
public class HLChoice extends HLProcessElement implements Cloneable {
	...
	/** maps the activities that might be enabled by this choice to their enabling conditions */
	protected HashMap<HLID,HLCondition> conditions = new HashMap<HLID,HLCondition>();
	
	// TODO Anne: move the configuration to the cpn export (i.e., out of the high level process)
	/** the choice configuration */
	protected ChoiceEnum myConfiguration;
	
	/**
	 * Default constructor (automatically registers the new choice for 
	 * the given high level process).
	 * @param name the name for this choice
	 * @param aProc the high level process this choice belongs to
	 */
	 ...
	 * @param actID the ID of the activity for which the pre-conditions are requested
	 * @return the condition object if activity involved in this choice. <code>Null</code> otherwise
	 */
	 ...
	 * @param cond the condition for which the target is requested
	 * @return the activity that is associated to this condition. <code>Null</code> if not found
	 */
	...
	/**
	 * Retrieves the high-level activities that may be activated by 
	 * this choice (activation can range from activating 0 to all, 
	...
	/**
	 * Replaces the given old target activity ID by the new ID. 
	 * The condition information stays the same, and will be from now 
	 * related to the new high level activity for this choice. 
	 * <br>
	 * If the old target activity is not part of this choice, 
	 * the method has no effect. 
	 * @param oldTarget the previous condition target
	 * @param newTarget the new condition target
	 */
	...
	/**
	 * Removes the condition for the specified target activity.
	 * <br>
	 * If the specified activity was never a target for this choice, 
	 * the method has no effect. 
	 * @param target the ID of the given target activity
	 */
	...
	/**
	 * Adds a new condition for the specified activity.
	 * <br>
	 * Note that if the activity is already part of this choice, 
	 * calling this method has no effect. 
	 * @param actID the ID of the activity associated with this choice target (branch)
	...
	// TODO Anne: move the configuration to the cpn export (i.e., out of the high level process)
	/**
	 * Indicates which kind of information should be used in the simulation model
	 * to choose for one of the alternative paths for this choice node.
	 ...
	 * @param aChoice the preferred choice configuration
	 */
