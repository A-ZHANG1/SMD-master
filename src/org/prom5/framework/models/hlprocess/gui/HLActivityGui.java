...
import org.prom5.framework.models.hlprocess.HLTypes.TransformationType;
import org.prom5.framework.models.hlprocess.gui.att.dist.HLDistributionGui;
import org.prom5.framework.models.hlprocess.gui.att.dist.HLDistributionGuiManager;
...
import org.prom5.framework.util.GuiPropertyObjectSet;

/**
 * Gui class for high level activity with input and output data attributes.
 * <br>
 * Allows to view and edit the activity characteristics
...
public class HLActivityGui implements GuiDisplayable, GuiNotificationTarget {
         ...
         * @param act the high level activity to be made Gui-displayable
         */
        ...
        private void createPanel() {
                ...
                heading.add(new JLabel("Options available for activity:   " + toString()));
                ...
                //
                // Fill myAttributesResampleOrReuse for indicating whether a data attribute has to
                // be resampled or reused.
                // When clicking on the add button the selected value in the combobox of myAttributesResampleOrReuse
                // determines whether the added data attribute has to be resampled or reused
                ...
                while (inputAttrs.hasNext()) {
                        ...
                        inputInitValPropDatAttr.put(input, hlActivity.getTransformationType(input.getID()));
                }

                Iterator<HLAttribute> availableInputAttributes = hlActivity.getHLProcess().getAttributes().iterator();
                ...
                inputAttributesProperty = new GuiPropertyObjectSet("Input data attributes",
                "Specifies the input data attributes that are provided by this activity",
                hlActivity.getInputDataAttributes(), inputSet, modType, inputInitValPropDatAttr, this);
                ...
                //
                // get the init values for the property of the data attributes
                HashMap<HLAttribute, TransformationType> outputInitValPropDatAttr = new HashMap<HLAttribute, TransformationType>(); //java.util.HashMap does not take parameters.
                ...
                while (outputAttrs.hasNext()) {
                        ...
                        outputInitValPropDatAttr.put(output, hlActivity.getTransformationType(output.getID()));
                }

                Iterator<HLAttribute> availableOutputAttributes = hlActivity.getHLProcess().getAttributes().iterator();
               ...
               outputAttributesProperty = new GuiPropertyObjectSet("Output data attributes",
                               "Specifies the output data attributes that are provided by this activity",
                               hlActivity.getOutputDataAttributes(), outputSet, modType, outputInitValPropDatAttr, this);
               content.add(outputAttributesProperty.getPropertyPanel());
               content.add(Box.createRigidArea(new Dimension(0, 30)));


                // take current group into account and put as the first element (i.e. initial value)
                ...
                Iterator<HLGroup> groupsIt = hlActivity.getHLProcess().getAllGroups().iterator();
                ...
                groupProperty = new GUIPropertyListEnumeration("Group", "Indicates which group of available resources may perform this activity", groups, this, 200);
                ...
                if (hlActivity.getExecutionTime() != null) {
                        ...
                        distGui = HLDistributionGuiManager.getDistributionGui(hlActivity.getExecutionTime(), this);
                ...
                if (hlActivity.getWaitingTime() != null) {
                        ...
                        content.add(wtPanel);
                        content.add(Box.createRigidArea(new Dimension(0, 10)));
                        distGui = HLDistributionGuiManager.getDistributionGui(hlActivity.getWaitingTime(), this);
                        content.add(distGui.getPanel());
                        content.add(Box.createRigidArea(new Dimension(0, 30)));
                }
                ...
                if (hlActivity.getSojournTime() != null) {
                        ...
                        content.add(stPanel);
                        content.add(Box.createRigidArea(new Dimension(0, 10)));
                        distGui = HLDistributionGuiManager.getDistributionGui(hlActivity.getSojournTime(), this);
                        content.add(distGui.getPanel());
                        content.add(Box.createRigidArea(new Dimension(0, 30)));
                }
        ...
        public void updateGUI() {
                ...
                // Together with each input data attribute it is saved whether it needs to be
                // resampled or reused
                Iterator<HLAttribute> inAttIt = hlActivity.getInputDataAttributes().iterator();
                while (inAttIt.hasNext()) {
                        HLAttribute inAtt = inAttIt.next();
                        hlActivity.removeInputDataAttribute(inAtt.getID());
                }
                HashMap<HLAttribute, TransformationType> newInputData = inputAttributesProperty.getAllValuesWithProperty();
                Iterator<Entry<HLAttribute, TransformationType>> allInputs = newInputData.entrySet().iterator();
                ...
                // (as might have been both added or removed via GUI panel)
                // Together with each output data attribute it is saved whether it needs to be
                // resampled or reused
                Iterator<HLAttribute> outAttIt = hlActivity.getOutputDataAttributes().iterator();
                while (outAttIt.hasNext()) {
                    HLAttribute outAtt = outAttIt.next();
                    hlActivity.removeOutputDataAttribute(outAtt.getID());
                }

                HashMap<HLAttribute, TransformationType> newOutputData = outputAttributesProperty.getAllValuesWithProperty();
                Iterator<Entry<HLAttribute, TransformationType>> allOutputs = newOutputData.entrySet().iterator();
                ...
                // For the selected data attribute, check whether it needs to be redistributed
                // or resampled
                ...
                // also update the gui as, for example, distribution type has changed and resulted
