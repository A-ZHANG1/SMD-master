...
import org.prom5.framework.models.hlprocess.gui.att.HLAttributeGuiManager;
...
/**
 * Gui class for high-level attribute objects. 
 * <br>
 * Allows to view and edit the attribute characteristics 
 * through a graphical user interface. 
 * <br>
 * Allows to choose a different type and transparently 
 * replaces the old attribute by the one with the new type.
 * 
 * @see HLProcessGui
 */
public class HLAttributeGui implements GuiDisplayable, GuiNotificationTarget {
	...
	/**
	 * Creates a new Gui object that allows to view and manipulate the data attribute 
	 * object. 
	 * @param att the attribute object reflected by this gui object
	 */
	public HLAttributeGui(HLAttribute att) { 
		hlAttribute = att;
		name = new GUIPropertyString("Attribute Name", 
				"Specifies the name of this data attribute", 
				hlAttribute.getName(), 
				this, 200);
		useInitialValue = new GUIPropertyBoolean("Use initial value", 
				"Indicates whether a fixed initial or default value is available", hlAttribute.usesInitialValue(), this);
		attType = new JComboBox();
		attType.setSize(new Dimension(200, (int) attType.getPreferredSize().getHeight()));
		attType.setPreferredSize(new Dimension(200, (int) attType.getPreferredSize().getHeight()));
		attType.setMaximumSize(new Dimension(200, (int) attType.getPreferredSize().getHeight()));
		attType.setMinimumSize(new Dimension(200, (int) attType.getPreferredSize().getHeight()));
		...
		initialValue = HLAttributeGuiManager.getAttributeValueGui(hlAttribute.getInitialValue(), this);
		possibleValues = HLAttributeGuiManager.getAttributeValueGui(hlAttribute.getPossibleValues(), this);
		initAttributePanel();
	...
	private void registerGuiActionListener() {
		attType.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				hlAttribute = HLAttributeManager.switchAttributeType(type, hlAttribute);
				initAttributePanel();
				// update the GUI
				panel.removeAll();
				createPanel();
	...
	public void updateGUI() {
		...
		if (initialValue.getValue() != null) { // null for nominal attributes as long as there are no values
			hlAttribute.setInitialValue(initialValue.getValue());
		}
		hlAttribute.setPossibleValues(possibleValues.getValue());
		// update gui as, for example, new possible values might be available for 
	...
	private void createPossibleValuesPanel() {
		if (possibleValuesPanel == null) {
			...
			possibleValuesPanel.setLayout(new BoxLayout(possibleValuesPanel, BoxLayout.PAGE_AXIS));
			possibleValuesPanel.setOpaque(false);
		}
		possibleValuesPanel.removeAll();
		JPanel possValuePanel = new JPanel();
		possValuePanel.setLayout(new BoxLayout(possValuePanel, BoxLayout.LINE_AXIS));
	...
	private void createInitialValuePanel() {
		if (initialValuePanel == null) {
			...
			initialValuePanel.setLayout(new BoxLayout(initialValuePanel, BoxLayout.PAGE_AXIS));
		...
		if (useInitialValue.getValue() == true) {
			...
			initValuePanel.setLayout(new BoxLayout(initValuePanel, BoxLayout.LINE_AXIS));
