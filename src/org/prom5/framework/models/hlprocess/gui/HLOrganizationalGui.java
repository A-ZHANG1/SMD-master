...
/**
* Creates a gui representation for the groups and resources of the
* high level process.
* <br>
* Allows to view and edit the organizational characteristics
...
public class HLOrganizationalGui extends JPanel{
		...
		// GUI attributes
		private JButton addGroupButton = new SlickerButton("Add Group");       /** adds a new group */
		private JButton removeUnassignedButton = new SlickerButton("Remove Unused"); /** removes all unused groups */
		private JButton addResourceButton = new SlickerButton("Add Resource"); /** adds a new resource */
		private JButton removeSelectedButton = new SlickerButton("Remove Selected");   /** removes existing resource */
		 ...
		 * @param hlprocess the high level process of which the organizational
		 * characteristics are to be displayed
		 */
		public HLOrganizationalGui(HLProcess hlprocess){
			process = hlprocess;
			// create the double click table and the content view for activity overview
			createResourceView();
		...
		private void createResourceView() {
			...
			String description = new String("Here you can specify which resources and groups of resources are available in the process. "
					+ "Groups that have been created here can then be assigned to activities in the 'Activities' tab.");
			GuiPropertyStringTextarea helpText = new GuiPropertyStringTextarea(description, 2);
			...
			removeSelectedButton.setToolTipText("Removes currently selected groups and resources from the process");
			removeUnassignedButton.setToolTipText("Removes all unassigned groups and resources from the process");
			// add buttons
		...
		private void registerGuiActionListener() {
			...
			removeSelectedButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						// remove all selected resources and groups from simulation model
		...
		class GroupsPanel extends JPanel implements TableModelListener{
				...
				groupTable.addKeyListener(new KeyAdapter() {
					public void keyPressed(KeyEvent e) {
						...
						if(e.getKeyCode()==KeyEvent.VK_UP){
							row=(groupTable.getSelectedRow()-1)<=0?0:(groupTable.getSelectedRow()-1);
						}
						else if(e.getKeyCode()==KeyEvent.VK_DOWN){
							row=(groupTable.getSelectedRow()+1)>=groupTable.getRowCount()?groupTable.getRowCount()-1:groupTable.getSelectedRow()+1;
						}
						else return; 
						
						((ResourceTable)resourceTable.getModel()).updateTable((HLGroup) groupTable.getValueAt(row,0));
					}
				});
			...
			private void createGrpResView() {
				...
				JSplitPane splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, groupPanel,
									   resourcePanel);
					...
					if(e.getColumn()==0) {
						...
						if (grp_row >= 0) {
							if((Boolean)resourceTable.getValueAt(src_row,0)){
								HLGroup t = (HLGroup) groupTable.getValueAt(grp_row, 0);
								t.addResource((HLResource) resourceTable.getValueAt(src_row,1));
							}
							else {
								HLGroup t = (HLGroup) groupTable.getValueAt(grp_row, 0);
								t.removeResource(((HLResource) resourceTable.getValueAt(src_row,1)).getID());
							}
						}
						else
			...
			public List<HLResource> getSelectedResources() {
				...
				for (int i=0; i<indexArray.length; i++) {
					selected.add((HLResource) resourceTable.getValueAt(indexArray[i], 1));
				}
				return selected;
			}
			...
			private class ResourceTable extends AbstractTableModel {
				...
				public Class getColumnClass(int c) {
					if(c==0) return getValueAt(0, c).getClass();
					else return (new String()).getClass();
				}

				public void updateTable() {
					if (process.getResources().size()==0) return;
					for (int i = 0; i < process.getResources().size(); i++) {
						choose.set(i, false);
					}
					super.fireTableDataChanged();
				...
				public void updateTable(HLGroup hgGroup) {
					...
					for (int i = 0; i < process.getResources().size(); i++) {
						choose.set(i, Boolean.valueOf(hgGroup.isInGroup((HLResource)(process.getResources().get(i)))));
					}
					super.fireTableDataChanged();
				}
			}
