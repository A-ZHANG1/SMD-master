...
import org.prom5.framework.util.GuiNotificationTargetStateful;

/**
 * Creates a Gui representation of the given high level process.
 * High level information can be manipulated via Gui elements and the changes
 * are instantly reported back to the underlying data structure.
 * <p> Depending on whether was initialized by a plain high level process
 * or by a process model type implementation (e.g., Petri net or YAWL)
 * a graphical visualization of affected graph nodes can be provided.
 *
 * @see HLActivityGui
 * @see HLAttributeGui
 * @see HLChoiceGui
 ...
 * @see HLGlobalGui
 */
public class HLProcessGui implements GuiDisplayable, GuiNotificationTargetStateful {
    ...
    protected JButton addAttributeButton = new SlickerButton("Add Attribute"); /** adds a new data attribute */
	protected JButton removeAttributeButton = new SlickerButton("Remove Selected"); /** removes existing data attribute */
	public static Color bgColor = new Color(190, 190, 190);

	/**
	 * Constructor based on high level process implementation
	 * (such as HLPetriNet or HLYawl).
	 * <br>
	 * Visualization options depend on the concrete model type and
	 * its high level implementation.
	 * @param proc the high level process to be displayed
	 */
	...
	/**
	 * Constructor based on the plain high level information
	 * (not related to a concrete process model).
	 * @param proc the high level information to be displayed
	 */
	public HLProcessGui(HLProcess proc) {
        ...
        /**
         * Returns a list of gui-displayable Activities, i.e., each activity
         * in this list can be displayed in a list of a gui property.
         * Input and output data attributes are displayed separately.
         * Without specification of input and output data, all data attributes
         * are displayed as output data attributes.
	...
	/**
	 * Returns a list of gui-displayable Choices, i.e., each choice
	 * in this list can be displayed in a list of a gui property.
	...
	public JPanel getDataAttributesPanel() {
		...
		attTable = new GenericTableModelPanel(dataAttrHL, "Attributes",
				...
				"create decision rules based on these attributes in the 'Choices' tab. ", this, bgColor);
	...
	/**
	 * Creates a gui object for the global characteristics of
	...
	/**
	 * Creates a gui object for the organizational characteristics of
	 * this high level process.
	...
	/////////////////////////////////// Integrated view on all the high level data ///////////////////

	/**
	 * Creates a panel representing the overall high level process.
	 * <br>
	 * The different types of process characteristics will be nested in tabs.
	 * @return the GUI panel with the gui representation of the high level process
	 */
	public JPanel getPanel() {

		JPanel result = new JPanel(new BorderLayout());
		addAttributeButton.setToolTipText("Press button to add a new data attribute");
		removeAttributeButton.setToolTipText("Select the attribute to be removed from the combo box at the left and press button");
		...
		tabPane.addTab("Activities", new GenericTableModelPanel(getActivityGuiDisplayables(), "Activities",
				...
				"provided data attributes, and require a resource from a specific group for the execution of this activity.", this, bgColor));
		tabPane.addTab("Choices", new GenericTableModelPanel(getChoiceGuiDisplayables(), "Choices",
				...
				"decision rules (based on data attributes), probabilities, frequencies, or random selection.", this, bgColor));
		tabPane.addTab("Resources", getOrganizationalGui());

		if (hlVisualization != null) {
			// initialize visualization
			visualizationPanel.add(hlVisualization.getPanel());
			// set up split pane between visualization and actual settings tab pane
	...
	protected void registerGuiActionListener() {
		...
		addAttributeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				HLAttribute newAtt = new HLNominalAttribute("New Attribute", hlProcess);
				// redraw GUI
		...
		removeAttributeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				for (GuiDisplayable attGui : attTable.getSelected()) {
					HLAttribute att = ((HLAttributeGui) attGui).getHLAttribute();
	...
	/*
	 *  (non-Javadoc)
	 * @see org.processmining.framework.util.GuiNotificationTargetStateful#stateHasChanged(java.lang.Object)
	 */
	public void stateHasChanged(Object involvedObj) {
		...
		if (hlProcessImpl != null && hlVisualization != null) {
			...
			// in case of new activity has been double-clicked in 'Activities' view
			if (involvedObj instanceof HLActivityGui) {
				ModelGraphVertex trans = hlProcessImpl.findModelGraphVertexForActivity(((HLActivityGui)involvedObj).getHLActivity().getID());
				vertices.add(trans);
			...
			// in case a new choice being double-clicked
				...
				ModelGraphVertex place = hlProcessImpl.findModelGraphVertexForChoice(((HLChoiceGui) involvedObj).getHLChoice().getID());
				vertices.add(place);
				...
				while (actIt.hasNext()) {
					...
					ModelGraphVertex trans = hlProcessImpl.findModelGraphVertexForActivity(actID);
					vertices.add(trans);
				}
				hlVisualization.highLightNodesInVisualization(vertices);
			}
			// in case an attribute was double-clicked - the activities should be highlighted
			else if (involvedObj instanceof HLAttributeGui) {
				HLAttribute att = ((HLAttributeGui) involvedObj).getHLAttribute();
				Iterator<HLActivity> actIt = hlProcess.getActivitiesForAttribute(att.getID()).iterator();
				while (actIt.hasNext()) {
					HLActivity act = actIt.next();
					ModelGraphVertex trans = hlProcessImpl.findModelGraphVertexForActivity(act.getID());
					vertices.add(trans);
				}
				hlVisualization.highLightNodesInVisualization(vertices);
			}
