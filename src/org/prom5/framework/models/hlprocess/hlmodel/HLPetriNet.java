...
import org.prom5.framework.models.hlprocess.visualization.HLPetriNetVisualization;
...
/**
 * High-level process model implementation for Petri nets.
 * <br>
 * Maps Transitions onto HLActivities and Places with more than one 
 * outgoing arc to HLChoices. 
 */
public class HLPetriNet extends HLModel {

	/**
	 * Creates a high-level process that belongs to the given
	 * Petri net process. 
	 * @param net PetriNet the underlying petri net process
	 */
	...
	private void initChoices(PetriNet petriNet) {
		for (Place place : petriNet.getPlaces()) {
			// find choice constructs in this Petri net and add them to simulation model
			// in Petri net terms a choice is a place with more than one outgoing arcs
			...
			if (noBranches > 1) {
				...
				HLChoice hlChoice = new HLChoice(place.getIdentifier(), hlProcess);
				...
				for (Edge edge : place.getOutEdges()) {
					Transition destTransition = (Transition) ((PNEdge) edge).getDest();
					// find hl activity for this transition 
					for (Entry<ModelGraphVertex,HLID> entry : vertexToHLActivityMapping.entrySet()) {
						if (entry.getKey() == destTransition) {
							HLID actID = vertexToHLActivityMapping.get(entry.getKey());
	...
	/*
	 * (non-Javadoc)
	 * @see org.processmining.framework.models.hlprocess.hlmodel.HLModel#getVisualization()
	 */
	public HLPetriNetVisualization getVisualization(Set<Perspective> perspectivesToShow) {
