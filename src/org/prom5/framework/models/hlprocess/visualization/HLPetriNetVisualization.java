 ...
 *                                                         *
 **********************************************************/
package org.prom5.framework.models.hlprocess.visualization;
...
import org.prom5.framework.models.hlprocess.HLTypes.Perspective;
...
public class HLPetriNetVisualization extends PetriNet {
	...
	/** the link to the highlevelPN to be able to access the simulation information
	...
	/**
	 * Creates a diagnostic petri net visualization based on the given perspectives.
	 * @param hlPetriNet the high level petri net to be visualized
	 */
	public HLPetriNetVisualization(HLPetriNet hlPetriNet, Set<Perspective> thePerspectivesToShow) {
		super();
		...
		while (transitions.hasNext()) {
			...
			if (e1.object != null && e1.object instanceof Transition) {
				//Node n = (Node) e1;
				Transition newTransition = new Transition((Transition) e1.object);
		...
		while (places.hasNext()) {
			...
			if (e1.object != null && e1.object instanceof Place) {
				//Node n = (Node) e1;
				Place newPlace = new Place( ((Place) e1.object).getIdentifier(), this);
	...
	public void writeToDot(Writer bw) throws IOException {
		initDotWriting(bw);
		// in the case that separate boxes needs to be created and that connections need
		// to be made between these boxes and transitions, some additional code needs
		...
		if (perspectivesToShow.contains(HLTypes.Perspective.DATA_AT_TASKS)) {
			...
			Iterator<HLAttribute> dataAttrs = highLevelPN.getHLProcess().getAttributes().iterator();
			...
			while (dataAttrs.hasNext()) {
				...
				// add the data attribute to the mapping together with the corresponding box id
		...
		if (perspectivesToShow.contains(HLTypes.Perspective.ROLES_AT_TASKS)) {
			...
			Iterator<HLGroup> groups = highLevelPN.getHLProcess().getGroups().iterator();
			int counterGroups = 0;
			while (groups.hasNext()) {
				...
				// add the group to the mapping together with the corresponding box id
				mappingBoxesToBoxId.put(group, idBox);
			}
		}
		// present organizational information
		if (perspectivesToShow.contains(HLTypes.Perspective.ORGANIZATIONAL_MODEL)) {
			// visualize the resources in one box
			Iterator<HLResource> resources = highLevelPN.getHLProcess().getResources().iterator();
			...
			if (!resourcesLabel.equals("")) {
				bw.write("resources" + " [shape=\"ellipse\", label=\"" + resourcesLabel + "\"];\n");
			}
		}
		writeTransitionsToDot(bw);
		// in the case that the case generation scheme perspective is selected, also write the general distribution to
		// the graph
		if (perspectivesToShow.contains(HLTypes.Perspective.CASE_GEN_SCHEME)) {
			highLevelPN.getHLProcess().getGlobalInfo().getCaseGenerationScheme().writeDistributionToDot("cgs", "", "Case Generation Scheme:", bw);
		}
		writePlacesToDot(bw);
		writeEdgesToDot(bw);
	...
	/*
	 *  (non-Javadoc)
	 * @see org.processmining.framework.models.petrinet.PetriNet#writeTransitionsToDot(java.io.Writer)
	 */
	protected void writeTransitionsToDot(Writer bw) throws IOException {
		if (perspectivesToShow.size() == 0) {
			// TODO is there a reason for not calling the super method in this case? (Anne)
			// i.e.:
			// use default
			...
			*/
			...
			while (it.hasNext()) {
				...
				String label = t.getIdentifier();
				// when the frequency perspective is selected, the frequency also needs to be displayed in the transition
				// for each transitions involved in a choice show the relative frequencies
				// (a) regardless the actual choice configuration (CHOICE_FREQ mode)
				// (b) only if the choice has been configured to take place based on frequencies (CHOICE_CONF)
				if (perspectivesToShow.contains(HLTypes.Perspective.CHOICE_CONF) ||
						perspectivesToShow.contains(HLTypes.Perspective.CHOICE_FREQ)) {
					...
					Iterator<HLChoice> choices = highLevelPN.getHLProcess().getChoices().iterator();
					while (choices.hasNext() && found == false) {
						HLChoice choice = choices.next();
						// is this the choice related to this transition?
						Iterator<HLID> targetIDs = choice.getChoiceTargetIDs().iterator();
						while (targetIDs.hasNext()) {
							...
							if (act.getID().equals(actID)) {
								...
								// now either (a) always include if CHOICE_FREQ mode (regardless choice configuration)
								if (perspectivesToShow.contains(HLTypes.Perspective.CHOICE_FREQ)) {
									label = label + "\\nfreq=" + cond.getFrequency();
									found = true; // stop looking for further choices
								}
								// or (b) only include if choice is configured as based on frequencies (CHOICE_CONF)
								else if (perspectivesToShow.contains(HLTypes.Perspective.CHOICE_CONF)) {
									if (choice.getChoiceConfiguration().equals(ChoiceEnum.FREQ)) {
										label = label + "\\nfreq=" + cond.getFrequency();
										found = true; // stop looking for further choices
									}
								}
...
//						Iterator<HLActivity> freqDeps = choice.getFrequencyDependencies().iterator();
//						while (freqDeps.hasNext() && found == false) {
//							HLActivity freqTarget = freqDeps.next();
//							if (freqTarget == act) {
//								// OK - transition is involed in choice
//								// now either (a) always include if CHOICE_FREQ mode (regardless choice configuration)
//								if (perspectivesToShow.contains(HighLevelTypes.Perspective.CHOICE_FREQ)) {
//									label = label + "\\nfreq=" + act.getFrequencyDependency();
//									found = true; // stop looking for further choices
//								}
//								// or (b) only include if choice is configured as based on frequencies (CHOICE_CONF)
//								else if (perspectivesToShow.contains(HighLevelTypes.Perspective.CHOICE_CONF)) {
//									if (choice.getChoiceConfiguration().equals(ChoiceEnum.FREQ)) {
//										label = label + "\\nfreq=" + act.getFrequencyDependency();
//										found = true; // stop looking for further choices
//									}
...
//						}
			...
			// if the time perspective is selected also add a box and link it to the transition
			...
			while (transitionsIt.hasNext()) {
				...
				if (act != null) {
					if (perspectivesToShow.contains(HLTypes.Perspective.TIMING_EXECTIME)) {
						counterTiming++;
						act.getExecutionTime().writeDistributionToDot("i" + counterTiming,
								"t" + transition.getNumber(), "Execution time:", bw);
					}
					if (perspectivesToShow.contains(HLTypes.Perspective.TIMING_WAITTIME)) {
						counterTiming++;
						act.getWaitingTime().writeDistributionToDot("i" + counterTiming,
								"t" + transition.getNumber(), "Waiting time:", bw);
					}
					if (perspectivesToShow.contains(HLTypes.Perspective.TIMING_SOJTIME)) {
						counterTiming++;
						act.getSojournTime().writeDistributionToDot("i" + counterTiming,
								"t" + transition.getNumber(), "Sojourn time:", bw);
					}
					// if the data perspective has been selected, then the data attributes of the highlevelactivities need
					// to be connected to the box of the corresponding data attribute
					if (perspectivesToShow.contains(HLTypes.Perspective.DATA_AT_TASKS)) {
						Iterator<HLAttribute> outputDataAttrs = act.getOutputDataAttributes().iterator();
						while (outputDataAttrs.hasNext()) {
							...
							// if is null, then the data attribute attached to this transition could not be found back in the global mapping!
							if (boxID != null) {
								// write the edge
								bw.write("t" + transition.getNumber() + " -> " + boxID + " [dir=none, style=dotted];\n");
							}
						}
                                                Iterator<HLAttribute> inputDataAttrs = act.getInputDataAttributes().iterator();
					...
					// if the organizational perspective has been selected, then the groups of the highlevelactivities need
					...
					if (perspectivesToShow.contains(HLTypes.Perspective.ROLES_AT_TASKS)) {
						if (act.getGroup() != null) {
							...
							// write the edge
							bw.write("t" + transition.getNumber() + " -> " + boxID +
	...
	/*
	 *  (non-Javadoc)
	 * @see org.processmining.framework.models.petrinet.PetriNet#writePlacesToDot(java.io.Writer)
	 */
	...
	/*
	 *  (non-Javadoc)
	 * @see org.processmining.framework.models.petrinet.PetriNet#writeEdgesToDot(java.io.Writer)
	 */
				...
				while (it.hasNext()) {
					...
					if (e.isPT()) {
						...
						if (p.getOutEdges().size() > 1) {
							// for the places with more than one outgoing edge, display the data or probability dependency, if needed
							// check which perspectives are selected
							String labelText = "";
							if (perspectivesToShow.contains(HLTypes.Perspective.CHOICE_DATA)) {
								// get corresponding highleveldata dependency
								Iterator<HLChoice> choiceIt = highLevelPN.getHLProcess().getChoices().iterator();
								while (choiceIt.hasNext()) {
									HLChoice choice = choiceIt.next();
									// only if this is the right choice node ..
									if (p.equals(highLevelPN.findModelGraphVertexForChoice(choice.getID()))) {
										Iterator<HLID> targetIDs = choice.getChoiceTargetIDs().iterator();
										while (targetIDs.hasNext()) {
											HLID actID = targetIDs.next();
											// .. and if target node is right transition
											if (t.equals(highLevelPN.findModelGraphVertexForActivity(actID))) {
												labelText = "\"" + choice.getCondition(actID).getEscapedExpression() + "\"";
												break;
											}
										}
									}
								}
//								Iterator<HighLevelDataDependency> dataDeps = highLevelPN.getDataDependencies().iterator();
//								while (dataDeps.hasNext()) {
//									HLDataDependency dataDep = (HLDataDependency)dataDeps.next();
//									//if (dataDep.getSourceNode().equals(p) &&
//									//		((Transition) dataDep.getTargetNode()).getLogEvent().equals(t.getLogEvent())) {
//									if (dataDep.getSourceNode().equals(p) &&
//											((Transition) dataDep.getTargetNode()).equals(t)) {
//										labelText = "\"" + dataDep.getEscapedExpression() + "\"";
//										break;
//									}
//								}
							}
							else if (perspectivesToShow.contains(HLTypes.Perspective.CHOICE_PROB)) {
								// get corresponding highlevelprobability dependency
								Iterator<HLChoice> choiceIt = highLevelPN.getHLProcess().getChoices().iterator();
								while (choiceIt.hasNext()) {
									HLChoice choice = choiceIt.next();
									// only if this is the right choice node ..
									if (p.equals(highLevelPN.findModelGraphVertexForChoice(choice.getID()))) {
										...
										while (targetIDs.hasNext()) {
											...
											// .. and if target node is right transition
											if (t.equals(highLevelPN.findModelGraphVertexForActivity(actID))) {
												double prob =  choice.getCondition(actID).getProbability();
												...
												//replace commas with dots, since dot forces use of US-Locale
								...
								///////
//								Iterator<HighLevelProbabilityDependency> probDeps = highLevelPN.getProbabilityDependencies().iterator();
//								while (probDeps.hasNext()) {
//									HighLevelProbabilityDependency probDep = probDeps.next();
//									//if (probDep.getSourceNode().equals(p) &&
//									//		((Transition) probDep.getTargetNode()).getLogEvent().equals(t.getLogEvent())) {
//									if (probDep.getSourceNode().equals(p) &&
//											((Transition) probDep.getTargetNode()).equals(t)) {
...
//								}
							...
							// visualize the choices according to the choice configuration
							// (needed to visualize actual choices in CPN export)
							// note that only information for (a) data-based decision rules
							// and (b) probabilites is put at the arcs
							else if (perspectivesToShow.contains(HLTypes.Perspective.CHOICE_CONF)) {
								Iterator<HLChoice> choices = highLevelPN.getHLProcess().getChoices().iterator();
								while (choices.hasNext()) {
									HLChoice choice = choices.next();
									// is this the choice related to the source node of the current edge?
									if (highLevelPN.findModelGraphVertexForChoice(choice.getID()).equals(p)) {
										// is the choice based on data attributes?
										if (choice.getChoiceConfiguration().equals(ChoiceEnum.DATA)) {
											// find the right alternative branch first
											Iterator<HLID> targetIDs = choice.getChoiceTargetIDs().iterator();
											while (targetIDs.hasNext()) {
												HLID actID = targetIDs.next();
												// .. and if target node is right transition
												if (t.equals(highLevelPN.findModelGraphVertexForActivity(actID))) {
													// now actually write the data expression at the arc
													labelText = "\"" + choice.getCondition(actID).getEscapedExpression() + "\"";
													break;
												}
											}
											///
//											Iterator<HighLevelDataDependency> dataDeps = choice.getDataDependencies().iterator();
//											while (dataDeps.hasNext()) {
//												HLDataDependency dataDep = (HLDataDependency) dataDeps.next();
//												if (((Transition) dataDep.getTargetNode()).equals(t)) {
//													// now actually write the data expression at the arc
//													labelText = "\"" + dataDep.getEscapedExpression() + "\"";
//													break;
//												}
//											}
										...
										// or is the choice based on probabilities?
										else if (choice.getChoiceConfiguration().equals(ChoiceEnum.PROB)) {
											// find the right alternative branch first
											Iterator<HLID> targetIDs = choice.getChoiceTargetIDs().iterator();
											while (targetIDs.hasNext()) {
												HLID actID = targetIDs.next();
...
//													 now actually write the data expression at the arc
											...
											//
//											Iterator<HighLevelProbabilityDependency> probDeps = choice.getProbabilityDependencies().iterator();
//											while (probDeps.hasNext()) {
//												HighLevelProbabilityDependency probDep = probDeps.next();
//												if (((Transition) probDep.getTargetNode()).equals(t)) {
//													// now actually write the data expression at the arc
//													double prob =  probDep.getProbability();
//													// display probability in 2 decimal digits
//													DecimalFormat digits = new DecimalFormat();
...
//											}
										...
										// in case choice is configured as unguided or based on frequencies,
							...
							bw.write("p" + p.getNumber() + " -> t" + t.getNumber() + " [label=" +
									labelText + "];\n");
						} else {
							//write a normal edge
