...
public class MultiSetAbstraction extends ProcessInstanceBasedAbstraction {
    ...
    private HashMap<LogEvent,
            Integer> processSteps = new HashMap<LogEvent, Integer>();
    ...
    public double calculateFitness(LogAbstraction abstr) {
        ...
        for (LogEvent step : incompleteExecution.processSteps.keySet()) {
            if (processSteps.containsKey(step)) {
                if (processSteps.get(step).intValue() >=
                    incompleteExecution.processSteps.get(step).intValue()) {
    ...
    public SortedSet getMultiSetPossibleNextLogEvents(LogAbstraction
            partialTrace) {
