...
public abstract class ProcessInstanceBasedAbstraction extends LogAbstraction {
	...
	public ProcessInstanceBasedAbstraction(ProcessInstanceScale scale, LogReader log) {
		super(log);
		this.scale = scale;
	}
	 ...
	 * as a list of process instances for performance (set semantics, if required,
	 * are supposed to be retained by the specific implementation, i.e. making
	 * sure no two equal process instances are contained in this list).
	 */
	protected ArrayList<ProcessInstance> instances = new ArrayList<ProcessInstance>(1);

	/**
	 * @return the set of process instances clustered by
	 * this contributor instance.
	 */
	...
	/**
	 * Adds a process instance to the set represented
	 * by this contributor instance.
	...
	// next step.
	public final Recommendation getRecommendation(LogEvent logEventToDo, LogAbstraction partialTrace, SortedSet<LogEvent> enabledEvents) {
		...
		if (weight>0) {
	        ...
	        if (possibleEvents.contains(logEventToDo) && 
				...
				// It is not possible to do this event, so give a "negative" recommendation
				double w=0;
				double e=0;
				double e2=0;
