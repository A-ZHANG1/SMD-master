...
import org.prom5.framework.ui.ProgressFromAnyThread;
...
public class OntologyCollection {
	 ...
	 * If a ontology with the same URI exists before creating a new ontology,
	 * then the old ontology will be removed from the collection.
	 * @param namespace The namespace URI of the new ontology (example: http://www.processmining.org/ontologies/TaskOntology)
	 * @param name The name of the new ontology (example: TaskOntology)
	 * @return The new ontology (or null if it could not be created because of a reasoner problem).
	 */
	...
	public ConceptModel findConceptByUriInOntology(String uriInOntology) {
		...
		for (OntologyModel ontology : ontologies.values()) {
			if (OntologyModel.getOntologyPart(ontology.getName()).equals(ontologyName)) {
	...
	public Set<String> translateToReferencesInOntology(Collection<String> urisInLog) {
	...
	public Set<String> translateToReferenceInLog(String uriInOntology, boolean includeSuperConcepts, boolean includeSubConcepts) {
		Set<String> result = new HashSet<String>();
		...
		for (Map.Entry<String, OntologyModel> item : ontologies.entrySet()) {
			...
			if (ontologyName.equals(OntologyModel.getOntologyPart(ontology.getName()))) {
				if (includeSuperConcepts) {
					for (ConceptModel concept : ontology.getSuperConcepts(ontology.findConcept(conceptName))) {
						result.add(ontology.getConceptURIInLog(concept));
					}
				}
				if (includeSubConcepts) {
					for (ConceptModel concept : ontology.getSubConcepts(ontology.findConcept(conceptName))) {
						result.add(ontology.getConceptURIInLog(concept));
					}
				}
				result.add(uriInLog + OntologyModel.ONTOLOGY_SEPARATOR + conceptName);
				return result;
			}
		}
	...
	private OntologyToConceptToInstances getConceptsPerOntology() {
		...
		for (Map.Entry<String, Set<String>> item : summary.getConceptsAndInstances().entrySet()) {
	...
	private void loadOntologies() {
		if (!loadedOntologies) {
			...
			progress.setNote("Building a list of all concepts used in the log...");
			...
			if (!conceptsPerOntology.isEmpty()) {
				...
				for (Map.Entry<String, ConceptToInstances> item : conceptsPerOntology.entrySet()) {
					String uri = item.getKey();
					...
					try {
						ontologies.remove(uri);
						ontologies.put(uri, new OntologyModel(uri, conceptsToInstances));
					} catch (MalformedURLException e) {
						errors.add(uri + ": malformed url: " + e.getMessage());
					} catch (IOException e) {
						errors.add(uri + ": could not read ontology: " + e.getMessage());
					} catch (ParserException e) {
						errors.add(uri + ": could not parse ontology: " + e.getMessage());
					} catch (InvalidModelException e) {
						errors.add(uri + ": invalid model: " + e.getMessage());
					} catch (InconsistencyException e) {
						errors.add(uri + ": inconsistency error: " + e.getMessage());
					} catch (InternalReasonerException e) {
						errors.add(uri + ": internal ontology reasoner error: " + e.getMessage());
					} catch (org.prom5.framework.models.ontology.InvalidModelException e) {
						errors.add(uri + ": invalid model: " + e.getMessage());
					}
					progress.inc();
				...
				if (errors.size() > 0) {
					StringBuffer message = new StringBuffer("The following ontologies could not be loaded due to errors:\n\n");
					int i = 0;
					for (String s : errors) {
						message.append(++i + ") " + s + "\n\n");
					}
					JOptionPane.showMessageDialog(MainUI.getInstance(), message.toString());
				}
			}
			progress.close();
		}
