package org.prom5.framework.models.ontology;
...
public class OntologyModel {
	...
	public static final Pattern ONTOLOGY_SEPARATOR_PATTERN = Pattern.compile(ONTOLOGY_SEPARATOR); // for faster splitting of URIs
	 ...
	 * @param uriInLog URI of ontology as it is used in a log file.
	 * @param instancesPerConcept Mapping from concept URIs in the log to a set of instance labels. 
	 * @throws IOException
	 * @throws ParserException
	 * @throws InvalidModelException
	 * @throws InconsistencyException
	 * @throws InvalidModelException 
	 * @throws org.prom5.framework.models.ontology.InvalidModelException 
	 */

	public OntologyModel(String uriInLog, Map<String, Set<String>> instancesPerConcept) throws IOException, ParserException, InconsistencyException, InvalidModelException, org.prom5.framework.models.ontology.InvalidModelException {
		...
		final TopEntity[] identifiable = wsmlParser.parse(new InputStreamReader(new URL(uriInLog).openStream()));
		...
		if (identifiable.length > 0 && identifiable[0] instanceof Ontology) {
			...
			throw new ParserException("First element of file is not an ontology!", null);
		}
	}

	/**
	 * Constructor for a new (empty) ontology.
	 * @param uri The URI of the new ontology.
	 * @param name The name of the ontology
	 * @throws InconsistencyException
	 * @throws org.prom5.framework.models.ontology.InvalidModelException 
	 */
	public OntologyModel(String namespace, String name) throws InconsistencyException, org.prom5.framework.models.ontology.InvalidModelException {
		cachedSuperConcepts = new HashMap<ConceptModel, Collection<ConceptModel>>();
		cachedSubConcepts = new HashMap<ConceptModel, Collection<ConceptModel>>();
		
		this.factory = Factory.createWsmoFactory(null);
		this.ontology = factory.createOntology(factory.createIRI(namespace + ONTOLOGY_SEPARATOR + name));
		this.ontology.setDefaultNamespace(factory.createNamespace("_", factory.createIRI(namespace)));
		this.ontology.setWsmlVariant("http://www.wsmo.org/wsml/wsml-syntax/wsml-flight");
	 ...
	 * @param graph The model graph which contains the ontology (nodes are concepts, edges are is-a relations)
	 * @param uri The URI of the new ontology.
	 * @param name The name of the ontology
	 * @throws InconsistencyException
	 * @throws org.prom5.framework.models.ontology.InvalidModelException 
	 */
	public OntologyModel(ModelGraph graph, String namespace, String name) throws InconsistencyException, org.prom5.framework.models.ontology.InvalidModelException {
		cachedSuperConcepts = new HashMap<ConceptModel, Collection<ConceptModel>>();
		cachedSubConcepts = new HashMap<ConceptModel, Collection<ConceptModel>>();
		...
		for (ModelGraphVertex vertex : graph.getVerticeList()) {
			...
			vertexMapping.put(vertex, addConcept(label == null ? vertex.getName() : label));
		}
		for (Object o : graph.getEdges()) {
			ModelGraphEdge edge = (ModelGraphEdge) o;
			vertexMapping.get(edge.getDest()).addSuperConcept(vertexMapping.get(edge.getSource()));
		}
	...
	private void init(Map<String, Set<String>> instancesPerConcept) throws InconsistencyException, org.prom5.framework.models.ontology.InvalidModelException {
		...
		reasonerParams.put(WSMLReasonerFactory.PARAM_BUILT_IN_REASONER, WSMLReasonerFactory.BuiltInReasoner.KAON2);
		recreateReasoner();
		
		directlyReferencedConcepts = new TreeSet<ConceptModel>();
		allConceptsByShortName = new HashMap<String, ConceptModel>();
		for (Concept concept : reasoner.getAllConcepts((IRI) ontology.getIdentifier())) {
			String conceptShortName = getConceptPart(concept.getIdentifier().toString());
			ConceptModel conceptModel = new ConceptModel(this, concept, instancesPerConcept.get(conceptShortName));
		...
		if (!conceptsInLog.isEmpty()) {
			Message.add("Log contains model references to concepts which are not in the ontology:");
			Message.add("  Concepts not in ontology: " + conceptsInLog.toString());
	...
	/**
	 * Returns true if the ontology model was changed (ie. concepts or attributes have been added/removed etc). 
	 */
	...
	/**
	 * Returns a list of all concepts which are directly referenced in the log. Directly referenced means
	 * that there is at least one element in this log which is annotated with such a concept.
	...
	/**
	 * Returns a list of all concepts which are directly referenced in the log, or which are superconcepts of
	 * any concept which is directly referenced in the log. Note that this is not necessarily the list of all
	 * concepts in the ontology.
	 * @return All directly referenced concepts in the log plus all their superconcepts.
	 */
	public Collection<ConceptModel> getAllConcepts() {
		return Collections.unmodifiableCollection(allReferencedConceptsByName.values());
	}
	 ...
	 * Note that this can be different from the name of the ontology returned by getName().
	 * @return the full ontology URI.
	 */
	...
	/**
	 * Returns a collection of all superconcepts of a given concept. The reasoner is used to determine the superconcepts.
	 * This includes all superconcepts (not only the direct superconcepts),
	 * though it only returns strict superconcepts (so the returned set will not include the given concept).
	 * The reasoner is used to determine the subconcepts.
	 */
	public Collection<ConceptModel> getSuperConcepts(ConceptModel concept) {
		...
		if (result == null) {
			result = translateConceptsToConceptModels(
					reasoner.getSuperConcepts((IRI) ontology.getIdentifier(), concept.getConcept()),
					concept, false);
	 ...
	 * This includes all subconcepts (not only the direct subconcepts),
	 * though it only returns strict sub concepts (so the returned set will not include the given concept).
	 * The reasoner is used to determine the subconcepts.
	 */
	public Collection<ConceptModel> getSubConcepts(ConceptModel concept) {
		...
		if (result == null) {
			result = translateConceptsToConceptModels(
				reasoner.getSubConcepts((IRI) ontology.getIdentifier(), concept.getConcept()),
				concept, false);
	...
	public Collection<ConceptModel> getDirectSubConcepts(ConceptModel concept) {
		assert concept.getConcept().getOntology() == ontology;
		return translateConceptsToConceptModels(
				concept.getConcept().listSubConcepts(), 
				null, true);
	}
	...
	public Collection<ConceptModel> getDirectSuperConcepts(ConceptModel concept) {
		assert concept.getConcept().getOntology() == ontology;
		return translateConceptsToConceptModels(
				concept.getConcept().listSuperConcepts(),
				null, true);
	}
	 ...
	 * @return
	 */
	public Collection<ConceptModel> getAllDirectSuperConcepts(ConceptModel concept) {
		assert concept.getConcept().getOntology() == ontology;
		return translateConceptsToConceptModels(
				concept.getConcept().listSuperConcepts(),
				null, false);
	...
	private Collection<ConceptModel> translateConceptsToConceptModels(Set concepts, ConceptModel conceptToExclude, boolean haveToBeReferenced) {
		...
		for (Object concept : concepts) {
			...
			if (conceptToExclude == null || !conceptName.equals(conceptToExclude.getName())) {
				ConceptModel conceptModel = haveToBeReferenced ? 
						allReferencedConceptsByName.get(conceptName) 
						: allConceptsByShortName.get(getConceptPart(conceptName));
	 ...
	 * Note that this concept is not referenced in any log, so it does not show up in any visualization yet!
	 * @param conceptShortName The short name of the concept to create
	 * @return The created concept
	 * @throws org.prom5.framework.models.ontology.InvalidModelException 
	 */
	public ConceptModel addConcept(String conceptShortName) throws org.prom5.framework.models.ontology.InvalidModelException {
		//System.out.println("ADDING: " + conceptShortName);
		if (findConcept(conceptShortName) != null) {
			throw new org.prom5.framework.models.ontology.InvalidModelException("Concept already exists in this ontology: " + conceptShortName);
		}
		
		String uri = getOntologyPart(getName()) + ONTOLOGY_SEPARATOR + conceptShortName;
		Concept concept = factory.createConcept(factory.createIRI(uri));
		try {
			concept.setOntology(ontology);
			ontology.addConcept(concept);
		} catch (SynchronisationException e) {
			throw new org.prom5.framework.models.ontology.InvalidModelException(e.getMessage());
	...
	void structureChanged() throws org.prom5.framework.models.ontology.InvalidModelException {
		...
		for (ConceptModel concept : directlyReferencedConcepts) {
			...
			for (ConceptModel superConcept : getSuperConcepts(concept)) {
				allReferencedConceptsByName.put(superConcept.getName(), superConcept);
			}
		}
		recreateReasoner();
	}

	void setDirectlyReferenced(ConceptModel concept) throws org.prom5.framework.models.ontology.InvalidModelException {
	...
	private void recreateReasoner() throws org.prom5.framework.models.ontology.InvalidModelException {
		reasoner = DefaultWSMLReasonerFactory.getFactory().createWSMLFlightReasoner(reasonerParams);
		try {
...
class OntologyModelGraph extends ModelGraph {
	...
	public OntologyModelGraph(OntologyModel ontology, boolean showInstances) {
		...
		for (ConceptModel concept : ontology.getAllConcepts()) {
			...
			for (ConceptModel superConcept : ontology.getDirectSuperConcepts(concept)) {
	...
	public void writeToDot(Writer bw, boolean withInstances) throws IOException {
		...
		bw.write("  node [style=\"filled\",color=\"antiquewhite\",shape=\"ellipse\",fontname=\"Arial\",fontsize=\"11\"];\n");

		bw.write("\"ontology URI\" [shape=\"plaintext\",style=\"filled\",color=\"antiquewhite3\",label=\"" + OntologyModel.getOntologyPart(getName()).replace('"', '\'') + OntologyModel.ONTOLOGY_SEPARATOR + "\"];\n");
		bw.write("\"ontology name\" [shape=\"box\",style=\"filled\",color=\"antiquewhite3\",label=\"" + ontology.getShortName().replace('"', '\'') + "\"];\n");
		bw.write("\"ontology URI\" -> \"ontology name\" [style=\"filled\",color=\"antiquewhite3\",arrowtail=\"none\",arrowhead=\"none\"];\n");

		// write nodes
		for (ConceptModel concept : concepts) {
			bw.write("E" + ids.get(concept) + " [label=\"" + concept.getShortName().replace('"', '\'') + "\"];\n");
		}
		if (withInstances) {
			int instanceId = 0;
			for (ConceptModel concept : concepts) {
				for (String instance : concept.getInstances()) {
					bw.write("I" + instanceId + " [shape=\"plaintext\",label=\"" + instance.replace('"', '\'') + "\"];\n");
					instanceId++;
				}
			}
		}
		...
		// write edges
		for (ConceptModel concept : concepts) {
			Collection<ConceptModel> superConcepts = ontology.getDirectSuperConcepts(concept);

			if (superConcepts.isEmpty()) {
				bw.write("\"ontology name\" -> E" + ids.get(concept) +  " [style=\"filled\",arrowtail=\"normal\",arrowhead=\"none\"];\n");
			} else {
				for (ConceptModel superConcept : superConcepts) {
					bw.write("E" + ids.get(superConcept) +
							" -> E" + ids.get(concept) +
		...
		if (withInstances) {
			...
			for (ConceptModel concept : concepts) {
				for (String instance : concept.getInstances()) {
					bw.write("E" + ids.get(concept) + " -> I" + instanceId + " [color=\"gray\",arrowtail=\"normal\",arrowhead=\"none\"];\n");
					instanceId++;
				}
			}
		}
