...
package org.prom5.framework.models.orgmodel;
...
/**
 * <p>Title: Organizational Model</p>
 * <p>Description: Holds an organizational model</p>
...
public class OrgModel implements Cloneable {
	...
	public OrgModel(HashMap<String,OrgEntity> orgEntities, HashMap<String,Resource> resources, HashMap<String,Task> tasks) {
	...
	public boolean changeOrgEntityID(String old_id, String new_id) {
		if (old_id.equals(new_id))return true;
	...
	public void removeOrgEntity(OrgEntity orgEntity) {
		...
		String[] key = (String[]) resources.keySet().toArray(new String[resources.keySet().size()]);
		for (int i = 0; i < key.length; i++) {
			Resource tempResource = (Resource) resources.get(key[i]);
			tempResource.removeOrgEntity(orgEntity);
		}

		key = (String[]) tasks.keySet().toArray(new String[tasks.keySet().size()]);
		for (int i = 0; i < key.length; i++) {
	...
	public Task getTask(LogEvent le) {
		String[] key = (String[]) tasks.keySet().toArray(new String[tasks.keySet().size()]);
		for (int i = 0; i < key.length; i++) {
			Task tempTask = (Task) tasks.get(key[i]);
	...
	public Task getTask(String name, String eventType) {
		...
		for (int i = 0; i < key.length; i++) {
			Task tempTask = (Task) tasks.get(key[i]);
			if (tempTask.getName().equals(name) && (tempTask.getEventType().equals(eventType)))
				return tempTask;
		}
		return null;
	}
	...
	public ArrayList<String> getOrgEntityStringList() {
		...
		String[] key = (String[]) orgEntities.keySet().toArray(new String[orgEntities.keySet().size()]);
	...
	public List<String> getOrgEntityList(Resource res) {
		...
		while (iterator.hasNext()) {
			...
			if (res.hasOrgEntity(orgunit)) list.add(key);
		}
	...
	public List<String> getOrgEntityList(Resource res, String type) {
		...
		while (iterator.hasNext()) {
				...
				if (res!=null && res.hasOrgEntity(orgentity) && orgentity != null &&
						orgentity.getEntityType() != null && orgentity.getEntityType().equals(type))
						list.add(key);
		}
	...
	public ArrayList<String> getResourceList(OrgEntity orgEntity) {
		...
		if(orgEntity.getSubEntityIDs()==null) return list;
	...
	public HashSet<String> getSubResourceList(HashSet<String> list, OrgEntity orgEntity) {
	 ...
	 * When this function is called, original model and parameter has the
	 * same task sets.
	 *
	 * @param listGroups ArrayList
	 *
	 */
	public void reallocateOrgModel(ArrayList<ArrayList<String>> listGroups, OrgModel originalModel){

		HashMap<String,Task> originalTasks = originalModel.getTasks();
		HashMap<String,Resource> originalResources = originalModel.getResources();

		// remove existing orgEntities from resources
		String[] key_resource = (String[]) resources.keySet().toArray(new String[resources.keySet().size()]);
		for (int i = 0; i < key_resource.length; i++) {
			Resource tempRes = (Resource) resources.get(key_resource[i]);
			String[] key_orgEntity = (String[]) orgEntities.keySet().toArray(new String[orgEntities.keySet().size()]);
			for (int j = 0; j < key_orgEntity.length; j++) {
				OrgEntity tempEntity = (OrgEntity) orgEntities.get(key_orgEntity[j]);
		...
		String[] key_task = (String[]) tasks.keySet().toArray(new String[tasks.keySet().size()]);
		for (int i = 0; i < key_task.length; i++) {
			Task tempTask = (Task) tasks.get(key_task[i]);
			String[] key_orgEntity = (String[]) orgEntities.keySet().toArray(new String[orgEntities.
									 keySet().size()]);
			for (int j = 0; j < key_orgEntity.length; j++) {
				OrgEntity tempEntity = (OrgEntity) orgEntities.get(key_orgEntity[j]);
		...
		for(int i=0;i<listGroups.size();i++)
		{
			...
			OrgEntity tempEntity = new OrgEntity("minedGroup"+i,"minedGroup"+i ,OrgEntity.ORGENTITYTYPE_MININGRESULT);
		...
		String[] originalTaskKey = (String[]) originalTasks.keySet().toArray(new String[originalTasks.keySet().size()]);
		for (int i = 0; i < originalTaskKey.length; i++) {
			...
			HashSet<OrgEntity> originalOrgEntities = originalTask.getOrgEntities();
			String[] originalResourceKey = (String[]) originalResources.keySet().toArray(new String[originalResources.keySet().size()]);

			ArrayList<String> originalResourceList = new ArrayList<String>(); // list for resource IDs for task
			for (int j = 0; j < originalResourceKey.length; j++) {
				Resource originalRes = originalModel.getResource(originalResourceKey[j]);

				Iterator<OrgEntity> it = originalOrgEntities.iterator(); // Actually an original task has an org entity
				while (it.hasNext()) {
			...
			for (int j=0;j<originalResourceList.size();j++){
				String[] orgEntitiesKey = (String[]) orgEntities.keySet().toArray(new String[
								orgEntities.keySet().size()]);
				for (int k = 0; k < orgEntitiesKey.length; k++) {
					OrgEntity existingOrgEntity = (OrgEntity) orgEntities.get(orgEntitiesKey[k]);
	...
	public void writeToXML(Writer bw) throws IOException {
		...
		key = (String[]) resources.keySet().toArray(new String[resources.keySet().size()]);
		for (int i = 0; i < key.length; i++) {
			((Resource) resources.get(key[i])).writeToXML(bw);
		}

		//write hierarchy relationship using implication
		key = (String[]) orgEntities.keySet().toArray(new String[orgEntities.keySet().size()]);
		for (int i = 0; i < key.length; i++) {
	...
	public void writeToDot(Writer bw) throws IOException {
		...
		key = (String[]) orgEntities.keySet().toArray(new String[orgEntities.keySet().
			  size()]);
		for (String aKey:key) {
			nodelist.put((OrgEntity) orgEntities.get(aKey), String.valueOf("node" + nodenum));
			bw.write("node" +
					nodenum++ +
		...
		key = (String[]) resources.keySet().toArray(new String[resources.keySet().size()]);
		for(String aKey: key){
			nodelist.put((Resource) resources.get(aKey), String.valueOf("node" + nodenum));
			bw.write("node" +
					nodenum++ +
		...
		key = (String[]) tasks.keySet().toArray(new String[tasks.keySet().size()]);
		for (int i = 0; i < key.length; i++) {
			nodelist.put((Task) tasks.get(key[i]), String.valueOf("node" + nodenum));
			bw.write("node" +
		...
		for (int i = 0; i < key.length; i++) {
			...
			String[] key2 = (String[]) orgEntities.keySet().toArray(new String[orgEntities.keySet().
							size()]);
			for (int j = 0; j < key2.length; j++) {
				OrgEntity orgEntity = (OrgEntity) orgEntities.get(key2[j]);

				if (res.hasOrgEntity(orgEntity))
					bw.write((String) nodelist.get(res) + " -> " + (String) nodelist.get(orgEntity) +
							" [label=\" \"];");
			}
		}
		...
		for (int i = 0; i < key.length; i++) {
			...
			for (int j = 0; j < key2.length; j++) {
				...
				if (task.hasOrgEntity(orgEntity))
					bw.write((String) nodelist.get(orgEntity) + " -> " + (String) nodelist.get(task) +
							" [label=\" \"];");
			}
		}
		bw.write("}\n");
	 ...
	 * @throws IOException in case there is a problem with writing to <code>bw</code>
	 */
	public void writeToDot(Writer bw, boolean bOrgEntity, boolean bResource, boolean bTask) throws IOException {
		...
		if(bTask){
			key = (String[]) tasks.keySet().toArray(new String[tasks.keySet().size()]);
			for (String aKey:key) {
				nodelist.put((Task) tasks.get(aKey), String.valueOf("node" + nodenum));
				bw.write("node" +
						nodenum++ +" [shape=\"box\",style=\"filled\",fillcolor=\"wheat\",label=\"" +
		...
		if (bResource&&bOrgEntity) {
			...
			for (String aKey:key) {
				...
				String[] key2 = (String[]) orgEntities.keySet().toArray(new String[orgEntities.
								keySet().
								size()]);
		...
		if (bTask&&bOrgEntity) {
			...
			for (String aKey:key) {
				...
				for (String bKey:key2) {
					...
					if (task.hasOrgEntity(orgEntity))
						bw.write((String) nodelist.get(task) + " -> " +
								(String) nodelist.get(orgEntity) +
	...
	public JPanel getGraphPanel(boolean bOrgEntity, boolean bResource, boolean bTask) {
		BufferedWriter bw;
		Graph graph;
		NumberFormat nf = NumberFormat.getInstance();
		File dotFile;
	...
	public void writeToTestLog() {
		...
		Message.add("<ORGMODELSummary numberOfOriginators=\"" + resources.size() + "\">" , Message.TEST);
		Message.add("<ORGMODELSummary numberOfOrgEntities=\"" + orgEntities.size() + "\">" , Message.TEST);
		Message.add("<ORGMODELSummary numberOfTasks=\"" + tasks.size() + "\">" , Message.TEST);
		...
		Message.add("<ORGMODELSummary numberOfLinkBetweenOriginatorAndOrgEntity=\"" + sum + "\">" , Message.TEST);
		...
		Message.add("<ORGMODELSummary numberOfUnconnectedTasks=\"" + sum + "\">" , Message.TEST);
	 ...
	 * structure with cloned resources, orgentities, and tasks. Note that
	...
	public Object clone() {
		...
		for (int i = 0; i < key.length; i++) {
			...
			String[] key2 = (String[]) orgEntities.keySet().toArray(new String[orgEntities.keySet().size()]);
			for (int j = 0; j < key2.length; j++) {
				OrgEntity orgEntity = (OrgEntity) orgEntities.get(key2[j]);
				if (res.hasOrgEntity(orgEntity)) {
					OrgEntity clonedOrgEntity = (OrgEntity) o.orgEntities.get(key2[j]);
