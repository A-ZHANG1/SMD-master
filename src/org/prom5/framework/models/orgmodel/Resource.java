	...
	public void removeOrgEntity(OrgEntity orgEntity){
		if (orgEntity == null) {
			Message.add((new StringBuilder(
					"Tried to remove a null-value to the Org Entity of Resource")).append(toString()).toString(), 3);
			return;
		}

		if (orgEntities.contains(orgEntity)) orgEntities.remove(orgEntity);
	}

	public void addOrgEntity(OrgEntity orgEntity) {
		if (orgEntity == null) {
			Message.add("Tried to add a null-value to the OrgEntity-list of Resource", 4);
//			Message.add((new StringBuilder(
//					"Tried to add a null-value to the OrgEntity-list of Resource")).append(toString()).toString(), 3);
			return;
		}
		if (!orgEntities.contains(orgEntity)) orgEntities.add(orgEntity);
	}
	...
	public boolean hasOrgEntity(OrgEntity orgEntity) {
			...
			if (orgEntity != null) {
				if (orgEntities.contains(orgEntity)) bResult = true;
			}
	...
	public String toString() {
		String s = (new StringBuilder(String.valueOf(resourceID))).append(", ").append(name).append(
				   "\n").toString();
		for (Iterator<OrgEntity> iterator = orgEntities.iterator(); iterator.hasNext(); ) {
			OrgEntity orgEntity = iterator.next();
			s = (new StringBuilder(String.valueOf(s))).append("  ").append(orgEntity.toString()).toString();
		}
	...
	/**
	 * Makes a deep copy of the object, i.e., reconstructs the Resource
	 * Note that
