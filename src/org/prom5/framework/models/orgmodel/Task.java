...
public class Task implements LogEventProvider, Cloneable {
	...
	public void removeOrgEntity(OrgEntity orgEntity){
		if (orgEntity == null) {
			Message.add((new StringBuilder(
					"Tried to remove a null-value to the Org Entity of Task")).append(toString()).toString(), 3);
			return;
		}
	...
	public void addOrgEntity(OrgEntity orgEntity) {
		if (orgEntity == null) {
			Message.add((new StringBuilder(
					"Tried to add a null-value to the OrgEntity-list of Task")).append(toString()).toString(), 3);
			return;
		}
	...
	/**
	 * Makes a deep copy of the object, i.e., reconstructs the Task
	 * Note that
