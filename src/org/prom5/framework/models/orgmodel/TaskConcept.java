...
public class TaskConcept extends Task{
	...
	public void removeTask(String res){
		...
		if (tasks.contains(res)) tasks.remove(res);
	}

	public void addTask(String res) {
		if (tasks == null) {
			Message.add("Tried to add a null-value to the OrgEntity-list of Task", 4);
//			Message.add((new StringBuilder(
//					"Tried to add a null-value to the OrgEntity-list of Task")).append(toString()).toString(), 3);
			return;
		}
		if (!tasks.contains(res)) tasks.add(res);
	}
	...
	public boolean hasTask(String res) {
			...
			if (res != null) {
				if (tasks.contains(res)) bResult = true;
			}
