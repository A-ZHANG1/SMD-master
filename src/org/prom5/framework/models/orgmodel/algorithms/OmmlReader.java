...
/**
 * Reads a Organizational Model from a OMML (Organizational Model Markup Language) file.
 *
 * @author Minseok Song
 * @version 1.0
 */
...
public class OmmlReader {
	...
	private static void addOrgEntity(Element element) throws IOException {
		...
		if (!orgentities.containsKey(id)) orgentities.put(id, orgentity);
	}
	...
	private static void addResource(Element element) throws IOException {
		...
		for (Iterator iterator = qualitites.iterator(); iterator.hasNext(); ) {
			...
			if (orgentities.containsKey(qualityID)) {
				...
				Message.add("OrgEntity (ID=\""+qualityID+"\") does not exists. The OrgEntity is ignored", 3);
			}
		}
		...
		if (!resources.containsKey(id))
			...
			Message.add((new StringBuilder(" Duplicate entry for Resource <")).append(id).append(
					", ").append(name).append("> Entry").append("ignored.").toString(), 3);
	}
	...
	private static void addTaskEntity(Element element) throws IOException {
		...
		if (!tasks.containsKey(id))
			...
			Message.add((new StringBuilder(" Duplicate entry for Task <")).append(id).append(", ").
					append(name).append("> Entry").append("ignored.").toString(), 3);
	}
}
