package org.prom5.framework.models.orgmodel.algorithms;
...
public class OrgModelUtil {
	...
	public static void aggregateTasks(OrgModel orgModel)
	{
		...
		String[] key = (String[]) oldTasks.keySet().toArray(new String[oldTasks.keySet().size()]);
	...
	public static void removeRedundantOrgEntity(OrgModel orgModel)
	{
		...
		for (int i = 0; i < key.length-1; i++) {
			...
			if(firstOrgEntity == null) continue;
			ArrayList<String> firstList = orgModel.getResourceList(firstOrgEntity);

			for (int j = i+1; j < key.length; j++) {
				OrgEntity secondOrgEntity = (OrgEntity) orgEntities.get(key[j]);
				if(secondOrgEntity == null) continue;
				ArrayList<String> secondList = orgModel.getResourceList(secondOrgEntity);

				if(firstList.size()!=secondList.size()) continue;
				...
				if (bFlag) {
					String[] key2 = (String[]) tasks.keySet().toArray(new String[tasks.keySet().size()]);
					...
					key2 = (String[]) resources.keySet().toArray(new String[resources.keySet().size()]);
					for (int k = 0; k < key2.length; k++) {
