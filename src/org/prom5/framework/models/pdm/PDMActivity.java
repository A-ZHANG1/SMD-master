...
/**
 * <p>Title: PDMActivity *
 * <p>Description: Represents an activity from a PDM Design</p> *
...
public class PDMActivity {
	...
	private Integer activityNR;                // the number of the activity, needed for Declare
	 ...
	 * contains the data element 'el' as input or as output element. Returns false otherwise.
	...
	public Boolean hasDataElement(PDMDataElement el) {
		...
		// the 'ops' array now contains al operations from the hashmap 'operations'
		// this is necessary because there is no construct to walk through the hashmap step-by-step
	 ...
	 * overlapping data element. This is determined by walking through all
	 * data elements of this activity and checking whether activity act also
	 * has the data element (act.hasDataElement).
	 *
	 * @param act PDMActivity
	 * @return Boolean
	 */
			...
			relcoh = 0.0;
					...
					for (int i = 0; i < ops.length; i++) {
						for (int j = 0; j < ops.length; j++) {
							...
							if ((!op1.equals(op2)) & (!op1.getOutputElements().equals(op2.getOutputElements()))) {
					...
					relcoh = (intersecs.doubleValue()) / ((s.doubleValue()) * (s.doubleValue() - 1));
				}
		return relcoh;
   ...
   public Double calculateActivityInformationCohesion() {
	...
	/**
	 * Returns a Hash Set containing all the data elements of the activity
	 * (i.e. for every operation it returns all the input and output elements).
	 *
	 * @return HashSet
	 */
	public HashSet getDataElements() {
		...
		for (int i=0; i<ops.length ; i++) {
			...
			// First, add the input data elements of operation i to the resulting HashSet result.
			...
			// Secondly, add the output data elements of operation i to the resulting HashSet result.
