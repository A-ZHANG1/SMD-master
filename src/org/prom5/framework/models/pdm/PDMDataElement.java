...
/**
 * <p>Title: PDM Data Element</p>
 * <p>Description: An object that represents a Data Element in a Product Data Model</p>
...
public class PDMDataElement extends ModelGraphVertex{
	...
	private Integer dataElementNR;     // the number of the data element, needed for Declare
	 ...
	 * each data element instead of the string identifier. This numerical reference
	...
	/**
	 * Returns the numerical identifier of a data element. This numerical reference
	 ...
	 * model contains all operations as activities with input data elements (type 0)
	 * and output data elements (type 1).
	 *
	 * @param bw Writer
	 * @param str String
	 * @throws IOException
	...
	public void writePDMToDeclare(Writer bw) throws IOException {
		bw.write("<dataelement id=\""+dataElementNR+"\" initial=\"\" name=\""+dataElementID+"\" type=\"string\"/>\n");
	}
	...
	public void writeToDot(Writer bw, PDMModel model) throws IOException {
		if (!(description == null)){
			bw.write(getID() + " [shape=circle, height=\".3\", label=\"" + dataElementID + "_" +
					description + "\"];\n");
		}
		else bw.write(getID() + " [shape=circle, height=\".3\", label=\"" + dataElementID + "\"];\n");
