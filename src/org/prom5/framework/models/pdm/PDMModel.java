...
package org.prom5.framework.models.pdm;
...
/**
 * <p>Title: PDMModel</p>
 * <p>Description: Represents a Product Data Model (PDM)</p>
...
public class PDMModel extends ModelGraph {
	...
	HashMap dataElements = new HashMap();    // the list of dataElements in the model
	 ...
	 * @param name The name of the PDM model
	 */
	 ...
	 *
	 * @param dataElement PDMDataElement
	 */
	...
	/**
	 * Adds a Resource to the set of Resources ot the Product Data Model
	 * NB: Later on this should be moved to the section of the Organizational Model
	 *
...
*/
	...
	public HashSet calculateExecutableOperations(HashSet dataElts, HashSet executed, HashSet failed, boolean root){
		HashSet result = new HashSet();
...
//					System.out.println("Enabled operation: "+ op.getID());
	...
	/**
	 * Calculates which data elements are the input elements of the PDM. In order
	 * to do so it checks for each operation if the input elements are also output
	 * elements to another operation. If so, the input element of the operation
	 * is not een input element of the PDM model. If not, then the input element
	 * of the operation is an input element to the PDM model.
	 *
	 * @return HashSet
	 */
	/*	public HashSet getPDMInputElements() {
		HashSet result = new HashSet();
		HashMap ops = getOperations();
		Object[] opsAr = ops.values().toArray();
		// walk through all operations of the PDM
		for (int i=0; i<opsAr.length; i++){
			PDMOperation op = (PDMOperation) opsAr[i];
			HashMap ins = op.getInputElements();
			Object[] insAr = ins.values().toArray();
			// check all input data elements of operation 'op'
			for (int j=0; j<insAr.length; j++){
				PDMDataElement data = (PDMDataElement) insAr[j];
				Boolean isInputElt = true;
				// check for on of the input data elements of 'op' whether there
				// is an operation that produces this data element
				for (int k=0; k<opsAr.length; k++){
					PDMOperation op2 = (PDMOperation) opsAr[k];
					if (op2.hasOutputDataElement(data)){
						isInputElt = false;
					}
				}
				if (isInputElt){
					result.add(data);
//					System.out.println("PDM input element: "+ data.getID());
				}
			}
		}
		return result;
...
*/
	...
	public PDMStateSpace calculateSimpleStateSpace(boolean root, boolean failure, boolean input, boolean colored, int numStates, int breadth){
		...
		if (!input) {
			...
			HashSet ins = new HashSet(); // this hashSet contains the input elements to the process (input elements of PDM)
			...
			PDMState start = new PDMState(result, name, ins, execOps, empty); // start state of the statespace
	...
	public HashSet calculateNextStates(PDMState state, PDMStateSpace statespace, boolean root, boolean failure, int numStates, int breadth) {
		HashSet result = new HashSet();
		...
		while (it.hasNext() && i<numStates && b<bLimit) {
			if (!failure){
					...
					// First, add the state with the operation 'op' succesfully executed
					HashSet ins2 = (HashSet) data.clone(); // NB: is it necessary to clear this clone again?
					ins2.add(d);
					...
					// If not, a new state is created and linked to the current state
					else {
						String name = "state" + i;
//					int num = checkStatusOfState(statespace, )
			...
			// Then, if failure of operations is considered, add the state with the failed operations 'op'.
			if (failure) {
					PDMOperation op = (PDMOperation) it.next();
					PDMDataElement d = op.getOutputElement();
					// First, add the state with the operation 'op' succesfully executed
	...
	public PDMState checkIfStateExists(PDMStateSpace statespace, HashSet data, HashSet exec, HashSet failed){
	...
	public void writePDMToDeclare(Writer bw) throws IOException {
	    ...
	    // write the activity definitions, i.e. each operation in the PDM is an activity definition in Declare
		bw.write("<activitydefinitions>\n");
		// start with an initial activity that puts the values for the leaf elements of the PDM
	    ...
	    // first remove input operations from the set of operations and then write all real operations
	    ...
	    // write the constraint definition, for now we do not have any constraints in the PDM that are translated to Declare
...
// TODO: improve graphical positioning of activities. Now they are presented in one long line.
		// write the graphical positioning information of the Declare model, first the initial operation, then the real operations and then the input operations.
		bw.write("<graphical>\n");
		bw.write("<cells>\n");
		...
		while (it6.hasNext()) {
			...
			bw.write("<cell activitydefinition=\""+ operation.getOperationNR() + "\" height=\"40.0\" width=\"80.0\" x=\""+ pos +"\" y=\"90.0\" />\n");
			pos=pos+85.0;
		}
		...
		while (it9.hasNext()) {
			...
			bw.write("<cell activitydefinition=\""+ operation.getOperationNR() + "\" height=\"40.0\" width=\"80.0\" x=\""+ pos +"\" y=\"180.0\" />\n");
			pos=pos+85.0;
		}
