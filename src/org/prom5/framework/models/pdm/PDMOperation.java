...
/**
 * <p>Title: PDM operation</p>
 * <p>Description: Represents an operation in a PDM model </p>
...
public class PDMOperation {
	...
	private Integer opNR;                    // the number of the operation, needed for Declare. The number is the hashCode of opID.
...
//	private String kindOf;                   // the kind of operation (specialisation, generalisation, has, etc.)
	private int rootDistance;                // the distance to the root element counted in the minimum number of operations to be executed after this operation
	private int remainingProcessingTime;     // the remaining processing time including the execution of this operation.
	HashMap resource = new HashMap(1);       // the resource that is allowed to execute the operation
	...
	ModelGraphVertex between;                // this object represents the 'knot' of an operation that takes together the arcs from all input elements


	/**
	 * Creates the operation with opID 'id'. At the same time also the 'knot'
	 * is created, as a new ModelGraphVertex, that takes the arcs from the
	 * input elements together.
	 ...
	 * @param id String
	 */
	public PDMOperation(PDMModel model, String id, Integer cost, Integer time, double probability, HashSet conditions) {
		this.opID = id;
		this.cost = cost;
		this.time = time;
		between = new ModelGraphVertex(model);
	 ...
	 * @param id String
	 */
	/*	public PDMOperation(PDMModel model, String id, Integer cost, Integer time, HashSet conditions) {
		this.opID = id;
		this.cost = cost;
		this.time = time;
		between = new ModelGraphVertex(model);
...
*/
	...
	/**
	 * Sets the integer number for the operation based on the ID. To be able
	 ...
	 * is generated from the string id by using the hashCode() of the identifier.
	 *
	 * @param in Integer
	 */
	...
	/**
	 * Returns the numerical identifier of an operation. This numerical reference
	 * is generated from the string identifier bij using the hashCode() of the
	 * string identifier. A numerical reference is needed to be able to generate
	 * a Declare model from the PDM model.
	 *
	 * @return Integer
	 */
	 ...
	 */
		...
		ModelGraphEdge edge = new ModelGraphEdge(model.getDataElement(dataElementID), between);
		model.addEdge(edge);
	}
	...
	public void addOutputElement(String dataElementID, PDMModel model) {
		...
		ModelGraphEdge edge = new ModelGraphEdge(between, model.getDataElement(dataElementID));
		model.addEdge(edge);
	}
	...
	/**
	 * Checks whether the specified data element (object) is an input element of the
	 * the operation.
	 * @param el PDMDataElement
	 * @return Boolean
	 */
	...
	/**
	 * Checks whether the specified data element (object) is an
	 * output elements of the operation.
	 * @param el PDMDataElement
	 * @return Boolean
	 */
	...
	public void writeToPDM(Writer bw) throws IOException {
		...
		while (it.hasNext()) {
			...
			bw.write("\t\t\t\t<DataElementRef>" + dataElement.getID() + "\"</DataElementRef>\n");
		}
		bw.write("\t\t\t</Input>\n");
		bw.write("\t\t\t<Output>\n");
		...
		while (it3.hasNext()) {
			...
			bw.write("\t\t\t\t<ResourceRef>" + resource.getID() + "\"</ResourceRef>\n");
		}
		bw.write("\t</Operation\n");
	}
	 ...
	 * the input to the output elements. In between there is a 'knot' to
	 * connect the arcs together that belong together in one operation.
	 * @param bw Writer
	 * @param model PDMModel
	 * @throws IOException
	 */
	public void writeToDot(Writer bw, PDMModel model) throws IOException {
		...
		bw.write(opID + " [shape=circle, fixedsize=true, height=\".1\", style=filled, label=\"\"];\n");

		//write the arcs from the input elements to the point in between
	    Iterator it = input.values().iterator();
		while (it.hasNext()) {
			PDMDataElement dataElement = (PDMDataElement) it.next();
			bw.write(opID + " -> " + dataElement.getID() + "[arrowhead=none];\n");
		}
		...
		while (it2.hasNext()) {
			...
			bw.write(dataElement.getID() + " -> " + opID + "[arrowhead=none, arrowtail=normal];\n");
		}
		...
		if (output.containsValue(root)){
			...
			for (int i=0;i<ops.length;i++) {
				...
				if (!(intersection.isEmpty())){
					int remTime = time + op.calculateRemainingProcessingTime(model);
					if (remTime < result) {
						result = remTime;
					}
				}
