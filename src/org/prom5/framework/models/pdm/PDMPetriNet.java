...
public class PDMPetriNet extends PetriNet {
	...
	/**
		 * This is the transition writing part of the {@link #writeToDot writeToDot} procedure.
		 *
			...
			protected void writeEdgesToDot(Writer bw) throws IOException {
				...
				while (it.hasNext()) {
					...
					if (e.isPT()) {
						...
						bw.write(p.getIdentifier() + " -> " + t.getIdentifier() + ";\n");
					} else {
						Place p = (Place) e.getDest();
						Transition t = (Transition) e.getSource();
						bw.write(t.getIdentifier() + " -> " + p.getIdentifier() + ";\n");
					}
				}
			}
/*			protected void writeEdgesToDot(Writer bw) throws IOException {
				...
				while (it.hasNext()) {
					...
					if (e.isPT()) {
						Place p = (Place) e.getSource();
						PDMTransition t = (PDMTransition) e.getDest();
						PDMOperation op = t.getAOperation();
//						bw.write(e.toString() +
//								" ["+  "lhead=cluster_" + t.getID()  +"]"+
//								";\n");
					} else if (e.isTP()) {
						Place p = (Place) e.getDest();
						PDMTransition t = (PDMTransition) e.getSource();
						PDMOperation op = t.getAOperation();
//						bw.write(t.getIdentifier()+ op.getID() + " -> " + p.getIdentifier() +
//								" ["+  "ltail=cluster_" + t.getID()  +"]" +
//								";\n");
