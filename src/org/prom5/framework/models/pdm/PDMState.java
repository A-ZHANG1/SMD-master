...
public class PDMState extends ModelGraphVertex {
	...
	int status; // the status of the state in the statespace (0:green; 1:orange; 2:red)

	public PDMState(PDMStateSpace statespace, String id) {
		super(statespace);
	    setIdentifier(id);
//	    String label = "{"+ getExecutedOperations() +"}{"+ getFailedOperations() +"}{"+ getDataElements()+"}";
//		setDotAttribute("label", label);
//		setDotAttribute("shape", "box");
	}

	public PDMState(PDMStateSpace statespace, String id, HashSet data, HashSet exec, HashSet failed) {
		super(statespace);
		setIdentifier(id);
...
/*	public PDMState(PDMStateSpace statespace, String id, HashSet data, HashSet exec, HashSet failed, int status) {
		super(statespace);
		setIdentifier(id);
		this.dataElements = data;
		this.executedOperations = exec;
	...
	public void writeToDot(Writer bw) throws IOException {
		...
		if (status == 0) {
			bw.write(this.getIdentifier() + " [shape=box, label=\"{" +
					getExecutedOperations() + "} {" + getFailedOperations() + "} {" + getDataElements() +
					"}\"];\n");
			...
			//write state
			bw.write(this.getIdentifier() + " [shape=box, style=filled, color=" + str + ", label=\"{" +
					getExecutedOperations() + "} {" + getFailedOperations() + "} {" + getDataElements() +
					"}\"];\n");
		}
	}
