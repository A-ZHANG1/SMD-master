...
//	String name;
	...
	Vector D = new Vector();
	Vector V = new Vector();
	Vector strategy;                      // vector representing the stationary Markov decision rule (optimal strategy)
	...
	/**
	 * Returns a hashset with set of states that follow upon state 'state' (i.e. the outgoing edges of 'state' point to these states)
	 *
	 * @param state PDMState
	 * @return HashSet
	 */
	 ...
	 * root element in the set of available data elements. Thus, it checks whether
	 * it is possible to succesfully complete the process from this state.
	 *
	 * @param state PDMState
	 * @return boolean
	 */
...
//				    HashSet outEdges = getOutEdges(state);
					HashSet decisions = calculateDecisions(state); // HashSet with decisions represented as Strings
...
/*			Vector vv2 = (Vector) vv.clone();
			while (!vv2.isEmpty()){
				Double val = (Double) vv2.firstElement();
			...
			Vector dd2 = (Vector) dd.clone();
			while (!dd2.isEmpty()){
				Double val = (Double) dd2.firstElement();
...
//				Double d4 = new Double(ddd);
	...
	public Vector calculateStrategyMinTime(){
		...
		V.add(v0);
		D.add(d0);
		// calculate decision rule for all timesteps n, 0< n < (depth of the statespace)
		int depth = getDepthOfStatespace() + 1;
