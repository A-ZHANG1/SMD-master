...
public class PDMTransition extends Transition {
	...
	public void writeContentToDot(Writer bw) throws IOException {
		...
		while(it.hasNext()) {
			...
			bw.write(identifier + d.getID() + " [shape=circle, height=\".3\", label=\"" + d.getID() +"\"];\n");
		}
		...
		for (int i=0 ; i<ops.length ; i++) {
			...
			bw.write(identifier + op.getID() + " [shape=circle, fixedsize=true, height=\".1\", style=filled, label=\"\"];\n");
			...
			while (it2.hasNext()) {
				...
				bw.write(identifier + dataElement.getID() + " -> " + identifier + op.getID() + "[arrowhead=none];\n");
			}
			...
			while (it3.hasNext()) {
				...
				bw.write(identifier + op.getID()  + " -> " + identifier + dataElement.getID() + ";\n");
			}
