...
package org.prom5.framework.models.pdm.algorithms;

import java.io.InputStream;

import javax.xml.parsers.DocumentBuilderFactory;

import org.prom5.framework.models.pdm.PDMCondition;
...
import org.prom5.framework.models.pdm.PDMResource;
import org.w3c.dom.Document;
import org.w3c.dom.Node;
import org.w3c.dom.NodeList;

/**
 * <p>Title: PDM reader</p>
 * <p>Description: Parses and reads the incoming PDM file, and loads the corresponding model into
 * the ProM framework. </p>
...
//				System.out.println("Name = " + name);
	...
	public static void parseDataElement(Node rootNode, PDMModel model){
		...
		if (!(descr == null)) {
			PDMDataElement dataElement1 = new PDMDataElement(model, idnode.getNodeValue(), descr.getNodeValue());
			model.addDataElement(dataElement1);
		}
		else {
	        PDMDataElement dataElement2 = new PDMDataElement(model, idnode.getNodeValue(), null);
...
//		Node refnode = rootNode.getAttributes().getNamedItem("RootElementRef");
//		System.out.println(refnode);
//		PDMDataElement dataElement = model.getDataElement(refnode.getNodeValue());
...
//				System.out.println(elt.getID());
	 ...
	 * @param model PDMModel
	 */
	public static void parseInputElements(Node rootNode, PDMOperation operation, PDMModel model) {
		NodeList nodes = rootNode.getChildNodes();
		for (int i=0; i<nodes.getLength(); i++) {
			Node node;
			node= nodes.item(i);
	 ...
	 * @param model PDMModel
	 */
	public static void parseOutputElements(Node rootNode, PDMOperation operation, PDMModel model) {
		NodeList nodes = rootNode.getChildNodes();
		for (int i=0; i<nodes.getLength(); i++) {
			Node node;
			node = nodes.item(i);
	 ...
	 * @param model PDMModel
	 */
	public static void parseResourceRef(Node rootNode, PDMOperation operation, PDMModel model) {
		Node node = rootNode;
	 ...
	 * @param operation PDMOperation
	 */
	private static void parseCondition(Node rootNode, PDMOperation operation, PDMModel model){
		Node node = rootNode;
...
/*			if (str.contains("!=")){
		  ...
		  if (str.contains("=")){
			...
			} else System.out.println("Condition in wrong text format");
		}
