 ...
 * Petri nets are bipartite graphs and therefore do only allow to connect
 * places with transitions or the other way around.
 *
 * @see PetriNet
 * @see Transition
 * @see Place
...
public class PNEdge extends ModelGraphEdge implements Cloneable {
	 ...
	 * This might be used to store the frequency of using certain paths in the
	 * Petri net model.
	 * @param p the probabilty to be associated to this arc
	 */
	 ...
	 * Note that there is not automatically a valid value but it must have been
	 * assigned upfront using {@link #setProbability setProbability}
	...
	/**
	 * Working method to reset source and target node with package access only.
	 * <br>
	 * This is needed to re-establish the structure of a Petri net.
	 * However, please note that there is no consistency check included (such
	 * as ensuring that one of the nodes is a place and the other a transition).
	 * So be careful when using it for other purposes.
	 * @param source the new tail node of this edge
	 * @param target the new head node of this edge
	 */
	...
	public Transition getTransition() {
		if (isPT()) return (Transition) getHead();
