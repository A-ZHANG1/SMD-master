...
public class PetriNet extends ModelGraph implements Cloneable {
	 ...
	 * link to be established use
	 * {@link addAndLinkTransition(Transition t) addAndLinkTransition(Transition t)}
	 * instead.
	 *
	 * @param t
	 ...
	 * established already use
	 * {@link addTransition(Transition t) addTransition(Transition t)} instead.
	 *
	 * @param t
	 *            the transition to be added
	 * @return the newly added transition
	 ...
	 * already (so t.getSubgraph() should return this net). If you need that
	 * link to be established use
	 * {@link addAndLinkPlace(Place p) addAndLinkPlace(Place p)} instead.
	 *
	 * @param p
	 ...
	 * @return <code>true</code> if the token could be added,
	...
	private ArrayList<Place> findPlaces(TransitionCluster transitions) {
        ...
        for (Place place: places) {
            ...
            /*
             * If there's only one neighbor, then this should be a source or sink place.
             * Do not add source or sink places to any cluster. Otherwise, add the place
             * if all its neighbors belong to the cluster.
             */
	...
	public Place findPlace(Set in, Set out) {
        ...
        public PNEdge findEdge(PNNode tail,PNNode head) {
                ...
                while (allEdges.hasNext()) {
                        ...
                        if (e.getHead().equals(head) &
                                e.getTail().equals(tail)) {
                                return e;
                        }
                }
                // not found
	 ...
	 *
	 * @return <code>True</code> if there are duplicate tasks with shared
	 *         input places. <code>False</code> otherwise
	 */
	...
	public int numberOfNodes() {
        ...
        /**
         * Determines if this net is a marked graph, i.e., each place has
         * at most one input transition and at most one output transition.
         *
        ...
        public boolean isMarkedGraph() {
            ...
            while (allPlaces.hasNext()) {
                Place p = allPlaces.next();
                if (p.getPredecessors().size() <= 1 && p.getSuccessors().size() <=1) {
                    list.add(p);
                }
            }
	...
	/**
	 * @see org.prom5.framework.models.ModelGraph#removeVertex(org.prom5.framework.models.ModelGraphVertex)
	 */
