 ...
 * It is the passive component in the Petri net structure, and may hold zero
 * to an arbitrary number of tokens. The marking of all the places in the net
 * relates to the state of the Petri net.
 *
 * @see Token
 * @see Transition
 * @see PetriNet
...
public class Place extends PNNode implements Comparable, Cloneable {
	...
	/**
	 * Constructor.
	 * @param identifier the name of the this place
	 * @param net the PetriNet this place belongs to
	 */
	 ...
	 * Note that if the given token is already held by this place, it is not
	 * added again.
	 * @param to the token that should be added
	 ...
	 * @param to the token that should be removed
	 ...
	 * @return the token that has been chosen to be removed if there is at
	 * least one available, <code>null</code> otherwise
	 */
	 ...
	 * If a timestamp should be considered, use
	 * {@link getNumberOfTokens(Date timestamp) getNumberOfTokens(Date timestamp)} instead.
	...
	/**
	 * Gets the number of tokens that are available at the given
	 * timestamp at that place.
	 * If a timestamp is not important, use
	 * {@link getNumberOfTokens() getNumberOfTokens()} instead.
	 * @param timestamp the time at which the tokens must be available
	...
	public int getNumberOfTokens(Date timestamp) {
		...
		while (it.hasNext()) {
			...
			if (!t2.isTimed() || (t2.getTimestamp() == null ?
					true :
					!(t2.getTimestamp().after(timestamp)))
					) {
				i++;
			}
		}
	 ...
	 * timestamp.
	 * If a timestamp is not important, use
	 * {@link getRandomAvailableToken() getRandomAvailableToken()} instead.
	 * @param timestamp the time at which the token must be available
	...
	public Token getRandomAvailableToken(Date timestamp) {
		...
		while (it.hasNext() && (t == null)) {
			Token t2 = (Token) it.next();
			if (!t2.isTimed() || (t2.getTimestamp() == null ? true :
					!(t2.getTimestamp().after(timestamp)))) {
				t = t2;
			}
		}
	 ...
	 * If a timestamp should be considered, use
	 * {@link getRandomAvailableToken(Date timestamp) getRandomAvailableToken(Date timestamp)} instead.
	 ...
	 * @param id the name to compare with
	...
	/**
	 * Overridden to specify when two Petri net places are considered to
	 * be equal.
	 * @param o the <code>Place</code> to be compared with
