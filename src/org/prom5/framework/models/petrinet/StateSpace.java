 ...
 *
 * @see org.prom5.framework.models.petrinet.algorithms.ReachabilityGraphBuilder
 * @see org.prom5.framework.models.petrinet.algorithms.CoverabilityGraphBuilder
 */
public class StateSpace extends AcceptFSM {
	...
	public void writeToFSM(BufferedWriter bw) throws IOException {
		...
		while (it.hasNext()) {
			...
			p.setNumber(i++);
			ArrayList pVals = (ArrayList) placeValues.get(p);
			bw.write((useIdentifier ? p.getIdentifier() : "p" + p.getNumber()) + "(" + (pVals.size() + 2) + ") ");
	...
	public List<List<ModelGraphVertex>> getStronglyConnectedComponents() {
		...
		List<List<ModelGraphVertex>> result = computeFinishingTimes(nodes.get(0), false);
		return result;
	}

	private List<List<ModelGraphVertex>> computeFinishingTimes(List<ModelGraphVertex> nodes,
			boolean forward) {
	...
	private void computeFinishingTimesDFS(ModelGraphVertex node, List<ModelGraphVertex> tree,
			Set<ModelGraphVertex> visited, boolean forward) {
		if (visited.add(node)) {
