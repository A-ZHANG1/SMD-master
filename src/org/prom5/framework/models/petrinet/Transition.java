...
public class Transition extends PNNode implements LogEventProvider, Cloneable {
	 ...
	 * @param logModelElement the log event associated
	 * @param net the Petri net it belongs to
	 */
	 ...
	 * @param identifier the name of the transition
	 * @param net the Petri net it belongs to
	 */
	...
	/**
	 * Copy Constructor. Note that the Transition will think it exists in the same
	 * graph. If you want to copy it to another graph, call setSubgraph() explicitely.
	 ...
	 * @param lme the new log event associated
	 */
	 ...
	 * using the {@link setNumber setNumber(int i)} method before.
	 *
	 ...
	 * Note that this is not necessarily unique for two different transitions,
	 * they can have the same name. For comparison the {@link equals equals}
	 ...
	 * That is the case if there are sufficient tokens available on all its input places,
	 * i.e., as many as would be consumed when firing the transition.
	 * @see isNegativeEnabled
	 *
	 * @return <code>true</code> if the transition is enabled, <code>false</code> otherwise
	 */
	public boolean isEnabled() {
		...
		while (it.hasNext() && b) {
			...
			b = p.getNumberOfTokens() >= ((PetriNet) getGraph()).getEdgesBetween(p, this).size();
		}
		return b;
	}
	 ...
	 * is not needed, {@link Transition#fireQuick() fireQuick()}
	 ...
	 * is needed, {@link Transition#fire() fire()} should be used instead.
	 * <p>
	 * The effect is that tokens are produced in the output places of this transition.
	 */
	 ...
	 * account.
	 * That is the case if, at the given point in time, there are sufficient tokens
	 * available on all its input places, i.e., as many as would be consumed when
	 * firing the transition.
	 *
	 * @param timestamp the time specified
	 * @return <code>true</code> if the transition is enabled, <code>false</code> otherwise
	 */
	public boolean isEnabled(Date timestamp) {
		...
		while (it.hasNext() && b) {
			Place p = (Place) it.next();
			b = p.getNumberOfTokens(timestamp) >= ((PetriNet) getGraph()).getEdgesBetween(p,
				this).size();
		}
		return b;
	}
	 ...
	 * <p>
	 * Note that this only happens if the transition is enabled. If this check
	 * is not needed, {@link Transition#fireQuick(Date timestamp) fireQuick(Date timestamp)}
	 * should be used instead.
	 * <p>
	 * The effect is that tokens are produced in the output places of this transition.
	 *
	 ...
	 * <p>
	 * Note that it is not checked whether the transition is enabled. If this
	 * is needed, {@link Transition#fire(Date timestamp) fire(Date timestamp)}
	 * should be used instead.
	 * <p>
	 * The effect is that tokens are produced in the output places of this transition.
	 *
	...
	public void fireQuick(Date timestamp) {
		...
		while (it.hasNext()) {
			((Place) ((PNEdge) it.next()).getDest()).addToken(new Token(timestamp));
		}
	}

	/**
	 * Checks whether this transition is negatively enabled.
	 * That is the case if there are sufficient tokens available on all its output places,
	 * i.e., as many as were produced when firing the transition.
	 * @see isEnabled
	 *
	 * @return <code>true</code> if the transition is enabled, <code>false</code> otherwise
	 */
	public boolean isNegativeEnabled() {
		...
		while (it.hasNext() && b) {
			Place p = (Place) it.next();
			b = p.getNumberOfTokens() >= ((PetriNet) getGraph()).getEdgesBetween(this, p).size();
		}
		return b;
	}
	 ...
	 * Note that this only happens if the transition is negatively enabled.
	 * If this check is not needed, {@link Transition#unFireQuick() unFireQuick()}
	 * should be used instead.
	 * @see isNegativeEnabled
	 */
	public void unFire() {
	...
	/**
	 * Unfires this transition.
	 * Note that it is not checked whether the transition is negatively enabled.
	 * If this is needed, {@link Transition#unFire() unFire()} should be used instead.
	 */
	 ...
	 * @param lme the log event to be compared with
	...
	protected boolean hasLogModelElement(LogEvent lme) {
		...
		return logModelElement.equals(lme.getModelElementName(), lme.getEventType());
	}

	/**
	 * A task is considered invisible if there is no log event associated to it.
	 * @return <code>true</code> if this transition is an invisible task, <code>false</code> otherwise
	 */
	...
	/**
	 * A task is considered as a duplicate task if there is at least one other task in the model which
	 * has the same log event associated.
	 * <br>
	 * Note that this leads to the fact that the occurrences of duplicate tasks cannot be distinguished
	 * in the event log.
	 * Note further that in the case of duplicate tasks the method
	 * {@link PetriNet#findTransitions(LogEvent) findTransitions} will return multiple tasks being
	 * associated to that log event.
	 * @return <code>true</code> if this transition is a duplicate task, <code>false</code> otherwise
	 */
	public boolean isDuplicateTask() {
		...
		Iterator allTransitions = ((PetriNet) this.getSubgraph()).getTransitions().iterator();
		while (allTransitions.hasNext()) {
			Transition transition = (Transition) allTransitions.next();
			if ((transition != this) && (transition != null) && (transition.getLogEvent() != null) &&
					(transition.getLogEvent().equals(this.getLogEvent()))) {
				// at least one other transition having the same log event found --> duplicate task
				return true;
			}
		}
	...
	/**
	 * Overridden to specify when two Petri net transitions are considered to
	 * be equal.
	 * <br>
	 ...
	 *
	 * @param o the <code>Transition</code> to be compared with
	 * @return <code>true</code> if the identifiers of the transitions and the
	 * identifiers of all input and output places are the same, <code>false</code> otherwise
	 */
	public boolean equals(Object o) {
		...
		// names of the predecessors must be equal (object identity is too strong
		// as nodes will be cloned if the Petri net is cloned)
		// however, same order is assumed for the time being (since is preserverd via cloning)
	 ...
	 * Only the {@link PetriNet#clone PetriNet.clone()} method will update the
