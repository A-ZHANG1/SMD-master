...
/**
 * A coverability graph is a finite state space model for a Petri net.
 * If the reachability graph is finite already, the coverability graph is
 * identical to it. However, is the reachability graph would be infinite (which
 * may happen if a Petri net is accumulating tokens), the coverability graph
 * introduces so-called Omega states in order to gain a final state space.
 * <p>
 ...
 * <p>
 * This static class provides an implementation of the algorithm in
 * definitions 3.36, 3.37 and 3.38 of "Verification of WF-nets", H.M.W. Verbeek, Technische
...
public class CoverabilityGraphBuilder {
	...
	/**
	 * Constructs the coverability graph with a potentially limited depth
	 * for a given marked net.
	 * If the full state space shall be constructed, use {@link #build(PetriNet)} instead.
	 * <br>
	 * Remembers the start state (that is, the marking of the given net) in the
	 * {@link StateSpace#initialState initialState} attribute.
	 * @param net The marked net
	 * @param depth The depth until which the state space should be constructed. If it is 0,
	 ...
	 * @param net The net to construct a (fragment) coverability graph for.
	 * @param depth The maximal depth for the fragment.  
	 * @param useAllPredecessors Whether to use all predecessors when checking for unbounded places 
	 * or only spanning tree predecessors.
	 * @param expandUnboundedStates Whether to expand unbounded states.
	 * @return A (fragment of) a coverability graph.
	 */
	public synchronized static StateSpace build(PetriNet net, int depth, boolean useAllPredecessors, boolean expandUnboundedStates) {
		...
		/// Association of toDo states with distance of their own depth level to the
		...
		// Get these in adavnce, just in case this takes time computing over and over again.
		transitions = net.getTransitions();

		/// temp variable to examine the distance from maximum depth for current toDo state
		Integer currentDistObject;
		int currentDistance = 0;
		
		// Expand states.
		while (!toDo.isEmpty() && (getAvailableMemory() > 0)) { // As long as there is a state to expand and memory left
			state = (State) toDo.removeFirst(); // Take the first (breadth-first)
			...
			/// (a) no positive depth was specified (full coverability graph will be constructed)
			/// (b) if depth limit was not reached yet (i.e., distance is greater than 0)
					...
					if (((Marking) preSet.get(transition))
						...
						if (treeMap.containsKey(newMarking)) { // If the
							...
							if (useAllPredecessors) {
								/**
								 * Check *all* predecessors, not only the ones according to the spanning tree
								 * of the graph.
								 */
								/**
								...
								/**
								 * Set of predecessors still to check for more predecessors.
								 */
								...
								/**
								 * As long as there is a predecessor left to check: check it.
								 */
								while (!predecessorsToDo.isEmpty()) {
									...
									if (predecessor.getMarking().isLessOrEqual(
										...
										while (pit.hasNext()) {
											place = (Place) pit.next();
											if (predecessor.getMarking().getTokens(
													place) < newMarking
													.getTokens(place)) {
												// Token generator found for place,
												// set to OMEGA
						...
						if (isFresh) { // If the marking is fresh in the
							...
							/// if depth limitation is to be kept track of, record the
						...
						// HV ModelGraphEdge edge = graph.addEdge(new ModelGraphEdge(state, newState));
		...
		if (!toDo.isEmpty()) {
			Message.add("Insufficient memory available to construct complete coverability graph. Constructed coverability graph is truncated!", Message.ERROR);
		}
		return graph;
	}
