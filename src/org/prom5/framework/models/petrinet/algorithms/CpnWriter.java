...
package org.prom5.framework.models.petrinet.algorithms;
...
import org.prom5.exporting.petrinet.CpnExport;
...
public class CpnWriter {
	...
	/**
	 * Offset specifying the displacement of the type label for the place on
	 * the x axis.
	 * The reference point is the center position of the place itself and the
	 * value is taken from the CPN Tools default position of the type label, if
	 * the type is CASE_ID.
	 */
	private static int Place_Type_Offset_X = 52;

	/**
	 * Offset specifying the displacement of the type label for a place on
	 * the y axis.
	 * The reference point is the center position of the place itself and
	 * the value is taken from the corresponding CPN Tools default position.
	 */
	private static int Place_Type_Offset_Y = -24;

	/**
	 * Offset specifying the displacement of the initial marking label for
	 * the place on the x axis.
	 * The reference point is the center position of the place itself and
	 * the value is taken from the corresponding CPN Tools default position.
	 */
	...
	/**
	 * Offset specifying the displacement of the port type tag for the place
	 * on the x axis.
	 * The reference point is the center position of the place itself and
	 * the value is taken from the corresponding CPN Tools default position.
	 */
	...
	/**
	 * Offset specifying the displacement of the guard condition label for
	 * a transition on the x axis.
	 * The reference point is the center position of the transition itself and
	 * the value is taken from the corresponding CPN Tools default position.
	 */
	...
	/**
	 * Offset specifying the displacement of the time delay label for
	 * a transition on the x axis.
	 * The reference point is the center position of the transition itself and
	 * the value is taken from the corresponding CPN Tools default position.
	 */
	...
	/**
	 * Offset specifying the displacement of the code specification for
	 * a transition on the x axis (that is input, output, action).
	 * The reference point is the center position of the transition itself and
	 * the value is taken from the corresponding CPN Tools default position.
	 */
	...
	/**
	 * Offset specifying the displacement of the code specification for
	 * a transition on the y axis (that is input, output, action).
	 * The reference point is the center position of the transition itself and
	 * the value is taken from the corresponding CPN Tools default position.
	 */
	...
	/**
	 * Offset specifying the displacement of the channel specification for
	 * a transition on the x axis (not used).
	 * The reference point is the center position of the transition itself and
	 * the value is taken from the corresponding CPN Tools default position.
	 */
	...
	/**
	 * Offset specifying the displacement of the channel specification for
	 * a transition on the y axis (not used).
	 * The reference point is the center position of the transition itself and
	 * the value is taken from the corresponding CPN Tools default position.
	 */
	...
	/**
	 * Offset specifying the displacement of the subpage tag for
	 * a transition on the x axis).
	 * The reference point is the center position of the transition itself and
	 * the value is taken from the corresponding CPN Tools default position.
	 */
	...
	/**
	 * The ID of the top level page of the exported CPN model.
	 * It will be used to link the module to an instance.
	 */
	...
	/**
	 * The ID of the page containing the actual process model to be exported.
	 * It will be used to link the module to an instance.
	 */
	 ...
	 * The ID of the instance built from the Overview module.
	 * Needs to be linked in order to establish hierarchy.
	 */
	private static String overviewInstanceID;

	/**
	 * The ID of the instance built from the Environment module.
	 * Needs to be linked in order to establish hierarchy.
	 */
	private static String environmentInstanceID;

	/**
	 * The ID of the instance built from the Process module.
	 * Needs to be linked in order to establish hierarchy.
	 */
	 ...
	 * Use the static method {@link #write write} to actually write a file.
	 */
	private CpnWriter() {}

	/**
	 *
	 * @param net PetriNet the PetriNet to be exported to CPN Tools
	 * @param bw BufferedWriter used to stream the data to the file
	 * @throws IOException
	 */
	public synchronized static void write(PetriNet net, BufferedWriter bw) throws IOException {

		Subgraph graph = net.getGrappaVisualization().getSubgraph();
		/* an ID counter for creating unique XML element IDs across the
		 whole document */
		int i = 1;
		...
		bw.write("\t\t<instances>\n");
		bw.write("\t\t\t<instance id=\"" + overviewInstanceID + "\" page=\"" + overviewPageID +
				"\">\n");
		bw.write("\t\t\t\t<instance id=\"" + environmentInstanceID + "\" trans=\"ID" +
				environmentTransitionID_overviewPage + "\"/>\n");
		bw.write("\t\t\t\t<instance id=\"" + processInstanceID + "\" trans=\"ID" +
				processTransitionID_overviewPage + "\"/>\n");
		bw.write("\t\t\t</instance>\n");
		bw.write("\t\t</instances>\n");
	 ...
	 *
	 * @param i the global ID counter used to uniquely identify XML elements
	...
	private static int assignIDs(int i) {
		...
		/* start and end place IDs on the sub page levels
		 (always keep following IDs for the node additions)*/
	...
	/**
	 * Writes the type declarations, that is the color sets of the CPN.
	 * Includes the default color sets defined by CPN Tools in a block called
	 * "Standard declarations" and in addition defines the custom color sets
	 * necessary for the chosen representation of a low-level Petri net in
	 * terms of a CPN.
	 *
	 ...
	 * @throws IOException
	 */
	private static int writeDeclarations(BufferedWriter bw, int i) throws IOException {

		bw.write("\t\t<globbox>\n");
		bw.write("\t\t\t<block id=\"ID" + (i++) + "\">\n");
		bw.write("\t\t\t\t<id>Standard declarations</id>\n");

		/** @todo Anne: provide general methods for writing types and
		 * variables */
		...
		/* custom color set defining the ID of a case
		 (used for all the low-level places on the actual process page) */
		bw.write("\t\t\t<color id=\"ID" + (i++) + "\">\n" +
				"\t\t\t\t<id>CASE_ID</id>\n" +
				"\t\t\t\t<alias>\n" +
				"\t\t\t\t\t<id>INT</id>\n" +
	...
	/**
	 * Writes the top level page of the CPN model.
	 * It only contains two transitions which are substituted by the Environment
	 * and the actual Process page.
	 *
	 * @param bw the BufferedWriter used to stream the data to the file
	 * @param i the global ID counter used to uniquely identify XML elements
	 * @return the ID counter
	 * @throws IOException
	 */
	private static int writeOverviewPage(BufferedWriter bw, int i) throws IOException {
		...
		writeTransitionTag(bw, environmentTransitionID_overviewPage, -116, 98, "Environment", 176,
				38, "", environmentPageID, "Environment", portToSocketList, false);
		...
		portToSocketList.add(portToSocket1);
		portToSocketList.add(portToSocket2);
		writeTransitionTag(bw, processTransitionID_overviewPage, -116, -10, "Process", 176, 94, "",
				processPageID, "Process", portToSocketList, false);
		...
		i = writeArcTag(bw, i, "PtoT", environmentTransitionID_overviewPage, endPlaceID, 0, 0, "",
			bendpoints);
		...
		bendpoint1.add("98");
		bendpoints.add(bendpoint1);
		i = writeArcTag(bw, i, "TtoP", environmentTransitionID_overviewPage, startPlaceID, 0, 0, "",
			bendpoints);

		/* write arc 3 */
		i = writeArcTag(bw, i, "PtoT", processTransitionID_overviewPage, startPlaceID, 0, 0, "", null);

		/* write arc 4 */
		i = writeArcTag(bw, i, "TtoP", processTransitionID_overviewPage, endPlaceID, 0, 0, "", null);
	 ...
	 * environment for the exported process.
	 * It generates cases and initializes their case data.
	 *
	 * @param bw the BufferedWriter used to stream the data to the file
	 * @param i the global ID counter used to uniquely identify XML elements
	 * @return the ID counter
	 * @throws IOException
	 */
	private static int writeEnvironmentPage(BufferedWriter bw, int i) throws IOException {
		...
		writePlaceTag(bw, startPlaceID_environmentPage, -292, -11, "Start", 48, 48, "CASE_ID", "",
				"Out");
		/* write End place (PORT place) */
		writePlaceTag(bw, endPlaceID_environmentPage, 259, -14, "End", 48, 48, "CASE_ID", "", "In");
		/* write case data place (FUSION) */
		writePlaceTag(bw, caseDataPlaceID_environmentPage, -18, 143, "Case data", 120, 58, "CASE",
				"", "");
		/* write case ID counting place (initial marking is 1) */
		writePlaceTag(bw, caseCounterPlaceID, -18, 263, "next\ncase ID", 120, 58, "CASE_ID", "1",
				"");
		/* write init transition (generating and initializing cases) */
		writeTransitionTag(bw, initTransitionID, -292, 140, "Init", 92, 56, "", null, null, null, false);
		/* write cleanup transition (removing cases) */
		writeTransitionTag(bw, cleanupTransitionID, 259, 139, "Clean-up", 92, 56, "", null, null, null, false);

		/* write arc Init --> Start */
		i = writeArcTag(bw, i, "TtoP", initTransitionID, startPlaceID_environmentPage, -317, 54,
			"c", null);
		/* write arc Init --> Casedata */
		i = writeArcTag(bw, i, "TtoP", initTransitionID, caseDataPlaceID_environmentPage, -157, 129,
			"(c,[])", null);
		/* write arc Casedata --> Cleanup */
		i = writeArcTag(bw, i, "PtoT", cleanupTransitionID, caseDataPlaceID_environmentPage, 122,
			129, "(c,attributes)", null);
		/* write arc End --> Cleanup */
		i = writeArcTag(bw, i, "PtoT", cleanupTransitionID, endPlaceID_environmentPage, 284, 61,
			"c", null);
		/* write arc Init --> CaseIDcounter */
		i = writeArcTag(bw, i, "TtoP", initTransitionID, caseCounterPlaceID, -185, 212, "c+1", null);
		...
		bendpoint1.add("263");
		bendpoints.add(bendpoint1);
		i = writeArcTag(bw, i, "PtoT", initTransitionID, caseCounterPlaceID, -317, 214, "c",
			bendpoints);
	 ...
	 *
	 * @param graph the graph structure containing the process model
	 * @param bw the BufferedWriter used to stream the data to the file
	 * @param i the global ID counter used to uniquely identify XML elements
	 * @return the ID counter
	 * @throws IOException
	 */
	private static int writeProcessPage(Subgraph graph, BufferedWriter bw,
	...
	/**
	 * Writes the places of the Petri net to be exported.
	 * Re-uses graphical information assigned by the DOT graph layouter used
	 ...
	 * @throws IOException
	 */
	private static int writePlaces(Subgraph graph, BufferedWriter bw,
			int i) throws IOException {
		...
		while (e.hasMoreElements()) {
			...
			if (el.object != null && el.object instanceof Place) {
				...
				int w = ((int) (((Double) n.getAttributeValue(Grappa.WIDTH_ATTR)).
						doubleValue() * 120.0));
				int h = ((int) (((Double) n.getAttributeValue(Grappa.HEIGHT_ATTR)).
						doubleValue() * 120.0));

				/* if place is Start place of the process
				   -> use pre-assigned ID */
				if (p.inDegree() == 0) {
					p.setNumber(startPlaceID_processPage);
					writePlaceTag(bw, p.getNumber(), x, y, p.getIdentifier(), w, h, "CASE_ID", "",
							"In");
				}
				/* if place is End place of the process
				 -> use pre-assigned ID */
				...
				/* els use running counter to generate unique IDs for each
				 XML element.
				 Since the <type> and the <initmark> elements do also get
				 such an ID, increment accordingly.
				 Remember the counter for the node in order to be able to make
				 a reference to it when writing the edges.
				 */
				else {
					p.setNumber(i);
					writePlaceTag(bw, p.getNumber(), x, y, p.getIdentifier(), w, h, "CASE_ID", "",
	...
	/**
	 * Writes the transitions of the Petri net to be exported.
	 * Re-uses graphical information assigned by the DOT graph layouter used
	 * within the ProM framework.
	 *
	 * @param graph the graph structure containing the process model
	 ...
	 * @throws IOException
	 */
	private static int writeTransitions(Subgraph graph, BufferedWriter bw,
			int i) throws IOException {
			...
			if (el.object != null && el.object instanceof Transition) {
				...
				/*
				 ...
				 Since the <cond>, <time>, <code> and <channel> elements do
				   also get such an ID, increment accordingly.
				   Remember the counter for the node in order to be able to make
				 a reference to it when writing the edges.
				 */
				t.setNumber(i);
				...
				boolean isInvisible = ((Transition) el.object).isInvisibleTask();
				//writeTransitionTag(bw, t.getNumber(), x, y, t.getIdentifier(), w, h, "", null, null, null);
				writeTransitionTag(bw, t.getNumber(), x, y, labelHack(t.getIdentifier()), w, h, "", null, null, null,
						isInvisible);
			}
		}
	 ...
	 * @throws IOException
	 */
	private static int writeArcs(Subgraph graph, BufferedWriter bw,
			int i) throws IOException {
		...
		while (e.hasMoreElements()) {
			...
			i = writeArcTag(bw, i, orientation, transIDno, placeIDno,
				getXCoordinateArcInscription(head, tail), getYCoordinateArcInscription(head, tail),
				"c", null);
		}
	 ...
	 *
	 * @param bw the BufferedWriter used to stream the data to the file
	 * @param placeIDno the ID number for the place (actual ID is "ID" + placeIDno)
	 ...
	 * @throws IOException
	 */
	private static int writePlaceTag(BufferedWriter bw, int placeIDno, int xPos,
			int yPos, String label, int width, int height, String placeType,
			String initMark, String portType) throws IOException {
		...
		bw.write("\t\t\t<place id=\"ID" +
				...
				/////////  type specification
				/* unique ID of the XML element (not displayed) */
				"\t\t\t\t<type id=\"ID" + placeIDno++ +"\">\n" +
				/* position of the type label for the place */
				getElementAdditionPositionAttributeTag(xPos, yPos, Place_Type_Offset_X,
				Place_Type_Offset_Y) +
				...
				/* unique ID of the XML element (not displayed) */
				"\t\t\t\t<initmark id=\"ID" + placeIDno++ +"\">\n" +
				/* position of the initial marking label for the place */
				getElementAdditionPositionAttributeTag(xPos, yPos, Place_InitMark_Offset_X,
				Place_InitMark_Offset_Y) +
	 ...
	 *
	 * @param bw the BufferedWriter used to stream the data to the file
	 * @param transIDno the ID number for the transition (actual ID is "ID" + transIDno)
	...
	private static int writeTransitionTag(BufferedWriter bw, int transIDno,
		...
		if (subpageID != null) {
			...
			bw.write("\">\n" +
					"\t\t\t\t\t<subpageinfo id=\"ID" + transIDno++
					+"\" name=\"" + subpageTagName + "\">\n" +
					"\t" + getElementAdditionPositionAttributeTag(xPos, yPos,
					Transition_Subpageinfo_Offset_X, Transition_Subpageinfo_Offset_Y) +
		...
		bw.write("\t\t\t\t<binding x=\"7.0\" y=\"-3.0\"/>\n" +
				...
				/* unique ID of the XML element (not displayed) */
				"\t\t\t\t<cond id=\"ID" + transIDno++ +"\">\n" +
				/* position of the guard label for the transition */
				getElementAdditionPositionAttributeTag(xPos, yPos, Transition_Cond_Offset_X,
				Transition_Cond_Offset_Y) +
				...
				/* unique ID of the XML element (not displayed) */
				"\t\t\t\t<time id=\"ID" + transIDno++ +"\">\n" +
				/* position of the time label for the transition */
				getElementAdditionPositionAttributeTag(xPos, yPos, Transition_Time_Offset_X,
				Transition_Time_Offset_Y) +
				...
				/* unique ID of the XML element (not displayed) */
				"\t\t\t\t<code id=\"ID" + transIDno++ +"\">\n" +
				/* position of the code label for the transition */
				getElementAdditionPositionAttributeTag(xPos, yPos, Transition_Code_Offset_X,
				Transition_Code_Offset_Y) +
				...
				/* unique ID of the XML element (not displayed) */
				"\t\t\t\t<channel id=\"ID" + transIDno++ +"\">\n" +
				/* position of the channel label for the transition */
				getElementAdditionPositionAttributeTag(xPos, yPos, Transition_Channel_Offset_X,
	 ...
	 *
	 * @param bw the BufferedWriter used to stream the data to the file
	 * @param arcIDno the ID number for the arc (actual ID is "ID" + arcIDno)
	...
	private static int writeArcTag(BufferedWriter bw, int arcIDno,
			String orientation, int transEnd, int placeEnd, int xPos,
		...
		if (bendpoints != null) {
			...
			while (it.hasNext()) {
				...
				/* unique ID of the XML element (not displayed) */
				bw.write("\t\t\t\t<bendpoint id=\"ID" + arcIDno++ +"\" serial=\"1\">\n" +
	 ...
	 *
	 * @param trans the head node connected to the arc
	 * @param place the tail node connected to the arc
	...
	private static String getElementAdditionTextTag(String text) {
		return "\t\t\t\t\t<text tool=\"CPN Tools\" version=\"1.4.0\">" + text + "</text>\n";
	}
