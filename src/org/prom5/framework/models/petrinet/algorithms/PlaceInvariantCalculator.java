...
/**
 * <p>Title: PlaceInvariantCalculator </p>
 * <p>Description: This static class returns an ArrayList of Bags. Each of these
 * bags contains places an represents a semi-positive place invariant for the given PetriNet.
 * The given set is minimal.</p>
...
public class PlaceInvariantCalculator {
	...
	public static synchronized ArrayList calculate(PetriNet net, int shortCircuit) {
		...
		DoubleMatrix2D m = InvariantCalculator.calculate(net.getIncidenceMatrix().viewDice());
		...
		rows:for (int i = 0; i < m.rows(); i++) {
			...
			columns:for (int j = 0; j < m.columns(); j++) {
				...
				if ((p.inDegree() == 0) || (p.outDegree() == 0)) {
					// check the value of this place. It should be at most shortCircuit
