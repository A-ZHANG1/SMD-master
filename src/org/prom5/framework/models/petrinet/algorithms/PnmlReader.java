 ...
 * (see http://www.informatik.hu-berlin.de/top/pnml/basicPNML.rng), although this is not checked.
 * <p>
 * In addition to this schema, the following tags are supported:
 * <ul>
 * <li><b>logevent</b> (inside the <b>toolspecific</b> tag of the transition tag):
 *     this tag has a name and a type tag as its children.
 *     For example,
 *     <pre>
 *        &lt;logevent&gt;
 *           &lt;name&gt;TASK_1&lt;/name&gt;
 ...
 *        &lt;/logevent&gt;
 *     </pre>
 * <li><b>transitionlabel</b> (inside the <b>toolspecific</b> tag of the transition tag):
 *     this tag is only read if there is no logevent tag
 * <li><b>name</b> (inside a transition tag):
 *     this tag is used in the Petrinet Kernel PNML files.
 *     If this tag is present, only the text in the sub-element &lt;value&gt; is used.
 *     For example,
 *     <pre>
 *        &lt;name&gt;
 *           &lt;graphics&gt;...&lt;/graphics&gt;
 ...
 * </ul>
 *
 * All values for the type attribute of a net tag are accepted.
 * This type attribute is also NOT used to decide whether to use
 * the logevent or name tags. These tags will ALWAYS be used when
 * they are present in the PNML file.
 * <p>
...
public class PnmlReader {
    ...
    private void parseNet(Node node, PetriNet net) throws Exception {
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            if (n.getNodeName().equals("toolspecific") &&
                n.getAttributes().getNamedItem("tool").getNodeValue().equals(
                    "ProM")) {
                foundToolSpecific();
                parseClusters(n, net);
            }
    ...
    private void parseClusters(Node node, PetriNet net) {
        ...
        /*
          if ((net.getClusters()!=null) && (net.getClusters().size()>0)) {
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            if (n.getNodeName().equals("name")) {
                ...
                for (int j = 0; j < nameChildren.getLength(); j++) {
                    ...
                    if ((gn.getNodeName().equals("text") ||
                         gn.getNodeName().equals("value")) && gn.hasChildNodes()) {
                        name = gn.getFirstChild().getNodeValue();
                    }
                }
            }
    ...
    private void parseArcs(Node node, PetriNet net) throws Exception {
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            if (n.getNodeName().equals("arc")) {
                ...
                if (id == null || id.getNodeValue() == null ||
                    source == null || source.getNodeValue() == null ||
                    target == null || target.getNodeValue() == null) {
                    throw new Exception(
