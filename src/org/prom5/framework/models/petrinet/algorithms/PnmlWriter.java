...
public class PnmlWriter {
	...
	public final static String basicPntdUri =
			"http://www.informatik.hu-berlin.de/top/pnml/basicPNML.rng";

	public final static String workflowPntdUri =
			"http://www.processmining.org/workflownet1.0";
	...
	private static int writeArcs(boolean withSpline, Subgraph graph,
		...
		while (e.hasMoreElements()) {
			...
			if (edge.goesForward()) {
				source = tail.object instanceof Transition
						 ? "trans_" + ((Transition) tail.object).getNumber()
						 : "place_" + ((Place) tail.object).getNumber();
				target = head.object instanceof Transition
						 ? "trans_" + ((Transition) head.object).getNumber()
						 : "place_" + ((Place) head.object).getNumber();
			} else {
				source = head.object instanceof Transition
						 ? "trans_" + ((Transition) head.object).getNumber()
						 : "place_" + ((Place) head.object).getNumber();
				target = tail.object instanceof Transition
						 ? "trans_" + ((Transition) tail.object).getNumber()
						 : "place_" + ((Place) tail.object).getNumber();
			}
			bw.write("    <arc id=\"arc_" + i + "\" " +
					"source=\"" + source + "\" " +
					"target=\"" + target + "\">\n");
			...
			if (withSpline) {
				...
				for (index = 0; index < set.length; index++) {
					...
					bw.write("                <" + tag + " x=\"" + (((int) set[index].x) + X) +
							"\" " +
							"y=\"" + (((int) set[index].y) + Y) + "\" />\n");
				}
				bw.write("            </spline>\n");
	...
	private static int writePlaces(boolean PNKernel, Subgraph graph, BufferedWriter bw,
		...
		while (e.hasMoreElements()) {
			...
			if (el.object != null && el.object instanceof Place) {
				...
				bw.write("    <place id=\"place_" + p.getNumber() + "\">\n" +
						"        <graphics>\n" +
						"            <position" + (PNKernel ? " " + (PNKernel ? " page=\"1\" " : "") +
						" " : "") + " x=\"" + (x + X) + "\" y=\"" + (y + Y) +
						"\" />\n" +
			...
			if (el.object != null && el.object instanceof Transition) {
				...
				if (!PNKernel) {
					...
					if (t.getLogEvent() != null) {
						bw.write("            <logevent>\n" +
								"                <name>" +
								StringNormalizer.escapeXMLCharacters(t.getLogEvent().getModelElementName()) +
								"</name>\n" +
								"                <type>" + StringNormalizer.escapeXMLCharacters(t.getLogEvent().getEventType()) +
								"</type>\n" +
								"            </logevent>\n");
					}
					bw.write("        </toolspecific>\n");
