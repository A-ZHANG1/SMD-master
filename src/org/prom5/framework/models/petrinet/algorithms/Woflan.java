...
public class Woflan {
	 ...
	 * @param theFileName The full path to the file containing the Petri net to be loaded.
	...
	/**
	 * Unload a loaded net.
	 * @param theNet The handle of the net to be unloaded.
	 * @return Not used.
	 */
	 ...
	 * @param theNet Handle to the loaded net
	 * @param theInfo The kind of info requested, see below.
	 * @param theIndex Some requests require an additional index, see below.
	 * @param theSubIndex Some requests also require a second index, see below.
	...
	public int InfoNofNotPInv = 502; // No  Number of places not covered by P-Invariants
	...
	public int InfoNofNotSPIn = 602; // No  Number of places not covered by semi-positive P-Invariants
	...
	static {
		...
		if(pathToFile.endsWith(File.separator)==false) {
			...
			try {
				...
				Message.add("Woflan JNI DLL loaded from: " + pathToFile, Message.DEBUG);
			} catch (Throwable t) {
				Message.add("Error while loading Woflan: " + t.getMessage(), Message.ERROR);
				Message.add(pathToFile + " could not be found", Message.ERROR);
			}
		} else {
			// signal missing library
			Message.add("Could not load Woflan JNI library: platform not supported!", Message.ERROR);
		}
	}
