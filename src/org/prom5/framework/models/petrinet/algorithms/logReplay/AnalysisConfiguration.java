 ...
 * {@link #getOptionsPanel getOptionsPanel} method, which can be used to display the
 ...
 * <br>
 * Note that the public methods are <code>synchronized</code> as the configuration object may
...
public class AnalysisConfiguration implements Cloneable {
	...
	private boolean mySelectionState = true; // whether this option has been chosen or not
	private ArrayList<AnalysisConfiguration> myChildConfigurations = new ArrayList<AnalysisConfiguration>(); // subordinate options

	/**
	 * The results needed to initialize the GUI (only used for top level categories).
	 * A set of AnalysisResult objects.
	 */
	private Set<AnalysisResult> myResultObjects = new HashSet<AnalysisResult>();

	/**
	 * The methods that are requested to provide their results (only used for top level categories).
	 * A set of AnalysisMethodEnum objects.
	 */
	private HashSet<AnalysisMethodEnum> myRequestedMethods = new HashSet<AnalysisMethodEnum>();

	/**
	 * The method for which this option is relevant, that is, it would either collect
	 * or not collect data related to this option (only used for metrics).
	 * 
	 * TODO - merge this field with the one above (check whether top level categories indeed need 
	 * to be treated differently here
	 */
	 ...
	 * category.
	 * This only used for top level categories as they get a seperate tab in the
	 * results frame, and need to initialize their GUI classes with the analysis
	 * results obtained.
	...
	public synchronized void addAnalysisResult(AnalysisResult result) {
		...
		// TODO - check if this works, otherwise make copy of analysis configuration 
		...
		while (it.hasNext()) {
			...
			if (result != null && current.getClass().getName().equals(result.getClass().getName()) == true) {
	...
	public synchronized Set<AnalysisResult> getChildrenResultObjects() {
		Iterator<AnalysisConfiguration> children = this.getChildConfigurations().iterator();
	...
	/**
	 * Registers this top level category to be provided by the results from
	 * the specified analysis method.
	 * Synchronized access guaranteed as each {@link AnalysisMethod AnalysisMethod}
	 * is executed in a seperate thread and their {@link CustomSwingworker CustomSwingworker}
	 ...
	 * @param methodEnum the AnalysisMethodEnum of the analysis method to register for
	 */
	 ...
	 * the results for the given analysis method.
	 * Synchronized access guaranteed as each {@link AnalysisMethod AnalysisMethod}
	 * is executed in a seperate thread and their {@link CustomSwingworker CustomSwingworker}
	 * asks for registered configuration objects in order to add the respective
	 * {@link AnalysisResult AnalysisResult} object.
	 * @see #addRequestedMethod
	 * @param methodEnum the identifier of the analysis method
	 * @return <code>true</code> if the results of this method have been requested,
	 ...
	 * @param newChild the new sub-ordinate analysis option
	 */
	public synchronized void addChildConfiguration(AnalysisConfiguration newChild) {
	...
	/**
	 * Recursively retrieves all those enabled analysis options that require
	 * the given type of analysis method to be performed.
	 * @param enabledOptions the list of the enabled options found for that analysis method so far
	 * @param analysisMethod the type of analysis method for which the list of enabled options is requested
	 * @return ArrayList the extended list, containing all the enabled child options which require that analysis method
	 */
	public synchronized ArrayList<AnalysisConfiguration> getEnabledOptionsForAnalysisMethod(ArrayList<AnalysisConfiguration> enabledOptions,
		...
		if (this.isSelected() == true) {
			...
			while (allChilds.hasNext()) {
				AnalysisConfiguration currentChild = (AnalysisConfiguration) allChilds.next();
				enabledOptions = currentChild.getEnabledOptionsForAnalysisMethod(enabledOptions,
								 analysisMethod);
			}
		}
	...
	/**
	 * Traverses the configuration option hierarchy in order to obtain the maximum number
	 * of levels in the tree.
	 * @param currentLevel indicates the current level of traversal (initially 0 should be passed)
	 * @param currentMaxDepth indicates the maximum depth so far encountered in the recursive process
	 * @return <code>0</code> if there is no element below this one,
	 * <code>1</code> if there are subordinate elements that do not have children,
	 * <code>2</code> if at least one of the subordinates has another child element
	...
	/**
	 * Traverses the configuration option hierarchy in order to determine the number of
	 * all subordinate child nodes (including leaf nodes).
	 ...
	 * @param optionsPanel the <code>JPanel</code> that should be filled with GUI elements
	 * @param currentLevel the current hierarchical level (initially 0 should be passed)
	 * @param currentRow the options row in the panel to be written (initially 0 should be passed)
	 * @param totalDepth the total depth of the configuration option hierarchy
	 * ({@link #getTreeDepth getTreeDepth} can be used to obtain this value)
	 * @return the resulting <code>JPanel</code> component
	 */
	public JPanel getOptionsPanel(JPanel optionsPanel, int currentLevel, int currentRow,
			int totalDepth) {
		...
		if (currentLevel == 0) {
			...
			myCheckbox.addActionListener(new ActionListener() {
				/**
				 * The method to be automatically invoked changing the
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (mySelectionState == true) {
						...
						// call method with false in order to not disable the root level
			...
			// embed description into HTML document (makes the lines wrap in the editor pane)
		...
		while (children.hasNext()) {
			AnalysisConfiguration currentChild = (AnalysisConfiguration) children.next();
			optionsPanel = currentChild.getOptionsPanel(optionsPanel, nextLevel, nextRow,
						   totalDepth);
	...
	/**
	 * Recursively enables all child configuration options (i.e., makes them editable).
	 * Note that this level is also affected by that.
	 */
	...
	/**
	 * Recursively disables all child configuration options (i.e., makes them non-editable).
	 * @param disableParent specifies whether this level is also affected (or the child nodes only)
	 */
	 ...
	 * @param newTextColor the text color to be set (e.g., in hexadecimal)
	 */
	private void setEditorPaneContent(String newTextColor) {
		/**@todo anne: make the actual width depend on the size of the frame (in order to fill all horizontal space)
		 * this will require a calculation of the available space each time the settings window is resized */
		myEditorPane.setText("<html><head></head><body><div style=\"background-color:" +
				myBackgroundColor_HEX +
				"; width:" + 670 + "px; text-align:left; color:" + newTextColor +
	 ...
	 * Note that only the adjacent level of subordineates is returned.
	 ...
	 * Note that the default value is <code>true</code> but that this attribute
	 * is held persistent via the configuration manager.
	 * @see #isEnabled
	 * @return <code>true</code> if currently selected, <code>false</code> otherwise
	 */
	...
	/**
	 * Sets the current selection status of this analysis option.
	 * Note that the default value is <code>true</code> but that this attribute
	 * can be initialized differently using this method.
	 * @see #isEnabled
	 * @return <code>true</code> if currently selected, <code>false</code> otherwise
	 */
	 ...
	 * Furthermore it serves as the label for the corresponding item in the settings window
	 ...
	 * Furthermore it serves as the label for the corresponding item in the settings window
	 * and the result window.
	 * @param name the String containing the name of this kind of analysis option
	 */
	 ...
	 * @return the String containing a brief description of this kind of analysis option
	 */
	 ...
	 * @param toolTip the String containing a brief description of this kind of analysis option
	 */
	...
	/**
	 * Gets the description attribute, which should contain sufficient information about this analysis option
	 * that the user is able to choose for it or not.
	...
	/**
	 * Sets the description attribute, which should contain sufficient information about this analysis option
	 * that the user is able to choose for it or not.
	 * @param description the String describing this kind of analysis option
	 */
	...
	/**
	 * Gets the analysis method attributes, which indicates which kind of technique
	 * must be used in order to make use of this analysis option.
	 * Note that this may be <code>null<code> for configuration objects not representing a concrete metric.
	...
	/**
	 * Adds the analysis method attribute, which indicates which kind of technique
	 * must be used in order to make use of this analysis option.
	 * Note that this may be <code>null<code> for configuration objects not representing a concrete metric.
	 * @param analysisMethod the belonging analysis method
	 */
	 ...
	 * Gets, for example, invoked on the current hierarchical
	 * {@link ConformanceAnalysisSettings#analysisOptions analysisOptions} object
	 ...
	 * <br>
	 * Note that the result objects are not cloned (but thrown away) as the clone
	 * method is intended to be used before invoking the actual analysis.
	 * Note further that the GUI-related members neither are cloned as they
	 * are not needed in the analysis stage anymore.
	 * Overrides {@link Object#clone clone}.
	...
	public Object clone() {
		...
		while (subNodes.hasNext()) {
			AnalysisConfiguration currentChild = (AnalysisConfiguration) subNodes.next();
			newObject.addChildConfiguration((AnalysisConfiguration) currentChild.clone());
		}
		return newObject;
	}
}
