...
public class ReplayedLogReader {
	 ...
	 * They can be accessed via corresponding iterating methods like reset(), 
	 * hasNext(), and next(), and the ordering corresponds to the ordering of the 
	 * log. 
	 */
	protected ArrayList logTraces = new ArrayList();

	/**
	 * Alternative data structure directly accessing all the diagnostic log traces
	 * contained in that log by their ID (i.e., their name).
	 */
	protected HashMap logTraceMap = new HashMap();

	/**
	 * List containing all log traces sorted with respect to the number of process
	 * instances represented by them in a descending manner (i.e., the trace
	 * representing the most process instances comes first, the one representing
	 * the least comes last). If there are two traces representing the same number
	 * of instances, their ordering is arbitrary.
	 */
	protected ArrayList sortedTraceList = new ArrayList();

	/**
	 * Step through the diagnostic log offering access to the diagnostic
	 * log traces via methods like reset(), hasNext(), and next().
	 */
	protected Iterator logIterator;

	/**
	 * Construct an enhanced log reader by wrapping the original one.
	 * Furthermore the diagnostic data structures are initialized by wrapping all process instances
	 * in a diagnostic log trace.
	 * Note that it is assumed here that the log might have been pre-processed
	 * and similar instances got summarized (since they share the same diagnostic results),
	 * i.e., that the log has already been aggregated on a logical level.
	 * @param logReader the original log reader
	 */
	...
	/**
	 * Constructs an enhanced log reader by wrapping the original one.
	 * <br>
	...
	private void init(LogReader logReader) {
		...
		while (it.hasNext()) {
			...
			// TODO - use actual ID to store the traces (in general, it cannot be assumed that the 
			// name of a process instance is unique) to be checked what is better to display to the user
			...
			if (sortedTraceList.size() == 0) {
				...
				while (alreadySortedTraces.hasNext()) {
					ReplayedLogTrace currentAlreadySorted = (ReplayedLogTrace) alreadySortedTraces.next();
					if (newTrace.getNumberOfProcessInstances() > 
							currentAlreadySorted.getNumberOfProcessInstances()) {
						// put it before that (smaller) trace
						sortedTraceList.add(sortedTraceList.indexOf(currentAlreadySorted), newTrace);
						break;
					} else if (alreadySortedTraces.hasNext() == false) {
						// make sure that the trace is put in the end, if it is the so far "smallest" one
						sortedTraceList.add(newTrace);
						break;
					}
				}
	 ...
	 * @param pi the process instance to be wrapped
	...
	/**
	 * Resets the diagnostic log reader to its start position (i.e., the iterator is placed
	 * in front of the first element).
	 * <br>
	 * Note that here the iteration is not over the original log reader (see 
	 * {@see #getLogReader()} but over the log traces enhanced with diagnostic information. 
	 */
	public void reset() {
	...
	/**
	 * Determines whether there is a log trace left to be read by the diagnostic log reader
	 * (i.e., the iterator is not yet located after the last element).
	 * <br>
	 * Note that here the iteration is not over the original log reader (see 
	 * {@see #getLogReader()} but over the log traces enhanced with diagnostic information. 
	 * @return <code>true</code> if there is a trace to be read, <code>false</code> otherwise
	 */
	...
	/**
	 * Reads the current diagnostic log trace and move to the next one
	 * (i.e., fetch the next element and place the iterator in behind it).
	 * Note that the existance of an element in that position should be checked
	 * before using logicalHasNext().
	 * <br>
	 * Note that here the iteration is not over the original log reader (see 
	 * {@see #getLogReader()} but over the log traces enhanced with diagnostic information. 
	 ...
	 * @param id the ID of the trace (accessible by getName()) in the log
	 * @return the specified diagnostic log trace if found,
	 ...
	 * each trace can represent a number of process instances. The value
	 * is then rounded up and can be retrieved using the method getPerCentage.
	 * @param perCentage the percentage ranging from 1 to 100
	 * @return a list of trace IDs referencing the most frequent instances in the log
	 */
	public ArrayList getMostFrequentTraces(int perCentage) {
		...
		while (sortedTraces.hasNext()) {
			ReplayedLogTrace currentTrace = (ReplayedLogTrace) sortedTraces.next();
	...
	/**
	 * Calculates the (rounded) percentage of process instances represented
	 * by the given log traces with respect to the whole log.
	 * @param selectedTraces the log traces (specified by their ID) for which
	...
	public int getPercentage(ArrayList selectedTraces) {
		...
		while (traceIDs.hasNext()) {
			...
			if (currentTrace != null) {
				selectedNumberOfProcessInstances += currentTrace.getNumberOfProcessInstances();
			}
		}
	 ...
	 * log traces (taking the number of instances represented by each trace
	 * into account).
