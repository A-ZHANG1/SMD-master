...
public class ReplayedPetriNet extends PetriNet {
	...
	/**
	 * This field is used to store the number of invisible tasks for performance reasons,
	 * since the conformance checker asks for whether there are any invisible tasks in the model
	 ...
	 * Initializes the diagnostic data structures for the given caseIDs.
	 *
	 * @param net the Petri net that is re-established in the replayed net
	 * @param caseIDs a list of strings containing the IDs of those instances
	 * that want to store diagnostic information (can be <case>null</case> if not needed)
	 */
	 ...
	 * Needed for subclasses that want to change the type of visualization.
	 *
	 * @param copyTemplate the ReplayedPetriNet serving as a template for this object
	 */
	public ReplayedPetriNet(ReplayedPetriNet copyTemplate) {
		...
		// needs to be extended as soon as there are attributes added to the class..
		//this.numberOfInvisibleTasks = copyTemplate.numberOfInvisibleTasks;
		this.invisibleTasks = this.getInvisibleTasks();
	}

	/**
	 * Re-establishes the structure of the template Petri net for the given target Petri net.
	 * Eigher constructs a ReplayedPetriNet out of an ordinary one and the list of
	 * case IDs of those process instances, for which diagnostic information should be recorded,
	 * or simply clones the given template net (depending on the <code>initialize</code> parameter).
	 * @see reproduceTransition
	 * @see reproducePlace
	 * @see reproduceEdge
	 *
	 * @param from the Petri net serving as a template
	 * @param to the target Petri net
	 * @param initialize <code>true</code> if the new ReplayedPetriNet is initially built
	 * (and therefore diagnostic data structures need to be initialized),
	 * <code>false</code> if the new ReplayedPetriNet should be a clone of the template net
	 * @param caseIDs the list of trace IDs for initializing diagnostic data structures
	 * (only considered if <code>initialize</code> has been set <code>true</code>)
	 */
	private void reproducePetriNetStructure(PetriNet from, ReplayedPetriNet to,
		...
		while (transitions.hasNext()) {
			...
			if (initialize == true) {
				// reproduce transition (polymorphicallly substituted by subclasses)
		...
		while (places.hasNext()) {
			...
			/**@todo anne: merge pure copy functionality with the clone method instead
			 * (should happen on the Petri net level already!) */
			if (initialize == true) {
		...
		while (edges.hasNext()) {
			...
			if (edge.isPT()) {
				 ...
				 * (should happen on the Petri net level already!) */
				if (initialize == true) {
					// reproduce edge (polymorphicallly substituted by subclasses)
					replayedEdge = makeEdge(edge, myPlace, myTransition, to, caseIDs);
					// connection to source and target node has been established already
					to.addEdge(replayedEdge);
				} else {
					replayedEdge = (PNEdge) edge.clone();
					// connection to source and target node needs to be established
			...
			// if transition is source
				...
				if (initialize == true) {
					// reproduce edge (polymorphicallly substituted by subclasses)
					replayedEdge = makeEdge(edge, myTransition, myPlace, to, caseIDs);
					// connection to source and target node has been established already
					to.addEdge(replayedEdge);
				} else {
					replayedEdge = (PNEdge) edge.clone();
	 ...
	 *
	 * @param template the Transition serving as a template for the new Transition
	 * @param targetNet the ReplayedPetriNet to which the new transition belongs to
	 * @param caseIDs the list of trace IDs for initializing diagnostic data structures
	 * @return the reproduced Transition
	 */
	protected Transition makeTransition(Transition template, ReplayedPetriNet targetNet,
			ArrayList caseIDs) {

		// note that it is necessary to use this constructor passing the right subgraph
	 ...
	 * diagnostic data structures instead.
	 *
	 * @param template the Place serving as a template for the new Place
	 * @param targetNet the ReplayedPetriNet to which the new place belongs to
	 * @param caseIDs the list of trace IDs for initializing diagnostic data structures
	 ...
	 * diagnostic data structures instead.
	 *
	 * @param template the PNEdge serving as a template for the new PNEdge
	 * @param sourceNode the Place being the source of the new edge
	 * @param targetNode the Transition being the target of the new edge
	 * @param targetNet the ReplayedPetriNet to which the new place belongs to
	 * @param caseIDs the list of trace IDs for initializing diagnostic data structures
	 ...
	 *
	 * @param template the PNEdge serving as a template for the new PNEdge
	 * @param sourceNode the Transition being the source of the new edge
	 * @param targetNode the Place being the target of the new edge
	 * @param targetNet the ReplayedPetriNet to which the new place belongs to
	 * @param caseIDs the list of trace IDs for initializing diagnostic data structures
	 * @return the reproduced PNEdge
	 */
	 ...
	 * @return <code>true</code> if there are any invisble tasks, <code>false</code> otherwise
	 */
