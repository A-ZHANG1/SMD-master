...
public class ReplayedPlace extends Place {
	 ...
	 * the diagnostic data structures.
	 *
	 ...
	 * Note that this may happen several times during the replay of one single
	 * trace if there are loops in the process model.
	 * Does nothing per default (to be overridden in subclasses if necessary).
	 *
	 * @param tokenAvailable the time at which the token became available in this place
	 * @param transEnabled the time at which the succeeding transition becomes enabled
	 * (which is only different from tokenAvailable if this place is involved in a synchronization)
	 * @param transFired the time at which the token was consumed from this place
	 * @param trace ReplayedLogTrace Process Instance currently replayed
	 */
	 ...
	 * @param trans ReplayedTransition
	 */
	 ...
	 * @param endTime Date time/date at which it is removed
	 * @param trace ReplayedLogTrace Process instance in which it is removed
	 * @param rt ReplayedTransition The transition from which this method is called
	 */
