 ...
 * <p>Title: oWFNet</p>
 *
 * <p>Description: Class for open workflow nets (owF nets)</p>
 *
...
public class oWFNet extends WFNet {
	...
	/*
	 * Maps the communication places (either input or output) to the transitions that are connected to these places.
	 */
	...
	protected void writeCommPlacesToDot(Writer bw) throws IOException {
		...
		for (String name : inputs.keySet()) {
			...
			if (i > 0) {
				bw.write("c" + (i-1) + " -> c" + i + " [style=\"invis\",weight=\"0\"];\n");
			}
			i++;
		}
	...
	protected void writeCommClustersToDot(Writer bw) throws IOException {
		...
		bw.write("style=\"filled\"; fillcolor=\"lightyellow\"; label=\"communication\"");
		int i = 0;
		for (String name : inputs.keySet()) {
			bw.write("c" + i + ";\n");
			i++;
