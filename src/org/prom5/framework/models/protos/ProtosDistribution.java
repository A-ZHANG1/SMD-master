...
/**
 * <p>Title: Protos distribution</p>
 *
 * <p>Description: Holds a Protos distribution</p>
 *
...
public class ProtosDistribution {
	 ...
	 * @param distributionNode Node The "distribution" node that contains the Distribution.
	 * @return String Any error message.
	 */
	public String readXMLExport(Node distributionNode) {
		String msg = "";
		ID = distributionNode.getAttributes().getNamedItem(ProtosString.Id).getNodeValue();
	 ...
	 * @param tag String The tag to use for this Distribution object.
	...
	public String writeXMLExport(String tag) {
		...
		xml += "\">";
		{
			...
			for (Iterator it = distributionDataIDs.iterator(); it.hasNext(); ) {
				...
				xml += ProtosUtil.writeString(ProtosString.GroupRole, distributionDataID);
			}
