...
/**
 * <p>Title: Protos drawing</p>
 *
 * <p>Description: Holds a Protos drawing</p>
 *
...
public class ProtosDrawing {
	...
	private String objectID; //       Ellipse, Line, Rectangle, Triangle, Text (optional, could be NULL)
	...
	/**
	 * Constructs a Drawing object (except for its type) out of a "ellipse", "line", "rectangle", "triangle", or "text" Node.
	 * @param drawingNode Node The "ellipse", "line", "rectangle", "triangle", or "text" node that contains the Drawing.
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			if (node.getNodeName().equals(ProtosString.X0)) {
				...
				for (int j = 1; j < subNodes.getLength(); j++) {
					...
					} else if (node.getNodeName().equals(ProtosString.Underline)) {
	 ...
	 * @param drawingNode Node The "drawing" node that contains the Drawing.
	 * @return String Any error message.
	 */
	 ...
	 * @param tag String The tag to use for this Drawing object.
	...
	public String writeXMLExport(String tag) {
		...
		if (type == DrawingEllipse || type == DrawingRectangle || type == DrawingTriangle ||
				type == DrawingText) {
			xml += ProtosUtil.writeInt(ProtosString.X2, x2);
			xml += ProtosUtil.writeInt(ProtosString.Y2, y2);
		}
		if (type == DrawingEllipse || type == DrawingRectangle || type == DrawingText) {
		...
		if (type == DrawingText) {
			...
			xml += "<" + ProtosString.Font + ">";
			{
				...
				xml += ProtosUtil.writeBool(ProtosString.Underline, fontUnderline);
