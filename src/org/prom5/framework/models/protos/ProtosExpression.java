...
/**
 * <p>Title: Protos expression</p>
 *
 * <p>Description: Holds a Protos expression</p>
 *
...
public class ProtosExpression {
	...
	static int ExpressionEnclosedExpression = 6; // Expression holding other expressions
	static int ExpressionMonadicExpression = 7; // Expression with operator + one expression (< expr1)
	static int ExpressionDyadicExpression = 8; // Expression with operator + two expressions (expr1 >= expr2)
	static int ExpressionChoiceExpression = 9; // Expression for if, then, else construction (if expr1, then expr2, else expr3)
	static int ExpressionFunctionExpression = 10; // Expression with a name and arguments
	...
	private String stringConstant;   // StringConstant, TimeConstant, DataExpression
	...
	private HashMap arguments;       // FunctionExpression (from 0..N-1 to N Expressions)

	public ProtosExpression() {
		expr1 = null; // Do not initialize here, that would lead to endless recursion...
		expr2 = null;
		expr3 = null;
	...
	/**
	 * Constructs an Expression object (except for its type) out of a "monadicExpression" Node.
	 * @param expressionNode Node The "monadicExpression" node that contains the Expression.
	...
	/**
	 * Constructs an Expression object (except for its type) out of a "dyadicExpression" Node.
	 * @param expressionNode Node The "dyadicExpression" node that contains the Expression.
	 * @return String Any error message.
	 */
	...
	/**
	 * Constructs an Expression object (except for its type) out of a "choiceExpression" Node.
	 * @param expressionNode Node The "choiceExpression" node that contains the Expression.
	 * @return String Any error message.
	 */
	...
	/**
	 * Constructs an Expression object (except for its type) out of a "functionExpression" Node.
	 * @param expressionNode Node The "functionExpression" node that contains the Expression.
	 * @return String Any error message.
	 */
	 ...
	 * @param expressionNode Node The expression node that contains the Expression.
	 * @return String Any error message.
	 */
		...
		for (int i=0; i < nodes.getLength(); i++) {
			...
			if (node.getNodeName().equals(ProtosString.BooleanConstant)) {
				...
				type = ExpressionDataExpression;
                                ...
                                stringConstant = ProtosUtil.readString(node);
			} else if (node.getNodeName().equals(ProtosString.EnclosedExpression)) {
				type = ExpressionEnclosedExpression;
				msg += expr1.readXMLExport(node);
			} else if (node.getNodeName().equals(ProtosString.MonadicExpression)) {
			...
			} else if (node.getNodeName().equals(ProtosString.FunctionExpression)) {
	 ...
	 * @param tag String The tag to use for this Expression object.
	...
	public String writeXMLExport(String tag) {
		...
		if (type == ExpressionBooleanConstant) {
			xml += ProtosUtil.writeBool(ProtosString.BooleanConstant, booleanConstant);
		} else if (type == ExpressionIntegerConstant) {
			xml += ProtosUtil.writeInt(ProtosString.IntegerConstant, integerConstant);
		} else if (type == ExpressionFloatConstant) {
			xml += ProtosUtil.writeFloat(ProtosString.FloatConstant, floatConstant);
		} else if (type == ExpressionStringConstant) {
			xml += ProtosUtil.writeString(ProtosString.StringConstant, stringConstant);
		} else if (type == ExpressionTimeConstant) {
			xml += ProtosUtil.writeString(ProtosString.TimeConstant, stringConstant);
		} else if (type == ExpressionDataExpression) {
			xml += ProtosUtil.writeString(ProtosString.DataExpression, stringConstant);
			...
			xml += "<" + ProtosString.FunctionExpression + ">";
			{
				...
				for (int i=0; i < arguments.size(); i++) {
					ProtosExpression expr = (ProtosExpression) arguments.get(new Integer(i));
