...
package org.prom5.framework.models.protos;
...
/**
 * <p>Title: Protos flow element</p>
 *
 * <p>Description: Holds a Protos flow element</p>
 *
...
public class ProtosFlowElement extends ModelGraphVertex {
	...
	private HashMap roleAssignments;                                            // ProtosLinkModelActivity (from formalRole (String) to actualRole (ID))
	private HashMap dataAssignments;                                            // ProtosLinkModelActivity (from formalData (String) to actualData (ID))
	...
	private String executorID;                                                  // Protos*Activity, equal to the ProtosRole variable ID
	...
	private HashSet datas;                                                       // Protos*Activity (contains ProtosActivityDatas), ProtosTrigger (contains IDs)
	private HashSet applications;                                               // Protos*Activity (contains IDs)
	private HashSet parties;                                                    // Protos*Activity (contains ProtosActivityParties), ProtosTrigger (contains IDs)
	private HashSet metaValues;                                                 // Protos*Activity, ProtosTrigger, ProtosBuffer, ProtosStatus (contains ProtosMetaValues)
	...
	public ProtosFlowElement(ProtosSubprocess subprocess, String ID, String name, int type) {
		super(subprocess);
		this.subprocess = subprocess;
	...
	/*
	 * Kept in sake of backwards compatibility. Initially, the ProtosFlowElement and the underlying ModelGraphVertex
	 * were two separate objects, and this methods returned the vertex for the element. Now, both have been unified,
	 * and the method doesn't do much any more. However, it is being used now by several other plug-ins.
	 */
	...
	/**
	 * Usign YAWLTask here ... As a result, this plug-in depends on the YAWL model.
	 * @return int
	 */
	...
	/**
	 * Constructs the Link Model Activity of a Flow Element object out of a "LinkModelActivity" Node.
	 * @param flowElementNode Node The "LinkModelActivity" node that contains the Link Model Activity.
		...
		for (int i=0; i < nodes.getLength(); i++) {
			...
			if (node.getNodeName().equals(ProtosString.Id)) {
				...
				for (int j=0; j < subNodes.getLength(); j++) {
					...
					} else if (subNode.getNodeName().equals(ProtosString.ActualRole)) {
				...
				for (int j=0; j < subNodes.getLength(); j++) {
					...
					if (subNode.getNodeName().equals(ProtosString.FormalData)) {
						formal = ProtosUtil.readString(subNode);
					} else if (subNode.getNodeName().equals(ProtosString.ActualData)) {
						actualID = ProtosUtil.readString(subNode);
					}
				}
				if (formal.length() > 0 && actualID.length() > 0) {
	...
	/**
	 * Constructs the type of an "activity" Flow Element object out of a "type" Node.
	 * @param typeNode Node The "type" node that contains the type.
	 * @return String Any error message.
	 */
	private String readXMLExportType(Node typeNode) {
		...
		for (int i=0; i < nodes.getLength(); i++) {
			...
			if (node.getNodeName().equals(ProtosString.BasicActivity)) {
				...
				type = FlowElementLogisticActivity;
			} else if (node.getNodeName().equals(ProtosString.AuthorisationActivity)) {
				type = FlowElementAuthorisationActivity;
			} else if (node.getNodeName().equals(ProtosString.CommunicationActivity)) {
				type = FlowElementCommunicationActivity;
			} else if (node.getNodeName().equals(ProtosString.ControlActivity)) {
				type = FlowElementControlActivity;
			} else if (node.getNodeName().equals(ProtosString.SubProcessActivity)) {
				type = FlowElementSubProcessActivity;
				subprocessID = ProtosUtil.readString(node);
			} else if (node.getNodeName().equals(ProtosString.LinkModelActivity)) {
				type = FlowElementLinkModelActivity;
	...
	/**
	 * Constructs a Flow Element object (except for its type if not activity) out of an "activity", "trigger", "buffer", or "status" Node.
	 * @param anyNode Node The "activity", "trigger", "buffer", or "status" node that contains the Flow Element.
	 * @return String Any error message.
	 */
	private String readXMLExportAny(Node anyNode) {
		String msg = "";
		ID = anyNode.getAttributes().getNamedItem(ProtosString.Id).getNodeValue();
		...
		for (int i=0; i < nodes.getLength(); i++) {
			...
			if (node.getNodeName().equals(ProtosString.Name)) {
			...
			} else if (node.getNodeName().equals(ProtosString.SameExecutorOperator)) {
				sameExecutorOperator = ProtosUtil.readString(node);
			} else if (node.getNodeName().equals(ProtosString.SameExecutorActivity)) {
				sameExecutorActivityID = ProtosUtil.readString(node);
			} else if (node.getNodeName().equals(ProtosString.ActivityDistribution)) {
				activityDistributionID = ProtosUtil.readString(node);
			} else if (node.getNodeName().equals(ProtosString.SameActivityDistributionOperator)) {
				sameActivityDistributionOperator = ProtosUtil.readString(node);
			} else if (node.getNodeName().equals(ProtosString.SameActivityDistributionActivity)) {
				sameActivityDistributionActivityID = ProtosUtil.readString(node);
			} else if (node.getNodeName().equals(ProtosString.Responsible)) {
				responsibleID = ProtosUtil.readString(node);
			} else if (node.getNodeName().equals(ProtosString.SameResponsibleOperator)) {
				sameResponsibleOperator = ProtosUtil.readString(node);
			} else if (node.getNodeName().equals(ProtosString.SameResponsibleActivity)) {
				sameResponsibleActivityID = ProtosUtil.readString(node);
			} else if (node.getNodeName().equals(ProtosString.SimulationAllocate)) {
			...
			} else if (node.getNodeName().equals(ProtosString.SimulationFrequency)) {
				...
				datas.add(data);
			} else if (node.getNodeName().equals(ProtosString.ActivityApplication)) {
	 ...
	 * @param flowElementNode Node The node that contains the Flow Element.
	 * @return String Any error message.
	 */
	 ...
	 * @param tag String The tag to use for this Activity object.
		...
		xml += "\">";
		{
			...
			xml += "<" + ProtosString.Type + ">";
			{
				if (type == FlowElementBasicActivity) {
					...
					xml += ProtosUtil.writeString(ProtosString.SubProcessActivity, subprocessID);
					...
					xml += "<" + ProtosString.LinkModelActivity + ">";
					{
						...
						for (Iterator it = roleAssignments.keySet().iterator(); it.hasNext(); ) {
							xml += "<" + ProtosString.RoleAssignment + ">";
							{
								String formal = (String) it.next();
								String actual = (String) roleAssignments.get(formal);
								xml += ProtosUtil.writeString(ProtosString.FormalRole, formal);
								xml += ProtosUtil.writeString(ProtosString.ActualRole, actual);
							}
							xml += "</" + ProtosString.RoleAssignment + ">";
						}
						for (Iterator it = dataAssignments.keySet().iterator(); it.hasNext(); ) {
							xml += "<" + ProtosString.DataAssignment + ">";
							{
								String formal = (String) it.next();
								String actual = (String) dataAssignments.get(formal);
								xml += ProtosUtil.writeString(ProtosString.FormalData, formal);
								xml += ProtosUtil.writeString(ProtosString.ActualData, actual);
							}
							xml += "</" + ProtosString.DataAssignment + ">";
						}
					}
			...
			xml += "</" + ProtosString.Type + ">";
			xml += ProtosUtil.writeBool(ProtosString.StartProcess, startProcess);
			...
			xml += ProtosUtil.writeBool(ProtosString.UserInitiative, userInitiative);
			xml += ProtosUtil.writeString(ProtosString.Iteration, iteration);
			xml += ProtosUtil.writeBool(ProtosString.BatchSpecified, batchSpecified);
			xml += ProtosUtil.writeInt(ProtosString.BatchSize, batchSize);
			xml += ProtosUtil.writeInt(ProtosString.BatchElapse, batchElapse);
			xml += ProtosUtil.writeBool(ProtosString.BatchSelective, batchSelective);
			xml += ProtosUtil.writeStringIfNonEmpty(ProtosString.Executor, executorID);
			xml += ProtosUtil.writeStringIfNonEmpty(ProtosString.SameExecutorOperator, sameExecutorOperator);
			xml += ProtosUtil.writeStringIfNonEmpty(ProtosString.SameExecutorActivity, sameExecutorActivityID);
			xml += ProtosUtil.writeStringIfNonEmpty(ProtosString.ActivityDistribution, activityDistributionID);
			xml += ProtosUtil.writeStringIfNonEmpty(ProtosString.SameActivityDistributionOperator, sameActivityDistributionOperator);
			xml += ProtosUtil.writeStringIfNonEmpty(ProtosString.SameActivityDistributionActivity, sameActivityDistributionActivityID);
			xml += ProtosUtil.writeStringIfNonEmpty(ProtosString.Responsible, responsibleID);
			xml += ProtosUtil.writeStringIfNonEmpty(ProtosString.SameResponsibleOperator, sameResponsibleOperator);
			xml += ProtosUtil.writeStringIfNonEmpty(ProtosString.SameResponsibleActivity, sameResponsibleActivityID);
			xml += ProtosUtil.writeInt(ProtosString.SimulationAllocate, simulationAllocate);
			xml += ProtosUtil.writeDouble(ProtosString.SimulationCost, simulationCost);
			xml += ProtosUtil.writeDouble(ProtosString.SimulationFrequency, simulationFrequency);
			...
			xml += ProtosUtil.writeString(ProtosString.AnalysisOut, analysisOut);
			xml += ProtosUtil.writeString(ProtosString.Description, description);
			xml += ProtosUtil.writeString(ProtosString.Workinstruction, workinstruction);
			...
			for (Iterator it = applications.iterator(); it.hasNext(); ) {
				...
				xml += ProtosUtil.writeString(ProtosString.ActivityApplication, application);;
			}
	 ...
	 * @param tag String The tag to use for this Trigger object.
	...
	private String writeXMLExportTrigger(String tag) {
		...
		xml += "\">";
		{
			...
			xml += ProtosUtil.writeDouble(ProtosString.SimulationFrequency, simulationFrequency);
			xml += simulationWaiting.writeXMLExport(ProtosString.SimulationWaiting);
			xml += ProtosUtil.writeString(ProtosString.AnalysisIn, analysisIn);
			xml += ProtosUtil.writeString(ProtosString.AnalysisOut, analysisOut);
			xml += ProtosUtil.writeString(ProtosString.Description, description);
			xml += ProtosUtil.writeString(ProtosString.Workinstruction, workinstruction);
			xml += ProtosUtil.writeStringIfNonEmpty(ProtosString.DeadlineObject, deadlineObjectID);
			for (Iterator it = datas.iterator(); it.hasNext(); ) {
			...
			for (Iterator it = applications.iterator(); it.hasNext(); ) {
				String application = (String) it.next();
				xml += ProtosUtil.writeString(ProtosString.TriggerApplication, application);;
			}
			for (Iterator it = parties.iterator(); it.hasNext(); ) {
	 ...
	 * @param tag String The tag to use for this Buffer or Status object.
	 ...
	 * @param tag String The tag to use for this Flow Element object.
