...
/**
 * <p>Title: Protos group</p>
 *
 * <p>Description: Holds a Protos group</p>
 *
...
public class ProtosGroup {
	 ...
	 * @param groupNode Node The node that contains the Group.
	 * @return String Any error message.
	 */
	public String readXMLExport(Node groupNode) {
		String msg = "";
		ID = groupNode.getAttributes().getNamedItem(ProtosString.Id).getNodeValue();
	 ...
	 * @param tag String The tag to use for this Group object.
	...
	public String writeXMLExport(String tag) {
		...
		xml += "\">";
		{
			...
			for (Iterator it = groupRoleIDs.iterator(); it.hasNext(); ) {
				...
				xml += ProtosUtil.writeString(ProtosString.GroupRole, groupRoleID);
			}
