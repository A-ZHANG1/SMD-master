...
/**
 * <p>Title: Protos meta value</p>
 *
 * <p>Description: Holds a Protos meta value</p>
 *
...
public class ProtosMetavalue {
	 ...
	 * @param metavalueNode Node The node that contains the Metavalue.
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			if (node.getNodeName().equals(ProtosString.MetavalueInteger)) {
				...
				for (int j = 0; j < subNodes.getLength(); j++) {
					...
					if (subNode.getNodeName().equals(ProtosString.WindowPlacement)) {
						windowPlacement = ProtosUtil.readString(subNode);
					} else if (subNode.getNodeName().equals(ProtosString.UrlBase)) {
						urlBase = ProtosUtil.readString(subNode);
					} else if (subNode.getNodeName().equals(ProtosString.ExternalName)) {
						externalName = ProtosUtil.readString(subNode);
					} else if (subNode.getNodeName().equals(ProtosString.InternalName)) {
	 ...
	 * @param tag String The tag to use for this Metavalue object.
	...
	public String writeXMLExport(String tag) {
		...
		xml += "<" + tag + ">";
		{
			if (type == MetavalueInteger) {
				xml += ProtosUtil.writeInt(ProtosString.MetavalueInteger, integerValue);
			} else if (type == MetavalueString) {
				xml += ProtosUtil.writeString(ProtosString.MetavalueString, stringValue);
			} else if (type == MetavalueFloat) {
				xml += ProtosUtil.writeFloat(ProtosString.MetavalueFloat, floatValue);
			} else if (type == MetavalueObject) {
				xml += ProtosUtil.writeString(ProtosString.MetavalueObject, objectValue);
			} else if (type == MetavalueUrl) {
				xml += "<" + ProtosString.MetavalueUrl + ">";
				{
					xml += ProtosUtil.writeString(ProtosString.WindowPlacement, windowPlacement);
					xml += ProtosUtil.writeString(ProtosString.UrlBase, urlBase);
					xml += ProtosUtil.writeString(ProtosString.ExternalName, externalName);
					xml += ProtosUtil.writeString(ProtosString.InternalName, internalName);
				}
				xml += "</" + ProtosString.MetavalueUrl + ">";
				...
				xml += ProtosUtil.writeString(ProtosString.MetavalueEnum, enumValue);
			}
		}
		xml += "</" + tag + ">";
		{
