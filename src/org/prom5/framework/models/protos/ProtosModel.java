...
/**
 * <p>Title: Protos model</p>
 *
 * <p>Description: Holds a Protos model</p>
 *
...
public class ProtosModel extends ModelGraph {
	...
	public HashSet<ProtosSubprocess> getSubprocesses() {
         ...
         * Returns the rolegraph of this Model.
	 ...
	 * @param doc Document The document that contains the Model.
	...
	public String readXMLExport(Document doc) {
		...
		if (!protosModelNode.getNodeName().equals(ProtosString.ProtosModel)) {
			...
			for (int i=0; i < nodes.getLength(); i++) {
				...
				} else if (node.getNodeName().equals(ProtosString.ProtosModelOptions)) {
	 ...
	 * @param tag String The tag to use for this Model object.
	...
	public String writeXMLExport(String tag) {
		...
		String xml = "<" + tag + ">";
		{
			...
			xml += protosModelOptions.writeXMLExport(ProtosString.ProtosModelOptions);
		}
		xml += "</" + tag + ">";
		return xml;
	}
	 ...
	 * @param bw Writer The writer for this file.
