...
/**
 * <p>Title: Protos process arc</p>
 *
 * <p>Description: Holds a Protos process arc</p>
 *
...
public class ProtosProcessArc {
	...
	public ProtosProcessArc(ProtosSubprocess subprocess, String fromID, String toID) {
		this.subprocess = subprocess;
		dataIDs = new HashSet();
		metaValues = new HashSet();
	 ...
	 * @param processArcNode Node The node that contains the Process Arc.
	 * @return String Any error message.
	 */
	public String readXMLExport(Node processArcNode) {
		String msg = "";
		fromID = processArcNode.getAttributes().getNamedItem(ProtosString.From).getNodeValue();
		toID = processArcNode.getAttributes().getNamedItem(ProtosString.To).getNodeValue();
		ProtosFlowElement from = subprocess.getFlowElement(fromID);
		ProtosFlowElement to = subprocess.getFlowElement(toID);
		if (from != null && to != null) {
			edge = new ModelGraphEdge(from.getVertex(), to.getVertex());
		...
		for (int i=0; i < nodes.getLength(); i++) {
			...
			if (node.getNodeName().equals(ProtosString.Type)) {
				...
				label = ProtosUtil.readString(node);
                               ...
                               // ProtosExpression condition = new ProtosExpression(); is changed in:
			        condition = new ProtosExpression(); //by Mariska to make condition work.
	 ...
	 * @param tag String The tag to use for this Process Arc object.
	...
	public String writeXMLExport(String tag) {
		...
		xml += "\" " + ProtosString.To + "=\"" + toID + "\">";
		{
			...
			xml += ProtosUtil.writeDouble(ProtosString.SimFrequency, simulationFrequency);
