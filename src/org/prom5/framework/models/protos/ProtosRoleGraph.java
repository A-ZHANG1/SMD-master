...
/**
 * <p>Title: Protos role graph</p>
 *
 * <p>Description: Holds a Protos role graph</p>
 *
...
public class ProtosRoleGraph extends ModelGraph {
	...
	public ProtosRoleGraph() {
         ...
         * Returns the roles of this RoleGraph.
	 ...
	 * @param roleGraphNode Node The node that contains the Role Graph.
	 * @return String Any error message.
	 */
	public String readXMLExport(Node roleGraphNode) {
		...
		for (int i=0; i < nodes.getLength(); i++) {
			...
			if (node.getNodeName().equalsIgnoreCase(ProtosString.Role)) {
				...
				roles.add(role);
			} else if (node.getNodeName().equalsIgnoreCase(ProtosString.Rolearc)) {
				ProtosRoleArc roleArc = new ProtosRoleArc();
				msg += roleArc.readXMLExport(node);
				roleArcs.add(roleArc);
			} else if (node.getNodeName().equalsIgnoreCase(ProtosString.Drawing)) {
	 ...
	 * @param tag String The tag to use for this Role Graph object.
