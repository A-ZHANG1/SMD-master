...
/**
 * <p>Title: Protos subprocess</p>
 *
 * <p>Description: Holds a Protos subprocess</p>
 *
...
public class ProtosSubprocess extends ModelGraph {
	 ...
	 * @return The flow element associated with the given vertex, null if no flow element associated to it.
	 */
	public ProtosFlowElement getFlowElement(ModelGraphVertex vertex) {
		/*
		 * For the time being, iterate over all flow elements and check whether the vertex matches.
		 * Better might be to change flowElements into a HashMap<ModelGraphVertex, ProtosFlowElement>, which
		 * would make the lookup more straightforward. However, this would require some additional changes.
		 * Let's first see whether this method is sufficient, and worry about performance later on.
		 */
         ...
         * Return the protos process arc that is associated to the given edge.
         * @param edge The given edge.
         * @return The protos process arc associated with the given edge, null if no arc associated to it.
         */
	...
	public HashSet<ProtosFlowElement> getActivities() {
        ...
        public ProtosFlowElement addActivity(String ID, String name) {
            ProtosFlowElement activity = new ProtosFlowElement(this, ID, name, ProtosFlowElement.FlowElementBasicActivity);
	...
	public ProtosFlowElement addStatus(String ID, String name) {
		ProtosFlowElement status = new ProtosFlowElement(this, ID, name, ProtosFlowElement.FlowElementStatus);
    ...
    /**
     * Added by Mariska Netjes
     * Returns the set of flowElements of this Subprocess.
	 ...
	 * @return ProtosFlowElement The flow element with that ID, null if not found.
	 */
	 ...
	 * @param subprocessNode Node The node that contains the Statistical Subprocess.
	 * @return String Any error message.
	 */
	public String readXMLExport(Node subprocessNode) {
		String msg = "";
		ID = subprocessNode.getAttributes().getNamedItem(ProtosString.Id).getNodeValue();
			...
			if (node.getNodeName().equals(ProtosString.Name)) {
				...
				for (int j=0; j < subNodes.getLength(); j++) {
					...
					if (subNode.getNodeName().equals(ProtosString.FlowElement)) {
						ProtosFlowElement flowElement = new ProtosFlowElement(this);
						msg += flowElement.readXMLExport(subNode);
						flowElements.add(flowElement);
					} else if (subNode.getNodeName().equals(ProtosString.ProcessArc)) {
						ProtosProcessArc processArc = new ProtosProcessArc(this);
						msg += processArc.readXMLExport(subNode);
						processArcs.add(processArc);
					} else if (subNode.getNodeName().equals(ProtosString.Drawing)) {
	 ...
	 * @param tag String The tag to use for this Subprocess object.
	...
	public String writeXMLExport(String tag) {
		...
		xml += "\">";
		{
			...
			xml += ProtosUtil.writeBool(ProtosString.ManualLayout, manualLayout);
