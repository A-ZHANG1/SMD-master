 ...
 *
 */
package org.prom5.framework.models.recommendation;
...
/**
 * A simple data structure for storing recommendations,
 * i.e. that a specific task should be executed and the confidence
 * with which this is recommended. Optionally, the recommendation
 * may also contain the resource specification for which it applies.
 *
...
public class Recommendation implements Comparable<Recommendation> {
	 ...
	 * @param aRationale human-readable justification or explanation string
	 */
...
//	}
	...
	public double getDoVariance() {
		return getDoExpectedSquaredValue() - getDoExpectedValue()* getDoExpectedValue();
	}

	public double getDontVariance() {
		return getDontExpectedSquaredValue() - getDontExpectedValue()* getDontExpectedValue();
	}

	public double get5PercentConfidenceIntervalForDo() {
		return 1.960*Math.sqrt(getDoVariance()/getDoWeight());
	}
	/* z.1	    z.05	z.025	z.01	z.005	z.001	z.0005
	...
	public String getDoString() {
		...
		} else if (Double.toString(get5PercentConfidenceIntervalForDo()).equalsIgnoreCase(NAN)) {
			return String.format(LOCALE, getDoExpectedValue());
		} else 	{
			return String.format(FULLLOCALE, getDoExpectedValue(),get5PercentConfidenceIntervalForDo());
		}
	}
	...
	public String getDontString() {
		if (Double.toString(dontExpectedValue).equalsIgnoreCase(NAN)) {
			return "unknown";
		} else if (Double.toString(get5PercentConfidenceIntervalForDont()).equalsIgnoreCase(NAN)) {
			return String.format(LOCALE,getDontExpectedValue());
		} else {
			return String.format(FULLLOCALE,getDontExpectedValue(),get5PercentConfidenceIntervalForDont());
		}
	}
...
//				") , confidence: " + getConfidence() +", weight: " + getWeight();
	...
	public boolean equals(Object o) {
		...
		return eventType.equals(rec.eventType) &&
				groups.equals(rec.groups) &&
				roles.equals(rec.roles) &&
				task.equals(rec.task) &&
	 ...
	 * recommendation. This means that it does <b>NOT</b> necessarily coincide with the
	 * <code>equals()</code> method's results!
	 */
	public int compareTo(Recommendation rec) {
		...
		if (Double.toString(doExpectedValue).equalsIgnoreCase(NAN) && Double.toString(rec.doExpectedValue).equalsIgnoreCase(NAN)) {
			...
			return Double.compare(get5PercentConfidenceIntervalForDo(), rec.get5PercentConfidenceIntervalForDo());
		}
	}

	public void addToDoRecommendation(double expectedDoValue, double expectedDoSquaredValue, double expectedDoWeight)  {
		double doW = doWeight+ expectedDoWeight;
		if (doW > 0) {
			this.setDoExpectedValue((  (doWeight>0 ?  doExpectedValue*doWeight : 0) +
					 (expectedDoWeight>0 ? expectedDoValue *expectedDoWeight:0)) / doW);
			this.setDoExpectedSquaredValue(( (doWeight>0 ? doExpectedSquaredValue *doWeight : 0) +
					 (expectedDoWeight>0 ? expectedDoSquaredValue *expectedDoWeight:0)) / doW);
			this.setDoWeight(doW);
		}
	}

	public void addToDontRecommendation(double expectedDontValue, double expectedDontSquaredValue, double expectedDontWeight) {
		double dontW = this.getDontWeight()+ expectedDontWeight;
		if (dontW > 0) {
			this.setDontExpectedValue(((dontWeight>0 ? dontExpectedValue * dontWeight : 0) +
					 (expectedDontWeight>0 ? expectedDontValue *expectedDontWeight:0)) / dontW);
			this.setDontExpectedSquaredValue((  (dontWeight>0 ?dontExpectedSquaredValue * dontWeight : 0) +
					(expectedDontWeight>0 ? expectedDontSquaredValue * expectedDontWeight:0)) / dontW);
			this.setDontWeight(dontW);
		}
	}

	public void addDoToDoRecommendation(Recommendation toAdd)  {
		this.addToDoRecommendation(toAdd.doExpectedValue,toAdd.doExpectedSquaredValue, toAdd.doWeight);
	}

	public void addDontToDontRecommendation(Recommendation toAdd)  {
		this.addToDontRecommendation(toAdd.dontExpectedValue,toAdd.dontExpectedSquaredValue, toAdd.dontWeight);
	}

	public void addDoToDontRecommendation(Recommendation toAdd) {
		this.addToDontRecommendation(toAdd.doExpectedValue,toAdd.doExpectedSquaredValue, toAdd.doWeight);
	}

	public void addRecommendation(Recommendation toAdd) throws NotEqualRecommendationException {
		if (!this.equals(toAdd)) {
			// Problem
			throw new NotEqualRecommendationException("Can only add recommendations that are the same.");
		}
		addToDoRecommendation(toAdd.doExpectedValue,toAdd.doExpectedSquaredValue, toAdd.doWeight);
		addToDontRecommendation(toAdd.dontExpectedValue,toAdd.dontExpectedSquaredValue, toAdd.dontWeight);
	}
}
