...
import org.prom5.framework.log.classic.ProcessInstanceClassic;
import org.prom5.framework.models.recommendation.net.RecommendationQueryMarshal;

/**
 * Data structure for storing a query requesting recommendation
 * for scheduling in a process management system.
 *
...
public class RecommendationQuery {
	 ...
	 */
	public RecommendationQuery(String queryId, String aProcessId, String aProcessInstanceId) {
		id = queryId;
	...
	public RecommendationQuery(String aProcessId, String aProcessInstanceId) {
		...
		id = "QUERY_"
			 + RecommendationQuery.hostId + "_"
			 + Long.toHexString(System.currentTimeMillis());
	}

	/**
	 * @param data the data, i.e. attribute, map of the instance which
	 ...
	 * @param group string representation of a group
	 */
	...
	/**
	 * Adds a user to the set of available resources
	...
	/**
	 * Adds a task to the set of available tasks
	 * to be used for recommendation.
	 * @param task string representation of a task
	 */
	...
	/**
	 * Adds an audit trail entry, or event, to the current
	 * incomplete audit trail of the process instance for which
	 * the query is being issued.
	 * @param ate an audit trail entry (event).
	 */
	 ...
	 *  of this query (used for correlation)
	 */
	public String getId() {
		return id;
	}
	...
	/**
	 * @return the unique ID of the process this
	 *  query refers to
	 */
	...
	/**
	 * @return the unique ID of the process instance
	...
	/**
	 * @return A sorted set of strings specifying the users
	 *  on which the result should be filtered
	 */
	...
	/**
	 * @return A sorted set of strings specifying the roles
	 *  on which the result should be filtered
	 */
	...
	/**
	 * @return A sorted set of strings specifying the groups
	 *  on which the result should be filtered
	 */
	...
	/**
	 * @return A sorted set of strings specifying the tasks
	 *  on which the result should be filtered
	 */
	...
	/**
	 * @return The current, incomplete, audit trail
	 *  (i.e. sequence of events) of the process instance
	 *  for which the query is issued
	 */
	...
	/**
	 * @return The data, i.e. attribute, map of the process
	 *  instance this query refers to
	 */
	...
	public ProcessInstance toProcessInstance(String process) {
		return new ProcessInstanceClassic(process, "undefined", "undefined",
				new HashMap<String, String>(), (ArrayList<AuditTrailEntry>) (getAuditTrail()));
	}
