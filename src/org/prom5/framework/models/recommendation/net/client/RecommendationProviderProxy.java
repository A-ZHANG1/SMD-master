 ...
 *
 */
package org.prom5.framework.models.recommendation.net.client;
...
import org.prom5.framework.models.recommendation.net.RecommendationCompletedTraceMarshal;
import org.prom5.framework.models.recommendation.net.RecommendationQueryMarshal;
import org.prom5.framework.models.recommendation.net.RecommendationQueryProtocol;
import org.prom5.framework.models.recommendation.net.RecommendationRestartMarshal;
import org.prom5.framework.models.recommendation.net.RecommendationResultMarshal;
...
public class RecommendationProviderProxy implements RecommendationProvider {
	...
	/* (non-Javadoc)
	 * @see org.processmining.framework.models.recommendation.RecommendationProvider#getRecommendation(org.processmining.framework.models.recommendation.RecommendationQuery)
	 */
	public RecommendationResult getRecommendation(RecommendationQuery query) throws Exception,
		...
		if (line.equals(RecommendationQueryProtocol.HELO_SERVER_PROLOG) == false) {
			throw new Exception("Fatal error: Remote is no recommendation server! (HELO mismatch!)");
		} while ((line = in.readLine()).equalsIgnoreCase(
						 RecommendationQueryProtocol.HELO_SERVER_EPILOG) == false) {
		...
		while ((line = in.readLine()).equalsIgnoreCase(
					   RecommendationQueryProtocol.SERVER_REQUEST_XML_RECEIVED) == false) {
			// wait for receipt
		}
		// wait for outcome
		while ((line = in.readLine()).equalsIgnoreCase(
					   RecommendationQueryProtocol.SERVER_RESPONSE_POSITIVE) == false) {
			if (line.equalsIgnoreCase(RecommendationQueryProtocol.SERVER_RESPONSE_NEGATIVE)) {
				// error. print message to stderr...
				String errorMessage = "Remote server error:\n";
				System.err.println("Remote server at " + host + ":" + port +
						" reported a fatal error:");
				while ((line = in.readLine()).equalsIgnoreCase(RecommendationQueryProtocol.
		...
		while ((line = in.readLine()).equalsIgnoreCase(RecommendationQueryProtocol.SERVER_GOODBYE) == false) {
			// wait for server goodbye
		}
		// close connection
		socket.close();
		...
		RecommendationResult result = resultMarshal.unmarshal(resultStringBuilder.toString());
		return result;
	}

	/* (non-Javadoc)
	 * @see org.processmining.framework.models.recommendation.RecommendationProvider#signalPickedResult(org.processmining.framework.models.recommendation.RecommendationResult, int)
	 */
	...
	/**
	 * <p>This method transmits a request to the server to restart with a new
	 * contributor and a new scale</p>
	 * @param contributor is the full classname of the contributor, as shown in the recommendations.ini file of ProM
	 * @param scale is the full classname of the contributor, as shown in the scales.ini file of ProM
	 */
	public void requestRestart(String contributor, String scale) throws Exception {
				...
				writeLine(out, RecommendationQueryProtocol.CLIENT_REQUEST_RESTART_PROLOG);
				writeLine(out, reqXML);
				writeLine(out, RecommendationQueryProtocol.CLIENT_REQUEST_RESTART_EPILOG);
				// wait for receipt confirmation
				while ((line = in.readLine()).equalsIgnoreCase(
							   RecommendationQueryProtocol.SERVER_REQUEST_RESTART_RECEIVED) == false) {
					// wait for receipt
				}
				// send goodbye
				writeLine(out, RecommendationQueryProtocol.CLIENT_GOODBYE);
	 ...
	 */
	public void handleCompletedExecution(ProcessInstance instance) throws Exception {
		// connect to socket
		socket = new Socket(host, port);
		socket.setKeepAlive(true);
		socket.setSoTimeout(0);
		...
		// wait for receipt confirmation
		while ((line = in.readLine()).equalsIgnoreCase(
					   RecommendationQueryProtocol.SERVER_TRANSMIT_TRACE_RECEIVED) == false) {
			// wait for receipt
		}
		// send goodbye
		writeLine(out, RecommendationQueryProtocol.CLIENT_GOODBYE);
	...
	public void requestClose() throws Exception {
		...
		// wait for receipt confirmation
		while ((line = in.readLine()).equalsIgnoreCase(
					   RecommendationQueryProtocol.SERVER_REQUEST_CLOSE_RECEIVED) == false) {
			// wait for receipt
		}
