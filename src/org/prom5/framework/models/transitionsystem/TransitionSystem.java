...
public class TransitionSystem extends AcceptFSM {
    ...
    public TransitionSystemVertex containsVertex(TransitionSystemVertex
                                                 newVertex) {
    ...
    public TransitionSystemVertex addVertexQuick(TransitionSystemVertex
                                                 newVertex) {
        super.addVertex(newVertex);
        return newVertex;
    }
    ...
    public TransitionSystemEdge addEdge(TransitionSystemEdge newEdge) {
        TransitionSystemVertex v1 = (TransitionSystemVertex) newEdge.getSource();
        ...
        if ((v1 != v2) && (edges.size() > 1)) { //1 - created edge
            // The edge has been created, hence the nodes v1 and v2 know about it.
        ...
        } else if ((v1 == v2) && (edges.size() >= 2)) { //a tricky check for the case of loops
            Iterator it = edges.iterator();
