package org.prom5.framework.models.transitionsystem;
...
public class TransitionSystemEdge extends FSMTransition
{
	...
	public TransitionSystemEdge(Collection<String> docs, TransitionSystemVertex source, TransitionSystemVertex destination)
	{
	...
	public TransitionSystemEdge(String identifier, TransitionSystemVertex source, TransitionSystemVertex destination)
	{
		super(source, destination, "");
	...
	void makeIdentifier()
	{
		...
		while(it.hasNext())
		{
			st = st.concat(it.next()).concat(PetrifyConstants.EDGEDOCSSEPARATOR);
		}
		st = st.substring(0, st.length() - PetrifyConstants.EDGEDOCSSEPARATOR.length());
		setIdentifier(st);
	}
