...
/**
 * <p>Title: YAWL Decomposition</p>
 * <p>Description: Holds an imported YAWL Decomposition</p>
...
public class YAWLDecomposition extends ModelGraph {
    ...
    private HashMap<String, YAWLNode> nodes = new HashMap(); // Maps node names to nodes
     ...
     * @param isRootNet Wether it is the root ("true") or not (anything else)
     ...
     * @param fromNode YAWLNode The given firs tnode.
    ...
    public YAWLTask addTask(String name, String join, String split,
        ...
        int joinType = join.equals("and") ? YAWLTask.AND : join.equals("xor") ?
                       YAWLTask.XOR :
                       join.equals("or") ? YAWLTask.OR : YAWLTask.NONE;
        int splitType = split.equals("and") ? YAWLTask.AND : split.equals("xor") ?
                        YAWLTask.XOR :
    ...
    public YAWLEdge addEdge(String fromName, String toName,
                            boolean isDefaultFLow, String predicate,
                            String ordering) {
        YAWLNode fromNode = (YAWLNode) nodes.get(fromName);
        YAWLNode toNode = (YAWLNode) nodes.get(toName);
        //nodes.remove(fromName);
        //nodes.remove(toName);
        YAWLEdge edge = new YAWLEdge(fromNode, toNode, isDefaultFLow, predicate,
                                     ordering); // Presence of extra parameters results in normal edge
        addEdge(edge);
    ...
    public void addResetEdge(String fromName, String toName) {
        ...
        //nodes.remove(fromName);
        //nodes.remove(toName);
        YAWLEdge edge = new YAWLEdge(fromNode, toNode); // Absence of extra parameters result in a reset edge
        addEdge(edge);
        //nodes.put(fromName, fromVertex);
        //nodes.put(toName, toVertex);
    }
    ...
    public void writeToDot(Writer bw, YAWLModel model) throws IOException {
        ...
        // A YAWL task might have a decomposition. Only now, we know the entire model and can we check for this.
        setDotAttributes();
    ...
    public void Test() {
        ...
        Message.add("  <Decomposition id=\"" + id +
                    "\" nofTasks=\"" + nofTasks +
                    "\" nofConditions=\"" + nofConditions +
                    "\" nofUnknowns=\"" + nofUnknowns +
                    "\" nofArcs=\"" + getEdges().size() +
                    "\"/>",
                    Message.TEST);
