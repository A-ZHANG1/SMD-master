...
package org.prom5.framework.models.yawl;
...
/**
 * <p>Title: YAWL Task</p>
 * <p>Description: Holds a YAWL task</p>
...
public class YAWLTask extends YAWLNode {
	 ...
	 * @param joinType The given join type
	 * @param splitType The given split type
	 * @param decomposesTo The name of the subdecomposition
	 * @param id X[\tY], where X is the task name, and Y makes the id unique.
	 */
	public YAWLTask(ModelGraph g, int joinType, int splitType, String decomposesTo, String id,
			LogEvent event) {
		super(g, id);
	...
	public YAWLTask(ModelGraph g, YAWLTask task) {
		super(g, task.id);
		init(task.joinType, task.splitType, task.decomposesTo, task.id, task.event);
	}

	private void init(int joinType, int splitType, String decomposesTo, String id,
			LogEvent event) {
		...
		String label = name + "\\n" + decomposesTo; // Stack name and name of subdecomposition as label
	...
	/**
	 * Get the join type. Used by att.grappa.yawl.SplitJoinTask to determine which symbol to draw for join and split.
	...
	/**
	 * Get the split type. Used by att.grappa.yawl.SplitJoinTask to determine which symbol to draw for join and split.
	...
	public void setDotAttributes(YAWLModel model) {
		if (model.isComposite(decomposesTo)) {
			setDotAttribute("shapefile", "att.grappa.yawl.SplitJoinCompositeTask");
		} else {
	...
	public String writeToYAWL(int phase) {
		...
		if (phase == 1) {
			...
			s +="\t\t\t\t\t<join code=\"" + (jt == AND ? "and" : (jt == OR ? "or" :
					"xor")) + "\"/>\n";
			s +="\t\t\t\t\t<split code=\"" + (st == AND ? "and" : (st == OR ?
					"or" :
					"xor")) + "\"/>\n";
            ...
            /*
             * Eric Verbeek, February 11, 2008.
             * Ticket #447: An empty decomposition should not be exported.
             */
