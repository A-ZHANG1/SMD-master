...
package org.prom5.framework.models.yawl.algorithms;
...
/**
 * <p>Title: YAWL parser/reader</p>
 * <p>Description: Parses and reads the incoming YAWL file, and loads the corresponding model into the ProM framework.</p>
...
public class YAWLReader {
	...
	public static YAWLModel read(InputStream input) throws Exception {
		...
		model = new YAWLModel(specificationNode.getAttributes().getNamedItem("uri").getNodeValue());
	 ...
	 */
	public static void parseDecomposition(Node rootNode, YAWLModel model, boolean isRootNet) {
		...
		YAWLDecomposition decomposition = new YAWLDecomposition(
										  ...
										  xsiTypeNode == null ? "<unknown>" :
										  xsiTypeNode.getNodeValue());
		model.addDecomposition(idNode == null ? "<unknown>" : idNode.getNodeValue(), decomposition);
	 ...
	 */
	public static void parseProcessControlElements(Node rootNode, YAWLDecomposition decomposition) {
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			if (node.getNodeName().equals("inputCondition") ||
				...
				// Found a task, add all normal outgoing edges and all reset outgoing edges
				parseFlowsInto(node, decomposition);
	 ...
	 * @param decomposition The given decomposition
	 */
	public static void parseInputCondition(Node rootNode, YAWLDecomposition decomposition) {
		Node node = rootNode.getAttributes().getNamedItem("id");
		decomposition.addInputCondition(node == null ? "" : node.getNodeValue());
	}
	 ...
	 * @param decomposition The given decomposition
	 */
	public static void parseOutputCondition(Node rootNode, YAWLDecomposition decomposition) {
		Node node = rootNode.getAttributes().getNamedItem("id");
		decomposition.addOutputCondition(node == null ? "" : node.getNodeValue());
	}
	 ...
	 * @param decomposition The given decomposition
	 */
	public static void parseCondition(Node rootNode, YAWLDecomposition decomposition) {
		Node node = rootNode.getAttributes().getNamedItem("id");
	...
	public static void parseTask(Node rootNode, YAWLDecomposition decomposition) {
		...
		// Add the task
		YAWLTask task = decomposition.addTask(idNode.getNodeValue(),
				joinNode == null ? "xor" : joinNode.getNodeValue(),
				splitNode == null ? "and" : splitNode.getNodeValue(),
				decomposesToNode == null ? "" : decomposesToNode.getNodeValue(), null);
	 ...
	 * @param decomposition The given decomposition
	 */
	public static void parseFlowsInto(Node rootNode, YAWLDecomposition decomposition) {
				...
				if (node.getNodeName().equals("flowsInto")) {
					...
					for (int j = 0; j < nodes2.getLength(); j++) {
						...
						if (node2.getNodeName().equals("nextElementRef")) {
							...
							predicateNode = node2.getFirstChild(); // Get the implicit text node
							orderingNode = node2.getAttributes().getNamedItem("ordering");
						}
					}

					// Add the edge, provided its destination has been found
					if (toNode != null) {
						decomposition.addEdge(fromNode.getNodeValue(),
								toNode.getNodeValue(),
								isDefaultFlowNode != null,
								predicateNode == null ? null : predicateNode.getNodeValue(),
								orderingNode == null ? null : orderingNode.getNodeValue());
					}
				}
			}
		}
	 ...
	 * @param decomposition The given decomposition
	 */
	public static void parseRemovesTokens(Node rootNode, YAWLDecomposition decomposition) {
		Node fromNode = rootNode.getAttributes().getNamedItem("id");
		Node toNode;
		if (fromNode != null) {
			NodeList nodes = rootNode.getChildNodes();
			for (int i = 0; i < nodes.getLength(); i++) {
				...
				if (node.getNodeName().equals("removesTokens")) {
					...
					// Add the edge, provided its destination has been found
					if (toNode != null) {
						decomposition.addResetEdge(fromNode.getNodeValue(), toNode.getNodeValue());
					}
				}
			}
		}
