...
public class AdvancedLogFilterSettingsComponent extends JPanel {
	...
	private void initialize(final LogSummary summary) {
		...
		JScrollPane sp = new JScrollPane(filterTable, JScrollPane.VERTICAL_SCROLLBAR_NEVER,
		...
		addButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				ProvidedObjectComboItem ce = (ProvidedObjectComboItem) comboBox.getSelectedItem();
				LogFilter f = (LogFilter) ((ProvidedObject) ce.getObject()).getObjects()[0];
		...
		delButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (filterTable.getSelectedRow() != -1) {
					int i = filterTable.getSelectedRow();
					tableModel.remove(i);
					if ((i - 1 < filterTable.getRowCount()) &&
							(i - 1 >= 0)) {
	...
	public void buildFilterCombo(OpenLogSettings curSettingsFrame) {
		...
		for (int i = MainUI.getInstance().getDesktop().getAllFrames().length - 1; i >= 0; i--) {
			JInternalFrame frame = MainUI.getInstance().getDesktop().getAllFrames()[i];
		...
		fillLogFilterCombo(MainUI.getInstance().getGlobalProvidedObjects(), "Global");
		comboBox.setPreferredSize(new Dimension(100, (int) comboBox.getPreferredSize().getHeight()));
		comboBox.setSize(new Dimension(100, (int) comboBox.getPreferredSize().getHeight()));
	}

	private ProvidedObjectComboItem makeComboItem(LogFilter f) {
		return new ProvidedObjectComboItem(f.getName(),
				new ProvidedObject("changes: " + (f.changesLog() ? "yes" :
				"no") + "; complexity: " + f.getComplexityAsString(),
				new Object[] {f}));
...
class LogFilterTableModel extends AbstractTableModel {
	...
	public void add(LogFilter f, String label, int index) {
		f.setName(label);
		logFilters.add(index, new Object[] {f, label, (f.changesLog() ? YES : NO),
				f.getComplexityAsString()});
	...
	public boolean moveUp(int i) {
		if ((i > 0) /* removed constraint for modifying / non-modifying &&
	...
	public boolean moveDown(int i) {
		if ((i < logFilters.size() - 1) /* removed constraint for modifying / non-modifying &&
						 (getValueAt(i, 1).equals(getValueAt(i + 1, 1)))*/) {
			Object o = logFilters.get(i);
			logFilters.remove(i);
			logFilters.add(i + 1, o);
