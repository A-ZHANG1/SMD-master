...
public class AlgorithmListItem implements Comparable<AlgorithmListItem> {
	...
	public AlgorithmListItem(final Plugin algorithm, InputItem[] items) {
		...
		for (int i = 0; i < items.length; i++) {
			panel.add(items[i].getPanel(), new GridBagConstraints(0, i, 1, 1, 0.0, 0.0,
					GridBagConstraints.NORTHWEST, GridBagConstraints.BOTH, new Insets(0, 5, 5, 0),
					0, 0));
		}
		...
		panel.add(docsButton, new GridBagConstraints(0, items.length, 1, 1, 0.0, 0.0,
	...
	public int compareTo(AlgorithmListItem o) {
		if (o==null) return -1;
		return -o.getAlgorithm().getName().toLowerCase().compareTo(getAlgorithm().getName().toLowerCase());
	}
}
