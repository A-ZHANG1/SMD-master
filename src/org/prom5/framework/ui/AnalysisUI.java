...
public class AnalysisUI extends JInternalFrame implements Provider {
	...
	public ProvidedObject[] getProvidedObjects() {
		return result != null && result instanceof Provider
	...
	private void showAnalysis() {
		...
		if (algorithmList.getSelectedValue() == null) {
			JOptionPane.showMessageDialog(this, "Please select an analysis algorithm first.",
		...
		AnalysisInputItem[] neededItems = ((AnalysisPlugin) algorithm.getAlgorithm()).getInputItems();
		...
		for (int i = 0; i < items.length; i++) {
			...
			if (configuredItems[i] == null && (neededItems[i].getMinimum() > 0)) {
				JOptionPane.showMessageDialog(this, "Please choose inputs for all items.",
						"Analysis",
						JOptionPane.INFORMATION_MESSAGE);
				return;
			}
		}
		UISettings.getInstance().setLastUsedAnalysis(algorithm.getAlgorithm().getName());

		MainUI.getInstance().addAction(algorithm.getAlgorithm(), LogStateMachine.START,
				configuredItems);

		SwingWorker w = new SwingWorker() {
			public Object construct() {
				result = ((AnalysisPlugin) algorithm.getAlgorithm()).analyse(configuredItems);
				return null;
			}

			public void finished() {

				MainUI.getInstance().addAction(algorithm.getAlgorithm(), LogStateMachine.COMPLETE,
	...
	private void jbInit() {
		...
		String lastUsedAnalysis = UISettings.getInstance().getLastUsedAnalysis();
		AnalysisPluginCollection algorithmCollection = AnalysisPluginCollection.getInstance();
		int selection = -1;
		...
		for (int i = 0; i < algorithmCollection.size(); i++) {
			AnalysisPlugin algorithm = (AnalysisPlugin) algorithmCollection.get(i);
			...
			if (algorithm != null) {
				...
				for (int j = 0; j < items.length; j++) {
					inputItems[j] = new InputItem(inputsScrollPane, items[j], desktop);
				}
		...
		splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, new JScrollPane(algorithmList),
					inputsScrollPane);
...
class InputItem {
	...
	public InputItem(JComponent parent, AnalysisInputItem item, JDesktopPane desktop) {
		this.item = item;
		this.desktop = desktop;
		this.parent = parent;
	...
	public void refresh() {
		...
		if (items.length == 0) {
			...
			for (int i = 0; i < combos.size(); i++) {
				...
				ProvidedObjectComboItem selected = (ProvidedObjectComboItem) box.getSelectedItem();
	...
	private void constructComboPanel() {
		...
		if (incompleteInput && inputRequired) {
			comboPanel.add(new JLabel("There is no input available for this item."),
			...
			comboPanel.add(new JLabel("There is no input required for this item."),
					new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.NORTHWEST,
					GridBagConstraints.NONE, new Insets(0, 20, 0, 0), 0, 0));
		} else {
			for (int i = 0; i < combos.size(); i++) {
				...
				comboPanel.add((Component) combos.get(i),
						new GridBagConstraints(hasDelButtons ? 1 : 0, i, 2, 1, 0.0,
						0.0,
						GridBagConstraints.NORTHWEST, GridBagConstraints.NONE, new Insets(0,
						hasDelButtons ? 5 : 20, 0, 0),
						0, 0));
				...
				if (hasDelButtons) {
					...
					comboPanel.add(delButton, new GridBagConstraints(0, i, 1, 1, 0.0, 0.0,
							GridBagConstraints.NORTHWEST, GridBagConstraints.NONE, new Insets(0, 20,
							0, 0),
							0, 0));
				}
			}
			if (combos.size() < item.getMaximum()) {
				...
				comboPanel.add(addButton, new GridBagConstraints(0, combos.size(), 1, 1, 0.0, 0.0,
						GridBagConstraints.NORTHWEST, GridBagConstraints.NONE, new Insets(5, 20, 0,
						0),
						0, 0));
				JLabel label = new JLabel("Click this button to add more inputs (if required).");
				label.setEnabled(false);
				comboPanel.add(label, new GridBagConstraints(1, combos.size(), 1, 1, 0.0, 0.0,
						GridBagConstraints.NORTHWEST, GridBagConstraints.NONE, new Insets(10, 2, 0,
						0),
						0, 0));
			}
		}
