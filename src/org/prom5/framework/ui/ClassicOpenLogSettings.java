...
import org.prom5.analysis.summary.LogSummaryUI;
...
import org.prom5.mining.DummyMiningPlugin;
import org.prom5.mining.DummyMiningResult;
...
public class ClassicOpenLogSettings extends OpenLogSettings {
	...
	protected AdvancedLogFilterSettingsComponent advancedPanel = new
			AdvancedLogFilterSettingsComponent(ClassicOpenLogSettings.this);
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		if (s.indexOf(File.pathSeparator) > 0) {
			s = s.substring(s.lastIndexOf(File.pathSeparator) +
				1);
		}
		if (s.indexOf(File.separator) > 0) {
			s = s.substring(s.lastIndexOf(File.separator) + 1);
		}
		objects[0] = new ProvidedObject("Filtered " + s, new Object[] {getSelectedLogReader()});

		for (int i = 0; i < res.length; i++) {
			objects[i + 1] = new ProvidedObject("Previewed " + res[i].getName(),
							 res[i].getObjects());
		}
		return objects;
	}
	...
	public void setupGui() {
		...
		SwingWorker loadWorker = new SwingWorker() {
			...
			public Object construct() {
				try {
					reader = LogReaderFactory.createInstance(new DefaultLogFilter(DefaultLogFilter.INCLUDE), file);
					currentLog = reader;
	...
	protected void logReadingFinished() {
		...
		if(summary.getProcesses().length == 0) {
			...
			// Check if maybe for all processes, the option to keep that process is set
			boolean set = true;
			Iterator it = currentLog.processIterator();
			while (set && it.hasNext() ) {
				org.prom5.framework.log.Process p = (org.prom5.framework.log.Process) it.next();
				set = ( p.getAttributes().containsKey(LogData.RETAIN_PROCESS_IF_EMPTY) &&
					  p.getAttributes().get(LogData.RETAIN_PROCESS_IF_EMPTY).equalsIgnoreCase("true"));
			}
			if(!set && (showMalformedLogWarning("No process instances contained in log!") == JOptionPane.YES_OPTION)) {
				dispose();
				return;
			}
	...
	protected void completeGui() {
		...
		playButton = new JButton(new BuildLogReaderAction(ClassicOpenLogSettings.this));
		...
		logInfoButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				MainUI.getInstance().createFrame("Summary of entire " + getTitle(),
		...
		jSplitPane1.setDividerLocation((int) logUI.getPreferredSize().getWidth() + 24);
		panel.add(lowerPanel, BorderLayout.SOUTH);

		getContentPane().setLayout(new BorderLayout());

		JLabel l = new JLabel("<html>Please click \"Preview log settings\" to inspect <br>" +
	...
	public LogReader getSelectedLogReader() {
		try {
			currentLog = LogReaderFactory.createInstance(getLogFilter(), currentLog);
			return currentLog;
		} catch(Exception e) {
			e.printStackTrace();
			return null;
