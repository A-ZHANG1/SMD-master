...
public class ConversionUI extends JInternalFrame implements Provider {
	...
	private void showConversion() {
		...
		if (algorithmList.getSelectedValue() == null) {
			JOptionPane.showMessageDialog(this, "Please select a conversion algorithm first.",
					"Conversion", JOptionPane.INFORMATION_MESSAGE);
			return;
		}

		final ConvertingPlugin algorithm = ((ConvertAlgorithm) algorithmList.getSelectedValue()).
									 getPlugin();
			...
			public Object construct() {
				...
				try {
					if (algorithm instanceof DoNotCreateNewInstance) {
						...
						result = ((ConvertingPlugin) algorithm.getClass().newInstance()).convert((
								ProvidedObject) comboItems.get(combo.getSelectedIndex()));
					}
				} catch (IllegalAccessException ex) {
					Message.add("No new instantiation of " + algorithm.getName() +
							" could be made, using" +
							" old instance instead", Message.ERROR);
					result = algorithm.convert((ProvidedObject) comboItems.get(combo.
							 getSelectedIndex()));
		...
		if (index >= 0) {
			...
			docsButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					MainUI.getInstance().showReference(((ConvertAlgorithm) algorithmList.
			...
			upperLeftPanel.add(docsButton, new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0,
					GridBagConstraints.NORTHWEST, GridBagConstraints.NONE, new Insets(30, 5, 5, 5),
					0, 0));

			combo = new ToolTipComboBox(getComboItems(((ConvertAlgorithm) algorithmList.
					getSelectedValue()).
					getPlugin()));
			if (combo.getItemCount() == 0) {
				upperLeftPanel.add(new JLabel("There is no input available for this item."),
						new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0,
						GridBagConstraints.NORTHWEST, GridBagConstraints.BOTH, new Insets(0, 5, 5,
						0),
						0, 0));
				combo = null;
			} else {
				upperLeftPanel.add(combo, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0,
						GridBagConstraints.NORTHWEST, GridBagConstraints.BOTH, new Insets(0, 5, 5,
						0),
						0, 0));
			}
...
class ConvertAlgorithm implements Comparable {
	...
	public int compareTo(Object o) {
		...
		return -((ConvertAlgorithm) o).getPlugin().getName().toLowerCase().compareTo(p.getName().
				toLowerCase());
	}
