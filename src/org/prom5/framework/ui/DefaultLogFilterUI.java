...
import org.prom5.framework.log.filter.EndLogFilter;
import org.prom5.framework.log.filter.StartLogFilter;
...
public class DefaultLogFilterUI extends JPanel {
	 ...
	 *
	 * @param summary LogSummary
	 * @param showStartEnd boolean to tell if the panel with compulsary start
	 * and end evens should be shown
	 */
	public DefaultLogFilterUI(LogSummary summary, boolean showStartEnd, String selectedProcess,
			HashMap events) {
		...
		processesTable.setModel(new ProcessesTableModel(summary.getProcesses()));
		...
		if (showStartEnd) {
			...
			logEvents[0] = new ComboBoxLogEvent(new LogEvent(ComboBoxLogEvent.NONE,
						   ComboBoxLogEvent.NONE));
			for (int i = 0; i < summary.getLogEvents().size(); i++) {
				logEvents[i + 1] = new ComboBoxLogEvent(summary.getLogEvents().getEvent(i));
			}
			Arrays.sort(logEvents);

			JPanel start = new JPanel(new BorderLayout());
			start.add(new JLabel("<html>Optionally select a compulsory start event</html>"),
					BorderLayout.NORTH);
			...
			end.add(new JLabel("<html>Optionally select a compulsory final event</html>"),
					BorderLayout.NORTH);
		...
		this.setPreferredSize(new Dimension((int) getPreferredSize().getWidth(), h + h2));
	...
	/**
	 * Give a logfilter that filters the log using the given options. Note that if
	 * no valid options are set, the returned LogFilter equals null
	 *
	...
	public LogFilter getLogFilter() {
		...
		for (int i = 0; i < eventTypeControls.size(); i++) {
			EventTypeControls control = (EventTypeControls) eventTypeControls.get(i);
			f.filterEventType(control.getName(), control.getState());
		}
		filter = f;
		// Check for filtering start events
		if (startEvents != null) {
			ComboBoxLogEvent start = (ComboBoxLogEvent) startEvents.getSelectedItem();
			if (!start.getLogEvent().getModelElementName().equals(ComboBoxLogEvent.NONE)) {
		...
		if (endEvents != null) {
			ComboBoxLogEvent end = (ComboBoxLogEvent) endEvents.getSelectedItem();
			if (!end.getLogEvent().getModelElementName().equals(ComboBoxLogEvent.NONE)) {
	...
	public String getSelectedProcess() {
		return processesTable.getSelectedRow() < 0
				? null
				: (String) processesTable.getModel().getValueAt(processesTable.
				getSelectedRow(), 0);
	}
	...
	private void setEventTypes(String[] types, HashMap events) {
		...
		eventsPanel.setBounds(new Rectangle(0, 200, WIDTH, 0 + h * types.length + 30));

		eventsPanel.setMinimumSize(new Dimension(120, 20 + h * types.length + 30));
		eventsPanel.setPreferredSize(new Dimension(120, 20 + h * types.length + 30));

		eventTypesPanel.setMinimumSize(new Dimension(120, h * types.length + 30));
		eventTypesPanel.setPreferredSize(new Dimension(120, h * types.length + 30));
		...
		for (int i = 0; i < types.length; i++) {
			...
			discardInstance.setFont(discardInstance.getFont().deriveFont(Font.PLAIN));
			...
			if ((events != null) && events.containsKey(types[i])) {
				...
				if (types[i].equals("reassign") || types[i].equals("suspend") ||
					...
					ignore.setSelected(true);
				} else if (types[i].equals("withdraw") || types[i].equals("ate_abort") ||
			...
			eventTypeControls.add(new EventTypeControls(types[i], ignore, include));
...
//		eventTypesPanel.setBackground(Color.green);
		...
		infoPanel.add(wflogTable, new GridBagConstraints(0, 1, 1, 1, 1.0, 1.0
				, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(0, 25, 0, 0), 0, 0));
		...
		procsPanel.add(processListLabel1, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0
				, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(5, 5, 0, 0), 0, 5));
		procsPanel.add(processesScrollPane, new GridBagConstraints(0, 1, 1, 1, 1.0, 1.0
				, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(0, 25, 5, 5), 0, 0));
	...
	public void selectProcess(String process) {
		for (int i = 0; i < processesTable.getRowCount(); i++) {
			if (((String) processesTable.getModel().getValueAt(i, 0)).equals(process)) {
	...
	public ProcessesTableModel(InfoItem[] data) {
		...
		for (InfoItem d : data) {
			hasModelReferences = hasModelReferences || (
					d.getModelReferences() != null && d.getModelReferences().size() > 0);
		}
		this.data = data;
	}

	public String getColumnName(int col) {
		return col == 0 ? "Name" : (col == 1 ? "Description" : "Model references");
	}
