...
import org.prom5.framework.log.filter.LogEventLogFilter;
...
public class ImportSettings extends JDialog {
	...
	public ImportSettings(LogReader log, String pluginLabel, ArrayList importedEvents,
			boolean fuzzyMatch) {
		...
		try {
			...
			eventsUI = new ImportEventsUI(importedEvents, summary.getLogEvents(), fuzzyMatch);
			jbInit();
			pack();
	...
	public HashMap getMapping() {
		...
		for (int i = 0; i < importedEvents.size(); i++) {
			...
			if (e.getModelElementName().equals(ComboBoxLogEvent.VISIBLE) &&
					e.getEventType().equals(ComboBoxLogEvent.VISIBLE)) {
				e = eventsUI.findLogEventIfProvided(event);
				if (e == null) {
					e = new LogEvent("Not in log: " + event.toString(), "unknown:not in log");
				}
			}
	...
	public int isLogReaderReady(boolean autoSelectYes) {
		try {
			// Check for the fact that each logEvent in result.getLogSummary.getLogEvents() is assigned to
			// precisely one logEvent from the imported list

			HashMap mapping = getEventMapping();
			// Now, each LogEvent in result.getLogSummary().getLogEvents() should be a
			// value in mapping
			...
			if (eventsToKeep.size() != log.getLogSummary().getLogEvents().size()) {
				// Unfortunately, we need to check it the hard way.
				// Ask the user for permission
				int i =   JOptionPane.YES_OPTION;
				if (!autoSelectYes) {
					i = JOptionPane.showConfirmDialog(ImportSettings.this,
				...
				if (i == JOptionPane.YES_OPTION) {
					...
					logReaderToReturn =
							LogReaderFactory.createInstance(filter, log);
					if (log.isSelection()) {
						logReaderToReturn =
								LogReaderFactory.createInstance(logReaderToReturn,
								log.processInstancesToKeep());
					}
				}
			} else {
				LogFilter filter = new LogEventLogFilter(eventsToKeep);
...
class ImportEventsUI extends JPanel {
	...
	public ImportEventsUI(ArrayList importedEvents, LogEvents eventsInLog, boolean fuzzyMatch) {
		this.importedEvents = importedEvents;
	...
	private void jbInit() throws Exception {
		...
		ComboBoxLogEvent visible = new ComboBoxLogEvent(LogReaderConnectionImportPlugin.
								   MAKE_VISIBLE);
		logEvents[0] = visible;
		ComboBoxLogEvent invisible = new ComboBoxLogEvent(LogReaderConnectionImportPlugin.
									 MAKE_INVISIBLE);
		...
		{
			final JLabel evtInMod = new JLabel("<html>Events found in<br>imported model:</html>");
			evtInMod.setPreferredSize(new Dimension(ml, (int) evtInMod.getPreferredSize().getHeight()));

			ToolTipComboBox newCombo = new ToolTipComboBox(logEvents);
			final JLabel evtInLog = new JLabel("<html>Events in Log:</html>");
			evtInLog.setPreferredSize(new Dimension((int) newCombo.getPreferredSize().getWidth(),
					(int) evtInLog.getPreferredSize().getHeight()));
		...
		for (int i = 0; i < importedEvents.size(); i++) {
			...
			// set the label in the textfield according the the object in the log
			...
			newCombo.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					String s = ((ToolTipComboBox) e.getSource()).getSelectedItem().toString();
					if (!(s.equals(ComboBoxLogEvent.VISIBLE) ||
							s.equals(ComboBoxLogEvent.INVISIBLE))) {
			...
			for (int j = 0; !foundMatch && j < newCombo.getItemCount(); j++) {
				LogEvent comboEvent = ((ComboBoxLogEvent) newCombo.getItemAt(j)).getLogEvent();
...
//					logEvents[0].setLogEvent(wantedEvent);
			...
			// Comment Anne: looks very ugly in OSX -> what is the reason to make this blue??
