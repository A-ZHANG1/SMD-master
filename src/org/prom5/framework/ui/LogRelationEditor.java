...
import org.prom5.mining.logabstraction.MutableLogRelations;
...
public class LogRelationEditor extends JDialog {
	...
	/**
	 * @param original the log relations to use if the user presses the 'Revert to original relations' button
	 */
	 ...
	 * If this method returns true, the new log relations can be obtained using
	 * <code>getLogRelations()</code>.
	 *
	 * @param current the log relations to show initially in the dialog
	 * @param t the log event to start editing (null if not specified)
	...
	/**
	 * Show edit dialog without pre-selecting a specific log event.
	 * Same as <code>edit(current, null)</code>.
	 *
	 * @param current the log relations to show initially in the dialog
	 * @return true if the user pressed ok, false otherwise
	 */
...
class RelationsUI extends DefaultComboBoxModel {
	...
	private JPanel buildPanel(int index) {
		...
		listsPanel.add(constructCausalList("Followed by (->)", index, INDEX_TO_ALL));
		listsPanel.add(constructCausalList("Follows (<-)", index, ALL_TO_INDEX));
		listsPanel.add(constructParallelList("Parallel with (||)", index));
		listsPanel.setBorder(BorderFactory.createEmptyBorder(0, 10, 10, 10));

		// glue UI together
		checkBoxPanel.add(start, new GridBagConstraints(0, 0, 1,
				1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0,
				0), 0, 0));
		checkBoxPanel.add(end, new GridBagConstraints(0, 1, 1,
				1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0,
				0), 0, 0));
		checkBoxPanel.add(oll, new GridBagConstraints(0, 2, 1, 1,
				0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0),
				0, 0));
	...
	private JPanel constructCausalList(String caption, final int index, final int direction) {
		...
		for (int i = 0; i < relations.getLogEvents().size(); i++) {
			...
			boxes[i] = new JCheckBox(ev.getModelElementName() + " (" + ev.getEventType() + ")");
	...
	private JPanel constructParallelList(String caption, final int index) {
		...
		for (int i = 0; i < relations.getLogEvents().size(); i++) {
			...
			if (parallelMatrix.get(index, i) > 0 || parallelMatrix.get(i, index) > 0) {
				boxes[i].setSelected(true);
			}
			boxes[i].addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					relations.setParallel(index, finalI,
							boxes[finalI].isSelected() ? 1.0 : 0.0);
					relations.setParallel(finalI, index,
							boxes[finalI].isSelected() ? 1.0 : 0.0);
					parallelBoxes[finalI][index].setSelected(boxes[finalI].isSelected());
				}
			});
		}
		list.setListData(boxes);
		...
		addKeyListener(new KeyAdapter() {
			public void keyTyped(KeyEvent e) {
				if (e.getKeyChar() == ' ' || e.getKeyChar() == '\n' || e.getKeyChar() == '\r' &&
						getSelectedIndex() != -1) {
					JCheckBox checkbox = (JCheckBox) getModel().getElementAt(getSelectedIndex());
					checkbox.doClick();
					repaint();
				}
			}
