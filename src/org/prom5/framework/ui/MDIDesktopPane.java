...
public class MDIDesktopPane extends JDesktopPane {
	...
	public MDIDesktopPane() {
		super();
		bgTileImage = null;
		String bgPath = UISettings.getInstance().getPreferredDesktopBackground();
		if(bgPath.equals("NONE") == false) {
			try {
				bgTileImage = GraphicsUtilities.loadCompatibleImage((new File(bgPath)).toURL());
	...
	protected void paintComponent(Graphics g) {
		if(bgTileImage == null) {
			...
			if(buffer == null || buffer.get() == null || 
					buffer.get().getWidth() != width || buffer.get().getHeight() != height) {
				// create new back buffer
				buffer = new SoftReference<BufferedImage>(GraphicsUtilities.createCompatibleImage(width, height));
				...
				// draw top shade
				GradientPaint gradient = new GradientPaint(0, 0, colorShade, 0, height / 5, colorTransparent);
				g2d.setPaint(gradient);
				g2d.fillRect(0, 0, width, height / 5);
				// draw bottom shade
				gradient = new GradientPaint(0, height - (height / 4), colorTransparent, 0, height, colorShade);
				g2d.setPaint(gradient);
	...
	public synchronized Component add(JInternalFrame frame, Plugin plugin) {
		...
		if ((array.length == 0) || (p.x + w > getWidth()) || (p.y + h > getHeight())) {
			p = new Point(FRAME_OFFSET, FRAME_OFFSET);
		}
