...
import org.prom5.mining.FileResult;
import org.prom5.mining.MiningPlugin;
import org.prom5.mining.MiningResult;
import org.prom5.mining.NewStyleMiningPlugin;
...
public class MiningSettings extends JInternalFrame {
	 ...
	 * the given LogSummary is a summary of the log, so it does not read the log.
	 * The right hand side is given by algorithm.getOptionsPanel().
	 * Note that the log is assumed to be open through the LogReader object. Hence
	 * no changes can be made to the log.
	 *
	 * @param log LogReader
	 * @param name String
	 * @param algorithm MiningPlugin
	 */
	public MiningSettings(LogReader log, String name, final MiningPlugin algorithm) {
		super("Settings for mining " + name + " using " + algorithm.getName(), true, true, true, true);
	...
	public LogReader getLogReader() {
		...
		try {
			sourceLog = LogReaderFactory.createInstance(getLogFilter(), sourceLog);
			return sourceLog;
		} catch(Exception e) {
			e.printStackTrace();
			return null;
	...
	private void initialize() {
		...
		MainUI.getInstance().addAction(algorithm, LogStateMachine.SCHEDULE, null);
		...
		if(algorithmPanel.isOpaque() == false || 
			...
			SlickerSwingUtils.injectBackgroundColor(algorithmPanel, COLOR_INNER_BG);
		...
		SwingWorker worker = new SwingWorker() {
				...
				try {
					...
					synchronized (log) {
						...
						if (plugin instanceof NewStyleMiningPlugin) {
							result = ((NewStyleMiningPlugin) plugin).mine(log, algorithmPanel);
						} else {
			...
			public void finished() {
				...
				if (result != null && result instanceof FileResult) {
					...
					String filename = Utils.saveFileDialog(MainUI.getInstance(),
									  new GenericFileFilter(fileResult.getExtension()));
					...
					MainUI.getInstance().createVisualizationFrame("Results - " + algorithm.getName() + " on " + objectName,
							result, plugin);
				} else {
					MainUI.getInstance().addAction(plugin, LogStateMachine.ATE_ABORT,
