...
import org.prom5.framework.ui.actions.MxmlFilenameFilter;
...
public class OpenMXMLLogDialog extends JDialog {
	...
	public OpenMXMLLogDialog(String lastOpenedLog) {
		...
		if (lastOpenedLog.startsWith("zip://")) {
			lastOpenedLog = lastOpenedLog.substring("zip://".length(), lastOpenedLog.indexOf('#'));
		}
	...
	public LogFile getFile() {
		return LogFile.getInstance(workflowLogFile.getText().toLowerCase().endsWith(".zip")
				?
				"zip://" + workflowLogFile.getText() + "#" + zipEntries.getSelectedValue().toString()
				: workflowLogFile.getText());
	}

	public void setSelectedFile(String filename) {
		if (filename.startsWith("zip://")) {
			String zipFile = filename.substring("zip://".length(), filename.indexOf('#'));
		...
		try {
			...
			while (enu.hasMoreElements()) {
				...
				if (entryName.toLowerCase().endsWith(".xml") ||
					...
					if (entryName.toLowerCase().startsWith(zipFileName.toLowerCase().substring(0,
			...
			JOptionPane.showMessageDialog(this, ex.getMessage(), "Error opening zip file",
	...
	private void jbInit() throws Exception {
		...
		settingsPanel.add(filenameLabel, new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0
				, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(10, 5, 0, 0), 10,
				0));
		settingsPanel.add(workflowLogFile, new GridBagConstraints(1, 1, 1, 1, 1.0, 0.0
				, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(10, 0, 0, 0),
				-44, 4));
		settingsPanel.add(chooseLogButton, new GridBagConstraints(2, 1, 1, 1, 0.0, 0.0
				, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(10, 10, 0, 10), 67,
				0));
		settingsPanel.add(zipContents, new GridBagConstraints(1, 2, 2, 1, 0.0, 0.0
				, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(5, 5, 5, 20), 0, 0));
	...
	void chooseLogButton_actionPerformed(ActionEvent e) {
		if (RuntimeUtils.isRunningMacOsX()) {
			// The AWT FileDialog class provides a more native user experience on Mac OS X
			if (fileDialog == null) {
				fileDialog = new FileDialog(MainUI.getInstance(), "Choose log file...",
							 FileDialog.LOAD);
			...
			JFileChooser chooser = new JFileChooser(workflowLogFile.getText() == null ||
								   workflowLogFile.getText().equals("") ?
								   lastOpenedLog :
								   workflowLogFile.getText());
			chooser.setFileFilter(new GenericMultipleExtFilter(new String[] {"xml", "zip", "mxml",
					"mxml.gz"}
					, "MXML and ZIP files (*.mxml, *mxml.gz, *.xml, *.zip)"));
			if (chooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
				String name = chooser.getSelectedFile().getPath();
				// Specific log might still have to be chosen (in case of zip file)!
	...
	void openButton_actionPerformed(ActionEvent e) {
		if (workflowLogFile.getText() == null || workflowLogFile.getText().equals("")) {
			JOptionPane.showMessageDialog(MainUI.getInstance(), "Please choose a log file first.");
