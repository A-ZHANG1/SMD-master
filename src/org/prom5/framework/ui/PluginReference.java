...
public class PluginReference extends JFrame {
		...
		if(url.startsWith("http://")) {
			...
			if (url.indexOf('/') > 0) {
				...
				if (type.equals("logfilter")) {
					...
					URLBase = URLBase.substring(URLBase.lastIndexOf(".") + 1) + File.separator;
					jep.setEditorKit(new ProMHTMLEditorKit(About.HELPLOCATION() + URLBase));
					...
					if (type.equals("mining")) {
						...
						plugin = AnalysisPluginCollection.getInstance().get(name);
						...
						plugin = ConvertingPluginCollection.getInstance().get(name);
					}
			...
			try {
				...
				JOptionPane.showMessageDialog(this, ex.getMessage(),
						"Could not display help page", JOptionPane.ERROR_MESSAGE);
			}
		}
	...
	private String getPluginsOverview(String type) {
		...
		html += "<tr><td colspan=2><table border=0 width=\"100%\" align=center><tr>" +
				...
				"</tr></table></td></tr>";
		html += "<tr><td colspan=2><h1>Overview of " + type + " plugins</h1></td></tr>";

		if (type.equals("") || type.equals("mining")) {
			html += "<tr><td colspan=2><b>Mining plugins</b></td></tr>";
			html += "<tr><td width=\"10%\">&nbsp;</td><td align=left>" +
					getPluginList(MiningPluginCollection.getInstance(), "mining") + "</td></tr>";
		}
		...
		if (type.equals("") || type.equals("export")) {
			...
			html += "<tr><td width=\"10%\">&nbsp;</td><td align=left>" +
					getPluginList(ExportPluginCollection.getInstance(), "export") + "</td></tr>";
		}
		if (type.equals("") || type.equals("import")) {
			html += "<tr><td colspan=2><b>Import plugins</b></td></tr>";
			html += "<tr><td width=\"10%\">&nbsp;</td><td align=left>" +
					getPluginList(ImportPluginCollection.getInstance(), "import") + "</td></tr>";
		}
		...
		if (type.equals("") || type.equals("logfilter")) {
			...
			html += "<tr><td width=\"10%\">&nbsp;</td><td align=left>" +
					getPluginList(LogFilterCollection.getInstance()) + "</td></tr>";
		}
		html += "</table>";
		html += "</body></html>";
		return html;
	...
	private String getPluginList(PluginCollection collection, String type) {
		...
		Arrays.sort(plugins, new Comparator<Plugin>() {
			public int compare(Plugin o1, Plugin o2) {
				if ((o1 == null ||o1.getName() == null) && (o2 == null || o2.getName() == null)) return 0;
		...
		for (int i = 0; i < plugins.length; i++) {
			...
			if (desc == null || desc.equals("")) {
				html += plugin.getName() + " <font size=2>(no documentation available)</font>";
			} else if (desc.startsWith("http://")) {
				html += "<a href=\"" + desc + "\">" +
						plugin.getName() + "</a> <font size=2>(online documentation)</font>";
			} else {
			...
			public void hyperlinkUpdate(HyperlinkEvent evt) {
				if (evt.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
					try {
						...
						JOptionPane.showMessageDialog(PluginReference.this, e.getMessage(),
								"Could not display help page", JOptionPane.ERROR_MESSAGE);
					}
				}
			}
	...
	private String getHTMLPluginDescription(Plugin p, String type) {
		...
		if (html == null || html.equals("")) {
			...
			// add <html> tags
			html = "<html>" +
				   "<body> &nbsp; <a href=\"plugin://" + type + "\">Back to overview</a><hr>" +
				   "<table border=0 width=\"95%\" align=center><tr><td>" +
				   html +
				   "</td></tr></table></body></html>";
		}
