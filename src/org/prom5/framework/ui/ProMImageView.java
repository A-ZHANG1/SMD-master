...
public class ProMImageView extends View implements ImageObserver, MouseListener,
		MouseMotionListener {
	...
	public void changedUpdate(DocumentEvent e, Shape a, ViewFactory f) {
		...
		if (hChanged || wChanged || getVerticalAlignment() != align) {
			if (DEBUG) {
				Message.add("ImageView: calling preferenceChanged", Message.DEBUG);
			}
			getParent().preferenceChanged(this, hChanged, wChanged);
		}
		if (DEBUG) {
			Message.add("ImageView: changedUpdate end; valign=" + getVerticalAlignment(),
					Message.DEBUG);
		}
	}
	...
	public boolean imageUpdate(Image img, int flags, int x, int y,
		...
		if (changed != 0) {
			...
			if (DEBUG) {
				Message.add("ImageView: resized to " + fWidth + "x" + fHeight, Message.DEBUG);
			}
	...
	public void mousePressed(MouseEvent e) {
		...
		if (e.getX() >= size.width - 7 && e.getY() >= size.height - 7
			...
			if (DEBUG) {
				Message.add("ImageView: grow!!! Size=" + fWidth + "x" + fHeight, Message.DEBUG);
			}
	...
	private Icon makeIcon(final String gifFile) throws IOException {
		...
		if (resource == null) {
			Message.add(ProMImageView.class.getName() + "/" +
					gifFile + " not found.", Message.ERROR);
			return null;
		}
		...
		if (buffer.length == 0) {
			Message.add("warning: " + gifFile +
					" is zero-length", Message.ERROR);
			return null;
		}
	...
	private static final String PENDING_IMAGE_SRC = System.getProperty("user.dir") + "/images/image-delayed.gif"; // both stolen from HotJava
	private static final String MISSING_IMAGE_SRC = System.getProperty("user.dir") + "/images/image-failed.gif";
