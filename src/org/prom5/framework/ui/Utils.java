...
public class Utils {
	...
	public static Icon getStandardIcon(String filename, String defaultJavaIconName) {
		String customIconPath = UISettings.getInstance().getPreferredIconTheme() + "/" + filename;
		String defaultIconPath = UISettings.getInstance().getDefaultIconPath() + "/" + filename;
	...
	public static synchronized String openImportFileDialog(Component parent,
		...
		JFileChooser chooser = new JFileChooser(UISettings.getInstance().getLastOpenedImportFile());

		chooser.setFileFilter(filter);
		if (chooser.showOpenDialog(parent) == JFileChooser.APPROVE_OPTION) {
			return filter instanceof GenericFileFilter
					? ((GenericFileFilter) filter).addExtension(chooser.getSelectedFile().getPath())
	...
	public static synchronized String saveFileDialog(Component parent,
			javax.swing.filechooser.FileFilter filter) {
		JFileChooser chooser = new JFileChooser(UISettings.getInstance().getLastExportLocation());

		chooser.setFileFilter(filter);
		if (chooser.showSaveDialog(parent) == JFileChooser.APPROVE_OPTION) {
			UISettings.getInstance().setLastExportLocation(chooser.getCurrentDirectory());
			return filter instanceof GenericFileFilter
					? ((GenericFileFilter) filter).addExtension(chooser.getSelectedFile().getPath())
					: chooser.getSelectedFile().getPath();
		}
