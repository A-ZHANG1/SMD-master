...
import org.prom5.framework.ui.menus.AnalysisAction;
...
public class MostRecentAnalyseAction extends CatchOutOfMemoryAction {
	...
	public static Icon getIcon() {
		String customIconPath = UISettings.getInstance().getPreferredIconTheme() + "/toolbar_analyse_mru.png";
	...
	public MostRecentAnalyseAction(MDIDesktopPane desktop) {
		super("Execute most recently used analysus action again (if possible)", MostRecentAnalyseAction.getIcon(), desktop);
		putValue(SHORT_DESCRIPTION, "Execute most recently used analysis  " + getShortcut());
	}
	...
	public void execute(ActionEvent e) {
		...
		if (lastUsed == null || lastUsed.length() == 0) {
			JOptionPane.showMessageDialog(MainUI.getInstance(), "There is no recently used analysis plugin yet.\nPlease select a plugin from the 'Analysis' menu first.");
			return;
		}

		ProvidedObject[] objects = MainUI.getInstance().getProvidedObjects();
		AnalysisPluginCollection collection = AnalysisPluginCollection.getInstance();

		if (objects.length == 0) {
			JOptionPane.showMessageDialog(MainUI.getInstance(), "Please select a window which provides an object to analyze.");
			return;
		}
		...
		for (int i = 0; i < collection.size(); i++) {
			...
			if (algorithm != null && algorithm.getName().equals(lastUsed)) {
				...
				if (items.length == 1 && (items[0].getMinimum() == 1 || items[0].getMaximum() == 1)) {
					for (int j = 0; j < objects.length; j++) {
						ProvidedObject object = objects[j];

						if (object != null && items[0].accepts(object)) {
							new AnalysisAction(algorithm, object).actionPerformed(null);
							return;
						}
					}
				} else {
					JOptionPane.showMessageDialog(MainUI.getInstance(), "The analysis plugin '" + lastUsed + "' needs multiple objects.\nPlease use the 'More analysis...' option in the 'Analysis' menu to run it again.");
					return;
				}
			}
		}
		JOptionPane.showMessageDialog(MainUI.getInstance(), "The analysis plugin '" + lastUsed + "' cannot be run on the objects provided by the window that currently has the focus.");
	}
