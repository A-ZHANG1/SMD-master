...
import org.prom5.framework.ui.menus.OpenRecentFileAction;
...
public class MostRecentLogAction extends CatchOutOfMemoryAction {
	...
	public static Icon getIcon() {
		String customIconPath = UISettings.getInstance().getPreferredIconTheme() + "/toolbar_open_mru.png";
	...
	public MostRecentLogAction(MDIDesktopPane desktop) {
		super("Open most recently opened log file again (if possible)", MostRecentLogAction.getIcon(), desktop);
		putValue(SHORT_DESCRIPTION, "Open most recently opened log  " + getShortcut());
	}
	...
	public void execute(ActionEvent e) {
		...
		if (lastUsed != null && lastUsed.length() > 0) { 
			new OpenRecentFileAction(lastUsed, (new File(lastUsed)).getName(), UISettings.LOGTYPE, desktop).execute(null);
		} else {
			JOptionPane.showMessageDialog(MainUI.getInstance(), "There is no recently used log file yet.\nPlease open a log file using the 'Open MXML Log file' in the 'File' menu first.");
		}
	}
