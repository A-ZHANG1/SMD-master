...
import org.prom5.framework.ui.OpenMXMLLogDialog;
...
public class OpenLogAction extends CatchOutOfMemoryAction {
	...
	public static Icon getIcon() {
		String customIconPath = UISettings.getInstance().getPreferredIconTheme() + "/toolbar_open.png";
	...
	public void execute(ActionEvent e) {
		OpenMXMLLogDialog dialog = new OpenMXMLLogDialog(UISettings.getInstance().
		...
		if (dialog.showDialog(false)) {
			...
			UISettings.getInstance().addRecentFile(dialog.getFile().toString(), null);
			UISettings.getInstance().setLastOpenedLogFile(dialog.getFile() == null ? "" :
					dialog.getFile().toString());
		}
	}
