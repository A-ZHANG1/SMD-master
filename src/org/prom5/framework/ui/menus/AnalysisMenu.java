...
public class AnalysisMenu extends JMenu {
	...
	private JMenuItem none = new JMenuItem(
			RuntimeUtils.stripHtmlForOsx("<html><font color=\"#999999\">No analysis available<br>for selected frame</font></html>"));
	private JMenuItem noneshort = new JMenuItem(
			RuntimeUtils.stripHtmlForOsx("<html><font color=\"#999999\">No analysis available</font></html>"));
	...
	private void buildChildMenus(ProvidedObject[] objects, JMenuItem noneFound) {
		...
		for (int j = 0; j < objects.length; j++) {
			if (objects[j] != null) {
				JMenu poMenu = addToSelf ? this : new JMenu(objects[j].getName());
	...
	private void buildSubMenus(JMenu superMenu, AnalysisPluginCollection collection, ProvidedObject po) {
		...
		for (int i = 0; i < collection.size(); i++) {
			...
			if (algorithm == null) {
				...
				if (items.length == 1 && (items[0].getMinimum() == 1 || items[0].getMaximum() == 1) && items[0].accepts(po)) {
					if (addSeparatorFirst) {
						// we never add a separator as the first item or the last item in a menu
						// we also never insert two seperators right after each other
						superMenu.addSeparator();
						addSeparatorFirst = false;
					}
					superMenu.add(new JMenuItem(new AnalysisAction(algorithm, po)));
				}
			}
		}
	}
