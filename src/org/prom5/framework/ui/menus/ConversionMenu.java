...
import org.prom5.framework.ui.actions.ConvertInternalAction;
...
public class ConversionMenu extends JMenu {
	...
	private JMenuItem none = new JMenuItem(
			RuntimeUtils.stripHtmlForOsx("<html><font color=\"#999999\">No converters available<br>for selected frame</font></html>"));
	private JMenuItem noneshort = new JMenuItem(
			RuntimeUtils.stripHtmlForOsx("<html><font color=\"#999999\">No converter available</font></html>"));
	...
	private void buildChildMenus(ProvidedObject[] objects, JMenuItem noneItem) {
		ConvertingPluginCollection collection = ConvertingPluginCollection.getInstance();
	...
	private void buildSubMenus(JMenu superMenu, ConvertingPluginCollection collection, ProvidedObject po) {
		for (int i = 0; i < collection.size(); i++) {
			ConvertingPlugin algorithm = (ConvertingPlugin) collection.get(i);

			if (algorithm.accepts(po)) {
				superMenu.add(new JMenuItem(new ConvertInternalAction(algorithm, po)));
			}
		}
	}
}
