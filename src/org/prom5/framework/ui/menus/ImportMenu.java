...
import org.prom5.framework.ui.actions.ImportFileAction;
import org.prom5.framework.ui.actions.OpenLogAction;
...
public class ImportMenu extends JMenu {
	...
	public ImportMenu(MDIDesktopPane desktop) {
		...
		anyFileAction = new ImportAnyFileAction(desktop, !RuntimeUtils.isRunningMacOsX());
	...
	private void buildChildMenus() {
		ImportPluginCollection collection = ImportPluginCollection.getInstance();
		...
		for (int i = 0; i < collection.size() ; i++) {
			...
			if (algorithm instanceof LogReaderConnectionImportPlugin) {
				...
				subMenu.add(new ImportFileAction(algorithm, null, desktop, "Without Log file"));
				Iterator it = logReaders.iterator();
				if (it.hasNext()) {
					subMenu.addSeparator();
				} while (it.hasNext()) {
					subMenu.add(new ImportFileAction(algorithm, (LogReader) it.next(), desktop,
							"With: " + (String) it.next()));
				}
				add(subMenu);
			} else {
