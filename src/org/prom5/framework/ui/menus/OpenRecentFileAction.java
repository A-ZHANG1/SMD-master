...
public class OpenRecentFileAction extends CatchOutOfMemoryAction {
	...
	public OpenRecentFileAction(String longFileName, String label, String algorithm,
			MDIDesktopPane desktop, LogReader log) {
		super(label, desktop);
	...
	public OpenRecentFileAction(String longFileName, String shortFileName, String algorithm,
	...
	public void execute(ActionEvent e) {
		...
		try {
			if (algorithmName.equals(UISettings.LOGTYPE)) {
				...
				MainUI.getInstance().addAction("import : MXML Log File", LogStateMachine.START, null );
				MainUI.getInstance().createOpenLogFrame(logFile);
			} else {
				algorithm = ImportPluginCollection.getInstance().get(algorithmName);
				if (!(algorithm instanceof DoNotCreateNewInstance)) {
					algorithm = (ImportPlugin) algorithm.getClass().newInstance();
				}
				MainUI.getInstance().importFromFile((ImportPlugin) algorithm, longFile, log);
			}
			UISettings.getInstance().addRecentFile(longFile, algorithmName);
		} catch (InstantiationException ex) {
			Message.add("Could not create a new instance of the selected algorithm!", Message.ERROR);
