...
public class RecentFileMenu extends JMenu {
	...
	private void buildChildMenus() {
		...
		for (int i = 0; i < files.size() && i < 20; i++) {
			...
			if ((plugin != null) && (plugin instanceof LogReaderConnectionImportPlugin)) {
				JMenu subMenu = new JMenu((new File(file[0])).getName() + " - " + file[1]);
				subMenu.add(new OpenRecentFileAction(file[0], "Without Log file",
						file[1], desktop, null));
				Iterator it = logReaders.iterator();
				if (it.hasNext()) {
					subMenu.addSeparator();
				} while (it.hasNext()) {
					subMenu.add(new OpenRecentFileAction(file[0], "With: " + (String) it.next(),
							file[1], desktop, (LogReader) it.next()));
				}
				add(subMenu);
			} else {
				add(new OpenRecentFileAction(file[0], (new File(file[0])).getName(),
						file[1], desktop));
			}
		}
