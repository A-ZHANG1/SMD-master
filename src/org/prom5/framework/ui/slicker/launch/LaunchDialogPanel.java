 ...
 *
 */
package org.prom5.framework.ui.slicker.launch;
...
import com.fluxicon.slickerbox.components.MouseOverLabel;
...
import com.fluxicon.slickerbox.components.ToggleSwitchBar;
...
public class LaunchDialogPanel extends RoundedPanel {
	...
	protected static String typeToggles[] = {
	...
	public LaunchDialogPanel() {
		...
		this.searchField = new SlickerSearchField(380, 25,
		...
		this.searchField.addKeyListener(new KeyListener() {
			public void keyPressed(KeyEvent e) {
				if(e.getKeyChar() == KeyEvent.VK_ENTER) {
					...
					if(actionListModel.getSize() > 0 && actionChoiceList.getSelectedIndex() < actionListModel.getSize() - 1) {
						actionChoiceList.setSelectedIndex(actionChoiceList.getSelectedIndex() + 1);
					}
				} else if(e.getKeyChar() == KeyEvent.VK_DOWN) {
					if(actionListModel.getSize() > 0 && actionChoiceList.getSelectedIndex() > 0) {
						actionChoiceList.setSelectedIndex(actionChoiceList.getSelectedIndex() - 1);
					}
				}
			}
		...
		this.actionChoiceList.setCellRenderer(new LaunchActionListCellRenderer(30));
		...
		vBar.setUI(new SlickerScrollBarUI(vBar, new Color(20, 20, 20), new Color(140, 140, 140), new Color(80, 80, 80), 4, 12));
		MouseOverLabel actionListLabel = new MouseOverLabel("available actions", 11f, new Color(100, 100, 100));
		JPanel actionLabelPanel = new JPanel();
		actionLabelPanel.setOpaque(false);
		actionLabelPanel.setLayout(new BoxLayout(actionLabelPanel, BoxLayout.X_AXIS));
		...
		if(RuntimeUtils.isRunningMacOsX() == false) {
			headerLabel.setFont(headerLabel.getFont().deriveFont(16f).deriveFont(Font.BOLD));
		} else {
		...
		MouseOverLabel objectLabel = new MouseOverLabel("select the object to act on", 11f, new Color(10, 10, 10));
		objectPanel.add(objectLabel);
		objectPanel.add(Box.createHorizontalGlue());
		JPanel filterPanel = new JPanel();
		filterPanel.setOpaque(false);
		filterPanel.setLayout(new BoxLayout(filterPanel, BoxLayout.X_AXIS));
		MouseOverLabel filterLabel = new MouseOverLabel("<html>Type a search string or disable plugin types to filter the available actions. " +
				"Hit return or double click to trigger the currently selected action.</html>", 11f, new Color(10, 10, 10));
	...
	public void actionSelected() {
		...
		final AbstractAction action = (AbstractAction)this.actionChoiceList.getSelectedValue();
		if(action != null) {
			parent.fadeOut();
			Thread actionThread = new Thread() {
				public void run(){
					...
					action.actionPerformed(new ActionEvent(this, 1, "launcher triggered this"));
				}
			};
			actionThread.start();
		}
	...
	public void updateActionList() {
		Thread updateThread = new Thread() {
			public void run() {
				Thread.yield();
				ProvidedObject obj = (ProvidedObject)objectBox.getSelectedItem();
	...
	public void updateActionFilter() {
		...
		ActionFilter filter = new ActionFilter(
				...
				typeFilterBar.getToggle(0),
				typeFilterBar.getToggle(1),
				typeFilterBar.getToggle(2),
				typeFilterBar.getToggle(3));
	...
	public void initialize(final ProvidedObject object) {
		Thread initThread = new Thread() {
			public void run() {
				...
				Thread.yield();
				if (object == null) {
					Provider provider = (Provider) MainUI.getInstance().getDesktop().getSelectedFrame();
					objectBox.setModel(new DefaultComboBoxModel(provider.getProvidedObjects()));
				} else {
					objectBox.setModel(new DefaultComboBoxModel(new ProvidedObject[] { object }));
				}
