...
public class LogDashboardUI extends JPanel {
	...
	protected void calculateData() {
		...
		for(int i=0; i<log.numberOfInstances(); i++) {
			...
			int eventClasses = this.summary.getEventsForInstance(log.getInstance(i)).size();
	...
	protected void setupGui() {
		...
		keyPanel.add(packInfo("Processes", Integer.toString(this.numberOfProcesses)));
		...
		keyPanel.add(Box.createVerticalStrut(15));
		keyPanel.add(packInfo("Event classes", Integer.toString(this.numberOfEventClasses)));
		keyPanel.add(Box.createVerticalStrut(3));
		keyPanel.add(packInfo("Event types", Integer.toString(this.numberOfEventTypes)));
		keyPanel.add(Box.createVerticalStrut(15));
		keyPanel.add(packInfo("Originators", Integer.toString(this.numberOfOriginators)));
		...
		infoPanel.add(getInfoPanel("Source", this.summary.getSource().getName()));
		String sourceProgram = this.summary.getSource().getData().get("program");
		if(sourceProgram != null && sourceProgram.length() > 0) {
			infoPanel.add(Box.createVerticalStrut(6));
			infoPanel.add(getInfoPanel("Source program", sourceProgram));
		}
		infoPanel.add(Box.createVerticalStrut(10));
		infoPanel.add(getInfoPanel("Start date", this.getStartTime(parent.getOriginalLog())));
		infoPanel.add(Box.createVerticalStrut(6));
		infoPanel.add(getInfoPanel("End date", this.getEndTime(parent.getOriginalLog())));
		...
		AutoFocusButton actionButton = new AutoFocusButton("start analyzing this log");
		...
		distributionPanel.add(getDistributionPanel("Events per case", this.eventsPerCases, this.meanEventsPerCase));
		distributionPanel.add(Box.createVerticalStrut(10));
		distributionPanel.add(getDistributionPanel("Event classes per case", this.eventClassesPerCases, this.meanEventClassesPerCase));
		// assemble
		this.add(keyPanel);
	...
	protected RoundedPanel getDistributionPanel(String title, int[] values, int meanValue) {
		...
		keyPanel.add(Box.createHorizontalGlue());
		keyPanel.add(Box.createHorizontalGlue());
		keyPanel.add(packInfo("Max", Integer.toString(values[values.length - 1])));
