...
public class LogPreviewUI extends JPanel {
	...
	public static final Color COLOR_LIST_SELECTION_BG_LOWER = new Color(30, 10, 10);
	...
	public LogPreviewUI(SlickerOpenLogSettings parent) {
		...
		vBar.setUI(new SlickerScrollBarUI(vBar, new Color(0, 0, 0, 0), new Color(160, 160, 160), COLOR_NON_FOCUS, 4, 12));
		...
		attributesScrollPane.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED);
		attributesScrollPane.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		vBar = attributesScrollPane.getVerticalScrollBar();
		vBar.setUI(new SlickerScrollBarUI(vBar, new Color(0, 0, 0, 0), new Color(200, 200, 200), COLOR_NON_FOCUS, 4, 12));
		vBar.setOpaque(false);
	...
	protected void eventsSelectionChanged() {
		...
		if(ate != null) {
			...
			eventLabel.setText("Attributes for event " + (eventsList.getSelectedIndex() + 1));
	...
	protected void showSelectedInstanceData() {
		...
		if (selectedIndices.length == 1) {
			...
			attributesList.setModel(new AttributesListModel((ProcessInstance) null));
		}
	}
