 ...
 *
 */
package org.prom5.framework.ui.slicker.logdialog;
...
public class SlickerAdvancedLogFilterConfiguration extends JPanel {
	...
	public SlickerAdvancedLogFilterConfiguration(LogReader log, ChangeListener updateListener) {
		this.log = log;
			...
			public void mouseClicked(MouseEvent evt) {
				if(evt.getClickCount() > 1 && evt.getButton() == MouseEvent.BUTTON1) {
					...
					if(filterList.getSelectedIndex() >= 0) {
						LogFilter filter = (LogFilter)filterListModel.get(filterList.getSelectedIndex());
		...
		filterListScrollPane.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED);
		filterListScrollPane.setViewportBorder(BorderFactory.createLineBorder(colorListBorder));
		filterListScrollPane.setBorder(BorderFactory.createEmptyBorder());
		JScrollBar vBar = filterListScrollPane.getVerticalScrollBar();
		vBar.setUI(new SlickerScrollBarUI(vBar, colorBg, new Color(30, 30, 30), new Color(80, 80, 80), 4, 12));
		...
		filterListHeaderPanel.setLayout(new BoxLayout(filterListHeaderPanel, BoxLayout.X_AXIS));
		filterListHeaderPanel.add(filterListHeader);
		filterListHeaderPanel.add(Box.createHorizontalGlue());
		JLabel descriptionLabel = new JLabel("<html>All filters in this list will be applied " +
		...
		filterDescriptionPanel.setLayout(new BoxLayout(filterDescriptionPanel, BoxLayout.X_AXIS));
		...
		editButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(selectIndex >= 0) {
					showFilterConfiguration((LogFilter)filterListModel.elementAt(selectIndex), false);
				}
			}
		});
		...
		addFilterListHeader.setFont(addFilterListHeader.getFont().deriveFont(14f));
		searchField = new SlickerSearchField(150, 23, new Color(210, 210, 210), new Color(140, 140, 140), new Color(90, 90, 90), new Color(160, 20, 20));
		searchField.addSearchListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				for(int i=0; i<allFilterList.getModel().getSize(); i++) {
					String filterName = ((LogFilter)allFilterList.getModel().getElementAt(i)).getName().toLowerCase();
		...
		addFilterListHeaderPanel.setLayout(new BoxLayout(addFilterListHeaderPanel, BoxLayout.X_AXIS));
		...
		allFilterListScrollPane.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
		allFilterListScrollPane.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
		allFilterListScrollPane.setViewportBorder(BorderFactory.createLineBorder(colorListBorder));
	...
	protected void showFilterConfiguration(LogFilter filter, final boolean filterToBeAdded) {
		...
		confHeaderPanel.setLayout(new BoxLayout(confHeaderPanel, BoxLayout.X_AXIS));
		confHeaderPanel.add(confHeader);
		confHeaderPanel.add(Box.createHorizontalGlue());
		// get parameter dialog
		final LogFilterParameterDialog dialog = filter.getParameterDialog(log.getLogSummary());
		dialog.setVisible(false);
		JPanel filterConfPanel = dialog.getConfigurationPanel();
		if(filterConfPanel != null) {
			SlickerSwingUtils.injectBackgroundColor(filterConfPanel, colorConfBg);
			JScrollPane scrollPane = new JScrollPane(filterConfPanel);
			scrollPane.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
			scrollPane.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED);
			scrollPane.setBorder(BorderFactory.createEmptyBorder());
			JScrollBar vBar = scrollPane.getVerticalScrollBar();
			vBar.setUI(new SlickerScrollBarUI(vBar, colorConfBg, new Color(30, 30, 30), new Color(80, 80, 80), 4, 12));
			JScrollBar hBar = scrollPane.getHorizontalScrollBar();
			hBar.setUI(new SlickerScrollBarUI(hBar, colorConfBg, new Color(30, 30, 30), new Color(80, 80, 80), 4, 12));
			...
			okButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					if(dialog.isAllParametersSet() == false) {
						// TODO: show some kind of error dialog here! (JOptionPane.showMessage..)
						return;
					}
	...
	protected void loadFilter() {
		FileDialog dialog = new FileDialog(MainUI.getInstance(), "Load filter configuration", FileDialog.LOAD);
		dialog.setDirectory(UISettings.getInstance().getLastExportLocation().getParent());
		...
		if(dialog.getFile() != null) {
			...
			File file = new File(dialog.getDirectory() + File.separator + dialog.getFile());
			...
			try {
				...
				NodeList netNodes = doc.getDocumentElement().getElementsByTagName("LogFilter");
				...
				filter = null;
				e.printStackTrace();
				Message.add("Could not load log filter from " + file.getAbsolutePath(), Message.ERROR);
			}
