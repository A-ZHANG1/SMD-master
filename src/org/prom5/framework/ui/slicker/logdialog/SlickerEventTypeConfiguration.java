...
public class SlickerEventTypeConfiguration extends JPanel {
	...
	public SlickerEventTypeConfiguration(String[] eventTypes) {
		...
		for(int i=0; i<eventTypes.length; i++) {
			this.configurationItems[i] = new EventTypeConfigurationItem(eventTypes[i]);
	...
	protected class EventTypeConfigurationItem extends GradientPanel {
		...
		public EventTypeConfigurationItem(String type) {
			...
			this.actionLabel.setFont(this.actionLabel.getFont().deriveFont(12f).deriveFont(Font.ITALIC));
			if(type.equals("reassign") || type.equals("suspend") || type.equals("resume")) {
				setAction(EventTypeAction.REMOVE);
			} else if(type.equals("withdraw") || type.equals("ate_abort") || type.equals("pi_abort")) {
