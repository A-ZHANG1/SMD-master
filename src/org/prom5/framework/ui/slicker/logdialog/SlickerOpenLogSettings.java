...
public class SlickerOpenLogSettings extends OpenLogSettings {
	...
	// icons
	protected Image dashboardIcon = (new ImageIcon("images/slicker/dashboard48.png")).getImage();
	protected Image filterIcon = (new ImageIcon("images/slicker/filter48.png")).getImage();
	protected Image inspectorIcon = (new ImageIcon("images/slicker/inspector48.png")).getImage();
	protected Image summaryIcon = (new ImageIcon("images/slicker/summary48-2.png")).getImage();
	 ...
	 * @param logFile Log file to wrap around
	 */
	...
	protected void initialize() {
		...
		dashboardIcon = (new ImageIcon(promDir + "images/slicker/dashboard48.png")).getImage();
		filterIcon = (new ImageIcon(promDir + "images/slicker/filter48.png")).getImage();
		inspectorIcon = (new ImageIcon(promDir + "images/slicker/inspector48.png")).getImage();
		summaryIcon = (new ImageIcon(promDir + "images/slicker/summary48-2.png")).getImage();
		...
		SwingWorker loadWorker = new SwingWorker() {
			...
			public Object construct() {
				try {
					...
					// this usually signals that something has gone wrong while reading the log;
					// we expect a malformed or otherwise erroneous input file, so we abort here
	...
	protected void errorReadingLog(Throwable e) {
		...
		Message.add("Error opening log file " + logFile.toString(), Message.ERROR);
	...
	protected void completeGui() {
		GradientPanel backPanel = new GradientPanel(new Color(130, 130, 130), new Color(40, 40, 40));
		backPanel.setLayout(new BorderLayout());
		// compile filter panel
		FlatTabbedPane filterPane = new FlatTabbedPane("Log filter", new Color(240, 240, 240, 230), new Color(180, 180, 180, 120), new Color(220, 220, 220, 150));
		simpleFilterSettings = new SlickerSimpleFilterSettings(log, new ChangeListener() {
		...
		advancedFilterSettings = new SlickerAdvancedLogFilterConfiguration(log, new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				LogFilter aFilter = advancedFilterSettings.getLogFilter();
				// advanced filter has a real setting
				activeFilter = ActiveFilter.ADVANCED;
				currentFilter = aFilter;
				// delete current log (no safe comparison for advanced log filters)
				currentLog = null;
			}
		});
		...
		iconTabs.addTab("Inspector", inspectorIcon, inspectorUI, inspectorUI.getActivationListener());
	...
	protected int showMalformedLogWarning(String error) {
		...
		return JOptionPane.showOptionDialog(null,
                message,
                "Input log appears to be invalid!",
		...
		try {
			if(this.currentLog == null) {
				...
				if(this.currentFilter == null) {
					...
					// filter original
					this.currentLog = LogReaderFactory.createInstance(this.currentFilter, this.log);
				}
			} else {
				// check if current log has the current filter, otherwise create new filtered logs
				if(this.currentFilter != null &&
						this.currentLog.getLogFilter().equals(this.currentFilter) == false) {
					this.currentFilter.setLowLevelFilter(this.log.getLogFilter());
					this.currentLog = LogReaderFactory.createInstance(this.currentFilter, this.log);
				}
			}
	...
	public String getActiveLogFilterName() {
		if(this.activeFilter == ActiveFilter.INITIAL || this.currentFilter == null) {
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		if(currentLog != log) {
			...
			providedObjects.add(new ProvidedObject("Filtered " + logName + " (" + filterName + ")", new Object[] {currentLog}));
		}
		LogReader browserSelection = inspectorUI.getBrowserSelectionLog();
		if(browserSelection != null) {
			providedObjects.add(new ProvidedObject("Previewed " + logName + " (browser inspector)", new Object[] {browserSelection}));
		}
		LogReader cloudchamber = inspectorUI.getCloudChamberPreviewedLog();
		if(cloudchamber != null) {
			providedObjects.add(new ProvidedObject("Previewed " + logName + " (dotplot inspector)", new Object[] {cloudchamber}));
		}
		providedObjects.add(new ProvidedObject("Raw " + logName + " (unfiltered)", new Object[] {log}));
		timer.stop();
        Message.add("Filtered log in " + timer.formatDuration());
		return providedObjects.toArray(new ProvidedObject[providedObjects.size()]);
	}
	...
	/* (non-Javadoc)
	 * @see org.processmining.framework.ui.OpenLogSettings#getSelectedLogReader()
	 */
	@Override
