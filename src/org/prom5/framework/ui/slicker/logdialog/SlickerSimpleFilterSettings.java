...
import org.prom5.framework.log.filter.SimpleLogFilter;
import org.prom5.framework.ui.slicker.logdialog.SlickerEventTypeConfiguration.EventTypeAction;
...
public class SlickerSimpleFilterSettings extends JPanel {
	...
	public SlickerSimpleFilterSettings(LogReader log, ChangeListener updateListener) {
		...
		eventTypeConfiguration = new SlickerEventTypeConfiguration(log.getLogSummary().getEventTypes());
		JPanel leftPanel = new JPanel();
		leftPanel.setOpaque(false);
		leftPanel.setBorder(BorderFactory.createEmptyBorder());
		leftPanel.setLayout(new BoxLayout(leftPanel, BoxLayout.Y_AXIS));
		leftPanel.add(configureList(processList, "Processes", "Only green processes will be used."));
		//leftPanel.add(Box.createVerticalStrut(5));
		leftPanel.add(configureAnyScrollable(eventTypeConfiguration, "Event types", "Event types may be removed, or their enclosing instances discarded."));
		JPanel middlePanel = new JPanel();
		middlePanel.setOpaque(false);
		middlePanel.setBorder(BorderFactory.createEmptyBorder());
		middlePanel.setLayout(new BoxLayout(middlePanel, BoxLayout.Y_AXIS));
		middlePanel.add(configureList(startEventList, "Start events", "Only instances starting with a green event will be used."));
		//middlePanel.add(Box.createVerticalStrut(5));
		middlePanel.add(configureList(endEventList, "End events", "Only instances ending with a green event will be used."));
		listPanel.add(leftPanel);
		listPanel.add(middlePanel);
		listPanel.add(configureList(eventFilterList, "Event filter", "Only green events will be used."));
		...
		filter.setRemoveTypes(eventTypeConfiguration.getFilteredEventTypes(EventTypeAction.REMOVE));
		filter.setSkipInstanceTypes(eventTypeConfiguration.getFilteredEventTypes(EventTypeAction.SKIP_INSTANCE));
		// add listeners
	...
	protected JComponent configureAnyScrollable(JComponent scrollable, String title, String description) {
		...
		vBar.setUI(new SlickerScrollBarUI(vBar, colorListEnclosureBg, new Color(30, 30, 30), new Color(80, 80, 80), 4, 12));
