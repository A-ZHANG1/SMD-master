...
public class CenterOnScreen {
	...
	public static void center(Window frame) {
		...
		frame.setSize(Math.min(w, width()) + frame.getInsets().left + frame.getInsets().right + 20,
				Math.min(h, height()) + frame.getInsets().top + frame.getInsets().bottom + 20);

		frame.setLocation(Math.max(0, (screenSize.width - frame.getSize().width) / 2),
				Math.max(0, (screenSize.height - frame.getSize().height) / 2));
		frame.validate();
	}
