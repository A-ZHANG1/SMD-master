...
public class Dot {
	 ...
	 * consideration anymore (please use {@link execute #execute execute} instead).
	 * @param dotFilename String
	 * @param deleteFileAfterwards not used
	 * @return Graph
	 * @throws Exception
	 */
...
class DOTDialog extends JDialog {
	...
	private Graph runDot() throws Exception {
		...
		Message.add("Starting DOT on: " + dotFile.getAbsolutePath(), Message.DEBUG);

		// use custom user-defined dot executable, if specified
		/* old implementation: lots of checks..
		File customDotExe = UISettings.getInstance().getCustomDotExecutable();
		if(customDotExe != null && customDotExe.exists() &&
				(customDotExe.getName().equalsIgnoreCase("dot") || customDotExe.getName().equalsIgnoreCase("dot.exe"))) {
			String dotCommandString = customDotExe.getAbsolutePath() + " -q 5 ";
		*/
		String customDot = UISettings.getInstance().getCustomDotLocation();
		if(customDot != null) {
			...
			Message.add("Using custom dot executable at: " + customDot, Message.DEBUG);
			dot = Runtime.getRuntime().exec(dotCommandString);
		// On Windows systems, use the dot.exe distributed with ProM by default
		} else if (RuntimeUtils.isRunningWindows()) {
			dot = Runtime.getRuntime().exec("dot" + System.getProperty("file.separator") +
		...
		// by default (assumes standard installation into system-wide /Applications folder)
			...
			// assume UNIX-like OS with dot executable in $PATH
			dot = Runtime.getRuntime().exec("dot -q5 " + dotFile.getAbsolutePath());
		}
		...
		dot.destroy();
		Message.add("DOT finished on: " + dotFile.getAbsolutePath(), Message.DEBUG);
