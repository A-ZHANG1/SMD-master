...
public class GUIPropertyInteger implements ChangeListener {
	 ...
	 */
	public GUIPropertyInteger(String name, int defaultValue, int minValue, int maxValue) {
	 ...
	 * @param maxValue the maximal value that can be assigned to this property
	 */
	public GUIPropertyInteger(String name, String description, int defaultValue, int minValue, int maxValue) {
		this(name, description, defaultValue, minValue, maxValue, null, 100, true);
	}
	 ...
	 * @param target the object to be notified as soon the state of this property changes
	 */
	public GUIPropertyInteger(String name, String description, int defaultValue, 
			GuiNotificationTarget target, int width, boolean editable) {
		myName = name;
		myDescription = description;
		myValue = defaultValue;
	...
	public JPanel getPropertyPanel() {
		...
		if (myNoMinAndMax == true) {
			myModel = new SpinnerNumberModel(myValue /*initial value*/, null, null, 1 /*step*/);
			mySpinner = new JSpinner(myModel);
			mySpinner.setSize(new Dimension(myWidth, (int) mySpinner.getPreferredSize().getHeight()));
			mySpinner.setPreferredSize(new Dimension(myWidth, (int) mySpinner.getPreferredSize().getHeight()));
			mySpinner.setMaximumSize(new Dimension(myWidth,	(int) mySpinner.getPreferredSize().getHeight()));
			mySpinner.setMinimumSize(new Dimension(myWidth,	(int) mySpinner.getPreferredSize().getHeight()));
			mySpinner.setOpaque(false);
			myModel.addChangeListener(this);
		}
		else {
	        myModel = new SpinnerNumberModel(myValue /*initial value*/, myMinValue /*min*/, myMaxValue /*max*/, 1 /*step*/);
			mySpinner = new JSpinner(myModel);
			mySpinner.setSize(new Dimension(myWidth, (int) mySpinner.getPreferredSize().getHeight()));
			mySpinner.setPreferredSize(new Dimension(myWidth, (int) mySpinner.getPreferredSize().getHeight()));
			mySpinner.setMaximumSize(new Dimension(myWidth,	(int) mySpinner.getPreferredSize().getHeight()));
