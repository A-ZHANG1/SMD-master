 ...
 * the contents of the double-clicked element on the right. 
 * <br>
...
public class GenericTableModelPanel extends JPanel {
	 ...
	 * @param set the set of objects to be displayed
	 * @param name the name to be displayed in the table
	 * @param target the object to be informed as soon a new element has 
	 * been double-clicked in this table model
	 */
	 ...
	 * @param list the list of objects to be displayed
	 * @param name the name to be displayed in the table
	 * @param helpText the user help text to be displayed above the table and its contents
	 * @param target the object to be informed as soon a new element has 
	 * been double-clicked in this table model
	 */
	public GenericTableModelPanel(List<GuiDisplayable> list, String name, String helpText, 
			GuiNotificationTargetStateful target) {
		this(list, name, helpText, target, null);
	}
	
	public GenericTableModelPanel(List<GuiDisplayable> list, String name, String helpText, 
			GuiNotificationTargetStateful target, Color color) {
	 ...
	 * @return the objects that belong to the currently selected rows in the table
	 */
	...
	/**
	 * Method actually creating the GUI representation (without a helpful comment at the top 
	 * of the "table element content" window).
	 */
	 ...
	 * @param helpDescription the help text to be displayed at the top of the content frame
	 */
	private void createGUI(String helpDescription) {
		this.setLayout(new BoxLayout(this, BoxLayout.PAGE_AXIS));
		
		if (helpDescription != null) {
			GuiPropertyStringTextarea helpText = new GuiPropertyStringTextarea(helpDescription);
			this.add(helpText.getPropertyPanel(), BorderLayout.NORTH);
		}
		
		// fill the table
		myTable = new DoubleClickTable(new DisplayableObjectsTable(), updateViewButton);
		// build left GUI part
		...
		JSplitPane splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, leftPanel, myViewPanel);
