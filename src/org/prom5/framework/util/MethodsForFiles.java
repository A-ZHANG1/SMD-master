...
public class MethodsForFiles {
	...
	public static String[] listOfFilesToMine(String directory, String endsWith) throws IOException {
		...
		for (int i = 0; i < potentialFilesToMine.length; i++) {
			if (potentialFilesToMine[i].endsWith(endsWith)) {
				filesToMine = extractFiles(directory + File.separator + potentialFilesToMine[i]);
	 ...
	 * The directory given as parameter is also included in the returned HashSet.
	 * @param file directory to look for subdirectories.
	...
	/**
	 * Returns all the subdirectories of a given directory.
	 * The directory given as parameter is also included if it ends in "dirTermination".
	 * @param dir directory to look for subdirectories.
	 * @param dirSuffix suffix name of the directory to list
	 * @return a HashSet of Files. Every string is a subdirectory.
	 */
	public static HashSet getAllSubDirectoriesInHash(File dir, String dirTermination) {
		...
		if (f != null) {
			...
			for (int i = 0; i < f.length; i++) {
				...
				if (listedFile.isDirectory()) {
					...
					dirHash.addAll(getAllSubDirectoriesInHash(listedFile, dirTermination));
				}
			}
		}
		return dirHash;
	 ...
	 * @param file directory to look for files.
	 ...
	 * The search is recursive.
	 * @param file directory to look for files.
	 * @param fileTermination String the file termination of the retrieved files.
	 * This termination is case sensitive.
	 * @return a HashSet of Files. Every string is a file.
	 */
		...
		if (f != null) {
			for (int i = 0; i < f.length; i++) {
				...
				if (!listedFile.isDirectory()) {
					...
					files.addAll(getAllSubFilesInHash(listedFile, fileTermination));
				}
			}
		}
	...
	/**
	 * This methods lists the direct subdirectories of a given directory. No recursion is done. Only the
	 * first-level sudirectories are reported.
	 * @param inputDirectory directory to search for subdirectories.
