...
/**
 * <p>Title: </p>
 * <p>Description: This class reads configuration files. The parameters are separated by "=".
 * As an example, please see "C:\processmining\ProcessMining\configurationFiles\geneticMiningExperiments\basicFile.txt" </p>
...
public class Parameters {
	...
	private void readParameters(String file) {
		...
		try {
			...
			if (file.indexOf(File.separator) >= 0) {
				currentDirectory = file.substring(0, file.lastIndexOf(File.separator) + 1);
			}
	...
	/**
	 * Expects an array in the format "[value1 value2 ... valuen]".
	 * The double string should be parseable by the method <code> Double.parseDouble </code>.
	 * @return double[] the array with the double parameters. This array will be empty (size = 0)
	 * if the parameter name is not defined or if it is equal to "[]".
	 */

	public double[] getArrayDoublesParameter(String name) {
		double[] arrayDoubles;
		try {
			StringTokenizer stringArray = new StringTokenizer(this.getParameter(name), "[ ]");
	...
	/**
	 * Expects an array in the format "[value1 value2 ... valuen]".
	 * The integer string should be parseable by the method <code> Integer.parseInt </code>.
	 * @return int[] the array with the integer parameters. This array will be empty (size = 0)
	 * if the parameter name is not defined or if it is equal to "[]".
	 */
	...
	public static void main(String[] args) {
		...
		System.out.println(param.getIntParameter(
				"POP_SIZE") + " " +
