...
/**
 * <p>Title: PluginDocumentationLoader</p>
 *
 * <p>Description: Static class to load documentation from external file in
 *                 /lib/documentation/ subfolder</p>
 *
...
public class PluginDocumentationLoader {
	...
	/**
	 * Call load with a reference to the calling plugin to load the
	 * appropriate HTML body from a file with the name p.getName()+".html"
	 *
	 * @param p Plugin to load HTML body for
	...
	public static String load(String className) {
		String filename = System.getProperty("user.dir", "") +
						  ...
						  File.separator + className.substring(className.lastIndexOf(".") + 1) +
		...
		try {
			...
			if(html.matches(PluginDocumentationLoader.HTML_BODY_REGEX)) {
				html = html.replaceAll(PluginDocumentationLoader.HTML_BODY_REGEX, "$4");
			}
