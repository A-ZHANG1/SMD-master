...
public class ProMLpSolve {
    ...
    private void loadLibrary() {
        ...
        if (System.getProperty("os.name",
            ...
            path = System.getProperty("user.dir") +
                   ...
                   "external" + System.getProperty("file.separator");
    ...
    public void putAbortListener(AbortListener listener, Object object) throws
            ProMLpSolveException {
        try {
    ...
    public void setBounds(int i, double low, double up) throws
            ProMLpSolveException {
        try {
            if (presolved) {
                i = org2new[i];
    ...
    public void strAddConstraint(String constraint, int type,
                                 double val) throws ProMLpSolveException {
        try {
            if (presolved) {
    ...
    public int solve() throws ProMLpSolveException {
        try {
            ...
            // the mapping
            if (!presolved &&
                (problem.getNcolumns() != problem.getNorigColumns() ||
                 problem.getNrows() != problem.getNorigRows())
                    ) {
...
//		Message.add("solution: " + Arrays.toString(sol), Message.DEBUG);
    ...
    public void updateBounds(TreeSet < int[] > bounds) throws
            ProMLpSolveException {
        try {
    ...
    public void addConstraint(int[] constraint, int type, double val) throws
            ProMLpSolveException {
        try {
            String s = "";
