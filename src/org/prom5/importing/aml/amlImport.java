package org.prom5.importing.aml;
...
public class amlImport implements ImportPlugin, LogReaderConnectionImportPlugin {
	...
	public MiningResult importFile(InputStream input) throws IOException {
		try {
			...
			traverseAMLforObjectNames(ObjDef_Name, doc.getDocumentElement(), ObjDef_LinkId, ModelId_ModelType);
			...
			result = traverseAML(result, doc.getDocumentElement(), null, ObjDef_Name, ObjDef_LinkId,
					 modelid_net, function_LinkId);
	...
	public static void traverseAMLforObjectNames(HashMap partialMap, Node currentNode,
			HashMap ObjDef_LinkId, HashMap ModelId_ModelType) {
...
//					traverseAMLforObjectNames(partialMap, currentChild, ObjDef_LinkId);
				}
				if (currentChild.getNodeName().equals("ObjDef")) {
					...
					for (int k = 0; k < currentChildren.getLength(); k++) {
						...
						if (!Child.getNodeName().equals("AttrDef")) {
							continue;
						} else if (!Child.getAttributes().getNamedItem("AttrDef.Type").
					...
					for (int j = 0; j < currentChild.getAttributes().getLength(); j++) {
						if (currentChild.getAttributes().item(j).getNodeName().equals(
								"LinkedModels.IdRefs")) {
							String links = currentChild.getAttributes().getNamedItem(
										   "LinkedModels.IdRefs").getNodeValue();
							/*if (links.indexOf(" ") > -1) {
								Message.add("yes, yes, yes");
	...
	public EPCResult traverseAML(EPCResult partialResult, Node currentNode, Object parent,
			HashMap ObjDef_Name, HashMap ObjDef_LinkId, HashMap modelid_net,
			HashMap function_LinkId) throws Exception {
		if (currentNode.hasChildNodes()) {
			for (int i = 0; i < currentNode.getChildNodes().getLength(); i++) {
				Node currentChild = currentNode.getChildNodes().item(i);
				if (currentChild.getNodeName().equals("Group")) {
					...
					if (currentChild.hasChildNodes()) {
						...
						for (int j = 0; j < currentChildren.getLength(); j++) {
							...
							if (Child.getAttributes().getNamedItem("AttrDef.Type").getNodeValue().
									equals("AT_NAME")) {
								if (Child.hasChildNodes()) {
									for (int l = 0; l < Child.getChildNodes().getLength(); l++) {
										if (!(Child.getChildNodes().item(l).getNodeName().equals(
												"AttrValue"))) {
											continue;
										} else {
					...
					partialResult = traverseAML(partialResult, currentChild, dir, ObjDef_Name,
									ObjDef_LinkId, modelid_net, function_LinkId);
				}
				if (currentChild.getNodeName().equals("Model") &&
						currentChild.getAttributes().getNamedItem("Model.Type").
						getNodeValue().equals("MT_EEPC")) {
					...
					try {
						...
						modelid_net.put(currentChild.getAttributes().getNamedItem("Model.ID").
								getNodeValue(), net);
					} catch (Throwable x) {
	...
	public static EPC read(Node node, HashMap ObjDef_Name, HashMap ObjDef_LinkId,
			HashMap function_LinkId,String ModelName) throws Exception {
		EPC result = new EPC(true);
		//Message.add("1.read " + node.getAttributes().getNamedItem("Model.ID").getNodeValue());
		parseNet(node, result, ObjDef_Name, ObjDef_LinkId, function_LinkId, ModelName);
		//Message.add("2.read " + node.getNodeName());
		return result;
	}

	private static void parseNet(Node node, EPC net, HashMap ObjDef_Name, HashMap ObjDef_LinkId,
			HashMap function_LinkId, String ModelName) throws Exception {
...
//			Message.add("YES " + n.getAttributes().getNamedItem("ObjOcc.ID").getNodeValue());
...
//			Message.add(id + " " + ownName);
			...
			if (symbolnum.equals("ST_FUNC")) {
				if (ObjDef_LinkId.containsKey(ObjDef)) {
					EPCSubstFunction sf = (EPCSubstFunction) net.addFunction(new EPCSubstFunction(new
										  LogEvent(ownName, "unknown:normal"), net, null));
					sf.setIdentifier(ownName);
					mapping.put(id, sf);
					function_LinkId.put(sf, ObjDef_LinkId.get(ObjDef));
				} else {
					EPCFunction f = net.addFunction(new EPCFunction(new LogEvent(ownName,
									"unknown:normal"), net));
				...
				EPCConnector c = net.addConnector(new EPCConnector(EPCConnector.OR, net));
				mapping.put(id, c);
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			if (n.hasChildNodes()) {
				for (int j = 0; j < n.getChildNodes().getLength(); j++) {
					if (n.getChildNodes().item(j).getNodeName().equals(
							"CxnOcc")) {
						Node CxnOcc = n.getChildNodes().item(j);
						String dest = CxnOcc.getAttributes().getNamedItem("ToObjOcc.IdRef").
									  getNodeValue();
