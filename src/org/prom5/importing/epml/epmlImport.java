package org.prom5.importing.epml;
...
import org.prom5.framework.models.ModelHierarchyDirectory;
import org.prom5.framework.models.epcpack.ConfigurableEPC;
import org.prom5.framework.models.epcpack.EPCConnector;
import org.prom5.framework.models.epcpack.EPCDataObject;
import org.prom5.framework.models.epcpack.EPCEvent;
import org.prom5.framework.models.epcpack.EPCFunction;
import org.prom5.framework.models.epcpack.EPCInfSysObject;
import org.prom5.framework.models.epcpack.EPCObject;
import org.prom5.framework.models.epcpack.EPCOrgObject;
import org.prom5.framework.models.epcpack.EPCSubstFunction;
...
public class epmlImport implements ImportPlugin, LogReaderConnectionImportPlugin {
	...
	public MiningResult importFile(InputStream input) throws IOException {
		try {
			...
			if (! (doc.getDocumentElement().getNodeName().equals("epml")||
					doc.getDocumentElement().getNodeName().indexOf(":epml")>-1) ) {
			...
			traverseForNonControlFlow(doc.getDocumentElement(),noncfId_Node,relation,"");
			...
			result = traverseEPML(result,doc.getDocumentElement(),null,function_epc,epcId_net,Id_noncfNodes);
			...
			while (hierarchicalFunctions.hasNext()) {
				...
				f.setSubstitutedEPC( (ConfigurableEPC) epcId_net.get(function_epc.get(f)) );
	...
	public static void traverseForNonControlFlow(Node currentNode,HashMap epcIdId_node,Vector relation, String epcId) {
		if (currentNode.hasChildNodes()) {
			for (int i = 0; i < currentNode.getChildNodes().getLength(); i++) {
				...
				if (currentChild.getNodeName().equals("directory")) {
					traverseForNonControlFlow(currentChild,epcIdId_node,relation,epcId);
				}
				if (currentChild.getNodeName().equals("epc")) {
					if (currentChild.getAttributes().getNamedItem("epcId")==null) {
						epcId = currentChild.getAttributes().getNamedItem("EpcId").getNodeValue();
					} else {
						epcId = currentChild.getAttributes().getNamedItem("epcId").getNodeValue();
					}
					traverseForNonControlFlow(currentChild,epcIdId_node,relation,epcId);
				...
				if (currentChild.getNodeName().equals("participant")) {
					String id = currentChild.getAttributes().getNamedItem("id").getNodeValue();
				...
				if (currentChild.getNodeName().equals("relation")) {
					String from = currentChild.getAttributes().getNamedItem("from").getNodeValue();
					String to = currentChild.getAttributes().getNamedItem("to").getNodeValue();
	...
	public EPCResult traverseEPML(EPCResult partialResult, Node currentNode,Object parent,HashMap function_epc,HashMap epcId_net,HashMap Id_noncfNodes) {
		if (currentNode.hasChildNodes()) {
			for (int i = 0; i < currentNode.getChildNodes().getLength(); i++) {
				Node currentChild = currentNode.getChildNodes().item(i) ;
				if (currentChild.getNodeName().equals("directory")) {
					if (currentChild.getAttributes().getNamedItem("name")==null) {
						...
						ModelHierarchyDirectory dir = new ModelHierarchyDirectory(domstring,name);
						partialResult.addInHierarchy(dir,parent,name);
						partialResult = traverseEPML(partialResult,currentChild,dir,function_epc,epcId_net,Id_noncfNodes);
					} else {
						String name = currentChild.getAttributes().getNamedItem("name").getNodeValue();
						Message.add("directory "+name);
						String domstring = "-"+currentChild.hashCode();
				...
				if (currentChild.getNodeName().equals("epc")) {
					try {
						ConfigurableEPC net = read(currentChild,function_epc,Id_noncfNodes);
						String name =null;
						if (currentChild.getAttributes().getNamedItem("name")==null) {
							name = currentChild.getAttributes().getNamedItem("Name").getNodeValue();
						} else {
							name = currentChild.getAttributes().getNamedItem("name").getNodeValue();
						}
						...
						if (currentChild.getAttributes().getNamedItem("epcId")==null) {
							epcId_net.put(currentChild.getAttributes().getNamedItem("EpcId").getNodeValue(),net);
						} else {
							epcId_net.put(currentChild.getAttributes().getNamedItem("epcId").getNodeValue(),net);
						}
					} catch (Throwable x) {
	...
	public static String getLinkedEpcId(Node epcnode) {
		...
		if (epcnode.hasChildNodes()){
			for (int i=0; i < epcnode.getChildNodes().getLength();i++) {
				...
				linkedEpc = current.getAttributes().getNamedItem("linkToEpcId").getNodeValue();
				break;
			}
		}
		return linkedEpc;
	...
	public static ConfigurableEPC read(Node node, HashMap function_epc, HashMap Id_noncfNodes) throws Exception {
		ConfigurableEPC result = new ConfigurableEPC(false);
		parseNet(node, result, function_epc, Id_noncfNodes);
		return result;
	}

	private static void parseNet(Node node, ConfigurableEPC net, HashMap function_epc, HashMap Id_noncfNodes) throws Exception {
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			if (!(n.getNodeName().equals("function") ||
					...
					n.getNodeName().equals("or") ||
					n.getNodeName().equals("xor"))) {
				continue;
			}
			...
			for (int j = 0; j < n.getChildNodes().getLength(); j++) {
				if (n.getChildNodes().item(j).getNodeName().equals("name")) {
					try {
						ownName = n.getChildNodes().item(j).getFirstChild().getNodeValue();
					} catch (Exception e) {
					}
				} else
				if ((n.getChildNodes().item(j).getAttributes()!=null) &&
						(n.getChildNodes().item(j).getAttributes().getNamedItem("defId") != null)) {
					try {
					} catch (Exception e) {
					}
				}
			...
			if (n.getNodeName().equals("function")) {
				...
				if (!potentialLink.equals("-1")) {
					f = (EPCSubstFunction) net.addFunction(new EPCSubstFunction(new LogEvent(ownName,"unknown:normal"),isConfigurable(n), net,null));
					f.setIdentifier(ownName);
					mapping.put(id, f);
					function_epc.put(f,potentialLink);
				} else {
					f = (EPCFunction) net.addFunction(new EPCFunction(new LogEvent(ownName,
					"unknown:normal"), isConfigurable(n),net));
					f.setIdentifier(ownName);
					mapping.put(id, f);
				}
				if (Id_noncfNodes != null) {
					if (Id_noncfNodes.containsKey(epcId+"_"+id)) {
						Vector noncfs = (Vector) Id_noncfNodes.get(epcId+"_"+id);
							...
							if (current.getNodeName().equals("application")) {
								...
								if (current.hasChildNodes()) {
									for (int j = 0; j < current.getChildNodes().getLength(); j++) {
										if (current.getChildNodes().item(j).getNodeName().equals("name")) {
											try {
												appname = current.getChildNodes().item(j).getFirstChild().getNodeValue();
											} catch (Exception e) {
											}
										}
									}
								}
								f.addInfSysObject(new EPCInfSysObject(appname,f));
							}
							if (current.getNodeName().equals("participant")) {
								...
								if (current.hasChildNodes()) {
									for (int j = 0; j < current.getChildNodes().getLength(); j++) {
										if (current.getChildNodes().item(j).getNodeName().equals("name")) {
											try {
												orgname = current.getChildNodes().item(j).getFirstChild().getNodeValue();
											} catch (Exception e) {
											}
										}
									}
							...
							if (current.getNodeName().equals("dataField")) {
								...
								if (current.hasChildNodes()) {
									for (int j = 0; j < current.getChildNodes().getLength(); j++) {
										if (current.getChildNodes().item(j).getNodeName().equals("name")) {
											try {
												dataname = current.getChildNodes().item(j).getFirstChild().getNodeValue();
											} catch (Exception e) {
											}
										}
									}
				...
				//Message.add(f.getNumDataObjects()+" "+ f.getNumInfSysObjects()+" "+ f.getNumOrgObjects());
				...
				EPCConnector c = net.addConnector(new EPCConnector(EPCConnector.AND, isConfigurable(n),net));
				mapping.put(id, c);
			} else if (n.getNodeName().equals("or")) {
				EPCConnector c = net.addConnector(new EPCConnector(EPCConnector.OR, isConfigurable(n),net));
				mapping.put(id, c);
			} else if (n.getNodeName().equals("xor")) {
				EPCConnector c = net.addConnector(new EPCConnector(EPCConnector.XOR, isConfigurable(n),net));
				mapping.put(id, c);
			}
		}
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			String dest = flow.getAttributes().getNamedItem("target").getNodeValue();
			if (mapping.get(dest) == null) {
				continue;
			}
			if (net.addEdge((EPCObject) mapping.get(source), (EPCObject) mapping.get(dest)) == null) {
				throw (new Exception(
						"<html>Structural properties of EPCs are violated in input file.<br>" +
						"The following edge could not be added:<br><br>" +
						mapping.get(source).toString() + " ==> " + mapping.get(dest).toString() +
