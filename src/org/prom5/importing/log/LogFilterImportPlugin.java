...
package org.prom5.importing.log;
...
public class LogFilterImportPlugin implements ImportPlugin {
	...
	public String getHtmlDescription() {
		return
				"Imports a LogFilter using an XML file. After importing, it adds the log Filter to the" +
	...
	private MiningResult importNow(InputStream input) throws Exception {
		...
		if (netNodes.getLength() > 0) {
			...
			f = new AbstractLogFilter(((ProMInputStream) input).getFileName(), f);
			MainUI.getInstance().addGlobalProvidedObject(new ProvidedObject(f.getName(),
					new Object[] {f}));
		}
		if (f != null) {
			Message.add("Import of LogFilter succesfull.");
			Message.add("The imported LogFilter is now available to the framework.");
		} else {
