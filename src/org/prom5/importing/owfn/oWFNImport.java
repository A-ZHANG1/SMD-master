...
package org.prom5.importing.owfn;
...
/**
 * <p>Title: oWFNImport</p>
 *
 * <p>Description: Import an oWFN file. Result will be an oWFNet object.</p>
 *
...
public class oWFNImport implements LogReaderConnectionImportPlugin {
	...
	public PetriNetResult importFile(InputStream input) throws IOException {
		...
		// Deflate the input, helps parsing it. Deflating means removing comments and white spaces.
	...
	/**
	 *
	 * @param contents String Deflated oWF net formatted string.
	 * @param petriNet oWFNet open WF net to hold the parsing results.
	...
	public String parseContents(String contents, oWFNet petriNet) {
		...
		while (contents.length() > 0) {
			...
			// This is a bit awkward: to find the end of the transition label, we need to find the next keyowrd (CONSUME).
	 ...
	 * @param token String
	 * @return String The contents with the token removed, if possible. Otherwise, the contents.
	 */
	...
	/**
	 * Read a list of places from the contents. Store the places in the given oWF net.
	 * Mode specifies whether the places are internal (0), input (1), or output (2).
	 * @param contents String
	 * @param petriNet oWFNet
	 * @param mode int
	 * @return String
	 */
	...
	public String parseMarking(String contents, oWFNet petriNet) {
		...
		while (data.length() > 1) {
			...
			if (j > 0 && k > 0 && k < j) {
				...
				String weight = data.substring(k+1, j); // assumed to be 1 for the time being
	...
	/**
	 * Read a list of weighted places, and store it as preset (isInput = true) or postset (isInpuyt = false) of the
	 * given transition in the given open WF net.
	 * @param contents String
	 * @param petriNet oWFNet
	 ...
	 * @return String
	 */
	public String parseEdges(String contents, oWFNet petriNet, Transition transition, boolean isInput) {
		int i = contents.indexOf(";");
		String data = contents.substring(0, i < 0 ? 0 : i) + ",";
		while (data.length() > 1) {
			String name;
