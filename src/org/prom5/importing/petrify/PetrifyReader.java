...
public class PetrifyReader {
    ...
    public static PetriNet read(InputStream input) throws IOException {
        ...
        sRead = in.readLine();
        sRead = sRead.replace("/", PetrifyConstants.EDGEDOCSSEPARATOR); //dot doesn't eat '/' symbols
        while (sRead.charAt(0) != '.') {
            // look for new transitions (with "__") and new places and add them to the lists
    ...
    static ArrayList<String> checkForNewTransitionsAndPlaces(String sRead,
            ...
            PetriNet pn) {
        ArrayList<String>
                stParts = new ArrayList<String>(Arrays.asList(sRead.split(" ")));
        int k;
    ...
    public static boolean isPlace(String st) {
        if (st.charAt(0) == 'p' &&
            ...
            Character.getType(st.charAt(st.length() - 1)) ==
    ...
    public static Transition findTransition(String identifier, ArrayList<Transition>
            transitions) {
    ...
    public static String repairTransitionStringForMapping(String st) {
        ...
        while (k != -1) {
            ...
            newk = st.indexOf(PetrifyConstants.EDGEDOCSSEPARATOR, newStartIndex);
            if (newk != -1) { // there is a docs separator in the string
                ...
                result = result.concat(st.substring(newStartIndex, st.length())).
                         concat(")");
                break;
            }
        }
