...
package org.prom5.importing.ppmgraphformat;
...
public class PPMGraphFormatImport implements LogReaderConnectionImportPlugin {
			...
			db.setEntityResolver(new EntityResolver() {
				public InputSource resolveEntity(String publicId, String systemId) {
					if (systemId.indexOf("graph.dtd") != -1) {
						return new InputSource("file:" + About.EXTLIBLOCATION() + "graph.dtd");
					} else {
						return null;
					}
				}
	...
	private void parseNet(Node node, EPC net) throws Exception {
		...
		net.setIdentifier(node.getAttributes().getNamedItem("id").getNodeValue());
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			for (int j = 0; j < n.getChildNodes().getLength(); j++) {
				if (n.getChildNodes().item(j).getAttributes().getNamedItem("type").getNodeValue().
						equals(
						"AT_OBJNAME")) {
					try {
						objName = n.getChildNodes().item(j).getChildNodes().item(0).getNodeValue();
					} catch (Exception e) {
					}
				}
			}
			...
			if (spaceIndex>0 && LogStateMachine.getInstance().isPossibleEvent(event)) {
				objName = objName.substring(0,spaceIndex).trim();
			} else {
				event = "unknown:normal";
			}

			if (nodes.item(i).getAttributes().getNamedItem("type").getNodeValue().equals("OT_FUNC")) {
				EPCFunction f = net.addFunction(new EPCFunction(new LogEvent(objName,
								event), net));
				f.setIdentifier(objName);
				mapping.put(id, f);
			} else if (nodes.item(i).getAttributes().getNamedItem("type").getNodeValue().equals(
					"OT_EVT")) {
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			if (!n.getAttributes().getNamedItem("type").getNodeValue().equals("CXN_FOLLOWS")) {
				continue;
			}
			...
			String dest = n.getAttributes().getNamedItem("target").getNodeValue();
