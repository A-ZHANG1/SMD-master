package org.prom5.importing.vdx;
...
public class vdxImport implements ImportPlugin, LogReaderConnectionImportPlugin {
	...
	public MiningResult importFile(InputStream input) throws IOException {
		...
		try {
			...
			for (int i=0; i<pagesNode.getLength() /*&& i<5*/; i++) {  // max of 5 pages
				...
				if (pageName == null || pageName.equals("")) pageName = currentChild.getAttribute("NameU");
				if (pageName == null || pageName.equals("")) pageName = "A";
				newEPC.setIdentifier(pageName);

				Message.add("parsing page: "+pageName,Message.DEBUG);

				// parse the shapes
				NodeList shapesNode = currentChild.getElementsByTagName("Shapes");
				...
				NodeList connectsNode = currentChild.getElementsByTagName("Connects");
				...
				while (shapesMapIt.hasNext()) {
					...
					String whatIsIt = (String)mastersMap.get(shapesMap.get(currKey));
					Message.add("Shapes mapping: " +currKey+ " " +whatIsIt, Message.DEBUG);
					...
					if (whatIsIt.equals("Event")) {
						...
						EPCEvent e = newEPC.addEvent(new EPCEvent(identifier, newEPC));
					...
					if (whatIsIt.equals("Function")) {
						...
						EPCFunction f = (EPCFunction) newEPC.addFunction(new EPCFunction(new LogEvent(identifier,
							"unknown:normal"), true,newEPC));
						f.setIdentifier(identifier);
						shapesEPCObjectsMap.put(currKey,f);
						continue;
					} else {
					if (whatIsIt.equals("AND")) {
						EPCConnector c = newEPC.addConnector(new EPCConnector(EPCConnector.AND, true,newEPC));
						c.setIdentifier("AND"+currKey);
						shapesEPCObjectsMap.put(currKey,c);
						continue;
					} else {
					if (whatIsIt.equals("OR")) {
						EPCConnector c = newEPC.addConnector(new EPCConnector(EPCConnector.OR, true,newEPC));
						c.setIdentifier("OR"+currKey);
						shapesEPCObjectsMap.put(currKey,c);
						continue;
					} else {
					if (whatIsIt.equals("XOR")) {
						EPCConnector c = newEPC.addConnector(new EPCConnector(EPCConnector.XOR, true,newEPC));
						c.setIdentifier("XOR"+currKey);
						shapesEPCObjectsMap.put(currKey,c);
						continue;
					} else {
						if (!whatIsIt.equals("Dynamic connector")) {
							Message.add("Ignoring not available Object: "+whatIsIt);
							shapesNotInEPCObjectsMap.put(currKey,"object not supported");
						}
					}
					} } } }
				}
				...
				while (connectsMapIt.hasNext()) {
					...
					// now draw the edge
					if (shapesMap.get(beginID) != null && shapesMap.get(endID) != null) {
						// only draw edge if the objects are not ignored
						if (shapesNotInEPCObjectsMap.get(beginID) == null && shapesNotInEPCObjectsMap.get(endID) == null) {
							newEPC.addEdge((EPCObject)shapesEPCObjectsMap.get(beginID),(EPCObject)shapesEPCObjectsMap.get(endID));
						}
					}
	...
	public static void parseMasters(NodeList mastersNode, HashMap mastersMap) {
		...
		Message.add(mastersNode.getLength()+" master tags found", Message.DEBUG);
		for (int i=0; i<mastersNode.getLength(); i++) {
			...
			Message.add(currentChild.getAttributeNode("ID").getValue(), Message.DEBUG);
			Message.add(currentChild.getAttributeNode("NameU").getValue(), Message.DEBUG);
		}

	}

	public static void parsePageShapes(NodeList shapeNodes, HashMap shapesMap, HashMap shapesTextMap) {
		...
		for (int i=0; i<shapeNodes.getLength(); i++) {
			...
			String masterId = currentChild.getAttributeNode("Master").getValue();
			shapesMap.put(shapeId,masterId);

			String labelText = "";
			for (int j = 0; j < currentChild.getChildNodes().getLength(); j++) {
				if (currentChild.getChildNodes().item(j).getNodeName().equals("Text")) {
					try {
						labelText = currentChild.getChildNodes().item(j).getFirstChild().getNodeValue();
					} catch (Exception e) {
					}
				}
			}
	...
	public static void parsePageConnects(NodeList connectNodes, HashMap connectsMap) {
		...
		for (int i=0; i<connectNodes.getLength(); i+=2) {
			...
			String connectIDBegin = currentChildBegin.getAttributeNode("FromSheet").getValue();
			String connectIDEnd = null;
			try {
				connectIDEnd = currentChildEnd.getAttributeNode("FromSheet").getValue();
			} catch (Exception e) {
			...
			if (!connectIDBegin.equals(connectIDEnd)) {
				...
				String whatIDBegin = currentChildBegin.getAttributeNode("ToSheet").getValue(); // which shape ID
				String whatIDEnde = currentChildEnd.getAttributeNode("ToSheet").getValue(); // which shape ID
