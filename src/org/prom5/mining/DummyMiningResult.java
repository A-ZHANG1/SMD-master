...
public class DummyMiningResult extends JPanel implements MiningResult, Provider {
	...
	public DummyMiningResult(LogReader log, ArrayList PINames) {
		this.log = log;
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		if (visualised != null) {
			a.add(new ProvidedObject("Instance " + visualised.getName() + " of " + s,
					new Object[] {visualised}));
		}

		if (jTable1.getSelectedRowCount() > 0) {
			try {
				a.add(new ProvidedObject("Selection of " + s, new Object[] {
						LogReaderFactory.createInstance(log,jTable1.getSelectedRows()) }));
			} catch(Exception e) {
				e.printStackTrace();
			}
		}
			...
			public void actionPerformed(ActionEvent actionEvent) {
				...
				if (i >= 0) {
					...
					jScrollPane1.getViewport().add(
							(new ProcessInstanceVisualization(visualised)).getGrappaVisualization() );
