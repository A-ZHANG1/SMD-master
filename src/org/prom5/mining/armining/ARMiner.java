...
/**
 * <p>Title: ARMiner</p>
 * <p>Description:The mining plug-in "Association Rule Miner" generates association rules
 * for the event log. It also calculates frequent itemsets. The algorithms implemented in this plug-in
 * are the Apriori algorithm and the Predictive Apriori algorithm.</p>
...
public class ARMiner implements MiningPlugin {
	...
	public String getHtmlDescription() {
		return ("<h1>" + getName() + "</h1>" + PluginDocumentationLoader.load(this));
	}
	...
	/**
	 * Return a JPanel that contains a user interface for setting the options specific to this plugin.
	 *
	...
	public MiningResult mine(LogReader logReader) {
		...
		//whether the user wants to insert a no name activity (sort of dummy activity)
		...
		if (dummyChkBoxValue == true && eventCareChkBoxValue == true) {
			...
			if (dummyChkBoxValue == true && eventCareChkBoxValue == false) {
				...
				if (dummyChkBoxValue == false && eventCareChkBoxValue == true) {
					...
					try { // dont do anything, work on original log (without event type information), both are false
		...
		//myBuildAssociations contains the actual method buildAssociations() available in Weka that actually does the mining work.
		myAnalyzer.myBuildAssociations();
		//The result in form of rules/FIS is given to the ARMinerResult class for display.
