package org.prom5.mining.armining;
...
/**
 * <p>Title:ARMinerResult </p>
 *
 * <p>Description: This class builds the output GUI. </p>
 *
...
//GUI variables
	...
	/**
	 * Obtains the names of the process instances in the log and stores them
	 * in the instanceIDs list
	 */
	...
	public ARMinerResult(AssociationAnalyzer currentAnalyzer, LogReader currentReader) {
		...
		try {
			...
			Message.add("Number of process instances (cases) = " +
					logReader.getLogSummary().getNumberOfProcessInstances(), Message.TEST);
			Message.add("Number of audit trail entries (events) = " +
					logReader.getLogSummary().getNumberOfAuditTrailEntries() + "\n", Message.TEST);

			Message.add("Algorithm selected:" + myAnalyzer.toString(), Message.TEST);
			Message.add("Number of rules: " + myAnalyzer.getRules().size(), Message.TEST);

			if (myAnalyzer instanceof AprioriAnalyzer) {
				Message.add("Confidence: " + myAnalyzer.getConfValue(), Message.TEST);
				Message.add("Upper Bound for Min Support: " + myAnalyzer.getUpperBoundMinSup(),
						Message.TEST);
				Message.add("Lower Bound for Min Support:  " + myAnalyzer.getLowerBoundMinSup(),
						Message.TEST);
				Message.add("Output FIS?" + myAnalyzer.getFreqItemSets(), Message.TEST);
			}

			Message.add("Care about Event Type?" + myAnalyzer.getETypeValue(), Message.TEST);
			Message.add("Save as ARFF file?: " + myAnalyzer.getSaveARFFValue() + "\n", Message.TEST);

			if (myAnalyzer.getFreqItemSets() == true) {
				Message.add(myAnalyzer.getRules().size() +
						"  association rules and frequent itemsets are generated successfully with the " +
						myAnalyzer.toString() + " algorithm", Message.TEST);
			} else {
				Message.add(myAnalyzer.getRules().size() +
						" association rules are generated successfully with the " + myAnalyzer.toString() +
						" algorithm", Message.TEST);
			}

			if (myAnalyzer.getETypeValue() == true) {
				Message.add("Association rules are generated with event type information about the tasks.",
						Message.TEST);
			}

			if (myAnalyzer.getSaveARFFValue() == true && myAnalyzer.getlocationARFFFile() != "") {
				Message.add("The intermediate input ARFF file has been saved to the location: " +
						myAnalyzer.getlocationARFFFile().toString() + ".", Message.TEST);
			}
			Message.add("</Association Rule Mining>", Message.TEST);
	...
	private void jbInit() throws Exception {
		...
		if (myAnalyzer instanceof AprioriAnalyzer) {
			...
			if (myFISValue == true && myAnalyzer.isCheckBoxECSelected() == true) {
				//if (myFISValue == true) {
				myTabPane.removeAll();
				myRulesPanel.setLayout(new BoxLayout(myRulesPanel, BoxLayout.Y_AXIS));
				myRulesPanel.add(Box.createVerticalStrut(15));
				//panel for showing rules in text area
				myPanel.setLayout(new BoxLayout(myPanel, BoxLayout.X_AXIS));
				rulesInTextPanel.setLayout(new BoxLayout(rulesInTextPanel, BoxLayout.X_AXIS));
				myRulesWithEType1 = new ArrayList(myAnalyzer.getRules());
				if (myRulesWithEType1.isEmpty() == true) {
					JOptionPane.showMessageDialog(rulesInTextPanel,
				...
				processInstanceIDsTable = new DoubleClickTable(new ExtendedLogTable(), null);
				...
				invertButton.setToolTipText("Invert the current selection of PIs.");
				...
				invertButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						if ("Invert".equals(e.getActionCommand())) {
							...
							for (int index = 0; index < logReader.numberOfInstances(); index++) {
				...
				myRulesListWithEType1 = new GUIPropertyListEnumeration("Select Rule: ", null,
										myRulesWithEType1, null, 500);
				...
				clusterRuleButton.setToolTipText(
						"Retrieve the cluster satisfying the selected rule.");
				...
				clusterRuleButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						if ("Cluster".equals(e.getActionCommand())) {
							...
							int ruleIndex = myRulesWithEType1.indexOf(myRulesListWithEType1.
											getValue());
						...
						numSelectedPI = new JLabel("Num of PIs satisfying this rule: " +
										myRulesPI.size());
				...
				myRulesPanel.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
//-----------------------------------------------------------------------------------------------------------
				//panel for showing FIS in combo box
				myFISPanel.setLayout(new BoxLayout(myFISPanel, BoxLayout.Y_AXIS));
				myFISPanel.add(Box.createVerticalStrut(15));

				myPanel2.setLayout(new BoxLayout(myPanel2, BoxLayout.X_AXIS));
				FISInTextPanel.setLayout(new BoxLayout(FISInTextPanel, BoxLayout.X_AXIS));
				myFISArrayListWithEType = new ArrayList(myAnalyzer.showFreqItemSets());
				FISInTextPanel.add(MyApriori.newDisplay(myFISArrayListWithEType));
				...
				clusterFISPanel.setLayout(new BoxLayout(clusterFISPanel, BoxLayout.Y_AXIS));
				processInstanceIDsTable2 = new DoubleClickTable(new ExtendedLogTable(), null);
				...
				clusterFISTablePanel.setLayout(new BoxLayout(clusterFISTablePanel, BoxLayout.Y_AXIS));
				clusterFISTablePanel.add(tableContainer2);

				JPanel invertFISButtonPanel = new JPanel();
				invertFISButtonPanel.setLayout(new BoxLayout(invertFISButtonPanel, BoxLayout.X_AXIS));
				invertFISButton.setEnabled(true);
				invertFISButton.setMnemonic(KeyEvent.VK_I);
				invertFISButton.setActionCommand("Invert");
				invertFISButton.setToolTipText("Invert the current selection of PIs.");
				...
				myFISListWithEType = new GUIPropertyListEnumeration("Select FIS", null,
									 myFISArrayListWithEType, null, 500);
				...
				clusterFISButton.setToolTipText("Retrieve the cluster satisfying the selected FIS.");
				...
				clusterFISButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						if ("Cluster".equals(e.getActionCommand())) {
							...
							//get the index of currently selected rule
							int FISIndex = myFISArrayListWithEType.indexOf(myFISListWithEType.
										   getValue());
							for (int j = 0; j < logReader.numberOfInstances(); j++) {
								ProcessInstance pi = logReader.getInstance(j);
						...
						numSelectedPIFIS = new JLabel("Num of PIs satisfying this FIS: " +
										   myFISPI.size());
				...
				myFISPanel.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
				myTabPane.addTab("Apriori-Frequent Itemsets", null, myFISPanel, "view FIS");
				myTabPane.addTab("Apriori- Association Rules", null, myRulesPanel,
						"View association rules");
				topPanel.add(myTabPane);
			}
//*************************************************************************
			 if (myFISValue == false && myAnalyzer.isCheckBoxECSelected() == true) { //show only rules with Event Type information
				 myTabPane.removeAll();
				 myRulesPanel.setLayout(new BoxLayout(myRulesPanel, BoxLayout.Y_AXIS));
				 myRulesPanel.add(Box.createVerticalStrut(15));
				 ...
				 //panel for showing rules in combo box
				 clusterRulesPanel.setLayout(new FlowLayout());
				 myRulesListWithEType2 = new GUIPropertyListEnumeration("Select Rule: ", null,
										 myRulesWithEType2, null, 500);
				 JButton clusterRuleButton = new JButton("Cluster");
				 clusterRuleButton.setEnabled(true);
				 clusterRuleButton.setMnemonic(KeyEvent.VK_C);
				 clusterRuleButton.setActionCommand("Cluster");
				 clusterRuleButton.setToolTipText(
						 "Click this button to retrieve the cluster satisfying the selected rule.");
				 ...
				 clusterRuleButton.addActionListener(new ActionListener() {
					 public void actionPerformed(ActionEvent e) {
						 if ("Cluster".equals(e.getActionCommand())) {
							 ...
							 numSelectedPI = new JLabel("");
							 //get the index of currently selected rule
							 int ruleIndex = myRulesWithEType2.indexOf(myRulesListWithEType2.
											 getValue());
							 for (int j = 0; j < logReader.numberOfInstances(); j++) {
								 ProcessInstance pi = logReader.getInstance(j);
								 if (myAnalyzer.checkWithEC(pi, ruleIndex) == true) {
				 ...
				 myRulesPanel.add(Box.createRigidArea(new Dimension(0, 25)));
				 myRulesPanel.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
				 myTabPane.addTab("Apriori algorithm-Association Rules", null, myRulesPanel,
						 "View association rules");
				 topPanel.add(myTabPane);
			 }
//****************************************************************
			 if (myFISValue == true && myAnalyzer.isCheckBoxECSelected() == false) {
				 myTabPane.removeAll();
				 myRulesPanel.setLayout(new BoxLayout(myRulesPanel, BoxLayout.Y_AXIS));
				 myRulesPanel.add(Box.createVerticalStrut(15));
				 ...
				 //panel for showing rules in combo box
				 clusterRulesPanel.setLayout(new FlowLayout());
				 myRulesList1 = new GUIPropertyListEnumeration("Select Rule: ", null, myRules1, null,
								500);
				 JButton clusterRuleButton = new JButton("Cluster");
				 clusterRuleButton.setEnabled(true);
				 clusterRuleButton.setMnemonic(KeyEvent.VK_C);
				 ...
				 clusterRuleButton.addActionListener(new ActionListener() {
					 public void actionPerformed(ActionEvent e) {
						 if ("Cluster".equals(e.getActionCommand())) {
							 ...
							 numSelectedPI = null;
							 //get the index of currently selected rule
							 int ruleIndex = myRules1.indexOf(myRulesList1.getValue());
				 ...
				 tableContainer2.setMinimumSize(new Dimension(100, 250));
				 clusterFISTablePanel = new JPanel();
				 clusterFISTablePanel.setLayout(new BoxLayout(clusterFISTablePanel,
						 BoxLayout.Y_AXIS));
				 clusterFISTablePanel.add(tableContainer2);

				 JPanel invertFISButtonPanel = new JPanel();
				 invertFISButtonPanel.setLayout(new BoxLayout(invertFISButtonPanel,
						 BoxLayout.X_AXIS));
				 invertFISButton.setEnabled(true);
				 invertFISButton.setMnemonic(KeyEvent.VK_I);
				 invertFISButton.setActionCommand("Invert");
				 ...
				 //panel for showing rules in combo box
				 clusterFISPanel2.setLayout(new FlowLayout());
				 myFISList = new GUIPropertyListEnumeration("Select FIS", null, myFISArrayList, null,
							 500);
				 JButton clusterFISButton = new JButton("Cluster");
				 clusterFISButton.setEnabled(true);
				 clusterFISButton.setMnemonic(KeyEvent.VK_C);
				 clusterFISButton.setActionCommand("Cluster");
				 clusterFISButton.setToolTipText(
						 "Retrieve the cluster satisfying the selected FIS.");
				 ...
				 clusterFISButton.addActionListener(new ActionListener() {
					 public void actionPerformed(ActionEvent e) {
						 if ("Cluster".equals(e.getActionCommand())) {
							 ...
							 numSelectedPIFIS = new JLabel("");
							 //get the index of currently selected rule
							 int FISIndex = myFISArrayList.indexOf(myFISList.getValue());
							 for (int j = 0; j < logReader.numberOfInstances(); j++) {
								 ProcessInstance pi = logReader.getInstance(j);
...
//**************_______________________________________________________________________
			 if (myFISValue == false && myAnalyzer.isCheckBoxECSelected() == false) {
				 myTabPane.removeAll();
				 myRulesPanel.setLayout(new BoxLayout(myRulesPanel, BoxLayout.Y_AXIS));
				 myRulesPanel.add(Box.createVerticalStrut(15));
				 ...
				 //panel for showing rules in combo box
				 clusterRulesPanel.setLayout(new FlowLayout());
				 myRulesList = new GUIPropertyListEnumeration("Select Rule: ", null, myRules, null,
							   500);
				 JButton clusterRuleButton = new JButton("Cluster");
				 clusterRuleButton.setEnabled(true);
				 clusterRuleButton.setMnemonic(KeyEvent.VK_C);
				 ...
				 clusterRuleButton.addActionListener(new ActionListener() {
					 public void actionPerformed(ActionEvent e) {
						 if ("Cluster".equals(e.getActionCommand())) {
							 ...
							 numSelectedPI = new JLabel("");
							 //get the index of currently selected rule
							 int ruleIndex = myRules.indexOf(myRulesList.getValue());
							 for (int j = 0; j < logReader.numberOfInstances(); j++) {
								 ProcessInstance pi = logReader.getInstance(j);
								 if (myAnalyzer.check(pi, ruleIndex) == true) {
									 myRulesPI.add(j);
...
//-----------------------------------------------
		//Algorithm chosen is Predictive Apriori
			...
			if (myAnalyzer.isCheckBoxECSelected() == false) {
				...
				myTabPane.addTab("Predictive Apriori-Association Rules", null, myRulesPanel,
						"View association rules");
				topPanel.add(myTabPane);
			}
				...
				myPanel.setLayout(new BoxLayout(myPanel, BoxLayout.X_AXIS));
				rulesInTextPanel.setLayout(new BoxLayout(rulesInTextPanel, BoxLayout.X_AXIS));
				myRules = new ArrayList(myAnalyzer.getRules()); //changed from getRulesWithEType 10th May 1810pm
				if (myRules.isEmpty() == true) {
					JOptionPane.showMessageDialog(rulesInTextPanel,
							"No rules can be found for this log using the Predictive Apriori algorithm, try using the Apriori algorithm.",
							"Information ", JOptionPane.INFORMATION_MESSAGE);
