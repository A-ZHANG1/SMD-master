...
/**
 * <p>Title:AprioriAnalyzer </p>
 *
 * <p>Description: This class tells the plug-in that the type of Association rule algorithm to be used is the Apriori algorithm. </p>
 * It also prepares the GUI objects accordingly (specific to the chosen algorithm).
 *
...
public class AprioriAnalyzer extends AssociationAnalyzer {
	...
	private void saveAsARFFBrowseButton_actionPerformed(ActionEvent e) {
		...
		chooser.setFileFilter(new GenericMultipleExtFilter(new String[] {"ARFF"}
				, "ARFF file (*.arff)"));
	...
	public AprioriAnalyzer() {
		...
		//Upper bound Minimum Support- minimum value is 0.1, default value is 1.0, and the max value is also 1.0
		upperBoundMinSupport = new GUIPropertyDouble(
							   ...
							   ((MyApriori) myAssociator).getUpperBoundMinSupport(), 0.1, 1.0, 0.1);

		//Lower bound Minimum Support- minimum value is 0.1, default value is 0.1, and the max value is 1.0
		lowerBoundMinSupport = new GUIPropertyDouble(
							   ...
							   ((MyApriori) myAssociator).getLowerBoundMinSupport(), 0.1, 1.0, 0.1);
	...
	public JPanel getParametersPanel() {
		...
		dummyChkBox.setToolTipText("If enabled then an activity 'noname' with type 'notype' is inserted in the log and the rules and FIS are accordingly derived from this log.");
		eventCareChkBox = new JCheckBox("care about Event Types?");
		eventCareChkBox.setToolTipText(
				"If enabled then FIS/Rules are generated with the event type information.");
		...
		locationARFFFile.setToolTipText("The location chosen for saving the file is displayed here");
		...
		saveAsARFFBrowseButton.setToolTipText("Specify the location for saving the ARFF file");
		...
		saveAsARFFBrowseButton.addActionListener(new java.awt.event.ActionListener() {
	...
	public void applyOptionalParameters() {
		((MyApriori) myAssociator).setUpperBoundMinSupport(upperBoundMinSupport.getValue());
		((MyApriori) myAssociator).setLowerBoundMinSupport(lowerBoundMinSupport.getValue());
	...
	/**
	 * Executes the actual algorithm. Generates FIS and then association rules from these FIS.
	 */
	...
	/**
	 *Calls the method to display association rules in the format we want (not in the Weka format)
	 */
