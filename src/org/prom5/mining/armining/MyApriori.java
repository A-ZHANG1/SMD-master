 ...
 * <p>Title: MyApriori </p>
 *
 * <p>Description:MyApriori extends Apriori() </p>
 *
...
public class MyApriori extends Apriori {
	...
	public ArrayList<String> get_m_allTheRules() {
		...
		for (int j = 0; j < m_allTheRules[0].size(); j++) {
			AprioriItemSet tempSet = (AprioriItemSet) m_allTheRules[0].elementAt(j);
			myPremise = new ArrayList<String>();
			for (int i = 0; i < m_instances.numAttributes(); i++) {
				if (tempSet.itemAt(i) != -1) {
					myPremise.add(m_instances.attribute(i).name() + "=" +
							m_instances.attribute(i).value(tempSet.itemAt(i)));
				}
			}
			...
			for (int i = 0; i < m_instances.numAttributes(); i++) {
				if (tempSet.itemAt(i) != -1) {
					myCons.add(m_instances.attribute(i).name() + "=" +
							m_instances.attribute(i).value(tempSet.itemAt(i)));
				}
			}
...
//String confValue2 = Utils.doubleToString(((Double) m_allTheRules[2].elementAt(q)).
//				   doubleValue(), 2);
...
//   }
//}
//myRulesArray2.add(numberRules2 + tempString2 + "   " + "(conf: " + confValue2 + ")");
//}
//System.out.println("original rules from weka: " + myRulesArray2);

//------------------------------------------------------------------------------------
//support value that is calculated on basis of values of lower bound and upper bound provided by the user
...
//System.out.println("m_instances.numInstances: " + m_instances.numInstances());
//------------------------------------------------------------------------------------
		...
		outer:for (int i = 0; i < myPremiseList.size(); i++) {
			...
			for (int j = i + 1; j < myPremiseList.size(); j++) {
				...
				ArrayList<String> ielement = (ArrayList<String>) myPremiseList.get(i);
				ArrayList<String> jelement = (ArrayList<String>) myPremiseList.get(j);
				//if L1=L2
				if (ielement.toString().equals(jelement.toString())) {

					ArrayList<String> ielementRHS = (ArrayList<String>) myConsList.get(i);
					ArrayList<String> jelementRHS = (ArrayList<String>) myConsList.get(j);

					if (ielementRHS.containsAll(jelementRHS)) { //R1 is superset of R2, then keep Rule1
						ruleIndToKeep.add(i);
						ruleIndToRemove.add(j);
					} else {
						if (jelementRHS.containsAll(ielementRHS)) { //if R2 is superset of R1, keep Rule2
							ruleIndToKeep.add(j);
							ruleIndToRemove.add(i);
						} else { //keep both rules but we must check if R1 and R2 are not dummy.
				...
				//L1 IS NOT EQUAL TO L2 then first we check if L1 is subset of L2. If not, then we check if
				//L2 is subset of L1
				else {
					...
					if (jelement.containsAll(ielement)) { //L1 is subset of L2
						...
						if (ielementRHS.containsAll(jelementRHS)) { //if R1 is superset of R2
							...
							//first we chk if the RHS of both the rules is not dummy.
							if (iSize == 1) {
								if (iContent.contains("noname")) {
									ruleIndToRemove.add(i);
								} else {
						...
						if (ielement.containsAll(jelement)) {
							...
							if (jelementRHS.containsAll(ielementRHS)) { // if R2 is superset of R1
								ruleIndToRemove.add(i); //remove R1
								//we need to keep only R2 but first we check if it is not dummy!
								if (jSize == 1) {
									if (jContent.contains("noname")) {
										ruleIndToRemove.add(j);
									} else {
...
//removing the unnecessary rules
		...
		for (int k = 0; k < myLHSList.size(); k++) {
			String confValue = Utils.doubleToString(((Double) m_allTheRules[2].elementAt(k)).
							   doubleValue(), 2);
			...
			for (int i = 0; i < myA.length; i++) {
				...
				if (individualPremise.contains("noname")) {
					individualPremise = individualPremise.replace("noname", "\u03A6");
				}
...
//				}
			...
			if (tempString.contains(",noname")) {
				...
				myRulesArray.add(numberRules + newTempString + "   " + "(conf: " + confValue +
						")");
			} else {
				if (tempString.contains(", noname")) {
					newTempString = tempString.replace(", noname", " ");
					...
					if (tempString.contains("noname,")) {
						newTempString = tempString.replace("noname,", " ");
						myRulesArray.add(numberRules + newTempString + "   " + "(conf: " +
								confValue +
								")");
					} else {
						if (tempString.contains("noname, ")) {
							...
							myRulesArray.add(numberRules + tempString + "   " + "(conf: " +
									confValue + ")");
						}
					}
				}
	...
	public ArrayList<String> get_m_Ls() {
		...
		for (int j = 0; j < m_Ls.size(); j++) {
			...
			for (int i = 0; i < myCurrentItemSets.size(); i++) {
				AprioriItemSet myCurrentItemSet = (AprioriItemSet) myCurrentItemSets.elementAt(
												  i);
				myFIS = new ArrayList<String>();
				for (int k = 0; k < m_instances.numAttributes(); k++) {

					if (myCurrentItemSet.itemAt(k) != -1 && m_instances.attribute(k) != null) {
						myFIS.add(m_instances.attribute(k).name() + "=" +
								m_instances.attribute(k).value(myCurrentItemSet.itemAt(k)));
					}
				}
				myFISList.add(myFIS);
			}
...
//Check whether a particular FIS is present in a log trace, do not consider event type information
			...
			for (int i = 0; i < mySet.size(); i++) {
				...
				for (int j = 0; j < myAIS.items().length; j++) {
					...
					if (myAIS.itemAt(j) == 1) { //=no
						...
						while (it2.hasNext()) {
							AuditTrailEntry individualAte = (AuditTrailEntry) it2.next();
							try {
								String ateDetail = (String) individualAte.getElement();
	...
	//Check whether a particular FIS is present in a log trace, consider event type information
...
//Check whether a particular association rule is present in a log trace, do not consider event type information
...
//flag for individual element of a rule
		...
		AprioriItemSet LHSSet = (AprioriItemSet) m_allTheRules[0].elementAt(ruleIndex);
		...
		for (int i = 0; i < LHSSet.items().length; i++) {
			...
			if (LHSSet.itemAt(i) == 1) { //=no
				...
				while (it2.hasNext()) {
					AuditTrailEntry individualAte = (AuditTrailEntry) it2.next();
					try {
						//String ateDetail = (String) individualAte.getElement() + " (" +individualAte.getType() + ")";
						String ateDetail = (String) individualAte.getElement();
...
//----for RHS
		AprioriItemSet RHSSet = (AprioriItemSet) m_allTheRules[1].elementAt(ruleIndex);
