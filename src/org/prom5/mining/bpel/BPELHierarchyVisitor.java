...
public class BPELHierarchyVisitor extends BPELVisitor {
	...
	public static synchronized void Build(BPEL model, BPELHierarchy hierarchy) {
		...
		instance.hierarchy.addHierarchyObject(model, null, "model "+ model.getName());
		stack.clear();
		stack.push(model);
	...
	public void visitProcess(BPELProcess process) {
		hierarchy.addHierarchyObject(process, stack.peek(), "process "+ process.getName());
		stack.push(process);
	...
	public void visitScope(BPELScope scopeActivity) {
		hierarchy.addHierarchyObject(scopeActivity, stack.peek(), "scope "+ scopeActivity.getName(false));
	...
	public void visitSequence(BPELSequence sequenceActivity) {
		hierarchy.addHierarchyObject(sequenceActivity, stack.peek(), "sequence "+ sequenceActivity.getName(false));
	...
	public void visitWhile(BPELWhile whileActivity) {
		hierarchy.addHierarchyObject(whileActivity, stack.peek(), "while " + whileActivity.getName(false));
	...
	public void visitSwitch(BPELSwitch switchActivity) {
		hierarchy.addHierarchyObject(switchActivity, stack.peek(), "switch "+ switchActivity.getName(false));
	...
	public void visitFlow(BPELFlow flowActivity) {
		hierarchy.addHierarchyObject(flowActivity, stack.peek(), "flow "+ flowActivity.getName(false));
	...
	public void visitPick(BPELPick pickActivity) {
		hierarchy.addHierarchyObject(pickActivity, stack.peek(), "pick " + pickActivity.getName(false));
