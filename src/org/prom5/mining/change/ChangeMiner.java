...
/**
 * This mining plugin uses the instance / multi-phase mining technique
 * to build and aggregate instance graphs describing change processes.
 * The ordering relation between change operations, i.e. events, is not
 * solely based on occurrence of events in the log, but (partially)
 * replaced by the defined notion of commutativity between change operations.
 *
 * @author Christian W. Guenther (christian at deckfour dot org)
 */
public class ChangeMiner implements MiningPlugin {
	...
	public MiningResult mine(LogReader log) {
		if(isChangeLog(log) == false) {
			...
			return new MiningResult() {
				...
				public JComponent getVisualization() {
					...
					dummyPanel.add(new JLabel("<html><b>The given log does not appear to include change information!<br>" +
					...
					"regular enactment log, which cannot be handled by this plugin!</b></html>"), BorderLayout.CENTER);
					return dummyPanel;
				}
			};
		}
		Progress progress = new Progress("Mining " + log.getFile().getShortName() + " using " +
							getName());
		// test output start
		if(UISettings.getInstance().getTest()==true) {
			Message.add("<ChangeMiner file=\"" + log.getFile().getShortName()
				+ "\" conflictingCausalityAllowed=\"" + optionsPanel.isConflictingCausalityAllowed()
				+ "\">", Message.TEST);
		}
		// test output end
		CommutativityLogRelationBuilder relationBuilder = new CommutativityLogRelationBuilder(log);
		relationBuilder.setAllowConflictingCausality(optionsPanel.isConflictingCausalityAllowed());
		...
		AggregationGraphResult aggResult = (AggregationGraphResult) poAgg.mine(genResult.getLogReader(), progress, false);
		progress.close();
		return aggResult;
	}
	...
	protected boolean isChangeLog(LogReader log) {
		if(log.numberOfInstances() > 0 
			...
			try {
				AuditTrailEntry ate = log.getInstance(0).getAuditTrailEntryList().get(0);
