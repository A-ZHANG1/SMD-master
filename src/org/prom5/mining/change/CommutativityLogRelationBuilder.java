 ...
 *                                                         *
 **********************************************************/
package org.prom5.mining.change;
...
/**
 * This class builds log relations based on the notion of commutativity
 * defined between change operations. It is expected that audit trail entries
 * in the log under consideration adhere to the change log format, as defined.
 * In particular, the presence of the following data attributes is expected
...
public class CommutativityLogRelationBuilder implements LogRelationBuilder {
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.logabstraction.LogRelationBuilder#getLogRelations()
	 */
	public LogRelations getLogRelations() {
		...
		if(UISettings.getInstance().getTest()==true) {
			...
			Message.add("\t<CausalMatrix hash=\"" + TestUtils.hash(causal) + "\">", Message.TEST);
			Message.add("\t<ParallelMatrix hash=\"" + TestUtils.hash(parallel) + "\">", Message.TEST);
			Message.add("\t<EndMatrix hash=\"" + TestUtils.hash(end) + "\">", Message.TEST);
			Message.add("\t<StartMatrix hash=\"" + TestUtils.hash(start) + "\">", Message.TEST);
			Message.add("\t<LoopsMatrix hash=\"" + TestUtils.hash(loops) + "\">", Message.TEST);
			Message.add("</CommutativityLogRelationBuilder>", Message.TEST);
		}
		// assemble and return new log relations
		return new LogRelationsImpl(causal, parallel, end, start, loops, log.getLogSummary().getLogEvents());
	}
	...
	protected DoubleMatrix2D generateParallelMatrix() {
		...
		for(log.reset(); log.hasNext();) {
			...
			for(int i=0; i<(ates.size()-1); i++) {
				...
				int indexA = logEvents.findLogEventNumber(a.getElement(), a.getType());
				for(int j=i; j<ates.size(); j++) {
					AuditTrailEntry b = (AuditTrailEntry)ates.get(j);
					int indexB = logEvents.findLogEventNumber(b.getElement(), b.getType());
					if(isCommutative(a, b)) {
						...
						if(parallel.get(indexA, indexB)>=0.0) {
							...
							parallel.set(indexA, indexB, parallel.get(indexA, indexB) + 1.0);
							parallel.set(indexB, indexA, parallel.get(indexB, indexA) + 1.0);
						}
					} else {
						// set 'veto' marker: once a relation has been found to
						// be not commutative, this cannot be overwritten later on!
		...
		// iterate through parallelity matrix and exchange 'veto' markers by zero
	...
	/**
	 * Generates the causal matrix with the option to include
	 * conflicting causalities in the final matrix.
	 * @param parallel
	 * @param directSuccession
	 * @return
	 */
	protected DoubleMatrix2D generateCausalMatrix(DoubleMatrix2D parallel, DoubleMatrix2D directSuccession) {
		...
		for(int i=0; i<s; i++) {
			for(int j=0; j<s; j++) {
				if(i==j) {
					...
					continue;
				} else if((directSuccession.get(i, j)>0) && (parallel.get(i, j)<=0)) {
					// record causal relation, if no conflicting present, or if
					// conflicting causal relations are allowed.
					if(this.allowConflictingCausality || (directSuccession.get(j, i)<=0)) {
	...
	/**
	 * Checks for two given audit trail entries, whether their represented change
	 * operations are considered commutative.
	 * Commutativity between two change operations A and B is assumed, when:
	 * <ul>
	 ...
	 * </ul>
	 * This method will return whether all three conditions hold on the specified
	 * audit trail entries.
	 * @param a
	 * @param b
	 * @return
	...
	/**
	 * This method returns the parameter list of a change operation. This parameter
	 * list is defined to contain all objects in the pre- and postset of a change
	 * operation. (What is actually done is, exploding two comma-separated list strings
	 * and insert the extracted elements into an ArrayList, which is returned.)
	 * @param ate
	 * @return
	 */
	protected ArrayList extractParameterList(AuditTrailEntry ate) {
		ArrayList paramList = new ArrayList();
		String preset[] = ((String)ate.getData().get("CHANGE.preset")).split(",");
		for(int i=0; i<preset.length; i++) {
			paramList.add(preset[i].trim());
		}
		String postset[] = ((String)ate.getData().get("CHANGE.postset")).split(",");
