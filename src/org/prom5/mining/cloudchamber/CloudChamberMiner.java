 ...
 *
 */
package org.prom5.mining.cloudchamber;
...
public class CloudChamberMiner implements MiningPlugin, MiningResult, Provider {
	...
	public JPanel getCloudChamberPanel(boolean includeHeader) {
		...
		vBar.setUI(new SlickerScrollBarUI(vBar, new Color(0, 0, 0, 0), new Color(10, 10, 10), new Color(50, 50, 50), 4, 12));
		vBar.setOpaque(false);
		JScrollBar hBar = scrollPane.getHorizontalScrollBar();
		hBar.setUI(new SlickerScrollBarUI(hBar, new Color(0, 0, 0, 0), new Color(10, 10, 10), new Color(50, 50, 50), 4, 12));
		...
		slider.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				int horizVal = (int)((double)scrollPane.getHorizontalScrollBar().getMaximum() * value);
				int vertiVal = (int)((double)scrollPane.getVerticalScrollBar().getMaximum() * value);
		...
		instancesList.setSelectionInterval(0, instanceNames.length -1 );	// select all
		...
		instancesScrollPane.setBorder(BorderFactory.createEmptyBorder(0, 5, 0, 5));
		...
		ivBar.setUI(new SlickerScrollBarUI(ivBar, COLOR_FG, new Color(140, 140, 140), new Color(80, 80, 80), 4, 12));
		JScrollBar ihBar = instancesScrollPane.getHorizontalScrollBar();
		ihBar.setUI(new SlickerScrollBarUI(ihBar, COLOR_FG, new Color(140, 140, 140), new Color(80, 80, 80), 4, 12));
		JLabel instancesLabel = new JLabel("Select cases:");
		instancesLabel.setForeground(COLOR_FG);
		instancesLabel.setOpaque(false);
		instancesLabel.setBorder(BorderFactory.createEmptyBorder(10, 10, 7, 10));
	...
	protected void scrollToInstance(int index) {
		...
		for(int i=0; i<index; i++) {
			coord += logReader.getInstance(selectedInstances[i]).getAuditTrailEntryList().size();
		}
	...
	protected class UpdateWorker extends SwingWorker {
		...
		public Object construct() {
			...
			scrollPane.repaint();
			try {
				if(isActive == false) { exit(); return null; }
				selectedInstances = instancesList.getSelectedIndices();
				progressPanel.setProgress(1);
				filteredReader = LogReaderFactory.createInstance(logReader, selectedInstances);
				...
				if(isActive == false) { exit(); return null; }
				scrollPane.getHorizontalScrollBar().setMaximum(ccPanel.getWidth());
				if(isActive == false) { exit(); return null; }
				scrollPane.getVerticalScrollBar().setMaximum(ccPanel.getHeight());
				if(isActive == false) { exit(); return null; }
				slider.setValue(0);
				if(isActive == false) { exit(); return null; }
				instanceJumpCombo.removeAllItems();
				for(int i=0; i<selectedInstances.length; i++) {
					instanceJumpCombo.addItem(logReader.getInstance(selectedInstances[i]).getName());
					if(isActive == false) { exit(); return null; }
				}
	...
	public LogReader getPreviewedLog() {
		try {
			return LogReaderFactory.createInstance(logReader, selectedInstances);
		} catch (Exception e) {
			// something was wrong...
			e.printStackTrace();
			return null;
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		return new ProvidedObject[] {new ProvidedObject("Previewed log selection", new Object[] {previewed})};
	}
}
