...
package org.prom5.mining.conversion;
...
import org.prom5.framework.log.ModelElement;
...
public class CaseDataExtractor implements MiningPlugin {
	...
	// To determine a low and high service times among instances of one model element.
	...
	/*
	 * Intially, the panel is blank. Only after mining we fill the panel.
	 * Reason is that before mining we lack sufficient information to
	 * fill the panel.
	 */
	...
	// Mine the log.
	/*
	 * Extracts information from the mined data to fill the panel. After
	 * the user has selected the appropriate settings and pressed "Save"
	 * the data is written to the file the user selected.
	 */
	...
	// Extract the set of data elements belonging to model element instances from the log.
	...
	// Extract the set of data elements belonging to process instances from the log.
	...
	// Print values of data elements belonging to the i-th model element instance of the process instance.
						...
						while (b && it.hasNext()) {
							...
							if (d > 0) {
								...
								if (des.containsKey(theATEDataElements[d])) {
									// Found the data element. Print it and stop searching.
									b = false;
									Print(quote);
									Print(des.get(theATEDataElements[d]).toString());
									Print(quote);
								}
							} else if (d == 0) {
								b = false;
	...
	// Return the timestamp (as long) of the given ate, and -1 if it has non timestamp.
	 ...
	 */
	private void SetServiceTime(ProcessInstance pi, int i, Set<AuditTrailEntry> ates) {
		...
		// For a high time (upper bound), we can use any event that can happen only
		...
		if (min[LogStateMachine.ORD_STARTED] != -1 && max[LogStateMachine.ORD_STARTED] != -1) {
			// Sufficient information to print a high service time
			if (min1[LogStateMachine.ORD_STARTED] != -1 && max1[LogStateMachine.ORD_STARTED] != -1) {
				// Sufficient information to print a low service time
				theLowServiceTime = (max1[LogStateMachine.ORD_STARTED] -
									min1[LogStateMachine.ORD_STARTED]) / timeUnitInMilliseconds;
			} else {
				// Insufficient information to print a low service time, but we have sufficicent information
				// to print a high service time. Print 0 as low service time.
				theLowServiceTime = 0;
			}
			theHighServiceTime = (max[LogStateMachine.ORD_STARTED] -
								 min[LogStateMachine.ORD_STARTED]) / timeUnitInMilliseconds;
		} else {
			theLowServiceTime = theHighServiceTime = -1;
		}
	}

	// Print the number of instances for the i-th model element in a given process instance.
	...
	// Print a low and a high service time for the i-th model element of the given process instance.
	private void PrintServiceTimeBounds(ProcessInstance pi, int i) {
		...
		while (pit.hasNext()) {
			...
			if (me.getName().compareTo(theModelElements[i]) == 0) {
				...
				if (theLowServiceTime != -1 && (l == -1 || theLowServiceTime < l)) {
					l = theLowServiceTime;
				}
				if (theHighServiceTime != -1 && (h != -1 || theHighServiceTime > h)) {
								...
								if (se.compareTo(theEventTypes[e]) == 0) {
									...
									for (int o = 0; o < theOriginators.length; o++) {
										if (theOptions.IsSelected2(o) &&
												so.compareTo(theOriginators[o]) == 0) {
											// This is the correct originator, and the user is interested in him/her.
											c[e][o]++;
	...
	private void Print(String aString) {
		if (theRun == 2 && (isNonNull[theColumn] || aString.compareTo(newline) == 0)) {
			try {
				if (aString.length() == 0) {
	...
	public void PrintLog(BufferedWriter aWriter, String tag) throws IOException {
		...
		SwingWorker w = new SwingWorker() {
			public Object construct() {
				Boolean result;
				Progress p = new Progress("Process instance:", 0,
							 2 * theLog.getLogSummary().getNumberOfProcessInstances());
				int i = 0;
				isNonNull = null;
