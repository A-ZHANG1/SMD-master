...
/**
 * Aggregate Data Modification Cluster (ADMC)
 * Contains a set of DMCs all modifying the same set of
 * data objects (footprint), regardless of order and
...
public class Admc implements Comparable {
	...
	/**
	 * constructor
	 * @param id custom id string for this ADMC
	 */
	...
	public boolean addIfMatches(Dmc aDmc) {
		if(footprint==null) {
			...
			// TODO find a better, i.e. more generic, criterium than matching footprints
			// TODO check whether the 'conflicts' relation on DMCs is really correct
			dmcs.add(aDmc);
			return true;
		} else {
			return false;
	 ...
	 * WARNING: This method is intended for use by more sophisticated AggregationMethods.
	 * It can be used in case the ADMC as is does not naiively match the footprint of
	 * the DMC to be added. It is up to the AggregationMethod to take care of the
	 * following necessities, as these are not provided within the implementation of
	 * this method:
	 * 	- test for and resolve potential conflicts between DMCs
	 * 	- test overall compatibility with all contained DMCs and the ADMC itself
	 * 	- potentially adjust the DMC to be added
	 * 	- potentially adjust previously contained DMCs
	 * 	- derive the ADMC's new footprint and supply it
	 * @param aDmc the new DMC to be added
	 * @param aFootprint the new footprint of the ADMC, calculated externally!
	 */
	 ...
	 * A conflict between two ADMCs is present, if any DMC from one ADMC
	 * is in conflict with any DMC from the other ADMC, thus this test
	 ...
	 */
	public HashSet<Dmc> getAllInProcessInstance(String processId, String processInstanceId) {
		HashSet<Dmc> resultList = new HashSet<Dmc>();
		for(Dmc targetDmc : dmcs) {
			if(targetDmc.getProcessInstance().getProcess().equals(processId)
					&& targetDmc.getProcessInstance().getName().equals(processInstanceId)) {
	 ...
	 */
	public Iterator<Dmc> allInProcessInstanceIterator(String processId, String processInstanceId) {
		return getAllInProcessInstance(processId, processInstanceId).iterator();
	}
	
	/**
	 * @return a sorted set of all process instances (as references) 
	 * the contained DMCs stem from
	 */
	...
	public int compareTo(Object arg0) {
		...
		return (int)(((this.size() * 10000) + this.getIdNumber())
					- ((other.size() * 10000) + other.getIdNumber()));
	}
}
