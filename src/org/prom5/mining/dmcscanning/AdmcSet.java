...
public class AdmcSet {
	...
	/**
	 * constructor
	 * @param aMethod the AggregationMethod instance to be used for DMC->ADMC aggregation (comparison and merging)
	 */
	...
	/**
	 * Attempts to merge a DMC with all included ADMCs. If none succeeds, a new ADMC
	 * is being created and inserted, based on the provided DMC.
	 * @param aDmc the DMC to add to ADMC (set).
	 * @return the number of ADMCs the DMC was merged with. Zero, if a new ADMC was created.
	 */
	 ...
	 * @param key id string of ADMC to retrieve
	 * @return
	 */
	 ...
	 * @param key id string of ADMC to remove
	 ...
	 * @param footprint the footprint to match
	 ...
	 * The implementation of this method is up to the specific aggregation method, the
	 * rationale behind it is to strip down a set of ADMCs to the most fundamental elements.
	 * As such, the method is expected to merge similar ADMCs together or to delete less
	 * significant elements, according to its intent and implementation.
	 ...
	 * This results in the specific ADMC no longer being part of the 'active' (or, consolidated)
	 * set of ADMCs within the ADMC set. However, it is being preserved in a list of consolidation
	 * victims that can be accessed further on.
	 * @param anAdmcId id string of the ADMC to victimize.
	 */
	...
	/**
	 * @return the set of consolidation victims (may be null, if not yet consolidated)
	 */
	...
	/**
	 * @return an iterator on all ADMCs, both actively contained and consolidation victims
	 */
	 ...
	 * @param processId id of the process to look up
	 * @param processInstanceId id of the process instance to look up
	 * @return ArrayList containing all matching ADMCs
	 */
	public ArrayList getAllInProcessInstance(String processId, String processInstanceId) {
		...
		for(Iterator itAdmc = iterator(); itAdmc.hasNext();) {
			...
			for(Iterator itDmc = tgtAdmc.iterator(); itDmc.hasNext();) {
				...
				if(tgtDmc.getProcessInstance().getProcess().equals(processId)
						&& tgtDmc.getProcessInstance().getName().equals(processInstanceId)) {
	 ...
	 */
	public Iterator allInProcessInstanceIterator(String processId, String processInstanceId) {
	...
	public void testOutput() {
		Message.add("<AdmcSet size=\"" + this.admcs.size() + "\">", Message.TEST);
		long hash = 0;
	...
	/**
	 * Convenience method. Builds, from a DMCSet of initially scanned DMCs together with
	 * an AggregationMethod, the set of ADMCs.
	 * @param aDmcSet set of initially scanned DMCs
	 * @param aMethod method to be used for ADMC aggregation
	 * @param progress the progress indicator used for status feedback
