...
public class BalancedMdmcSet extends MdmcSet {
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.dmcscanning.MdmcSet#addADMC(org.processmining.mining.dmcscanning.Admc)
	 */
	...
	protected double getDecisionValue(Admc candidate) {
		return ( (candidate.size() * balance) 
				+ (candidate.footprint().size() * (1.0 - balance)) );
	}
	
	/**
	 * This method should always choose for the same candidate of the two provided
	 * and thus prevent 'flattering' of choices in multiple iterations.
	 * @param a
	 * @param b
	 * @return
	 */
	 ...
	 */
	public static MdmcSet buildMDMC(AdmcSet admcSet, double balance, int iterations, Progress progress) {
		if(iterations<=0) {	iterations = 1; }
		progress.setNote("Deriving minimal conflict-free set from aggregated set...");
		progress.setMinMax(0, (admcSet.size() * iterations));
		Message.add("Balanced iterated derivation of minimal conflict-free set..");
		int counter = 0;
		int turnover = 0;
		BalancedMdmcSet mdmc = new BalancedMdmcSet(balance);
		for(int iteration=1; iteration<=iterations; iteration++) {
			progress.setNote("Deriving minimal conflict-free set from aggregated set: Iteration #" + iteration);
			turnover = 0;
			for(Admc candidate : admcSet.getAll()) {
				if(mdmc.admcs.contains(candidate) == false) {
					if(mdmc.addADMC(candidate) == true) {
						...
						/* debug output
						if(turnover % 50 == 0) {
							System.out.println("MDMC: added " + turnover + " ADMCs.");
						}
						*/
					}
				}
			...
			Message.add("Iteration #" + iteration + " completed. Turnover: " + turnover + " (" + turnoverPct + "%)");
		}
		return mdmc;
	}
}
