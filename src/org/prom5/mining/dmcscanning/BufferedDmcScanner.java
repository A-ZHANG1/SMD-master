...
import org.prom5.mining.dmcscanning.logutils.AbstractEvent;
import org.prom5.mining.dmcscanning.logutils.LogItemOrder;
...
public class BufferedDmcScanner {
	 ...
	 * 
	 * @param reader the LogReader to be used for extracting the log in question
	 * @param progress a Progress window to display status information
	 * @param maxProximity the temporal extension (in milliseconds) of the scan window 
	 * 	(i.e. the maximum time between the first and last events within a DMC) - ignored if <=0.
	 * @param maxEvents maximum number of events to be contained within a DMC - ignored if <=0.
	 * @param enforceOriginator only events with the same originator are contained within one DMC
	 * @param enforceEventType only events with the same type are contained within one DMC
	 * @return a DMCSet instance containing the scanning result, i.e. all extracted DMCs
	 */
	public static DmcSet scanInitialDmcs(LogReader reader, Progress progress, 
		...
		progress.setMinMax(0, reader.getLogSummary().getNumberOfAuditTrailEntries());
		...
		for(int i=0; i<reader.numberOfInstances(); i++) {
			...
			if(ateIterator.hasNext()) {	// skip empty process instances
				event = AbstractEvent.create(procInst, (AuditTrailEntry)ateIterator.next());	// initial event
				buffer.add(event);
				curMaxTime = event.getTimestamp().getTime() + maxProximity;	// initialize maximal proximity
				// accumulate next DMC, respecting time and counter boundaries
				while(buffer.size()>=1) {
					// get next event, if available
					if(ateIterator.hasNext()) {	
						event = AbstractEvent.create(procInst, (AuditTrailEntry)ateIterator.next());
					}
					// extract next DMC, if boundary conditions satisfied (checking validity first),
					// or if no further events in process instance
					if((ateIterator.hasNext()==false) 
							|| ((maxProximity>0) && (event.getTimestamp().getTime()>curMaxTime)) 
						...
						for(Iterator it=buffer.iterator(); it.hasNext();) {
							...
							if(enforceOriginator == true
									...
									&& (tmpEvent.getOriginator().equals(refOriginator)==false)) {
								continue;
							} else if(enforceEventType && (tmpEvent.getType().equals(refEventType)==false)) {
								continue;
							} else if((maxProximity>0) && (tmpEvent.getTimestamp().getTime()>curMaxTime)) {
								continue;
							} else {
								nextDmc.addEvent(tmpEvent);
							}
						}
						nextDmc.makeFootprintCanonical(equivalence);	// make canonical
						...
						if(buffer.size()>0) {	// buffer may be empty
							curMaxTime = ((AbstractEvent)buffer.get(0)).getTimestamp().getTime() + maxProximity;
						}
						// progress notification
						progress.setProgress(set.size());
					}
					// if (above) new event has been extracted, append to buffer
					if((buffer.size()>0) && (event!=buffer.get(buffer.size()-1))) {
						buffer.add(event);
					}
				}
			}
