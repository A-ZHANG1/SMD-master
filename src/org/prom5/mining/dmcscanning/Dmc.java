...
/**
 * This class represents a Data Modification Cluster (DMC).
...
public class Dmc implements LogItem {
	...
	/* by definition, all events within a DMC are from the same
	 * process instance. This property is cached to provide uniform access. */
	...
	/**
	 * copy constructor
	 * @param copy the original DMC to copy
	 */
	public Dmc(Dmc copy) {
	...
	public void addEvent(AbstractEvent entry) {
		...
		if((startDate==null)||(startDate.after(entry.getLeftBoundaryTimestamp()))) {
			startDate = new Date(entry.getLeftBoundaryTimestamp().getTime());
		}
		if((endDate==null)||(endDate.before(entry.getRightBoundaryTimestamp()))) {
	...
	/**
	 * checks whether this DMC is in conflict with another one,
	 * i.e. they share at least one event.
	 * @param other DMC to test conflict with
	 * @return
	 */
	 ...
	 * Two DMCs are considered similar if their footprints match.
	 * @param other
	 * @return
	 */
	...
	/**
	 * tests with equality to another DMC
	...
	/**
	 * deep check whether a given AbstractEvent is contained
	 * within this DMC's event set
	 * @param entry
	 * @return
	 */
	 ...
	 * representations of the data objects, following the rules implemented
	 * in the given equivalence relation implementation.
	 * @param equiv equivalence relation implementation used for conversion
	 */
	...
	public boolean isConsistent(ObjectEquivalence equiv) {
		...
		for(String dataTest : footprint.getData()) {
			...
			for(AbstractEvent evt : events) {
				if((evt.getElement().equals(dataTest))||(equiv.equivalent(evt.getElement(), dataTest))) {
					isCovered = true;
					break;
				}
			}
