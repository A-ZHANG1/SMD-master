 ...
 */
package org.prom5.mining.dmcscanning;
...
import org.prom5.mining.dmcscanning.aggregation.AggregationMethod;
import org.prom5.mining.dmcscanning.aggregation.GreedyAggregation;
import org.prom5.mining.dmcscanning.aggregation.SimpleAggregation;
import org.prom5.mining.dmcscanning.aggregation.TolerantAggregation;
import org.prom5.mining.dmcscanning.equivalence.FlowerEquivalence;
import org.prom5.mining.dmcscanning.equivalence.ObjectEquivalence;
import org.prom5.mining.dmcscanning.equivalence.StrictEquivalence;
import org.prom5.mining.dmcscanning.ui.DmcOptionsPanel;

/**
 * DMCMiner.
 * This ProM mining plugin implements the concept of Modification Cluster Scanning.
 * Applicable for any sort of event logs, preferrably for sc. data modification logs.
 * 
 * @author Christian W. Guenther (christian at deckfour dot org)
 */
public class DmcMiner implements MiningPlugin {
	...
	protected boolean useBreakingProximity = false;	// interpret proximity as split-on-break
	protected boolean enforceOriginator = true;		// require originators to be uniform within DMCs
	protected boolean enforceEventType = true;		// require event types to be uniform within DMCs
	protected boolean consolidateADMC = false;		// whether ADMC shall be consolidated
	protected AggregationMethod aggregator = null;	// method used for ADMC aggregation
	protected double mdmcSelectionBalance = 0.5;	// balance for MDMC derivation
	protected int mdmcSelectionIterations = 1;		// number of iterations in MDMC derivation
	protected boolean checkDmcConsistency = false;	// whether to periodically check consistency of the DMC set
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.MiningPlugin#getOptionsPanel(org.processmining.framework.log.LogSummary)
	 */
	...
	protected void displayErrorMessage(String message) {
		JOptionPane.showMessageDialog(null, message, "Unable to start activity clustering miner!", JOptionPane.ERROR_MESSAGE);
	}
	...
	public MiningResult mine(LogReader log) {
		...
		try {
			if(log.getInstance(0) != null
					...
					&& log.getInstance(0).getAuditTrailEntryList().get(0).getTimestamp() == null) {
				displayErrorMessage(
						"The activity clustering miner depends on the\n"
		...
		if(useBreakingProximity==true) {
			...
			initialDmcs = BufferedDmcScanner.scanInitialDmcs(log, progress, maxProximity, 
					maxNoEvents, enforceOriginator, enforceEventType, equivalence);
		}
		Message.add("Initial cluster scan done. Extracted " + initialDmcs.size() + " initial clusters in "
				+ ((System.currentTimeMillis() - timer)/1000.0) + " sec.");
		timer = System.currentTimeMillis();
		progress.close();
		progress = new Progress("Aggregating clusters...");
		if(checkDmcConsistency) { Message.add("[Initial cluster set consistency ratio: " + initialDmcs.checkConsistency(equivalence) + "]"); }
		// aggregate to ADMC
		Message.add("Aggregating initial clusters to aggregated set...");
		admcs = AdmcSet.buildAdmcSet(initialDmcs, aggregator, progress);
		if(checkDmcConsistency) { Message.add("[Initial cluster set consistency ratio: " + initialDmcs.checkConsistency(equivalence) + "]"); }
		Message.add("...done. Derived " + admcs.size() + " aggregated clusters in "
				+ ((System.currentTimeMillis() - timer)/1000.0) + " sec.");
		timer = System.currentTimeMillis();
		if(consolidateADMC==true) {
			...
			Message.add("...done. Consolidation ratio: " + consolidationRatio 
					+ " (" + admcs.consolidationVictimsSize() + " victims), "
					+ ((System.currentTimeMillis() - timer)/1000.0) + " sec.");
			timer = System.currentTimeMillis();
		}
		progress.close();
		progress = new Progress("Deriving minimal conflict-free set...");
		Message.add("Deriving minimal conflict-free set of clusters from aggregated set...");
		mdmcs = BalancedMdmcSet.buildMDMC(admcs, mdmcSelectionBalance, mdmcSelectionIterations, progress);
		if(checkDmcConsistency) { Message.add("[Initial cluster set consistency ratio: " + initialDmcs.checkConsistency(equivalence) + "]"); }
		Message.add("...done. Determined " + mdmcs.size() + " ADMCs in MDMC, in "
				+ ((System.currentTimeMillis() - timer)/1000.0) + " sec.");
	...
	/**
	 * @param consolidateADMC Whether ADMC is to be consolidated after derivation.
	 */
	...
	/**
	 * @param maxNoEvents The maximal number of events within a DMC.
	 */
	public void setMaxNoEvents(long maxNoEvents) {
		this.maxNoEvents = maxNoEvents;
	}
	/**
	 * @return Returns the maximal proximity (in milliseconds) between the first and last events within a DMC.
	 */
	public long getMaxProximity() {
		return maxProximity;
	}
	/**
	 * @param maxProximity The maximal proximity (in milliseconds) between the first and last events within a DMC.
	 */
	public void setMaxProximity(long maxProximity) {
		this.maxProximity = maxProximity;
	}
	/**
	 * @return whether all events within a DMC are enforced to have the same event type
	 */
	public boolean isEnforceEventType() {
		return enforceEventType;
	}
	/**
	 * @param enforceEventType whether all events within a DMC are enforced to have the same event type
	 */
	public void setEnforceEventType(boolean enforceEventType) {
		this.enforceEventType = enforceEventType;
	}
	/**
	 * @return whether all events within a DMC are enforced to have the same originator
	 */
	public boolean isEnforceOriginator() {
		return enforceOriginator;
	}
	/**
	 * @param enforceOriginator whether all events within a DMC are enforced to have the same originator
	 */
	...
	/**
	 * @param mdmcSelectionBalance The mdmcSelectionBalance to set.
	 */
	...
	/**
	 * @param mdmcSelectionIterations The mdmcSelectionIterations to set.
	 */
	...
	/**
	 * @param checkDmcConsistency The checkDmcConsistency to set.
	 */
	...
	public void testOutput() {
		...
		Message.add("\t\t<Option name=\"maxProximityTime\" value=\"" + optionsPanel.getMaxProximityTime() + "\"/>", Message.TEST);
		Message.add("\t\t<Option name=\"maxNumberOfEvents\" value=\"" + optionsPanel.getMaxNumberOfEvents() + "\"/>", Message.TEST);
		Message.add("\t\t<Option name=\"isUsingBreakingProximity\" value=\"" + optionsPanel.isUsingBreakingProximity() + "\"/>", Message.TEST);
		Message.add("\t\t<Option name=\"isEnforceOriginator\" value=\"" + optionsPanel.isEnforceOriginator() + "\"/>", Message.TEST);
		Message.add("\t\t<Option name=\"isEnforceEventType\" value=\"" + optionsPanel.isEnforceEventType() + "\"/>", Message.TEST);
		Message.add("\t\t<Option name=\"isMonitorDmcConsistency\" value=\"" + optionsPanel.isMonitorDmcConsistency() + "\"/>", Message.TEST);
		Message.add("\t\t<Option name=\"isConsolidateAdmc\" value=\"" + optionsPanel.isConsolidateAdmc() + "\">", Message.TEST);
		Message.add("\t\t<Option name=\"aggregationMethod\" value=\"" + optionsPanel.getAggregationMethod().toString() + "\"/>", Message.TEST);
		Message.add("\t\t<Option name=\"mdmcSelectionBalance\" value=\"" + optionsPanel.getMdmcSelectionBalance() + "\"/>", Message.TEST);
		Message.add("\t\t<Option name=\"mdmcSelectionIterations\" value=\"" + optionsPanel.getMdmcSelectionIterations() + "\"/>", Message.TEST);
		Message.add("\t\t<Option name=\"objectEquivalence\" value=\"" + optionsPanel.getObjectEquivalence().toString() + "\"/>", Message.TEST);
		Message.add("\t</Options>", Message.TEST);
