...
/**
 * DMCSet provides an ordered and structured way to store
 * and access a set of DMCs. Typical usage is intended to cover
 * the management of initially scanned DMCs in a way that provides
 * for automatically created structure, to ease later access and 
 * search.
 * 
 * @author Christian W. Guenther (christian at deckfour dot org)
 */
public class DmcSet implements LogItem {
	...
	/**
	 * the arraylist allDMCs is to contain references
	 * to all DMCs contained in the order of their having
	 * been added.
	 * The hashmap processes contains a set of hashmaps that
	 * are keyed by the present process ids/names. The value
	 * hashmaps inside contain as key the process instance ids
	 * and as value arraylists that contain references to the
	 * dmcs, belonging to this process instance.
	 * Such, the structure is: process->processInstance->DMCs
	 */
	...
	public void insert(Dmc aDmc) {
		...
		// retrieve map referring to process instances first, create and insert if not present
		if(processes.containsKey(aDmc.getProcessInstance().getProcess())) {
			processInstanceMap = (HashMap)processes.get(aDmc.getProcessInstance().getProcess());
		} else {
			processInstanceMap = new HashMap();
			processes.put(aDmc.getProcessInstance().getProcess(), processInstanceMap);
		}
		...
		if(processInstanceMap.containsKey(aDmc.getProcessInstance().getName())) {
			dmcList = (TreeSet)processInstanceMap.get(aDmc.getProcessInstance().getName());
		} else {
			dmcList = new TreeSet();
			processInstanceMap.put(aDmc.getProcessInstance().getName(), dmcList);
		}
		// insert new DMC to appropriate list
		dmcList.add(aDmc);
		// adjust time boundaries
		if((leftBoundaryTime==null) || aDmc.getLeftBoundaryTimestamp().before(leftBoundaryTime)) {
			leftBoundaryTime = aDmc.getLeftBoundaryTimestamp();
		}
		if((rightBoundaryTime==null) || aDmc.getRightBoundaryTimestamp().after(rightBoundaryTime)) {
	...
	public SortedSet getAllInProcess(String processName) {
		...
		for(Iterator it=((HashMap)processes.get(processName)).values().iterator(); it.hasNext();) {
	...
	/**
	 * Convenience method, directly returning an iterator on a collection of all DMCs
	 ...
	 * @param processInstanceName id of process instance
	 * @return ArrayList of all specified DMCS
	 */
	public SortedSet getDMCs(String processName, String processInstanceName) {
		return (TreeSet)((HashMap)processes.get(processName)).get(processInstanceName);
	}
	
	/**
	 * Convenience method, directly delivering an iterator on all specified DMCs
	 * @param processName name of the process
	 * @param processInstanceName id of the process instance
	 * @return Iterator on all specified DMCs
	 */
	public Iterator getDMCsIterator(String processName, String processInstanceName) {
		return getDMCs(processName, processInstanceName).iterator();
	}
	
	/**
	 * Returns a list of all DMCs in a given process instance
	 * @param processInstance reference to a ProcessInstance
	 * @return ArrayList of all specified DMCS
	 */
	...
	/**
	 * @param processName ID/name of respective process
	 * @return iterator on the set of all process instance IDs contained for that process
	 */
	...
	/**
	 * @return names/ids of all process instances represented by any DMC within the set
	 */
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.dmcscanning.LogItem#getLeftBoundaryPosition()
	 */
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.dmcscanning.LogItem#getLeftBoundaryTimestamp()
	 */
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.dmcscanning.LogItem#getRightBoundaryTimestamp()
	 */
	 ...
	 * Returns the consistency ratio: 1.0==completely consistent, 0.0==forget it!
	 * @param equiv
	 * @return
	 */
	public double checkConsistency(ObjectEquivalence equiv) {
		...
		for(Iterator it=this.iterator(); it.hasNext();) {
			...
			if(!test.isConsistent(equiv)) {
				faults++;
				System.out.println("DMC #" + test.getIdNumber() + " found inconsistent!");
			}
		}
	...
	/**
	 * provides the required test output in pseudo-xml format 
	 * to Message.TEST
	 */
	public void testOutput() {
		Message.add("<DmcSet size=\"" + this.allDMCs.size() + "\">", Message.TEST);
		long hash = 0;
		for(Dmc dmc : this.allDMCs) {
			hash += dmc.size();
		}
		Message.add("\t<Hash>" + Long.toHexString(hash) + "</Hash>", Message.TEST);
