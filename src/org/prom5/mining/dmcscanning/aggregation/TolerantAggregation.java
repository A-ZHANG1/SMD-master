...
public class TolerantAggregation implements AggregationMethod {
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.dmcscanning.aggregation.AggregationMethod#isCompliant(org.processmining.mining.dmcscanning.Admc, org.processmining.mining.dmcscanning.Dmc)
	 */
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.dmcscanning.aggregation.AggregationMethod#compliance(org.processmining.mining.dmcscanning.Admc, org.processmining.mining.dmcscanning.Dmc)
	 */
	public double compliance(Admc anAdmc, Dmc aDmc) {
		if(anAdmc.footprint().equals(aDmc.footprint())) {
			...
			return ((double)(anAdmc.size() - conflicts) / (double)anAdmc.size());
		} else {
			return 0.0;
		}
	}
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.dmcscanning.aggregation.AggregationMethod#merge(org.processmining.mining.dmcscanning.Admc, org.processmining.mining.dmcscanning.Dmc)
	 */
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.dmcscanning.aggregation.AggregationMethod#consolidate(org.processmining.mining.dmcscanning.AdmcSet)
	 */
	public double consolidate(AdmcSet anAdmcSet) {
		int sizeBefore = anAdmcSet.size();
		for(Admc refAdmc : anAdmcSet.getAllCopy()) {
			if((anAdmcSet.consolidationVictims()!=null)&&(anAdmcSet.consolidationVictims().contains(refAdmc))) {
				continue;	// avoid checking already victimized elements
			}
			for(Admc testAdmc : anAdmcSet.getAllCopy()) {
				if((anAdmcSet.consolidationVictims()!=null)&&(anAdmcSet.consolidationVictims().contains(testAdmc))) {
					continue;	// avoid consolidating previous victims
				} else if((testAdmc!=refAdmc)&&(testAdmc.footprint().equals(refAdmc.footprint()))) {
					...
					for(Dmc toBeAdded : testAdmc.getDMCs()) {
						if(refAdmc.getDMCs().contains(toBeAdded)==false) {
							refAdmc.addOverridingChecks(toBeAdded, refAdmc.footprint());
						}
					}
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.dmcscanning.aggregation.AggregationMethod#getDescription()
	 */
