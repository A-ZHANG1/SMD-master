...
public class DistributionPanel extends JPanel {
	...
	protected void paintComponent(Graphics g) {
		...
		g.drawPolygon(curveFillXCoords, curveFillYCoords, curveFillXCoords.length);
		// draw grid
	...
	protected void doTheMath() {
		...
		for(int i=0; i<values.length; i++) {
			xCoords[i] = (int)(((double)(i) / (double)values.length) * this.getWidth());
	...
	protected void adjustValues() {
		for(int i=0; i<values.length; i++) {
			values[i] = (this.getHeight() - (int)(((double)values[i] / (double)maxVal) * this.getHeight()));
		}
	}
}
