...
public class DmcProximitySensing implements MiningPlugin {
	...
	public MiningResult mine(LogReader log) {
		...
		while(log.hasNext()) {
			...
			while(ates.hasNext()) {
				...
				if((last!=null)&&(last!=current)) {
					//Message.add("this: " + current.getTimestamp().getTime() + ", last: " + last.getTimestamp().getTime());
					currentProximity = current.getTimestamp().getTime() - last.getTimestamp().getTime();
		...
		Message.add("Done scanning proximity profile. Built " + statistics.getNumberOfEntries() + " entries.");
		Message.add("Distinct values recorded: " + statistics.getNumberOfValues());
		Message.add("Value range is: [" + statistics.getMinValue() + ", " + statistics.getMaxValue() + "].");
