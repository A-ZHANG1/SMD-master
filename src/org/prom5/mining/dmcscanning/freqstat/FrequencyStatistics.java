 ...
 */
package org.prom5.mining.dmcscanning.freqstat;
...
/**
 * Class for keeping statistics of and performing analysis
 * on the frequency in which (low-level) events occur.
 * 
 * @author Christian W. Guenther (christian at deckfour dot org)
 */
public class FrequencyStatistics {
	...
	/**
	 * Retrieves, for a given value, the number of entries that have
	 * been stored with that value (e.g., how many times '5' has been
	 * stored).
	 * @param value
	 * @return
	 */
	 ...
	 * The interval is interpreted as: [minVal, maxVal] (boundary-inlcusive).
	 * @param minVal
	 * @param maxVal
	 * @return
	 */
	public long getNumberOfEntriesBetweenValues(long minVal, long maxVal) {
		...
		for(Iterator it=distances.keySet().iterator(); it.hasNext();) {
			keyObj = (Long)it.next();
			if((keyObj.longValue()>=minVal)&&(keyObj.longValue()<=maxVal)) {
	...
	/**
	 * Creates an array of distribution values for e.g. displaying results
	 * in a plotted graph.
	 * @param maxValue the value the highest number of occurrences should be mapped to (e.g., display graph height)
	 * @param granularity the number of values to normalize to
	 * @return an integer array of size 'granularity', with values between [0, maxValue].
	 */
	public int[] getNormalizedDistribution(int maxValue, int granularity) {
		...
		for(int i=0; i<granularity; i++) {
			values[i] = getNumberOfEntriesBetweenValues((i*step), ((i+1)*step));
