 ...
 */
package org.prom5.mining.dmcscanning.logutils;
...
/**
 * AnchoredEvent is a wrapper class for AuditTrailEntry.
 * It transparently provides all functionality of AuditTrailEntry, augmented
 * by a reference to the originating ProcessInstance, plus the sequence number
 * (or, position) within that trace.
 * Notice: If the AuditTrailEntry is extended with a copy constructor this class
 * can be simplified by deriving it directly from AuditTrailEntry.
 * 
 * @author Christian W. Guenther (christian at deckfour dot org)
 */
public class AnchoredEvent implements LogItem {
	 ...
	 * key-unique counters. In short, we maintain to each process instance id
	 * a long counter. Requesting a new counter value to a process instance id
	 ...
	 * @param processInstanceId the process instance id within which the counter value is unique
	...
	protected static long nextCounterFor(String processInstanceId) {
		if(positions.containsKey(processInstanceId)) {
			...
			long counter = ((Long)positions.get(processInstanceId)).longValue();
	 ...
	 * @param anEntry the AuditTrailEntry instance to be wrapped
	 * @param aProcessInstance process instance the ATE belongs to
	 * @param aPosition position of the ATE within the process instance (starting from 0)
	 */
	...
	/**
	 * Creates a new AnchoredEvent that is automatically equipped with a sequence/position
	 * counter value, unique to the provided process instance.
	 * @param anEntry the AuditTrailEntry instance to be wrapped
	 * @param aProcessInstance process instance the ATE belongs to 
	 */
	public AnchoredEvent(AuditTrailEntry anEntry, ProcessInstance aProcessInstance) {
		this(anEntry, aProcessInstance,
			nextCounterFor(aProcessInstance.getProcess() + aProcessInstance.getName()) );
	}
	...
	public boolean equals(AnchoredEvent other) {
		return (isInSameProcessInstanceAs(other) && (position==other.getPosition()));
	}
	...
	/**
	 * @return the absolute position of this event with the process instance (starting from 0)
	 */
	 ...
	 * @param other another AnchoredEvent
	 * @return whether both events refer to the same (object identity) process instance
	 */
	 ...
	 * This distance can also be negative, indicating that the other event is in fact 
	 * located after the calling one.
	 * @param other
	 * @return
	 */
	...
	public String toString() {
		return "[AnchoredEntry wrapping: " + entry.toString() 
			+ ", ProcInst: " + processInstance.getName()
			+ ", SeqNrPos: " + position + "]";
	}
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.dmcscanning.logutils.LogItem#getComparisonIndex()
	 */
