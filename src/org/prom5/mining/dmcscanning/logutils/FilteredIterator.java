...
import org.prom5.mining.dmcscanning.logutils.filter.EventFilter;

/**
 * FilteredIterator
 * This class implements a read-only iterator over a
 * ProcessInstance, or rather the AuditTrailEntries
 * contained within it. Notice that this implies that
 * you cannot alter the iterated set in any way, i.e.
 * no removal or insertion methods are implemented.
 * Instances of this class can be equipped with an
 * EventFilter instance. This is used to filter
 * the set of audit trail entries from the wrapped
 * ProcessInstance following its specified 
 * constraints. 
 * Notice: Although a FilteredIterator can only be
 * equipped with one EventFilter instance at a time,
 * you can use e.g. the BooleanFilter to compose more
 * sophisticated filter criteria (e.g. multiple filters
 * of which all (AND), one (OR), etc. have to match).
 * 
 * @author Christian W. Guenther (christian at deckfour dot org)
 */
public class FilteredIterator implements Iterator {
	...
	/**
	 * @return the next matched AuditTrailEntry instance (casted to Object for compliance)
	 */
	public Object next() {
		if(cachedNextPosition<0) {
	...
	/**
	 * This class implements a read-only iterator, i.e. neither
	 * insert nor remove operations are supported.
	 * Thus, this method (from Iterator) has been overwritten
	 * with an empty (NOP) implementation.
	 */
	public void remove() {
	...
	/**
	 * @return bookmark containing the current iterator state (i.e. position) for later reset
	 */
	...
	/**
	 * Convenience method.
	 * @return the subset of the wrapped audit trail entries that match the filter criteria, in their original order
	 */
