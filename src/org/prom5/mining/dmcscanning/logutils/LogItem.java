...
/**
 * Interface 'LogItem'
 * This interface is to be used to access position and extension information
 * about log-anchored items. Implementing classes can represent log items
 * spread over multiple sub-items, and as such the start- and end-getter
 * methods are expected to return the respective boundary values.
...
public interface LogItem extends Comparable {
	 ...
	 * log item, the respective start- and end-getters are expected to return
	 * an identical value.
	 ...
	 * the sequence of events (starting from zero and ascending).
	 * If not implemented, return a negative value here.
	 * @return
	 */
