 ...
 */
package org.prom5.mining.dmcscanning.logutils.filter;
...
/**
 * Event filter matching all events before, after or at a given date.
 * For creating date-based range filters, compose two of these (left & right boundary)
 * 
 * @author Christian W. Guenther (christian at deckfour dot org)
 */
public class TimestampFilter implements EventFilter {
	 ...
	 * @param aTimestamp timestamp to match
	 * @param aMode mode in which to match (before, after, exactly)
	 */
	...
	public static TimestampFilter before(long aTimeMillis) {
		return new TimestampFilter(new Date(aTimeMillis), TimestampFilter.IS_BEFORE);
	}
	...
	public static TimestampFilter after(long aTimeMillis) {
		return new TimestampFilter(new Date(aTimeMillis), TimestampFilter.IS_AFTER);
	}
	...
	public static TimestampFilter exactly(long aTimeMillis) {
		return new TimestampFilter(new Date(aTimeMillis), TimestampFilter.IS_EXACTLY);
	}
	
	
	/* (non-Javadoc)
	 * @see org.processmining.mining.dmcscanning.abstractlog.EventFilter#matches(org.processmining.framework.log.AuditTrailEntry)
	 */
	public boolean matches(AuditTrailEntry entry) {
		if(mode==TimestampFilter.IS_AFTER) {
			...
			return true;	// incorrect filter matches all (excluded by factory methods)
		}
	}
