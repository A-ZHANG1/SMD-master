...
import org.prom5.mining.dmcscanning.DmcMiner;
...
public class DmcOptionsPanel extends JPanel implements ActionListener, ChangeListener {
	...
	public void setReasonableDefaults() {
		maxProxEvents.setText("9999");	// we almost always want that parameter to be 'unlimited'
		// setting the temporal proximity to 15 minutes seems a reasonable default value:
		...
		for(Iterator it=DmcMiner.equivalenceRelations().iterator(); it.hasNext();) {
			...
			if(equiv.matches((String)summary.getSource().getName())) {
				// the equivalence relation impl. seems to be fit for that source program - assign
				equivalences.setSelectedItem(equiv);
				break;
			}
		}
		consolidateAdmc.setSelected(true);		// consolidation hasn't proven beneficial so far
