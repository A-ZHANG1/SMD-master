...
import org.prom5.mining.dmcscanning.Admc;
import org.prom5.mining.dmcscanning.Dmc;
import org.prom5.mining.dmcscanning.DmcScanningResult;

/**
 * @author Christian W. Guenther (christian at deckfour dot org)
 */
public class DmcResultView extends JPanel implements ChangeListener, ListSelectionListener, ActionListener {
	...
	protected JSplitPane logItemsVerticalSplit = null;		// splits the LogItemPanel view horizontally
	protected JPanel logItemViewTop = null;					// top container for the LogItemPanel view
	protected JScrollPane logItemPanelScroll = null;		// scroll pane for LogItemPanel
	protected LogItemPanel logItemPanel = null;				// the LogItemPanel used for log item visualization
	protected JSlider zoomSlider = null;					// slider for zooming the view
	
	protected JTabbedPane madmcTabs = null;					// tabbed pane to choose between MDMC/ADMC display
	...
	protected JComboBox procInstSelect = null;				// combo box for process instance filtering/selection
	protected JComboBox handleVisSelect = null;				// combo box for handle visualization selection
	

	/**
	 * constructor
	 * @param aResult the result container to query data from
	 */
	...
	protected void initializeGui() {
		...
		logItemPanel = new LogItemPanel(result.getDmcSet().getLeftBoundaryTimestamp(), result.getDmcSet().getRightBoundaryTimestamp());
		logItemPanelScroll = new JScrollPane(logItemPanel, JScrollPane.VERTICAL_SCROLLBAR_NEVER, JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
		...
		logItemViewTop.setLayout(new BoxLayout(logItemViewTop, BoxLayout.X_AXIS));
		...
		logViewOptions.setLayout(new BoxLayout(logViewOptions, BoxLayout.Y_AXIS));
		logViewOptions.setAlignmentX(JPanel.LEFT_ALIGNMENT);
		logViewOptions.setBorder(BorderFactory.createTitledBorder("View options"));
		...
		procInstSelect = new JComboBox(result.getDmcSet().getProcessInstances().toArray());
		...
		admcList = new JList();
		dmcList = new JList();
		JSplitPane logItemLowSplitGeneral = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true);
		JSplitPane logItemLowSplitBrowser = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true);
		...
		mdmcScrollPane = new JScrollPane(mdmcList, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
		admcList.addListSelectionListener(this);
		admcList.setCellRenderer(new DmcListCellRenderer());
		admcScrollPane = new JScrollPane(admcList, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
		...
		JScrollPane dmcScrollPane = new JScrollPane(dmcList, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
		JPanel dmcBrowserPanel = new JPanel();
		dmcBrowserPanel.setLayout(new BoxLayout(dmcBrowserPanel, BoxLayout.Y_AXIS));
		...
		JScrollPane footprintScrollPane = new JScrollPane(footprintList, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
	...
	public void initializeBrowser() {
		mdmcList.setListData(reverseOrder(result.getMdmcSet().getAll().toArray()));
		admcList.setListData(reverseOrder(result.getAdmcSet().getAllSorted().toArray()));
	...
	public void stateChanged(ChangeEvent e) {
		// handle zoom events
		if((e.getSource()==zoomSlider) ){ //&& !zoomSlider.getValueIsAdjusting()) {
			double zoom = StrictMath.pow(10.0, ((double)zoomSlider.getValue() / 1000.0)) ;
	...
	public void valueChanged(ListSelectionEvent e) {
		...
		if(e.getSource().equals(mdmcList)) {
			...
			dmcIdField.setText("-/-");
			if(values.length==1) {
				footprintList.setListData(((Admc)values[0]).footprint().toArray());
			} else {
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==this.procInstSelect) {
			...
			if(this.procInstSelect.getSelectedItem().equals("show all")) {
				...
				processInstanceFilter = (String)this.procInstSelect.getSelectedItem();
			}
			updateLogItemPanel();
		} else if(e.getSource()==this.handleVisSelect) {
			// change handle visualization
			this.logItemPanel.setItemHandle((String)this.handleVisSelect.getSelectedItem());
			updateLogItemPanel();
		}
	}
	
	/**
	 * adjusts the LogItemPanel view to the current selection
	 * within the browser components.
	 */
		...
		if(selected.length>0) {
			...
			for(int i=0; i<selected.length; i++) {
				if((processInstanceFilter==null) 
						|| ((Dmc)selected[i]).getProcessInstance().getName().equals(processInstanceFilter) ) {
					logItemPanel.addItem(dmc2key((Dmc)selected[i]), (Dmc)selected[i]);
				}
			}
		} else {
			...
			for(int i=0; i<admcSelection.length; i++) {
				if(processInstanceFilter==null) {
					logItemPanel.addItemSet(((Admc)admcSelection[i]).getIdString(), 
							new TreeSet<Dmc>(((Admc)admcSelection[i]).getDMCs()) );
				} else {
					SortedSet dmcs = new TreeSet<Dmc>(((Admc)admcSelection[i]).getDMCs());
					String admcKey = ((Admc)admcSelection[i]).getIdString();
					for(Iterator it=dmcs.iterator(); it.hasNext();) {
						Dmc dmc = (Dmc)it.next();
						if(dmc.getProcessInstance().getName().equals(processInstanceFilter)) {
	...
	/**
	 * convenience method.
	 * returns to a given DMC (from e.g. a selection) the id key string
	 * of the (first found) ADMC it is contained within
	 * @param aDmc
	 * @return
	 */
	...
	/**
	 * test main routine
	 * TODO: remove after ProM integration
	 * @param args
	 */
