 ...
 */
package org.prom5.mining.dmcscanning.ui;
...
import org.prom5.mining.dmcscanning.logutils.LogItem;

/**
 * LogItemPanel.
 * 
 * The purpose of this class is to provide a view of a
 * log, in form of a horizontal ribbon, with abstract items
 * (that have a start and end time) laid out on it, equipped
 * with handles to distinguish.
 * This class expects SortedSets of LogItems, which are
 * grouped automatically with a specific color and can be
 * removed again bulk-wise.
 * Make sure that the added LogItems fully support the methods
 * to retrieve start and end in temporal fashion, as the 
...
public class LogItemPanel extends JPanel {
	...
	/**
	 * constructor
	 * @param leftBoundary Date representing the left boundary (start) of the log to be visualized
	 * @param rightBoundary Date representing the right boundary (end) of the log to be visualized
	 */
	...
	/**
	 * Adds a SortedSet of LogItems to be displayed as a group of items
	 * within this viewport. They are referenced by a string key, to be
	 * able to e.g. selectively remove groups of items later on.
	 * @param key reference to this item set, for e.g. later selective removal
	 * @param anItemSet expects a sorted set of 'LogItem' instances.
	 ...
	 * The supplied key is used to group items belonging together (e.g. of
	 * one cluster or process instance).
	 * @param key identifier used to group related items (by color etc.)
	 ...
	 * @param key reference string for the set to remove
	 * @return success indicator: if false, the set was not contained and thus not removed
	 */
	...
	/**
	 * initializes componente, i.e. all item sets formerly displayed
	 * are removed.
	 */
	 ...
	 * @param key the key referencing the item set
	 ...
	 * Notice: non-checking, care for sane values!
	 * @param aBoundary the left log boundary
	 */
	 ...
	 * adjusts the log boundaries to be displayed.
	 * Notice: non-checking, care for sane values!
	 * @param aBoundary the right log boundary
	 */
	 ...
	 * adjusts the log boundaries to be displayed.
	 * Notice: non-checking, care for sane values!
	 * @param leftBoundary the left log boundary
	 * @param rightBoundary the right log boundary
	 */
	 ...
	 * Interpretation: displayedYcoordinate = timeInMillis * milli2pixels;
	 */
	protected void updateMilli2pixelsRatio() {
		milli2pixels = (double)this.getWidth() / (double)(logBoundaryRight.getTime() - logBoundaryLeft.getTime());
	 ...
	 * @param anItemHandle use one of the static ITEM_HANDLE_* strings provided, otherwise it will be a mess
	 */
	 ...
	 * @param aRatio a value within [0.1, 0.9]
	 */
	 ...
	 * paints a log item according to the given coordinates
	 * @param xStart horizontal start coordinate of the event
	 * @param xEnd horizontal end coordinate of the event
	 * @param yLogTop vertical coordinate of the log pane level (top)
	 * @param yLogHeight vertical size (height) of log items
	 * @param yHandle vertical coordinate of the handle level (center)
	 * @param g the Graphics object used for painting
	 */
	protected void paintItem(int xStart, int xEnd, int yLogTop, int yLogHeight, int yHandle, Graphics g) {
		if((xEnd-xStart)<=1) { xEnd = xStart + 2; }	// ensure minimum width of 2
	...
	protected void paintLogLane(Graphics g) {
		...
		long clipLeftTs  = (coord2timeMillis(clipL) / logLaneStep) * logLaneStep;	// smallest multiple of step left to clip border
	...
	public void paintComponent(Graphics grx) {
		...
		//System.out.println("Clip: " + clip.getMinX() +  " - " + clip.getMaxX());
		...
		if(itemSets.size()>0) {
			...
			for(Iterator itSets = itemSets.keySet().iterator(); itSets.hasNext();) {
				key = (String)itSets.next();
				g.setColor(itemColors.getColor(key));
				// iterate through items
				for(Iterator itItm=((SortedSet)itemSets.get(key)).iterator(); itItm.hasNext();) {
					item = (LogItem)itItm.next();
					if(intersects(clipLeftDt, clipRightDt, item.getLeftBoundaryTimestamp(), item.getRightBoundaryTimestamp())) {
						// paint only if visible
						this.paintItem(time2coord(item.getLeftBoundaryTimestamp()), 
									time2coord(item.getRightBoundaryTimestamp()),
									logItemTop, logItemHeight, handleHeight, g);
					}
				}
				// move handles up
				...
				if(logItemTop < (getLogPaneHeight()+border)) {	// prevent flip-over
					logItemTop = handleHeight + getLogPaneHeight() - (2 * border);
				}
			}
		}
	}
