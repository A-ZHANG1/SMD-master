...
package org.prom5.mining.epcmining;
...
public class EPCResult implements LogReaderConnection, MiningResult, Provider {
    ...
    public EPCResult(LogReader log, ConfigurableEPC epc, MiningPlugin algorithm) {
        ...
        if (epc != null) {
            ...
            if (showWarnings) {
                ...
                if (m.length() > 0) {
                    // TODO: showWarnings = showWarningDialog(m);
                    showWarnings = false;
                }
            }
        }
    ...
    private boolean showWarningDialog(String message) {
        ...
        dialog.getContentPane().add(noMoreCheck,
    ...
    public ProvidedObject[] getProvidedObjects() {
        ConfigurableEPC epc = getSelectedEPC();
        ProvidedObject[] providedObjects = new ProvidedObject[1 + (epc == null ? 0 : 1)];


        if (log == null) {
            providedObjects[0]= new ProvidedObject("EPC Hierarchy", new Object[] {hierarchy});
        } else {
            providedObjects[0] =new ProvidedObject("EPC Hierarchy", new Object[] {hierarchy, log});
        }
        if (epc!=null) {
            providedObjects[1] = new ProvidedObject("Selected EPC", new Object[] {getSelectedEPC()});
        }
