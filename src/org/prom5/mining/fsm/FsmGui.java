...
import org.prom5.converting.fsm.FsmModificationGui;
...
public class FsmGui extends JPanel {
    ...
    // The check box to select whether or not the attribute settings should be used.
    private JCheckBox useAttributesCheckBox;
    // A JTree to browse and change the attribute settings.
    /**
     * The JTree will have three levels:
     * 1. The attribute name.
     * 2. The attribute value.
     * 3. The value group.
     * The user can edit the value group. Only attributes which have more than one
     * value groups will be taken into account. In the FSM, the attributes will be
     * assigned the value group (not the value itself, but the group). This way,
     * it is possible to cluster certain values.
     */
     ...
     * @param summary LogSummary the smmary to get the model elements from.
     * @param title String the label of the tab.
     */
     ...
     * @param summary LogSummary the summary to get the attrbiutes andsofroth from.
     * @param title String the label of the tab.
     */
    private void addAttributeTab(LogSummary summary, String title) {
        ...
        String elements[] = new String[defaults.getAttributeSettings().keySet().size()];
        int i = 0;
        ...
        /*
         * Use a simple GUI (a JList) to select visible attributes.
         * This does not allow values to be grouped, though.
         */
        ...
        attributeFilterList.setBackground(FsmGuiColorScheme.FIELDBACKGROUNDCOLOR);

        // Initially, all model elements are selected.
        // attributeFilterList.setSelectionInterval(0, elements.length - 1);
        TitledBorder border = new TitledBorder(
                "Select the visible attributes");
        ...
        /*
         * Forget about the complex (JTree) GUI that did allow for values to be grouped.
         * 
        JPanel attributePanel = new JPanel(new BorderLayout());
        tabbedPane.addTab(title, attributePanel);
        tabbedPane.setForeground(FsmGuiColorScheme.FOREGROUNDCOLOR);
        ...
        // Get the aatribute info from the summary.
        // Results are stored in the default settings.
        getAttributeInfo();
        ...
        for (Object attribute : defaults.getAttributeSettings().keySet()) {
            ...
            HashMap map = defaults.getAttributeSettings().get(attribute);
            for (Object value : map.keySet()) {
                ...
                DefaultMutableTreeNode clusterNode = new DefaultMutableTreeNode(
                        map.get(value));
                valueNode.add(clusterNode);
            }
        }
        DefaultTreeModel treeModel = new DefaultTreeModel(rootNode);
        attributeTree = new JTree(treeModel);
        ...
        treeRenderer.setBackgroundNonSelectionColor(FsmGuiColorScheme.FIELDBACKGROUNDCOLOR);
        treeRenderer.setForeground(FsmGuiColorScheme.FOREGROUNDCOLOR);
        attributeTree.setCellRenderer(treeRenderer);
        JScrollPane treePane = new JScrollPane(attributeTree);
        treePane.setForeground(FsmGuiColorScheme.FOREGROUNDCOLOR);
        treePane.setBackground(FsmGuiColorScheme.FIELDBACKGROUNDCOLOR);
        attributePanel.setForeground(FsmGuiColorScheme.FOREGROUNDCOLOR);
        attributePanel.setBackground(FsmGuiColorScheme.BACKGROUNDCOLOR);
        attributePanel.setBorder(new TitledBorder("Set labels for the attribute values (only attributes with different labels will be taken into account)"));
        attributePanel.add("Center", treePane);
         */
    }

    private void getAttributeInfo() {
        // This is awkward: the summary provides no information on data attributes.
        ...
        // Next iterate over all process instances and all audit trails to collect data attribute information.
    ...
    public FsmSettings getSettings() {
        ...
        for (int mode = 0; mode < FsmMinerPayload.LAST; mode++) {
            ...
            settings.setHorizonSettings(false, mode, fwdTab[mode].GetSettings());
        }
        ...
        for (String element: defaults.getAttributeSettings().keySet()) {
        	if (attributeFilterList.isSelectedIndex(i)) {
                ...
                HashMap<String,String> defaultMap = defaults.getAttributeSettings().get(element);
        ...
        DefaultMutableTreeNode rootNode = (DefaultMutableTreeNode)
                                          attributeTree.getModel().getRoot();
        if (rootNode.getChildCount() > 0) {
            for (DefaultMutableTreeNode attributeNode = (DefaultMutableTreeNode)
                    rootNode.
                    getFirstChild();
                    attributeNode != null;
                    attributeNode = attributeNode.getNextSibling()) {
                HashMap<String, String> map = new HashMap<String, String>();
                for (DefaultMutableTreeNode valueNode = (DefaultMutableTreeNode)
                        attributeNode.
                        getFirstChild();
                        valueNode != null;
                        valueNode = valueNode.getNextSibling()) {
                    DefaultMutableTreeNode clusterNode = valueNode.getFirstLeaf();
                    String clusterName = clusterNode.toString();
                    for (String mappedClusterName : map.values()) {
                        if (clusterName.compareTo(mappedClusterName) == 0) {
                            clusterName = mappedClusterName;
                        }
                    }
                    map.put(valueNode.toString(), clusterName);
                }

                // The map seems to be bale to contain multiple keys which have the same string value.
                // Convert the map to a TreeMap and check whiheter this one contains
                // multiple string values.
                TreeSet set = new TreeSet(map.values());
                if (set.size() > 1) {
                    // Multiple values found, add it.
                    settings.setUseAttributes(true);
                    settings.getAttributeSettings().put(attributeNode.toString(),
                            map);
                }
            }
        }
