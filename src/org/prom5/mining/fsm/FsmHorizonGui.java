...
/**
 * <p>Title: FsmHorizonGui</p>
 *
 * <p>Description: GUI for changing a horizon setting.</p>
 *
...
public class FsmHorizonGui extends JPanel {
    ...
    /**
     * Creates the GUI.
     * @param settings FsmHorizonSettings the settings to display.
     * @param tabbedPane JTabbedPane the parent.
    ...
    public FsmHorizonGui(FsmHorizonSettings settings, JTabbedPane tabbedPane,
        ...
        abstractionComboBox.setBorder(new TitledBorder("Select an abstraction"));
        ...
        abstractionComboBox.setEditable(true); // Turn on the Editor (to get white background)
        abstractionComboBox.getEditor().getEditorComponent().setBackground(FsmGuiColorScheme.FIELDBACKGROUNDCOLOR);
        abstractionComboBox.getEditor().getEditorComponent().setFocusable(false); // But denied focus (thus, user can only select from dropdown)
        ...
        filteredPanel.setBorder(new TitledBorder("Only filtered items will count for the filtered horizon"));
        ...
        JSpinner.DefaultEditor editor = (JSpinner.DefaultEditor)horizonSpinner.getEditor();
        editor.getTextField().setBackground(FsmGuiColorScheme.FIELDBACKGROUNDCOLOR);
        editor.getTextField().setForeground(FsmGuiColorScheme.FOREGROUNDCOLOR);
        southPanel.add("North", horizonSpinner);

        filteredHorizonSpinner = new JSpinner(new SpinnerListModel(spinnerStrings));
        filteredHorizonSpinner.setBorder(new TitledBorder("Select a filtered horizon"));
