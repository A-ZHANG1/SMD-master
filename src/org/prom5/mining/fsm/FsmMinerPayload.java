...
/**
 * <p>Title: FsmMinerPayload</p>
 *
 * <p>Description: Payload for the states mined by the FsmMiner</p>
 *
...
public class FsmMinerPayload extends FSMPayload {
     ...
     * of the list of abstractions. Its value should always be equal to the number
    ...
    /**
     * The settings used to obtain this payload.
     * To store the settings for every payload seems like overkill, but because we
     * have the settings we can nicely forget about abstractions that were not
     * selected. Thus, by adding the 1 handle (settings) we can remove 4 (the abstractions
     ...
     * abstractiosn differ, we use the very generic Object class here. Ny using the
     * settings, we can tell which class the abstraction should be and cast it to
     * this class.
     */
            ...
            for (int mode = 0; mode < LAST; mode++) {
                ...
                if (horizonSettings.getUse()) {
                    ...
                    case (FsmHorizonSettings.SET): {
                        ...
                        mergedPayload.getBwdSet(mode).addAll(minerPayload.
                                getBwdSet(mode));
                        break;
                    }
                    case (FsmHorizonSettings.BAG): {
            ...
            for (int mode = 0; mode < LAST; mode++) {
                ...
                if (horizonSettings.getUse()) {
                    ...
                    case (FsmHorizonSettings.SET): {
                        ...
                        mergedPayload.getFwdSet(mode).addAll(minerPayload.
                                getFwdSet(mode));
                        break;
                    }
                    case (FsmHorizonSettings.BAG): {
     ...
     * @param object Object the other payload.
     * @return int
     */
