...
/**
 * <p>Title: FSMSettings</p>
 *
 * <p>Description: Holds settings for the FSM miner</p>
 *
...
public class FsmSettings {
	...
	public FsmSettings(LogSummary summary) {
		...
		/**
		 * Transitions in the resulting FSM will be labeled by the model elements.
		 * Irrelevant model elements can be filtered out by this filter. Typically,
		 * model elemtns that were filtered out result in unlabeled transitions.
		 * As only one transition with a certain label (unlabeled corresponds to an
		 * empty label), restricting this filter one obtains less transitions in
		 * the resulting FSM.
		 *
		 * For originators and event types it makes no sense to have similar filters.
		 */
		visibleFilter = new HashSet<String>();

		// Initialize the filters for all horizon settings.
		bwdHorizonSettings[FsmMinerPayload.MODELELEMENT].addAllToFilter(summary.getModelElements());
		fwdHorizonSettings[FsmMinerPayload.MODELELEMENT].addAllToFilter(summary.getModelElements());
		bwdHorizonSettings[FsmMinerPayload.ORIGINATOR].addAllToFilter(summary.getOriginators());
		fwdHorizonSettings[FsmMinerPayload.ORIGINATOR].addAllToFilter(summary.getOriginators());
		bwdHorizonSettings[FsmMinerPayload.EVENTTYPE].addAllToFilter(summary.getEventTypes());
		fwdHorizonSettings[FsmMinerPayload.EVENTTYPE].addAllToFilter(summary.getEventTypes());
	 ...
	 * @param isBwd boolean if true, then get backward horizon setting, else get forward horizon setting.
	 * @param mode int if FsmMinerPayload.MODELELEMENT then get model element horizon setting, etc.
	 ...
	 * @param isBwd boolean if true, then get backward horizon setting, else get forward horizon setting.
	 * @param mode int if FsmMinerPayload.MODELELEMENT then get model element horizon setting, etc.
	 * @param newSettings FsmHorizonSettings new settings.
	 * @return FsmHorizonSettings old settings, possibly null.
	 */
	public FsmHorizonSettings setHorizonSettings(boolean isBwd, int mode, FsmHorizonSettings newSettings) {
	 ...
	 * @param newUse boolean new value.
	 ...
	 * @param newMap HashMap new settings.
	 * @return Map old settings.
	 */
	public Map<String, HashMap<String, String>> setAttributeSettings(HashMap<String, HashMap<String, String>> newMap) {
	 ...
	 * @param newFilter HashSet new filter.
	...
	public FsmModificationSettings setModificationSettings(FsmModificationSettings newSettings) {
	 ...
	 * @param newGUI boolean new value.
	 * @return boolean old value.
	 */
