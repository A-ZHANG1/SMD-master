...
public class LoadPopulation {
	...
	public LoadPopulation(String inputDirIndividuals) {
		...
		try {
			...
			System.err.println("No individuals in directory '" + inputDirIndividuals + "'");
	...
	private void loadPopulation(String inputDirIndividuals) throws IOException {
		...
		individualsPath = MethodsForFiles.listOfFilesToMine(inputDirIndividuals,
						  HnExport.FILE_TERMINATION);
		if (individualsPath.length > 0) {
			for (int i = 0; i < individualsPath.length; i++) {
				try {
					auxPopulation.add(new HeuristicsNetFromFile(new FileInputStream(individualsPath[
							i])).getNet());
				} catch (Exception exc) {
					System.err.println("Could not load individual at => " + individualsPath[i]);
				}
			}
		}
...
//    private EnhancedHeuristicsNet[] removeNullIndividuals(EnhancedHeuristicsNet[] population){
//		Vector v = new Vector();
//
//	}
