	...
	public Performance(Fitness fit, HashSet pop) {
		...
		for (int i = 0; i < populations.length; i++) {
			try {
				...
				//copying populations
				populations[i].setPopulation(fit.calculate(populations[i].getPopulation()));
...
//					populations[i][populations[i].length - 1].getFitness());
				...
				if (Double.isNaN(BFE) || BFE < auxPop[auxPop.length - 1].getFitness()) {
				...
				if (Double.isNaN(WFE) || WFE > auxPop[auxPop.length - 1].getFitness()) {
				...
				try {
					for (int j = 0; j < currentPopulation.length; j++) {
						if (currentPopulation[j].getFitness() == fitness) {
							sb.append(populations[i].getIndividualsPath()[j]).append(";");
						}
					}
				} catch (NullPointerException npe) {
					//just proceed...
