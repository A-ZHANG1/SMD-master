...
package org.prom5.mining.geneticmining.analysis;
...
public class SummaryReport implements Report {
	...
	public SummaryReport(String inputDirLogs, String inputDirRuns,
			String logTermination, int fitness, String reportName) throws IOException {
...
//creating report file
	...
	private void mapLogsToRuns(String inputDirLogs, String inputDirRuns,
		...
		for (int i = 0; i < dirs.length; i++) {
			filteredLogs.addAll(Arrays.asList(MethodsForFiles.listOfFilesToMine(dirs[i],
					logTermination)));
		}
		...
		for (int i = 0; i < dirs.length; i++) {
			for (int j = 0; j < logs.length; j++) {
				if (dirs[i].indexOf(new File(logs[j]).getName()) >
						-1) {
	...
	private void checkInput(String inputDirLogs, String inputDirRuns) {
		...
		if (!f.isDirectory()) {
			System.err.println("Input directory for log files = '" + f.getAbsolutePath() +
					"' is not a directory!");
			System.exit( -1);
		}
		...
		if (!f.isDirectory()) {
			System.err.println("Input directory for runs = '" + f.getAbsolutePath() +
					"' is not a directory!");
			System.exit( -1);
		}
...
//				}
	...
	private void writePerformance(String log, TreeSet runs) {
		...
		try {
			logReader = LogReaderFactory.createInstance(logFilter, LogFile.getInstance(log));
		} catch(Exception e) {
			e.printStackTrace();
			logReader = null;
		}
		...
		while (iter.hasNext()) {
			...
			if (pop != null && pop.getPopulation() != null) {
				...
				if (currentRunDir.lastIndexOf(DTGeneticMiner.GENERATION_TAG) > 0) {
					generation = Integer.parseInt(currentRunDir.substring(currentRunDir.lastIndexOf(
								 DTGeneticMiner.GENERATION_TAG) +
		...
		perf = new Performance(FitnessFactory.getFitness(fitness, logReader,DTFitnessFactory.ALL_FITNESS_PARAMETERS), pops4log);

		logReader = null;

		writeReportLine(log, pops4log.size(), Double.toString(perf.getBFE()),
				Double.toString(perf.getWFE()), Double.toString(perf.getMBF()),
				Double.toString(perf.getNumRunsBFE()), Double.toString(perf.getNumRunsWFE()),
				maxGeneration, minGeneration,
				sumGenerations, DescriptiveStatistics.mode(msGenerations), msGenerations.toString(),
				perf.getBestIndividuals());
	...
	private void writeReportHeader() {
		...
		try {
			...
			System.err.println("Could not write line = '" + sb.toString() + "' to the report!");
		}

	}

	private void writeReportLine(String log, int numRuns, String BFE, String WFE, String MBF,
			String numRunsBFE, String numRunsWFE,
		...
		sb.append(numRuns).append(REPORT_SEPARATOR);
		sb.append(FitnessFactory.getAllFitnessTypes()[this.fitness]).append(REPORT_SEPARATOR);
		sb.append(bestIndividuals).append(REPORT_SEPARATOR);
	...
	public static void main(String[] args) throws IOException {
		...
		sr = new SummaryReport(param.getParameter("DIR_LOGS"), param.getParameter("DIR_INDIVIDUALS"),
			 param.getParameter("LOGS_FILE_TERMINATION"),
			 Integer.parseInt(param.getParameter("FITNESS_TYPE")), param.getParameter("OUTPUT_FILE"));
		sr.generateReport();
