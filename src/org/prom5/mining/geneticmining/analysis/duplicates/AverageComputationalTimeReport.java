...
/**
 * <p>Title: Average Computational Time for GA's Runs </p>
 *
 * <p>Description: This class calculates the average computational time that runs of a log file take. </p>
 *
...
public class AverageComputationalTimeReport {
	...
	public AverageComputationalTimeReport(String dirSolutionModels,
			...
			String fieldsSeparator) throws FileNotFoundException, IOException {
	...
	private void generateReport() throws IOException {
		...
		bw.write("Net" + fieldsSeparator + "Number of Runs" + fieldsSeparator +
				"Average Time per Run (seconds)" + fieldsSeparator + "Average Time per Generation (seconds)" +
				fieldsSeparator + "Average Number of Generations");
		bw.newLine();
		...
		for (int i = 0; i < foundLogsWithInformationAboutRuns.length; i++) {
			bw.write(desiredSolutions.getNames()[i] + fieldsSeparator +
					foundLogsWithInformationAboutRuns[i].length + fieldsSeparator +
					averageTimeAllRuns[i] + fieldsSeparator +
	...
	private void calculateAverageComputationalTimes() throws FileNotFoundException, IOException {
		...
		for (int i = 0; i < foundLogsWithInformationAboutRuns.length; i++) {
			...
			for (int j = 0; j < foundLogsWithInformationAboutRuns[i].length; j++) {
				...
				String[] lineTokens = previousLine.split(this.fieldsSeparatorRunLogs);

				double totalTimeRun = Double.parseDouble(lineTokens[GeneticMinerConstants.
									  getIndexConstantInLogLine("Elapsed Time(ms)")]);
				double numGenerationsRun = (Double.parseDouble(lineTokens[GeneticMinerConstants.
										   getIndexConstantInLogLine("Generation Number")])) + 1; //because it starts at zero
			...
			averageNumberOfGenerationsPerRun[i] /= (double) foundLogsWithInformationAboutRuns[i].
					length;
