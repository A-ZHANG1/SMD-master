 ...
 * with respect to the connection between the tasks.</p>
 *
...
public class ConnectionEquivalent implements PrecisionRecall {
	...
	public ConnectionEquivalent(HeuristicsNet baseHN,
		...
		if (foundHN != null) {
			...
			//making the foundHN and the baseHN work on the same indeces for the tasks/elements...
			try {
	...
	private void calculatePrecisionAndRecall() {
		...
		for (int element = 0; element < baseHN.getLogEvents().size(); element++) {
			...
			if (inputBaseNetMS.size() == 0) { //there are not connections for this element.
				numberOfElementsBaseHNWithoutCausalityRelations++;
			} else if (inputBaseNetMS.size() > 0) {
				recall += ((double) intersectionMS.size() / inputBaseNetMS.size());
			} //else, division by zero, no need to increment the recall

			if (inputFoundNetMS.size() == 0) { //there are not connections for this element.
				numberOfElementsFoundHNWithoutCausalityRelations++;
			} else if (inputFoundNetMS.size() > 0) {
				precision += ((double) intersectionMS.size() / inputFoundNetMS.size());
	...
	private static Multiset retrieveInputTasks(HeuristicsNet net,
			int element) {
		...
		try {
			...
			for (int i = 0; i < duplicates.size(); i++) {
				ms.addAll(toMultiSet(net.getAllElementsInputSet(duplicates.get(i)),
						net.getDuplicatesMapping(),
						net.getLogEvents()));
			}
	...
	private static Multiset toMultiSet(HNSubSet subset, int[] duplicatesMapping, LogEvents le) {
		Multiset ms = new HashMultiset();
