...
import org.prom5.mining.geneticmining.GeneticMinerConstants;
...
public class DoubleFieldValuesPerGeneration {
	...
	public DoubleFieldValuesPerGeneration(String dirSolutionModels,
			String fileTerminationSolutionModels,
			String dirFoundModels, String fileTerminationRunsLogs,
			String fieldsSeparatorRunLogs,
			String reportFileName,
			String fieldsSeparator, String generationNumberColumnName, int[] generationsToConsider,
			String fieldColumnName) throws FileNotFoundException,
			IOException {

		//retrieving the desired solutions
		desiredSolutions = new RetrieveFiles(dirSolutionModels, fileTerminationSolutionModels, false);

		//retrieving the logs of every run of the genetic algorithm to an event log file
		foundLogsWithInformationAboutRuns = new RetrieveFilesPerElement(desiredSolutions.getNames(),
											dirFoundModels,
		...
		Arrays.sort(this.listOfGenerationsToConsider); //ordering the generations
	...
	private void calculateDoubleValuesForField() throws FileNotFoundException, IOException {

	averageFieldValuesPerGeneration = new double[foundLogsWithInformationAboutRuns.length][
										  listOfGenerationsToConsider.length];

	numberOfFoundGenerations = 	new double[foundLogsWithInformationAboutRuns.length][
										  listOfGenerationsToConsider.length];
			...
			for (int j = 0; j < foundLogsWithInformationAboutRuns[i].length; j++) {
				...
				while ((line = br.readLine()) != null) {
					if (line.indexOf(generationNumberColumnName) < 0) {
						String[] lineTokens = line.split(this.fieldsSeparatorRunLogs);
						int numGenerationsRun = (Integer.parseInt(lineTokens[GeneticMinerConstants.
												getIndexConstantInLogLine(
												generationNumberColumnName)]));
						if (numGenerationsRun ==
								this.listOfGenerationsToConsider[indexListGenerations]) {
							...
							if (indexListGenerations >= (listOfGenerationsToConsider.length - 1)) {
								//all the elements in the list of generations have been considered!
								break;
							} else {
								//increment to the next generation in the list
								indexListGenerations++;
							}
						} else if (numGenerationsRun >
								this.listOfGenerationsToConsider[indexListGenerations]) {
							//testing if the other generations in the list of generations to consider (variable
							//'listOfGenerationsToConsider') can be equal to the current generation in the file

							for (int index = indexListGenerations;
											 index < listOfGenerationsToConsider.length; index++) {
								if (numGenerationsRun == this.listOfGenerationsToConsider[index]) {
									averageFieldValuesPerGeneration[i][indexListGenerations] +=
											Double.	parseDouble(lineTokens[GeneticMinerConstants.
											getIndexConstantInLogLine(this.fieldColumnName)]);
									numberOfFoundGenerations[i][indexListGenerations] += 1;
								...
								if (indexListGenerations >= listOfGenerationsToConsider.length) {
									break; //all the list of generation has been considered
								}
							}
						}
					}
	...
	private void generateReport() throws IOException {
			...
			for(int i=0; i<this.listOfGenerationsToConsider.length; i++){
				bw.write(fieldsSeparator + "gen" + this.listOfGenerationsToConsider[i]);
			}
			bw.newLine();
			...
			for (int i = 0; i < foundLogsWithInformationAboutRuns.length; i++) {
				...
				for(int j=0; j<this.listOfGenerationsToConsider.length; j++){
					double value = (numberOfFoundGenerations[i][j] > 0.0 ? (this.averageFieldValuesPerGeneration[i][j]/numberOfFoundGenerations[i][j]):0.0);
	...
	public static void main(String args[]) {
		...
		try {
			...
			DoubleFieldValuesPerGeneration fvpg = new DoubleFieldValuesPerGeneration(
												  ...
												  p.getParameter("REPORT_FILE_NAME"),
												  p.getParameter("REPORT_FIELDS_SEPARATOR"),
												  p.getParameter("GENERATION_NUMBER_COLUMN_NAME"),
												  p.getArrayIntegerParameter(
												  "GENERATIONS_TO_CONSIDER"),
												  p.getParameter(
												  "NAME_COLUMN_FIELD_TO_CONSIDER"));
