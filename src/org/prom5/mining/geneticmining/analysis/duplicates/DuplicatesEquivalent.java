 ...
 * with respect to the number of duplicates that they have in common.</p>
 *
...
public class DuplicatesEquivalent implements PrecisionRecall {
	...
	private void calculatePrecisionAndRecall() {
		...
		try {
			recall = ((double) intersectionTasksMS.size() / tasksBaseNetMS.size());
		...
		try {
			precision = ((double) intersectionTasksMS.size() / taskstFoundNetMS.size());
		} catch (ArithmeticException exc) {
			//division by zero
			precision = 0.0;
		}
	...
	private static Multiset toMultiSet(HeuristicsNet net) {
		...
		for (int i = 0; i < net.getDuplicatesMapping().length; i++) {
			if ((net.getInputSet(i).size() > 0) || (net.getOutputSet(i).size() > 0)) {
				ms.add(net.getLogEvents().getEvent(net.getDuplicatesMapping()[i]));
			}
		}
