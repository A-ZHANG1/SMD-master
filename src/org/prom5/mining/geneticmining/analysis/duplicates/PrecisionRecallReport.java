...
import org.prom5.importing.heuristicsnet.HeuristicsNetFromFile;
import org.prom5.mining.geneticmining.analysis.Report;

/**
 * <p>Title: Precision and Recall Report</p>
 *
 * <p>Description: This class generates statistic information about mined models in batch.</p>
 *
 *
...
public class PrecisionRecallReport implements Report {
	...
	public PrecisionRecallReport(String dirSolutionModels, String fileTerminationSolutionModels,
			String dirFoundModels, String fileTerminationFoundModels, String dirLogs,
			String fileTerminationLogs, String processInstance,
			String reportFileName,
			String fieldsSeparator) throws Exception {

		//retrieving the desired solutions
		desiredSolutions = new RetrieveFiles(dirSolutionModels, fileTerminationSolutionModels, true);

		//retrieving the logs for the desired solutions
		logs = extractZipEntry(new RetrieveFilesPerElement(desiredSolutions.getNames(), dirLogs,
			   fileTerminationLogs,true).getFiles());
		this.processInstance = processInstance;

		//retrieving the found solutions for the desired solutions
		foundSolutions = new RetrieveFilesPerElement(desiredSolutions.getNames(), dirFoundModels,
						 fileTerminationFoundModels,true).getFiles();
	...
	public static String[][] extractZipEntry(File[][] files) {
		...
		for (int i = 0; i < files.length; i++) {
			...
			for (int j = 0; j < files[i].length; j++) {
				try {
					refinedFiles[i][j] = MethodsForFiles.extractFiles(
										 files[i][j].getAbsolutePath())[0];
	...
	public static LogReader createLogReader(String logFile, String processInstance) {
		DefaultLogFilter logFilter = new DefaultLogFilter(DefaultLogFilter.INCLUDE);
		logFilter.setProcess(processInstance);
		try {
			return LogReaderFactory.createInstance(logFilter, LogFile.getInstance(logFile));
		} catch(Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	private void calculatePrecisionRecall() throws Exception {
		...
		//Allocating enough space to store the precision and recall of every found solution,
		...
		for (int i = 0; i < foundSolutions.length; i++) {
			...
			improvedContinuousSemanticsFitnessValues[i] = new FitnessReport[foundSolutions[i].
					length];
		}

		//calculating the structural precision and recall for every found solution,
		...
		for (int i = 0; i < foundSolutions.length; i++) {
			...
			if (logs[i].length > 0) {
				...
				for (int j = 0; j < foundSolutionForHnSolution.length; j++) {
					...
					//	System.out.println("Processing net = " + desiredSolutions.getNames()[i]);
					...
					try{
						...
						properCompletionFitnessValues[i][j] = new FitnessReport(logReader,
								hnFoundSolution, 0);
						...
						System.err.println("Could not calculate metrics for net '" +foundSolutionForHnSolution[j].toString() + "'. " + exception.getMessage());
					}
				}
			} else {
				System.err.println("No log for net = " + desiredSolutions.getNames()[i]);
			}
	...
	private static PrecisionRecall calculateBehavioralPrecisionRecall(LogReader logReader,
			HeuristicsNet baseHN,
			HeuristicsNet foundHN) throws Exception {

		return PrecisionRecallFactory.getPrecisionRecall(PrecisionRecallFactory.IND_BEHAVIORAL_PR,
				logReader, baseHN,
				foundHN);
	}

	/*
	 Calculates structural precision and recall based on two nets: the base solution
	 and the found solution.
	 */
	private static PrecisionRecall calculateStructuralPrecisionRecall(HeuristicsNet baseHN,
			HeuristicsNet foundHN) throws Exception {

		return PrecisionRecallFactory.getPrecisionRecall(PrecisionRecallFactory.IND_STRUCTURAL_PR, null,
				baseHN,
				foundHN);
	}

	/*
	 Calculates duplicates precision and recall based on two nets: the base solution
	 and the found solution.
	 */
	private static PrecisionRecall calculateDuplicatesPrecisionRecall(HeuristicsNet baseHN,
			HeuristicsNet foundHN) throws Exception {

		return PrecisionRecallFactory.getPrecisionRecall(PrecisionRecallFactory.IND_DUPLICATES_PR, null,
				baseHN,
				foundHN);
	}
	...
	public void generateReport() {
		try {
			...
			BufferedWriter report = new BufferedWriter(new FileWriter(this.reportFileName));
			BufferedWriter reportAllValues = new BufferedWriter(new FileWriter(this.reportFileName +
											 "_allPrecisionRecallValues.txt"));
			...
			//calculating the average and best precision and recall for every found solution
			for (int i = 0; i < foundSolutions.length; i++) {
				...
				for (int j = 0; j < foundSolutions[i].length; j++) {
					...
					if ((structuralPrecisionRecall[i][j] != null) &&
						...
						averageStructPrecision += structuralPrecisionRecall[i][j].getPrecision();
						msStructPrecision.add(structuralPrecisionRecall[i][j].
								getPrecision());

						averageStructRecall += structuralPrecisionRecall[i][j].getRecall();
						msStructRecall.add(structuralPrecisionRecall[i][j].getRecall());

						//duplicates precision
						averageDuplicatesPrecision += duplicatesPrecisionRecall[i][j].getPrecision();
						msDuplicatesPrecision.add(duplicatesPrecisionRecall[i][j].
								getPrecision());

						averageDuplicatesRecall += duplicatesPrecisionRecall[i][j].getRecall();
						msDuplicatesRecall.add(duplicatesPrecisionRecall[i][j].getRecall());

						//behavioral precision
						averageBehavPrecision += behavioralPrecisionRecall[i][j].getPrecision();
						msBehavPrecision.add(behavioralPrecisionRecall[i][j].
								getPrecision());

						averageBehavRecall += behavioralPrecisionRecall[i][j].getRecall();
						msBehavRecall.add(behavioralPrecisionRecall[i][j].getRecall());

						msProperCompletion.add(properCompletionFitnessValues[i][j].getFitness());
						msImprovedContinuousSemantics.add(improvedContinuousSemanticsFitnessValues[
								i][j].getFitness());

						averageProperCompletion += properCompletionFitnessValues[i][j].getFitness();

						if (properCompletionFitnessValues[i][j].getFitness() == 1.0 &&
								behavioralPrecisionRecall[i][j].getPrecision() == 1.0) {
							numberProperCompleteAndPreciseModels++;
							numberProperCompleteModels++;
						} else if (properCompletionFitnessValues[i][j].getFitness() == 1.0) {
							numberProperCompleteModels++;
						}
					}
				}
...
//writing entry in the report
				...
				report.write(
						...
						+ this.fieldsSeparator +
						//"% of Complete Models"
						numberProperCompleteModels / msStructPrecision.size()
						+ this.fieldsSeparator +
						//"% of Complete Models that are Precise"
						numberProperCompleteAndPreciseModels / numberProperCompleteModels
						+ this.fieldsSeparator +
						//"Average Structural Precision"
				...
				writeLineToReportAllValues(reportAllValues, desiredSolutions.getNames()[i] +
						" - All Values of Structural Precision", msStructPrecision);
				writeLineToReportAllValues(reportAllValues, desiredSolutions.getNames()[i] +
						" - All Values of Structural Recall", msStructRecall);

				//behavioral precisition/recall
				writeLineToReportAllValues(reportAllValues, desiredSolutions.getNames()[i] +
						" - All Values of Behavioral Precision", msBehavPrecision);
				writeLineToReportAllValues(reportAllValues, desiredSolutions.getNames()[i] +
						" - All Values of Behavioral Recall", msBehavRecall);

				//duplicates precisition/recall
				writeLineToReportAllValues(reportAllValues, desiredSolutions.getNames()[i] +
						" - All Values of Duplicates Precision", msDuplicatesPrecision);
				writeLineToReportAllValues(reportAllValues, desiredSolutions.getNames()[i] +
						" - All Values of Duplicates Recall", msDuplicatesRecall);

				//fitness
				writeLineToReportAllValues(reportAllValues, desiredSolutions.getNames()[i] +
						" - All Values of Proper Completion", msProperCompletion);
				writeLineToReportAllValues(reportAllValues, desiredSolutions.getNames()[i] +
			...
			System.err.println("Problems while creating the file: " + this.reportFileName);
		}
	}

	private void writeLineToReportAllValues(BufferedWriter reportAllValues, String netDescription,
			File[] individuals) throws IOException {
	...
	private void writeLineToReportAllValues(BufferedWriter reportAllValues, String netDescription,
			MyDoubleMultiset ms) throws IOException {
	...
	private String buildLogLineForMyMultiset(MyDoubleMultiset ms) {
		StringBuffer line = new StringBuffer();
		line.append(ms.toString().replace('[', ' ').replace(']', ' ').trim().replaceAll(",",
				fieldsSeparator));
		return line.toString();
	...
	public static void main(String args[]) {
		...
		try {
			...
			PrecisionRecallReport prr = new PrecisionRecallReport(
										p.getParameter("DIR_SOLUTION_MODELS"),
										p.getParameter("FILE_TERMINATION_SOLUTION_MODELS"),
										p.getParameter("DIR_FOUND_MODELS"),
										p.getParameter("FILE_TERMINATION_FOUND_MODELS"),
										p.getParameter("DIR_LOGS"),
...
/**
 *
 * Implements a multiset of <code>double</code> that is backed by <code>LinkedList</code>.
 * The advantage is that the order of insertion in the multiset is preserved.
