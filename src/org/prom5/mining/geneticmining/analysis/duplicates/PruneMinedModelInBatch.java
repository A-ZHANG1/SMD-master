...
import org.prom5.analysis.genetic.PruneNoisyArcs;
...
/**
 * <p>Title: Prune Mined Models in Batch </p>
 *
 * <p>Description: This class can prune arcs from  mined models that are stored in a certain directory of the computer.
 * It works pretty much like the "Prune Arcs Analysis Plug-in" of the ProM tool. The main difference
 * is that the whole pruning process is executed in batch.</p>
 *
...
public class PruneMinedModelInBatch {
	...
	public PruneMinedModelInBatch(String dirSolutionModels, String fileTerminationSolutionModels,
			...
			String outputDirPrunedModels) throws FileNotFoundException, IOException {
		...
		logs = PrecisionRecallReport.extractZipEntry(new RetrieveFilesPerElement(desiredSolutions.
			   getNames(), dirLogs,
			   fileTerminationLogs, true).getFiles());

		//retrieving the found solutions for the desired solutions
		foundSolutionsFiles = new RetrieveFilesPerElement(desiredSolutions.getNames(),
							  dirFoundModels,
	...
	private void pruneModels() throws IOException {
		...
		for (int i = 0; i < foundSolutionsFiles.length; i++) {
			if (logs[i].length > 0) {
				...
				LogReader logReader = logReader = PrecisionRecallReport.createLogReader(logs[i][0],
												  processInstance);
				...
				for (int j = 0; j < foundSolutionForHnSolution.length; j++) {
					HeuristicsNet hnFoundSolution = PrecisionRecallReport.createNet(
					...
					//pruning the arcs
					parsedSolutions[i][j].disconnectArcsUsedBelowThreshold(
							PruneNoisyArcs.calculatePruningThreshold(parsedSolutions[i][j],
	...
	private void writePrunedModels() throws IOException {
		...
		for (int i = 0; i < desiredSolutions.getNames().length; i++) {
			...
			for (int j = 0; j < parsedSolutions[i].length; j++) {
				try {
					parsedSolutions[i][j].toFile(new FileOutputStream(subDirToPrunedModels.
							getAbsolutePath() + File.separator + "ind_" + j +
							fileTerminationFoundModels));

				} catch (IOException ioe) {
					System.err.println(
							"Warning >>> Could not log the pruned individual for  the run " +
							foundSolutionsFiles[i][j].getAbsolutePath());
				}
			}
	...
	try {
		...
		PruneMinedModelInBatch prr = new PruneMinedModelInBatch(
