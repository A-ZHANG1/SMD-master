...
import org.prom5.framework.models.heuristics.MarkingHeuristicsNet;
...
/**
 * <p>This class calculates the precision and recall of two <code>HeuristicsNet</code>
 * with respect to the parsing of the traces.</p>
 *
...
//	private boolean useContinuousSemantics;
	 ...
	 * @param log LogReader log to use during the calculation of the precision and recall
	 * @param baseHN HeuristicsNet base heuristics net. The precision and recall consider this net
	 * as the correct solution.
	 * @param foundHN HeuristicsNet found heuristis net. The precision and recall compare this
	 * net to the base heuristics net.
	 */
//	 * @param useContinuousSemantics boolean indicates if the continuous semantics parser
//	 * should be used while computing the precision and recall. When this value is false, the
//	 * stop semantics parser is used.
	...
	public TraceParsing(LogReader log, HeuristicsNet baseHN,
		...
		if (baseHN != null) {
			this.baseHN = baseHN;
			//making the baseHN and the log work on the same indeces for the tasks/elements...
			try {
		...
		if (foundHN != null) {
			...
			//making the foundHN and the log work on the same indeces for the tasks/elements...
			try {
...
//		this.useContinuousSemantics = useContinuousSemantics;
	...
	private void calculatePrecisionAndRecall() {
		...
		while (logReaderInstanceIterator.hasNext()) {
			...
			ProcessInstance pi = (ProcessInstance) logReaderInstanceIterator.next();
			...
			for (int indexPi = 0; indexPi < pi.getAuditTrailEntryList().size(); indexPi++) {
				...
				if(baseMS.size()>0){
					partialRecall += ((double)intersection.size() / baseMS.size());
				} //else, division by zero, no need to increment the partialRecall

				if(foundMS.size()>0){
					partialPrecision += ((double)intersection.size() / foundMS.size());
				} //else, division by zero, no need to increment the partialPrecision

				//firing the element at position "indexPi"
				ate = (AuditTrailEntry) atesIterator.next();
				element = baseHN.getLogEvents().findLogEventNumber(ate.getElement(), ate.getType());
				try {
					baseHNmarking.fire(element, pi, indexPi);
				} catch (ArrayIndexOutOfBoundsException exc) {
					//The individual does not contain the element... just proceed.
				}
			...
			//divinding by the size of the trace times the frequency of the trace
			double piFrequency = MethodsForWorkflowLogDataStructures.
								 getNumberSimilarProcessInstances(pi);
			partialRecall = ((piFrequency * partialRecall) / pi.getAuditTrailEntryList().size());
			partialPrecision = ((piFrequency * partialPrecision) / pi.getAuditTrailEntryList().size());
	...
	private static Multiset toSet(HNSubSet subset, int[] duplicatesMapping, LogEvents le) {
		...
		for (int i = 0; i < subset.size(); i++) {
			String element = null;
			try{
				element = le.getEvent(duplicatesMapping[subset.get(i)]).getModelElementName() +
								 " " + le.getEvent(duplicatesMapping[subset.get(i)]).getEventType();
			}catch(ArrayIndexOutOfBoundsException exc){
				//the task is in the net, but not in the event log
				//this may happen when the net is connected to a log that has fewer events than the net
				element = subset.toString()+" "+i; //we just use some string to count as an enabled event!
			}
	...
	public static void main(String[] args) throws Exception {
		...
		LogReader logReader = LogReaderFactory.createInstance(logFilter,
							  LogFile.getInstance(
							  MethodsForFiles.extractFiles(
							  "C:\\AKThesisExperiments\\GenAlgNoiseFree\\setup\\logs\\bn1\\groupedFollowsbn1.zip")[
							  0]));

		HeuristicsNet baseHN = new HeuristicsNetFromFile(new FileInputStream(new File(
							   "C:\\AKThesisExperiments\\GenAlgNoiseFree\\setup\\desiredNets\\bn1.hn"))).
							   getNet();

		HeuristicsNet foundHN = new HeuristicsNetFromFile(new FileInputStream(new File("C:\\AKThesisExperiments\\GenAlgNoiseFree\\Backup_resultsHeuristicsAndGO\\bn1\\individuals\\bn1_groupedFollowsbn1.zip#groupedFollowsbn1.xml_seed5718490662849960997.txt\\gen_999\\ind_0.hn"))).
								getNet();
