...
public class DTGeneticMinerResult extends JPanel implements MiningResult, Provider {
	...
	public ProvidedObject[] getProvidedObjects() {
		ProvidedObject[] objects = new ProvidedObject[table.getSelectedRowCount()];
		if (table.getSelectedRowCount() > 0) {

			for (int i = 0; i < table.getSelectedRows().length; i++) {
				int j = table.getSelectedRows()[i];
				objects[i] = new ProvidedObject("Individual " + j + ", fitness " +
	...
	private void updateGraph() {
		...
		result = new HeuristicsNetResult(population[table.getSelectedRow()], log,
