...
package org.prom5.mining.geneticmining.fitness.duplicates;
...
import org.prom5.framework.models.heuristics.ExtraBehaviorParser;
...
/**
 * <p>Title: </p>
 * <p>Description: This class uses the continuous parsing plus (1) a punishment based
 * on the amount of enabled tasks and (2) a punishment for duplicate tasks that have common elements
 * in their input/ouput sets.</p>
 * <p> NOTE: We assume that the nets do not have interconnecting duplicate tasks!!
...
//	}
	 ...
	 * NOTE: If the array parameters does not contain all the parameter values, the missing
	...
	private void calculateStructuralPunishment() {
		...
		for (int i = 0; i < population.length; i++) {
			numIntersectingDuplicatesPerInputElement[i] = checkIntersectingElements(population[i].
					getReverseDuplicatesMapping(), population[i].getDuplicatesMapping(),
			...
			if (maximumNumberIntersectingElements <
	...
	private double checkIntersectingElements(HNSubSet[] duplicates, int[] duplicatesLabel,
			HNSet[] sets) {
		...
		for (int i = 0; i < duplicates.length; i++) {
			...
			if (duplicates[i].size() > 1) { //the element has duplicates...
				for (int k = 0; k < duplicates[i].size(); k++) {
					int[] union = mapToDuplicateLabel(duplicatesLabel,
								  HNSet.getUnionSet(sets[duplicates[i].get(k)]));
	...
	private void calculatePartialFitness() {
		...
		while (logReaderInstanceIterator.hasNext()) {
			...
			for (int i = 0; i < population.length; i++) {
				if (mapping.getMap(i) < 0) { //we need to compute the partial fitness
					parser[i].parse(pi);
	 ...
	 * where:
	 * f1 is based on  parsed tasks, missing tokens and tokens left behind.
	 * f2 is based on the number of enabled tasks during the parsing.
	 * f3 is based on the number of duplicates with intersecting I/O elements.
	 */
...
//				fitness = k*f1 + (k - (k*f2)) + (k - (k*f3));
				...
				population[i].setDuplicatesActualFiring(population[indexIdenticalIndividual].
