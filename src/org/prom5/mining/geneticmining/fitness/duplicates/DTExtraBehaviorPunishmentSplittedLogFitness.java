...
/**
 * <p>Title: </p>
 * <p>Description: This class uses the continuous parsing plus a punishment based
 * on the amount of enabled tasks.</p>
...
//		System.out.println("pis = " + actuallyConsideredPIs + " and wmes = " + actuallyConsideredWMEs);
			...
			if (generator.nextDouble() < 0.25) {
				...
				for (int i = 0; i < population.length; i++) {
					if (mapping.getMap(i) < 0) { //we need to compute the partial fitness
						...
						numEnabledWMEs[i] += (parser[i].getNumTotalEnabledElements() *
								numSimilarPIs);
					}
				}
			}
		...
		if (actuallyConsideredPIs != 0 && actuallyConsideredWMEs != 0) {
			...
			for (int i = 0; i < population.length; i++) {
				...
				if (indexIdenticalIndividual < 0) {
					...
					missingTokensDenominator = actuallyConsideredPIs - numPIsWithMissingTokens[i] +
											   1;

					unusedTokensDenominator = actuallyConsideredPIs -
											  numPIsWithExtraTokensLeftBehind[i] +
											  1;
					...
					population[i].setFitness((population[i].getFitness() + fitness) / 2);
