...
import org.prom5.framework.models.heuristics.StopSemanticsParser;
	...
	private HeuristicsNet[] assignFitness() {
		...
		for (int i = 0; i < population.length; i++) {
			...
			if (indexIdenticalIndividual < 0) {
				...
				parsed = (numParsedWMEs[i] / logReader.getLogSummary().getNumberOfAuditTrailEntries());
				completed = (numCompletedPIs[i] /
							logReader.getLogSummary().getNumberOfProcessInstances());
				properlyCompleted = (numProperlyCompletedPIs[i] /
									logReader.getLogSummary().getNumberOfProcessInstances());
