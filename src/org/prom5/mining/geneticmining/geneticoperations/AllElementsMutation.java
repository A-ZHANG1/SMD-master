...
public class AllElementsMutation implements Mutation {
	 ...
	 * <ol>
	 * <li> Randomly choose a double number r. If r <i>less than</i> mutation rate, do:
	 * <ol>
	 * <li> Randomly choose if add or remove and entry from the INPUT of this workflow model element.</li>
	 * <li> Randomly choose if add or remove and entry from the OUTPUT of this workflow model element.</li>
			...
			if (mutate < mutationRate) {
				...
				if (ind.getInputSet(i).size() > 0) {
					...
					if (modifyInputMatrix) {
						...
						if (addEntry) {
							MethodsOverIndividuals.randomlyAddEntry(inputMatrix, generator);
						} else {
							MethodsOverIndividuals.randomlyRemoveEntry(inputMatrix, generator);
						}
				...
				if (ind.getOutputSet(i).size() > 0) {
					...
					if (modifyOutputMatrix) {
						addEntry = generator.nextBoolean();
						if (addEntry) {
							MethodsOverIndividuals.randomlyAddEntry(outputMatrix, generator);
						} else {
							MethodsOverIndividuals.randomlyRemoveEntry(outputMatrix, generator);
						}
					}
				}
				...
				//rebuild set
				if (ind.getInputSet(i).size() > 0) {
					inputHashSet = MethodsOverIndividuals.buildSet(0, inputMatrix, generator);
				} else {
					inputHashSet = ind.getInputSet(i);
				}

				if (ind.getOutputSet(i).size() > 0) {
					outputHashSet = MethodsOverIndividuals.buildSet(0, outputMatrix, generator);
				} else {
