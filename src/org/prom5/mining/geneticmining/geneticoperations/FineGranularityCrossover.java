...
package org.prom5.mining.geneticmining.geneticoperations;
...
/**
 * <p>Title: Fine Granularity Crossover</p>
 * <p>Description: This class does crossover over the subsets of the input/output sets.</p>
...
public class FineGranularityCrossover implements Crossover {
	 ...
	 * - If the subsets have common elements, randomly choose if these subsets are going
	 *   to be merged or if the common elements are going to be removed from the subset that
	 *   wasn't transferred during the crossover.
	...
	public HeuristicsNet[] doCrossover(HeuristicsNet[] population) {
		...
		MethodsOverIndividuals.updateRelatedElements(offspring1, pointToCrossover, generator);
		MethodsOverIndividuals.updateRelatedElements(offspring2, pointToCrossover, generator);
	...
	private HNSet addSubsets(HNSet set, HNSet setsToAdd) {
		...
		if (returnSet.size() == 0) {
			...
			//check if there are overlapping between the subsets in "setsToAdd" and
			//the subsets in "set"
			...
			for (int isetElements = 0; isetElements < returnSet.size(); isetElements++) {
				...
				for (int isetsToAddElements = 0; isetsToAddElements < setsToAdd.size();
											  isetsToAddElements++) {
					currentSubSetToAdd = setsToAdd.get(isetsToAddElements);
					//checking the elements in common
					elementsInCommon = new HNSubSet();
					for (int ielements = 0; ielements < currentSubSetToAdd.size(); ielements++) {
						element = currentSubSetToAdd.get(ielements);
						if (currentSubSet.contains(element)) {
							elementsInCommon.add(element);
						}
					}
					//in case there are elements in common, update subsets either
					//by merging them or by removing the elements in common from set
					if (elementsInCommon.size() > 0) {
						mergeElements = generator.nextBoolean();
						if (mergeElements) {
							//the next two steps make sure we keep the partition restriction
							//when merging the sets
							currentSubSet.removeAll(elementsInCommon);
							currentSubSet.removeAll(findIntersectingElements(currentSubSet,
									setsToAdd));
