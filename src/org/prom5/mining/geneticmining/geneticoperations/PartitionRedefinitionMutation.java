...
public class PartitionRedefinitionMutation implements Mutation {
	...
	/**
	 * This method changes the AND/OR-split/join at the individual. It works as follows: <br/>
	 * <ol>
	 * <li> For every workflow model element in the net (individual) do:
	 * <ol>
	 * <li> Randomly choose a double number r. If r <i>less than</i> mutation rate, do the following for the INPUT and for
	 * the OUTPUT set:
	 * <ol>
	 ...
	 * <li> Randomly distribute the elements at the union set U among the partition in S.</li>
	 * </li>
	 * </ol>
	 * </li>
	 * </ol>
	...
	public HeuristicsNet doMutation(HeuristicsNet ind) {
		...
		for (int i = 0; i < ind.size(); i++) {
			...
			if (mutate < mutationRate) {
				//input set...
				inputHashSet = mutateSet(ind.getInputSet(i), ind.getAllElementsInputSet(i),
							   ind.size());
				//output set...
				outputHashSet = mutateSet(ind.getOutputSet(i), ind.getAllElementsOutputSet(i),
								ind.size());
	...
	private HNSet mutateSet(HNSet set, HNSubSet unionSet, int numIndividualWMElements) {
