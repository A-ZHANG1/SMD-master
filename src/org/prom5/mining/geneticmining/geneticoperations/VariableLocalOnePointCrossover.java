...
public class VariableLocalOnePointCrossover implements Crossover {
	 ...
	 * always swaps: <br/>
	 * - both INPUT and OUTPUT sets of a workflow model element, if the randomly selected
	 * double x is between 0 and 0.5 (i.e. 0 <= x < 0.5) <br/>
	 * - INPUT set only, if 0.5 <= x < 0.75
	 * - OUTPUT set only, if 0.75 <= x < 1.0
	 *
	...
	public HeuristicsNet[] doCrossover(HeuristicsNet[] population) {
		...
		if (setsToCrossover < 0.75) {
			...
			offspring1.setInputSet(pointToCrossover, offspring2.getInputSet(pointToCrossover));
		...
		if (setsToCrossover < 0.5 || setsToCrossover >= 0.75) {
			...
			offspring1.setOutputSet(pointToCrossover, offspring2.getOutputSet(pointToCrossover));
	...
	public static void main(String[] args) {
		//VariableLocalOnePointCrossover variableLocalOnePointCrossover1 = new VariableLocalOnePointCrossover();
	}
