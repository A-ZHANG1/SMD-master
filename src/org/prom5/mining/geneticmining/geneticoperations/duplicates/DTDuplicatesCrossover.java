...
package org.prom5.mining.geneticmining.geneticoperations.duplicates;
...
public class DTDuplicatesCrossover implements Crossover {
	...
	/**
	 * Do crossover over the input and output sets of <b>two</b> individuals in the population.
	 * The crossover point is a task label. So, if two or more tasks have the same label, one of them
	 * is randomly selected at one individual and another task is selected at the other individual.
	 * This crossover works as follows: <br/>
	 * (This procedure is done for both input and output set) <br/>
	 * - Select a random task label l to be the crossover point in both individuals. <br/>
	 * - Select a random point at the input set of t at individual x. t has label l. <br/>
	 * - Select a random point at the input set of t' at individual x'. t' has label l.<br/>
	 * - Do crossover for the input sets by performing one of the following operation for every
	 * subset sp to swap:<br/>
	 ...
	 * elements that are also in sp, and
	 * add sp to the individual.<br/>
	 * - Check for dangling arcs and correct them.
	 * @param population has the two individuals to crossover
	 * @return array with the two offsprings that the crossover produced. If population
	 * size is less than two, no crossover is done and the population itself is returned.
	 */
	public HeuristicsNet[] doCrossover(HeuristicsNet[] population) {
		...
		//here a choose one of the duplicates to individual 1 and to individual 2
		possibleDuplicates = offspring1.getReverseDuplicatesMapping()[generator.nextInt(offspring1.
							 getReverseDuplicatesMapping().length)];
		pointToCrossover1 = possibleDuplicates.get(generator.nextInt(possibleDuplicates.size()));
		pointToCrossover2 = possibleDuplicates.get(generator.nextInt(possibleDuplicates.size()));
		...
		MethodsOverIndividuals.updateRelatedElements(offspring1, pointToCrossover1, generator);
		MethodsOverIndividuals.updateRelatedElements(offspring2, pointToCrossover2, generator);
	...
	private HNSet[] enhancedCrossover(HNSet[] sets) { // set1, HashSet set2) {
		...
		if (set1.size() == 0) {
			...
			subsetsToSwap1 = extractSubsets(set1, generator.nextInt(set1.size()));
		}

		if (set2.size() == 0) {
			subsetsToSwap2 = new HNSet();
		} else {
			subsetsToSwap2 = extractSubsets(set2, generator.nextInt(set2.size()));
		}
	...
	private HNSet crossoverSubsets(HNSet set, HNSet setsToAdd) {
		...
		if (returnSet.size() == 0) {
			...
			for (int iSetsToAdd = 0; iSetsToAdd < setsToAdd.size(); iSetsToAdd++) {
				...
				} else if (randomNumber < 2.0 / 3.0) { // 1/3 <= random number < 2/3
					returnSet = joinWithOneSet(returnSet, currentSubsetToAdd);
				} else { // 2/3 <= random number < 1.0
					returnSet = addByRemovingIntersectionFromOneSet(returnSet, currentSubsetToAdd);
				}
			}
		}
	...
	private HNSet addByRemovingIntersectionFromOneSet(HNSet returnSet, HNSubSet subsetToAdd) {
