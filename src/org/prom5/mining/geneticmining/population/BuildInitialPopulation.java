...
package org.prom5.mining.geneticmining.population;
...
/**
 * <p>Title: Build Initial Population</p>
 * <p>Description: Builds an initial population. Every individual is based on
 * causal, start and end matrices that are randomly created whenever an
 * individual is going to be built.<br/>
 * <b>Note:<b/> The created individuals do not have a fitness value.</p>
...
public class BuildInitialPopulation implements BuildPopulation {
	 ...
	 * @param oldPopulation DuplicateTasksHeuristicsNet[] population to be filled in.
	 * @return DuplicateTasksHeuristicsNet[] population with new individuals. There are no duplicated individuals in this population.
	 */
	...
	public HeuristicsNet[] build(HeuristicsNet[] oldPopulation) {
		...
		for (int i = 0; i < size; i++) {
			...
			population[i] = new HeuristicsNet(logReader.getLogSummary().getLogEvents());
...
//                System.out.println("individual number " + i + " "+ j + " = " + population[i].getInputSet(j).toString());
//                System.out.println("individual number " + i + " "+ j + " = " + population[i].getOutputSet(j).toString());
...
//           System.out.println("individual number " + i + " start task  = " + population[i].getStartTask() + " and end task = " + population[i].getEndTask());
	...
	private HNSet buildInputSet(int index) {
		...
		return MethodsOverIndividuals.buildSet(geneticMiningMatrices.getStartMatrix().get(index),
				geneticMiningMatrices.getCausalMatrix().viewColumn(index), generator);
	}

	private HNSet buildOutputSet(int index) {
		return MethodsOverIndividuals.buildSet(geneticMiningMatrices.getEndMatrix().get(index),
