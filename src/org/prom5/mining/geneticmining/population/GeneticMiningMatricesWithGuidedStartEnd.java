...
public class GeneticMiningMatricesWithGuidedStartEnd implements GeneticMiningMatrices {
	...
	public GeneticMiningMatricesWithGuidedStartEnd(Random gen, LogReader logReader) {
		generator = gen;
		depRelBuilder = new DependencyRelationBuilder(logReader);
		buildMatrices(logReader.getLogSummary().getLogEvents().size());
	}

	public GeneticMiningMatricesWithGuidedStartEnd(Random gen, LogReader logReader, double power) {
		this.power = power;
		generator = gen;
		depRelBuilder = new DependencyRelationBuilder(logReader);
		buildMatrices(logReader.getLogSummary().getLogEvents().size());
