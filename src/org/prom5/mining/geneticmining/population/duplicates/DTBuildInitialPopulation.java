...
public class DTBuildInitialPopulation implements BuildPopulation {
	 ...
	 * @return EnhancedHeuristicsNet[] population with new individuals. There are no duplicated individuals in this population.
	 */
	...
	private HNSet buildInputSet(int index) {
		...
		return MethodsOverIndividuals.buildHNSet(geneticMiningMatrices.getStartMatrix().get(index),
				geneticMiningMatrices.getCausalMatrix().viewColumn(index), generator);
	}

	private HNSet buildOutputSet(int index) {
		return MethodsOverIndividuals.buildHNSet(geneticMiningMatrices.getEndMatrix().get(index),
