 ...
 *
 * <p>The arcs are set in the following way: Whenever a task <i>t</i> causally follows
 * a tasks <i>t'</i>, there is a 50% probability that the individual
 * has an arc from <i>t</i> to <i>t'</i>.</p>
 *
...
public class DTGeneticMiningMatricesWithHeuristics implements DTGeneticMiningMatrices {
			...
			for (int column = 0; column < this.reverseDuplicatesMapping.length; column++) {
				if (basicCausality.get(row, column) > 0) {
					if (generator.nextBoolean()) {
						...
						int duplicateTask_column = this.reverseDuplicatesMapping[column].get(
								generator.nextInt(this.
