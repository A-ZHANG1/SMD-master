...
import org.prom5.mining.geneticmining.duplicates.DTGeneticMinerConstants;
import org.prom5.mining.logabstraction.DependencyRelationBuilder;
...
/**
 * <p>This class uses the causal matrices to set the maximum number
 * of duplicated tasks that
 * an individual (HeuristicsNet) can have. Basically, the number of duplicates of a task <i>t</i>
 * is the minimum between the number of tasks that causally follow <i>t</i> and
 * the number of tasks that are causally followed by <i>t</i>.</p>
 *
 * <p>The arcs are set based on the dependency matrix and the power value. The more
 * often a task <i>t</i> is followed by a task <i>t'</i>, the higher the probability
 * that an arc from a duplicate of <i>t</i> to a duplicate of <i>t'</i> will be set.</p>
 *
...
public class DTGeneticMiningMatricesWithHeuristicsTasksArcs implements DTGeneticMiningMatrices {

	private double power = DTGeneticMinerConstants.POWER;

	//These matrices are derived from the log and do not consider duplicate tasks
	...
	public DTGeneticMiningMatricesWithHeuristicsTasksArcs(Random gen, LogReader logReader,
			double power) {
		this.power = power;
		generator = gen;
	...
	/**
	 * Returns the mapping from the task code in the individuals to the task code in the
	 * LogReader object that was provided to the constructor of this class.
	 */
	...
	/**
	 * Returns the mapping from the task code in the LogReader object that
	 * was provided to the constructor of this class to the task code in the individuals.
	 */
	...
	private void setDuplicatesMapping() {
		...
		//Heuristics: for every task, the #duplicates = min(#inputCausalRelations, #outputCausalRelations)
		...
		for (int i = 0; i < outCausalRelations.length; i++) {
			numDuplicatesPerTask[i] = Math.min(inCausalRelations[i], outCausalRelations[i]);
			if (numDuplicatesPerTask[i] == 0) {
				numDuplicatesPerTask[i] = 1; //for the start/end tasks, and duplicate
		...
		reverseDuplicatesMapping = HeuristicsNet.buildReverseDuplicatesMapping(duplicatesMapping);
	}

	private void buildBasicCausalityRelation() {

		basicCausality = new SparseDoubleMatrix2D(basicFollows.rows(), basicFollows.columns());
		...
		for (int row = 0; row < basicCausality.rows(); row++) {
			for (int column = 0; column < basicCausality.columns(); column++) {
				if (row != column) {
					...
					} else if (basicFollows.get(row, column) > 0 && basicFollows.get(column,
							row) <= 0) {
						basicCausality.set(row, column, 1);
					}
	...
	public DoubleMatrix2D rebuildCausalMatrix() {
		double random;
		causal = new SparseDoubleMatrix2D(duplicatesMapping.length, duplicatesMapping.length);
		...
		for (int row = 0; row < this.reverseDuplicatesMapping.length; row++) {
			for (int column = 0; column < this.reverseDuplicatesMapping.length; column++) {
				...
				if (random < Math.pow(depRelBuilder.getFollowsDependency(row, column), power)) {
					...
					//and add an arc
					int duplicateTask_row = this.reverseDuplicatesMapping[row].get(generator.
											nextInt(this.
											reverseDuplicatesMapping[row].size()));
					int duplicateTask_column = this.reverseDuplicatesMapping[column].get(generator.
											   nextInt(this.
											   reverseDuplicatesMapping[column].size()));
