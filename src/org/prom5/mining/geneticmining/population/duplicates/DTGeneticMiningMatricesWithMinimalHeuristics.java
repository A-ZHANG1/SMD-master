...
/**
 * <p>This class uses the follows matrices to set the maximum number
 * of duplicated tasks that
 * an individual (HeuristicsNet) can have. Basically, the number of duplicates of a task <i>t</i>
 * is the minimum between the number of tasks that directly follow <i>t</i> and
 * the number of tasks that are directly followed by <i>t</i>.</p>
 *
...
public class DTGeneticMiningMatricesWithMinimalHeuristics implements DTGeneticMiningMatrices {
	...
	private void setDuplicatesMapping() {
		...
		for (int i = 0; i < outFollowsRelations.length; i++) {
			if (basicStart.get(i) > 0 || basicEnd.get(i) > 0) {
				...
				numDuplicatesPerTask[i] = Math.min(inFollowsRelations[i], outFollowsRelations[i]);
			}
