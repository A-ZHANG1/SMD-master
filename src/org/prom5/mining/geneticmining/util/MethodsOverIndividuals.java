...
package org.prom5.mining.geneticmining.util;
...
public class MethodsOverIndividuals {
	 ...
	 * @param localPopulation DuplicateTasksHeuristicsNet[] population to be cleaned.
	 * @param fitness Fitness fitness to be used to recompute the individual's fitness after the cleaning.
	 * @return DuplicateTasksHeuristicsNet[] cleaned population with the updated fitness values.
	 */
	 ...
	 * @param ind  individual to update.
	 * @param element  has the INPUT/OUTPUT sets that are going to guide the update of
...
//		individual = updateArtificialStartEndSets(individual);
	...
	private static HeuristicsNet updateArtificialStartEndSets(HeuristicsNet
			individual) {

		int index = 0;
		HNSet singleton = null;

		//making sure that the artificial START tasks have a single output place
		for(int indexStartTask=0; indexStartTask<individual.getStartTasks().size();indexStartTask++){
		...
		for(int indexEndTask=0; indexEndTask<individual.getEndTasks().size();indexEndTask++){
	...
	private static HeuristicsNet addConnections(HeuristicsNet ind, int element,
		...
		for (int otherElement = 0; otherElement < individual.size(); otherElement++) {
			if (otherElement != element) {
				...
				allElementsInOtherElementSets = individual.getAllElementsOutputSet(otherElement);
				allElementsInElementSets = individual.getAllElementsInputSet(element);
				if (!allElementsInOtherElementSets.contains(element)
						&& allElementsInElementSets.contains(otherElement)) {

					individual.setOutputSet(otherElement, addConnectionsSet(
							individual.getOutputSet(otherElement), element, gen));

				}

				//checking the otherElement's INPUT versus element's OUTPUT
				allElementsInOtherElementSets = individual.getAllElementsInputSet(otherElement);
				allElementsInElementSets = individual.getAllElementsOutputSet(element);
				if (!allElementsInOtherElementSets.contains(element)
						&& allElementsInElementSets.contains(otherElement)) {

					individual.setInputSet(otherElement, addConnectionsSet(
							individual.getInputSet(otherElement), element, gen));
				}
	...
	private static HeuristicsNet removeConnections(HeuristicsNet ind,
		...
		//removing element from the INPUT/OUTPUT set of other elements that are not pointed
			...
			if (otherElement != element) {
				...
				if (allElementsInOtherElementSets.contains(element)
					...
					individual.setOutputSet(otherElement, removeConnectionsSet(
							individual.getOutputSet(otherElement),
							individual.getOutputSetsWithElement(otherElement, element),
							element));
				...
				if (allElementsInOtherElementSets.contains(element)
					...
					individual.setInputSet(otherElement, removeConnectionsSet(
							...
							individual.getInputSetsWithElement(otherElement, element),
							element));
	...
	private static HNSet removeConnectionsSet(HNSet setOtherElement,
		...
		for (int iSubsets = 0; iSubsets < subSetsOtherElementThatContainsElement.size(); iSubsets++) {
	...
	private static HNSet addConnectionsSet(HNSet setOtherElement, int element, Random gen) {
	...
	public static HeuristicsNet[] removeDuplicates(HeuristicsNet[]
			population) {
		...
		for (int i = 0; i < auxVector.size(); i++) {
			cleanedPopulation[i] = removeDanglingElementReferences((HeuristicsNet)
								   auxVector.get(i));
		}
	 ...
	 * @param individual to check and correct if necessary
		...
		for (int from = 0; from < individual.size(); from++) {
			...
			for (int iSet = 0; iSet < set.size(); iSet++) {
				...
				if (!individual.getAllElementsInputSet(to).contains(from)) {
					...
					individual.setOutputSet(from,
							HNSet.removeElementFromSubsets(individual.getOutputSet(from), to));
				}
			}
		}
		...
		for (int from = 0; from < individual.size(); from++) {
			...
			for (int iSet = 0; iSet < set.size(); iSet++) {
				...
				if (!individual.getAllElementsOutputSet(to).contains(from)) {
					//remove from "iFrom" outpuset subsets any reference to "iTo"
					individual.setInputSet(from,
							HNSet.removeElementFromSubsets(individual.getInputSet(from), to));
				}
			}
		}
	 ...
	 * The INPUT/OUTPUT sets are general because their subsets do not need to be
	 * a partition set of the set of tasks in the log.
	 * @param startOrEndEntry
	 * @param inOrOutRowOrColumn
	 * @param generator random generator.
	 * @return
	 */
	public static HNSet buildSet(double startOrEndEntry, DoubleMatrix1D inOrOutRowOrColumn,
			Random generator) {
		HNSet hs = null;
		...
		if (startOrEndEntry <= 0) {
			...
			for (int i = 0; i < inOrOutRowOrColumn.size(); i++) {
				if (inOrOutRowOrColumn.get(i) > 0) {
					numSubsetsToInsertElement = generator.nextInt(arraySubsets.length) + 1;
	 ...
	 * @return
	 */
	public static HNSet buildHNSet(double startOrEndEntry, DoubleMatrix1D inOrOutRowOrColumn,
			Random generator) {
		HNSet hs = null;
		HNSubSet[] arraySubsets = null;
	...
	public static DoubleMatrix1D randomlyAddEntry(DoubleMatrix1D matrix, Random generator) {
	...
	public static DoubleMatrix1D randomlyRemoveEntry(DoubleMatrix1D matrix, Random generator) {
	 ...
	 * @return a randomly selected Integer in <i>subsets</i>. If "subsets" is empty, this method
	 * returns null;
	 */

	public static Integer selectAnElementInSubset(TreeSet subset, Random generator) {
	 ...
	 * @return a randomly selected subset in <i>sets</i>. If "sets" is empty, this method
	 * returns null;
	 */
	...
	public static HashSet removeElementFromSubsets(HashSet set, Integer element) {
		...
		ts = new TreeSet();
		ts.add(element);
		//removing singletons (I had to do this way because the set.remove(ts) if ts.size()==1
	...
	/**
	 * This method assumes that the population is INCREASINGLY order by
	 * the fitness measure.
	 * @return all the individuals that have a fitness X, where X is the
	 * best fitness in this. This method returns null if the population is empty or null.
	 * populations.
	 */

	public static HeuristicsNet[] extractBestIndividuals(HeuristicsNet[]
			population) {
		...
		if (population != null && population.length > 0) {
			...
			while (iBestIndividuals.hasNext()) {
				bestIndividualsResult[index] = (HeuristicsNet) iBestIndividuals.next();
				index++;
			}
		}
