...
package org.prom5.mining.heuristicsmining;
...
import org.prom5.mining.geneticmining.fitness.duplicates.DTContinuousSemanticsFitness;
import org.prom5.mining.geneticmining.fitness.duplicates.DTImprovedContinuousSemanticsFitness;
import org.prom5.mining.heuristicsmining.models.DependencyHeuristicsNet;
...
public class HeuristicsMiner implements MiningPlugin {
	...
	public HeuristicsMinerParameters getParameters() {
		return parameters;
	}
	...
	public MiningResult mine(LogReader log) {
		// TODO: all Progress related lines have been commented to prevent a long lock
//		Progress progress;
		LogAbstraction logAbstraction;
		HeuristicsNet net; //because we have a single individual

		// show a small progress window
//		progress = new Progress("Mining " + log.getFile().getShortName() + " using " + getName());
//		progress.setMinMax(0, 5);
		...
		Message.add("Number of process instances (cases) = " +
				log.getLogSummary().getNumberOfProcessInstances(), Message.TEST);
		Message.add("Number of audit trail entries (events) = " +
				log.getLogSummary().getNumberOfAuditTrailEntries(), Message.TEST);
...
//			}
			...
			Message.add("Error while reading the log: " + ex.getMessage(), Message.ERROR);
			return null;
		}
...
//		progress.close();
	...
	private void showExtraInfo() {
		...
		if (parameters.useLongDistanceDependency) {
			...
			Message.add(showHeuristicsMiningMatrixI(events, longRangeSuccessionCount));
		}

		Message.add("All (also not accepted) L1L-dependency values:");
		Message.add(showHeuristicsMiningListR(events, L1LdependencyMeasuresAll));

		Message.add("All (also not accepted) L2L-dependency values:");
		Message.add(showHeuristicsMiningMatrixR(events, L2LdependencyMeasuresAll));

		Message.add("All (also not accepted) A>B-dependency values:");
		Message.add(showHeuristicsMiningMatrixR(events, ABdependencyMeasuresAll));
		...
		if (parameters.useLongDistanceDependency) {
			...
			Message.add("All (also not accepted) A>>>B-dependency values:");
			Message.add(showHeuristicsMiningMatrixR(events, longRangeDependencyMeasures));
		}

		Message.add("Accepted (L1L, L2L, A>B, A>>>B) dependency values:");
		Message.add(showHeuristicsMiningMatrixR(events, dependencyMeasuresAccepted));
		...
		Message.add(
				"\"Wrong\" observations (#B>A but A->B accepted) between accepted dependency relations:",
				Message.TEST);
		Message.add(showHeuristicsMiningMatrixI(events, noiseCounters), Message.TEST);
		...
		Message.add("Total \"wrong\" observations = " + noiseTotal, Message.TEST);

	}

	private double calculateDependencyMeasure(int i, int j) {
		return ((double) directSuccessionCount.get(i, j) - directSuccessionCount.get(j, i)) /
	...
	private double calculateLongDistanceDependencyMeasure(int i, int j) {
		return ((double) longRangeSuccessionCount.get(i, j) /
	...
	private String showHeuristicsMiningListI(LogEvents events, DoubleMatrix1D matrix) {
		DecimalFormat eventNum = new DecimalFormat("00");
	...
	private String showHeuristicsMiningListR(LogEvents events, DoubleMatrix1D matrix) {
		DecimalFormat eventNum = new DecimalFormat("00");
		...
		// Events name list
		for (int j = 0; j < matrix.size(); j++) {
			res = res + eventNum.format(j) + " " + realNum.format(matrix.get(j)) +
				  " (" + events.getEvent(j).getModelElementName() + ") \n";
		}
		return res;
	}

	private String showHeuristicsMiningMatrixI(LogEvents events, DoubleMatrix2D matrix) {
		DecimalFormat eventNum = new DecimalFormat("00");
		DecimalFormat intNum = new DecimalFormat("0000");
		...
		for (int i = 0; i < matrix.rows(); i++) {
			...
			if (L < NAMELENGTH) {
				dummy2 = events.getEvent(i).getModelElementName() + dummy1.substring(0,
						 NAMELENGTH - L);
			} else {
				dummy2 = events.getEvent(i).getModelElementName().substring(0, NAMELENGTH);
			}
	...
	private String showHeuristicsMiningMatrixR(LogEvents events, DoubleMatrix2D matrix) {
		DecimalFormat eventNum = new DecimalFormat("00");
		DecimalFormat realNum = new DecimalFormat("+0.000;-0.000");
	...
	private double andInMeasureF(int ownerE, int oldE, int newE) {
		if (ownerE == newE) {
			...
			return ((double) directSuccessionCount.get(oldE, newE) + directSuccessionCount.get(newE,
					oldE)) /
	...
	private double andInMeasureF2(int ownerE, int oldE, int newE) {
		if ((ownerE == newE) || (newE == oldE)) {
			...
			return ((double) (longRangeSuccessionCount.get(newE,
					ownerE) + longRangeSuccessionCount.get(oldE, ownerE)) /
					// relevantInObservations;
	...
	private double andOutMeasureF2(int ownerE, int oldE, int newE) {
		if ((ownerE == newE) || (newE == oldE)) {
			...
			return ((double) longRangeSuccessionCount.get(ownerE,
					newE) + longRangeSuccessionCount.get(ownerE, oldE)) /
					// relevantInObservations;
					((2 * events.getEvent(ownerE).getOccurrenceCount()) + 1);
	...
	private HeuristicsNet makeHeuristicsRelations(LogReader log) {
		...
		if (Math.abs(startCount.get(bestStart) -
			...
			Message.add("Use the 'Add an artificial BEGIN-task' option!", Message.TEST);
		}
		...
		if (Math.abs(endCount.get(bestEnd) - log.getLogSummary().getNumberOfProcessInstances()) >
			...
			Message.add("Use the 'Add an artificial END-task' option!", Message.TEST);
		}
		...
		Message.add("Total number of connections = " + numberOfConnections);
		Message.add("Total number of connections = " + numberOfConnections, Message.TEST);
