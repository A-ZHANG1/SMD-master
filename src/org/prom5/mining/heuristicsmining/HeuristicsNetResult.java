...
public class HeuristicsNetResult implements MiningResult, Provider {
	...
	private GrappaAdapter grappaAdapter = new GrappaAdapter() {
		 ...
		 */
		protected JMenuItem getCustomMenu(Subgraph subg, Element elem, GrappaPoint pt, int modifiers, GrappaPanel panel) {
			return checkMenu;
		}
	};

	public HeuristicsNetResult(HeuristicsNet net, LogReader log, boolean showSplitJoinSemantics) {
		this.net = net;
		this.log = log;
	...
	public ProvidedObject[] getProvidedObjects() {
		if (log == null) {
			return new ProvidedObject[] { new ProvidedObject("Heuristics net", new Object[] { net }) };

		} else {
			return new ProvidedObject[] { new ProvidedObject("Heuristics net", new Object[] { net, log }) };
		}
	}
