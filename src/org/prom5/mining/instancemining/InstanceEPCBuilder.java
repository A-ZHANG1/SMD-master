...
import org.prom5.framework.models.epcpack.EPCConfigurableObject;
...
public class InstanceEPCBuilder extends InstanceModelGraphBuilder {
	...
	private ModelGraph toEPC(ModelGraph graph) {
		...
		while (i.hasNext()) {
			...
			EPCFunction f = epc.addFunction(new EPCFunction(((LogEvent) v.object), epc));
			f.object2 = v.object2;
			mapping.put(v, f);
		}
		...
		while (i.hasNext()) {
			...
			while (successors.hasNext()) {
				...
				epc.addEdge((EPCFunction) mapping.get(v), (EPCFunction) mapping.get(v2));
			}
		}

		// First, for all sets of outgoing arcs, if the size of this set is more than one,
		// add a connector
		...
		// Second, for all sets of incoming arcs, if the size of this set is more than one,
		// add a connector
		...
		while (it.hasNext()) {
			...
			EPCEvent e = epc.addEvent(new EPCEvent("status changed to " + f.getIdentifier(), epc));
		...
		// If two connectors are of the same type (i.e. join/split) and they have
		// the same input (output respectively) they can be replaced by two other connectors.
		...
		while (it.hasNext()) {
			...
			while (it2.hasNext()) {
				...
				if (c1.getPredecessors().equals(c2.getPredecessors()) && (c1.inDegree() > 1)) {
					// OK, c1 and c2 have the same set of inputs.
					// Note that they both have exactly one output, and that these outputs are
					// not the same.
					EPCObject o1 = (EPCObject) c1.getSuccessors().iterator().next();
					EPCObject o2 = (EPCObject) c2.getSuccessors().iterator().next();
				...
				if (c1.getSuccessors().equals(c2.getSuccessors()) && (c1.outDegree() > 1)) {
					// OK, c1 and c2 have the same set of outputs.
					// Note that they both have exactly one input, and that these inputs are
					// not the same.
					EPCObject o1 = (EPCObject) c1.getPredecessors().iterator().next();
					EPCObject o2 = (EPCObject) c2.getPredecessors().iterator().next();
