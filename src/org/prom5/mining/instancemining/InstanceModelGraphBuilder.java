 ...
 * <p>
 * The net is returned as a <code>ModelGraph</code> object.
 * Use the class <code>InstancePetriNetBuilder</code>
 * to obtain the instance net as a <code>PetriNet</code> object,
 * or the class <code>InstanceEPCBuilder</code>
 * to obtain the instance net as an <code>EPC</code> object.
 *
...
public class InstanceModelGraphBuilder implements InstanceNetBuilder {
	...
	public InstanceModelGraphBuilder(LogRelations relations) {
		...
		//    exactly the ModelElements of which AuditTrailEntries are in the instance
	...
	public ModelGraph build(ProcessInstance instance) {
		...
		for (int k = 0; k < processInstance.size(); k++) {
			ModelGraphVertex v = findVertex((AuditTrailEntry) processInstance.get(k));
		...
		for (int k = 0; k < processInstance.size(); k++) {
			ModelGraphVertex v = findVertex((AuditTrailEntry) processInstance.get(k));
			ModelGraphVertex ev = (ModelGraphVertex) net.getVerticeList().get(k);

			// Find the first predecessor in the log that is a causal predecessor of ev
			// according to the main graph
			int n = k - 1;
			while (n >= 0) {
				ModelGraphVertex v2 = findVertex((AuditTrailEntry) processInstance.get(n));
				...
				if (v2.getSuccessors().contains(v)) {
					...
					ModelGraphVertex ev2 = (ModelGraphVertex) net.getVerticeList().get(n);
			...
			// Find the first successor in the log that is a causal successor of ev
			// according to the main graph
			n = k + 1;
