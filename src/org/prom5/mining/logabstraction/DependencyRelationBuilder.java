...
/**
 * <p>Title: Dependency Relation Builder</p>
 * <p>Description: This class has information about the dependency relation D between workflow model elements.</p>
...
public class DependencyRelationBuilder {
	...
	public DependencyRelationBuilder(LogReader log) {
		...
		try {
			...
			Message.add("Problem while reading the log: " + ex.getMessage(), Message.ERROR);
			return;
		}
	...
	public double getFollowsDependency(int fromElement, int toElement) {
		...
		if (fromElement != toElement) {
			if (L2L.get(fromElement, toElement) > 0) {
				numerator = L2L.get(fromElement, toElement) + L2L.get(toElement, fromElement);
				denominator = numerator;
			} else {
				numerator = follows.get(fromElement, toElement) - follows.get(toElement,
							fromElement);
				denominator = follows.get(fromElement, toElement) + follows.get(toElement,
							  fromElement);
			}
		} else {
