...
public class FSMLogRelationBuilder implements LogRelationBuilder {
	...
	public FSMLogRelationBuilder(LogRelations relations) {
		...
		causalRelations = (DoubleMatrix2D) relations.getCausalFollowerMatrix().clone();
		parallelRelations = (DoubleMatrix2D) relations.getParallelMatrix().clone();

		// Now walk trough the log, looking for a start event.
		InferCausality();

		relations = new LogRelationsImpl(causalRelations, parallelRelations, relations.getEndInfo(),
	...
	private boolean InferCausality(String eventType1, String eventType2, ArrayList exclude) {
		...
		for (int i = 0; i < modelElements.size(); i++) {
			...
			// If b then for this ModelElement, a type given in the exclude list is in the modelElements
			...
			while (j < modelElements.size() - 1) {
				...
				if (!modelElements.getEvent(j).getEventType().equals(eventType2)) {
					continue;
				}
				// We found a pair (i,j):
