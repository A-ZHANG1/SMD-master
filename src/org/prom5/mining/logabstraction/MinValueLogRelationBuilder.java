	...
	public MinValueLogRelationBuilder(LogAbstraction abstraction, int minValue, LogEvents events) {
		...
		for (int i = 0; i < s; i++) {
			for (int j = 0; j < s; j++) {
				...
				if ((directSuccession.get(i, j) > minValue) &&
						...
						(oneLoop.get(i) == 0) && (oneLoop.get(j) == 0)) {
					causalRelations.set(i, j, causalRelations.get(i, j) + 1);
				}
			}
		}
		...
		for (int i = 0; i < s; i++) {
			for (int j = 0; j < s; j++) {
				if ((directSuccession.get(i, j) > minValue) &&
						(directSuccession.get(j, i) > minValue) &&
						(causalRelations.get(i, j) == 0) && (causalRelations.get(j, i) == 0)) {
		...
		relations = new LogRelationsImpl(causalRelations, parallelRelations, finalModelElements,
					startModelElements, oneLoop, events);
	}
