...
package org.prom5.mining.logabstraction;
...
public class TimeIntervalLogRelationBuilder implements LogRelationBuilder {
	...
	public TimeIntervalLogRelationBuilder(LogRelations relations, LogReader log, String startEvent,
			String finalEvent) {
		...
		DoubleMatrix2D causalRelations = relations.getCausalFollowerMatrix().copy();
		...
		findParallellism(startEvent, finalEvent, causalRelations, parallelRelations);
		insertNewCausalities(startEvent, finalEvent, causalRelations, parallelRelations);

		this.relations = new LogRelationsImpl(causalRelationsMatrix,
						 parallelRelationsMatrix,
						 relations.getEndInfo(),
						 relations.getStartInfo(),
						 relations.getOneLengthLoopsInfo(),
		...
		while (log.hasNext()) {
			...
			while (ates.hasNext()) {
				...
				if (ate.getType().equals(startEvent)) {
					...
					// if b=true, then there are two overlapping intervals of the same
					// ModelElement (ate's with the same getElement). The intervals are now
					// undistinguishable, so skip.
					// In other words, if !b then this start event has an end event within
					...
					while (j < parallel.size()) {
						if (((AuditTrailEntry) parallel.get(j)).getElement().equals(ate.getElement())) {
	...
	private void setParallellism(ArrayList parallels, DoubleMatrix2D causalRelations,
			DoubleMatrix2D parallelRelations) {

		// Now parallels contains a list of lists. Each individual list contains
		// a number of AuditTrailEntry's.
		// Now, for the original ModelElements, set parallels for all event types of
		// the ate's per list.
				...
				for (int k = j + 1; k < parallel.size(); k++) {
					...
					for (int l = 0; l < modelElements.size(); l++) {
						for (int m = 0; m < modelElements.size(); m++) {
							if (modelElements.getEvent(l).getModelElementName().equals(ate_j.
									getElement()) &&
									modelElements.getEvent(m).getModelElementName().equals(ate_k.
									getElement())) {

								// Now set a parallel between modelElements[l] and modelElements[m]
	...
	private void insertNewCausalities(String startEvent, String finalEvent,
		...
		// the old situation is still stores in parallelRelationsMatrix and causalRelationsMatrix
		//
		// if (i,j) were not in parallel in the old situations and are in the new situation
		// and i->j in the old situation then :
		//     All causal predecessors k of i that in the new situation are not in parallel with j will
		//       become causal predecessors of j.
		//     All causal successors k of i that in the new situation are not in parallel with j will
		//       become causal successors of j.
		DoubleMatrix2D C = causalRelations.copy();
		for (int i = 0; i < nme; i++) {
			for (int j = 0; j < nme; j++) {
				if (!(modelElements.getEvent(i).getEventType().equals(modelElements.getEvent(j).
						getEventType()))) {
					continue;
				}
				if ((parallelRelationsMatrix.get(i, j) == 0) &&
					...
					for (int k = 0; k < nme; k++) {
						if ((causalRelations.get(k, i) == 1) &&
								...
								modelElements.getEvent(i).getEventType().equals(startEvent)) {
							C.set(k, j, 1);
						}
						if ((causalRelations.get(j, k) == 1) &&
								(parallelRelations.get(k, i) == 0) &&
								modelElements.getEvent(i).getEventType().equals(finalEvent)) {
							C.set(i, k, 1);
						}
					}
				}
