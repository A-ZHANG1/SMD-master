...
import org.prom5.mining.organizationmining.algorithm.AHCAlgorithm;
import org.prom5.mining.organizationmining.algorithm.ClusteringAlgorithm;
import org.prom5.mining.organizationmining.algorithm.ClusteringInput;
import org.prom5.mining.organizationmining.algorithm.SOMAlgorithm;
import org.prom5.mining.organizationmining.distance.DistanceMetric;
import org.prom5.mining.organizationmining.profile.ActivityProfile;
import org.prom5.mining.organizationmining.profile.AggregateProfile;
import org.prom5.mining.organizationmining.profile.Profile;
...
public class OrgMiner implements MiningPlugin {
	...
	public MiningResult mine(LogReader log) {
		...
		switch (ui.getSelectedMetrics()){
			...
			case OrgMinerOptions.SIMILAR_TASK:
				...
				snMatrix = null;
				indexType = OrgMinerOptions.SIMILAR_TASK + ui.getSimilarTaskSetting();
			...
			case OrgMinerOptions.WORKING_TOGETHER:
				...
				snMatrix = null;
				indexType = OrgMinerOptions.WORKING_TOGETHER + ui.getWorkingTogetherSetting();
				baseOprtation = OperationFactory.getOperation(indexType, summary, log);
				if (baseOprtation!=null){
	...
	public void doDefalutMining(LogReader log, LogSummary summary) {
		...
		for (int i = 0; i < modelElements.size(); i++) {
			OrgEntity tempEntity = new OrgEntity("minedRole" + i,
								   modelElements.get(i).getModelElementName() + "_" +
	...
	public void doAHCMining(LogReader log)
	{
		...
		DistanceMetric metric = ui.getDistanceMetrics();//new EuclideanDistance(); 
