...
import org.prom5.mining.organizationmining.distance.CorrelationCoefficientDistance;
import org.prom5.mining.organizationmining.distance.DistanceMetric;
import org.prom5.mining.organizationmining.distance.EuclideanDistance;
import org.prom5.mining.organizationmining.distance.HammingDistance;
import org.prom5.mining.organizationmining.distance.JaccardIndexDistance;
...
public class OrgMinerOptions extends JPanel {
	...
	private void init() {
		...
		doingSimilarTask = new GUIPropertyListEnumeration(" : Options for DST - ", null, values2, null, 260);
		...
		workingTogether = new GUIPropertyListEnumeration("   : Options for WT  - ", null, values3, null, 260);
	...
	public int getWorkingTogetherSetting() {
			...
			if (st.equals("Simultaneous appearance ratio"))return OrgMinerOptions.SIMULTANEOUS_APPEARANCE_RATIO;
			else if (st.equals("Consider distance with causality"))return OrgMinerOptions.DISTANCE_WITH_CAUSALITY;
			else return OrgMinerOptions.DISTANCE_WITHOUT_CAUSALITY;
	}
	...
	public int getSelectedMetrics() {
		int nIndex = 0;
		if(group.getSelection() == jrbDefaultMining.getModel()) nIndex = OrgMinerOptions.DEFAULT_MINING;
		else if (group.getSelection() == jrbHierarchicalMining.getModel()) nIndex = OrgMinerOptions.AHC_MINING;
		else if (group.getSelection() == jrbDoingSimilarTask.getModel()) nIndex = OrgMinerOptions.SIMILAR_TASK;
		else if (group.getSelection() == jrbSOMMining.getModel()) nIndex = OrgMinerOptions.SOM_MINING;
		else nIndex = OrgMinerOptions.WORKING_TOGETHER;
		return nIndex;
	}
