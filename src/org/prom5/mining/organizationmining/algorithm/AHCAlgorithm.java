package org.prom5.mining.organizationmining.algorithm;
...
import org.prom5.mining.organizationmining.distance.DistanceMatrix;
import org.prom5.mining.organizationmining.model.AHCTreeNode;
import org.prom5.mining.organizationmining.model.Cluster;
import org.prom5.mining.organizationmining.model.ClusterSet;
import org.prom5.mining.organizationmining.profile.AggregateProfile;
import org.prom5.mining.organizationmining.ui.hierarchicalui.DendroGramUI;

import att.grappa.Graph;
import att.grappa.GrappaPanel;

public class AHCAlgorithm extends ClusteringAlgorithm implements GuiNotificationTarget {
	...
	protected double upperBound, lowerBound, upperBoundUI, lowerBoundUI, cutPoint, upperMaxBound;
	...
	public void cluster() {
		...
		progress.setMinMax(0,
				input.getLog().numberOfInstances() - 1 + (input.getLog().numberOfInstances()) / 100);
	...
	protected void build()
	{
		...
		for (int i = 0; i < originatorSize; i++) {
			...
			AHCTreeNode tempCluster = new AHCTreeNode(input.getLog(),"OrgUnit " + clusterCounter++, temp);
			tempCluster.addOriginator(input.getLog().getLogSummary().getOriginators()[i]);
			listAHCTree.add(tempCluster);
			if(i%100==0) {
				progress.setProgress(step++);
				progress.setNote((i/100) + " of " + (originatorSize/100) +"steps (initialzing clusters) ...");
			}
		}
	...
	private void findNearestCluster_SingleLinkage() {
		...
		{   Cluster aTempNode = listAHCTree.get(i);
			for (int j = i + 1; j < listAHCTree.size(); j++) {
				...
				while(itr.hasNext())
				{
					int trace1 = agProfiles.getIndexOfOriginator((String) itr.next());
					Iterator itr2 = bTempNode.getOriginatorList().iterator();
					while(itr2.hasNext())
					{
						int trace2 = agProfiles.getIndexOfOriginator((String) itr2.next());
						if(tempMinDistance> distances.get(trace1,trace2)) tempMinDistance = distances.get(trace1,trace2);
					}
				}
		...
		AHCTreeNode aAHCTreeNode = new AHCTreeNode(input.getLog(), "OrgUnit " + clusterCounter++, (AHCTreeNode) aNode, (AHCTreeNode) bNode, minDistance);
	...
	private void findNearestCluster_CompleteLinkage() {
		...
		AHCTreeNode aAHCTreeNode = new AHCTreeNode(input.getLog(), "OrgUnit " + clusterCounter++,
								   (AHCTreeNode) aNode,(AHCTreeNode) bNode, minDistance);
		listAHCTree.remove(aNode);
		listAHCTree.remove(bNode);
		listAHCTree.add(aAHCTreeNode);
	}
	...
	private void findNearestCluster_AverageLinkage() {
		...
		for (int i = 0; i < listAHCTree.size()-1; i++) {
			...
			for (int j = i + 1; j < listAHCTree.size(); j++) {
				...
				tempDistanceSum = tempDistanceSum/(aTempNode.size()*bTempNode.size());
	...
	private void findNearestCluster_CentroidLinkage() {
		...
		if (minDistance>upperMaxBound) upperMaxBound=minDistance;
		AHCTreeNode aAHCTreeNode = new AHCTreeNode(input.getLog(), "OrgUnit " + clusterCounter++,
								  (AHCTreeNode) aNode,(AHCTreeNode) bNode, minDistance);
		listAHCTree.remove(aNode);
		listAHCTree.remove(bNode);
	...
	private void buildOrgModelDendrogram() {
		...
		{
			OrgEntity tempEntity = new OrgEntity("minedRole" + temp++, cluster.getName(),
		...
		for (int i = 0; i < modelElements.size(); i++) {
			Task tempTask = new Task("minedtask" + i, modelElements.get(i).getModelElementName(),
							modelElements.get(i).getEventType());
		...
		while (it.hasNext()) {
			...
			while (ates.hasNext()) {
				...
				LogEvent le = modelElements.findLogEvent(ate.getElement(), ate.getType());
				Resource res = orgModel.getResource(ate.getOriginator());
				if(ate.getOriginator()==null){
					res = orgModel.getResource("");
				}
				if(res==null) {
					if (ate.getOriginator() != null) res = new Resource(ate.getOriginator(),ate.getOriginator());
					else res = new Resource("","");
				}
				// add to org model
				for(String entityID :orgModel.getOrgEntityList(res)) {
					orgModel.getTask(le).addOrgEntity(orgModel.getOrgEntity(entityID));
	...
	private void buildOrgModelTreeView() {
		...
		for(Cluster cluster:clusters.getClusters())
		{
			...
			AHCTreeNode right = (AHCTreeNode)((AHCTreeNode)cluster).getRight();
			...
			for(String user:cluster.getOriginatorList()){
				Resource res = orgModel.getResource(user);
				if(!((isInRange(right.getDistance())&&right.getOriginatorList().contains(user))||
						(isInRange(left.getDistance())&&left.getOriginatorList().contains(user))))
	...
	private boolean isInRange(double dist)
	{
		...
		if(dist>lowerBoundUI&&dist>lowerBound&&dist<=upperBound&&dist<=upperBoundUI) bResult=true;
		return bResult;
	}
	...
	protected void setupGui() {
		...
		JLabel emptyLabel = new JLabel("click to start clustering on the right panel");
		...
		sliderPanel_Dendo.setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 5));
		sliderPanel_Dendo.setLayout(new BoxLayout(sliderPanel_Dendo, BoxLayout.X_AXIS));
		...
		sliderCutPointUIPanel.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
		...
		sliderCutPointUI.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				cutPoint = (double)sliderCutPointUI.getValue() / 10000.0 * (upperMaxBound + upperMaxBound*0.005);
		...
		diameterTextField5.addKeyListener(new KeyAdapter(){
			...
			public void keyReleased(KeyEvent e) {
				if(e.getKeyCode()==KeyEvent.VK_ENTER)
					sliderCutPointUI.setValue((int) (Double.valueOf(diameterTextField5.getText()) / (upperMaxBound + upperMaxBound*0.005) * 10000.0));
			}
		});
...
//		values.add(ST_Centroid_linkage);  // blocked Centroid method. It should be improved with distance measures.
		values.add(ST_Average_linkage);
		clusteringMethod = new GUIPropertyListEnumeration("Clustering Method", "", values, null, 150);

		ArrayList<String> values2 = new ArrayList<String>();
		values2.add(ST_DendroGram);
		values2.add(ST_Tree);
		displayMethod = new GUIPropertyListEnumeration("Display Method", "", values2, this, 150);
		...
		startButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(rootAHCTree==null||!currentMethod.equals(clusteringMethod.getValue())){
					currentMethod=(String)clusteringMethod.getValue();
					Message.add("AH clustering with " + clusteringMethod.getValue() + "...", Message.NORMAL);
					cluster();
				}
			  }
	        });
	...
	protected void initSliderPanel(){
		...
		sliderUpperUIPanel.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
		...
		sliderUpperUI.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				upperBoundUI = (double) sliderUpperUI.getValue() / 10000.0 * upperMaxBound;
		...
		diameterTextField1.addKeyListener(new KeyAdapter(){
			...
			public void keyReleased(KeyEvent e) {
				if(e.getKeyCode()==KeyEvent.VK_ENTER)
					sliderUpperUI.setValue((int) (Double.valueOf(diameterTextField1.getText()) / upperMaxBound * 10000.0));
			}
		});
		...
		sliderLowerUIPanel.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
		...
		sliderLowerUI.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				lowerBoundUI = (double) sliderLowerUI.getValue() / 10000.0 * upperMaxBound;
		...
		diameterTextField2.addKeyListener(new KeyAdapter(){
			...
			public void keyReleased(KeyEvent e) {
				if(e.getKeyCode()==KeyEvent.VK_ENTER)
					sliderLowerUI.setValue((int) (Double.valueOf(diameterTextField2.getText()) / upperMaxBound * 10000.0));
			}
		});
		...
		sliderUpperPanel.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
		...
		sliderUpper.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				upperBound = (double) sliderUpper.getValue() / 10000.0 * upperMaxBound;
		...
		diameterTextField3.addKeyListener(new KeyAdapter(){
			...
			public void keyReleased(KeyEvent e) {
				if(e.getKeyCode()==KeyEvent.VK_ENTER)
					sliderUpper.setValue((int) (Double.valueOf(diameterTextField3.getText()) / upperMaxBound * 10000.0));
			}
		});
		...
		sliderLowerPanel.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
		...
		sliderLower = new JSlider(JSlider.VERTICAL, 0, 10000, (int) (lowerBound / upperBoundUI * 10000));
		sliderLower.setOpaque(false);
		sliderLower.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				lowerBound = (double) sliderLower.getValue() / 10000.0 * upperMaxBound;
		...
		diameterTextField4.addKeyListener(new KeyAdapter(){
			...
			public void keyReleased(KeyEvent e) {
				if(e.getKeyCode()==KeyEvent.VK_ENTER)
					sliderLower.setValue((int) (Double.valueOf(diameterTextField4.getText()) / upperMaxBound * 10000.0));
			}
		});
	...
	{
		if(temp.getDistance()<cutPoint) return;
		if (temp.getLeft()==null) return;
		...
		else findCluster((AHCTreeNode)temp.getLeft());

		if (temp.getRight()==null) return;
		...
		else findCluster((AHCTreeNode)temp.getRight());
	}
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.hierachicalorgminer.algorithm.ClusteringAlgorithm#getClusters()
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.hierachicalorgminer.algorithm.ClusteringAlgorithm#getUI()
	 */
	@Override
	...
	{
		if(temp.getDistance()<=lowerBoundUI) return;
		...
		if(dist<=upperBoundUI)
			bw.write(temp.getName().replace(" ","") +
					" [shape=\"octagon\",style=\"filled\",fillcolor=\""+color+"\",label=\""
					+ temp.getName() + "\\n" + temp.size() + "\\n" + temp.getDistance() + "\"];\n");
	...
	public void writeToDotEachArc(Writer bw, AHCTreeNode temp) throws IOException
	{
		if(temp.getLeft()!=null){
			if(((AHCTreeNode)temp.getLeft()).getDistance()>lowerBoundUI
				...
				if(temp.getDistance()<=upperBoundUI)
					bw.write(temp.getName().replace(" ","") + " -- " +
						((AHCTreeNode) temp.getLeft()).getName().replace(" ","") + " [label=\""
						+ temp.getDistance() + "\"];\n");
		...
		if(temp.getRight()!=null){
			if(((AHCTreeNode)temp.getRight()).getDistance()>lowerBoundUI
				...
				if(temp.getDistance()<=upperBoundUI)
					bw.write(temp.getName().replace(" ","") + " -- " +
						((AHCTreeNode) temp.getRight()).getName().replace(" ","") + " [label=\""
						+ temp.getDistance() + "\"];\n");
	...
	/* (non-Javadoc)
	 * @see org.processmining.framework.models.DotFileWriter#writeToDot(java.io.Writer)
	 */
	public void writeToDot(Writer bw) throws IOException {
		bw.write("graph G { ranksep=\".3\"; compound=true; fontsize=\"8\"; remincross=true; margin=\"0.0,0.0\"; ");
		bw.write("fontname=\"Helvetica\";rankdir=\"TB\"; \n");
		bw.write("edge [decorate=false,fontname=\"Helvetica\",fontsize=\"8\"];\n");
		bw.write("node [height=\".1\",width=\".2\",fontname=\"Helvetica\",fontsize=\"8\",shape=\"box\",style=\"filled\",fillcolor=\"palegoldenrod\"];\n");
		clusterCounter = 0;
		clusters.clear();
