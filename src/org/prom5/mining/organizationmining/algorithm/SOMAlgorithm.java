...
import org.prom5.mining.organizationmining.distance.DistanceMetric;
import org.prom5.mining.organizationmining.model.Cluster;
import org.prom5.mining.organizationmining.model.ClusterSet;
import org.prom5.mining.organizationmining.model.InstancePoint;
import org.prom5.mining.organizationmining.profile.AggregateProfile;
import org.prom5.mining.organizationmining.ui.hierarchicalui.SOMUI;
...
public class SOMAlgorithm extends ClusteringAlgorithm implements GuiNotificationTarget {
	...
	protected GUIPropertyInteger clusterXDimBox = new GUIPropertyInteger("Width = ", 3, 1, 100);
	protected GUIPropertyInteger clusterYDimBox = new GUIPropertyInteger("Height = ", 3, 1, 100);
	protected GUIPropertyInteger    randomSeedBox = new GUIPropertyInteger("Random Seed = ", 999, 1, 100000);
	protected GUIPropertyInteger numberOfTrainingBox;
	protected GUIPropertyInteger radiusBox = new GUIPropertyInteger("Radius = ", 2, 0, 50);
	protected GUIPropertyBoolean hideNullCluster = new GUIPropertyBoolean("Hide null clusters",true, null);// this);
	...
	/* (non-Javadoc)
	 * @see org.processmining.analysis.traceclustering.algorithm.ClusteringAlgorithm#getClusters()
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see org.processmining.analysis.traceclustering.algorithm.ClusteringAlgorithm#getUI()
	 */
	@Override
	...
	public void build()
	{
		...
		for(int i=0;i<originatorSize;i++)
			originatorList.add(input.getLog().getLogSummary().getOriginators()[i]);
		...
		clusters = runSOM();
		// make result GUI
//		somView = new SOMClusterView(clusters, input, clusterXDimBox.getValue(),clusterYDimBox.getValue(), scatterSlider.getValue(), (String) backgroundStyle.getValue());
		buildOrgModel();
		somUI = new SOMUI(clusters,clusterXDimBox.getValue(),clusterYDimBox.getValue(),scatterSlider.getValue(), (String) backgroundStyle.getValue(), (String) coloringStyle.getValue(), getUMatrix());

//		somUI = new SOMUI(clusters,clusterXDimBox.getValue(),clusterYDimBox.getValue(),scatterSlider.getValue(), (String) backgroundStyle.getValue());
	}
	...
	private ClusterSet runSOM()
	{
		...
		for(int k =0; k<trainingSize; k++)
		{
			...
			progress.setNote( k + " of " + trainingSize +" training steps (initialzing cells) ...");
		...
		for(int i=0;i<originatorSize;i++){
			...
			progress.setNote( i + " of " + originatorSize +" clustering steps (allocating instances) ...");
...
//		agProfiles.getpro
		for (int i = 0; i < clusterXDim; i++)
			for (int j = 0; j < clusterYDim; j++) {
				clusterArray[i][j] = new Cluster(input.getLog(),"OrgUnit("+j+","+i+")"); // name --> Cluster (y,x)
	...
	private void calcualteDistance(int index, double weight)
	{
		...
		for (int j = 0; j < clusterXDim; j++)
			for (int k = 0; k < clusterYDim; k++) {
				double tempDistance = distanceMeasures.getDistance(agProfiles.getPoint(originatorList.get(index)),
									  pointArray[j][k]);
		...
		if(bTraining){
				...
				for (int l = col1; l < col2; l++) {
					for (int k = 0; k < agProfiles.numberOfItems(); k++) {
						double currentValue = pointArray[j][l].get(agProfiles.
											  getItemKey(k));
						currentValue += weight * (agProfiles.getValue(index, k) - currentValue);
	        ...
	        clusterArray[rowMinIndex][colMinIndex].addOriginator(originatorList.get(index));
		}
	}
	...
	private double[][] getUMatrix() {
		...
		for (int i = 0; i < clusterXDim; i++) {
			for (int j = 0; j < clusterYDim; j++) {
				...
				(pointArray[i][j],pointArray[i][((j+1)%clusterYDim)]);
				tempDistance += distanceMeasures.getDistance
				(pointArray[i][j],pointArray[(i+1)%clusterXDim][j]);
				tempDistance += distanceMeasures.getDistance
				(pointArray[i][j],pointArray[(i+1)%clusterXDim][((j+1)%clusterYDim)]);
				tempDistance += distanceMeasures.getDistance
				(pointArray[i][(j+1)%clusterYDim],pointArray[(i+1)%clusterXDim][((j+1)%clusterYDim)]);				
				tempDistance += distanceMeasures.getDistance
				(pointArray[(i+1)%clusterXDim][j],pointArray[i][((j+1)%clusterYDim)]);
				tempDistance += distanceMeasures.getDistance
				(pointArray[(i+1)%clusterXDim][j],pointArray[(i+1)%clusterXDim][((j+1)%clusterYDim)]);
	...
	protected void setupGUI()
	{
		...
		numberOfTrainingBox = new GUIPropertyInteger("Number of training = ", Math.round(agProfiles.getNumberOfOriginators()/10+1), 1, agProfiles.getNumberOfOriginators());
		...
		backgroundStyle = new GUIPropertyListEnumeration("Background Style", "", values, this, 150);
		...
		coloringStyle = new GUIPropertyListEnumeration("Color Style", "", values2, this, 150);
		...
		sliderPanel_Scatter.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
	...
	public void updateGUI(){
		if(somUI!=null)
			somUI.setBackground((String)backgroundStyle.getValue(), (String)coloringStyle.getValue());
	}
}
