...
public class OrgEntityResourceTablePanel extends JPanel implements TableModelListener {
	...
	public OrgEntityResourceTablePanel(OrgModel orgmod, OrgMiningResult parentpanel) {
		...
		orgEntityTable.addKeyListener(new KeyAdapter() {
			public void keyPressed(KeyEvent e) {
				...
				if(e.getKeyCode()==KeyEvent.VK_UP){
					row=(orgEntityTable.getSelectedRow()-1)<=0?0:(orgEntityTable.getSelectedRow()-1);
				}
				else if(e.getKeyCode()==KeyEvent.VK_DOWN){
					row=(orgEntityTable.getSelectedRow()+1)>=orgEntityTable.getRowCount()?orgEntityTable.getRowCount()-1:orgEntityTable.getSelectedRow()+1;
				}
				else return; 
					currentEntity = ((OrgEntityTable) orgEntityTable.getModel()).getOrgEntity(row);
					((ResourceTable) resourceTable.getModel()).updateTable(currentEntity);
			}
		});
			...
			if (e.getColumn() == 0) {
				...
				if (grp_row >= 0) {
					if ((Boolean) resourceTable.getValueAt(src_row, 0)) {
						OrgEntity orgEntity = ((OrgEntityTable) orgEntityTable.getModel()).getOrgEntity(grp_row);
						((ResourceTable) resourceTable.getModel()).getResource(src_row).addOrgEntity(orgEntity);
					} else {
						OrgEntity orgEntity = ((OrgEntityTable) orgEntityTable.getModel()).getOrgEntity(grp_row);
						((ResourceTable) resourceTable.getModel()).getResource(src_row).removeOrgEntity(orgEntity);
					}
				} else {
	...
	public OrgEntity getSeletedOrgEntity()
	{
		if (orgEntityTable.getSelectedRow() < 0) {
			...
			return ((OrgEntityTable) orgEntityTable.getModel()).getOrgEntity(orgEntityTable.getSelectedRow());
		}
	}
	...
	public Resource getSeletedResource()
	{
		if (resourceTable.getSelectedRow() < 0) {
			...
			return ((ResourceTable) resourceTable.getModel()).getResource(resourceTable.getSelectedRow());
		}
	}
	...
	private class OrgEntityTable extends AbstractTableModel {
		...
		public int getRowCount() {
			int num = 0;
			if (orgEntities != null) num += orgEntities.size();
			return num;
		}
	...
	private class ResourceTable extends AbstractTableModel {
		...
		public Object getValueAt(int row, int column) {
			...
			switch (column){
				case 0: return (Boolean) choose.get(row);
				case 1: return (String) resources.get(row).getID();
				case 2: return (String) resources.get(row).getName();
			}
			return null;
		}
		...
		public boolean isCellEditable(int row, int col) {
			if (col == 0) {
				HashSet<String> resourceSubset = orgModel.getSubResourceList(currentEntity);
		...
		public void updateTable(OrgEntity orgEntity) {
			...
			int numRes = resources.size();
			HashSet<String> resourceSubset = orgModel.getSubResourceList(orgEntity);
			for (int i = 0; i < numRes; i++) {
				choose.set(i, Boolean.valueOf(resources.get(i).hasOrgEntity(orgEntity)));
				if(resourceSubset.contains(resources.get(i).getID())) {
					choose.set(i, true);
				}
			}
