...
public class TaskOrgEntityTablePanel extends JSplitPane implements TableModelListener {
	...
	public TaskOrgEntityTablePanel(OrgModel orgmod, OrgMiningResult parentpanel) {
		...
		taskTable.addKeyListener(new KeyAdapter() {
			public void keyPressed(KeyEvent e) {
				...
				if(e.getKeyCode()==KeyEvent.VK_UP){
					row=(taskTable.getSelectedRow()-1)<=0?0:(taskTable.getSelectedRow()-1);
				}
				else if(e.getKeyCode()==KeyEvent.VK_DOWN){
					row=(taskTable.getSelectedRow()+1)>=taskTable.getRowCount()?taskTable.getRowCount()-1:taskTable.getSelectedRow()+1;
				}
				else return; 
	...
	public OrgEntity getSeletedOrgEntity()
	{
		if (orgEntitySelectTable.getSelectedRow() < 0) {
			...
			return ((OrgEntitySelectTable) orgEntitySelectTable.getModel()).getOrgEntity(orgEntitySelectTable.getSelectedRow());
		}
	}
	...
	public Task getSeletedTask()
	{
		if (taskTable.getSelectedRow() < 0) {
			...
			return ((TaskTable) taskTable.getModel()).getTask(taskTable.getSelectedRow());
		}
	}
	...
	private class OrgEntitySelectTable extends AbstractTableModel {
		...
		public Object getValueAt(int row, int column) {
			...
			switch (column){
				...
				case 2: return orgEntities.get(row).getName();
			}
			return null;
		}
