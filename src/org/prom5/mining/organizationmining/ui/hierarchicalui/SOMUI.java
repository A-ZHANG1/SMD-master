package org.prom5.mining.organizationmining.ui.hierarchicalui;
...
public class SOMUI extends JComponent {
	...
	public SOMUI(ClusterSet aClusterSet, int colNumber, int rowNumber, int aScatteredRatio, String aBackgroundStyle, String aColorStyle, double[][] uMatrix) {
	...
	protected void paintComponent(Graphics g) {
		...
		if(pixWidth%unitWidth!=0)pixWidth = unitWidth * (pixWidth/unitWidth);
		if(pixHeight%unitWidth!=0)pixHeight = unitHeight * (pixHeight/unitHeight);
			...
			if(cluster.size()>0){
				...
				if(scatteredRatio>0){
					for (int i = 0; i < cluster.size(); i++) {
						...
						pertX = ((rnd.nextDouble() * (upperBound - lowerBound)) +
								lowerBound) * (scatteredRatio / 80.0d);
						lowerBound = -unitHeight / 2.0d;
						upperBound = unitHeight / 2.0d;
						pertY = ((rnd.nextDouble() * (upperBound - lowerBound)) +
								lowerBound) * (scatteredRatio / 80.0d);
				...
				if(backgroundStyle.equals(ST_BLACK)) g2d.setColor(Color.yellow);
		...
		if(backgroundStyle.equals(ST_BLACK)) {
			...
			if(colorStyle.equals(ST_DENSITY)) {
				while (itr.hasNext()) {
					Cluster cluster = (Cluster) itr.next();
					printMatrix[(k -
							(k / colNumber) * colNumber) % colNumber][k / colNumber] = cluster.size();
				...
				for (int i = 0; i < colNumber; i++) {
					for (int j = 0; j < rowNumber; j++) {
						interpolateRect(buffer, width * i, height * j, width, height, printMatrix, i, j,
								max, min,
			...
			// U-matrix
				...
				for (int i = 0; i < colNumber; i++) {
					for (int j = 0; j < rowNumber; j++) {
						interpolateRect2(buffer, width * i, height * j, width, height, uMatrix, i, j,
								max, min,
								new SOMLandscapeColorizer());
					}
				}
	...
	private void interpolateRect(BufferedImage image, int posX, int posY, double width,
			double height, double[][] matrix, int matrixX, int matrixY, double colorScale, double colorMin,
		...
		double p22 = matrix[(matrixX + 1) % colNumber][(matrixY + 1) % rowNumber];
...
//				double colorValue = interpolatedValue  / colorScale;
				...
				double colorValue = (interpolatedValue - colorMin)/ (colorScale - colorMin);
	...
	private void interpolateRect2(BufferedImage image, int posX, int posY, double width, double height, double[][] matrix, int matrixX, int matrixY, double colorScale, double colorMin,
			SOMLandscapeColorizer colorizer) {
        // top-left
        double p11 = matrix[matrixX][matrixY];
        double p21 = matrix[(matrixX + 1) % colNumber][matrixY];
...
class SOMLandscapeColorizer {
	private Color[] colors = new Color[] { new Color(0, 0, 255), new Color(0, 255, 255), new Color(255, 255, 185), new Color(255, 255, 100),
			new Color(110, 255, 110), new Color(0, 170, 0), new Color(255, 195, 105), new Color(155, 95, 0), new Color(195, 195, 195),
	...
	public Color getPointColor(double value) {
		...
		if (intervall >= 5) return Color.BLACK;
