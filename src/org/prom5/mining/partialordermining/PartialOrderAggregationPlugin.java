...
public class PartialOrderAggregationPlugin implements MiningPlugin {
	...
	public MiningResult mine(LogReader log, Progress p, boolean useTransRed) {
		...
		for (LogEvent evt : events) {
			ModelGraphVertex v = result.addVertex(new ModelGraphVertex(evt, result));
		...
		while (it.hasNext()) {
			...
			if (!pi.getAttributes().containsKey(ProcessInstance.ATT_PI_PO) ||
				...
				// skip the process instance if it already is not a partial order
				Message.add("Process Instance " + pi.getName() + " is not a partial order.");
				continue;
			}
			nrTreatPis++;
			...
			while (ateIt.hasNext()) {
				...
				LogEvent le = events.findLogEvent(ate.getElement(), ate.getType());
				ModelGraphVertex v = (ModelGraphVertex) event2Node.get(le);
				// Increase the occurences of v
				v.setValue(v.getValue() + 1);
				id2node.put(ate.getAttributes().get(ProcessInstance.ATT_ATE_ID), v);
			}

			ModelGraph instanceGraph = getModelGraphInstance(pi, useTransRed);

			for (ModelGraphVertex v : (ArrayList<ModelGraphVertex>) instanceGraph.getVerticeList()) {
			...
			for (ModelGraphEdge e : (ArrayList<ModelGraphEdge>) instanceGraph.getEdges()) {
		...
		Message.add("  <numberOfTreatedInstances=" + nrTreatPis + "/>", Message.TEST);
		result.Test("ResultGraph");
		Message.add("</ParitalOrderAggregator>", Message.TEST);
		return new AggregationGraphResult(log, result);
	}

	private ModelGraph getModelGraphInstance(ProcessInstance pi, boolean useTransRed) {
	...
	public JPanel getOptionsPanel(LogSummary summary) {
		...
		check = new JCheckBox("<html>Check this box if transitive reduction should " +
