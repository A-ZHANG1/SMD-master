...
public class PartialOrderGeneratorPlugin extends LogRelationBasedAlgorithm {
	...
	public MiningResult mineWithProgressSet(LogReader log, LogRelations relations,
			Progress progress) {
		...
		while (it.hasNext()) {
			...
			for (int k = 0; k < maxIndex; k++) {
				...
				for (int i = k + 1; i < maxIndex; i++) {
					...
					int nextAteIndex = events.findLogEventNumber(nextAte.getElement(),
									   nextAte.getType());
					...
					if (causal.get(ateIndex, nextAteIndex) > 0) {
						...
						post += (post.length() == 0 ? "" :
								",") + nextAte.getAttributes().get(ProcessInstance.ATT_ATE_ID);
						pre += (pre.length() == 0 ? "" :
				...
				for (int i = k - 1; i >= 0; i--) {
					...
					int prevAteIndex = events.findLogEventNumber(prevAte.getElement(),
									   prevAte.getType());
					if (causal.get(prevAteIndex, ateIndex) > 0) {
						...
						post += (post.length() == 0 ? "" :
								",") + ate.getAttributes().get(ProcessInstance.ATT_ATE_ID);
						pre += (pre.length() == 0 ? "" :
								",") + prevAte.getAttributes().get(ProcessInstance.ATT_ATE_ID);
						// set the pre and post set
