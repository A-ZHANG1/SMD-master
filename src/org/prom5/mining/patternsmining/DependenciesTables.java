...
public class DependenciesTables {
	...
	private DoubleMatrix2D directPrecedents; //= logAbstraction.getFollowerInfo(1).copy();
	...
	public DependenciesTables(LogReader log) {
		...
		this.finalfollowers = finalSuccessionright(log,ACWWidth,directfollowers);
	...
	public DependenciesTables(DoubleMatrix2D causal, DoubleMatrix2D parallel,
			DoubleMatrix1D end, DoubleMatrix1D start, DoubleMatrix1D loops, LogEvents events) {
		this.causal = causal;
	...
	public DependenciesTables(DependenciesTables relations) {
		this(relations.getCausalFollowerMatrix(), relations.getParallelMatrix(),
				relations.getEndInfo(), relations.getStartInfo(), relations.getOneLengthLoopsInfo(),
	 ...
	 * @param log
	 * @return first matrix
	 * to do: factorize logeevents param modelElements
	 * to do: division by act freq may be done in the last not here
	 */
	...
	private DoubleMatrix2D DirectPreviousInfo(LogReader log) {
		...
		while (log.hasNext()) {
			...
			int memory = modelElements.findLogEventNumber(ate.getElement(),
					ate.getType());
	...
	private DoubleMatrix2D Causalfollowers(DoubleMatrix2D directPrecedents) {
		...
		for (int j=0; j<s; j++){
			...
			for (int k=0; k<s; k++){
				//D.set(j, k, D.get(j, k)/freq);  a cause des boucle dans thread concurrent
	...
	private DoubleMatrix2D Causalprecedents(DoubleMatrix2D directPrecedents) {
		...
		for (int i=0; i<s; i++){
			...
			for (int j=0; j<s; j++){
				//D.set(i, j, directPrecedents.get(j, i)/freq); a cause des boucle dans thread concurrent
	...
	private DoubleMatrix1D BackwardACWWidth(DoubleMatrix2D directPrecedents) {
		...
		private DoubleMatrix2D finalSuccessionright(LogReader log, DoubleMatrix1D ACW, DoubleMatrix2D directfollowers) {
			int numSimilarPIs = 0;
			log.reset();
			int s = modelElements.size();
			DoubleMatrix2D D = DoubleFactory2D.sparse.make(s, s, 0);
			...
			while (log.hasNext()) {
				...
				while (i<instanceList.size()-1) {
					...
					//////////////////////////////////////////////////////////////////////////
					/////////////////////////////// changement a cause des boucle dans le comportement concurrent
					int k=1;
					int m=0;
					...
					while(k<ACWwidth+1 && i+m+1<instanceList.size()){
						...
						if (i+m<instanceList.size()){
							AuditTrailEntry atefollow =(AuditTrailEntry) instanceList.get(i+m);
							int followerActivity = modelElements.findLogEventNumber(atefollow.getElement(), atefollow.getType());
							if (boucle[followerActivity]){
								if (directfollowers.get(courantActivity , followerActivity)!=0){
	...
	//a corriger il faut changer seulement les valeurs qui ne sont pas dans la matrice directfollower
	...
	public void build() {	
				...
				if (finalcausalprecedents.get(i, j)==1){
					...
					if (finalcausalprecedents.get(i, j)<0){					
						...
						if (finalcausalprecedents.get(i, j)>0){
							...
							sumPreceeder[i]=sumPreceeder[i]+finalcausalprecedents.get(i, j);
						}
					}
				}
			}
			...
			for (int j=0; j<s; j++){
				if (finalcausalfollowers.get(i, j)==1){
					...
					if (finalcausalfollowers.get(i, j)>0){
						...
						sumfollowers[i]=sumfollowers[i]+ finalcausalfollowers.get(i, j);
					}
				}
			}
		...
		for (int i=0; i<s; i++){
			...
			getStartInfo().set(i,nbcompletePreceeder[i]+nbpartialPreceeder[i]);
		}
