...
class RelationsUI extends DefaultComboBoxModel {
	...
	private JPanel buildPanel(int index) {
		...
		listsPanel.add(constructCompletePreceederList("Always preceed(s)", index));
		listsPanel.add(constructCompletefollowersList("Always follow(s)", index));
		listsPanel.add(constructConcurrentList("Parallel with ", index));
		listsPanel.add(constructPartialPreceederList("Possibly  preceed(s)", index));
		listsPanel.add(constructPartialfollowersList("Possibly follow(s)", index));
	...
	private JPanel constructCompletePreceederList(String caption, final int index) {
		JPanel panel = new JPanel(new BorderLayout());
	...
	private JPanel constructPartialPreceederList(String caption, final int index) {
		...
		for (int i = 0; i <nbPreceeder; i++) {
			...
			LogEvent ev = relations.getLogEvents().getEvent(temppreceder);
			double finalI = relations.getfinalcausalprecedents().get(index,temppreceder)*100;
			
			boxes[i] = ev.getModelElementName() + " (" + ev.getEventType() + ")  "+finalI+"%";	
		}
	...
	private JPanel constructCompletefollowersList(String caption, final int index) {
		JPanel panel = new JPanel(new BorderLayout());
		JList list = new JList();
	...
	private JPanel constructPartialfollowersList(String caption, final int index) {
		...
		for (int i = 0; i <nbPreceeder; i++) {
			...
			LogEvent ev = relations.getLogEvents().getEvent(temppreceder);
			double finalI = relations.getfinalcausalfollowers().get(index,temppreceder)*100;
