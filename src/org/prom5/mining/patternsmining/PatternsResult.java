 ...
 */
package org.prom5.mining.patternsmining;
...
import org.prom5.framework.models.epcpack.InstanceEPC;
...
public class PatternsResult implements MiningResult, Provider {
	...
	public ModelGraph build() {
		...
		DoubleMatrix2D finalcausalfollowers=relations.getfinalcausalfollowers();
		DoubleMatrix2D finalcausalprecedents= relations.getfinalcausalprecedents();
		...
		//
		// 1) Build a DEPENDENCIES RELATION TAB (done by DependenciesTables class)
		//
		...
		for (int i=0; i<s; i++){
			ModelGraphVertex v = new ModelGraphVertex(modelElements.getEvent(i), mainGraph);
		...
		for (int i=0; i<s; i++){
			...
			if(nbcompletePreceeder[i]==1){
				...
				if(nbcompletefollowers[source]==1 ){
					int temp= ((Integer)completefollowers[source].get(0)).intValue();
					if (temp==i){
						mainGraph.addEdge((ModelGraphVertex) vertices.get(source),
							(ModelGraphVertex) vertices.get(i));
					}
				}
			...
			if(nbcompletePreceeder[i]>1){
				...
				for (int j=0; j<nbcompletePreceeder[i]; j++) {
					int source = ((Integer)completePreceeder[i].get(j)).intValue();
					mainGraph.addEdge((ModelGraphVertex) vertices.get(source),
							andjoin);
			...
			if(nbpartialPreceeder[i]>0){
				if (sumPreceeder[i]==1){
					...
					mainGraph.addEdge(xorjoin,(ModelGraphVertex) vertices.get(i));
					
					for (int j=0; j<nbpartialPreceeder[i]; j++) {
						int source = ((Integer)partialPreceeder[i].get(j)).intValue();
						mainGraph.addEdge((ModelGraphVertex) vertices.get(source),
								xorjoin);
				...
				if (sumPreceeder[i]>1){
					...
					orjoin.setIdentifier( temp.intValue() + " out of" + nbpartialPreceeder[i]+ " pattern");
					mainGraph.addEdge(orjoin,(ModelGraphVertex) vertices.get(i));
			...
			if(nbcompletefollowers[i]>1){
				...
				for (int j=0; j<nbcompletefollowers[i]; j++) {
					int dest = ((Integer)completefollowers[i].get(j)).intValue();
					mainGraph.addEdge(andsplit, (ModelGraphVertex) vertices.get(dest));
			...
			if(nbpartialfollowers[i]>0){
				if (sumfollowers[i]==1){
					...
					mainGraph.addEdge((ModelGraphVertex) vertices.get(i),xorsplit);
					
					for (int j=0; j<nbpartialfollowers[i]; j++) {
						int dest = ((Integer)partialfollowers[i].get(j)).intValue();
						mainGraph.addEdge(xorsplit,(ModelGraphVertex) vertices.get(dest)
						);
				...
				if (sumfollowers[i]>1){
					...
					mainGraph.addEdge((ModelGraphVertex) vertices.get(i),orsplit);
					
					for (int j=0; j<nbpartialfollowers[i]; j++) {
						int dest = ((Integer)partialfollowers[i].get(j)).intValue();
						mainGraph.addEdge(orsplit,(ModelGraphVertex) vertices.get(dest)
						);
		...
		//if (epc.getGrappaVisualization()==null) System.out.println("nn**************");
		return mainGraph;
	}
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		return new ProvidedObject[] {
				new ProvidedObject("Patterns", new Object[] {relations, log})
		};
	}
