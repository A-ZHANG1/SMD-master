...
public class AlphaProcessMiner extends LogRelationBasedAlgorithm implements ConvertingPlugin {
	...
	public MiningResult mine(LogReader log, LogRelations relations, final Progress progress) {
		...
		if (showIntermediateUpdates()) {
			SwingWorker worker = new SwingWorker() {
				public Object construct() {
					findPlaces(petrinet, result, transitions, new ProgressDummy());
					return null;
				}
	...
	private void findPlaces(PetriNet petrinet, PetriNetResult result,  ArrayList<Transition> transitions, Progress progress) {
		...
		for (int i = 0; i < nme; i++) {
			...
			for (int j = 0; j < nme; j++) {
				...
				// since {i} -> {j}
				ExpandTree(tuples, A, B, 0, 0, petrinet, places, transitions, result);
			}
		...
		for (int i = 0; i < nme; i++) {
			...
			IntArrayList tupleNumbers = getTupleNumbersContaining(tuples, pre_i, suc_i);
...
private boolean removeSmallerThan(ArrayList tuples, IntArrayList[] tuple, int tupleIndex,
		PetriNet net, ArrayList<Place> places) {
	int i = tupleIndex;
	...
	while (i >= 0 && !foundLarger) {
		...
		if (tuple[0].toList().containsAll(tuple_i[0].toList()) &&
			...
			// tuple_i contains tuple (hence, there are no smaller tuples before
			foundLarger = true;
		}
...
private Place addPlaceForTuple(IntArrayList[] tuple, PetriNet petrinet,
	...
	for (int j = 0; j < tuple[0].size(); j++) {
		petrinet.addEdge(getTransition(tuple[0].get(j), transitions, petrinet), p);
	}
	for (int j = 0; j < tuple[1].size(); j++) {
		petrinet.addEdge(p, getTransition(tuple[1].get(j), transitions, petrinet));
	}
	return p;
}

private Transition getTransition(int i, ArrayList<Transition> transitions, PetriNet net) {
...
private IntArrayList getTupleNumbersContaining(ArrayList tuples, IntArrayList A, IntArrayList B) {
	int i = -1;
	...
	while (i < tuples.size() - 1) {
		...
		if ((tA.toList().containsAll(tuple[0].toList()) &&
				tB.toList().containsAll(tuple[1].toList()))) {
...
private boolean ExpandTree(ArrayList tuples, IntArrayList A, IntArrayList B, int sA, int sB,
		PetriNet net, ArrayList<Place> places, ArrayList<Transition> transitions,
	...
	for (int i = s; i < nme; i++) {
		...
		if (c) {
			for (int j = 0; j < A.size(); j++) {
				c = c && (logRelations.getCausalFollowerMatrix().get(i, A.get(j)) == 0)
					&& (logRelations.getCausalFollowerMatrix().get(A.get(j), i) == 0)
					&& (logRelations.getParallelMatrix().get(i, A.get(j)) == 0);
				// c == i does not have a relation with any element of A
			}
		}
		if (c) {
			for (int j = 0; j < B.size(); j++) {
				c = c && (logRelations.getCausalFollowerMatrix().get(i, B.get(j)) > 0);
		...
		if (d) {
			for (int j = 0; j < B.size(); j++) {
				d = d && (logRelations.getCausalFollowerMatrix().get(i, B.get(j)) == 0)
					&& (logRelations.getCausalFollowerMatrix().get(B.get(j), i) == 0)
					&& (logRelations.getParallelMatrix().get(i, B.get(j)) == 0);
				// d == i does not have a relation with any element of B
			}
		}
		if (d) {
			for (int j = 0; j < A.size(); j++) {
				d = d && (logRelations.getCausalFollowerMatrix().get(A.get(j), i) > 0);
		...
		if (c) {
			// i can be added to A
			A.add(i);
			expanded = ExpandTree(tuples, A, B, i + 1, sB, net, places, transitions, result);
			A = tA;
		}
		if (d) {
			// i can be added to A
			B.add(i);
			expanded = ExpandTree(tuples, A, B, sA, i + 1, net, places, transitions, result);
			B = tB;
		}
	}
	if (!expanded) {
...
public MiningResult convert(ProvidedObject providedObject) {
	...
	Progress p = new Progress("Converting Transition System to Log logRelations", 0, 5);

	// First, create the bogus log events from the TS
	LogEvents events = new LogEvents();
	for (TransitionSystemEdge edge : (ArrayList<TransitionSystemEdge>) ts.getEdges()) {
	...
	logRelations = (new MinValueLogRelationBuilder(abstraction, 0,
				   events)).getLogRelations();
