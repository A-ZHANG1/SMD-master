...
import org.prom5.framework.models.petrinet.PetriNetHierarchy;
...
import org.prom5.framework.ui.ComboBoxLogEvent;
...
public class PetriNetResult implements MiningResult, Provider, LogReaderConnection {
    ...
    private PetriNetGrappaAdapter adapter;
    private PetriNet net;

    private PetriNetHierarchy hierarchy;

    private void newHierarchy() {
    ...
    public PetriNetResult(LogReader log, PetriNet net, MiningPlugin algorithm) {
        ...
        this.log = log;
        this.net = net;
        this.algorithm = (algorithm instanceof LogRelationBasedAlgorithm
                          ? (LogRelationBasedAlgorithm) algorithm
                          : null);
        ...
        if (log != null) {
            if (getPetriNet() == null) {
                return new ProvidedObject[] {
                        new ProvidedObject("Petri net hierarchy", new Object[] {hierarchy, log})
                };
            } else {
                PetriNet net = getPetriNet();
                return new ProvidedObject[] {
                        new ProvidedObject("Selected Petri net", new Object[] {net, log}),
                        new ProvidedObject("Petri net hierarchy", new Object[] {hierarchy, log})
                };
            }
        } else {
            if (getPetriNet() == null) {
                return new ProvidedObject[] {
                        new ProvidedObject("Petri net hierarchy", new Object[] {hierarchy})
                };
            } else {
                PetriNet net = getPetriNet();
                return new ProvidedObject[] {
                        new ProvidedObject("Selected Petri net", new Object[] {net}),
                        new ProvidedObject("Petri net hierarchy", new Object[] {hierarchy})
                };
            }
        }
    ...
    void editRelations(Transition t) {
        if (algorithm != null) {
            MiningResult result = algorithm.editRelations((t == null ? null : t.getLogEvent()));
            if (result != null) {
                ModelGraphPanel gp;

                if (result instanceof PetriNetResult) {
                    //gp = (ModelGraphPanel)((PetriNetResult) result).netContainer.getViewport().getView();
                    newHierarchy();
            ...
            while (it.hasNext()) {
                ...
                if (o instanceof PetriNet) {
                    ...
                    while (it2.hasNext()) {
                        ...
                        // t.setModelElement((LogEvent) eventsMapping.get(t.getModelElement()));
                    }
                }
            }
        }
...
class PetriNetGrappaAdapter extends GrappaAdapter {
    ...
    public PetriNetGrappaAdapter(PetriNetResult r, boolean editRelations) {
        ...
        if (r.log != null) {
            allEvents.addAll(result.getLogReader().getLogSummary().getLogEvents());
        }
        this.editRelations = editRelations;
        if (r.getLogReader() != null) {
            eventLogFilter = new LogEventLogFilter(r.getLogReader().getLogSummary().getLogEvents());
    ...
    private void makeVisible(Transition t, boolean showDialog) {
        ...
        if (showDialog) {
            LogEventIntroductionDialog d = new LogEventIntroductionDialog(allEvents,
                    t.getIdentifier());
            ...
            newEvent = new LogEvent(ComboBoxLogEvent.NONE, ComboBoxLogEvent.NONE);
        }
        ...
        if (result.log != null) {
            ...
            if ((result.log.getLogFilter() == eventLogFilter) &&
                ...
                try {
                    result.log = LogReaderFactory.createInstance(oldFilter, result.log);
                } catch (Exception e) {
                    e.printStackTrace();
                    result.log = null;
                }
    ...
    private void makeInvisible(Transition t) {
        ...
        if ((result.log != null) &&
            ...
            try {
                result.log = LogReaderFactory.createInstance(eventLogFilter, result.log);
            } catch (Exception e) {
                e.printStackTrace();
                result.log = null;
            }
     ...
     */
    protected JMenuItem getCustomMenu(Subgraph subg, Element elem, GrappaPoint pt, int modifiers,
                                      GrappaPanel panel) {
    ...
    private class LogEventIntroductionDialog extends JDialog {
        ...
        public LogEventIntroductionDialog(LogEvents events, String label) {
            ...
            logEvents.addItemListener(new ItemListener() {
                public void itemStateChanged(ItemEvent e) {
                    LogEvent selected = ((ComboBoxLogEvent) logEvents.getSelectedItem()).
                                        getLogEvent();
        ...
        public LogEvent showDialog() {
            ...
            LogEvent e = events.findLogEvent(eventName.getText(), eventType.getText());
            if (e == null) {
