...
public class CrossValidationResult {
	...
	/**
	 * stores the result for the k-th cross validtion part and the r-th repititions.
	 * note k and r start at 0
	 * @param result
	 * @param k
	 * @param r
	 */
	...
	/**
	 * 
	 * returns a vector with 9 rows; containing all predictions which can be exported to the events in the log
	 * in order: y, np pred, avg pred, np sd, avg sd,pid,aid, np error, avg error
	 * @return
	 */
	public Vector<Vector> exportablepredictions(){
		...
		Vector<Double> row3=new Vector<Double>();//row 3 is the average estimation
		Vector<Double> row4=new Vector<Double>();//row 4 is the NP standard deviation over the repititions
		Vector<Double> row5=new Vector<Double>();//row 5 is the average standard deviation
		Vector<Integer> row6=new Vector<Integer>();//row 7 are the pids(process instance id's) corresponding to these values
		Vector<Integer> row7=new Vector<Integer>();//row 7 are the evnums(event sequence numbers) corresponding to these values
		Vector<Double> row8=new Vector<Double>();//row 8 is the error of the NP estimation
		Vector<Double> row9=new Vector<Double>();//row 9 is the error of the average estimation
		Vector<Double> row10=new Vector<Double>();//row 10 is the relative error of the average estimation
		Vector<Double> row11=new Vector<Double>();//row 11 is the relative error of the NP estimation
		Vector<Double> row12=new Vector<Double>();//row 12 is the absolute error of the NP estimation
		Vector<Double> row13=new Vector<Double>();//row 13 is the absolute error of the average estimation
		for (Vector<SingleRegressionResult> kresult:list){//for all crossvalidation parts
			for(int i=0;i<kresult.firstElement().nppredictions.size();i++){//for all estimations in this cross validation part
				...
				row10.add(Math.abs(avgerroravg)/kresult.firstElement().actualvalues.get(i));
				row11.add(Math.abs(nperroravg)/kresult.firstElement().actualvalues.get(i));
	...
	/**
	 * returns a vector which is suited for as a header for the table returned by estimationsTable()
	 * @return
	 */
	public Vector<String> estimationsTableHeader(){
		Vector<String> result=new Vector<String>(5);
		result.add("Y");result.add("my prediction");result.add("dumb predictor");
		result.add("my prediction error");result.add("dumb predictor error");
		result.add("my prediction SD");result.add("dumb predictor SD");
		return result;
	}
	...
	public Vector<Vector> resultsTable(int type){
		...
		for (Vector<SingleRegressionResult> kresult:list){
			...
			for (SingleRegressionResult s: kresult){
				switch(type){
				case 0: row1.add(s.avgMAE);
						row2.add(s.npMAE);
						break;
				case 1: row1.add(s.avgMAPE);
						row2.add(s.npMAPE);
						break;
				case 2: row1.add(s.avgMSE);
						row2.add(s.npMSE);
						break;					
				}
			}
	...
	/**
	 * returns an array with the bandwidths from the first result, which
	...
	/**
	 * returns a vector of the form Vector(crossval-part) of Vector(repitition) of Vector(variable) of Double.
	 * This vector should be split in tabs with a table on each tab
	 */
	public Vector<Vector<Vector>> bandwidthsTable(){
		...
		for (Vector<SingleRegressionResult> kresult:list){
			...
			firstrow.add("MAE");firstrow.add("MAPE");firstrow.add("MSE");
			for (SingleRegressionResult s: kresult){
	...
	/**
	 * returns a vector of Strings which is the sequence of variables for the bandwidth tables headers
	 */
	public Vector<Vector<String>> bandwidthsHeader(){
		...
		for (Vector<SingleRegressionResult> kresult:list){
			...
			row.add("MAE");row.add("MAPE");row.add("MSE");
			table.add(row);
		}
		return table;
	}
