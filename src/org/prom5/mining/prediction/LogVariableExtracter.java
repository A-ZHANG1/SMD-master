...
public class LogVariableExtracter {
	...
	public LogVariableExtracter(LogReader logreader){
		...
		if (!(logreader instanceof BufferedLogReader)) {
			...
			try{
				newlog=BufferedLogReader.createInstance(logreader,logreader.processInstancesToKeep());
			}catch (Exception ex2){}
		...
		/*
		 * if settings.target==1 (remaining time until X)
		 * 	then filter the log to contain only traces with X, X being settings.targetelement
		 */
	...
	/**
	 * map with values for audittrail entries stored under attrib
	 * all arrays must be equal in size
	 * @param values
	 * @param atrib
	 */
	public void setLogEventDataAttributes(BufferedLogReader exportlog,Integer[] procInstNumbers,Integer[] eventNumbers, Double[] values, String attribname){
		for(int i=0;i<procInstNumbers.length;i++){
			try{
				ProcessInstance pi=exportlog.getInstance(procInstNumbers[i]);
				AuditTrailEntry ate=pi.getAuditTrailEntryList().get(eventNumbers[i]);
				Double val=values[i];
	...
	/**
	 * this function writes the array of 'values' to the process as an attribute under the 'name'
	 * @param values
	 * @param names
	 */
	public void setLogProcessAttribute(BufferedLogReader exportlog,String value, String name){
		try{
	...
	/**
	 * this function takes an array with possible duplicate integers,
	 * the array is filtered to unique integers and the log is filtered to contain
	...
	private void filter(){
		...
		int i=0;
		for (Integer val: instancelist.toArray(new Integer[instancelist.size()])){
			result[i]=val;
			i++;
		}
		try{
		...
		for(ProcessInstance pi:log.getInstances()){
			...
			for(AuditTrailEntry ate:pi.getListOfATEs()){
				if (localSettings.useElements.contains(ate.getElement())){
					...
					if(localSettings.startEvents.contains(ate.getType())){
					...
					}else if(localSettings.completeEvents.contains(ate.getType())){
						//complete means occurrence
						
						if (localSettings.target==2){
							if (localSettings.targetElement.equals(ate.getElement())){
								extractedVariables.storeTargetElementOccurrence();
								extractedVariables.storeOccurrence(element,ate.getTimestamp().getTime());
							}else{
								extractedVariables.storeOccurrence(element,ate.getTimestamp().getTime());
							}
						}else{
						...
						if (localSettings.useDurations&&startedElement.contains(element)){
							if (localSettings.target==2){
								if (localSettings.targetElement.equals(ate.getElement())){
									//take max(0,timediff) of timediff then divide by timesize 
									Long tempdur=Math.max(0,ate.getTimestamp().getTime()-startTime.get(startedElement.indexOf(element)));
									extractedVariables.storeDuration(element, tempdur);
									/* remove start for this event, remove only 1 element, to enable parrallel tasks */
									startTime.remove(startedElement.indexOf(element));
						...
						if (localSettings.useAttributes){
							for(String attrib:ate.getDataAttributes().keySet()){
								String value=ate.getAttributes().get(attrib);
								if (!(value==null)){
									extractedVariables.storeEventAttribute( attrib,ate.getDataAttributes().get(attrib));
								}
							}
							//cases.storeEventAttribute(element+"_originator", ate.getOriginator());
						}
...
class CaseSet{ 
	/**
	 * this function checks every variable to eleminate constants. Currently only for attributes since these are the only problem
	 */
	public void removeAttributeConstants(){
		...
		while(i<this.atrributeNames.size()){
			...
			if (isConstant){
				System.out.println("removing attribute "+this.atrributeNames.get(i));
				//System.out.println("size before "+this.atrributeNames.size());
				//copy array without j to new array
				String[][] newPrefixedAttributes=new String[this.prefixedAttributes.length][this.atrributeNames.size()-1];
				...
				//copy array without j to new array
				String[][] newCompleteAttributes=new String[this.completeAttributes.length][this.atrributeNames.size()-1];
	...
	public void removeDurationConstants(){
		...
		while(i<this.durationNames.size()){
			...
			if (count<minimumcount){isConstant=true;}else{isConstant=false;}
			if (isConstant){
				System.out.println("removing duration "+this.durationNames.get(i));
				//System.out.println("size before "+this.atrributeNames.size());
				//copy array without j to new array
				double[][] newPrefixedDurations=new double[this.prefixedDurations.length][this.durationNames.size()-1];
				...
				//copy array without j to new array
				double[][] newCompleteDurations=new double[this.completeDurations.length][this.durationNames.size()-1];
	...
	/**
	 * stores the timestamps needed for remaining and total cycle time, corrects prefixes to be empty
	 * @param pi
	 */
	public void initProcessInstance(ProcessInstance pi){
		...
		this.timeOfEnd=pi.getListOfATEs().get(pi.getListOfATEs().size()-1).getTimestamp().getTime();
		Double totalduration=Math.max(0.0,(double)(this.timeOfEnd-this.timeOfBegin))/(double)localSettings.timeSize;

		this.completeTotalCycleTimes[this.piID]=totalduration.doubleValue();

		this.prefixedTotalCycleTimes[this.prefixID]=totalduration.doubleValue();
		this.prefixedRemainingCycleTimes[this.prefixID]=totalduration.doubleValue();
		this.prefixedTargetElement[this.prefixID]=0;

		//reset the current row becase there is a new proces instance. storeprefix copied the last row into this one.
		this.prefixedDurations[this.prefixID]=new double[this.durationNames.size()];
		this.prefixedAttributes[this.prefixID]=new String[this.atrributeNames.size()];
		this.prefixedOccurrences[this.prefixID]=new int[this.occurrenceNames.size()];
	}
	...
	public void storeOccurrence(String elementname,Long time){
		...
		Double sofar=Math.max(0.0,(double)(time-this.timeOfBegin))/(double)localSettings.timeSize;
		Double remaining=Math.max(0.0,(double)(this.timeOfEnd-time))/(double)localSettings.timeSize;

		this.prefixedRemainingCycleTimes[this.prefixID]=remaining.doubleValue();
	...
	/**
	 * while !newcase copy last row, increment counter. called after the current prefix was written.
	 */
	public void nextPrefix(){
		/* precondition: data is written, meaning no data should be written (in the current loop) after this */
		...
		this.prefixedAttributes[this.prefixID+1]=this.prefixedAttributes[this.prefixID].clone();
		this.prefixedDurations[this.prefixID+1]=this.prefixedDurations[this.prefixID].clone();
		this.prefixedOccurrences[this.prefixID+1]=this.prefixedOccurrences[this.prefixID].clone();
		...
		/* postcondition: the current prefix is copied to the next, indexes were stored, previous row done */
		this.prefixID++;
	...
	public CaseSet(BufferedLogReader log){
		...
		int prefixes=0;HashSet<String> starts=new HashSet<String>();
		HashSet<String> dn=new HashSet<String>();HashSet<String> on=new HashSet<String>();HashSet<String> an=new HashSet<String>();
		...
		this.piids2prefix=new ArrayList<ArrayList<Integer>>(log.numberOfInstances());
