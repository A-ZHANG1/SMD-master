...
public class NonParametricRegression extends Thread{
	...
	/**
	 * start bandwidth selection and test the bandwidth and the average predictor
	 */
	public void run(){
		busy=true;
	...
	/**
	 * This function calculates the bandwidth, all
	 * data is expected to be imported at this point.
	 */
	private void findBandwidth(){	
		Rresult result=null;String set=null;
		...
		result=rconnection.eval("bw<-npregbw(xdat=x"+set+",ydat=y"+set +
				"," +"tol="+localSettings.tol+
				"," +"ftol="+localSettings.tol +
				"," +"ckertype='"+localSettings.getcker()+"'" +
				"," +"okertype='"+localSettings.getoker()+"'" +
				"," +"ukertype='"+localSettings.getuker()+"'" +
		")");			
	...
	/**
	 * this function must be called AFTER bandwidth is set or calculated,
	 * the bandwidth will be tested and results will be stored
	 * @param factor
	 */
	private void testValidationData(boolean factor){
		Rresult result=null;
		rconnection.eval("bw2<-npregbw(bws=bw$bw,bandwidth.compute=FALSE,xdat=xTrain,ydat=yTrain"+
				"," +"ckertype='"+localSettings.getcker()+"'" +
				"," +"okertype='"+localSettings.getoker()+"'" +
				"," +"ukertype='"+localSettings.getuker()+"'" +
		")");	
		//if (localSettings.crossvalidate){
			rconnection.eval("results<-npreg(exdat=xVal,eydat=yVal,txdat=xTrain,tydat=yTrain,bws=bw2)");
...
//		}
		try{
			if (factor){
				rconnection.eval("estimations<-round(as.numeric(results$mean))");
	...
	public double regressOnce(double[] bandwidth){
		...
		rconnection.eval("bw2<-npregbw(bws=bw,bandwidth.compute=FALSE,xdat=xTrain,ydat=yTrain"+
				"," +"ckertype='"+localSettings.getcker()+"'" +
				"," +"okertype='"+localSettings.getoker()+"'" +
				"," +"ukertype='"+localSettings.getuker()+"'" +
		")");
		rconnection.eval("results<-npreg(exdat=xVal,txdat=xTrain,tydat=yTrain,bws=bw2)");
	...
	/**
	 * this function needs all variables to be set, it will calculate the performance
	 * for an average based estimation
	 * @param factor
	 */
	private void testAvgPrediction(boolean factor){
		...
		if (factor){
			...
			rconnection.eval("for(i in 1:length(absAvgEstError)){ absAvgEstError[i]<-abs(absAvgEstError[i])}");
		}else{
			rconnection.eval("avgdur=mean(durations)");
			rconnection.eval("avgEstimations=avgdur-timesofar");
			rconnection.eval("for(i in 1:length(avgEstimations)){ avgEstimations[i]<-max(avgEstimations[i],0)}");
		...
		try{
			...
			if (factor){
				reresult=rconnection.eval("mean(abs(absAvgEstError))/mean(round(as.numeric(yVal)))");
			}else{
				reresult=rconnection.eval("mean(abs(absAvgEstError))/mean(yVal)");
			}
	...
	/**
	 * this is a private helper function that uses a list of strings(names of lists) and a name
	 * to create a dataframe under the name 'framename' with the given variables as tables
	 * @param names
	 * @param framename
	 */
	public void setDataFrames(ArrayList<String> names,String framename){
		String name=null;String command=null;
		if (names.size()>0){
	...
	public void importNewVariable(Double[] values,String name,String postfix,ArrayList<String> place){
	...
	public void importNewVariable(double[] values,String name,String postfix,ArrayList<String> place){
		...
		double variance=1.0;
		try{
			variance=rconnection.eval("var("+name+postfix+")").asDouble();
		}catch (Exception e){System.out.println(e); }
		Boolean removeconstant=false;
		removeconstant=!(variance>0);
		//if all of the bandwidth must be used, ignore constants in the band set. it will not be used
		if (localSettings.useCompleteTrainingForBandwidth&&postfix.equals("_duration_band")){
		...
		if (!removeconstant){
			...
			System.out.println(name+"_duration removed, variance of "+name+postfix+" is : "+ variance);
	...
	public void importNewVariable(double value,String name,String postfix,ArrayList<String> place){
	...
	public void importNewVariable(String[] values,String name,String postfix,ArrayList<String> place){
		rconnection.assign(name+postfix, values);
		rconnection.eval(name+postfix+"<-factor("+name+postfix+")",false);
		if (!(place==null)){
			place.add(name+postfix);
	...
	public void importNewVariable(String value,String name,String postfix,ArrayList<String> place){
		rconnection.assign(name+postfix, value);
		rconnection.eval(name+postfix+"<-factor("+name+postfix+")",false);
		if (!(place==null)){
			place.add(name+postfix);
	...
	public void importNewVariable(Integer[] values,String name,String postfix,ArrayList<String> place){
	...
	public void importNewVariable(int[] values,String name,String postfix,ArrayList<String> place){
		rconnection.assign(name+postfix, values);
		rconnection.eval(name+postfix+"<-ordered("+name+postfix+")",false);
		if (!(place==null)){
			place.add(name+postfix);
	...
	public void importNewVariable(int value,String name,String postfix,ArrayList<String> place){
		rconnection.assign(name+postfix, value);
		rconnection.eval(name+postfix+"<-ordered("+name+postfix+")",false);
		if (!(place==null)){
			place.add(name+postfix);
