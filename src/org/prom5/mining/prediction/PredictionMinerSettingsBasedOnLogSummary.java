...
public class PredictionMinerSettingsBasedOnLogSummary {
	...
	public Boolean bychance;//false implies choose N random cases, true implies choose cases by chance
	public double  chance;
	public Integer target;//0: remaining cyle time, 1:remaining time until X, 2: X?
	...
	public Boolean crossvalidate;//default true, if false no crossvalidation will be performed but pure bandwidth selection
	...
	public PredictionMinerSettingsBasedOnLogSummary(LogSummary summary){
		 ...
		 completeEvents=new HashSet<String>();completeEvents.add("complete");
		 startEvents=new HashSet<String>(); startEvents.add("start");
		 ...
		 useCaseDataAttributes=false; crossvalidate=true;useCompleteTrainingForBandwidth=false;
		 useOccurrences=true;
		 casesize=summary.getNumberOfProcessInstances();
		 /*
		  * cases divided by (the number of cases that would lead to 100 prefixed cases)
		  * this gives us the number K for K-fold cross validation that would have nice performance 
		  */
		 cvsize=Math.min(casesize, 10);
		 bandSize=Math.max(1,((Double)Math.floor(200/(summary.getNumberOfAuditTrailEntries()/casesize))).intValue());
