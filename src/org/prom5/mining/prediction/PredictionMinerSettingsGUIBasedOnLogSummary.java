...
public class PredictionMinerSettingsGUIBasedOnLogSummary extends JPanel {
	...
	public PredictionMinerSettingsGUIBasedOnLogSummary(PredictionMinerSettingsBasedOnLogSummary settings, LogSummary summary){
	...
	public JPanel RconnectionsTab(){
		...
		addRconnectionButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (RconnectGUI()){
					listofRconnections.setListData(localSettings.Rconnections.keySet().toArray());
				}
			};
		});
		...
		listofRconnections.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
	...
	private boolean RconnectGUI(){
		String host=null;Integer port=null;String name=null;
		RConnector rconnector=new RConnector();
		Boolean localchosen=false;Boolean adddefaults=false;
		int again=JOptionPane.YES_OPTION;
		while(!rconnector.succes&&again==JOptionPane.YES_OPTION){
			if (!this.onelocalconnection){
				//String[] options={"local R installation","local or remote R server","add all 4 sviscl0x.win.tue.nl servers"};
				String[] options={"local or remote Rserve-server","add all 4 sviscl0x.win.tue.nl servers"};
...
//			}else if(!localchosen&&!adddefaults){
			if(localchosen){
				...
				String Sport=JOptionPane.showInputDialog(null, "specify port");
				try{
			...
			//}else if(adddefaults){	
				...
				for(Integer i=1;i<4;i++){
					String servername="sviscl0"+i.toString()+".win.tue.nl";
			...
			if (!rconnector.succes){
				again=JOptionPane.showConfirmDialog(null,
						"Connection failed! try again?", "Connection failed! try again?", JOptionPane.YES_NO_OPTION);
			}
		}
		if(rconnector.succes){
	...
	private JPanel settingsGUI(LogSummary summary){
		...
		elementsList.setSelectionInterval(0, summary.getModelElements().length-1);
		...
		conalgs.add("Uniform");	
		kernelContlabel=new JLabel("Kernel function for continuous variables:");
		...
		kernelCatlabel=new JLabel("Kernel function for categorical variables:");
		...
		kernelOrdlabel=new JLabel("Kernel function for ordered discrete variables:");
		...
		useCrossvalidate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				chancebox.setVisible(localSettings.crossvalidate&&localSettings.bychance);
				...
				if (cb.isSelected()){	
					BSpinner.setValue(Math.min((Integer)BSpinner.getValue(),localSettings.casesize-localSettings.casesize/localSettings.cvsize));
					SpinnerModel Bmodel = new SpinnerNumberModel(localSettings.bandSize.intValue(),2,localSettings.casesize-localSettings.casesize/localSettings.cvsize, 1);
					BSpinner.setModel(Bmodel);
				}else{
					SpinnerModel Bmodel = new SpinnerNumberModel(localSettings.bandSize.intValue(),2,localSettings.casesize.intValue(), 1);
					BSpinner.setModel(Bmodel);
				}
			};
		});
		...
		useCompleteBandwidthlabel=new JLabel("use All cases for bandwidth selection");
		useCompleteBandwidthbox.setSelected(localSettings.useCompleteTrainingForBandwidth);
		useCompleteBandwidthbox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				JCheckBox cb = (JCheckBox)e.getSource();
				localSettings.useCompleteTrainingForBandwidth=cb.isSelected();
				BSpinner.setVisible(!localSettings.useCompleteTrainingForBandwidth&&!localSettings.bychance);
				chancebox.setVisible(!localSettings.useCompleteTrainingForBandwidth&&localSettings.bychance);
				useChanceBox.setVisible(!localSettings.useCompleteTrainingForBandwidth);
			};
		});
		...
		useChanceLabel=new JLabel("choose cases by chance(or choose N random cases)?");
		...
		SpinnerModel Kmodel = new SpinnerNumberModel(localSettings.cvsize.intValue(),2,localSettings.casesize.intValue(), 1);
		...
		SpinnerModel Rmodel = new SpinnerNumberModel(localSettings.repeatOpt.intValue(),1,10, 1);
		...
		tolerancelabel=new JLabel("Set the tolerance for the bandwith selection");
