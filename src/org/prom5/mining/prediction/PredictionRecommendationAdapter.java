...
/**
 * @author Ronald Crooy
 * this class is used to predict remaining cycle time for a single case, based on a log with bandwidth variables.
 */
public class PredictionRecommendationAdapter{

	/**
	 * log must contain correct bandwidth variables, partial case and log must match activities.
	 * @param log
	 * @param partialcase
	 * @return
	 */
	public double predictTotalRemainingcycleTime(LogReader log,ProcessInstance partialcase){
		//make default settings for single predictions
		PredictionMinerSettingsBasedOnLogSummary settings=new PredictionMinerSettingsBasedOnLogSummary(log.getLogSummary());
		...
		for (String bandVar:log.getProcess(0).getAttributes().keySet()){
			for (int i=0;i<extlog.durationNames.size();i++){
				if (bandVar.equals(extlog.durationNames.get(i)+"_duration")){
					...
					R.importNewVariable(values, extlog.durationNames.get(i), "_duration", R.trainVarNames);
				}
			}
			for (int i=0;i<extlog.atrributeNames.size();i++){
				if (bandVar.equals(extlog.atrributeNames.get(i)+"_attribute")){
					...
					R.importNewVariable(values, extlog.atrributeNames.get(i), "_attribute", R.trainVarNames);
				}
			}
			for (int i=0;i<extlog.occurrenceNames.size();i++){
				if (bandVar.equals(extlog.occurrenceNames.get(i)+"_occurrence")){
					int[] values=new int[extlog.prefixedOccurrences.length];
					for(int j=0;j<values.length;j++){
						values[j]=extlog.prefixedOccurrences[j][i];
					}
					R.importNewVariable(values, extlog.occurrenceNames.get(i), "_occurrence", R.trainVarNames);
				}
			}
		}
		...
		for(String key:partialcase.getAttributes().keySet()){
			attributes.put(cleanString(key),partialcase.getAttributes().get(key));
		}
		...
		for(AuditTrailEntry ate:partialcase.getListOfATEs()){
			...
			if (settings.completeEvents.contains(ate.getType())){
				...
				if (startednames.contains(elname)){
					...
					double duration=((Long)Math.max(0,ate.getTimestamp().getTime()-started.get(index))).doubleValue();
		...
		for (String bandVar:log.getProcess(0).getAttributes().keySet()){
			...
			for(String duration:durations.keySet()){
				if (bandVar.equals(duration+"_duration")){
					R.importNewVariable(durations.get(duration), duration, "_duration", R.valVarNames);
					wasfound=true;
				}
			}
			for(String attribute:attributes.keySet()){
				if (bandVar.equals(attribute+"_attribute")){
					R.importNewVariable(durations.get(attribute), attribute, "_attribute", R.valVarNames);
					wasfound=true;
				}
			}
			for(String occurrence:occurrences.keySet()){
				if (bandVar.equals(occurrence+"_occurrence")){
					R.importNewVariable(durations.get(occurrence), occurrence, "_occurrence", R.valVarNames);
					wasfound=true;
				}
			}
			if (!wasfound){
