...
public class RConnector  {
	...
	/**
	 * evaluates the R code in the correct R-engine
	 * if waitforresult is set to false, the evalution will not return anyting.
	 */
	...
	/**
	 * evaluates the R code in the correct R-engine
	 * if blocking is set to false, eval will return null while busy
	 * if convert is set to true the resulting Rresult will contain native representation of the contents, otherwise an empty Rresult will be returned.
	 * NOTE that using multiple separate non-blocking evals is very likely a BAD thing!!
	 */
		...
		if (useLocalR){
			...
			if (blocking){
				try{
					...
					//connection error
					System.out.println(this.name+" had problems with: "+rcode+ "; it resulted in :"+e.getMessage());
	...
	public void assign(String name,String[] value){
		if (useLocalR){
			...
			try{
				...
				//what to do now
				System.out.println("assignment error for variable'"+name+"' "+e);
			}
		}
	}
	public void assign(String name, String value){
...
//        }
		...
		try{
			...
			if (Rlocal.eval("require(\"np\", quietly=TRUE)").asBool().isFALSE()){
				JOptionPane.showMessageDialog(null, "the extra package NP needs to be installed, this will be done automatically" +
						", however you must select a mirror to download the packages from");
				Rlocal.eval("if (require(\"np\", quietly=TRUE)){ library(np)}else{install.packages(\"np\")}");
			}
	...
	public Boolean testRconnection(String host,Integer port){
		...
		try{
			...
			if (Rserve.needLogin()){
				username=JOptionPane.showInputDialog(null, "R server needs login, please enter username");
				password=JOptionPane.showInputDialog(null, "Now please enter password");
...
class Rresult extends org.rosuda.JRclient.REXP{
	...
	/**
	 * @author Ronald Crooy
	 * always use constructors for this value
	 * @param value
	 */
