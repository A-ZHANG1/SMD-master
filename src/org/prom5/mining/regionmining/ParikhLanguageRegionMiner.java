package org.prom5.mining.regionmining;
...
import org.prom5.framework.util.ProMLpSolve;
import org.prom5.framework.util.ProMLpSolveException;
...
public class ParikhLanguageRegionMiner extends LogRelationBasedAlgorithm {
    ...
    // used by the analysisplugin version of this plugin. If set, the petri net is extended with places.
...
//			petriNet = null;
        ...
        ProMLpSolve problem = setUpProblem(log, progress, trans,
        ...
        // ****************************************************************************************
        // Place removal phase. Each place violating the base ILP is removed from the net.
        progress.setNote(
                "Renaming and removing existing places violating base ILP");
        renameAndRemovePlaces(net, problem,
                              ui.getOptions().getRemoveExistingPlaces(),
                              progress);
        ...
        if (!ui.getOptions().getRestrictions().equals(
            ...
            // ****************************************************************************************
            // Causal dependencies not expressed by the reduced net are collected and the ILP problem is
            TreeSet<int[]> stack;
            stack = getNotExpressedCausalDependencies(net, relations, progress,
                    ui.getOptions().getRestrictToDisconnectedTrans(),
                    trans);
            // ****************************************************************************************


            // ****************************************************************************************
            // Discovery phase. The ILP is solved many times and places might be added in each iteration.
            findPlaces(progress, relations, events, result, net, trans,
                       colSum_x,
                       colSum_y, problem, stack);
            // ****************************************************************************************
        }
    ...
    private ProMLpSolve setUpProblem(LogReader log, Progress progress,
                                     Transition[] trans,
                                     DoubleMatrix2D causalDependencies,
                                     int[] colSum_x,
                                     int[] colSum_y) {
        ...
        try {
            ...
            problem.putAbortListener(new AbortListener() {
                public boolean abortfunc(LpSolve lpSolve, Object object) throws
                        LpSolveException {
        ...
        for (int i = 0; i < events.size() && !progress.isCanceled(); i++) {
            ...
            try {
                ...
                problem.setBinary(i + events.size() + 1,
                                  ui.getOptions().getIntVars());
        ...
        try {
            ...
            problem.setBinary(problem.getNcolumns() - 1 + 1,
                              ui.getOptions().getIntVars());
        ...
        try {
            ...
            // init + A'.x - A.y >= 0
            TreeSet<int[]>
                    completed = (ui.getOptions().getEmptyAfterCompleteCase() ?
                                 new TreeSet<int[]>(intArrayComp) : null);
            TreeSet<int[]>
                    eqs = getEquations(events, log, progress, chunk, colSum_x,
                                       colSum_y, completed,
                                       ui.getOptions().
            ...
            for (int[] eq : eqs) {
                ...
                if (bound > 0) {
                    for (int i = 0; i < events.size(); i++) {
                        // after firing the last transition, there should not be more than bound tokens
            ...
            if (maxIn < events.size()) {
                ...
                problem.strAddConstraint(s_1 + s_0 + "0", ProMLpSolve.LE,
                                         maxIn);
            }
            ...
            if (total < maxIn + maxOut) {
                Message.add("Adding constraints limiting total edges per place",
                            Message.DEBUG);
                // y.(1) + x.(1) <= total (provided by user)
                problem.strAddConstraint(s_1 + s_1 + "0", ProMLpSolve.LE,
                                         total);
            }
            // x.(1) + y.(1) >= 1
                ...
                for (int i = 0; i < events.size(); i++) {
                    for (int j = 0; j < events.size(); j++) {
                        ...
                        for (int k = 0; k < 2 * events.size() + 1; k++) {
                            s +=
                                    ((k == i || k == j + events.size()) ? "1 " :
                                     "0 ");
                        }
                        // the incoming arrows from i +
                        // the outgoing to j should be <= 1 (i.e. only one of them is present)
            ...
            problem.setAddRowmode(false);
            problem.addTarget( getTargetFunction(colSum_x, colSum_y, 1,problem.getNrows()+1));
    ...
    /*
     ...
     Each iteration adds bounds/constraints, while the presolve might have removed
        ...
        private void preSolve(ProMLpSolve problem) {
            ...
            problem.setPresolve(
                    ProMLpSolve.PRESOLVE_ROWDOMINATE |
                    ProMLpSolve.PRESOLVE_ROWS |
                    ProMLpSolve.PRESOLVE_COLS |
                    ProMLpSolve.PRESOLVE_LINDEP |
                    ProMLpSolve.PRESOLVE_ELIMEQ2 |
                    ProMLpSolve.PRESOLVE_MERGEROWS |
                    ProMLpSolve.PRESOLVE_DUALS |
                    ProMLpSolve.PRESOLVE_PROBEFIX |
                    ProMLpSolve.PRESOLVE_PROBEREDUCE |
                    ProMLpSolve.PRESOLVE_MERGEROWS |
                    ProMLpSolve.PRESOLVE_IMPLIEDSLK |
                    ProMLpSolve.PRESOLVE_REDUCEMIP,
                    problem.getPresolveloops());
            try {
                if (problem.isErrorSolveCode(problem.solve())) {
                    // error found in presolve
                    return;
                }
            } catch (ProMLpSolveException ex) {
                Message.add(ex.getMessage(), Message.ERROR);
    ...
    private void findPlaces(Progress progress, LogRelations relations,
                            LogEvents events,
                            PetriNetResult result, PetriNet net,
                            Transition[] trans, int[] colSum_x,
                            int[] colSum_y,
        ...
        while (!stack.isEmpty() &&
            ...
            if ((t1 != INITIAL)) {
                ...
                int added = 0;
                try {
                    // this is a causal dependency and not a start-event situation
                    ...
                    if (t1 != UNKNOWN) {
                        if (Math.round(problem.getUpbo(t1 + 1)) < 1) {
                            // there is no possible solution, since t1 cannot have any more
                            // outgoing arcs
                    ...
                    if (t2 != UNKNOWN) {
                        if (Math.round(problem.getUpbo(t2 + events.size() + 1)) <
                            1) {
                            // there is no possible solution, since t2 cannot have any more
                            // incoming arcs
                            continue;
                        }
                    }
                ...
                if (ui.getOptions().getSearchInitialMarking() &&
                    ...
                    try {
                        ...
                        // y(t1) = 1  (output to t1)
                        // cannot be more than 1, since there can only be 1 initial token
                        if (Math.round(problem.getUpbo(t1 + events.size() +
                                1)) <
                            1) {
                        ...
                        Place p = solve(events, net, trans, stack,
                                        relations,
                                        bounds, colSum_x,
                                        colSum_y,
                                        problem);
    ...
    private TreeSet<int[]> getNotExpressedCausalDependencies(PetriNet net,
            ...
            CancelationComponent cancel, boolean restrictToDisconnectedTrans,
            Transition[] trans) {
        TreeSet<int[]>
                notDone = new TreeSet<int[]>(new IntArrayCausalComp(
                        relations.
                        getCausalFollowerMatrix()));

        for (int t1 = 0;
                      t1 < relations.getCausalFollowerMatrix().rows() &&
                      !cancel.isCanceled();
                      t1++) {
            if (trans[t1] == null) {
                continue;
            ...
            for (int t2 = 0;
                          t2 < relations.getCausalFollowerMatrix().columns() &&
                          !cancel.isCanceled();
                          t2++) {
                if (trans[t2] == null) {
                    continue;
                ...
                if (ui.getOptions().getUseCausalDependencies() && (
                        !((relations.getCausalFollowerMatrix().get(t1, t2) >
                           0) ||
                          (t1 == t2 &&
                           relations.getOneLengthLoopsInfo().get(t1) > 0)
                        ))) {
                    continue;
                }
                // If the begin transition has output or the end transition
                // has input, then check if this combination should be considered
                // at all.
                int i = t1;
                int j = t2;
                if (restrictToDisconnectedTrans) {
                    ...
                    if ((i + 1) * (j + 1) > 0) {
                        // i*j >0 implies that they are both UNKNOWN (-2 * -2 = 4, or
        ...
        while (it.hasNext() && !cancel.isCanceled()) {
            ...
            for (Transition post : (HashSet<Transition>) p.getSuccessors()) {
                for (Transition pre : (HashSet<Transition>) p.
                        getPredecessors()) {
    ...
    private Place solve(LogEvents events, PetriNet net,
                        Transition[] trans, TreeSet < int[] > stack,
                        LogRelations relations, TreeSet < int[] > bounds,
        ...
        if (!problem.isErrorSolveCode(code)) {
            ...
            if (net.findPlace(id) == null) {
                ...
                if (ui.getOptions().getRestrictions().equals(
                        ParikhLanguageRegionMinerOptions.
                        EXT_FREE_CHOICE) && p.outDegree() > 1) {
                    ...
                    for (Transition t1 : (HashSet<Transition>) p.
                        ...
                        for (Place q : (HashSet<Place>) t1.getPredecessors()) {
                            if (q.outDegree() < p.outDegree()) {
                                ...
                                // to be checked again
                                for (Transition t2 : (HashSet<Transition>)
                                        q.
                                        getPredecessors()) {
                                    // it expresses t2 -> t1
                                    // hence
//									foundSets.remove(new int[] {t2.getNumber(), t1.getNumber()});
                                    if (!ui.getOptions().
                                        getUseCausalDependencies() || (
                                                relations.
                                                getCausalFollowerMatrix().get(
                                            t2.getNumber(),
                                            t1.getNumber()) > 0)) {
                                        // a causal dependency between t2 and t1 was also
                                        // detected in the log
                    ...
                    problem.addTarget(getTargetFunction(colSum_x, colSum_y,
                            1, problem.getNrows()+1));
                    ...
                    if (solution[i] > 0) {
                        for (int j = 0; j < events.size(); j++) {
                            if (solution[j + events.size()] > 0) {
                                // this place connects transition i to transition j
                                if (ui.getOptions().getRestrictions().
                                    equals(
                                            ParikhLanguageRegionMinerOptions.
                                            STATEMACHINE)) {
                                    ...
                                    bounds.add(new int[] {j + events.size() +
                                               1,
                                               0, 0});
                                }
    ...
    public TreeSet<int[]> getEquations(LogEvents events, LogReader log,
                                       Progress progress, int chunk,
                                       int[] colSum_x, int[] colSum_y) {
        return getEquations(events, log, progress, chunk, colSum_x,
                            colSum_y, null, false);
    }
    ...
    public TreeSet<int[]> getEquations(LogEvents events, LogReader log,
                                       Progress progress, int chunk,
                                       int[] colSum_x, int[] colSum_y,
                                       TreeSet < int[] > completeCases,
                                       boolean emptyAfterCaseCompletion) {

        HashMap<LogEvent, Integer>
                map = new HashMap<LogEvent, Integer>(events.size());
        for (int i = 0; i < events.size(); i++) {
        ...
        while (it.hasNext() && (progress == null || !progress.isCanceled())) {
            ...
            if (pi.getAttributes().containsKey(ProcessInstance.ATT_PI_PO) &&
                ...
                Iterator<AuditTrailEntry>
                        ateIt = pi.getAuditTrailEntryList().iterator();
                while (ateIt.hasNext()) {
                    AuditTrailEntry ate = ateIt.next();
                    id2node.put(ate.getAttributes().get(ProcessInstance.
                            ATT_ATE_ID), ate);
                }
                po = true;
            }
            for (int i = 0; i < ateList.size(); i++) {
                ...
                LogEvent thisEvent = new LogEvent(ate.getElement(),
                                                  ate.getType());
                ...
                if (!po) {
                    ...
                    // we know now that all predecessors of this node have been fired,
    ...
    private void fillWithPreLogEvents(AuditTrailEntry ate,
                                      ...
                                      final HashMap<String, AuditTrailEntry>
                                      id2Node) {
        ...
        while (tokenizer.hasMoreTokens()) {
            ...
            LogEvent preEvent = new LogEvent(preAte.getElement(),
                                             preAte.getType());
    ...
    public JPanel getOptionsPanel(LogSummary summary) {
        ui = new ParikhLanguageRegionMinerUI(summary,
                                             super.getOptionsPanel(summary), this);
        return ui;
    }
