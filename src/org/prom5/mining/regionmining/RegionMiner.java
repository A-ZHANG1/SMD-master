...
import org.prom5.converting.RegionsToPetriNet;
...
import org.prom5.framework.models.Region;
import org.prom5.framework.models.RegionList;
...
public class RegionMiner implements MiningPlugin {
	...
	private RegionMinerOptionsPanel options;
	...
	public MiningResult mine(final LogReader log) {
		...
		ModelGraphVertex initialVertex = new ModelGraphVertex("initial", piGraph);
		int id = 0;
		...
		while (logIt.hasNext() && !progress.isCanceled()) {
			...
			if (options.maxSize() != 0) {
				piRegions = piGraph.calculateRegionsMaxSize(eventsCurrentPI, options.maxSize());
			...
			while (it.hasNext()) {
				...
				while (it2.hasNext()) {
					Region r2 = it2.next();
					if (areCompatibleRegions(r1, eventsSoFar, r2, eventsCurrentPI)) { // && !hasEmptyIntersection(r1,r2)) {
		...
		progress.setMaximum(progmax + log.getLogSummary().getLogEvents().size() + regions.size() +
				4);
		...
		while (it.hasNext()) {
			...
			Transition t = net.addAndLinkTransition(new Transition((LogEvent) obj, net));
		...
		if (options.doPostProcessing()) {
			...
			MiningResult result = conv.convert(new ProvidedObject() {
				public Object[] getObjects() {
					return (new Object[] {log, statespace,
							statespace.calculateMinimalRegions(net.getTransitions())});
				}
			});
			Message.add("</RegionMiner>", Message.TEST);
			return result;
	...
	private ModelGraph pi2graph(ProcessInstance pi, ModelGraphVertex initial, HashSet edgeObjects) {
		...
		while (it.hasNext()) {
			...
			ModelGraphVertex newNode = graph.addVertex(new ModelGraphVertex("vertex" + id++,
									   graph));
			LogEvent le = new LogEvent(ate.getElement(), ate.getType());
			graph.addEdge(prevNode,
					newNode).object = le;
			edgeObjects.add(le);
			prevNode = newNode;
		}
	...
	private boolean areCompatibleRegions(Region r1, Collection objects1, Region r2,
			Collection objects2) {
		boolean result = true;
		...
		/*
		 ...
		 while (it.hasNext() && result) {
		  Object o = it.next();
		  result &= r2.getInput().contains(o) || !objects2.contains(o);
		 }
		 it = r1.getOutput().iterator();
		 while (it.hasNext() && result) {
		  Object o = it.next();
		  result &= r2.getOutput().contains(o) || !objects2.contains(o);
		 }
		 it = r2.getInput().iterator();
		 while (it.hasNext() && result) {
		  Object o = it.next();
		  result &= r1.getInput().contains(o) || !objects1.contains(o);
		 }
		 it = r2.getOutput().iterator();
		 while (it.hasNext() && result) {
		  Object o = it.next();
		  result &= r1.getOutput().contains(o) || !objects1.contains(o);
		 }
		 return result;
		 */
	}
	...
	private boolean hasEmptyIntersection(Region r1, Region r2) {
		...
		return!(r1.getInput().isEmpty() && r1.getOutput().isEmpty() && r2.getInput().isEmpty() &&
				r2.getOutput().isEmpty());
	}
