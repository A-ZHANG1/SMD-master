...
import org.prom5.mining.semanticorganizationmining.SemanticOrgMiningResult;
...
public class OrgEntityResourcePanel extends JPanel {
	...
	private JButton changeOrgEntityButton = new JButton("Change OrgEntity Property");
	private JButton changeResourceButton = new JButton("Change Resource Property");
	private JButton addOrgEntityButton = new JButton("Add OrgEntity");       /** adds a new group */
	private JButton removeOrgEntityButton = new JButton("Remove OrgEntity"); /** removes existing group */
	private JButton addResourceButton = new JButton("Add Resource"); /** adds a new resource */
	private JButton removeResourceButton = new JButton("Remove Resource");   /** removes existing resource */

	public OrgEntityResourcePanel(OrgModel orgmod, 	SemanticOrgMiningResult parentpanel) {
	...
	private void registerGuiActionListener() {
		addOrgEntityButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// Input Org Entity ID
				String id = JOptionPane.showInputDialog(null, "input Org Entity ID?");

				// check wheather ID is already in Org Entities
				if (id != null && !orgModel.hasOrgEntity(id)) {
					// Input Org Entity Name
					String name = JOptionPane.showInputDialog(null, "input Org Entity Name?");
					// Input Org Entity Type
					String type = (String) JOptionPane.showInputDialog(
								  null, "Which type?", "Type", JOptionPane.QUESTION_MESSAGE,
				...
				} else JOptionPane.showMessageDialog(null, "ID is duplicated or has null value");
			}
		});
		...
		changeOrgEntityButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				// input Org Entity ID
				String id = JOptionPane.showInputDialog(null, "input Org Entity ID?", tempEntity.getID());

				if (id != null) {
					// check wheather ID is already in Org Entities
					// if not, change both ID and key of HashMap for Org Entities
					if (orgModel.changeOrgEntityID(tempEntity.getID(), id)) {
						// change Org Entity Name
						String name = JOptionPane.showInputDialog(null, "input Org Entity Name?",
									  tempEntity.getName());
						if (name != null) tempEntity.setName(name);

						// change Org Entity Type
						String type = (String) JOptionPane.showInputDialog(null,
									  "choose Org Entity Type?", "Org Entity Type",
									  JOptionPane.QUESTION_MESSAGE, null, // Use default icon
									  OrgEntity.ORGENTITYTYPE_ARRAYLIST.toArray(),
									  tempEntity.getEntityType());
						if (type != null) tempEntity.setEntityType(type);
		...
		addResourceButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// Input Org Entity ID
				String id = JOptionPane.showInputDialog(null, "input Resource ID?");

				// check wheather ID is already in Org Entities
				if (id != null && !orgModel.hasResource(id)) {
					// Input Org Entity Name
					String name = JOptionPane.showInputDialog(null, "input Resource Name?");
					// Input Org Entity Type
					if (name != null) {
		...
		changeResourceButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				// input Org Entity ID
				String id = JOptionPane.showInputDialog(null, "input Resource ID?", tempResource.getID());
				...
				if (id != null) {
					...
					if (orgModel.changeResourceID(tempResource.getID(), id)) {
						...
						if (name != null) tempResource.setName(name);
						// Redraw tablePanel
						redrawTable();
					} else JOptionPane.showMessageDialog(null, "ID is duplicated or has null value");
				}
