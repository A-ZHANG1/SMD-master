...
public class OrgEntityResourceTablePanel extends JPanel implements TableModelListener {
	...
	public OrgEntityResourceTablePanel(OrgModel orgmod, SemanticOrgMiningResult parentpanel) {
		...
		orgEntityTable.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 1) {
					...
					OrgEntity orgEntity = ((OrgEntityTable) orgEntityTable.getModel()).getOrgEntity(row);
					((ResourceTable) resourceTable.getModel()).updateTable(orgEntity);
				}
			}
		});
	...
	{
		if (orgEntityTable.getSelectedRow() < 0) return null;
		else return ((OrgEntityTable) orgEntityTable.getModel()).getOrgEntity(orgEntityTable.getSelectedRow());
	}

	public Resource getSeletedResource()
	{
		if (resourceTable.getSelectedRow() < 0) return null;
		else return ((ResourceTable) resourceTable.getModel()).getResource(resourceTable.getSelectedRow());
	}
...
//		private static final long serialVersionUID = -6165029231436957878L;
		...
		public Object getValueAt(int row, int column) {
			...
			if (column == 0)return (Boolean) choose.get(row);
			else return (String) resources.get(row).getID();
		}
