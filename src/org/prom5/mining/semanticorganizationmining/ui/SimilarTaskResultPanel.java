...
//	private JButton generateButton = new JButton("Generate Org Model ");
	...
	public SimilarTaskResultPanel(OrgModel orgmodel, SocialNetworkMatrix snmatrix, SemanticOrgMiningResult parent) {
	    ...
	    orgModel.reallocateOrgModel(snMatrix.listGroupOriginator(Double.valueOf(getThresholdFromSlider())),orgOriginalModel);
	...
	protected void updateThresholdLabel() {
			...
			if(thresholdSlider.getValueIsAdjusting() == false) {
				generateGroup();
				redrawGraphPanel(snMatrix.getNodeNames(), getSNMatrix().applyThresholdValueToTempMatrix(Double.valueOf(getThresholdFromSlider())));
			}
	}
	...
	private JPanel getGraphPanel(String[] users, DoubleMatrix2D matrix) {
		...
		try {
			...
			bw.write(
					"edge [arrowsize=\"0.5\",decorate=true,fontname=\"Arial\",fontsize=\"8\"];\n");
			...
			Message.add("Error while performing graph layout: " + ex.getMessage(),
					Message.ERROR);
		   return null;
		}
	}
