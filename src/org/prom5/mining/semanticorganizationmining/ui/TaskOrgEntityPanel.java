...
public class TaskOrgEntityPanel extends JSplitPane {
	...
	private JButton addTaskButton = new JButton("Add Task");       /** adds a new group */
	private JButton removeTaskButton = new JButton("Remove Task"); /** removes existing group */

	private JButton changeOrgEntityButton = new JButton("Change OrgEntity Property");
	private JButton addOrgEntityButton = new JButton("Add OrgEntity"); /** adds a new resource */
	private JButton removeOrgEntityButton = new JButton("Remove OrgEntity");   /** removes existing resource */

	public TaskOrgEntityPanel(OrgModel orgmod, 	SemanticOrgMiningResult parentpanel) {
		...
		addTaskButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (id != null && !orgModel.hasTask(id)) {
					// Input Task Name
					String name = JOptionPane.showInputDialog(null, "input Task Name?");
					// Input Task Type
					String type = JOptionPane.showInputDialog(null, "input Type?");
...
*/
		...
		changeTaskButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (tempTask == null)return;
				// input Task ID
				String id = JOptionPane.showInputDialog(null, "input Task ID?", tempTask.getID());
				...
				if (id != null) {
					...
					if (orgModel.changeTaskID(tempTask.getID(), id)) {
						...
						String name = JOptionPane.showInputDialog(null, "input Task Name?",
									  tempTask.getName());
						if (name != null) tempTask.setName(name);
						// change Org Entity Type
						String type = JOptionPane.showInputDialog(null, "input Task Name?",
									  tempTask.getEventType());
						if (type != null) tempTask.setEventType(type);
						// Redraw tablePanel
						redrawTable();
					} else JOptionPane.showMessageDialog(null, "ID is duplicated or has null value");
				}
