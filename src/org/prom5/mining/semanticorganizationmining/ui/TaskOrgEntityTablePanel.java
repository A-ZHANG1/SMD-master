 ...
 * @version 1.0
 */
public class TaskOrgEntityTablePanel extends JSplitPane implements TableModelListener {
	...
	public TaskOrgEntityTablePanel(OrgModel orgmod, SemanticOrgMiningResult parentpanel) {
				...
				public void mouseClicked(MouseEvent e) {
					if (e.getClickCount() == 1) {
						...
						int row = taskTable.rowAtPoint(p);
							((OrgEntitySelectTable) orgEntitySelectTable.getModel()).updateTable(((TaskTable) taskTable.getModel()).getTask(row));
					}
				}
			});
	...
	public void tableChanged(TableModelEvent e) {
			if (e.getSource() instanceof OrgEntitySelectTable) {
				if (e.getColumn() == 0) {
					...
					if (grp_row >= 0) {
						Task t = ((TaskTable) taskTable.getModel()).getTask(grp_row);
						OrgEntity orgEntity = ((OrgEntitySelectTable)orgEntitySelectTable.getModel()).getOrgEntity(src_row);
						...
						((OrgEntitySelectTable) orgEntitySelectTable.getModel()).updateTable();
				}
			}
		...
		{
			if (orgEntitySelectTable.getSelectedRow() < 0) return null;
			else return ((OrgEntitySelectTable) orgEntitySelectTable.getModel()).getOrgEntity(orgEntitySelectTable.getSelectedRow());
		}

		public Task getSeletedTask()
		{
			if (taskTable.getSelectedRow() < 0) return null;
			else return ((TaskTable) taskTable.getModel()).getTask(taskTable.getSelectedRow());
		}
		...
		private class TaskTable extends AbstractTableModel {
			...
			public Object getValueAt(int row, int column) {
				...
				if(column==0) return ((Task) tasks.get(row)).getName();
				else return ((Task) tasks.get(row)).getEventType();
			}
		...
		private class OrgEntitySelectTable extends AbstractTableModel {
			...
			public Object getValueAt(int row, int column) {
				...
				switch (column){
					...
					case 1: return orgEntities.get(row).getID();
					case 2: return orgEntities.get(row).getEntityType();
				}
				return null;
			}
			...
			public void updateTable(Task task) {
				if (orgEntities != null) {
					...
					for (int i = 0; i < numRes; i++) {
							choose.set(i, Boolean.valueOf(task.hasOrgEntity(orgEntities.get(i))));
					}
