...
public class SocialNetworkResults extends JPanel implements MiningResult, Provider {
	...
	public SocialNetworkResults(LogReader log, SocialNetworkMatrix snMatrix, OTMatrix2DTableModel otMatrix)
	{
		   ...
		   menuPane.add(thresholdLabel, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0
				   , GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		   menuPane.add(thresholdValueLabel, new GridBagConstraints(1, 0, 1, 1, 0.0, 0.0
				   , GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		   menuPane.add(thresholdSlider, new GridBagConstraints(0, 2, 3, 1, 0.0, 0.0
				   , GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		   menuPane.add(removeButton, new GridBagConstraints(0, 3, 3, 1, 0.0, 0.0
			...
			splitPaneTop = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, menuPane, jscrollTable);
			splitPaneTop.setDividerLocation(0.3);
			splitPaneTop.setResizeWeight(0.3);

			// draw graph panel
			drawGraphPanel();
			splitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT, splitPaneTop, jscroll);
		...
		public void initOriginalMatrix(){
			...
			originalMatrix = null;
			originalMatrix = DoubleFactory2D.sparse.make(snMatrix.getNodeNames().length,snMatrix.getNodeNames().length,0);
		...
		protected void updateThresholdLabel() {
			thresholdValueLabel.setText(Double.valueOf(getThresholdFromSlider()).toString());
			if (thresholdSlider.getValueIsAdjusting() == false) {
				DoubleMatrix2D tempMatrix = DoubleFactory2D.sparse.make(snMatrix.getNodeNames().length,snMatrix.getNodeNames().length,0);
				tempMatrix.assign(originalMatrix.copy());
				snMatrix.setMatrix(tempMatrix);
				getSNMatrix().applyThresholdValue(Double.valueOf(getThresholdFromSlider()));
				redrawGraphPanel();
			}
		}
		...
		private JPanel getGraphPanel(String[] users, DoubleMatrix2D matrix) {
			...
			try {
				...
				bw.write(
						"digraph G {ranksep=\".3\"; fontsize=\"8\"; remincross=true; margin=\"0.0,0.0\"; ");
	...
	//                      create the visual component and return it
		...
		public void drawMatrixPanel() {
			...
			dataMatrix = new ResultTableModel(snMatrix.getNodeNames(), snMatrix.getMatrix());
		...
		public void drawGraphPanel() {
			jscroll = null;
			graphPanel = null;
			graphPanel = getGraphPanel(snMatrix.getNodeNames(), snMatrix.getMatrix());
		...
		public ProvidedObject[] getProvidedObjects() {
			return (new ProvidedObject[] {
					new ProvidedObject("SNA", new Object[] {
	    ...
	    public void writeToTestLog() {
			...
			Message.add("<SNASummary numberOfOriginators=\"" + snMatrix.getNodeNames().length + "\">" , Message.TEST);
			Message.add("<SNASummary minimumValue=\"" + snMatrix.getMinValue() + "\" maximumValue=\""
					+ snMatrix.getMaxValue() + "\">" , Message.TEST);
			Message.add("<SNASummary minimumValue=\"" + snMatrix.getMinValue() + "\" maximumValue=\""
					+ snMatrix.getMaxValue() + "\">" , Message.TEST);
			Message.add("<SNASummary minimumFlowValue=\"" + snMatrix.getMinFlowValue() + "\" maximumFlowValue=\""
...
class ColorRenderer extends DefaultTableCellRenderer {
	...
	public Component getTableCellRendererComponent
		...
		if (column == 0) cell.setBackground(new Color(240,240,239));
