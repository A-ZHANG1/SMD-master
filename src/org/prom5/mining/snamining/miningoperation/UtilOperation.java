...
public class UtilOperation {
	...
	public static boolean isInCase(AuditTrailEntryList atelists, String user1, String user2) {
			...
			while (ates.hasNext()) {
				...
				if (ate.getOriginator()!=null&&ate.getOriginator().equals(user1)) bResult1 = true;
				if (ate.getOriginator()!=null&&ate.getOriginator().equals(user2)) bResult2 = true;
				if (bResult1 & bResult2)break;
			}
	...
	public static boolean isInCase(AuditTrailEntryList atelists, String user1) {
		...
		while (ates.hasNext()) {
			...
			if (ate.getOriginator()!=null&&ate.getOriginator().equals(user1)) {
				bResult1 = true;
				break;
			}
		}
		...
		for (int i = 0; i < row - 1; i++) {
			for (int j = i + 1; j < row; j++) {
				...
				for (int k = 0; k < column; k++) {
					temp += (double) ((m.get(i, k) - m.get(j, k)) * (m.get(i, k) - m.get(j, k)));
				}
