package org.prom5.mining.snamining.miningoperation.handover;
...
public class Handover_CCIDCM extends BasicOperation {
	...
	// consider casuality, ignore direct succession, consider multiple appearance
	...
	public DoubleMatrix2D calculation(double beta, int depth) {
		...
		relations = (new MinValueLogRelationBuilder(logAbstraction, 0, log.getLogSummary().getLogEvents())).getLogRelations();
	    ...
	    while (it.hasNext()) {
			...
			if (ates.hasNext()) {
				...
				while (ates.hasNext()) {
					...
					row_from = modelElements.findLogEventNumber(ate.getElement(), ate.getType());
					column_to = modelElements.findLogEventNumber(ate2.getElement(), ate2.getType());

					if (parallelMatrix.get(row_from, column_to) == 0) {
						normal++;
						if (ate.getOriginator() != null && ate2.getOriginator() != null) {
					...
					if(ate2.getOriginator()!=null) {
						...
						for (int i = 0; i < depth - 1 && i < flag - 1; i++) {
							...
							ateTemp = (AuditTrailEntry) ates_list_by_entries.get(
									  ates_list_by_entries.size() - i - 2);
							row_from = modelElements.findLogEventNumber(ateTemp.getElement(),
									   ateTemp.getType());

							if (parallelMatrix.get(row_from, column_to) == 0) {
								if (users_list_by_entries.get(users_list_by_entries.size() - i - 2)!= null){
									row = Arrays.binarySearch(users, users_list_by_entries.get(users_list_by_entries.size() - i -  2));
