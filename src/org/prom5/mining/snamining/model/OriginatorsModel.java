...
public class OriginatorsModel {
	...
	public OriginatorsModel(LogSummary summary)
	{
		...
		else
		{
			for(int i=0; i<sOriginators.length;i++) {
				Map<LogEvent,Integer> eventOccurrenceMap = summary.getEventsForOriginator(sOriginators[i]);
				int counter = 0;
	...
	public int getMinFrequency() {
		...
		for (int i = 0; i < sOriginators.length; i++){
			temp = Integer.parseInt(mapOriginators.get(sOriginators[i]).toString());
	...
	public void filterOriginator(int min, int max)
	{
		...
		for (int i = 0; i < sOriginators.length; i++)
		{
			freq = Integer.parseInt(mapOriginators.get(sOriginators[i]).toString());
	...
	public DoubleMatrix2D filterResultMatrix() {
		DoubleMatrix2D m = DoubleFactory2D.sparse.make(getNumberOfSelectedOriginators(), getNumberOfSelectedOriginators(), 0);

		int k=0;
		for(int i=0; i < sOriginators.length; i++){
			if(!boolSelected[i]) continue;
			int l=0;
			for(int j=0; j < sOriginators.length; j++){
				if(!boolSelected[j]) continue;
	...
	public DoubleMatrix2D getRoleAssignmentMatrix() {
		...
		if (orgModel!=null){
			ArrayList<String> roleList = orgModel.getOrgEntityStringList(OrgEntity.ORGENTITYTYPE_ROLE);
//			ArrayList<String> roleList = orgModel.getRoleStringList();
			roleMatrix = DoubleFactory2D.sparse.make(getNumberofOriginators(), roleList.size(), 0);
			String[] sOriginators = getSelectedOriginators();

			for (int i = 0; i < sOriginators.length; i++) {
				Resource res = orgModel.getResource(sOriginators[i]);
				if (res != null) {
					List<String> roleListforOriginator = orgModel.getOrgEntityList(res,OrgEntity.ORGENTITYTYPE_ROLE);
	...
	public DoubleMatrix2D getOrgUnitAssignmentMatrix() {
			...
			if (orgModel!=null){
				ArrayList<String> orgUnitList = orgModel.getOrgEntityStringList(OrgEntity.ORGENTITYTYPE_ORGUNIT);
				orgUnitMatrix = DoubleFactory2D.sparse.make(getNumberofOriginators(), orgUnitList.size(), 0);
				String[] sOriginators = getSelectedOriginators();

				for (int i = 0; i < sOriginators.length; i++) {
					Resource res = orgModel.getResource(sOriginators[i]);
					if (res != null) {
						List<String> orgUnitListforOriginator = orgModel.getOrgEntityList(res,OrgEntity.ORGENTITYTYPE_ORGUNIT);
						Iterator it = orgUnitListforOriginator.iterator();
						while (it.hasNext()) {
							String orgUnitName = (String) it.next();
							orgUnitMatrix.set(i, orgUnitList.indexOf(orgUnitName), 1.0);
						}
