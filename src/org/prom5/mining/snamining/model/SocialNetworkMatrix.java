package org.prom5.mining.snamining.model;
...
public class SocialNetworkMatrix implements Cloneable {
	...
	public DoubleMatrix2D applyThresholdValue(double threshold) {
			for(int i=0; i < matrix.rows(); i++)
				for(int j=0; j < matrix.columns(); j++)
					if(matrix.get(i,j)<threshold) matrix.set(i,j, 0.0);
			return matrix;
	}

	public DoubleMatrix2D applyThresholdValueToTempMatrix(double threshold) {
		DoubleMatrix2D tempMatrix = DoubleFactory2D.sparse.make(matrix.rows(),matrix.columns());
		for(int i=0; i < tempMatrix.rows(); i++)
			for(int j=0; j < tempMatrix.columns(); j++)
				if(matrix.get(i,j)<threshold) tempMatrix.set(i,j, 0.0);
				else tempMatrix.set(i,j, matrix.get(i,j));
		return tempMatrix;
	}
	...
	{ 	ArrayList<String> removedNodeNames = new ArrayList<String>();
		...
		for(int i=0; i < nodeNames.length; i++){
			...
			if(!flag) removedNodeNames.add(nodeNames[i]);
		}

		newNodeNames = new String[nodeNames.length-removedNodeNames.size()];
		newMatrix = DoubleFactory2D.sparse.make(newNodeNames.length, newNodeNames.length, 0);

		int k=-1;
		for(int i=0; i < nodeNames.length; i++){
			if(removedNodeNames.contains(nodeNames[i])) continue;
			k++;
			newNodeNames[k]=nodeNames[i];
			int l=-1;
			for(int j=0; j < nodeNames.length; j++){
				if(removedNodeNames.contains(nodeNames[j])) continue;
				l++;
		...
		for(int i=0; i < nodeNames.length; i++){
			...
			for (int j = i+1; j < nodeNames.length; j++) {
				if ((tempMatrix.get(i, j) != 0) || (tempMatrix.get(j, i) != 0)) {
					...
					if(!flag) group = cal2(j,group,tempMatrix);
				}
			}
			listGroup.add(group);
		}
	...
	private ArrayList<String> cal2(int i, ArrayList<String> group, DoubleMatrix2D tempMatrix){
			group.add(nodeNames[i]);
	...
	{
		ArrayList<String> newNodeNames = new ArrayList(Arrays.asList(groupNames));
		groupOriginators(newNodeNames,groupAssignments);
	}

	public SocialNetworkMatrix groupOriginators(ArrayList<String> newNodeNames, String[] groupAssignments)
	{
		DoubleMatrix2D newMatrix = DoubleFactory2D.sparse.make(newNodeNames.size(),newNodeNames.size(), 0);;

		for(int i=0; i < nodeNames.length; i++)
			for (int j = 0; j < nodeNames.length; j++) {
				int iIndex = newNodeNames.indexOf(groupAssignments[i]);
				int jIndex = newNodeNames.indexOf(groupAssignments[j]);
				newMatrix.set(iIndex, jIndex, newMatrix.get(iIndex, jIndex) + matrix.get(i, j));
			}
	...
	{
		if(roleMatrix==null) return false;
		else return true;
	}

	public boolean hasOrgUnitModel()
	{
		if(orgUnitMatrix==null) return false;
		else return true;
	}
	...
	public double getMaxValue()
	{
		...
		for(int i=0; i < matrix.rows(); i++)
			for(int j=0; j < matrix.columns(); j++)
				if(max<matrix.get(i,j)) max = matrix.get(i,j);
		return max;
	}
	...
	public double getMinValue()
	{
		...
		for(int i=0; i < matrix.rows(); i++)
			for(int j=0; j < matrix.columns(); j++)
				if(min>matrix.get(i,j)) min = matrix.get(i,j);
		return min;
	}
	...
	public double getMaxFlowValue() {
		...
		for (int i = 0; i < matrix.rows(); i++)
			for (int j = 0; j < matrix.columns(); j++)
				if (i!=j && max < matrix.get(i, j)) max = matrix.get(i, j);
		return max;
	}
	...
	public double getMinFlowValue() {
		...
		for (int i = 0; i < matrix.rows(); i++)
			for (int j = 0; j < matrix.columns(); j++)
				if (i!=j && min > matrix.get(i, j)) min = matrix.get(i, j);
		return min;
	}
	...
	public Object clone()
	{
		...
		if(matrix!=null){
			DoubleMatrix2D tempSNMatrix = DoubleFactory2D.sparse.make(matrix.rows(), matrix.columns());
		...
		if(orgUnitMatrix!=null){
			DoubleMatrix2D temporgUnitMatrix = DoubleFactory2D.sparse.make(orgUnitMatrix.rows(),
											   orgUnitMatrix.columns());
			temporgUnitMatrix.assign(orgUnitMatrix);
			o.setOrgUnitMatrix(temporgUnitMatrix);
		}

		if(roleMatrix!=null){
			DoubleMatrix2D temproleMatrix = DoubleFactory2D.sparse.make(roleMatrix.rows(),
											roleMatrix.columns());
		...
		if(otMatrix!=null){
			DoubleMatrix2D tempotMatrix = DoubleFactory2D.sparse.make(otMatrix.rows(), otMatrix.columns());
