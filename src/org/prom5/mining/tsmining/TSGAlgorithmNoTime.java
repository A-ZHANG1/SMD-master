...
public class TSGAlgorithmNoTime extends TSGAlgorithm
{
	...
	public TSGAlgorithmNoTime(LogReader log, int typeOfTS, int genFlags, int visFlags)
	{
		super(log, typeOfTS, genFlags, visFlags);
	}
	@Override
	/*
	 * The log contains no timestamps, so it's ordered
	 * Only one activity occurs at one point of time (no transactions)
	 */
	public TransitionSystem generateTS()
	{
		...
		while( logIt.hasNext() )
		{
			...
			{
				TransitionSystemEdge finalEdge = new TransitionSystemEdge(finalDocSet, lastVertex, finalVertex);
