...
public class TSGAlgorithmTime extends TSGAlgorithm
{
	...
	public TSGAlgorithmTime(LogReader log, int typeOfTS, int genFlags, int visFlags)
	{
	 ...
	 * Effective algorithm: adding states and transitions running through the log
	 * Here, one commit can contain several documents!
	 */
	public TransitionSystem generateTS()
	{
		TransitionSystem ts = new TransitionSystem("Labeled Transition System", genFlags, visFlags);
		Iterator logIt = log.instanceIterator();

		Collection<String> finalDocSet = null;
		TransitionSystemVertex finalVertex = null;
		if( (genFlags & TSConstants.EXPLICIT_END) == TSConstants.EXPLICIT_END) //create one final vertex for all cases
		{
		...
		// main loop iterating through all the process instances and audit trails in them
		while( logIt.hasNext() )
		{
			...
			TransitionSystemVertex zeroVertex = factory.createVertex(setsOrBags, docs, exLogName, ts);
			...
			if ((genFlags & TSConstants.EVENT_TYPES) == TSConstants.EVENT_TYPES)
				doc = doc.concat(PetrifyConstants.EVENTTYPESEPARATOR).concat(entry.getType());
			docs.add(doc);
			commit.add(doc);
			...
			while( entryIt.hasNext() )
			{
				...
				{
					TransitionSystemVertex newVertex = factory.createVertex(setsOrBags, docs, exLogName, ts);
					...
					{
						TransitionSystemEdge edge = new TransitionSystemEdge(commit, lastVertex, createdVertex);
						ts.addEdge(edge);
					}
			...
			if( (genFlags & TSConstants.EXPLICIT_END) == TSConstants.EXPLICIT_END) // add the edge to the final Vertex
			{
				TransitionSystemEdge finalEdge = new TransitionSystemEdge(finalDocSet, createdVertex, finalVertex);
