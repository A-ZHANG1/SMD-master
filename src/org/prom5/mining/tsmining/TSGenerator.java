...
public class TSGenerator implements MiningPlugin
{
	...
	{
		int genFlags = TSConstants.MODEL_ELEMENTS; // options for Transition System Generation
		int visFlags = TransitionSystem.IDENTIFIER; // option for Transition System VIsualization
		int typeOfTS = 0;
		int typeOfStrat = 0;
		...
		if ( ui.haveTimestamps() )
			...
			algorithm = new TSGAlgorithmNoTime(log, typeOfTS, genFlags, visFlags);
		...
		{
			if( ui.haveTimestamps() )
				modificationStrategy = new TSStrategyExtendTime(log, ts, genFlags, typeOfTS);
			else
				modificationStrategy = new TSStrategyExtendNoTime(log, ts, genFlags, typeOfTS);
	...
	{
		Message.add("<TransitionSystemGenerator file=\"" + log.getFile().getShortName() + "\">", Message.TEST);
		Message.add("<NumberOfStates = \"" + ts.getVerticeList().size() + "\"/>", Message.TEST);
		Message.add("<NumberOfTransitions = \"" + ts.getEdges().size() + "\"/>", Message.TEST);
