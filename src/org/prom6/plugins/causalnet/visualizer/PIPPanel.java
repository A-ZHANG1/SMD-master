...
public class PIPPanel extends JPanel implements MouseListener, MouseMotionListener {
	...
	private final AnnotatedScalableView view;


	public PIPPanel(AnnotatedScalableView view) {
	...
	public synchronized void initializeImage() {
		...
		if (size.getWidth() > 0 && size.getHeight() > 0) {
			...
			double oldScale = view.getScalable().getScale();
			view.getScalable().setScale(r * oldScale);
			view.getComponent().paint(g2ds);
			view.getScalable().setScale(oldScale);
			g2ds.dispose();
