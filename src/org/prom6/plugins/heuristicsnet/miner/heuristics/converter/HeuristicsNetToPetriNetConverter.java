package org.prom6.plugins.heuristicsnet.miner.heuristics.converter;
...
import org.prom6.plugins.heuristicsnet.miner.heuristics.miner.operators.Stats;
...
public class HeuristicsNetToPetriNetConverter {
	...
	@UITopiaVariant(affiliation = UITopiaVariant.EHV, author = "J.T.S. Ribeiro", email = "j.t.s.ribeiro@tue.nl", website = "http://is.tm.tue.nl/staff/jribeiro")
	...
	public static Object[] converter(PluginContext context, HeuristicsNet hn) {
		...
		if (hn instanceof AnnotatedHeuristicsNet) {
			...
			for(java.util.Map.Entry<String, String> entry : keys.entrySet()){
				...
				inputPlaces[index] = net.addPlace("");
				outputPlaces[index] = net.addPlace("");
...
//			context.addConnection(new InitialMarkingConnection(net, m));
			...
			for (int activityIndex = 0; activityIndex < activities.length; activityIndex++) {
				...
				inputPlaces[activityIndex] = net.addPlace("");
				outputPlaces[activityIndex] = net.addPlace("");
